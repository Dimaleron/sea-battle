...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15666=0;yPoint_15666<this.gameFieldBorderY.length; yPoint_15666++){
                for(var xPoint_15666=0;xPoint_15666<this.gameFieldBorderX.length; xPoint_15666++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15666, xPoint_15666);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15666][xPoint_15666] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15666, xPoint_15666, 'user');
                    if(this._userShipsMap[yPoint_15666][xPoint_15666] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15666, xPoint_15666, type){
            var id = this.getPointBlockIdByCoords(yPoint_15666, xPoint_15666, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15666);
                block.setAttribute('data-y', yPoint_15666);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15666
         * @param {type} xPoint_15666
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15666, xPoint_15666, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15666 + '_y' + yPoint_15666;
            }
            return 'pc_x' + xPoint_15666 + '_y' + yPoint_15666;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15666=0;yPoint_15666<this.gameFieldBorderY.length; yPoint_15666++){
                for(var xPoint_15666=0;xPoint_15666<this.gameFieldBorderX.length; xPoint_15666++){
                    map.push({y: yPoint_15666, x: xPoint_15666});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15666=-1;yPoint_15666<(this.gameFieldBorderY.length+1); yPoint_15666++){
                for(var xPoint_15666=-1;xPoint_15666<(this.gameFieldBorderX.length+1); xPoint_15666++){
                    if(!map[yPoint_15666]){
                        map[yPoint_15666] = [];
                    }
                    map[yPoint_15666][xPoint_15666] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15666 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15666 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15666, yPoint_15666) === true){
                    if(this.canPutHorizontal(map, xPoint_15666, yPoint_15666, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15666][xPoint_15666 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15666, yPoint_15666, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15666 + i][xPoint_15666] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15666, yPoint_15666){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15666][xPoint_15666] === this.CELL_EMPTY
                && map[yPoint_15666-1][xPoint_15666] === this.CELL_EMPTY
                && map[yPoint_15666-1][xPoint_15666+1] === this.CELL_EMPTY
                && map[yPoint_15666][xPoint_15666+1] === this.CELL_EMPTY
                && map[yPoint_15666+1][xPoint_15666+1] === this.CELL_EMPTY
                && map[yPoint_15666+1][xPoint_15666] === this.CELL_EMPTY
                && map[yPoint_15666+1][xPoint_15666-1] === this.CELL_EMPTY
                && map[yPoint_15666][xPoint_15666-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15666, yPoint_15666, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15666;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15666][x] === this.CELL_EMPTY
                    && map[yPoint_15666-1][x] === this.CELL_EMPTY
                    && map[yPoint_15666-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15666][x+1] === this.CELL_EMPTY
                    && map[yPoint_15666+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15666+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15666, yPoint_15666, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15666;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15666] === this.CELL_EMPTY
                    && map[y+1][xPoint_15666] === this.CELL_EMPTY
                    && map[y+1][xPoint_15666+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15666] === this.CELL_EMPTY
                    && map[y][xPoint_15666-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15666-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
