...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4567=0;yPoint_4567<this.gameFieldBorderY.length; yPoint_4567++){
                for(var xPoint_4567=0;xPoint_4567<this.gameFieldBorderX.length; xPoint_4567++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4567, xPoint_4567);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4567][xPoint_4567] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4567, xPoint_4567, 'user');
                    if(this._userShipsMap[yPoint_4567][xPoint_4567] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4567, xPoint_4567, type){
            var id = this.getPointBlockIdByCoords(yPoint_4567, xPoint_4567, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4567);
                block.setAttribute('data-y', yPoint_4567);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4567
         * @param {type} xPoint_4567
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4567, xPoint_4567, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4567 + '_y' + yPoint_4567;
            }
            return 'pc_x' + xPoint_4567 + '_y' + yPoint_4567;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4567=0;yPoint_4567<this.gameFieldBorderY.length; yPoint_4567++){
                for(var xPoint_4567=0;xPoint_4567<this.gameFieldBorderX.length; xPoint_4567++){
                    map.push({y: yPoint_4567, x: xPoint_4567});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4567=-1;yPoint_4567<(this.gameFieldBorderY.length+1); yPoint_4567++){
                for(var xPoint_4567=-1;xPoint_4567<(this.gameFieldBorderX.length+1); xPoint_4567++){
                    if(!map[yPoint_4567]){
                        map[yPoint_4567] = [];
                    }
                    map[yPoint_4567][xPoint_4567] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4567 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4567 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4567, yPoint_4567) === true){
                    if(this.canPutHorizontal(map, xPoint_4567, yPoint_4567, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4567][xPoint_4567 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4567, yPoint_4567, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4567 + i][xPoint_4567] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4567, yPoint_4567){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4567][xPoint_4567] === this.CELL_EMPTY
                && map[yPoint_4567-1][xPoint_4567] === this.CELL_EMPTY
                && map[yPoint_4567-1][xPoint_4567+1] === this.CELL_EMPTY
                && map[yPoint_4567][xPoint_4567+1] === this.CELL_EMPTY
                && map[yPoint_4567+1][xPoint_4567+1] === this.CELL_EMPTY
                && map[yPoint_4567+1][xPoint_4567] === this.CELL_EMPTY
                && map[yPoint_4567+1][xPoint_4567-1] === this.CELL_EMPTY
                && map[yPoint_4567][xPoint_4567-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4567, yPoint_4567, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4567;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4567][x] === this.CELL_EMPTY
                    && map[yPoint_4567-1][x] === this.CELL_EMPTY
                    && map[yPoint_4567-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4567][x+1] === this.CELL_EMPTY
                    && map[yPoint_4567+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4567+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4567, yPoint_4567, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4567;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4567] === this.CELL_EMPTY
                    && map[y+1][xPoint_4567] === this.CELL_EMPTY
                    && map[y+1][xPoint_4567+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4567] === this.CELL_EMPTY
                    && map[y][xPoint_4567-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4567-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
