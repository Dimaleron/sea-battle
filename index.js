...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18946=0;yPoint_18946<this.gameFieldBorderY.length; yPoint_18946++){
                for(var xPoint_18946=0;xPoint_18946<this.gameFieldBorderX.length; xPoint_18946++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18946, xPoint_18946);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18946][xPoint_18946] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18946, xPoint_18946, 'user');
                    if(this._userShipsMap[yPoint_18946][xPoint_18946] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18946, xPoint_18946, type){
            var id = this.getPointBlockIdByCoords(yPoint_18946, xPoint_18946, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18946);
                block.setAttribute('data-y', yPoint_18946);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18946
         * @param {type} xPoint_18946
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18946, xPoint_18946, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18946 + '_y' + yPoint_18946;
            }
            return 'pc_x' + xPoint_18946 + '_y' + yPoint_18946;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18946=0;yPoint_18946<this.gameFieldBorderY.length; yPoint_18946++){
                for(var xPoint_18946=0;xPoint_18946<this.gameFieldBorderX.length; xPoint_18946++){
                    map.push({y: yPoint_18946, x: xPoint_18946});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18946=-1;yPoint_18946<(this.gameFieldBorderY.length+1); yPoint_18946++){
                for(var xPoint_18946=-1;xPoint_18946<(this.gameFieldBorderX.length+1); xPoint_18946++){
                    if(!map[yPoint_18946]){
                        map[yPoint_18946] = [];
                    }
                    map[yPoint_18946][xPoint_18946] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18946 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18946 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18946, yPoint_18946) === true){
                    if(this.canPutHorizontal(map, xPoint_18946, yPoint_18946, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18946][xPoint_18946 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18946, yPoint_18946, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18946 + i][xPoint_18946] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18946, yPoint_18946){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18946][xPoint_18946] === this.CELL_EMPTY
                && map[yPoint_18946-1][xPoint_18946] === this.CELL_EMPTY
                && map[yPoint_18946-1][xPoint_18946+1] === this.CELL_EMPTY
                && map[yPoint_18946][xPoint_18946+1] === this.CELL_EMPTY
                && map[yPoint_18946+1][xPoint_18946+1] === this.CELL_EMPTY
                && map[yPoint_18946+1][xPoint_18946] === this.CELL_EMPTY
                && map[yPoint_18946+1][xPoint_18946-1] === this.CELL_EMPTY
                && map[yPoint_18946][xPoint_18946-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18946, yPoint_18946, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18946;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18946][x] === this.CELL_EMPTY
                    && map[yPoint_18946-1][x] === this.CELL_EMPTY
                    && map[yPoint_18946-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18946][x+1] === this.CELL_EMPTY
                    && map[yPoint_18946+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18946+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18946, yPoint_18946, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18946;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18946] === this.CELL_EMPTY
                    && map[y+1][xPoint_18946] === this.CELL_EMPTY
                    && map[y+1][xPoint_18946+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18946] === this.CELL_EMPTY
                    && map[y][xPoint_18946-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18946-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
