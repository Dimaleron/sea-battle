...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_241=0;yPoint_241<this.gameFieldBorderY.length; yPoint_241++){
                for(var xPoint_241=0;xPoint_241<this.gameFieldBorderX.length; xPoint_241++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_241, xPoint_241);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_241][xPoint_241] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_241, xPoint_241, 'user');
                    if(this._userShipsMap[yPoint_241][xPoint_241] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_241, xPoint_241, type){
            var id = this.getPointBlockIdByCoords(yPoint_241, xPoint_241, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_241);
                block.setAttribute('data-y', yPoint_241);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_241
         * @param {type} xPoint_241
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_241, xPoint_241, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_241 + '_y' + yPoint_241;
            }
            return 'pc_x' + xPoint_241 + '_y' + yPoint_241;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_241=0;yPoint_241<this.gameFieldBorderY.length; yPoint_241++){
                for(var xPoint_241=0;xPoint_241<this.gameFieldBorderX.length; xPoint_241++){
                    map.push({y: yPoint_241, x: xPoint_241});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_241=-1;yPoint_241<(this.gameFieldBorderY.length+1); yPoint_241++){
                for(var xPoint_241=-1;xPoint_241<(this.gameFieldBorderX.length+1); xPoint_241++){
                    if(!map[yPoint_241]){
                        map[yPoint_241] = [];
                    }
                    map[yPoint_241][xPoint_241] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_241 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_241 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_241, yPoint_241) === true){
                    if(this.canPutHorizontal(map, xPoint_241, yPoint_241, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_241][xPoint_241 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_241, yPoint_241, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_241 + i][xPoint_241] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_241, yPoint_241){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_241][xPoint_241] === this.CELL_EMPTY
                && map[yPoint_241-1][xPoint_241] === this.CELL_EMPTY
                && map[yPoint_241-1][xPoint_241+1] === this.CELL_EMPTY
                && map[yPoint_241][xPoint_241+1] === this.CELL_EMPTY
                && map[yPoint_241+1][xPoint_241+1] === this.CELL_EMPTY
                && map[yPoint_241+1][xPoint_241] === this.CELL_EMPTY
                && map[yPoint_241+1][xPoint_241-1] === this.CELL_EMPTY
                && map[yPoint_241][xPoint_241-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_241, yPoint_241, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_241;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_241][x] === this.CELL_EMPTY
                    && map[yPoint_241-1][x] === this.CELL_EMPTY
                    && map[yPoint_241-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_241][x+1] === this.CELL_EMPTY
                    && map[yPoint_241+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_241+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_241, yPoint_241, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_241;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_241] === this.CELL_EMPTY
                    && map[y+1][xPoint_241] === this.CELL_EMPTY
                    && map[y+1][xPoint_241+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_241] === this.CELL_EMPTY
                    && map[y][xPoint_241-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_241-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
