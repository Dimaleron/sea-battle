...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31545=0;yPoint_31545<this.gameFieldBorderY.length; yPoint_31545++){
                for(var xPoint_31545=0;xPoint_31545<this.gameFieldBorderX.length; xPoint_31545++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31545, xPoint_31545);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31545][xPoint_31545] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31545, xPoint_31545, 'user');
                    if(this._userShipsMap[yPoint_31545][xPoint_31545] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31545, xPoint_31545, type){
            var id = this.getPointBlockIdByCoords(yPoint_31545, xPoint_31545, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31545);
                block.setAttribute('data-y', yPoint_31545);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31545
         * @param {type} xPoint_31545
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31545, xPoint_31545, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31545 + '_y' + yPoint_31545;
            }
            return 'pc_x' + xPoint_31545 + '_y' + yPoint_31545;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31545=0;yPoint_31545<this.gameFieldBorderY.length; yPoint_31545++){
                for(var xPoint_31545=0;xPoint_31545<this.gameFieldBorderX.length; xPoint_31545++){
                    map.push({y: yPoint_31545, x: xPoint_31545});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31545=-1;yPoint_31545<(this.gameFieldBorderY.length+1); yPoint_31545++){
                for(var xPoint_31545=-1;xPoint_31545<(this.gameFieldBorderX.length+1); xPoint_31545++){
                    if(!map[yPoint_31545]){
                        map[yPoint_31545] = [];
                    }
                    map[yPoint_31545][xPoint_31545] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31545 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31545 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31545, yPoint_31545) === true){
                    if(this.canPutHorizontal(map, xPoint_31545, yPoint_31545, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31545][xPoint_31545 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31545, yPoint_31545, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31545 + i][xPoint_31545] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31545, yPoint_31545){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31545][xPoint_31545] === this.CELL_EMPTY
                && map[yPoint_31545-1][xPoint_31545] === this.CELL_EMPTY
                && map[yPoint_31545-1][xPoint_31545+1] === this.CELL_EMPTY
                && map[yPoint_31545][xPoint_31545+1] === this.CELL_EMPTY
                && map[yPoint_31545+1][xPoint_31545+1] === this.CELL_EMPTY
                && map[yPoint_31545+1][xPoint_31545] === this.CELL_EMPTY
                && map[yPoint_31545+1][xPoint_31545-1] === this.CELL_EMPTY
                && map[yPoint_31545][xPoint_31545-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31545, yPoint_31545, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31545;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31545][x] === this.CELL_EMPTY
                    && map[yPoint_31545-1][x] === this.CELL_EMPTY
                    && map[yPoint_31545-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31545][x+1] === this.CELL_EMPTY
                    && map[yPoint_31545+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31545+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31545, yPoint_31545, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31545;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31545] === this.CELL_EMPTY
                    && map[y+1][xPoint_31545] === this.CELL_EMPTY
                    && map[y+1][xPoint_31545+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31545] === this.CELL_EMPTY
                    && map[y][xPoint_31545-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31545-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
