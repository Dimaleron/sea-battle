...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17647=0;yPoint_17647<this.gameFieldBorderY.length; yPoint_17647++){
                for(var xPoint_17647=0;xPoint_17647<this.gameFieldBorderX.length; xPoint_17647++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17647, xPoint_17647);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17647][xPoint_17647] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17647, xPoint_17647, 'user');
                    if(this._userShipsMap[yPoint_17647][xPoint_17647] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17647, xPoint_17647, type){
            var id = this.getPointBlockIdByCoords(yPoint_17647, xPoint_17647, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17647);
                block.setAttribute('data-y', yPoint_17647);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17647
         * @param {type} xPoint_17647
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17647, xPoint_17647, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17647 + '_y' + yPoint_17647;
            }
            return 'pc_x' + xPoint_17647 + '_y' + yPoint_17647;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17647=0;yPoint_17647<this.gameFieldBorderY.length; yPoint_17647++){
                for(var xPoint_17647=0;xPoint_17647<this.gameFieldBorderX.length; xPoint_17647++){
                    map.push({y: yPoint_17647, x: xPoint_17647});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17647=-1;yPoint_17647<(this.gameFieldBorderY.length+1); yPoint_17647++){
                for(var xPoint_17647=-1;xPoint_17647<(this.gameFieldBorderX.length+1); xPoint_17647++){
                    if(!map[yPoint_17647]){
                        map[yPoint_17647] = [];
                    }
                    map[yPoint_17647][xPoint_17647] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17647 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17647 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17647, yPoint_17647) === true){
                    if(this.canPutHorizontal(map, xPoint_17647, yPoint_17647, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17647][xPoint_17647 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17647, yPoint_17647, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17647 + i][xPoint_17647] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17647, yPoint_17647){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17647][xPoint_17647] === this.CELL_EMPTY
                && map[yPoint_17647-1][xPoint_17647] === this.CELL_EMPTY
                && map[yPoint_17647-1][xPoint_17647+1] === this.CELL_EMPTY
                && map[yPoint_17647][xPoint_17647+1] === this.CELL_EMPTY
                && map[yPoint_17647+1][xPoint_17647+1] === this.CELL_EMPTY
                && map[yPoint_17647+1][xPoint_17647] === this.CELL_EMPTY
                && map[yPoint_17647+1][xPoint_17647-1] === this.CELL_EMPTY
                && map[yPoint_17647][xPoint_17647-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17647, yPoint_17647, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17647;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17647][x] === this.CELL_EMPTY
                    && map[yPoint_17647-1][x] === this.CELL_EMPTY
                    && map[yPoint_17647-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17647][x+1] === this.CELL_EMPTY
                    && map[yPoint_17647+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17647+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17647, yPoint_17647, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17647;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17647] === this.CELL_EMPTY
                    && map[y+1][xPoint_17647] === this.CELL_EMPTY
                    && map[y+1][xPoint_17647+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17647] === this.CELL_EMPTY
                    && map[y][xPoint_17647-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17647-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
