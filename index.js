...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11237=0;yPoint_11237<this.gameFieldBorderY.length; yPoint_11237++){
                for(var xPoint_11237=0;xPoint_11237<this.gameFieldBorderX.length; xPoint_11237++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11237, xPoint_11237);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11237][xPoint_11237] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11237, xPoint_11237, 'user');
                    if(this._userShipsMap[yPoint_11237][xPoint_11237] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11237, xPoint_11237, type){
            var id = this.getPointBlockIdByCoords(yPoint_11237, xPoint_11237, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11237);
                block.setAttribute('data-y', yPoint_11237);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11237
         * @param {type} xPoint_11237
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11237, xPoint_11237, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11237 + '_y' + yPoint_11237;
            }
            return 'pc_x' + xPoint_11237 + '_y' + yPoint_11237;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11237=0;yPoint_11237<this.gameFieldBorderY.length; yPoint_11237++){
                for(var xPoint_11237=0;xPoint_11237<this.gameFieldBorderX.length; xPoint_11237++){
                    map.push({y: yPoint_11237, x: xPoint_11237});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11237=-1;yPoint_11237<(this.gameFieldBorderY.length+1); yPoint_11237++){
                for(var xPoint_11237=-1;xPoint_11237<(this.gameFieldBorderX.length+1); xPoint_11237++){
                    if(!map[yPoint_11237]){
                        map[yPoint_11237] = [];
                    }
                    map[yPoint_11237][xPoint_11237] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11237 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11237 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11237, yPoint_11237) === true){
                    if(this.canPutHorizontal(map, xPoint_11237, yPoint_11237, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11237][xPoint_11237 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11237, yPoint_11237, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11237 + i][xPoint_11237] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11237, yPoint_11237){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11237][xPoint_11237] === this.CELL_EMPTY
                && map[yPoint_11237-1][xPoint_11237] === this.CELL_EMPTY
                && map[yPoint_11237-1][xPoint_11237+1] === this.CELL_EMPTY
                && map[yPoint_11237][xPoint_11237+1] === this.CELL_EMPTY
                && map[yPoint_11237+1][xPoint_11237+1] === this.CELL_EMPTY
                && map[yPoint_11237+1][xPoint_11237] === this.CELL_EMPTY
                && map[yPoint_11237+1][xPoint_11237-1] === this.CELL_EMPTY
                && map[yPoint_11237][xPoint_11237-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11237, yPoint_11237, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11237;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11237][x] === this.CELL_EMPTY
                    && map[yPoint_11237-1][x] === this.CELL_EMPTY
                    && map[yPoint_11237-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11237][x+1] === this.CELL_EMPTY
                    && map[yPoint_11237+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11237+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11237, yPoint_11237, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11237;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11237] === this.CELL_EMPTY
                    && map[y+1][xPoint_11237] === this.CELL_EMPTY
                    && map[y+1][xPoint_11237+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11237] === this.CELL_EMPTY
                    && map[y][xPoint_11237-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11237-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
