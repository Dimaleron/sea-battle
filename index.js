...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24116=0;yPoint_24116<this.gameFieldBorderY.length; yPoint_24116++){
                for(var xPoint_24116=0;xPoint_24116<this.gameFieldBorderX.length; xPoint_24116++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24116, xPoint_24116);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24116][xPoint_24116] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24116, xPoint_24116, 'user');
                    if(this._userShipsMap[yPoint_24116][xPoint_24116] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24116, xPoint_24116, type){
            var id = this.getPointBlockIdByCoords(yPoint_24116, xPoint_24116, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24116);
                block.setAttribute('data-y', yPoint_24116);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24116
         * @param {type} xPoint_24116
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24116, xPoint_24116, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24116 + '_y' + yPoint_24116;
            }
            return 'pc_x' + xPoint_24116 + '_y' + yPoint_24116;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24116=0;yPoint_24116<this.gameFieldBorderY.length; yPoint_24116++){
                for(var xPoint_24116=0;xPoint_24116<this.gameFieldBorderX.length; xPoint_24116++){
                    map.push({y: yPoint_24116, x: xPoint_24116});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24116=-1;yPoint_24116<(this.gameFieldBorderY.length+1); yPoint_24116++){
                for(var xPoint_24116=-1;xPoint_24116<(this.gameFieldBorderX.length+1); xPoint_24116++){
                    if(!map[yPoint_24116]){
                        map[yPoint_24116] = [];
                    }
                    map[yPoint_24116][xPoint_24116] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24116 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24116 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24116, yPoint_24116) === true){
                    if(this.canPutHorizontal(map, xPoint_24116, yPoint_24116, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24116][xPoint_24116 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24116, yPoint_24116, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24116 + i][xPoint_24116] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24116, yPoint_24116){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24116][xPoint_24116] === this.CELL_EMPTY
                && map[yPoint_24116-1][xPoint_24116] === this.CELL_EMPTY
                && map[yPoint_24116-1][xPoint_24116+1] === this.CELL_EMPTY
                && map[yPoint_24116][xPoint_24116+1] === this.CELL_EMPTY
                && map[yPoint_24116+1][xPoint_24116+1] === this.CELL_EMPTY
                && map[yPoint_24116+1][xPoint_24116] === this.CELL_EMPTY
                && map[yPoint_24116+1][xPoint_24116-1] === this.CELL_EMPTY
                && map[yPoint_24116][xPoint_24116-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24116, yPoint_24116, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24116;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24116][x] === this.CELL_EMPTY
                    && map[yPoint_24116-1][x] === this.CELL_EMPTY
                    && map[yPoint_24116-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24116][x+1] === this.CELL_EMPTY
                    && map[yPoint_24116+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24116+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24116, yPoint_24116, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24116;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24116] === this.CELL_EMPTY
                    && map[y+1][xPoint_24116] === this.CELL_EMPTY
                    && map[y+1][xPoint_24116+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24116] === this.CELL_EMPTY
                    && map[y][xPoint_24116-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24116-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
