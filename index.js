...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10704=0;yPoint_10704<this.gameFieldBorderY.length; yPoint_10704++){
                for(var xPoint_10704=0;xPoint_10704<this.gameFieldBorderX.length; xPoint_10704++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10704, xPoint_10704);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10704][xPoint_10704] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10704, xPoint_10704, 'user');
                    if(this._userShipsMap[yPoint_10704][xPoint_10704] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10704, xPoint_10704, type){
            var id = this.getPointBlockIdByCoords(yPoint_10704, xPoint_10704, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10704);
                block.setAttribute('data-y', yPoint_10704);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10704
         * @param {type} xPoint_10704
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10704, xPoint_10704, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10704 + '_y' + yPoint_10704;
            }
            return 'pc_x' + xPoint_10704 + '_y' + yPoint_10704;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10704=0;yPoint_10704<this.gameFieldBorderY.length; yPoint_10704++){
                for(var xPoint_10704=0;xPoint_10704<this.gameFieldBorderX.length; xPoint_10704++){
                    map.push({y: yPoint_10704, x: xPoint_10704});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10704=-1;yPoint_10704<(this.gameFieldBorderY.length+1); yPoint_10704++){
                for(var xPoint_10704=-1;xPoint_10704<(this.gameFieldBorderX.length+1); xPoint_10704++){
                    if(!map[yPoint_10704]){
                        map[yPoint_10704] = [];
                    }
                    map[yPoint_10704][xPoint_10704] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10704 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10704 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10704, yPoint_10704) === true){
                    if(this.canPutHorizontal(map, xPoint_10704, yPoint_10704, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10704][xPoint_10704 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10704, yPoint_10704, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10704 + i][xPoint_10704] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10704, yPoint_10704){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10704][xPoint_10704] === this.CELL_EMPTY
                && map[yPoint_10704-1][xPoint_10704] === this.CELL_EMPTY
                && map[yPoint_10704-1][xPoint_10704+1] === this.CELL_EMPTY
                && map[yPoint_10704][xPoint_10704+1] === this.CELL_EMPTY
                && map[yPoint_10704+1][xPoint_10704+1] === this.CELL_EMPTY
                && map[yPoint_10704+1][xPoint_10704] === this.CELL_EMPTY
                && map[yPoint_10704+1][xPoint_10704-1] === this.CELL_EMPTY
                && map[yPoint_10704][xPoint_10704-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10704, yPoint_10704, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10704;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10704][x] === this.CELL_EMPTY
                    && map[yPoint_10704-1][x] === this.CELL_EMPTY
                    && map[yPoint_10704-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10704][x+1] === this.CELL_EMPTY
                    && map[yPoint_10704+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10704+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10704, yPoint_10704, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10704;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10704] === this.CELL_EMPTY
                    && map[y+1][xPoint_10704] === this.CELL_EMPTY
                    && map[y+1][xPoint_10704+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10704] === this.CELL_EMPTY
                    && map[y][xPoint_10704-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10704-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
