...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24888=0;yPoint_24888<this.gameFieldBorderY.length; yPoint_24888++){
                for(var xPoint_24888=0;xPoint_24888<this.gameFieldBorderX.length; xPoint_24888++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24888, xPoint_24888);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24888][xPoint_24888] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24888, xPoint_24888, 'user');
                    if(this._userShipsMap[yPoint_24888][xPoint_24888] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24888, xPoint_24888, type){
            var id = this.getPointBlockIdByCoords(yPoint_24888, xPoint_24888, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24888);
                block.setAttribute('data-y', yPoint_24888);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24888
         * @param {type} xPoint_24888
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24888, xPoint_24888, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24888 + '_y' + yPoint_24888;
            }
            return 'pc_x' + xPoint_24888 + '_y' + yPoint_24888;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24888=0;yPoint_24888<this.gameFieldBorderY.length; yPoint_24888++){
                for(var xPoint_24888=0;xPoint_24888<this.gameFieldBorderX.length; xPoint_24888++){
                    map.push({y: yPoint_24888, x: xPoint_24888});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24888=-1;yPoint_24888<(this.gameFieldBorderY.length+1); yPoint_24888++){
                for(var xPoint_24888=-1;xPoint_24888<(this.gameFieldBorderX.length+1); xPoint_24888++){
                    if(!map[yPoint_24888]){
                        map[yPoint_24888] = [];
                    }
                    map[yPoint_24888][xPoint_24888] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24888 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24888 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24888, yPoint_24888) === true){
                    if(this.canPutHorizontal(map, xPoint_24888, yPoint_24888, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24888][xPoint_24888 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24888, yPoint_24888, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24888 + i][xPoint_24888] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24888, yPoint_24888){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24888][xPoint_24888] === this.CELL_EMPTY
                && map[yPoint_24888-1][xPoint_24888] === this.CELL_EMPTY
                && map[yPoint_24888-1][xPoint_24888+1] === this.CELL_EMPTY
                && map[yPoint_24888][xPoint_24888+1] === this.CELL_EMPTY
                && map[yPoint_24888+1][xPoint_24888+1] === this.CELL_EMPTY
                && map[yPoint_24888+1][xPoint_24888] === this.CELL_EMPTY
                && map[yPoint_24888+1][xPoint_24888-1] === this.CELL_EMPTY
                && map[yPoint_24888][xPoint_24888-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24888, yPoint_24888, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24888;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24888][x] === this.CELL_EMPTY
                    && map[yPoint_24888-1][x] === this.CELL_EMPTY
                    && map[yPoint_24888-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24888][x+1] === this.CELL_EMPTY
                    && map[yPoint_24888+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24888+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24888, yPoint_24888, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24888;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24888] === this.CELL_EMPTY
                    && map[y+1][xPoint_24888] === this.CELL_EMPTY
                    && map[y+1][xPoint_24888+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24888] === this.CELL_EMPTY
                    && map[y][xPoint_24888-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24888-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
