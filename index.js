...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11071=0;yPoint_11071<this.gameFieldBorderY.length; yPoint_11071++){
                for(var xPoint_11071=0;xPoint_11071<this.gameFieldBorderX.length; xPoint_11071++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11071, xPoint_11071);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11071][xPoint_11071] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11071, xPoint_11071, 'user');
                    if(this._userShipsMap[yPoint_11071][xPoint_11071] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11071, xPoint_11071, type){
            var id = this.getPointBlockIdByCoords(yPoint_11071, xPoint_11071, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11071);
                block.setAttribute('data-y', yPoint_11071);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11071
         * @param {type} xPoint_11071
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11071, xPoint_11071, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11071 + '_y' + yPoint_11071;
            }
            return 'pc_x' + xPoint_11071 + '_y' + yPoint_11071;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11071=0;yPoint_11071<this.gameFieldBorderY.length; yPoint_11071++){
                for(var xPoint_11071=0;xPoint_11071<this.gameFieldBorderX.length; xPoint_11071++){
                    map.push({y: yPoint_11071, x: xPoint_11071});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11071=-1;yPoint_11071<(this.gameFieldBorderY.length+1); yPoint_11071++){
                for(var xPoint_11071=-1;xPoint_11071<(this.gameFieldBorderX.length+1); xPoint_11071++){
                    if(!map[yPoint_11071]){
                        map[yPoint_11071] = [];
                    }
                    map[yPoint_11071][xPoint_11071] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11071 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11071 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11071, yPoint_11071) === true){
                    if(this.canPutHorizontal(map, xPoint_11071, yPoint_11071, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11071][xPoint_11071 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11071, yPoint_11071, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11071 + i][xPoint_11071] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11071, yPoint_11071){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11071][xPoint_11071] === this.CELL_EMPTY
                && map[yPoint_11071-1][xPoint_11071] === this.CELL_EMPTY
                && map[yPoint_11071-1][xPoint_11071+1] === this.CELL_EMPTY
                && map[yPoint_11071][xPoint_11071+1] === this.CELL_EMPTY
                && map[yPoint_11071+1][xPoint_11071+1] === this.CELL_EMPTY
                && map[yPoint_11071+1][xPoint_11071] === this.CELL_EMPTY
                && map[yPoint_11071+1][xPoint_11071-1] === this.CELL_EMPTY
                && map[yPoint_11071][xPoint_11071-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11071, yPoint_11071, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11071;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11071][x] === this.CELL_EMPTY
                    && map[yPoint_11071-1][x] === this.CELL_EMPTY
                    && map[yPoint_11071-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11071][x+1] === this.CELL_EMPTY
                    && map[yPoint_11071+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11071+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11071, yPoint_11071, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11071;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11071] === this.CELL_EMPTY
                    && map[y+1][xPoint_11071] === this.CELL_EMPTY
                    && map[y+1][xPoint_11071+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11071] === this.CELL_EMPTY
                    && map[y][xPoint_11071-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11071-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
