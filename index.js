...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4139=0;yPoint_4139<this.gameFieldBorderY.length; yPoint_4139++){
                for(var xPoint_4139=0;xPoint_4139<this.gameFieldBorderX.length; xPoint_4139++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4139, xPoint_4139);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4139][xPoint_4139] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4139, xPoint_4139, 'user');
                    if(this._userShipsMap[yPoint_4139][xPoint_4139] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4139, xPoint_4139, type){
            var id = this.getPointBlockIdByCoords(yPoint_4139, xPoint_4139, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4139);
                block.setAttribute('data-y', yPoint_4139);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4139
         * @param {type} xPoint_4139
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4139, xPoint_4139, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4139 + '_y' + yPoint_4139;
            }
            return 'pc_x' + xPoint_4139 + '_y' + yPoint_4139;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4139=0;yPoint_4139<this.gameFieldBorderY.length; yPoint_4139++){
                for(var xPoint_4139=0;xPoint_4139<this.gameFieldBorderX.length; xPoint_4139++){
                    map.push({y: yPoint_4139, x: xPoint_4139});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4139=-1;yPoint_4139<(this.gameFieldBorderY.length+1); yPoint_4139++){
                for(var xPoint_4139=-1;xPoint_4139<(this.gameFieldBorderX.length+1); xPoint_4139++){
                    if(!map[yPoint_4139]){
                        map[yPoint_4139] = [];
                    }
                    map[yPoint_4139][xPoint_4139] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4139 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4139 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4139, yPoint_4139) === true){
                    if(this.canPutHorizontal(map, xPoint_4139, yPoint_4139, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4139][xPoint_4139 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4139, yPoint_4139, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4139 + i][xPoint_4139] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4139, yPoint_4139){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4139][xPoint_4139] === this.CELL_EMPTY
                && map[yPoint_4139-1][xPoint_4139] === this.CELL_EMPTY
                && map[yPoint_4139-1][xPoint_4139+1] === this.CELL_EMPTY
                && map[yPoint_4139][xPoint_4139+1] === this.CELL_EMPTY
                && map[yPoint_4139+1][xPoint_4139+1] === this.CELL_EMPTY
                && map[yPoint_4139+1][xPoint_4139] === this.CELL_EMPTY
                && map[yPoint_4139+1][xPoint_4139-1] === this.CELL_EMPTY
                && map[yPoint_4139][xPoint_4139-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4139, yPoint_4139, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4139;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4139][x] === this.CELL_EMPTY
                    && map[yPoint_4139-1][x] === this.CELL_EMPTY
                    && map[yPoint_4139-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4139][x+1] === this.CELL_EMPTY
                    && map[yPoint_4139+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4139+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4139, yPoint_4139, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4139;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4139] === this.CELL_EMPTY
                    && map[y+1][xPoint_4139] === this.CELL_EMPTY
                    && map[y+1][xPoint_4139+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4139] === this.CELL_EMPTY
                    && map[y][xPoint_4139-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4139-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
