...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10521=0;yPoint_10521<this.gameFieldBorderY.length; yPoint_10521++){
                for(var xPoint_10521=0;xPoint_10521<this.gameFieldBorderX.length; xPoint_10521++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10521, xPoint_10521);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10521][xPoint_10521] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10521, xPoint_10521, 'user');
                    if(this._userShipsMap[yPoint_10521][xPoint_10521] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10521, xPoint_10521, type){
            var id = this.getPointBlockIdByCoords(yPoint_10521, xPoint_10521, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10521);
                block.setAttribute('data-y', yPoint_10521);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10521
         * @param {type} xPoint_10521
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10521, xPoint_10521, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10521 + '_y' + yPoint_10521;
            }
            return 'pc_x' + xPoint_10521 + '_y' + yPoint_10521;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10521=0;yPoint_10521<this.gameFieldBorderY.length; yPoint_10521++){
                for(var xPoint_10521=0;xPoint_10521<this.gameFieldBorderX.length; xPoint_10521++){
                    map.push({y: yPoint_10521, x: xPoint_10521});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10521=-1;yPoint_10521<(this.gameFieldBorderY.length+1); yPoint_10521++){
                for(var xPoint_10521=-1;xPoint_10521<(this.gameFieldBorderX.length+1); xPoint_10521++){
                    if(!map[yPoint_10521]){
                        map[yPoint_10521] = [];
                    }
                    map[yPoint_10521][xPoint_10521] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10521 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10521 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10521, yPoint_10521) === true){
                    if(this.canPutHorizontal(map, xPoint_10521, yPoint_10521, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10521][xPoint_10521 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10521, yPoint_10521, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10521 + i][xPoint_10521] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10521, yPoint_10521){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10521][xPoint_10521] === this.CELL_EMPTY
                && map[yPoint_10521-1][xPoint_10521] === this.CELL_EMPTY
                && map[yPoint_10521-1][xPoint_10521+1] === this.CELL_EMPTY
                && map[yPoint_10521][xPoint_10521+1] === this.CELL_EMPTY
                && map[yPoint_10521+1][xPoint_10521+1] === this.CELL_EMPTY
                && map[yPoint_10521+1][xPoint_10521] === this.CELL_EMPTY
                && map[yPoint_10521+1][xPoint_10521-1] === this.CELL_EMPTY
                && map[yPoint_10521][xPoint_10521-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10521, yPoint_10521, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10521;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10521][x] === this.CELL_EMPTY
                    && map[yPoint_10521-1][x] === this.CELL_EMPTY
                    && map[yPoint_10521-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10521][x+1] === this.CELL_EMPTY
                    && map[yPoint_10521+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10521+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10521, yPoint_10521, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10521;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10521] === this.CELL_EMPTY
                    && map[y+1][xPoint_10521] === this.CELL_EMPTY
                    && map[y+1][xPoint_10521+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10521] === this.CELL_EMPTY
                    && map[y][xPoint_10521-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10521-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
