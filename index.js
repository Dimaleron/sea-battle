...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_991=0;yPoint_991<this.gameFieldBorderY.length; yPoint_991++){
                for(var xPoint_991=0;xPoint_991<this.gameFieldBorderX.length; xPoint_991++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_991, xPoint_991);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_991][xPoint_991] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_991, xPoint_991, 'user');
                    if(this._userShipsMap[yPoint_991][xPoint_991] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_991, xPoint_991, type){
            var id = this.getPointBlockIdByCoords(yPoint_991, xPoint_991, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_991);
                block.setAttribute('data-y', yPoint_991);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_991
         * @param {type} xPoint_991
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_991, xPoint_991, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_991 + '_y' + yPoint_991;
            }
            return 'pc_x' + xPoint_991 + '_y' + yPoint_991;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_991=0;yPoint_991<this.gameFieldBorderY.length; yPoint_991++){
                for(var xPoint_991=0;xPoint_991<this.gameFieldBorderX.length; xPoint_991++){
                    map.push({y: yPoint_991, x: xPoint_991});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_991=-1;yPoint_991<(this.gameFieldBorderY.length+1); yPoint_991++){
                for(var xPoint_991=-1;xPoint_991<(this.gameFieldBorderX.length+1); xPoint_991++){
                    if(!map[yPoint_991]){
                        map[yPoint_991] = [];
                    }
                    map[yPoint_991][xPoint_991] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_991 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_991 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_991, yPoint_991) === true){
                    if(this.canPutHorizontal(map, xPoint_991, yPoint_991, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_991][xPoint_991 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_991, yPoint_991, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_991 + i][xPoint_991] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_991, yPoint_991){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_991][xPoint_991] === this.CELL_EMPTY
                && map[yPoint_991-1][xPoint_991] === this.CELL_EMPTY
                && map[yPoint_991-1][xPoint_991+1] === this.CELL_EMPTY
                && map[yPoint_991][xPoint_991+1] === this.CELL_EMPTY
                && map[yPoint_991+1][xPoint_991+1] === this.CELL_EMPTY
                && map[yPoint_991+1][xPoint_991] === this.CELL_EMPTY
                && map[yPoint_991+1][xPoint_991-1] === this.CELL_EMPTY
                && map[yPoint_991][xPoint_991-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_991, yPoint_991, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_991;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_991][x] === this.CELL_EMPTY
                    && map[yPoint_991-1][x] === this.CELL_EMPTY
                    && map[yPoint_991-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_991][x+1] === this.CELL_EMPTY
                    && map[yPoint_991+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_991+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_991, yPoint_991, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_991;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_991] === this.CELL_EMPTY
                    && map[y+1][xPoint_991] === this.CELL_EMPTY
                    && map[y+1][xPoint_991+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_991] === this.CELL_EMPTY
                    && map[y][xPoint_991-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_991-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
