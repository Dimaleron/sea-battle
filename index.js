...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8735=0;yPoint_8735<this.gameFieldBorderY.length; yPoint_8735++){
                for(var xPoint_8735=0;xPoint_8735<this.gameFieldBorderX.length; xPoint_8735++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8735, xPoint_8735);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8735][xPoint_8735] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8735, xPoint_8735, 'user');
                    if(this._userShipsMap[yPoint_8735][xPoint_8735] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8735, xPoint_8735, type){
            var id = this.getPointBlockIdByCoords(yPoint_8735, xPoint_8735, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8735);
                block.setAttribute('data-y', yPoint_8735);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8735
         * @param {type} xPoint_8735
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8735, xPoint_8735, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8735 + '_y' + yPoint_8735;
            }
            return 'pc_x' + xPoint_8735 + '_y' + yPoint_8735;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8735=0;yPoint_8735<this.gameFieldBorderY.length; yPoint_8735++){
                for(var xPoint_8735=0;xPoint_8735<this.gameFieldBorderX.length; xPoint_8735++){
                    map.push({y: yPoint_8735, x: xPoint_8735});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8735=-1;yPoint_8735<(this.gameFieldBorderY.length+1); yPoint_8735++){
                for(var xPoint_8735=-1;xPoint_8735<(this.gameFieldBorderX.length+1); xPoint_8735++){
                    if(!map[yPoint_8735]){
                        map[yPoint_8735] = [];
                    }
                    map[yPoint_8735][xPoint_8735] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8735 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8735 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8735, yPoint_8735) === true){
                    if(this.canPutHorizontal(map, xPoint_8735, yPoint_8735, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8735][xPoint_8735 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8735, yPoint_8735, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8735 + i][xPoint_8735] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8735, yPoint_8735){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8735][xPoint_8735] === this.CELL_EMPTY
                && map[yPoint_8735-1][xPoint_8735] === this.CELL_EMPTY
                && map[yPoint_8735-1][xPoint_8735+1] === this.CELL_EMPTY
                && map[yPoint_8735][xPoint_8735+1] === this.CELL_EMPTY
                && map[yPoint_8735+1][xPoint_8735+1] === this.CELL_EMPTY
                && map[yPoint_8735+1][xPoint_8735] === this.CELL_EMPTY
                && map[yPoint_8735+1][xPoint_8735-1] === this.CELL_EMPTY
                && map[yPoint_8735][xPoint_8735-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8735, yPoint_8735, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8735;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8735][x] === this.CELL_EMPTY
                    && map[yPoint_8735-1][x] === this.CELL_EMPTY
                    && map[yPoint_8735-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8735][x+1] === this.CELL_EMPTY
                    && map[yPoint_8735+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8735+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8735, yPoint_8735, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8735;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8735] === this.CELL_EMPTY
                    && map[y+1][xPoint_8735] === this.CELL_EMPTY
                    && map[y+1][xPoint_8735+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8735] === this.CELL_EMPTY
                    && map[y][xPoint_8735-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8735-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
