...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7650=0;yPoint_7650<this.gameFieldBorderY.length; yPoint_7650++){
                for(var xPoint_7650=0;xPoint_7650<this.gameFieldBorderX.length; xPoint_7650++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7650, xPoint_7650);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7650][xPoint_7650] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7650, xPoint_7650, 'user');
                    if(this._userShipsMap[yPoint_7650][xPoint_7650] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7650, xPoint_7650, type){
            var id = this.getPointBlockIdByCoords(yPoint_7650, xPoint_7650, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7650);
                block.setAttribute('data-y', yPoint_7650);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7650
         * @param {type} xPoint_7650
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7650, xPoint_7650, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7650 + '_y' + yPoint_7650;
            }
            return 'pc_x' + xPoint_7650 + '_y' + yPoint_7650;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7650=0;yPoint_7650<this.gameFieldBorderY.length; yPoint_7650++){
                for(var xPoint_7650=0;xPoint_7650<this.gameFieldBorderX.length; xPoint_7650++){
                    map.push({y: yPoint_7650, x: xPoint_7650});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7650=-1;yPoint_7650<(this.gameFieldBorderY.length+1); yPoint_7650++){
                for(var xPoint_7650=-1;xPoint_7650<(this.gameFieldBorderX.length+1); xPoint_7650++){
                    if(!map[yPoint_7650]){
                        map[yPoint_7650] = [];
                    }
                    map[yPoint_7650][xPoint_7650] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7650 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7650 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7650, yPoint_7650) === true){
                    if(this.canPutHorizontal(map, xPoint_7650, yPoint_7650, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7650][xPoint_7650 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7650, yPoint_7650, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7650 + i][xPoint_7650] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7650, yPoint_7650){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7650][xPoint_7650] === this.CELL_EMPTY
                && map[yPoint_7650-1][xPoint_7650] === this.CELL_EMPTY
                && map[yPoint_7650-1][xPoint_7650+1] === this.CELL_EMPTY
                && map[yPoint_7650][xPoint_7650+1] === this.CELL_EMPTY
                && map[yPoint_7650+1][xPoint_7650+1] === this.CELL_EMPTY
                && map[yPoint_7650+1][xPoint_7650] === this.CELL_EMPTY
                && map[yPoint_7650+1][xPoint_7650-1] === this.CELL_EMPTY
                && map[yPoint_7650][xPoint_7650-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7650, yPoint_7650, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7650;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7650][x] === this.CELL_EMPTY
                    && map[yPoint_7650-1][x] === this.CELL_EMPTY
                    && map[yPoint_7650-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7650][x+1] === this.CELL_EMPTY
                    && map[yPoint_7650+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7650+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7650, yPoint_7650, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7650;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7650] === this.CELL_EMPTY
                    && map[y+1][xPoint_7650] === this.CELL_EMPTY
                    && map[y+1][xPoint_7650+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7650] === this.CELL_EMPTY
                    && map[y][xPoint_7650-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7650-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
