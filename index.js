...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_925=0;yPoint_925<this.gameFieldBorderY.length; yPoint_925++){
                for(var xPoint_925=0;xPoint_925<this.gameFieldBorderX.length; xPoint_925++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_925, xPoint_925);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_925][xPoint_925] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_925, xPoint_925, 'user');
                    if(this._userShipsMap[yPoint_925][xPoint_925] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_925, xPoint_925, type){
            var id = this.getPointBlockIdByCoords(yPoint_925, xPoint_925, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_925);
                block.setAttribute('data-y', yPoint_925);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_925
         * @param {type} xPoint_925
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_925, xPoint_925, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_925 + '_y' + yPoint_925;
            }
            return 'pc_x' + xPoint_925 + '_y' + yPoint_925;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_925=0;yPoint_925<this.gameFieldBorderY.length; yPoint_925++){
                for(var xPoint_925=0;xPoint_925<this.gameFieldBorderX.length; xPoint_925++){
                    map.push({y: yPoint_925, x: xPoint_925});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_925=-1;yPoint_925<(this.gameFieldBorderY.length+1); yPoint_925++){
                for(var xPoint_925=-1;xPoint_925<(this.gameFieldBorderX.length+1); xPoint_925++){
                    if(!map[yPoint_925]){
                        map[yPoint_925] = [];
                    }
                    map[yPoint_925][xPoint_925] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_925 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_925 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_925, yPoint_925) === true){
                    if(this.canPutHorizontal(map, xPoint_925, yPoint_925, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_925][xPoint_925 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_925, yPoint_925, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_925 + i][xPoint_925] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_925, yPoint_925){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_925][xPoint_925] === this.CELL_EMPTY
                && map[yPoint_925-1][xPoint_925] === this.CELL_EMPTY
                && map[yPoint_925-1][xPoint_925+1] === this.CELL_EMPTY
                && map[yPoint_925][xPoint_925+1] === this.CELL_EMPTY
                && map[yPoint_925+1][xPoint_925+1] === this.CELL_EMPTY
                && map[yPoint_925+1][xPoint_925] === this.CELL_EMPTY
                && map[yPoint_925+1][xPoint_925-1] === this.CELL_EMPTY
                && map[yPoint_925][xPoint_925-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_925, yPoint_925, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_925;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_925][x] === this.CELL_EMPTY
                    && map[yPoint_925-1][x] === this.CELL_EMPTY
                    && map[yPoint_925-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_925][x+1] === this.CELL_EMPTY
                    && map[yPoint_925+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_925+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_925, yPoint_925, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_925;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_925] === this.CELL_EMPTY
                    && map[y+1][xPoint_925] === this.CELL_EMPTY
                    && map[y+1][xPoint_925+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_925] === this.CELL_EMPTY
                    && map[y][xPoint_925-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_925-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
