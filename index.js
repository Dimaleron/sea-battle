...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7321=0;yPoint_7321<this.gameFieldBorderY.length; yPoint_7321++){
                for(var xPoint_7321=0;xPoint_7321<this.gameFieldBorderX.length; xPoint_7321++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7321, xPoint_7321);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7321][xPoint_7321] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7321, xPoint_7321, 'user');
                    if(this._userShipsMap[yPoint_7321][xPoint_7321] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7321, xPoint_7321, type){
            var id = this.getPointBlockIdByCoords(yPoint_7321, xPoint_7321, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7321);
                block.setAttribute('data-y', yPoint_7321);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7321
         * @param {type} xPoint_7321
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7321, xPoint_7321, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7321 + '_y' + yPoint_7321;
            }
            return 'pc_x' + xPoint_7321 + '_y' + yPoint_7321;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7321=0;yPoint_7321<this.gameFieldBorderY.length; yPoint_7321++){
                for(var xPoint_7321=0;xPoint_7321<this.gameFieldBorderX.length; xPoint_7321++){
                    map.push({y: yPoint_7321, x: xPoint_7321});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7321=-1;yPoint_7321<(this.gameFieldBorderY.length+1); yPoint_7321++){
                for(var xPoint_7321=-1;xPoint_7321<(this.gameFieldBorderX.length+1); xPoint_7321++){
                    if(!map[yPoint_7321]){
                        map[yPoint_7321] = [];
                    }
                    map[yPoint_7321][xPoint_7321] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7321 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7321 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7321, yPoint_7321) === true){
                    if(this.canPutHorizontal(map, xPoint_7321, yPoint_7321, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7321][xPoint_7321 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7321, yPoint_7321, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7321 + i][xPoint_7321] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7321, yPoint_7321){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7321][xPoint_7321] === this.CELL_EMPTY
                && map[yPoint_7321-1][xPoint_7321] === this.CELL_EMPTY
                && map[yPoint_7321-1][xPoint_7321+1] === this.CELL_EMPTY
                && map[yPoint_7321][xPoint_7321+1] === this.CELL_EMPTY
                && map[yPoint_7321+1][xPoint_7321+1] === this.CELL_EMPTY
                && map[yPoint_7321+1][xPoint_7321] === this.CELL_EMPTY
                && map[yPoint_7321+1][xPoint_7321-1] === this.CELL_EMPTY
                && map[yPoint_7321][xPoint_7321-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7321, yPoint_7321, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7321;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7321][x] === this.CELL_EMPTY
                    && map[yPoint_7321-1][x] === this.CELL_EMPTY
                    && map[yPoint_7321-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7321][x+1] === this.CELL_EMPTY
                    && map[yPoint_7321+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7321+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7321, yPoint_7321, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7321;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7321] === this.CELL_EMPTY
                    && map[y+1][xPoint_7321] === this.CELL_EMPTY
                    && map[y+1][xPoint_7321+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7321] === this.CELL_EMPTY
                    && map[y][xPoint_7321-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7321-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
