...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_588=0;yPoint_588<this.gameFieldBorderY.length; yPoint_588++){
                for(var xPoint_588=0;xPoint_588<this.gameFieldBorderX.length; xPoint_588++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_588, xPoint_588);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_588][xPoint_588] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_588, xPoint_588, 'user');
                    if(this._userShipsMap[yPoint_588][xPoint_588] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_588, xPoint_588, type){
            var id = this.getPointBlockIdByCoords(yPoint_588, xPoint_588, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_588);
                block.setAttribute('data-y', yPoint_588);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_588
         * @param {type} xPoint_588
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_588, xPoint_588, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_588 + '_y' + yPoint_588;
            }
            return 'pc_x' + xPoint_588 + '_y' + yPoint_588;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_588=0;yPoint_588<this.gameFieldBorderY.length; yPoint_588++){
                for(var xPoint_588=0;xPoint_588<this.gameFieldBorderX.length; xPoint_588++){
                    map.push({y: yPoint_588, x: xPoint_588});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_588=-1;yPoint_588<(this.gameFieldBorderY.length+1); yPoint_588++){
                for(var xPoint_588=-1;xPoint_588<(this.gameFieldBorderX.length+1); xPoint_588++){
                    if(!map[yPoint_588]){
                        map[yPoint_588] = [];
                    }
                    map[yPoint_588][xPoint_588] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_588 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_588 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_588, yPoint_588) === true){
                    if(this.canPutHorizontal(map, xPoint_588, yPoint_588, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_588][xPoint_588 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_588, yPoint_588, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_588 + i][xPoint_588] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_588, yPoint_588){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_588][xPoint_588] === this.CELL_EMPTY
                && map[yPoint_588-1][xPoint_588] === this.CELL_EMPTY
                && map[yPoint_588-1][xPoint_588+1] === this.CELL_EMPTY
                && map[yPoint_588][xPoint_588+1] === this.CELL_EMPTY
                && map[yPoint_588+1][xPoint_588+1] === this.CELL_EMPTY
                && map[yPoint_588+1][xPoint_588] === this.CELL_EMPTY
                && map[yPoint_588+1][xPoint_588-1] === this.CELL_EMPTY
                && map[yPoint_588][xPoint_588-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_588, yPoint_588, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_588;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_588][x] === this.CELL_EMPTY
                    && map[yPoint_588-1][x] === this.CELL_EMPTY
                    && map[yPoint_588-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_588][x+1] === this.CELL_EMPTY
                    && map[yPoint_588+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_588+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_588, yPoint_588, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_588;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_588] === this.CELL_EMPTY
                    && map[y+1][xPoint_588] === this.CELL_EMPTY
                    && map[y+1][xPoint_588+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_588] === this.CELL_EMPTY
                    && map[y][xPoint_588-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_588-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
