...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_255=0;yPoint_255<this.gameFieldBorderY.length; yPoint_255++){
                for(var xPoint_255=0;xPoint_255<this.gameFieldBorderX.length; xPoint_255++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_255, xPoint_255);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_255][xPoint_255] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_255, xPoint_255, 'user');
                    if(this._userShipsMap[yPoint_255][xPoint_255] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_255, xPoint_255, type){
            var id = this.getPointBlockIdByCoords(yPoint_255, xPoint_255, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_255);
                block.setAttribute('data-y', yPoint_255);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_255
         * @param {type} xPoint_255
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_255, xPoint_255, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_255 + '_y' + yPoint_255;
            }
            return 'pc_x' + xPoint_255 + '_y' + yPoint_255;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_255=0;yPoint_255<this.gameFieldBorderY.length; yPoint_255++){
                for(var xPoint_255=0;xPoint_255<this.gameFieldBorderX.length; xPoint_255++){
                    map.push({y: yPoint_255, x: xPoint_255});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_255=-1;yPoint_255<(this.gameFieldBorderY.length+1); yPoint_255++){
                for(var xPoint_255=-1;xPoint_255<(this.gameFieldBorderX.length+1); xPoint_255++){
                    if(!map[yPoint_255]){
                        map[yPoint_255] = [];
                    }
                    map[yPoint_255][xPoint_255] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_255 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_255 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_255, yPoint_255) === true){
                    if(this.canPutHorizontal(map, xPoint_255, yPoint_255, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_255][xPoint_255 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_255, yPoint_255, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_255 + i][xPoint_255] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_255, yPoint_255){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_255][xPoint_255] === this.CELL_EMPTY
                && map[yPoint_255-1][xPoint_255] === this.CELL_EMPTY
                && map[yPoint_255-1][xPoint_255+1] === this.CELL_EMPTY
                && map[yPoint_255][xPoint_255+1] === this.CELL_EMPTY
                && map[yPoint_255+1][xPoint_255+1] === this.CELL_EMPTY
                && map[yPoint_255+1][xPoint_255] === this.CELL_EMPTY
                && map[yPoint_255+1][xPoint_255-1] === this.CELL_EMPTY
                && map[yPoint_255][xPoint_255-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_255, yPoint_255, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_255;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_255][x] === this.CELL_EMPTY
                    && map[yPoint_255-1][x] === this.CELL_EMPTY
                    && map[yPoint_255-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_255][x+1] === this.CELL_EMPTY
                    && map[yPoint_255+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_255+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_255, yPoint_255, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_255;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_255] === this.CELL_EMPTY
                    && map[y+1][xPoint_255] === this.CELL_EMPTY
                    && map[y+1][xPoint_255+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_255] === this.CELL_EMPTY
                    && map[y][xPoint_255-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_255-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
