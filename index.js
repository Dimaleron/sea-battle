...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28439=0;yPoint_28439<this.gameFieldBorderY.length; yPoint_28439++){
                for(var xPoint_28439=0;xPoint_28439<this.gameFieldBorderX.length; xPoint_28439++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28439, xPoint_28439);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28439][xPoint_28439] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28439, xPoint_28439, 'user');
                    if(this._userShipsMap[yPoint_28439][xPoint_28439] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28439, xPoint_28439, type){
            var id = this.getPointBlockIdByCoords(yPoint_28439, xPoint_28439, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28439);
                block.setAttribute('data-y', yPoint_28439);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28439
         * @param {type} xPoint_28439
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28439, xPoint_28439, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28439 + '_y' + yPoint_28439;
            }
            return 'pc_x' + xPoint_28439 + '_y' + yPoint_28439;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28439=0;yPoint_28439<this.gameFieldBorderY.length; yPoint_28439++){
                for(var xPoint_28439=0;xPoint_28439<this.gameFieldBorderX.length; xPoint_28439++){
                    map.push({y: yPoint_28439, x: xPoint_28439});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28439=-1;yPoint_28439<(this.gameFieldBorderY.length+1); yPoint_28439++){
                for(var xPoint_28439=-1;xPoint_28439<(this.gameFieldBorderX.length+1); xPoint_28439++){
                    if(!map[yPoint_28439]){
                        map[yPoint_28439] = [];
                    }
                    map[yPoint_28439][xPoint_28439] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28439 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28439 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28439, yPoint_28439) === true){
                    if(this.canPutHorizontal(map, xPoint_28439, yPoint_28439, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28439][xPoint_28439 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28439, yPoint_28439, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28439 + i][xPoint_28439] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28439, yPoint_28439){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28439][xPoint_28439] === this.CELL_EMPTY
                && map[yPoint_28439-1][xPoint_28439] === this.CELL_EMPTY
                && map[yPoint_28439-1][xPoint_28439+1] === this.CELL_EMPTY
                && map[yPoint_28439][xPoint_28439+1] === this.CELL_EMPTY
                && map[yPoint_28439+1][xPoint_28439+1] === this.CELL_EMPTY
                && map[yPoint_28439+1][xPoint_28439] === this.CELL_EMPTY
                && map[yPoint_28439+1][xPoint_28439-1] === this.CELL_EMPTY
                && map[yPoint_28439][xPoint_28439-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28439, yPoint_28439, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28439;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28439][x] === this.CELL_EMPTY
                    && map[yPoint_28439-1][x] === this.CELL_EMPTY
                    && map[yPoint_28439-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28439][x+1] === this.CELL_EMPTY
                    && map[yPoint_28439+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28439+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28439, yPoint_28439, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28439;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28439] === this.CELL_EMPTY
                    && map[y+1][xPoint_28439] === this.CELL_EMPTY
                    && map[y+1][xPoint_28439+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28439] === this.CELL_EMPTY
                    && map[y][xPoint_28439-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28439-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
