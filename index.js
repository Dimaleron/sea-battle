...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6447=0;yPoint_6447<this.gameFieldBorderY.length; yPoint_6447++){
                for(var xPoint_6447=0;xPoint_6447<this.gameFieldBorderX.length; xPoint_6447++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6447, xPoint_6447);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6447][xPoint_6447] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6447, xPoint_6447, 'user');
                    if(this._userShipsMap[yPoint_6447][xPoint_6447] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6447, xPoint_6447, type){
            var id = this.getPointBlockIdByCoords(yPoint_6447, xPoint_6447, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6447);
                block.setAttribute('data-y', yPoint_6447);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6447
         * @param {type} xPoint_6447
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6447, xPoint_6447, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6447 + '_y' + yPoint_6447;
            }
            return 'pc_x' + xPoint_6447 + '_y' + yPoint_6447;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6447=0;yPoint_6447<this.gameFieldBorderY.length; yPoint_6447++){
                for(var xPoint_6447=0;xPoint_6447<this.gameFieldBorderX.length; xPoint_6447++){
                    map.push({y: yPoint_6447, x: xPoint_6447});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6447=-1;yPoint_6447<(this.gameFieldBorderY.length+1); yPoint_6447++){
                for(var xPoint_6447=-1;xPoint_6447<(this.gameFieldBorderX.length+1); xPoint_6447++){
                    if(!map[yPoint_6447]){
                        map[yPoint_6447] = [];
                    }
                    map[yPoint_6447][xPoint_6447] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6447 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6447 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6447, yPoint_6447) === true){
                    if(this.canPutHorizontal(map, xPoint_6447, yPoint_6447, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6447][xPoint_6447 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6447, yPoint_6447, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6447 + i][xPoint_6447] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6447, yPoint_6447){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6447][xPoint_6447] === this.CELL_EMPTY
                && map[yPoint_6447-1][xPoint_6447] === this.CELL_EMPTY
                && map[yPoint_6447-1][xPoint_6447+1] === this.CELL_EMPTY
                && map[yPoint_6447][xPoint_6447+1] === this.CELL_EMPTY
                && map[yPoint_6447+1][xPoint_6447+1] === this.CELL_EMPTY
                && map[yPoint_6447+1][xPoint_6447] === this.CELL_EMPTY
                && map[yPoint_6447+1][xPoint_6447-1] === this.CELL_EMPTY
                && map[yPoint_6447][xPoint_6447-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6447, yPoint_6447, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6447;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6447][x] === this.CELL_EMPTY
                    && map[yPoint_6447-1][x] === this.CELL_EMPTY
                    && map[yPoint_6447-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6447][x+1] === this.CELL_EMPTY
                    && map[yPoint_6447+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6447+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6447, yPoint_6447, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6447;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6447] === this.CELL_EMPTY
                    && map[y+1][xPoint_6447] === this.CELL_EMPTY
                    && map[y+1][xPoint_6447+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6447] === this.CELL_EMPTY
                    && map[y][xPoint_6447-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6447-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
