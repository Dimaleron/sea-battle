...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6875=0;yPoint_6875<this.gameFieldBorderY.length; yPoint_6875++){
                for(var xPoint_6875=0;xPoint_6875<this.gameFieldBorderX.length; xPoint_6875++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6875, xPoint_6875);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6875][xPoint_6875] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6875, xPoint_6875, 'user');
                    if(this._userShipsMap[yPoint_6875][xPoint_6875] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6875, xPoint_6875, type){
            var id = this.getPointBlockIdByCoords(yPoint_6875, xPoint_6875, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6875);
                block.setAttribute('data-y', yPoint_6875);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6875
         * @param {type} xPoint_6875
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6875, xPoint_6875, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6875 + '_y' + yPoint_6875;
            }
            return 'pc_x' + xPoint_6875 + '_y' + yPoint_6875;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6875=0;yPoint_6875<this.gameFieldBorderY.length; yPoint_6875++){
                for(var xPoint_6875=0;xPoint_6875<this.gameFieldBorderX.length; xPoint_6875++){
                    map.push({y: yPoint_6875, x: xPoint_6875});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6875=-1;yPoint_6875<(this.gameFieldBorderY.length+1); yPoint_6875++){
                for(var xPoint_6875=-1;xPoint_6875<(this.gameFieldBorderX.length+1); xPoint_6875++){
                    if(!map[yPoint_6875]){
                        map[yPoint_6875] = [];
                    }
                    map[yPoint_6875][xPoint_6875] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6875 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6875 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6875, yPoint_6875) === true){
                    if(this.canPutHorizontal(map, xPoint_6875, yPoint_6875, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6875][xPoint_6875 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6875, yPoint_6875, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6875 + i][xPoint_6875] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6875, yPoint_6875){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6875][xPoint_6875] === this.CELL_EMPTY
                && map[yPoint_6875-1][xPoint_6875] === this.CELL_EMPTY
                && map[yPoint_6875-1][xPoint_6875+1] === this.CELL_EMPTY
                && map[yPoint_6875][xPoint_6875+1] === this.CELL_EMPTY
                && map[yPoint_6875+1][xPoint_6875+1] === this.CELL_EMPTY
                && map[yPoint_6875+1][xPoint_6875] === this.CELL_EMPTY
                && map[yPoint_6875+1][xPoint_6875-1] === this.CELL_EMPTY
                && map[yPoint_6875][xPoint_6875-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6875, yPoint_6875, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6875;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6875][x] === this.CELL_EMPTY
                    && map[yPoint_6875-1][x] === this.CELL_EMPTY
                    && map[yPoint_6875-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6875][x+1] === this.CELL_EMPTY
                    && map[yPoint_6875+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6875+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6875, yPoint_6875, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6875;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6875] === this.CELL_EMPTY
                    && map[y+1][xPoint_6875] === this.CELL_EMPTY
                    && map[y+1][xPoint_6875+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6875] === this.CELL_EMPTY
                    && map[y][xPoint_6875-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6875-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
