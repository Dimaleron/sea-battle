...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3210=0;yPoint_3210<this.gameFieldBorderY.length; yPoint_3210++){
                for(var xPoint_3210=0;xPoint_3210<this.gameFieldBorderX.length; xPoint_3210++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3210, xPoint_3210);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3210][xPoint_3210] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3210, xPoint_3210, 'user');
                    if(this._userShipsMap[yPoint_3210][xPoint_3210] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3210, xPoint_3210, type){
            var id = this.getPointBlockIdByCoords(yPoint_3210, xPoint_3210, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3210);
                block.setAttribute('data-y', yPoint_3210);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3210
         * @param {type} xPoint_3210
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3210, xPoint_3210, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3210 + '_y' + yPoint_3210;
            }
            return 'pc_x' + xPoint_3210 + '_y' + yPoint_3210;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3210=0;yPoint_3210<this.gameFieldBorderY.length; yPoint_3210++){
                for(var xPoint_3210=0;xPoint_3210<this.gameFieldBorderX.length; xPoint_3210++){
                    map.push({y: yPoint_3210, x: xPoint_3210});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3210=-1;yPoint_3210<(this.gameFieldBorderY.length+1); yPoint_3210++){
                for(var xPoint_3210=-1;xPoint_3210<(this.gameFieldBorderX.length+1); xPoint_3210++){
                    if(!map[yPoint_3210]){
                        map[yPoint_3210] = [];
                    }
                    map[yPoint_3210][xPoint_3210] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3210 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3210 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3210, yPoint_3210) === true){
                    if(this.canPutHorizontal(map, xPoint_3210, yPoint_3210, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3210][xPoint_3210 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3210, yPoint_3210, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3210 + i][xPoint_3210] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3210, yPoint_3210){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3210][xPoint_3210] === this.CELL_EMPTY
                && map[yPoint_3210-1][xPoint_3210] === this.CELL_EMPTY
                && map[yPoint_3210-1][xPoint_3210+1] === this.CELL_EMPTY
                && map[yPoint_3210][xPoint_3210+1] === this.CELL_EMPTY
                && map[yPoint_3210+1][xPoint_3210+1] === this.CELL_EMPTY
                && map[yPoint_3210+1][xPoint_3210] === this.CELL_EMPTY
                && map[yPoint_3210+1][xPoint_3210-1] === this.CELL_EMPTY
                && map[yPoint_3210][xPoint_3210-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3210, yPoint_3210, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3210;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3210][x] === this.CELL_EMPTY
                    && map[yPoint_3210-1][x] === this.CELL_EMPTY
                    && map[yPoint_3210-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3210][x+1] === this.CELL_EMPTY
                    && map[yPoint_3210+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3210+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3210, yPoint_3210, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3210;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3210] === this.CELL_EMPTY
                    && map[y+1][xPoint_3210] === this.CELL_EMPTY
                    && map[y+1][xPoint_3210+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3210] === this.CELL_EMPTY
                    && map[y][xPoint_3210-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3210-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
