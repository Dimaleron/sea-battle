...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_887=0;yPoint_887<this.gameFieldBorderY.length; yPoint_887++){
                for(var xPoint_887=0;xPoint_887<this.gameFieldBorderX.length; xPoint_887++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_887, xPoint_887);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_887][xPoint_887] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_887, xPoint_887, 'user');
                    if(this._userShipsMap[yPoint_887][xPoint_887] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_887, xPoint_887, type){
            var id = this.getPointBlockIdByCoords(yPoint_887, xPoint_887, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_887);
                block.setAttribute('data-y', yPoint_887);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_887
         * @param {type} xPoint_887
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_887, xPoint_887, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_887 + '_y' + yPoint_887;
            }
            return 'pc_x' + xPoint_887 + '_y' + yPoint_887;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_887=0;yPoint_887<this.gameFieldBorderY.length; yPoint_887++){
                for(var xPoint_887=0;xPoint_887<this.gameFieldBorderX.length; xPoint_887++){
                    map.push({y: yPoint_887, x: xPoint_887});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_887=-1;yPoint_887<(this.gameFieldBorderY.length+1); yPoint_887++){
                for(var xPoint_887=-1;xPoint_887<(this.gameFieldBorderX.length+1); xPoint_887++){
                    if(!map[yPoint_887]){
                        map[yPoint_887] = [];
                    }
                    map[yPoint_887][xPoint_887] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_887 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_887 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_887, yPoint_887) === true){
                    if(this.canPutHorizontal(map, xPoint_887, yPoint_887, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_887][xPoint_887 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_887, yPoint_887, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_887 + i][xPoint_887] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_887, yPoint_887){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_887][xPoint_887] === this.CELL_EMPTY
                && map[yPoint_887-1][xPoint_887] === this.CELL_EMPTY
                && map[yPoint_887-1][xPoint_887+1] === this.CELL_EMPTY
                && map[yPoint_887][xPoint_887+1] === this.CELL_EMPTY
                && map[yPoint_887+1][xPoint_887+1] === this.CELL_EMPTY
                && map[yPoint_887+1][xPoint_887] === this.CELL_EMPTY
                && map[yPoint_887+1][xPoint_887-1] === this.CELL_EMPTY
                && map[yPoint_887][xPoint_887-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_887, yPoint_887, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_887;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_887][x] === this.CELL_EMPTY
                    && map[yPoint_887-1][x] === this.CELL_EMPTY
                    && map[yPoint_887-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_887][x+1] === this.CELL_EMPTY
                    && map[yPoint_887+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_887+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_887, yPoint_887, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_887;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_887] === this.CELL_EMPTY
                    && map[y+1][xPoint_887] === this.CELL_EMPTY
                    && map[y+1][xPoint_887+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_887] === this.CELL_EMPTY
                    && map[y][xPoint_887-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_887-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
