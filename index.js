...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25079=0;yPoint_25079<this.gameFieldBorderY.length; yPoint_25079++){
                for(var xPoint_25079=0;xPoint_25079<this.gameFieldBorderX.length; xPoint_25079++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25079, xPoint_25079);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25079][xPoint_25079] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25079, xPoint_25079, 'user');
                    if(this._userShipsMap[yPoint_25079][xPoint_25079] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25079, xPoint_25079, type){
            var id = this.getPointBlockIdByCoords(yPoint_25079, xPoint_25079, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25079);
                block.setAttribute('data-y', yPoint_25079);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25079
         * @param {type} xPoint_25079
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25079, xPoint_25079, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25079 + '_y' + yPoint_25079;
            }
            return 'pc_x' + xPoint_25079 + '_y' + yPoint_25079;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25079=0;yPoint_25079<this.gameFieldBorderY.length; yPoint_25079++){
                for(var xPoint_25079=0;xPoint_25079<this.gameFieldBorderX.length; xPoint_25079++){
                    map.push({y: yPoint_25079, x: xPoint_25079});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25079=-1;yPoint_25079<(this.gameFieldBorderY.length+1); yPoint_25079++){
                for(var xPoint_25079=-1;xPoint_25079<(this.gameFieldBorderX.length+1); xPoint_25079++){
                    if(!map[yPoint_25079]){
                        map[yPoint_25079] = [];
                    }
                    map[yPoint_25079][xPoint_25079] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25079 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25079 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25079, yPoint_25079) === true){
                    if(this.canPutHorizontal(map, xPoint_25079, yPoint_25079, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25079][xPoint_25079 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25079, yPoint_25079, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25079 + i][xPoint_25079] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25079, yPoint_25079){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25079][xPoint_25079] === this.CELL_EMPTY
                && map[yPoint_25079-1][xPoint_25079] === this.CELL_EMPTY
                && map[yPoint_25079-1][xPoint_25079+1] === this.CELL_EMPTY
                && map[yPoint_25079][xPoint_25079+1] === this.CELL_EMPTY
                && map[yPoint_25079+1][xPoint_25079+1] === this.CELL_EMPTY
                && map[yPoint_25079+1][xPoint_25079] === this.CELL_EMPTY
                && map[yPoint_25079+1][xPoint_25079-1] === this.CELL_EMPTY
                && map[yPoint_25079][xPoint_25079-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25079, yPoint_25079, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25079;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25079][x] === this.CELL_EMPTY
                    && map[yPoint_25079-1][x] === this.CELL_EMPTY
                    && map[yPoint_25079-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25079][x+1] === this.CELL_EMPTY
                    && map[yPoint_25079+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25079+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25079, yPoint_25079, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25079;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25079] === this.CELL_EMPTY
                    && map[y+1][xPoint_25079] === this.CELL_EMPTY
                    && map[y+1][xPoint_25079+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25079] === this.CELL_EMPTY
                    && map[y][xPoint_25079-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25079-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
