...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18336=0;yPoint_18336<this.gameFieldBorderY.length; yPoint_18336++){
                for(var xPoint_18336=0;xPoint_18336<this.gameFieldBorderX.length; xPoint_18336++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18336, xPoint_18336);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18336][xPoint_18336] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18336, xPoint_18336, 'user');
                    if(this._userShipsMap[yPoint_18336][xPoint_18336] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18336, xPoint_18336, type){
            var id = this.getPointBlockIdByCoords(yPoint_18336, xPoint_18336, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18336);
                block.setAttribute('data-y', yPoint_18336);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18336
         * @param {type} xPoint_18336
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18336, xPoint_18336, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18336 + '_y' + yPoint_18336;
            }
            return 'pc_x' + xPoint_18336 + '_y' + yPoint_18336;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18336=0;yPoint_18336<this.gameFieldBorderY.length; yPoint_18336++){
                for(var xPoint_18336=0;xPoint_18336<this.gameFieldBorderX.length; xPoint_18336++){
                    map.push({y: yPoint_18336, x: xPoint_18336});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18336=-1;yPoint_18336<(this.gameFieldBorderY.length+1); yPoint_18336++){
                for(var xPoint_18336=-1;xPoint_18336<(this.gameFieldBorderX.length+1); xPoint_18336++){
                    if(!map[yPoint_18336]){
                        map[yPoint_18336] = [];
                    }
                    map[yPoint_18336][xPoint_18336] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18336 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18336 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18336, yPoint_18336) === true){
                    if(this.canPutHorizontal(map, xPoint_18336, yPoint_18336, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18336][xPoint_18336 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18336, yPoint_18336, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18336 + i][xPoint_18336] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18336, yPoint_18336){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18336][xPoint_18336] === this.CELL_EMPTY
                && map[yPoint_18336-1][xPoint_18336] === this.CELL_EMPTY
                && map[yPoint_18336-1][xPoint_18336+1] === this.CELL_EMPTY
                && map[yPoint_18336][xPoint_18336+1] === this.CELL_EMPTY
                && map[yPoint_18336+1][xPoint_18336+1] === this.CELL_EMPTY
                && map[yPoint_18336+1][xPoint_18336] === this.CELL_EMPTY
                && map[yPoint_18336+1][xPoint_18336-1] === this.CELL_EMPTY
                && map[yPoint_18336][xPoint_18336-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18336, yPoint_18336, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18336;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18336][x] === this.CELL_EMPTY
                    && map[yPoint_18336-1][x] === this.CELL_EMPTY
                    && map[yPoint_18336-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18336][x+1] === this.CELL_EMPTY
                    && map[yPoint_18336+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18336+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18336, yPoint_18336, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18336;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18336] === this.CELL_EMPTY
                    && map[y+1][xPoint_18336] === this.CELL_EMPTY
                    && map[y+1][xPoint_18336+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18336] === this.CELL_EMPTY
                    && map[y][xPoint_18336-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18336-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
