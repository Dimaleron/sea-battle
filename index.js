...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_392=0;yPoint_392<this.gameFieldBorderY.length; yPoint_392++){
                for(var xPoint_392=0;xPoint_392<this.gameFieldBorderX.length; xPoint_392++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_392, xPoint_392);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_392][xPoint_392] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_392, xPoint_392, 'user');
                    if(this._userShipsMap[yPoint_392][xPoint_392] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_392, xPoint_392, type){
            var id = this.getPointBlockIdByCoords(yPoint_392, xPoint_392, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_392);
                block.setAttribute('data-y', yPoint_392);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_392
         * @param {type} xPoint_392
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_392, xPoint_392, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_392 + '_y' + yPoint_392;
            }
            return 'pc_x' + xPoint_392 + '_y' + yPoint_392;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_392=0;yPoint_392<this.gameFieldBorderY.length; yPoint_392++){
                for(var xPoint_392=0;xPoint_392<this.gameFieldBorderX.length; xPoint_392++){
                    map.push({y: yPoint_392, x: xPoint_392});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_392=-1;yPoint_392<(this.gameFieldBorderY.length+1); yPoint_392++){
                for(var xPoint_392=-1;xPoint_392<(this.gameFieldBorderX.length+1); xPoint_392++){
                    if(!map[yPoint_392]){
                        map[yPoint_392] = [];
                    }
                    map[yPoint_392][xPoint_392] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_392 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_392 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_392, yPoint_392) === true){
                    if(this.canPutHorizontal(map, xPoint_392, yPoint_392, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_392][xPoint_392 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_392, yPoint_392, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_392 + i][xPoint_392] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_392, yPoint_392){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_392][xPoint_392] === this.CELL_EMPTY
                && map[yPoint_392-1][xPoint_392] === this.CELL_EMPTY
                && map[yPoint_392-1][xPoint_392+1] === this.CELL_EMPTY
                && map[yPoint_392][xPoint_392+1] === this.CELL_EMPTY
                && map[yPoint_392+1][xPoint_392+1] === this.CELL_EMPTY
                && map[yPoint_392+1][xPoint_392] === this.CELL_EMPTY
                && map[yPoint_392+1][xPoint_392-1] === this.CELL_EMPTY
                && map[yPoint_392][xPoint_392-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_392, yPoint_392, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_392;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_392][x] === this.CELL_EMPTY
                    && map[yPoint_392-1][x] === this.CELL_EMPTY
                    && map[yPoint_392-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_392][x+1] === this.CELL_EMPTY
                    && map[yPoint_392+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_392+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_392, yPoint_392, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_392;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_392] === this.CELL_EMPTY
                    && map[y+1][xPoint_392] === this.CELL_EMPTY
                    && map[y+1][xPoint_392+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_392] === this.CELL_EMPTY
                    && map[y][xPoint_392-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_392-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
