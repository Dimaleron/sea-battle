...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7546=0;yPoint_7546<this.gameFieldBorderY.length; yPoint_7546++){
                for(var xPoint_7546=0;xPoint_7546<this.gameFieldBorderX.length; xPoint_7546++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7546, xPoint_7546);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7546][xPoint_7546] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7546, xPoint_7546, 'user');
                    if(this._userShipsMap[yPoint_7546][xPoint_7546] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7546, xPoint_7546, type){
            var id = this.getPointBlockIdByCoords(yPoint_7546, xPoint_7546, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7546);
                block.setAttribute('data-y', yPoint_7546);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7546
         * @param {type} xPoint_7546
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7546, xPoint_7546, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7546 + '_y' + yPoint_7546;
            }
            return 'pc_x' + xPoint_7546 + '_y' + yPoint_7546;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7546=0;yPoint_7546<this.gameFieldBorderY.length; yPoint_7546++){
                for(var xPoint_7546=0;xPoint_7546<this.gameFieldBorderX.length; xPoint_7546++){
                    map.push({y: yPoint_7546, x: xPoint_7546});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7546=-1;yPoint_7546<(this.gameFieldBorderY.length+1); yPoint_7546++){
                for(var xPoint_7546=-1;xPoint_7546<(this.gameFieldBorderX.length+1); xPoint_7546++){
                    if(!map[yPoint_7546]){
                        map[yPoint_7546] = [];
                    }
                    map[yPoint_7546][xPoint_7546] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7546 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7546 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7546, yPoint_7546) === true){
                    if(this.canPutHorizontal(map, xPoint_7546, yPoint_7546, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7546][xPoint_7546 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7546, yPoint_7546, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7546 + i][xPoint_7546] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7546, yPoint_7546){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7546][xPoint_7546] === this.CELL_EMPTY
                && map[yPoint_7546-1][xPoint_7546] === this.CELL_EMPTY
                && map[yPoint_7546-1][xPoint_7546+1] === this.CELL_EMPTY
                && map[yPoint_7546][xPoint_7546+1] === this.CELL_EMPTY
                && map[yPoint_7546+1][xPoint_7546+1] === this.CELL_EMPTY
                && map[yPoint_7546+1][xPoint_7546] === this.CELL_EMPTY
                && map[yPoint_7546+1][xPoint_7546-1] === this.CELL_EMPTY
                && map[yPoint_7546][xPoint_7546-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7546, yPoint_7546, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7546;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7546][x] === this.CELL_EMPTY
                    && map[yPoint_7546-1][x] === this.CELL_EMPTY
                    && map[yPoint_7546-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7546][x+1] === this.CELL_EMPTY
                    && map[yPoint_7546+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7546+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7546, yPoint_7546, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7546;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7546] === this.CELL_EMPTY
                    && map[y+1][xPoint_7546] === this.CELL_EMPTY
                    && map[y+1][xPoint_7546+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7546] === this.CELL_EMPTY
                    && map[y][xPoint_7546-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7546-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
