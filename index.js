...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9123=0;yPoint_9123<this.gameFieldBorderY.length; yPoint_9123++){
                for(var xPoint_9123=0;xPoint_9123<this.gameFieldBorderX.length; xPoint_9123++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9123, xPoint_9123);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9123][xPoint_9123] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9123, xPoint_9123, 'user');
                    if(this._userShipsMap[yPoint_9123][xPoint_9123] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9123, xPoint_9123, type){
            var id = this.getPointBlockIdByCoords(yPoint_9123, xPoint_9123, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9123);
                block.setAttribute('data-y', yPoint_9123);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9123
         * @param {type} xPoint_9123
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9123, xPoint_9123, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9123 + '_y' + yPoint_9123;
            }
            return 'pc_x' + xPoint_9123 + '_y' + yPoint_9123;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9123=0;yPoint_9123<this.gameFieldBorderY.length; yPoint_9123++){
                for(var xPoint_9123=0;xPoint_9123<this.gameFieldBorderX.length; xPoint_9123++){
                    map.push({y: yPoint_9123, x: xPoint_9123});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9123=-1;yPoint_9123<(this.gameFieldBorderY.length+1); yPoint_9123++){
                for(var xPoint_9123=-1;xPoint_9123<(this.gameFieldBorderX.length+1); xPoint_9123++){
                    if(!map[yPoint_9123]){
                        map[yPoint_9123] = [];
                    }
                    map[yPoint_9123][xPoint_9123] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9123 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9123 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9123, yPoint_9123) === true){
                    if(this.canPutHorizontal(map, xPoint_9123, yPoint_9123, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9123][xPoint_9123 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9123, yPoint_9123, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9123 + i][xPoint_9123] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9123, yPoint_9123){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9123][xPoint_9123] === this.CELL_EMPTY
                && map[yPoint_9123-1][xPoint_9123] === this.CELL_EMPTY
                && map[yPoint_9123-1][xPoint_9123+1] === this.CELL_EMPTY
                && map[yPoint_9123][xPoint_9123+1] === this.CELL_EMPTY
                && map[yPoint_9123+1][xPoint_9123+1] === this.CELL_EMPTY
                && map[yPoint_9123+1][xPoint_9123] === this.CELL_EMPTY
                && map[yPoint_9123+1][xPoint_9123-1] === this.CELL_EMPTY
                && map[yPoint_9123][xPoint_9123-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9123, yPoint_9123, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9123;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9123][x] === this.CELL_EMPTY
                    && map[yPoint_9123-1][x] === this.CELL_EMPTY
                    && map[yPoint_9123-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9123][x+1] === this.CELL_EMPTY
                    && map[yPoint_9123+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9123+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9123, yPoint_9123, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9123;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9123] === this.CELL_EMPTY
                    && map[y+1][xPoint_9123] === this.CELL_EMPTY
                    && map[y+1][xPoint_9123+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9123] === this.CELL_EMPTY
                    && map[y][xPoint_9123-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9123-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
