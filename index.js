...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2166=0;yPoint_2166<this.gameFieldBorderY.length; yPoint_2166++){
                for(var xPoint_2166=0;xPoint_2166<this.gameFieldBorderX.length; xPoint_2166++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2166, xPoint_2166);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2166][xPoint_2166] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2166, xPoint_2166, 'user');
                    if(this._userShipsMap[yPoint_2166][xPoint_2166] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2166, xPoint_2166, type){
            var id = this.getPointBlockIdByCoords(yPoint_2166, xPoint_2166, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2166);
                block.setAttribute('data-y', yPoint_2166);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2166
         * @param {type} xPoint_2166
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2166, xPoint_2166, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2166 + '_y' + yPoint_2166;
            }
            return 'pc_x' + xPoint_2166 + '_y' + yPoint_2166;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2166=0;yPoint_2166<this.gameFieldBorderY.length; yPoint_2166++){
                for(var xPoint_2166=0;xPoint_2166<this.gameFieldBorderX.length; xPoint_2166++){
                    map.push({y: yPoint_2166, x: xPoint_2166});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2166=-1;yPoint_2166<(this.gameFieldBorderY.length+1); yPoint_2166++){
                for(var xPoint_2166=-1;xPoint_2166<(this.gameFieldBorderX.length+1); xPoint_2166++){
                    if(!map[yPoint_2166]){
                        map[yPoint_2166] = [];
                    }
                    map[yPoint_2166][xPoint_2166] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2166 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2166 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2166, yPoint_2166) === true){
                    if(this.canPutHorizontal(map, xPoint_2166, yPoint_2166, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2166][xPoint_2166 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2166, yPoint_2166, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2166 + i][xPoint_2166] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2166, yPoint_2166){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2166][xPoint_2166] === this.CELL_EMPTY
                && map[yPoint_2166-1][xPoint_2166] === this.CELL_EMPTY
                && map[yPoint_2166-1][xPoint_2166+1] === this.CELL_EMPTY
                && map[yPoint_2166][xPoint_2166+1] === this.CELL_EMPTY
                && map[yPoint_2166+1][xPoint_2166+1] === this.CELL_EMPTY
                && map[yPoint_2166+1][xPoint_2166] === this.CELL_EMPTY
                && map[yPoint_2166+1][xPoint_2166-1] === this.CELL_EMPTY
                && map[yPoint_2166][xPoint_2166-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2166, yPoint_2166, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2166;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2166][x] === this.CELL_EMPTY
                    && map[yPoint_2166-1][x] === this.CELL_EMPTY
                    && map[yPoint_2166-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2166][x+1] === this.CELL_EMPTY
                    && map[yPoint_2166+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2166+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2166, yPoint_2166, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2166;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2166] === this.CELL_EMPTY
                    && map[y+1][xPoint_2166] === this.CELL_EMPTY
                    && map[y+1][xPoint_2166+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2166] === this.CELL_EMPTY
                    && map[y][xPoint_2166-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2166-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
