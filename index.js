...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11678=0;yPoint_11678<this.gameFieldBorderY.length; yPoint_11678++){
                for(var xPoint_11678=0;xPoint_11678<this.gameFieldBorderX.length; xPoint_11678++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11678, xPoint_11678);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11678][xPoint_11678] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11678, xPoint_11678, 'user');
                    if(this._userShipsMap[yPoint_11678][xPoint_11678] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11678, xPoint_11678, type){
            var id = this.getPointBlockIdByCoords(yPoint_11678, xPoint_11678, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11678);
                block.setAttribute('data-y', yPoint_11678);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11678
         * @param {type} xPoint_11678
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11678, xPoint_11678, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11678 + '_y' + yPoint_11678;
            }
            return 'pc_x' + xPoint_11678 + '_y' + yPoint_11678;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11678=0;yPoint_11678<this.gameFieldBorderY.length; yPoint_11678++){
                for(var xPoint_11678=0;xPoint_11678<this.gameFieldBorderX.length; xPoint_11678++){
                    map.push({y: yPoint_11678, x: xPoint_11678});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11678=-1;yPoint_11678<(this.gameFieldBorderY.length+1); yPoint_11678++){
                for(var xPoint_11678=-1;xPoint_11678<(this.gameFieldBorderX.length+1); xPoint_11678++){
                    if(!map[yPoint_11678]){
                        map[yPoint_11678] = [];
                    }
                    map[yPoint_11678][xPoint_11678] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11678 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11678 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11678, yPoint_11678) === true){
                    if(this.canPutHorizontal(map, xPoint_11678, yPoint_11678, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11678][xPoint_11678 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11678, yPoint_11678, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11678 + i][xPoint_11678] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11678, yPoint_11678){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11678][xPoint_11678] === this.CELL_EMPTY
                && map[yPoint_11678-1][xPoint_11678] === this.CELL_EMPTY
                && map[yPoint_11678-1][xPoint_11678+1] === this.CELL_EMPTY
                && map[yPoint_11678][xPoint_11678+1] === this.CELL_EMPTY
                && map[yPoint_11678+1][xPoint_11678+1] === this.CELL_EMPTY
                && map[yPoint_11678+1][xPoint_11678] === this.CELL_EMPTY
                && map[yPoint_11678+1][xPoint_11678-1] === this.CELL_EMPTY
                && map[yPoint_11678][xPoint_11678-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11678, yPoint_11678, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11678;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11678][x] === this.CELL_EMPTY
                    && map[yPoint_11678-1][x] === this.CELL_EMPTY
                    && map[yPoint_11678-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11678][x+1] === this.CELL_EMPTY
                    && map[yPoint_11678+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11678+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11678, yPoint_11678, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11678;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11678] === this.CELL_EMPTY
                    && map[y+1][xPoint_11678] === this.CELL_EMPTY
                    && map[y+1][xPoint_11678+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11678] === this.CELL_EMPTY
                    && map[y][xPoint_11678-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11678-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
