...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24426=0;yPoint_24426<this.gameFieldBorderY.length; yPoint_24426++){
                for(var xPoint_24426=0;xPoint_24426<this.gameFieldBorderX.length; xPoint_24426++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24426, xPoint_24426);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24426][xPoint_24426] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24426, xPoint_24426, 'user');
                    if(this._userShipsMap[yPoint_24426][xPoint_24426] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24426, xPoint_24426, type){
            var id = this.getPointBlockIdByCoords(yPoint_24426, xPoint_24426, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24426);
                block.setAttribute('data-y', yPoint_24426);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24426
         * @param {type} xPoint_24426
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24426, xPoint_24426, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24426 + '_y' + yPoint_24426;
            }
            return 'pc_x' + xPoint_24426 + '_y' + yPoint_24426;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24426=0;yPoint_24426<this.gameFieldBorderY.length; yPoint_24426++){
                for(var xPoint_24426=0;xPoint_24426<this.gameFieldBorderX.length; xPoint_24426++){
                    map.push({y: yPoint_24426, x: xPoint_24426});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24426=-1;yPoint_24426<(this.gameFieldBorderY.length+1); yPoint_24426++){
                for(var xPoint_24426=-1;xPoint_24426<(this.gameFieldBorderX.length+1); xPoint_24426++){
                    if(!map[yPoint_24426]){
                        map[yPoint_24426] = [];
                    }
                    map[yPoint_24426][xPoint_24426] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24426 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24426 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24426, yPoint_24426) === true){
                    if(this.canPutHorizontal(map, xPoint_24426, yPoint_24426, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24426][xPoint_24426 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24426, yPoint_24426, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24426 + i][xPoint_24426] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24426, yPoint_24426){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24426][xPoint_24426] === this.CELL_EMPTY
                && map[yPoint_24426-1][xPoint_24426] === this.CELL_EMPTY
                && map[yPoint_24426-1][xPoint_24426+1] === this.CELL_EMPTY
                && map[yPoint_24426][xPoint_24426+1] === this.CELL_EMPTY
                && map[yPoint_24426+1][xPoint_24426+1] === this.CELL_EMPTY
                && map[yPoint_24426+1][xPoint_24426] === this.CELL_EMPTY
                && map[yPoint_24426+1][xPoint_24426-1] === this.CELL_EMPTY
                && map[yPoint_24426][xPoint_24426-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24426, yPoint_24426, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24426;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24426][x] === this.CELL_EMPTY
                    && map[yPoint_24426-1][x] === this.CELL_EMPTY
                    && map[yPoint_24426-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24426][x+1] === this.CELL_EMPTY
                    && map[yPoint_24426+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24426+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24426, yPoint_24426, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24426;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24426] === this.CELL_EMPTY
                    && map[y+1][xPoint_24426] === this.CELL_EMPTY
                    && map[y+1][xPoint_24426+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24426] === this.CELL_EMPTY
                    && map[y][xPoint_24426-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24426-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
