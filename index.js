...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14387=0;yPoint_14387<this.gameFieldBorderY.length; yPoint_14387++){
                for(var xPoint_14387=0;xPoint_14387<this.gameFieldBorderX.length; xPoint_14387++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14387, xPoint_14387);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14387][xPoint_14387] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14387, xPoint_14387, 'user');
                    if(this._userShipsMap[yPoint_14387][xPoint_14387] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14387, xPoint_14387, type){
            var id = this.getPointBlockIdByCoords(yPoint_14387, xPoint_14387, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14387);
                block.setAttribute('data-y', yPoint_14387);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14387
         * @param {type} xPoint_14387
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14387, xPoint_14387, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14387 + '_y' + yPoint_14387;
            }
            return 'pc_x' + xPoint_14387 + '_y' + yPoint_14387;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14387=0;yPoint_14387<this.gameFieldBorderY.length; yPoint_14387++){
                for(var xPoint_14387=0;xPoint_14387<this.gameFieldBorderX.length; xPoint_14387++){
                    map.push({y: yPoint_14387, x: xPoint_14387});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14387=-1;yPoint_14387<(this.gameFieldBorderY.length+1); yPoint_14387++){
                for(var xPoint_14387=-1;xPoint_14387<(this.gameFieldBorderX.length+1); xPoint_14387++){
                    if(!map[yPoint_14387]){
                        map[yPoint_14387] = [];
                    }
                    map[yPoint_14387][xPoint_14387] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14387 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14387 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14387, yPoint_14387) === true){
                    if(this.canPutHorizontal(map, xPoint_14387, yPoint_14387, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14387][xPoint_14387 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14387, yPoint_14387, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14387 + i][xPoint_14387] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14387, yPoint_14387){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14387][xPoint_14387] === this.CELL_EMPTY
                && map[yPoint_14387-1][xPoint_14387] === this.CELL_EMPTY
                && map[yPoint_14387-1][xPoint_14387+1] === this.CELL_EMPTY
                && map[yPoint_14387][xPoint_14387+1] === this.CELL_EMPTY
                && map[yPoint_14387+1][xPoint_14387+1] === this.CELL_EMPTY
                && map[yPoint_14387+1][xPoint_14387] === this.CELL_EMPTY
                && map[yPoint_14387+1][xPoint_14387-1] === this.CELL_EMPTY
                && map[yPoint_14387][xPoint_14387-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14387, yPoint_14387, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14387;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14387][x] === this.CELL_EMPTY
                    && map[yPoint_14387-1][x] === this.CELL_EMPTY
                    && map[yPoint_14387-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14387][x+1] === this.CELL_EMPTY
                    && map[yPoint_14387+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14387+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14387, yPoint_14387, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14387;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14387] === this.CELL_EMPTY
                    && map[y+1][xPoint_14387] === this.CELL_EMPTY
                    && map[y+1][xPoint_14387+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14387] === this.CELL_EMPTY
                    && map[y][xPoint_14387-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14387-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
