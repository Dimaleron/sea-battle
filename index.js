...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27255=0;yPoint_27255<this.gameFieldBorderY.length; yPoint_27255++){
                for(var xPoint_27255=0;xPoint_27255<this.gameFieldBorderX.length; xPoint_27255++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27255, xPoint_27255);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27255][xPoint_27255] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27255, xPoint_27255, 'user');
                    if(this._userShipsMap[yPoint_27255][xPoint_27255] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27255, xPoint_27255, type){
            var id = this.getPointBlockIdByCoords(yPoint_27255, xPoint_27255, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27255);
                block.setAttribute('data-y', yPoint_27255);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27255
         * @param {type} xPoint_27255
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27255, xPoint_27255, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27255 + '_y' + yPoint_27255;
            }
            return 'pc_x' + xPoint_27255 + '_y' + yPoint_27255;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27255=0;yPoint_27255<this.gameFieldBorderY.length; yPoint_27255++){
                for(var xPoint_27255=0;xPoint_27255<this.gameFieldBorderX.length; xPoint_27255++){
                    map.push({y: yPoint_27255, x: xPoint_27255});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27255=-1;yPoint_27255<(this.gameFieldBorderY.length+1); yPoint_27255++){
                for(var xPoint_27255=-1;xPoint_27255<(this.gameFieldBorderX.length+1); xPoint_27255++){
                    if(!map[yPoint_27255]){
                        map[yPoint_27255] = [];
                    }
                    map[yPoint_27255][xPoint_27255] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27255 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27255 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27255, yPoint_27255) === true){
                    if(this.canPutHorizontal(map, xPoint_27255, yPoint_27255, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27255][xPoint_27255 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27255, yPoint_27255, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27255 + i][xPoint_27255] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27255, yPoint_27255){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27255][xPoint_27255] === this.CELL_EMPTY
                && map[yPoint_27255-1][xPoint_27255] === this.CELL_EMPTY
                && map[yPoint_27255-1][xPoint_27255+1] === this.CELL_EMPTY
                && map[yPoint_27255][xPoint_27255+1] === this.CELL_EMPTY
                && map[yPoint_27255+1][xPoint_27255+1] === this.CELL_EMPTY
                && map[yPoint_27255+1][xPoint_27255] === this.CELL_EMPTY
                && map[yPoint_27255+1][xPoint_27255-1] === this.CELL_EMPTY
                && map[yPoint_27255][xPoint_27255-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27255, yPoint_27255, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27255;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27255][x] === this.CELL_EMPTY
                    && map[yPoint_27255-1][x] === this.CELL_EMPTY
                    && map[yPoint_27255-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27255][x+1] === this.CELL_EMPTY
                    && map[yPoint_27255+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27255+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27255, yPoint_27255, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27255;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27255] === this.CELL_EMPTY
                    && map[y+1][xPoint_27255] === this.CELL_EMPTY
                    && map[y+1][xPoint_27255+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27255] === this.CELL_EMPTY
                    && map[y][xPoint_27255-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27255-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
