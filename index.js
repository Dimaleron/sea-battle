...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31598=0;yPoint_31598<this.gameFieldBorderY.length; yPoint_31598++){
                for(var xPoint_31598=0;xPoint_31598<this.gameFieldBorderX.length; xPoint_31598++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31598, xPoint_31598);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31598][xPoint_31598] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31598, xPoint_31598, 'user');
                    if(this._userShipsMap[yPoint_31598][xPoint_31598] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31598, xPoint_31598, type){
            var id = this.getPointBlockIdByCoords(yPoint_31598, xPoint_31598, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31598);
                block.setAttribute('data-y', yPoint_31598);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31598
         * @param {type} xPoint_31598
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31598, xPoint_31598, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31598 + '_y' + yPoint_31598;
            }
            return 'pc_x' + xPoint_31598 + '_y' + yPoint_31598;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31598=0;yPoint_31598<this.gameFieldBorderY.length; yPoint_31598++){
                for(var xPoint_31598=0;xPoint_31598<this.gameFieldBorderX.length; xPoint_31598++){
                    map.push({y: yPoint_31598, x: xPoint_31598});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31598=-1;yPoint_31598<(this.gameFieldBorderY.length+1); yPoint_31598++){
                for(var xPoint_31598=-1;xPoint_31598<(this.gameFieldBorderX.length+1); xPoint_31598++){
                    if(!map[yPoint_31598]){
                        map[yPoint_31598] = [];
                    }
                    map[yPoint_31598][xPoint_31598] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31598 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31598 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31598, yPoint_31598) === true){
                    if(this.canPutHorizontal(map, xPoint_31598, yPoint_31598, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31598][xPoint_31598 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31598, yPoint_31598, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31598 + i][xPoint_31598] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31598, yPoint_31598){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31598][xPoint_31598] === this.CELL_EMPTY
                && map[yPoint_31598-1][xPoint_31598] === this.CELL_EMPTY
                && map[yPoint_31598-1][xPoint_31598+1] === this.CELL_EMPTY
                && map[yPoint_31598][xPoint_31598+1] === this.CELL_EMPTY
                && map[yPoint_31598+1][xPoint_31598+1] === this.CELL_EMPTY
                && map[yPoint_31598+1][xPoint_31598] === this.CELL_EMPTY
                && map[yPoint_31598+1][xPoint_31598-1] === this.CELL_EMPTY
                && map[yPoint_31598][xPoint_31598-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31598, yPoint_31598, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31598;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31598][x] === this.CELL_EMPTY
                    && map[yPoint_31598-1][x] === this.CELL_EMPTY
                    && map[yPoint_31598-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31598][x+1] === this.CELL_EMPTY
                    && map[yPoint_31598+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31598+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31598, yPoint_31598, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31598;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31598] === this.CELL_EMPTY
                    && map[y+1][xPoint_31598] === this.CELL_EMPTY
                    && map[y+1][xPoint_31598+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31598] === this.CELL_EMPTY
                    && map[y][xPoint_31598-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31598-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
