...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3225=0;yPoint_3225<this.gameFieldBorderY.length; yPoint_3225++){
                for(var xPoint_3225=0;xPoint_3225<this.gameFieldBorderX.length; xPoint_3225++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3225, xPoint_3225);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3225][xPoint_3225] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3225, xPoint_3225, 'user');
                    if(this._userShipsMap[yPoint_3225][xPoint_3225] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3225, xPoint_3225, type){
            var id = this.getPointBlockIdByCoords(yPoint_3225, xPoint_3225, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3225);
                block.setAttribute('data-y', yPoint_3225);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3225
         * @param {type} xPoint_3225
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3225, xPoint_3225, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3225 + '_y' + yPoint_3225;
            }
            return 'pc_x' + xPoint_3225 + '_y' + yPoint_3225;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3225=0;yPoint_3225<this.gameFieldBorderY.length; yPoint_3225++){
                for(var xPoint_3225=0;xPoint_3225<this.gameFieldBorderX.length; xPoint_3225++){
                    map.push({y: yPoint_3225, x: xPoint_3225});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3225=-1;yPoint_3225<(this.gameFieldBorderY.length+1); yPoint_3225++){
                for(var xPoint_3225=-1;xPoint_3225<(this.gameFieldBorderX.length+1); xPoint_3225++){
                    if(!map[yPoint_3225]){
                        map[yPoint_3225] = [];
                    }
                    map[yPoint_3225][xPoint_3225] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3225 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3225 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3225, yPoint_3225) === true){
                    if(this.canPutHorizontal(map, xPoint_3225, yPoint_3225, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3225][xPoint_3225 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3225, yPoint_3225, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3225 + i][xPoint_3225] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3225, yPoint_3225){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3225][xPoint_3225] === this.CELL_EMPTY
                && map[yPoint_3225-1][xPoint_3225] === this.CELL_EMPTY
                && map[yPoint_3225-1][xPoint_3225+1] === this.CELL_EMPTY
                && map[yPoint_3225][xPoint_3225+1] === this.CELL_EMPTY
                && map[yPoint_3225+1][xPoint_3225+1] === this.CELL_EMPTY
                && map[yPoint_3225+1][xPoint_3225] === this.CELL_EMPTY
                && map[yPoint_3225+1][xPoint_3225-1] === this.CELL_EMPTY
                && map[yPoint_3225][xPoint_3225-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3225, yPoint_3225, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3225;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3225][x] === this.CELL_EMPTY
                    && map[yPoint_3225-1][x] === this.CELL_EMPTY
                    && map[yPoint_3225-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3225][x+1] === this.CELL_EMPTY
                    && map[yPoint_3225+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3225+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3225, yPoint_3225, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3225;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3225] === this.CELL_EMPTY
                    && map[y+1][xPoint_3225] === this.CELL_EMPTY
                    && map[y+1][xPoint_3225+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3225] === this.CELL_EMPTY
                    && map[y][xPoint_3225-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3225-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
