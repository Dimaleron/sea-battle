...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2779=0;yPoint_2779<this.gameFieldBorderY.length; yPoint_2779++){
                for(var xPoint_2779=0;xPoint_2779<this.gameFieldBorderX.length; xPoint_2779++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2779, xPoint_2779);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2779][xPoint_2779] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2779, xPoint_2779, 'user');
                    if(this._userShipsMap[yPoint_2779][xPoint_2779] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2779, xPoint_2779, type){
            var id = this.getPointBlockIdByCoords(yPoint_2779, xPoint_2779, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2779);
                block.setAttribute('data-y', yPoint_2779);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2779
         * @param {type} xPoint_2779
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2779, xPoint_2779, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2779 + '_y' + yPoint_2779;
            }
            return 'pc_x' + xPoint_2779 + '_y' + yPoint_2779;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2779=0;yPoint_2779<this.gameFieldBorderY.length; yPoint_2779++){
                for(var xPoint_2779=0;xPoint_2779<this.gameFieldBorderX.length; xPoint_2779++){
                    map.push({y: yPoint_2779, x: xPoint_2779});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2779=-1;yPoint_2779<(this.gameFieldBorderY.length+1); yPoint_2779++){
                for(var xPoint_2779=-1;xPoint_2779<(this.gameFieldBorderX.length+1); xPoint_2779++){
                    if(!map[yPoint_2779]){
                        map[yPoint_2779] = [];
                    }
                    map[yPoint_2779][xPoint_2779] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2779 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2779 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2779, yPoint_2779) === true){
                    if(this.canPutHorizontal(map, xPoint_2779, yPoint_2779, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2779][xPoint_2779 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2779, yPoint_2779, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2779 + i][xPoint_2779] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2779, yPoint_2779){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2779][xPoint_2779] === this.CELL_EMPTY
                && map[yPoint_2779-1][xPoint_2779] === this.CELL_EMPTY
                && map[yPoint_2779-1][xPoint_2779+1] === this.CELL_EMPTY
                && map[yPoint_2779][xPoint_2779+1] === this.CELL_EMPTY
                && map[yPoint_2779+1][xPoint_2779+1] === this.CELL_EMPTY
                && map[yPoint_2779+1][xPoint_2779] === this.CELL_EMPTY
                && map[yPoint_2779+1][xPoint_2779-1] === this.CELL_EMPTY
                && map[yPoint_2779][xPoint_2779-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2779, yPoint_2779, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2779;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2779][x] === this.CELL_EMPTY
                    && map[yPoint_2779-1][x] === this.CELL_EMPTY
                    && map[yPoint_2779-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2779][x+1] === this.CELL_EMPTY
                    && map[yPoint_2779+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2779+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2779, yPoint_2779, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2779;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2779] === this.CELL_EMPTY
                    && map[y+1][xPoint_2779] === this.CELL_EMPTY
                    && map[y+1][xPoint_2779+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2779] === this.CELL_EMPTY
                    && map[y][xPoint_2779-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2779-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
