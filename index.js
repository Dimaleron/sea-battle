...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20953=0;yPoint_20953<this.gameFieldBorderY.length; yPoint_20953++){
                for(var xPoint_20953=0;xPoint_20953<this.gameFieldBorderX.length; xPoint_20953++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20953, xPoint_20953);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20953][xPoint_20953] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20953, xPoint_20953, 'user');
                    if(this._userShipsMap[yPoint_20953][xPoint_20953] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20953, xPoint_20953, type){
            var id = this.getPointBlockIdByCoords(yPoint_20953, xPoint_20953, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20953);
                block.setAttribute('data-y', yPoint_20953);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20953
         * @param {type} xPoint_20953
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20953, xPoint_20953, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20953 + '_y' + yPoint_20953;
            }
            return 'pc_x' + xPoint_20953 + '_y' + yPoint_20953;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20953=0;yPoint_20953<this.gameFieldBorderY.length; yPoint_20953++){
                for(var xPoint_20953=0;xPoint_20953<this.gameFieldBorderX.length; xPoint_20953++){
                    map.push({y: yPoint_20953, x: xPoint_20953});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20953=-1;yPoint_20953<(this.gameFieldBorderY.length+1); yPoint_20953++){
                for(var xPoint_20953=-1;xPoint_20953<(this.gameFieldBorderX.length+1); xPoint_20953++){
                    if(!map[yPoint_20953]){
                        map[yPoint_20953] = [];
                    }
                    map[yPoint_20953][xPoint_20953] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20953 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20953 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20953, yPoint_20953) === true){
                    if(this.canPutHorizontal(map, xPoint_20953, yPoint_20953, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20953][xPoint_20953 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20953, yPoint_20953, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20953 + i][xPoint_20953] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20953, yPoint_20953){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20953][xPoint_20953] === this.CELL_EMPTY
                && map[yPoint_20953-1][xPoint_20953] === this.CELL_EMPTY
                && map[yPoint_20953-1][xPoint_20953+1] === this.CELL_EMPTY
                && map[yPoint_20953][xPoint_20953+1] === this.CELL_EMPTY
                && map[yPoint_20953+1][xPoint_20953+1] === this.CELL_EMPTY
                && map[yPoint_20953+1][xPoint_20953] === this.CELL_EMPTY
                && map[yPoint_20953+1][xPoint_20953-1] === this.CELL_EMPTY
                && map[yPoint_20953][xPoint_20953-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20953, yPoint_20953, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20953;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20953][x] === this.CELL_EMPTY
                    && map[yPoint_20953-1][x] === this.CELL_EMPTY
                    && map[yPoint_20953-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20953][x+1] === this.CELL_EMPTY
                    && map[yPoint_20953+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20953+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20953, yPoint_20953, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20953;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20953] === this.CELL_EMPTY
                    && map[y+1][xPoint_20953] === this.CELL_EMPTY
                    && map[y+1][xPoint_20953+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20953] === this.CELL_EMPTY
                    && map[y][xPoint_20953-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20953-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
