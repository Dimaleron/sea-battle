...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7191=0;yPoint_7191<this.gameFieldBorderY.length; yPoint_7191++){
                for(var xPoint_7191=0;xPoint_7191<this.gameFieldBorderX.length; xPoint_7191++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7191, xPoint_7191);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7191][xPoint_7191] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7191, xPoint_7191, 'user');
                    if(this._userShipsMap[yPoint_7191][xPoint_7191] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7191, xPoint_7191, type){
            var id = this.getPointBlockIdByCoords(yPoint_7191, xPoint_7191, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7191);
                block.setAttribute('data-y', yPoint_7191);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7191
         * @param {type} xPoint_7191
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7191, xPoint_7191, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7191 + '_y' + yPoint_7191;
            }
            return 'pc_x' + xPoint_7191 + '_y' + yPoint_7191;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7191=0;yPoint_7191<this.gameFieldBorderY.length; yPoint_7191++){
                for(var xPoint_7191=0;xPoint_7191<this.gameFieldBorderX.length; xPoint_7191++){
                    map.push({y: yPoint_7191, x: xPoint_7191});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7191=-1;yPoint_7191<(this.gameFieldBorderY.length+1); yPoint_7191++){
                for(var xPoint_7191=-1;xPoint_7191<(this.gameFieldBorderX.length+1); xPoint_7191++){
                    if(!map[yPoint_7191]){
                        map[yPoint_7191] = [];
                    }
                    map[yPoint_7191][xPoint_7191] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7191 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7191 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7191, yPoint_7191) === true){
                    if(this.canPutHorizontal(map, xPoint_7191, yPoint_7191, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7191][xPoint_7191 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7191, yPoint_7191, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7191 + i][xPoint_7191] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7191, yPoint_7191){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7191][xPoint_7191] === this.CELL_EMPTY
                && map[yPoint_7191-1][xPoint_7191] === this.CELL_EMPTY
                && map[yPoint_7191-1][xPoint_7191+1] === this.CELL_EMPTY
                && map[yPoint_7191][xPoint_7191+1] === this.CELL_EMPTY
                && map[yPoint_7191+1][xPoint_7191+1] === this.CELL_EMPTY
                && map[yPoint_7191+1][xPoint_7191] === this.CELL_EMPTY
                && map[yPoint_7191+1][xPoint_7191-1] === this.CELL_EMPTY
                && map[yPoint_7191][xPoint_7191-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7191, yPoint_7191, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7191;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7191][x] === this.CELL_EMPTY
                    && map[yPoint_7191-1][x] === this.CELL_EMPTY
                    && map[yPoint_7191-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7191][x+1] === this.CELL_EMPTY
                    && map[yPoint_7191+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7191+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7191, yPoint_7191, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7191;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7191] === this.CELL_EMPTY
                    && map[y+1][xPoint_7191] === this.CELL_EMPTY
                    && map[y+1][xPoint_7191+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7191] === this.CELL_EMPTY
                    && map[y][xPoint_7191-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7191-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
