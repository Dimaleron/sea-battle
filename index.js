...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31816=0;yPoint_31816<this.gameFieldBorderY.length; yPoint_31816++){
                for(var xPoint_31816=0;xPoint_31816<this.gameFieldBorderX.length; xPoint_31816++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31816, xPoint_31816);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31816][xPoint_31816] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31816, xPoint_31816, 'user');
                    if(this._userShipsMap[yPoint_31816][xPoint_31816] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31816, xPoint_31816, type){
            var id = this.getPointBlockIdByCoords(yPoint_31816, xPoint_31816, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31816);
                block.setAttribute('data-y', yPoint_31816);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31816
         * @param {type} xPoint_31816
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31816, xPoint_31816, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31816 + '_y' + yPoint_31816;
            }
            return 'pc_x' + xPoint_31816 + '_y' + yPoint_31816;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31816=0;yPoint_31816<this.gameFieldBorderY.length; yPoint_31816++){
                for(var xPoint_31816=0;xPoint_31816<this.gameFieldBorderX.length; xPoint_31816++){
                    map.push({y: yPoint_31816, x: xPoint_31816});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31816=-1;yPoint_31816<(this.gameFieldBorderY.length+1); yPoint_31816++){
                for(var xPoint_31816=-1;xPoint_31816<(this.gameFieldBorderX.length+1); xPoint_31816++){
                    if(!map[yPoint_31816]){
                        map[yPoint_31816] = [];
                    }
                    map[yPoint_31816][xPoint_31816] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31816 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31816 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31816, yPoint_31816) === true){
                    if(this.canPutHorizontal(map, xPoint_31816, yPoint_31816, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31816][xPoint_31816 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31816, yPoint_31816, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31816 + i][xPoint_31816] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31816, yPoint_31816){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31816][xPoint_31816] === this.CELL_EMPTY
                && map[yPoint_31816-1][xPoint_31816] === this.CELL_EMPTY
                && map[yPoint_31816-1][xPoint_31816+1] === this.CELL_EMPTY
                && map[yPoint_31816][xPoint_31816+1] === this.CELL_EMPTY
                && map[yPoint_31816+1][xPoint_31816+1] === this.CELL_EMPTY
                && map[yPoint_31816+1][xPoint_31816] === this.CELL_EMPTY
                && map[yPoint_31816+1][xPoint_31816-1] === this.CELL_EMPTY
                && map[yPoint_31816][xPoint_31816-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31816, yPoint_31816, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31816;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31816][x] === this.CELL_EMPTY
                    && map[yPoint_31816-1][x] === this.CELL_EMPTY
                    && map[yPoint_31816-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31816][x+1] === this.CELL_EMPTY
                    && map[yPoint_31816+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31816+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31816, yPoint_31816, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31816;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31816] === this.CELL_EMPTY
                    && map[y+1][xPoint_31816] === this.CELL_EMPTY
                    && map[y+1][xPoint_31816+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31816] === this.CELL_EMPTY
                    && map[y][xPoint_31816-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31816-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
