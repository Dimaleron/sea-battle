...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13401=0;yPoint_13401<this.gameFieldBorderY.length; yPoint_13401++){
                for(var xPoint_13401=0;xPoint_13401<this.gameFieldBorderX.length; xPoint_13401++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13401, xPoint_13401);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13401][xPoint_13401] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13401, xPoint_13401, 'user');
                    if(this._userShipsMap[yPoint_13401][xPoint_13401] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13401, xPoint_13401, type){
            var id = this.getPointBlockIdByCoords(yPoint_13401, xPoint_13401, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13401);
                block.setAttribute('data-y', yPoint_13401);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13401
         * @param {type} xPoint_13401
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13401, xPoint_13401, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13401 + '_y' + yPoint_13401;
            }
            return 'pc_x' + xPoint_13401 + '_y' + yPoint_13401;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13401=0;yPoint_13401<this.gameFieldBorderY.length; yPoint_13401++){
                for(var xPoint_13401=0;xPoint_13401<this.gameFieldBorderX.length; xPoint_13401++){
                    map.push({y: yPoint_13401, x: xPoint_13401});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13401=-1;yPoint_13401<(this.gameFieldBorderY.length+1); yPoint_13401++){
                for(var xPoint_13401=-1;xPoint_13401<(this.gameFieldBorderX.length+1); xPoint_13401++){
                    if(!map[yPoint_13401]){
                        map[yPoint_13401] = [];
                    }
                    map[yPoint_13401][xPoint_13401] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13401 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13401 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13401, yPoint_13401) === true){
                    if(this.canPutHorizontal(map, xPoint_13401, yPoint_13401, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13401][xPoint_13401 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13401, yPoint_13401, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13401 + i][xPoint_13401] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13401, yPoint_13401){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13401][xPoint_13401] === this.CELL_EMPTY
                && map[yPoint_13401-1][xPoint_13401] === this.CELL_EMPTY
                && map[yPoint_13401-1][xPoint_13401+1] === this.CELL_EMPTY
                && map[yPoint_13401][xPoint_13401+1] === this.CELL_EMPTY
                && map[yPoint_13401+1][xPoint_13401+1] === this.CELL_EMPTY
                && map[yPoint_13401+1][xPoint_13401] === this.CELL_EMPTY
                && map[yPoint_13401+1][xPoint_13401-1] === this.CELL_EMPTY
                && map[yPoint_13401][xPoint_13401-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13401, yPoint_13401, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13401;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13401][x] === this.CELL_EMPTY
                    && map[yPoint_13401-1][x] === this.CELL_EMPTY
                    && map[yPoint_13401-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13401][x+1] === this.CELL_EMPTY
                    && map[yPoint_13401+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13401+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13401, yPoint_13401, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13401;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13401] === this.CELL_EMPTY
                    && map[y+1][xPoint_13401] === this.CELL_EMPTY
                    && map[y+1][xPoint_13401+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13401] === this.CELL_EMPTY
                    && map[y][xPoint_13401-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13401-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
