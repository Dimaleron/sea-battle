...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28464=0;yPoint_28464<this.gameFieldBorderY.length; yPoint_28464++){
                for(var xPoint_28464=0;xPoint_28464<this.gameFieldBorderX.length; xPoint_28464++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28464, xPoint_28464);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28464][xPoint_28464] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28464, xPoint_28464, 'user');
                    if(this._userShipsMap[yPoint_28464][xPoint_28464] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28464, xPoint_28464, type){
            var id = this.getPointBlockIdByCoords(yPoint_28464, xPoint_28464, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28464);
                block.setAttribute('data-y', yPoint_28464);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28464
         * @param {type} xPoint_28464
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28464, xPoint_28464, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28464 + '_y' + yPoint_28464;
            }
            return 'pc_x' + xPoint_28464 + '_y' + yPoint_28464;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28464=0;yPoint_28464<this.gameFieldBorderY.length; yPoint_28464++){
                for(var xPoint_28464=0;xPoint_28464<this.gameFieldBorderX.length; xPoint_28464++){
                    map.push({y: yPoint_28464, x: xPoint_28464});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28464=-1;yPoint_28464<(this.gameFieldBorderY.length+1); yPoint_28464++){
                for(var xPoint_28464=-1;xPoint_28464<(this.gameFieldBorderX.length+1); xPoint_28464++){
                    if(!map[yPoint_28464]){
                        map[yPoint_28464] = [];
                    }
                    map[yPoint_28464][xPoint_28464] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28464 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28464 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28464, yPoint_28464) === true){
                    if(this.canPutHorizontal(map, xPoint_28464, yPoint_28464, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28464][xPoint_28464 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28464, yPoint_28464, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28464 + i][xPoint_28464] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28464, yPoint_28464){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28464][xPoint_28464] === this.CELL_EMPTY
                && map[yPoint_28464-1][xPoint_28464] === this.CELL_EMPTY
                && map[yPoint_28464-1][xPoint_28464+1] === this.CELL_EMPTY
                && map[yPoint_28464][xPoint_28464+1] === this.CELL_EMPTY
                && map[yPoint_28464+1][xPoint_28464+1] === this.CELL_EMPTY
                && map[yPoint_28464+1][xPoint_28464] === this.CELL_EMPTY
                && map[yPoint_28464+1][xPoint_28464-1] === this.CELL_EMPTY
                && map[yPoint_28464][xPoint_28464-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28464, yPoint_28464, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28464;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28464][x] === this.CELL_EMPTY
                    && map[yPoint_28464-1][x] === this.CELL_EMPTY
                    && map[yPoint_28464-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28464][x+1] === this.CELL_EMPTY
                    && map[yPoint_28464+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28464+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28464, yPoint_28464, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28464;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28464] === this.CELL_EMPTY
                    && map[y+1][xPoint_28464] === this.CELL_EMPTY
                    && map[y+1][xPoint_28464+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28464] === this.CELL_EMPTY
                    && map[y][xPoint_28464-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28464-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
