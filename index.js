...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19776=0;yPoint_19776<this.gameFieldBorderY.length; yPoint_19776++){
                for(var xPoint_19776=0;xPoint_19776<this.gameFieldBorderX.length; xPoint_19776++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19776, xPoint_19776);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19776][xPoint_19776] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19776, xPoint_19776, 'user');
                    if(this._userShipsMap[yPoint_19776][xPoint_19776] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19776, xPoint_19776, type){
            var id = this.getPointBlockIdByCoords(yPoint_19776, xPoint_19776, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19776);
                block.setAttribute('data-y', yPoint_19776);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19776
         * @param {type} xPoint_19776
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19776, xPoint_19776, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19776 + '_y' + yPoint_19776;
            }
            return 'pc_x' + xPoint_19776 + '_y' + yPoint_19776;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19776=0;yPoint_19776<this.gameFieldBorderY.length; yPoint_19776++){
                for(var xPoint_19776=0;xPoint_19776<this.gameFieldBorderX.length; xPoint_19776++){
                    map.push({y: yPoint_19776, x: xPoint_19776});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19776=-1;yPoint_19776<(this.gameFieldBorderY.length+1); yPoint_19776++){
                for(var xPoint_19776=-1;xPoint_19776<(this.gameFieldBorderX.length+1); xPoint_19776++){
                    if(!map[yPoint_19776]){
                        map[yPoint_19776] = [];
                    }
                    map[yPoint_19776][xPoint_19776] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19776 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19776 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19776, yPoint_19776) === true){
                    if(this.canPutHorizontal(map, xPoint_19776, yPoint_19776, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19776][xPoint_19776 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19776, yPoint_19776, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19776 + i][xPoint_19776] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19776, yPoint_19776){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19776][xPoint_19776] === this.CELL_EMPTY
                && map[yPoint_19776-1][xPoint_19776] === this.CELL_EMPTY
                && map[yPoint_19776-1][xPoint_19776+1] === this.CELL_EMPTY
                && map[yPoint_19776][xPoint_19776+1] === this.CELL_EMPTY
                && map[yPoint_19776+1][xPoint_19776+1] === this.CELL_EMPTY
                && map[yPoint_19776+1][xPoint_19776] === this.CELL_EMPTY
                && map[yPoint_19776+1][xPoint_19776-1] === this.CELL_EMPTY
                && map[yPoint_19776][xPoint_19776-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19776, yPoint_19776, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19776;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19776][x] === this.CELL_EMPTY
                    && map[yPoint_19776-1][x] === this.CELL_EMPTY
                    && map[yPoint_19776-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19776][x+1] === this.CELL_EMPTY
                    && map[yPoint_19776+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19776+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19776, yPoint_19776, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19776;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19776] === this.CELL_EMPTY
                    && map[y+1][xPoint_19776] === this.CELL_EMPTY
                    && map[y+1][xPoint_19776+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19776] === this.CELL_EMPTY
                    && map[y][xPoint_19776-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19776-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
