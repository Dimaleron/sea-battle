...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_12706=0;yPoint_12706<this.gameFieldBorderY.length; yPoint_12706++){
                for(var xPoint_12706=0;xPoint_12706<this.gameFieldBorderX.length; xPoint_12706++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_12706, xPoint_12706);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_12706][xPoint_12706] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_12706, xPoint_12706, 'user');
                    if(this._userShipsMap[yPoint_12706][xPoint_12706] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_12706, xPoint_12706, type){
            var id = this.getPointBlockIdByCoords(yPoint_12706, xPoint_12706, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_12706);
                block.setAttribute('data-y', yPoint_12706);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_12706
         * @param {type} xPoint_12706
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_12706, xPoint_12706, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_12706 + '_y' + yPoint_12706;
            }
            return 'pc_x' + xPoint_12706 + '_y' + yPoint_12706;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_12706=0;yPoint_12706<this.gameFieldBorderY.length; yPoint_12706++){
                for(var xPoint_12706=0;xPoint_12706<this.gameFieldBorderX.length; xPoint_12706++){
                    map.push({y: yPoint_12706, x: xPoint_12706});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_12706=-1;yPoint_12706<(this.gameFieldBorderY.length+1); yPoint_12706++){
                for(var xPoint_12706=-1;xPoint_12706<(this.gameFieldBorderX.length+1); xPoint_12706++){
                    if(!map[yPoint_12706]){
                        map[yPoint_12706] = [];
                    }
                    map[yPoint_12706][xPoint_12706] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_12706 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_12706 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_12706, yPoint_12706) === true){
                    if(this.canPutHorizontal(map, xPoint_12706, yPoint_12706, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12706][xPoint_12706 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_12706, yPoint_12706, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12706 + i][xPoint_12706] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_12706, yPoint_12706){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_12706][xPoint_12706] === this.CELL_EMPTY
                && map[yPoint_12706-1][xPoint_12706] === this.CELL_EMPTY
                && map[yPoint_12706-1][xPoint_12706+1] === this.CELL_EMPTY
                && map[yPoint_12706][xPoint_12706+1] === this.CELL_EMPTY
                && map[yPoint_12706+1][xPoint_12706+1] === this.CELL_EMPTY
                && map[yPoint_12706+1][xPoint_12706] === this.CELL_EMPTY
                && map[yPoint_12706+1][xPoint_12706-1] === this.CELL_EMPTY
                && map[yPoint_12706][xPoint_12706-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_12706, yPoint_12706, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_12706;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_12706][x] === this.CELL_EMPTY
                    && map[yPoint_12706-1][x] === this.CELL_EMPTY
                    && map[yPoint_12706-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12706][x+1] === this.CELL_EMPTY
                    && map[yPoint_12706+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12706+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_12706, yPoint_12706, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_12706;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_12706] === this.CELL_EMPTY
                    && map[y+1][xPoint_12706] === this.CELL_EMPTY
                    && map[y+1][xPoint_12706+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_12706] === this.CELL_EMPTY
                    && map[y][xPoint_12706-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_12706-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
