...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26734=0;yPoint_26734<this.gameFieldBorderY.length; yPoint_26734++){
                for(var xPoint_26734=0;xPoint_26734<this.gameFieldBorderX.length; xPoint_26734++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26734, xPoint_26734);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26734][xPoint_26734] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26734, xPoint_26734, 'user');
                    if(this._userShipsMap[yPoint_26734][xPoint_26734] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26734, xPoint_26734, type){
            var id = this.getPointBlockIdByCoords(yPoint_26734, xPoint_26734, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26734);
                block.setAttribute('data-y', yPoint_26734);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26734
         * @param {type} xPoint_26734
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26734, xPoint_26734, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26734 + '_y' + yPoint_26734;
            }
            return 'pc_x' + xPoint_26734 + '_y' + yPoint_26734;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26734=0;yPoint_26734<this.gameFieldBorderY.length; yPoint_26734++){
                for(var xPoint_26734=0;xPoint_26734<this.gameFieldBorderX.length; xPoint_26734++){
                    map.push({y: yPoint_26734, x: xPoint_26734});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26734=-1;yPoint_26734<(this.gameFieldBorderY.length+1); yPoint_26734++){
                for(var xPoint_26734=-1;xPoint_26734<(this.gameFieldBorderX.length+1); xPoint_26734++){
                    if(!map[yPoint_26734]){
                        map[yPoint_26734] = [];
                    }
                    map[yPoint_26734][xPoint_26734] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26734 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26734 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26734, yPoint_26734) === true){
                    if(this.canPutHorizontal(map, xPoint_26734, yPoint_26734, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26734][xPoint_26734 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26734, yPoint_26734, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26734 + i][xPoint_26734] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26734, yPoint_26734){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26734][xPoint_26734] === this.CELL_EMPTY
                && map[yPoint_26734-1][xPoint_26734] === this.CELL_EMPTY
                && map[yPoint_26734-1][xPoint_26734+1] === this.CELL_EMPTY
                && map[yPoint_26734][xPoint_26734+1] === this.CELL_EMPTY
                && map[yPoint_26734+1][xPoint_26734+1] === this.CELL_EMPTY
                && map[yPoint_26734+1][xPoint_26734] === this.CELL_EMPTY
                && map[yPoint_26734+1][xPoint_26734-1] === this.CELL_EMPTY
                && map[yPoint_26734][xPoint_26734-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26734, yPoint_26734, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26734;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26734][x] === this.CELL_EMPTY
                    && map[yPoint_26734-1][x] === this.CELL_EMPTY
                    && map[yPoint_26734-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26734][x+1] === this.CELL_EMPTY
                    && map[yPoint_26734+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26734+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26734, yPoint_26734, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26734;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26734] === this.CELL_EMPTY
                    && map[y+1][xPoint_26734] === this.CELL_EMPTY
                    && map[y+1][xPoint_26734+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26734] === this.CELL_EMPTY
                    && map[y][xPoint_26734-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26734-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
