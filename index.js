...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19060=0;yPoint_19060<this.gameFieldBorderY.length; yPoint_19060++){
                for(var xPoint_19060=0;xPoint_19060<this.gameFieldBorderX.length; xPoint_19060++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19060, xPoint_19060);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19060][xPoint_19060] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19060, xPoint_19060, 'user');
                    if(this._userShipsMap[yPoint_19060][xPoint_19060] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19060, xPoint_19060, type){
            var id = this.getPointBlockIdByCoords(yPoint_19060, xPoint_19060, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19060);
                block.setAttribute('data-y', yPoint_19060);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19060
         * @param {type} xPoint_19060
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19060, xPoint_19060, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19060 + '_y' + yPoint_19060;
            }
            return 'pc_x' + xPoint_19060 + '_y' + yPoint_19060;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19060=0;yPoint_19060<this.gameFieldBorderY.length; yPoint_19060++){
                for(var xPoint_19060=0;xPoint_19060<this.gameFieldBorderX.length; xPoint_19060++){
                    map.push({y: yPoint_19060, x: xPoint_19060});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19060=-1;yPoint_19060<(this.gameFieldBorderY.length+1); yPoint_19060++){
                for(var xPoint_19060=-1;xPoint_19060<(this.gameFieldBorderX.length+1); xPoint_19060++){
                    if(!map[yPoint_19060]){
                        map[yPoint_19060] = [];
                    }
                    map[yPoint_19060][xPoint_19060] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19060 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19060 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19060, yPoint_19060) === true){
                    if(this.canPutHorizontal(map, xPoint_19060, yPoint_19060, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19060][xPoint_19060 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19060, yPoint_19060, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19060 + i][xPoint_19060] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19060, yPoint_19060){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19060][xPoint_19060] === this.CELL_EMPTY
                && map[yPoint_19060-1][xPoint_19060] === this.CELL_EMPTY
                && map[yPoint_19060-1][xPoint_19060+1] === this.CELL_EMPTY
                && map[yPoint_19060][xPoint_19060+1] === this.CELL_EMPTY
                && map[yPoint_19060+1][xPoint_19060+1] === this.CELL_EMPTY
                && map[yPoint_19060+1][xPoint_19060] === this.CELL_EMPTY
                && map[yPoint_19060+1][xPoint_19060-1] === this.CELL_EMPTY
                && map[yPoint_19060][xPoint_19060-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19060, yPoint_19060, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19060;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19060][x] === this.CELL_EMPTY
                    && map[yPoint_19060-1][x] === this.CELL_EMPTY
                    && map[yPoint_19060-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19060][x+1] === this.CELL_EMPTY
                    && map[yPoint_19060+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19060+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19060, yPoint_19060, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19060;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19060] === this.CELL_EMPTY
                    && map[y+1][xPoint_19060] === this.CELL_EMPTY
                    && map[y+1][xPoint_19060+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19060] === this.CELL_EMPTY
                    && map[y][xPoint_19060-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19060-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
