...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23019=0;yPoint_23019<this.gameFieldBorderY.length; yPoint_23019++){
                for(var xPoint_23019=0;xPoint_23019<this.gameFieldBorderX.length; xPoint_23019++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23019, xPoint_23019);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23019][xPoint_23019] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23019, xPoint_23019, 'user');
                    if(this._userShipsMap[yPoint_23019][xPoint_23019] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23019, xPoint_23019, type){
            var id = this.getPointBlockIdByCoords(yPoint_23019, xPoint_23019, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23019);
                block.setAttribute('data-y', yPoint_23019);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23019
         * @param {type} xPoint_23019
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23019, xPoint_23019, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23019 + '_y' + yPoint_23019;
            }
            return 'pc_x' + xPoint_23019 + '_y' + yPoint_23019;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23019=0;yPoint_23019<this.gameFieldBorderY.length; yPoint_23019++){
                for(var xPoint_23019=0;xPoint_23019<this.gameFieldBorderX.length; xPoint_23019++){
                    map.push({y: yPoint_23019, x: xPoint_23019});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23019=-1;yPoint_23019<(this.gameFieldBorderY.length+1); yPoint_23019++){
                for(var xPoint_23019=-1;xPoint_23019<(this.gameFieldBorderX.length+1); xPoint_23019++){
                    if(!map[yPoint_23019]){
                        map[yPoint_23019] = [];
                    }
                    map[yPoint_23019][xPoint_23019] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23019 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23019 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23019, yPoint_23019) === true){
                    if(this.canPutHorizontal(map, xPoint_23019, yPoint_23019, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23019][xPoint_23019 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23019, yPoint_23019, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23019 + i][xPoint_23019] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23019, yPoint_23019){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23019][xPoint_23019] === this.CELL_EMPTY
                && map[yPoint_23019-1][xPoint_23019] === this.CELL_EMPTY
                && map[yPoint_23019-1][xPoint_23019+1] === this.CELL_EMPTY
                && map[yPoint_23019][xPoint_23019+1] === this.CELL_EMPTY
                && map[yPoint_23019+1][xPoint_23019+1] === this.CELL_EMPTY
                && map[yPoint_23019+1][xPoint_23019] === this.CELL_EMPTY
                && map[yPoint_23019+1][xPoint_23019-1] === this.CELL_EMPTY
                && map[yPoint_23019][xPoint_23019-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23019, yPoint_23019, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23019;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23019][x] === this.CELL_EMPTY
                    && map[yPoint_23019-1][x] === this.CELL_EMPTY
                    && map[yPoint_23019-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23019][x+1] === this.CELL_EMPTY
                    && map[yPoint_23019+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23019+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23019, yPoint_23019, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23019;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23019] === this.CELL_EMPTY
                    && map[y+1][xPoint_23019] === this.CELL_EMPTY
                    && map[y+1][xPoint_23019+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23019] === this.CELL_EMPTY
                    && map[y][xPoint_23019-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23019-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
