...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10901=0;yPoint_10901<this.gameFieldBorderY.length; yPoint_10901++){
                for(var xPoint_10901=0;xPoint_10901<this.gameFieldBorderX.length; xPoint_10901++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10901, xPoint_10901);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10901][xPoint_10901] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10901, xPoint_10901, 'user');
                    if(this._userShipsMap[yPoint_10901][xPoint_10901] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10901, xPoint_10901, type){
            var id = this.getPointBlockIdByCoords(yPoint_10901, xPoint_10901, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10901);
                block.setAttribute('data-y', yPoint_10901);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10901
         * @param {type} xPoint_10901
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10901, xPoint_10901, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10901 + '_y' + yPoint_10901;
            }
            return 'pc_x' + xPoint_10901 + '_y' + yPoint_10901;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10901=0;yPoint_10901<this.gameFieldBorderY.length; yPoint_10901++){
                for(var xPoint_10901=0;xPoint_10901<this.gameFieldBorderX.length; xPoint_10901++){
                    map.push({y: yPoint_10901, x: xPoint_10901});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10901=-1;yPoint_10901<(this.gameFieldBorderY.length+1); yPoint_10901++){
                for(var xPoint_10901=-1;xPoint_10901<(this.gameFieldBorderX.length+1); xPoint_10901++){
                    if(!map[yPoint_10901]){
                        map[yPoint_10901] = [];
                    }
                    map[yPoint_10901][xPoint_10901] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10901 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10901 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10901, yPoint_10901) === true){
                    if(this.canPutHorizontal(map, xPoint_10901, yPoint_10901, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10901][xPoint_10901 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10901, yPoint_10901, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10901 + i][xPoint_10901] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10901, yPoint_10901){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10901][xPoint_10901] === this.CELL_EMPTY
                && map[yPoint_10901-1][xPoint_10901] === this.CELL_EMPTY
                && map[yPoint_10901-1][xPoint_10901+1] === this.CELL_EMPTY
                && map[yPoint_10901][xPoint_10901+1] === this.CELL_EMPTY
                && map[yPoint_10901+1][xPoint_10901+1] === this.CELL_EMPTY
                && map[yPoint_10901+1][xPoint_10901] === this.CELL_EMPTY
                && map[yPoint_10901+1][xPoint_10901-1] === this.CELL_EMPTY
                && map[yPoint_10901][xPoint_10901-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10901, yPoint_10901, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10901;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10901][x] === this.CELL_EMPTY
                    && map[yPoint_10901-1][x] === this.CELL_EMPTY
                    && map[yPoint_10901-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10901][x+1] === this.CELL_EMPTY
                    && map[yPoint_10901+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10901+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10901, yPoint_10901, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10901;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10901] === this.CELL_EMPTY
                    && map[y+1][xPoint_10901] === this.CELL_EMPTY
                    && map[y+1][xPoint_10901+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10901] === this.CELL_EMPTY
                    && map[y][xPoint_10901-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10901-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
