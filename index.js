...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5223=0;yPoint_5223<this.gameFieldBorderY.length; yPoint_5223++){
                for(var xPoint_5223=0;xPoint_5223<this.gameFieldBorderX.length; xPoint_5223++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5223, xPoint_5223);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5223][xPoint_5223] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5223, xPoint_5223, 'user');
                    if(this._userShipsMap[yPoint_5223][xPoint_5223] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5223, xPoint_5223, type){
            var id = this.getPointBlockIdByCoords(yPoint_5223, xPoint_5223, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5223);
                block.setAttribute('data-y', yPoint_5223);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5223
         * @param {type} xPoint_5223
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5223, xPoint_5223, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5223 + '_y' + yPoint_5223;
            }
            return 'pc_x' + xPoint_5223 + '_y' + yPoint_5223;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5223=0;yPoint_5223<this.gameFieldBorderY.length; yPoint_5223++){
                for(var xPoint_5223=0;xPoint_5223<this.gameFieldBorderX.length; xPoint_5223++){
                    map.push({y: yPoint_5223, x: xPoint_5223});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5223=-1;yPoint_5223<(this.gameFieldBorderY.length+1); yPoint_5223++){
                for(var xPoint_5223=-1;xPoint_5223<(this.gameFieldBorderX.length+1); xPoint_5223++){
                    if(!map[yPoint_5223]){
                        map[yPoint_5223] = [];
                    }
                    map[yPoint_5223][xPoint_5223] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5223 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5223 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5223, yPoint_5223) === true){
                    if(this.canPutHorizontal(map, xPoint_5223, yPoint_5223, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5223][xPoint_5223 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5223, yPoint_5223, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5223 + i][xPoint_5223] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5223, yPoint_5223){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5223][xPoint_5223] === this.CELL_EMPTY
                && map[yPoint_5223-1][xPoint_5223] === this.CELL_EMPTY
                && map[yPoint_5223-1][xPoint_5223+1] === this.CELL_EMPTY
                && map[yPoint_5223][xPoint_5223+1] === this.CELL_EMPTY
                && map[yPoint_5223+1][xPoint_5223+1] === this.CELL_EMPTY
                && map[yPoint_5223+1][xPoint_5223] === this.CELL_EMPTY
                && map[yPoint_5223+1][xPoint_5223-1] === this.CELL_EMPTY
                && map[yPoint_5223][xPoint_5223-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5223, yPoint_5223, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5223;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5223][x] === this.CELL_EMPTY
                    && map[yPoint_5223-1][x] === this.CELL_EMPTY
                    && map[yPoint_5223-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5223][x+1] === this.CELL_EMPTY
                    && map[yPoint_5223+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5223+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5223, yPoint_5223, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5223;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5223] === this.CELL_EMPTY
                    && map[y+1][xPoint_5223] === this.CELL_EMPTY
                    && map[y+1][xPoint_5223+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5223] === this.CELL_EMPTY
                    && map[y][xPoint_5223-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5223-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
