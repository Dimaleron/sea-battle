...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_801=0;yPoint_801<this.gameFieldBorderY.length; yPoint_801++){
                for(var xPoint_801=0;xPoint_801<this.gameFieldBorderX.length; xPoint_801++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_801, xPoint_801);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_801][xPoint_801] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_801, xPoint_801, 'user');
                    if(this._userShipsMap[yPoint_801][xPoint_801] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_801, xPoint_801, type){
            var id = this.getPointBlockIdByCoords(yPoint_801, xPoint_801, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_801);
                block.setAttribute('data-y', yPoint_801);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_801
         * @param {type} xPoint_801
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_801, xPoint_801, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_801 + '_y' + yPoint_801;
            }
            return 'pc_x' + xPoint_801 + '_y' + yPoint_801;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_801=0;yPoint_801<this.gameFieldBorderY.length; yPoint_801++){
                for(var xPoint_801=0;xPoint_801<this.gameFieldBorderX.length; xPoint_801++){
                    map.push({y: yPoint_801, x: xPoint_801});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_801=-1;yPoint_801<(this.gameFieldBorderY.length+1); yPoint_801++){
                for(var xPoint_801=-1;xPoint_801<(this.gameFieldBorderX.length+1); xPoint_801++){
                    if(!map[yPoint_801]){
                        map[yPoint_801] = [];
                    }
                    map[yPoint_801][xPoint_801] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_801 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_801 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_801, yPoint_801) === true){
                    if(this.canPutHorizontal(map, xPoint_801, yPoint_801, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_801][xPoint_801 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_801, yPoint_801, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_801 + i][xPoint_801] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_801, yPoint_801){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_801][xPoint_801] === this.CELL_EMPTY
                && map[yPoint_801-1][xPoint_801] === this.CELL_EMPTY
                && map[yPoint_801-1][xPoint_801+1] === this.CELL_EMPTY
                && map[yPoint_801][xPoint_801+1] === this.CELL_EMPTY
                && map[yPoint_801+1][xPoint_801+1] === this.CELL_EMPTY
                && map[yPoint_801+1][xPoint_801] === this.CELL_EMPTY
                && map[yPoint_801+1][xPoint_801-1] === this.CELL_EMPTY
                && map[yPoint_801][xPoint_801-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_801, yPoint_801, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_801;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_801][x] === this.CELL_EMPTY
                    && map[yPoint_801-1][x] === this.CELL_EMPTY
                    && map[yPoint_801-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_801][x+1] === this.CELL_EMPTY
                    && map[yPoint_801+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_801+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_801, yPoint_801, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_801;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_801] === this.CELL_EMPTY
                    && map[y+1][xPoint_801] === this.CELL_EMPTY
                    && map[y+1][xPoint_801+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_801] === this.CELL_EMPTY
                    && map[y][xPoint_801-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_801-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
