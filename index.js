...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11159=0;yPoint_11159<this.gameFieldBorderY.length; yPoint_11159++){
                for(var xPoint_11159=0;xPoint_11159<this.gameFieldBorderX.length; xPoint_11159++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11159, xPoint_11159);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11159][xPoint_11159] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11159, xPoint_11159, 'user');
                    if(this._userShipsMap[yPoint_11159][xPoint_11159] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11159, xPoint_11159, type){
            var id = this.getPointBlockIdByCoords(yPoint_11159, xPoint_11159, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11159);
                block.setAttribute('data-y', yPoint_11159);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11159
         * @param {type} xPoint_11159
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11159, xPoint_11159, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11159 + '_y' + yPoint_11159;
            }
            return 'pc_x' + xPoint_11159 + '_y' + yPoint_11159;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11159=0;yPoint_11159<this.gameFieldBorderY.length; yPoint_11159++){
                for(var xPoint_11159=0;xPoint_11159<this.gameFieldBorderX.length; xPoint_11159++){
                    map.push({y: yPoint_11159, x: xPoint_11159});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11159=-1;yPoint_11159<(this.gameFieldBorderY.length+1); yPoint_11159++){
                for(var xPoint_11159=-1;xPoint_11159<(this.gameFieldBorderX.length+1); xPoint_11159++){
                    if(!map[yPoint_11159]){
                        map[yPoint_11159] = [];
                    }
                    map[yPoint_11159][xPoint_11159] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11159 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11159 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11159, yPoint_11159) === true){
                    if(this.canPutHorizontal(map, xPoint_11159, yPoint_11159, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11159][xPoint_11159 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11159, yPoint_11159, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11159 + i][xPoint_11159] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11159, yPoint_11159){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11159][xPoint_11159] === this.CELL_EMPTY
                && map[yPoint_11159-1][xPoint_11159] === this.CELL_EMPTY
                && map[yPoint_11159-1][xPoint_11159+1] === this.CELL_EMPTY
                && map[yPoint_11159][xPoint_11159+1] === this.CELL_EMPTY
                && map[yPoint_11159+1][xPoint_11159+1] === this.CELL_EMPTY
                && map[yPoint_11159+1][xPoint_11159] === this.CELL_EMPTY
                && map[yPoint_11159+1][xPoint_11159-1] === this.CELL_EMPTY
                && map[yPoint_11159][xPoint_11159-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11159, yPoint_11159, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11159;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11159][x] === this.CELL_EMPTY
                    && map[yPoint_11159-1][x] === this.CELL_EMPTY
                    && map[yPoint_11159-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11159][x+1] === this.CELL_EMPTY
                    && map[yPoint_11159+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11159+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11159, yPoint_11159, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11159;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11159] === this.CELL_EMPTY
                    && map[y+1][xPoint_11159] === this.CELL_EMPTY
                    && map[y+1][xPoint_11159+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11159] === this.CELL_EMPTY
                    && map[y][xPoint_11159-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11159-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
