...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7952=0;yPoint_7952<this.gameFieldBorderY.length; yPoint_7952++){
                for(var xPoint_7952=0;xPoint_7952<this.gameFieldBorderX.length; xPoint_7952++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7952, xPoint_7952);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7952][xPoint_7952] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7952, xPoint_7952, 'user');
                    if(this._userShipsMap[yPoint_7952][xPoint_7952] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7952, xPoint_7952, type){
            var id = this.getPointBlockIdByCoords(yPoint_7952, xPoint_7952, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7952);
                block.setAttribute('data-y', yPoint_7952);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7952
         * @param {type} xPoint_7952
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7952, xPoint_7952, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7952 + '_y' + yPoint_7952;
            }
            return 'pc_x' + xPoint_7952 + '_y' + yPoint_7952;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7952=0;yPoint_7952<this.gameFieldBorderY.length; yPoint_7952++){
                for(var xPoint_7952=0;xPoint_7952<this.gameFieldBorderX.length; xPoint_7952++){
                    map.push({y: yPoint_7952, x: xPoint_7952});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7952=-1;yPoint_7952<(this.gameFieldBorderY.length+1); yPoint_7952++){
                for(var xPoint_7952=-1;xPoint_7952<(this.gameFieldBorderX.length+1); xPoint_7952++){
                    if(!map[yPoint_7952]){
                        map[yPoint_7952] = [];
                    }
                    map[yPoint_7952][xPoint_7952] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7952 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7952 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7952, yPoint_7952) === true){
                    if(this.canPutHorizontal(map, xPoint_7952, yPoint_7952, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7952][xPoint_7952 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7952, yPoint_7952, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7952 + i][xPoint_7952] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7952, yPoint_7952){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7952][xPoint_7952] === this.CELL_EMPTY
                && map[yPoint_7952-1][xPoint_7952] === this.CELL_EMPTY
                && map[yPoint_7952-1][xPoint_7952+1] === this.CELL_EMPTY
                && map[yPoint_7952][xPoint_7952+1] === this.CELL_EMPTY
                && map[yPoint_7952+1][xPoint_7952+1] === this.CELL_EMPTY
                && map[yPoint_7952+1][xPoint_7952] === this.CELL_EMPTY
                && map[yPoint_7952+1][xPoint_7952-1] === this.CELL_EMPTY
                && map[yPoint_7952][xPoint_7952-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7952, yPoint_7952, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7952;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7952][x] === this.CELL_EMPTY
                    && map[yPoint_7952-1][x] === this.CELL_EMPTY
                    && map[yPoint_7952-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7952][x+1] === this.CELL_EMPTY
                    && map[yPoint_7952+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7952+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7952, yPoint_7952, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7952;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7952] === this.CELL_EMPTY
                    && map[y+1][xPoint_7952] === this.CELL_EMPTY
                    && map[y+1][xPoint_7952+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7952] === this.CELL_EMPTY
                    && map[y][xPoint_7952-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7952-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
