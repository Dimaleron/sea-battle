...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22331=0;yPoint_22331<this.gameFieldBorderY.length; yPoint_22331++){
                for(var xPoint_22331=0;xPoint_22331<this.gameFieldBorderX.length; xPoint_22331++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22331, xPoint_22331);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22331][xPoint_22331] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22331, xPoint_22331, 'user');
                    if(this._userShipsMap[yPoint_22331][xPoint_22331] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22331, xPoint_22331, type){
            var id = this.getPointBlockIdByCoords(yPoint_22331, xPoint_22331, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22331);
                block.setAttribute('data-y', yPoint_22331);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22331
         * @param {type} xPoint_22331
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22331, xPoint_22331, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22331 + '_y' + yPoint_22331;
            }
            return 'pc_x' + xPoint_22331 + '_y' + yPoint_22331;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22331=0;yPoint_22331<this.gameFieldBorderY.length; yPoint_22331++){
                for(var xPoint_22331=0;xPoint_22331<this.gameFieldBorderX.length; xPoint_22331++){
                    map.push({y: yPoint_22331, x: xPoint_22331});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22331=-1;yPoint_22331<(this.gameFieldBorderY.length+1); yPoint_22331++){
                for(var xPoint_22331=-1;xPoint_22331<(this.gameFieldBorderX.length+1); xPoint_22331++){
                    if(!map[yPoint_22331]){
                        map[yPoint_22331] = [];
                    }
                    map[yPoint_22331][xPoint_22331] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22331 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22331 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22331, yPoint_22331) === true){
                    if(this.canPutHorizontal(map, xPoint_22331, yPoint_22331, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22331][xPoint_22331 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22331, yPoint_22331, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22331 + i][xPoint_22331] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22331, yPoint_22331){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22331][xPoint_22331] === this.CELL_EMPTY
                && map[yPoint_22331-1][xPoint_22331] === this.CELL_EMPTY
                && map[yPoint_22331-1][xPoint_22331+1] === this.CELL_EMPTY
                && map[yPoint_22331][xPoint_22331+1] === this.CELL_EMPTY
                && map[yPoint_22331+1][xPoint_22331+1] === this.CELL_EMPTY
                && map[yPoint_22331+1][xPoint_22331] === this.CELL_EMPTY
                && map[yPoint_22331+1][xPoint_22331-1] === this.CELL_EMPTY
                && map[yPoint_22331][xPoint_22331-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22331, yPoint_22331, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22331;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22331][x] === this.CELL_EMPTY
                    && map[yPoint_22331-1][x] === this.CELL_EMPTY
                    && map[yPoint_22331-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22331][x+1] === this.CELL_EMPTY
                    && map[yPoint_22331+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22331+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22331, yPoint_22331, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22331;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22331] === this.CELL_EMPTY
                    && map[y+1][xPoint_22331] === this.CELL_EMPTY
                    && map[y+1][xPoint_22331+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22331] === this.CELL_EMPTY
                    && map[y][xPoint_22331-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22331-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
