...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20061=0;yPoint_20061<this.gameFieldBorderY.length; yPoint_20061++){
                for(var xPoint_20061=0;xPoint_20061<this.gameFieldBorderX.length; xPoint_20061++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20061, xPoint_20061);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20061][xPoint_20061] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20061, xPoint_20061, 'user');
                    if(this._userShipsMap[yPoint_20061][xPoint_20061] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20061, xPoint_20061, type){
            var id = this.getPointBlockIdByCoords(yPoint_20061, xPoint_20061, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20061);
                block.setAttribute('data-y', yPoint_20061);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20061
         * @param {type} xPoint_20061
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20061, xPoint_20061, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20061 + '_y' + yPoint_20061;
            }
            return 'pc_x' + xPoint_20061 + '_y' + yPoint_20061;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20061=0;yPoint_20061<this.gameFieldBorderY.length; yPoint_20061++){
                for(var xPoint_20061=0;xPoint_20061<this.gameFieldBorderX.length; xPoint_20061++){
                    map.push({y: yPoint_20061, x: xPoint_20061});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20061=-1;yPoint_20061<(this.gameFieldBorderY.length+1); yPoint_20061++){
                for(var xPoint_20061=-1;xPoint_20061<(this.gameFieldBorderX.length+1); xPoint_20061++){
                    if(!map[yPoint_20061]){
                        map[yPoint_20061] = [];
                    }
                    map[yPoint_20061][xPoint_20061] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20061 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20061 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20061, yPoint_20061) === true){
                    if(this.canPutHorizontal(map, xPoint_20061, yPoint_20061, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20061][xPoint_20061 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20061, yPoint_20061, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20061 + i][xPoint_20061] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20061, yPoint_20061){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20061][xPoint_20061] === this.CELL_EMPTY
                && map[yPoint_20061-1][xPoint_20061] === this.CELL_EMPTY
                && map[yPoint_20061-1][xPoint_20061+1] === this.CELL_EMPTY
                && map[yPoint_20061][xPoint_20061+1] === this.CELL_EMPTY
                && map[yPoint_20061+1][xPoint_20061+1] === this.CELL_EMPTY
                && map[yPoint_20061+1][xPoint_20061] === this.CELL_EMPTY
                && map[yPoint_20061+1][xPoint_20061-1] === this.CELL_EMPTY
                && map[yPoint_20061][xPoint_20061-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20061, yPoint_20061, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20061;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20061][x] === this.CELL_EMPTY
                    && map[yPoint_20061-1][x] === this.CELL_EMPTY
                    && map[yPoint_20061-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20061][x+1] === this.CELL_EMPTY
                    && map[yPoint_20061+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20061+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20061, yPoint_20061, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20061;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20061] === this.CELL_EMPTY
                    && map[y+1][xPoint_20061] === this.CELL_EMPTY
                    && map[y+1][xPoint_20061+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20061] === this.CELL_EMPTY
                    && map[y][xPoint_20061-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20061-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
