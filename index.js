...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8337=0;yPoint_8337<this.gameFieldBorderY.length; yPoint_8337++){
                for(var xPoint_8337=0;xPoint_8337<this.gameFieldBorderX.length; xPoint_8337++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8337, xPoint_8337);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8337][xPoint_8337] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8337, xPoint_8337, 'user');
                    if(this._userShipsMap[yPoint_8337][xPoint_8337] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8337, xPoint_8337, type){
            var id = this.getPointBlockIdByCoords(yPoint_8337, xPoint_8337, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8337);
                block.setAttribute('data-y', yPoint_8337);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8337
         * @param {type} xPoint_8337
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8337, xPoint_8337, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8337 + '_y' + yPoint_8337;
            }
            return 'pc_x' + xPoint_8337 + '_y' + yPoint_8337;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8337=0;yPoint_8337<this.gameFieldBorderY.length; yPoint_8337++){
                for(var xPoint_8337=0;xPoint_8337<this.gameFieldBorderX.length; xPoint_8337++){
                    map.push({y: yPoint_8337, x: xPoint_8337});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8337=-1;yPoint_8337<(this.gameFieldBorderY.length+1); yPoint_8337++){
                for(var xPoint_8337=-1;xPoint_8337<(this.gameFieldBorderX.length+1); xPoint_8337++){
                    if(!map[yPoint_8337]){
                        map[yPoint_8337] = [];
                    }
                    map[yPoint_8337][xPoint_8337] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8337 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8337 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8337, yPoint_8337) === true){
                    if(this.canPutHorizontal(map, xPoint_8337, yPoint_8337, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8337][xPoint_8337 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8337, yPoint_8337, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8337 + i][xPoint_8337] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8337, yPoint_8337){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8337][xPoint_8337] === this.CELL_EMPTY
                && map[yPoint_8337-1][xPoint_8337] === this.CELL_EMPTY
                && map[yPoint_8337-1][xPoint_8337+1] === this.CELL_EMPTY
                && map[yPoint_8337][xPoint_8337+1] === this.CELL_EMPTY
                && map[yPoint_8337+1][xPoint_8337+1] === this.CELL_EMPTY
                && map[yPoint_8337+1][xPoint_8337] === this.CELL_EMPTY
                && map[yPoint_8337+1][xPoint_8337-1] === this.CELL_EMPTY
                && map[yPoint_8337][xPoint_8337-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8337, yPoint_8337, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8337;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8337][x] === this.CELL_EMPTY
                    && map[yPoint_8337-1][x] === this.CELL_EMPTY
                    && map[yPoint_8337-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8337][x+1] === this.CELL_EMPTY
                    && map[yPoint_8337+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8337+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8337, yPoint_8337, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8337;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8337] === this.CELL_EMPTY
                    && map[y+1][xPoint_8337] === this.CELL_EMPTY
                    && map[y+1][xPoint_8337+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8337] === this.CELL_EMPTY
                    && map[y][xPoint_8337-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8337-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
