...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21455=0;yPoint_21455<this.gameFieldBorderY.length; yPoint_21455++){
                for(var xPoint_21455=0;xPoint_21455<this.gameFieldBorderX.length; xPoint_21455++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21455, xPoint_21455);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21455][xPoint_21455] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21455, xPoint_21455, 'user');
                    if(this._userShipsMap[yPoint_21455][xPoint_21455] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21455, xPoint_21455, type){
            var id = this.getPointBlockIdByCoords(yPoint_21455, xPoint_21455, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21455);
                block.setAttribute('data-y', yPoint_21455);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21455
         * @param {type} xPoint_21455
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21455, xPoint_21455, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21455 + '_y' + yPoint_21455;
            }
            return 'pc_x' + xPoint_21455 + '_y' + yPoint_21455;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21455=0;yPoint_21455<this.gameFieldBorderY.length; yPoint_21455++){
                for(var xPoint_21455=0;xPoint_21455<this.gameFieldBorderX.length; xPoint_21455++){
                    map.push({y: yPoint_21455, x: xPoint_21455});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21455=-1;yPoint_21455<(this.gameFieldBorderY.length+1); yPoint_21455++){
                for(var xPoint_21455=-1;xPoint_21455<(this.gameFieldBorderX.length+1); xPoint_21455++){
                    if(!map[yPoint_21455]){
                        map[yPoint_21455] = [];
                    }
                    map[yPoint_21455][xPoint_21455] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21455 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21455 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21455, yPoint_21455) === true){
                    if(this.canPutHorizontal(map, xPoint_21455, yPoint_21455, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21455][xPoint_21455 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21455, yPoint_21455, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21455 + i][xPoint_21455] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21455, yPoint_21455){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21455][xPoint_21455] === this.CELL_EMPTY
                && map[yPoint_21455-1][xPoint_21455] === this.CELL_EMPTY
                && map[yPoint_21455-1][xPoint_21455+1] === this.CELL_EMPTY
                && map[yPoint_21455][xPoint_21455+1] === this.CELL_EMPTY
                && map[yPoint_21455+1][xPoint_21455+1] === this.CELL_EMPTY
                && map[yPoint_21455+1][xPoint_21455] === this.CELL_EMPTY
                && map[yPoint_21455+1][xPoint_21455-1] === this.CELL_EMPTY
                && map[yPoint_21455][xPoint_21455-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21455, yPoint_21455, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21455;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21455][x] === this.CELL_EMPTY
                    && map[yPoint_21455-1][x] === this.CELL_EMPTY
                    && map[yPoint_21455-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21455][x+1] === this.CELL_EMPTY
                    && map[yPoint_21455+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21455+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21455, yPoint_21455, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21455;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21455] === this.CELL_EMPTY
                    && map[y+1][xPoint_21455] === this.CELL_EMPTY
                    && map[y+1][xPoint_21455+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21455] === this.CELL_EMPTY
                    && map[y][xPoint_21455-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21455-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
