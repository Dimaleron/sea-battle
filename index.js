...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3154=0;yPoint_3154<this.gameFieldBorderY.length; yPoint_3154++){
                for(var xPoint_3154=0;xPoint_3154<this.gameFieldBorderX.length; xPoint_3154++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3154, xPoint_3154);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3154][xPoint_3154] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3154, xPoint_3154, 'user');
                    if(this._userShipsMap[yPoint_3154][xPoint_3154] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3154, xPoint_3154, type){
            var id = this.getPointBlockIdByCoords(yPoint_3154, xPoint_3154, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3154);
                block.setAttribute('data-y', yPoint_3154);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3154
         * @param {type} xPoint_3154
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3154, xPoint_3154, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3154 + '_y' + yPoint_3154;
            }
            return 'pc_x' + xPoint_3154 + '_y' + yPoint_3154;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3154=0;yPoint_3154<this.gameFieldBorderY.length; yPoint_3154++){
                for(var xPoint_3154=0;xPoint_3154<this.gameFieldBorderX.length; xPoint_3154++){
                    map.push({y: yPoint_3154, x: xPoint_3154});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3154=-1;yPoint_3154<(this.gameFieldBorderY.length+1); yPoint_3154++){
                for(var xPoint_3154=-1;xPoint_3154<(this.gameFieldBorderX.length+1); xPoint_3154++){
                    if(!map[yPoint_3154]){
                        map[yPoint_3154] = [];
                    }
                    map[yPoint_3154][xPoint_3154] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3154 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3154 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3154, yPoint_3154) === true){
                    if(this.canPutHorizontal(map, xPoint_3154, yPoint_3154, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3154][xPoint_3154 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3154, yPoint_3154, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3154 + i][xPoint_3154] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3154, yPoint_3154){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3154][xPoint_3154] === this.CELL_EMPTY
                && map[yPoint_3154-1][xPoint_3154] === this.CELL_EMPTY
                && map[yPoint_3154-1][xPoint_3154+1] === this.CELL_EMPTY
                && map[yPoint_3154][xPoint_3154+1] === this.CELL_EMPTY
                && map[yPoint_3154+1][xPoint_3154+1] === this.CELL_EMPTY
                && map[yPoint_3154+1][xPoint_3154] === this.CELL_EMPTY
                && map[yPoint_3154+1][xPoint_3154-1] === this.CELL_EMPTY
                && map[yPoint_3154][xPoint_3154-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3154, yPoint_3154, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3154;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3154][x] === this.CELL_EMPTY
                    && map[yPoint_3154-1][x] === this.CELL_EMPTY
                    && map[yPoint_3154-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3154][x+1] === this.CELL_EMPTY
                    && map[yPoint_3154+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3154+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3154, yPoint_3154, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3154;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3154] === this.CELL_EMPTY
                    && map[y+1][xPoint_3154] === this.CELL_EMPTY
                    && map[y+1][xPoint_3154+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3154] === this.CELL_EMPTY
                    && map[y][xPoint_3154-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3154-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
