...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26166=0;yPoint_26166<this.gameFieldBorderY.length; yPoint_26166++){
                for(var xPoint_26166=0;xPoint_26166<this.gameFieldBorderX.length; xPoint_26166++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26166, xPoint_26166);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26166][xPoint_26166] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26166, xPoint_26166, 'user');
                    if(this._userShipsMap[yPoint_26166][xPoint_26166] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26166, xPoint_26166, type){
            var id = this.getPointBlockIdByCoords(yPoint_26166, xPoint_26166, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26166);
                block.setAttribute('data-y', yPoint_26166);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26166
         * @param {type} xPoint_26166
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26166, xPoint_26166, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26166 + '_y' + yPoint_26166;
            }
            return 'pc_x' + xPoint_26166 + '_y' + yPoint_26166;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26166=0;yPoint_26166<this.gameFieldBorderY.length; yPoint_26166++){
                for(var xPoint_26166=0;xPoint_26166<this.gameFieldBorderX.length; xPoint_26166++){
                    map.push({y: yPoint_26166, x: xPoint_26166});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26166=-1;yPoint_26166<(this.gameFieldBorderY.length+1); yPoint_26166++){
                for(var xPoint_26166=-1;xPoint_26166<(this.gameFieldBorderX.length+1); xPoint_26166++){
                    if(!map[yPoint_26166]){
                        map[yPoint_26166] = [];
                    }
                    map[yPoint_26166][xPoint_26166] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26166 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26166 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26166, yPoint_26166) === true){
                    if(this.canPutHorizontal(map, xPoint_26166, yPoint_26166, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26166][xPoint_26166 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26166, yPoint_26166, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26166 + i][xPoint_26166] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26166, yPoint_26166){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26166][xPoint_26166] === this.CELL_EMPTY
                && map[yPoint_26166-1][xPoint_26166] === this.CELL_EMPTY
                && map[yPoint_26166-1][xPoint_26166+1] === this.CELL_EMPTY
                && map[yPoint_26166][xPoint_26166+1] === this.CELL_EMPTY
                && map[yPoint_26166+1][xPoint_26166+1] === this.CELL_EMPTY
                && map[yPoint_26166+1][xPoint_26166] === this.CELL_EMPTY
                && map[yPoint_26166+1][xPoint_26166-1] === this.CELL_EMPTY
                && map[yPoint_26166][xPoint_26166-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26166, yPoint_26166, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26166;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26166][x] === this.CELL_EMPTY
                    && map[yPoint_26166-1][x] === this.CELL_EMPTY
                    && map[yPoint_26166-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26166][x+1] === this.CELL_EMPTY
                    && map[yPoint_26166+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26166+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26166, yPoint_26166, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26166;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26166] === this.CELL_EMPTY
                    && map[y+1][xPoint_26166] === this.CELL_EMPTY
                    && map[y+1][xPoint_26166+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26166] === this.CELL_EMPTY
                    && map[y][xPoint_26166-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26166-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
