...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_591=0;yPoint_591<this.gameFieldBorderY.length; yPoint_591++){
                for(var xPoint_591=0;xPoint_591<this.gameFieldBorderX.length; xPoint_591++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_591, xPoint_591);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_591][xPoint_591] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_591, xPoint_591, 'user');
                    if(this._userShipsMap[yPoint_591][xPoint_591] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_591, xPoint_591, type){
            var id = this.getPointBlockIdByCoords(yPoint_591, xPoint_591, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_591);
                block.setAttribute('data-y', yPoint_591);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_591
         * @param {type} xPoint_591
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_591, xPoint_591, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_591 + '_y' + yPoint_591;
            }
            return 'pc_x' + xPoint_591 + '_y' + yPoint_591;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_591=0;yPoint_591<this.gameFieldBorderY.length; yPoint_591++){
                for(var xPoint_591=0;xPoint_591<this.gameFieldBorderX.length; xPoint_591++){
                    map.push({y: yPoint_591, x: xPoint_591});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_591=-1;yPoint_591<(this.gameFieldBorderY.length+1); yPoint_591++){
                for(var xPoint_591=-1;xPoint_591<(this.gameFieldBorderX.length+1); xPoint_591++){
                    if(!map[yPoint_591]){
                        map[yPoint_591] = [];
                    }
                    map[yPoint_591][xPoint_591] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_591 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_591 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_591, yPoint_591) === true){
                    if(this.canPutHorizontal(map, xPoint_591, yPoint_591, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_591][xPoint_591 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_591, yPoint_591, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_591 + i][xPoint_591] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_591, yPoint_591){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_591][xPoint_591] === this.CELL_EMPTY
                && map[yPoint_591-1][xPoint_591] === this.CELL_EMPTY
                && map[yPoint_591-1][xPoint_591+1] === this.CELL_EMPTY
                && map[yPoint_591][xPoint_591+1] === this.CELL_EMPTY
                && map[yPoint_591+1][xPoint_591+1] === this.CELL_EMPTY
                && map[yPoint_591+1][xPoint_591] === this.CELL_EMPTY
                && map[yPoint_591+1][xPoint_591-1] === this.CELL_EMPTY
                && map[yPoint_591][xPoint_591-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_591, yPoint_591, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_591;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_591][x] === this.CELL_EMPTY
                    && map[yPoint_591-1][x] === this.CELL_EMPTY
                    && map[yPoint_591-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_591][x+1] === this.CELL_EMPTY
                    && map[yPoint_591+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_591+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_591, yPoint_591, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_591;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_591] === this.CELL_EMPTY
                    && map[y+1][xPoint_591] === this.CELL_EMPTY
                    && map[y+1][xPoint_591+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_591] === this.CELL_EMPTY
                    && map[y][xPoint_591-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_591-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
