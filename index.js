...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18968=0;yPoint_18968<this.gameFieldBorderY.length; yPoint_18968++){
                for(var xPoint_18968=0;xPoint_18968<this.gameFieldBorderX.length; xPoint_18968++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18968, xPoint_18968);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18968][xPoint_18968] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18968, xPoint_18968, 'user');
                    if(this._userShipsMap[yPoint_18968][xPoint_18968] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18968, xPoint_18968, type){
            var id = this.getPointBlockIdByCoords(yPoint_18968, xPoint_18968, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18968);
                block.setAttribute('data-y', yPoint_18968);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18968
         * @param {type} xPoint_18968
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18968, xPoint_18968, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18968 + '_y' + yPoint_18968;
            }
            return 'pc_x' + xPoint_18968 + '_y' + yPoint_18968;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18968=0;yPoint_18968<this.gameFieldBorderY.length; yPoint_18968++){
                for(var xPoint_18968=0;xPoint_18968<this.gameFieldBorderX.length; xPoint_18968++){
                    map.push({y: yPoint_18968, x: xPoint_18968});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18968=-1;yPoint_18968<(this.gameFieldBorderY.length+1); yPoint_18968++){
                for(var xPoint_18968=-1;xPoint_18968<(this.gameFieldBorderX.length+1); xPoint_18968++){
                    if(!map[yPoint_18968]){
                        map[yPoint_18968] = [];
                    }
                    map[yPoint_18968][xPoint_18968] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18968 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18968 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18968, yPoint_18968) === true){
                    if(this.canPutHorizontal(map, xPoint_18968, yPoint_18968, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18968][xPoint_18968 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18968, yPoint_18968, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18968 + i][xPoint_18968] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18968, yPoint_18968){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18968][xPoint_18968] === this.CELL_EMPTY
                && map[yPoint_18968-1][xPoint_18968] === this.CELL_EMPTY
                && map[yPoint_18968-1][xPoint_18968+1] === this.CELL_EMPTY
                && map[yPoint_18968][xPoint_18968+1] === this.CELL_EMPTY
                && map[yPoint_18968+1][xPoint_18968+1] === this.CELL_EMPTY
                && map[yPoint_18968+1][xPoint_18968] === this.CELL_EMPTY
                && map[yPoint_18968+1][xPoint_18968-1] === this.CELL_EMPTY
                && map[yPoint_18968][xPoint_18968-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18968, yPoint_18968, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18968;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18968][x] === this.CELL_EMPTY
                    && map[yPoint_18968-1][x] === this.CELL_EMPTY
                    && map[yPoint_18968-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18968][x+1] === this.CELL_EMPTY
                    && map[yPoint_18968+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18968+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18968, yPoint_18968, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18968;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18968] === this.CELL_EMPTY
                    && map[y+1][xPoint_18968] === this.CELL_EMPTY
                    && map[y+1][xPoint_18968+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18968] === this.CELL_EMPTY
                    && map[y][xPoint_18968-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18968-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
