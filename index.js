...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19559=0;yPoint_19559<this.gameFieldBorderY.length; yPoint_19559++){
                for(var xPoint_19559=0;xPoint_19559<this.gameFieldBorderX.length; xPoint_19559++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19559, xPoint_19559);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19559][xPoint_19559] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19559, xPoint_19559, 'user');
                    if(this._userShipsMap[yPoint_19559][xPoint_19559] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19559, xPoint_19559, type){
            var id = this.getPointBlockIdByCoords(yPoint_19559, xPoint_19559, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19559);
                block.setAttribute('data-y', yPoint_19559);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19559
         * @param {type} xPoint_19559
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19559, xPoint_19559, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19559 + '_y' + yPoint_19559;
            }
            return 'pc_x' + xPoint_19559 + '_y' + yPoint_19559;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19559=0;yPoint_19559<this.gameFieldBorderY.length; yPoint_19559++){
                for(var xPoint_19559=0;xPoint_19559<this.gameFieldBorderX.length; xPoint_19559++){
                    map.push({y: yPoint_19559, x: xPoint_19559});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19559=-1;yPoint_19559<(this.gameFieldBorderY.length+1); yPoint_19559++){
                for(var xPoint_19559=-1;xPoint_19559<(this.gameFieldBorderX.length+1); xPoint_19559++){
                    if(!map[yPoint_19559]){
                        map[yPoint_19559] = [];
                    }
                    map[yPoint_19559][xPoint_19559] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19559 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19559 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19559, yPoint_19559) === true){
                    if(this.canPutHorizontal(map, xPoint_19559, yPoint_19559, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19559][xPoint_19559 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19559, yPoint_19559, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19559 + i][xPoint_19559] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19559, yPoint_19559){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19559][xPoint_19559] === this.CELL_EMPTY
                && map[yPoint_19559-1][xPoint_19559] === this.CELL_EMPTY
                && map[yPoint_19559-1][xPoint_19559+1] === this.CELL_EMPTY
                && map[yPoint_19559][xPoint_19559+1] === this.CELL_EMPTY
                && map[yPoint_19559+1][xPoint_19559+1] === this.CELL_EMPTY
                && map[yPoint_19559+1][xPoint_19559] === this.CELL_EMPTY
                && map[yPoint_19559+1][xPoint_19559-1] === this.CELL_EMPTY
                && map[yPoint_19559][xPoint_19559-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19559, yPoint_19559, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19559;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19559][x] === this.CELL_EMPTY
                    && map[yPoint_19559-1][x] === this.CELL_EMPTY
                    && map[yPoint_19559-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19559][x+1] === this.CELL_EMPTY
                    && map[yPoint_19559+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19559+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19559, yPoint_19559, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19559;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19559] === this.CELL_EMPTY
                    && map[y+1][xPoint_19559] === this.CELL_EMPTY
                    && map[y+1][xPoint_19559+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19559] === this.CELL_EMPTY
                    && map[y][xPoint_19559-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19559-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
