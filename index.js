...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31675=0;yPoint_31675<this.gameFieldBorderY.length; yPoint_31675++){
                for(var xPoint_31675=0;xPoint_31675<this.gameFieldBorderX.length; xPoint_31675++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31675, xPoint_31675);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31675][xPoint_31675] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31675, xPoint_31675, 'user');
                    if(this._userShipsMap[yPoint_31675][xPoint_31675] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31675, xPoint_31675, type){
            var id = this.getPointBlockIdByCoords(yPoint_31675, xPoint_31675, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31675);
                block.setAttribute('data-y', yPoint_31675);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31675
         * @param {type} xPoint_31675
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31675, xPoint_31675, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31675 + '_y' + yPoint_31675;
            }
            return 'pc_x' + xPoint_31675 + '_y' + yPoint_31675;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31675=0;yPoint_31675<this.gameFieldBorderY.length; yPoint_31675++){
                for(var xPoint_31675=0;xPoint_31675<this.gameFieldBorderX.length; xPoint_31675++){
                    map.push({y: yPoint_31675, x: xPoint_31675});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31675=-1;yPoint_31675<(this.gameFieldBorderY.length+1); yPoint_31675++){
                for(var xPoint_31675=-1;xPoint_31675<(this.gameFieldBorderX.length+1); xPoint_31675++){
                    if(!map[yPoint_31675]){
                        map[yPoint_31675] = [];
                    }
                    map[yPoint_31675][xPoint_31675] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31675 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31675 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31675, yPoint_31675) === true){
                    if(this.canPutHorizontal(map, xPoint_31675, yPoint_31675, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31675][xPoint_31675 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31675, yPoint_31675, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31675 + i][xPoint_31675] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31675, yPoint_31675){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31675][xPoint_31675] === this.CELL_EMPTY
                && map[yPoint_31675-1][xPoint_31675] === this.CELL_EMPTY
                && map[yPoint_31675-1][xPoint_31675+1] === this.CELL_EMPTY
                && map[yPoint_31675][xPoint_31675+1] === this.CELL_EMPTY
                && map[yPoint_31675+1][xPoint_31675+1] === this.CELL_EMPTY
                && map[yPoint_31675+1][xPoint_31675] === this.CELL_EMPTY
                && map[yPoint_31675+1][xPoint_31675-1] === this.CELL_EMPTY
                && map[yPoint_31675][xPoint_31675-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31675, yPoint_31675, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31675;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31675][x] === this.CELL_EMPTY
                    && map[yPoint_31675-1][x] === this.CELL_EMPTY
                    && map[yPoint_31675-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31675][x+1] === this.CELL_EMPTY
                    && map[yPoint_31675+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31675+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31675, yPoint_31675, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31675;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31675] === this.CELL_EMPTY
                    && map[y+1][xPoint_31675] === this.CELL_EMPTY
                    && map[y+1][xPoint_31675+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31675] === this.CELL_EMPTY
                    && map[y][xPoint_31675-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31675-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
