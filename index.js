...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3288=0;yPoint_3288<this.gameFieldBorderY.length; yPoint_3288++){
                for(var xPoint_3288=0;xPoint_3288<this.gameFieldBorderX.length; xPoint_3288++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3288, xPoint_3288);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3288][xPoint_3288] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3288, xPoint_3288, 'user');
                    if(this._userShipsMap[yPoint_3288][xPoint_3288] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3288, xPoint_3288, type){
            var id = this.getPointBlockIdByCoords(yPoint_3288, xPoint_3288, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3288);
                block.setAttribute('data-y', yPoint_3288);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3288
         * @param {type} xPoint_3288
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3288, xPoint_3288, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3288 + '_y' + yPoint_3288;
            }
            return 'pc_x' + xPoint_3288 + '_y' + yPoint_3288;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3288=0;yPoint_3288<this.gameFieldBorderY.length; yPoint_3288++){
                for(var xPoint_3288=0;xPoint_3288<this.gameFieldBorderX.length; xPoint_3288++){
                    map.push({y: yPoint_3288, x: xPoint_3288});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3288=-1;yPoint_3288<(this.gameFieldBorderY.length+1); yPoint_3288++){
                for(var xPoint_3288=-1;xPoint_3288<(this.gameFieldBorderX.length+1); xPoint_3288++){
                    if(!map[yPoint_3288]){
                        map[yPoint_3288] = [];
                    }
                    map[yPoint_3288][xPoint_3288] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3288 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3288 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3288, yPoint_3288) === true){
                    if(this.canPutHorizontal(map, xPoint_3288, yPoint_3288, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3288][xPoint_3288 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3288, yPoint_3288, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3288 + i][xPoint_3288] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3288, yPoint_3288){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3288][xPoint_3288] === this.CELL_EMPTY
                && map[yPoint_3288-1][xPoint_3288] === this.CELL_EMPTY
                && map[yPoint_3288-1][xPoint_3288+1] === this.CELL_EMPTY
                && map[yPoint_3288][xPoint_3288+1] === this.CELL_EMPTY
                && map[yPoint_3288+1][xPoint_3288+1] === this.CELL_EMPTY
                && map[yPoint_3288+1][xPoint_3288] === this.CELL_EMPTY
                && map[yPoint_3288+1][xPoint_3288-1] === this.CELL_EMPTY
                && map[yPoint_3288][xPoint_3288-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3288, yPoint_3288, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3288;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3288][x] === this.CELL_EMPTY
                    && map[yPoint_3288-1][x] === this.CELL_EMPTY
                    && map[yPoint_3288-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3288][x+1] === this.CELL_EMPTY
                    && map[yPoint_3288+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3288+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3288, yPoint_3288, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3288;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3288] === this.CELL_EMPTY
                    && map[y+1][xPoint_3288] === this.CELL_EMPTY
                    && map[y+1][xPoint_3288+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3288] === this.CELL_EMPTY
                    && map[y][xPoint_3288-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3288-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
