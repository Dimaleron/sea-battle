...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_320=0;yPoint_320<this.gameFieldBorderY.length; yPoint_320++){
                for(var xPoint_320=0;xPoint_320<this.gameFieldBorderX.length; xPoint_320++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_320, xPoint_320);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_320][xPoint_320] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_320, xPoint_320, 'user');
                    if(this._userShipsMap[yPoint_320][xPoint_320] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_320, xPoint_320, type){
            var id = this.getPointBlockIdByCoords(yPoint_320, xPoint_320, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_320);
                block.setAttribute('data-y', yPoint_320);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_320
         * @param {type} xPoint_320
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_320, xPoint_320, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_320 + '_y' + yPoint_320;
            }
            return 'pc_x' + xPoint_320 + '_y' + yPoint_320;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_320=0;yPoint_320<this.gameFieldBorderY.length; yPoint_320++){
                for(var xPoint_320=0;xPoint_320<this.gameFieldBorderX.length; xPoint_320++){
                    map.push({y: yPoint_320, x: xPoint_320});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_320=-1;yPoint_320<(this.gameFieldBorderY.length+1); yPoint_320++){
                for(var xPoint_320=-1;xPoint_320<(this.gameFieldBorderX.length+1); xPoint_320++){
                    if(!map[yPoint_320]){
                        map[yPoint_320] = [];
                    }
                    map[yPoint_320][xPoint_320] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_320 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_320 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_320, yPoint_320) === true){
                    if(this.canPutHorizontal(map, xPoint_320, yPoint_320, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_320][xPoint_320 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_320, yPoint_320, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_320 + i][xPoint_320] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_320, yPoint_320){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_320][xPoint_320] === this.CELL_EMPTY
                && map[yPoint_320-1][xPoint_320] === this.CELL_EMPTY
                && map[yPoint_320-1][xPoint_320+1] === this.CELL_EMPTY
                && map[yPoint_320][xPoint_320+1] === this.CELL_EMPTY
                && map[yPoint_320+1][xPoint_320+1] === this.CELL_EMPTY
                && map[yPoint_320+1][xPoint_320] === this.CELL_EMPTY
                && map[yPoint_320+1][xPoint_320-1] === this.CELL_EMPTY
                && map[yPoint_320][xPoint_320-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_320, yPoint_320, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_320;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_320][x] === this.CELL_EMPTY
                    && map[yPoint_320-1][x] === this.CELL_EMPTY
                    && map[yPoint_320-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_320][x+1] === this.CELL_EMPTY
                    && map[yPoint_320+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_320+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_320, yPoint_320, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_320;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_320] === this.CELL_EMPTY
                    && map[y+1][xPoint_320] === this.CELL_EMPTY
                    && map[y+1][xPoint_320+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_320] === this.CELL_EMPTY
                    && map[y][xPoint_320-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_320-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
