...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21419=0;yPoint_21419<this.gameFieldBorderY.length; yPoint_21419++){
                for(var xPoint_21419=0;xPoint_21419<this.gameFieldBorderX.length; xPoint_21419++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21419, xPoint_21419);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21419][xPoint_21419] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21419, xPoint_21419, 'user');
                    if(this._userShipsMap[yPoint_21419][xPoint_21419] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21419, xPoint_21419, type){
            var id = this.getPointBlockIdByCoords(yPoint_21419, xPoint_21419, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21419);
                block.setAttribute('data-y', yPoint_21419);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21419
         * @param {type} xPoint_21419
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21419, xPoint_21419, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21419 + '_y' + yPoint_21419;
            }
            return 'pc_x' + xPoint_21419 + '_y' + yPoint_21419;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21419=0;yPoint_21419<this.gameFieldBorderY.length; yPoint_21419++){
                for(var xPoint_21419=0;xPoint_21419<this.gameFieldBorderX.length; xPoint_21419++){
                    map.push({y: yPoint_21419, x: xPoint_21419});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21419=-1;yPoint_21419<(this.gameFieldBorderY.length+1); yPoint_21419++){
                for(var xPoint_21419=-1;xPoint_21419<(this.gameFieldBorderX.length+1); xPoint_21419++){
                    if(!map[yPoint_21419]){
                        map[yPoint_21419] = [];
                    }
                    map[yPoint_21419][xPoint_21419] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21419 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21419 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21419, yPoint_21419) === true){
                    if(this.canPutHorizontal(map, xPoint_21419, yPoint_21419, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21419][xPoint_21419 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21419, yPoint_21419, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21419 + i][xPoint_21419] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21419, yPoint_21419){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21419][xPoint_21419] === this.CELL_EMPTY
                && map[yPoint_21419-1][xPoint_21419] === this.CELL_EMPTY
                && map[yPoint_21419-1][xPoint_21419+1] === this.CELL_EMPTY
                && map[yPoint_21419][xPoint_21419+1] === this.CELL_EMPTY
                && map[yPoint_21419+1][xPoint_21419+1] === this.CELL_EMPTY
                && map[yPoint_21419+1][xPoint_21419] === this.CELL_EMPTY
                && map[yPoint_21419+1][xPoint_21419-1] === this.CELL_EMPTY
                && map[yPoint_21419][xPoint_21419-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21419, yPoint_21419, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21419;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21419][x] === this.CELL_EMPTY
                    && map[yPoint_21419-1][x] === this.CELL_EMPTY
                    && map[yPoint_21419-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21419][x+1] === this.CELL_EMPTY
                    && map[yPoint_21419+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21419+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21419, yPoint_21419, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21419;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21419] === this.CELL_EMPTY
                    && map[y+1][xPoint_21419] === this.CELL_EMPTY
                    && map[y+1][xPoint_21419+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21419] === this.CELL_EMPTY
                    && map[y][xPoint_21419-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21419-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
