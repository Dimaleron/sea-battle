...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13558=0;yPoint_13558<this.gameFieldBorderY.length; yPoint_13558++){
                for(var xPoint_13558=0;xPoint_13558<this.gameFieldBorderX.length; xPoint_13558++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13558, xPoint_13558);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13558][xPoint_13558] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13558, xPoint_13558, 'user');
                    if(this._userShipsMap[yPoint_13558][xPoint_13558] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13558, xPoint_13558, type){
            var id = this.getPointBlockIdByCoords(yPoint_13558, xPoint_13558, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13558);
                block.setAttribute('data-y', yPoint_13558);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13558
         * @param {type} xPoint_13558
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13558, xPoint_13558, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13558 + '_y' + yPoint_13558;
            }
            return 'pc_x' + xPoint_13558 + '_y' + yPoint_13558;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13558=0;yPoint_13558<this.gameFieldBorderY.length; yPoint_13558++){
                for(var xPoint_13558=0;xPoint_13558<this.gameFieldBorderX.length; xPoint_13558++){
                    map.push({y: yPoint_13558, x: xPoint_13558});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13558=-1;yPoint_13558<(this.gameFieldBorderY.length+1); yPoint_13558++){
                for(var xPoint_13558=-1;xPoint_13558<(this.gameFieldBorderX.length+1); xPoint_13558++){
                    if(!map[yPoint_13558]){
                        map[yPoint_13558] = [];
                    }
                    map[yPoint_13558][xPoint_13558] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13558 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13558 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13558, yPoint_13558) === true){
                    if(this.canPutHorizontal(map, xPoint_13558, yPoint_13558, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13558][xPoint_13558 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13558, yPoint_13558, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13558 + i][xPoint_13558] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13558, yPoint_13558){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13558][xPoint_13558] === this.CELL_EMPTY
                && map[yPoint_13558-1][xPoint_13558] === this.CELL_EMPTY
                && map[yPoint_13558-1][xPoint_13558+1] === this.CELL_EMPTY
                && map[yPoint_13558][xPoint_13558+1] === this.CELL_EMPTY
                && map[yPoint_13558+1][xPoint_13558+1] === this.CELL_EMPTY
                && map[yPoint_13558+1][xPoint_13558] === this.CELL_EMPTY
                && map[yPoint_13558+1][xPoint_13558-1] === this.CELL_EMPTY
                && map[yPoint_13558][xPoint_13558-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13558, yPoint_13558, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13558;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13558][x] === this.CELL_EMPTY
                    && map[yPoint_13558-1][x] === this.CELL_EMPTY
                    && map[yPoint_13558-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13558][x+1] === this.CELL_EMPTY
                    && map[yPoint_13558+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13558+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13558, yPoint_13558, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13558;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13558] === this.CELL_EMPTY
                    && map[y+1][xPoint_13558] === this.CELL_EMPTY
                    && map[y+1][xPoint_13558+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13558] === this.CELL_EMPTY
                    && map[y][xPoint_13558-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13558-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
