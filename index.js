...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27911=0;yPoint_27911<this.gameFieldBorderY.length; yPoint_27911++){
                for(var xPoint_27911=0;xPoint_27911<this.gameFieldBorderX.length; xPoint_27911++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27911, xPoint_27911);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27911][xPoint_27911] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27911, xPoint_27911, 'user');
                    if(this._userShipsMap[yPoint_27911][xPoint_27911] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27911, xPoint_27911, type){
            var id = this.getPointBlockIdByCoords(yPoint_27911, xPoint_27911, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27911);
                block.setAttribute('data-y', yPoint_27911);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27911
         * @param {type} xPoint_27911
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27911, xPoint_27911, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27911 + '_y' + yPoint_27911;
            }
            return 'pc_x' + xPoint_27911 + '_y' + yPoint_27911;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27911=0;yPoint_27911<this.gameFieldBorderY.length; yPoint_27911++){
                for(var xPoint_27911=0;xPoint_27911<this.gameFieldBorderX.length; xPoint_27911++){
                    map.push({y: yPoint_27911, x: xPoint_27911});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27911=-1;yPoint_27911<(this.gameFieldBorderY.length+1); yPoint_27911++){
                for(var xPoint_27911=-1;xPoint_27911<(this.gameFieldBorderX.length+1); xPoint_27911++){
                    if(!map[yPoint_27911]){
                        map[yPoint_27911] = [];
                    }
                    map[yPoint_27911][xPoint_27911] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27911 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27911 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27911, yPoint_27911) === true){
                    if(this.canPutHorizontal(map, xPoint_27911, yPoint_27911, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27911][xPoint_27911 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27911, yPoint_27911, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27911 + i][xPoint_27911] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27911, yPoint_27911){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27911][xPoint_27911] === this.CELL_EMPTY
                && map[yPoint_27911-1][xPoint_27911] === this.CELL_EMPTY
                && map[yPoint_27911-1][xPoint_27911+1] === this.CELL_EMPTY
                && map[yPoint_27911][xPoint_27911+1] === this.CELL_EMPTY
                && map[yPoint_27911+1][xPoint_27911+1] === this.CELL_EMPTY
                && map[yPoint_27911+1][xPoint_27911] === this.CELL_EMPTY
                && map[yPoint_27911+1][xPoint_27911-1] === this.CELL_EMPTY
                && map[yPoint_27911][xPoint_27911-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27911, yPoint_27911, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27911;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27911][x] === this.CELL_EMPTY
                    && map[yPoint_27911-1][x] === this.CELL_EMPTY
                    && map[yPoint_27911-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27911][x+1] === this.CELL_EMPTY
                    && map[yPoint_27911+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27911+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27911, yPoint_27911, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27911;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27911] === this.CELL_EMPTY
                    && map[y+1][xPoint_27911] === this.CELL_EMPTY
                    && map[y+1][xPoint_27911+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27911] === this.CELL_EMPTY
                    && map[y][xPoint_27911-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27911-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
