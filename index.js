...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19962=0;yPoint_19962<this.gameFieldBorderY.length; yPoint_19962++){
                for(var xPoint_19962=0;xPoint_19962<this.gameFieldBorderX.length; xPoint_19962++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19962, xPoint_19962);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19962][xPoint_19962] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19962, xPoint_19962, 'user');
                    if(this._userShipsMap[yPoint_19962][xPoint_19962] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19962, xPoint_19962, type){
            var id = this.getPointBlockIdByCoords(yPoint_19962, xPoint_19962, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19962);
                block.setAttribute('data-y', yPoint_19962);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19962
         * @param {type} xPoint_19962
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19962, xPoint_19962, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19962 + '_y' + yPoint_19962;
            }
            return 'pc_x' + xPoint_19962 + '_y' + yPoint_19962;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19962=0;yPoint_19962<this.gameFieldBorderY.length; yPoint_19962++){
                for(var xPoint_19962=0;xPoint_19962<this.gameFieldBorderX.length; xPoint_19962++){
                    map.push({y: yPoint_19962, x: xPoint_19962});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19962=-1;yPoint_19962<(this.gameFieldBorderY.length+1); yPoint_19962++){
                for(var xPoint_19962=-1;xPoint_19962<(this.gameFieldBorderX.length+1); xPoint_19962++){
                    if(!map[yPoint_19962]){
                        map[yPoint_19962] = [];
                    }
                    map[yPoint_19962][xPoint_19962] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19962 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19962 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19962, yPoint_19962) === true){
                    if(this.canPutHorizontal(map, xPoint_19962, yPoint_19962, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19962][xPoint_19962 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19962, yPoint_19962, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19962 + i][xPoint_19962] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19962, yPoint_19962){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19962][xPoint_19962] === this.CELL_EMPTY
                && map[yPoint_19962-1][xPoint_19962] === this.CELL_EMPTY
                && map[yPoint_19962-1][xPoint_19962+1] === this.CELL_EMPTY
                && map[yPoint_19962][xPoint_19962+1] === this.CELL_EMPTY
                && map[yPoint_19962+1][xPoint_19962+1] === this.CELL_EMPTY
                && map[yPoint_19962+1][xPoint_19962] === this.CELL_EMPTY
                && map[yPoint_19962+1][xPoint_19962-1] === this.CELL_EMPTY
                && map[yPoint_19962][xPoint_19962-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19962, yPoint_19962, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19962;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19962][x] === this.CELL_EMPTY
                    && map[yPoint_19962-1][x] === this.CELL_EMPTY
                    && map[yPoint_19962-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19962][x+1] === this.CELL_EMPTY
                    && map[yPoint_19962+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19962+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19962, yPoint_19962, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19962;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19962] === this.CELL_EMPTY
                    && map[y+1][xPoint_19962] === this.CELL_EMPTY
                    && map[y+1][xPoint_19962+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19962] === this.CELL_EMPTY
                    && map[y][xPoint_19962-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19962-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
