...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5559=0;yPoint_5559<this.gameFieldBorderY.length; yPoint_5559++){
                for(var xPoint_5559=0;xPoint_5559<this.gameFieldBorderX.length; xPoint_5559++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5559, xPoint_5559);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5559][xPoint_5559] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5559, xPoint_5559, 'user');
                    if(this._userShipsMap[yPoint_5559][xPoint_5559] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5559, xPoint_5559, type){
            var id = this.getPointBlockIdByCoords(yPoint_5559, xPoint_5559, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5559);
                block.setAttribute('data-y', yPoint_5559);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5559
         * @param {type} xPoint_5559
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5559, xPoint_5559, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5559 + '_y' + yPoint_5559;
            }
            return 'pc_x' + xPoint_5559 + '_y' + yPoint_5559;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5559=0;yPoint_5559<this.gameFieldBorderY.length; yPoint_5559++){
                for(var xPoint_5559=0;xPoint_5559<this.gameFieldBorderX.length; xPoint_5559++){
                    map.push({y: yPoint_5559, x: xPoint_5559});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5559=-1;yPoint_5559<(this.gameFieldBorderY.length+1); yPoint_5559++){
                for(var xPoint_5559=-1;xPoint_5559<(this.gameFieldBorderX.length+1); xPoint_5559++){
                    if(!map[yPoint_5559]){
                        map[yPoint_5559] = [];
                    }
                    map[yPoint_5559][xPoint_5559] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5559 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5559 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5559, yPoint_5559) === true){
                    if(this.canPutHorizontal(map, xPoint_5559, yPoint_5559, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5559][xPoint_5559 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5559, yPoint_5559, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5559 + i][xPoint_5559] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5559, yPoint_5559){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5559][xPoint_5559] === this.CELL_EMPTY
                && map[yPoint_5559-1][xPoint_5559] === this.CELL_EMPTY
                && map[yPoint_5559-1][xPoint_5559+1] === this.CELL_EMPTY
                && map[yPoint_5559][xPoint_5559+1] === this.CELL_EMPTY
                && map[yPoint_5559+1][xPoint_5559+1] === this.CELL_EMPTY
                && map[yPoint_5559+1][xPoint_5559] === this.CELL_EMPTY
                && map[yPoint_5559+1][xPoint_5559-1] === this.CELL_EMPTY
                && map[yPoint_5559][xPoint_5559-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5559, yPoint_5559, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5559;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5559][x] === this.CELL_EMPTY
                    && map[yPoint_5559-1][x] === this.CELL_EMPTY
                    && map[yPoint_5559-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5559][x+1] === this.CELL_EMPTY
                    && map[yPoint_5559+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5559+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5559, yPoint_5559, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5559;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5559] === this.CELL_EMPTY
                    && map[y+1][xPoint_5559] === this.CELL_EMPTY
                    && map[y+1][xPoint_5559+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5559] === this.CELL_EMPTY
                    && map[y][xPoint_5559-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5559-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
