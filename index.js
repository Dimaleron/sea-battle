...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25532=0;yPoint_25532<this.gameFieldBorderY.length; yPoint_25532++){
                for(var xPoint_25532=0;xPoint_25532<this.gameFieldBorderX.length; xPoint_25532++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25532, xPoint_25532);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25532][xPoint_25532] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25532, xPoint_25532, 'user');
                    if(this._userShipsMap[yPoint_25532][xPoint_25532] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25532, xPoint_25532, type){
            var id = this.getPointBlockIdByCoords(yPoint_25532, xPoint_25532, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25532);
                block.setAttribute('data-y', yPoint_25532);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25532
         * @param {type} xPoint_25532
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25532, xPoint_25532, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25532 + '_y' + yPoint_25532;
            }
            return 'pc_x' + xPoint_25532 + '_y' + yPoint_25532;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25532=0;yPoint_25532<this.gameFieldBorderY.length; yPoint_25532++){
                for(var xPoint_25532=0;xPoint_25532<this.gameFieldBorderX.length; xPoint_25532++){
                    map.push({y: yPoint_25532, x: xPoint_25532});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25532=-1;yPoint_25532<(this.gameFieldBorderY.length+1); yPoint_25532++){
                for(var xPoint_25532=-1;xPoint_25532<(this.gameFieldBorderX.length+1); xPoint_25532++){
                    if(!map[yPoint_25532]){
                        map[yPoint_25532] = [];
                    }
                    map[yPoint_25532][xPoint_25532] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25532 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25532 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25532, yPoint_25532) === true){
                    if(this.canPutHorizontal(map, xPoint_25532, yPoint_25532, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25532][xPoint_25532 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25532, yPoint_25532, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25532 + i][xPoint_25532] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25532, yPoint_25532){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25532][xPoint_25532] === this.CELL_EMPTY
                && map[yPoint_25532-1][xPoint_25532] === this.CELL_EMPTY
                && map[yPoint_25532-1][xPoint_25532+1] === this.CELL_EMPTY
                && map[yPoint_25532][xPoint_25532+1] === this.CELL_EMPTY
                && map[yPoint_25532+1][xPoint_25532+1] === this.CELL_EMPTY
                && map[yPoint_25532+1][xPoint_25532] === this.CELL_EMPTY
                && map[yPoint_25532+1][xPoint_25532-1] === this.CELL_EMPTY
                && map[yPoint_25532][xPoint_25532-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25532, yPoint_25532, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25532;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25532][x] === this.CELL_EMPTY
                    && map[yPoint_25532-1][x] === this.CELL_EMPTY
                    && map[yPoint_25532-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25532][x+1] === this.CELL_EMPTY
                    && map[yPoint_25532+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25532+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25532, yPoint_25532, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25532;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25532] === this.CELL_EMPTY
                    && map[y+1][xPoint_25532] === this.CELL_EMPTY
                    && map[y+1][xPoint_25532+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25532] === this.CELL_EMPTY
                    && map[y][xPoint_25532-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25532-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
