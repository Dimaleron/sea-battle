...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23597=0;yPoint_23597<this.gameFieldBorderY.length; yPoint_23597++){
                for(var xPoint_23597=0;xPoint_23597<this.gameFieldBorderX.length; xPoint_23597++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23597, xPoint_23597);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23597][xPoint_23597] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23597, xPoint_23597, 'user');
                    if(this._userShipsMap[yPoint_23597][xPoint_23597] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23597, xPoint_23597, type){
            var id = this.getPointBlockIdByCoords(yPoint_23597, xPoint_23597, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23597);
                block.setAttribute('data-y', yPoint_23597);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23597
         * @param {type} xPoint_23597
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23597, xPoint_23597, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23597 + '_y' + yPoint_23597;
            }
            return 'pc_x' + xPoint_23597 + '_y' + yPoint_23597;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23597=0;yPoint_23597<this.gameFieldBorderY.length; yPoint_23597++){
                for(var xPoint_23597=0;xPoint_23597<this.gameFieldBorderX.length; xPoint_23597++){
                    map.push({y: yPoint_23597, x: xPoint_23597});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23597=-1;yPoint_23597<(this.gameFieldBorderY.length+1); yPoint_23597++){
                for(var xPoint_23597=-1;xPoint_23597<(this.gameFieldBorderX.length+1); xPoint_23597++){
                    if(!map[yPoint_23597]){
                        map[yPoint_23597] = [];
                    }
                    map[yPoint_23597][xPoint_23597] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23597 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23597 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23597, yPoint_23597) === true){
                    if(this.canPutHorizontal(map, xPoint_23597, yPoint_23597, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23597][xPoint_23597 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23597, yPoint_23597, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23597 + i][xPoint_23597] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23597, yPoint_23597){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23597][xPoint_23597] === this.CELL_EMPTY
                && map[yPoint_23597-1][xPoint_23597] === this.CELL_EMPTY
                && map[yPoint_23597-1][xPoint_23597+1] === this.CELL_EMPTY
                && map[yPoint_23597][xPoint_23597+1] === this.CELL_EMPTY
                && map[yPoint_23597+1][xPoint_23597+1] === this.CELL_EMPTY
                && map[yPoint_23597+1][xPoint_23597] === this.CELL_EMPTY
                && map[yPoint_23597+1][xPoint_23597-1] === this.CELL_EMPTY
                && map[yPoint_23597][xPoint_23597-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23597, yPoint_23597, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23597;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23597][x] === this.CELL_EMPTY
                    && map[yPoint_23597-1][x] === this.CELL_EMPTY
                    && map[yPoint_23597-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23597][x+1] === this.CELL_EMPTY
                    && map[yPoint_23597+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23597+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23597, yPoint_23597, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23597;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23597] === this.CELL_EMPTY
                    && map[y+1][xPoint_23597] === this.CELL_EMPTY
                    && map[y+1][xPoint_23597+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23597] === this.CELL_EMPTY
                    && map[y][xPoint_23597-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23597-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
