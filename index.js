...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2417=0;yPoint_2417<this.gameFieldBorderY.length; yPoint_2417++){
                for(var xPoint_2417=0;xPoint_2417<this.gameFieldBorderX.length; xPoint_2417++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2417, xPoint_2417);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2417][xPoint_2417] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2417, xPoint_2417, 'user');
                    if(this._userShipsMap[yPoint_2417][xPoint_2417] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2417, xPoint_2417, type){
            var id = this.getPointBlockIdByCoords(yPoint_2417, xPoint_2417, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2417);
                block.setAttribute('data-y', yPoint_2417);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2417
         * @param {type} xPoint_2417
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2417, xPoint_2417, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2417 + '_y' + yPoint_2417;
            }
            return 'pc_x' + xPoint_2417 + '_y' + yPoint_2417;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2417=0;yPoint_2417<this.gameFieldBorderY.length; yPoint_2417++){
                for(var xPoint_2417=0;xPoint_2417<this.gameFieldBorderX.length; xPoint_2417++){
                    map.push({y: yPoint_2417, x: xPoint_2417});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2417=-1;yPoint_2417<(this.gameFieldBorderY.length+1); yPoint_2417++){
                for(var xPoint_2417=-1;xPoint_2417<(this.gameFieldBorderX.length+1); xPoint_2417++){
                    if(!map[yPoint_2417]){
                        map[yPoint_2417] = [];
                    }
                    map[yPoint_2417][xPoint_2417] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2417 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2417 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2417, yPoint_2417) === true){
                    if(this.canPutHorizontal(map, xPoint_2417, yPoint_2417, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2417][xPoint_2417 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2417, yPoint_2417, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2417 + i][xPoint_2417] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2417, yPoint_2417){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2417][xPoint_2417] === this.CELL_EMPTY
                && map[yPoint_2417-1][xPoint_2417] === this.CELL_EMPTY
                && map[yPoint_2417-1][xPoint_2417+1] === this.CELL_EMPTY
                && map[yPoint_2417][xPoint_2417+1] === this.CELL_EMPTY
                && map[yPoint_2417+1][xPoint_2417+1] === this.CELL_EMPTY
                && map[yPoint_2417+1][xPoint_2417] === this.CELL_EMPTY
                && map[yPoint_2417+1][xPoint_2417-1] === this.CELL_EMPTY
                && map[yPoint_2417][xPoint_2417-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2417, yPoint_2417, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2417;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2417][x] === this.CELL_EMPTY
                    && map[yPoint_2417-1][x] === this.CELL_EMPTY
                    && map[yPoint_2417-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2417][x+1] === this.CELL_EMPTY
                    && map[yPoint_2417+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2417+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2417, yPoint_2417, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2417;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2417] === this.CELL_EMPTY
                    && map[y+1][xPoint_2417] === this.CELL_EMPTY
                    && map[y+1][xPoint_2417+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2417] === this.CELL_EMPTY
                    && map[y][xPoint_2417-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2417-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
