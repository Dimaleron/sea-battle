...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19761=0;yPoint_19761<this.gameFieldBorderY.length; yPoint_19761++){
                for(var xPoint_19761=0;xPoint_19761<this.gameFieldBorderX.length; xPoint_19761++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19761, xPoint_19761);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19761][xPoint_19761] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19761, xPoint_19761, 'user');
                    if(this._userShipsMap[yPoint_19761][xPoint_19761] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19761, xPoint_19761, type){
            var id = this.getPointBlockIdByCoords(yPoint_19761, xPoint_19761, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19761);
                block.setAttribute('data-y', yPoint_19761);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19761
         * @param {type} xPoint_19761
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19761, xPoint_19761, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19761 + '_y' + yPoint_19761;
            }
            return 'pc_x' + xPoint_19761 + '_y' + yPoint_19761;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19761=0;yPoint_19761<this.gameFieldBorderY.length; yPoint_19761++){
                for(var xPoint_19761=0;xPoint_19761<this.gameFieldBorderX.length; xPoint_19761++){
                    map.push({y: yPoint_19761, x: xPoint_19761});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19761=-1;yPoint_19761<(this.gameFieldBorderY.length+1); yPoint_19761++){
                for(var xPoint_19761=-1;xPoint_19761<(this.gameFieldBorderX.length+1); xPoint_19761++){
                    if(!map[yPoint_19761]){
                        map[yPoint_19761] = [];
                    }
                    map[yPoint_19761][xPoint_19761] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19761 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19761 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19761, yPoint_19761) === true){
                    if(this.canPutHorizontal(map, xPoint_19761, yPoint_19761, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19761][xPoint_19761 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19761, yPoint_19761, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19761 + i][xPoint_19761] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19761, yPoint_19761){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19761][xPoint_19761] === this.CELL_EMPTY
                && map[yPoint_19761-1][xPoint_19761] === this.CELL_EMPTY
                && map[yPoint_19761-1][xPoint_19761+1] === this.CELL_EMPTY
                && map[yPoint_19761][xPoint_19761+1] === this.CELL_EMPTY
                && map[yPoint_19761+1][xPoint_19761+1] === this.CELL_EMPTY
                && map[yPoint_19761+1][xPoint_19761] === this.CELL_EMPTY
                && map[yPoint_19761+1][xPoint_19761-1] === this.CELL_EMPTY
                && map[yPoint_19761][xPoint_19761-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19761, yPoint_19761, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19761;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19761][x] === this.CELL_EMPTY
                    && map[yPoint_19761-1][x] === this.CELL_EMPTY
                    && map[yPoint_19761-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19761][x+1] === this.CELL_EMPTY
                    && map[yPoint_19761+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19761+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19761, yPoint_19761, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19761;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19761] === this.CELL_EMPTY
                    && map[y+1][xPoint_19761] === this.CELL_EMPTY
                    && map[y+1][xPoint_19761+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19761] === this.CELL_EMPTY
                    && map[y][xPoint_19761-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19761-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
