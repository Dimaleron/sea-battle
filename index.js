...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3176=0;yPoint_3176<this.gameFieldBorderY.length; yPoint_3176++){
                for(var xPoint_3176=0;xPoint_3176<this.gameFieldBorderX.length; xPoint_3176++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3176, xPoint_3176);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3176][xPoint_3176] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3176, xPoint_3176, 'user');
                    if(this._userShipsMap[yPoint_3176][xPoint_3176] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3176, xPoint_3176, type){
            var id = this.getPointBlockIdByCoords(yPoint_3176, xPoint_3176, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3176);
                block.setAttribute('data-y', yPoint_3176);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3176
         * @param {type} xPoint_3176
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3176, xPoint_3176, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3176 + '_y' + yPoint_3176;
            }
            return 'pc_x' + xPoint_3176 + '_y' + yPoint_3176;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3176=0;yPoint_3176<this.gameFieldBorderY.length; yPoint_3176++){
                for(var xPoint_3176=0;xPoint_3176<this.gameFieldBorderX.length; xPoint_3176++){
                    map.push({y: yPoint_3176, x: xPoint_3176});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3176=-1;yPoint_3176<(this.gameFieldBorderY.length+1); yPoint_3176++){
                for(var xPoint_3176=-1;xPoint_3176<(this.gameFieldBorderX.length+1); xPoint_3176++){
                    if(!map[yPoint_3176]){
                        map[yPoint_3176] = [];
                    }
                    map[yPoint_3176][xPoint_3176] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3176 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3176 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3176, yPoint_3176) === true){
                    if(this.canPutHorizontal(map, xPoint_3176, yPoint_3176, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3176][xPoint_3176 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3176, yPoint_3176, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3176 + i][xPoint_3176] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3176, yPoint_3176){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3176][xPoint_3176] === this.CELL_EMPTY
                && map[yPoint_3176-1][xPoint_3176] === this.CELL_EMPTY
                && map[yPoint_3176-1][xPoint_3176+1] === this.CELL_EMPTY
                && map[yPoint_3176][xPoint_3176+1] === this.CELL_EMPTY
                && map[yPoint_3176+1][xPoint_3176+1] === this.CELL_EMPTY
                && map[yPoint_3176+1][xPoint_3176] === this.CELL_EMPTY
                && map[yPoint_3176+1][xPoint_3176-1] === this.CELL_EMPTY
                && map[yPoint_3176][xPoint_3176-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3176, yPoint_3176, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3176;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3176][x] === this.CELL_EMPTY
                    && map[yPoint_3176-1][x] === this.CELL_EMPTY
                    && map[yPoint_3176-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3176][x+1] === this.CELL_EMPTY
                    && map[yPoint_3176+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3176+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3176, yPoint_3176, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3176;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3176] === this.CELL_EMPTY
                    && map[y+1][xPoint_3176] === this.CELL_EMPTY
                    && map[y+1][xPoint_3176+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3176] === this.CELL_EMPTY
                    && map[y][xPoint_3176-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3176-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
