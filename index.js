...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_319=0;yPoint_319<this.gameFieldBorderY.length; yPoint_319++){
                for(var xPoint_319=0;xPoint_319<this.gameFieldBorderX.length; xPoint_319++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_319, xPoint_319);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_319][xPoint_319] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_319, xPoint_319, 'user');
                    if(this._userShipsMap[yPoint_319][xPoint_319] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_319, xPoint_319, type){
            var id = this.getPointBlockIdByCoords(yPoint_319, xPoint_319, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_319);
                block.setAttribute('data-y', yPoint_319);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_319
         * @param {type} xPoint_319
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_319, xPoint_319, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_319 + '_y' + yPoint_319;
            }
            return 'pc_x' + xPoint_319 + '_y' + yPoint_319;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_319=0;yPoint_319<this.gameFieldBorderY.length; yPoint_319++){
                for(var xPoint_319=0;xPoint_319<this.gameFieldBorderX.length; xPoint_319++){
                    map.push({y: yPoint_319, x: xPoint_319});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_319=-1;yPoint_319<(this.gameFieldBorderY.length+1); yPoint_319++){
                for(var xPoint_319=-1;xPoint_319<(this.gameFieldBorderX.length+1); xPoint_319++){
                    if(!map[yPoint_319]){
                        map[yPoint_319] = [];
                    }
                    map[yPoint_319][xPoint_319] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_319 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_319 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_319, yPoint_319) === true){
                    if(this.canPutHorizontal(map, xPoint_319, yPoint_319, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_319][xPoint_319 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_319, yPoint_319, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_319 + i][xPoint_319] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_319, yPoint_319){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_319][xPoint_319] === this.CELL_EMPTY
                && map[yPoint_319-1][xPoint_319] === this.CELL_EMPTY
                && map[yPoint_319-1][xPoint_319+1] === this.CELL_EMPTY
                && map[yPoint_319][xPoint_319+1] === this.CELL_EMPTY
                && map[yPoint_319+1][xPoint_319+1] === this.CELL_EMPTY
                && map[yPoint_319+1][xPoint_319] === this.CELL_EMPTY
                && map[yPoint_319+1][xPoint_319-1] === this.CELL_EMPTY
                && map[yPoint_319][xPoint_319-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_319, yPoint_319, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_319;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_319][x] === this.CELL_EMPTY
                    && map[yPoint_319-1][x] === this.CELL_EMPTY
                    && map[yPoint_319-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_319][x+1] === this.CELL_EMPTY
                    && map[yPoint_319+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_319+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_319, yPoint_319, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_319;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_319] === this.CELL_EMPTY
                    && map[y+1][xPoint_319] === this.CELL_EMPTY
                    && map[y+1][xPoint_319+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_319] === this.CELL_EMPTY
                    && map[y][xPoint_319-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_319-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
