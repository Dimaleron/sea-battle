...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_709=0;yPoint_709<this.gameFieldBorderY.length; yPoint_709++){
                for(var xPoint_709=0;xPoint_709<this.gameFieldBorderX.length; xPoint_709++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_709, xPoint_709);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_709][xPoint_709] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_709, xPoint_709, 'user');
                    if(this._userShipsMap[yPoint_709][xPoint_709] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_709, xPoint_709, type){
            var id = this.getPointBlockIdByCoords(yPoint_709, xPoint_709, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_709);
                block.setAttribute('data-y', yPoint_709);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_709
         * @param {type} xPoint_709
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_709, xPoint_709, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_709 + '_y' + yPoint_709;
            }
            return 'pc_x' + xPoint_709 + '_y' + yPoint_709;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_709=0;yPoint_709<this.gameFieldBorderY.length; yPoint_709++){
                for(var xPoint_709=0;xPoint_709<this.gameFieldBorderX.length; xPoint_709++){
                    map.push({y: yPoint_709, x: xPoint_709});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_709=-1;yPoint_709<(this.gameFieldBorderY.length+1); yPoint_709++){
                for(var xPoint_709=-1;xPoint_709<(this.gameFieldBorderX.length+1); xPoint_709++){
                    if(!map[yPoint_709]){
                        map[yPoint_709] = [];
                    }
                    map[yPoint_709][xPoint_709] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_709 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_709 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_709, yPoint_709) === true){
                    if(this.canPutHorizontal(map, xPoint_709, yPoint_709, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_709][xPoint_709 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_709, yPoint_709, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_709 + i][xPoint_709] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_709, yPoint_709){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_709][xPoint_709] === this.CELL_EMPTY
                && map[yPoint_709-1][xPoint_709] === this.CELL_EMPTY
                && map[yPoint_709-1][xPoint_709+1] === this.CELL_EMPTY
                && map[yPoint_709][xPoint_709+1] === this.CELL_EMPTY
                && map[yPoint_709+1][xPoint_709+1] === this.CELL_EMPTY
                && map[yPoint_709+1][xPoint_709] === this.CELL_EMPTY
                && map[yPoint_709+1][xPoint_709-1] === this.CELL_EMPTY
                && map[yPoint_709][xPoint_709-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_709, yPoint_709, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_709;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_709][x] === this.CELL_EMPTY
                    && map[yPoint_709-1][x] === this.CELL_EMPTY
                    && map[yPoint_709-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_709][x+1] === this.CELL_EMPTY
                    && map[yPoint_709+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_709+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_709, yPoint_709, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_709;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_709] === this.CELL_EMPTY
                    && map[y+1][xPoint_709] === this.CELL_EMPTY
                    && map[y+1][xPoint_709+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_709] === this.CELL_EMPTY
                    && map[y][xPoint_709-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_709-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
