...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18615=0;yPoint_18615<this.gameFieldBorderY.length; yPoint_18615++){
                for(var xPoint_18615=0;xPoint_18615<this.gameFieldBorderX.length; xPoint_18615++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18615, xPoint_18615);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18615][xPoint_18615] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18615, xPoint_18615, 'user');
                    if(this._userShipsMap[yPoint_18615][xPoint_18615] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18615, xPoint_18615, type){
            var id = this.getPointBlockIdByCoords(yPoint_18615, xPoint_18615, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18615);
                block.setAttribute('data-y', yPoint_18615);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18615
         * @param {type} xPoint_18615
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18615, xPoint_18615, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18615 + '_y' + yPoint_18615;
            }
            return 'pc_x' + xPoint_18615 + '_y' + yPoint_18615;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18615=0;yPoint_18615<this.gameFieldBorderY.length; yPoint_18615++){
                for(var xPoint_18615=0;xPoint_18615<this.gameFieldBorderX.length; xPoint_18615++){
                    map.push({y: yPoint_18615, x: xPoint_18615});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18615=-1;yPoint_18615<(this.gameFieldBorderY.length+1); yPoint_18615++){
                for(var xPoint_18615=-1;xPoint_18615<(this.gameFieldBorderX.length+1); xPoint_18615++){
                    if(!map[yPoint_18615]){
                        map[yPoint_18615] = [];
                    }
                    map[yPoint_18615][xPoint_18615] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18615 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18615 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18615, yPoint_18615) === true){
                    if(this.canPutHorizontal(map, xPoint_18615, yPoint_18615, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18615][xPoint_18615 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18615, yPoint_18615, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18615 + i][xPoint_18615] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18615, yPoint_18615){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18615][xPoint_18615] === this.CELL_EMPTY
                && map[yPoint_18615-1][xPoint_18615] === this.CELL_EMPTY
                && map[yPoint_18615-1][xPoint_18615+1] === this.CELL_EMPTY
                && map[yPoint_18615][xPoint_18615+1] === this.CELL_EMPTY
                && map[yPoint_18615+1][xPoint_18615+1] === this.CELL_EMPTY
                && map[yPoint_18615+1][xPoint_18615] === this.CELL_EMPTY
                && map[yPoint_18615+1][xPoint_18615-1] === this.CELL_EMPTY
                && map[yPoint_18615][xPoint_18615-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18615, yPoint_18615, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18615;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18615][x] === this.CELL_EMPTY
                    && map[yPoint_18615-1][x] === this.CELL_EMPTY
                    && map[yPoint_18615-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18615][x+1] === this.CELL_EMPTY
                    && map[yPoint_18615+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18615+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18615, yPoint_18615, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18615;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18615] === this.CELL_EMPTY
                    && map[y+1][xPoint_18615] === this.CELL_EMPTY
                    && map[y+1][xPoint_18615+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18615] === this.CELL_EMPTY
                    && map[y][xPoint_18615-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18615-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
