...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1327=0;yPoint_1327<this.gameFieldBorderY.length; yPoint_1327++){
                for(var xPoint_1327=0;xPoint_1327<this.gameFieldBorderX.length; xPoint_1327++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1327, xPoint_1327);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1327][xPoint_1327] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1327, xPoint_1327, 'user');
                    if(this._userShipsMap[yPoint_1327][xPoint_1327] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1327, xPoint_1327, type){
            var id = this.getPointBlockIdByCoords(yPoint_1327, xPoint_1327, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1327);
                block.setAttribute('data-y', yPoint_1327);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1327
         * @param {type} xPoint_1327
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1327, xPoint_1327, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1327 + '_y' + yPoint_1327;
            }
            return 'pc_x' + xPoint_1327 + '_y' + yPoint_1327;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1327=0;yPoint_1327<this.gameFieldBorderY.length; yPoint_1327++){
                for(var xPoint_1327=0;xPoint_1327<this.gameFieldBorderX.length; xPoint_1327++){
                    map.push({y: yPoint_1327, x: xPoint_1327});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1327=-1;yPoint_1327<(this.gameFieldBorderY.length+1); yPoint_1327++){
                for(var xPoint_1327=-1;xPoint_1327<(this.gameFieldBorderX.length+1); xPoint_1327++){
                    if(!map[yPoint_1327]){
                        map[yPoint_1327] = [];
                    }
                    map[yPoint_1327][xPoint_1327] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1327 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1327 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1327, yPoint_1327) === true){
                    if(this.canPutHorizontal(map, xPoint_1327, yPoint_1327, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1327][xPoint_1327 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1327, yPoint_1327, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1327 + i][xPoint_1327] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1327, yPoint_1327){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1327][xPoint_1327] === this.CELL_EMPTY
                && map[yPoint_1327-1][xPoint_1327] === this.CELL_EMPTY
                && map[yPoint_1327-1][xPoint_1327+1] === this.CELL_EMPTY
                && map[yPoint_1327][xPoint_1327+1] === this.CELL_EMPTY
                && map[yPoint_1327+1][xPoint_1327+1] === this.CELL_EMPTY
                && map[yPoint_1327+1][xPoint_1327] === this.CELL_EMPTY
                && map[yPoint_1327+1][xPoint_1327-1] === this.CELL_EMPTY
                && map[yPoint_1327][xPoint_1327-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1327, yPoint_1327, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1327;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1327][x] === this.CELL_EMPTY
                    && map[yPoint_1327-1][x] === this.CELL_EMPTY
                    && map[yPoint_1327-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1327][x+1] === this.CELL_EMPTY
                    && map[yPoint_1327+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1327+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1327, yPoint_1327, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1327;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1327] === this.CELL_EMPTY
                    && map[y+1][xPoint_1327] === this.CELL_EMPTY
                    && map[y+1][xPoint_1327+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1327] === this.CELL_EMPTY
                    && map[y][xPoint_1327-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1327-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
