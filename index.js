...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18045=0;yPoint_18045<this.gameFieldBorderY.length; yPoint_18045++){
                for(var xPoint_18045=0;xPoint_18045<this.gameFieldBorderX.length; xPoint_18045++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18045, xPoint_18045);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18045][xPoint_18045] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18045, xPoint_18045, 'user');
                    if(this._userShipsMap[yPoint_18045][xPoint_18045] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18045, xPoint_18045, type){
            var id = this.getPointBlockIdByCoords(yPoint_18045, xPoint_18045, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18045);
                block.setAttribute('data-y', yPoint_18045);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18045
         * @param {type} xPoint_18045
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18045, xPoint_18045, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18045 + '_y' + yPoint_18045;
            }
            return 'pc_x' + xPoint_18045 + '_y' + yPoint_18045;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18045=0;yPoint_18045<this.gameFieldBorderY.length; yPoint_18045++){
                for(var xPoint_18045=0;xPoint_18045<this.gameFieldBorderX.length; xPoint_18045++){
                    map.push({y: yPoint_18045, x: xPoint_18045});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18045=-1;yPoint_18045<(this.gameFieldBorderY.length+1); yPoint_18045++){
                for(var xPoint_18045=-1;xPoint_18045<(this.gameFieldBorderX.length+1); xPoint_18045++){
                    if(!map[yPoint_18045]){
                        map[yPoint_18045] = [];
                    }
                    map[yPoint_18045][xPoint_18045] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18045 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18045 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18045, yPoint_18045) === true){
                    if(this.canPutHorizontal(map, xPoint_18045, yPoint_18045, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18045][xPoint_18045 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18045, yPoint_18045, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18045 + i][xPoint_18045] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18045, yPoint_18045){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18045][xPoint_18045] === this.CELL_EMPTY
                && map[yPoint_18045-1][xPoint_18045] === this.CELL_EMPTY
                && map[yPoint_18045-1][xPoint_18045+1] === this.CELL_EMPTY
                && map[yPoint_18045][xPoint_18045+1] === this.CELL_EMPTY
                && map[yPoint_18045+1][xPoint_18045+1] === this.CELL_EMPTY
                && map[yPoint_18045+1][xPoint_18045] === this.CELL_EMPTY
                && map[yPoint_18045+1][xPoint_18045-1] === this.CELL_EMPTY
                && map[yPoint_18045][xPoint_18045-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18045, yPoint_18045, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18045;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18045][x] === this.CELL_EMPTY
                    && map[yPoint_18045-1][x] === this.CELL_EMPTY
                    && map[yPoint_18045-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18045][x+1] === this.CELL_EMPTY
                    && map[yPoint_18045+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18045+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18045, yPoint_18045, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18045;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18045] === this.CELL_EMPTY
                    && map[y+1][xPoint_18045] === this.CELL_EMPTY
                    && map[y+1][xPoint_18045+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18045] === this.CELL_EMPTY
                    && map[y][xPoint_18045-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18045-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
