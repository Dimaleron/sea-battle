...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5543=0;yPoint_5543<this.gameFieldBorderY.length; yPoint_5543++){
                for(var xPoint_5543=0;xPoint_5543<this.gameFieldBorderX.length; xPoint_5543++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5543, xPoint_5543);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5543][xPoint_5543] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5543, xPoint_5543, 'user');
                    if(this._userShipsMap[yPoint_5543][xPoint_5543] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5543, xPoint_5543, type){
            var id = this.getPointBlockIdByCoords(yPoint_5543, xPoint_5543, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5543);
                block.setAttribute('data-y', yPoint_5543);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5543
         * @param {type} xPoint_5543
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5543, xPoint_5543, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5543 + '_y' + yPoint_5543;
            }
            return 'pc_x' + xPoint_5543 + '_y' + yPoint_5543;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5543=0;yPoint_5543<this.gameFieldBorderY.length; yPoint_5543++){
                for(var xPoint_5543=0;xPoint_5543<this.gameFieldBorderX.length; xPoint_5543++){
                    map.push({y: yPoint_5543, x: xPoint_5543});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5543=-1;yPoint_5543<(this.gameFieldBorderY.length+1); yPoint_5543++){
                for(var xPoint_5543=-1;xPoint_5543<(this.gameFieldBorderX.length+1); xPoint_5543++){
                    if(!map[yPoint_5543]){
                        map[yPoint_5543] = [];
                    }
                    map[yPoint_5543][xPoint_5543] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5543 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5543 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5543, yPoint_5543) === true){
                    if(this.canPutHorizontal(map, xPoint_5543, yPoint_5543, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5543][xPoint_5543 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5543, yPoint_5543, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5543 + i][xPoint_5543] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5543, yPoint_5543){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5543][xPoint_5543] === this.CELL_EMPTY
                && map[yPoint_5543-1][xPoint_5543] === this.CELL_EMPTY
                && map[yPoint_5543-1][xPoint_5543+1] === this.CELL_EMPTY
                && map[yPoint_5543][xPoint_5543+1] === this.CELL_EMPTY
                && map[yPoint_5543+1][xPoint_5543+1] === this.CELL_EMPTY
                && map[yPoint_5543+1][xPoint_5543] === this.CELL_EMPTY
                && map[yPoint_5543+1][xPoint_5543-1] === this.CELL_EMPTY
                && map[yPoint_5543][xPoint_5543-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5543, yPoint_5543, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5543;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5543][x] === this.CELL_EMPTY
                    && map[yPoint_5543-1][x] === this.CELL_EMPTY
                    && map[yPoint_5543-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5543][x+1] === this.CELL_EMPTY
                    && map[yPoint_5543+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5543+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5543, yPoint_5543, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5543;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5543] === this.CELL_EMPTY
                    && map[y+1][xPoint_5543] === this.CELL_EMPTY
                    && map[y+1][xPoint_5543+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5543] === this.CELL_EMPTY
                    && map[y][xPoint_5543-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5543-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
