...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16510=0;yPoint_16510<this.gameFieldBorderY.length; yPoint_16510++){
                for(var xPoint_16510=0;xPoint_16510<this.gameFieldBorderX.length; xPoint_16510++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16510, xPoint_16510);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16510][xPoint_16510] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16510, xPoint_16510, 'user');
                    if(this._userShipsMap[yPoint_16510][xPoint_16510] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16510, xPoint_16510, type){
            var id = this.getPointBlockIdByCoords(yPoint_16510, xPoint_16510, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16510);
                block.setAttribute('data-y', yPoint_16510);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16510
         * @param {type} xPoint_16510
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16510, xPoint_16510, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16510 + '_y' + yPoint_16510;
            }
            return 'pc_x' + xPoint_16510 + '_y' + yPoint_16510;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16510=0;yPoint_16510<this.gameFieldBorderY.length; yPoint_16510++){
                for(var xPoint_16510=0;xPoint_16510<this.gameFieldBorderX.length; xPoint_16510++){
                    map.push({y: yPoint_16510, x: xPoint_16510});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16510=-1;yPoint_16510<(this.gameFieldBorderY.length+1); yPoint_16510++){
                for(var xPoint_16510=-1;xPoint_16510<(this.gameFieldBorderX.length+1); xPoint_16510++){
                    if(!map[yPoint_16510]){
                        map[yPoint_16510] = [];
                    }
                    map[yPoint_16510][xPoint_16510] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16510 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16510 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16510, yPoint_16510) === true){
                    if(this.canPutHorizontal(map, xPoint_16510, yPoint_16510, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16510][xPoint_16510 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16510, yPoint_16510, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16510 + i][xPoint_16510] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16510, yPoint_16510){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16510][xPoint_16510] === this.CELL_EMPTY
                && map[yPoint_16510-1][xPoint_16510] === this.CELL_EMPTY
                && map[yPoint_16510-1][xPoint_16510+1] === this.CELL_EMPTY
                && map[yPoint_16510][xPoint_16510+1] === this.CELL_EMPTY
                && map[yPoint_16510+1][xPoint_16510+1] === this.CELL_EMPTY
                && map[yPoint_16510+1][xPoint_16510] === this.CELL_EMPTY
                && map[yPoint_16510+1][xPoint_16510-1] === this.CELL_EMPTY
                && map[yPoint_16510][xPoint_16510-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16510, yPoint_16510, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16510;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16510][x] === this.CELL_EMPTY
                    && map[yPoint_16510-1][x] === this.CELL_EMPTY
                    && map[yPoint_16510-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16510][x+1] === this.CELL_EMPTY
                    && map[yPoint_16510+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16510+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16510, yPoint_16510, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16510;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16510] === this.CELL_EMPTY
                    && map[y+1][xPoint_16510] === this.CELL_EMPTY
                    && map[y+1][xPoint_16510+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16510] === this.CELL_EMPTY
                    && map[y][xPoint_16510-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16510-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
