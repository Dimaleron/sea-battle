...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17314=0;yPoint_17314<this.gameFieldBorderY.length; yPoint_17314++){
                for(var xPoint_17314=0;xPoint_17314<this.gameFieldBorderX.length; xPoint_17314++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17314, xPoint_17314);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17314][xPoint_17314] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17314, xPoint_17314, 'user');
                    if(this._userShipsMap[yPoint_17314][xPoint_17314] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17314, xPoint_17314, type){
            var id = this.getPointBlockIdByCoords(yPoint_17314, xPoint_17314, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17314);
                block.setAttribute('data-y', yPoint_17314);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17314
         * @param {type} xPoint_17314
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17314, xPoint_17314, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17314 + '_y' + yPoint_17314;
            }
            return 'pc_x' + xPoint_17314 + '_y' + yPoint_17314;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17314=0;yPoint_17314<this.gameFieldBorderY.length; yPoint_17314++){
                for(var xPoint_17314=0;xPoint_17314<this.gameFieldBorderX.length; xPoint_17314++){
                    map.push({y: yPoint_17314, x: xPoint_17314});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17314=-1;yPoint_17314<(this.gameFieldBorderY.length+1); yPoint_17314++){
                for(var xPoint_17314=-1;xPoint_17314<(this.gameFieldBorderX.length+1); xPoint_17314++){
                    if(!map[yPoint_17314]){
                        map[yPoint_17314] = [];
                    }
                    map[yPoint_17314][xPoint_17314] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17314 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17314 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17314, yPoint_17314) === true){
                    if(this.canPutHorizontal(map, xPoint_17314, yPoint_17314, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17314][xPoint_17314 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17314, yPoint_17314, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17314 + i][xPoint_17314] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17314, yPoint_17314){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17314][xPoint_17314] === this.CELL_EMPTY
                && map[yPoint_17314-1][xPoint_17314] === this.CELL_EMPTY
                && map[yPoint_17314-1][xPoint_17314+1] === this.CELL_EMPTY
                && map[yPoint_17314][xPoint_17314+1] === this.CELL_EMPTY
                && map[yPoint_17314+1][xPoint_17314+1] === this.CELL_EMPTY
                && map[yPoint_17314+1][xPoint_17314] === this.CELL_EMPTY
                && map[yPoint_17314+1][xPoint_17314-1] === this.CELL_EMPTY
                && map[yPoint_17314][xPoint_17314-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17314, yPoint_17314, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17314;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17314][x] === this.CELL_EMPTY
                    && map[yPoint_17314-1][x] === this.CELL_EMPTY
                    && map[yPoint_17314-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17314][x+1] === this.CELL_EMPTY
                    && map[yPoint_17314+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17314+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17314, yPoint_17314, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17314;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17314] === this.CELL_EMPTY
                    && map[y+1][xPoint_17314] === this.CELL_EMPTY
                    && map[y+1][xPoint_17314+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17314] === this.CELL_EMPTY
                    && map[y][xPoint_17314-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17314-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
