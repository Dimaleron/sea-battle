...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22586=0;yPoint_22586<this.gameFieldBorderY.length; yPoint_22586++){
                for(var xPoint_22586=0;xPoint_22586<this.gameFieldBorderX.length; xPoint_22586++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22586, xPoint_22586);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22586][xPoint_22586] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22586, xPoint_22586, 'user');
                    if(this._userShipsMap[yPoint_22586][xPoint_22586] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22586, xPoint_22586, type){
            var id = this.getPointBlockIdByCoords(yPoint_22586, xPoint_22586, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22586);
                block.setAttribute('data-y', yPoint_22586);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22586
         * @param {type} xPoint_22586
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22586, xPoint_22586, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22586 + '_y' + yPoint_22586;
            }
            return 'pc_x' + xPoint_22586 + '_y' + yPoint_22586;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22586=0;yPoint_22586<this.gameFieldBorderY.length; yPoint_22586++){
                for(var xPoint_22586=0;xPoint_22586<this.gameFieldBorderX.length; xPoint_22586++){
                    map.push({y: yPoint_22586, x: xPoint_22586});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22586=-1;yPoint_22586<(this.gameFieldBorderY.length+1); yPoint_22586++){
                for(var xPoint_22586=-1;xPoint_22586<(this.gameFieldBorderX.length+1); xPoint_22586++){
                    if(!map[yPoint_22586]){
                        map[yPoint_22586] = [];
                    }
                    map[yPoint_22586][xPoint_22586] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22586 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22586 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22586, yPoint_22586) === true){
                    if(this.canPutHorizontal(map, xPoint_22586, yPoint_22586, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22586][xPoint_22586 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22586, yPoint_22586, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22586 + i][xPoint_22586] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22586, yPoint_22586){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22586][xPoint_22586] === this.CELL_EMPTY
                && map[yPoint_22586-1][xPoint_22586] === this.CELL_EMPTY
                && map[yPoint_22586-1][xPoint_22586+1] === this.CELL_EMPTY
                && map[yPoint_22586][xPoint_22586+1] === this.CELL_EMPTY
                && map[yPoint_22586+1][xPoint_22586+1] === this.CELL_EMPTY
                && map[yPoint_22586+1][xPoint_22586] === this.CELL_EMPTY
                && map[yPoint_22586+1][xPoint_22586-1] === this.CELL_EMPTY
                && map[yPoint_22586][xPoint_22586-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22586, yPoint_22586, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22586;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22586][x] === this.CELL_EMPTY
                    && map[yPoint_22586-1][x] === this.CELL_EMPTY
                    && map[yPoint_22586-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22586][x+1] === this.CELL_EMPTY
                    && map[yPoint_22586+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22586+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22586, yPoint_22586, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22586;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22586] === this.CELL_EMPTY
                    && map[y+1][xPoint_22586] === this.CELL_EMPTY
                    && map[y+1][xPoint_22586+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22586] === this.CELL_EMPTY
                    && map[y][xPoint_22586-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22586-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
