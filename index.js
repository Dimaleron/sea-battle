...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1706=0;yPoint_1706<this.gameFieldBorderY.length; yPoint_1706++){
                for(var xPoint_1706=0;xPoint_1706<this.gameFieldBorderX.length; xPoint_1706++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1706, xPoint_1706);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1706][xPoint_1706] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1706, xPoint_1706, 'user');
                    if(this._userShipsMap[yPoint_1706][xPoint_1706] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1706, xPoint_1706, type){
            var id = this.getPointBlockIdByCoords(yPoint_1706, xPoint_1706, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1706);
                block.setAttribute('data-y', yPoint_1706);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1706
         * @param {type} xPoint_1706
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1706, xPoint_1706, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1706 + '_y' + yPoint_1706;
            }
            return 'pc_x' + xPoint_1706 + '_y' + yPoint_1706;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1706=0;yPoint_1706<this.gameFieldBorderY.length; yPoint_1706++){
                for(var xPoint_1706=0;xPoint_1706<this.gameFieldBorderX.length; xPoint_1706++){
                    map.push({y: yPoint_1706, x: xPoint_1706});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1706=-1;yPoint_1706<(this.gameFieldBorderY.length+1); yPoint_1706++){
                for(var xPoint_1706=-1;xPoint_1706<(this.gameFieldBorderX.length+1); xPoint_1706++){
                    if(!map[yPoint_1706]){
                        map[yPoint_1706] = [];
                    }
                    map[yPoint_1706][xPoint_1706] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1706 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1706 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1706, yPoint_1706) === true){
                    if(this.canPutHorizontal(map, xPoint_1706, yPoint_1706, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1706][xPoint_1706 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1706, yPoint_1706, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1706 + i][xPoint_1706] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1706, yPoint_1706){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1706][xPoint_1706] === this.CELL_EMPTY
                && map[yPoint_1706-1][xPoint_1706] === this.CELL_EMPTY
                && map[yPoint_1706-1][xPoint_1706+1] === this.CELL_EMPTY
                && map[yPoint_1706][xPoint_1706+1] === this.CELL_EMPTY
                && map[yPoint_1706+1][xPoint_1706+1] === this.CELL_EMPTY
                && map[yPoint_1706+1][xPoint_1706] === this.CELL_EMPTY
                && map[yPoint_1706+1][xPoint_1706-1] === this.CELL_EMPTY
                && map[yPoint_1706][xPoint_1706-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1706, yPoint_1706, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1706;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1706][x] === this.CELL_EMPTY
                    && map[yPoint_1706-1][x] === this.CELL_EMPTY
                    && map[yPoint_1706-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1706][x+1] === this.CELL_EMPTY
                    && map[yPoint_1706+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1706+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1706, yPoint_1706, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1706;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1706] === this.CELL_EMPTY
                    && map[y+1][xPoint_1706] === this.CELL_EMPTY
                    && map[y+1][xPoint_1706+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1706] === this.CELL_EMPTY
                    && map[y][xPoint_1706-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1706-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
