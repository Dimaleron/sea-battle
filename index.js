...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16838=0;yPoint_16838<this.gameFieldBorderY.length; yPoint_16838++){
                for(var xPoint_16838=0;xPoint_16838<this.gameFieldBorderX.length; xPoint_16838++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16838, xPoint_16838);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16838][xPoint_16838] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16838, xPoint_16838, 'user');
                    if(this._userShipsMap[yPoint_16838][xPoint_16838] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16838, xPoint_16838, type){
            var id = this.getPointBlockIdByCoords(yPoint_16838, xPoint_16838, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16838);
                block.setAttribute('data-y', yPoint_16838);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16838
         * @param {type} xPoint_16838
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16838, xPoint_16838, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16838 + '_y' + yPoint_16838;
            }
            return 'pc_x' + xPoint_16838 + '_y' + yPoint_16838;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16838=0;yPoint_16838<this.gameFieldBorderY.length; yPoint_16838++){
                for(var xPoint_16838=0;xPoint_16838<this.gameFieldBorderX.length; xPoint_16838++){
                    map.push({y: yPoint_16838, x: xPoint_16838});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16838=-1;yPoint_16838<(this.gameFieldBorderY.length+1); yPoint_16838++){
                for(var xPoint_16838=-1;xPoint_16838<(this.gameFieldBorderX.length+1); xPoint_16838++){
                    if(!map[yPoint_16838]){
                        map[yPoint_16838] = [];
                    }
                    map[yPoint_16838][xPoint_16838] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16838 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16838 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16838, yPoint_16838) === true){
                    if(this.canPutHorizontal(map, xPoint_16838, yPoint_16838, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16838][xPoint_16838 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16838, yPoint_16838, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16838 + i][xPoint_16838] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16838, yPoint_16838){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16838][xPoint_16838] === this.CELL_EMPTY
                && map[yPoint_16838-1][xPoint_16838] === this.CELL_EMPTY
                && map[yPoint_16838-1][xPoint_16838+1] === this.CELL_EMPTY
                && map[yPoint_16838][xPoint_16838+1] === this.CELL_EMPTY
                && map[yPoint_16838+1][xPoint_16838+1] === this.CELL_EMPTY
                && map[yPoint_16838+1][xPoint_16838] === this.CELL_EMPTY
                && map[yPoint_16838+1][xPoint_16838-1] === this.CELL_EMPTY
                && map[yPoint_16838][xPoint_16838-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16838, yPoint_16838, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16838;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16838][x] === this.CELL_EMPTY
                    && map[yPoint_16838-1][x] === this.CELL_EMPTY
                    && map[yPoint_16838-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16838][x+1] === this.CELL_EMPTY
                    && map[yPoint_16838+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16838+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16838, yPoint_16838, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16838;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16838] === this.CELL_EMPTY
                    && map[y+1][xPoint_16838] === this.CELL_EMPTY
                    && map[y+1][xPoint_16838+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16838] === this.CELL_EMPTY
                    && map[y][xPoint_16838-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16838-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
