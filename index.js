...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24110=0;yPoint_24110<this.gameFieldBorderY.length; yPoint_24110++){
                for(var xPoint_24110=0;xPoint_24110<this.gameFieldBorderX.length; xPoint_24110++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24110, xPoint_24110);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24110][xPoint_24110] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24110, xPoint_24110, 'user');
                    if(this._userShipsMap[yPoint_24110][xPoint_24110] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24110, xPoint_24110, type){
            var id = this.getPointBlockIdByCoords(yPoint_24110, xPoint_24110, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24110);
                block.setAttribute('data-y', yPoint_24110);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24110
         * @param {type} xPoint_24110
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24110, xPoint_24110, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24110 + '_y' + yPoint_24110;
            }
            return 'pc_x' + xPoint_24110 + '_y' + yPoint_24110;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24110=0;yPoint_24110<this.gameFieldBorderY.length; yPoint_24110++){
                for(var xPoint_24110=0;xPoint_24110<this.gameFieldBorderX.length; xPoint_24110++){
                    map.push({y: yPoint_24110, x: xPoint_24110});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24110=-1;yPoint_24110<(this.gameFieldBorderY.length+1); yPoint_24110++){
                for(var xPoint_24110=-1;xPoint_24110<(this.gameFieldBorderX.length+1); xPoint_24110++){
                    if(!map[yPoint_24110]){
                        map[yPoint_24110] = [];
                    }
                    map[yPoint_24110][xPoint_24110] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24110 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24110 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24110, yPoint_24110) === true){
                    if(this.canPutHorizontal(map, xPoint_24110, yPoint_24110, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24110][xPoint_24110 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24110, yPoint_24110, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24110 + i][xPoint_24110] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24110, yPoint_24110){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24110][xPoint_24110] === this.CELL_EMPTY
                && map[yPoint_24110-1][xPoint_24110] === this.CELL_EMPTY
                && map[yPoint_24110-1][xPoint_24110+1] === this.CELL_EMPTY
                && map[yPoint_24110][xPoint_24110+1] === this.CELL_EMPTY
                && map[yPoint_24110+1][xPoint_24110+1] === this.CELL_EMPTY
                && map[yPoint_24110+1][xPoint_24110] === this.CELL_EMPTY
                && map[yPoint_24110+1][xPoint_24110-1] === this.CELL_EMPTY
                && map[yPoint_24110][xPoint_24110-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24110, yPoint_24110, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24110;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24110][x] === this.CELL_EMPTY
                    && map[yPoint_24110-1][x] === this.CELL_EMPTY
                    && map[yPoint_24110-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24110][x+1] === this.CELL_EMPTY
                    && map[yPoint_24110+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24110+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24110, yPoint_24110, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24110;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24110] === this.CELL_EMPTY
                    && map[y+1][xPoint_24110] === this.CELL_EMPTY
                    && map[y+1][xPoint_24110+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24110] === this.CELL_EMPTY
                    && map[y][xPoint_24110-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24110-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
