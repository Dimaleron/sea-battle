...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1334=0;yPoint_1334<this.gameFieldBorderY.length; yPoint_1334++){
                for(var xPoint_1334=0;xPoint_1334<this.gameFieldBorderX.length; xPoint_1334++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1334, xPoint_1334);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1334][xPoint_1334] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1334, xPoint_1334, 'user');
                    if(this._userShipsMap[yPoint_1334][xPoint_1334] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1334, xPoint_1334, type){
            var id = this.getPointBlockIdByCoords(yPoint_1334, xPoint_1334, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1334);
                block.setAttribute('data-y', yPoint_1334);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1334
         * @param {type} xPoint_1334
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1334, xPoint_1334, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1334 + '_y' + yPoint_1334;
            }
            return 'pc_x' + xPoint_1334 + '_y' + yPoint_1334;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1334=0;yPoint_1334<this.gameFieldBorderY.length; yPoint_1334++){
                for(var xPoint_1334=0;xPoint_1334<this.gameFieldBorderX.length; xPoint_1334++){
                    map.push({y: yPoint_1334, x: xPoint_1334});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1334=-1;yPoint_1334<(this.gameFieldBorderY.length+1); yPoint_1334++){
                for(var xPoint_1334=-1;xPoint_1334<(this.gameFieldBorderX.length+1); xPoint_1334++){
                    if(!map[yPoint_1334]){
                        map[yPoint_1334] = [];
                    }
                    map[yPoint_1334][xPoint_1334] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1334 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1334 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1334, yPoint_1334) === true){
                    if(this.canPutHorizontal(map, xPoint_1334, yPoint_1334, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1334][xPoint_1334 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1334, yPoint_1334, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1334 + i][xPoint_1334] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1334, yPoint_1334){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1334][xPoint_1334] === this.CELL_EMPTY
                && map[yPoint_1334-1][xPoint_1334] === this.CELL_EMPTY
                && map[yPoint_1334-1][xPoint_1334+1] === this.CELL_EMPTY
                && map[yPoint_1334][xPoint_1334+1] === this.CELL_EMPTY
                && map[yPoint_1334+1][xPoint_1334+1] === this.CELL_EMPTY
                && map[yPoint_1334+1][xPoint_1334] === this.CELL_EMPTY
                && map[yPoint_1334+1][xPoint_1334-1] === this.CELL_EMPTY
                && map[yPoint_1334][xPoint_1334-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1334, yPoint_1334, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1334;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1334][x] === this.CELL_EMPTY
                    && map[yPoint_1334-1][x] === this.CELL_EMPTY
                    && map[yPoint_1334-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1334][x+1] === this.CELL_EMPTY
                    && map[yPoint_1334+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1334+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1334, yPoint_1334, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1334;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1334] === this.CELL_EMPTY
                    && map[y+1][xPoint_1334] === this.CELL_EMPTY
                    && map[y+1][xPoint_1334+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1334] === this.CELL_EMPTY
                    && map[y][xPoint_1334-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1334-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
