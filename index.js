...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20182=0;yPoint_20182<this.gameFieldBorderY.length; yPoint_20182++){
                for(var xPoint_20182=0;xPoint_20182<this.gameFieldBorderX.length; xPoint_20182++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20182, xPoint_20182);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20182][xPoint_20182] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20182, xPoint_20182, 'user');
                    if(this._userShipsMap[yPoint_20182][xPoint_20182] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20182, xPoint_20182, type){
            var id = this.getPointBlockIdByCoords(yPoint_20182, xPoint_20182, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20182);
                block.setAttribute('data-y', yPoint_20182);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20182
         * @param {type} xPoint_20182
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20182, xPoint_20182, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20182 + '_y' + yPoint_20182;
            }
            return 'pc_x' + xPoint_20182 + '_y' + yPoint_20182;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20182=0;yPoint_20182<this.gameFieldBorderY.length; yPoint_20182++){
                for(var xPoint_20182=0;xPoint_20182<this.gameFieldBorderX.length; xPoint_20182++){
                    map.push({y: yPoint_20182, x: xPoint_20182});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20182=-1;yPoint_20182<(this.gameFieldBorderY.length+1); yPoint_20182++){
                for(var xPoint_20182=-1;xPoint_20182<(this.gameFieldBorderX.length+1); xPoint_20182++){
                    if(!map[yPoint_20182]){
                        map[yPoint_20182] = [];
                    }
                    map[yPoint_20182][xPoint_20182] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20182 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20182 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20182, yPoint_20182) === true){
                    if(this.canPutHorizontal(map, xPoint_20182, yPoint_20182, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20182][xPoint_20182 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20182, yPoint_20182, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20182 + i][xPoint_20182] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20182, yPoint_20182){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20182][xPoint_20182] === this.CELL_EMPTY
                && map[yPoint_20182-1][xPoint_20182] === this.CELL_EMPTY
                && map[yPoint_20182-1][xPoint_20182+1] === this.CELL_EMPTY
                && map[yPoint_20182][xPoint_20182+1] === this.CELL_EMPTY
                && map[yPoint_20182+1][xPoint_20182+1] === this.CELL_EMPTY
                && map[yPoint_20182+1][xPoint_20182] === this.CELL_EMPTY
                && map[yPoint_20182+1][xPoint_20182-1] === this.CELL_EMPTY
                && map[yPoint_20182][xPoint_20182-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20182, yPoint_20182, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20182;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20182][x] === this.CELL_EMPTY
                    && map[yPoint_20182-1][x] === this.CELL_EMPTY
                    && map[yPoint_20182-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20182][x+1] === this.CELL_EMPTY
                    && map[yPoint_20182+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20182+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20182, yPoint_20182, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20182;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20182] === this.CELL_EMPTY
                    && map[y+1][xPoint_20182] === this.CELL_EMPTY
                    && map[y+1][xPoint_20182+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20182] === this.CELL_EMPTY
                    && map[y][xPoint_20182-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20182-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
