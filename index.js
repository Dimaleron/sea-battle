...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14050=0;yPoint_14050<this.gameFieldBorderY.length; yPoint_14050++){
                for(var xPoint_14050=0;xPoint_14050<this.gameFieldBorderX.length; xPoint_14050++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14050, xPoint_14050);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14050][xPoint_14050] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14050, xPoint_14050, 'user');
                    if(this._userShipsMap[yPoint_14050][xPoint_14050] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14050, xPoint_14050, type){
            var id = this.getPointBlockIdByCoords(yPoint_14050, xPoint_14050, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14050);
                block.setAttribute('data-y', yPoint_14050);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14050
         * @param {type} xPoint_14050
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14050, xPoint_14050, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14050 + '_y' + yPoint_14050;
            }
            return 'pc_x' + xPoint_14050 + '_y' + yPoint_14050;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14050=0;yPoint_14050<this.gameFieldBorderY.length; yPoint_14050++){
                for(var xPoint_14050=0;xPoint_14050<this.gameFieldBorderX.length; xPoint_14050++){
                    map.push({y: yPoint_14050, x: xPoint_14050});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14050=-1;yPoint_14050<(this.gameFieldBorderY.length+1); yPoint_14050++){
                for(var xPoint_14050=-1;xPoint_14050<(this.gameFieldBorderX.length+1); xPoint_14050++){
                    if(!map[yPoint_14050]){
                        map[yPoint_14050] = [];
                    }
                    map[yPoint_14050][xPoint_14050] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14050 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14050 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14050, yPoint_14050) === true){
                    if(this.canPutHorizontal(map, xPoint_14050, yPoint_14050, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14050][xPoint_14050 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14050, yPoint_14050, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14050 + i][xPoint_14050] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14050, yPoint_14050){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14050][xPoint_14050] === this.CELL_EMPTY
                && map[yPoint_14050-1][xPoint_14050] === this.CELL_EMPTY
                && map[yPoint_14050-1][xPoint_14050+1] === this.CELL_EMPTY
                && map[yPoint_14050][xPoint_14050+1] === this.CELL_EMPTY
                && map[yPoint_14050+1][xPoint_14050+1] === this.CELL_EMPTY
                && map[yPoint_14050+1][xPoint_14050] === this.CELL_EMPTY
                && map[yPoint_14050+1][xPoint_14050-1] === this.CELL_EMPTY
                && map[yPoint_14050][xPoint_14050-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14050, yPoint_14050, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14050;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14050][x] === this.CELL_EMPTY
                    && map[yPoint_14050-1][x] === this.CELL_EMPTY
                    && map[yPoint_14050-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14050][x+1] === this.CELL_EMPTY
                    && map[yPoint_14050+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14050+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14050, yPoint_14050, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14050;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14050] === this.CELL_EMPTY
                    && map[y+1][xPoint_14050] === this.CELL_EMPTY
                    && map[y+1][xPoint_14050+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14050] === this.CELL_EMPTY
                    && map[y][xPoint_14050-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14050-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
