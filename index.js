...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_475=0;yPoint_475<this.gameFieldBorderY.length; yPoint_475++){
                for(var xPoint_475=0;xPoint_475<this.gameFieldBorderX.length; xPoint_475++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_475, xPoint_475);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_475][xPoint_475] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_475, xPoint_475, 'user');
                    if(this._userShipsMap[yPoint_475][xPoint_475] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_475, xPoint_475, type){
            var id = this.getPointBlockIdByCoords(yPoint_475, xPoint_475, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_475);
                block.setAttribute('data-y', yPoint_475);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_475
         * @param {type} xPoint_475
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_475, xPoint_475, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_475 + '_y' + yPoint_475;
            }
            return 'pc_x' + xPoint_475 + '_y' + yPoint_475;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_475=0;yPoint_475<this.gameFieldBorderY.length; yPoint_475++){
                for(var xPoint_475=0;xPoint_475<this.gameFieldBorderX.length; xPoint_475++){
                    map.push({y: yPoint_475, x: xPoint_475});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_475=-1;yPoint_475<(this.gameFieldBorderY.length+1); yPoint_475++){
                for(var xPoint_475=-1;xPoint_475<(this.gameFieldBorderX.length+1); xPoint_475++){
                    if(!map[yPoint_475]){
                        map[yPoint_475] = [];
                    }
                    map[yPoint_475][xPoint_475] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_475 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_475 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_475, yPoint_475) === true){
                    if(this.canPutHorizontal(map, xPoint_475, yPoint_475, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_475][xPoint_475 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_475, yPoint_475, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_475 + i][xPoint_475] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_475, yPoint_475){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_475][xPoint_475] === this.CELL_EMPTY
                && map[yPoint_475-1][xPoint_475] === this.CELL_EMPTY
                && map[yPoint_475-1][xPoint_475+1] === this.CELL_EMPTY
                && map[yPoint_475][xPoint_475+1] === this.CELL_EMPTY
                && map[yPoint_475+1][xPoint_475+1] === this.CELL_EMPTY
                && map[yPoint_475+1][xPoint_475] === this.CELL_EMPTY
                && map[yPoint_475+1][xPoint_475-1] === this.CELL_EMPTY
                && map[yPoint_475][xPoint_475-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_475, yPoint_475, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_475;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_475][x] === this.CELL_EMPTY
                    && map[yPoint_475-1][x] === this.CELL_EMPTY
                    && map[yPoint_475-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_475][x+1] === this.CELL_EMPTY
                    && map[yPoint_475+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_475+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_475, yPoint_475, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_475;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_475] === this.CELL_EMPTY
                    && map[y+1][xPoint_475] === this.CELL_EMPTY
                    && map[y+1][xPoint_475+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_475] === this.CELL_EMPTY
                    && map[y][xPoint_475-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_475-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
