...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17449=0;yPoint_17449<this.gameFieldBorderY.length; yPoint_17449++){
                for(var xPoint_17449=0;xPoint_17449<this.gameFieldBorderX.length; xPoint_17449++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17449, xPoint_17449);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17449][xPoint_17449] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17449, xPoint_17449, 'user');
                    if(this._userShipsMap[yPoint_17449][xPoint_17449] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17449, xPoint_17449, type){
            var id = this.getPointBlockIdByCoords(yPoint_17449, xPoint_17449, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17449);
                block.setAttribute('data-y', yPoint_17449);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17449
         * @param {type} xPoint_17449
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17449, xPoint_17449, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17449 + '_y' + yPoint_17449;
            }
            return 'pc_x' + xPoint_17449 + '_y' + yPoint_17449;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17449=0;yPoint_17449<this.gameFieldBorderY.length; yPoint_17449++){
                for(var xPoint_17449=0;xPoint_17449<this.gameFieldBorderX.length; xPoint_17449++){
                    map.push({y: yPoint_17449, x: xPoint_17449});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17449=-1;yPoint_17449<(this.gameFieldBorderY.length+1); yPoint_17449++){
                for(var xPoint_17449=-1;xPoint_17449<(this.gameFieldBorderX.length+1); xPoint_17449++){
                    if(!map[yPoint_17449]){
                        map[yPoint_17449] = [];
                    }
                    map[yPoint_17449][xPoint_17449] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17449 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17449 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17449, yPoint_17449) === true){
                    if(this.canPutHorizontal(map, xPoint_17449, yPoint_17449, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17449][xPoint_17449 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17449, yPoint_17449, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17449 + i][xPoint_17449] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17449, yPoint_17449){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17449][xPoint_17449] === this.CELL_EMPTY
                && map[yPoint_17449-1][xPoint_17449] === this.CELL_EMPTY
                && map[yPoint_17449-1][xPoint_17449+1] === this.CELL_EMPTY
                && map[yPoint_17449][xPoint_17449+1] === this.CELL_EMPTY
                && map[yPoint_17449+1][xPoint_17449+1] === this.CELL_EMPTY
                && map[yPoint_17449+1][xPoint_17449] === this.CELL_EMPTY
                && map[yPoint_17449+1][xPoint_17449-1] === this.CELL_EMPTY
                && map[yPoint_17449][xPoint_17449-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17449, yPoint_17449, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17449;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17449][x] === this.CELL_EMPTY
                    && map[yPoint_17449-1][x] === this.CELL_EMPTY
                    && map[yPoint_17449-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17449][x+1] === this.CELL_EMPTY
                    && map[yPoint_17449+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17449+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17449, yPoint_17449, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17449;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17449] === this.CELL_EMPTY
                    && map[y+1][xPoint_17449] === this.CELL_EMPTY
                    && map[y+1][xPoint_17449+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17449] === this.CELL_EMPTY
                    && map[y][xPoint_17449-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17449-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
