...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9919=0;yPoint_9919<this.gameFieldBorderY.length; yPoint_9919++){
                for(var xPoint_9919=0;xPoint_9919<this.gameFieldBorderX.length; xPoint_9919++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9919, xPoint_9919);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9919][xPoint_9919] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9919, xPoint_9919, 'user');
                    if(this._userShipsMap[yPoint_9919][xPoint_9919] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9919, xPoint_9919, type){
            var id = this.getPointBlockIdByCoords(yPoint_9919, xPoint_9919, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9919);
                block.setAttribute('data-y', yPoint_9919);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9919
         * @param {type} xPoint_9919
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9919, xPoint_9919, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9919 + '_y' + yPoint_9919;
            }
            return 'pc_x' + xPoint_9919 + '_y' + yPoint_9919;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9919=0;yPoint_9919<this.gameFieldBorderY.length; yPoint_9919++){
                for(var xPoint_9919=0;xPoint_9919<this.gameFieldBorderX.length; xPoint_9919++){
                    map.push({y: yPoint_9919, x: xPoint_9919});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9919=-1;yPoint_9919<(this.gameFieldBorderY.length+1); yPoint_9919++){
                for(var xPoint_9919=-1;xPoint_9919<(this.gameFieldBorderX.length+1); xPoint_9919++){
                    if(!map[yPoint_9919]){
                        map[yPoint_9919] = [];
                    }
                    map[yPoint_9919][xPoint_9919] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9919 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9919 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9919, yPoint_9919) === true){
                    if(this.canPutHorizontal(map, xPoint_9919, yPoint_9919, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9919][xPoint_9919 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9919, yPoint_9919, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9919 + i][xPoint_9919] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9919, yPoint_9919){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9919][xPoint_9919] === this.CELL_EMPTY
                && map[yPoint_9919-1][xPoint_9919] === this.CELL_EMPTY
                && map[yPoint_9919-1][xPoint_9919+1] === this.CELL_EMPTY
                && map[yPoint_9919][xPoint_9919+1] === this.CELL_EMPTY
                && map[yPoint_9919+1][xPoint_9919+1] === this.CELL_EMPTY
                && map[yPoint_9919+1][xPoint_9919] === this.CELL_EMPTY
                && map[yPoint_9919+1][xPoint_9919-1] === this.CELL_EMPTY
                && map[yPoint_9919][xPoint_9919-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9919, yPoint_9919, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9919;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9919][x] === this.CELL_EMPTY
                    && map[yPoint_9919-1][x] === this.CELL_EMPTY
                    && map[yPoint_9919-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9919][x+1] === this.CELL_EMPTY
                    && map[yPoint_9919+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9919+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9919, yPoint_9919, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9919;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9919] === this.CELL_EMPTY
                    && map[y+1][xPoint_9919] === this.CELL_EMPTY
                    && map[y+1][xPoint_9919+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9919] === this.CELL_EMPTY
                    && map[y][xPoint_9919-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9919-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
