...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23023=0;yPoint_23023<this.gameFieldBorderY.length; yPoint_23023++){
                for(var xPoint_23023=0;xPoint_23023<this.gameFieldBorderX.length; xPoint_23023++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23023, xPoint_23023);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23023][xPoint_23023] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23023, xPoint_23023, 'user');
                    if(this._userShipsMap[yPoint_23023][xPoint_23023] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23023, xPoint_23023, type){
            var id = this.getPointBlockIdByCoords(yPoint_23023, xPoint_23023, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23023);
                block.setAttribute('data-y', yPoint_23023);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23023
         * @param {type} xPoint_23023
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23023, xPoint_23023, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23023 + '_y' + yPoint_23023;
            }
            return 'pc_x' + xPoint_23023 + '_y' + yPoint_23023;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23023=0;yPoint_23023<this.gameFieldBorderY.length; yPoint_23023++){
                for(var xPoint_23023=0;xPoint_23023<this.gameFieldBorderX.length; xPoint_23023++){
                    map.push({y: yPoint_23023, x: xPoint_23023});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23023=-1;yPoint_23023<(this.gameFieldBorderY.length+1); yPoint_23023++){
                for(var xPoint_23023=-1;xPoint_23023<(this.gameFieldBorderX.length+1); xPoint_23023++){
                    if(!map[yPoint_23023]){
                        map[yPoint_23023] = [];
                    }
                    map[yPoint_23023][xPoint_23023] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23023 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23023 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23023, yPoint_23023) === true){
                    if(this.canPutHorizontal(map, xPoint_23023, yPoint_23023, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23023][xPoint_23023 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23023, yPoint_23023, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23023 + i][xPoint_23023] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23023, yPoint_23023){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23023][xPoint_23023] === this.CELL_EMPTY
                && map[yPoint_23023-1][xPoint_23023] === this.CELL_EMPTY
                && map[yPoint_23023-1][xPoint_23023+1] === this.CELL_EMPTY
                && map[yPoint_23023][xPoint_23023+1] === this.CELL_EMPTY
                && map[yPoint_23023+1][xPoint_23023+1] === this.CELL_EMPTY
                && map[yPoint_23023+1][xPoint_23023] === this.CELL_EMPTY
                && map[yPoint_23023+1][xPoint_23023-1] === this.CELL_EMPTY
                && map[yPoint_23023][xPoint_23023-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23023, yPoint_23023, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23023;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23023][x] === this.CELL_EMPTY
                    && map[yPoint_23023-1][x] === this.CELL_EMPTY
                    && map[yPoint_23023-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23023][x+1] === this.CELL_EMPTY
                    && map[yPoint_23023+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23023+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23023, yPoint_23023, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23023;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23023] === this.CELL_EMPTY
                    && map[y+1][xPoint_23023] === this.CELL_EMPTY
                    && map[y+1][xPoint_23023+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23023] === this.CELL_EMPTY
                    && map[y][xPoint_23023-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23023-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
