...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5647=0;yPoint_5647<this.gameFieldBorderY.length; yPoint_5647++){
                for(var xPoint_5647=0;xPoint_5647<this.gameFieldBorderX.length; xPoint_5647++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5647, xPoint_5647);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5647][xPoint_5647] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5647, xPoint_5647, 'user');
                    if(this._userShipsMap[yPoint_5647][xPoint_5647] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5647, xPoint_5647, type){
            var id = this.getPointBlockIdByCoords(yPoint_5647, xPoint_5647, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5647);
                block.setAttribute('data-y', yPoint_5647);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5647
         * @param {type} xPoint_5647
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5647, xPoint_5647, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5647 + '_y' + yPoint_5647;
            }
            return 'pc_x' + xPoint_5647 + '_y' + yPoint_5647;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5647=0;yPoint_5647<this.gameFieldBorderY.length; yPoint_5647++){
                for(var xPoint_5647=0;xPoint_5647<this.gameFieldBorderX.length; xPoint_5647++){
                    map.push({y: yPoint_5647, x: xPoint_5647});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5647=-1;yPoint_5647<(this.gameFieldBorderY.length+1); yPoint_5647++){
                for(var xPoint_5647=-1;xPoint_5647<(this.gameFieldBorderX.length+1); xPoint_5647++){
                    if(!map[yPoint_5647]){
                        map[yPoint_5647] = [];
                    }
                    map[yPoint_5647][xPoint_5647] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5647 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5647 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5647, yPoint_5647) === true){
                    if(this.canPutHorizontal(map, xPoint_5647, yPoint_5647, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5647][xPoint_5647 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5647, yPoint_5647, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5647 + i][xPoint_5647] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5647, yPoint_5647){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5647][xPoint_5647] === this.CELL_EMPTY
                && map[yPoint_5647-1][xPoint_5647] === this.CELL_EMPTY
                && map[yPoint_5647-1][xPoint_5647+1] === this.CELL_EMPTY
                && map[yPoint_5647][xPoint_5647+1] === this.CELL_EMPTY
                && map[yPoint_5647+1][xPoint_5647+1] === this.CELL_EMPTY
                && map[yPoint_5647+1][xPoint_5647] === this.CELL_EMPTY
                && map[yPoint_5647+1][xPoint_5647-1] === this.CELL_EMPTY
                && map[yPoint_5647][xPoint_5647-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5647, yPoint_5647, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5647;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5647][x] === this.CELL_EMPTY
                    && map[yPoint_5647-1][x] === this.CELL_EMPTY
                    && map[yPoint_5647-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5647][x+1] === this.CELL_EMPTY
                    && map[yPoint_5647+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5647+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5647, yPoint_5647, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5647;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5647] === this.CELL_EMPTY
                    && map[y+1][xPoint_5647] === this.CELL_EMPTY
                    && map[y+1][xPoint_5647+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5647] === this.CELL_EMPTY
                    && map[y][xPoint_5647-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5647-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
