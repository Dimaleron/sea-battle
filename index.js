...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_326=0;yPoint_326<this.gameFieldBorderY.length; yPoint_326++){
                for(var xPoint_326=0;xPoint_326<this.gameFieldBorderX.length; xPoint_326++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_326, xPoint_326);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_326][xPoint_326] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_326, xPoint_326, 'user');
                    if(this._userShipsMap[yPoint_326][xPoint_326] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_326, xPoint_326, type){
            var id = this.getPointBlockIdByCoords(yPoint_326, xPoint_326, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_326);
                block.setAttribute('data-y', yPoint_326);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_326
         * @param {type} xPoint_326
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_326, xPoint_326, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_326 + '_y' + yPoint_326;
            }
            return 'pc_x' + xPoint_326 + '_y' + yPoint_326;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_326=0;yPoint_326<this.gameFieldBorderY.length; yPoint_326++){
                for(var xPoint_326=0;xPoint_326<this.gameFieldBorderX.length; xPoint_326++){
                    map.push({y: yPoint_326, x: xPoint_326});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_326=-1;yPoint_326<(this.gameFieldBorderY.length+1); yPoint_326++){
                for(var xPoint_326=-1;xPoint_326<(this.gameFieldBorderX.length+1); xPoint_326++){
                    if(!map[yPoint_326]){
                        map[yPoint_326] = [];
                    }
                    map[yPoint_326][xPoint_326] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_326 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_326 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_326, yPoint_326) === true){
                    if(this.canPutHorizontal(map, xPoint_326, yPoint_326, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_326][xPoint_326 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_326, yPoint_326, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_326 + i][xPoint_326] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_326, yPoint_326){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_326][xPoint_326] === this.CELL_EMPTY
                && map[yPoint_326-1][xPoint_326] === this.CELL_EMPTY
                && map[yPoint_326-1][xPoint_326+1] === this.CELL_EMPTY
                && map[yPoint_326][xPoint_326+1] === this.CELL_EMPTY
                && map[yPoint_326+1][xPoint_326+1] === this.CELL_EMPTY
                && map[yPoint_326+1][xPoint_326] === this.CELL_EMPTY
                && map[yPoint_326+1][xPoint_326-1] === this.CELL_EMPTY
                && map[yPoint_326][xPoint_326-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_326, yPoint_326, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_326;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_326][x] === this.CELL_EMPTY
                    && map[yPoint_326-1][x] === this.CELL_EMPTY
                    && map[yPoint_326-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_326][x+1] === this.CELL_EMPTY
                    && map[yPoint_326+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_326+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_326, yPoint_326, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_326;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_326] === this.CELL_EMPTY
                    && map[y+1][xPoint_326] === this.CELL_EMPTY
                    && map[y+1][xPoint_326+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_326] === this.CELL_EMPTY
                    && map[y][xPoint_326-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_326-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
