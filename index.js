...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27207=0;yPoint_27207<this.gameFieldBorderY.length; yPoint_27207++){
                for(var xPoint_27207=0;xPoint_27207<this.gameFieldBorderX.length; xPoint_27207++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27207, xPoint_27207);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27207][xPoint_27207] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27207, xPoint_27207, 'user');
                    if(this._userShipsMap[yPoint_27207][xPoint_27207] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27207, xPoint_27207, type){
            var id = this.getPointBlockIdByCoords(yPoint_27207, xPoint_27207, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27207);
                block.setAttribute('data-y', yPoint_27207);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27207
         * @param {type} xPoint_27207
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27207, xPoint_27207, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27207 + '_y' + yPoint_27207;
            }
            return 'pc_x' + xPoint_27207 + '_y' + yPoint_27207;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27207=0;yPoint_27207<this.gameFieldBorderY.length; yPoint_27207++){
                for(var xPoint_27207=0;xPoint_27207<this.gameFieldBorderX.length; xPoint_27207++){
                    map.push({y: yPoint_27207, x: xPoint_27207});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27207=-1;yPoint_27207<(this.gameFieldBorderY.length+1); yPoint_27207++){
                for(var xPoint_27207=-1;xPoint_27207<(this.gameFieldBorderX.length+1); xPoint_27207++){
                    if(!map[yPoint_27207]){
                        map[yPoint_27207] = [];
                    }
                    map[yPoint_27207][xPoint_27207] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27207 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27207 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27207, yPoint_27207) === true){
                    if(this.canPutHorizontal(map, xPoint_27207, yPoint_27207, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27207][xPoint_27207 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27207, yPoint_27207, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27207 + i][xPoint_27207] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27207, yPoint_27207){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27207][xPoint_27207] === this.CELL_EMPTY
                && map[yPoint_27207-1][xPoint_27207] === this.CELL_EMPTY
                && map[yPoint_27207-1][xPoint_27207+1] === this.CELL_EMPTY
                && map[yPoint_27207][xPoint_27207+1] === this.CELL_EMPTY
                && map[yPoint_27207+1][xPoint_27207+1] === this.CELL_EMPTY
                && map[yPoint_27207+1][xPoint_27207] === this.CELL_EMPTY
                && map[yPoint_27207+1][xPoint_27207-1] === this.CELL_EMPTY
                && map[yPoint_27207][xPoint_27207-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27207, yPoint_27207, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27207;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27207][x] === this.CELL_EMPTY
                    && map[yPoint_27207-1][x] === this.CELL_EMPTY
                    && map[yPoint_27207-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27207][x+1] === this.CELL_EMPTY
                    && map[yPoint_27207+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27207+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27207, yPoint_27207, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27207;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27207] === this.CELL_EMPTY
                    && map[y+1][xPoint_27207] === this.CELL_EMPTY
                    && map[y+1][xPoint_27207+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27207] === this.CELL_EMPTY
                    && map[y][xPoint_27207-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27207-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
