...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22405=0;yPoint_22405<this.gameFieldBorderY.length; yPoint_22405++){
                for(var xPoint_22405=0;xPoint_22405<this.gameFieldBorderX.length; xPoint_22405++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22405, xPoint_22405);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22405][xPoint_22405] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22405, xPoint_22405, 'user');
                    if(this._userShipsMap[yPoint_22405][xPoint_22405] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22405, xPoint_22405, type){
            var id = this.getPointBlockIdByCoords(yPoint_22405, xPoint_22405, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22405);
                block.setAttribute('data-y', yPoint_22405);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22405
         * @param {type} xPoint_22405
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22405, xPoint_22405, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22405 + '_y' + yPoint_22405;
            }
            return 'pc_x' + xPoint_22405 + '_y' + yPoint_22405;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22405=0;yPoint_22405<this.gameFieldBorderY.length; yPoint_22405++){
                for(var xPoint_22405=0;xPoint_22405<this.gameFieldBorderX.length; xPoint_22405++){
                    map.push({y: yPoint_22405, x: xPoint_22405});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22405=-1;yPoint_22405<(this.gameFieldBorderY.length+1); yPoint_22405++){
                for(var xPoint_22405=-1;xPoint_22405<(this.gameFieldBorderX.length+1); xPoint_22405++){
                    if(!map[yPoint_22405]){
                        map[yPoint_22405] = [];
                    }
                    map[yPoint_22405][xPoint_22405] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22405 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22405 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22405, yPoint_22405) === true){
                    if(this.canPutHorizontal(map, xPoint_22405, yPoint_22405, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22405][xPoint_22405 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22405, yPoint_22405, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22405 + i][xPoint_22405] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22405, yPoint_22405){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22405][xPoint_22405] === this.CELL_EMPTY
                && map[yPoint_22405-1][xPoint_22405] === this.CELL_EMPTY
                && map[yPoint_22405-1][xPoint_22405+1] === this.CELL_EMPTY
                && map[yPoint_22405][xPoint_22405+1] === this.CELL_EMPTY
                && map[yPoint_22405+1][xPoint_22405+1] === this.CELL_EMPTY
                && map[yPoint_22405+1][xPoint_22405] === this.CELL_EMPTY
                && map[yPoint_22405+1][xPoint_22405-1] === this.CELL_EMPTY
                && map[yPoint_22405][xPoint_22405-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22405, yPoint_22405, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22405;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22405][x] === this.CELL_EMPTY
                    && map[yPoint_22405-1][x] === this.CELL_EMPTY
                    && map[yPoint_22405-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22405][x+1] === this.CELL_EMPTY
                    && map[yPoint_22405+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22405+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22405, yPoint_22405, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22405;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22405] === this.CELL_EMPTY
                    && map[y+1][xPoint_22405] === this.CELL_EMPTY
                    && map[y+1][xPoint_22405+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22405] === this.CELL_EMPTY
                    && map[y][xPoint_22405-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22405-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
