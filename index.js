...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_417=0;yPoint_417<this.gameFieldBorderY.length; yPoint_417++){
                for(var xPoint_417=0;xPoint_417<this.gameFieldBorderX.length; xPoint_417++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_417, xPoint_417);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_417][xPoint_417] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_417, xPoint_417, 'user');
                    if(this._userShipsMap[yPoint_417][xPoint_417] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_417, xPoint_417, type){
            var id = this.getPointBlockIdByCoords(yPoint_417, xPoint_417, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_417);
                block.setAttribute('data-y', yPoint_417);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_417
         * @param {type} xPoint_417
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_417, xPoint_417, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_417 + '_y' + yPoint_417;
            }
            return 'pc_x' + xPoint_417 + '_y' + yPoint_417;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_417=0;yPoint_417<this.gameFieldBorderY.length; yPoint_417++){
                for(var xPoint_417=0;xPoint_417<this.gameFieldBorderX.length; xPoint_417++){
                    map.push({y: yPoint_417, x: xPoint_417});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_417=-1;yPoint_417<(this.gameFieldBorderY.length+1); yPoint_417++){
                for(var xPoint_417=-1;xPoint_417<(this.gameFieldBorderX.length+1); xPoint_417++){
                    if(!map[yPoint_417]){
                        map[yPoint_417] = [];
                    }
                    map[yPoint_417][xPoint_417] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_417 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_417 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_417, yPoint_417) === true){
                    if(this.canPutHorizontal(map, xPoint_417, yPoint_417, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_417][xPoint_417 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_417, yPoint_417, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_417 + i][xPoint_417] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_417, yPoint_417){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_417][xPoint_417] === this.CELL_EMPTY
                && map[yPoint_417-1][xPoint_417] === this.CELL_EMPTY
                && map[yPoint_417-1][xPoint_417+1] === this.CELL_EMPTY
                && map[yPoint_417][xPoint_417+1] === this.CELL_EMPTY
                && map[yPoint_417+1][xPoint_417+1] === this.CELL_EMPTY
                && map[yPoint_417+1][xPoint_417] === this.CELL_EMPTY
                && map[yPoint_417+1][xPoint_417-1] === this.CELL_EMPTY
                && map[yPoint_417][xPoint_417-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_417, yPoint_417, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_417;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_417][x] === this.CELL_EMPTY
                    && map[yPoint_417-1][x] === this.CELL_EMPTY
                    && map[yPoint_417-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_417][x+1] === this.CELL_EMPTY
                    && map[yPoint_417+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_417+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_417, yPoint_417, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_417;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_417] === this.CELL_EMPTY
                    && map[y+1][xPoint_417] === this.CELL_EMPTY
                    && map[y+1][xPoint_417+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_417] === this.CELL_EMPTY
                    && map[y][xPoint_417-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_417-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
