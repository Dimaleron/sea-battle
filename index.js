...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10841=0;yPoint_10841<this.gameFieldBorderY.length; yPoint_10841++){
                for(var xPoint_10841=0;xPoint_10841<this.gameFieldBorderX.length; xPoint_10841++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10841, xPoint_10841);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10841][xPoint_10841] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10841, xPoint_10841, 'user');
                    if(this._userShipsMap[yPoint_10841][xPoint_10841] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10841, xPoint_10841, type){
            var id = this.getPointBlockIdByCoords(yPoint_10841, xPoint_10841, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10841);
                block.setAttribute('data-y', yPoint_10841);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10841
         * @param {type} xPoint_10841
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10841, xPoint_10841, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10841 + '_y' + yPoint_10841;
            }
            return 'pc_x' + xPoint_10841 + '_y' + yPoint_10841;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10841=0;yPoint_10841<this.gameFieldBorderY.length; yPoint_10841++){
                for(var xPoint_10841=0;xPoint_10841<this.gameFieldBorderX.length; xPoint_10841++){
                    map.push({y: yPoint_10841, x: xPoint_10841});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10841=-1;yPoint_10841<(this.gameFieldBorderY.length+1); yPoint_10841++){
                for(var xPoint_10841=-1;xPoint_10841<(this.gameFieldBorderX.length+1); xPoint_10841++){
                    if(!map[yPoint_10841]){
                        map[yPoint_10841] = [];
                    }
                    map[yPoint_10841][xPoint_10841] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10841 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10841 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10841, yPoint_10841) === true){
                    if(this.canPutHorizontal(map, xPoint_10841, yPoint_10841, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10841][xPoint_10841 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10841, yPoint_10841, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10841 + i][xPoint_10841] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10841, yPoint_10841){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10841][xPoint_10841] === this.CELL_EMPTY
                && map[yPoint_10841-1][xPoint_10841] === this.CELL_EMPTY
                && map[yPoint_10841-1][xPoint_10841+1] === this.CELL_EMPTY
                && map[yPoint_10841][xPoint_10841+1] === this.CELL_EMPTY
                && map[yPoint_10841+1][xPoint_10841+1] === this.CELL_EMPTY
                && map[yPoint_10841+1][xPoint_10841] === this.CELL_EMPTY
                && map[yPoint_10841+1][xPoint_10841-1] === this.CELL_EMPTY
                && map[yPoint_10841][xPoint_10841-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10841, yPoint_10841, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10841;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10841][x] === this.CELL_EMPTY
                    && map[yPoint_10841-1][x] === this.CELL_EMPTY
                    && map[yPoint_10841-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10841][x+1] === this.CELL_EMPTY
                    && map[yPoint_10841+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10841+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10841, yPoint_10841, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10841;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10841] === this.CELL_EMPTY
                    && map[y+1][xPoint_10841] === this.CELL_EMPTY
                    && map[y+1][xPoint_10841+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10841] === this.CELL_EMPTY
                    && map[y][xPoint_10841-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10841-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
