...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9944=0;yPoint_9944<this.gameFieldBorderY.length; yPoint_9944++){
                for(var xPoint_9944=0;xPoint_9944<this.gameFieldBorderX.length; xPoint_9944++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9944, xPoint_9944);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9944][xPoint_9944] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9944, xPoint_9944, 'user');
                    if(this._userShipsMap[yPoint_9944][xPoint_9944] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9944, xPoint_9944, type){
            var id = this.getPointBlockIdByCoords(yPoint_9944, xPoint_9944, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9944);
                block.setAttribute('data-y', yPoint_9944);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9944
         * @param {type} xPoint_9944
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9944, xPoint_9944, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9944 + '_y' + yPoint_9944;
            }
            return 'pc_x' + xPoint_9944 + '_y' + yPoint_9944;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9944=0;yPoint_9944<this.gameFieldBorderY.length; yPoint_9944++){
                for(var xPoint_9944=0;xPoint_9944<this.gameFieldBorderX.length; xPoint_9944++){
                    map.push({y: yPoint_9944, x: xPoint_9944});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9944=-1;yPoint_9944<(this.gameFieldBorderY.length+1); yPoint_9944++){
                for(var xPoint_9944=-1;xPoint_9944<(this.gameFieldBorderX.length+1); xPoint_9944++){
                    if(!map[yPoint_9944]){
                        map[yPoint_9944] = [];
                    }
                    map[yPoint_9944][xPoint_9944] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9944 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9944 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9944, yPoint_9944) === true){
                    if(this.canPutHorizontal(map, xPoint_9944, yPoint_9944, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9944][xPoint_9944 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9944, yPoint_9944, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9944 + i][xPoint_9944] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9944, yPoint_9944){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9944][xPoint_9944] === this.CELL_EMPTY
                && map[yPoint_9944-1][xPoint_9944] === this.CELL_EMPTY
                && map[yPoint_9944-1][xPoint_9944+1] === this.CELL_EMPTY
                && map[yPoint_9944][xPoint_9944+1] === this.CELL_EMPTY
                && map[yPoint_9944+1][xPoint_9944+1] === this.CELL_EMPTY
                && map[yPoint_9944+1][xPoint_9944] === this.CELL_EMPTY
                && map[yPoint_9944+1][xPoint_9944-1] === this.CELL_EMPTY
                && map[yPoint_9944][xPoint_9944-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9944, yPoint_9944, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9944;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9944][x] === this.CELL_EMPTY
                    && map[yPoint_9944-1][x] === this.CELL_EMPTY
                    && map[yPoint_9944-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9944][x+1] === this.CELL_EMPTY
                    && map[yPoint_9944+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9944+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9944, yPoint_9944, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9944;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9944] === this.CELL_EMPTY
                    && map[y+1][xPoint_9944] === this.CELL_EMPTY
                    && map[y+1][xPoint_9944+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9944] === this.CELL_EMPTY
                    && map[y][xPoint_9944-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9944-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
