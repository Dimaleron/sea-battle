...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20879=0;yPoint_20879<this.gameFieldBorderY.length; yPoint_20879++){
                for(var xPoint_20879=0;xPoint_20879<this.gameFieldBorderX.length; xPoint_20879++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20879, xPoint_20879);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20879][xPoint_20879] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20879, xPoint_20879, 'user');
                    if(this._userShipsMap[yPoint_20879][xPoint_20879] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20879, xPoint_20879, type){
            var id = this.getPointBlockIdByCoords(yPoint_20879, xPoint_20879, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20879);
                block.setAttribute('data-y', yPoint_20879);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20879
         * @param {type} xPoint_20879
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20879, xPoint_20879, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20879 + '_y' + yPoint_20879;
            }
            return 'pc_x' + xPoint_20879 + '_y' + yPoint_20879;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20879=0;yPoint_20879<this.gameFieldBorderY.length; yPoint_20879++){
                for(var xPoint_20879=0;xPoint_20879<this.gameFieldBorderX.length; xPoint_20879++){
                    map.push({y: yPoint_20879, x: xPoint_20879});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20879=-1;yPoint_20879<(this.gameFieldBorderY.length+1); yPoint_20879++){
                for(var xPoint_20879=-1;xPoint_20879<(this.gameFieldBorderX.length+1); xPoint_20879++){
                    if(!map[yPoint_20879]){
                        map[yPoint_20879] = [];
                    }
                    map[yPoint_20879][xPoint_20879] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20879 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20879 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20879, yPoint_20879) === true){
                    if(this.canPutHorizontal(map, xPoint_20879, yPoint_20879, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20879][xPoint_20879 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20879, yPoint_20879, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20879 + i][xPoint_20879] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20879, yPoint_20879){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20879][xPoint_20879] === this.CELL_EMPTY
                && map[yPoint_20879-1][xPoint_20879] === this.CELL_EMPTY
                && map[yPoint_20879-1][xPoint_20879+1] === this.CELL_EMPTY
                && map[yPoint_20879][xPoint_20879+1] === this.CELL_EMPTY
                && map[yPoint_20879+1][xPoint_20879+1] === this.CELL_EMPTY
                && map[yPoint_20879+1][xPoint_20879] === this.CELL_EMPTY
                && map[yPoint_20879+1][xPoint_20879-1] === this.CELL_EMPTY
                && map[yPoint_20879][xPoint_20879-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20879, yPoint_20879, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20879;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20879][x] === this.CELL_EMPTY
                    && map[yPoint_20879-1][x] === this.CELL_EMPTY
                    && map[yPoint_20879-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20879][x+1] === this.CELL_EMPTY
                    && map[yPoint_20879+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20879+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20879, yPoint_20879, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20879;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20879] === this.CELL_EMPTY
                    && map[y+1][xPoint_20879] === this.CELL_EMPTY
                    && map[y+1][xPoint_20879+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20879] === this.CELL_EMPTY
                    && map[y][xPoint_20879-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20879-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
