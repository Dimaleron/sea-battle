...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17508=0;yPoint_17508<this.gameFieldBorderY.length; yPoint_17508++){
                for(var xPoint_17508=0;xPoint_17508<this.gameFieldBorderX.length; xPoint_17508++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17508, xPoint_17508);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17508][xPoint_17508] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17508, xPoint_17508, 'user');
                    if(this._userShipsMap[yPoint_17508][xPoint_17508] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17508, xPoint_17508, type){
            var id = this.getPointBlockIdByCoords(yPoint_17508, xPoint_17508, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17508);
                block.setAttribute('data-y', yPoint_17508);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17508
         * @param {type} xPoint_17508
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17508, xPoint_17508, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17508 + '_y' + yPoint_17508;
            }
            return 'pc_x' + xPoint_17508 + '_y' + yPoint_17508;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17508=0;yPoint_17508<this.gameFieldBorderY.length; yPoint_17508++){
                for(var xPoint_17508=0;xPoint_17508<this.gameFieldBorderX.length; xPoint_17508++){
                    map.push({y: yPoint_17508, x: xPoint_17508});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17508=-1;yPoint_17508<(this.gameFieldBorderY.length+1); yPoint_17508++){
                for(var xPoint_17508=-1;xPoint_17508<(this.gameFieldBorderX.length+1); xPoint_17508++){
                    if(!map[yPoint_17508]){
                        map[yPoint_17508] = [];
                    }
                    map[yPoint_17508][xPoint_17508] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17508 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17508 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17508, yPoint_17508) === true){
                    if(this.canPutHorizontal(map, xPoint_17508, yPoint_17508, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17508][xPoint_17508 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17508, yPoint_17508, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17508 + i][xPoint_17508] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17508, yPoint_17508){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17508][xPoint_17508] === this.CELL_EMPTY
                && map[yPoint_17508-1][xPoint_17508] === this.CELL_EMPTY
                && map[yPoint_17508-1][xPoint_17508+1] === this.CELL_EMPTY
                && map[yPoint_17508][xPoint_17508+1] === this.CELL_EMPTY
                && map[yPoint_17508+1][xPoint_17508+1] === this.CELL_EMPTY
                && map[yPoint_17508+1][xPoint_17508] === this.CELL_EMPTY
                && map[yPoint_17508+1][xPoint_17508-1] === this.CELL_EMPTY
                && map[yPoint_17508][xPoint_17508-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17508, yPoint_17508, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17508;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17508][x] === this.CELL_EMPTY
                    && map[yPoint_17508-1][x] === this.CELL_EMPTY
                    && map[yPoint_17508-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17508][x+1] === this.CELL_EMPTY
                    && map[yPoint_17508+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17508+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17508, yPoint_17508, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17508;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17508] === this.CELL_EMPTY
                    && map[y+1][xPoint_17508] === this.CELL_EMPTY
                    && map[y+1][xPoint_17508+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17508] === this.CELL_EMPTY
                    && map[y][xPoint_17508-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17508-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
