...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6950=0;yPoint_6950<this.gameFieldBorderY.length; yPoint_6950++){
                for(var xPoint_6950=0;xPoint_6950<this.gameFieldBorderX.length; xPoint_6950++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6950, xPoint_6950);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6950][xPoint_6950] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6950, xPoint_6950, 'user');
                    if(this._userShipsMap[yPoint_6950][xPoint_6950] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6950, xPoint_6950, type){
            var id = this.getPointBlockIdByCoords(yPoint_6950, xPoint_6950, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6950);
                block.setAttribute('data-y', yPoint_6950);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6950
         * @param {type} xPoint_6950
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6950, xPoint_6950, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6950 + '_y' + yPoint_6950;
            }
            return 'pc_x' + xPoint_6950 + '_y' + yPoint_6950;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6950=0;yPoint_6950<this.gameFieldBorderY.length; yPoint_6950++){
                for(var xPoint_6950=0;xPoint_6950<this.gameFieldBorderX.length; xPoint_6950++){
                    map.push({y: yPoint_6950, x: xPoint_6950});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6950=-1;yPoint_6950<(this.gameFieldBorderY.length+1); yPoint_6950++){
                for(var xPoint_6950=-1;xPoint_6950<(this.gameFieldBorderX.length+1); xPoint_6950++){
                    if(!map[yPoint_6950]){
                        map[yPoint_6950] = [];
                    }
                    map[yPoint_6950][xPoint_6950] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6950 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6950 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6950, yPoint_6950) === true){
                    if(this.canPutHorizontal(map, xPoint_6950, yPoint_6950, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6950][xPoint_6950 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6950, yPoint_6950, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6950 + i][xPoint_6950] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6950, yPoint_6950){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6950][xPoint_6950] === this.CELL_EMPTY
                && map[yPoint_6950-1][xPoint_6950] === this.CELL_EMPTY
                && map[yPoint_6950-1][xPoint_6950+1] === this.CELL_EMPTY
                && map[yPoint_6950][xPoint_6950+1] === this.CELL_EMPTY
                && map[yPoint_6950+1][xPoint_6950+1] === this.CELL_EMPTY
                && map[yPoint_6950+1][xPoint_6950] === this.CELL_EMPTY
                && map[yPoint_6950+1][xPoint_6950-1] === this.CELL_EMPTY
                && map[yPoint_6950][xPoint_6950-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6950, yPoint_6950, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6950;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6950][x] === this.CELL_EMPTY
                    && map[yPoint_6950-1][x] === this.CELL_EMPTY
                    && map[yPoint_6950-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6950][x+1] === this.CELL_EMPTY
                    && map[yPoint_6950+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6950+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6950, yPoint_6950, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6950;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6950] === this.CELL_EMPTY
                    && map[y+1][xPoint_6950] === this.CELL_EMPTY
                    && map[y+1][xPoint_6950+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6950] === this.CELL_EMPTY
                    && map[y][xPoint_6950-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6950-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
