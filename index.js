...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14681=0;yPoint_14681<this.gameFieldBorderY.length; yPoint_14681++){
                for(var xPoint_14681=0;xPoint_14681<this.gameFieldBorderX.length; xPoint_14681++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14681, xPoint_14681);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14681][xPoint_14681] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14681, xPoint_14681, 'user');
                    if(this._userShipsMap[yPoint_14681][xPoint_14681] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14681, xPoint_14681, type){
            var id = this.getPointBlockIdByCoords(yPoint_14681, xPoint_14681, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14681);
                block.setAttribute('data-y', yPoint_14681);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14681
         * @param {type} xPoint_14681
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14681, xPoint_14681, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14681 + '_y' + yPoint_14681;
            }
            return 'pc_x' + xPoint_14681 + '_y' + yPoint_14681;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14681=0;yPoint_14681<this.gameFieldBorderY.length; yPoint_14681++){
                for(var xPoint_14681=0;xPoint_14681<this.gameFieldBorderX.length; xPoint_14681++){
                    map.push({y: yPoint_14681, x: xPoint_14681});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14681=-1;yPoint_14681<(this.gameFieldBorderY.length+1); yPoint_14681++){
                for(var xPoint_14681=-1;xPoint_14681<(this.gameFieldBorderX.length+1); xPoint_14681++){
                    if(!map[yPoint_14681]){
                        map[yPoint_14681] = [];
                    }
                    map[yPoint_14681][xPoint_14681] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14681 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14681 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14681, yPoint_14681) === true){
                    if(this.canPutHorizontal(map, xPoint_14681, yPoint_14681, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14681][xPoint_14681 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14681, yPoint_14681, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14681 + i][xPoint_14681] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14681, yPoint_14681){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14681][xPoint_14681] === this.CELL_EMPTY
                && map[yPoint_14681-1][xPoint_14681] === this.CELL_EMPTY
                && map[yPoint_14681-1][xPoint_14681+1] === this.CELL_EMPTY
                && map[yPoint_14681][xPoint_14681+1] === this.CELL_EMPTY
                && map[yPoint_14681+1][xPoint_14681+1] === this.CELL_EMPTY
                && map[yPoint_14681+1][xPoint_14681] === this.CELL_EMPTY
                && map[yPoint_14681+1][xPoint_14681-1] === this.CELL_EMPTY
                && map[yPoint_14681][xPoint_14681-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14681, yPoint_14681, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14681;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14681][x] === this.CELL_EMPTY
                    && map[yPoint_14681-1][x] === this.CELL_EMPTY
                    && map[yPoint_14681-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14681][x+1] === this.CELL_EMPTY
                    && map[yPoint_14681+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14681+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14681, yPoint_14681, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14681;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14681] === this.CELL_EMPTY
                    && map[y+1][xPoint_14681] === this.CELL_EMPTY
                    && map[y+1][xPoint_14681+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14681] === this.CELL_EMPTY
                    && map[y][xPoint_14681-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14681-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
