...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8922=0;yPoint_8922<this.gameFieldBorderY.length; yPoint_8922++){
                for(var xPoint_8922=0;xPoint_8922<this.gameFieldBorderX.length; xPoint_8922++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8922, xPoint_8922);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8922][xPoint_8922] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8922, xPoint_8922, 'user');
                    if(this._userShipsMap[yPoint_8922][xPoint_8922] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8922, xPoint_8922, type){
            var id = this.getPointBlockIdByCoords(yPoint_8922, xPoint_8922, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8922);
                block.setAttribute('data-y', yPoint_8922);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8922
         * @param {type} xPoint_8922
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8922, xPoint_8922, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8922 + '_y' + yPoint_8922;
            }
            return 'pc_x' + xPoint_8922 + '_y' + yPoint_8922;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8922=0;yPoint_8922<this.gameFieldBorderY.length; yPoint_8922++){
                for(var xPoint_8922=0;xPoint_8922<this.gameFieldBorderX.length; xPoint_8922++){
                    map.push({y: yPoint_8922, x: xPoint_8922});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8922=-1;yPoint_8922<(this.gameFieldBorderY.length+1); yPoint_8922++){
                for(var xPoint_8922=-1;xPoint_8922<(this.gameFieldBorderX.length+1); xPoint_8922++){
                    if(!map[yPoint_8922]){
                        map[yPoint_8922] = [];
                    }
                    map[yPoint_8922][xPoint_8922] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8922 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8922 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8922, yPoint_8922) === true){
                    if(this.canPutHorizontal(map, xPoint_8922, yPoint_8922, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8922][xPoint_8922 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8922, yPoint_8922, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8922 + i][xPoint_8922] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8922, yPoint_8922){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8922][xPoint_8922] === this.CELL_EMPTY
                && map[yPoint_8922-1][xPoint_8922] === this.CELL_EMPTY
                && map[yPoint_8922-1][xPoint_8922+1] === this.CELL_EMPTY
                && map[yPoint_8922][xPoint_8922+1] === this.CELL_EMPTY
                && map[yPoint_8922+1][xPoint_8922+1] === this.CELL_EMPTY
                && map[yPoint_8922+1][xPoint_8922] === this.CELL_EMPTY
                && map[yPoint_8922+1][xPoint_8922-1] === this.CELL_EMPTY
                && map[yPoint_8922][xPoint_8922-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8922, yPoint_8922, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8922;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8922][x] === this.CELL_EMPTY
                    && map[yPoint_8922-1][x] === this.CELL_EMPTY
                    && map[yPoint_8922-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8922][x+1] === this.CELL_EMPTY
                    && map[yPoint_8922+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8922+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8922, yPoint_8922, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8922;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8922] === this.CELL_EMPTY
                    && map[y+1][xPoint_8922] === this.CELL_EMPTY
                    && map[y+1][xPoint_8922+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8922] === this.CELL_EMPTY
                    && map[y][xPoint_8922-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8922-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
