...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14274=0;yPoint_14274<this.gameFieldBorderY.length; yPoint_14274++){
                for(var xPoint_14274=0;xPoint_14274<this.gameFieldBorderX.length; xPoint_14274++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14274, xPoint_14274);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14274][xPoint_14274] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14274, xPoint_14274, 'user');
                    if(this._userShipsMap[yPoint_14274][xPoint_14274] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14274, xPoint_14274, type){
            var id = this.getPointBlockIdByCoords(yPoint_14274, xPoint_14274, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14274);
                block.setAttribute('data-y', yPoint_14274);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14274
         * @param {type} xPoint_14274
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14274, xPoint_14274, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14274 + '_y' + yPoint_14274;
            }
            return 'pc_x' + xPoint_14274 + '_y' + yPoint_14274;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14274=0;yPoint_14274<this.gameFieldBorderY.length; yPoint_14274++){
                for(var xPoint_14274=0;xPoint_14274<this.gameFieldBorderX.length; xPoint_14274++){
                    map.push({y: yPoint_14274, x: xPoint_14274});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14274=-1;yPoint_14274<(this.gameFieldBorderY.length+1); yPoint_14274++){
                for(var xPoint_14274=-1;xPoint_14274<(this.gameFieldBorderX.length+1); xPoint_14274++){
                    if(!map[yPoint_14274]){
                        map[yPoint_14274] = [];
                    }
                    map[yPoint_14274][xPoint_14274] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14274 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14274 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14274, yPoint_14274) === true){
                    if(this.canPutHorizontal(map, xPoint_14274, yPoint_14274, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14274][xPoint_14274 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14274, yPoint_14274, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14274 + i][xPoint_14274] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14274, yPoint_14274){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14274][xPoint_14274] === this.CELL_EMPTY
                && map[yPoint_14274-1][xPoint_14274] === this.CELL_EMPTY
                && map[yPoint_14274-1][xPoint_14274+1] === this.CELL_EMPTY
                && map[yPoint_14274][xPoint_14274+1] === this.CELL_EMPTY
                && map[yPoint_14274+1][xPoint_14274+1] === this.CELL_EMPTY
                && map[yPoint_14274+1][xPoint_14274] === this.CELL_EMPTY
                && map[yPoint_14274+1][xPoint_14274-1] === this.CELL_EMPTY
                && map[yPoint_14274][xPoint_14274-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14274, yPoint_14274, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14274;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14274][x] === this.CELL_EMPTY
                    && map[yPoint_14274-1][x] === this.CELL_EMPTY
                    && map[yPoint_14274-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14274][x+1] === this.CELL_EMPTY
                    && map[yPoint_14274+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14274+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14274, yPoint_14274, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14274;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14274] === this.CELL_EMPTY
                    && map[y+1][xPoint_14274] === this.CELL_EMPTY
                    && map[y+1][xPoint_14274+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14274] === this.CELL_EMPTY
                    && map[y][xPoint_14274-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14274-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
