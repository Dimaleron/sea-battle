...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27923=0;yPoint_27923<this.gameFieldBorderY.length; yPoint_27923++){
                for(var xPoint_27923=0;xPoint_27923<this.gameFieldBorderX.length; xPoint_27923++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27923, xPoint_27923);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27923][xPoint_27923] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27923, xPoint_27923, 'user');
                    if(this._userShipsMap[yPoint_27923][xPoint_27923] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27923, xPoint_27923, type){
            var id = this.getPointBlockIdByCoords(yPoint_27923, xPoint_27923, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27923);
                block.setAttribute('data-y', yPoint_27923);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27923
         * @param {type} xPoint_27923
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27923, xPoint_27923, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27923 + '_y' + yPoint_27923;
            }
            return 'pc_x' + xPoint_27923 + '_y' + yPoint_27923;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27923=0;yPoint_27923<this.gameFieldBorderY.length; yPoint_27923++){
                for(var xPoint_27923=0;xPoint_27923<this.gameFieldBorderX.length; xPoint_27923++){
                    map.push({y: yPoint_27923, x: xPoint_27923});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27923=-1;yPoint_27923<(this.gameFieldBorderY.length+1); yPoint_27923++){
                for(var xPoint_27923=-1;xPoint_27923<(this.gameFieldBorderX.length+1); xPoint_27923++){
                    if(!map[yPoint_27923]){
                        map[yPoint_27923] = [];
                    }
                    map[yPoint_27923][xPoint_27923] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27923 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27923 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27923, yPoint_27923) === true){
                    if(this.canPutHorizontal(map, xPoint_27923, yPoint_27923, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27923][xPoint_27923 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27923, yPoint_27923, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27923 + i][xPoint_27923] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27923, yPoint_27923){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27923][xPoint_27923] === this.CELL_EMPTY
                && map[yPoint_27923-1][xPoint_27923] === this.CELL_EMPTY
                && map[yPoint_27923-1][xPoint_27923+1] === this.CELL_EMPTY
                && map[yPoint_27923][xPoint_27923+1] === this.CELL_EMPTY
                && map[yPoint_27923+1][xPoint_27923+1] === this.CELL_EMPTY
                && map[yPoint_27923+1][xPoint_27923] === this.CELL_EMPTY
                && map[yPoint_27923+1][xPoint_27923-1] === this.CELL_EMPTY
                && map[yPoint_27923][xPoint_27923-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27923, yPoint_27923, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27923;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27923][x] === this.CELL_EMPTY
                    && map[yPoint_27923-1][x] === this.CELL_EMPTY
                    && map[yPoint_27923-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27923][x+1] === this.CELL_EMPTY
                    && map[yPoint_27923+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27923+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27923, yPoint_27923, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27923;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27923] === this.CELL_EMPTY
                    && map[y+1][xPoint_27923] === this.CELL_EMPTY
                    && map[y+1][xPoint_27923+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27923] === this.CELL_EMPTY
                    && map[y][xPoint_27923-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27923-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
