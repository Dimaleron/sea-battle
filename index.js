...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27265=0;yPoint_27265<this.gameFieldBorderY.length; yPoint_27265++){
                for(var xPoint_27265=0;xPoint_27265<this.gameFieldBorderX.length; xPoint_27265++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27265, xPoint_27265);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27265][xPoint_27265] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27265, xPoint_27265, 'user');
                    if(this._userShipsMap[yPoint_27265][xPoint_27265] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27265, xPoint_27265, type){
            var id = this.getPointBlockIdByCoords(yPoint_27265, xPoint_27265, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27265);
                block.setAttribute('data-y', yPoint_27265);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27265
         * @param {type} xPoint_27265
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27265, xPoint_27265, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27265 + '_y' + yPoint_27265;
            }
            return 'pc_x' + xPoint_27265 + '_y' + yPoint_27265;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27265=0;yPoint_27265<this.gameFieldBorderY.length; yPoint_27265++){
                for(var xPoint_27265=0;xPoint_27265<this.gameFieldBorderX.length; xPoint_27265++){
                    map.push({y: yPoint_27265, x: xPoint_27265});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27265=-1;yPoint_27265<(this.gameFieldBorderY.length+1); yPoint_27265++){
                for(var xPoint_27265=-1;xPoint_27265<(this.gameFieldBorderX.length+1); xPoint_27265++){
                    if(!map[yPoint_27265]){
                        map[yPoint_27265] = [];
                    }
                    map[yPoint_27265][xPoint_27265] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27265 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27265 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27265, yPoint_27265) === true){
                    if(this.canPutHorizontal(map, xPoint_27265, yPoint_27265, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27265][xPoint_27265 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27265, yPoint_27265, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27265 + i][xPoint_27265] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27265, yPoint_27265){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27265][xPoint_27265] === this.CELL_EMPTY
                && map[yPoint_27265-1][xPoint_27265] === this.CELL_EMPTY
                && map[yPoint_27265-1][xPoint_27265+1] === this.CELL_EMPTY
                && map[yPoint_27265][xPoint_27265+1] === this.CELL_EMPTY
                && map[yPoint_27265+1][xPoint_27265+1] === this.CELL_EMPTY
                && map[yPoint_27265+1][xPoint_27265] === this.CELL_EMPTY
                && map[yPoint_27265+1][xPoint_27265-1] === this.CELL_EMPTY
                && map[yPoint_27265][xPoint_27265-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27265, yPoint_27265, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27265;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27265][x] === this.CELL_EMPTY
                    && map[yPoint_27265-1][x] === this.CELL_EMPTY
                    && map[yPoint_27265-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27265][x+1] === this.CELL_EMPTY
                    && map[yPoint_27265+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27265+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27265, yPoint_27265, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27265;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27265] === this.CELL_EMPTY
                    && map[y+1][xPoint_27265] === this.CELL_EMPTY
                    && map[y+1][xPoint_27265+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27265] === this.CELL_EMPTY
                    && map[y][xPoint_27265-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27265-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
