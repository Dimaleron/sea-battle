...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_382=0;yPoint_382<this.gameFieldBorderY.length; yPoint_382++){
                for(var xPoint_382=0;xPoint_382<this.gameFieldBorderX.length; xPoint_382++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_382, xPoint_382);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_382][xPoint_382] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_382, xPoint_382, 'user');
                    if(this._userShipsMap[yPoint_382][xPoint_382] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_382, xPoint_382, type){
            var id = this.getPointBlockIdByCoords(yPoint_382, xPoint_382, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_382);
                block.setAttribute('data-y', yPoint_382);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_382
         * @param {type} xPoint_382
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_382, xPoint_382, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_382 + '_y' + yPoint_382;
            }
            return 'pc_x' + xPoint_382 + '_y' + yPoint_382;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_382=0;yPoint_382<this.gameFieldBorderY.length; yPoint_382++){
                for(var xPoint_382=0;xPoint_382<this.gameFieldBorderX.length; xPoint_382++){
                    map.push({y: yPoint_382, x: xPoint_382});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_382=-1;yPoint_382<(this.gameFieldBorderY.length+1); yPoint_382++){
                for(var xPoint_382=-1;xPoint_382<(this.gameFieldBorderX.length+1); xPoint_382++){
                    if(!map[yPoint_382]){
                        map[yPoint_382] = [];
                    }
                    map[yPoint_382][xPoint_382] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_382 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_382 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_382, yPoint_382) === true){
                    if(this.canPutHorizontal(map, xPoint_382, yPoint_382, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_382][xPoint_382 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_382, yPoint_382, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_382 + i][xPoint_382] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_382, yPoint_382){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_382][xPoint_382] === this.CELL_EMPTY
                && map[yPoint_382-1][xPoint_382] === this.CELL_EMPTY
                && map[yPoint_382-1][xPoint_382+1] === this.CELL_EMPTY
                && map[yPoint_382][xPoint_382+1] === this.CELL_EMPTY
                && map[yPoint_382+1][xPoint_382+1] === this.CELL_EMPTY
                && map[yPoint_382+1][xPoint_382] === this.CELL_EMPTY
                && map[yPoint_382+1][xPoint_382-1] === this.CELL_EMPTY
                && map[yPoint_382][xPoint_382-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_382, yPoint_382, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_382;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_382][x] === this.CELL_EMPTY
                    && map[yPoint_382-1][x] === this.CELL_EMPTY
                    && map[yPoint_382-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_382][x+1] === this.CELL_EMPTY
                    && map[yPoint_382+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_382+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_382, yPoint_382, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_382;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_382] === this.CELL_EMPTY
                    && map[y+1][xPoint_382] === this.CELL_EMPTY
                    && map[y+1][xPoint_382+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_382] === this.CELL_EMPTY
                    && map[y][xPoint_382-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_382-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
