...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29964=0;yPoint_29964<this.gameFieldBorderY.length; yPoint_29964++){
                for(var xPoint_29964=0;xPoint_29964<this.gameFieldBorderX.length; xPoint_29964++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29964, xPoint_29964);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29964][xPoint_29964] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29964, xPoint_29964, 'user');
                    if(this._userShipsMap[yPoint_29964][xPoint_29964] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29964, xPoint_29964, type){
            var id = this.getPointBlockIdByCoords(yPoint_29964, xPoint_29964, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29964);
                block.setAttribute('data-y', yPoint_29964);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29964
         * @param {type} xPoint_29964
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29964, xPoint_29964, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29964 + '_y' + yPoint_29964;
            }
            return 'pc_x' + xPoint_29964 + '_y' + yPoint_29964;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29964=0;yPoint_29964<this.gameFieldBorderY.length; yPoint_29964++){
                for(var xPoint_29964=0;xPoint_29964<this.gameFieldBorderX.length; xPoint_29964++){
                    map.push({y: yPoint_29964, x: xPoint_29964});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29964=-1;yPoint_29964<(this.gameFieldBorderY.length+1); yPoint_29964++){
                for(var xPoint_29964=-1;xPoint_29964<(this.gameFieldBorderX.length+1); xPoint_29964++){
                    if(!map[yPoint_29964]){
                        map[yPoint_29964] = [];
                    }
                    map[yPoint_29964][xPoint_29964] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29964 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29964 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29964, yPoint_29964) === true){
                    if(this.canPutHorizontal(map, xPoint_29964, yPoint_29964, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29964][xPoint_29964 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29964, yPoint_29964, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29964 + i][xPoint_29964] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29964, yPoint_29964){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29964][xPoint_29964] === this.CELL_EMPTY
                && map[yPoint_29964-1][xPoint_29964] === this.CELL_EMPTY
                && map[yPoint_29964-1][xPoint_29964+1] === this.CELL_EMPTY
                && map[yPoint_29964][xPoint_29964+1] === this.CELL_EMPTY
                && map[yPoint_29964+1][xPoint_29964+1] === this.CELL_EMPTY
                && map[yPoint_29964+1][xPoint_29964] === this.CELL_EMPTY
                && map[yPoint_29964+1][xPoint_29964-1] === this.CELL_EMPTY
                && map[yPoint_29964][xPoint_29964-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29964, yPoint_29964, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29964;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29964][x] === this.CELL_EMPTY
                    && map[yPoint_29964-1][x] === this.CELL_EMPTY
                    && map[yPoint_29964-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29964][x+1] === this.CELL_EMPTY
                    && map[yPoint_29964+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29964+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29964, yPoint_29964, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29964;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29964] === this.CELL_EMPTY
                    && map[y+1][xPoint_29964] === this.CELL_EMPTY
                    && map[y+1][xPoint_29964+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29964] === this.CELL_EMPTY
                    && map[y][xPoint_29964-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29964-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
