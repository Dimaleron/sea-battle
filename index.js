...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6531=0;yPoint_6531<this.gameFieldBorderY.length; yPoint_6531++){
                for(var xPoint_6531=0;xPoint_6531<this.gameFieldBorderX.length; xPoint_6531++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6531, xPoint_6531);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6531][xPoint_6531] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6531, xPoint_6531, 'user');
                    if(this._userShipsMap[yPoint_6531][xPoint_6531] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6531, xPoint_6531, type){
            var id = this.getPointBlockIdByCoords(yPoint_6531, xPoint_6531, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6531);
                block.setAttribute('data-y', yPoint_6531);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6531
         * @param {type} xPoint_6531
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6531, xPoint_6531, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6531 + '_y' + yPoint_6531;
            }
            return 'pc_x' + xPoint_6531 + '_y' + yPoint_6531;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6531=0;yPoint_6531<this.gameFieldBorderY.length; yPoint_6531++){
                for(var xPoint_6531=0;xPoint_6531<this.gameFieldBorderX.length; xPoint_6531++){
                    map.push({y: yPoint_6531, x: xPoint_6531});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6531=-1;yPoint_6531<(this.gameFieldBorderY.length+1); yPoint_6531++){
                for(var xPoint_6531=-1;xPoint_6531<(this.gameFieldBorderX.length+1); xPoint_6531++){
                    if(!map[yPoint_6531]){
                        map[yPoint_6531] = [];
                    }
                    map[yPoint_6531][xPoint_6531] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6531 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6531 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6531, yPoint_6531) === true){
                    if(this.canPutHorizontal(map, xPoint_6531, yPoint_6531, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6531][xPoint_6531 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6531, yPoint_6531, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6531 + i][xPoint_6531] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6531, yPoint_6531){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6531][xPoint_6531] === this.CELL_EMPTY
                && map[yPoint_6531-1][xPoint_6531] === this.CELL_EMPTY
                && map[yPoint_6531-1][xPoint_6531+1] === this.CELL_EMPTY
                && map[yPoint_6531][xPoint_6531+1] === this.CELL_EMPTY
                && map[yPoint_6531+1][xPoint_6531+1] === this.CELL_EMPTY
                && map[yPoint_6531+1][xPoint_6531] === this.CELL_EMPTY
                && map[yPoint_6531+1][xPoint_6531-1] === this.CELL_EMPTY
                && map[yPoint_6531][xPoint_6531-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6531, yPoint_6531, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6531;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6531][x] === this.CELL_EMPTY
                    && map[yPoint_6531-1][x] === this.CELL_EMPTY
                    && map[yPoint_6531-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6531][x+1] === this.CELL_EMPTY
                    && map[yPoint_6531+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6531+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6531, yPoint_6531, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6531;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6531] === this.CELL_EMPTY
                    && map[y+1][xPoint_6531] === this.CELL_EMPTY
                    && map[y+1][xPoint_6531+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6531] === this.CELL_EMPTY
                    && map[y][xPoint_6531-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6531-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
