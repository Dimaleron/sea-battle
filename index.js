...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17666=0;yPoint_17666<this.gameFieldBorderY.length; yPoint_17666++){
                for(var xPoint_17666=0;xPoint_17666<this.gameFieldBorderX.length; xPoint_17666++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17666, xPoint_17666);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17666][xPoint_17666] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17666, xPoint_17666, 'user');
                    if(this._userShipsMap[yPoint_17666][xPoint_17666] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17666, xPoint_17666, type){
            var id = this.getPointBlockIdByCoords(yPoint_17666, xPoint_17666, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17666);
                block.setAttribute('data-y', yPoint_17666);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17666
         * @param {type} xPoint_17666
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17666, xPoint_17666, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17666 + '_y' + yPoint_17666;
            }
            return 'pc_x' + xPoint_17666 + '_y' + yPoint_17666;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17666=0;yPoint_17666<this.gameFieldBorderY.length; yPoint_17666++){
                for(var xPoint_17666=0;xPoint_17666<this.gameFieldBorderX.length; xPoint_17666++){
                    map.push({y: yPoint_17666, x: xPoint_17666});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17666=-1;yPoint_17666<(this.gameFieldBorderY.length+1); yPoint_17666++){
                for(var xPoint_17666=-1;xPoint_17666<(this.gameFieldBorderX.length+1); xPoint_17666++){
                    if(!map[yPoint_17666]){
                        map[yPoint_17666] = [];
                    }
                    map[yPoint_17666][xPoint_17666] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17666 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17666 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17666, yPoint_17666) === true){
                    if(this.canPutHorizontal(map, xPoint_17666, yPoint_17666, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17666][xPoint_17666 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17666, yPoint_17666, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17666 + i][xPoint_17666] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17666, yPoint_17666){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17666][xPoint_17666] === this.CELL_EMPTY
                && map[yPoint_17666-1][xPoint_17666] === this.CELL_EMPTY
                && map[yPoint_17666-1][xPoint_17666+1] === this.CELL_EMPTY
                && map[yPoint_17666][xPoint_17666+1] === this.CELL_EMPTY
                && map[yPoint_17666+1][xPoint_17666+1] === this.CELL_EMPTY
                && map[yPoint_17666+1][xPoint_17666] === this.CELL_EMPTY
                && map[yPoint_17666+1][xPoint_17666-1] === this.CELL_EMPTY
                && map[yPoint_17666][xPoint_17666-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17666, yPoint_17666, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17666;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17666][x] === this.CELL_EMPTY
                    && map[yPoint_17666-1][x] === this.CELL_EMPTY
                    && map[yPoint_17666-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17666][x+1] === this.CELL_EMPTY
                    && map[yPoint_17666+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17666+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17666, yPoint_17666, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17666;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17666] === this.CELL_EMPTY
                    && map[y+1][xPoint_17666] === this.CELL_EMPTY
                    && map[y+1][xPoint_17666+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17666] === this.CELL_EMPTY
                    && map[y][xPoint_17666-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17666-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
