...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21023=0;yPoint_21023<this.gameFieldBorderY.length; yPoint_21023++){
                for(var xPoint_21023=0;xPoint_21023<this.gameFieldBorderX.length; xPoint_21023++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21023, xPoint_21023);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21023][xPoint_21023] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21023, xPoint_21023, 'user');
                    if(this._userShipsMap[yPoint_21023][xPoint_21023] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21023, xPoint_21023, type){
            var id = this.getPointBlockIdByCoords(yPoint_21023, xPoint_21023, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21023);
                block.setAttribute('data-y', yPoint_21023);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21023
         * @param {type} xPoint_21023
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21023, xPoint_21023, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21023 + '_y' + yPoint_21023;
            }
            return 'pc_x' + xPoint_21023 + '_y' + yPoint_21023;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21023=0;yPoint_21023<this.gameFieldBorderY.length; yPoint_21023++){
                for(var xPoint_21023=0;xPoint_21023<this.gameFieldBorderX.length; xPoint_21023++){
                    map.push({y: yPoint_21023, x: xPoint_21023});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21023=-1;yPoint_21023<(this.gameFieldBorderY.length+1); yPoint_21023++){
                for(var xPoint_21023=-1;xPoint_21023<(this.gameFieldBorderX.length+1); xPoint_21023++){
                    if(!map[yPoint_21023]){
                        map[yPoint_21023] = [];
                    }
                    map[yPoint_21023][xPoint_21023] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21023 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21023 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21023, yPoint_21023) === true){
                    if(this.canPutHorizontal(map, xPoint_21023, yPoint_21023, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21023][xPoint_21023 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21023, yPoint_21023, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21023 + i][xPoint_21023] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21023, yPoint_21023){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21023][xPoint_21023] === this.CELL_EMPTY
                && map[yPoint_21023-1][xPoint_21023] === this.CELL_EMPTY
                && map[yPoint_21023-1][xPoint_21023+1] === this.CELL_EMPTY
                && map[yPoint_21023][xPoint_21023+1] === this.CELL_EMPTY
                && map[yPoint_21023+1][xPoint_21023+1] === this.CELL_EMPTY
                && map[yPoint_21023+1][xPoint_21023] === this.CELL_EMPTY
                && map[yPoint_21023+1][xPoint_21023-1] === this.CELL_EMPTY
                && map[yPoint_21023][xPoint_21023-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21023, yPoint_21023, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21023;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21023][x] === this.CELL_EMPTY
                    && map[yPoint_21023-1][x] === this.CELL_EMPTY
                    && map[yPoint_21023-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21023][x+1] === this.CELL_EMPTY
                    && map[yPoint_21023+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21023+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21023, yPoint_21023, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21023;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21023] === this.CELL_EMPTY
                    && map[y+1][xPoint_21023] === this.CELL_EMPTY
                    && map[y+1][xPoint_21023+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21023] === this.CELL_EMPTY
                    && map[y][xPoint_21023-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21023-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
