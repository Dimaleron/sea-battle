...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_439=0;yPoint_439<this.gameFieldBorderY.length; yPoint_439++){
                for(var xPoint_439=0;xPoint_439<this.gameFieldBorderX.length; xPoint_439++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_439, xPoint_439);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_439][xPoint_439] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_439, xPoint_439, 'user');
                    if(this._userShipsMap[yPoint_439][xPoint_439] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_439, xPoint_439, type){
            var id = this.getPointBlockIdByCoords(yPoint_439, xPoint_439, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_439);
                block.setAttribute('data-y', yPoint_439);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_439
         * @param {type} xPoint_439
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_439, xPoint_439, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_439 + '_y' + yPoint_439;
            }
            return 'pc_x' + xPoint_439 + '_y' + yPoint_439;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_439=0;yPoint_439<this.gameFieldBorderY.length; yPoint_439++){
                for(var xPoint_439=0;xPoint_439<this.gameFieldBorderX.length; xPoint_439++){
                    map.push({y: yPoint_439, x: xPoint_439});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_439=-1;yPoint_439<(this.gameFieldBorderY.length+1); yPoint_439++){
                for(var xPoint_439=-1;xPoint_439<(this.gameFieldBorderX.length+1); xPoint_439++){
                    if(!map[yPoint_439]){
                        map[yPoint_439] = [];
                    }
                    map[yPoint_439][xPoint_439] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_439 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_439 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_439, yPoint_439) === true){
                    if(this.canPutHorizontal(map, xPoint_439, yPoint_439, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_439][xPoint_439 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_439, yPoint_439, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_439 + i][xPoint_439] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_439, yPoint_439){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_439][xPoint_439] === this.CELL_EMPTY
                && map[yPoint_439-1][xPoint_439] === this.CELL_EMPTY
                && map[yPoint_439-1][xPoint_439+1] === this.CELL_EMPTY
                && map[yPoint_439][xPoint_439+1] === this.CELL_EMPTY
                && map[yPoint_439+1][xPoint_439+1] === this.CELL_EMPTY
                && map[yPoint_439+1][xPoint_439] === this.CELL_EMPTY
                && map[yPoint_439+1][xPoint_439-1] === this.CELL_EMPTY
                && map[yPoint_439][xPoint_439-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_439, yPoint_439, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_439;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_439][x] === this.CELL_EMPTY
                    && map[yPoint_439-1][x] === this.CELL_EMPTY
                    && map[yPoint_439-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_439][x+1] === this.CELL_EMPTY
                    && map[yPoint_439+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_439+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_439, yPoint_439, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_439;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_439] === this.CELL_EMPTY
                    && map[y+1][xPoint_439] === this.CELL_EMPTY
                    && map[y+1][xPoint_439+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_439] === this.CELL_EMPTY
                    && map[y][xPoint_439-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_439-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
