...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16407=0;yPoint_16407<this.gameFieldBorderY.length; yPoint_16407++){
                for(var xPoint_16407=0;xPoint_16407<this.gameFieldBorderX.length; xPoint_16407++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16407, xPoint_16407);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16407][xPoint_16407] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16407, xPoint_16407, 'user');
                    if(this._userShipsMap[yPoint_16407][xPoint_16407] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16407, xPoint_16407, type){
            var id = this.getPointBlockIdByCoords(yPoint_16407, xPoint_16407, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16407);
                block.setAttribute('data-y', yPoint_16407);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16407
         * @param {type} xPoint_16407
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16407, xPoint_16407, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16407 + '_y' + yPoint_16407;
            }
            return 'pc_x' + xPoint_16407 + '_y' + yPoint_16407;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16407=0;yPoint_16407<this.gameFieldBorderY.length; yPoint_16407++){
                for(var xPoint_16407=0;xPoint_16407<this.gameFieldBorderX.length; xPoint_16407++){
                    map.push({y: yPoint_16407, x: xPoint_16407});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16407=-1;yPoint_16407<(this.gameFieldBorderY.length+1); yPoint_16407++){
                for(var xPoint_16407=-1;xPoint_16407<(this.gameFieldBorderX.length+1); xPoint_16407++){
                    if(!map[yPoint_16407]){
                        map[yPoint_16407] = [];
                    }
                    map[yPoint_16407][xPoint_16407] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16407 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16407 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16407, yPoint_16407) === true){
                    if(this.canPutHorizontal(map, xPoint_16407, yPoint_16407, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16407][xPoint_16407 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16407, yPoint_16407, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16407 + i][xPoint_16407] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16407, yPoint_16407){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16407][xPoint_16407] === this.CELL_EMPTY
                && map[yPoint_16407-1][xPoint_16407] === this.CELL_EMPTY
                && map[yPoint_16407-1][xPoint_16407+1] === this.CELL_EMPTY
                && map[yPoint_16407][xPoint_16407+1] === this.CELL_EMPTY
                && map[yPoint_16407+1][xPoint_16407+1] === this.CELL_EMPTY
                && map[yPoint_16407+1][xPoint_16407] === this.CELL_EMPTY
                && map[yPoint_16407+1][xPoint_16407-1] === this.CELL_EMPTY
                && map[yPoint_16407][xPoint_16407-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16407, yPoint_16407, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16407;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16407][x] === this.CELL_EMPTY
                    && map[yPoint_16407-1][x] === this.CELL_EMPTY
                    && map[yPoint_16407-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16407][x+1] === this.CELL_EMPTY
                    && map[yPoint_16407+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16407+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16407, yPoint_16407, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16407;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16407] === this.CELL_EMPTY
                    && map[y+1][xPoint_16407] === this.CELL_EMPTY
                    && map[y+1][xPoint_16407+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16407] === this.CELL_EMPTY
                    && map[y][xPoint_16407-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16407-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
