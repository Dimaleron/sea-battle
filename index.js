...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10604=0;yPoint_10604<this.gameFieldBorderY.length; yPoint_10604++){
                for(var xPoint_10604=0;xPoint_10604<this.gameFieldBorderX.length; xPoint_10604++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10604, xPoint_10604);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10604][xPoint_10604] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10604, xPoint_10604, 'user');
                    if(this._userShipsMap[yPoint_10604][xPoint_10604] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10604, xPoint_10604, type){
            var id = this.getPointBlockIdByCoords(yPoint_10604, xPoint_10604, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10604);
                block.setAttribute('data-y', yPoint_10604);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10604
         * @param {type} xPoint_10604
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10604, xPoint_10604, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10604 + '_y' + yPoint_10604;
            }
            return 'pc_x' + xPoint_10604 + '_y' + yPoint_10604;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10604=0;yPoint_10604<this.gameFieldBorderY.length; yPoint_10604++){
                for(var xPoint_10604=0;xPoint_10604<this.gameFieldBorderX.length; xPoint_10604++){
                    map.push({y: yPoint_10604, x: xPoint_10604});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10604=-1;yPoint_10604<(this.gameFieldBorderY.length+1); yPoint_10604++){
                for(var xPoint_10604=-1;xPoint_10604<(this.gameFieldBorderX.length+1); xPoint_10604++){
                    if(!map[yPoint_10604]){
                        map[yPoint_10604] = [];
                    }
                    map[yPoint_10604][xPoint_10604] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10604 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10604 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10604, yPoint_10604) === true){
                    if(this.canPutHorizontal(map, xPoint_10604, yPoint_10604, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10604][xPoint_10604 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10604, yPoint_10604, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10604 + i][xPoint_10604] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10604, yPoint_10604){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10604][xPoint_10604] === this.CELL_EMPTY
                && map[yPoint_10604-1][xPoint_10604] === this.CELL_EMPTY
                && map[yPoint_10604-1][xPoint_10604+1] === this.CELL_EMPTY
                && map[yPoint_10604][xPoint_10604+1] === this.CELL_EMPTY
                && map[yPoint_10604+1][xPoint_10604+1] === this.CELL_EMPTY
                && map[yPoint_10604+1][xPoint_10604] === this.CELL_EMPTY
                && map[yPoint_10604+1][xPoint_10604-1] === this.CELL_EMPTY
                && map[yPoint_10604][xPoint_10604-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10604, yPoint_10604, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10604;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10604][x] === this.CELL_EMPTY
                    && map[yPoint_10604-1][x] === this.CELL_EMPTY
                    && map[yPoint_10604-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10604][x+1] === this.CELL_EMPTY
                    && map[yPoint_10604+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10604+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10604, yPoint_10604, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10604;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10604] === this.CELL_EMPTY
                    && map[y+1][xPoint_10604] === this.CELL_EMPTY
                    && map[y+1][xPoint_10604+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10604] === this.CELL_EMPTY
                    && map[y][xPoint_10604-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10604-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
