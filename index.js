...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3095=0;yPoint_3095<this.gameFieldBorderY.length; yPoint_3095++){
                for(var xPoint_3095=0;xPoint_3095<this.gameFieldBorderX.length; xPoint_3095++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3095, xPoint_3095);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3095][xPoint_3095] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3095, xPoint_3095, 'user');
                    if(this._userShipsMap[yPoint_3095][xPoint_3095] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3095, xPoint_3095, type){
            var id = this.getPointBlockIdByCoords(yPoint_3095, xPoint_3095, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3095);
                block.setAttribute('data-y', yPoint_3095);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3095
         * @param {type} xPoint_3095
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3095, xPoint_3095, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3095 + '_y' + yPoint_3095;
            }
            return 'pc_x' + xPoint_3095 + '_y' + yPoint_3095;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3095=0;yPoint_3095<this.gameFieldBorderY.length; yPoint_3095++){
                for(var xPoint_3095=0;xPoint_3095<this.gameFieldBorderX.length; xPoint_3095++){
                    map.push({y: yPoint_3095, x: xPoint_3095});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3095=-1;yPoint_3095<(this.gameFieldBorderY.length+1); yPoint_3095++){
                for(var xPoint_3095=-1;xPoint_3095<(this.gameFieldBorderX.length+1); xPoint_3095++){
                    if(!map[yPoint_3095]){
                        map[yPoint_3095] = [];
                    }
                    map[yPoint_3095][xPoint_3095] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3095 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3095 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3095, yPoint_3095) === true){
                    if(this.canPutHorizontal(map, xPoint_3095, yPoint_3095, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3095][xPoint_3095 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3095, yPoint_3095, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3095 + i][xPoint_3095] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3095, yPoint_3095){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3095][xPoint_3095] === this.CELL_EMPTY
                && map[yPoint_3095-1][xPoint_3095] === this.CELL_EMPTY
                && map[yPoint_3095-1][xPoint_3095+1] === this.CELL_EMPTY
                && map[yPoint_3095][xPoint_3095+1] === this.CELL_EMPTY
                && map[yPoint_3095+1][xPoint_3095+1] === this.CELL_EMPTY
                && map[yPoint_3095+1][xPoint_3095] === this.CELL_EMPTY
                && map[yPoint_3095+1][xPoint_3095-1] === this.CELL_EMPTY
                && map[yPoint_3095][xPoint_3095-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3095, yPoint_3095, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3095;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3095][x] === this.CELL_EMPTY
                    && map[yPoint_3095-1][x] === this.CELL_EMPTY
                    && map[yPoint_3095-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3095][x+1] === this.CELL_EMPTY
                    && map[yPoint_3095+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3095+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3095, yPoint_3095, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3095;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3095] === this.CELL_EMPTY
                    && map[y+1][xPoint_3095] === this.CELL_EMPTY
                    && map[y+1][xPoint_3095+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3095] === this.CELL_EMPTY
                    && map[y][xPoint_3095-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3095-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
