...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23466=0;yPoint_23466<this.gameFieldBorderY.length; yPoint_23466++){
                for(var xPoint_23466=0;xPoint_23466<this.gameFieldBorderX.length; xPoint_23466++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23466, xPoint_23466);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23466][xPoint_23466] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23466, xPoint_23466, 'user');
                    if(this._userShipsMap[yPoint_23466][xPoint_23466] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23466, xPoint_23466, type){
            var id = this.getPointBlockIdByCoords(yPoint_23466, xPoint_23466, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23466);
                block.setAttribute('data-y', yPoint_23466);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23466
         * @param {type} xPoint_23466
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23466, xPoint_23466, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23466 + '_y' + yPoint_23466;
            }
            return 'pc_x' + xPoint_23466 + '_y' + yPoint_23466;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23466=0;yPoint_23466<this.gameFieldBorderY.length; yPoint_23466++){
                for(var xPoint_23466=0;xPoint_23466<this.gameFieldBorderX.length; xPoint_23466++){
                    map.push({y: yPoint_23466, x: xPoint_23466});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23466=-1;yPoint_23466<(this.gameFieldBorderY.length+1); yPoint_23466++){
                for(var xPoint_23466=-1;xPoint_23466<(this.gameFieldBorderX.length+1); xPoint_23466++){
                    if(!map[yPoint_23466]){
                        map[yPoint_23466] = [];
                    }
                    map[yPoint_23466][xPoint_23466] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23466 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23466 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23466, yPoint_23466) === true){
                    if(this.canPutHorizontal(map, xPoint_23466, yPoint_23466, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23466][xPoint_23466 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23466, yPoint_23466, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23466 + i][xPoint_23466] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23466, yPoint_23466){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23466][xPoint_23466] === this.CELL_EMPTY
                && map[yPoint_23466-1][xPoint_23466] === this.CELL_EMPTY
                && map[yPoint_23466-1][xPoint_23466+1] === this.CELL_EMPTY
                && map[yPoint_23466][xPoint_23466+1] === this.CELL_EMPTY
                && map[yPoint_23466+1][xPoint_23466+1] === this.CELL_EMPTY
                && map[yPoint_23466+1][xPoint_23466] === this.CELL_EMPTY
                && map[yPoint_23466+1][xPoint_23466-1] === this.CELL_EMPTY
                && map[yPoint_23466][xPoint_23466-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23466, yPoint_23466, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23466;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23466][x] === this.CELL_EMPTY
                    && map[yPoint_23466-1][x] === this.CELL_EMPTY
                    && map[yPoint_23466-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23466][x+1] === this.CELL_EMPTY
                    && map[yPoint_23466+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23466+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23466, yPoint_23466, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23466;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23466] === this.CELL_EMPTY
                    && map[y+1][xPoint_23466] === this.CELL_EMPTY
                    && map[y+1][xPoint_23466+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23466] === this.CELL_EMPTY
                    && map[y][xPoint_23466-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23466-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
