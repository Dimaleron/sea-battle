...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1080=0;yPoint_1080<this.gameFieldBorderY.length; yPoint_1080++){
                for(var xPoint_1080=0;xPoint_1080<this.gameFieldBorderX.length; xPoint_1080++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1080, xPoint_1080);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1080][xPoint_1080] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1080, xPoint_1080, 'user');
                    if(this._userShipsMap[yPoint_1080][xPoint_1080] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1080, xPoint_1080, type){
            var id = this.getPointBlockIdByCoords(yPoint_1080, xPoint_1080, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1080);
                block.setAttribute('data-y', yPoint_1080);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1080
         * @param {type} xPoint_1080
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1080, xPoint_1080, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1080 + '_y' + yPoint_1080;
            }
            return 'pc_x' + xPoint_1080 + '_y' + yPoint_1080;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1080=0;yPoint_1080<this.gameFieldBorderY.length; yPoint_1080++){
                for(var xPoint_1080=0;xPoint_1080<this.gameFieldBorderX.length; xPoint_1080++){
                    map.push({y: yPoint_1080, x: xPoint_1080});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1080=-1;yPoint_1080<(this.gameFieldBorderY.length+1); yPoint_1080++){
                for(var xPoint_1080=-1;xPoint_1080<(this.gameFieldBorderX.length+1); xPoint_1080++){
                    if(!map[yPoint_1080]){
                        map[yPoint_1080] = [];
                    }
                    map[yPoint_1080][xPoint_1080] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1080 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1080 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1080, yPoint_1080) === true){
                    if(this.canPutHorizontal(map, xPoint_1080, yPoint_1080, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1080][xPoint_1080 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1080, yPoint_1080, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1080 + i][xPoint_1080] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1080, yPoint_1080){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1080][xPoint_1080] === this.CELL_EMPTY
                && map[yPoint_1080-1][xPoint_1080] === this.CELL_EMPTY
                && map[yPoint_1080-1][xPoint_1080+1] === this.CELL_EMPTY
                && map[yPoint_1080][xPoint_1080+1] === this.CELL_EMPTY
                && map[yPoint_1080+1][xPoint_1080+1] === this.CELL_EMPTY
                && map[yPoint_1080+1][xPoint_1080] === this.CELL_EMPTY
                && map[yPoint_1080+1][xPoint_1080-1] === this.CELL_EMPTY
                && map[yPoint_1080][xPoint_1080-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1080, yPoint_1080, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1080;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1080][x] === this.CELL_EMPTY
                    && map[yPoint_1080-1][x] === this.CELL_EMPTY
                    && map[yPoint_1080-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1080][x+1] === this.CELL_EMPTY
                    && map[yPoint_1080+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1080+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1080, yPoint_1080, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1080;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1080] === this.CELL_EMPTY
                    && map[y+1][xPoint_1080] === this.CELL_EMPTY
                    && map[y+1][xPoint_1080+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1080] === this.CELL_EMPTY
                    && map[y][xPoint_1080-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1080-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
