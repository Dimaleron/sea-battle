...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10093=0;yPoint_10093<this.gameFieldBorderY.length; yPoint_10093++){
                for(var xPoint_10093=0;xPoint_10093<this.gameFieldBorderX.length; xPoint_10093++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10093, xPoint_10093);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10093][xPoint_10093] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10093, xPoint_10093, 'user');
                    if(this._userShipsMap[yPoint_10093][xPoint_10093] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10093, xPoint_10093, type){
            var id = this.getPointBlockIdByCoords(yPoint_10093, xPoint_10093, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10093);
                block.setAttribute('data-y', yPoint_10093);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10093
         * @param {type} xPoint_10093
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10093, xPoint_10093, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10093 + '_y' + yPoint_10093;
            }
            return 'pc_x' + xPoint_10093 + '_y' + yPoint_10093;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10093=0;yPoint_10093<this.gameFieldBorderY.length; yPoint_10093++){
                for(var xPoint_10093=0;xPoint_10093<this.gameFieldBorderX.length; xPoint_10093++){
                    map.push({y: yPoint_10093, x: xPoint_10093});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10093=-1;yPoint_10093<(this.gameFieldBorderY.length+1); yPoint_10093++){
                for(var xPoint_10093=-1;xPoint_10093<(this.gameFieldBorderX.length+1); xPoint_10093++){
                    if(!map[yPoint_10093]){
                        map[yPoint_10093] = [];
                    }
                    map[yPoint_10093][xPoint_10093] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10093 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10093 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10093, yPoint_10093) === true){
                    if(this.canPutHorizontal(map, xPoint_10093, yPoint_10093, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10093][xPoint_10093 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10093, yPoint_10093, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10093 + i][xPoint_10093] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10093, yPoint_10093){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10093][xPoint_10093] === this.CELL_EMPTY
                && map[yPoint_10093-1][xPoint_10093] === this.CELL_EMPTY
                && map[yPoint_10093-1][xPoint_10093+1] === this.CELL_EMPTY
                && map[yPoint_10093][xPoint_10093+1] === this.CELL_EMPTY
                && map[yPoint_10093+1][xPoint_10093+1] === this.CELL_EMPTY
                && map[yPoint_10093+1][xPoint_10093] === this.CELL_EMPTY
                && map[yPoint_10093+1][xPoint_10093-1] === this.CELL_EMPTY
                && map[yPoint_10093][xPoint_10093-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10093, yPoint_10093, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10093;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10093][x] === this.CELL_EMPTY
                    && map[yPoint_10093-1][x] === this.CELL_EMPTY
                    && map[yPoint_10093-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10093][x+1] === this.CELL_EMPTY
                    && map[yPoint_10093+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10093+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10093, yPoint_10093, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10093;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10093] === this.CELL_EMPTY
                    && map[y+1][xPoint_10093] === this.CELL_EMPTY
                    && map[y+1][xPoint_10093+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10093] === this.CELL_EMPTY
                    && map[y][xPoint_10093-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10093-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
