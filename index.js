...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31602=0;yPoint_31602<this.gameFieldBorderY.length; yPoint_31602++){
                for(var xPoint_31602=0;xPoint_31602<this.gameFieldBorderX.length; xPoint_31602++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31602, xPoint_31602);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31602][xPoint_31602] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31602, xPoint_31602, 'user');
                    if(this._userShipsMap[yPoint_31602][xPoint_31602] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31602, xPoint_31602, type){
            var id = this.getPointBlockIdByCoords(yPoint_31602, xPoint_31602, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31602);
                block.setAttribute('data-y', yPoint_31602);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31602
         * @param {type} xPoint_31602
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31602, xPoint_31602, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31602 + '_y' + yPoint_31602;
            }
            return 'pc_x' + xPoint_31602 + '_y' + yPoint_31602;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31602=0;yPoint_31602<this.gameFieldBorderY.length; yPoint_31602++){
                for(var xPoint_31602=0;xPoint_31602<this.gameFieldBorderX.length; xPoint_31602++){
                    map.push({y: yPoint_31602, x: xPoint_31602});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31602=-1;yPoint_31602<(this.gameFieldBorderY.length+1); yPoint_31602++){
                for(var xPoint_31602=-1;xPoint_31602<(this.gameFieldBorderX.length+1); xPoint_31602++){
                    if(!map[yPoint_31602]){
                        map[yPoint_31602] = [];
                    }
                    map[yPoint_31602][xPoint_31602] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31602 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31602 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31602, yPoint_31602) === true){
                    if(this.canPutHorizontal(map, xPoint_31602, yPoint_31602, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31602][xPoint_31602 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31602, yPoint_31602, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31602 + i][xPoint_31602] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31602, yPoint_31602){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31602][xPoint_31602] === this.CELL_EMPTY
                && map[yPoint_31602-1][xPoint_31602] === this.CELL_EMPTY
                && map[yPoint_31602-1][xPoint_31602+1] === this.CELL_EMPTY
                && map[yPoint_31602][xPoint_31602+1] === this.CELL_EMPTY
                && map[yPoint_31602+1][xPoint_31602+1] === this.CELL_EMPTY
                && map[yPoint_31602+1][xPoint_31602] === this.CELL_EMPTY
                && map[yPoint_31602+1][xPoint_31602-1] === this.CELL_EMPTY
                && map[yPoint_31602][xPoint_31602-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31602, yPoint_31602, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31602;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31602][x] === this.CELL_EMPTY
                    && map[yPoint_31602-1][x] === this.CELL_EMPTY
                    && map[yPoint_31602-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31602][x+1] === this.CELL_EMPTY
                    && map[yPoint_31602+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31602+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31602, yPoint_31602, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31602;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31602] === this.CELL_EMPTY
                    && map[y+1][xPoint_31602] === this.CELL_EMPTY
                    && map[y+1][xPoint_31602+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31602] === this.CELL_EMPTY
                    && map[y][xPoint_31602-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31602-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
