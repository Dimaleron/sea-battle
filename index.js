...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16633=0;yPoint_16633<this.gameFieldBorderY.length; yPoint_16633++){
                for(var xPoint_16633=0;xPoint_16633<this.gameFieldBorderX.length; xPoint_16633++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16633, xPoint_16633);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16633][xPoint_16633] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16633, xPoint_16633, 'user');
                    if(this._userShipsMap[yPoint_16633][xPoint_16633] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16633, xPoint_16633, type){
            var id = this.getPointBlockIdByCoords(yPoint_16633, xPoint_16633, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16633);
                block.setAttribute('data-y', yPoint_16633);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16633
         * @param {type} xPoint_16633
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16633, xPoint_16633, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16633 + '_y' + yPoint_16633;
            }
            return 'pc_x' + xPoint_16633 + '_y' + yPoint_16633;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16633=0;yPoint_16633<this.gameFieldBorderY.length; yPoint_16633++){
                for(var xPoint_16633=0;xPoint_16633<this.gameFieldBorderX.length; xPoint_16633++){
                    map.push({y: yPoint_16633, x: xPoint_16633});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16633=-1;yPoint_16633<(this.gameFieldBorderY.length+1); yPoint_16633++){
                for(var xPoint_16633=-1;xPoint_16633<(this.gameFieldBorderX.length+1); xPoint_16633++){
                    if(!map[yPoint_16633]){
                        map[yPoint_16633] = [];
                    }
                    map[yPoint_16633][xPoint_16633] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16633 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16633 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16633, yPoint_16633) === true){
                    if(this.canPutHorizontal(map, xPoint_16633, yPoint_16633, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16633][xPoint_16633 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16633, yPoint_16633, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16633 + i][xPoint_16633] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16633, yPoint_16633){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16633][xPoint_16633] === this.CELL_EMPTY
                && map[yPoint_16633-1][xPoint_16633] === this.CELL_EMPTY
                && map[yPoint_16633-1][xPoint_16633+1] === this.CELL_EMPTY
                && map[yPoint_16633][xPoint_16633+1] === this.CELL_EMPTY
                && map[yPoint_16633+1][xPoint_16633+1] === this.CELL_EMPTY
                && map[yPoint_16633+1][xPoint_16633] === this.CELL_EMPTY
                && map[yPoint_16633+1][xPoint_16633-1] === this.CELL_EMPTY
                && map[yPoint_16633][xPoint_16633-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16633, yPoint_16633, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16633;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16633][x] === this.CELL_EMPTY
                    && map[yPoint_16633-1][x] === this.CELL_EMPTY
                    && map[yPoint_16633-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16633][x+1] === this.CELL_EMPTY
                    && map[yPoint_16633+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16633+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16633, yPoint_16633, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16633;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16633] === this.CELL_EMPTY
                    && map[y+1][xPoint_16633] === this.CELL_EMPTY
                    && map[y+1][xPoint_16633+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16633] === this.CELL_EMPTY
                    && map[y][xPoint_16633-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16633-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
