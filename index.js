...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27736=0;yPoint_27736<this.gameFieldBorderY.length; yPoint_27736++){
                for(var xPoint_27736=0;xPoint_27736<this.gameFieldBorderX.length; xPoint_27736++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27736, xPoint_27736);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27736][xPoint_27736] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27736, xPoint_27736, 'user');
                    if(this._userShipsMap[yPoint_27736][xPoint_27736] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27736, xPoint_27736, type){
            var id = this.getPointBlockIdByCoords(yPoint_27736, xPoint_27736, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27736);
                block.setAttribute('data-y', yPoint_27736);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27736
         * @param {type} xPoint_27736
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27736, xPoint_27736, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27736 + '_y' + yPoint_27736;
            }
            return 'pc_x' + xPoint_27736 + '_y' + yPoint_27736;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27736=0;yPoint_27736<this.gameFieldBorderY.length; yPoint_27736++){
                for(var xPoint_27736=0;xPoint_27736<this.gameFieldBorderX.length; xPoint_27736++){
                    map.push({y: yPoint_27736, x: xPoint_27736});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27736=-1;yPoint_27736<(this.gameFieldBorderY.length+1); yPoint_27736++){
                for(var xPoint_27736=-1;xPoint_27736<(this.gameFieldBorderX.length+1); xPoint_27736++){
                    if(!map[yPoint_27736]){
                        map[yPoint_27736] = [];
                    }
                    map[yPoint_27736][xPoint_27736] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27736 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27736 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27736, yPoint_27736) === true){
                    if(this.canPutHorizontal(map, xPoint_27736, yPoint_27736, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27736][xPoint_27736 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27736, yPoint_27736, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27736 + i][xPoint_27736] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27736, yPoint_27736){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27736][xPoint_27736] === this.CELL_EMPTY
                && map[yPoint_27736-1][xPoint_27736] === this.CELL_EMPTY
                && map[yPoint_27736-1][xPoint_27736+1] === this.CELL_EMPTY
                && map[yPoint_27736][xPoint_27736+1] === this.CELL_EMPTY
                && map[yPoint_27736+1][xPoint_27736+1] === this.CELL_EMPTY
                && map[yPoint_27736+1][xPoint_27736] === this.CELL_EMPTY
                && map[yPoint_27736+1][xPoint_27736-1] === this.CELL_EMPTY
                && map[yPoint_27736][xPoint_27736-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27736, yPoint_27736, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27736;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27736][x] === this.CELL_EMPTY
                    && map[yPoint_27736-1][x] === this.CELL_EMPTY
                    && map[yPoint_27736-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27736][x+1] === this.CELL_EMPTY
                    && map[yPoint_27736+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27736+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27736, yPoint_27736, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27736;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27736] === this.CELL_EMPTY
                    && map[y+1][xPoint_27736] === this.CELL_EMPTY
                    && map[y+1][xPoint_27736+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27736] === this.CELL_EMPTY
                    && map[y][xPoint_27736-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27736-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
