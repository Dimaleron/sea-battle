...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24787=0;yPoint_24787<this.gameFieldBorderY.length; yPoint_24787++){
                for(var xPoint_24787=0;xPoint_24787<this.gameFieldBorderX.length; xPoint_24787++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24787, xPoint_24787);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24787][xPoint_24787] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24787, xPoint_24787, 'user');
                    if(this._userShipsMap[yPoint_24787][xPoint_24787] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24787, xPoint_24787, type){
            var id = this.getPointBlockIdByCoords(yPoint_24787, xPoint_24787, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24787);
                block.setAttribute('data-y', yPoint_24787);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24787
         * @param {type} xPoint_24787
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24787, xPoint_24787, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24787 + '_y' + yPoint_24787;
            }
            return 'pc_x' + xPoint_24787 + '_y' + yPoint_24787;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24787=0;yPoint_24787<this.gameFieldBorderY.length; yPoint_24787++){
                for(var xPoint_24787=0;xPoint_24787<this.gameFieldBorderX.length; xPoint_24787++){
                    map.push({y: yPoint_24787, x: xPoint_24787});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24787=-1;yPoint_24787<(this.gameFieldBorderY.length+1); yPoint_24787++){
                for(var xPoint_24787=-1;xPoint_24787<(this.gameFieldBorderX.length+1); xPoint_24787++){
                    if(!map[yPoint_24787]){
                        map[yPoint_24787] = [];
                    }
                    map[yPoint_24787][xPoint_24787] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24787 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24787 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24787, yPoint_24787) === true){
                    if(this.canPutHorizontal(map, xPoint_24787, yPoint_24787, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24787][xPoint_24787 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24787, yPoint_24787, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24787 + i][xPoint_24787] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24787, yPoint_24787){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24787][xPoint_24787] === this.CELL_EMPTY
                && map[yPoint_24787-1][xPoint_24787] === this.CELL_EMPTY
                && map[yPoint_24787-1][xPoint_24787+1] === this.CELL_EMPTY
                && map[yPoint_24787][xPoint_24787+1] === this.CELL_EMPTY
                && map[yPoint_24787+1][xPoint_24787+1] === this.CELL_EMPTY
                && map[yPoint_24787+1][xPoint_24787] === this.CELL_EMPTY
                && map[yPoint_24787+1][xPoint_24787-1] === this.CELL_EMPTY
                && map[yPoint_24787][xPoint_24787-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24787, yPoint_24787, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24787;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24787][x] === this.CELL_EMPTY
                    && map[yPoint_24787-1][x] === this.CELL_EMPTY
                    && map[yPoint_24787-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24787][x+1] === this.CELL_EMPTY
                    && map[yPoint_24787+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24787+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24787, yPoint_24787, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24787;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24787] === this.CELL_EMPTY
                    && map[y+1][xPoint_24787] === this.CELL_EMPTY
                    && map[y+1][xPoint_24787+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24787] === this.CELL_EMPTY
                    && map[y][xPoint_24787-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24787-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
