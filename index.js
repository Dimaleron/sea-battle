...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_340=0;yPoint_340<this.gameFieldBorderY.length; yPoint_340++){
                for(var xPoint_340=0;xPoint_340<this.gameFieldBorderX.length; xPoint_340++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_340, xPoint_340);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_340][xPoint_340] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_340, xPoint_340, 'user');
                    if(this._userShipsMap[yPoint_340][xPoint_340] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_340, xPoint_340, type){
            var id = this.getPointBlockIdByCoords(yPoint_340, xPoint_340, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_340);
                block.setAttribute('data-y', yPoint_340);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_340
         * @param {type} xPoint_340
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_340, xPoint_340, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_340 + '_y' + yPoint_340;
            }
            return 'pc_x' + xPoint_340 + '_y' + yPoint_340;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_340=0;yPoint_340<this.gameFieldBorderY.length; yPoint_340++){
                for(var xPoint_340=0;xPoint_340<this.gameFieldBorderX.length; xPoint_340++){
                    map.push({y: yPoint_340, x: xPoint_340});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_340=-1;yPoint_340<(this.gameFieldBorderY.length+1); yPoint_340++){
                for(var xPoint_340=-1;xPoint_340<(this.gameFieldBorderX.length+1); xPoint_340++){
                    if(!map[yPoint_340]){
                        map[yPoint_340] = [];
                    }
                    map[yPoint_340][xPoint_340] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_340 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_340 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_340, yPoint_340) === true){
                    if(this.canPutHorizontal(map, xPoint_340, yPoint_340, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_340][xPoint_340 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_340, yPoint_340, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_340 + i][xPoint_340] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_340, yPoint_340){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_340][xPoint_340] === this.CELL_EMPTY
                && map[yPoint_340-1][xPoint_340] === this.CELL_EMPTY
                && map[yPoint_340-1][xPoint_340+1] === this.CELL_EMPTY
                && map[yPoint_340][xPoint_340+1] === this.CELL_EMPTY
                && map[yPoint_340+1][xPoint_340+1] === this.CELL_EMPTY
                && map[yPoint_340+1][xPoint_340] === this.CELL_EMPTY
                && map[yPoint_340+1][xPoint_340-1] === this.CELL_EMPTY
                && map[yPoint_340][xPoint_340-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_340, yPoint_340, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_340;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_340][x] === this.CELL_EMPTY
                    && map[yPoint_340-1][x] === this.CELL_EMPTY
                    && map[yPoint_340-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_340][x+1] === this.CELL_EMPTY
                    && map[yPoint_340+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_340+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_340, yPoint_340, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_340;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_340] === this.CELL_EMPTY
                    && map[y+1][xPoint_340] === this.CELL_EMPTY
                    && map[y+1][xPoint_340+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_340] === this.CELL_EMPTY
                    && map[y][xPoint_340-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_340-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
