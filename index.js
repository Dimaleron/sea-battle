...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6800=0;yPoint_6800<this.gameFieldBorderY.length; yPoint_6800++){
                for(var xPoint_6800=0;xPoint_6800<this.gameFieldBorderX.length; xPoint_6800++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6800, xPoint_6800);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6800][xPoint_6800] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6800, xPoint_6800, 'user');
                    if(this._userShipsMap[yPoint_6800][xPoint_6800] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6800, xPoint_6800, type){
            var id = this.getPointBlockIdByCoords(yPoint_6800, xPoint_6800, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6800);
                block.setAttribute('data-y', yPoint_6800);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6800
         * @param {type} xPoint_6800
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6800, xPoint_6800, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6800 + '_y' + yPoint_6800;
            }
            return 'pc_x' + xPoint_6800 + '_y' + yPoint_6800;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6800=0;yPoint_6800<this.gameFieldBorderY.length; yPoint_6800++){
                for(var xPoint_6800=0;xPoint_6800<this.gameFieldBorderX.length; xPoint_6800++){
                    map.push({y: yPoint_6800, x: xPoint_6800});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6800=-1;yPoint_6800<(this.gameFieldBorderY.length+1); yPoint_6800++){
                for(var xPoint_6800=-1;xPoint_6800<(this.gameFieldBorderX.length+1); xPoint_6800++){
                    if(!map[yPoint_6800]){
                        map[yPoint_6800] = [];
                    }
                    map[yPoint_6800][xPoint_6800] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6800 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6800 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6800, yPoint_6800) === true){
                    if(this.canPutHorizontal(map, xPoint_6800, yPoint_6800, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6800][xPoint_6800 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6800, yPoint_6800, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6800 + i][xPoint_6800] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6800, yPoint_6800){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6800][xPoint_6800] === this.CELL_EMPTY
                && map[yPoint_6800-1][xPoint_6800] === this.CELL_EMPTY
                && map[yPoint_6800-1][xPoint_6800+1] === this.CELL_EMPTY
                && map[yPoint_6800][xPoint_6800+1] === this.CELL_EMPTY
                && map[yPoint_6800+1][xPoint_6800+1] === this.CELL_EMPTY
                && map[yPoint_6800+1][xPoint_6800] === this.CELL_EMPTY
                && map[yPoint_6800+1][xPoint_6800-1] === this.CELL_EMPTY
                && map[yPoint_6800][xPoint_6800-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6800, yPoint_6800, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6800;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6800][x] === this.CELL_EMPTY
                    && map[yPoint_6800-1][x] === this.CELL_EMPTY
                    && map[yPoint_6800-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6800][x+1] === this.CELL_EMPTY
                    && map[yPoint_6800+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6800+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6800, yPoint_6800, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6800;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6800] === this.CELL_EMPTY
                    && map[y+1][xPoint_6800] === this.CELL_EMPTY
                    && map[y+1][xPoint_6800+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6800] === this.CELL_EMPTY
                    && map[y][xPoint_6800-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6800-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
