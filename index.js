...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30492=0;yPoint_30492<this.gameFieldBorderY.length; yPoint_30492++){
                for(var xPoint_30492=0;xPoint_30492<this.gameFieldBorderX.length; xPoint_30492++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30492, xPoint_30492);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30492][xPoint_30492] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30492, xPoint_30492, 'user');
                    if(this._userShipsMap[yPoint_30492][xPoint_30492] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30492, xPoint_30492, type){
            var id = this.getPointBlockIdByCoords(yPoint_30492, xPoint_30492, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30492);
                block.setAttribute('data-y', yPoint_30492);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30492
         * @param {type} xPoint_30492
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30492, xPoint_30492, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30492 + '_y' + yPoint_30492;
            }
            return 'pc_x' + xPoint_30492 + '_y' + yPoint_30492;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30492=0;yPoint_30492<this.gameFieldBorderY.length; yPoint_30492++){
                for(var xPoint_30492=0;xPoint_30492<this.gameFieldBorderX.length; xPoint_30492++){
                    map.push({y: yPoint_30492, x: xPoint_30492});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30492=-1;yPoint_30492<(this.gameFieldBorderY.length+1); yPoint_30492++){
                for(var xPoint_30492=-1;xPoint_30492<(this.gameFieldBorderX.length+1); xPoint_30492++){
                    if(!map[yPoint_30492]){
                        map[yPoint_30492] = [];
                    }
                    map[yPoint_30492][xPoint_30492] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30492 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30492 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30492, yPoint_30492) === true){
                    if(this.canPutHorizontal(map, xPoint_30492, yPoint_30492, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30492][xPoint_30492 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30492, yPoint_30492, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30492 + i][xPoint_30492] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30492, yPoint_30492){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30492][xPoint_30492] === this.CELL_EMPTY
                && map[yPoint_30492-1][xPoint_30492] === this.CELL_EMPTY
                && map[yPoint_30492-1][xPoint_30492+1] === this.CELL_EMPTY
                && map[yPoint_30492][xPoint_30492+1] === this.CELL_EMPTY
                && map[yPoint_30492+1][xPoint_30492+1] === this.CELL_EMPTY
                && map[yPoint_30492+1][xPoint_30492] === this.CELL_EMPTY
                && map[yPoint_30492+1][xPoint_30492-1] === this.CELL_EMPTY
                && map[yPoint_30492][xPoint_30492-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30492, yPoint_30492, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30492;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30492][x] === this.CELL_EMPTY
                    && map[yPoint_30492-1][x] === this.CELL_EMPTY
                    && map[yPoint_30492-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30492][x+1] === this.CELL_EMPTY
                    && map[yPoint_30492+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30492+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30492, yPoint_30492, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30492;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30492] === this.CELL_EMPTY
                    && map[y+1][xPoint_30492] === this.CELL_EMPTY
                    && map[y+1][xPoint_30492+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30492] === this.CELL_EMPTY
                    && map[y][xPoint_30492-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30492-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
