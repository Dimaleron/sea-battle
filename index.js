...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22324=0;yPoint_22324<this.gameFieldBorderY.length; yPoint_22324++){
                for(var xPoint_22324=0;xPoint_22324<this.gameFieldBorderX.length; xPoint_22324++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22324, xPoint_22324);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22324][xPoint_22324] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22324, xPoint_22324, 'user');
                    if(this._userShipsMap[yPoint_22324][xPoint_22324] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22324, xPoint_22324, type){
            var id = this.getPointBlockIdByCoords(yPoint_22324, xPoint_22324, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22324);
                block.setAttribute('data-y', yPoint_22324);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22324
         * @param {type} xPoint_22324
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22324, xPoint_22324, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22324 + '_y' + yPoint_22324;
            }
            return 'pc_x' + xPoint_22324 + '_y' + yPoint_22324;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22324=0;yPoint_22324<this.gameFieldBorderY.length; yPoint_22324++){
                for(var xPoint_22324=0;xPoint_22324<this.gameFieldBorderX.length; xPoint_22324++){
                    map.push({y: yPoint_22324, x: xPoint_22324});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22324=-1;yPoint_22324<(this.gameFieldBorderY.length+1); yPoint_22324++){
                for(var xPoint_22324=-1;xPoint_22324<(this.gameFieldBorderX.length+1); xPoint_22324++){
                    if(!map[yPoint_22324]){
                        map[yPoint_22324] = [];
                    }
                    map[yPoint_22324][xPoint_22324] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22324 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22324 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22324, yPoint_22324) === true){
                    if(this.canPutHorizontal(map, xPoint_22324, yPoint_22324, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22324][xPoint_22324 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22324, yPoint_22324, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22324 + i][xPoint_22324] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22324, yPoint_22324){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22324][xPoint_22324] === this.CELL_EMPTY
                && map[yPoint_22324-1][xPoint_22324] === this.CELL_EMPTY
                && map[yPoint_22324-1][xPoint_22324+1] === this.CELL_EMPTY
                && map[yPoint_22324][xPoint_22324+1] === this.CELL_EMPTY
                && map[yPoint_22324+1][xPoint_22324+1] === this.CELL_EMPTY
                && map[yPoint_22324+1][xPoint_22324] === this.CELL_EMPTY
                && map[yPoint_22324+1][xPoint_22324-1] === this.CELL_EMPTY
                && map[yPoint_22324][xPoint_22324-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22324, yPoint_22324, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22324;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22324][x] === this.CELL_EMPTY
                    && map[yPoint_22324-1][x] === this.CELL_EMPTY
                    && map[yPoint_22324-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22324][x+1] === this.CELL_EMPTY
                    && map[yPoint_22324+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22324+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22324, yPoint_22324, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22324;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22324] === this.CELL_EMPTY
                    && map[y+1][xPoint_22324] === this.CELL_EMPTY
                    && map[y+1][xPoint_22324+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22324] === this.CELL_EMPTY
                    && map[y][xPoint_22324-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22324-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
