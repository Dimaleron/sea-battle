...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11119=0;yPoint_11119<this.gameFieldBorderY.length; yPoint_11119++){
                for(var xPoint_11119=0;xPoint_11119<this.gameFieldBorderX.length; xPoint_11119++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11119, xPoint_11119);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11119][xPoint_11119] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11119, xPoint_11119, 'user');
                    if(this._userShipsMap[yPoint_11119][xPoint_11119] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11119, xPoint_11119, type){
            var id = this.getPointBlockIdByCoords(yPoint_11119, xPoint_11119, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11119);
                block.setAttribute('data-y', yPoint_11119);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11119
         * @param {type} xPoint_11119
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11119, xPoint_11119, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11119 + '_y' + yPoint_11119;
            }
            return 'pc_x' + xPoint_11119 + '_y' + yPoint_11119;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11119=0;yPoint_11119<this.gameFieldBorderY.length; yPoint_11119++){
                for(var xPoint_11119=0;xPoint_11119<this.gameFieldBorderX.length; xPoint_11119++){
                    map.push({y: yPoint_11119, x: xPoint_11119});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11119=-1;yPoint_11119<(this.gameFieldBorderY.length+1); yPoint_11119++){
                for(var xPoint_11119=-1;xPoint_11119<(this.gameFieldBorderX.length+1); xPoint_11119++){
                    if(!map[yPoint_11119]){
                        map[yPoint_11119] = [];
                    }
                    map[yPoint_11119][xPoint_11119] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11119 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11119 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11119, yPoint_11119) === true){
                    if(this.canPutHorizontal(map, xPoint_11119, yPoint_11119, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11119][xPoint_11119 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11119, yPoint_11119, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11119 + i][xPoint_11119] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11119, yPoint_11119){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11119][xPoint_11119] === this.CELL_EMPTY
                && map[yPoint_11119-1][xPoint_11119] === this.CELL_EMPTY
                && map[yPoint_11119-1][xPoint_11119+1] === this.CELL_EMPTY
                && map[yPoint_11119][xPoint_11119+1] === this.CELL_EMPTY
                && map[yPoint_11119+1][xPoint_11119+1] === this.CELL_EMPTY
                && map[yPoint_11119+1][xPoint_11119] === this.CELL_EMPTY
                && map[yPoint_11119+1][xPoint_11119-1] === this.CELL_EMPTY
                && map[yPoint_11119][xPoint_11119-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11119, yPoint_11119, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11119;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11119][x] === this.CELL_EMPTY
                    && map[yPoint_11119-1][x] === this.CELL_EMPTY
                    && map[yPoint_11119-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11119][x+1] === this.CELL_EMPTY
                    && map[yPoint_11119+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11119+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11119, yPoint_11119, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11119;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11119] === this.CELL_EMPTY
                    && map[y+1][xPoint_11119] === this.CELL_EMPTY
                    && map[y+1][xPoint_11119+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11119] === this.CELL_EMPTY
                    && map[y][xPoint_11119-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11119-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
