...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7730=0;yPoint_7730<this.gameFieldBorderY.length; yPoint_7730++){
                for(var xPoint_7730=0;xPoint_7730<this.gameFieldBorderX.length; xPoint_7730++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7730, xPoint_7730);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7730][xPoint_7730] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7730, xPoint_7730, 'user');
                    if(this._userShipsMap[yPoint_7730][xPoint_7730] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7730, xPoint_7730, type){
            var id = this.getPointBlockIdByCoords(yPoint_7730, xPoint_7730, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7730);
                block.setAttribute('data-y', yPoint_7730);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7730
         * @param {type} xPoint_7730
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7730, xPoint_7730, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7730 + '_y' + yPoint_7730;
            }
            return 'pc_x' + xPoint_7730 + '_y' + yPoint_7730;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7730=0;yPoint_7730<this.gameFieldBorderY.length; yPoint_7730++){
                for(var xPoint_7730=0;xPoint_7730<this.gameFieldBorderX.length; xPoint_7730++){
                    map.push({y: yPoint_7730, x: xPoint_7730});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7730=-1;yPoint_7730<(this.gameFieldBorderY.length+1); yPoint_7730++){
                for(var xPoint_7730=-1;xPoint_7730<(this.gameFieldBorderX.length+1); xPoint_7730++){
                    if(!map[yPoint_7730]){
                        map[yPoint_7730] = [];
                    }
                    map[yPoint_7730][xPoint_7730] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7730 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7730 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7730, yPoint_7730) === true){
                    if(this.canPutHorizontal(map, xPoint_7730, yPoint_7730, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7730][xPoint_7730 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7730, yPoint_7730, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7730 + i][xPoint_7730] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7730, yPoint_7730){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7730][xPoint_7730] === this.CELL_EMPTY
                && map[yPoint_7730-1][xPoint_7730] === this.CELL_EMPTY
                && map[yPoint_7730-1][xPoint_7730+1] === this.CELL_EMPTY
                && map[yPoint_7730][xPoint_7730+1] === this.CELL_EMPTY
                && map[yPoint_7730+1][xPoint_7730+1] === this.CELL_EMPTY
                && map[yPoint_7730+1][xPoint_7730] === this.CELL_EMPTY
                && map[yPoint_7730+1][xPoint_7730-1] === this.CELL_EMPTY
                && map[yPoint_7730][xPoint_7730-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7730, yPoint_7730, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7730;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7730][x] === this.CELL_EMPTY
                    && map[yPoint_7730-1][x] === this.CELL_EMPTY
                    && map[yPoint_7730-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7730][x+1] === this.CELL_EMPTY
                    && map[yPoint_7730+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7730+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7730, yPoint_7730, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7730;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7730] === this.CELL_EMPTY
                    && map[y+1][xPoint_7730] === this.CELL_EMPTY
                    && map[y+1][xPoint_7730+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7730] === this.CELL_EMPTY
                    && map[y][xPoint_7730-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7730-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
