...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26377=0;yPoint_26377<this.gameFieldBorderY.length; yPoint_26377++){
                for(var xPoint_26377=0;xPoint_26377<this.gameFieldBorderX.length; xPoint_26377++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26377, xPoint_26377);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26377][xPoint_26377] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26377, xPoint_26377, 'user');
                    if(this._userShipsMap[yPoint_26377][xPoint_26377] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26377, xPoint_26377, type){
            var id = this.getPointBlockIdByCoords(yPoint_26377, xPoint_26377, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26377);
                block.setAttribute('data-y', yPoint_26377);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26377
         * @param {type} xPoint_26377
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26377, xPoint_26377, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26377 + '_y' + yPoint_26377;
            }
            return 'pc_x' + xPoint_26377 + '_y' + yPoint_26377;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26377=0;yPoint_26377<this.gameFieldBorderY.length; yPoint_26377++){
                for(var xPoint_26377=0;xPoint_26377<this.gameFieldBorderX.length; xPoint_26377++){
                    map.push({y: yPoint_26377, x: xPoint_26377});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26377=-1;yPoint_26377<(this.gameFieldBorderY.length+1); yPoint_26377++){
                for(var xPoint_26377=-1;xPoint_26377<(this.gameFieldBorderX.length+1); xPoint_26377++){
                    if(!map[yPoint_26377]){
                        map[yPoint_26377] = [];
                    }
                    map[yPoint_26377][xPoint_26377] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26377 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26377 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26377, yPoint_26377) === true){
                    if(this.canPutHorizontal(map, xPoint_26377, yPoint_26377, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26377][xPoint_26377 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26377, yPoint_26377, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26377 + i][xPoint_26377] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26377, yPoint_26377){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26377][xPoint_26377] === this.CELL_EMPTY
                && map[yPoint_26377-1][xPoint_26377] === this.CELL_EMPTY
                && map[yPoint_26377-1][xPoint_26377+1] === this.CELL_EMPTY
                && map[yPoint_26377][xPoint_26377+1] === this.CELL_EMPTY
                && map[yPoint_26377+1][xPoint_26377+1] === this.CELL_EMPTY
                && map[yPoint_26377+1][xPoint_26377] === this.CELL_EMPTY
                && map[yPoint_26377+1][xPoint_26377-1] === this.CELL_EMPTY
                && map[yPoint_26377][xPoint_26377-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26377, yPoint_26377, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26377;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26377][x] === this.CELL_EMPTY
                    && map[yPoint_26377-1][x] === this.CELL_EMPTY
                    && map[yPoint_26377-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26377][x+1] === this.CELL_EMPTY
                    && map[yPoint_26377+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26377+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26377, yPoint_26377, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26377;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26377] === this.CELL_EMPTY
                    && map[y+1][xPoint_26377] === this.CELL_EMPTY
                    && map[y+1][xPoint_26377+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26377] === this.CELL_EMPTY
                    && map[y][xPoint_26377-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26377-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
