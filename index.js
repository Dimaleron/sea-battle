...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16674=0;yPoint_16674<this.gameFieldBorderY.length; yPoint_16674++){
                for(var xPoint_16674=0;xPoint_16674<this.gameFieldBorderX.length; xPoint_16674++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16674, xPoint_16674);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16674][xPoint_16674] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16674, xPoint_16674, 'user');
                    if(this._userShipsMap[yPoint_16674][xPoint_16674] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16674, xPoint_16674, type){
            var id = this.getPointBlockIdByCoords(yPoint_16674, xPoint_16674, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16674);
                block.setAttribute('data-y', yPoint_16674);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16674
         * @param {type} xPoint_16674
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16674, xPoint_16674, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16674 + '_y' + yPoint_16674;
            }
            return 'pc_x' + xPoint_16674 + '_y' + yPoint_16674;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16674=0;yPoint_16674<this.gameFieldBorderY.length; yPoint_16674++){
                for(var xPoint_16674=0;xPoint_16674<this.gameFieldBorderX.length; xPoint_16674++){
                    map.push({y: yPoint_16674, x: xPoint_16674});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16674=-1;yPoint_16674<(this.gameFieldBorderY.length+1); yPoint_16674++){
                for(var xPoint_16674=-1;xPoint_16674<(this.gameFieldBorderX.length+1); xPoint_16674++){
                    if(!map[yPoint_16674]){
                        map[yPoint_16674] = [];
                    }
                    map[yPoint_16674][xPoint_16674] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16674 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16674 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16674, yPoint_16674) === true){
                    if(this.canPutHorizontal(map, xPoint_16674, yPoint_16674, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16674][xPoint_16674 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16674, yPoint_16674, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16674 + i][xPoint_16674] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16674, yPoint_16674){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16674][xPoint_16674] === this.CELL_EMPTY
                && map[yPoint_16674-1][xPoint_16674] === this.CELL_EMPTY
                && map[yPoint_16674-1][xPoint_16674+1] === this.CELL_EMPTY
                && map[yPoint_16674][xPoint_16674+1] === this.CELL_EMPTY
                && map[yPoint_16674+1][xPoint_16674+1] === this.CELL_EMPTY
                && map[yPoint_16674+1][xPoint_16674] === this.CELL_EMPTY
                && map[yPoint_16674+1][xPoint_16674-1] === this.CELL_EMPTY
                && map[yPoint_16674][xPoint_16674-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16674, yPoint_16674, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16674;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16674][x] === this.CELL_EMPTY
                    && map[yPoint_16674-1][x] === this.CELL_EMPTY
                    && map[yPoint_16674-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16674][x+1] === this.CELL_EMPTY
                    && map[yPoint_16674+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16674+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16674, yPoint_16674, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16674;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16674] === this.CELL_EMPTY
                    && map[y+1][xPoint_16674] === this.CELL_EMPTY
                    && map[y+1][xPoint_16674+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16674] === this.CELL_EMPTY
                    && map[y][xPoint_16674-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16674-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
