...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13037=0;yPoint_13037<this.gameFieldBorderY.length; yPoint_13037++){
                for(var xPoint_13037=0;xPoint_13037<this.gameFieldBorderX.length; xPoint_13037++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13037, xPoint_13037);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13037][xPoint_13037] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13037, xPoint_13037, 'user');
                    if(this._userShipsMap[yPoint_13037][xPoint_13037] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13037, xPoint_13037, type){
            var id = this.getPointBlockIdByCoords(yPoint_13037, xPoint_13037, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13037);
                block.setAttribute('data-y', yPoint_13037);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13037
         * @param {type} xPoint_13037
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13037, xPoint_13037, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13037 + '_y' + yPoint_13037;
            }
            return 'pc_x' + xPoint_13037 + '_y' + yPoint_13037;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13037=0;yPoint_13037<this.gameFieldBorderY.length; yPoint_13037++){
                for(var xPoint_13037=0;xPoint_13037<this.gameFieldBorderX.length; xPoint_13037++){
                    map.push({y: yPoint_13037, x: xPoint_13037});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13037=-1;yPoint_13037<(this.gameFieldBorderY.length+1); yPoint_13037++){
                for(var xPoint_13037=-1;xPoint_13037<(this.gameFieldBorderX.length+1); xPoint_13037++){
                    if(!map[yPoint_13037]){
                        map[yPoint_13037] = [];
                    }
                    map[yPoint_13037][xPoint_13037] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13037 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13037 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13037, yPoint_13037) === true){
                    if(this.canPutHorizontal(map, xPoint_13037, yPoint_13037, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13037][xPoint_13037 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13037, yPoint_13037, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13037 + i][xPoint_13037] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13037, yPoint_13037){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13037][xPoint_13037] === this.CELL_EMPTY
                && map[yPoint_13037-1][xPoint_13037] === this.CELL_EMPTY
                && map[yPoint_13037-1][xPoint_13037+1] === this.CELL_EMPTY
                && map[yPoint_13037][xPoint_13037+1] === this.CELL_EMPTY
                && map[yPoint_13037+1][xPoint_13037+1] === this.CELL_EMPTY
                && map[yPoint_13037+1][xPoint_13037] === this.CELL_EMPTY
                && map[yPoint_13037+1][xPoint_13037-1] === this.CELL_EMPTY
                && map[yPoint_13037][xPoint_13037-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13037, yPoint_13037, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13037;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13037][x] === this.CELL_EMPTY
                    && map[yPoint_13037-1][x] === this.CELL_EMPTY
                    && map[yPoint_13037-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13037][x+1] === this.CELL_EMPTY
                    && map[yPoint_13037+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13037+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13037, yPoint_13037, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13037;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13037] === this.CELL_EMPTY
                    && map[y+1][xPoint_13037] === this.CELL_EMPTY
                    && map[y+1][xPoint_13037+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13037] === this.CELL_EMPTY
                    && map[y][xPoint_13037-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13037-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
