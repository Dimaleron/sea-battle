...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13813=0;yPoint_13813<this.gameFieldBorderY.length; yPoint_13813++){
                for(var xPoint_13813=0;xPoint_13813<this.gameFieldBorderX.length; xPoint_13813++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13813, xPoint_13813);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13813][xPoint_13813] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13813, xPoint_13813, 'user');
                    if(this._userShipsMap[yPoint_13813][xPoint_13813] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13813, xPoint_13813, type){
            var id = this.getPointBlockIdByCoords(yPoint_13813, xPoint_13813, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13813);
                block.setAttribute('data-y', yPoint_13813);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13813
         * @param {type} xPoint_13813
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13813, xPoint_13813, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13813 + '_y' + yPoint_13813;
            }
            return 'pc_x' + xPoint_13813 + '_y' + yPoint_13813;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13813=0;yPoint_13813<this.gameFieldBorderY.length; yPoint_13813++){
                for(var xPoint_13813=0;xPoint_13813<this.gameFieldBorderX.length; xPoint_13813++){
                    map.push({y: yPoint_13813, x: xPoint_13813});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13813=-1;yPoint_13813<(this.gameFieldBorderY.length+1); yPoint_13813++){
                for(var xPoint_13813=-1;xPoint_13813<(this.gameFieldBorderX.length+1); xPoint_13813++){
                    if(!map[yPoint_13813]){
                        map[yPoint_13813] = [];
                    }
                    map[yPoint_13813][xPoint_13813] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13813 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13813 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13813, yPoint_13813) === true){
                    if(this.canPutHorizontal(map, xPoint_13813, yPoint_13813, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13813][xPoint_13813 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13813, yPoint_13813, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13813 + i][xPoint_13813] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13813, yPoint_13813){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13813][xPoint_13813] === this.CELL_EMPTY
                && map[yPoint_13813-1][xPoint_13813] === this.CELL_EMPTY
                && map[yPoint_13813-1][xPoint_13813+1] === this.CELL_EMPTY
                && map[yPoint_13813][xPoint_13813+1] === this.CELL_EMPTY
                && map[yPoint_13813+1][xPoint_13813+1] === this.CELL_EMPTY
                && map[yPoint_13813+1][xPoint_13813] === this.CELL_EMPTY
                && map[yPoint_13813+1][xPoint_13813-1] === this.CELL_EMPTY
                && map[yPoint_13813][xPoint_13813-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13813, yPoint_13813, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13813;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13813][x] === this.CELL_EMPTY
                    && map[yPoint_13813-1][x] === this.CELL_EMPTY
                    && map[yPoint_13813-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13813][x+1] === this.CELL_EMPTY
                    && map[yPoint_13813+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13813+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13813, yPoint_13813, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13813;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13813] === this.CELL_EMPTY
                    && map[y+1][xPoint_13813] === this.CELL_EMPTY
                    && map[y+1][xPoint_13813+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13813] === this.CELL_EMPTY
                    && map[y][xPoint_13813-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13813-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
