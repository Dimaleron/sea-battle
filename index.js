...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18737=0;yPoint_18737<this.gameFieldBorderY.length; yPoint_18737++){
                for(var xPoint_18737=0;xPoint_18737<this.gameFieldBorderX.length; xPoint_18737++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18737, xPoint_18737);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18737][xPoint_18737] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18737, xPoint_18737, 'user');
                    if(this._userShipsMap[yPoint_18737][xPoint_18737] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18737, xPoint_18737, type){
            var id = this.getPointBlockIdByCoords(yPoint_18737, xPoint_18737, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18737);
                block.setAttribute('data-y', yPoint_18737);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18737
         * @param {type} xPoint_18737
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18737, xPoint_18737, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18737 + '_y' + yPoint_18737;
            }
            return 'pc_x' + xPoint_18737 + '_y' + yPoint_18737;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18737=0;yPoint_18737<this.gameFieldBorderY.length; yPoint_18737++){
                for(var xPoint_18737=0;xPoint_18737<this.gameFieldBorderX.length; xPoint_18737++){
                    map.push({y: yPoint_18737, x: xPoint_18737});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18737=-1;yPoint_18737<(this.gameFieldBorderY.length+1); yPoint_18737++){
                for(var xPoint_18737=-1;xPoint_18737<(this.gameFieldBorderX.length+1); xPoint_18737++){
                    if(!map[yPoint_18737]){
                        map[yPoint_18737] = [];
                    }
                    map[yPoint_18737][xPoint_18737] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18737 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18737 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18737, yPoint_18737) === true){
                    if(this.canPutHorizontal(map, xPoint_18737, yPoint_18737, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18737][xPoint_18737 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18737, yPoint_18737, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18737 + i][xPoint_18737] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18737, yPoint_18737){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18737][xPoint_18737] === this.CELL_EMPTY
                && map[yPoint_18737-1][xPoint_18737] === this.CELL_EMPTY
                && map[yPoint_18737-1][xPoint_18737+1] === this.CELL_EMPTY
                && map[yPoint_18737][xPoint_18737+1] === this.CELL_EMPTY
                && map[yPoint_18737+1][xPoint_18737+1] === this.CELL_EMPTY
                && map[yPoint_18737+1][xPoint_18737] === this.CELL_EMPTY
                && map[yPoint_18737+1][xPoint_18737-1] === this.CELL_EMPTY
                && map[yPoint_18737][xPoint_18737-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18737, yPoint_18737, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18737;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18737][x] === this.CELL_EMPTY
                    && map[yPoint_18737-1][x] === this.CELL_EMPTY
                    && map[yPoint_18737-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18737][x+1] === this.CELL_EMPTY
                    && map[yPoint_18737+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18737+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18737, yPoint_18737, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18737;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18737] === this.CELL_EMPTY
                    && map[y+1][xPoint_18737] === this.CELL_EMPTY
                    && map[y+1][xPoint_18737+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18737] === this.CELL_EMPTY
                    && map[y][xPoint_18737-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18737-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
