...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32027=0;yPoint_32027<this.gameFieldBorderY.length; yPoint_32027++){
                for(var xPoint_32027=0;xPoint_32027<this.gameFieldBorderX.length; xPoint_32027++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32027, xPoint_32027);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32027][xPoint_32027] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32027, xPoint_32027, 'user');
                    if(this._userShipsMap[yPoint_32027][xPoint_32027] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32027, xPoint_32027, type){
            var id = this.getPointBlockIdByCoords(yPoint_32027, xPoint_32027, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32027);
                block.setAttribute('data-y', yPoint_32027);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32027
         * @param {type} xPoint_32027
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32027, xPoint_32027, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32027 + '_y' + yPoint_32027;
            }
            return 'pc_x' + xPoint_32027 + '_y' + yPoint_32027;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32027=0;yPoint_32027<this.gameFieldBorderY.length; yPoint_32027++){
                for(var xPoint_32027=0;xPoint_32027<this.gameFieldBorderX.length; xPoint_32027++){
                    map.push({y: yPoint_32027, x: xPoint_32027});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32027=-1;yPoint_32027<(this.gameFieldBorderY.length+1); yPoint_32027++){
                for(var xPoint_32027=-1;xPoint_32027<(this.gameFieldBorderX.length+1); xPoint_32027++){
                    if(!map[yPoint_32027]){
                        map[yPoint_32027] = [];
                    }
                    map[yPoint_32027][xPoint_32027] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32027 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32027 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32027, yPoint_32027) === true){
                    if(this.canPutHorizontal(map, xPoint_32027, yPoint_32027, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32027][xPoint_32027 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32027, yPoint_32027, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32027 + i][xPoint_32027] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32027, yPoint_32027){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32027][xPoint_32027] === this.CELL_EMPTY
                && map[yPoint_32027-1][xPoint_32027] === this.CELL_EMPTY
                && map[yPoint_32027-1][xPoint_32027+1] === this.CELL_EMPTY
                && map[yPoint_32027][xPoint_32027+1] === this.CELL_EMPTY
                && map[yPoint_32027+1][xPoint_32027+1] === this.CELL_EMPTY
                && map[yPoint_32027+1][xPoint_32027] === this.CELL_EMPTY
                && map[yPoint_32027+1][xPoint_32027-1] === this.CELL_EMPTY
                && map[yPoint_32027][xPoint_32027-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32027, yPoint_32027, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32027;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32027][x] === this.CELL_EMPTY
                    && map[yPoint_32027-1][x] === this.CELL_EMPTY
                    && map[yPoint_32027-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32027][x+1] === this.CELL_EMPTY
                    && map[yPoint_32027+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32027+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32027, yPoint_32027, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32027;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32027] === this.CELL_EMPTY
                    && map[y+1][xPoint_32027] === this.CELL_EMPTY
                    && map[y+1][xPoint_32027+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32027] === this.CELL_EMPTY
                    && map[y][xPoint_32027-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32027-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
