...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5506=0;yPoint_5506<this.gameFieldBorderY.length; yPoint_5506++){
                for(var xPoint_5506=0;xPoint_5506<this.gameFieldBorderX.length; xPoint_5506++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5506, xPoint_5506);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5506][xPoint_5506] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5506, xPoint_5506, 'user');
                    if(this._userShipsMap[yPoint_5506][xPoint_5506] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5506, xPoint_5506, type){
            var id = this.getPointBlockIdByCoords(yPoint_5506, xPoint_5506, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5506);
                block.setAttribute('data-y', yPoint_5506);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5506
         * @param {type} xPoint_5506
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5506, xPoint_5506, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5506 + '_y' + yPoint_5506;
            }
            return 'pc_x' + xPoint_5506 + '_y' + yPoint_5506;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5506=0;yPoint_5506<this.gameFieldBorderY.length; yPoint_5506++){
                for(var xPoint_5506=0;xPoint_5506<this.gameFieldBorderX.length; xPoint_5506++){
                    map.push({y: yPoint_5506, x: xPoint_5506});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5506=-1;yPoint_5506<(this.gameFieldBorderY.length+1); yPoint_5506++){
                for(var xPoint_5506=-1;xPoint_5506<(this.gameFieldBorderX.length+1); xPoint_5506++){
                    if(!map[yPoint_5506]){
                        map[yPoint_5506] = [];
                    }
                    map[yPoint_5506][xPoint_5506] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5506 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5506 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5506, yPoint_5506) === true){
                    if(this.canPutHorizontal(map, xPoint_5506, yPoint_5506, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5506][xPoint_5506 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5506, yPoint_5506, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5506 + i][xPoint_5506] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5506, yPoint_5506){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5506][xPoint_5506] === this.CELL_EMPTY
                && map[yPoint_5506-1][xPoint_5506] === this.CELL_EMPTY
                && map[yPoint_5506-1][xPoint_5506+1] === this.CELL_EMPTY
                && map[yPoint_5506][xPoint_5506+1] === this.CELL_EMPTY
                && map[yPoint_5506+1][xPoint_5506+1] === this.CELL_EMPTY
                && map[yPoint_5506+1][xPoint_5506] === this.CELL_EMPTY
                && map[yPoint_5506+1][xPoint_5506-1] === this.CELL_EMPTY
                && map[yPoint_5506][xPoint_5506-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5506, yPoint_5506, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5506;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5506][x] === this.CELL_EMPTY
                    && map[yPoint_5506-1][x] === this.CELL_EMPTY
                    && map[yPoint_5506-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5506][x+1] === this.CELL_EMPTY
                    && map[yPoint_5506+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5506+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5506, yPoint_5506, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5506;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5506] === this.CELL_EMPTY
                    && map[y+1][xPoint_5506] === this.CELL_EMPTY
                    && map[y+1][xPoint_5506+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5506] === this.CELL_EMPTY
                    && map[y][xPoint_5506-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5506-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
