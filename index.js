...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20053=0;yPoint_20053<this.gameFieldBorderY.length; yPoint_20053++){
                for(var xPoint_20053=0;xPoint_20053<this.gameFieldBorderX.length; xPoint_20053++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20053, xPoint_20053);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20053][xPoint_20053] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20053, xPoint_20053, 'user');
                    if(this._userShipsMap[yPoint_20053][xPoint_20053] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20053, xPoint_20053, type){
            var id = this.getPointBlockIdByCoords(yPoint_20053, xPoint_20053, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20053);
                block.setAttribute('data-y', yPoint_20053);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20053
         * @param {type} xPoint_20053
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20053, xPoint_20053, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20053 + '_y' + yPoint_20053;
            }
            return 'pc_x' + xPoint_20053 + '_y' + yPoint_20053;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20053=0;yPoint_20053<this.gameFieldBorderY.length; yPoint_20053++){
                for(var xPoint_20053=0;xPoint_20053<this.gameFieldBorderX.length; xPoint_20053++){
                    map.push({y: yPoint_20053, x: xPoint_20053});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20053=-1;yPoint_20053<(this.gameFieldBorderY.length+1); yPoint_20053++){
                for(var xPoint_20053=-1;xPoint_20053<(this.gameFieldBorderX.length+1); xPoint_20053++){
                    if(!map[yPoint_20053]){
                        map[yPoint_20053] = [];
                    }
                    map[yPoint_20053][xPoint_20053] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20053 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20053 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20053, yPoint_20053) === true){
                    if(this.canPutHorizontal(map, xPoint_20053, yPoint_20053, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20053][xPoint_20053 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20053, yPoint_20053, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20053 + i][xPoint_20053] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20053, yPoint_20053){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20053][xPoint_20053] === this.CELL_EMPTY
                && map[yPoint_20053-1][xPoint_20053] === this.CELL_EMPTY
                && map[yPoint_20053-1][xPoint_20053+1] === this.CELL_EMPTY
                && map[yPoint_20053][xPoint_20053+1] === this.CELL_EMPTY
                && map[yPoint_20053+1][xPoint_20053+1] === this.CELL_EMPTY
                && map[yPoint_20053+1][xPoint_20053] === this.CELL_EMPTY
                && map[yPoint_20053+1][xPoint_20053-1] === this.CELL_EMPTY
                && map[yPoint_20053][xPoint_20053-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20053, yPoint_20053, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20053;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20053][x] === this.CELL_EMPTY
                    && map[yPoint_20053-1][x] === this.CELL_EMPTY
                    && map[yPoint_20053-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20053][x+1] === this.CELL_EMPTY
                    && map[yPoint_20053+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20053+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20053, yPoint_20053, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20053;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20053] === this.CELL_EMPTY
                    && map[y+1][xPoint_20053] === this.CELL_EMPTY
                    && map[y+1][xPoint_20053+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20053] === this.CELL_EMPTY
                    && map[y][xPoint_20053-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20053-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
