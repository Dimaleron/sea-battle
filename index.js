...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17522=0;yPoint_17522<this.gameFieldBorderY.length; yPoint_17522++){
                for(var xPoint_17522=0;xPoint_17522<this.gameFieldBorderX.length; xPoint_17522++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17522, xPoint_17522);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17522][xPoint_17522] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17522, xPoint_17522, 'user');
                    if(this._userShipsMap[yPoint_17522][xPoint_17522] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17522, xPoint_17522, type){
            var id = this.getPointBlockIdByCoords(yPoint_17522, xPoint_17522, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17522);
                block.setAttribute('data-y', yPoint_17522);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17522
         * @param {type} xPoint_17522
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17522, xPoint_17522, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17522 + '_y' + yPoint_17522;
            }
            return 'pc_x' + xPoint_17522 + '_y' + yPoint_17522;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17522=0;yPoint_17522<this.gameFieldBorderY.length; yPoint_17522++){
                for(var xPoint_17522=0;xPoint_17522<this.gameFieldBorderX.length; xPoint_17522++){
                    map.push({y: yPoint_17522, x: xPoint_17522});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17522=-1;yPoint_17522<(this.gameFieldBorderY.length+1); yPoint_17522++){
                for(var xPoint_17522=-1;xPoint_17522<(this.gameFieldBorderX.length+1); xPoint_17522++){
                    if(!map[yPoint_17522]){
                        map[yPoint_17522] = [];
                    }
                    map[yPoint_17522][xPoint_17522] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17522 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17522 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17522, yPoint_17522) === true){
                    if(this.canPutHorizontal(map, xPoint_17522, yPoint_17522, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17522][xPoint_17522 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17522, yPoint_17522, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17522 + i][xPoint_17522] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17522, yPoint_17522){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17522][xPoint_17522] === this.CELL_EMPTY
                && map[yPoint_17522-1][xPoint_17522] === this.CELL_EMPTY
                && map[yPoint_17522-1][xPoint_17522+1] === this.CELL_EMPTY
                && map[yPoint_17522][xPoint_17522+1] === this.CELL_EMPTY
                && map[yPoint_17522+1][xPoint_17522+1] === this.CELL_EMPTY
                && map[yPoint_17522+1][xPoint_17522] === this.CELL_EMPTY
                && map[yPoint_17522+1][xPoint_17522-1] === this.CELL_EMPTY
                && map[yPoint_17522][xPoint_17522-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17522, yPoint_17522, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17522;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17522][x] === this.CELL_EMPTY
                    && map[yPoint_17522-1][x] === this.CELL_EMPTY
                    && map[yPoint_17522-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17522][x+1] === this.CELL_EMPTY
                    && map[yPoint_17522+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17522+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17522, yPoint_17522, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17522;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17522] === this.CELL_EMPTY
                    && map[y+1][xPoint_17522] === this.CELL_EMPTY
                    && map[y+1][xPoint_17522+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17522] === this.CELL_EMPTY
                    && map[y][xPoint_17522-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17522-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
