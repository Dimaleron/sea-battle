...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14926=0;yPoint_14926<this.gameFieldBorderY.length; yPoint_14926++){
                for(var xPoint_14926=0;xPoint_14926<this.gameFieldBorderX.length; xPoint_14926++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14926, xPoint_14926);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14926][xPoint_14926] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14926, xPoint_14926, 'user');
                    if(this._userShipsMap[yPoint_14926][xPoint_14926] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14926, xPoint_14926, type){
            var id = this.getPointBlockIdByCoords(yPoint_14926, xPoint_14926, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14926);
                block.setAttribute('data-y', yPoint_14926);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14926
         * @param {type} xPoint_14926
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14926, xPoint_14926, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14926 + '_y' + yPoint_14926;
            }
            return 'pc_x' + xPoint_14926 + '_y' + yPoint_14926;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14926=0;yPoint_14926<this.gameFieldBorderY.length; yPoint_14926++){
                for(var xPoint_14926=0;xPoint_14926<this.gameFieldBorderX.length; xPoint_14926++){
                    map.push({y: yPoint_14926, x: xPoint_14926});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14926=-1;yPoint_14926<(this.gameFieldBorderY.length+1); yPoint_14926++){
                for(var xPoint_14926=-1;xPoint_14926<(this.gameFieldBorderX.length+1); xPoint_14926++){
                    if(!map[yPoint_14926]){
                        map[yPoint_14926] = [];
                    }
                    map[yPoint_14926][xPoint_14926] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14926 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14926 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14926, yPoint_14926) === true){
                    if(this.canPutHorizontal(map, xPoint_14926, yPoint_14926, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14926][xPoint_14926 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14926, yPoint_14926, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14926 + i][xPoint_14926] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14926, yPoint_14926){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14926][xPoint_14926] === this.CELL_EMPTY
                && map[yPoint_14926-1][xPoint_14926] === this.CELL_EMPTY
                && map[yPoint_14926-1][xPoint_14926+1] === this.CELL_EMPTY
                && map[yPoint_14926][xPoint_14926+1] === this.CELL_EMPTY
                && map[yPoint_14926+1][xPoint_14926+1] === this.CELL_EMPTY
                && map[yPoint_14926+1][xPoint_14926] === this.CELL_EMPTY
                && map[yPoint_14926+1][xPoint_14926-1] === this.CELL_EMPTY
                && map[yPoint_14926][xPoint_14926-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14926, yPoint_14926, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14926;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14926][x] === this.CELL_EMPTY
                    && map[yPoint_14926-1][x] === this.CELL_EMPTY
                    && map[yPoint_14926-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14926][x+1] === this.CELL_EMPTY
                    && map[yPoint_14926+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14926+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14926, yPoint_14926, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14926;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14926] === this.CELL_EMPTY
                    && map[y+1][xPoint_14926] === this.CELL_EMPTY
                    && map[y+1][xPoint_14926+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14926] === this.CELL_EMPTY
                    && map[y][xPoint_14926-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14926-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
