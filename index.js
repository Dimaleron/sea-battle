...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21887=0;yPoint_21887<this.gameFieldBorderY.length; yPoint_21887++){
                for(var xPoint_21887=0;xPoint_21887<this.gameFieldBorderX.length; xPoint_21887++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21887, xPoint_21887);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21887][xPoint_21887] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21887, xPoint_21887, 'user');
                    if(this._userShipsMap[yPoint_21887][xPoint_21887] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21887, xPoint_21887, type){
            var id = this.getPointBlockIdByCoords(yPoint_21887, xPoint_21887, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21887);
                block.setAttribute('data-y', yPoint_21887);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21887
         * @param {type} xPoint_21887
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21887, xPoint_21887, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21887 + '_y' + yPoint_21887;
            }
            return 'pc_x' + xPoint_21887 + '_y' + yPoint_21887;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21887=0;yPoint_21887<this.gameFieldBorderY.length; yPoint_21887++){
                for(var xPoint_21887=0;xPoint_21887<this.gameFieldBorderX.length; xPoint_21887++){
                    map.push({y: yPoint_21887, x: xPoint_21887});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21887=-1;yPoint_21887<(this.gameFieldBorderY.length+1); yPoint_21887++){
                for(var xPoint_21887=-1;xPoint_21887<(this.gameFieldBorderX.length+1); xPoint_21887++){
                    if(!map[yPoint_21887]){
                        map[yPoint_21887] = [];
                    }
                    map[yPoint_21887][xPoint_21887] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21887 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21887 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21887, yPoint_21887) === true){
                    if(this.canPutHorizontal(map, xPoint_21887, yPoint_21887, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21887][xPoint_21887 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21887, yPoint_21887, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21887 + i][xPoint_21887] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21887, yPoint_21887){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21887][xPoint_21887] === this.CELL_EMPTY
                && map[yPoint_21887-1][xPoint_21887] === this.CELL_EMPTY
                && map[yPoint_21887-1][xPoint_21887+1] === this.CELL_EMPTY
                && map[yPoint_21887][xPoint_21887+1] === this.CELL_EMPTY
                && map[yPoint_21887+1][xPoint_21887+1] === this.CELL_EMPTY
                && map[yPoint_21887+1][xPoint_21887] === this.CELL_EMPTY
                && map[yPoint_21887+1][xPoint_21887-1] === this.CELL_EMPTY
                && map[yPoint_21887][xPoint_21887-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21887, yPoint_21887, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21887;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21887][x] === this.CELL_EMPTY
                    && map[yPoint_21887-1][x] === this.CELL_EMPTY
                    && map[yPoint_21887-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21887][x+1] === this.CELL_EMPTY
                    && map[yPoint_21887+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21887+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21887, yPoint_21887, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21887;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21887] === this.CELL_EMPTY
                    && map[y+1][xPoint_21887] === this.CELL_EMPTY
                    && map[y+1][xPoint_21887+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21887] === this.CELL_EMPTY
                    && map[y][xPoint_21887-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21887-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
