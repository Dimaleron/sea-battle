...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7394=0;yPoint_7394<this.gameFieldBorderY.length; yPoint_7394++){
                for(var xPoint_7394=0;xPoint_7394<this.gameFieldBorderX.length; xPoint_7394++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7394, xPoint_7394);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7394][xPoint_7394] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7394, xPoint_7394, 'user');
                    if(this._userShipsMap[yPoint_7394][xPoint_7394] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7394, xPoint_7394, type){
            var id = this.getPointBlockIdByCoords(yPoint_7394, xPoint_7394, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7394);
                block.setAttribute('data-y', yPoint_7394);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7394
         * @param {type} xPoint_7394
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7394, xPoint_7394, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7394 + '_y' + yPoint_7394;
            }
            return 'pc_x' + xPoint_7394 + '_y' + yPoint_7394;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7394=0;yPoint_7394<this.gameFieldBorderY.length; yPoint_7394++){
                for(var xPoint_7394=0;xPoint_7394<this.gameFieldBorderX.length; xPoint_7394++){
                    map.push({y: yPoint_7394, x: xPoint_7394});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7394=-1;yPoint_7394<(this.gameFieldBorderY.length+1); yPoint_7394++){
                for(var xPoint_7394=-1;xPoint_7394<(this.gameFieldBorderX.length+1); xPoint_7394++){
                    if(!map[yPoint_7394]){
                        map[yPoint_7394] = [];
                    }
                    map[yPoint_7394][xPoint_7394] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7394 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7394 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7394, yPoint_7394) === true){
                    if(this.canPutHorizontal(map, xPoint_7394, yPoint_7394, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7394][xPoint_7394 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7394, yPoint_7394, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7394 + i][xPoint_7394] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7394, yPoint_7394){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7394][xPoint_7394] === this.CELL_EMPTY
                && map[yPoint_7394-1][xPoint_7394] === this.CELL_EMPTY
                && map[yPoint_7394-1][xPoint_7394+1] === this.CELL_EMPTY
                && map[yPoint_7394][xPoint_7394+1] === this.CELL_EMPTY
                && map[yPoint_7394+1][xPoint_7394+1] === this.CELL_EMPTY
                && map[yPoint_7394+1][xPoint_7394] === this.CELL_EMPTY
                && map[yPoint_7394+1][xPoint_7394-1] === this.CELL_EMPTY
                && map[yPoint_7394][xPoint_7394-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7394, yPoint_7394, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7394;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7394][x] === this.CELL_EMPTY
                    && map[yPoint_7394-1][x] === this.CELL_EMPTY
                    && map[yPoint_7394-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7394][x+1] === this.CELL_EMPTY
                    && map[yPoint_7394+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7394+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7394, yPoint_7394, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7394;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7394] === this.CELL_EMPTY
                    && map[y+1][xPoint_7394] === this.CELL_EMPTY
                    && map[y+1][xPoint_7394+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7394] === this.CELL_EMPTY
                    && map[y][xPoint_7394-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7394-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
