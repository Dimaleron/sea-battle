...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13494=0;yPoint_13494<this.gameFieldBorderY.length; yPoint_13494++){
                for(var xPoint_13494=0;xPoint_13494<this.gameFieldBorderX.length; xPoint_13494++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13494, xPoint_13494);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13494][xPoint_13494] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13494, xPoint_13494, 'user');
                    if(this._userShipsMap[yPoint_13494][xPoint_13494] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13494, xPoint_13494, type){
            var id = this.getPointBlockIdByCoords(yPoint_13494, xPoint_13494, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13494);
                block.setAttribute('data-y', yPoint_13494);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13494
         * @param {type} xPoint_13494
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13494, xPoint_13494, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13494 + '_y' + yPoint_13494;
            }
            return 'pc_x' + xPoint_13494 + '_y' + yPoint_13494;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13494=0;yPoint_13494<this.gameFieldBorderY.length; yPoint_13494++){
                for(var xPoint_13494=0;xPoint_13494<this.gameFieldBorderX.length; xPoint_13494++){
                    map.push({y: yPoint_13494, x: xPoint_13494});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13494=-1;yPoint_13494<(this.gameFieldBorderY.length+1); yPoint_13494++){
                for(var xPoint_13494=-1;xPoint_13494<(this.gameFieldBorderX.length+1); xPoint_13494++){
                    if(!map[yPoint_13494]){
                        map[yPoint_13494] = [];
                    }
                    map[yPoint_13494][xPoint_13494] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13494 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13494 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13494, yPoint_13494) === true){
                    if(this.canPutHorizontal(map, xPoint_13494, yPoint_13494, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13494][xPoint_13494 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13494, yPoint_13494, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13494 + i][xPoint_13494] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13494, yPoint_13494){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13494][xPoint_13494] === this.CELL_EMPTY
                && map[yPoint_13494-1][xPoint_13494] === this.CELL_EMPTY
                && map[yPoint_13494-1][xPoint_13494+1] === this.CELL_EMPTY
                && map[yPoint_13494][xPoint_13494+1] === this.CELL_EMPTY
                && map[yPoint_13494+1][xPoint_13494+1] === this.CELL_EMPTY
                && map[yPoint_13494+1][xPoint_13494] === this.CELL_EMPTY
                && map[yPoint_13494+1][xPoint_13494-1] === this.CELL_EMPTY
                && map[yPoint_13494][xPoint_13494-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13494, yPoint_13494, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13494;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13494][x] === this.CELL_EMPTY
                    && map[yPoint_13494-1][x] === this.CELL_EMPTY
                    && map[yPoint_13494-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13494][x+1] === this.CELL_EMPTY
                    && map[yPoint_13494+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13494+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13494, yPoint_13494, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13494;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13494] === this.CELL_EMPTY
                    && map[y+1][xPoint_13494] === this.CELL_EMPTY
                    && map[y+1][xPoint_13494+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13494] === this.CELL_EMPTY
                    && map[y][xPoint_13494-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13494-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
