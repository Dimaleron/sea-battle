...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22424=0;yPoint_22424<this.gameFieldBorderY.length; yPoint_22424++){
                for(var xPoint_22424=0;xPoint_22424<this.gameFieldBorderX.length; xPoint_22424++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22424, xPoint_22424);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22424][xPoint_22424] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22424, xPoint_22424, 'user');
                    if(this._userShipsMap[yPoint_22424][xPoint_22424] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22424, xPoint_22424, type){
            var id = this.getPointBlockIdByCoords(yPoint_22424, xPoint_22424, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22424);
                block.setAttribute('data-y', yPoint_22424);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22424
         * @param {type} xPoint_22424
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22424, xPoint_22424, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22424 + '_y' + yPoint_22424;
            }
            return 'pc_x' + xPoint_22424 + '_y' + yPoint_22424;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22424=0;yPoint_22424<this.gameFieldBorderY.length; yPoint_22424++){
                for(var xPoint_22424=0;xPoint_22424<this.gameFieldBorderX.length; xPoint_22424++){
                    map.push({y: yPoint_22424, x: xPoint_22424});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22424=-1;yPoint_22424<(this.gameFieldBorderY.length+1); yPoint_22424++){
                for(var xPoint_22424=-1;xPoint_22424<(this.gameFieldBorderX.length+1); xPoint_22424++){
                    if(!map[yPoint_22424]){
                        map[yPoint_22424] = [];
                    }
                    map[yPoint_22424][xPoint_22424] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22424 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22424 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22424, yPoint_22424) === true){
                    if(this.canPutHorizontal(map, xPoint_22424, yPoint_22424, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22424][xPoint_22424 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22424, yPoint_22424, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22424 + i][xPoint_22424] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22424, yPoint_22424){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22424][xPoint_22424] === this.CELL_EMPTY
                && map[yPoint_22424-1][xPoint_22424] === this.CELL_EMPTY
                && map[yPoint_22424-1][xPoint_22424+1] === this.CELL_EMPTY
                && map[yPoint_22424][xPoint_22424+1] === this.CELL_EMPTY
                && map[yPoint_22424+1][xPoint_22424+1] === this.CELL_EMPTY
                && map[yPoint_22424+1][xPoint_22424] === this.CELL_EMPTY
                && map[yPoint_22424+1][xPoint_22424-1] === this.CELL_EMPTY
                && map[yPoint_22424][xPoint_22424-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22424, yPoint_22424, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22424;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22424][x] === this.CELL_EMPTY
                    && map[yPoint_22424-1][x] === this.CELL_EMPTY
                    && map[yPoint_22424-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22424][x+1] === this.CELL_EMPTY
                    && map[yPoint_22424+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22424+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22424, yPoint_22424, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22424;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22424] === this.CELL_EMPTY
                    && map[y+1][xPoint_22424] === this.CELL_EMPTY
                    && map[y+1][xPoint_22424+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22424] === this.CELL_EMPTY
                    && map[y][xPoint_22424-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22424-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
