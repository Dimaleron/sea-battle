...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14548=0;yPoint_14548<this.gameFieldBorderY.length; yPoint_14548++){
                for(var xPoint_14548=0;xPoint_14548<this.gameFieldBorderX.length; xPoint_14548++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14548, xPoint_14548);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14548][xPoint_14548] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14548, xPoint_14548, 'user');
                    if(this._userShipsMap[yPoint_14548][xPoint_14548] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14548, xPoint_14548, type){
            var id = this.getPointBlockIdByCoords(yPoint_14548, xPoint_14548, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14548);
                block.setAttribute('data-y', yPoint_14548);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14548
         * @param {type} xPoint_14548
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14548, xPoint_14548, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14548 + '_y' + yPoint_14548;
            }
            return 'pc_x' + xPoint_14548 + '_y' + yPoint_14548;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14548=0;yPoint_14548<this.gameFieldBorderY.length; yPoint_14548++){
                for(var xPoint_14548=0;xPoint_14548<this.gameFieldBorderX.length; xPoint_14548++){
                    map.push({y: yPoint_14548, x: xPoint_14548});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14548=-1;yPoint_14548<(this.gameFieldBorderY.length+1); yPoint_14548++){
                for(var xPoint_14548=-1;xPoint_14548<(this.gameFieldBorderX.length+1); xPoint_14548++){
                    if(!map[yPoint_14548]){
                        map[yPoint_14548] = [];
                    }
                    map[yPoint_14548][xPoint_14548] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14548 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14548 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14548, yPoint_14548) === true){
                    if(this.canPutHorizontal(map, xPoint_14548, yPoint_14548, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14548][xPoint_14548 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14548, yPoint_14548, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14548 + i][xPoint_14548] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14548, yPoint_14548){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14548][xPoint_14548] === this.CELL_EMPTY
                && map[yPoint_14548-1][xPoint_14548] === this.CELL_EMPTY
                && map[yPoint_14548-1][xPoint_14548+1] === this.CELL_EMPTY
                && map[yPoint_14548][xPoint_14548+1] === this.CELL_EMPTY
                && map[yPoint_14548+1][xPoint_14548+1] === this.CELL_EMPTY
                && map[yPoint_14548+1][xPoint_14548] === this.CELL_EMPTY
                && map[yPoint_14548+1][xPoint_14548-1] === this.CELL_EMPTY
                && map[yPoint_14548][xPoint_14548-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14548, yPoint_14548, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14548;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14548][x] === this.CELL_EMPTY
                    && map[yPoint_14548-1][x] === this.CELL_EMPTY
                    && map[yPoint_14548-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14548][x+1] === this.CELL_EMPTY
                    && map[yPoint_14548+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14548+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14548, yPoint_14548, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14548;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14548] === this.CELL_EMPTY
                    && map[y+1][xPoint_14548] === this.CELL_EMPTY
                    && map[y+1][xPoint_14548+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14548] === this.CELL_EMPTY
                    && map[y][xPoint_14548-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14548-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
