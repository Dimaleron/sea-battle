...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24765=0;yPoint_24765<this.gameFieldBorderY.length; yPoint_24765++){
                for(var xPoint_24765=0;xPoint_24765<this.gameFieldBorderX.length; xPoint_24765++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24765, xPoint_24765);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24765][xPoint_24765] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24765, xPoint_24765, 'user');
                    if(this._userShipsMap[yPoint_24765][xPoint_24765] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24765, xPoint_24765, type){
            var id = this.getPointBlockIdByCoords(yPoint_24765, xPoint_24765, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24765);
                block.setAttribute('data-y', yPoint_24765);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24765
         * @param {type} xPoint_24765
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24765, xPoint_24765, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24765 + '_y' + yPoint_24765;
            }
            return 'pc_x' + xPoint_24765 + '_y' + yPoint_24765;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24765=0;yPoint_24765<this.gameFieldBorderY.length; yPoint_24765++){
                for(var xPoint_24765=0;xPoint_24765<this.gameFieldBorderX.length; xPoint_24765++){
                    map.push({y: yPoint_24765, x: xPoint_24765});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24765=-1;yPoint_24765<(this.gameFieldBorderY.length+1); yPoint_24765++){
                for(var xPoint_24765=-1;xPoint_24765<(this.gameFieldBorderX.length+1); xPoint_24765++){
                    if(!map[yPoint_24765]){
                        map[yPoint_24765] = [];
                    }
                    map[yPoint_24765][xPoint_24765] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24765 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24765 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24765, yPoint_24765) === true){
                    if(this.canPutHorizontal(map, xPoint_24765, yPoint_24765, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24765][xPoint_24765 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24765, yPoint_24765, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24765 + i][xPoint_24765] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24765, yPoint_24765){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24765][xPoint_24765] === this.CELL_EMPTY
                && map[yPoint_24765-1][xPoint_24765] === this.CELL_EMPTY
                && map[yPoint_24765-1][xPoint_24765+1] === this.CELL_EMPTY
                && map[yPoint_24765][xPoint_24765+1] === this.CELL_EMPTY
                && map[yPoint_24765+1][xPoint_24765+1] === this.CELL_EMPTY
                && map[yPoint_24765+1][xPoint_24765] === this.CELL_EMPTY
                && map[yPoint_24765+1][xPoint_24765-1] === this.CELL_EMPTY
                && map[yPoint_24765][xPoint_24765-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24765, yPoint_24765, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24765;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24765][x] === this.CELL_EMPTY
                    && map[yPoint_24765-1][x] === this.CELL_EMPTY
                    && map[yPoint_24765-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24765][x+1] === this.CELL_EMPTY
                    && map[yPoint_24765+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24765+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24765, yPoint_24765, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24765;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24765] === this.CELL_EMPTY
                    && map[y+1][xPoint_24765] === this.CELL_EMPTY
                    && map[y+1][xPoint_24765+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24765] === this.CELL_EMPTY
                    && map[y][xPoint_24765-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24765-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
