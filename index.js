...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19617=0;yPoint_19617<this.gameFieldBorderY.length; yPoint_19617++){
                for(var xPoint_19617=0;xPoint_19617<this.gameFieldBorderX.length; xPoint_19617++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19617, xPoint_19617);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19617][xPoint_19617] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19617, xPoint_19617, 'user');
                    if(this._userShipsMap[yPoint_19617][xPoint_19617] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19617, xPoint_19617, type){
            var id = this.getPointBlockIdByCoords(yPoint_19617, xPoint_19617, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19617);
                block.setAttribute('data-y', yPoint_19617);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19617
         * @param {type} xPoint_19617
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19617, xPoint_19617, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19617 + '_y' + yPoint_19617;
            }
            return 'pc_x' + xPoint_19617 + '_y' + yPoint_19617;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19617=0;yPoint_19617<this.gameFieldBorderY.length; yPoint_19617++){
                for(var xPoint_19617=0;xPoint_19617<this.gameFieldBorderX.length; xPoint_19617++){
                    map.push({y: yPoint_19617, x: xPoint_19617});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19617=-1;yPoint_19617<(this.gameFieldBorderY.length+1); yPoint_19617++){
                for(var xPoint_19617=-1;xPoint_19617<(this.gameFieldBorderX.length+1); xPoint_19617++){
                    if(!map[yPoint_19617]){
                        map[yPoint_19617] = [];
                    }
                    map[yPoint_19617][xPoint_19617] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19617 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19617 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19617, yPoint_19617) === true){
                    if(this.canPutHorizontal(map, xPoint_19617, yPoint_19617, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19617][xPoint_19617 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19617, yPoint_19617, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19617 + i][xPoint_19617] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19617, yPoint_19617){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19617][xPoint_19617] === this.CELL_EMPTY
                && map[yPoint_19617-1][xPoint_19617] === this.CELL_EMPTY
                && map[yPoint_19617-1][xPoint_19617+1] === this.CELL_EMPTY
                && map[yPoint_19617][xPoint_19617+1] === this.CELL_EMPTY
                && map[yPoint_19617+1][xPoint_19617+1] === this.CELL_EMPTY
                && map[yPoint_19617+1][xPoint_19617] === this.CELL_EMPTY
                && map[yPoint_19617+1][xPoint_19617-1] === this.CELL_EMPTY
                && map[yPoint_19617][xPoint_19617-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19617, yPoint_19617, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19617;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19617][x] === this.CELL_EMPTY
                    && map[yPoint_19617-1][x] === this.CELL_EMPTY
                    && map[yPoint_19617-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19617][x+1] === this.CELL_EMPTY
                    && map[yPoint_19617+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19617+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19617, yPoint_19617, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19617;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19617] === this.CELL_EMPTY
                    && map[y+1][xPoint_19617] === this.CELL_EMPTY
                    && map[y+1][xPoint_19617+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19617] === this.CELL_EMPTY
                    && map[y][xPoint_19617-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19617-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
