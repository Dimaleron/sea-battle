...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14071=0;yPoint_14071<this.gameFieldBorderY.length; yPoint_14071++){
                for(var xPoint_14071=0;xPoint_14071<this.gameFieldBorderX.length; xPoint_14071++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14071, xPoint_14071);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14071][xPoint_14071] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14071, xPoint_14071, 'user');
                    if(this._userShipsMap[yPoint_14071][xPoint_14071] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14071, xPoint_14071, type){
            var id = this.getPointBlockIdByCoords(yPoint_14071, xPoint_14071, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14071);
                block.setAttribute('data-y', yPoint_14071);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14071
         * @param {type} xPoint_14071
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14071, xPoint_14071, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14071 + '_y' + yPoint_14071;
            }
            return 'pc_x' + xPoint_14071 + '_y' + yPoint_14071;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14071=0;yPoint_14071<this.gameFieldBorderY.length; yPoint_14071++){
                for(var xPoint_14071=0;xPoint_14071<this.gameFieldBorderX.length; xPoint_14071++){
                    map.push({y: yPoint_14071, x: xPoint_14071});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14071=-1;yPoint_14071<(this.gameFieldBorderY.length+1); yPoint_14071++){
                for(var xPoint_14071=-1;xPoint_14071<(this.gameFieldBorderX.length+1); xPoint_14071++){
                    if(!map[yPoint_14071]){
                        map[yPoint_14071] = [];
                    }
                    map[yPoint_14071][xPoint_14071] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14071 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14071 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14071, yPoint_14071) === true){
                    if(this.canPutHorizontal(map, xPoint_14071, yPoint_14071, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14071][xPoint_14071 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14071, yPoint_14071, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14071 + i][xPoint_14071] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14071, yPoint_14071){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14071][xPoint_14071] === this.CELL_EMPTY
                && map[yPoint_14071-1][xPoint_14071] === this.CELL_EMPTY
                && map[yPoint_14071-1][xPoint_14071+1] === this.CELL_EMPTY
                && map[yPoint_14071][xPoint_14071+1] === this.CELL_EMPTY
                && map[yPoint_14071+1][xPoint_14071+1] === this.CELL_EMPTY
                && map[yPoint_14071+1][xPoint_14071] === this.CELL_EMPTY
                && map[yPoint_14071+1][xPoint_14071-1] === this.CELL_EMPTY
                && map[yPoint_14071][xPoint_14071-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14071, yPoint_14071, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14071;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14071][x] === this.CELL_EMPTY
                    && map[yPoint_14071-1][x] === this.CELL_EMPTY
                    && map[yPoint_14071-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14071][x+1] === this.CELL_EMPTY
                    && map[yPoint_14071+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14071+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14071, yPoint_14071, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14071;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14071] === this.CELL_EMPTY
                    && map[y+1][xPoint_14071] === this.CELL_EMPTY
                    && map[y+1][xPoint_14071+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14071] === this.CELL_EMPTY
                    && map[y][xPoint_14071-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14071-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
