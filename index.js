...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_12631=0;yPoint_12631<this.gameFieldBorderY.length; yPoint_12631++){
                for(var xPoint_12631=0;xPoint_12631<this.gameFieldBorderX.length; xPoint_12631++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_12631, xPoint_12631);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_12631][xPoint_12631] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_12631, xPoint_12631, 'user');
                    if(this._userShipsMap[yPoint_12631][xPoint_12631] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_12631, xPoint_12631, type){
            var id = this.getPointBlockIdByCoords(yPoint_12631, xPoint_12631, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_12631);
                block.setAttribute('data-y', yPoint_12631);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_12631
         * @param {type} xPoint_12631
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_12631, xPoint_12631, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_12631 + '_y' + yPoint_12631;
            }
            return 'pc_x' + xPoint_12631 + '_y' + yPoint_12631;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_12631=0;yPoint_12631<this.gameFieldBorderY.length; yPoint_12631++){
                for(var xPoint_12631=0;xPoint_12631<this.gameFieldBorderX.length; xPoint_12631++){
                    map.push({y: yPoint_12631, x: xPoint_12631});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_12631=-1;yPoint_12631<(this.gameFieldBorderY.length+1); yPoint_12631++){
                for(var xPoint_12631=-1;xPoint_12631<(this.gameFieldBorderX.length+1); xPoint_12631++){
                    if(!map[yPoint_12631]){
                        map[yPoint_12631] = [];
                    }
                    map[yPoint_12631][xPoint_12631] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_12631 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_12631 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_12631, yPoint_12631) === true){
                    if(this.canPutHorizontal(map, xPoint_12631, yPoint_12631, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12631][xPoint_12631 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_12631, yPoint_12631, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12631 + i][xPoint_12631] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_12631, yPoint_12631){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_12631][xPoint_12631] === this.CELL_EMPTY
                && map[yPoint_12631-1][xPoint_12631] === this.CELL_EMPTY
                && map[yPoint_12631-1][xPoint_12631+1] === this.CELL_EMPTY
                && map[yPoint_12631][xPoint_12631+1] === this.CELL_EMPTY
                && map[yPoint_12631+1][xPoint_12631+1] === this.CELL_EMPTY
                && map[yPoint_12631+1][xPoint_12631] === this.CELL_EMPTY
                && map[yPoint_12631+1][xPoint_12631-1] === this.CELL_EMPTY
                && map[yPoint_12631][xPoint_12631-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_12631, yPoint_12631, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_12631;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_12631][x] === this.CELL_EMPTY
                    && map[yPoint_12631-1][x] === this.CELL_EMPTY
                    && map[yPoint_12631-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12631][x+1] === this.CELL_EMPTY
                    && map[yPoint_12631+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12631+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_12631, yPoint_12631, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_12631;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_12631] === this.CELL_EMPTY
                    && map[y+1][xPoint_12631] === this.CELL_EMPTY
                    && map[y+1][xPoint_12631+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_12631] === this.CELL_EMPTY
                    && map[y][xPoint_12631-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_12631-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
