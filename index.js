...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11354=0;yPoint_11354<this.gameFieldBorderY.length; yPoint_11354++){
                for(var xPoint_11354=0;xPoint_11354<this.gameFieldBorderX.length; xPoint_11354++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11354, xPoint_11354);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11354][xPoint_11354] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11354, xPoint_11354, 'user');
                    if(this._userShipsMap[yPoint_11354][xPoint_11354] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11354, xPoint_11354, type){
            var id = this.getPointBlockIdByCoords(yPoint_11354, xPoint_11354, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11354);
                block.setAttribute('data-y', yPoint_11354);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11354
         * @param {type} xPoint_11354
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11354, xPoint_11354, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11354 + '_y' + yPoint_11354;
            }
            return 'pc_x' + xPoint_11354 + '_y' + yPoint_11354;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11354=0;yPoint_11354<this.gameFieldBorderY.length; yPoint_11354++){
                for(var xPoint_11354=0;xPoint_11354<this.gameFieldBorderX.length; xPoint_11354++){
                    map.push({y: yPoint_11354, x: xPoint_11354});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11354=-1;yPoint_11354<(this.gameFieldBorderY.length+1); yPoint_11354++){
                for(var xPoint_11354=-1;xPoint_11354<(this.gameFieldBorderX.length+1); xPoint_11354++){
                    if(!map[yPoint_11354]){
                        map[yPoint_11354] = [];
                    }
                    map[yPoint_11354][xPoint_11354] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11354 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11354 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11354, yPoint_11354) === true){
                    if(this.canPutHorizontal(map, xPoint_11354, yPoint_11354, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11354][xPoint_11354 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11354, yPoint_11354, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11354 + i][xPoint_11354] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11354, yPoint_11354){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11354][xPoint_11354] === this.CELL_EMPTY
                && map[yPoint_11354-1][xPoint_11354] === this.CELL_EMPTY
                && map[yPoint_11354-1][xPoint_11354+1] === this.CELL_EMPTY
                && map[yPoint_11354][xPoint_11354+1] === this.CELL_EMPTY
                && map[yPoint_11354+1][xPoint_11354+1] === this.CELL_EMPTY
                && map[yPoint_11354+1][xPoint_11354] === this.CELL_EMPTY
                && map[yPoint_11354+1][xPoint_11354-1] === this.CELL_EMPTY
                && map[yPoint_11354][xPoint_11354-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11354, yPoint_11354, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11354;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11354][x] === this.CELL_EMPTY
                    && map[yPoint_11354-1][x] === this.CELL_EMPTY
                    && map[yPoint_11354-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11354][x+1] === this.CELL_EMPTY
                    && map[yPoint_11354+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11354+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11354, yPoint_11354, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11354;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11354] === this.CELL_EMPTY
                    && map[y+1][xPoint_11354] === this.CELL_EMPTY
                    && map[y+1][xPoint_11354+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11354] === this.CELL_EMPTY
                    && map[y][xPoint_11354-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11354-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
