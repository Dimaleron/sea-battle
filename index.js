...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6769=0;yPoint_6769<this.gameFieldBorderY.length; yPoint_6769++){
                for(var xPoint_6769=0;xPoint_6769<this.gameFieldBorderX.length; xPoint_6769++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6769, xPoint_6769);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6769][xPoint_6769] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6769, xPoint_6769, 'user');
                    if(this._userShipsMap[yPoint_6769][xPoint_6769] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6769, xPoint_6769, type){
            var id = this.getPointBlockIdByCoords(yPoint_6769, xPoint_6769, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6769);
                block.setAttribute('data-y', yPoint_6769);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6769
         * @param {type} xPoint_6769
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6769, xPoint_6769, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6769 + '_y' + yPoint_6769;
            }
            return 'pc_x' + xPoint_6769 + '_y' + yPoint_6769;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6769=0;yPoint_6769<this.gameFieldBorderY.length; yPoint_6769++){
                for(var xPoint_6769=0;xPoint_6769<this.gameFieldBorderX.length; xPoint_6769++){
                    map.push({y: yPoint_6769, x: xPoint_6769});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6769=-1;yPoint_6769<(this.gameFieldBorderY.length+1); yPoint_6769++){
                for(var xPoint_6769=-1;xPoint_6769<(this.gameFieldBorderX.length+1); xPoint_6769++){
                    if(!map[yPoint_6769]){
                        map[yPoint_6769] = [];
                    }
                    map[yPoint_6769][xPoint_6769] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6769 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6769 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6769, yPoint_6769) === true){
                    if(this.canPutHorizontal(map, xPoint_6769, yPoint_6769, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6769][xPoint_6769 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6769, yPoint_6769, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6769 + i][xPoint_6769] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6769, yPoint_6769){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6769][xPoint_6769] === this.CELL_EMPTY
                && map[yPoint_6769-1][xPoint_6769] === this.CELL_EMPTY
                && map[yPoint_6769-1][xPoint_6769+1] === this.CELL_EMPTY
                && map[yPoint_6769][xPoint_6769+1] === this.CELL_EMPTY
                && map[yPoint_6769+1][xPoint_6769+1] === this.CELL_EMPTY
                && map[yPoint_6769+1][xPoint_6769] === this.CELL_EMPTY
                && map[yPoint_6769+1][xPoint_6769-1] === this.CELL_EMPTY
                && map[yPoint_6769][xPoint_6769-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6769, yPoint_6769, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6769;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6769][x] === this.CELL_EMPTY
                    && map[yPoint_6769-1][x] === this.CELL_EMPTY
                    && map[yPoint_6769-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6769][x+1] === this.CELL_EMPTY
                    && map[yPoint_6769+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6769+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6769, yPoint_6769, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6769;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6769] === this.CELL_EMPTY
                    && map[y+1][xPoint_6769] === this.CELL_EMPTY
                    && map[y+1][xPoint_6769+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6769] === this.CELL_EMPTY
                    && map[y][xPoint_6769-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6769-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
