...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25682=0;yPoint_25682<this.gameFieldBorderY.length; yPoint_25682++){
                for(var xPoint_25682=0;xPoint_25682<this.gameFieldBorderX.length; xPoint_25682++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25682, xPoint_25682);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25682][xPoint_25682] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25682, xPoint_25682, 'user');
                    if(this._userShipsMap[yPoint_25682][xPoint_25682] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25682, xPoint_25682, type){
            var id = this.getPointBlockIdByCoords(yPoint_25682, xPoint_25682, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25682);
                block.setAttribute('data-y', yPoint_25682);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25682
         * @param {type} xPoint_25682
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25682, xPoint_25682, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25682 + '_y' + yPoint_25682;
            }
            return 'pc_x' + xPoint_25682 + '_y' + yPoint_25682;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25682=0;yPoint_25682<this.gameFieldBorderY.length; yPoint_25682++){
                for(var xPoint_25682=0;xPoint_25682<this.gameFieldBorderX.length; xPoint_25682++){
                    map.push({y: yPoint_25682, x: xPoint_25682});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25682=-1;yPoint_25682<(this.gameFieldBorderY.length+1); yPoint_25682++){
                for(var xPoint_25682=-1;xPoint_25682<(this.gameFieldBorderX.length+1); xPoint_25682++){
                    if(!map[yPoint_25682]){
                        map[yPoint_25682] = [];
                    }
                    map[yPoint_25682][xPoint_25682] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25682 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25682 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25682, yPoint_25682) === true){
                    if(this.canPutHorizontal(map, xPoint_25682, yPoint_25682, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25682][xPoint_25682 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25682, yPoint_25682, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25682 + i][xPoint_25682] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25682, yPoint_25682){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25682][xPoint_25682] === this.CELL_EMPTY
                && map[yPoint_25682-1][xPoint_25682] === this.CELL_EMPTY
                && map[yPoint_25682-1][xPoint_25682+1] === this.CELL_EMPTY
                && map[yPoint_25682][xPoint_25682+1] === this.CELL_EMPTY
                && map[yPoint_25682+1][xPoint_25682+1] === this.CELL_EMPTY
                && map[yPoint_25682+1][xPoint_25682] === this.CELL_EMPTY
                && map[yPoint_25682+1][xPoint_25682-1] === this.CELL_EMPTY
                && map[yPoint_25682][xPoint_25682-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25682, yPoint_25682, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25682;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25682][x] === this.CELL_EMPTY
                    && map[yPoint_25682-1][x] === this.CELL_EMPTY
                    && map[yPoint_25682-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25682][x+1] === this.CELL_EMPTY
                    && map[yPoint_25682+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25682+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25682, yPoint_25682, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25682;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25682] === this.CELL_EMPTY
                    && map[y+1][xPoint_25682] === this.CELL_EMPTY
                    && map[y+1][xPoint_25682+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25682] === this.CELL_EMPTY
                    && map[y][xPoint_25682-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25682-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
