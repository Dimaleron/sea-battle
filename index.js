...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22866=0;yPoint_22866<this.gameFieldBorderY.length; yPoint_22866++){
                for(var xPoint_22866=0;xPoint_22866<this.gameFieldBorderX.length; xPoint_22866++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22866, xPoint_22866);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22866][xPoint_22866] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22866, xPoint_22866, 'user');
                    if(this._userShipsMap[yPoint_22866][xPoint_22866] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22866, xPoint_22866, type){
            var id = this.getPointBlockIdByCoords(yPoint_22866, xPoint_22866, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22866);
                block.setAttribute('data-y', yPoint_22866);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22866
         * @param {type} xPoint_22866
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22866, xPoint_22866, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22866 + '_y' + yPoint_22866;
            }
            return 'pc_x' + xPoint_22866 + '_y' + yPoint_22866;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22866=0;yPoint_22866<this.gameFieldBorderY.length; yPoint_22866++){
                for(var xPoint_22866=0;xPoint_22866<this.gameFieldBorderX.length; xPoint_22866++){
                    map.push({y: yPoint_22866, x: xPoint_22866});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22866=-1;yPoint_22866<(this.gameFieldBorderY.length+1); yPoint_22866++){
                for(var xPoint_22866=-1;xPoint_22866<(this.gameFieldBorderX.length+1); xPoint_22866++){
                    if(!map[yPoint_22866]){
                        map[yPoint_22866] = [];
                    }
                    map[yPoint_22866][xPoint_22866] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22866 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22866 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22866, yPoint_22866) === true){
                    if(this.canPutHorizontal(map, xPoint_22866, yPoint_22866, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22866][xPoint_22866 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22866, yPoint_22866, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22866 + i][xPoint_22866] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22866, yPoint_22866){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22866][xPoint_22866] === this.CELL_EMPTY
                && map[yPoint_22866-1][xPoint_22866] === this.CELL_EMPTY
                && map[yPoint_22866-1][xPoint_22866+1] === this.CELL_EMPTY
                && map[yPoint_22866][xPoint_22866+1] === this.CELL_EMPTY
                && map[yPoint_22866+1][xPoint_22866+1] === this.CELL_EMPTY
                && map[yPoint_22866+1][xPoint_22866] === this.CELL_EMPTY
                && map[yPoint_22866+1][xPoint_22866-1] === this.CELL_EMPTY
                && map[yPoint_22866][xPoint_22866-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22866, yPoint_22866, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22866;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22866][x] === this.CELL_EMPTY
                    && map[yPoint_22866-1][x] === this.CELL_EMPTY
                    && map[yPoint_22866-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22866][x+1] === this.CELL_EMPTY
                    && map[yPoint_22866+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22866+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22866, yPoint_22866, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22866;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22866] === this.CELL_EMPTY
                    && map[y+1][xPoint_22866] === this.CELL_EMPTY
                    && map[y+1][xPoint_22866+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22866] === this.CELL_EMPTY
                    && map[y][xPoint_22866-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22866-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
