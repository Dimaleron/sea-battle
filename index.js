...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19419=0;yPoint_19419<this.gameFieldBorderY.length; yPoint_19419++){
                for(var xPoint_19419=0;xPoint_19419<this.gameFieldBorderX.length; xPoint_19419++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19419, xPoint_19419);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19419][xPoint_19419] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19419, xPoint_19419, 'user');
                    if(this._userShipsMap[yPoint_19419][xPoint_19419] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19419, xPoint_19419, type){
            var id = this.getPointBlockIdByCoords(yPoint_19419, xPoint_19419, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19419);
                block.setAttribute('data-y', yPoint_19419);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19419
         * @param {type} xPoint_19419
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19419, xPoint_19419, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19419 + '_y' + yPoint_19419;
            }
            return 'pc_x' + xPoint_19419 + '_y' + yPoint_19419;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19419=0;yPoint_19419<this.gameFieldBorderY.length; yPoint_19419++){
                for(var xPoint_19419=0;xPoint_19419<this.gameFieldBorderX.length; xPoint_19419++){
                    map.push({y: yPoint_19419, x: xPoint_19419});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19419=-1;yPoint_19419<(this.gameFieldBorderY.length+1); yPoint_19419++){
                for(var xPoint_19419=-1;xPoint_19419<(this.gameFieldBorderX.length+1); xPoint_19419++){
                    if(!map[yPoint_19419]){
                        map[yPoint_19419] = [];
                    }
                    map[yPoint_19419][xPoint_19419] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19419 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19419 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19419, yPoint_19419) === true){
                    if(this.canPutHorizontal(map, xPoint_19419, yPoint_19419, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19419][xPoint_19419 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19419, yPoint_19419, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19419 + i][xPoint_19419] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19419, yPoint_19419){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19419][xPoint_19419] === this.CELL_EMPTY
                && map[yPoint_19419-1][xPoint_19419] === this.CELL_EMPTY
                && map[yPoint_19419-1][xPoint_19419+1] === this.CELL_EMPTY
                && map[yPoint_19419][xPoint_19419+1] === this.CELL_EMPTY
                && map[yPoint_19419+1][xPoint_19419+1] === this.CELL_EMPTY
                && map[yPoint_19419+1][xPoint_19419] === this.CELL_EMPTY
                && map[yPoint_19419+1][xPoint_19419-1] === this.CELL_EMPTY
                && map[yPoint_19419][xPoint_19419-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19419, yPoint_19419, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19419;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19419][x] === this.CELL_EMPTY
                    && map[yPoint_19419-1][x] === this.CELL_EMPTY
                    && map[yPoint_19419-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19419][x+1] === this.CELL_EMPTY
                    && map[yPoint_19419+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19419+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19419, yPoint_19419, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19419;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19419] === this.CELL_EMPTY
                    && map[y+1][xPoint_19419] === this.CELL_EMPTY
                    && map[y+1][xPoint_19419+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19419] === this.CELL_EMPTY
                    && map[y][xPoint_19419-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19419-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
