...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20682=0;yPoint_20682<this.gameFieldBorderY.length; yPoint_20682++){
                for(var xPoint_20682=0;xPoint_20682<this.gameFieldBorderX.length; xPoint_20682++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20682, xPoint_20682);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20682][xPoint_20682] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20682, xPoint_20682, 'user');
                    if(this._userShipsMap[yPoint_20682][xPoint_20682] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20682, xPoint_20682, type){
            var id = this.getPointBlockIdByCoords(yPoint_20682, xPoint_20682, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20682);
                block.setAttribute('data-y', yPoint_20682);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20682
         * @param {type} xPoint_20682
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20682, xPoint_20682, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20682 + '_y' + yPoint_20682;
            }
            return 'pc_x' + xPoint_20682 + '_y' + yPoint_20682;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20682=0;yPoint_20682<this.gameFieldBorderY.length; yPoint_20682++){
                for(var xPoint_20682=0;xPoint_20682<this.gameFieldBorderX.length; xPoint_20682++){
                    map.push({y: yPoint_20682, x: xPoint_20682});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20682=-1;yPoint_20682<(this.gameFieldBorderY.length+1); yPoint_20682++){
                for(var xPoint_20682=-1;xPoint_20682<(this.gameFieldBorderX.length+1); xPoint_20682++){
                    if(!map[yPoint_20682]){
                        map[yPoint_20682] = [];
                    }
                    map[yPoint_20682][xPoint_20682] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20682 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20682 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20682, yPoint_20682) === true){
                    if(this.canPutHorizontal(map, xPoint_20682, yPoint_20682, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20682][xPoint_20682 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20682, yPoint_20682, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20682 + i][xPoint_20682] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20682, yPoint_20682){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20682][xPoint_20682] === this.CELL_EMPTY
                && map[yPoint_20682-1][xPoint_20682] === this.CELL_EMPTY
                && map[yPoint_20682-1][xPoint_20682+1] === this.CELL_EMPTY
                && map[yPoint_20682][xPoint_20682+1] === this.CELL_EMPTY
                && map[yPoint_20682+1][xPoint_20682+1] === this.CELL_EMPTY
                && map[yPoint_20682+1][xPoint_20682] === this.CELL_EMPTY
                && map[yPoint_20682+1][xPoint_20682-1] === this.CELL_EMPTY
                && map[yPoint_20682][xPoint_20682-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20682, yPoint_20682, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20682;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20682][x] === this.CELL_EMPTY
                    && map[yPoint_20682-1][x] === this.CELL_EMPTY
                    && map[yPoint_20682-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20682][x+1] === this.CELL_EMPTY
                    && map[yPoint_20682+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20682+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20682, yPoint_20682, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20682;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20682] === this.CELL_EMPTY
                    && map[y+1][xPoint_20682] === this.CELL_EMPTY
                    && map[y+1][xPoint_20682+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20682] === this.CELL_EMPTY
                    && map[y][xPoint_20682-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20682-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
