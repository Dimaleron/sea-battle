...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16725=0;yPoint_16725<this.gameFieldBorderY.length; yPoint_16725++){
                for(var xPoint_16725=0;xPoint_16725<this.gameFieldBorderX.length; xPoint_16725++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16725, xPoint_16725);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16725][xPoint_16725] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16725, xPoint_16725, 'user');
                    if(this._userShipsMap[yPoint_16725][xPoint_16725] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16725, xPoint_16725, type){
            var id = this.getPointBlockIdByCoords(yPoint_16725, xPoint_16725, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16725);
                block.setAttribute('data-y', yPoint_16725);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16725
         * @param {type} xPoint_16725
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16725, xPoint_16725, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16725 + '_y' + yPoint_16725;
            }
            return 'pc_x' + xPoint_16725 + '_y' + yPoint_16725;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16725=0;yPoint_16725<this.gameFieldBorderY.length; yPoint_16725++){
                for(var xPoint_16725=0;xPoint_16725<this.gameFieldBorderX.length; xPoint_16725++){
                    map.push({y: yPoint_16725, x: xPoint_16725});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16725=-1;yPoint_16725<(this.gameFieldBorderY.length+1); yPoint_16725++){
                for(var xPoint_16725=-1;xPoint_16725<(this.gameFieldBorderX.length+1); xPoint_16725++){
                    if(!map[yPoint_16725]){
                        map[yPoint_16725] = [];
                    }
                    map[yPoint_16725][xPoint_16725] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16725 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16725 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16725, yPoint_16725) === true){
                    if(this.canPutHorizontal(map, xPoint_16725, yPoint_16725, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16725][xPoint_16725 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16725, yPoint_16725, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16725 + i][xPoint_16725] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16725, yPoint_16725){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16725][xPoint_16725] === this.CELL_EMPTY
                && map[yPoint_16725-1][xPoint_16725] === this.CELL_EMPTY
                && map[yPoint_16725-1][xPoint_16725+1] === this.CELL_EMPTY
                && map[yPoint_16725][xPoint_16725+1] === this.CELL_EMPTY
                && map[yPoint_16725+1][xPoint_16725+1] === this.CELL_EMPTY
                && map[yPoint_16725+1][xPoint_16725] === this.CELL_EMPTY
                && map[yPoint_16725+1][xPoint_16725-1] === this.CELL_EMPTY
                && map[yPoint_16725][xPoint_16725-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16725, yPoint_16725, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16725;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16725][x] === this.CELL_EMPTY
                    && map[yPoint_16725-1][x] === this.CELL_EMPTY
                    && map[yPoint_16725-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16725][x+1] === this.CELL_EMPTY
                    && map[yPoint_16725+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16725+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16725, yPoint_16725, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16725;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16725] === this.CELL_EMPTY
                    && map[y+1][xPoint_16725] === this.CELL_EMPTY
                    && map[y+1][xPoint_16725+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16725] === this.CELL_EMPTY
                    && map[y][xPoint_16725-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16725-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
