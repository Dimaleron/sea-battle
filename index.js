...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16413=0;yPoint_16413<this.gameFieldBorderY.length; yPoint_16413++){
                for(var xPoint_16413=0;xPoint_16413<this.gameFieldBorderX.length; xPoint_16413++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16413, xPoint_16413);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16413][xPoint_16413] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16413, xPoint_16413, 'user');
                    if(this._userShipsMap[yPoint_16413][xPoint_16413] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16413, xPoint_16413, type){
            var id = this.getPointBlockIdByCoords(yPoint_16413, xPoint_16413, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16413);
                block.setAttribute('data-y', yPoint_16413);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16413
         * @param {type} xPoint_16413
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16413, xPoint_16413, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16413 + '_y' + yPoint_16413;
            }
            return 'pc_x' + xPoint_16413 + '_y' + yPoint_16413;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16413=0;yPoint_16413<this.gameFieldBorderY.length; yPoint_16413++){
                for(var xPoint_16413=0;xPoint_16413<this.gameFieldBorderX.length; xPoint_16413++){
                    map.push({y: yPoint_16413, x: xPoint_16413});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16413=-1;yPoint_16413<(this.gameFieldBorderY.length+1); yPoint_16413++){
                for(var xPoint_16413=-1;xPoint_16413<(this.gameFieldBorderX.length+1); xPoint_16413++){
                    if(!map[yPoint_16413]){
                        map[yPoint_16413] = [];
                    }
                    map[yPoint_16413][xPoint_16413] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16413 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16413 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16413, yPoint_16413) === true){
                    if(this.canPutHorizontal(map, xPoint_16413, yPoint_16413, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16413][xPoint_16413 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16413, yPoint_16413, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16413 + i][xPoint_16413] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16413, yPoint_16413){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16413][xPoint_16413] === this.CELL_EMPTY
                && map[yPoint_16413-1][xPoint_16413] === this.CELL_EMPTY
                && map[yPoint_16413-1][xPoint_16413+1] === this.CELL_EMPTY
                && map[yPoint_16413][xPoint_16413+1] === this.CELL_EMPTY
                && map[yPoint_16413+1][xPoint_16413+1] === this.CELL_EMPTY
                && map[yPoint_16413+1][xPoint_16413] === this.CELL_EMPTY
                && map[yPoint_16413+1][xPoint_16413-1] === this.CELL_EMPTY
                && map[yPoint_16413][xPoint_16413-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16413, yPoint_16413, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16413;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16413][x] === this.CELL_EMPTY
                    && map[yPoint_16413-1][x] === this.CELL_EMPTY
                    && map[yPoint_16413-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16413][x+1] === this.CELL_EMPTY
                    && map[yPoint_16413+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16413+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16413, yPoint_16413, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16413;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16413] === this.CELL_EMPTY
                    && map[y+1][xPoint_16413] === this.CELL_EMPTY
                    && map[y+1][xPoint_16413+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16413] === this.CELL_EMPTY
                    && map[y][xPoint_16413-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16413-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
