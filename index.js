...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28088=0;yPoint_28088<this.gameFieldBorderY.length; yPoint_28088++){
                for(var xPoint_28088=0;xPoint_28088<this.gameFieldBorderX.length; xPoint_28088++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28088, xPoint_28088);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28088][xPoint_28088] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28088, xPoint_28088, 'user');
                    if(this._userShipsMap[yPoint_28088][xPoint_28088] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28088, xPoint_28088, type){
            var id = this.getPointBlockIdByCoords(yPoint_28088, xPoint_28088, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28088);
                block.setAttribute('data-y', yPoint_28088);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28088
         * @param {type} xPoint_28088
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28088, xPoint_28088, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28088 + '_y' + yPoint_28088;
            }
            return 'pc_x' + xPoint_28088 + '_y' + yPoint_28088;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28088=0;yPoint_28088<this.gameFieldBorderY.length; yPoint_28088++){
                for(var xPoint_28088=0;xPoint_28088<this.gameFieldBorderX.length; xPoint_28088++){
                    map.push({y: yPoint_28088, x: xPoint_28088});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28088=-1;yPoint_28088<(this.gameFieldBorderY.length+1); yPoint_28088++){
                for(var xPoint_28088=-1;xPoint_28088<(this.gameFieldBorderX.length+1); xPoint_28088++){
                    if(!map[yPoint_28088]){
                        map[yPoint_28088] = [];
                    }
                    map[yPoint_28088][xPoint_28088] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28088 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28088 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28088, yPoint_28088) === true){
                    if(this.canPutHorizontal(map, xPoint_28088, yPoint_28088, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28088][xPoint_28088 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28088, yPoint_28088, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28088 + i][xPoint_28088] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28088, yPoint_28088){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28088][xPoint_28088] === this.CELL_EMPTY
                && map[yPoint_28088-1][xPoint_28088] === this.CELL_EMPTY
                && map[yPoint_28088-1][xPoint_28088+1] === this.CELL_EMPTY
                && map[yPoint_28088][xPoint_28088+1] === this.CELL_EMPTY
                && map[yPoint_28088+1][xPoint_28088+1] === this.CELL_EMPTY
                && map[yPoint_28088+1][xPoint_28088] === this.CELL_EMPTY
                && map[yPoint_28088+1][xPoint_28088-1] === this.CELL_EMPTY
                && map[yPoint_28088][xPoint_28088-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28088, yPoint_28088, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28088;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28088][x] === this.CELL_EMPTY
                    && map[yPoint_28088-1][x] === this.CELL_EMPTY
                    && map[yPoint_28088-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28088][x+1] === this.CELL_EMPTY
                    && map[yPoint_28088+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28088+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28088, yPoint_28088, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28088;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28088] === this.CELL_EMPTY
                    && map[y+1][xPoint_28088] === this.CELL_EMPTY
                    && map[y+1][xPoint_28088+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28088] === this.CELL_EMPTY
                    && map[y][xPoint_28088-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28088-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
