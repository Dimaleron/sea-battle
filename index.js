...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22409=0;yPoint_22409<this.gameFieldBorderY.length; yPoint_22409++){
                for(var xPoint_22409=0;xPoint_22409<this.gameFieldBorderX.length; xPoint_22409++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22409, xPoint_22409);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22409][xPoint_22409] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22409, xPoint_22409, 'user');
                    if(this._userShipsMap[yPoint_22409][xPoint_22409] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22409, xPoint_22409, type){
            var id = this.getPointBlockIdByCoords(yPoint_22409, xPoint_22409, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22409);
                block.setAttribute('data-y', yPoint_22409);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22409
         * @param {type} xPoint_22409
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22409, xPoint_22409, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22409 + '_y' + yPoint_22409;
            }
            return 'pc_x' + xPoint_22409 + '_y' + yPoint_22409;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22409=0;yPoint_22409<this.gameFieldBorderY.length; yPoint_22409++){
                for(var xPoint_22409=0;xPoint_22409<this.gameFieldBorderX.length; xPoint_22409++){
                    map.push({y: yPoint_22409, x: xPoint_22409});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22409=-1;yPoint_22409<(this.gameFieldBorderY.length+1); yPoint_22409++){
                for(var xPoint_22409=-1;xPoint_22409<(this.gameFieldBorderX.length+1); xPoint_22409++){
                    if(!map[yPoint_22409]){
                        map[yPoint_22409] = [];
                    }
                    map[yPoint_22409][xPoint_22409] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22409 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22409 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22409, yPoint_22409) === true){
                    if(this.canPutHorizontal(map, xPoint_22409, yPoint_22409, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22409][xPoint_22409 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22409, yPoint_22409, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22409 + i][xPoint_22409] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22409, yPoint_22409){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22409][xPoint_22409] === this.CELL_EMPTY
                && map[yPoint_22409-1][xPoint_22409] === this.CELL_EMPTY
                && map[yPoint_22409-1][xPoint_22409+1] === this.CELL_EMPTY
                && map[yPoint_22409][xPoint_22409+1] === this.CELL_EMPTY
                && map[yPoint_22409+1][xPoint_22409+1] === this.CELL_EMPTY
                && map[yPoint_22409+1][xPoint_22409] === this.CELL_EMPTY
                && map[yPoint_22409+1][xPoint_22409-1] === this.CELL_EMPTY
                && map[yPoint_22409][xPoint_22409-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22409, yPoint_22409, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22409;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22409][x] === this.CELL_EMPTY
                    && map[yPoint_22409-1][x] === this.CELL_EMPTY
                    && map[yPoint_22409-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22409][x+1] === this.CELL_EMPTY
                    && map[yPoint_22409+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22409+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22409, yPoint_22409, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22409;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22409] === this.CELL_EMPTY
                    && map[y+1][xPoint_22409] === this.CELL_EMPTY
                    && map[y+1][xPoint_22409+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22409] === this.CELL_EMPTY
                    && map[y][xPoint_22409-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22409-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
