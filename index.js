...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9100=0;yPoint_9100<this.gameFieldBorderY.length; yPoint_9100++){
                for(var xPoint_9100=0;xPoint_9100<this.gameFieldBorderX.length; xPoint_9100++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9100, xPoint_9100);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9100][xPoint_9100] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9100, xPoint_9100, 'user');
                    if(this._userShipsMap[yPoint_9100][xPoint_9100] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9100, xPoint_9100, type){
            var id = this.getPointBlockIdByCoords(yPoint_9100, xPoint_9100, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9100);
                block.setAttribute('data-y', yPoint_9100);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9100
         * @param {type} xPoint_9100
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9100, xPoint_9100, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9100 + '_y' + yPoint_9100;
            }
            return 'pc_x' + xPoint_9100 + '_y' + yPoint_9100;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9100=0;yPoint_9100<this.gameFieldBorderY.length; yPoint_9100++){
                for(var xPoint_9100=0;xPoint_9100<this.gameFieldBorderX.length; xPoint_9100++){
                    map.push({y: yPoint_9100, x: xPoint_9100});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9100=-1;yPoint_9100<(this.gameFieldBorderY.length+1); yPoint_9100++){
                for(var xPoint_9100=-1;xPoint_9100<(this.gameFieldBorderX.length+1); xPoint_9100++){
                    if(!map[yPoint_9100]){
                        map[yPoint_9100] = [];
                    }
                    map[yPoint_9100][xPoint_9100] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9100 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9100 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9100, yPoint_9100) === true){
                    if(this.canPutHorizontal(map, xPoint_9100, yPoint_9100, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9100][xPoint_9100 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9100, yPoint_9100, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9100 + i][xPoint_9100] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9100, yPoint_9100){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9100][xPoint_9100] === this.CELL_EMPTY
                && map[yPoint_9100-1][xPoint_9100] === this.CELL_EMPTY
                && map[yPoint_9100-1][xPoint_9100+1] === this.CELL_EMPTY
                && map[yPoint_9100][xPoint_9100+1] === this.CELL_EMPTY
                && map[yPoint_9100+1][xPoint_9100+1] === this.CELL_EMPTY
                && map[yPoint_9100+1][xPoint_9100] === this.CELL_EMPTY
                && map[yPoint_9100+1][xPoint_9100-1] === this.CELL_EMPTY
                && map[yPoint_9100][xPoint_9100-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9100, yPoint_9100, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9100;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9100][x] === this.CELL_EMPTY
                    && map[yPoint_9100-1][x] === this.CELL_EMPTY
                    && map[yPoint_9100-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9100][x+1] === this.CELL_EMPTY
                    && map[yPoint_9100+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9100+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9100, yPoint_9100, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9100;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9100] === this.CELL_EMPTY
                    && map[y+1][xPoint_9100] === this.CELL_EMPTY
                    && map[y+1][xPoint_9100+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9100] === this.CELL_EMPTY
                    && map[y][xPoint_9100-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9100-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
