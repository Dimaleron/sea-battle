...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20601=0;yPoint_20601<this.gameFieldBorderY.length; yPoint_20601++){
                for(var xPoint_20601=0;xPoint_20601<this.gameFieldBorderX.length; xPoint_20601++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20601, xPoint_20601);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20601][xPoint_20601] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20601, xPoint_20601, 'user');
                    if(this._userShipsMap[yPoint_20601][xPoint_20601] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20601, xPoint_20601, type){
            var id = this.getPointBlockIdByCoords(yPoint_20601, xPoint_20601, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20601);
                block.setAttribute('data-y', yPoint_20601);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20601
         * @param {type} xPoint_20601
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20601, xPoint_20601, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20601 + '_y' + yPoint_20601;
            }
            return 'pc_x' + xPoint_20601 + '_y' + yPoint_20601;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20601=0;yPoint_20601<this.gameFieldBorderY.length; yPoint_20601++){
                for(var xPoint_20601=0;xPoint_20601<this.gameFieldBorderX.length; xPoint_20601++){
                    map.push({y: yPoint_20601, x: xPoint_20601});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20601=-1;yPoint_20601<(this.gameFieldBorderY.length+1); yPoint_20601++){
                for(var xPoint_20601=-1;xPoint_20601<(this.gameFieldBorderX.length+1); xPoint_20601++){
                    if(!map[yPoint_20601]){
                        map[yPoint_20601] = [];
                    }
                    map[yPoint_20601][xPoint_20601] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20601 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20601 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20601, yPoint_20601) === true){
                    if(this.canPutHorizontal(map, xPoint_20601, yPoint_20601, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20601][xPoint_20601 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20601, yPoint_20601, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20601 + i][xPoint_20601] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20601, yPoint_20601){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20601][xPoint_20601] === this.CELL_EMPTY
                && map[yPoint_20601-1][xPoint_20601] === this.CELL_EMPTY
                && map[yPoint_20601-1][xPoint_20601+1] === this.CELL_EMPTY
                && map[yPoint_20601][xPoint_20601+1] === this.CELL_EMPTY
                && map[yPoint_20601+1][xPoint_20601+1] === this.CELL_EMPTY
                && map[yPoint_20601+1][xPoint_20601] === this.CELL_EMPTY
                && map[yPoint_20601+1][xPoint_20601-1] === this.CELL_EMPTY
                && map[yPoint_20601][xPoint_20601-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20601, yPoint_20601, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20601;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20601][x] === this.CELL_EMPTY
                    && map[yPoint_20601-1][x] === this.CELL_EMPTY
                    && map[yPoint_20601-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20601][x+1] === this.CELL_EMPTY
                    && map[yPoint_20601+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20601+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20601, yPoint_20601, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20601;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20601] === this.CELL_EMPTY
                    && map[y+1][xPoint_20601] === this.CELL_EMPTY
                    && map[y+1][xPoint_20601+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20601] === this.CELL_EMPTY
                    && map[y][xPoint_20601-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20601-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
