...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15114=0;yPoint_15114<this.gameFieldBorderY.length; yPoint_15114++){
                for(var xPoint_15114=0;xPoint_15114<this.gameFieldBorderX.length; xPoint_15114++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15114, xPoint_15114);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15114][xPoint_15114] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15114, xPoint_15114, 'user');
                    if(this._userShipsMap[yPoint_15114][xPoint_15114] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15114, xPoint_15114, type){
            var id = this.getPointBlockIdByCoords(yPoint_15114, xPoint_15114, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15114);
                block.setAttribute('data-y', yPoint_15114);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15114
         * @param {type} xPoint_15114
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15114, xPoint_15114, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15114 + '_y' + yPoint_15114;
            }
            return 'pc_x' + xPoint_15114 + '_y' + yPoint_15114;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15114=0;yPoint_15114<this.gameFieldBorderY.length; yPoint_15114++){
                for(var xPoint_15114=0;xPoint_15114<this.gameFieldBorderX.length; xPoint_15114++){
                    map.push({y: yPoint_15114, x: xPoint_15114});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15114=-1;yPoint_15114<(this.gameFieldBorderY.length+1); yPoint_15114++){
                for(var xPoint_15114=-1;xPoint_15114<(this.gameFieldBorderX.length+1); xPoint_15114++){
                    if(!map[yPoint_15114]){
                        map[yPoint_15114] = [];
                    }
                    map[yPoint_15114][xPoint_15114] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15114 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15114 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15114, yPoint_15114) === true){
                    if(this.canPutHorizontal(map, xPoint_15114, yPoint_15114, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15114][xPoint_15114 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15114, yPoint_15114, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15114 + i][xPoint_15114] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15114, yPoint_15114){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15114][xPoint_15114] === this.CELL_EMPTY
                && map[yPoint_15114-1][xPoint_15114] === this.CELL_EMPTY
                && map[yPoint_15114-1][xPoint_15114+1] === this.CELL_EMPTY
                && map[yPoint_15114][xPoint_15114+1] === this.CELL_EMPTY
                && map[yPoint_15114+1][xPoint_15114+1] === this.CELL_EMPTY
                && map[yPoint_15114+1][xPoint_15114] === this.CELL_EMPTY
                && map[yPoint_15114+1][xPoint_15114-1] === this.CELL_EMPTY
                && map[yPoint_15114][xPoint_15114-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15114, yPoint_15114, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15114;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15114][x] === this.CELL_EMPTY
                    && map[yPoint_15114-1][x] === this.CELL_EMPTY
                    && map[yPoint_15114-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15114][x+1] === this.CELL_EMPTY
                    && map[yPoint_15114+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15114+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15114, yPoint_15114, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15114;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15114] === this.CELL_EMPTY
                    && map[y+1][xPoint_15114] === this.CELL_EMPTY
                    && map[y+1][xPoint_15114+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15114] === this.CELL_EMPTY
                    && map[y][xPoint_15114-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15114-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
