...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_607=0;yPoint_607<this.gameFieldBorderY.length; yPoint_607++){
                for(var xPoint_607=0;xPoint_607<this.gameFieldBorderX.length; xPoint_607++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_607, xPoint_607);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_607][xPoint_607] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_607, xPoint_607, 'user');
                    if(this._userShipsMap[yPoint_607][xPoint_607] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_607, xPoint_607, type){
            var id = this.getPointBlockIdByCoords(yPoint_607, xPoint_607, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_607);
                block.setAttribute('data-y', yPoint_607);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_607
         * @param {type} xPoint_607
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_607, xPoint_607, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_607 + '_y' + yPoint_607;
            }
            return 'pc_x' + xPoint_607 + '_y' + yPoint_607;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_607=0;yPoint_607<this.gameFieldBorderY.length; yPoint_607++){
                for(var xPoint_607=0;xPoint_607<this.gameFieldBorderX.length; xPoint_607++){
                    map.push({y: yPoint_607, x: xPoint_607});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_607=-1;yPoint_607<(this.gameFieldBorderY.length+1); yPoint_607++){
                for(var xPoint_607=-1;xPoint_607<(this.gameFieldBorderX.length+1); xPoint_607++){
                    if(!map[yPoint_607]){
                        map[yPoint_607] = [];
                    }
                    map[yPoint_607][xPoint_607] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_607 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_607 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_607, yPoint_607) === true){
                    if(this.canPutHorizontal(map, xPoint_607, yPoint_607, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_607][xPoint_607 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_607, yPoint_607, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_607 + i][xPoint_607] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_607, yPoint_607){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_607][xPoint_607] === this.CELL_EMPTY
                && map[yPoint_607-1][xPoint_607] === this.CELL_EMPTY
                && map[yPoint_607-1][xPoint_607+1] === this.CELL_EMPTY
                && map[yPoint_607][xPoint_607+1] === this.CELL_EMPTY
                && map[yPoint_607+1][xPoint_607+1] === this.CELL_EMPTY
                && map[yPoint_607+1][xPoint_607] === this.CELL_EMPTY
                && map[yPoint_607+1][xPoint_607-1] === this.CELL_EMPTY
                && map[yPoint_607][xPoint_607-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_607, yPoint_607, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_607;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_607][x] === this.CELL_EMPTY
                    && map[yPoint_607-1][x] === this.CELL_EMPTY
                    && map[yPoint_607-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_607][x+1] === this.CELL_EMPTY
                    && map[yPoint_607+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_607+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_607, yPoint_607, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_607;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_607] === this.CELL_EMPTY
                    && map[y+1][xPoint_607] === this.CELL_EMPTY
                    && map[y+1][xPoint_607+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_607] === this.CELL_EMPTY
                    && map[y][xPoint_607-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_607-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
