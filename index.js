...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19849=0;yPoint_19849<this.gameFieldBorderY.length; yPoint_19849++){
                for(var xPoint_19849=0;xPoint_19849<this.gameFieldBorderX.length; xPoint_19849++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19849, xPoint_19849);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19849][xPoint_19849] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19849, xPoint_19849, 'user');
                    if(this._userShipsMap[yPoint_19849][xPoint_19849] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19849, xPoint_19849, type){
            var id = this.getPointBlockIdByCoords(yPoint_19849, xPoint_19849, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19849);
                block.setAttribute('data-y', yPoint_19849);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19849
         * @param {type} xPoint_19849
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19849, xPoint_19849, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19849 + '_y' + yPoint_19849;
            }
            return 'pc_x' + xPoint_19849 + '_y' + yPoint_19849;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19849=0;yPoint_19849<this.gameFieldBorderY.length; yPoint_19849++){
                for(var xPoint_19849=0;xPoint_19849<this.gameFieldBorderX.length; xPoint_19849++){
                    map.push({y: yPoint_19849, x: xPoint_19849});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19849=-1;yPoint_19849<(this.gameFieldBorderY.length+1); yPoint_19849++){
                for(var xPoint_19849=-1;xPoint_19849<(this.gameFieldBorderX.length+1); xPoint_19849++){
                    if(!map[yPoint_19849]){
                        map[yPoint_19849] = [];
                    }
                    map[yPoint_19849][xPoint_19849] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19849 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19849 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19849, yPoint_19849) === true){
                    if(this.canPutHorizontal(map, xPoint_19849, yPoint_19849, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19849][xPoint_19849 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19849, yPoint_19849, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19849 + i][xPoint_19849] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19849, yPoint_19849){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19849][xPoint_19849] === this.CELL_EMPTY
                && map[yPoint_19849-1][xPoint_19849] === this.CELL_EMPTY
                && map[yPoint_19849-1][xPoint_19849+1] === this.CELL_EMPTY
                && map[yPoint_19849][xPoint_19849+1] === this.CELL_EMPTY
                && map[yPoint_19849+1][xPoint_19849+1] === this.CELL_EMPTY
                && map[yPoint_19849+1][xPoint_19849] === this.CELL_EMPTY
                && map[yPoint_19849+1][xPoint_19849-1] === this.CELL_EMPTY
                && map[yPoint_19849][xPoint_19849-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19849, yPoint_19849, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19849;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19849][x] === this.CELL_EMPTY
                    && map[yPoint_19849-1][x] === this.CELL_EMPTY
                    && map[yPoint_19849-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19849][x+1] === this.CELL_EMPTY
                    && map[yPoint_19849+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19849+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19849, yPoint_19849, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19849;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19849] === this.CELL_EMPTY
                    && map[y+1][xPoint_19849] === this.CELL_EMPTY
                    && map[y+1][xPoint_19849+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19849] === this.CELL_EMPTY
                    && map[y][xPoint_19849-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19849-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
