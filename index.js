...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21619=0;yPoint_21619<this.gameFieldBorderY.length; yPoint_21619++){
                for(var xPoint_21619=0;xPoint_21619<this.gameFieldBorderX.length; xPoint_21619++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21619, xPoint_21619);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21619][xPoint_21619] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21619, xPoint_21619, 'user');
                    if(this._userShipsMap[yPoint_21619][xPoint_21619] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21619, xPoint_21619, type){
            var id = this.getPointBlockIdByCoords(yPoint_21619, xPoint_21619, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21619);
                block.setAttribute('data-y', yPoint_21619);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21619
         * @param {type} xPoint_21619
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21619, xPoint_21619, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21619 + '_y' + yPoint_21619;
            }
            return 'pc_x' + xPoint_21619 + '_y' + yPoint_21619;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21619=0;yPoint_21619<this.gameFieldBorderY.length; yPoint_21619++){
                for(var xPoint_21619=0;xPoint_21619<this.gameFieldBorderX.length; xPoint_21619++){
                    map.push({y: yPoint_21619, x: xPoint_21619});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21619=-1;yPoint_21619<(this.gameFieldBorderY.length+1); yPoint_21619++){
                for(var xPoint_21619=-1;xPoint_21619<(this.gameFieldBorderX.length+1); xPoint_21619++){
                    if(!map[yPoint_21619]){
                        map[yPoint_21619] = [];
                    }
                    map[yPoint_21619][xPoint_21619] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21619 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21619 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21619, yPoint_21619) === true){
                    if(this.canPutHorizontal(map, xPoint_21619, yPoint_21619, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21619][xPoint_21619 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21619, yPoint_21619, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21619 + i][xPoint_21619] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21619, yPoint_21619){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21619][xPoint_21619] === this.CELL_EMPTY
                && map[yPoint_21619-1][xPoint_21619] === this.CELL_EMPTY
                && map[yPoint_21619-1][xPoint_21619+1] === this.CELL_EMPTY
                && map[yPoint_21619][xPoint_21619+1] === this.CELL_EMPTY
                && map[yPoint_21619+1][xPoint_21619+1] === this.CELL_EMPTY
                && map[yPoint_21619+1][xPoint_21619] === this.CELL_EMPTY
                && map[yPoint_21619+1][xPoint_21619-1] === this.CELL_EMPTY
                && map[yPoint_21619][xPoint_21619-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21619, yPoint_21619, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21619;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21619][x] === this.CELL_EMPTY
                    && map[yPoint_21619-1][x] === this.CELL_EMPTY
                    && map[yPoint_21619-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21619][x+1] === this.CELL_EMPTY
                    && map[yPoint_21619+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21619+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21619, yPoint_21619, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21619;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21619] === this.CELL_EMPTY
                    && map[y+1][xPoint_21619] === this.CELL_EMPTY
                    && map[y+1][xPoint_21619+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21619] === this.CELL_EMPTY
                    && map[y][xPoint_21619-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21619-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
