...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22076=0;yPoint_22076<this.gameFieldBorderY.length; yPoint_22076++){
                for(var xPoint_22076=0;xPoint_22076<this.gameFieldBorderX.length; xPoint_22076++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22076, xPoint_22076);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22076][xPoint_22076] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22076, xPoint_22076, 'user');
                    if(this._userShipsMap[yPoint_22076][xPoint_22076] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22076, xPoint_22076, type){
            var id = this.getPointBlockIdByCoords(yPoint_22076, xPoint_22076, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22076);
                block.setAttribute('data-y', yPoint_22076);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22076
         * @param {type} xPoint_22076
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22076, xPoint_22076, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22076 + '_y' + yPoint_22076;
            }
            return 'pc_x' + xPoint_22076 + '_y' + yPoint_22076;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22076=0;yPoint_22076<this.gameFieldBorderY.length; yPoint_22076++){
                for(var xPoint_22076=0;xPoint_22076<this.gameFieldBorderX.length; xPoint_22076++){
                    map.push({y: yPoint_22076, x: xPoint_22076});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22076=-1;yPoint_22076<(this.gameFieldBorderY.length+1); yPoint_22076++){
                for(var xPoint_22076=-1;xPoint_22076<(this.gameFieldBorderX.length+1); xPoint_22076++){
                    if(!map[yPoint_22076]){
                        map[yPoint_22076] = [];
                    }
                    map[yPoint_22076][xPoint_22076] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22076 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22076 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22076, yPoint_22076) === true){
                    if(this.canPutHorizontal(map, xPoint_22076, yPoint_22076, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22076][xPoint_22076 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22076, yPoint_22076, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22076 + i][xPoint_22076] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22076, yPoint_22076){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22076][xPoint_22076] === this.CELL_EMPTY
                && map[yPoint_22076-1][xPoint_22076] === this.CELL_EMPTY
                && map[yPoint_22076-1][xPoint_22076+1] === this.CELL_EMPTY
                && map[yPoint_22076][xPoint_22076+1] === this.CELL_EMPTY
                && map[yPoint_22076+1][xPoint_22076+1] === this.CELL_EMPTY
                && map[yPoint_22076+1][xPoint_22076] === this.CELL_EMPTY
                && map[yPoint_22076+1][xPoint_22076-1] === this.CELL_EMPTY
                && map[yPoint_22076][xPoint_22076-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22076, yPoint_22076, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22076;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22076][x] === this.CELL_EMPTY
                    && map[yPoint_22076-1][x] === this.CELL_EMPTY
                    && map[yPoint_22076-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22076][x+1] === this.CELL_EMPTY
                    && map[yPoint_22076+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22076+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22076, yPoint_22076, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22076;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22076] === this.CELL_EMPTY
                    && map[y+1][xPoint_22076] === this.CELL_EMPTY
                    && map[y+1][xPoint_22076+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22076] === this.CELL_EMPTY
                    && map[y][xPoint_22076-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22076-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
