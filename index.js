...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20000=0;yPoint_20000<this.gameFieldBorderY.length; yPoint_20000++){
                for(var xPoint_20000=0;xPoint_20000<this.gameFieldBorderX.length; xPoint_20000++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20000, xPoint_20000);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20000][xPoint_20000] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20000, xPoint_20000, 'user');
                    if(this._userShipsMap[yPoint_20000][xPoint_20000] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20000, xPoint_20000, type){
            var id = this.getPointBlockIdByCoords(yPoint_20000, xPoint_20000, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20000);
                block.setAttribute('data-y', yPoint_20000);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20000
         * @param {type} xPoint_20000
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20000, xPoint_20000, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20000 + '_y' + yPoint_20000;
            }
            return 'pc_x' + xPoint_20000 + '_y' + yPoint_20000;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20000=0;yPoint_20000<this.gameFieldBorderY.length; yPoint_20000++){
                for(var xPoint_20000=0;xPoint_20000<this.gameFieldBorderX.length; xPoint_20000++){
                    map.push({y: yPoint_20000, x: xPoint_20000});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20000=-1;yPoint_20000<(this.gameFieldBorderY.length+1); yPoint_20000++){
                for(var xPoint_20000=-1;xPoint_20000<(this.gameFieldBorderX.length+1); xPoint_20000++){
                    if(!map[yPoint_20000]){
                        map[yPoint_20000] = [];
                    }
                    map[yPoint_20000][xPoint_20000] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20000 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20000 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20000, yPoint_20000) === true){
                    if(this.canPutHorizontal(map, xPoint_20000, yPoint_20000, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20000][xPoint_20000 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20000, yPoint_20000, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20000 + i][xPoint_20000] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20000, yPoint_20000){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20000][xPoint_20000] === this.CELL_EMPTY
                && map[yPoint_20000-1][xPoint_20000] === this.CELL_EMPTY
                && map[yPoint_20000-1][xPoint_20000+1] === this.CELL_EMPTY
                && map[yPoint_20000][xPoint_20000+1] === this.CELL_EMPTY
                && map[yPoint_20000+1][xPoint_20000+1] === this.CELL_EMPTY
                && map[yPoint_20000+1][xPoint_20000] === this.CELL_EMPTY
                && map[yPoint_20000+1][xPoint_20000-1] === this.CELL_EMPTY
                && map[yPoint_20000][xPoint_20000-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20000, yPoint_20000, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20000;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20000][x] === this.CELL_EMPTY
                    && map[yPoint_20000-1][x] === this.CELL_EMPTY
                    && map[yPoint_20000-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20000][x+1] === this.CELL_EMPTY
                    && map[yPoint_20000+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20000+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20000, yPoint_20000, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20000;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20000] === this.CELL_EMPTY
                    && map[y+1][xPoint_20000] === this.CELL_EMPTY
                    && map[y+1][xPoint_20000+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20000] === this.CELL_EMPTY
                    && map[y][xPoint_20000-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20000-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
