...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17163=0;yPoint_17163<this.gameFieldBorderY.length; yPoint_17163++){
                for(var xPoint_17163=0;xPoint_17163<this.gameFieldBorderX.length; xPoint_17163++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17163, xPoint_17163);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17163][xPoint_17163] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17163, xPoint_17163, 'user');
                    if(this._userShipsMap[yPoint_17163][xPoint_17163] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17163, xPoint_17163, type){
            var id = this.getPointBlockIdByCoords(yPoint_17163, xPoint_17163, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17163);
                block.setAttribute('data-y', yPoint_17163);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17163
         * @param {type} xPoint_17163
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17163, xPoint_17163, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17163 + '_y' + yPoint_17163;
            }
            return 'pc_x' + xPoint_17163 + '_y' + yPoint_17163;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17163=0;yPoint_17163<this.gameFieldBorderY.length; yPoint_17163++){
                for(var xPoint_17163=0;xPoint_17163<this.gameFieldBorderX.length; xPoint_17163++){
                    map.push({y: yPoint_17163, x: xPoint_17163});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17163=-1;yPoint_17163<(this.gameFieldBorderY.length+1); yPoint_17163++){
                for(var xPoint_17163=-1;xPoint_17163<(this.gameFieldBorderX.length+1); xPoint_17163++){
                    if(!map[yPoint_17163]){
                        map[yPoint_17163] = [];
                    }
                    map[yPoint_17163][xPoint_17163] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17163 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17163 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17163, yPoint_17163) === true){
                    if(this.canPutHorizontal(map, xPoint_17163, yPoint_17163, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17163][xPoint_17163 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17163, yPoint_17163, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17163 + i][xPoint_17163] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17163, yPoint_17163){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17163][xPoint_17163] === this.CELL_EMPTY
                && map[yPoint_17163-1][xPoint_17163] === this.CELL_EMPTY
                && map[yPoint_17163-1][xPoint_17163+1] === this.CELL_EMPTY
                && map[yPoint_17163][xPoint_17163+1] === this.CELL_EMPTY
                && map[yPoint_17163+1][xPoint_17163+1] === this.CELL_EMPTY
                && map[yPoint_17163+1][xPoint_17163] === this.CELL_EMPTY
                && map[yPoint_17163+1][xPoint_17163-1] === this.CELL_EMPTY
                && map[yPoint_17163][xPoint_17163-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17163, yPoint_17163, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17163;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17163][x] === this.CELL_EMPTY
                    && map[yPoint_17163-1][x] === this.CELL_EMPTY
                    && map[yPoint_17163-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17163][x+1] === this.CELL_EMPTY
                    && map[yPoint_17163+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17163+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17163, yPoint_17163, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17163;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17163] === this.CELL_EMPTY
                    && map[y+1][xPoint_17163] === this.CELL_EMPTY
                    && map[y+1][xPoint_17163+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17163] === this.CELL_EMPTY
                    && map[y][xPoint_17163-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17163-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
