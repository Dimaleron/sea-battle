...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30635=0;yPoint_30635<this.gameFieldBorderY.length; yPoint_30635++){
                for(var xPoint_30635=0;xPoint_30635<this.gameFieldBorderX.length; xPoint_30635++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30635, xPoint_30635);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30635][xPoint_30635] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30635, xPoint_30635, 'user');
                    if(this._userShipsMap[yPoint_30635][xPoint_30635] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30635, xPoint_30635, type){
            var id = this.getPointBlockIdByCoords(yPoint_30635, xPoint_30635, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30635);
                block.setAttribute('data-y', yPoint_30635);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30635
         * @param {type} xPoint_30635
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30635, xPoint_30635, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30635 + '_y' + yPoint_30635;
            }
            return 'pc_x' + xPoint_30635 + '_y' + yPoint_30635;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30635=0;yPoint_30635<this.gameFieldBorderY.length; yPoint_30635++){
                for(var xPoint_30635=0;xPoint_30635<this.gameFieldBorderX.length; xPoint_30635++){
                    map.push({y: yPoint_30635, x: xPoint_30635});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30635=-1;yPoint_30635<(this.gameFieldBorderY.length+1); yPoint_30635++){
                for(var xPoint_30635=-1;xPoint_30635<(this.gameFieldBorderX.length+1); xPoint_30635++){
                    if(!map[yPoint_30635]){
                        map[yPoint_30635] = [];
                    }
                    map[yPoint_30635][xPoint_30635] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30635 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30635 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30635, yPoint_30635) === true){
                    if(this.canPutHorizontal(map, xPoint_30635, yPoint_30635, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30635][xPoint_30635 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30635, yPoint_30635, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30635 + i][xPoint_30635] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30635, yPoint_30635){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30635][xPoint_30635] === this.CELL_EMPTY
                && map[yPoint_30635-1][xPoint_30635] === this.CELL_EMPTY
                && map[yPoint_30635-1][xPoint_30635+1] === this.CELL_EMPTY
                && map[yPoint_30635][xPoint_30635+1] === this.CELL_EMPTY
                && map[yPoint_30635+1][xPoint_30635+1] === this.CELL_EMPTY
                && map[yPoint_30635+1][xPoint_30635] === this.CELL_EMPTY
                && map[yPoint_30635+1][xPoint_30635-1] === this.CELL_EMPTY
                && map[yPoint_30635][xPoint_30635-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30635, yPoint_30635, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30635;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30635][x] === this.CELL_EMPTY
                    && map[yPoint_30635-1][x] === this.CELL_EMPTY
                    && map[yPoint_30635-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30635][x+1] === this.CELL_EMPTY
                    && map[yPoint_30635+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30635+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30635, yPoint_30635, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30635;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30635] === this.CELL_EMPTY
                    && map[y+1][xPoint_30635] === this.CELL_EMPTY
                    && map[y+1][xPoint_30635+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30635] === this.CELL_EMPTY
                    && map[y][xPoint_30635-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30635-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
