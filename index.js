...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_315=0;yPoint_315<this.gameFieldBorderY.length; yPoint_315++){
                for(var xPoint_315=0;xPoint_315<this.gameFieldBorderX.length; xPoint_315++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_315, xPoint_315);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_315][xPoint_315] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_315, xPoint_315, 'user');
                    if(this._userShipsMap[yPoint_315][xPoint_315] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_315, xPoint_315, type){
            var id = this.getPointBlockIdByCoords(yPoint_315, xPoint_315, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_315);
                block.setAttribute('data-y', yPoint_315);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_315
         * @param {type} xPoint_315
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_315, xPoint_315, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_315 + '_y' + yPoint_315;
            }
            return 'pc_x' + xPoint_315 + '_y' + yPoint_315;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_315=0;yPoint_315<this.gameFieldBorderY.length; yPoint_315++){
                for(var xPoint_315=0;xPoint_315<this.gameFieldBorderX.length; xPoint_315++){
                    map.push({y: yPoint_315, x: xPoint_315});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_315=-1;yPoint_315<(this.gameFieldBorderY.length+1); yPoint_315++){
                for(var xPoint_315=-1;xPoint_315<(this.gameFieldBorderX.length+1); xPoint_315++){
                    if(!map[yPoint_315]){
                        map[yPoint_315] = [];
                    }
                    map[yPoint_315][xPoint_315] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_315 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_315 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_315, yPoint_315) === true){
                    if(this.canPutHorizontal(map, xPoint_315, yPoint_315, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_315][xPoint_315 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_315, yPoint_315, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_315 + i][xPoint_315] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_315, yPoint_315){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_315][xPoint_315] === this.CELL_EMPTY
                && map[yPoint_315-1][xPoint_315] === this.CELL_EMPTY
                && map[yPoint_315-1][xPoint_315+1] === this.CELL_EMPTY
                && map[yPoint_315][xPoint_315+1] === this.CELL_EMPTY
                && map[yPoint_315+1][xPoint_315+1] === this.CELL_EMPTY
                && map[yPoint_315+1][xPoint_315] === this.CELL_EMPTY
                && map[yPoint_315+1][xPoint_315-1] === this.CELL_EMPTY
                && map[yPoint_315][xPoint_315-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_315, yPoint_315, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_315;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_315][x] === this.CELL_EMPTY
                    && map[yPoint_315-1][x] === this.CELL_EMPTY
                    && map[yPoint_315-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_315][x+1] === this.CELL_EMPTY
                    && map[yPoint_315+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_315+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_315, yPoint_315, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_315;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_315] === this.CELL_EMPTY
                    && map[y+1][xPoint_315] === this.CELL_EMPTY
                    && map[y+1][xPoint_315+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_315] === this.CELL_EMPTY
                    && map[y][xPoint_315-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_315-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
