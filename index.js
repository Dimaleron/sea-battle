...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32647=0;yPoint_32647<this.gameFieldBorderY.length; yPoint_32647++){
                for(var xPoint_32647=0;xPoint_32647<this.gameFieldBorderX.length; xPoint_32647++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32647, xPoint_32647);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32647][xPoint_32647] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32647, xPoint_32647, 'user');
                    if(this._userShipsMap[yPoint_32647][xPoint_32647] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32647, xPoint_32647, type){
            var id = this.getPointBlockIdByCoords(yPoint_32647, xPoint_32647, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32647);
                block.setAttribute('data-y', yPoint_32647);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32647
         * @param {type} xPoint_32647
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32647, xPoint_32647, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32647 + '_y' + yPoint_32647;
            }
            return 'pc_x' + xPoint_32647 + '_y' + yPoint_32647;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32647=0;yPoint_32647<this.gameFieldBorderY.length; yPoint_32647++){
                for(var xPoint_32647=0;xPoint_32647<this.gameFieldBorderX.length; xPoint_32647++){
                    map.push({y: yPoint_32647, x: xPoint_32647});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32647=-1;yPoint_32647<(this.gameFieldBorderY.length+1); yPoint_32647++){
                for(var xPoint_32647=-1;xPoint_32647<(this.gameFieldBorderX.length+1); xPoint_32647++){
                    if(!map[yPoint_32647]){
                        map[yPoint_32647] = [];
                    }
                    map[yPoint_32647][xPoint_32647] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32647 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32647 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32647, yPoint_32647) === true){
                    if(this.canPutHorizontal(map, xPoint_32647, yPoint_32647, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32647][xPoint_32647 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32647, yPoint_32647, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32647 + i][xPoint_32647] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32647, yPoint_32647){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32647][xPoint_32647] === this.CELL_EMPTY
                && map[yPoint_32647-1][xPoint_32647] === this.CELL_EMPTY
                && map[yPoint_32647-1][xPoint_32647+1] === this.CELL_EMPTY
                && map[yPoint_32647][xPoint_32647+1] === this.CELL_EMPTY
                && map[yPoint_32647+1][xPoint_32647+1] === this.CELL_EMPTY
                && map[yPoint_32647+1][xPoint_32647] === this.CELL_EMPTY
                && map[yPoint_32647+1][xPoint_32647-1] === this.CELL_EMPTY
                && map[yPoint_32647][xPoint_32647-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32647, yPoint_32647, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32647;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32647][x] === this.CELL_EMPTY
                    && map[yPoint_32647-1][x] === this.CELL_EMPTY
                    && map[yPoint_32647-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32647][x+1] === this.CELL_EMPTY
                    && map[yPoint_32647+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32647+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32647, yPoint_32647, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32647;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32647] === this.CELL_EMPTY
                    && map[y+1][xPoint_32647] === this.CELL_EMPTY
                    && map[y+1][xPoint_32647+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32647] === this.CELL_EMPTY
                    && map[y][xPoint_32647-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32647-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
