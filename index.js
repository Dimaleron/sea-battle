...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16526=0;yPoint_16526<this.gameFieldBorderY.length; yPoint_16526++){
                for(var xPoint_16526=0;xPoint_16526<this.gameFieldBorderX.length; xPoint_16526++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16526, xPoint_16526);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16526][xPoint_16526] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16526, xPoint_16526, 'user');
                    if(this._userShipsMap[yPoint_16526][xPoint_16526] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16526, xPoint_16526, type){
            var id = this.getPointBlockIdByCoords(yPoint_16526, xPoint_16526, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16526);
                block.setAttribute('data-y', yPoint_16526);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16526
         * @param {type} xPoint_16526
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16526, xPoint_16526, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16526 + '_y' + yPoint_16526;
            }
            return 'pc_x' + xPoint_16526 + '_y' + yPoint_16526;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16526=0;yPoint_16526<this.gameFieldBorderY.length; yPoint_16526++){
                for(var xPoint_16526=0;xPoint_16526<this.gameFieldBorderX.length; xPoint_16526++){
                    map.push({y: yPoint_16526, x: xPoint_16526});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16526=-1;yPoint_16526<(this.gameFieldBorderY.length+1); yPoint_16526++){
                for(var xPoint_16526=-1;xPoint_16526<(this.gameFieldBorderX.length+1); xPoint_16526++){
                    if(!map[yPoint_16526]){
                        map[yPoint_16526] = [];
                    }
                    map[yPoint_16526][xPoint_16526] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16526 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16526 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16526, yPoint_16526) === true){
                    if(this.canPutHorizontal(map, xPoint_16526, yPoint_16526, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16526][xPoint_16526 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16526, yPoint_16526, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16526 + i][xPoint_16526] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16526, yPoint_16526){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16526][xPoint_16526] === this.CELL_EMPTY
                && map[yPoint_16526-1][xPoint_16526] === this.CELL_EMPTY
                && map[yPoint_16526-1][xPoint_16526+1] === this.CELL_EMPTY
                && map[yPoint_16526][xPoint_16526+1] === this.CELL_EMPTY
                && map[yPoint_16526+1][xPoint_16526+1] === this.CELL_EMPTY
                && map[yPoint_16526+1][xPoint_16526] === this.CELL_EMPTY
                && map[yPoint_16526+1][xPoint_16526-1] === this.CELL_EMPTY
                && map[yPoint_16526][xPoint_16526-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16526, yPoint_16526, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16526;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16526][x] === this.CELL_EMPTY
                    && map[yPoint_16526-1][x] === this.CELL_EMPTY
                    && map[yPoint_16526-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16526][x+1] === this.CELL_EMPTY
                    && map[yPoint_16526+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16526+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16526, yPoint_16526, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16526;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16526] === this.CELL_EMPTY
                    && map[y+1][xPoint_16526] === this.CELL_EMPTY
                    && map[y+1][xPoint_16526+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16526] === this.CELL_EMPTY
                    && map[y][xPoint_16526-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16526-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
