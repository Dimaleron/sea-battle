...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8574=0;yPoint_8574<this.gameFieldBorderY.length; yPoint_8574++){
                for(var xPoint_8574=0;xPoint_8574<this.gameFieldBorderX.length; xPoint_8574++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8574, xPoint_8574);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8574][xPoint_8574] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8574, xPoint_8574, 'user');
                    if(this._userShipsMap[yPoint_8574][xPoint_8574] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8574, xPoint_8574, type){
            var id = this.getPointBlockIdByCoords(yPoint_8574, xPoint_8574, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8574);
                block.setAttribute('data-y', yPoint_8574);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8574
         * @param {type} xPoint_8574
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8574, xPoint_8574, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8574 + '_y' + yPoint_8574;
            }
            return 'pc_x' + xPoint_8574 + '_y' + yPoint_8574;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8574=0;yPoint_8574<this.gameFieldBorderY.length; yPoint_8574++){
                for(var xPoint_8574=0;xPoint_8574<this.gameFieldBorderX.length; xPoint_8574++){
                    map.push({y: yPoint_8574, x: xPoint_8574});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8574=-1;yPoint_8574<(this.gameFieldBorderY.length+1); yPoint_8574++){
                for(var xPoint_8574=-1;xPoint_8574<(this.gameFieldBorderX.length+1); xPoint_8574++){
                    if(!map[yPoint_8574]){
                        map[yPoint_8574] = [];
                    }
                    map[yPoint_8574][xPoint_8574] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8574 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8574 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8574, yPoint_8574) === true){
                    if(this.canPutHorizontal(map, xPoint_8574, yPoint_8574, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8574][xPoint_8574 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8574, yPoint_8574, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8574 + i][xPoint_8574] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8574, yPoint_8574){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8574][xPoint_8574] === this.CELL_EMPTY
                && map[yPoint_8574-1][xPoint_8574] === this.CELL_EMPTY
                && map[yPoint_8574-1][xPoint_8574+1] === this.CELL_EMPTY
                && map[yPoint_8574][xPoint_8574+1] === this.CELL_EMPTY
                && map[yPoint_8574+1][xPoint_8574+1] === this.CELL_EMPTY
                && map[yPoint_8574+1][xPoint_8574] === this.CELL_EMPTY
                && map[yPoint_8574+1][xPoint_8574-1] === this.CELL_EMPTY
                && map[yPoint_8574][xPoint_8574-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8574, yPoint_8574, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8574;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8574][x] === this.CELL_EMPTY
                    && map[yPoint_8574-1][x] === this.CELL_EMPTY
                    && map[yPoint_8574-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8574][x+1] === this.CELL_EMPTY
                    && map[yPoint_8574+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8574+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8574, yPoint_8574, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8574;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8574] === this.CELL_EMPTY
                    && map[y+1][xPoint_8574] === this.CELL_EMPTY
                    && map[y+1][xPoint_8574+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8574] === this.CELL_EMPTY
                    && map[y][xPoint_8574-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8574-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
