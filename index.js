...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8894=0;yPoint_8894<this.gameFieldBorderY.length; yPoint_8894++){
                for(var xPoint_8894=0;xPoint_8894<this.gameFieldBorderX.length; xPoint_8894++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8894, xPoint_8894);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8894][xPoint_8894] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8894, xPoint_8894, 'user');
                    if(this._userShipsMap[yPoint_8894][xPoint_8894] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8894, xPoint_8894, type){
            var id = this.getPointBlockIdByCoords(yPoint_8894, xPoint_8894, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8894);
                block.setAttribute('data-y', yPoint_8894);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8894
         * @param {type} xPoint_8894
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8894, xPoint_8894, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8894 + '_y' + yPoint_8894;
            }
            return 'pc_x' + xPoint_8894 + '_y' + yPoint_8894;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8894=0;yPoint_8894<this.gameFieldBorderY.length; yPoint_8894++){
                for(var xPoint_8894=0;xPoint_8894<this.gameFieldBorderX.length; xPoint_8894++){
                    map.push({y: yPoint_8894, x: xPoint_8894});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8894=-1;yPoint_8894<(this.gameFieldBorderY.length+1); yPoint_8894++){
                for(var xPoint_8894=-1;xPoint_8894<(this.gameFieldBorderX.length+1); xPoint_8894++){
                    if(!map[yPoint_8894]){
                        map[yPoint_8894] = [];
                    }
                    map[yPoint_8894][xPoint_8894] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8894 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8894 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8894, yPoint_8894) === true){
                    if(this.canPutHorizontal(map, xPoint_8894, yPoint_8894, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8894][xPoint_8894 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8894, yPoint_8894, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8894 + i][xPoint_8894] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8894, yPoint_8894){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8894][xPoint_8894] === this.CELL_EMPTY
                && map[yPoint_8894-1][xPoint_8894] === this.CELL_EMPTY
                && map[yPoint_8894-1][xPoint_8894+1] === this.CELL_EMPTY
                && map[yPoint_8894][xPoint_8894+1] === this.CELL_EMPTY
                && map[yPoint_8894+1][xPoint_8894+1] === this.CELL_EMPTY
                && map[yPoint_8894+1][xPoint_8894] === this.CELL_EMPTY
                && map[yPoint_8894+1][xPoint_8894-1] === this.CELL_EMPTY
                && map[yPoint_8894][xPoint_8894-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8894, yPoint_8894, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8894;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8894][x] === this.CELL_EMPTY
                    && map[yPoint_8894-1][x] === this.CELL_EMPTY
                    && map[yPoint_8894-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8894][x+1] === this.CELL_EMPTY
                    && map[yPoint_8894+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8894+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8894, yPoint_8894, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8894;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8894] === this.CELL_EMPTY
                    && map[y+1][xPoint_8894] === this.CELL_EMPTY
                    && map[y+1][xPoint_8894+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8894] === this.CELL_EMPTY
                    && map[y][xPoint_8894-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8894-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
