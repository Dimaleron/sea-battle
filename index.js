...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_735=0;yPoint_735<this.gameFieldBorderY.length; yPoint_735++){
                for(var xPoint_735=0;xPoint_735<this.gameFieldBorderX.length; xPoint_735++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_735, xPoint_735);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_735][xPoint_735] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_735, xPoint_735, 'user');
                    if(this._userShipsMap[yPoint_735][xPoint_735] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_735, xPoint_735, type){
            var id = this.getPointBlockIdByCoords(yPoint_735, xPoint_735, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_735);
                block.setAttribute('data-y', yPoint_735);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_735
         * @param {type} xPoint_735
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_735, xPoint_735, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_735 + '_y' + yPoint_735;
            }
            return 'pc_x' + xPoint_735 + '_y' + yPoint_735;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_735=0;yPoint_735<this.gameFieldBorderY.length; yPoint_735++){
                for(var xPoint_735=0;xPoint_735<this.gameFieldBorderX.length; xPoint_735++){
                    map.push({y: yPoint_735, x: xPoint_735});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_735=-1;yPoint_735<(this.gameFieldBorderY.length+1); yPoint_735++){
                for(var xPoint_735=-1;xPoint_735<(this.gameFieldBorderX.length+1); xPoint_735++){
                    if(!map[yPoint_735]){
                        map[yPoint_735] = [];
                    }
                    map[yPoint_735][xPoint_735] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_735 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_735 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_735, yPoint_735) === true){
                    if(this.canPutHorizontal(map, xPoint_735, yPoint_735, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_735][xPoint_735 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_735, yPoint_735, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_735 + i][xPoint_735] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_735, yPoint_735){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_735][xPoint_735] === this.CELL_EMPTY
                && map[yPoint_735-1][xPoint_735] === this.CELL_EMPTY
                && map[yPoint_735-1][xPoint_735+1] === this.CELL_EMPTY
                && map[yPoint_735][xPoint_735+1] === this.CELL_EMPTY
                && map[yPoint_735+1][xPoint_735+1] === this.CELL_EMPTY
                && map[yPoint_735+1][xPoint_735] === this.CELL_EMPTY
                && map[yPoint_735+1][xPoint_735-1] === this.CELL_EMPTY
                && map[yPoint_735][xPoint_735-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_735, yPoint_735, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_735;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_735][x] === this.CELL_EMPTY
                    && map[yPoint_735-1][x] === this.CELL_EMPTY
                    && map[yPoint_735-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_735][x+1] === this.CELL_EMPTY
                    && map[yPoint_735+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_735+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_735, yPoint_735, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_735;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_735] === this.CELL_EMPTY
                    && map[y+1][xPoint_735] === this.CELL_EMPTY
                    && map[y+1][xPoint_735+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_735] === this.CELL_EMPTY
                    && map[y][xPoint_735-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_735-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
