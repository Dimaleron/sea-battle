...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10736=0;yPoint_10736<this.gameFieldBorderY.length; yPoint_10736++){
                for(var xPoint_10736=0;xPoint_10736<this.gameFieldBorderX.length; xPoint_10736++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10736, xPoint_10736);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10736][xPoint_10736] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10736, xPoint_10736, 'user');
                    if(this._userShipsMap[yPoint_10736][xPoint_10736] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10736, xPoint_10736, type){
            var id = this.getPointBlockIdByCoords(yPoint_10736, xPoint_10736, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10736);
                block.setAttribute('data-y', yPoint_10736);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10736
         * @param {type} xPoint_10736
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10736, xPoint_10736, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10736 + '_y' + yPoint_10736;
            }
            return 'pc_x' + xPoint_10736 + '_y' + yPoint_10736;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10736=0;yPoint_10736<this.gameFieldBorderY.length; yPoint_10736++){
                for(var xPoint_10736=0;xPoint_10736<this.gameFieldBorderX.length; xPoint_10736++){
                    map.push({y: yPoint_10736, x: xPoint_10736});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10736=-1;yPoint_10736<(this.gameFieldBorderY.length+1); yPoint_10736++){
                for(var xPoint_10736=-1;xPoint_10736<(this.gameFieldBorderX.length+1); xPoint_10736++){
                    if(!map[yPoint_10736]){
                        map[yPoint_10736] = [];
                    }
                    map[yPoint_10736][xPoint_10736] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10736 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10736 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10736, yPoint_10736) === true){
                    if(this.canPutHorizontal(map, xPoint_10736, yPoint_10736, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10736][xPoint_10736 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10736, yPoint_10736, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10736 + i][xPoint_10736] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10736, yPoint_10736){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10736][xPoint_10736] === this.CELL_EMPTY
                && map[yPoint_10736-1][xPoint_10736] === this.CELL_EMPTY
                && map[yPoint_10736-1][xPoint_10736+1] === this.CELL_EMPTY
                && map[yPoint_10736][xPoint_10736+1] === this.CELL_EMPTY
                && map[yPoint_10736+1][xPoint_10736+1] === this.CELL_EMPTY
                && map[yPoint_10736+1][xPoint_10736] === this.CELL_EMPTY
                && map[yPoint_10736+1][xPoint_10736-1] === this.CELL_EMPTY
                && map[yPoint_10736][xPoint_10736-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10736, yPoint_10736, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10736;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10736][x] === this.CELL_EMPTY
                    && map[yPoint_10736-1][x] === this.CELL_EMPTY
                    && map[yPoint_10736-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10736][x+1] === this.CELL_EMPTY
                    && map[yPoint_10736+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10736+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10736, yPoint_10736, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10736;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10736] === this.CELL_EMPTY
                    && map[y+1][xPoint_10736] === this.CELL_EMPTY
                    && map[y+1][xPoint_10736+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10736] === this.CELL_EMPTY
                    && map[y][xPoint_10736-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10736-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
