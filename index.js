...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_883=0;yPoint_883<this.gameFieldBorderY.length; yPoint_883++){
                for(var xPoint_883=0;xPoint_883<this.gameFieldBorderX.length; xPoint_883++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_883, xPoint_883);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_883][xPoint_883] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_883, xPoint_883, 'user');
                    if(this._userShipsMap[yPoint_883][xPoint_883] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_883, xPoint_883, type){
            var id = this.getPointBlockIdByCoords(yPoint_883, xPoint_883, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_883);
                block.setAttribute('data-y', yPoint_883);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_883
         * @param {type} xPoint_883
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_883, xPoint_883, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_883 + '_y' + yPoint_883;
            }
            return 'pc_x' + xPoint_883 + '_y' + yPoint_883;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_883=0;yPoint_883<this.gameFieldBorderY.length; yPoint_883++){
                for(var xPoint_883=0;xPoint_883<this.gameFieldBorderX.length; xPoint_883++){
                    map.push({y: yPoint_883, x: xPoint_883});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_883=-1;yPoint_883<(this.gameFieldBorderY.length+1); yPoint_883++){
                for(var xPoint_883=-1;xPoint_883<(this.gameFieldBorderX.length+1); xPoint_883++){
                    if(!map[yPoint_883]){
                        map[yPoint_883] = [];
                    }
                    map[yPoint_883][xPoint_883] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_883 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_883 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_883, yPoint_883) === true){
                    if(this.canPutHorizontal(map, xPoint_883, yPoint_883, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_883][xPoint_883 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_883, yPoint_883, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_883 + i][xPoint_883] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_883, yPoint_883){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_883][xPoint_883] === this.CELL_EMPTY
                && map[yPoint_883-1][xPoint_883] === this.CELL_EMPTY
                && map[yPoint_883-1][xPoint_883+1] === this.CELL_EMPTY
                && map[yPoint_883][xPoint_883+1] === this.CELL_EMPTY
                && map[yPoint_883+1][xPoint_883+1] === this.CELL_EMPTY
                && map[yPoint_883+1][xPoint_883] === this.CELL_EMPTY
                && map[yPoint_883+1][xPoint_883-1] === this.CELL_EMPTY
                && map[yPoint_883][xPoint_883-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_883, yPoint_883, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_883;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_883][x] === this.CELL_EMPTY
                    && map[yPoint_883-1][x] === this.CELL_EMPTY
                    && map[yPoint_883-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_883][x+1] === this.CELL_EMPTY
                    && map[yPoint_883+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_883+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_883, yPoint_883, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_883;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_883] === this.CELL_EMPTY
                    && map[y+1][xPoint_883] === this.CELL_EMPTY
                    && map[y+1][xPoint_883+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_883] === this.CELL_EMPTY
                    && map[y][xPoint_883-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_883-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
