...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_861=0;yPoint_861<this.gameFieldBorderY.length; yPoint_861++){
                for(var xPoint_861=0;xPoint_861<this.gameFieldBorderX.length; xPoint_861++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_861, xPoint_861);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_861][xPoint_861] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_861, xPoint_861, 'user');
                    if(this._userShipsMap[yPoint_861][xPoint_861] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_861, xPoint_861, type){
            var id = this.getPointBlockIdByCoords(yPoint_861, xPoint_861, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_861);
                block.setAttribute('data-y', yPoint_861);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_861
         * @param {type} xPoint_861
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_861, xPoint_861, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_861 + '_y' + yPoint_861;
            }
            return 'pc_x' + xPoint_861 + '_y' + yPoint_861;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_861=0;yPoint_861<this.gameFieldBorderY.length; yPoint_861++){
                for(var xPoint_861=0;xPoint_861<this.gameFieldBorderX.length; xPoint_861++){
                    map.push({y: yPoint_861, x: xPoint_861});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_861=-1;yPoint_861<(this.gameFieldBorderY.length+1); yPoint_861++){
                for(var xPoint_861=-1;xPoint_861<(this.gameFieldBorderX.length+1); xPoint_861++){
                    if(!map[yPoint_861]){
                        map[yPoint_861] = [];
                    }
                    map[yPoint_861][xPoint_861] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_861 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_861 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_861, yPoint_861) === true){
                    if(this.canPutHorizontal(map, xPoint_861, yPoint_861, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_861][xPoint_861 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_861, yPoint_861, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_861 + i][xPoint_861] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_861, yPoint_861){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_861][xPoint_861] === this.CELL_EMPTY
                && map[yPoint_861-1][xPoint_861] === this.CELL_EMPTY
                && map[yPoint_861-1][xPoint_861+1] === this.CELL_EMPTY
                && map[yPoint_861][xPoint_861+1] === this.CELL_EMPTY
                && map[yPoint_861+1][xPoint_861+1] === this.CELL_EMPTY
                && map[yPoint_861+1][xPoint_861] === this.CELL_EMPTY
                && map[yPoint_861+1][xPoint_861-1] === this.CELL_EMPTY
                && map[yPoint_861][xPoint_861-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_861, yPoint_861, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_861;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_861][x] === this.CELL_EMPTY
                    && map[yPoint_861-1][x] === this.CELL_EMPTY
                    && map[yPoint_861-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_861][x+1] === this.CELL_EMPTY
                    && map[yPoint_861+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_861+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_861, yPoint_861, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_861;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_861] === this.CELL_EMPTY
                    && map[y+1][xPoint_861] === this.CELL_EMPTY
                    && map[y+1][xPoint_861+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_861] === this.CELL_EMPTY
                    && map[y][xPoint_861-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_861-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
