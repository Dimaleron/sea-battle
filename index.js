...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20740=0;yPoint_20740<this.gameFieldBorderY.length; yPoint_20740++){
                for(var xPoint_20740=0;xPoint_20740<this.gameFieldBorderX.length; xPoint_20740++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20740, xPoint_20740);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20740][xPoint_20740] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20740, xPoint_20740, 'user');
                    if(this._userShipsMap[yPoint_20740][xPoint_20740] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20740, xPoint_20740, type){
            var id = this.getPointBlockIdByCoords(yPoint_20740, xPoint_20740, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20740);
                block.setAttribute('data-y', yPoint_20740);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20740
         * @param {type} xPoint_20740
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20740, xPoint_20740, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20740 + '_y' + yPoint_20740;
            }
            return 'pc_x' + xPoint_20740 + '_y' + yPoint_20740;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20740=0;yPoint_20740<this.gameFieldBorderY.length; yPoint_20740++){
                for(var xPoint_20740=0;xPoint_20740<this.gameFieldBorderX.length; xPoint_20740++){
                    map.push({y: yPoint_20740, x: xPoint_20740});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20740=-1;yPoint_20740<(this.gameFieldBorderY.length+1); yPoint_20740++){
                for(var xPoint_20740=-1;xPoint_20740<(this.gameFieldBorderX.length+1); xPoint_20740++){
                    if(!map[yPoint_20740]){
                        map[yPoint_20740] = [];
                    }
                    map[yPoint_20740][xPoint_20740] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20740 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20740 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20740, yPoint_20740) === true){
                    if(this.canPutHorizontal(map, xPoint_20740, yPoint_20740, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20740][xPoint_20740 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20740, yPoint_20740, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20740 + i][xPoint_20740] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20740, yPoint_20740){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20740][xPoint_20740] === this.CELL_EMPTY
                && map[yPoint_20740-1][xPoint_20740] === this.CELL_EMPTY
                && map[yPoint_20740-1][xPoint_20740+1] === this.CELL_EMPTY
                && map[yPoint_20740][xPoint_20740+1] === this.CELL_EMPTY
                && map[yPoint_20740+1][xPoint_20740+1] === this.CELL_EMPTY
                && map[yPoint_20740+1][xPoint_20740] === this.CELL_EMPTY
                && map[yPoint_20740+1][xPoint_20740-1] === this.CELL_EMPTY
                && map[yPoint_20740][xPoint_20740-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20740, yPoint_20740, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20740;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20740][x] === this.CELL_EMPTY
                    && map[yPoint_20740-1][x] === this.CELL_EMPTY
                    && map[yPoint_20740-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20740][x+1] === this.CELL_EMPTY
                    && map[yPoint_20740+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20740+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20740, yPoint_20740, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20740;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20740] === this.CELL_EMPTY
                    && map[y+1][xPoint_20740] === this.CELL_EMPTY
                    && map[y+1][xPoint_20740+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20740] === this.CELL_EMPTY
                    && map[y][xPoint_20740-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20740-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
