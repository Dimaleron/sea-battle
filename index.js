...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17325=0;yPoint_17325<this.gameFieldBorderY.length; yPoint_17325++){
                for(var xPoint_17325=0;xPoint_17325<this.gameFieldBorderX.length; xPoint_17325++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17325, xPoint_17325);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17325][xPoint_17325] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17325, xPoint_17325, 'user');
                    if(this._userShipsMap[yPoint_17325][xPoint_17325] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17325, xPoint_17325, type){
            var id = this.getPointBlockIdByCoords(yPoint_17325, xPoint_17325, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17325);
                block.setAttribute('data-y', yPoint_17325);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17325
         * @param {type} xPoint_17325
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17325, xPoint_17325, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17325 + '_y' + yPoint_17325;
            }
            return 'pc_x' + xPoint_17325 + '_y' + yPoint_17325;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17325=0;yPoint_17325<this.gameFieldBorderY.length; yPoint_17325++){
                for(var xPoint_17325=0;xPoint_17325<this.gameFieldBorderX.length; xPoint_17325++){
                    map.push({y: yPoint_17325, x: xPoint_17325});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17325=-1;yPoint_17325<(this.gameFieldBorderY.length+1); yPoint_17325++){
                for(var xPoint_17325=-1;xPoint_17325<(this.gameFieldBorderX.length+1); xPoint_17325++){
                    if(!map[yPoint_17325]){
                        map[yPoint_17325] = [];
                    }
                    map[yPoint_17325][xPoint_17325] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17325 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17325 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17325, yPoint_17325) === true){
                    if(this.canPutHorizontal(map, xPoint_17325, yPoint_17325, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17325][xPoint_17325 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17325, yPoint_17325, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17325 + i][xPoint_17325] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17325, yPoint_17325){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17325][xPoint_17325] === this.CELL_EMPTY
                && map[yPoint_17325-1][xPoint_17325] === this.CELL_EMPTY
                && map[yPoint_17325-1][xPoint_17325+1] === this.CELL_EMPTY
                && map[yPoint_17325][xPoint_17325+1] === this.CELL_EMPTY
                && map[yPoint_17325+1][xPoint_17325+1] === this.CELL_EMPTY
                && map[yPoint_17325+1][xPoint_17325] === this.CELL_EMPTY
                && map[yPoint_17325+1][xPoint_17325-1] === this.CELL_EMPTY
                && map[yPoint_17325][xPoint_17325-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17325, yPoint_17325, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17325;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17325][x] === this.CELL_EMPTY
                    && map[yPoint_17325-1][x] === this.CELL_EMPTY
                    && map[yPoint_17325-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17325][x+1] === this.CELL_EMPTY
                    && map[yPoint_17325+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17325+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17325, yPoint_17325, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17325;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17325] === this.CELL_EMPTY
                    && map[y+1][xPoint_17325] === this.CELL_EMPTY
                    && map[y+1][xPoint_17325+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17325] === this.CELL_EMPTY
                    && map[y][xPoint_17325-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17325-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
