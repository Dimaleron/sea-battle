...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30477=0;yPoint_30477<this.gameFieldBorderY.length; yPoint_30477++){
                for(var xPoint_30477=0;xPoint_30477<this.gameFieldBorderX.length; xPoint_30477++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30477, xPoint_30477);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30477][xPoint_30477] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30477, xPoint_30477, 'user');
                    if(this._userShipsMap[yPoint_30477][xPoint_30477] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30477, xPoint_30477, type){
            var id = this.getPointBlockIdByCoords(yPoint_30477, xPoint_30477, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30477);
                block.setAttribute('data-y', yPoint_30477);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30477
         * @param {type} xPoint_30477
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30477, xPoint_30477, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30477 + '_y' + yPoint_30477;
            }
            return 'pc_x' + xPoint_30477 + '_y' + yPoint_30477;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30477=0;yPoint_30477<this.gameFieldBorderY.length; yPoint_30477++){
                for(var xPoint_30477=0;xPoint_30477<this.gameFieldBorderX.length; xPoint_30477++){
                    map.push({y: yPoint_30477, x: xPoint_30477});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30477=-1;yPoint_30477<(this.gameFieldBorderY.length+1); yPoint_30477++){
                for(var xPoint_30477=-1;xPoint_30477<(this.gameFieldBorderX.length+1); xPoint_30477++){
                    if(!map[yPoint_30477]){
                        map[yPoint_30477] = [];
                    }
                    map[yPoint_30477][xPoint_30477] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30477 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30477 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30477, yPoint_30477) === true){
                    if(this.canPutHorizontal(map, xPoint_30477, yPoint_30477, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30477][xPoint_30477 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30477, yPoint_30477, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30477 + i][xPoint_30477] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30477, yPoint_30477){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30477][xPoint_30477] === this.CELL_EMPTY
                && map[yPoint_30477-1][xPoint_30477] === this.CELL_EMPTY
                && map[yPoint_30477-1][xPoint_30477+1] === this.CELL_EMPTY
                && map[yPoint_30477][xPoint_30477+1] === this.CELL_EMPTY
                && map[yPoint_30477+1][xPoint_30477+1] === this.CELL_EMPTY
                && map[yPoint_30477+1][xPoint_30477] === this.CELL_EMPTY
                && map[yPoint_30477+1][xPoint_30477-1] === this.CELL_EMPTY
                && map[yPoint_30477][xPoint_30477-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30477, yPoint_30477, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30477;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30477][x] === this.CELL_EMPTY
                    && map[yPoint_30477-1][x] === this.CELL_EMPTY
                    && map[yPoint_30477-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30477][x+1] === this.CELL_EMPTY
                    && map[yPoint_30477+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30477+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30477, yPoint_30477, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30477;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30477] === this.CELL_EMPTY
                    && map[y+1][xPoint_30477] === this.CELL_EMPTY
                    && map[y+1][xPoint_30477+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30477] === this.CELL_EMPTY
                    && map[y][xPoint_30477-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30477-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
