...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11422=0;yPoint_11422<this.gameFieldBorderY.length; yPoint_11422++){
                for(var xPoint_11422=0;xPoint_11422<this.gameFieldBorderX.length; xPoint_11422++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11422, xPoint_11422);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11422][xPoint_11422] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11422, xPoint_11422, 'user');
                    if(this._userShipsMap[yPoint_11422][xPoint_11422] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11422, xPoint_11422, type){
            var id = this.getPointBlockIdByCoords(yPoint_11422, xPoint_11422, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11422);
                block.setAttribute('data-y', yPoint_11422);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11422
         * @param {type} xPoint_11422
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11422, xPoint_11422, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11422 + '_y' + yPoint_11422;
            }
            return 'pc_x' + xPoint_11422 + '_y' + yPoint_11422;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11422=0;yPoint_11422<this.gameFieldBorderY.length; yPoint_11422++){
                for(var xPoint_11422=0;xPoint_11422<this.gameFieldBorderX.length; xPoint_11422++){
                    map.push({y: yPoint_11422, x: xPoint_11422});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11422=-1;yPoint_11422<(this.gameFieldBorderY.length+1); yPoint_11422++){
                for(var xPoint_11422=-1;xPoint_11422<(this.gameFieldBorderX.length+1); xPoint_11422++){
                    if(!map[yPoint_11422]){
                        map[yPoint_11422] = [];
                    }
                    map[yPoint_11422][xPoint_11422] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11422 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11422 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11422, yPoint_11422) === true){
                    if(this.canPutHorizontal(map, xPoint_11422, yPoint_11422, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11422][xPoint_11422 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11422, yPoint_11422, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11422 + i][xPoint_11422] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11422, yPoint_11422){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11422][xPoint_11422] === this.CELL_EMPTY
                && map[yPoint_11422-1][xPoint_11422] === this.CELL_EMPTY
                && map[yPoint_11422-1][xPoint_11422+1] === this.CELL_EMPTY
                && map[yPoint_11422][xPoint_11422+1] === this.CELL_EMPTY
                && map[yPoint_11422+1][xPoint_11422+1] === this.CELL_EMPTY
                && map[yPoint_11422+1][xPoint_11422] === this.CELL_EMPTY
                && map[yPoint_11422+1][xPoint_11422-1] === this.CELL_EMPTY
                && map[yPoint_11422][xPoint_11422-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11422, yPoint_11422, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11422;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11422][x] === this.CELL_EMPTY
                    && map[yPoint_11422-1][x] === this.CELL_EMPTY
                    && map[yPoint_11422-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11422][x+1] === this.CELL_EMPTY
                    && map[yPoint_11422+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11422+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11422, yPoint_11422, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11422;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11422] === this.CELL_EMPTY
                    && map[y+1][xPoint_11422] === this.CELL_EMPTY
                    && map[y+1][xPoint_11422+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11422] === this.CELL_EMPTY
                    && map[y][xPoint_11422-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11422-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
