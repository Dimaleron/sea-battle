...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_142=0;yPoint_142<this.gameFieldBorderY.length; yPoint_142++){
                for(var xPoint_142=0;xPoint_142<this.gameFieldBorderX.length; xPoint_142++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_142, xPoint_142);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_142][xPoint_142] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_142, xPoint_142, 'user');
                    if(this._userShipsMap[yPoint_142][xPoint_142] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_142, xPoint_142, type){
            var id = this.getPointBlockIdByCoords(yPoint_142, xPoint_142, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_142);
                block.setAttribute('data-y', yPoint_142);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_142
         * @param {type} xPoint_142
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_142, xPoint_142, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_142 + '_y' + yPoint_142;
            }
            return 'pc_x' + xPoint_142 + '_y' + yPoint_142;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_142=0;yPoint_142<this.gameFieldBorderY.length; yPoint_142++){
                for(var xPoint_142=0;xPoint_142<this.gameFieldBorderX.length; xPoint_142++){
                    map.push({y: yPoint_142, x: xPoint_142});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_142=-1;yPoint_142<(this.gameFieldBorderY.length+1); yPoint_142++){
                for(var xPoint_142=-1;xPoint_142<(this.gameFieldBorderX.length+1); xPoint_142++){
                    if(!map[yPoint_142]){
                        map[yPoint_142] = [];
                    }
                    map[yPoint_142][xPoint_142] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_142 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_142 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_142, yPoint_142) === true){
                    if(this.canPutHorizontal(map, xPoint_142, yPoint_142, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_142][xPoint_142 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_142, yPoint_142, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_142 + i][xPoint_142] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_142, yPoint_142){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_142][xPoint_142] === this.CELL_EMPTY
                && map[yPoint_142-1][xPoint_142] === this.CELL_EMPTY
                && map[yPoint_142-1][xPoint_142+1] === this.CELL_EMPTY
                && map[yPoint_142][xPoint_142+1] === this.CELL_EMPTY
                && map[yPoint_142+1][xPoint_142+1] === this.CELL_EMPTY
                && map[yPoint_142+1][xPoint_142] === this.CELL_EMPTY
                && map[yPoint_142+1][xPoint_142-1] === this.CELL_EMPTY
                && map[yPoint_142][xPoint_142-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_142, yPoint_142, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_142;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_142][x] === this.CELL_EMPTY
                    && map[yPoint_142-1][x] === this.CELL_EMPTY
                    && map[yPoint_142-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_142][x+1] === this.CELL_EMPTY
                    && map[yPoint_142+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_142+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_142, yPoint_142, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_142;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_142] === this.CELL_EMPTY
                    && map[y+1][xPoint_142] === this.CELL_EMPTY
                    && map[y+1][xPoint_142+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_142] === this.CELL_EMPTY
                    && map[y][xPoint_142-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_142-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
