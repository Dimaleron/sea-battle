...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_463=0;yPoint_463<this.gameFieldBorderY.length; yPoint_463++){
                for(var xPoint_463=0;xPoint_463<this.gameFieldBorderX.length; xPoint_463++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_463, xPoint_463);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_463][xPoint_463] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_463, xPoint_463, 'user');
                    if(this._userShipsMap[yPoint_463][xPoint_463] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_463, xPoint_463, type){
            var id = this.getPointBlockIdByCoords(yPoint_463, xPoint_463, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_463);
                block.setAttribute('data-y', yPoint_463);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_463
         * @param {type} xPoint_463
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_463, xPoint_463, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_463 + '_y' + yPoint_463;
            }
            return 'pc_x' + xPoint_463 + '_y' + yPoint_463;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_463=0;yPoint_463<this.gameFieldBorderY.length; yPoint_463++){
                for(var xPoint_463=0;xPoint_463<this.gameFieldBorderX.length; xPoint_463++){
                    map.push({y: yPoint_463, x: xPoint_463});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_463=-1;yPoint_463<(this.gameFieldBorderY.length+1); yPoint_463++){
                for(var xPoint_463=-1;xPoint_463<(this.gameFieldBorderX.length+1); xPoint_463++){
                    if(!map[yPoint_463]){
                        map[yPoint_463] = [];
                    }
                    map[yPoint_463][xPoint_463] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_463 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_463 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_463, yPoint_463) === true){
                    if(this.canPutHorizontal(map, xPoint_463, yPoint_463, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_463][xPoint_463 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_463, yPoint_463, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_463 + i][xPoint_463] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_463, yPoint_463){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_463][xPoint_463] === this.CELL_EMPTY
                && map[yPoint_463-1][xPoint_463] === this.CELL_EMPTY
                && map[yPoint_463-1][xPoint_463+1] === this.CELL_EMPTY
                && map[yPoint_463][xPoint_463+1] === this.CELL_EMPTY
                && map[yPoint_463+1][xPoint_463+1] === this.CELL_EMPTY
                && map[yPoint_463+1][xPoint_463] === this.CELL_EMPTY
                && map[yPoint_463+1][xPoint_463-1] === this.CELL_EMPTY
                && map[yPoint_463][xPoint_463-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_463, yPoint_463, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_463;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_463][x] === this.CELL_EMPTY
                    && map[yPoint_463-1][x] === this.CELL_EMPTY
                    && map[yPoint_463-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_463][x+1] === this.CELL_EMPTY
                    && map[yPoint_463+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_463+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_463, yPoint_463, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_463;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_463] === this.CELL_EMPTY
                    && map[y+1][xPoint_463] === this.CELL_EMPTY
                    && map[y+1][xPoint_463+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_463] === this.CELL_EMPTY
                    && map[y][xPoint_463-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_463-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
