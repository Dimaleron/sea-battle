...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23991=0;yPoint_23991<this.gameFieldBorderY.length; yPoint_23991++){
                for(var xPoint_23991=0;xPoint_23991<this.gameFieldBorderX.length; xPoint_23991++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23991, xPoint_23991);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23991][xPoint_23991] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23991, xPoint_23991, 'user');
                    if(this._userShipsMap[yPoint_23991][xPoint_23991] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23991, xPoint_23991, type){
            var id = this.getPointBlockIdByCoords(yPoint_23991, xPoint_23991, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23991);
                block.setAttribute('data-y', yPoint_23991);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23991
         * @param {type} xPoint_23991
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23991, xPoint_23991, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23991 + '_y' + yPoint_23991;
            }
            return 'pc_x' + xPoint_23991 + '_y' + yPoint_23991;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23991=0;yPoint_23991<this.gameFieldBorderY.length; yPoint_23991++){
                for(var xPoint_23991=0;xPoint_23991<this.gameFieldBorderX.length; xPoint_23991++){
                    map.push({y: yPoint_23991, x: xPoint_23991});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23991=-1;yPoint_23991<(this.gameFieldBorderY.length+1); yPoint_23991++){
                for(var xPoint_23991=-1;xPoint_23991<(this.gameFieldBorderX.length+1); xPoint_23991++){
                    if(!map[yPoint_23991]){
                        map[yPoint_23991] = [];
                    }
                    map[yPoint_23991][xPoint_23991] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23991 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23991 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23991, yPoint_23991) === true){
                    if(this.canPutHorizontal(map, xPoint_23991, yPoint_23991, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23991][xPoint_23991 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23991, yPoint_23991, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23991 + i][xPoint_23991] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23991, yPoint_23991){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23991][xPoint_23991] === this.CELL_EMPTY
                && map[yPoint_23991-1][xPoint_23991] === this.CELL_EMPTY
                && map[yPoint_23991-1][xPoint_23991+1] === this.CELL_EMPTY
                && map[yPoint_23991][xPoint_23991+1] === this.CELL_EMPTY
                && map[yPoint_23991+1][xPoint_23991+1] === this.CELL_EMPTY
                && map[yPoint_23991+1][xPoint_23991] === this.CELL_EMPTY
                && map[yPoint_23991+1][xPoint_23991-1] === this.CELL_EMPTY
                && map[yPoint_23991][xPoint_23991-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23991, yPoint_23991, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23991;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23991][x] === this.CELL_EMPTY
                    && map[yPoint_23991-1][x] === this.CELL_EMPTY
                    && map[yPoint_23991-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23991][x+1] === this.CELL_EMPTY
                    && map[yPoint_23991+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23991+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23991, yPoint_23991, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23991;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23991] === this.CELL_EMPTY
                    && map[y+1][xPoint_23991] === this.CELL_EMPTY
                    && map[y+1][xPoint_23991+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23991] === this.CELL_EMPTY
                    && map[y][xPoint_23991-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23991-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
