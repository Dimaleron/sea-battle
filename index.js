...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27091=0;yPoint_27091<this.gameFieldBorderY.length; yPoint_27091++){
                for(var xPoint_27091=0;xPoint_27091<this.gameFieldBorderX.length; xPoint_27091++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27091, xPoint_27091);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27091][xPoint_27091] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27091, xPoint_27091, 'user');
                    if(this._userShipsMap[yPoint_27091][xPoint_27091] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27091, xPoint_27091, type){
            var id = this.getPointBlockIdByCoords(yPoint_27091, xPoint_27091, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27091);
                block.setAttribute('data-y', yPoint_27091);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27091
         * @param {type} xPoint_27091
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27091, xPoint_27091, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27091 + '_y' + yPoint_27091;
            }
            return 'pc_x' + xPoint_27091 + '_y' + yPoint_27091;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27091=0;yPoint_27091<this.gameFieldBorderY.length; yPoint_27091++){
                for(var xPoint_27091=0;xPoint_27091<this.gameFieldBorderX.length; xPoint_27091++){
                    map.push({y: yPoint_27091, x: xPoint_27091});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27091=-1;yPoint_27091<(this.gameFieldBorderY.length+1); yPoint_27091++){
                for(var xPoint_27091=-1;xPoint_27091<(this.gameFieldBorderX.length+1); xPoint_27091++){
                    if(!map[yPoint_27091]){
                        map[yPoint_27091] = [];
                    }
                    map[yPoint_27091][xPoint_27091] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27091 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27091 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27091, yPoint_27091) === true){
                    if(this.canPutHorizontal(map, xPoint_27091, yPoint_27091, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27091][xPoint_27091 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27091, yPoint_27091, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27091 + i][xPoint_27091] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27091, yPoint_27091){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27091][xPoint_27091] === this.CELL_EMPTY
                && map[yPoint_27091-1][xPoint_27091] === this.CELL_EMPTY
                && map[yPoint_27091-1][xPoint_27091+1] === this.CELL_EMPTY
                && map[yPoint_27091][xPoint_27091+1] === this.CELL_EMPTY
                && map[yPoint_27091+1][xPoint_27091+1] === this.CELL_EMPTY
                && map[yPoint_27091+1][xPoint_27091] === this.CELL_EMPTY
                && map[yPoint_27091+1][xPoint_27091-1] === this.CELL_EMPTY
                && map[yPoint_27091][xPoint_27091-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27091, yPoint_27091, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27091;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27091][x] === this.CELL_EMPTY
                    && map[yPoint_27091-1][x] === this.CELL_EMPTY
                    && map[yPoint_27091-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27091][x+1] === this.CELL_EMPTY
                    && map[yPoint_27091+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27091+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27091, yPoint_27091, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27091;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27091] === this.CELL_EMPTY
                    && map[y+1][xPoint_27091] === this.CELL_EMPTY
                    && map[y+1][xPoint_27091+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27091] === this.CELL_EMPTY
                    && map[y][xPoint_27091-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27091-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
