...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16344=0;yPoint_16344<this.gameFieldBorderY.length; yPoint_16344++){
                for(var xPoint_16344=0;xPoint_16344<this.gameFieldBorderX.length; xPoint_16344++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16344, xPoint_16344);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16344][xPoint_16344] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16344, xPoint_16344, 'user');
                    if(this._userShipsMap[yPoint_16344][xPoint_16344] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16344, xPoint_16344, type){
            var id = this.getPointBlockIdByCoords(yPoint_16344, xPoint_16344, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16344);
                block.setAttribute('data-y', yPoint_16344);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16344
         * @param {type} xPoint_16344
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16344, xPoint_16344, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16344 + '_y' + yPoint_16344;
            }
            return 'pc_x' + xPoint_16344 + '_y' + yPoint_16344;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16344=0;yPoint_16344<this.gameFieldBorderY.length; yPoint_16344++){
                for(var xPoint_16344=0;xPoint_16344<this.gameFieldBorderX.length; xPoint_16344++){
                    map.push({y: yPoint_16344, x: xPoint_16344});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16344=-1;yPoint_16344<(this.gameFieldBorderY.length+1); yPoint_16344++){
                for(var xPoint_16344=-1;xPoint_16344<(this.gameFieldBorderX.length+1); xPoint_16344++){
                    if(!map[yPoint_16344]){
                        map[yPoint_16344] = [];
                    }
                    map[yPoint_16344][xPoint_16344] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16344 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16344 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16344, yPoint_16344) === true){
                    if(this.canPutHorizontal(map, xPoint_16344, yPoint_16344, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16344][xPoint_16344 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16344, yPoint_16344, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16344 + i][xPoint_16344] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16344, yPoint_16344){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16344][xPoint_16344] === this.CELL_EMPTY
                && map[yPoint_16344-1][xPoint_16344] === this.CELL_EMPTY
                && map[yPoint_16344-1][xPoint_16344+1] === this.CELL_EMPTY
                && map[yPoint_16344][xPoint_16344+1] === this.CELL_EMPTY
                && map[yPoint_16344+1][xPoint_16344+1] === this.CELL_EMPTY
                && map[yPoint_16344+1][xPoint_16344] === this.CELL_EMPTY
                && map[yPoint_16344+1][xPoint_16344-1] === this.CELL_EMPTY
                && map[yPoint_16344][xPoint_16344-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16344, yPoint_16344, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16344;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16344][x] === this.CELL_EMPTY
                    && map[yPoint_16344-1][x] === this.CELL_EMPTY
                    && map[yPoint_16344-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16344][x+1] === this.CELL_EMPTY
                    && map[yPoint_16344+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16344+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16344, yPoint_16344, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16344;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16344] === this.CELL_EMPTY
                    && map[y+1][xPoint_16344] === this.CELL_EMPTY
                    && map[y+1][xPoint_16344+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16344] === this.CELL_EMPTY
                    && map[y][xPoint_16344-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16344-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
