...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24363=0;yPoint_24363<this.gameFieldBorderY.length; yPoint_24363++){
                for(var xPoint_24363=0;xPoint_24363<this.gameFieldBorderX.length; xPoint_24363++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24363, xPoint_24363);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24363][xPoint_24363] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24363, xPoint_24363, 'user');
                    if(this._userShipsMap[yPoint_24363][xPoint_24363] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24363, xPoint_24363, type){
            var id = this.getPointBlockIdByCoords(yPoint_24363, xPoint_24363, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24363);
                block.setAttribute('data-y', yPoint_24363);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24363
         * @param {type} xPoint_24363
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24363, xPoint_24363, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24363 + '_y' + yPoint_24363;
            }
            return 'pc_x' + xPoint_24363 + '_y' + yPoint_24363;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24363=0;yPoint_24363<this.gameFieldBorderY.length; yPoint_24363++){
                for(var xPoint_24363=0;xPoint_24363<this.gameFieldBorderX.length; xPoint_24363++){
                    map.push({y: yPoint_24363, x: xPoint_24363});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24363=-1;yPoint_24363<(this.gameFieldBorderY.length+1); yPoint_24363++){
                for(var xPoint_24363=-1;xPoint_24363<(this.gameFieldBorderX.length+1); xPoint_24363++){
                    if(!map[yPoint_24363]){
                        map[yPoint_24363] = [];
                    }
                    map[yPoint_24363][xPoint_24363] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24363 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24363 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24363, yPoint_24363) === true){
                    if(this.canPutHorizontal(map, xPoint_24363, yPoint_24363, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24363][xPoint_24363 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24363, yPoint_24363, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24363 + i][xPoint_24363] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24363, yPoint_24363){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24363][xPoint_24363] === this.CELL_EMPTY
                && map[yPoint_24363-1][xPoint_24363] === this.CELL_EMPTY
                && map[yPoint_24363-1][xPoint_24363+1] === this.CELL_EMPTY
                && map[yPoint_24363][xPoint_24363+1] === this.CELL_EMPTY
                && map[yPoint_24363+1][xPoint_24363+1] === this.CELL_EMPTY
                && map[yPoint_24363+1][xPoint_24363] === this.CELL_EMPTY
                && map[yPoint_24363+1][xPoint_24363-1] === this.CELL_EMPTY
                && map[yPoint_24363][xPoint_24363-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24363, yPoint_24363, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24363;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24363][x] === this.CELL_EMPTY
                    && map[yPoint_24363-1][x] === this.CELL_EMPTY
                    && map[yPoint_24363-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24363][x+1] === this.CELL_EMPTY
                    && map[yPoint_24363+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24363+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24363, yPoint_24363, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24363;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24363] === this.CELL_EMPTY
                    && map[y+1][xPoint_24363] === this.CELL_EMPTY
                    && map[y+1][xPoint_24363+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24363] === this.CELL_EMPTY
                    && map[y][xPoint_24363-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24363-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
