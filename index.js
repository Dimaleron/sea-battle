...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_71=0;yPoint_71<this.gameFieldBorderY.length; yPoint_71++){
                for(var xPoint_71=0;xPoint_71<this.gameFieldBorderX.length; xPoint_71++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_71, xPoint_71);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_71][xPoint_71] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_71, xPoint_71, 'user');
                    if(this._userShipsMap[yPoint_71][xPoint_71] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_71, xPoint_71, type){
            var id = this.getPointBlockIdByCoords(yPoint_71, xPoint_71, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_71);
                block.setAttribute('data-y', yPoint_71);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_71
         * @param {type} xPoint_71
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_71, xPoint_71, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_71 + '_y' + yPoint_71;
            }
            return 'pc_x' + xPoint_71 + '_y' + yPoint_71;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_71=0;yPoint_71<this.gameFieldBorderY.length; yPoint_71++){
                for(var xPoint_71=0;xPoint_71<this.gameFieldBorderX.length; xPoint_71++){
                    map.push({y: yPoint_71, x: xPoint_71});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_71=-1;yPoint_71<(this.gameFieldBorderY.length+1); yPoint_71++){
                for(var xPoint_71=-1;xPoint_71<(this.gameFieldBorderX.length+1); xPoint_71++){
                    if(!map[yPoint_71]){
                        map[yPoint_71] = [];
                    }
                    map[yPoint_71][xPoint_71] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_71 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_71 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_71, yPoint_71) === true){
                    if(this.canPutHorizontal(map, xPoint_71, yPoint_71, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_71][xPoint_71 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_71, yPoint_71, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_71 + i][xPoint_71] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_71, yPoint_71){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_71][xPoint_71] === this.CELL_EMPTY
                && map[yPoint_71-1][xPoint_71] === this.CELL_EMPTY
                && map[yPoint_71-1][xPoint_71+1] === this.CELL_EMPTY
                && map[yPoint_71][xPoint_71+1] === this.CELL_EMPTY
                && map[yPoint_71+1][xPoint_71+1] === this.CELL_EMPTY
                && map[yPoint_71+1][xPoint_71] === this.CELL_EMPTY
                && map[yPoint_71+1][xPoint_71-1] === this.CELL_EMPTY
                && map[yPoint_71][xPoint_71-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_71, yPoint_71, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_71;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_71][x] === this.CELL_EMPTY
                    && map[yPoint_71-1][x] === this.CELL_EMPTY
                    && map[yPoint_71-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_71][x+1] === this.CELL_EMPTY
                    && map[yPoint_71+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_71+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_71, yPoint_71, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_71;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_71] === this.CELL_EMPTY
                    && map[y+1][xPoint_71] === this.CELL_EMPTY
                    && map[y+1][xPoint_71+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_71] === this.CELL_EMPTY
                    && map[y][xPoint_71-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_71-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
