...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_263=0;yPoint_263<this.gameFieldBorderY.length; yPoint_263++){
                for(var xPoint_263=0;xPoint_263<this.gameFieldBorderX.length; xPoint_263++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_263, xPoint_263);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_263][xPoint_263] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_263, xPoint_263, 'user');
                    if(this._userShipsMap[yPoint_263][xPoint_263] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_263, xPoint_263, type){
            var id = this.getPointBlockIdByCoords(yPoint_263, xPoint_263, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_263);
                block.setAttribute('data-y', yPoint_263);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_263
         * @param {type} xPoint_263
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_263, xPoint_263, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_263 + '_y' + yPoint_263;
            }
            return 'pc_x' + xPoint_263 + '_y' + yPoint_263;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_263=0;yPoint_263<this.gameFieldBorderY.length; yPoint_263++){
                for(var xPoint_263=0;xPoint_263<this.gameFieldBorderX.length; xPoint_263++){
                    map.push({y: yPoint_263, x: xPoint_263});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_263=-1;yPoint_263<(this.gameFieldBorderY.length+1); yPoint_263++){
                for(var xPoint_263=-1;xPoint_263<(this.gameFieldBorderX.length+1); xPoint_263++){
                    if(!map[yPoint_263]){
                        map[yPoint_263] = [];
                    }
                    map[yPoint_263][xPoint_263] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_263 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_263 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_263, yPoint_263) === true){
                    if(this.canPutHorizontal(map, xPoint_263, yPoint_263, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_263][xPoint_263 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_263, yPoint_263, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_263 + i][xPoint_263] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_263, yPoint_263){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_263][xPoint_263] === this.CELL_EMPTY
                && map[yPoint_263-1][xPoint_263] === this.CELL_EMPTY
                && map[yPoint_263-1][xPoint_263+1] === this.CELL_EMPTY
                && map[yPoint_263][xPoint_263+1] === this.CELL_EMPTY
                && map[yPoint_263+1][xPoint_263+1] === this.CELL_EMPTY
                && map[yPoint_263+1][xPoint_263] === this.CELL_EMPTY
                && map[yPoint_263+1][xPoint_263-1] === this.CELL_EMPTY
                && map[yPoint_263][xPoint_263-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_263, yPoint_263, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_263;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_263][x] === this.CELL_EMPTY
                    && map[yPoint_263-1][x] === this.CELL_EMPTY
                    && map[yPoint_263-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_263][x+1] === this.CELL_EMPTY
                    && map[yPoint_263+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_263+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_263, yPoint_263, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_263;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_263] === this.CELL_EMPTY
                    && map[y+1][xPoint_263] === this.CELL_EMPTY
                    && map[y+1][xPoint_263+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_263] === this.CELL_EMPTY
                    && map[y][xPoint_263-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_263-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
