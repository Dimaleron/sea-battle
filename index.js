...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27039=0;yPoint_27039<this.gameFieldBorderY.length; yPoint_27039++){
                for(var xPoint_27039=0;xPoint_27039<this.gameFieldBorderX.length; xPoint_27039++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27039, xPoint_27039);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27039][xPoint_27039] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27039, xPoint_27039, 'user');
                    if(this._userShipsMap[yPoint_27039][xPoint_27039] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27039, xPoint_27039, type){
            var id = this.getPointBlockIdByCoords(yPoint_27039, xPoint_27039, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27039);
                block.setAttribute('data-y', yPoint_27039);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27039
         * @param {type} xPoint_27039
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27039, xPoint_27039, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27039 + '_y' + yPoint_27039;
            }
            return 'pc_x' + xPoint_27039 + '_y' + yPoint_27039;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27039=0;yPoint_27039<this.gameFieldBorderY.length; yPoint_27039++){
                for(var xPoint_27039=0;xPoint_27039<this.gameFieldBorderX.length; xPoint_27039++){
                    map.push({y: yPoint_27039, x: xPoint_27039});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27039=-1;yPoint_27039<(this.gameFieldBorderY.length+1); yPoint_27039++){
                for(var xPoint_27039=-1;xPoint_27039<(this.gameFieldBorderX.length+1); xPoint_27039++){
                    if(!map[yPoint_27039]){
                        map[yPoint_27039] = [];
                    }
                    map[yPoint_27039][xPoint_27039] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27039 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27039 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27039, yPoint_27039) === true){
                    if(this.canPutHorizontal(map, xPoint_27039, yPoint_27039, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27039][xPoint_27039 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27039, yPoint_27039, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27039 + i][xPoint_27039] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27039, yPoint_27039){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27039][xPoint_27039] === this.CELL_EMPTY
                && map[yPoint_27039-1][xPoint_27039] === this.CELL_EMPTY
                && map[yPoint_27039-1][xPoint_27039+1] === this.CELL_EMPTY
                && map[yPoint_27039][xPoint_27039+1] === this.CELL_EMPTY
                && map[yPoint_27039+1][xPoint_27039+1] === this.CELL_EMPTY
                && map[yPoint_27039+1][xPoint_27039] === this.CELL_EMPTY
                && map[yPoint_27039+1][xPoint_27039-1] === this.CELL_EMPTY
                && map[yPoint_27039][xPoint_27039-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27039, yPoint_27039, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27039;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27039][x] === this.CELL_EMPTY
                    && map[yPoint_27039-1][x] === this.CELL_EMPTY
                    && map[yPoint_27039-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27039][x+1] === this.CELL_EMPTY
                    && map[yPoint_27039+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27039+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27039, yPoint_27039, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27039;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27039] === this.CELL_EMPTY
                    && map[y+1][xPoint_27039] === this.CELL_EMPTY
                    && map[y+1][xPoint_27039+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27039] === this.CELL_EMPTY
                    && map[y][xPoint_27039-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27039-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
