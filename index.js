...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_42=0;yPoint_42<this.gameFieldBorderY.length; yPoint_42++){
                for(var xPoint_42=0;xPoint_42<this.gameFieldBorderX.length; xPoint_42++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_42, xPoint_42);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_42][xPoint_42] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_42, xPoint_42, 'user');
                    if(this._userShipsMap[yPoint_42][xPoint_42] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_42, xPoint_42, type){
            var id = this.getPointBlockIdByCoords(yPoint_42, xPoint_42, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_42);
                block.setAttribute('data-y', yPoint_42);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_42
         * @param {type} xPoint_42
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_42, xPoint_42, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_42 + '_y' + yPoint_42;
            }
            return 'pc_x' + xPoint_42 + '_y' + yPoint_42;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_42=0;yPoint_42<this.gameFieldBorderY.length; yPoint_42++){
                for(var xPoint_42=0;xPoint_42<this.gameFieldBorderX.length; xPoint_42++){
                    map.push({y: yPoint_42, x: xPoint_42});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_42=-1;yPoint_42<(this.gameFieldBorderY.length+1); yPoint_42++){
                for(var xPoint_42=-1;xPoint_42<(this.gameFieldBorderX.length+1); xPoint_42++){
                    if(!map[yPoint_42]){
                        map[yPoint_42] = [];
                    }
                    map[yPoint_42][xPoint_42] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_42 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_42 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_42, yPoint_42) === true){
                    if(this.canPutHorizontal(map, xPoint_42, yPoint_42, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_42][xPoint_42 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_42, yPoint_42, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_42 + i][xPoint_42] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_42, yPoint_42){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_42][xPoint_42] === this.CELL_EMPTY
                && map[yPoint_42-1][xPoint_42] === this.CELL_EMPTY
                && map[yPoint_42-1][xPoint_42+1] === this.CELL_EMPTY
                && map[yPoint_42][xPoint_42+1] === this.CELL_EMPTY
                && map[yPoint_42+1][xPoint_42+1] === this.CELL_EMPTY
                && map[yPoint_42+1][xPoint_42] === this.CELL_EMPTY
                && map[yPoint_42+1][xPoint_42-1] === this.CELL_EMPTY
                && map[yPoint_42][xPoint_42-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_42, yPoint_42, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_42;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_42][x] === this.CELL_EMPTY
                    && map[yPoint_42-1][x] === this.CELL_EMPTY
                    && map[yPoint_42-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_42][x+1] === this.CELL_EMPTY
                    && map[yPoint_42+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_42+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_42, yPoint_42, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_42;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_42] === this.CELL_EMPTY
                    && map[y+1][xPoint_42] === this.CELL_EMPTY
                    && map[y+1][xPoint_42+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_42] === this.CELL_EMPTY
                    && map[y][xPoint_42-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_42-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
