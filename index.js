...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18196=0;yPoint_18196<this.gameFieldBorderY.length; yPoint_18196++){
                for(var xPoint_18196=0;xPoint_18196<this.gameFieldBorderX.length; xPoint_18196++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18196, xPoint_18196);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18196][xPoint_18196] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18196, xPoint_18196, 'user');
                    if(this._userShipsMap[yPoint_18196][xPoint_18196] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18196, xPoint_18196, type){
            var id = this.getPointBlockIdByCoords(yPoint_18196, xPoint_18196, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18196);
                block.setAttribute('data-y', yPoint_18196);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18196
         * @param {type} xPoint_18196
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18196, xPoint_18196, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18196 + '_y' + yPoint_18196;
            }
            return 'pc_x' + xPoint_18196 + '_y' + yPoint_18196;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18196=0;yPoint_18196<this.gameFieldBorderY.length; yPoint_18196++){
                for(var xPoint_18196=0;xPoint_18196<this.gameFieldBorderX.length; xPoint_18196++){
                    map.push({y: yPoint_18196, x: xPoint_18196});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18196=-1;yPoint_18196<(this.gameFieldBorderY.length+1); yPoint_18196++){
                for(var xPoint_18196=-1;xPoint_18196<(this.gameFieldBorderX.length+1); xPoint_18196++){
                    if(!map[yPoint_18196]){
                        map[yPoint_18196] = [];
                    }
                    map[yPoint_18196][xPoint_18196] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18196 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18196 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18196, yPoint_18196) === true){
                    if(this.canPutHorizontal(map, xPoint_18196, yPoint_18196, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18196][xPoint_18196 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18196, yPoint_18196, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18196 + i][xPoint_18196] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18196, yPoint_18196){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18196][xPoint_18196] === this.CELL_EMPTY
                && map[yPoint_18196-1][xPoint_18196] === this.CELL_EMPTY
                && map[yPoint_18196-1][xPoint_18196+1] === this.CELL_EMPTY
                && map[yPoint_18196][xPoint_18196+1] === this.CELL_EMPTY
                && map[yPoint_18196+1][xPoint_18196+1] === this.CELL_EMPTY
                && map[yPoint_18196+1][xPoint_18196] === this.CELL_EMPTY
                && map[yPoint_18196+1][xPoint_18196-1] === this.CELL_EMPTY
                && map[yPoint_18196][xPoint_18196-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18196, yPoint_18196, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18196;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18196][x] === this.CELL_EMPTY
                    && map[yPoint_18196-1][x] === this.CELL_EMPTY
                    && map[yPoint_18196-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18196][x+1] === this.CELL_EMPTY
                    && map[yPoint_18196+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18196+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18196, yPoint_18196, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18196;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18196] === this.CELL_EMPTY
                    && map[y+1][xPoint_18196] === this.CELL_EMPTY
                    && map[y+1][xPoint_18196+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18196] === this.CELL_EMPTY
                    && map[y][xPoint_18196-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18196-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
