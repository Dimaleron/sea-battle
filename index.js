...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14835=0;yPoint_14835<this.gameFieldBorderY.length; yPoint_14835++){
                for(var xPoint_14835=0;xPoint_14835<this.gameFieldBorderX.length; xPoint_14835++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14835, xPoint_14835);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14835][xPoint_14835] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14835, xPoint_14835, 'user');
                    if(this._userShipsMap[yPoint_14835][xPoint_14835] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14835, xPoint_14835, type){
            var id = this.getPointBlockIdByCoords(yPoint_14835, xPoint_14835, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14835);
                block.setAttribute('data-y', yPoint_14835);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14835
         * @param {type} xPoint_14835
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14835, xPoint_14835, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14835 + '_y' + yPoint_14835;
            }
            return 'pc_x' + xPoint_14835 + '_y' + yPoint_14835;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14835=0;yPoint_14835<this.gameFieldBorderY.length; yPoint_14835++){
                for(var xPoint_14835=0;xPoint_14835<this.gameFieldBorderX.length; xPoint_14835++){
                    map.push({y: yPoint_14835, x: xPoint_14835});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14835=-1;yPoint_14835<(this.gameFieldBorderY.length+1); yPoint_14835++){
                for(var xPoint_14835=-1;xPoint_14835<(this.gameFieldBorderX.length+1); xPoint_14835++){
                    if(!map[yPoint_14835]){
                        map[yPoint_14835] = [];
                    }
                    map[yPoint_14835][xPoint_14835] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14835 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14835 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14835, yPoint_14835) === true){
                    if(this.canPutHorizontal(map, xPoint_14835, yPoint_14835, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14835][xPoint_14835 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14835, yPoint_14835, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14835 + i][xPoint_14835] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14835, yPoint_14835){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14835][xPoint_14835] === this.CELL_EMPTY
                && map[yPoint_14835-1][xPoint_14835] === this.CELL_EMPTY
                && map[yPoint_14835-1][xPoint_14835+1] === this.CELL_EMPTY
                && map[yPoint_14835][xPoint_14835+1] === this.CELL_EMPTY
                && map[yPoint_14835+1][xPoint_14835+1] === this.CELL_EMPTY
                && map[yPoint_14835+1][xPoint_14835] === this.CELL_EMPTY
                && map[yPoint_14835+1][xPoint_14835-1] === this.CELL_EMPTY
                && map[yPoint_14835][xPoint_14835-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14835, yPoint_14835, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14835;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14835][x] === this.CELL_EMPTY
                    && map[yPoint_14835-1][x] === this.CELL_EMPTY
                    && map[yPoint_14835-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14835][x+1] === this.CELL_EMPTY
                    && map[yPoint_14835+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14835+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14835, yPoint_14835, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14835;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14835] === this.CELL_EMPTY
                    && map[y+1][xPoint_14835] === this.CELL_EMPTY
                    && map[y+1][xPoint_14835+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14835] === this.CELL_EMPTY
                    && map[y][xPoint_14835-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14835-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
