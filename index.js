...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24563=0;yPoint_24563<this.gameFieldBorderY.length; yPoint_24563++){
                for(var xPoint_24563=0;xPoint_24563<this.gameFieldBorderX.length; xPoint_24563++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24563, xPoint_24563);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24563][xPoint_24563] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24563, xPoint_24563, 'user');
                    if(this._userShipsMap[yPoint_24563][xPoint_24563] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24563, xPoint_24563, type){
            var id = this.getPointBlockIdByCoords(yPoint_24563, xPoint_24563, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24563);
                block.setAttribute('data-y', yPoint_24563);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24563
         * @param {type} xPoint_24563
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24563, xPoint_24563, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24563 + '_y' + yPoint_24563;
            }
            return 'pc_x' + xPoint_24563 + '_y' + yPoint_24563;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24563=0;yPoint_24563<this.gameFieldBorderY.length; yPoint_24563++){
                for(var xPoint_24563=0;xPoint_24563<this.gameFieldBorderX.length; xPoint_24563++){
                    map.push({y: yPoint_24563, x: xPoint_24563});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24563=-1;yPoint_24563<(this.gameFieldBorderY.length+1); yPoint_24563++){
                for(var xPoint_24563=-1;xPoint_24563<(this.gameFieldBorderX.length+1); xPoint_24563++){
                    if(!map[yPoint_24563]){
                        map[yPoint_24563] = [];
                    }
                    map[yPoint_24563][xPoint_24563] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24563 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24563 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24563, yPoint_24563) === true){
                    if(this.canPutHorizontal(map, xPoint_24563, yPoint_24563, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24563][xPoint_24563 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24563, yPoint_24563, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24563 + i][xPoint_24563] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24563, yPoint_24563){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24563][xPoint_24563] === this.CELL_EMPTY
                && map[yPoint_24563-1][xPoint_24563] === this.CELL_EMPTY
                && map[yPoint_24563-1][xPoint_24563+1] === this.CELL_EMPTY
                && map[yPoint_24563][xPoint_24563+1] === this.CELL_EMPTY
                && map[yPoint_24563+1][xPoint_24563+1] === this.CELL_EMPTY
                && map[yPoint_24563+1][xPoint_24563] === this.CELL_EMPTY
                && map[yPoint_24563+1][xPoint_24563-1] === this.CELL_EMPTY
                && map[yPoint_24563][xPoint_24563-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24563, yPoint_24563, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24563;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24563][x] === this.CELL_EMPTY
                    && map[yPoint_24563-1][x] === this.CELL_EMPTY
                    && map[yPoint_24563-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24563][x+1] === this.CELL_EMPTY
                    && map[yPoint_24563+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24563+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24563, yPoint_24563, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24563;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24563] === this.CELL_EMPTY
                    && map[y+1][xPoint_24563] === this.CELL_EMPTY
                    && map[y+1][xPoint_24563+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24563] === this.CELL_EMPTY
                    && map[y][xPoint_24563-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24563-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
