...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5366=0;yPoint_5366<this.gameFieldBorderY.length; yPoint_5366++){
                for(var xPoint_5366=0;xPoint_5366<this.gameFieldBorderX.length; xPoint_5366++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5366, xPoint_5366);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5366][xPoint_5366] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5366, xPoint_5366, 'user');
                    if(this._userShipsMap[yPoint_5366][xPoint_5366] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5366, xPoint_5366, type){
            var id = this.getPointBlockIdByCoords(yPoint_5366, xPoint_5366, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5366);
                block.setAttribute('data-y', yPoint_5366);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5366
         * @param {type} xPoint_5366
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5366, xPoint_5366, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5366 + '_y' + yPoint_5366;
            }
            return 'pc_x' + xPoint_5366 + '_y' + yPoint_5366;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5366=0;yPoint_5366<this.gameFieldBorderY.length; yPoint_5366++){
                for(var xPoint_5366=0;xPoint_5366<this.gameFieldBorderX.length; xPoint_5366++){
                    map.push({y: yPoint_5366, x: xPoint_5366});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5366=-1;yPoint_5366<(this.gameFieldBorderY.length+1); yPoint_5366++){
                for(var xPoint_5366=-1;xPoint_5366<(this.gameFieldBorderX.length+1); xPoint_5366++){
                    if(!map[yPoint_5366]){
                        map[yPoint_5366] = [];
                    }
                    map[yPoint_5366][xPoint_5366] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5366 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5366 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5366, yPoint_5366) === true){
                    if(this.canPutHorizontal(map, xPoint_5366, yPoint_5366, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5366][xPoint_5366 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5366, yPoint_5366, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5366 + i][xPoint_5366] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5366, yPoint_5366){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5366][xPoint_5366] === this.CELL_EMPTY
                && map[yPoint_5366-1][xPoint_5366] === this.CELL_EMPTY
                && map[yPoint_5366-1][xPoint_5366+1] === this.CELL_EMPTY
                && map[yPoint_5366][xPoint_5366+1] === this.CELL_EMPTY
                && map[yPoint_5366+1][xPoint_5366+1] === this.CELL_EMPTY
                && map[yPoint_5366+1][xPoint_5366] === this.CELL_EMPTY
                && map[yPoint_5366+1][xPoint_5366-1] === this.CELL_EMPTY
                && map[yPoint_5366][xPoint_5366-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5366, yPoint_5366, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5366;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5366][x] === this.CELL_EMPTY
                    && map[yPoint_5366-1][x] === this.CELL_EMPTY
                    && map[yPoint_5366-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5366][x+1] === this.CELL_EMPTY
                    && map[yPoint_5366+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5366+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5366, yPoint_5366, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5366;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5366] === this.CELL_EMPTY
                    && map[y+1][xPoint_5366] === this.CELL_EMPTY
                    && map[y+1][xPoint_5366+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5366] === this.CELL_EMPTY
                    && map[y][xPoint_5366-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5366-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
