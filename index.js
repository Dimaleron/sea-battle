...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16336=0;yPoint_16336<this.gameFieldBorderY.length; yPoint_16336++){
                for(var xPoint_16336=0;xPoint_16336<this.gameFieldBorderX.length; xPoint_16336++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16336, xPoint_16336);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16336][xPoint_16336] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16336, xPoint_16336, 'user');
                    if(this._userShipsMap[yPoint_16336][xPoint_16336] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16336, xPoint_16336, type){
            var id = this.getPointBlockIdByCoords(yPoint_16336, xPoint_16336, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16336);
                block.setAttribute('data-y', yPoint_16336);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16336
         * @param {type} xPoint_16336
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16336, xPoint_16336, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16336 + '_y' + yPoint_16336;
            }
            return 'pc_x' + xPoint_16336 + '_y' + yPoint_16336;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16336=0;yPoint_16336<this.gameFieldBorderY.length; yPoint_16336++){
                for(var xPoint_16336=0;xPoint_16336<this.gameFieldBorderX.length; xPoint_16336++){
                    map.push({y: yPoint_16336, x: xPoint_16336});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16336=-1;yPoint_16336<(this.gameFieldBorderY.length+1); yPoint_16336++){
                for(var xPoint_16336=-1;xPoint_16336<(this.gameFieldBorderX.length+1); xPoint_16336++){
                    if(!map[yPoint_16336]){
                        map[yPoint_16336] = [];
                    }
                    map[yPoint_16336][xPoint_16336] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16336 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16336 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16336, yPoint_16336) === true){
                    if(this.canPutHorizontal(map, xPoint_16336, yPoint_16336, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16336][xPoint_16336 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16336, yPoint_16336, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16336 + i][xPoint_16336] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16336, yPoint_16336){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16336][xPoint_16336] === this.CELL_EMPTY
                && map[yPoint_16336-1][xPoint_16336] === this.CELL_EMPTY
                && map[yPoint_16336-1][xPoint_16336+1] === this.CELL_EMPTY
                && map[yPoint_16336][xPoint_16336+1] === this.CELL_EMPTY
                && map[yPoint_16336+1][xPoint_16336+1] === this.CELL_EMPTY
                && map[yPoint_16336+1][xPoint_16336] === this.CELL_EMPTY
                && map[yPoint_16336+1][xPoint_16336-1] === this.CELL_EMPTY
                && map[yPoint_16336][xPoint_16336-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16336, yPoint_16336, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16336;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16336][x] === this.CELL_EMPTY
                    && map[yPoint_16336-1][x] === this.CELL_EMPTY
                    && map[yPoint_16336-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16336][x+1] === this.CELL_EMPTY
                    && map[yPoint_16336+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16336+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16336, yPoint_16336, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16336;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16336] === this.CELL_EMPTY
                    && map[y+1][xPoint_16336] === this.CELL_EMPTY
                    && map[y+1][xPoint_16336+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16336] === this.CELL_EMPTY
                    && map[y][xPoint_16336-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16336-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
