...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10509=0;yPoint_10509<this.gameFieldBorderY.length; yPoint_10509++){
                for(var xPoint_10509=0;xPoint_10509<this.gameFieldBorderX.length; xPoint_10509++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10509, xPoint_10509);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10509][xPoint_10509] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10509, xPoint_10509, 'user');
                    if(this._userShipsMap[yPoint_10509][xPoint_10509] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10509, xPoint_10509, type){
            var id = this.getPointBlockIdByCoords(yPoint_10509, xPoint_10509, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10509);
                block.setAttribute('data-y', yPoint_10509);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10509
         * @param {type} xPoint_10509
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10509, xPoint_10509, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10509 + '_y' + yPoint_10509;
            }
            return 'pc_x' + xPoint_10509 + '_y' + yPoint_10509;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10509=0;yPoint_10509<this.gameFieldBorderY.length; yPoint_10509++){
                for(var xPoint_10509=0;xPoint_10509<this.gameFieldBorderX.length; xPoint_10509++){
                    map.push({y: yPoint_10509, x: xPoint_10509});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10509=-1;yPoint_10509<(this.gameFieldBorderY.length+1); yPoint_10509++){
                for(var xPoint_10509=-1;xPoint_10509<(this.gameFieldBorderX.length+1); xPoint_10509++){
                    if(!map[yPoint_10509]){
                        map[yPoint_10509] = [];
                    }
                    map[yPoint_10509][xPoint_10509] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10509 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10509 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10509, yPoint_10509) === true){
                    if(this.canPutHorizontal(map, xPoint_10509, yPoint_10509, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10509][xPoint_10509 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10509, yPoint_10509, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10509 + i][xPoint_10509] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10509, yPoint_10509){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10509][xPoint_10509] === this.CELL_EMPTY
                && map[yPoint_10509-1][xPoint_10509] === this.CELL_EMPTY
                && map[yPoint_10509-1][xPoint_10509+1] === this.CELL_EMPTY
                && map[yPoint_10509][xPoint_10509+1] === this.CELL_EMPTY
                && map[yPoint_10509+1][xPoint_10509+1] === this.CELL_EMPTY
                && map[yPoint_10509+1][xPoint_10509] === this.CELL_EMPTY
                && map[yPoint_10509+1][xPoint_10509-1] === this.CELL_EMPTY
                && map[yPoint_10509][xPoint_10509-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10509, yPoint_10509, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10509;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10509][x] === this.CELL_EMPTY
                    && map[yPoint_10509-1][x] === this.CELL_EMPTY
                    && map[yPoint_10509-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10509][x+1] === this.CELL_EMPTY
                    && map[yPoint_10509+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10509+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10509, yPoint_10509, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10509;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10509] === this.CELL_EMPTY
                    && map[y+1][xPoint_10509] === this.CELL_EMPTY
                    && map[y+1][xPoint_10509+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10509] === this.CELL_EMPTY
                    && map[y][xPoint_10509-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10509-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
