...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27337=0;yPoint_27337<this.gameFieldBorderY.length; yPoint_27337++){
                for(var xPoint_27337=0;xPoint_27337<this.gameFieldBorderX.length; xPoint_27337++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27337, xPoint_27337);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27337][xPoint_27337] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27337, xPoint_27337, 'user');
                    if(this._userShipsMap[yPoint_27337][xPoint_27337] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27337, xPoint_27337, type){
            var id = this.getPointBlockIdByCoords(yPoint_27337, xPoint_27337, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27337);
                block.setAttribute('data-y', yPoint_27337);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27337
         * @param {type} xPoint_27337
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27337, xPoint_27337, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27337 + '_y' + yPoint_27337;
            }
            return 'pc_x' + xPoint_27337 + '_y' + yPoint_27337;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27337=0;yPoint_27337<this.gameFieldBorderY.length; yPoint_27337++){
                for(var xPoint_27337=0;xPoint_27337<this.gameFieldBorderX.length; xPoint_27337++){
                    map.push({y: yPoint_27337, x: xPoint_27337});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27337=-1;yPoint_27337<(this.gameFieldBorderY.length+1); yPoint_27337++){
                for(var xPoint_27337=-1;xPoint_27337<(this.gameFieldBorderX.length+1); xPoint_27337++){
                    if(!map[yPoint_27337]){
                        map[yPoint_27337] = [];
                    }
                    map[yPoint_27337][xPoint_27337] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27337 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27337 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27337, yPoint_27337) === true){
                    if(this.canPutHorizontal(map, xPoint_27337, yPoint_27337, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27337][xPoint_27337 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27337, yPoint_27337, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27337 + i][xPoint_27337] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27337, yPoint_27337){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27337][xPoint_27337] === this.CELL_EMPTY
                && map[yPoint_27337-1][xPoint_27337] === this.CELL_EMPTY
                && map[yPoint_27337-1][xPoint_27337+1] === this.CELL_EMPTY
                && map[yPoint_27337][xPoint_27337+1] === this.CELL_EMPTY
                && map[yPoint_27337+1][xPoint_27337+1] === this.CELL_EMPTY
                && map[yPoint_27337+1][xPoint_27337] === this.CELL_EMPTY
                && map[yPoint_27337+1][xPoint_27337-1] === this.CELL_EMPTY
                && map[yPoint_27337][xPoint_27337-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27337, yPoint_27337, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27337;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27337][x] === this.CELL_EMPTY
                    && map[yPoint_27337-1][x] === this.CELL_EMPTY
                    && map[yPoint_27337-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27337][x+1] === this.CELL_EMPTY
                    && map[yPoint_27337+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27337+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27337, yPoint_27337, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27337;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27337] === this.CELL_EMPTY
                    && map[y+1][xPoint_27337] === this.CELL_EMPTY
                    && map[y+1][xPoint_27337+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27337] === this.CELL_EMPTY
                    && map[y][xPoint_27337-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27337-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
