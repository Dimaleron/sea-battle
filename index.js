...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22522=0;yPoint_22522<this.gameFieldBorderY.length; yPoint_22522++){
                for(var xPoint_22522=0;xPoint_22522<this.gameFieldBorderX.length; xPoint_22522++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22522, xPoint_22522);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22522][xPoint_22522] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22522, xPoint_22522, 'user');
                    if(this._userShipsMap[yPoint_22522][xPoint_22522] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22522, xPoint_22522, type){
            var id = this.getPointBlockIdByCoords(yPoint_22522, xPoint_22522, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22522);
                block.setAttribute('data-y', yPoint_22522);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22522
         * @param {type} xPoint_22522
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22522, xPoint_22522, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22522 + '_y' + yPoint_22522;
            }
            return 'pc_x' + xPoint_22522 + '_y' + yPoint_22522;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22522=0;yPoint_22522<this.gameFieldBorderY.length; yPoint_22522++){
                for(var xPoint_22522=0;xPoint_22522<this.gameFieldBorderX.length; xPoint_22522++){
                    map.push({y: yPoint_22522, x: xPoint_22522});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22522=-1;yPoint_22522<(this.gameFieldBorderY.length+1); yPoint_22522++){
                for(var xPoint_22522=-1;xPoint_22522<(this.gameFieldBorderX.length+1); xPoint_22522++){
                    if(!map[yPoint_22522]){
                        map[yPoint_22522] = [];
                    }
                    map[yPoint_22522][xPoint_22522] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22522 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22522 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22522, yPoint_22522) === true){
                    if(this.canPutHorizontal(map, xPoint_22522, yPoint_22522, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22522][xPoint_22522 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22522, yPoint_22522, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22522 + i][xPoint_22522] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22522, yPoint_22522){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22522][xPoint_22522] === this.CELL_EMPTY
                && map[yPoint_22522-1][xPoint_22522] === this.CELL_EMPTY
                && map[yPoint_22522-1][xPoint_22522+1] === this.CELL_EMPTY
                && map[yPoint_22522][xPoint_22522+1] === this.CELL_EMPTY
                && map[yPoint_22522+1][xPoint_22522+1] === this.CELL_EMPTY
                && map[yPoint_22522+1][xPoint_22522] === this.CELL_EMPTY
                && map[yPoint_22522+1][xPoint_22522-1] === this.CELL_EMPTY
                && map[yPoint_22522][xPoint_22522-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22522, yPoint_22522, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22522;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22522][x] === this.CELL_EMPTY
                    && map[yPoint_22522-1][x] === this.CELL_EMPTY
                    && map[yPoint_22522-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22522][x+1] === this.CELL_EMPTY
                    && map[yPoint_22522+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22522+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22522, yPoint_22522, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22522;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22522] === this.CELL_EMPTY
                    && map[y+1][xPoint_22522] === this.CELL_EMPTY
                    && map[y+1][xPoint_22522+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22522] === this.CELL_EMPTY
                    && map[y][xPoint_22522-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22522-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
