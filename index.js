...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14727=0;yPoint_14727<this.gameFieldBorderY.length; yPoint_14727++){
                for(var xPoint_14727=0;xPoint_14727<this.gameFieldBorderX.length; xPoint_14727++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14727, xPoint_14727);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14727][xPoint_14727] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14727, xPoint_14727, 'user');
                    if(this._userShipsMap[yPoint_14727][xPoint_14727] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14727, xPoint_14727, type){
            var id = this.getPointBlockIdByCoords(yPoint_14727, xPoint_14727, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14727);
                block.setAttribute('data-y', yPoint_14727);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14727
         * @param {type} xPoint_14727
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14727, xPoint_14727, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14727 + '_y' + yPoint_14727;
            }
            return 'pc_x' + xPoint_14727 + '_y' + yPoint_14727;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14727=0;yPoint_14727<this.gameFieldBorderY.length; yPoint_14727++){
                for(var xPoint_14727=0;xPoint_14727<this.gameFieldBorderX.length; xPoint_14727++){
                    map.push({y: yPoint_14727, x: xPoint_14727});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14727=-1;yPoint_14727<(this.gameFieldBorderY.length+1); yPoint_14727++){
                for(var xPoint_14727=-1;xPoint_14727<(this.gameFieldBorderX.length+1); xPoint_14727++){
                    if(!map[yPoint_14727]){
                        map[yPoint_14727] = [];
                    }
                    map[yPoint_14727][xPoint_14727] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14727 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14727 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14727, yPoint_14727) === true){
                    if(this.canPutHorizontal(map, xPoint_14727, yPoint_14727, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14727][xPoint_14727 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14727, yPoint_14727, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14727 + i][xPoint_14727] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14727, yPoint_14727){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14727][xPoint_14727] === this.CELL_EMPTY
                && map[yPoint_14727-1][xPoint_14727] === this.CELL_EMPTY
                && map[yPoint_14727-1][xPoint_14727+1] === this.CELL_EMPTY
                && map[yPoint_14727][xPoint_14727+1] === this.CELL_EMPTY
                && map[yPoint_14727+1][xPoint_14727+1] === this.CELL_EMPTY
                && map[yPoint_14727+1][xPoint_14727] === this.CELL_EMPTY
                && map[yPoint_14727+1][xPoint_14727-1] === this.CELL_EMPTY
                && map[yPoint_14727][xPoint_14727-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14727, yPoint_14727, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14727;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14727][x] === this.CELL_EMPTY
                    && map[yPoint_14727-1][x] === this.CELL_EMPTY
                    && map[yPoint_14727-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14727][x+1] === this.CELL_EMPTY
                    && map[yPoint_14727+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14727+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14727, yPoint_14727, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14727;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14727] === this.CELL_EMPTY
                    && map[y+1][xPoint_14727] === this.CELL_EMPTY
                    && map[y+1][xPoint_14727+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14727] === this.CELL_EMPTY
                    && map[y][xPoint_14727-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14727-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
