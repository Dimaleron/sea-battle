...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_59=0;yPoint_59<this.gameFieldBorderY.length; yPoint_59++){
                for(var xPoint_59=0;xPoint_59<this.gameFieldBorderX.length; xPoint_59++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_59, xPoint_59);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_59][xPoint_59] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_59, xPoint_59, 'user');
                    if(this._userShipsMap[yPoint_59][xPoint_59] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_59, xPoint_59, type){
            var id = this.getPointBlockIdByCoords(yPoint_59, xPoint_59, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_59);
                block.setAttribute('data-y', yPoint_59);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_59
         * @param {type} xPoint_59
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_59, xPoint_59, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_59 + '_y' + yPoint_59;
            }
            return 'pc_x' + xPoint_59 + '_y' + yPoint_59;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_59=0;yPoint_59<this.gameFieldBorderY.length; yPoint_59++){
                for(var xPoint_59=0;xPoint_59<this.gameFieldBorderX.length; xPoint_59++){
                    map.push({y: yPoint_59, x: xPoint_59});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_59=-1;yPoint_59<(this.gameFieldBorderY.length+1); yPoint_59++){
                for(var xPoint_59=-1;xPoint_59<(this.gameFieldBorderX.length+1); xPoint_59++){
                    if(!map[yPoint_59]){
                        map[yPoint_59] = [];
                    }
                    map[yPoint_59][xPoint_59] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_59 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_59 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_59, yPoint_59) === true){
                    if(this.canPutHorizontal(map, xPoint_59, yPoint_59, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_59][xPoint_59 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_59, yPoint_59, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_59 + i][xPoint_59] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_59, yPoint_59){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_59][xPoint_59] === this.CELL_EMPTY
                && map[yPoint_59-1][xPoint_59] === this.CELL_EMPTY
                && map[yPoint_59-1][xPoint_59+1] === this.CELL_EMPTY
                && map[yPoint_59][xPoint_59+1] === this.CELL_EMPTY
                && map[yPoint_59+1][xPoint_59+1] === this.CELL_EMPTY
                && map[yPoint_59+1][xPoint_59] === this.CELL_EMPTY
                && map[yPoint_59+1][xPoint_59-1] === this.CELL_EMPTY
                && map[yPoint_59][xPoint_59-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_59, yPoint_59, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_59;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_59][x] === this.CELL_EMPTY
                    && map[yPoint_59-1][x] === this.CELL_EMPTY
                    && map[yPoint_59-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_59][x+1] === this.CELL_EMPTY
                    && map[yPoint_59+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_59+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_59, yPoint_59, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_59;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_59] === this.CELL_EMPTY
                    && map[y+1][xPoint_59] === this.CELL_EMPTY
                    && map[y+1][xPoint_59+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_59] === this.CELL_EMPTY
                    && map[y][xPoint_59-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_59-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
