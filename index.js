...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8243=0;yPoint_8243<this.gameFieldBorderY.length; yPoint_8243++){
                for(var xPoint_8243=0;xPoint_8243<this.gameFieldBorderX.length; xPoint_8243++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8243, xPoint_8243);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8243][xPoint_8243] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8243, xPoint_8243, 'user');
                    if(this._userShipsMap[yPoint_8243][xPoint_8243] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8243, xPoint_8243, type){
            var id = this.getPointBlockIdByCoords(yPoint_8243, xPoint_8243, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8243);
                block.setAttribute('data-y', yPoint_8243);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8243
         * @param {type} xPoint_8243
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8243, xPoint_8243, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8243 + '_y' + yPoint_8243;
            }
            return 'pc_x' + xPoint_8243 + '_y' + yPoint_8243;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8243=0;yPoint_8243<this.gameFieldBorderY.length; yPoint_8243++){
                for(var xPoint_8243=0;xPoint_8243<this.gameFieldBorderX.length; xPoint_8243++){
                    map.push({y: yPoint_8243, x: xPoint_8243});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8243=-1;yPoint_8243<(this.gameFieldBorderY.length+1); yPoint_8243++){
                for(var xPoint_8243=-1;xPoint_8243<(this.gameFieldBorderX.length+1); xPoint_8243++){
                    if(!map[yPoint_8243]){
                        map[yPoint_8243] = [];
                    }
                    map[yPoint_8243][xPoint_8243] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8243 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8243 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8243, yPoint_8243) === true){
                    if(this.canPutHorizontal(map, xPoint_8243, yPoint_8243, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8243][xPoint_8243 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8243, yPoint_8243, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8243 + i][xPoint_8243] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8243, yPoint_8243){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8243][xPoint_8243] === this.CELL_EMPTY
                && map[yPoint_8243-1][xPoint_8243] === this.CELL_EMPTY
                && map[yPoint_8243-1][xPoint_8243+1] === this.CELL_EMPTY
                && map[yPoint_8243][xPoint_8243+1] === this.CELL_EMPTY
                && map[yPoint_8243+1][xPoint_8243+1] === this.CELL_EMPTY
                && map[yPoint_8243+1][xPoint_8243] === this.CELL_EMPTY
                && map[yPoint_8243+1][xPoint_8243-1] === this.CELL_EMPTY
                && map[yPoint_8243][xPoint_8243-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8243, yPoint_8243, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8243;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8243][x] === this.CELL_EMPTY
                    && map[yPoint_8243-1][x] === this.CELL_EMPTY
                    && map[yPoint_8243-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8243][x+1] === this.CELL_EMPTY
                    && map[yPoint_8243+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8243+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8243, yPoint_8243, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8243;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8243] === this.CELL_EMPTY
                    && map[y+1][xPoint_8243] === this.CELL_EMPTY
                    && map[y+1][xPoint_8243+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8243] === this.CELL_EMPTY
                    && map[y][xPoint_8243-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8243-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
