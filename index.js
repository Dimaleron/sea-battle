...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7006=0;yPoint_7006<this.gameFieldBorderY.length; yPoint_7006++){
                for(var xPoint_7006=0;xPoint_7006<this.gameFieldBorderX.length; xPoint_7006++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7006, xPoint_7006);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7006][xPoint_7006] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7006, xPoint_7006, 'user');
                    if(this._userShipsMap[yPoint_7006][xPoint_7006] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7006, xPoint_7006, type){
            var id = this.getPointBlockIdByCoords(yPoint_7006, xPoint_7006, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7006);
                block.setAttribute('data-y', yPoint_7006);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7006
         * @param {type} xPoint_7006
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7006, xPoint_7006, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7006 + '_y' + yPoint_7006;
            }
            return 'pc_x' + xPoint_7006 + '_y' + yPoint_7006;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7006=0;yPoint_7006<this.gameFieldBorderY.length; yPoint_7006++){
                for(var xPoint_7006=0;xPoint_7006<this.gameFieldBorderX.length; xPoint_7006++){
                    map.push({y: yPoint_7006, x: xPoint_7006});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7006=-1;yPoint_7006<(this.gameFieldBorderY.length+1); yPoint_7006++){
                for(var xPoint_7006=-1;xPoint_7006<(this.gameFieldBorderX.length+1); xPoint_7006++){
                    if(!map[yPoint_7006]){
                        map[yPoint_7006] = [];
                    }
                    map[yPoint_7006][xPoint_7006] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7006 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7006 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7006, yPoint_7006) === true){
                    if(this.canPutHorizontal(map, xPoint_7006, yPoint_7006, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7006][xPoint_7006 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7006, yPoint_7006, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7006 + i][xPoint_7006] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7006, yPoint_7006){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7006][xPoint_7006] === this.CELL_EMPTY
                && map[yPoint_7006-1][xPoint_7006] === this.CELL_EMPTY
                && map[yPoint_7006-1][xPoint_7006+1] === this.CELL_EMPTY
                && map[yPoint_7006][xPoint_7006+1] === this.CELL_EMPTY
                && map[yPoint_7006+1][xPoint_7006+1] === this.CELL_EMPTY
                && map[yPoint_7006+1][xPoint_7006] === this.CELL_EMPTY
                && map[yPoint_7006+1][xPoint_7006-1] === this.CELL_EMPTY
                && map[yPoint_7006][xPoint_7006-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7006, yPoint_7006, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7006;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7006][x] === this.CELL_EMPTY
                    && map[yPoint_7006-1][x] === this.CELL_EMPTY
                    && map[yPoint_7006-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7006][x+1] === this.CELL_EMPTY
                    && map[yPoint_7006+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7006+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7006, yPoint_7006, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7006;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7006] === this.CELL_EMPTY
                    && map[y+1][xPoint_7006] === this.CELL_EMPTY
                    && map[y+1][xPoint_7006+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7006] === this.CELL_EMPTY
                    && map[y][xPoint_7006-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7006-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
