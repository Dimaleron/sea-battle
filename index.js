...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7407=0;yPoint_7407<this.gameFieldBorderY.length; yPoint_7407++){
                for(var xPoint_7407=0;xPoint_7407<this.gameFieldBorderX.length; xPoint_7407++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7407, xPoint_7407);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7407][xPoint_7407] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7407, xPoint_7407, 'user');
                    if(this._userShipsMap[yPoint_7407][xPoint_7407] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7407, xPoint_7407, type){
            var id = this.getPointBlockIdByCoords(yPoint_7407, xPoint_7407, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7407);
                block.setAttribute('data-y', yPoint_7407);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7407
         * @param {type} xPoint_7407
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7407, xPoint_7407, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7407 + '_y' + yPoint_7407;
            }
            return 'pc_x' + xPoint_7407 + '_y' + yPoint_7407;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7407=0;yPoint_7407<this.gameFieldBorderY.length; yPoint_7407++){
                for(var xPoint_7407=0;xPoint_7407<this.gameFieldBorderX.length; xPoint_7407++){
                    map.push({y: yPoint_7407, x: xPoint_7407});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7407=-1;yPoint_7407<(this.gameFieldBorderY.length+1); yPoint_7407++){
                for(var xPoint_7407=-1;xPoint_7407<(this.gameFieldBorderX.length+1); xPoint_7407++){
                    if(!map[yPoint_7407]){
                        map[yPoint_7407] = [];
                    }
                    map[yPoint_7407][xPoint_7407] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7407 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7407 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7407, yPoint_7407) === true){
                    if(this.canPutHorizontal(map, xPoint_7407, yPoint_7407, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7407][xPoint_7407 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7407, yPoint_7407, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7407 + i][xPoint_7407] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7407, yPoint_7407){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7407][xPoint_7407] === this.CELL_EMPTY
                && map[yPoint_7407-1][xPoint_7407] === this.CELL_EMPTY
                && map[yPoint_7407-1][xPoint_7407+1] === this.CELL_EMPTY
                && map[yPoint_7407][xPoint_7407+1] === this.CELL_EMPTY
                && map[yPoint_7407+1][xPoint_7407+1] === this.CELL_EMPTY
                && map[yPoint_7407+1][xPoint_7407] === this.CELL_EMPTY
                && map[yPoint_7407+1][xPoint_7407-1] === this.CELL_EMPTY
                && map[yPoint_7407][xPoint_7407-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7407, yPoint_7407, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7407;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7407][x] === this.CELL_EMPTY
                    && map[yPoint_7407-1][x] === this.CELL_EMPTY
                    && map[yPoint_7407-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7407][x+1] === this.CELL_EMPTY
                    && map[yPoint_7407+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7407+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7407, yPoint_7407, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7407;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7407] === this.CELL_EMPTY
                    && map[y+1][xPoint_7407] === this.CELL_EMPTY
                    && map[y+1][xPoint_7407+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7407] === this.CELL_EMPTY
                    && map[y][xPoint_7407-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7407-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
