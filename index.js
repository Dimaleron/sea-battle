...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31315=0;yPoint_31315<this.gameFieldBorderY.length; yPoint_31315++){
                for(var xPoint_31315=0;xPoint_31315<this.gameFieldBorderX.length; xPoint_31315++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31315, xPoint_31315);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31315][xPoint_31315] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31315, xPoint_31315, 'user');
                    if(this._userShipsMap[yPoint_31315][xPoint_31315] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31315, xPoint_31315, type){
            var id = this.getPointBlockIdByCoords(yPoint_31315, xPoint_31315, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31315);
                block.setAttribute('data-y', yPoint_31315);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31315
         * @param {type} xPoint_31315
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31315, xPoint_31315, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31315 + '_y' + yPoint_31315;
            }
            return 'pc_x' + xPoint_31315 + '_y' + yPoint_31315;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31315=0;yPoint_31315<this.gameFieldBorderY.length; yPoint_31315++){
                for(var xPoint_31315=0;xPoint_31315<this.gameFieldBorderX.length; xPoint_31315++){
                    map.push({y: yPoint_31315, x: xPoint_31315});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31315=-1;yPoint_31315<(this.gameFieldBorderY.length+1); yPoint_31315++){
                for(var xPoint_31315=-1;xPoint_31315<(this.gameFieldBorderX.length+1); xPoint_31315++){
                    if(!map[yPoint_31315]){
                        map[yPoint_31315] = [];
                    }
                    map[yPoint_31315][xPoint_31315] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31315 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31315 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31315, yPoint_31315) === true){
                    if(this.canPutHorizontal(map, xPoint_31315, yPoint_31315, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31315][xPoint_31315 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31315, yPoint_31315, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31315 + i][xPoint_31315] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31315, yPoint_31315){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31315][xPoint_31315] === this.CELL_EMPTY
                && map[yPoint_31315-1][xPoint_31315] === this.CELL_EMPTY
                && map[yPoint_31315-1][xPoint_31315+1] === this.CELL_EMPTY
                && map[yPoint_31315][xPoint_31315+1] === this.CELL_EMPTY
                && map[yPoint_31315+1][xPoint_31315+1] === this.CELL_EMPTY
                && map[yPoint_31315+1][xPoint_31315] === this.CELL_EMPTY
                && map[yPoint_31315+1][xPoint_31315-1] === this.CELL_EMPTY
                && map[yPoint_31315][xPoint_31315-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31315, yPoint_31315, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31315;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31315][x] === this.CELL_EMPTY
                    && map[yPoint_31315-1][x] === this.CELL_EMPTY
                    && map[yPoint_31315-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31315][x+1] === this.CELL_EMPTY
                    && map[yPoint_31315+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31315+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31315, yPoint_31315, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31315;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31315] === this.CELL_EMPTY
                    && map[y+1][xPoint_31315] === this.CELL_EMPTY
                    && map[y+1][xPoint_31315+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31315] === this.CELL_EMPTY
                    && map[y][xPoint_31315-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31315-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
