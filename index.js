...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30094=0;yPoint_30094<this.gameFieldBorderY.length; yPoint_30094++){
                for(var xPoint_30094=0;xPoint_30094<this.gameFieldBorderX.length; xPoint_30094++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30094, xPoint_30094);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30094][xPoint_30094] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30094, xPoint_30094, 'user');
                    if(this._userShipsMap[yPoint_30094][xPoint_30094] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30094, xPoint_30094, type){
            var id = this.getPointBlockIdByCoords(yPoint_30094, xPoint_30094, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30094);
                block.setAttribute('data-y', yPoint_30094);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30094
         * @param {type} xPoint_30094
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30094, xPoint_30094, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30094 + '_y' + yPoint_30094;
            }
            return 'pc_x' + xPoint_30094 + '_y' + yPoint_30094;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30094=0;yPoint_30094<this.gameFieldBorderY.length; yPoint_30094++){
                for(var xPoint_30094=0;xPoint_30094<this.gameFieldBorderX.length; xPoint_30094++){
                    map.push({y: yPoint_30094, x: xPoint_30094});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30094=-1;yPoint_30094<(this.gameFieldBorderY.length+1); yPoint_30094++){
                for(var xPoint_30094=-1;xPoint_30094<(this.gameFieldBorderX.length+1); xPoint_30094++){
                    if(!map[yPoint_30094]){
                        map[yPoint_30094] = [];
                    }
                    map[yPoint_30094][xPoint_30094] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30094 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30094 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30094, yPoint_30094) === true){
                    if(this.canPutHorizontal(map, xPoint_30094, yPoint_30094, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30094][xPoint_30094 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30094, yPoint_30094, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30094 + i][xPoint_30094] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30094, yPoint_30094){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30094][xPoint_30094] === this.CELL_EMPTY
                && map[yPoint_30094-1][xPoint_30094] === this.CELL_EMPTY
                && map[yPoint_30094-1][xPoint_30094+1] === this.CELL_EMPTY
                && map[yPoint_30094][xPoint_30094+1] === this.CELL_EMPTY
                && map[yPoint_30094+1][xPoint_30094+1] === this.CELL_EMPTY
                && map[yPoint_30094+1][xPoint_30094] === this.CELL_EMPTY
                && map[yPoint_30094+1][xPoint_30094-1] === this.CELL_EMPTY
                && map[yPoint_30094][xPoint_30094-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30094, yPoint_30094, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30094;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30094][x] === this.CELL_EMPTY
                    && map[yPoint_30094-1][x] === this.CELL_EMPTY
                    && map[yPoint_30094-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30094][x+1] === this.CELL_EMPTY
                    && map[yPoint_30094+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30094+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30094, yPoint_30094, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30094;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30094] === this.CELL_EMPTY
                    && map[y+1][xPoint_30094] === this.CELL_EMPTY
                    && map[y+1][xPoint_30094+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30094] === this.CELL_EMPTY
                    && map[y][xPoint_30094-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30094-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
