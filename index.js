...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24643=0;yPoint_24643<this.gameFieldBorderY.length; yPoint_24643++){
                for(var xPoint_24643=0;xPoint_24643<this.gameFieldBorderX.length; xPoint_24643++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24643, xPoint_24643);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24643][xPoint_24643] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24643, xPoint_24643, 'user');
                    if(this._userShipsMap[yPoint_24643][xPoint_24643] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24643, xPoint_24643, type){
            var id = this.getPointBlockIdByCoords(yPoint_24643, xPoint_24643, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24643);
                block.setAttribute('data-y', yPoint_24643);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24643
         * @param {type} xPoint_24643
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24643, xPoint_24643, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24643 + '_y' + yPoint_24643;
            }
            return 'pc_x' + xPoint_24643 + '_y' + yPoint_24643;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24643=0;yPoint_24643<this.gameFieldBorderY.length; yPoint_24643++){
                for(var xPoint_24643=0;xPoint_24643<this.gameFieldBorderX.length; xPoint_24643++){
                    map.push({y: yPoint_24643, x: xPoint_24643});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24643=-1;yPoint_24643<(this.gameFieldBorderY.length+1); yPoint_24643++){
                for(var xPoint_24643=-1;xPoint_24643<(this.gameFieldBorderX.length+1); xPoint_24643++){
                    if(!map[yPoint_24643]){
                        map[yPoint_24643] = [];
                    }
                    map[yPoint_24643][xPoint_24643] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24643 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24643 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24643, yPoint_24643) === true){
                    if(this.canPutHorizontal(map, xPoint_24643, yPoint_24643, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24643][xPoint_24643 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24643, yPoint_24643, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24643 + i][xPoint_24643] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24643, yPoint_24643){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24643][xPoint_24643] === this.CELL_EMPTY
                && map[yPoint_24643-1][xPoint_24643] === this.CELL_EMPTY
                && map[yPoint_24643-1][xPoint_24643+1] === this.CELL_EMPTY
                && map[yPoint_24643][xPoint_24643+1] === this.CELL_EMPTY
                && map[yPoint_24643+1][xPoint_24643+1] === this.CELL_EMPTY
                && map[yPoint_24643+1][xPoint_24643] === this.CELL_EMPTY
                && map[yPoint_24643+1][xPoint_24643-1] === this.CELL_EMPTY
                && map[yPoint_24643][xPoint_24643-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24643, yPoint_24643, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24643;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24643][x] === this.CELL_EMPTY
                    && map[yPoint_24643-1][x] === this.CELL_EMPTY
                    && map[yPoint_24643-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24643][x+1] === this.CELL_EMPTY
                    && map[yPoint_24643+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24643+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24643, yPoint_24643, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24643;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24643] === this.CELL_EMPTY
                    && map[y+1][xPoint_24643] === this.CELL_EMPTY
                    && map[y+1][xPoint_24643+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24643] === this.CELL_EMPTY
                    && map[y][xPoint_24643-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24643-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
