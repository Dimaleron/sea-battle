...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32627=0;yPoint_32627<this.gameFieldBorderY.length; yPoint_32627++){
                for(var xPoint_32627=0;xPoint_32627<this.gameFieldBorderX.length; xPoint_32627++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32627, xPoint_32627);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32627][xPoint_32627] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32627, xPoint_32627, 'user');
                    if(this._userShipsMap[yPoint_32627][xPoint_32627] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32627, xPoint_32627, type){
            var id = this.getPointBlockIdByCoords(yPoint_32627, xPoint_32627, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32627);
                block.setAttribute('data-y', yPoint_32627);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32627
         * @param {type} xPoint_32627
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32627, xPoint_32627, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32627 + '_y' + yPoint_32627;
            }
            return 'pc_x' + xPoint_32627 + '_y' + yPoint_32627;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32627=0;yPoint_32627<this.gameFieldBorderY.length; yPoint_32627++){
                for(var xPoint_32627=0;xPoint_32627<this.gameFieldBorderX.length; xPoint_32627++){
                    map.push({y: yPoint_32627, x: xPoint_32627});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32627=-1;yPoint_32627<(this.gameFieldBorderY.length+1); yPoint_32627++){
                for(var xPoint_32627=-1;xPoint_32627<(this.gameFieldBorderX.length+1); xPoint_32627++){
                    if(!map[yPoint_32627]){
                        map[yPoint_32627] = [];
                    }
                    map[yPoint_32627][xPoint_32627] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32627 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32627 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32627, yPoint_32627) === true){
                    if(this.canPutHorizontal(map, xPoint_32627, yPoint_32627, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32627][xPoint_32627 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32627, yPoint_32627, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32627 + i][xPoint_32627] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32627, yPoint_32627){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32627][xPoint_32627] === this.CELL_EMPTY
                && map[yPoint_32627-1][xPoint_32627] === this.CELL_EMPTY
                && map[yPoint_32627-1][xPoint_32627+1] === this.CELL_EMPTY
                && map[yPoint_32627][xPoint_32627+1] === this.CELL_EMPTY
                && map[yPoint_32627+1][xPoint_32627+1] === this.CELL_EMPTY
                && map[yPoint_32627+1][xPoint_32627] === this.CELL_EMPTY
                && map[yPoint_32627+1][xPoint_32627-1] === this.CELL_EMPTY
                && map[yPoint_32627][xPoint_32627-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32627, yPoint_32627, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32627;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32627][x] === this.CELL_EMPTY
                    && map[yPoint_32627-1][x] === this.CELL_EMPTY
                    && map[yPoint_32627-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32627][x+1] === this.CELL_EMPTY
                    && map[yPoint_32627+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32627+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32627, yPoint_32627, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32627;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32627] === this.CELL_EMPTY
                    && map[y+1][xPoint_32627] === this.CELL_EMPTY
                    && map[y+1][xPoint_32627+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32627] === this.CELL_EMPTY
                    && map[y][xPoint_32627-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32627-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
