...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26438=0;yPoint_26438<this.gameFieldBorderY.length; yPoint_26438++){
                for(var xPoint_26438=0;xPoint_26438<this.gameFieldBorderX.length; xPoint_26438++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26438, xPoint_26438);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26438][xPoint_26438] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26438, xPoint_26438, 'user');
                    if(this._userShipsMap[yPoint_26438][xPoint_26438] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26438, xPoint_26438, type){
            var id = this.getPointBlockIdByCoords(yPoint_26438, xPoint_26438, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26438);
                block.setAttribute('data-y', yPoint_26438);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26438
         * @param {type} xPoint_26438
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26438, xPoint_26438, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26438 + '_y' + yPoint_26438;
            }
            return 'pc_x' + xPoint_26438 + '_y' + yPoint_26438;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26438=0;yPoint_26438<this.gameFieldBorderY.length; yPoint_26438++){
                for(var xPoint_26438=0;xPoint_26438<this.gameFieldBorderX.length; xPoint_26438++){
                    map.push({y: yPoint_26438, x: xPoint_26438});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26438=-1;yPoint_26438<(this.gameFieldBorderY.length+1); yPoint_26438++){
                for(var xPoint_26438=-1;xPoint_26438<(this.gameFieldBorderX.length+1); xPoint_26438++){
                    if(!map[yPoint_26438]){
                        map[yPoint_26438] = [];
                    }
                    map[yPoint_26438][xPoint_26438] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26438 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26438 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26438, yPoint_26438) === true){
                    if(this.canPutHorizontal(map, xPoint_26438, yPoint_26438, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26438][xPoint_26438 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26438, yPoint_26438, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26438 + i][xPoint_26438] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26438, yPoint_26438){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26438][xPoint_26438] === this.CELL_EMPTY
                && map[yPoint_26438-1][xPoint_26438] === this.CELL_EMPTY
                && map[yPoint_26438-1][xPoint_26438+1] === this.CELL_EMPTY
                && map[yPoint_26438][xPoint_26438+1] === this.CELL_EMPTY
                && map[yPoint_26438+1][xPoint_26438+1] === this.CELL_EMPTY
                && map[yPoint_26438+1][xPoint_26438] === this.CELL_EMPTY
                && map[yPoint_26438+1][xPoint_26438-1] === this.CELL_EMPTY
                && map[yPoint_26438][xPoint_26438-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26438, yPoint_26438, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26438;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26438][x] === this.CELL_EMPTY
                    && map[yPoint_26438-1][x] === this.CELL_EMPTY
                    && map[yPoint_26438-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26438][x+1] === this.CELL_EMPTY
                    && map[yPoint_26438+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26438+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26438, yPoint_26438, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26438;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26438] === this.CELL_EMPTY
                    && map[y+1][xPoint_26438] === this.CELL_EMPTY
                    && map[y+1][xPoint_26438+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26438] === this.CELL_EMPTY
                    && map[y][xPoint_26438-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26438-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
