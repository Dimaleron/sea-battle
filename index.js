...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31467=0;yPoint_31467<this.gameFieldBorderY.length; yPoint_31467++){
                for(var xPoint_31467=0;xPoint_31467<this.gameFieldBorderX.length; xPoint_31467++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31467, xPoint_31467);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31467][xPoint_31467] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31467, xPoint_31467, 'user');
                    if(this._userShipsMap[yPoint_31467][xPoint_31467] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31467, xPoint_31467, type){
            var id = this.getPointBlockIdByCoords(yPoint_31467, xPoint_31467, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31467);
                block.setAttribute('data-y', yPoint_31467);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31467
         * @param {type} xPoint_31467
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31467, xPoint_31467, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31467 + '_y' + yPoint_31467;
            }
            return 'pc_x' + xPoint_31467 + '_y' + yPoint_31467;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31467=0;yPoint_31467<this.gameFieldBorderY.length; yPoint_31467++){
                for(var xPoint_31467=0;xPoint_31467<this.gameFieldBorderX.length; xPoint_31467++){
                    map.push({y: yPoint_31467, x: xPoint_31467});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31467=-1;yPoint_31467<(this.gameFieldBorderY.length+1); yPoint_31467++){
                for(var xPoint_31467=-1;xPoint_31467<(this.gameFieldBorderX.length+1); xPoint_31467++){
                    if(!map[yPoint_31467]){
                        map[yPoint_31467] = [];
                    }
                    map[yPoint_31467][xPoint_31467] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31467 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31467 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31467, yPoint_31467) === true){
                    if(this.canPutHorizontal(map, xPoint_31467, yPoint_31467, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31467][xPoint_31467 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31467, yPoint_31467, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31467 + i][xPoint_31467] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31467, yPoint_31467){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31467][xPoint_31467] === this.CELL_EMPTY
                && map[yPoint_31467-1][xPoint_31467] === this.CELL_EMPTY
                && map[yPoint_31467-1][xPoint_31467+1] === this.CELL_EMPTY
                && map[yPoint_31467][xPoint_31467+1] === this.CELL_EMPTY
                && map[yPoint_31467+1][xPoint_31467+1] === this.CELL_EMPTY
                && map[yPoint_31467+1][xPoint_31467] === this.CELL_EMPTY
                && map[yPoint_31467+1][xPoint_31467-1] === this.CELL_EMPTY
                && map[yPoint_31467][xPoint_31467-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31467, yPoint_31467, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31467;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31467][x] === this.CELL_EMPTY
                    && map[yPoint_31467-1][x] === this.CELL_EMPTY
                    && map[yPoint_31467-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31467][x+1] === this.CELL_EMPTY
                    && map[yPoint_31467+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31467+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31467, yPoint_31467, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31467;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31467] === this.CELL_EMPTY
                    && map[y+1][xPoint_31467] === this.CELL_EMPTY
                    && map[y+1][xPoint_31467+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31467] === this.CELL_EMPTY
                    && map[y][xPoint_31467-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31467-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
