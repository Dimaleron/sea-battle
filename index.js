...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23559=0;yPoint_23559<this.gameFieldBorderY.length; yPoint_23559++){
                for(var xPoint_23559=0;xPoint_23559<this.gameFieldBorderX.length; xPoint_23559++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23559, xPoint_23559);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23559][xPoint_23559] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23559, xPoint_23559, 'user');
                    if(this._userShipsMap[yPoint_23559][xPoint_23559] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23559, xPoint_23559, type){
            var id = this.getPointBlockIdByCoords(yPoint_23559, xPoint_23559, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23559);
                block.setAttribute('data-y', yPoint_23559);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23559
         * @param {type} xPoint_23559
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23559, xPoint_23559, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23559 + '_y' + yPoint_23559;
            }
            return 'pc_x' + xPoint_23559 + '_y' + yPoint_23559;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23559=0;yPoint_23559<this.gameFieldBorderY.length; yPoint_23559++){
                for(var xPoint_23559=0;xPoint_23559<this.gameFieldBorderX.length; xPoint_23559++){
                    map.push({y: yPoint_23559, x: xPoint_23559});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23559=-1;yPoint_23559<(this.gameFieldBorderY.length+1); yPoint_23559++){
                for(var xPoint_23559=-1;xPoint_23559<(this.gameFieldBorderX.length+1); xPoint_23559++){
                    if(!map[yPoint_23559]){
                        map[yPoint_23559] = [];
                    }
                    map[yPoint_23559][xPoint_23559] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23559 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23559 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23559, yPoint_23559) === true){
                    if(this.canPutHorizontal(map, xPoint_23559, yPoint_23559, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23559][xPoint_23559 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23559, yPoint_23559, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23559 + i][xPoint_23559] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23559, yPoint_23559){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23559][xPoint_23559] === this.CELL_EMPTY
                && map[yPoint_23559-1][xPoint_23559] === this.CELL_EMPTY
                && map[yPoint_23559-1][xPoint_23559+1] === this.CELL_EMPTY
                && map[yPoint_23559][xPoint_23559+1] === this.CELL_EMPTY
                && map[yPoint_23559+1][xPoint_23559+1] === this.CELL_EMPTY
                && map[yPoint_23559+1][xPoint_23559] === this.CELL_EMPTY
                && map[yPoint_23559+1][xPoint_23559-1] === this.CELL_EMPTY
                && map[yPoint_23559][xPoint_23559-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23559, yPoint_23559, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23559;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23559][x] === this.CELL_EMPTY
                    && map[yPoint_23559-1][x] === this.CELL_EMPTY
                    && map[yPoint_23559-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23559][x+1] === this.CELL_EMPTY
                    && map[yPoint_23559+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23559+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23559, yPoint_23559, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23559;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23559] === this.CELL_EMPTY
                    && map[y+1][xPoint_23559] === this.CELL_EMPTY
                    && map[y+1][xPoint_23559+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23559] === this.CELL_EMPTY
                    && map[y][xPoint_23559-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23559-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
