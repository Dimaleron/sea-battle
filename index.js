...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_600=0;yPoint_600<this.gameFieldBorderY.length; yPoint_600++){
                for(var xPoint_600=0;xPoint_600<this.gameFieldBorderX.length; xPoint_600++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_600, xPoint_600);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_600][xPoint_600] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_600, xPoint_600, 'user');
                    if(this._userShipsMap[yPoint_600][xPoint_600] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_600, xPoint_600, type){
            var id = this.getPointBlockIdByCoords(yPoint_600, xPoint_600, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_600);
                block.setAttribute('data-y', yPoint_600);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_600
         * @param {type} xPoint_600
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_600, xPoint_600, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_600 + '_y' + yPoint_600;
            }
            return 'pc_x' + xPoint_600 + '_y' + yPoint_600;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_600=0;yPoint_600<this.gameFieldBorderY.length; yPoint_600++){
                for(var xPoint_600=0;xPoint_600<this.gameFieldBorderX.length; xPoint_600++){
                    map.push({y: yPoint_600, x: xPoint_600});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_600=-1;yPoint_600<(this.gameFieldBorderY.length+1); yPoint_600++){
                for(var xPoint_600=-1;xPoint_600<(this.gameFieldBorderX.length+1); xPoint_600++){
                    if(!map[yPoint_600]){
                        map[yPoint_600] = [];
                    }
                    map[yPoint_600][xPoint_600] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_600 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_600 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_600, yPoint_600) === true){
                    if(this.canPutHorizontal(map, xPoint_600, yPoint_600, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_600][xPoint_600 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_600, yPoint_600, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_600 + i][xPoint_600] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_600, yPoint_600){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_600][xPoint_600] === this.CELL_EMPTY
                && map[yPoint_600-1][xPoint_600] === this.CELL_EMPTY
                && map[yPoint_600-1][xPoint_600+1] === this.CELL_EMPTY
                && map[yPoint_600][xPoint_600+1] === this.CELL_EMPTY
                && map[yPoint_600+1][xPoint_600+1] === this.CELL_EMPTY
                && map[yPoint_600+1][xPoint_600] === this.CELL_EMPTY
                && map[yPoint_600+1][xPoint_600-1] === this.CELL_EMPTY
                && map[yPoint_600][xPoint_600-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_600, yPoint_600, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_600;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_600][x] === this.CELL_EMPTY
                    && map[yPoint_600-1][x] === this.CELL_EMPTY
                    && map[yPoint_600-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_600][x+1] === this.CELL_EMPTY
                    && map[yPoint_600+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_600+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_600, yPoint_600, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_600;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_600] === this.CELL_EMPTY
                    && map[y+1][xPoint_600] === this.CELL_EMPTY
                    && map[y+1][xPoint_600+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_600] === this.CELL_EMPTY
                    && map[y][xPoint_600-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_600-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
