...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13719=0;yPoint_13719<this.gameFieldBorderY.length; yPoint_13719++){
                for(var xPoint_13719=0;xPoint_13719<this.gameFieldBorderX.length; xPoint_13719++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13719, xPoint_13719);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13719][xPoint_13719] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13719, xPoint_13719, 'user');
                    if(this._userShipsMap[yPoint_13719][xPoint_13719] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13719, xPoint_13719, type){
            var id = this.getPointBlockIdByCoords(yPoint_13719, xPoint_13719, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13719);
                block.setAttribute('data-y', yPoint_13719);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13719
         * @param {type} xPoint_13719
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13719, xPoint_13719, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13719 + '_y' + yPoint_13719;
            }
            return 'pc_x' + xPoint_13719 + '_y' + yPoint_13719;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13719=0;yPoint_13719<this.gameFieldBorderY.length; yPoint_13719++){
                for(var xPoint_13719=0;xPoint_13719<this.gameFieldBorderX.length; xPoint_13719++){
                    map.push({y: yPoint_13719, x: xPoint_13719});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13719=-1;yPoint_13719<(this.gameFieldBorderY.length+1); yPoint_13719++){
                for(var xPoint_13719=-1;xPoint_13719<(this.gameFieldBorderX.length+1); xPoint_13719++){
                    if(!map[yPoint_13719]){
                        map[yPoint_13719] = [];
                    }
                    map[yPoint_13719][xPoint_13719] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13719 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13719 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13719, yPoint_13719) === true){
                    if(this.canPutHorizontal(map, xPoint_13719, yPoint_13719, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13719][xPoint_13719 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13719, yPoint_13719, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13719 + i][xPoint_13719] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13719, yPoint_13719){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13719][xPoint_13719] === this.CELL_EMPTY
                && map[yPoint_13719-1][xPoint_13719] === this.CELL_EMPTY
                && map[yPoint_13719-1][xPoint_13719+1] === this.CELL_EMPTY
                && map[yPoint_13719][xPoint_13719+1] === this.CELL_EMPTY
                && map[yPoint_13719+1][xPoint_13719+1] === this.CELL_EMPTY
                && map[yPoint_13719+1][xPoint_13719] === this.CELL_EMPTY
                && map[yPoint_13719+1][xPoint_13719-1] === this.CELL_EMPTY
                && map[yPoint_13719][xPoint_13719-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13719, yPoint_13719, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13719;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13719][x] === this.CELL_EMPTY
                    && map[yPoint_13719-1][x] === this.CELL_EMPTY
                    && map[yPoint_13719-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13719][x+1] === this.CELL_EMPTY
                    && map[yPoint_13719+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13719+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13719, yPoint_13719, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13719;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13719] === this.CELL_EMPTY
                    && map[y+1][xPoint_13719] === this.CELL_EMPTY
                    && map[y+1][xPoint_13719+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13719] === this.CELL_EMPTY
                    && map[y][xPoint_13719-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13719-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
