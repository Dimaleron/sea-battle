...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11598=0;yPoint_11598<this.gameFieldBorderY.length; yPoint_11598++){
                for(var xPoint_11598=0;xPoint_11598<this.gameFieldBorderX.length; xPoint_11598++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11598, xPoint_11598);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11598][xPoint_11598] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11598, xPoint_11598, 'user');
                    if(this._userShipsMap[yPoint_11598][xPoint_11598] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11598, xPoint_11598, type){
            var id = this.getPointBlockIdByCoords(yPoint_11598, xPoint_11598, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11598);
                block.setAttribute('data-y', yPoint_11598);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11598
         * @param {type} xPoint_11598
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11598, xPoint_11598, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11598 + '_y' + yPoint_11598;
            }
            return 'pc_x' + xPoint_11598 + '_y' + yPoint_11598;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11598=0;yPoint_11598<this.gameFieldBorderY.length; yPoint_11598++){
                for(var xPoint_11598=0;xPoint_11598<this.gameFieldBorderX.length; xPoint_11598++){
                    map.push({y: yPoint_11598, x: xPoint_11598});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11598=-1;yPoint_11598<(this.gameFieldBorderY.length+1); yPoint_11598++){
                for(var xPoint_11598=-1;xPoint_11598<(this.gameFieldBorderX.length+1); xPoint_11598++){
                    if(!map[yPoint_11598]){
                        map[yPoint_11598] = [];
                    }
                    map[yPoint_11598][xPoint_11598] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11598 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11598 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11598, yPoint_11598) === true){
                    if(this.canPutHorizontal(map, xPoint_11598, yPoint_11598, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11598][xPoint_11598 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11598, yPoint_11598, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11598 + i][xPoint_11598] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11598, yPoint_11598){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11598][xPoint_11598] === this.CELL_EMPTY
                && map[yPoint_11598-1][xPoint_11598] === this.CELL_EMPTY
                && map[yPoint_11598-1][xPoint_11598+1] === this.CELL_EMPTY
                && map[yPoint_11598][xPoint_11598+1] === this.CELL_EMPTY
                && map[yPoint_11598+1][xPoint_11598+1] === this.CELL_EMPTY
                && map[yPoint_11598+1][xPoint_11598] === this.CELL_EMPTY
                && map[yPoint_11598+1][xPoint_11598-1] === this.CELL_EMPTY
                && map[yPoint_11598][xPoint_11598-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11598, yPoint_11598, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11598;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11598][x] === this.CELL_EMPTY
                    && map[yPoint_11598-1][x] === this.CELL_EMPTY
                    && map[yPoint_11598-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11598][x+1] === this.CELL_EMPTY
                    && map[yPoint_11598+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11598+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11598, yPoint_11598, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11598;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11598] === this.CELL_EMPTY
                    && map[y+1][xPoint_11598] === this.CELL_EMPTY
                    && map[y+1][xPoint_11598+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11598] === this.CELL_EMPTY
                    && map[y][xPoint_11598-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11598-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
