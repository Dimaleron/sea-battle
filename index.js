...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_48=0;yPoint_48<this.gameFieldBorderY.length; yPoint_48++){
                for(var xPoint_48=0;xPoint_48<this.gameFieldBorderX.length; xPoint_48++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_48, xPoint_48);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_48][xPoint_48] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_48, xPoint_48, 'user');
                    if(this._userShipsMap[yPoint_48][xPoint_48] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_48, xPoint_48, type){
            var id = this.getPointBlockIdByCoords(yPoint_48, xPoint_48, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_48);
                block.setAttribute('data-y', yPoint_48);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_48
         * @param {type} xPoint_48
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_48, xPoint_48, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_48 + '_y' + yPoint_48;
            }
            return 'pc_x' + xPoint_48 + '_y' + yPoint_48;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_48=0;yPoint_48<this.gameFieldBorderY.length; yPoint_48++){
                for(var xPoint_48=0;xPoint_48<this.gameFieldBorderX.length; xPoint_48++){
                    map.push({y: yPoint_48, x: xPoint_48});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_48=-1;yPoint_48<(this.gameFieldBorderY.length+1); yPoint_48++){
                for(var xPoint_48=-1;xPoint_48<(this.gameFieldBorderX.length+1); xPoint_48++){
                    if(!map[yPoint_48]){
                        map[yPoint_48] = [];
                    }
                    map[yPoint_48][xPoint_48] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_48 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_48 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_48, yPoint_48) === true){
                    if(this.canPutHorizontal(map, xPoint_48, yPoint_48, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_48][xPoint_48 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_48, yPoint_48, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_48 + i][xPoint_48] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_48, yPoint_48){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_48][xPoint_48] === this.CELL_EMPTY
                && map[yPoint_48-1][xPoint_48] === this.CELL_EMPTY
                && map[yPoint_48-1][xPoint_48+1] === this.CELL_EMPTY
                && map[yPoint_48][xPoint_48+1] === this.CELL_EMPTY
                && map[yPoint_48+1][xPoint_48+1] === this.CELL_EMPTY
                && map[yPoint_48+1][xPoint_48] === this.CELL_EMPTY
                && map[yPoint_48+1][xPoint_48-1] === this.CELL_EMPTY
                && map[yPoint_48][xPoint_48-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_48, yPoint_48, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_48;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_48][x] === this.CELL_EMPTY
                    && map[yPoint_48-1][x] === this.CELL_EMPTY
                    && map[yPoint_48-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_48][x+1] === this.CELL_EMPTY
                    && map[yPoint_48+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_48+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_48, yPoint_48, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_48;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_48] === this.CELL_EMPTY
                    && map[y+1][xPoint_48] === this.CELL_EMPTY
                    && map[y+1][xPoint_48+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_48] === this.CELL_EMPTY
                    && map[y][xPoint_48-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_48-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
