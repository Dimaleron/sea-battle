...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23012=0;yPoint_23012<this.gameFieldBorderY.length; yPoint_23012++){
                for(var xPoint_23012=0;xPoint_23012<this.gameFieldBorderX.length; xPoint_23012++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23012, xPoint_23012);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23012][xPoint_23012] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23012, xPoint_23012, 'user');
                    if(this._userShipsMap[yPoint_23012][xPoint_23012] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23012, xPoint_23012, type){
            var id = this.getPointBlockIdByCoords(yPoint_23012, xPoint_23012, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23012);
                block.setAttribute('data-y', yPoint_23012);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23012
         * @param {type} xPoint_23012
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23012, xPoint_23012, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23012 + '_y' + yPoint_23012;
            }
            return 'pc_x' + xPoint_23012 + '_y' + yPoint_23012;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23012=0;yPoint_23012<this.gameFieldBorderY.length; yPoint_23012++){
                for(var xPoint_23012=0;xPoint_23012<this.gameFieldBorderX.length; xPoint_23012++){
                    map.push({y: yPoint_23012, x: xPoint_23012});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23012=-1;yPoint_23012<(this.gameFieldBorderY.length+1); yPoint_23012++){
                for(var xPoint_23012=-1;xPoint_23012<(this.gameFieldBorderX.length+1); xPoint_23012++){
                    if(!map[yPoint_23012]){
                        map[yPoint_23012] = [];
                    }
                    map[yPoint_23012][xPoint_23012] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23012 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23012 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23012, yPoint_23012) === true){
                    if(this.canPutHorizontal(map, xPoint_23012, yPoint_23012, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23012][xPoint_23012 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23012, yPoint_23012, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23012 + i][xPoint_23012] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23012, yPoint_23012){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23012][xPoint_23012] === this.CELL_EMPTY
                && map[yPoint_23012-1][xPoint_23012] === this.CELL_EMPTY
                && map[yPoint_23012-1][xPoint_23012+1] === this.CELL_EMPTY
                && map[yPoint_23012][xPoint_23012+1] === this.CELL_EMPTY
                && map[yPoint_23012+1][xPoint_23012+1] === this.CELL_EMPTY
                && map[yPoint_23012+1][xPoint_23012] === this.CELL_EMPTY
                && map[yPoint_23012+1][xPoint_23012-1] === this.CELL_EMPTY
                && map[yPoint_23012][xPoint_23012-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23012, yPoint_23012, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23012;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23012][x] === this.CELL_EMPTY
                    && map[yPoint_23012-1][x] === this.CELL_EMPTY
                    && map[yPoint_23012-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23012][x+1] === this.CELL_EMPTY
                    && map[yPoint_23012+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23012+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23012, yPoint_23012, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23012;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23012] === this.CELL_EMPTY
                    && map[y+1][xPoint_23012] === this.CELL_EMPTY
                    && map[y+1][xPoint_23012+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23012] === this.CELL_EMPTY
                    && map[y][xPoint_23012-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23012-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
