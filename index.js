...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1977=0;yPoint_1977<this.gameFieldBorderY.length; yPoint_1977++){
                for(var xPoint_1977=0;xPoint_1977<this.gameFieldBorderX.length; xPoint_1977++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1977, xPoint_1977);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1977][xPoint_1977] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1977, xPoint_1977, 'user');
                    if(this._userShipsMap[yPoint_1977][xPoint_1977] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1977, xPoint_1977, type){
            var id = this.getPointBlockIdByCoords(yPoint_1977, xPoint_1977, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1977);
                block.setAttribute('data-y', yPoint_1977);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1977
         * @param {type} xPoint_1977
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1977, xPoint_1977, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1977 + '_y' + yPoint_1977;
            }
            return 'pc_x' + xPoint_1977 + '_y' + yPoint_1977;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1977=0;yPoint_1977<this.gameFieldBorderY.length; yPoint_1977++){
                for(var xPoint_1977=0;xPoint_1977<this.gameFieldBorderX.length; xPoint_1977++){
                    map.push({y: yPoint_1977, x: xPoint_1977});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1977=-1;yPoint_1977<(this.gameFieldBorderY.length+1); yPoint_1977++){
                for(var xPoint_1977=-1;xPoint_1977<(this.gameFieldBorderX.length+1); xPoint_1977++){
                    if(!map[yPoint_1977]){
                        map[yPoint_1977] = [];
                    }
                    map[yPoint_1977][xPoint_1977] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1977 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1977 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1977, yPoint_1977) === true){
                    if(this.canPutHorizontal(map, xPoint_1977, yPoint_1977, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1977][xPoint_1977 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1977, yPoint_1977, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1977 + i][xPoint_1977] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1977, yPoint_1977){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1977][xPoint_1977] === this.CELL_EMPTY
                && map[yPoint_1977-1][xPoint_1977] === this.CELL_EMPTY
                && map[yPoint_1977-1][xPoint_1977+1] === this.CELL_EMPTY
                && map[yPoint_1977][xPoint_1977+1] === this.CELL_EMPTY
                && map[yPoint_1977+1][xPoint_1977+1] === this.CELL_EMPTY
                && map[yPoint_1977+1][xPoint_1977] === this.CELL_EMPTY
                && map[yPoint_1977+1][xPoint_1977-1] === this.CELL_EMPTY
                && map[yPoint_1977][xPoint_1977-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1977, yPoint_1977, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1977;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1977][x] === this.CELL_EMPTY
                    && map[yPoint_1977-1][x] === this.CELL_EMPTY
                    && map[yPoint_1977-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1977][x+1] === this.CELL_EMPTY
                    && map[yPoint_1977+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1977+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1977, yPoint_1977, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1977;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1977] === this.CELL_EMPTY
                    && map[y+1][xPoint_1977] === this.CELL_EMPTY
                    && map[y+1][xPoint_1977+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1977] === this.CELL_EMPTY
                    && map[y][xPoint_1977-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1977-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
