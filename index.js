...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20663=0;yPoint_20663<this.gameFieldBorderY.length; yPoint_20663++){
                for(var xPoint_20663=0;xPoint_20663<this.gameFieldBorderX.length; xPoint_20663++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20663, xPoint_20663);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20663][xPoint_20663] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20663, xPoint_20663, 'user');
                    if(this._userShipsMap[yPoint_20663][xPoint_20663] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20663, xPoint_20663, type){
            var id = this.getPointBlockIdByCoords(yPoint_20663, xPoint_20663, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20663);
                block.setAttribute('data-y', yPoint_20663);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20663
         * @param {type} xPoint_20663
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20663, xPoint_20663, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20663 + '_y' + yPoint_20663;
            }
            return 'pc_x' + xPoint_20663 + '_y' + yPoint_20663;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20663=0;yPoint_20663<this.gameFieldBorderY.length; yPoint_20663++){
                for(var xPoint_20663=0;xPoint_20663<this.gameFieldBorderX.length; xPoint_20663++){
                    map.push({y: yPoint_20663, x: xPoint_20663});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20663=-1;yPoint_20663<(this.gameFieldBorderY.length+1); yPoint_20663++){
                for(var xPoint_20663=-1;xPoint_20663<(this.gameFieldBorderX.length+1); xPoint_20663++){
                    if(!map[yPoint_20663]){
                        map[yPoint_20663] = [];
                    }
                    map[yPoint_20663][xPoint_20663] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20663 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20663 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20663, yPoint_20663) === true){
                    if(this.canPutHorizontal(map, xPoint_20663, yPoint_20663, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20663][xPoint_20663 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20663, yPoint_20663, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20663 + i][xPoint_20663] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20663, yPoint_20663){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20663][xPoint_20663] === this.CELL_EMPTY
                && map[yPoint_20663-1][xPoint_20663] === this.CELL_EMPTY
                && map[yPoint_20663-1][xPoint_20663+1] === this.CELL_EMPTY
                && map[yPoint_20663][xPoint_20663+1] === this.CELL_EMPTY
                && map[yPoint_20663+1][xPoint_20663+1] === this.CELL_EMPTY
                && map[yPoint_20663+1][xPoint_20663] === this.CELL_EMPTY
                && map[yPoint_20663+1][xPoint_20663-1] === this.CELL_EMPTY
                && map[yPoint_20663][xPoint_20663-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20663, yPoint_20663, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20663;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20663][x] === this.CELL_EMPTY
                    && map[yPoint_20663-1][x] === this.CELL_EMPTY
                    && map[yPoint_20663-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20663][x+1] === this.CELL_EMPTY
                    && map[yPoint_20663+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20663+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20663, yPoint_20663, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20663;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20663] === this.CELL_EMPTY
                    && map[y+1][xPoint_20663] === this.CELL_EMPTY
                    && map[y+1][xPoint_20663+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20663] === this.CELL_EMPTY
                    && map[y][xPoint_20663-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20663-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
