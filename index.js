...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24208=0;yPoint_24208<this.gameFieldBorderY.length; yPoint_24208++){
                for(var xPoint_24208=0;xPoint_24208<this.gameFieldBorderX.length; xPoint_24208++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24208, xPoint_24208);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24208][xPoint_24208] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24208, xPoint_24208, 'user');
                    if(this._userShipsMap[yPoint_24208][xPoint_24208] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24208, xPoint_24208, type){
            var id = this.getPointBlockIdByCoords(yPoint_24208, xPoint_24208, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24208);
                block.setAttribute('data-y', yPoint_24208);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24208
         * @param {type} xPoint_24208
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24208, xPoint_24208, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24208 + '_y' + yPoint_24208;
            }
            return 'pc_x' + xPoint_24208 + '_y' + yPoint_24208;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24208=0;yPoint_24208<this.gameFieldBorderY.length; yPoint_24208++){
                for(var xPoint_24208=0;xPoint_24208<this.gameFieldBorderX.length; xPoint_24208++){
                    map.push({y: yPoint_24208, x: xPoint_24208});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24208=-1;yPoint_24208<(this.gameFieldBorderY.length+1); yPoint_24208++){
                for(var xPoint_24208=-1;xPoint_24208<(this.gameFieldBorderX.length+1); xPoint_24208++){
                    if(!map[yPoint_24208]){
                        map[yPoint_24208] = [];
                    }
                    map[yPoint_24208][xPoint_24208] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24208 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24208 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24208, yPoint_24208) === true){
                    if(this.canPutHorizontal(map, xPoint_24208, yPoint_24208, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24208][xPoint_24208 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24208, yPoint_24208, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24208 + i][xPoint_24208] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24208, yPoint_24208){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24208][xPoint_24208] === this.CELL_EMPTY
                && map[yPoint_24208-1][xPoint_24208] === this.CELL_EMPTY
                && map[yPoint_24208-1][xPoint_24208+1] === this.CELL_EMPTY
                && map[yPoint_24208][xPoint_24208+1] === this.CELL_EMPTY
                && map[yPoint_24208+1][xPoint_24208+1] === this.CELL_EMPTY
                && map[yPoint_24208+1][xPoint_24208] === this.CELL_EMPTY
                && map[yPoint_24208+1][xPoint_24208-1] === this.CELL_EMPTY
                && map[yPoint_24208][xPoint_24208-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24208, yPoint_24208, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24208;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24208][x] === this.CELL_EMPTY
                    && map[yPoint_24208-1][x] === this.CELL_EMPTY
                    && map[yPoint_24208-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24208][x+1] === this.CELL_EMPTY
                    && map[yPoint_24208+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24208+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24208, yPoint_24208, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24208;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24208] === this.CELL_EMPTY
                    && map[y+1][xPoint_24208] === this.CELL_EMPTY
                    && map[y+1][xPoint_24208+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24208] === this.CELL_EMPTY
                    && map[y][xPoint_24208-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24208-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
