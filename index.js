...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2546=0;yPoint_2546<this.gameFieldBorderY.length; yPoint_2546++){
                for(var xPoint_2546=0;xPoint_2546<this.gameFieldBorderX.length; xPoint_2546++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2546, xPoint_2546);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2546][xPoint_2546] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2546, xPoint_2546, 'user');
                    if(this._userShipsMap[yPoint_2546][xPoint_2546] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2546, xPoint_2546, type){
            var id = this.getPointBlockIdByCoords(yPoint_2546, xPoint_2546, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2546);
                block.setAttribute('data-y', yPoint_2546);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2546
         * @param {type} xPoint_2546
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2546, xPoint_2546, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2546 + '_y' + yPoint_2546;
            }
            return 'pc_x' + xPoint_2546 + '_y' + yPoint_2546;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2546=0;yPoint_2546<this.gameFieldBorderY.length; yPoint_2546++){
                for(var xPoint_2546=0;xPoint_2546<this.gameFieldBorderX.length; xPoint_2546++){
                    map.push({y: yPoint_2546, x: xPoint_2546});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2546=-1;yPoint_2546<(this.gameFieldBorderY.length+1); yPoint_2546++){
                for(var xPoint_2546=-1;xPoint_2546<(this.gameFieldBorderX.length+1); xPoint_2546++){
                    if(!map[yPoint_2546]){
                        map[yPoint_2546] = [];
                    }
                    map[yPoint_2546][xPoint_2546] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2546 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2546 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2546, yPoint_2546) === true){
                    if(this.canPutHorizontal(map, xPoint_2546, yPoint_2546, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2546][xPoint_2546 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2546, yPoint_2546, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2546 + i][xPoint_2546] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2546, yPoint_2546){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2546][xPoint_2546] === this.CELL_EMPTY
                && map[yPoint_2546-1][xPoint_2546] === this.CELL_EMPTY
                && map[yPoint_2546-1][xPoint_2546+1] === this.CELL_EMPTY
                && map[yPoint_2546][xPoint_2546+1] === this.CELL_EMPTY
                && map[yPoint_2546+1][xPoint_2546+1] === this.CELL_EMPTY
                && map[yPoint_2546+1][xPoint_2546] === this.CELL_EMPTY
                && map[yPoint_2546+1][xPoint_2546-1] === this.CELL_EMPTY
                && map[yPoint_2546][xPoint_2546-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2546, yPoint_2546, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2546;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2546][x] === this.CELL_EMPTY
                    && map[yPoint_2546-1][x] === this.CELL_EMPTY
                    && map[yPoint_2546-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2546][x+1] === this.CELL_EMPTY
                    && map[yPoint_2546+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2546+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2546, yPoint_2546, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2546;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2546] === this.CELL_EMPTY
                    && map[y+1][xPoint_2546] === this.CELL_EMPTY
                    && map[y+1][xPoint_2546+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2546] === this.CELL_EMPTY
                    && map[y][xPoint_2546-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2546-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
