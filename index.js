...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3205=0;yPoint_3205<this.gameFieldBorderY.length; yPoint_3205++){
                for(var xPoint_3205=0;xPoint_3205<this.gameFieldBorderX.length; xPoint_3205++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3205, xPoint_3205);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3205][xPoint_3205] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3205, xPoint_3205, 'user');
                    if(this._userShipsMap[yPoint_3205][xPoint_3205] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3205, xPoint_3205, type){
            var id = this.getPointBlockIdByCoords(yPoint_3205, xPoint_3205, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3205);
                block.setAttribute('data-y', yPoint_3205);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3205
         * @param {type} xPoint_3205
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3205, xPoint_3205, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3205 + '_y' + yPoint_3205;
            }
            return 'pc_x' + xPoint_3205 + '_y' + yPoint_3205;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3205=0;yPoint_3205<this.gameFieldBorderY.length; yPoint_3205++){
                for(var xPoint_3205=0;xPoint_3205<this.gameFieldBorderX.length; xPoint_3205++){
                    map.push({y: yPoint_3205, x: xPoint_3205});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3205=-1;yPoint_3205<(this.gameFieldBorderY.length+1); yPoint_3205++){
                for(var xPoint_3205=-1;xPoint_3205<(this.gameFieldBorderX.length+1); xPoint_3205++){
                    if(!map[yPoint_3205]){
                        map[yPoint_3205] = [];
                    }
                    map[yPoint_3205][xPoint_3205] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3205 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3205 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3205, yPoint_3205) === true){
                    if(this.canPutHorizontal(map, xPoint_3205, yPoint_3205, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3205][xPoint_3205 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3205, yPoint_3205, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3205 + i][xPoint_3205] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3205, yPoint_3205){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3205][xPoint_3205] === this.CELL_EMPTY
                && map[yPoint_3205-1][xPoint_3205] === this.CELL_EMPTY
                && map[yPoint_3205-1][xPoint_3205+1] === this.CELL_EMPTY
                && map[yPoint_3205][xPoint_3205+1] === this.CELL_EMPTY
                && map[yPoint_3205+1][xPoint_3205+1] === this.CELL_EMPTY
                && map[yPoint_3205+1][xPoint_3205] === this.CELL_EMPTY
                && map[yPoint_3205+1][xPoint_3205-1] === this.CELL_EMPTY
                && map[yPoint_3205][xPoint_3205-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3205, yPoint_3205, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3205;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3205][x] === this.CELL_EMPTY
                    && map[yPoint_3205-1][x] === this.CELL_EMPTY
                    && map[yPoint_3205-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3205][x+1] === this.CELL_EMPTY
                    && map[yPoint_3205+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3205+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3205, yPoint_3205, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3205;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3205] === this.CELL_EMPTY
                    && map[y+1][xPoint_3205] === this.CELL_EMPTY
                    && map[y+1][xPoint_3205+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3205] === this.CELL_EMPTY
                    && map[y][xPoint_3205-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3205-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
