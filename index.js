...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23788=0;yPoint_23788<this.gameFieldBorderY.length; yPoint_23788++){
                for(var xPoint_23788=0;xPoint_23788<this.gameFieldBorderX.length; xPoint_23788++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23788, xPoint_23788);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23788][xPoint_23788] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23788, xPoint_23788, 'user');
                    if(this._userShipsMap[yPoint_23788][xPoint_23788] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23788, xPoint_23788, type){
            var id = this.getPointBlockIdByCoords(yPoint_23788, xPoint_23788, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23788);
                block.setAttribute('data-y', yPoint_23788);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23788
         * @param {type} xPoint_23788
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23788, xPoint_23788, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23788 + '_y' + yPoint_23788;
            }
            return 'pc_x' + xPoint_23788 + '_y' + yPoint_23788;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23788=0;yPoint_23788<this.gameFieldBorderY.length; yPoint_23788++){
                for(var xPoint_23788=0;xPoint_23788<this.gameFieldBorderX.length; xPoint_23788++){
                    map.push({y: yPoint_23788, x: xPoint_23788});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23788=-1;yPoint_23788<(this.gameFieldBorderY.length+1); yPoint_23788++){
                for(var xPoint_23788=-1;xPoint_23788<(this.gameFieldBorderX.length+1); xPoint_23788++){
                    if(!map[yPoint_23788]){
                        map[yPoint_23788] = [];
                    }
                    map[yPoint_23788][xPoint_23788] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23788 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23788 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23788, yPoint_23788) === true){
                    if(this.canPutHorizontal(map, xPoint_23788, yPoint_23788, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23788][xPoint_23788 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23788, yPoint_23788, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23788 + i][xPoint_23788] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23788, yPoint_23788){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23788][xPoint_23788] === this.CELL_EMPTY
                && map[yPoint_23788-1][xPoint_23788] === this.CELL_EMPTY
                && map[yPoint_23788-1][xPoint_23788+1] === this.CELL_EMPTY
                && map[yPoint_23788][xPoint_23788+1] === this.CELL_EMPTY
                && map[yPoint_23788+1][xPoint_23788+1] === this.CELL_EMPTY
                && map[yPoint_23788+1][xPoint_23788] === this.CELL_EMPTY
                && map[yPoint_23788+1][xPoint_23788-1] === this.CELL_EMPTY
                && map[yPoint_23788][xPoint_23788-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23788, yPoint_23788, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23788;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23788][x] === this.CELL_EMPTY
                    && map[yPoint_23788-1][x] === this.CELL_EMPTY
                    && map[yPoint_23788-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23788][x+1] === this.CELL_EMPTY
                    && map[yPoint_23788+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23788+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23788, yPoint_23788, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23788;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23788] === this.CELL_EMPTY
                    && map[y+1][xPoint_23788] === this.CELL_EMPTY
                    && map[y+1][xPoint_23788+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23788] === this.CELL_EMPTY
                    && map[y][xPoint_23788-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23788-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
