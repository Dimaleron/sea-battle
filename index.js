...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16920=0;yPoint_16920<this.gameFieldBorderY.length; yPoint_16920++){
                for(var xPoint_16920=0;xPoint_16920<this.gameFieldBorderX.length; xPoint_16920++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16920, xPoint_16920);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16920][xPoint_16920] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16920, xPoint_16920, 'user');
                    if(this._userShipsMap[yPoint_16920][xPoint_16920] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16920, xPoint_16920, type){
            var id = this.getPointBlockIdByCoords(yPoint_16920, xPoint_16920, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16920);
                block.setAttribute('data-y', yPoint_16920);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16920
         * @param {type} xPoint_16920
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16920, xPoint_16920, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16920 + '_y' + yPoint_16920;
            }
            return 'pc_x' + xPoint_16920 + '_y' + yPoint_16920;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16920=0;yPoint_16920<this.gameFieldBorderY.length; yPoint_16920++){
                for(var xPoint_16920=0;xPoint_16920<this.gameFieldBorderX.length; xPoint_16920++){
                    map.push({y: yPoint_16920, x: xPoint_16920});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16920=-1;yPoint_16920<(this.gameFieldBorderY.length+1); yPoint_16920++){
                for(var xPoint_16920=-1;xPoint_16920<(this.gameFieldBorderX.length+1); xPoint_16920++){
                    if(!map[yPoint_16920]){
                        map[yPoint_16920] = [];
                    }
                    map[yPoint_16920][xPoint_16920] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16920 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16920 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16920, yPoint_16920) === true){
                    if(this.canPutHorizontal(map, xPoint_16920, yPoint_16920, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16920][xPoint_16920 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16920, yPoint_16920, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16920 + i][xPoint_16920] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16920, yPoint_16920){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16920][xPoint_16920] === this.CELL_EMPTY
                && map[yPoint_16920-1][xPoint_16920] === this.CELL_EMPTY
                && map[yPoint_16920-1][xPoint_16920+1] === this.CELL_EMPTY
                && map[yPoint_16920][xPoint_16920+1] === this.CELL_EMPTY
                && map[yPoint_16920+1][xPoint_16920+1] === this.CELL_EMPTY
                && map[yPoint_16920+1][xPoint_16920] === this.CELL_EMPTY
                && map[yPoint_16920+1][xPoint_16920-1] === this.CELL_EMPTY
                && map[yPoint_16920][xPoint_16920-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16920, yPoint_16920, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16920;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16920][x] === this.CELL_EMPTY
                    && map[yPoint_16920-1][x] === this.CELL_EMPTY
                    && map[yPoint_16920-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16920][x+1] === this.CELL_EMPTY
                    && map[yPoint_16920+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16920+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16920, yPoint_16920, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16920;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16920] === this.CELL_EMPTY
                    && map[y+1][xPoint_16920] === this.CELL_EMPTY
                    && map[y+1][xPoint_16920+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16920] === this.CELL_EMPTY
                    && map[y][xPoint_16920-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16920-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
