...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4918=0;yPoint_4918<this.gameFieldBorderY.length; yPoint_4918++){
                for(var xPoint_4918=0;xPoint_4918<this.gameFieldBorderX.length; xPoint_4918++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4918, xPoint_4918);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4918][xPoint_4918] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4918, xPoint_4918, 'user');
                    if(this._userShipsMap[yPoint_4918][xPoint_4918] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4918, xPoint_4918, type){
            var id = this.getPointBlockIdByCoords(yPoint_4918, xPoint_4918, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4918);
                block.setAttribute('data-y', yPoint_4918);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4918
         * @param {type} xPoint_4918
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4918, xPoint_4918, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4918 + '_y' + yPoint_4918;
            }
            return 'pc_x' + xPoint_4918 + '_y' + yPoint_4918;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4918=0;yPoint_4918<this.gameFieldBorderY.length; yPoint_4918++){
                for(var xPoint_4918=0;xPoint_4918<this.gameFieldBorderX.length; xPoint_4918++){
                    map.push({y: yPoint_4918, x: xPoint_4918});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4918=-1;yPoint_4918<(this.gameFieldBorderY.length+1); yPoint_4918++){
                for(var xPoint_4918=-1;xPoint_4918<(this.gameFieldBorderX.length+1); xPoint_4918++){
                    if(!map[yPoint_4918]){
                        map[yPoint_4918] = [];
                    }
                    map[yPoint_4918][xPoint_4918] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4918 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4918 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4918, yPoint_4918) === true){
                    if(this.canPutHorizontal(map, xPoint_4918, yPoint_4918, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4918][xPoint_4918 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4918, yPoint_4918, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4918 + i][xPoint_4918] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4918, yPoint_4918){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4918][xPoint_4918] === this.CELL_EMPTY
                && map[yPoint_4918-1][xPoint_4918] === this.CELL_EMPTY
                && map[yPoint_4918-1][xPoint_4918+1] === this.CELL_EMPTY
                && map[yPoint_4918][xPoint_4918+1] === this.CELL_EMPTY
                && map[yPoint_4918+1][xPoint_4918+1] === this.CELL_EMPTY
                && map[yPoint_4918+1][xPoint_4918] === this.CELL_EMPTY
                && map[yPoint_4918+1][xPoint_4918-1] === this.CELL_EMPTY
                && map[yPoint_4918][xPoint_4918-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4918, yPoint_4918, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4918;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4918][x] === this.CELL_EMPTY
                    && map[yPoint_4918-1][x] === this.CELL_EMPTY
                    && map[yPoint_4918-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4918][x+1] === this.CELL_EMPTY
                    && map[yPoint_4918+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4918+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4918, yPoint_4918, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4918;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4918] === this.CELL_EMPTY
                    && map[y+1][xPoint_4918] === this.CELL_EMPTY
                    && map[y+1][xPoint_4918+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4918] === this.CELL_EMPTY
                    && map[y][xPoint_4918-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4918-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
