...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7359=0;yPoint_7359<this.gameFieldBorderY.length; yPoint_7359++){
                for(var xPoint_7359=0;xPoint_7359<this.gameFieldBorderX.length; xPoint_7359++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7359, xPoint_7359);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7359][xPoint_7359] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7359, xPoint_7359, 'user');
                    if(this._userShipsMap[yPoint_7359][xPoint_7359] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7359, xPoint_7359, type){
            var id = this.getPointBlockIdByCoords(yPoint_7359, xPoint_7359, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7359);
                block.setAttribute('data-y', yPoint_7359);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7359
         * @param {type} xPoint_7359
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7359, xPoint_7359, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7359 + '_y' + yPoint_7359;
            }
            return 'pc_x' + xPoint_7359 + '_y' + yPoint_7359;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7359=0;yPoint_7359<this.gameFieldBorderY.length; yPoint_7359++){
                for(var xPoint_7359=0;xPoint_7359<this.gameFieldBorderX.length; xPoint_7359++){
                    map.push({y: yPoint_7359, x: xPoint_7359});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7359=-1;yPoint_7359<(this.gameFieldBorderY.length+1); yPoint_7359++){
                for(var xPoint_7359=-1;xPoint_7359<(this.gameFieldBorderX.length+1); xPoint_7359++){
                    if(!map[yPoint_7359]){
                        map[yPoint_7359] = [];
                    }
                    map[yPoint_7359][xPoint_7359] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7359 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7359 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7359, yPoint_7359) === true){
                    if(this.canPutHorizontal(map, xPoint_7359, yPoint_7359, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7359][xPoint_7359 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7359, yPoint_7359, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7359 + i][xPoint_7359] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7359, yPoint_7359){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7359][xPoint_7359] === this.CELL_EMPTY
                && map[yPoint_7359-1][xPoint_7359] === this.CELL_EMPTY
                && map[yPoint_7359-1][xPoint_7359+1] === this.CELL_EMPTY
                && map[yPoint_7359][xPoint_7359+1] === this.CELL_EMPTY
                && map[yPoint_7359+1][xPoint_7359+1] === this.CELL_EMPTY
                && map[yPoint_7359+1][xPoint_7359] === this.CELL_EMPTY
                && map[yPoint_7359+1][xPoint_7359-1] === this.CELL_EMPTY
                && map[yPoint_7359][xPoint_7359-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7359, yPoint_7359, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7359;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7359][x] === this.CELL_EMPTY
                    && map[yPoint_7359-1][x] === this.CELL_EMPTY
                    && map[yPoint_7359-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7359][x+1] === this.CELL_EMPTY
                    && map[yPoint_7359+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7359+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7359, yPoint_7359, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7359;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7359] === this.CELL_EMPTY
                    && map[y+1][xPoint_7359] === this.CELL_EMPTY
                    && map[y+1][xPoint_7359+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7359] === this.CELL_EMPTY
                    && map[y][xPoint_7359-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7359-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
