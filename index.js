...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6282=0;yPoint_6282<this.gameFieldBorderY.length; yPoint_6282++){
                for(var xPoint_6282=0;xPoint_6282<this.gameFieldBorderX.length; xPoint_6282++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6282, xPoint_6282);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6282][xPoint_6282] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6282, xPoint_6282, 'user');
                    if(this._userShipsMap[yPoint_6282][xPoint_6282] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6282, xPoint_6282, type){
            var id = this.getPointBlockIdByCoords(yPoint_6282, xPoint_6282, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6282);
                block.setAttribute('data-y', yPoint_6282);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6282
         * @param {type} xPoint_6282
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6282, xPoint_6282, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6282 + '_y' + yPoint_6282;
            }
            return 'pc_x' + xPoint_6282 + '_y' + yPoint_6282;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6282=0;yPoint_6282<this.gameFieldBorderY.length; yPoint_6282++){
                for(var xPoint_6282=0;xPoint_6282<this.gameFieldBorderX.length; xPoint_6282++){
                    map.push({y: yPoint_6282, x: xPoint_6282});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6282=-1;yPoint_6282<(this.gameFieldBorderY.length+1); yPoint_6282++){
                for(var xPoint_6282=-1;xPoint_6282<(this.gameFieldBorderX.length+1); xPoint_6282++){
                    if(!map[yPoint_6282]){
                        map[yPoint_6282] = [];
                    }
                    map[yPoint_6282][xPoint_6282] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6282 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6282 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6282, yPoint_6282) === true){
                    if(this.canPutHorizontal(map, xPoint_6282, yPoint_6282, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6282][xPoint_6282 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6282, yPoint_6282, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6282 + i][xPoint_6282] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6282, yPoint_6282){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6282][xPoint_6282] === this.CELL_EMPTY
                && map[yPoint_6282-1][xPoint_6282] === this.CELL_EMPTY
                && map[yPoint_6282-1][xPoint_6282+1] === this.CELL_EMPTY
                && map[yPoint_6282][xPoint_6282+1] === this.CELL_EMPTY
                && map[yPoint_6282+1][xPoint_6282+1] === this.CELL_EMPTY
                && map[yPoint_6282+1][xPoint_6282] === this.CELL_EMPTY
                && map[yPoint_6282+1][xPoint_6282-1] === this.CELL_EMPTY
                && map[yPoint_6282][xPoint_6282-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6282, yPoint_6282, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6282;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6282][x] === this.CELL_EMPTY
                    && map[yPoint_6282-1][x] === this.CELL_EMPTY
                    && map[yPoint_6282-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6282][x+1] === this.CELL_EMPTY
                    && map[yPoint_6282+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6282+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6282, yPoint_6282, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6282;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6282] === this.CELL_EMPTY
                    && map[y+1][xPoint_6282] === this.CELL_EMPTY
                    && map[y+1][xPoint_6282+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6282] === this.CELL_EMPTY
                    && map[y][xPoint_6282-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6282-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
