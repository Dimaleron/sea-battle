...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21741=0;yPoint_21741<this.gameFieldBorderY.length; yPoint_21741++){
                for(var xPoint_21741=0;xPoint_21741<this.gameFieldBorderX.length; xPoint_21741++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21741, xPoint_21741);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21741][xPoint_21741] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21741, xPoint_21741, 'user');
                    if(this._userShipsMap[yPoint_21741][xPoint_21741] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21741, xPoint_21741, type){
            var id = this.getPointBlockIdByCoords(yPoint_21741, xPoint_21741, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21741);
                block.setAttribute('data-y', yPoint_21741);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21741
         * @param {type} xPoint_21741
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21741, xPoint_21741, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21741 + '_y' + yPoint_21741;
            }
            return 'pc_x' + xPoint_21741 + '_y' + yPoint_21741;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21741=0;yPoint_21741<this.gameFieldBorderY.length; yPoint_21741++){
                for(var xPoint_21741=0;xPoint_21741<this.gameFieldBorderX.length; xPoint_21741++){
                    map.push({y: yPoint_21741, x: xPoint_21741});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21741=-1;yPoint_21741<(this.gameFieldBorderY.length+1); yPoint_21741++){
                for(var xPoint_21741=-1;xPoint_21741<(this.gameFieldBorderX.length+1); xPoint_21741++){
                    if(!map[yPoint_21741]){
                        map[yPoint_21741] = [];
                    }
                    map[yPoint_21741][xPoint_21741] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21741 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21741 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21741, yPoint_21741) === true){
                    if(this.canPutHorizontal(map, xPoint_21741, yPoint_21741, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21741][xPoint_21741 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21741, yPoint_21741, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21741 + i][xPoint_21741] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21741, yPoint_21741){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21741][xPoint_21741] === this.CELL_EMPTY
                && map[yPoint_21741-1][xPoint_21741] === this.CELL_EMPTY
                && map[yPoint_21741-1][xPoint_21741+1] === this.CELL_EMPTY
                && map[yPoint_21741][xPoint_21741+1] === this.CELL_EMPTY
                && map[yPoint_21741+1][xPoint_21741+1] === this.CELL_EMPTY
                && map[yPoint_21741+1][xPoint_21741] === this.CELL_EMPTY
                && map[yPoint_21741+1][xPoint_21741-1] === this.CELL_EMPTY
                && map[yPoint_21741][xPoint_21741-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21741, yPoint_21741, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21741;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21741][x] === this.CELL_EMPTY
                    && map[yPoint_21741-1][x] === this.CELL_EMPTY
                    && map[yPoint_21741-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21741][x+1] === this.CELL_EMPTY
                    && map[yPoint_21741+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21741+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21741, yPoint_21741, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21741;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21741] === this.CELL_EMPTY
                    && map[y+1][xPoint_21741] === this.CELL_EMPTY
                    && map[y+1][xPoint_21741+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21741] === this.CELL_EMPTY
                    && map[y][xPoint_21741-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21741-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
