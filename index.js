...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28204=0;yPoint_28204<this.gameFieldBorderY.length; yPoint_28204++){
                for(var xPoint_28204=0;xPoint_28204<this.gameFieldBorderX.length; xPoint_28204++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28204, xPoint_28204);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28204][xPoint_28204] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28204, xPoint_28204, 'user');
                    if(this._userShipsMap[yPoint_28204][xPoint_28204] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28204, xPoint_28204, type){
            var id = this.getPointBlockIdByCoords(yPoint_28204, xPoint_28204, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28204);
                block.setAttribute('data-y', yPoint_28204);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28204
         * @param {type} xPoint_28204
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28204, xPoint_28204, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28204 + '_y' + yPoint_28204;
            }
            return 'pc_x' + xPoint_28204 + '_y' + yPoint_28204;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28204=0;yPoint_28204<this.gameFieldBorderY.length; yPoint_28204++){
                for(var xPoint_28204=0;xPoint_28204<this.gameFieldBorderX.length; xPoint_28204++){
                    map.push({y: yPoint_28204, x: xPoint_28204});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28204=-1;yPoint_28204<(this.gameFieldBorderY.length+1); yPoint_28204++){
                for(var xPoint_28204=-1;xPoint_28204<(this.gameFieldBorderX.length+1); xPoint_28204++){
                    if(!map[yPoint_28204]){
                        map[yPoint_28204] = [];
                    }
                    map[yPoint_28204][xPoint_28204] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28204 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28204 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28204, yPoint_28204) === true){
                    if(this.canPutHorizontal(map, xPoint_28204, yPoint_28204, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28204][xPoint_28204 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28204, yPoint_28204, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28204 + i][xPoint_28204] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28204, yPoint_28204){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28204][xPoint_28204] === this.CELL_EMPTY
                && map[yPoint_28204-1][xPoint_28204] === this.CELL_EMPTY
                && map[yPoint_28204-1][xPoint_28204+1] === this.CELL_EMPTY
                && map[yPoint_28204][xPoint_28204+1] === this.CELL_EMPTY
                && map[yPoint_28204+1][xPoint_28204+1] === this.CELL_EMPTY
                && map[yPoint_28204+1][xPoint_28204] === this.CELL_EMPTY
                && map[yPoint_28204+1][xPoint_28204-1] === this.CELL_EMPTY
                && map[yPoint_28204][xPoint_28204-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28204, yPoint_28204, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28204;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28204][x] === this.CELL_EMPTY
                    && map[yPoint_28204-1][x] === this.CELL_EMPTY
                    && map[yPoint_28204-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28204][x+1] === this.CELL_EMPTY
                    && map[yPoint_28204+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28204+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28204, yPoint_28204, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28204;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28204] === this.CELL_EMPTY
                    && map[y+1][xPoint_28204] === this.CELL_EMPTY
                    && map[y+1][xPoint_28204+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28204] === this.CELL_EMPTY
                    && map[y][xPoint_28204-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28204-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
