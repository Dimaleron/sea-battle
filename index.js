...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10296=0;yPoint_10296<this.gameFieldBorderY.length; yPoint_10296++){
                for(var xPoint_10296=0;xPoint_10296<this.gameFieldBorderX.length; xPoint_10296++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10296, xPoint_10296);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10296][xPoint_10296] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10296, xPoint_10296, 'user');
                    if(this._userShipsMap[yPoint_10296][xPoint_10296] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10296, xPoint_10296, type){
            var id = this.getPointBlockIdByCoords(yPoint_10296, xPoint_10296, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10296);
                block.setAttribute('data-y', yPoint_10296);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10296
         * @param {type} xPoint_10296
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10296, xPoint_10296, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10296 + '_y' + yPoint_10296;
            }
            return 'pc_x' + xPoint_10296 + '_y' + yPoint_10296;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10296=0;yPoint_10296<this.gameFieldBorderY.length; yPoint_10296++){
                for(var xPoint_10296=0;xPoint_10296<this.gameFieldBorderX.length; xPoint_10296++){
                    map.push({y: yPoint_10296, x: xPoint_10296});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10296=-1;yPoint_10296<(this.gameFieldBorderY.length+1); yPoint_10296++){
                for(var xPoint_10296=-1;xPoint_10296<(this.gameFieldBorderX.length+1); xPoint_10296++){
                    if(!map[yPoint_10296]){
                        map[yPoint_10296] = [];
                    }
                    map[yPoint_10296][xPoint_10296] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10296 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10296 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10296, yPoint_10296) === true){
                    if(this.canPutHorizontal(map, xPoint_10296, yPoint_10296, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10296][xPoint_10296 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10296, yPoint_10296, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10296 + i][xPoint_10296] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10296, yPoint_10296){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10296][xPoint_10296] === this.CELL_EMPTY
                && map[yPoint_10296-1][xPoint_10296] === this.CELL_EMPTY
                && map[yPoint_10296-1][xPoint_10296+1] === this.CELL_EMPTY
                && map[yPoint_10296][xPoint_10296+1] === this.CELL_EMPTY
                && map[yPoint_10296+1][xPoint_10296+1] === this.CELL_EMPTY
                && map[yPoint_10296+1][xPoint_10296] === this.CELL_EMPTY
                && map[yPoint_10296+1][xPoint_10296-1] === this.CELL_EMPTY
                && map[yPoint_10296][xPoint_10296-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10296, yPoint_10296, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10296;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10296][x] === this.CELL_EMPTY
                    && map[yPoint_10296-1][x] === this.CELL_EMPTY
                    && map[yPoint_10296-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10296][x+1] === this.CELL_EMPTY
                    && map[yPoint_10296+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10296+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10296, yPoint_10296, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10296;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10296] === this.CELL_EMPTY
                    && map[y+1][xPoint_10296] === this.CELL_EMPTY
                    && map[y+1][xPoint_10296+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10296] === this.CELL_EMPTY
                    && map[y][xPoint_10296-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10296-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
