...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3115=0;yPoint_3115<this.gameFieldBorderY.length; yPoint_3115++){
                for(var xPoint_3115=0;xPoint_3115<this.gameFieldBorderX.length; xPoint_3115++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3115, xPoint_3115);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3115][xPoint_3115] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3115, xPoint_3115, 'user');
                    if(this._userShipsMap[yPoint_3115][xPoint_3115] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3115, xPoint_3115, type){
            var id = this.getPointBlockIdByCoords(yPoint_3115, xPoint_3115, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3115);
                block.setAttribute('data-y', yPoint_3115);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3115
         * @param {type} xPoint_3115
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3115, xPoint_3115, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3115 + '_y' + yPoint_3115;
            }
            return 'pc_x' + xPoint_3115 + '_y' + yPoint_3115;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3115=0;yPoint_3115<this.gameFieldBorderY.length; yPoint_3115++){
                for(var xPoint_3115=0;xPoint_3115<this.gameFieldBorderX.length; xPoint_3115++){
                    map.push({y: yPoint_3115, x: xPoint_3115});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3115=-1;yPoint_3115<(this.gameFieldBorderY.length+1); yPoint_3115++){
                for(var xPoint_3115=-1;xPoint_3115<(this.gameFieldBorderX.length+1); xPoint_3115++){
                    if(!map[yPoint_3115]){
                        map[yPoint_3115] = [];
                    }
                    map[yPoint_3115][xPoint_3115] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3115 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3115 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3115, yPoint_3115) === true){
                    if(this.canPutHorizontal(map, xPoint_3115, yPoint_3115, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3115][xPoint_3115 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3115, yPoint_3115, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3115 + i][xPoint_3115] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3115, yPoint_3115){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3115][xPoint_3115] === this.CELL_EMPTY
                && map[yPoint_3115-1][xPoint_3115] === this.CELL_EMPTY
                && map[yPoint_3115-1][xPoint_3115+1] === this.CELL_EMPTY
                && map[yPoint_3115][xPoint_3115+1] === this.CELL_EMPTY
                && map[yPoint_3115+1][xPoint_3115+1] === this.CELL_EMPTY
                && map[yPoint_3115+1][xPoint_3115] === this.CELL_EMPTY
                && map[yPoint_3115+1][xPoint_3115-1] === this.CELL_EMPTY
                && map[yPoint_3115][xPoint_3115-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3115, yPoint_3115, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3115;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3115][x] === this.CELL_EMPTY
                    && map[yPoint_3115-1][x] === this.CELL_EMPTY
                    && map[yPoint_3115-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3115][x+1] === this.CELL_EMPTY
                    && map[yPoint_3115+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3115+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3115, yPoint_3115, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3115;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3115] === this.CELL_EMPTY
                    && map[y+1][xPoint_3115] === this.CELL_EMPTY
                    && map[y+1][xPoint_3115+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3115] === this.CELL_EMPTY
                    && map[y][xPoint_3115-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3115-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
