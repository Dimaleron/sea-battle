...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7433=0;yPoint_7433<this.gameFieldBorderY.length; yPoint_7433++){
                for(var xPoint_7433=0;xPoint_7433<this.gameFieldBorderX.length; xPoint_7433++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7433, xPoint_7433);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7433][xPoint_7433] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7433, xPoint_7433, 'user');
                    if(this._userShipsMap[yPoint_7433][xPoint_7433] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7433, xPoint_7433, type){
            var id = this.getPointBlockIdByCoords(yPoint_7433, xPoint_7433, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7433);
                block.setAttribute('data-y', yPoint_7433);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7433
         * @param {type} xPoint_7433
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7433, xPoint_7433, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7433 + '_y' + yPoint_7433;
            }
            return 'pc_x' + xPoint_7433 + '_y' + yPoint_7433;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7433=0;yPoint_7433<this.gameFieldBorderY.length; yPoint_7433++){
                for(var xPoint_7433=0;xPoint_7433<this.gameFieldBorderX.length; xPoint_7433++){
                    map.push({y: yPoint_7433, x: xPoint_7433});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7433=-1;yPoint_7433<(this.gameFieldBorderY.length+1); yPoint_7433++){
                for(var xPoint_7433=-1;xPoint_7433<(this.gameFieldBorderX.length+1); xPoint_7433++){
                    if(!map[yPoint_7433]){
                        map[yPoint_7433] = [];
                    }
                    map[yPoint_7433][xPoint_7433] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7433 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7433 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7433, yPoint_7433) === true){
                    if(this.canPutHorizontal(map, xPoint_7433, yPoint_7433, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7433][xPoint_7433 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7433, yPoint_7433, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7433 + i][xPoint_7433] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7433, yPoint_7433){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7433][xPoint_7433] === this.CELL_EMPTY
                && map[yPoint_7433-1][xPoint_7433] === this.CELL_EMPTY
                && map[yPoint_7433-1][xPoint_7433+1] === this.CELL_EMPTY
                && map[yPoint_7433][xPoint_7433+1] === this.CELL_EMPTY
                && map[yPoint_7433+1][xPoint_7433+1] === this.CELL_EMPTY
                && map[yPoint_7433+1][xPoint_7433] === this.CELL_EMPTY
                && map[yPoint_7433+1][xPoint_7433-1] === this.CELL_EMPTY
                && map[yPoint_7433][xPoint_7433-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7433, yPoint_7433, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7433;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7433][x] === this.CELL_EMPTY
                    && map[yPoint_7433-1][x] === this.CELL_EMPTY
                    && map[yPoint_7433-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7433][x+1] === this.CELL_EMPTY
                    && map[yPoint_7433+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7433+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7433, yPoint_7433, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7433;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7433] === this.CELL_EMPTY
                    && map[y+1][xPoint_7433] === this.CELL_EMPTY
                    && map[y+1][xPoint_7433+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7433] === this.CELL_EMPTY
                    && map[y][xPoint_7433-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7433-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
