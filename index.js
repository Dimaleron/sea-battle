...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6566=0;yPoint_6566<this.gameFieldBorderY.length; yPoint_6566++){
                for(var xPoint_6566=0;xPoint_6566<this.gameFieldBorderX.length; xPoint_6566++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6566, xPoint_6566);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6566][xPoint_6566] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6566, xPoint_6566, 'user');
                    if(this._userShipsMap[yPoint_6566][xPoint_6566] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6566, xPoint_6566, type){
            var id = this.getPointBlockIdByCoords(yPoint_6566, xPoint_6566, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6566);
                block.setAttribute('data-y', yPoint_6566);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6566
         * @param {type} xPoint_6566
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6566, xPoint_6566, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6566 + '_y' + yPoint_6566;
            }
            return 'pc_x' + xPoint_6566 + '_y' + yPoint_6566;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6566=0;yPoint_6566<this.gameFieldBorderY.length; yPoint_6566++){
                for(var xPoint_6566=0;xPoint_6566<this.gameFieldBorderX.length; xPoint_6566++){
                    map.push({y: yPoint_6566, x: xPoint_6566});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6566=-1;yPoint_6566<(this.gameFieldBorderY.length+1); yPoint_6566++){
                for(var xPoint_6566=-1;xPoint_6566<(this.gameFieldBorderX.length+1); xPoint_6566++){
                    if(!map[yPoint_6566]){
                        map[yPoint_6566] = [];
                    }
                    map[yPoint_6566][xPoint_6566] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6566 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6566 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6566, yPoint_6566) === true){
                    if(this.canPutHorizontal(map, xPoint_6566, yPoint_6566, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6566][xPoint_6566 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6566, yPoint_6566, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6566 + i][xPoint_6566] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6566, yPoint_6566){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6566][xPoint_6566] === this.CELL_EMPTY
                && map[yPoint_6566-1][xPoint_6566] === this.CELL_EMPTY
                && map[yPoint_6566-1][xPoint_6566+1] === this.CELL_EMPTY
                && map[yPoint_6566][xPoint_6566+1] === this.CELL_EMPTY
                && map[yPoint_6566+1][xPoint_6566+1] === this.CELL_EMPTY
                && map[yPoint_6566+1][xPoint_6566] === this.CELL_EMPTY
                && map[yPoint_6566+1][xPoint_6566-1] === this.CELL_EMPTY
                && map[yPoint_6566][xPoint_6566-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6566, yPoint_6566, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6566;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6566][x] === this.CELL_EMPTY
                    && map[yPoint_6566-1][x] === this.CELL_EMPTY
                    && map[yPoint_6566-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6566][x+1] === this.CELL_EMPTY
                    && map[yPoint_6566+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6566+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6566, yPoint_6566, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6566;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6566] === this.CELL_EMPTY
                    && map[y+1][xPoint_6566] === this.CELL_EMPTY
                    && map[y+1][xPoint_6566+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6566] === this.CELL_EMPTY
                    && map[y][xPoint_6566-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6566-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
