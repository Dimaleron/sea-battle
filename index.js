...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10168=0;yPoint_10168<this.gameFieldBorderY.length; yPoint_10168++){
                for(var xPoint_10168=0;xPoint_10168<this.gameFieldBorderX.length; xPoint_10168++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10168, xPoint_10168);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10168][xPoint_10168] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10168, xPoint_10168, 'user');
                    if(this._userShipsMap[yPoint_10168][xPoint_10168] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10168, xPoint_10168, type){
            var id = this.getPointBlockIdByCoords(yPoint_10168, xPoint_10168, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10168);
                block.setAttribute('data-y', yPoint_10168);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10168
         * @param {type} xPoint_10168
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10168, xPoint_10168, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10168 + '_y' + yPoint_10168;
            }
            return 'pc_x' + xPoint_10168 + '_y' + yPoint_10168;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10168=0;yPoint_10168<this.gameFieldBorderY.length; yPoint_10168++){
                for(var xPoint_10168=0;xPoint_10168<this.gameFieldBorderX.length; xPoint_10168++){
                    map.push({y: yPoint_10168, x: xPoint_10168});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10168=-1;yPoint_10168<(this.gameFieldBorderY.length+1); yPoint_10168++){
                for(var xPoint_10168=-1;xPoint_10168<(this.gameFieldBorderX.length+1); xPoint_10168++){
                    if(!map[yPoint_10168]){
                        map[yPoint_10168] = [];
                    }
                    map[yPoint_10168][xPoint_10168] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10168 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10168 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10168, yPoint_10168) === true){
                    if(this.canPutHorizontal(map, xPoint_10168, yPoint_10168, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10168][xPoint_10168 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10168, yPoint_10168, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10168 + i][xPoint_10168] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10168, yPoint_10168){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10168][xPoint_10168] === this.CELL_EMPTY
                && map[yPoint_10168-1][xPoint_10168] === this.CELL_EMPTY
                && map[yPoint_10168-1][xPoint_10168+1] === this.CELL_EMPTY
                && map[yPoint_10168][xPoint_10168+1] === this.CELL_EMPTY
                && map[yPoint_10168+1][xPoint_10168+1] === this.CELL_EMPTY
                && map[yPoint_10168+1][xPoint_10168] === this.CELL_EMPTY
                && map[yPoint_10168+1][xPoint_10168-1] === this.CELL_EMPTY
                && map[yPoint_10168][xPoint_10168-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10168, yPoint_10168, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10168;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10168][x] === this.CELL_EMPTY
                    && map[yPoint_10168-1][x] === this.CELL_EMPTY
                    && map[yPoint_10168-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10168][x+1] === this.CELL_EMPTY
                    && map[yPoint_10168+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10168+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10168, yPoint_10168, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10168;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10168] === this.CELL_EMPTY
                    && map[y+1][xPoint_10168] === this.CELL_EMPTY
                    && map[y+1][xPoint_10168+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10168] === this.CELL_EMPTY
                    && map[y][xPoint_10168-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10168-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
