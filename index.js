...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2290=0;yPoint_2290<this.gameFieldBorderY.length; yPoint_2290++){
                for(var xPoint_2290=0;xPoint_2290<this.gameFieldBorderX.length; xPoint_2290++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2290, xPoint_2290);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2290][xPoint_2290] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2290, xPoint_2290, 'user');
                    if(this._userShipsMap[yPoint_2290][xPoint_2290] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2290, xPoint_2290, type){
            var id = this.getPointBlockIdByCoords(yPoint_2290, xPoint_2290, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2290);
                block.setAttribute('data-y', yPoint_2290);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2290
         * @param {type} xPoint_2290
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2290, xPoint_2290, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2290 + '_y' + yPoint_2290;
            }
            return 'pc_x' + xPoint_2290 + '_y' + yPoint_2290;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2290=0;yPoint_2290<this.gameFieldBorderY.length; yPoint_2290++){
                for(var xPoint_2290=0;xPoint_2290<this.gameFieldBorderX.length; xPoint_2290++){
                    map.push({y: yPoint_2290, x: xPoint_2290});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2290=-1;yPoint_2290<(this.gameFieldBorderY.length+1); yPoint_2290++){
                for(var xPoint_2290=-1;xPoint_2290<(this.gameFieldBorderX.length+1); xPoint_2290++){
                    if(!map[yPoint_2290]){
                        map[yPoint_2290] = [];
                    }
                    map[yPoint_2290][xPoint_2290] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2290 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2290 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2290, yPoint_2290) === true){
                    if(this.canPutHorizontal(map, xPoint_2290, yPoint_2290, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2290][xPoint_2290 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2290, yPoint_2290, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2290 + i][xPoint_2290] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2290, yPoint_2290){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2290][xPoint_2290] === this.CELL_EMPTY
                && map[yPoint_2290-1][xPoint_2290] === this.CELL_EMPTY
                && map[yPoint_2290-1][xPoint_2290+1] === this.CELL_EMPTY
                && map[yPoint_2290][xPoint_2290+1] === this.CELL_EMPTY
                && map[yPoint_2290+1][xPoint_2290+1] === this.CELL_EMPTY
                && map[yPoint_2290+1][xPoint_2290] === this.CELL_EMPTY
                && map[yPoint_2290+1][xPoint_2290-1] === this.CELL_EMPTY
                && map[yPoint_2290][xPoint_2290-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2290, yPoint_2290, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2290;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2290][x] === this.CELL_EMPTY
                    && map[yPoint_2290-1][x] === this.CELL_EMPTY
                    && map[yPoint_2290-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2290][x+1] === this.CELL_EMPTY
                    && map[yPoint_2290+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2290+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2290, yPoint_2290, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2290;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2290] === this.CELL_EMPTY
                    && map[y+1][xPoint_2290] === this.CELL_EMPTY
                    && map[y+1][xPoint_2290+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2290] === this.CELL_EMPTY
                    && map[y][xPoint_2290-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2290-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
