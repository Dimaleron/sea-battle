...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14575=0;yPoint_14575<this.gameFieldBorderY.length; yPoint_14575++){
                for(var xPoint_14575=0;xPoint_14575<this.gameFieldBorderX.length; xPoint_14575++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14575, xPoint_14575);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14575][xPoint_14575] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14575, xPoint_14575, 'user');
                    if(this._userShipsMap[yPoint_14575][xPoint_14575] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14575, xPoint_14575, type){
            var id = this.getPointBlockIdByCoords(yPoint_14575, xPoint_14575, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14575);
                block.setAttribute('data-y', yPoint_14575);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14575
         * @param {type} xPoint_14575
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14575, xPoint_14575, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14575 + '_y' + yPoint_14575;
            }
            return 'pc_x' + xPoint_14575 + '_y' + yPoint_14575;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14575=0;yPoint_14575<this.gameFieldBorderY.length; yPoint_14575++){
                for(var xPoint_14575=0;xPoint_14575<this.gameFieldBorderX.length; xPoint_14575++){
                    map.push({y: yPoint_14575, x: xPoint_14575});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14575=-1;yPoint_14575<(this.gameFieldBorderY.length+1); yPoint_14575++){
                for(var xPoint_14575=-1;xPoint_14575<(this.gameFieldBorderX.length+1); xPoint_14575++){
                    if(!map[yPoint_14575]){
                        map[yPoint_14575] = [];
                    }
                    map[yPoint_14575][xPoint_14575] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14575 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14575 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14575, yPoint_14575) === true){
                    if(this.canPutHorizontal(map, xPoint_14575, yPoint_14575, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14575][xPoint_14575 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14575, yPoint_14575, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14575 + i][xPoint_14575] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14575, yPoint_14575){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14575][xPoint_14575] === this.CELL_EMPTY
                && map[yPoint_14575-1][xPoint_14575] === this.CELL_EMPTY
                && map[yPoint_14575-1][xPoint_14575+1] === this.CELL_EMPTY
                && map[yPoint_14575][xPoint_14575+1] === this.CELL_EMPTY
                && map[yPoint_14575+1][xPoint_14575+1] === this.CELL_EMPTY
                && map[yPoint_14575+1][xPoint_14575] === this.CELL_EMPTY
                && map[yPoint_14575+1][xPoint_14575-1] === this.CELL_EMPTY
                && map[yPoint_14575][xPoint_14575-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14575, yPoint_14575, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14575;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14575][x] === this.CELL_EMPTY
                    && map[yPoint_14575-1][x] === this.CELL_EMPTY
                    && map[yPoint_14575-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14575][x+1] === this.CELL_EMPTY
                    && map[yPoint_14575+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14575+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14575, yPoint_14575, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14575;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14575] === this.CELL_EMPTY
                    && map[y+1][xPoint_14575] === this.CELL_EMPTY
                    && map[y+1][xPoint_14575+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14575] === this.CELL_EMPTY
                    && map[y][xPoint_14575-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14575-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
