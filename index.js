...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25285=0;yPoint_25285<this.gameFieldBorderY.length; yPoint_25285++){
                for(var xPoint_25285=0;xPoint_25285<this.gameFieldBorderX.length; xPoint_25285++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25285, xPoint_25285);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25285][xPoint_25285] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25285, xPoint_25285, 'user');
                    if(this._userShipsMap[yPoint_25285][xPoint_25285] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25285, xPoint_25285, type){
            var id = this.getPointBlockIdByCoords(yPoint_25285, xPoint_25285, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25285);
                block.setAttribute('data-y', yPoint_25285);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25285
         * @param {type} xPoint_25285
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25285, xPoint_25285, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25285 + '_y' + yPoint_25285;
            }
            return 'pc_x' + xPoint_25285 + '_y' + yPoint_25285;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25285=0;yPoint_25285<this.gameFieldBorderY.length; yPoint_25285++){
                for(var xPoint_25285=0;xPoint_25285<this.gameFieldBorderX.length; xPoint_25285++){
                    map.push({y: yPoint_25285, x: xPoint_25285});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25285=-1;yPoint_25285<(this.gameFieldBorderY.length+1); yPoint_25285++){
                for(var xPoint_25285=-1;xPoint_25285<(this.gameFieldBorderX.length+1); xPoint_25285++){
                    if(!map[yPoint_25285]){
                        map[yPoint_25285] = [];
                    }
                    map[yPoint_25285][xPoint_25285] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25285 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25285 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25285, yPoint_25285) === true){
                    if(this.canPutHorizontal(map, xPoint_25285, yPoint_25285, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25285][xPoint_25285 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25285, yPoint_25285, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25285 + i][xPoint_25285] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25285, yPoint_25285){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25285][xPoint_25285] === this.CELL_EMPTY
                && map[yPoint_25285-1][xPoint_25285] === this.CELL_EMPTY
                && map[yPoint_25285-1][xPoint_25285+1] === this.CELL_EMPTY
                && map[yPoint_25285][xPoint_25285+1] === this.CELL_EMPTY
                && map[yPoint_25285+1][xPoint_25285+1] === this.CELL_EMPTY
                && map[yPoint_25285+1][xPoint_25285] === this.CELL_EMPTY
                && map[yPoint_25285+1][xPoint_25285-1] === this.CELL_EMPTY
                && map[yPoint_25285][xPoint_25285-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25285, yPoint_25285, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25285;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25285][x] === this.CELL_EMPTY
                    && map[yPoint_25285-1][x] === this.CELL_EMPTY
                    && map[yPoint_25285-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25285][x+1] === this.CELL_EMPTY
                    && map[yPoint_25285+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25285+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25285, yPoint_25285, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25285;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25285] === this.CELL_EMPTY
                    && map[y+1][xPoint_25285] === this.CELL_EMPTY
                    && map[y+1][xPoint_25285+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25285] === this.CELL_EMPTY
                    && map[y][xPoint_25285-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25285-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
