...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4817=0;yPoint_4817<this.gameFieldBorderY.length; yPoint_4817++){
                for(var xPoint_4817=0;xPoint_4817<this.gameFieldBorderX.length; xPoint_4817++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4817, xPoint_4817);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4817][xPoint_4817] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4817, xPoint_4817, 'user');
                    if(this._userShipsMap[yPoint_4817][xPoint_4817] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4817, xPoint_4817, type){
            var id = this.getPointBlockIdByCoords(yPoint_4817, xPoint_4817, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4817);
                block.setAttribute('data-y', yPoint_4817);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4817
         * @param {type} xPoint_4817
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4817, xPoint_4817, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4817 + '_y' + yPoint_4817;
            }
            return 'pc_x' + xPoint_4817 + '_y' + yPoint_4817;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4817=0;yPoint_4817<this.gameFieldBorderY.length; yPoint_4817++){
                for(var xPoint_4817=0;xPoint_4817<this.gameFieldBorderX.length; xPoint_4817++){
                    map.push({y: yPoint_4817, x: xPoint_4817});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4817=-1;yPoint_4817<(this.gameFieldBorderY.length+1); yPoint_4817++){
                for(var xPoint_4817=-1;xPoint_4817<(this.gameFieldBorderX.length+1); xPoint_4817++){
                    if(!map[yPoint_4817]){
                        map[yPoint_4817] = [];
                    }
                    map[yPoint_4817][xPoint_4817] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4817 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4817 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4817, yPoint_4817) === true){
                    if(this.canPutHorizontal(map, xPoint_4817, yPoint_4817, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4817][xPoint_4817 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4817, yPoint_4817, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4817 + i][xPoint_4817] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4817, yPoint_4817){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4817][xPoint_4817] === this.CELL_EMPTY
                && map[yPoint_4817-1][xPoint_4817] === this.CELL_EMPTY
                && map[yPoint_4817-1][xPoint_4817+1] === this.CELL_EMPTY
                && map[yPoint_4817][xPoint_4817+1] === this.CELL_EMPTY
                && map[yPoint_4817+1][xPoint_4817+1] === this.CELL_EMPTY
                && map[yPoint_4817+1][xPoint_4817] === this.CELL_EMPTY
                && map[yPoint_4817+1][xPoint_4817-1] === this.CELL_EMPTY
                && map[yPoint_4817][xPoint_4817-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4817, yPoint_4817, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4817;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4817][x] === this.CELL_EMPTY
                    && map[yPoint_4817-1][x] === this.CELL_EMPTY
                    && map[yPoint_4817-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4817][x+1] === this.CELL_EMPTY
                    && map[yPoint_4817+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4817+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4817, yPoint_4817, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4817;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4817] === this.CELL_EMPTY
                    && map[y+1][xPoint_4817] === this.CELL_EMPTY
                    && map[y+1][xPoint_4817+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4817] === this.CELL_EMPTY
                    && map[y][xPoint_4817-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4817-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
