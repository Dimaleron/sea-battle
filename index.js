...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11893=0;yPoint_11893<this.gameFieldBorderY.length; yPoint_11893++){
                for(var xPoint_11893=0;xPoint_11893<this.gameFieldBorderX.length; xPoint_11893++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11893, xPoint_11893);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11893][xPoint_11893] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11893, xPoint_11893, 'user');
                    if(this._userShipsMap[yPoint_11893][xPoint_11893] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11893, xPoint_11893, type){
            var id = this.getPointBlockIdByCoords(yPoint_11893, xPoint_11893, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11893);
                block.setAttribute('data-y', yPoint_11893);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11893
         * @param {type} xPoint_11893
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11893, xPoint_11893, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11893 + '_y' + yPoint_11893;
            }
            return 'pc_x' + xPoint_11893 + '_y' + yPoint_11893;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11893=0;yPoint_11893<this.gameFieldBorderY.length; yPoint_11893++){
                for(var xPoint_11893=0;xPoint_11893<this.gameFieldBorderX.length; xPoint_11893++){
                    map.push({y: yPoint_11893, x: xPoint_11893});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11893=-1;yPoint_11893<(this.gameFieldBorderY.length+1); yPoint_11893++){
                for(var xPoint_11893=-1;xPoint_11893<(this.gameFieldBorderX.length+1); xPoint_11893++){
                    if(!map[yPoint_11893]){
                        map[yPoint_11893] = [];
                    }
                    map[yPoint_11893][xPoint_11893] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11893 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11893 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11893, yPoint_11893) === true){
                    if(this.canPutHorizontal(map, xPoint_11893, yPoint_11893, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11893][xPoint_11893 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11893, yPoint_11893, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11893 + i][xPoint_11893] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11893, yPoint_11893){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11893][xPoint_11893] === this.CELL_EMPTY
                && map[yPoint_11893-1][xPoint_11893] === this.CELL_EMPTY
                && map[yPoint_11893-1][xPoint_11893+1] === this.CELL_EMPTY
                && map[yPoint_11893][xPoint_11893+1] === this.CELL_EMPTY
                && map[yPoint_11893+1][xPoint_11893+1] === this.CELL_EMPTY
                && map[yPoint_11893+1][xPoint_11893] === this.CELL_EMPTY
                && map[yPoint_11893+1][xPoint_11893-1] === this.CELL_EMPTY
                && map[yPoint_11893][xPoint_11893-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11893, yPoint_11893, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11893;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11893][x] === this.CELL_EMPTY
                    && map[yPoint_11893-1][x] === this.CELL_EMPTY
                    && map[yPoint_11893-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11893][x+1] === this.CELL_EMPTY
                    && map[yPoint_11893+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11893+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11893, yPoint_11893, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11893;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11893] === this.CELL_EMPTY
                    && map[y+1][xPoint_11893] === this.CELL_EMPTY
                    && map[y+1][xPoint_11893+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11893] === this.CELL_EMPTY
                    && map[y][xPoint_11893-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11893-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
