...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19116=0;yPoint_19116<this.gameFieldBorderY.length; yPoint_19116++){
                for(var xPoint_19116=0;xPoint_19116<this.gameFieldBorderX.length; xPoint_19116++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19116, xPoint_19116);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19116][xPoint_19116] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19116, xPoint_19116, 'user');
                    if(this._userShipsMap[yPoint_19116][xPoint_19116] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19116, xPoint_19116, type){
            var id = this.getPointBlockIdByCoords(yPoint_19116, xPoint_19116, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19116);
                block.setAttribute('data-y', yPoint_19116);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19116
         * @param {type} xPoint_19116
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19116, xPoint_19116, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19116 + '_y' + yPoint_19116;
            }
            return 'pc_x' + xPoint_19116 + '_y' + yPoint_19116;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19116=0;yPoint_19116<this.gameFieldBorderY.length; yPoint_19116++){
                for(var xPoint_19116=0;xPoint_19116<this.gameFieldBorderX.length; xPoint_19116++){
                    map.push({y: yPoint_19116, x: xPoint_19116});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19116=-1;yPoint_19116<(this.gameFieldBorderY.length+1); yPoint_19116++){
                for(var xPoint_19116=-1;xPoint_19116<(this.gameFieldBorderX.length+1); xPoint_19116++){
                    if(!map[yPoint_19116]){
                        map[yPoint_19116] = [];
                    }
                    map[yPoint_19116][xPoint_19116] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19116 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19116 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19116, yPoint_19116) === true){
                    if(this.canPutHorizontal(map, xPoint_19116, yPoint_19116, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19116][xPoint_19116 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19116, yPoint_19116, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19116 + i][xPoint_19116] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19116, yPoint_19116){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19116][xPoint_19116] === this.CELL_EMPTY
                && map[yPoint_19116-1][xPoint_19116] === this.CELL_EMPTY
                && map[yPoint_19116-1][xPoint_19116+1] === this.CELL_EMPTY
                && map[yPoint_19116][xPoint_19116+1] === this.CELL_EMPTY
                && map[yPoint_19116+1][xPoint_19116+1] === this.CELL_EMPTY
                && map[yPoint_19116+1][xPoint_19116] === this.CELL_EMPTY
                && map[yPoint_19116+1][xPoint_19116-1] === this.CELL_EMPTY
                && map[yPoint_19116][xPoint_19116-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19116, yPoint_19116, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19116;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19116][x] === this.CELL_EMPTY
                    && map[yPoint_19116-1][x] === this.CELL_EMPTY
                    && map[yPoint_19116-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19116][x+1] === this.CELL_EMPTY
                    && map[yPoint_19116+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19116+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19116, yPoint_19116, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19116;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19116] === this.CELL_EMPTY
                    && map[y+1][xPoint_19116] === this.CELL_EMPTY
                    && map[y+1][xPoint_19116+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19116] === this.CELL_EMPTY
                    && map[y][xPoint_19116-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19116-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
