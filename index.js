...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17058=0;yPoint_17058<this.gameFieldBorderY.length; yPoint_17058++){
                for(var xPoint_17058=0;xPoint_17058<this.gameFieldBorderX.length; xPoint_17058++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17058, xPoint_17058);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17058][xPoint_17058] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17058, xPoint_17058, 'user');
                    if(this._userShipsMap[yPoint_17058][xPoint_17058] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17058, xPoint_17058, type){
            var id = this.getPointBlockIdByCoords(yPoint_17058, xPoint_17058, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17058);
                block.setAttribute('data-y', yPoint_17058);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17058
         * @param {type} xPoint_17058
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17058, xPoint_17058, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17058 + '_y' + yPoint_17058;
            }
            return 'pc_x' + xPoint_17058 + '_y' + yPoint_17058;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17058=0;yPoint_17058<this.gameFieldBorderY.length; yPoint_17058++){
                for(var xPoint_17058=0;xPoint_17058<this.gameFieldBorderX.length; xPoint_17058++){
                    map.push({y: yPoint_17058, x: xPoint_17058});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17058=-1;yPoint_17058<(this.gameFieldBorderY.length+1); yPoint_17058++){
                for(var xPoint_17058=-1;xPoint_17058<(this.gameFieldBorderX.length+1); xPoint_17058++){
                    if(!map[yPoint_17058]){
                        map[yPoint_17058] = [];
                    }
                    map[yPoint_17058][xPoint_17058] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17058 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17058 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17058, yPoint_17058) === true){
                    if(this.canPutHorizontal(map, xPoint_17058, yPoint_17058, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17058][xPoint_17058 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17058, yPoint_17058, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17058 + i][xPoint_17058] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17058, yPoint_17058){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17058][xPoint_17058] === this.CELL_EMPTY
                && map[yPoint_17058-1][xPoint_17058] === this.CELL_EMPTY
                && map[yPoint_17058-1][xPoint_17058+1] === this.CELL_EMPTY
                && map[yPoint_17058][xPoint_17058+1] === this.CELL_EMPTY
                && map[yPoint_17058+1][xPoint_17058+1] === this.CELL_EMPTY
                && map[yPoint_17058+1][xPoint_17058] === this.CELL_EMPTY
                && map[yPoint_17058+1][xPoint_17058-1] === this.CELL_EMPTY
                && map[yPoint_17058][xPoint_17058-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17058, yPoint_17058, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17058;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17058][x] === this.CELL_EMPTY
                    && map[yPoint_17058-1][x] === this.CELL_EMPTY
                    && map[yPoint_17058-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17058][x+1] === this.CELL_EMPTY
                    && map[yPoint_17058+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17058+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17058, yPoint_17058, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17058;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17058] === this.CELL_EMPTY
                    && map[y+1][xPoint_17058] === this.CELL_EMPTY
                    && map[y+1][xPoint_17058+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17058] === this.CELL_EMPTY
                    && map[y][xPoint_17058-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17058-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
