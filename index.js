...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26446=0;yPoint_26446<this.gameFieldBorderY.length; yPoint_26446++){
                for(var xPoint_26446=0;xPoint_26446<this.gameFieldBorderX.length; xPoint_26446++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26446, xPoint_26446);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26446][xPoint_26446] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26446, xPoint_26446, 'user');
                    if(this._userShipsMap[yPoint_26446][xPoint_26446] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26446, xPoint_26446, type){
            var id = this.getPointBlockIdByCoords(yPoint_26446, xPoint_26446, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26446);
                block.setAttribute('data-y', yPoint_26446);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26446
         * @param {type} xPoint_26446
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26446, xPoint_26446, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26446 + '_y' + yPoint_26446;
            }
            return 'pc_x' + xPoint_26446 + '_y' + yPoint_26446;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26446=0;yPoint_26446<this.gameFieldBorderY.length; yPoint_26446++){
                for(var xPoint_26446=0;xPoint_26446<this.gameFieldBorderX.length; xPoint_26446++){
                    map.push({y: yPoint_26446, x: xPoint_26446});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26446=-1;yPoint_26446<(this.gameFieldBorderY.length+1); yPoint_26446++){
                for(var xPoint_26446=-1;xPoint_26446<(this.gameFieldBorderX.length+1); xPoint_26446++){
                    if(!map[yPoint_26446]){
                        map[yPoint_26446] = [];
                    }
                    map[yPoint_26446][xPoint_26446] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26446 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26446 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26446, yPoint_26446) === true){
                    if(this.canPutHorizontal(map, xPoint_26446, yPoint_26446, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26446][xPoint_26446 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26446, yPoint_26446, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26446 + i][xPoint_26446] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26446, yPoint_26446){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26446][xPoint_26446] === this.CELL_EMPTY
                && map[yPoint_26446-1][xPoint_26446] === this.CELL_EMPTY
                && map[yPoint_26446-1][xPoint_26446+1] === this.CELL_EMPTY
                && map[yPoint_26446][xPoint_26446+1] === this.CELL_EMPTY
                && map[yPoint_26446+1][xPoint_26446+1] === this.CELL_EMPTY
                && map[yPoint_26446+1][xPoint_26446] === this.CELL_EMPTY
                && map[yPoint_26446+1][xPoint_26446-1] === this.CELL_EMPTY
                && map[yPoint_26446][xPoint_26446-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26446, yPoint_26446, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26446;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26446][x] === this.CELL_EMPTY
                    && map[yPoint_26446-1][x] === this.CELL_EMPTY
                    && map[yPoint_26446-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26446][x+1] === this.CELL_EMPTY
                    && map[yPoint_26446+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26446+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26446, yPoint_26446, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26446;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26446] === this.CELL_EMPTY
                    && map[y+1][xPoint_26446] === this.CELL_EMPTY
                    && map[y+1][xPoint_26446+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26446] === this.CELL_EMPTY
                    && map[y][xPoint_26446-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26446-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
