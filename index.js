...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8904=0;yPoint_8904<this.gameFieldBorderY.length; yPoint_8904++){
                for(var xPoint_8904=0;xPoint_8904<this.gameFieldBorderX.length; xPoint_8904++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8904, xPoint_8904);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8904][xPoint_8904] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8904, xPoint_8904, 'user');
                    if(this._userShipsMap[yPoint_8904][xPoint_8904] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8904, xPoint_8904, type){
            var id = this.getPointBlockIdByCoords(yPoint_8904, xPoint_8904, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8904);
                block.setAttribute('data-y', yPoint_8904);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8904
         * @param {type} xPoint_8904
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8904, xPoint_8904, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8904 + '_y' + yPoint_8904;
            }
            return 'pc_x' + xPoint_8904 + '_y' + yPoint_8904;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8904=0;yPoint_8904<this.gameFieldBorderY.length; yPoint_8904++){
                for(var xPoint_8904=0;xPoint_8904<this.gameFieldBorderX.length; xPoint_8904++){
                    map.push({y: yPoint_8904, x: xPoint_8904});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8904=-1;yPoint_8904<(this.gameFieldBorderY.length+1); yPoint_8904++){
                for(var xPoint_8904=-1;xPoint_8904<(this.gameFieldBorderX.length+1); xPoint_8904++){
                    if(!map[yPoint_8904]){
                        map[yPoint_8904] = [];
                    }
                    map[yPoint_8904][xPoint_8904] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8904 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8904 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8904, yPoint_8904) === true){
                    if(this.canPutHorizontal(map, xPoint_8904, yPoint_8904, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8904][xPoint_8904 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8904, yPoint_8904, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8904 + i][xPoint_8904] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8904, yPoint_8904){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8904][xPoint_8904] === this.CELL_EMPTY
                && map[yPoint_8904-1][xPoint_8904] === this.CELL_EMPTY
                && map[yPoint_8904-1][xPoint_8904+1] === this.CELL_EMPTY
                && map[yPoint_8904][xPoint_8904+1] === this.CELL_EMPTY
                && map[yPoint_8904+1][xPoint_8904+1] === this.CELL_EMPTY
                && map[yPoint_8904+1][xPoint_8904] === this.CELL_EMPTY
                && map[yPoint_8904+1][xPoint_8904-1] === this.CELL_EMPTY
                && map[yPoint_8904][xPoint_8904-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8904, yPoint_8904, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8904;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8904][x] === this.CELL_EMPTY
                    && map[yPoint_8904-1][x] === this.CELL_EMPTY
                    && map[yPoint_8904-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8904][x+1] === this.CELL_EMPTY
                    && map[yPoint_8904+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8904+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8904, yPoint_8904, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8904;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8904] === this.CELL_EMPTY
                    && map[y+1][xPoint_8904] === this.CELL_EMPTY
                    && map[y+1][xPoint_8904+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8904] === this.CELL_EMPTY
                    && map[y][xPoint_8904-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8904-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
