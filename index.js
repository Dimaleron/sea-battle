...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9752=0;yPoint_9752<this.gameFieldBorderY.length; yPoint_9752++){
                for(var xPoint_9752=0;xPoint_9752<this.gameFieldBorderX.length; xPoint_9752++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9752, xPoint_9752);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9752][xPoint_9752] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9752, xPoint_9752, 'user');
                    if(this._userShipsMap[yPoint_9752][xPoint_9752] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9752, xPoint_9752, type){
            var id = this.getPointBlockIdByCoords(yPoint_9752, xPoint_9752, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9752);
                block.setAttribute('data-y', yPoint_9752);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9752
         * @param {type} xPoint_9752
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9752, xPoint_9752, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9752 + '_y' + yPoint_9752;
            }
            return 'pc_x' + xPoint_9752 + '_y' + yPoint_9752;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9752=0;yPoint_9752<this.gameFieldBorderY.length; yPoint_9752++){
                for(var xPoint_9752=0;xPoint_9752<this.gameFieldBorderX.length; xPoint_9752++){
                    map.push({y: yPoint_9752, x: xPoint_9752});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9752=-1;yPoint_9752<(this.gameFieldBorderY.length+1); yPoint_9752++){
                for(var xPoint_9752=-1;xPoint_9752<(this.gameFieldBorderX.length+1); xPoint_9752++){
                    if(!map[yPoint_9752]){
                        map[yPoint_9752] = [];
                    }
                    map[yPoint_9752][xPoint_9752] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9752 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9752 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9752, yPoint_9752) === true){
                    if(this.canPutHorizontal(map, xPoint_9752, yPoint_9752, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9752][xPoint_9752 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9752, yPoint_9752, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9752 + i][xPoint_9752] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9752, yPoint_9752){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9752][xPoint_9752] === this.CELL_EMPTY
                && map[yPoint_9752-1][xPoint_9752] === this.CELL_EMPTY
                && map[yPoint_9752-1][xPoint_9752+1] === this.CELL_EMPTY
                && map[yPoint_9752][xPoint_9752+1] === this.CELL_EMPTY
                && map[yPoint_9752+1][xPoint_9752+1] === this.CELL_EMPTY
                && map[yPoint_9752+1][xPoint_9752] === this.CELL_EMPTY
                && map[yPoint_9752+1][xPoint_9752-1] === this.CELL_EMPTY
                && map[yPoint_9752][xPoint_9752-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9752, yPoint_9752, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9752;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9752][x] === this.CELL_EMPTY
                    && map[yPoint_9752-1][x] === this.CELL_EMPTY
                    && map[yPoint_9752-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9752][x+1] === this.CELL_EMPTY
                    && map[yPoint_9752+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9752+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9752, yPoint_9752, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9752;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9752] === this.CELL_EMPTY
                    && map[y+1][xPoint_9752] === this.CELL_EMPTY
                    && map[y+1][xPoint_9752+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9752] === this.CELL_EMPTY
                    && map[y][xPoint_9752-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9752-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
