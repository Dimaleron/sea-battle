...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_104=0;yPoint_104<this.gameFieldBorderY.length; yPoint_104++){
                for(var xPoint_104=0;xPoint_104<this.gameFieldBorderX.length; xPoint_104++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_104, xPoint_104);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_104][xPoint_104] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_104, xPoint_104, 'user');
                    if(this._userShipsMap[yPoint_104][xPoint_104] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_104, xPoint_104, type){
            var id = this.getPointBlockIdByCoords(yPoint_104, xPoint_104, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_104);
                block.setAttribute('data-y', yPoint_104);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_104
         * @param {type} xPoint_104
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_104, xPoint_104, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_104 + '_y' + yPoint_104;
            }
            return 'pc_x' + xPoint_104 + '_y' + yPoint_104;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_104=0;yPoint_104<this.gameFieldBorderY.length; yPoint_104++){
                for(var xPoint_104=0;xPoint_104<this.gameFieldBorderX.length; xPoint_104++){
                    map.push({y: yPoint_104, x: xPoint_104});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_104=-1;yPoint_104<(this.gameFieldBorderY.length+1); yPoint_104++){
                for(var xPoint_104=-1;xPoint_104<(this.gameFieldBorderX.length+1); xPoint_104++){
                    if(!map[yPoint_104]){
                        map[yPoint_104] = [];
                    }
                    map[yPoint_104][xPoint_104] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_104 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_104 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_104, yPoint_104) === true){
                    if(this.canPutHorizontal(map, xPoint_104, yPoint_104, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_104][xPoint_104 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_104, yPoint_104, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_104 + i][xPoint_104] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_104, yPoint_104){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_104][xPoint_104] === this.CELL_EMPTY
                && map[yPoint_104-1][xPoint_104] === this.CELL_EMPTY
                && map[yPoint_104-1][xPoint_104+1] === this.CELL_EMPTY
                && map[yPoint_104][xPoint_104+1] === this.CELL_EMPTY
                && map[yPoint_104+1][xPoint_104+1] === this.CELL_EMPTY
                && map[yPoint_104+1][xPoint_104] === this.CELL_EMPTY
                && map[yPoint_104+1][xPoint_104-1] === this.CELL_EMPTY
                && map[yPoint_104][xPoint_104-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_104, yPoint_104, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_104;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_104][x] === this.CELL_EMPTY
                    && map[yPoint_104-1][x] === this.CELL_EMPTY
                    && map[yPoint_104-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_104][x+1] === this.CELL_EMPTY
                    && map[yPoint_104+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_104+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_104, yPoint_104, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_104;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_104] === this.CELL_EMPTY
                    && map[y+1][xPoint_104] === this.CELL_EMPTY
                    && map[y+1][xPoint_104+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_104] === this.CELL_EMPTY
                    && map[y][xPoint_104-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_104-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
