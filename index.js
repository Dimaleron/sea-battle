...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25589=0;yPoint_25589<this.gameFieldBorderY.length; yPoint_25589++){
                for(var xPoint_25589=0;xPoint_25589<this.gameFieldBorderX.length; xPoint_25589++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25589, xPoint_25589);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25589][xPoint_25589] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25589, xPoint_25589, 'user');
                    if(this._userShipsMap[yPoint_25589][xPoint_25589] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25589, xPoint_25589, type){
            var id = this.getPointBlockIdByCoords(yPoint_25589, xPoint_25589, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25589);
                block.setAttribute('data-y', yPoint_25589);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25589
         * @param {type} xPoint_25589
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25589, xPoint_25589, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25589 + '_y' + yPoint_25589;
            }
            return 'pc_x' + xPoint_25589 + '_y' + yPoint_25589;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25589=0;yPoint_25589<this.gameFieldBorderY.length; yPoint_25589++){
                for(var xPoint_25589=0;xPoint_25589<this.gameFieldBorderX.length; xPoint_25589++){
                    map.push({y: yPoint_25589, x: xPoint_25589});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25589=-1;yPoint_25589<(this.gameFieldBorderY.length+1); yPoint_25589++){
                for(var xPoint_25589=-1;xPoint_25589<(this.gameFieldBorderX.length+1); xPoint_25589++){
                    if(!map[yPoint_25589]){
                        map[yPoint_25589] = [];
                    }
                    map[yPoint_25589][xPoint_25589] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25589 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25589 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25589, yPoint_25589) === true){
                    if(this.canPutHorizontal(map, xPoint_25589, yPoint_25589, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25589][xPoint_25589 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25589, yPoint_25589, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25589 + i][xPoint_25589] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25589, yPoint_25589){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25589][xPoint_25589] === this.CELL_EMPTY
                && map[yPoint_25589-1][xPoint_25589] === this.CELL_EMPTY
                && map[yPoint_25589-1][xPoint_25589+1] === this.CELL_EMPTY
                && map[yPoint_25589][xPoint_25589+1] === this.CELL_EMPTY
                && map[yPoint_25589+1][xPoint_25589+1] === this.CELL_EMPTY
                && map[yPoint_25589+1][xPoint_25589] === this.CELL_EMPTY
                && map[yPoint_25589+1][xPoint_25589-1] === this.CELL_EMPTY
                && map[yPoint_25589][xPoint_25589-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25589, yPoint_25589, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25589;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25589][x] === this.CELL_EMPTY
                    && map[yPoint_25589-1][x] === this.CELL_EMPTY
                    && map[yPoint_25589-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25589][x+1] === this.CELL_EMPTY
                    && map[yPoint_25589+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25589+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25589, yPoint_25589, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25589;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25589] === this.CELL_EMPTY
                    && map[y+1][xPoint_25589] === this.CELL_EMPTY
                    && map[y+1][xPoint_25589+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25589] === this.CELL_EMPTY
                    && map[y][xPoint_25589-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25589-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
