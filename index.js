...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16301=0;yPoint_16301<this.gameFieldBorderY.length; yPoint_16301++){
                for(var xPoint_16301=0;xPoint_16301<this.gameFieldBorderX.length; xPoint_16301++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16301, xPoint_16301);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16301][xPoint_16301] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16301, xPoint_16301, 'user');
                    if(this._userShipsMap[yPoint_16301][xPoint_16301] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16301, xPoint_16301, type){
            var id = this.getPointBlockIdByCoords(yPoint_16301, xPoint_16301, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16301);
                block.setAttribute('data-y', yPoint_16301);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16301
         * @param {type} xPoint_16301
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16301, xPoint_16301, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16301 + '_y' + yPoint_16301;
            }
            return 'pc_x' + xPoint_16301 + '_y' + yPoint_16301;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16301=0;yPoint_16301<this.gameFieldBorderY.length; yPoint_16301++){
                for(var xPoint_16301=0;xPoint_16301<this.gameFieldBorderX.length; xPoint_16301++){
                    map.push({y: yPoint_16301, x: xPoint_16301});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16301=-1;yPoint_16301<(this.gameFieldBorderY.length+1); yPoint_16301++){
                for(var xPoint_16301=-1;xPoint_16301<(this.gameFieldBorderX.length+1); xPoint_16301++){
                    if(!map[yPoint_16301]){
                        map[yPoint_16301] = [];
                    }
                    map[yPoint_16301][xPoint_16301] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16301 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16301 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16301, yPoint_16301) === true){
                    if(this.canPutHorizontal(map, xPoint_16301, yPoint_16301, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16301][xPoint_16301 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16301, yPoint_16301, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16301 + i][xPoint_16301] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16301, yPoint_16301){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16301][xPoint_16301] === this.CELL_EMPTY
                && map[yPoint_16301-1][xPoint_16301] === this.CELL_EMPTY
                && map[yPoint_16301-1][xPoint_16301+1] === this.CELL_EMPTY
                && map[yPoint_16301][xPoint_16301+1] === this.CELL_EMPTY
                && map[yPoint_16301+1][xPoint_16301+1] === this.CELL_EMPTY
                && map[yPoint_16301+1][xPoint_16301] === this.CELL_EMPTY
                && map[yPoint_16301+1][xPoint_16301-1] === this.CELL_EMPTY
                && map[yPoint_16301][xPoint_16301-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16301, yPoint_16301, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16301;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16301][x] === this.CELL_EMPTY
                    && map[yPoint_16301-1][x] === this.CELL_EMPTY
                    && map[yPoint_16301-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16301][x+1] === this.CELL_EMPTY
                    && map[yPoint_16301+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16301+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16301, yPoint_16301, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16301;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16301] === this.CELL_EMPTY
                    && map[y+1][xPoint_16301] === this.CELL_EMPTY
                    && map[y+1][xPoint_16301+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16301] === this.CELL_EMPTY
                    && map[y][xPoint_16301-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16301-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
