...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_176=0;yPoint_176<this.gameFieldBorderY.length; yPoint_176++){
                for(var xPoint_176=0;xPoint_176<this.gameFieldBorderX.length; xPoint_176++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_176, xPoint_176);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_176][xPoint_176] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_176, xPoint_176, 'user');
                    if(this._userShipsMap[yPoint_176][xPoint_176] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_176, xPoint_176, type){
            var id = this.getPointBlockIdByCoords(yPoint_176, xPoint_176, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_176);
                block.setAttribute('data-y', yPoint_176);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_176
         * @param {type} xPoint_176
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_176, xPoint_176, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_176 + '_y' + yPoint_176;
            }
            return 'pc_x' + xPoint_176 + '_y' + yPoint_176;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_176=0;yPoint_176<this.gameFieldBorderY.length; yPoint_176++){
                for(var xPoint_176=0;xPoint_176<this.gameFieldBorderX.length; xPoint_176++){
                    map.push({y: yPoint_176, x: xPoint_176});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_176=-1;yPoint_176<(this.gameFieldBorderY.length+1); yPoint_176++){
                for(var xPoint_176=-1;xPoint_176<(this.gameFieldBorderX.length+1); xPoint_176++){
                    if(!map[yPoint_176]){
                        map[yPoint_176] = [];
                    }
                    map[yPoint_176][xPoint_176] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_176 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_176 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_176, yPoint_176) === true){
                    if(this.canPutHorizontal(map, xPoint_176, yPoint_176, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_176][xPoint_176 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_176, yPoint_176, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_176 + i][xPoint_176] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_176, yPoint_176){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_176][xPoint_176] === this.CELL_EMPTY
                && map[yPoint_176-1][xPoint_176] === this.CELL_EMPTY
                && map[yPoint_176-1][xPoint_176+1] === this.CELL_EMPTY
                && map[yPoint_176][xPoint_176+1] === this.CELL_EMPTY
                && map[yPoint_176+1][xPoint_176+1] === this.CELL_EMPTY
                && map[yPoint_176+1][xPoint_176] === this.CELL_EMPTY
                && map[yPoint_176+1][xPoint_176-1] === this.CELL_EMPTY
                && map[yPoint_176][xPoint_176-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_176, yPoint_176, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_176;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_176][x] === this.CELL_EMPTY
                    && map[yPoint_176-1][x] === this.CELL_EMPTY
                    && map[yPoint_176-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_176][x+1] === this.CELL_EMPTY
                    && map[yPoint_176+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_176+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_176, yPoint_176, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_176;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_176] === this.CELL_EMPTY
                    && map[y+1][xPoint_176] === this.CELL_EMPTY
                    && map[y+1][xPoint_176+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_176] === this.CELL_EMPTY
                    && map[y][xPoint_176-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_176-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
