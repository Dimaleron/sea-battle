...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31331=0;yPoint_31331<this.gameFieldBorderY.length; yPoint_31331++){
                for(var xPoint_31331=0;xPoint_31331<this.gameFieldBorderX.length; xPoint_31331++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31331, xPoint_31331);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31331][xPoint_31331] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31331, xPoint_31331, 'user');
                    if(this._userShipsMap[yPoint_31331][xPoint_31331] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31331, xPoint_31331, type){
            var id = this.getPointBlockIdByCoords(yPoint_31331, xPoint_31331, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31331);
                block.setAttribute('data-y', yPoint_31331);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31331
         * @param {type} xPoint_31331
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31331, xPoint_31331, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31331 + '_y' + yPoint_31331;
            }
            return 'pc_x' + xPoint_31331 + '_y' + yPoint_31331;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31331=0;yPoint_31331<this.gameFieldBorderY.length; yPoint_31331++){
                for(var xPoint_31331=0;xPoint_31331<this.gameFieldBorderX.length; xPoint_31331++){
                    map.push({y: yPoint_31331, x: xPoint_31331});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31331=-1;yPoint_31331<(this.gameFieldBorderY.length+1); yPoint_31331++){
                for(var xPoint_31331=-1;xPoint_31331<(this.gameFieldBorderX.length+1); xPoint_31331++){
                    if(!map[yPoint_31331]){
                        map[yPoint_31331] = [];
                    }
                    map[yPoint_31331][xPoint_31331] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31331 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31331 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31331, yPoint_31331) === true){
                    if(this.canPutHorizontal(map, xPoint_31331, yPoint_31331, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31331][xPoint_31331 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31331, yPoint_31331, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31331 + i][xPoint_31331] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31331, yPoint_31331){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31331][xPoint_31331] === this.CELL_EMPTY
                && map[yPoint_31331-1][xPoint_31331] === this.CELL_EMPTY
                && map[yPoint_31331-1][xPoint_31331+1] === this.CELL_EMPTY
                && map[yPoint_31331][xPoint_31331+1] === this.CELL_EMPTY
                && map[yPoint_31331+1][xPoint_31331+1] === this.CELL_EMPTY
                && map[yPoint_31331+1][xPoint_31331] === this.CELL_EMPTY
                && map[yPoint_31331+1][xPoint_31331-1] === this.CELL_EMPTY
                && map[yPoint_31331][xPoint_31331-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31331, yPoint_31331, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31331;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31331][x] === this.CELL_EMPTY
                    && map[yPoint_31331-1][x] === this.CELL_EMPTY
                    && map[yPoint_31331-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31331][x+1] === this.CELL_EMPTY
                    && map[yPoint_31331+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31331+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31331, yPoint_31331, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31331;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31331] === this.CELL_EMPTY
                    && map[y+1][xPoint_31331] === this.CELL_EMPTY
                    && map[y+1][xPoint_31331+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31331] === this.CELL_EMPTY
                    && map[y][xPoint_31331-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31331-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
