...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3320=0;yPoint_3320<this.gameFieldBorderY.length; yPoint_3320++){
                for(var xPoint_3320=0;xPoint_3320<this.gameFieldBorderX.length; xPoint_3320++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3320, xPoint_3320);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3320][xPoint_3320] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3320, xPoint_3320, 'user');
                    if(this._userShipsMap[yPoint_3320][xPoint_3320] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3320, xPoint_3320, type){
            var id = this.getPointBlockIdByCoords(yPoint_3320, xPoint_3320, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3320);
                block.setAttribute('data-y', yPoint_3320);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3320
         * @param {type} xPoint_3320
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3320, xPoint_3320, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3320 + '_y' + yPoint_3320;
            }
            return 'pc_x' + xPoint_3320 + '_y' + yPoint_3320;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3320=0;yPoint_3320<this.gameFieldBorderY.length; yPoint_3320++){
                for(var xPoint_3320=0;xPoint_3320<this.gameFieldBorderX.length; xPoint_3320++){
                    map.push({y: yPoint_3320, x: xPoint_3320});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3320=-1;yPoint_3320<(this.gameFieldBorderY.length+1); yPoint_3320++){
                for(var xPoint_3320=-1;xPoint_3320<(this.gameFieldBorderX.length+1); xPoint_3320++){
                    if(!map[yPoint_3320]){
                        map[yPoint_3320] = [];
                    }
                    map[yPoint_3320][xPoint_3320] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3320 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3320 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3320, yPoint_3320) === true){
                    if(this.canPutHorizontal(map, xPoint_3320, yPoint_3320, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3320][xPoint_3320 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3320, yPoint_3320, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3320 + i][xPoint_3320] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3320, yPoint_3320){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3320][xPoint_3320] === this.CELL_EMPTY
                && map[yPoint_3320-1][xPoint_3320] === this.CELL_EMPTY
                && map[yPoint_3320-1][xPoint_3320+1] === this.CELL_EMPTY
                && map[yPoint_3320][xPoint_3320+1] === this.CELL_EMPTY
                && map[yPoint_3320+1][xPoint_3320+1] === this.CELL_EMPTY
                && map[yPoint_3320+1][xPoint_3320] === this.CELL_EMPTY
                && map[yPoint_3320+1][xPoint_3320-1] === this.CELL_EMPTY
                && map[yPoint_3320][xPoint_3320-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3320, yPoint_3320, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3320;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3320][x] === this.CELL_EMPTY
                    && map[yPoint_3320-1][x] === this.CELL_EMPTY
                    && map[yPoint_3320-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3320][x+1] === this.CELL_EMPTY
                    && map[yPoint_3320+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3320+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3320, yPoint_3320, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3320;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3320] === this.CELL_EMPTY
                    && map[y+1][xPoint_3320] === this.CELL_EMPTY
                    && map[y+1][xPoint_3320+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3320] === this.CELL_EMPTY
                    && map[y][xPoint_3320-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3320-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
