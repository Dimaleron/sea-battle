...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31529=0;yPoint_31529<this.gameFieldBorderY.length; yPoint_31529++){
                for(var xPoint_31529=0;xPoint_31529<this.gameFieldBorderX.length; xPoint_31529++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31529, xPoint_31529);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31529][xPoint_31529] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31529, xPoint_31529, 'user');
                    if(this._userShipsMap[yPoint_31529][xPoint_31529] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31529, xPoint_31529, type){
            var id = this.getPointBlockIdByCoords(yPoint_31529, xPoint_31529, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31529);
                block.setAttribute('data-y', yPoint_31529);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31529
         * @param {type} xPoint_31529
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31529, xPoint_31529, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31529 + '_y' + yPoint_31529;
            }
            return 'pc_x' + xPoint_31529 + '_y' + yPoint_31529;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31529=0;yPoint_31529<this.gameFieldBorderY.length; yPoint_31529++){
                for(var xPoint_31529=0;xPoint_31529<this.gameFieldBorderX.length; xPoint_31529++){
                    map.push({y: yPoint_31529, x: xPoint_31529});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31529=-1;yPoint_31529<(this.gameFieldBorderY.length+1); yPoint_31529++){
                for(var xPoint_31529=-1;xPoint_31529<(this.gameFieldBorderX.length+1); xPoint_31529++){
                    if(!map[yPoint_31529]){
                        map[yPoint_31529] = [];
                    }
                    map[yPoint_31529][xPoint_31529] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31529 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31529 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31529, yPoint_31529) === true){
                    if(this.canPutHorizontal(map, xPoint_31529, yPoint_31529, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31529][xPoint_31529 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31529, yPoint_31529, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31529 + i][xPoint_31529] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31529, yPoint_31529){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31529][xPoint_31529] === this.CELL_EMPTY
                && map[yPoint_31529-1][xPoint_31529] === this.CELL_EMPTY
                && map[yPoint_31529-1][xPoint_31529+1] === this.CELL_EMPTY
                && map[yPoint_31529][xPoint_31529+1] === this.CELL_EMPTY
                && map[yPoint_31529+1][xPoint_31529+1] === this.CELL_EMPTY
                && map[yPoint_31529+1][xPoint_31529] === this.CELL_EMPTY
                && map[yPoint_31529+1][xPoint_31529-1] === this.CELL_EMPTY
                && map[yPoint_31529][xPoint_31529-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31529, yPoint_31529, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31529;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31529][x] === this.CELL_EMPTY
                    && map[yPoint_31529-1][x] === this.CELL_EMPTY
                    && map[yPoint_31529-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31529][x+1] === this.CELL_EMPTY
                    && map[yPoint_31529+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31529+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31529, yPoint_31529, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31529;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31529] === this.CELL_EMPTY
                    && map[y+1][xPoint_31529] === this.CELL_EMPTY
                    && map[y+1][xPoint_31529+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31529] === this.CELL_EMPTY
                    && map[y][xPoint_31529-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31529-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
