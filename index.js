...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28391=0;yPoint_28391<this.gameFieldBorderY.length; yPoint_28391++){
                for(var xPoint_28391=0;xPoint_28391<this.gameFieldBorderX.length; xPoint_28391++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28391, xPoint_28391);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28391][xPoint_28391] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28391, xPoint_28391, 'user');
                    if(this._userShipsMap[yPoint_28391][xPoint_28391] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28391, xPoint_28391, type){
            var id = this.getPointBlockIdByCoords(yPoint_28391, xPoint_28391, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28391);
                block.setAttribute('data-y', yPoint_28391);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28391
         * @param {type} xPoint_28391
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28391, xPoint_28391, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28391 + '_y' + yPoint_28391;
            }
            return 'pc_x' + xPoint_28391 + '_y' + yPoint_28391;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28391=0;yPoint_28391<this.gameFieldBorderY.length; yPoint_28391++){
                for(var xPoint_28391=0;xPoint_28391<this.gameFieldBorderX.length; xPoint_28391++){
                    map.push({y: yPoint_28391, x: xPoint_28391});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28391=-1;yPoint_28391<(this.gameFieldBorderY.length+1); yPoint_28391++){
                for(var xPoint_28391=-1;xPoint_28391<(this.gameFieldBorderX.length+1); xPoint_28391++){
                    if(!map[yPoint_28391]){
                        map[yPoint_28391] = [];
                    }
                    map[yPoint_28391][xPoint_28391] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28391 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28391 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28391, yPoint_28391) === true){
                    if(this.canPutHorizontal(map, xPoint_28391, yPoint_28391, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28391][xPoint_28391 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28391, yPoint_28391, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28391 + i][xPoint_28391] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28391, yPoint_28391){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28391][xPoint_28391] === this.CELL_EMPTY
                && map[yPoint_28391-1][xPoint_28391] === this.CELL_EMPTY
                && map[yPoint_28391-1][xPoint_28391+1] === this.CELL_EMPTY
                && map[yPoint_28391][xPoint_28391+1] === this.CELL_EMPTY
                && map[yPoint_28391+1][xPoint_28391+1] === this.CELL_EMPTY
                && map[yPoint_28391+1][xPoint_28391] === this.CELL_EMPTY
                && map[yPoint_28391+1][xPoint_28391-1] === this.CELL_EMPTY
                && map[yPoint_28391][xPoint_28391-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28391, yPoint_28391, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28391;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28391][x] === this.CELL_EMPTY
                    && map[yPoint_28391-1][x] === this.CELL_EMPTY
                    && map[yPoint_28391-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28391][x+1] === this.CELL_EMPTY
                    && map[yPoint_28391+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28391+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28391, yPoint_28391, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28391;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28391] === this.CELL_EMPTY
                    && map[y+1][xPoint_28391] === this.CELL_EMPTY
                    && map[y+1][xPoint_28391+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28391] === this.CELL_EMPTY
                    && map[y][xPoint_28391-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28391-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
