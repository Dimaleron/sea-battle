...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3600=0;yPoint_3600<this.gameFieldBorderY.length; yPoint_3600++){
                for(var xPoint_3600=0;xPoint_3600<this.gameFieldBorderX.length; xPoint_3600++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3600, xPoint_3600);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3600][xPoint_3600] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3600, xPoint_3600, 'user');
                    if(this._userShipsMap[yPoint_3600][xPoint_3600] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3600, xPoint_3600, type){
            var id = this.getPointBlockIdByCoords(yPoint_3600, xPoint_3600, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3600);
                block.setAttribute('data-y', yPoint_3600);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3600
         * @param {type} xPoint_3600
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3600, xPoint_3600, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3600 + '_y' + yPoint_3600;
            }
            return 'pc_x' + xPoint_3600 + '_y' + yPoint_3600;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3600=0;yPoint_3600<this.gameFieldBorderY.length; yPoint_3600++){
                for(var xPoint_3600=0;xPoint_3600<this.gameFieldBorderX.length; xPoint_3600++){
                    map.push({y: yPoint_3600, x: xPoint_3600});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3600=-1;yPoint_3600<(this.gameFieldBorderY.length+1); yPoint_3600++){
                for(var xPoint_3600=-1;xPoint_3600<(this.gameFieldBorderX.length+1); xPoint_3600++){
                    if(!map[yPoint_3600]){
                        map[yPoint_3600] = [];
                    }
                    map[yPoint_3600][xPoint_3600] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3600 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3600 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3600, yPoint_3600) === true){
                    if(this.canPutHorizontal(map, xPoint_3600, yPoint_3600, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3600][xPoint_3600 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3600, yPoint_3600, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3600 + i][xPoint_3600] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3600, yPoint_3600){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3600][xPoint_3600] === this.CELL_EMPTY
                && map[yPoint_3600-1][xPoint_3600] === this.CELL_EMPTY
                && map[yPoint_3600-1][xPoint_3600+1] === this.CELL_EMPTY
                && map[yPoint_3600][xPoint_3600+1] === this.CELL_EMPTY
                && map[yPoint_3600+1][xPoint_3600+1] === this.CELL_EMPTY
                && map[yPoint_3600+1][xPoint_3600] === this.CELL_EMPTY
                && map[yPoint_3600+1][xPoint_3600-1] === this.CELL_EMPTY
                && map[yPoint_3600][xPoint_3600-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3600, yPoint_3600, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3600;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3600][x] === this.CELL_EMPTY
                    && map[yPoint_3600-1][x] === this.CELL_EMPTY
                    && map[yPoint_3600-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3600][x+1] === this.CELL_EMPTY
                    && map[yPoint_3600+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3600+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3600, yPoint_3600, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3600;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3600] === this.CELL_EMPTY
                    && map[y+1][xPoint_3600] === this.CELL_EMPTY
                    && map[y+1][xPoint_3600+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3600] === this.CELL_EMPTY
                    && map[y][xPoint_3600-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3600-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
