...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_692=0;yPoint_692<this.gameFieldBorderY.length; yPoint_692++){
                for(var xPoint_692=0;xPoint_692<this.gameFieldBorderX.length; xPoint_692++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_692, xPoint_692);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_692][xPoint_692] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_692, xPoint_692, 'user');
                    if(this._userShipsMap[yPoint_692][xPoint_692] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_692, xPoint_692, type){
            var id = this.getPointBlockIdByCoords(yPoint_692, xPoint_692, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_692);
                block.setAttribute('data-y', yPoint_692);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_692
         * @param {type} xPoint_692
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_692, xPoint_692, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_692 + '_y' + yPoint_692;
            }
            return 'pc_x' + xPoint_692 + '_y' + yPoint_692;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_692=0;yPoint_692<this.gameFieldBorderY.length; yPoint_692++){
                for(var xPoint_692=0;xPoint_692<this.gameFieldBorderX.length; xPoint_692++){
                    map.push({y: yPoint_692, x: xPoint_692});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_692=-1;yPoint_692<(this.gameFieldBorderY.length+1); yPoint_692++){
                for(var xPoint_692=-1;xPoint_692<(this.gameFieldBorderX.length+1); xPoint_692++){
                    if(!map[yPoint_692]){
                        map[yPoint_692] = [];
                    }
                    map[yPoint_692][xPoint_692] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_692 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_692 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_692, yPoint_692) === true){
                    if(this.canPutHorizontal(map, xPoint_692, yPoint_692, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_692][xPoint_692 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_692, yPoint_692, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_692 + i][xPoint_692] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_692, yPoint_692){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_692][xPoint_692] === this.CELL_EMPTY
                && map[yPoint_692-1][xPoint_692] === this.CELL_EMPTY
                && map[yPoint_692-1][xPoint_692+1] === this.CELL_EMPTY
                && map[yPoint_692][xPoint_692+1] === this.CELL_EMPTY
                && map[yPoint_692+1][xPoint_692+1] === this.CELL_EMPTY
                && map[yPoint_692+1][xPoint_692] === this.CELL_EMPTY
                && map[yPoint_692+1][xPoint_692-1] === this.CELL_EMPTY
                && map[yPoint_692][xPoint_692-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_692, yPoint_692, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_692;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_692][x] === this.CELL_EMPTY
                    && map[yPoint_692-1][x] === this.CELL_EMPTY
                    && map[yPoint_692-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_692][x+1] === this.CELL_EMPTY
                    && map[yPoint_692+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_692+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_692, yPoint_692, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_692;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_692] === this.CELL_EMPTY
                    && map[y+1][xPoint_692] === this.CELL_EMPTY
                    && map[y+1][xPoint_692+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_692] === this.CELL_EMPTY
                    && map[y][xPoint_692-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_692-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
