...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24971=0;yPoint_24971<this.gameFieldBorderY.length; yPoint_24971++){
                for(var xPoint_24971=0;xPoint_24971<this.gameFieldBorderX.length; xPoint_24971++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24971, xPoint_24971);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24971][xPoint_24971] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24971, xPoint_24971, 'user');
                    if(this._userShipsMap[yPoint_24971][xPoint_24971] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24971, xPoint_24971, type){
            var id = this.getPointBlockIdByCoords(yPoint_24971, xPoint_24971, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24971);
                block.setAttribute('data-y', yPoint_24971);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24971
         * @param {type} xPoint_24971
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24971, xPoint_24971, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24971 + '_y' + yPoint_24971;
            }
            return 'pc_x' + xPoint_24971 + '_y' + yPoint_24971;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24971=0;yPoint_24971<this.gameFieldBorderY.length; yPoint_24971++){
                for(var xPoint_24971=0;xPoint_24971<this.gameFieldBorderX.length; xPoint_24971++){
                    map.push({y: yPoint_24971, x: xPoint_24971});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24971=-1;yPoint_24971<(this.gameFieldBorderY.length+1); yPoint_24971++){
                for(var xPoint_24971=-1;xPoint_24971<(this.gameFieldBorderX.length+1); xPoint_24971++){
                    if(!map[yPoint_24971]){
                        map[yPoint_24971] = [];
                    }
                    map[yPoint_24971][xPoint_24971] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24971 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24971 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24971, yPoint_24971) === true){
                    if(this.canPutHorizontal(map, xPoint_24971, yPoint_24971, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24971][xPoint_24971 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24971, yPoint_24971, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24971 + i][xPoint_24971] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24971, yPoint_24971){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24971][xPoint_24971] === this.CELL_EMPTY
                && map[yPoint_24971-1][xPoint_24971] === this.CELL_EMPTY
                && map[yPoint_24971-1][xPoint_24971+1] === this.CELL_EMPTY
                && map[yPoint_24971][xPoint_24971+1] === this.CELL_EMPTY
                && map[yPoint_24971+1][xPoint_24971+1] === this.CELL_EMPTY
                && map[yPoint_24971+1][xPoint_24971] === this.CELL_EMPTY
                && map[yPoint_24971+1][xPoint_24971-1] === this.CELL_EMPTY
                && map[yPoint_24971][xPoint_24971-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24971, yPoint_24971, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24971;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24971][x] === this.CELL_EMPTY
                    && map[yPoint_24971-1][x] === this.CELL_EMPTY
                    && map[yPoint_24971-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24971][x+1] === this.CELL_EMPTY
                    && map[yPoint_24971+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24971+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24971, yPoint_24971, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24971;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24971] === this.CELL_EMPTY
                    && map[y+1][xPoint_24971] === this.CELL_EMPTY
                    && map[y+1][xPoint_24971+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24971] === this.CELL_EMPTY
                    && map[y][xPoint_24971-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24971-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
