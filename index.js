...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6182=0;yPoint_6182<this.gameFieldBorderY.length; yPoint_6182++){
                for(var xPoint_6182=0;xPoint_6182<this.gameFieldBorderX.length; xPoint_6182++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6182, xPoint_6182);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6182][xPoint_6182] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6182, xPoint_6182, 'user');
                    if(this._userShipsMap[yPoint_6182][xPoint_6182] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6182, xPoint_6182, type){
            var id = this.getPointBlockIdByCoords(yPoint_6182, xPoint_6182, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6182);
                block.setAttribute('data-y', yPoint_6182);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6182
         * @param {type} xPoint_6182
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6182, xPoint_6182, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6182 + '_y' + yPoint_6182;
            }
            return 'pc_x' + xPoint_6182 + '_y' + yPoint_6182;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6182=0;yPoint_6182<this.gameFieldBorderY.length; yPoint_6182++){
                for(var xPoint_6182=0;xPoint_6182<this.gameFieldBorderX.length; xPoint_6182++){
                    map.push({y: yPoint_6182, x: xPoint_6182});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6182=-1;yPoint_6182<(this.gameFieldBorderY.length+1); yPoint_6182++){
                for(var xPoint_6182=-1;xPoint_6182<(this.gameFieldBorderX.length+1); xPoint_6182++){
                    if(!map[yPoint_6182]){
                        map[yPoint_6182] = [];
                    }
                    map[yPoint_6182][xPoint_6182] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6182 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6182 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6182, yPoint_6182) === true){
                    if(this.canPutHorizontal(map, xPoint_6182, yPoint_6182, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6182][xPoint_6182 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6182, yPoint_6182, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6182 + i][xPoint_6182] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6182, yPoint_6182){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6182][xPoint_6182] === this.CELL_EMPTY
                && map[yPoint_6182-1][xPoint_6182] === this.CELL_EMPTY
                && map[yPoint_6182-1][xPoint_6182+1] === this.CELL_EMPTY
                && map[yPoint_6182][xPoint_6182+1] === this.CELL_EMPTY
                && map[yPoint_6182+1][xPoint_6182+1] === this.CELL_EMPTY
                && map[yPoint_6182+1][xPoint_6182] === this.CELL_EMPTY
                && map[yPoint_6182+1][xPoint_6182-1] === this.CELL_EMPTY
                && map[yPoint_6182][xPoint_6182-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6182, yPoint_6182, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6182;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6182][x] === this.CELL_EMPTY
                    && map[yPoint_6182-1][x] === this.CELL_EMPTY
                    && map[yPoint_6182-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6182][x+1] === this.CELL_EMPTY
                    && map[yPoint_6182+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6182+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6182, yPoint_6182, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6182;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6182] === this.CELL_EMPTY
                    && map[y+1][xPoint_6182] === this.CELL_EMPTY
                    && map[y+1][xPoint_6182+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6182] === this.CELL_EMPTY
                    && map[y][xPoint_6182-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6182-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
