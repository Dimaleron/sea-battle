...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28788=0;yPoint_28788<this.gameFieldBorderY.length; yPoint_28788++){
                for(var xPoint_28788=0;xPoint_28788<this.gameFieldBorderX.length; xPoint_28788++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28788, xPoint_28788);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28788][xPoint_28788] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28788, xPoint_28788, 'user');
                    if(this._userShipsMap[yPoint_28788][xPoint_28788] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28788, xPoint_28788, type){
            var id = this.getPointBlockIdByCoords(yPoint_28788, xPoint_28788, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28788);
                block.setAttribute('data-y', yPoint_28788);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28788
         * @param {type} xPoint_28788
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28788, xPoint_28788, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28788 + '_y' + yPoint_28788;
            }
            return 'pc_x' + xPoint_28788 + '_y' + yPoint_28788;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28788=0;yPoint_28788<this.gameFieldBorderY.length; yPoint_28788++){
                for(var xPoint_28788=0;xPoint_28788<this.gameFieldBorderX.length; xPoint_28788++){
                    map.push({y: yPoint_28788, x: xPoint_28788});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28788=-1;yPoint_28788<(this.gameFieldBorderY.length+1); yPoint_28788++){
                for(var xPoint_28788=-1;xPoint_28788<(this.gameFieldBorderX.length+1); xPoint_28788++){
                    if(!map[yPoint_28788]){
                        map[yPoint_28788] = [];
                    }
                    map[yPoint_28788][xPoint_28788] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28788 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28788 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28788, yPoint_28788) === true){
                    if(this.canPutHorizontal(map, xPoint_28788, yPoint_28788, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28788][xPoint_28788 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28788, yPoint_28788, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28788 + i][xPoint_28788] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28788, yPoint_28788){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28788][xPoint_28788] === this.CELL_EMPTY
                && map[yPoint_28788-1][xPoint_28788] === this.CELL_EMPTY
                && map[yPoint_28788-1][xPoint_28788+1] === this.CELL_EMPTY
                && map[yPoint_28788][xPoint_28788+1] === this.CELL_EMPTY
                && map[yPoint_28788+1][xPoint_28788+1] === this.CELL_EMPTY
                && map[yPoint_28788+1][xPoint_28788] === this.CELL_EMPTY
                && map[yPoint_28788+1][xPoint_28788-1] === this.CELL_EMPTY
                && map[yPoint_28788][xPoint_28788-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28788, yPoint_28788, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28788;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28788][x] === this.CELL_EMPTY
                    && map[yPoint_28788-1][x] === this.CELL_EMPTY
                    && map[yPoint_28788-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28788][x+1] === this.CELL_EMPTY
                    && map[yPoint_28788+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28788+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28788, yPoint_28788, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28788;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28788] === this.CELL_EMPTY
                    && map[y+1][xPoint_28788] === this.CELL_EMPTY
                    && map[y+1][xPoint_28788+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28788] === this.CELL_EMPTY
                    && map[y][xPoint_28788-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28788-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
