...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_456=0;yPoint_456<this.gameFieldBorderY.length; yPoint_456++){
                for(var xPoint_456=0;xPoint_456<this.gameFieldBorderX.length; xPoint_456++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_456, xPoint_456);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_456][xPoint_456] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_456, xPoint_456, 'user');
                    if(this._userShipsMap[yPoint_456][xPoint_456] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_456, xPoint_456, type){
            var id = this.getPointBlockIdByCoords(yPoint_456, xPoint_456, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_456);
                block.setAttribute('data-y', yPoint_456);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_456
         * @param {type} xPoint_456
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_456, xPoint_456, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_456 + '_y' + yPoint_456;
            }
            return 'pc_x' + xPoint_456 + '_y' + yPoint_456;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_456=0;yPoint_456<this.gameFieldBorderY.length; yPoint_456++){
                for(var xPoint_456=0;xPoint_456<this.gameFieldBorderX.length; xPoint_456++){
                    map.push({y: yPoint_456, x: xPoint_456});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_456=-1;yPoint_456<(this.gameFieldBorderY.length+1); yPoint_456++){
                for(var xPoint_456=-1;xPoint_456<(this.gameFieldBorderX.length+1); xPoint_456++){
                    if(!map[yPoint_456]){
                        map[yPoint_456] = [];
                    }
                    map[yPoint_456][xPoint_456] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_456 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_456 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_456, yPoint_456) === true){
                    if(this.canPutHorizontal(map, xPoint_456, yPoint_456, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_456][xPoint_456 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_456, yPoint_456, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_456 + i][xPoint_456] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_456, yPoint_456){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_456][xPoint_456] === this.CELL_EMPTY
                && map[yPoint_456-1][xPoint_456] === this.CELL_EMPTY
                && map[yPoint_456-1][xPoint_456+1] === this.CELL_EMPTY
                && map[yPoint_456][xPoint_456+1] === this.CELL_EMPTY
                && map[yPoint_456+1][xPoint_456+1] === this.CELL_EMPTY
                && map[yPoint_456+1][xPoint_456] === this.CELL_EMPTY
                && map[yPoint_456+1][xPoint_456-1] === this.CELL_EMPTY
                && map[yPoint_456][xPoint_456-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_456, yPoint_456, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_456;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_456][x] === this.CELL_EMPTY
                    && map[yPoint_456-1][x] === this.CELL_EMPTY
                    && map[yPoint_456-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_456][x+1] === this.CELL_EMPTY
                    && map[yPoint_456+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_456+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_456, yPoint_456, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_456;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_456] === this.CELL_EMPTY
                    && map[y+1][xPoint_456] === this.CELL_EMPTY
                    && map[y+1][xPoint_456+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_456] === this.CELL_EMPTY
                    && map[y][xPoint_456-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_456-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
