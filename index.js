...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31607=0;yPoint_31607<this.gameFieldBorderY.length; yPoint_31607++){
                for(var xPoint_31607=0;xPoint_31607<this.gameFieldBorderX.length; xPoint_31607++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31607, xPoint_31607);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31607][xPoint_31607] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31607, xPoint_31607, 'user');
                    if(this._userShipsMap[yPoint_31607][xPoint_31607] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31607, xPoint_31607, type){
            var id = this.getPointBlockIdByCoords(yPoint_31607, xPoint_31607, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31607);
                block.setAttribute('data-y', yPoint_31607);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31607
         * @param {type} xPoint_31607
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31607, xPoint_31607, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31607 + '_y' + yPoint_31607;
            }
            return 'pc_x' + xPoint_31607 + '_y' + yPoint_31607;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31607=0;yPoint_31607<this.gameFieldBorderY.length; yPoint_31607++){
                for(var xPoint_31607=0;xPoint_31607<this.gameFieldBorderX.length; xPoint_31607++){
                    map.push({y: yPoint_31607, x: xPoint_31607});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31607=-1;yPoint_31607<(this.gameFieldBorderY.length+1); yPoint_31607++){
                for(var xPoint_31607=-1;xPoint_31607<(this.gameFieldBorderX.length+1); xPoint_31607++){
                    if(!map[yPoint_31607]){
                        map[yPoint_31607] = [];
                    }
                    map[yPoint_31607][xPoint_31607] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31607 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31607 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31607, yPoint_31607) === true){
                    if(this.canPutHorizontal(map, xPoint_31607, yPoint_31607, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31607][xPoint_31607 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31607, yPoint_31607, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31607 + i][xPoint_31607] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31607, yPoint_31607){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31607][xPoint_31607] === this.CELL_EMPTY
                && map[yPoint_31607-1][xPoint_31607] === this.CELL_EMPTY
                && map[yPoint_31607-1][xPoint_31607+1] === this.CELL_EMPTY
                && map[yPoint_31607][xPoint_31607+1] === this.CELL_EMPTY
                && map[yPoint_31607+1][xPoint_31607+1] === this.CELL_EMPTY
                && map[yPoint_31607+1][xPoint_31607] === this.CELL_EMPTY
                && map[yPoint_31607+1][xPoint_31607-1] === this.CELL_EMPTY
                && map[yPoint_31607][xPoint_31607-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31607, yPoint_31607, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31607;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31607][x] === this.CELL_EMPTY
                    && map[yPoint_31607-1][x] === this.CELL_EMPTY
                    && map[yPoint_31607-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31607][x+1] === this.CELL_EMPTY
                    && map[yPoint_31607+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31607+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31607, yPoint_31607, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31607;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31607] === this.CELL_EMPTY
                    && map[y+1][xPoint_31607] === this.CELL_EMPTY
                    && map[y+1][xPoint_31607+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31607] === this.CELL_EMPTY
                    && map[y][xPoint_31607-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31607-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
