...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1140=0;yPoint_1140<this.gameFieldBorderY.length; yPoint_1140++){
                for(var xPoint_1140=0;xPoint_1140<this.gameFieldBorderX.length; xPoint_1140++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1140, xPoint_1140);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1140][xPoint_1140] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1140, xPoint_1140, 'user');
                    if(this._userShipsMap[yPoint_1140][xPoint_1140] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1140, xPoint_1140, type){
            var id = this.getPointBlockIdByCoords(yPoint_1140, xPoint_1140, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1140);
                block.setAttribute('data-y', yPoint_1140);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1140
         * @param {type} xPoint_1140
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1140, xPoint_1140, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1140 + '_y' + yPoint_1140;
            }
            return 'pc_x' + xPoint_1140 + '_y' + yPoint_1140;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1140=0;yPoint_1140<this.gameFieldBorderY.length; yPoint_1140++){
                for(var xPoint_1140=0;xPoint_1140<this.gameFieldBorderX.length; xPoint_1140++){
                    map.push({y: yPoint_1140, x: xPoint_1140});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1140=-1;yPoint_1140<(this.gameFieldBorderY.length+1); yPoint_1140++){
                for(var xPoint_1140=-1;xPoint_1140<(this.gameFieldBorderX.length+1); xPoint_1140++){
                    if(!map[yPoint_1140]){
                        map[yPoint_1140] = [];
                    }
                    map[yPoint_1140][xPoint_1140] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1140 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1140 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1140, yPoint_1140) === true){
                    if(this.canPutHorizontal(map, xPoint_1140, yPoint_1140, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1140][xPoint_1140 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1140, yPoint_1140, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1140 + i][xPoint_1140] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1140, yPoint_1140){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1140][xPoint_1140] === this.CELL_EMPTY
                && map[yPoint_1140-1][xPoint_1140] === this.CELL_EMPTY
                && map[yPoint_1140-1][xPoint_1140+1] === this.CELL_EMPTY
                && map[yPoint_1140][xPoint_1140+1] === this.CELL_EMPTY
                && map[yPoint_1140+1][xPoint_1140+1] === this.CELL_EMPTY
                && map[yPoint_1140+1][xPoint_1140] === this.CELL_EMPTY
                && map[yPoint_1140+1][xPoint_1140-1] === this.CELL_EMPTY
                && map[yPoint_1140][xPoint_1140-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1140, yPoint_1140, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1140;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1140][x] === this.CELL_EMPTY
                    && map[yPoint_1140-1][x] === this.CELL_EMPTY
                    && map[yPoint_1140-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1140][x+1] === this.CELL_EMPTY
                    && map[yPoint_1140+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1140+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1140, yPoint_1140, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1140;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1140] === this.CELL_EMPTY
                    && map[y+1][xPoint_1140] === this.CELL_EMPTY
                    && map[y+1][xPoint_1140+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1140] === this.CELL_EMPTY
                    && map[y][xPoint_1140-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1140-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
