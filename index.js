...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28996=0;yPoint_28996<this.gameFieldBorderY.length; yPoint_28996++){
                for(var xPoint_28996=0;xPoint_28996<this.gameFieldBorderX.length; xPoint_28996++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28996, xPoint_28996);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28996][xPoint_28996] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28996, xPoint_28996, 'user');
                    if(this._userShipsMap[yPoint_28996][xPoint_28996] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28996, xPoint_28996, type){
            var id = this.getPointBlockIdByCoords(yPoint_28996, xPoint_28996, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28996);
                block.setAttribute('data-y', yPoint_28996);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28996
         * @param {type} xPoint_28996
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28996, xPoint_28996, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28996 + '_y' + yPoint_28996;
            }
            return 'pc_x' + xPoint_28996 + '_y' + yPoint_28996;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28996=0;yPoint_28996<this.gameFieldBorderY.length; yPoint_28996++){
                for(var xPoint_28996=0;xPoint_28996<this.gameFieldBorderX.length; xPoint_28996++){
                    map.push({y: yPoint_28996, x: xPoint_28996});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28996=-1;yPoint_28996<(this.gameFieldBorderY.length+1); yPoint_28996++){
                for(var xPoint_28996=-1;xPoint_28996<(this.gameFieldBorderX.length+1); xPoint_28996++){
                    if(!map[yPoint_28996]){
                        map[yPoint_28996] = [];
                    }
                    map[yPoint_28996][xPoint_28996] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28996 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28996 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28996, yPoint_28996) === true){
                    if(this.canPutHorizontal(map, xPoint_28996, yPoint_28996, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28996][xPoint_28996 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28996, yPoint_28996, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28996 + i][xPoint_28996] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28996, yPoint_28996){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28996][xPoint_28996] === this.CELL_EMPTY
                && map[yPoint_28996-1][xPoint_28996] === this.CELL_EMPTY
                && map[yPoint_28996-1][xPoint_28996+1] === this.CELL_EMPTY
                && map[yPoint_28996][xPoint_28996+1] === this.CELL_EMPTY
                && map[yPoint_28996+1][xPoint_28996+1] === this.CELL_EMPTY
                && map[yPoint_28996+1][xPoint_28996] === this.CELL_EMPTY
                && map[yPoint_28996+1][xPoint_28996-1] === this.CELL_EMPTY
                && map[yPoint_28996][xPoint_28996-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28996, yPoint_28996, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28996;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28996][x] === this.CELL_EMPTY
                    && map[yPoint_28996-1][x] === this.CELL_EMPTY
                    && map[yPoint_28996-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28996][x+1] === this.CELL_EMPTY
                    && map[yPoint_28996+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28996+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28996, yPoint_28996, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28996;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28996] === this.CELL_EMPTY
                    && map[y+1][xPoint_28996] === this.CELL_EMPTY
                    && map[y+1][xPoint_28996+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28996] === this.CELL_EMPTY
                    && map[y][xPoint_28996-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28996-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
