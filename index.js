...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30915=0;yPoint_30915<this.gameFieldBorderY.length; yPoint_30915++){
                for(var xPoint_30915=0;xPoint_30915<this.gameFieldBorderX.length; xPoint_30915++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30915, xPoint_30915);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30915][xPoint_30915] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30915, xPoint_30915, 'user');
                    if(this._userShipsMap[yPoint_30915][xPoint_30915] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30915, xPoint_30915, type){
            var id = this.getPointBlockIdByCoords(yPoint_30915, xPoint_30915, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30915);
                block.setAttribute('data-y', yPoint_30915);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30915
         * @param {type} xPoint_30915
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30915, xPoint_30915, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30915 + '_y' + yPoint_30915;
            }
            return 'pc_x' + xPoint_30915 + '_y' + yPoint_30915;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30915=0;yPoint_30915<this.gameFieldBorderY.length; yPoint_30915++){
                for(var xPoint_30915=0;xPoint_30915<this.gameFieldBorderX.length; xPoint_30915++){
                    map.push({y: yPoint_30915, x: xPoint_30915});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30915=-1;yPoint_30915<(this.gameFieldBorderY.length+1); yPoint_30915++){
                for(var xPoint_30915=-1;xPoint_30915<(this.gameFieldBorderX.length+1); xPoint_30915++){
                    if(!map[yPoint_30915]){
                        map[yPoint_30915] = [];
                    }
                    map[yPoint_30915][xPoint_30915] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30915 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30915 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30915, yPoint_30915) === true){
                    if(this.canPutHorizontal(map, xPoint_30915, yPoint_30915, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30915][xPoint_30915 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30915, yPoint_30915, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30915 + i][xPoint_30915] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30915, yPoint_30915){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30915][xPoint_30915] === this.CELL_EMPTY
                && map[yPoint_30915-1][xPoint_30915] === this.CELL_EMPTY
                && map[yPoint_30915-1][xPoint_30915+1] === this.CELL_EMPTY
                && map[yPoint_30915][xPoint_30915+1] === this.CELL_EMPTY
                && map[yPoint_30915+1][xPoint_30915+1] === this.CELL_EMPTY
                && map[yPoint_30915+1][xPoint_30915] === this.CELL_EMPTY
                && map[yPoint_30915+1][xPoint_30915-1] === this.CELL_EMPTY
                && map[yPoint_30915][xPoint_30915-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30915, yPoint_30915, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30915;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30915][x] === this.CELL_EMPTY
                    && map[yPoint_30915-1][x] === this.CELL_EMPTY
                    && map[yPoint_30915-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30915][x+1] === this.CELL_EMPTY
                    && map[yPoint_30915+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30915+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30915, yPoint_30915, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30915;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30915] === this.CELL_EMPTY
                    && map[y+1][xPoint_30915] === this.CELL_EMPTY
                    && map[y+1][xPoint_30915+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30915] === this.CELL_EMPTY
                    && map[y][xPoint_30915-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30915-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
