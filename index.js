...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_933=0;yPoint_933<this.gameFieldBorderY.length; yPoint_933++){
                for(var xPoint_933=0;xPoint_933<this.gameFieldBorderX.length; xPoint_933++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_933, xPoint_933);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_933][xPoint_933] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_933, xPoint_933, 'user');
                    if(this._userShipsMap[yPoint_933][xPoint_933] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_933, xPoint_933, type){
            var id = this.getPointBlockIdByCoords(yPoint_933, xPoint_933, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_933);
                block.setAttribute('data-y', yPoint_933);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_933
         * @param {type} xPoint_933
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_933, xPoint_933, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_933 + '_y' + yPoint_933;
            }
            return 'pc_x' + xPoint_933 + '_y' + yPoint_933;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_933=0;yPoint_933<this.gameFieldBorderY.length; yPoint_933++){
                for(var xPoint_933=0;xPoint_933<this.gameFieldBorderX.length; xPoint_933++){
                    map.push({y: yPoint_933, x: xPoint_933});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_933=-1;yPoint_933<(this.gameFieldBorderY.length+1); yPoint_933++){
                for(var xPoint_933=-1;xPoint_933<(this.gameFieldBorderX.length+1); xPoint_933++){
                    if(!map[yPoint_933]){
                        map[yPoint_933] = [];
                    }
                    map[yPoint_933][xPoint_933] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_933 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_933 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_933, yPoint_933) === true){
                    if(this.canPutHorizontal(map, xPoint_933, yPoint_933, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_933][xPoint_933 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_933, yPoint_933, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_933 + i][xPoint_933] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_933, yPoint_933){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_933][xPoint_933] === this.CELL_EMPTY
                && map[yPoint_933-1][xPoint_933] === this.CELL_EMPTY
                && map[yPoint_933-1][xPoint_933+1] === this.CELL_EMPTY
                && map[yPoint_933][xPoint_933+1] === this.CELL_EMPTY
                && map[yPoint_933+1][xPoint_933+1] === this.CELL_EMPTY
                && map[yPoint_933+1][xPoint_933] === this.CELL_EMPTY
                && map[yPoint_933+1][xPoint_933-1] === this.CELL_EMPTY
                && map[yPoint_933][xPoint_933-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_933, yPoint_933, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_933;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_933][x] === this.CELL_EMPTY
                    && map[yPoint_933-1][x] === this.CELL_EMPTY
                    && map[yPoint_933-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_933][x+1] === this.CELL_EMPTY
                    && map[yPoint_933+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_933+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_933, yPoint_933, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_933;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_933] === this.CELL_EMPTY
                    && map[y+1][xPoint_933] === this.CELL_EMPTY
                    && map[y+1][xPoint_933+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_933] === this.CELL_EMPTY
                    && map[y][xPoint_933-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_933-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
