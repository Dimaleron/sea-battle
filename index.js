...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16955=0;yPoint_16955<this.gameFieldBorderY.length; yPoint_16955++){
                for(var xPoint_16955=0;xPoint_16955<this.gameFieldBorderX.length; xPoint_16955++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16955, xPoint_16955);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16955][xPoint_16955] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16955, xPoint_16955, 'user');
                    if(this._userShipsMap[yPoint_16955][xPoint_16955] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16955, xPoint_16955, type){
            var id = this.getPointBlockIdByCoords(yPoint_16955, xPoint_16955, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16955);
                block.setAttribute('data-y', yPoint_16955);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16955
         * @param {type} xPoint_16955
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16955, xPoint_16955, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16955 + '_y' + yPoint_16955;
            }
            return 'pc_x' + xPoint_16955 + '_y' + yPoint_16955;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16955=0;yPoint_16955<this.gameFieldBorderY.length; yPoint_16955++){
                for(var xPoint_16955=0;xPoint_16955<this.gameFieldBorderX.length; xPoint_16955++){
                    map.push({y: yPoint_16955, x: xPoint_16955});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16955=-1;yPoint_16955<(this.gameFieldBorderY.length+1); yPoint_16955++){
                for(var xPoint_16955=-1;xPoint_16955<(this.gameFieldBorderX.length+1); xPoint_16955++){
                    if(!map[yPoint_16955]){
                        map[yPoint_16955] = [];
                    }
                    map[yPoint_16955][xPoint_16955] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16955 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16955 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16955, yPoint_16955) === true){
                    if(this.canPutHorizontal(map, xPoint_16955, yPoint_16955, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16955][xPoint_16955 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16955, yPoint_16955, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16955 + i][xPoint_16955] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16955, yPoint_16955){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16955][xPoint_16955] === this.CELL_EMPTY
                && map[yPoint_16955-1][xPoint_16955] === this.CELL_EMPTY
                && map[yPoint_16955-1][xPoint_16955+1] === this.CELL_EMPTY
                && map[yPoint_16955][xPoint_16955+1] === this.CELL_EMPTY
                && map[yPoint_16955+1][xPoint_16955+1] === this.CELL_EMPTY
                && map[yPoint_16955+1][xPoint_16955] === this.CELL_EMPTY
                && map[yPoint_16955+1][xPoint_16955-1] === this.CELL_EMPTY
                && map[yPoint_16955][xPoint_16955-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16955, yPoint_16955, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16955;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16955][x] === this.CELL_EMPTY
                    && map[yPoint_16955-1][x] === this.CELL_EMPTY
                    && map[yPoint_16955-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16955][x+1] === this.CELL_EMPTY
                    && map[yPoint_16955+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16955+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16955, yPoint_16955, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16955;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16955] === this.CELL_EMPTY
                    && map[y+1][xPoint_16955] === this.CELL_EMPTY
                    && map[y+1][xPoint_16955+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16955] === this.CELL_EMPTY
                    && map[y][xPoint_16955-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16955-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
