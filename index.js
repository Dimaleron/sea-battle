...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14477=0;yPoint_14477<this.gameFieldBorderY.length; yPoint_14477++){
                for(var xPoint_14477=0;xPoint_14477<this.gameFieldBorderX.length; xPoint_14477++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14477, xPoint_14477);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14477][xPoint_14477] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14477, xPoint_14477, 'user');
                    if(this._userShipsMap[yPoint_14477][xPoint_14477] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14477, xPoint_14477, type){
            var id = this.getPointBlockIdByCoords(yPoint_14477, xPoint_14477, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14477);
                block.setAttribute('data-y', yPoint_14477);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14477
         * @param {type} xPoint_14477
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14477, xPoint_14477, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14477 + '_y' + yPoint_14477;
            }
            return 'pc_x' + xPoint_14477 + '_y' + yPoint_14477;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14477=0;yPoint_14477<this.gameFieldBorderY.length; yPoint_14477++){
                for(var xPoint_14477=0;xPoint_14477<this.gameFieldBorderX.length; xPoint_14477++){
                    map.push({y: yPoint_14477, x: xPoint_14477});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14477=-1;yPoint_14477<(this.gameFieldBorderY.length+1); yPoint_14477++){
                for(var xPoint_14477=-1;xPoint_14477<(this.gameFieldBorderX.length+1); xPoint_14477++){
                    if(!map[yPoint_14477]){
                        map[yPoint_14477] = [];
                    }
                    map[yPoint_14477][xPoint_14477] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14477 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14477 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14477, yPoint_14477) === true){
                    if(this.canPutHorizontal(map, xPoint_14477, yPoint_14477, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14477][xPoint_14477 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14477, yPoint_14477, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14477 + i][xPoint_14477] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14477, yPoint_14477){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14477][xPoint_14477] === this.CELL_EMPTY
                && map[yPoint_14477-1][xPoint_14477] === this.CELL_EMPTY
                && map[yPoint_14477-1][xPoint_14477+1] === this.CELL_EMPTY
                && map[yPoint_14477][xPoint_14477+1] === this.CELL_EMPTY
                && map[yPoint_14477+1][xPoint_14477+1] === this.CELL_EMPTY
                && map[yPoint_14477+1][xPoint_14477] === this.CELL_EMPTY
                && map[yPoint_14477+1][xPoint_14477-1] === this.CELL_EMPTY
                && map[yPoint_14477][xPoint_14477-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14477, yPoint_14477, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14477;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14477][x] === this.CELL_EMPTY
                    && map[yPoint_14477-1][x] === this.CELL_EMPTY
                    && map[yPoint_14477-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14477][x+1] === this.CELL_EMPTY
                    && map[yPoint_14477+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14477+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14477, yPoint_14477, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14477;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14477] === this.CELL_EMPTY
                    && map[y+1][xPoint_14477] === this.CELL_EMPTY
                    && map[y+1][xPoint_14477+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14477] === this.CELL_EMPTY
                    && map[y][xPoint_14477-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14477-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
