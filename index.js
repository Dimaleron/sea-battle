...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7398=0;yPoint_7398<this.gameFieldBorderY.length; yPoint_7398++){
                for(var xPoint_7398=0;xPoint_7398<this.gameFieldBorderX.length; xPoint_7398++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7398, xPoint_7398);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7398][xPoint_7398] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7398, xPoint_7398, 'user');
                    if(this._userShipsMap[yPoint_7398][xPoint_7398] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7398, xPoint_7398, type){
            var id = this.getPointBlockIdByCoords(yPoint_7398, xPoint_7398, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7398);
                block.setAttribute('data-y', yPoint_7398);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7398
         * @param {type} xPoint_7398
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7398, xPoint_7398, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7398 + '_y' + yPoint_7398;
            }
            return 'pc_x' + xPoint_7398 + '_y' + yPoint_7398;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7398=0;yPoint_7398<this.gameFieldBorderY.length; yPoint_7398++){
                for(var xPoint_7398=0;xPoint_7398<this.gameFieldBorderX.length; xPoint_7398++){
                    map.push({y: yPoint_7398, x: xPoint_7398});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7398=-1;yPoint_7398<(this.gameFieldBorderY.length+1); yPoint_7398++){
                for(var xPoint_7398=-1;xPoint_7398<(this.gameFieldBorderX.length+1); xPoint_7398++){
                    if(!map[yPoint_7398]){
                        map[yPoint_7398] = [];
                    }
                    map[yPoint_7398][xPoint_7398] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7398 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7398 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7398, yPoint_7398) === true){
                    if(this.canPutHorizontal(map, xPoint_7398, yPoint_7398, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7398][xPoint_7398 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7398, yPoint_7398, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7398 + i][xPoint_7398] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7398, yPoint_7398){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7398][xPoint_7398] === this.CELL_EMPTY
                && map[yPoint_7398-1][xPoint_7398] === this.CELL_EMPTY
                && map[yPoint_7398-1][xPoint_7398+1] === this.CELL_EMPTY
                && map[yPoint_7398][xPoint_7398+1] === this.CELL_EMPTY
                && map[yPoint_7398+1][xPoint_7398+1] === this.CELL_EMPTY
                && map[yPoint_7398+1][xPoint_7398] === this.CELL_EMPTY
                && map[yPoint_7398+1][xPoint_7398-1] === this.CELL_EMPTY
                && map[yPoint_7398][xPoint_7398-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7398, yPoint_7398, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7398;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7398][x] === this.CELL_EMPTY
                    && map[yPoint_7398-1][x] === this.CELL_EMPTY
                    && map[yPoint_7398-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7398][x+1] === this.CELL_EMPTY
                    && map[yPoint_7398+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7398+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7398, yPoint_7398, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7398;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7398] === this.CELL_EMPTY
                    && map[y+1][xPoint_7398] === this.CELL_EMPTY
                    && map[y+1][xPoint_7398+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7398] === this.CELL_EMPTY
                    && map[y][xPoint_7398-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7398-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
