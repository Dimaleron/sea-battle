...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6678=0;yPoint_6678<this.gameFieldBorderY.length; yPoint_6678++){
                for(var xPoint_6678=0;xPoint_6678<this.gameFieldBorderX.length; xPoint_6678++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6678, xPoint_6678);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6678][xPoint_6678] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6678, xPoint_6678, 'user');
                    if(this._userShipsMap[yPoint_6678][xPoint_6678] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6678, xPoint_6678, type){
            var id = this.getPointBlockIdByCoords(yPoint_6678, xPoint_6678, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6678);
                block.setAttribute('data-y', yPoint_6678);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6678
         * @param {type} xPoint_6678
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6678, xPoint_6678, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6678 + '_y' + yPoint_6678;
            }
            return 'pc_x' + xPoint_6678 + '_y' + yPoint_6678;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6678=0;yPoint_6678<this.gameFieldBorderY.length; yPoint_6678++){
                for(var xPoint_6678=0;xPoint_6678<this.gameFieldBorderX.length; xPoint_6678++){
                    map.push({y: yPoint_6678, x: xPoint_6678});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6678=-1;yPoint_6678<(this.gameFieldBorderY.length+1); yPoint_6678++){
                for(var xPoint_6678=-1;xPoint_6678<(this.gameFieldBorderX.length+1); xPoint_6678++){
                    if(!map[yPoint_6678]){
                        map[yPoint_6678] = [];
                    }
                    map[yPoint_6678][xPoint_6678] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6678 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6678 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6678, yPoint_6678) === true){
                    if(this.canPutHorizontal(map, xPoint_6678, yPoint_6678, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6678][xPoint_6678 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6678, yPoint_6678, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6678 + i][xPoint_6678] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6678, yPoint_6678){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6678][xPoint_6678] === this.CELL_EMPTY
                && map[yPoint_6678-1][xPoint_6678] === this.CELL_EMPTY
                && map[yPoint_6678-1][xPoint_6678+1] === this.CELL_EMPTY
                && map[yPoint_6678][xPoint_6678+1] === this.CELL_EMPTY
                && map[yPoint_6678+1][xPoint_6678+1] === this.CELL_EMPTY
                && map[yPoint_6678+1][xPoint_6678] === this.CELL_EMPTY
                && map[yPoint_6678+1][xPoint_6678-1] === this.CELL_EMPTY
                && map[yPoint_6678][xPoint_6678-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6678, yPoint_6678, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6678;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6678][x] === this.CELL_EMPTY
                    && map[yPoint_6678-1][x] === this.CELL_EMPTY
                    && map[yPoint_6678-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6678][x+1] === this.CELL_EMPTY
                    && map[yPoint_6678+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6678+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6678, yPoint_6678, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6678;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6678] === this.CELL_EMPTY
                    && map[y+1][xPoint_6678] === this.CELL_EMPTY
                    && map[y+1][xPoint_6678+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6678] === this.CELL_EMPTY
                    && map[y][xPoint_6678-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6678-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
