...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3478=0;yPoint_3478<this.gameFieldBorderY.length; yPoint_3478++){
                for(var xPoint_3478=0;xPoint_3478<this.gameFieldBorderX.length; xPoint_3478++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3478, xPoint_3478);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3478][xPoint_3478] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3478, xPoint_3478, 'user');
                    if(this._userShipsMap[yPoint_3478][xPoint_3478] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3478, xPoint_3478, type){
            var id = this.getPointBlockIdByCoords(yPoint_3478, xPoint_3478, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3478);
                block.setAttribute('data-y', yPoint_3478);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3478
         * @param {type} xPoint_3478
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3478, xPoint_3478, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3478 + '_y' + yPoint_3478;
            }
            return 'pc_x' + xPoint_3478 + '_y' + yPoint_3478;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3478=0;yPoint_3478<this.gameFieldBorderY.length; yPoint_3478++){
                for(var xPoint_3478=0;xPoint_3478<this.gameFieldBorderX.length; xPoint_3478++){
                    map.push({y: yPoint_3478, x: xPoint_3478});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3478=-1;yPoint_3478<(this.gameFieldBorderY.length+1); yPoint_3478++){
                for(var xPoint_3478=-1;xPoint_3478<(this.gameFieldBorderX.length+1); xPoint_3478++){
                    if(!map[yPoint_3478]){
                        map[yPoint_3478] = [];
                    }
                    map[yPoint_3478][xPoint_3478] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3478 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3478 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3478, yPoint_3478) === true){
                    if(this.canPutHorizontal(map, xPoint_3478, yPoint_3478, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3478][xPoint_3478 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3478, yPoint_3478, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3478 + i][xPoint_3478] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3478, yPoint_3478){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3478][xPoint_3478] === this.CELL_EMPTY
                && map[yPoint_3478-1][xPoint_3478] === this.CELL_EMPTY
                && map[yPoint_3478-1][xPoint_3478+1] === this.CELL_EMPTY
                && map[yPoint_3478][xPoint_3478+1] === this.CELL_EMPTY
                && map[yPoint_3478+1][xPoint_3478+1] === this.CELL_EMPTY
                && map[yPoint_3478+1][xPoint_3478] === this.CELL_EMPTY
                && map[yPoint_3478+1][xPoint_3478-1] === this.CELL_EMPTY
                && map[yPoint_3478][xPoint_3478-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3478, yPoint_3478, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3478;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3478][x] === this.CELL_EMPTY
                    && map[yPoint_3478-1][x] === this.CELL_EMPTY
                    && map[yPoint_3478-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3478][x+1] === this.CELL_EMPTY
                    && map[yPoint_3478+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3478+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3478, yPoint_3478, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3478;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3478] === this.CELL_EMPTY
                    && map[y+1][xPoint_3478] === this.CELL_EMPTY
                    && map[y+1][xPoint_3478+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3478] === this.CELL_EMPTY
                    && map[y][xPoint_3478-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3478-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
