...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27828=0;yPoint_27828<this.gameFieldBorderY.length; yPoint_27828++){
                for(var xPoint_27828=0;xPoint_27828<this.gameFieldBorderX.length; xPoint_27828++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27828, xPoint_27828);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27828][xPoint_27828] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27828, xPoint_27828, 'user');
                    if(this._userShipsMap[yPoint_27828][xPoint_27828] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27828, xPoint_27828, type){
            var id = this.getPointBlockIdByCoords(yPoint_27828, xPoint_27828, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27828);
                block.setAttribute('data-y', yPoint_27828);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27828
         * @param {type} xPoint_27828
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27828, xPoint_27828, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27828 + '_y' + yPoint_27828;
            }
            return 'pc_x' + xPoint_27828 + '_y' + yPoint_27828;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27828=0;yPoint_27828<this.gameFieldBorderY.length; yPoint_27828++){
                for(var xPoint_27828=0;xPoint_27828<this.gameFieldBorderX.length; xPoint_27828++){
                    map.push({y: yPoint_27828, x: xPoint_27828});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27828=-1;yPoint_27828<(this.gameFieldBorderY.length+1); yPoint_27828++){
                for(var xPoint_27828=-1;xPoint_27828<(this.gameFieldBorderX.length+1); xPoint_27828++){
                    if(!map[yPoint_27828]){
                        map[yPoint_27828] = [];
                    }
                    map[yPoint_27828][xPoint_27828] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27828 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27828 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27828, yPoint_27828) === true){
                    if(this.canPutHorizontal(map, xPoint_27828, yPoint_27828, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27828][xPoint_27828 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27828, yPoint_27828, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27828 + i][xPoint_27828] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27828, yPoint_27828){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27828][xPoint_27828] === this.CELL_EMPTY
                && map[yPoint_27828-1][xPoint_27828] === this.CELL_EMPTY
                && map[yPoint_27828-1][xPoint_27828+1] === this.CELL_EMPTY
                && map[yPoint_27828][xPoint_27828+1] === this.CELL_EMPTY
                && map[yPoint_27828+1][xPoint_27828+1] === this.CELL_EMPTY
                && map[yPoint_27828+1][xPoint_27828] === this.CELL_EMPTY
                && map[yPoint_27828+1][xPoint_27828-1] === this.CELL_EMPTY
                && map[yPoint_27828][xPoint_27828-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27828, yPoint_27828, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27828;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27828][x] === this.CELL_EMPTY
                    && map[yPoint_27828-1][x] === this.CELL_EMPTY
                    && map[yPoint_27828-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27828][x+1] === this.CELL_EMPTY
                    && map[yPoint_27828+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27828+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27828, yPoint_27828, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27828;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27828] === this.CELL_EMPTY
                    && map[y+1][xPoint_27828] === this.CELL_EMPTY
                    && map[y+1][xPoint_27828+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27828] === this.CELL_EMPTY
                    && map[y][xPoint_27828-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27828-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
