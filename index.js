...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11839=0;yPoint_11839<this.gameFieldBorderY.length; yPoint_11839++){
                for(var xPoint_11839=0;xPoint_11839<this.gameFieldBorderX.length; xPoint_11839++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11839, xPoint_11839);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11839][xPoint_11839] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11839, xPoint_11839, 'user');
                    if(this._userShipsMap[yPoint_11839][xPoint_11839] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11839, xPoint_11839, type){
            var id = this.getPointBlockIdByCoords(yPoint_11839, xPoint_11839, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11839);
                block.setAttribute('data-y', yPoint_11839);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11839
         * @param {type} xPoint_11839
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11839, xPoint_11839, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11839 + '_y' + yPoint_11839;
            }
            return 'pc_x' + xPoint_11839 + '_y' + yPoint_11839;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11839=0;yPoint_11839<this.gameFieldBorderY.length; yPoint_11839++){
                for(var xPoint_11839=0;xPoint_11839<this.gameFieldBorderX.length; xPoint_11839++){
                    map.push({y: yPoint_11839, x: xPoint_11839});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11839=-1;yPoint_11839<(this.gameFieldBorderY.length+1); yPoint_11839++){
                for(var xPoint_11839=-1;xPoint_11839<(this.gameFieldBorderX.length+1); xPoint_11839++){
                    if(!map[yPoint_11839]){
                        map[yPoint_11839] = [];
                    }
                    map[yPoint_11839][xPoint_11839] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11839 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11839 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11839, yPoint_11839) === true){
                    if(this.canPutHorizontal(map, xPoint_11839, yPoint_11839, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11839][xPoint_11839 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11839, yPoint_11839, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11839 + i][xPoint_11839] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11839, yPoint_11839){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11839][xPoint_11839] === this.CELL_EMPTY
                && map[yPoint_11839-1][xPoint_11839] === this.CELL_EMPTY
                && map[yPoint_11839-1][xPoint_11839+1] === this.CELL_EMPTY
                && map[yPoint_11839][xPoint_11839+1] === this.CELL_EMPTY
                && map[yPoint_11839+1][xPoint_11839+1] === this.CELL_EMPTY
                && map[yPoint_11839+1][xPoint_11839] === this.CELL_EMPTY
                && map[yPoint_11839+1][xPoint_11839-1] === this.CELL_EMPTY
                && map[yPoint_11839][xPoint_11839-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11839, yPoint_11839, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11839;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11839][x] === this.CELL_EMPTY
                    && map[yPoint_11839-1][x] === this.CELL_EMPTY
                    && map[yPoint_11839-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11839][x+1] === this.CELL_EMPTY
                    && map[yPoint_11839+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11839+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11839, yPoint_11839, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11839;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11839] === this.CELL_EMPTY
                    && map[y+1][xPoint_11839] === this.CELL_EMPTY
                    && map[y+1][xPoint_11839+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11839] === this.CELL_EMPTY
                    && map[y][xPoint_11839-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11839-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
