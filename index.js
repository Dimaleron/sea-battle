...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31538=0;yPoint_31538<this.gameFieldBorderY.length; yPoint_31538++){
                for(var xPoint_31538=0;xPoint_31538<this.gameFieldBorderX.length; xPoint_31538++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31538, xPoint_31538);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31538][xPoint_31538] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31538, xPoint_31538, 'user');
                    if(this._userShipsMap[yPoint_31538][xPoint_31538] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31538, xPoint_31538, type){
            var id = this.getPointBlockIdByCoords(yPoint_31538, xPoint_31538, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31538);
                block.setAttribute('data-y', yPoint_31538);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31538
         * @param {type} xPoint_31538
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31538, xPoint_31538, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31538 + '_y' + yPoint_31538;
            }
            return 'pc_x' + xPoint_31538 + '_y' + yPoint_31538;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31538=0;yPoint_31538<this.gameFieldBorderY.length; yPoint_31538++){
                for(var xPoint_31538=0;xPoint_31538<this.gameFieldBorderX.length; xPoint_31538++){
                    map.push({y: yPoint_31538, x: xPoint_31538});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31538=-1;yPoint_31538<(this.gameFieldBorderY.length+1); yPoint_31538++){
                for(var xPoint_31538=-1;xPoint_31538<(this.gameFieldBorderX.length+1); xPoint_31538++){
                    if(!map[yPoint_31538]){
                        map[yPoint_31538] = [];
                    }
                    map[yPoint_31538][xPoint_31538] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31538 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31538 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31538, yPoint_31538) === true){
                    if(this.canPutHorizontal(map, xPoint_31538, yPoint_31538, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31538][xPoint_31538 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31538, yPoint_31538, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31538 + i][xPoint_31538] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31538, yPoint_31538){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31538][xPoint_31538] === this.CELL_EMPTY
                && map[yPoint_31538-1][xPoint_31538] === this.CELL_EMPTY
                && map[yPoint_31538-1][xPoint_31538+1] === this.CELL_EMPTY
                && map[yPoint_31538][xPoint_31538+1] === this.CELL_EMPTY
                && map[yPoint_31538+1][xPoint_31538+1] === this.CELL_EMPTY
                && map[yPoint_31538+1][xPoint_31538] === this.CELL_EMPTY
                && map[yPoint_31538+1][xPoint_31538-1] === this.CELL_EMPTY
                && map[yPoint_31538][xPoint_31538-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31538, yPoint_31538, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31538;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31538][x] === this.CELL_EMPTY
                    && map[yPoint_31538-1][x] === this.CELL_EMPTY
                    && map[yPoint_31538-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31538][x+1] === this.CELL_EMPTY
                    && map[yPoint_31538+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31538+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31538, yPoint_31538, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31538;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31538] === this.CELL_EMPTY
                    && map[y+1][xPoint_31538] === this.CELL_EMPTY
                    && map[y+1][xPoint_31538+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31538] === this.CELL_EMPTY
                    && map[y][xPoint_31538-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31538-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
