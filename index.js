...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10077=0;yPoint_10077<this.gameFieldBorderY.length; yPoint_10077++){
                for(var xPoint_10077=0;xPoint_10077<this.gameFieldBorderX.length; xPoint_10077++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10077, xPoint_10077);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10077][xPoint_10077] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10077, xPoint_10077, 'user');
                    if(this._userShipsMap[yPoint_10077][xPoint_10077] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10077, xPoint_10077, type){
            var id = this.getPointBlockIdByCoords(yPoint_10077, xPoint_10077, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10077);
                block.setAttribute('data-y', yPoint_10077);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10077
         * @param {type} xPoint_10077
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10077, xPoint_10077, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10077 + '_y' + yPoint_10077;
            }
            return 'pc_x' + xPoint_10077 + '_y' + yPoint_10077;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10077=0;yPoint_10077<this.gameFieldBorderY.length; yPoint_10077++){
                for(var xPoint_10077=0;xPoint_10077<this.gameFieldBorderX.length; xPoint_10077++){
                    map.push({y: yPoint_10077, x: xPoint_10077});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10077=-1;yPoint_10077<(this.gameFieldBorderY.length+1); yPoint_10077++){
                for(var xPoint_10077=-1;xPoint_10077<(this.gameFieldBorderX.length+1); xPoint_10077++){
                    if(!map[yPoint_10077]){
                        map[yPoint_10077] = [];
                    }
                    map[yPoint_10077][xPoint_10077] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10077 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10077 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10077, yPoint_10077) === true){
                    if(this.canPutHorizontal(map, xPoint_10077, yPoint_10077, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10077][xPoint_10077 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10077, yPoint_10077, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10077 + i][xPoint_10077] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10077, yPoint_10077){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10077][xPoint_10077] === this.CELL_EMPTY
                && map[yPoint_10077-1][xPoint_10077] === this.CELL_EMPTY
                && map[yPoint_10077-1][xPoint_10077+1] === this.CELL_EMPTY
                && map[yPoint_10077][xPoint_10077+1] === this.CELL_EMPTY
                && map[yPoint_10077+1][xPoint_10077+1] === this.CELL_EMPTY
                && map[yPoint_10077+1][xPoint_10077] === this.CELL_EMPTY
                && map[yPoint_10077+1][xPoint_10077-1] === this.CELL_EMPTY
                && map[yPoint_10077][xPoint_10077-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10077, yPoint_10077, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10077;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10077][x] === this.CELL_EMPTY
                    && map[yPoint_10077-1][x] === this.CELL_EMPTY
                    && map[yPoint_10077-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10077][x+1] === this.CELL_EMPTY
                    && map[yPoint_10077+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10077+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10077, yPoint_10077, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10077;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10077] === this.CELL_EMPTY
                    && map[y+1][xPoint_10077] === this.CELL_EMPTY
                    && map[y+1][xPoint_10077+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10077] === this.CELL_EMPTY
                    && map[y][xPoint_10077-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10077-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
