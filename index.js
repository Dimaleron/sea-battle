...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6148=0;yPoint_6148<this.gameFieldBorderY.length; yPoint_6148++){
                for(var xPoint_6148=0;xPoint_6148<this.gameFieldBorderX.length; xPoint_6148++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6148, xPoint_6148);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6148][xPoint_6148] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6148, xPoint_6148, 'user');
                    if(this._userShipsMap[yPoint_6148][xPoint_6148] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6148, xPoint_6148, type){
            var id = this.getPointBlockIdByCoords(yPoint_6148, xPoint_6148, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6148);
                block.setAttribute('data-y', yPoint_6148);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6148
         * @param {type} xPoint_6148
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6148, xPoint_6148, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6148 + '_y' + yPoint_6148;
            }
            return 'pc_x' + xPoint_6148 + '_y' + yPoint_6148;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6148=0;yPoint_6148<this.gameFieldBorderY.length; yPoint_6148++){
                for(var xPoint_6148=0;xPoint_6148<this.gameFieldBorderX.length; xPoint_6148++){
                    map.push({y: yPoint_6148, x: xPoint_6148});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6148=-1;yPoint_6148<(this.gameFieldBorderY.length+1); yPoint_6148++){
                for(var xPoint_6148=-1;xPoint_6148<(this.gameFieldBorderX.length+1); xPoint_6148++){
                    if(!map[yPoint_6148]){
                        map[yPoint_6148] = [];
                    }
                    map[yPoint_6148][xPoint_6148] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6148 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6148 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6148, yPoint_6148) === true){
                    if(this.canPutHorizontal(map, xPoint_6148, yPoint_6148, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6148][xPoint_6148 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6148, yPoint_6148, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6148 + i][xPoint_6148] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6148, yPoint_6148){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6148][xPoint_6148] === this.CELL_EMPTY
                && map[yPoint_6148-1][xPoint_6148] === this.CELL_EMPTY
                && map[yPoint_6148-1][xPoint_6148+1] === this.CELL_EMPTY
                && map[yPoint_6148][xPoint_6148+1] === this.CELL_EMPTY
                && map[yPoint_6148+1][xPoint_6148+1] === this.CELL_EMPTY
                && map[yPoint_6148+1][xPoint_6148] === this.CELL_EMPTY
                && map[yPoint_6148+1][xPoint_6148-1] === this.CELL_EMPTY
                && map[yPoint_6148][xPoint_6148-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6148, yPoint_6148, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6148;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6148][x] === this.CELL_EMPTY
                    && map[yPoint_6148-1][x] === this.CELL_EMPTY
                    && map[yPoint_6148-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6148][x+1] === this.CELL_EMPTY
                    && map[yPoint_6148+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6148+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6148, yPoint_6148, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6148;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6148] === this.CELL_EMPTY
                    && map[y+1][xPoint_6148] === this.CELL_EMPTY
                    && map[y+1][xPoint_6148+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6148] === this.CELL_EMPTY
                    && map[y][xPoint_6148-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6148-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
