...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25429=0;yPoint_25429<this.gameFieldBorderY.length; yPoint_25429++){
                for(var xPoint_25429=0;xPoint_25429<this.gameFieldBorderX.length; xPoint_25429++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25429, xPoint_25429);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25429][xPoint_25429] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25429, xPoint_25429, 'user');
                    if(this._userShipsMap[yPoint_25429][xPoint_25429] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25429, xPoint_25429, type){
            var id = this.getPointBlockIdByCoords(yPoint_25429, xPoint_25429, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25429);
                block.setAttribute('data-y', yPoint_25429);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25429
         * @param {type} xPoint_25429
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25429, xPoint_25429, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25429 + '_y' + yPoint_25429;
            }
            return 'pc_x' + xPoint_25429 + '_y' + yPoint_25429;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25429=0;yPoint_25429<this.gameFieldBorderY.length; yPoint_25429++){
                for(var xPoint_25429=0;xPoint_25429<this.gameFieldBorderX.length; xPoint_25429++){
                    map.push({y: yPoint_25429, x: xPoint_25429});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25429=-1;yPoint_25429<(this.gameFieldBorderY.length+1); yPoint_25429++){
                for(var xPoint_25429=-1;xPoint_25429<(this.gameFieldBorderX.length+1); xPoint_25429++){
                    if(!map[yPoint_25429]){
                        map[yPoint_25429] = [];
                    }
                    map[yPoint_25429][xPoint_25429] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25429 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25429 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25429, yPoint_25429) === true){
                    if(this.canPutHorizontal(map, xPoint_25429, yPoint_25429, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25429][xPoint_25429 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25429, yPoint_25429, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25429 + i][xPoint_25429] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25429, yPoint_25429){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25429][xPoint_25429] === this.CELL_EMPTY
                && map[yPoint_25429-1][xPoint_25429] === this.CELL_EMPTY
                && map[yPoint_25429-1][xPoint_25429+1] === this.CELL_EMPTY
                && map[yPoint_25429][xPoint_25429+1] === this.CELL_EMPTY
                && map[yPoint_25429+1][xPoint_25429+1] === this.CELL_EMPTY
                && map[yPoint_25429+1][xPoint_25429] === this.CELL_EMPTY
                && map[yPoint_25429+1][xPoint_25429-1] === this.CELL_EMPTY
                && map[yPoint_25429][xPoint_25429-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25429, yPoint_25429, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25429;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25429][x] === this.CELL_EMPTY
                    && map[yPoint_25429-1][x] === this.CELL_EMPTY
                    && map[yPoint_25429-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25429][x+1] === this.CELL_EMPTY
                    && map[yPoint_25429+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25429+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25429, yPoint_25429, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25429;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25429] === this.CELL_EMPTY
                    && map[y+1][xPoint_25429] === this.CELL_EMPTY
                    && map[y+1][xPoint_25429+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25429] === this.CELL_EMPTY
                    && map[y][xPoint_25429-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25429-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
