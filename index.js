...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19537=0;yPoint_19537<this.gameFieldBorderY.length; yPoint_19537++){
                for(var xPoint_19537=0;xPoint_19537<this.gameFieldBorderX.length; xPoint_19537++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19537, xPoint_19537);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19537][xPoint_19537] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19537, xPoint_19537, 'user');
                    if(this._userShipsMap[yPoint_19537][xPoint_19537] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19537, xPoint_19537, type){
            var id = this.getPointBlockIdByCoords(yPoint_19537, xPoint_19537, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19537);
                block.setAttribute('data-y', yPoint_19537);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19537
         * @param {type} xPoint_19537
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19537, xPoint_19537, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19537 + '_y' + yPoint_19537;
            }
            return 'pc_x' + xPoint_19537 + '_y' + yPoint_19537;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19537=0;yPoint_19537<this.gameFieldBorderY.length; yPoint_19537++){
                for(var xPoint_19537=0;xPoint_19537<this.gameFieldBorderX.length; xPoint_19537++){
                    map.push({y: yPoint_19537, x: xPoint_19537});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19537=-1;yPoint_19537<(this.gameFieldBorderY.length+1); yPoint_19537++){
                for(var xPoint_19537=-1;xPoint_19537<(this.gameFieldBorderX.length+1); xPoint_19537++){
                    if(!map[yPoint_19537]){
                        map[yPoint_19537] = [];
                    }
                    map[yPoint_19537][xPoint_19537] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19537 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19537 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19537, yPoint_19537) === true){
                    if(this.canPutHorizontal(map, xPoint_19537, yPoint_19537, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19537][xPoint_19537 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19537, yPoint_19537, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19537 + i][xPoint_19537] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19537, yPoint_19537){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19537][xPoint_19537] === this.CELL_EMPTY
                && map[yPoint_19537-1][xPoint_19537] === this.CELL_EMPTY
                && map[yPoint_19537-1][xPoint_19537+1] === this.CELL_EMPTY
                && map[yPoint_19537][xPoint_19537+1] === this.CELL_EMPTY
                && map[yPoint_19537+1][xPoint_19537+1] === this.CELL_EMPTY
                && map[yPoint_19537+1][xPoint_19537] === this.CELL_EMPTY
                && map[yPoint_19537+1][xPoint_19537-1] === this.CELL_EMPTY
                && map[yPoint_19537][xPoint_19537-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19537, yPoint_19537, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19537;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19537][x] === this.CELL_EMPTY
                    && map[yPoint_19537-1][x] === this.CELL_EMPTY
                    && map[yPoint_19537-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19537][x+1] === this.CELL_EMPTY
                    && map[yPoint_19537+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19537+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19537, yPoint_19537, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19537;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19537] === this.CELL_EMPTY
                    && map[y+1][xPoint_19537] === this.CELL_EMPTY
                    && map[y+1][xPoint_19537+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19537] === this.CELL_EMPTY
                    && map[y][xPoint_19537-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19537-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
