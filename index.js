...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13649=0;yPoint_13649<this.gameFieldBorderY.length; yPoint_13649++){
                for(var xPoint_13649=0;xPoint_13649<this.gameFieldBorderX.length; xPoint_13649++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13649, xPoint_13649);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13649][xPoint_13649] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13649, xPoint_13649, 'user');
                    if(this._userShipsMap[yPoint_13649][xPoint_13649] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13649, xPoint_13649, type){
            var id = this.getPointBlockIdByCoords(yPoint_13649, xPoint_13649, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13649);
                block.setAttribute('data-y', yPoint_13649);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13649
         * @param {type} xPoint_13649
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13649, xPoint_13649, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13649 + '_y' + yPoint_13649;
            }
            return 'pc_x' + xPoint_13649 + '_y' + yPoint_13649;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13649=0;yPoint_13649<this.gameFieldBorderY.length; yPoint_13649++){
                for(var xPoint_13649=0;xPoint_13649<this.gameFieldBorderX.length; xPoint_13649++){
                    map.push({y: yPoint_13649, x: xPoint_13649});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13649=-1;yPoint_13649<(this.gameFieldBorderY.length+1); yPoint_13649++){
                for(var xPoint_13649=-1;xPoint_13649<(this.gameFieldBorderX.length+1); xPoint_13649++){
                    if(!map[yPoint_13649]){
                        map[yPoint_13649] = [];
                    }
                    map[yPoint_13649][xPoint_13649] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13649 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13649 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13649, yPoint_13649) === true){
                    if(this.canPutHorizontal(map, xPoint_13649, yPoint_13649, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13649][xPoint_13649 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13649, yPoint_13649, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13649 + i][xPoint_13649] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13649, yPoint_13649){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13649][xPoint_13649] === this.CELL_EMPTY
                && map[yPoint_13649-1][xPoint_13649] === this.CELL_EMPTY
                && map[yPoint_13649-1][xPoint_13649+1] === this.CELL_EMPTY
                && map[yPoint_13649][xPoint_13649+1] === this.CELL_EMPTY
                && map[yPoint_13649+1][xPoint_13649+1] === this.CELL_EMPTY
                && map[yPoint_13649+1][xPoint_13649] === this.CELL_EMPTY
                && map[yPoint_13649+1][xPoint_13649-1] === this.CELL_EMPTY
                && map[yPoint_13649][xPoint_13649-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13649, yPoint_13649, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13649;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13649][x] === this.CELL_EMPTY
                    && map[yPoint_13649-1][x] === this.CELL_EMPTY
                    && map[yPoint_13649-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13649][x+1] === this.CELL_EMPTY
                    && map[yPoint_13649+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13649+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13649, yPoint_13649, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13649;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13649] === this.CELL_EMPTY
                    && map[y+1][xPoint_13649] === this.CELL_EMPTY
                    && map[y+1][xPoint_13649+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13649] === this.CELL_EMPTY
                    && map[y][xPoint_13649-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13649-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
