...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20197=0;yPoint_20197<this.gameFieldBorderY.length; yPoint_20197++){
                for(var xPoint_20197=0;xPoint_20197<this.gameFieldBorderX.length; xPoint_20197++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20197, xPoint_20197);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20197][xPoint_20197] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20197, xPoint_20197, 'user');
                    if(this._userShipsMap[yPoint_20197][xPoint_20197] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20197, xPoint_20197, type){
            var id = this.getPointBlockIdByCoords(yPoint_20197, xPoint_20197, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20197);
                block.setAttribute('data-y', yPoint_20197);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20197
         * @param {type} xPoint_20197
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20197, xPoint_20197, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20197 + '_y' + yPoint_20197;
            }
            return 'pc_x' + xPoint_20197 + '_y' + yPoint_20197;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20197=0;yPoint_20197<this.gameFieldBorderY.length; yPoint_20197++){
                for(var xPoint_20197=0;xPoint_20197<this.gameFieldBorderX.length; xPoint_20197++){
                    map.push({y: yPoint_20197, x: xPoint_20197});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20197=-1;yPoint_20197<(this.gameFieldBorderY.length+1); yPoint_20197++){
                for(var xPoint_20197=-1;xPoint_20197<(this.gameFieldBorderX.length+1); xPoint_20197++){
                    if(!map[yPoint_20197]){
                        map[yPoint_20197] = [];
                    }
                    map[yPoint_20197][xPoint_20197] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20197 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20197 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20197, yPoint_20197) === true){
                    if(this.canPutHorizontal(map, xPoint_20197, yPoint_20197, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20197][xPoint_20197 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20197, yPoint_20197, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20197 + i][xPoint_20197] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20197, yPoint_20197){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20197][xPoint_20197] === this.CELL_EMPTY
                && map[yPoint_20197-1][xPoint_20197] === this.CELL_EMPTY
                && map[yPoint_20197-1][xPoint_20197+1] === this.CELL_EMPTY
                && map[yPoint_20197][xPoint_20197+1] === this.CELL_EMPTY
                && map[yPoint_20197+1][xPoint_20197+1] === this.CELL_EMPTY
                && map[yPoint_20197+1][xPoint_20197] === this.CELL_EMPTY
                && map[yPoint_20197+1][xPoint_20197-1] === this.CELL_EMPTY
                && map[yPoint_20197][xPoint_20197-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20197, yPoint_20197, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20197;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20197][x] === this.CELL_EMPTY
                    && map[yPoint_20197-1][x] === this.CELL_EMPTY
                    && map[yPoint_20197-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20197][x+1] === this.CELL_EMPTY
                    && map[yPoint_20197+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20197+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20197, yPoint_20197, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20197;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20197] === this.CELL_EMPTY
                    && map[y+1][xPoint_20197] === this.CELL_EMPTY
                    && map[y+1][xPoint_20197+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20197] === this.CELL_EMPTY
                    && map[y][xPoint_20197-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20197-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
