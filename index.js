...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20037=0;yPoint_20037<this.gameFieldBorderY.length; yPoint_20037++){
                for(var xPoint_20037=0;xPoint_20037<this.gameFieldBorderX.length; xPoint_20037++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20037, xPoint_20037);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20037][xPoint_20037] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20037, xPoint_20037, 'user');
                    if(this._userShipsMap[yPoint_20037][xPoint_20037] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20037, xPoint_20037, type){
            var id = this.getPointBlockIdByCoords(yPoint_20037, xPoint_20037, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20037);
                block.setAttribute('data-y', yPoint_20037);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20037
         * @param {type} xPoint_20037
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20037, xPoint_20037, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20037 + '_y' + yPoint_20037;
            }
            return 'pc_x' + xPoint_20037 + '_y' + yPoint_20037;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20037=0;yPoint_20037<this.gameFieldBorderY.length; yPoint_20037++){
                for(var xPoint_20037=0;xPoint_20037<this.gameFieldBorderX.length; xPoint_20037++){
                    map.push({y: yPoint_20037, x: xPoint_20037});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20037=-1;yPoint_20037<(this.gameFieldBorderY.length+1); yPoint_20037++){
                for(var xPoint_20037=-1;xPoint_20037<(this.gameFieldBorderX.length+1); xPoint_20037++){
                    if(!map[yPoint_20037]){
                        map[yPoint_20037] = [];
                    }
                    map[yPoint_20037][xPoint_20037] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20037 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20037 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20037, yPoint_20037) === true){
                    if(this.canPutHorizontal(map, xPoint_20037, yPoint_20037, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20037][xPoint_20037 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20037, yPoint_20037, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20037 + i][xPoint_20037] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20037, yPoint_20037){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20037][xPoint_20037] === this.CELL_EMPTY
                && map[yPoint_20037-1][xPoint_20037] === this.CELL_EMPTY
                && map[yPoint_20037-1][xPoint_20037+1] === this.CELL_EMPTY
                && map[yPoint_20037][xPoint_20037+1] === this.CELL_EMPTY
                && map[yPoint_20037+1][xPoint_20037+1] === this.CELL_EMPTY
                && map[yPoint_20037+1][xPoint_20037] === this.CELL_EMPTY
                && map[yPoint_20037+1][xPoint_20037-1] === this.CELL_EMPTY
                && map[yPoint_20037][xPoint_20037-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20037, yPoint_20037, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20037;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20037][x] === this.CELL_EMPTY
                    && map[yPoint_20037-1][x] === this.CELL_EMPTY
                    && map[yPoint_20037-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20037][x+1] === this.CELL_EMPTY
                    && map[yPoint_20037+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20037+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20037, yPoint_20037, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20037;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20037] === this.CELL_EMPTY
                    && map[y+1][xPoint_20037] === this.CELL_EMPTY
                    && map[y+1][xPoint_20037+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20037] === this.CELL_EMPTY
                    && map[y][xPoint_20037-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20037-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
