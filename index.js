...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15302=0;yPoint_15302<this.gameFieldBorderY.length; yPoint_15302++){
                for(var xPoint_15302=0;xPoint_15302<this.gameFieldBorderX.length; xPoint_15302++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15302, xPoint_15302);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15302][xPoint_15302] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15302, xPoint_15302, 'user');
                    if(this._userShipsMap[yPoint_15302][xPoint_15302] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15302, xPoint_15302, type){
            var id = this.getPointBlockIdByCoords(yPoint_15302, xPoint_15302, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15302);
                block.setAttribute('data-y', yPoint_15302);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15302
         * @param {type} xPoint_15302
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15302, xPoint_15302, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15302 + '_y' + yPoint_15302;
            }
            return 'pc_x' + xPoint_15302 + '_y' + yPoint_15302;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15302=0;yPoint_15302<this.gameFieldBorderY.length; yPoint_15302++){
                for(var xPoint_15302=0;xPoint_15302<this.gameFieldBorderX.length; xPoint_15302++){
                    map.push({y: yPoint_15302, x: xPoint_15302});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15302=-1;yPoint_15302<(this.gameFieldBorderY.length+1); yPoint_15302++){
                for(var xPoint_15302=-1;xPoint_15302<(this.gameFieldBorderX.length+1); xPoint_15302++){
                    if(!map[yPoint_15302]){
                        map[yPoint_15302] = [];
                    }
                    map[yPoint_15302][xPoint_15302] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15302 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15302 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15302, yPoint_15302) === true){
                    if(this.canPutHorizontal(map, xPoint_15302, yPoint_15302, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15302][xPoint_15302 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15302, yPoint_15302, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15302 + i][xPoint_15302] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15302, yPoint_15302){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15302][xPoint_15302] === this.CELL_EMPTY
                && map[yPoint_15302-1][xPoint_15302] === this.CELL_EMPTY
                && map[yPoint_15302-1][xPoint_15302+1] === this.CELL_EMPTY
                && map[yPoint_15302][xPoint_15302+1] === this.CELL_EMPTY
                && map[yPoint_15302+1][xPoint_15302+1] === this.CELL_EMPTY
                && map[yPoint_15302+1][xPoint_15302] === this.CELL_EMPTY
                && map[yPoint_15302+1][xPoint_15302-1] === this.CELL_EMPTY
                && map[yPoint_15302][xPoint_15302-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15302, yPoint_15302, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15302;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15302][x] === this.CELL_EMPTY
                    && map[yPoint_15302-1][x] === this.CELL_EMPTY
                    && map[yPoint_15302-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15302][x+1] === this.CELL_EMPTY
                    && map[yPoint_15302+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15302+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15302, yPoint_15302, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15302;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15302] === this.CELL_EMPTY
                    && map[y+1][xPoint_15302] === this.CELL_EMPTY
                    && map[y+1][xPoint_15302+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15302] === this.CELL_EMPTY
                    && map[y][xPoint_15302-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15302-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
