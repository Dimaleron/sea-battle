...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8892=0;yPoint_8892<this.gameFieldBorderY.length; yPoint_8892++){
                for(var xPoint_8892=0;xPoint_8892<this.gameFieldBorderX.length; xPoint_8892++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8892, xPoint_8892);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8892][xPoint_8892] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8892, xPoint_8892, 'user');
                    if(this._userShipsMap[yPoint_8892][xPoint_8892] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8892, xPoint_8892, type){
            var id = this.getPointBlockIdByCoords(yPoint_8892, xPoint_8892, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8892);
                block.setAttribute('data-y', yPoint_8892);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8892
         * @param {type} xPoint_8892
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8892, xPoint_8892, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8892 + '_y' + yPoint_8892;
            }
            return 'pc_x' + xPoint_8892 + '_y' + yPoint_8892;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8892=0;yPoint_8892<this.gameFieldBorderY.length; yPoint_8892++){
                for(var xPoint_8892=0;xPoint_8892<this.gameFieldBorderX.length; xPoint_8892++){
                    map.push({y: yPoint_8892, x: xPoint_8892});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8892=-1;yPoint_8892<(this.gameFieldBorderY.length+1); yPoint_8892++){
                for(var xPoint_8892=-1;xPoint_8892<(this.gameFieldBorderX.length+1); xPoint_8892++){
                    if(!map[yPoint_8892]){
                        map[yPoint_8892] = [];
                    }
                    map[yPoint_8892][xPoint_8892] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8892 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8892 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8892, yPoint_8892) === true){
                    if(this.canPutHorizontal(map, xPoint_8892, yPoint_8892, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8892][xPoint_8892 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8892, yPoint_8892, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8892 + i][xPoint_8892] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8892, yPoint_8892){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8892][xPoint_8892] === this.CELL_EMPTY
                && map[yPoint_8892-1][xPoint_8892] === this.CELL_EMPTY
                && map[yPoint_8892-1][xPoint_8892+1] === this.CELL_EMPTY
                && map[yPoint_8892][xPoint_8892+1] === this.CELL_EMPTY
                && map[yPoint_8892+1][xPoint_8892+1] === this.CELL_EMPTY
                && map[yPoint_8892+1][xPoint_8892] === this.CELL_EMPTY
                && map[yPoint_8892+1][xPoint_8892-1] === this.CELL_EMPTY
                && map[yPoint_8892][xPoint_8892-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8892, yPoint_8892, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8892;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8892][x] === this.CELL_EMPTY
                    && map[yPoint_8892-1][x] === this.CELL_EMPTY
                    && map[yPoint_8892-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8892][x+1] === this.CELL_EMPTY
                    && map[yPoint_8892+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8892+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8892, yPoint_8892, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8892;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8892] === this.CELL_EMPTY
                    && map[y+1][xPoint_8892] === this.CELL_EMPTY
                    && map[y+1][xPoint_8892+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8892] === this.CELL_EMPTY
                    && map[y][xPoint_8892-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8892-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
