...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14016=0;yPoint_14016<this.gameFieldBorderY.length; yPoint_14016++){
                for(var xPoint_14016=0;xPoint_14016<this.gameFieldBorderX.length; xPoint_14016++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14016, xPoint_14016);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14016][xPoint_14016] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14016, xPoint_14016, 'user');
                    if(this._userShipsMap[yPoint_14016][xPoint_14016] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14016, xPoint_14016, type){
            var id = this.getPointBlockIdByCoords(yPoint_14016, xPoint_14016, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14016);
                block.setAttribute('data-y', yPoint_14016);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14016
         * @param {type} xPoint_14016
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14016, xPoint_14016, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14016 + '_y' + yPoint_14016;
            }
            return 'pc_x' + xPoint_14016 + '_y' + yPoint_14016;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14016=0;yPoint_14016<this.gameFieldBorderY.length; yPoint_14016++){
                for(var xPoint_14016=0;xPoint_14016<this.gameFieldBorderX.length; xPoint_14016++){
                    map.push({y: yPoint_14016, x: xPoint_14016});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14016=-1;yPoint_14016<(this.gameFieldBorderY.length+1); yPoint_14016++){
                for(var xPoint_14016=-1;xPoint_14016<(this.gameFieldBorderX.length+1); xPoint_14016++){
                    if(!map[yPoint_14016]){
                        map[yPoint_14016] = [];
                    }
                    map[yPoint_14016][xPoint_14016] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14016 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14016 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14016, yPoint_14016) === true){
                    if(this.canPutHorizontal(map, xPoint_14016, yPoint_14016, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14016][xPoint_14016 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14016, yPoint_14016, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14016 + i][xPoint_14016] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14016, yPoint_14016){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14016][xPoint_14016] === this.CELL_EMPTY
                && map[yPoint_14016-1][xPoint_14016] === this.CELL_EMPTY
                && map[yPoint_14016-1][xPoint_14016+1] === this.CELL_EMPTY
                && map[yPoint_14016][xPoint_14016+1] === this.CELL_EMPTY
                && map[yPoint_14016+1][xPoint_14016+1] === this.CELL_EMPTY
                && map[yPoint_14016+1][xPoint_14016] === this.CELL_EMPTY
                && map[yPoint_14016+1][xPoint_14016-1] === this.CELL_EMPTY
                && map[yPoint_14016][xPoint_14016-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14016, yPoint_14016, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14016;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14016][x] === this.CELL_EMPTY
                    && map[yPoint_14016-1][x] === this.CELL_EMPTY
                    && map[yPoint_14016-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14016][x+1] === this.CELL_EMPTY
                    && map[yPoint_14016+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14016+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14016, yPoint_14016, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14016;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14016] === this.CELL_EMPTY
                    && map[y+1][xPoint_14016] === this.CELL_EMPTY
                    && map[y+1][xPoint_14016+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14016] === this.CELL_EMPTY
                    && map[y][xPoint_14016-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14016-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
