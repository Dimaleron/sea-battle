...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16775=0;yPoint_16775<this.gameFieldBorderY.length; yPoint_16775++){
                for(var xPoint_16775=0;xPoint_16775<this.gameFieldBorderX.length; xPoint_16775++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16775, xPoint_16775);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16775][xPoint_16775] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16775, xPoint_16775, 'user');
                    if(this._userShipsMap[yPoint_16775][xPoint_16775] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16775, xPoint_16775, type){
            var id = this.getPointBlockIdByCoords(yPoint_16775, xPoint_16775, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16775);
                block.setAttribute('data-y', yPoint_16775);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16775
         * @param {type} xPoint_16775
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16775, xPoint_16775, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16775 + '_y' + yPoint_16775;
            }
            return 'pc_x' + xPoint_16775 + '_y' + yPoint_16775;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16775=0;yPoint_16775<this.gameFieldBorderY.length; yPoint_16775++){
                for(var xPoint_16775=0;xPoint_16775<this.gameFieldBorderX.length; xPoint_16775++){
                    map.push({y: yPoint_16775, x: xPoint_16775});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16775=-1;yPoint_16775<(this.gameFieldBorderY.length+1); yPoint_16775++){
                for(var xPoint_16775=-1;xPoint_16775<(this.gameFieldBorderX.length+1); xPoint_16775++){
                    if(!map[yPoint_16775]){
                        map[yPoint_16775] = [];
                    }
                    map[yPoint_16775][xPoint_16775] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16775 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16775 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16775, yPoint_16775) === true){
                    if(this.canPutHorizontal(map, xPoint_16775, yPoint_16775, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16775][xPoint_16775 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16775, yPoint_16775, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16775 + i][xPoint_16775] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16775, yPoint_16775){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16775][xPoint_16775] === this.CELL_EMPTY
                && map[yPoint_16775-1][xPoint_16775] === this.CELL_EMPTY
                && map[yPoint_16775-1][xPoint_16775+1] === this.CELL_EMPTY
                && map[yPoint_16775][xPoint_16775+1] === this.CELL_EMPTY
                && map[yPoint_16775+1][xPoint_16775+1] === this.CELL_EMPTY
                && map[yPoint_16775+1][xPoint_16775] === this.CELL_EMPTY
                && map[yPoint_16775+1][xPoint_16775-1] === this.CELL_EMPTY
                && map[yPoint_16775][xPoint_16775-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16775, yPoint_16775, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16775;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16775][x] === this.CELL_EMPTY
                    && map[yPoint_16775-1][x] === this.CELL_EMPTY
                    && map[yPoint_16775-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16775][x+1] === this.CELL_EMPTY
                    && map[yPoint_16775+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16775+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16775, yPoint_16775, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16775;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16775] === this.CELL_EMPTY
                    && map[y+1][xPoint_16775] === this.CELL_EMPTY
                    && map[y+1][xPoint_16775+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16775] === this.CELL_EMPTY
                    && map[y][xPoint_16775-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16775-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
