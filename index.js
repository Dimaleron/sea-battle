...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10607=0;yPoint_10607<this.gameFieldBorderY.length; yPoint_10607++){
                for(var xPoint_10607=0;xPoint_10607<this.gameFieldBorderX.length; xPoint_10607++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10607, xPoint_10607);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10607][xPoint_10607] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10607, xPoint_10607, 'user');
                    if(this._userShipsMap[yPoint_10607][xPoint_10607] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10607, xPoint_10607, type){
            var id = this.getPointBlockIdByCoords(yPoint_10607, xPoint_10607, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10607);
                block.setAttribute('data-y', yPoint_10607);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10607
         * @param {type} xPoint_10607
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10607, xPoint_10607, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10607 + '_y' + yPoint_10607;
            }
            return 'pc_x' + xPoint_10607 + '_y' + yPoint_10607;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10607=0;yPoint_10607<this.gameFieldBorderY.length; yPoint_10607++){
                for(var xPoint_10607=0;xPoint_10607<this.gameFieldBorderX.length; xPoint_10607++){
                    map.push({y: yPoint_10607, x: xPoint_10607});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10607=-1;yPoint_10607<(this.gameFieldBorderY.length+1); yPoint_10607++){
                for(var xPoint_10607=-1;xPoint_10607<(this.gameFieldBorderX.length+1); xPoint_10607++){
                    if(!map[yPoint_10607]){
                        map[yPoint_10607] = [];
                    }
                    map[yPoint_10607][xPoint_10607] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10607 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10607 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10607, yPoint_10607) === true){
                    if(this.canPutHorizontal(map, xPoint_10607, yPoint_10607, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10607][xPoint_10607 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10607, yPoint_10607, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10607 + i][xPoint_10607] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10607, yPoint_10607){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10607][xPoint_10607] === this.CELL_EMPTY
                && map[yPoint_10607-1][xPoint_10607] === this.CELL_EMPTY
                && map[yPoint_10607-1][xPoint_10607+1] === this.CELL_EMPTY
                && map[yPoint_10607][xPoint_10607+1] === this.CELL_EMPTY
                && map[yPoint_10607+1][xPoint_10607+1] === this.CELL_EMPTY
                && map[yPoint_10607+1][xPoint_10607] === this.CELL_EMPTY
                && map[yPoint_10607+1][xPoint_10607-1] === this.CELL_EMPTY
                && map[yPoint_10607][xPoint_10607-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10607, yPoint_10607, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10607;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10607][x] === this.CELL_EMPTY
                    && map[yPoint_10607-1][x] === this.CELL_EMPTY
                    && map[yPoint_10607-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10607][x+1] === this.CELL_EMPTY
                    && map[yPoint_10607+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10607+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10607, yPoint_10607, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10607;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10607] === this.CELL_EMPTY
                    && map[y+1][xPoint_10607] === this.CELL_EMPTY
                    && map[y+1][xPoint_10607+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10607] === this.CELL_EMPTY
                    && map[y][xPoint_10607-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10607-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
