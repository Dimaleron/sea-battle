...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_12082=0;yPoint_12082<this.gameFieldBorderY.length; yPoint_12082++){
                for(var xPoint_12082=0;xPoint_12082<this.gameFieldBorderX.length; xPoint_12082++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_12082, xPoint_12082);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_12082][xPoint_12082] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_12082, xPoint_12082, 'user');
                    if(this._userShipsMap[yPoint_12082][xPoint_12082] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_12082, xPoint_12082, type){
            var id = this.getPointBlockIdByCoords(yPoint_12082, xPoint_12082, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_12082);
                block.setAttribute('data-y', yPoint_12082);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_12082
         * @param {type} xPoint_12082
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_12082, xPoint_12082, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_12082 + '_y' + yPoint_12082;
            }
            return 'pc_x' + xPoint_12082 + '_y' + yPoint_12082;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_12082=0;yPoint_12082<this.gameFieldBorderY.length; yPoint_12082++){
                for(var xPoint_12082=0;xPoint_12082<this.gameFieldBorderX.length; xPoint_12082++){
                    map.push({y: yPoint_12082, x: xPoint_12082});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_12082=-1;yPoint_12082<(this.gameFieldBorderY.length+1); yPoint_12082++){
                for(var xPoint_12082=-1;xPoint_12082<(this.gameFieldBorderX.length+1); xPoint_12082++){
                    if(!map[yPoint_12082]){
                        map[yPoint_12082] = [];
                    }
                    map[yPoint_12082][xPoint_12082] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_12082 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_12082 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_12082, yPoint_12082) === true){
                    if(this.canPutHorizontal(map, xPoint_12082, yPoint_12082, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12082][xPoint_12082 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_12082, yPoint_12082, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12082 + i][xPoint_12082] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_12082, yPoint_12082){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_12082][xPoint_12082] === this.CELL_EMPTY
                && map[yPoint_12082-1][xPoint_12082] === this.CELL_EMPTY
                && map[yPoint_12082-1][xPoint_12082+1] === this.CELL_EMPTY
                && map[yPoint_12082][xPoint_12082+1] === this.CELL_EMPTY
                && map[yPoint_12082+1][xPoint_12082+1] === this.CELL_EMPTY
                && map[yPoint_12082+1][xPoint_12082] === this.CELL_EMPTY
                && map[yPoint_12082+1][xPoint_12082-1] === this.CELL_EMPTY
                && map[yPoint_12082][xPoint_12082-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_12082, yPoint_12082, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_12082;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_12082][x] === this.CELL_EMPTY
                    && map[yPoint_12082-1][x] === this.CELL_EMPTY
                    && map[yPoint_12082-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12082][x+1] === this.CELL_EMPTY
                    && map[yPoint_12082+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12082+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_12082, yPoint_12082, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_12082;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_12082] === this.CELL_EMPTY
                    && map[y+1][xPoint_12082] === this.CELL_EMPTY
                    && map[y+1][xPoint_12082+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_12082] === this.CELL_EMPTY
                    && map[y][xPoint_12082-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_12082-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
