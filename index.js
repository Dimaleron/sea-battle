...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4728=0;yPoint_4728<this.gameFieldBorderY.length; yPoint_4728++){
                for(var xPoint_4728=0;xPoint_4728<this.gameFieldBorderX.length; xPoint_4728++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4728, xPoint_4728);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4728][xPoint_4728] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4728, xPoint_4728, 'user');
                    if(this._userShipsMap[yPoint_4728][xPoint_4728] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4728, xPoint_4728, type){
            var id = this.getPointBlockIdByCoords(yPoint_4728, xPoint_4728, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4728);
                block.setAttribute('data-y', yPoint_4728);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4728
         * @param {type} xPoint_4728
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4728, xPoint_4728, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4728 + '_y' + yPoint_4728;
            }
            return 'pc_x' + xPoint_4728 + '_y' + yPoint_4728;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4728=0;yPoint_4728<this.gameFieldBorderY.length; yPoint_4728++){
                for(var xPoint_4728=0;xPoint_4728<this.gameFieldBorderX.length; xPoint_4728++){
                    map.push({y: yPoint_4728, x: xPoint_4728});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4728=-1;yPoint_4728<(this.gameFieldBorderY.length+1); yPoint_4728++){
                for(var xPoint_4728=-1;xPoint_4728<(this.gameFieldBorderX.length+1); xPoint_4728++){
                    if(!map[yPoint_4728]){
                        map[yPoint_4728] = [];
                    }
                    map[yPoint_4728][xPoint_4728] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4728 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4728 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4728, yPoint_4728) === true){
                    if(this.canPutHorizontal(map, xPoint_4728, yPoint_4728, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4728][xPoint_4728 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4728, yPoint_4728, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4728 + i][xPoint_4728] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4728, yPoint_4728){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4728][xPoint_4728] === this.CELL_EMPTY
                && map[yPoint_4728-1][xPoint_4728] === this.CELL_EMPTY
                && map[yPoint_4728-1][xPoint_4728+1] === this.CELL_EMPTY
                && map[yPoint_4728][xPoint_4728+1] === this.CELL_EMPTY
                && map[yPoint_4728+1][xPoint_4728+1] === this.CELL_EMPTY
                && map[yPoint_4728+1][xPoint_4728] === this.CELL_EMPTY
                && map[yPoint_4728+1][xPoint_4728-1] === this.CELL_EMPTY
                && map[yPoint_4728][xPoint_4728-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4728, yPoint_4728, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4728;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4728][x] === this.CELL_EMPTY
                    && map[yPoint_4728-1][x] === this.CELL_EMPTY
                    && map[yPoint_4728-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4728][x+1] === this.CELL_EMPTY
                    && map[yPoint_4728+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4728+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4728, yPoint_4728, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4728;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4728] === this.CELL_EMPTY
                    && map[y+1][xPoint_4728] === this.CELL_EMPTY
                    && map[y+1][xPoint_4728+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4728] === this.CELL_EMPTY
                    && map[y][xPoint_4728-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4728-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
