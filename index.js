...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19860=0;yPoint_19860<this.gameFieldBorderY.length; yPoint_19860++){
                for(var xPoint_19860=0;xPoint_19860<this.gameFieldBorderX.length; xPoint_19860++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19860, xPoint_19860);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19860][xPoint_19860] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19860, xPoint_19860, 'user');
                    if(this._userShipsMap[yPoint_19860][xPoint_19860] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19860, xPoint_19860, type){
            var id = this.getPointBlockIdByCoords(yPoint_19860, xPoint_19860, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19860);
                block.setAttribute('data-y', yPoint_19860);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19860
         * @param {type} xPoint_19860
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19860, xPoint_19860, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19860 + '_y' + yPoint_19860;
            }
            return 'pc_x' + xPoint_19860 + '_y' + yPoint_19860;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19860=0;yPoint_19860<this.gameFieldBorderY.length; yPoint_19860++){
                for(var xPoint_19860=0;xPoint_19860<this.gameFieldBorderX.length; xPoint_19860++){
                    map.push({y: yPoint_19860, x: xPoint_19860});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19860=-1;yPoint_19860<(this.gameFieldBorderY.length+1); yPoint_19860++){
                for(var xPoint_19860=-1;xPoint_19860<(this.gameFieldBorderX.length+1); xPoint_19860++){
                    if(!map[yPoint_19860]){
                        map[yPoint_19860] = [];
                    }
                    map[yPoint_19860][xPoint_19860] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19860 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19860 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19860, yPoint_19860) === true){
                    if(this.canPutHorizontal(map, xPoint_19860, yPoint_19860, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19860][xPoint_19860 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19860, yPoint_19860, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19860 + i][xPoint_19860] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19860, yPoint_19860){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19860][xPoint_19860] === this.CELL_EMPTY
                && map[yPoint_19860-1][xPoint_19860] === this.CELL_EMPTY
                && map[yPoint_19860-1][xPoint_19860+1] === this.CELL_EMPTY
                && map[yPoint_19860][xPoint_19860+1] === this.CELL_EMPTY
                && map[yPoint_19860+1][xPoint_19860+1] === this.CELL_EMPTY
                && map[yPoint_19860+1][xPoint_19860] === this.CELL_EMPTY
                && map[yPoint_19860+1][xPoint_19860-1] === this.CELL_EMPTY
                && map[yPoint_19860][xPoint_19860-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19860, yPoint_19860, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19860;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19860][x] === this.CELL_EMPTY
                    && map[yPoint_19860-1][x] === this.CELL_EMPTY
                    && map[yPoint_19860-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19860][x+1] === this.CELL_EMPTY
                    && map[yPoint_19860+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19860+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19860, yPoint_19860, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19860;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19860] === this.CELL_EMPTY
                    && map[y+1][xPoint_19860] === this.CELL_EMPTY
                    && map[y+1][xPoint_19860+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19860] === this.CELL_EMPTY
                    && map[y][xPoint_19860-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19860-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
