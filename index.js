...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22662=0;yPoint_22662<this.gameFieldBorderY.length; yPoint_22662++){
                for(var xPoint_22662=0;xPoint_22662<this.gameFieldBorderX.length; xPoint_22662++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22662, xPoint_22662);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22662][xPoint_22662] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22662, xPoint_22662, 'user');
                    if(this._userShipsMap[yPoint_22662][xPoint_22662] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22662, xPoint_22662, type){
            var id = this.getPointBlockIdByCoords(yPoint_22662, xPoint_22662, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22662);
                block.setAttribute('data-y', yPoint_22662);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22662
         * @param {type} xPoint_22662
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22662, xPoint_22662, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22662 + '_y' + yPoint_22662;
            }
            return 'pc_x' + xPoint_22662 + '_y' + yPoint_22662;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22662=0;yPoint_22662<this.gameFieldBorderY.length; yPoint_22662++){
                for(var xPoint_22662=0;xPoint_22662<this.gameFieldBorderX.length; xPoint_22662++){
                    map.push({y: yPoint_22662, x: xPoint_22662});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22662=-1;yPoint_22662<(this.gameFieldBorderY.length+1); yPoint_22662++){
                for(var xPoint_22662=-1;xPoint_22662<(this.gameFieldBorderX.length+1); xPoint_22662++){
                    if(!map[yPoint_22662]){
                        map[yPoint_22662] = [];
                    }
                    map[yPoint_22662][xPoint_22662] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22662 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22662 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22662, yPoint_22662) === true){
                    if(this.canPutHorizontal(map, xPoint_22662, yPoint_22662, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22662][xPoint_22662 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22662, yPoint_22662, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22662 + i][xPoint_22662] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22662, yPoint_22662){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22662][xPoint_22662] === this.CELL_EMPTY
                && map[yPoint_22662-1][xPoint_22662] === this.CELL_EMPTY
                && map[yPoint_22662-1][xPoint_22662+1] === this.CELL_EMPTY
                && map[yPoint_22662][xPoint_22662+1] === this.CELL_EMPTY
                && map[yPoint_22662+1][xPoint_22662+1] === this.CELL_EMPTY
                && map[yPoint_22662+1][xPoint_22662] === this.CELL_EMPTY
                && map[yPoint_22662+1][xPoint_22662-1] === this.CELL_EMPTY
                && map[yPoint_22662][xPoint_22662-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22662, yPoint_22662, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22662;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22662][x] === this.CELL_EMPTY
                    && map[yPoint_22662-1][x] === this.CELL_EMPTY
                    && map[yPoint_22662-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22662][x+1] === this.CELL_EMPTY
                    && map[yPoint_22662+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22662+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22662, yPoint_22662, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22662;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22662] === this.CELL_EMPTY
                    && map[y+1][xPoint_22662] === this.CELL_EMPTY
                    && map[y+1][xPoint_22662+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22662] === this.CELL_EMPTY
                    && map[y][xPoint_22662-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22662-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
