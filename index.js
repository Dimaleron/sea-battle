...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_974=0;yPoint_974<this.gameFieldBorderY.length; yPoint_974++){
                for(var xPoint_974=0;xPoint_974<this.gameFieldBorderX.length; xPoint_974++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_974, xPoint_974);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_974][xPoint_974] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_974, xPoint_974, 'user');
                    if(this._userShipsMap[yPoint_974][xPoint_974] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_974, xPoint_974, type){
            var id = this.getPointBlockIdByCoords(yPoint_974, xPoint_974, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_974);
                block.setAttribute('data-y', yPoint_974);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_974
         * @param {type} xPoint_974
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_974, xPoint_974, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_974 + '_y' + yPoint_974;
            }
            return 'pc_x' + xPoint_974 + '_y' + yPoint_974;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_974=0;yPoint_974<this.gameFieldBorderY.length; yPoint_974++){
                for(var xPoint_974=0;xPoint_974<this.gameFieldBorderX.length; xPoint_974++){
                    map.push({y: yPoint_974, x: xPoint_974});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_974=-1;yPoint_974<(this.gameFieldBorderY.length+1); yPoint_974++){
                for(var xPoint_974=-1;xPoint_974<(this.gameFieldBorderX.length+1); xPoint_974++){
                    if(!map[yPoint_974]){
                        map[yPoint_974] = [];
                    }
                    map[yPoint_974][xPoint_974] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_974 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_974 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_974, yPoint_974) === true){
                    if(this.canPutHorizontal(map, xPoint_974, yPoint_974, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_974][xPoint_974 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_974, yPoint_974, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_974 + i][xPoint_974] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_974, yPoint_974){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_974][xPoint_974] === this.CELL_EMPTY
                && map[yPoint_974-1][xPoint_974] === this.CELL_EMPTY
                && map[yPoint_974-1][xPoint_974+1] === this.CELL_EMPTY
                && map[yPoint_974][xPoint_974+1] === this.CELL_EMPTY
                && map[yPoint_974+1][xPoint_974+1] === this.CELL_EMPTY
                && map[yPoint_974+1][xPoint_974] === this.CELL_EMPTY
                && map[yPoint_974+1][xPoint_974-1] === this.CELL_EMPTY
                && map[yPoint_974][xPoint_974-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_974, yPoint_974, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_974;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_974][x] === this.CELL_EMPTY
                    && map[yPoint_974-1][x] === this.CELL_EMPTY
                    && map[yPoint_974-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_974][x+1] === this.CELL_EMPTY
                    && map[yPoint_974+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_974+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_974, yPoint_974, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_974;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_974] === this.CELL_EMPTY
                    && map[y+1][xPoint_974] === this.CELL_EMPTY
                    && map[y+1][xPoint_974+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_974] === this.CELL_EMPTY
                    && map[y][xPoint_974-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_974-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
