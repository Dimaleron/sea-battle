...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14941=0;yPoint_14941<this.gameFieldBorderY.length; yPoint_14941++){
                for(var xPoint_14941=0;xPoint_14941<this.gameFieldBorderX.length; xPoint_14941++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14941, xPoint_14941);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14941][xPoint_14941] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14941, xPoint_14941, 'user');
                    if(this._userShipsMap[yPoint_14941][xPoint_14941] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14941, xPoint_14941, type){
            var id = this.getPointBlockIdByCoords(yPoint_14941, xPoint_14941, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14941);
                block.setAttribute('data-y', yPoint_14941);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14941
         * @param {type} xPoint_14941
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14941, xPoint_14941, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14941 + '_y' + yPoint_14941;
            }
            return 'pc_x' + xPoint_14941 + '_y' + yPoint_14941;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14941=0;yPoint_14941<this.gameFieldBorderY.length; yPoint_14941++){
                for(var xPoint_14941=0;xPoint_14941<this.gameFieldBorderX.length; xPoint_14941++){
                    map.push({y: yPoint_14941, x: xPoint_14941});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14941=-1;yPoint_14941<(this.gameFieldBorderY.length+1); yPoint_14941++){
                for(var xPoint_14941=-1;xPoint_14941<(this.gameFieldBorderX.length+1); xPoint_14941++){
                    if(!map[yPoint_14941]){
                        map[yPoint_14941] = [];
                    }
                    map[yPoint_14941][xPoint_14941] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14941 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14941 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14941, yPoint_14941) === true){
                    if(this.canPutHorizontal(map, xPoint_14941, yPoint_14941, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14941][xPoint_14941 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14941, yPoint_14941, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14941 + i][xPoint_14941] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14941, yPoint_14941){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14941][xPoint_14941] === this.CELL_EMPTY
                && map[yPoint_14941-1][xPoint_14941] === this.CELL_EMPTY
                && map[yPoint_14941-1][xPoint_14941+1] === this.CELL_EMPTY
                && map[yPoint_14941][xPoint_14941+1] === this.CELL_EMPTY
                && map[yPoint_14941+1][xPoint_14941+1] === this.CELL_EMPTY
                && map[yPoint_14941+1][xPoint_14941] === this.CELL_EMPTY
                && map[yPoint_14941+1][xPoint_14941-1] === this.CELL_EMPTY
                && map[yPoint_14941][xPoint_14941-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14941, yPoint_14941, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14941;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14941][x] === this.CELL_EMPTY
                    && map[yPoint_14941-1][x] === this.CELL_EMPTY
                    && map[yPoint_14941-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14941][x+1] === this.CELL_EMPTY
                    && map[yPoint_14941+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14941+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14941, yPoint_14941, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14941;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14941] === this.CELL_EMPTY
                    && map[y+1][xPoint_14941] === this.CELL_EMPTY
                    && map[y+1][xPoint_14941+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14941] === this.CELL_EMPTY
                    && map[y][xPoint_14941-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14941-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
