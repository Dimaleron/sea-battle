...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31024=0;yPoint_31024<this.gameFieldBorderY.length; yPoint_31024++){
                for(var xPoint_31024=0;xPoint_31024<this.gameFieldBorderX.length; xPoint_31024++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31024, xPoint_31024);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31024][xPoint_31024] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31024, xPoint_31024, 'user');
                    if(this._userShipsMap[yPoint_31024][xPoint_31024] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31024, xPoint_31024, type){
            var id = this.getPointBlockIdByCoords(yPoint_31024, xPoint_31024, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31024);
                block.setAttribute('data-y', yPoint_31024);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31024
         * @param {type} xPoint_31024
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31024, xPoint_31024, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31024 + '_y' + yPoint_31024;
            }
            return 'pc_x' + xPoint_31024 + '_y' + yPoint_31024;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31024=0;yPoint_31024<this.gameFieldBorderY.length; yPoint_31024++){
                for(var xPoint_31024=0;xPoint_31024<this.gameFieldBorderX.length; xPoint_31024++){
                    map.push({y: yPoint_31024, x: xPoint_31024});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31024=-1;yPoint_31024<(this.gameFieldBorderY.length+1); yPoint_31024++){
                for(var xPoint_31024=-1;xPoint_31024<(this.gameFieldBorderX.length+1); xPoint_31024++){
                    if(!map[yPoint_31024]){
                        map[yPoint_31024] = [];
                    }
                    map[yPoint_31024][xPoint_31024] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31024 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31024 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31024, yPoint_31024) === true){
                    if(this.canPutHorizontal(map, xPoint_31024, yPoint_31024, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31024][xPoint_31024 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31024, yPoint_31024, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31024 + i][xPoint_31024] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31024, yPoint_31024){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31024][xPoint_31024] === this.CELL_EMPTY
                && map[yPoint_31024-1][xPoint_31024] === this.CELL_EMPTY
                && map[yPoint_31024-1][xPoint_31024+1] === this.CELL_EMPTY
                && map[yPoint_31024][xPoint_31024+1] === this.CELL_EMPTY
                && map[yPoint_31024+1][xPoint_31024+1] === this.CELL_EMPTY
                && map[yPoint_31024+1][xPoint_31024] === this.CELL_EMPTY
                && map[yPoint_31024+1][xPoint_31024-1] === this.CELL_EMPTY
                && map[yPoint_31024][xPoint_31024-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31024, yPoint_31024, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31024;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31024][x] === this.CELL_EMPTY
                    && map[yPoint_31024-1][x] === this.CELL_EMPTY
                    && map[yPoint_31024-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31024][x+1] === this.CELL_EMPTY
                    && map[yPoint_31024+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31024+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31024, yPoint_31024, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31024;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31024] === this.CELL_EMPTY
                    && map[y+1][xPoint_31024] === this.CELL_EMPTY
                    && map[y+1][xPoint_31024+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31024] === this.CELL_EMPTY
                    && map[y][xPoint_31024-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31024-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
