...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22577=0;yPoint_22577<this.gameFieldBorderY.length; yPoint_22577++){
                for(var xPoint_22577=0;xPoint_22577<this.gameFieldBorderX.length; xPoint_22577++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22577, xPoint_22577);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22577][xPoint_22577] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22577, xPoint_22577, 'user');
                    if(this._userShipsMap[yPoint_22577][xPoint_22577] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22577, xPoint_22577, type){
            var id = this.getPointBlockIdByCoords(yPoint_22577, xPoint_22577, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22577);
                block.setAttribute('data-y', yPoint_22577);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22577
         * @param {type} xPoint_22577
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22577, xPoint_22577, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22577 + '_y' + yPoint_22577;
            }
            return 'pc_x' + xPoint_22577 + '_y' + yPoint_22577;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22577=0;yPoint_22577<this.gameFieldBorderY.length; yPoint_22577++){
                for(var xPoint_22577=0;xPoint_22577<this.gameFieldBorderX.length; xPoint_22577++){
                    map.push({y: yPoint_22577, x: xPoint_22577});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22577=-1;yPoint_22577<(this.gameFieldBorderY.length+1); yPoint_22577++){
                for(var xPoint_22577=-1;xPoint_22577<(this.gameFieldBorderX.length+1); xPoint_22577++){
                    if(!map[yPoint_22577]){
                        map[yPoint_22577] = [];
                    }
                    map[yPoint_22577][xPoint_22577] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22577 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22577 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22577, yPoint_22577) === true){
                    if(this.canPutHorizontal(map, xPoint_22577, yPoint_22577, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22577][xPoint_22577 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22577, yPoint_22577, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22577 + i][xPoint_22577] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22577, yPoint_22577){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22577][xPoint_22577] === this.CELL_EMPTY
                && map[yPoint_22577-1][xPoint_22577] === this.CELL_EMPTY
                && map[yPoint_22577-1][xPoint_22577+1] === this.CELL_EMPTY
                && map[yPoint_22577][xPoint_22577+1] === this.CELL_EMPTY
                && map[yPoint_22577+1][xPoint_22577+1] === this.CELL_EMPTY
                && map[yPoint_22577+1][xPoint_22577] === this.CELL_EMPTY
                && map[yPoint_22577+1][xPoint_22577-1] === this.CELL_EMPTY
                && map[yPoint_22577][xPoint_22577-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22577, yPoint_22577, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22577;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22577][x] === this.CELL_EMPTY
                    && map[yPoint_22577-1][x] === this.CELL_EMPTY
                    && map[yPoint_22577-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22577][x+1] === this.CELL_EMPTY
                    && map[yPoint_22577+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22577+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22577, yPoint_22577, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22577;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22577] === this.CELL_EMPTY
                    && map[y+1][xPoint_22577] === this.CELL_EMPTY
                    && map[y+1][xPoint_22577+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22577] === this.CELL_EMPTY
                    && map[y][xPoint_22577-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22577-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
