...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14127=0;yPoint_14127<this.gameFieldBorderY.length; yPoint_14127++){
                for(var xPoint_14127=0;xPoint_14127<this.gameFieldBorderX.length; xPoint_14127++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14127, xPoint_14127);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14127][xPoint_14127] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14127, xPoint_14127, 'user');
                    if(this._userShipsMap[yPoint_14127][xPoint_14127] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14127, xPoint_14127, type){
            var id = this.getPointBlockIdByCoords(yPoint_14127, xPoint_14127, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14127);
                block.setAttribute('data-y', yPoint_14127);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14127
         * @param {type} xPoint_14127
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14127, xPoint_14127, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14127 + '_y' + yPoint_14127;
            }
            return 'pc_x' + xPoint_14127 + '_y' + yPoint_14127;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14127=0;yPoint_14127<this.gameFieldBorderY.length; yPoint_14127++){
                for(var xPoint_14127=0;xPoint_14127<this.gameFieldBorderX.length; xPoint_14127++){
                    map.push({y: yPoint_14127, x: xPoint_14127});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14127=-1;yPoint_14127<(this.gameFieldBorderY.length+1); yPoint_14127++){
                for(var xPoint_14127=-1;xPoint_14127<(this.gameFieldBorderX.length+1); xPoint_14127++){
                    if(!map[yPoint_14127]){
                        map[yPoint_14127] = [];
                    }
                    map[yPoint_14127][xPoint_14127] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14127 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14127 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14127, yPoint_14127) === true){
                    if(this.canPutHorizontal(map, xPoint_14127, yPoint_14127, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14127][xPoint_14127 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14127, yPoint_14127, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14127 + i][xPoint_14127] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14127, yPoint_14127){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14127][xPoint_14127] === this.CELL_EMPTY
                && map[yPoint_14127-1][xPoint_14127] === this.CELL_EMPTY
                && map[yPoint_14127-1][xPoint_14127+1] === this.CELL_EMPTY
                && map[yPoint_14127][xPoint_14127+1] === this.CELL_EMPTY
                && map[yPoint_14127+1][xPoint_14127+1] === this.CELL_EMPTY
                && map[yPoint_14127+1][xPoint_14127] === this.CELL_EMPTY
                && map[yPoint_14127+1][xPoint_14127-1] === this.CELL_EMPTY
                && map[yPoint_14127][xPoint_14127-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14127, yPoint_14127, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14127;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14127][x] === this.CELL_EMPTY
                    && map[yPoint_14127-1][x] === this.CELL_EMPTY
                    && map[yPoint_14127-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14127][x+1] === this.CELL_EMPTY
                    && map[yPoint_14127+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14127+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14127, yPoint_14127, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14127;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14127] === this.CELL_EMPTY
                    && map[y+1][xPoint_14127] === this.CELL_EMPTY
                    && map[y+1][xPoint_14127+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14127] === this.CELL_EMPTY
                    && map[y][xPoint_14127-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14127-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
