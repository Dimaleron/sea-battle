...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1435=0;yPoint_1435<this.gameFieldBorderY.length; yPoint_1435++){
                for(var xPoint_1435=0;xPoint_1435<this.gameFieldBorderX.length; xPoint_1435++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1435, xPoint_1435);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1435][xPoint_1435] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1435, xPoint_1435, 'user');
                    if(this._userShipsMap[yPoint_1435][xPoint_1435] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1435, xPoint_1435, type){
            var id = this.getPointBlockIdByCoords(yPoint_1435, xPoint_1435, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1435);
                block.setAttribute('data-y', yPoint_1435);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1435
         * @param {type} xPoint_1435
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1435, xPoint_1435, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1435 + '_y' + yPoint_1435;
            }
            return 'pc_x' + xPoint_1435 + '_y' + yPoint_1435;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1435=0;yPoint_1435<this.gameFieldBorderY.length; yPoint_1435++){
                for(var xPoint_1435=0;xPoint_1435<this.gameFieldBorderX.length; xPoint_1435++){
                    map.push({y: yPoint_1435, x: xPoint_1435});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1435=-1;yPoint_1435<(this.gameFieldBorderY.length+1); yPoint_1435++){
                for(var xPoint_1435=-1;xPoint_1435<(this.gameFieldBorderX.length+1); xPoint_1435++){
                    if(!map[yPoint_1435]){
                        map[yPoint_1435] = [];
                    }
                    map[yPoint_1435][xPoint_1435] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1435 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1435 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1435, yPoint_1435) === true){
                    if(this.canPutHorizontal(map, xPoint_1435, yPoint_1435, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1435][xPoint_1435 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1435, yPoint_1435, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1435 + i][xPoint_1435] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1435, yPoint_1435){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1435][xPoint_1435] === this.CELL_EMPTY
                && map[yPoint_1435-1][xPoint_1435] === this.CELL_EMPTY
                && map[yPoint_1435-1][xPoint_1435+1] === this.CELL_EMPTY
                && map[yPoint_1435][xPoint_1435+1] === this.CELL_EMPTY
                && map[yPoint_1435+1][xPoint_1435+1] === this.CELL_EMPTY
                && map[yPoint_1435+1][xPoint_1435] === this.CELL_EMPTY
                && map[yPoint_1435+1][xPoint_1435-1] === this.CELL_EMPTY
                && map[yPoint_1435][xPoint_1435-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1435, yPoint_1435, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1435;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1435][x] === this.CELL_EMPTY
                    && map[yPoint_1435-1][x] === this.CELL_EMPTY
                    && map[yPoint_1435-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1435][x+1] === this.CELL_EMPTY
                    && map[yPoint_1435+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1435+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1435, yPoint_1435, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1435;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1435] === this.CELL_EMPTY
                    && map[y+1][xPoint_1435] === this.CELL_EMPTY
                    && map[y+1][xPoint_1435+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1435] === this.CELL_EMPTY
                    && map[y][xPoint_1435-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1435-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
