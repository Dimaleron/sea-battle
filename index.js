...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16908=0;yPoint_16908<this.gameFieldBorderY.length; yPoint_16908++){
                for(var xPoint_16908=0;xPoint_16908<this.gameFieldBorderX.length; xPoint_16908++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16908, xPoint_16908);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16908][xPoint_16908] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16908, xPoint_16908, 'user');
                    if(this._userShipsMap[yPoint_16908][xPoint_16908] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16908, xPoint_16908, type){
            var id = this.getPointBlockIdByCoords(yPoint_16908, xPoint_16908, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16908);
                block.setAttribute('data-y', yPoint_16908);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16908
         * @param {type} xPoint_16908
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16908, xPoint_16908, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16908 + '_y' + yPoint_16908;
            }
            return 'pc_x' + xPoint_16908 + '_y' + yPoint_16908;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16908=0;yPoint_16908<this.gameFieldBorderY.length; yPoint_16908++){
                for(var xPoint_16908=0;xPoint_16908<this.gameFieldBorderX.length; xPoint_16908++){
                    map.push({y: yPoint_16908, x: xPoint_16908});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16908=-1;yPoint_16908<(this.gameFieldBorderY.length+1); yPoint_16908++){
                for(var xPoint_16908=-1;xPoint_16908<(this.gameFieldBorderX.length+1); xPoint_16908++){
                    if(!map[yPoint_16908]){
                        map[yPoint_16908] = [];
                    }
                    map[yPoint_16908][xPoint_16908] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16908 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16908 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16908, yPoint_16908) === true){
                    if(this.canPutHorizontal(map, xPoint_16908, yPoint_16908, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16908][xPoint_16908 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16908, yPoint_16908, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16908 + i][xPoint_16908] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16908, yPoint_16908){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16908][xPoint_16908] === this.CELL_EMPTY
                && map[yPoint_16908-1][xPoint_16908] === this.CELL_EMPTY
                && map[yPoint_16908-1][xPoint_16908+1] === this.CELL_EMPTY
                && map[yPoint_16908][xPoint_16908+1] === this.CELL_EMPTY
                && map[yPoint_16908+1][xPoint_16908+1] === this.CELL_EMPTY
                && map[yPoint_16908+1][xPoint_16908] === this.CELL_EMPTY
                && map[yPoint_16908+1][xPoint_16908-1] === this.CELL_EMPTY
                && map[yPoint_16908][xPoint_16908-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16908, yPoint_16908, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16908;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16908][x] === this.CELL_EMPTY
                    && map[yPoint_16908-1][x] === this.CELL_EMPTY
                    && map[yPoint_16908-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16908][x+1] === this.CELL_EMPTY
                    && map[yPoint_16908+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16908+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16908, yPoint_16908, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16908;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16908] === this.CELL_EMPTY
                    && map[y+1][xPoint_16908] === this.CELL_EMPTY
                    && map[y+1][xPoint_16908+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16908] === this.CELL_EMPTY
                    && map[y][xPoint_16908-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16908-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
