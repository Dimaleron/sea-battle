...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31981=0;yPoint_31981<this.gameFieldBorderY.length; yPoint_31981++){
                for(var xPoint_31981=0;xPoint_31981<this.gameFieldBorderX.length; xPoint_31981++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31981, xPoint_31981);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31981][xPoint_31981] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31981, xPoint_31981, 'user');
                    if(this._userShipsMap[yPoint_31981][xPoint_31981] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31981, xPoint_31981, type){
            var id = this.getPointBlockIdByCoords(yPoint_31981, xPoint_31981, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31981);
                block.setAttribute('data-y', yPoint_31981);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31981
         * @param {type} xPoint_31981
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31981, xPoint_31981, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31981 + '_y' + yPoint_31981;
            }
            return 'pc_x' + xPoint_31981 + '_y' + yPoint_31981;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31981=0;yPoint_31981<this.gameFieldBorderY.length; yPoint_31981++){
                for(var xPoint_31981=0;xPoint_31981<this.gameFieldBorderX.length; xPoint_31981++){
                    map.push({y: yPoint_31981, x: xPoint_31981});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31981=-1;yPoint_31981<(this.gameFieldBorderY.length+1); yPoint_31981++){
                for(var xPoint_31981=-1;xPoint_31981<(this.gameFieldBorderX.length+1); xPoint_31981++){
                    if(!map[yPoint_31981]){
                        map[yPoint_31981] = [];
                    }
                    map[yPoint_31981][xPoint_31981] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31981 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31981 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31981, yPoint_31981) === true){
                    if(this.canPutHorizontal(map, xPoint_31981, yPoint_31981, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31981][xPoint_31981 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31981, yPoint_31981, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31981 + i][xPoint_31981] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31981, yPoint_31981){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31981][xPoint_31981] === this.CELL_EMPTY
                && map[yPoint_31981-1][xPoint_31981] === this.CELL_EMPTY
                && map[yPoint_31981-1][xPoint_31981+1] === this.CELL_EMPTY
                && map[yPoint_31981][xPoint_31981+1] === this.CELL_EMPTY
                && map[yPoint_31981+1][xPoint_31981+1] === this.CELL_EMPTY
                && map[yPoint_31981+1][xPoint_31981] === this.CELL_EMPTY
                && map[yPoint_31981+1][xPoint_31981-1] === this.CELL_EMPTY
                && map[yPoint_31981][xPoint_31981-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31981, yPoint_31981, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31981;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31981][x] === this.CELL_EMPTY
                    && map[yPoint_31981-1][x] === this.CELL_EMPTY
                    && map[yPoint_31981-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31981][x+1] === this.CELL_EMPTY
                    && map[yPoint_31981+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31981+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31981, yPoint_31981, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31981;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31981] === this.CELL_EMPTY
                    && map[y+1][xPoint_31981] === this.CELL_EMPTY
                    && map[y+1][xPoint_31981+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31981] === this.CELL_EMPTY
                    && map[y][xPoint_31981-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31981-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
