...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11615=0;yPoint_11615<this.gameFieldBorderY.length; yPoint_11615++){
                for(var xPoint_11615=0;xPoint_11615<this.gameFieldBorderX.length; xPoint_11615++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11615, xPoint_11615);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11615][xPoint_11615] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11615, xPoint_11615, 'user');
                    if(this._userShipsMap[yPoint_11615][xPoint_11615] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11615, xPoint_11615, type){
            var id = this.getPointBlockIdByCoords(yPoint_11615, xPoint_11615, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11615);
                block.setAttribute('data-y', yPoint_11615);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11615
         * @param {type} xPoint_11615
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11615, xPoint_11615, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11615 + '_y' + yPoint_11615;
            }
            return 'pc_x' + xPoint_11615 + '_y' + yPoint_11615;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11615=0;yPoint_11615<this.gameFieldBorderY.length; yPoint_11615++){
                for(var xPoint_11615=0;xPoint_11615<this.gameFieldBorderX.length; xPoint_11615++){
                    map.push({y: yPoint_11615, x: xPoint_11615});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11615=-1;yPoint_11615<(this.gameFieldBorderY.length+1); yPoint_11615++){
                for(var xPoint_11615=-1;xPoint_11615<(this.gameFieldBorderX.length+1); xPoint_11615++){
                    if(!map[yPoint_11615]){
                        map[yPoint_11615] = [];
                    }
                    map[yPoint_11615][xPoint_11615] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11615 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11615 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11615, yPoint_11615) === true){
                    if(this.canPutHorizontal(map, xPoint_11615, yPoint_11615, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11615][xPoint_11615 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11615, yPoint_11615, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11615 + i][xPoint_11615] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11615, yPoint_11615){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11615][xPoint_11615] === this.CELL_EMPTY
                && map[yPoint_11615-1][xPoint_11615] === this.CELL_EMPTY
                && map[yPoint_11615-1][xPoint_11615+1] === this.CELL_EMPTY
                && map[yPoint_11615][xPoint_11615+1] === this.CELL_EMPTY
                && map[yPoint_11615+1][xPoint_11615+1] === this.CELL_EMPTY
                && map[yPoint_11615+1][xPoint_11615] === this.CELL_EMPTY
                && map[yPoint_11615+1][xPoint_11615-1] === this.CELL_EMPTY
                && map[yPoint_11615][xPoint_11615-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11615, yPoint_11615, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11615;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11615][x] === this.CELL_EMPTY
                    && map[yPoint_11615-1][x] === this.CELL_EMPTY
                    && map[yPoint_11615-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11615][x+1] === this.CELL_EMPTY
                    && map[yPoint_11615+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11615+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11615, yPoint_11615, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11615;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11615] === this.CELL_EMPTY
                    && map[y+1][xPoint_11615] === this.CELL_EMPTY
                    && map[y+1][xPoint_11615+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11615] === this.CELL_EMPTY
                    && map[y][xPoint_11615-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11615-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
