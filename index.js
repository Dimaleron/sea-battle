...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14741=0;yPoint_14741<this.gameFieldBorderY.length; yPoint_14741++){
                for(var xPoint_14741=0;xPoint_14741<this.gameFieldBorderX.length; xPoint_14741++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14741, xPoint_14741);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14741][xPoint_14741] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14741, xPoint_14741, 'user');
                    if(this._userShipsMap[yPoint_14741][xPoint_14741] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14741, xPoint_14741, type){
            var id = this.getPointBlockIdByCoords(yPoint_14741, xPoint_14741, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14741);
                block.setAttribute('data-y', yPoint_14741);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14741
         * @param {type} xPoint_14741
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14741, xPoint_14741, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14741 + '_y' + yPoint_14741;
            }
            return 'pc_x' + xPoint_14741 + '_y' + yPoint_14741;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14741=0;yPoint_14741<this.gameFieldBorderY.length; yPoint_14741++){
                for(var xPoint_14741=0;xPoint_14741<this.gameFieldBorderX.length; xPoint_14741++){
                    map.push({y: yPoint_14741, x: xPoint_14741});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14741=-1;yPoint_14741<(this.gameFieldBorderY.length+1); yPoint_14741++){
                for(var xPoint_14741=-1;xPoint_14741<(this.gameFieldBorderX.length+1); xPoint_14741++){
                    if(!map[yPoint_14741]){
                        map[yPoint_14741] = [];
                    }
                    map[yPoint_14741][xPoint_14741] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14741 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14741 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14741, yPoint_14741) === true){
                    if(this.canPutHorizontal(map, xPoint_14741, yPoint_14741, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14741][xPoint_14741 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14741, yPoint_14741, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14741 + i][xPoint_14741] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14741, yPoint_14741){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14741][xPoint_14741] === this.CELL_EMPTY
                && map[yPoint_14741-1][xPoint_14741] === this.CELL_EMPTY
                && map[yPoint_14741-1][xPoint_14741+1] === this.CELL_EMPTY
                && map[yPoint_14741][xPoint_14741+1] === this.CELL_EMPTY
                && map[yPoint_14741+1][xPoint_14741+1] === this.CELL_EMPTY
                && map[yPoint_14741+1][xPoint_14741] === this.CELL_EMPTY
                && map[yPoint_14741+1][xPoint_14741-1] === this.CELL_EMPTY
                && map[yPoint_14741][xPoint_14741-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14741, yPoint_14741, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14741;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14741][x] === this.CELL_EMPTY
                    && map[yPoint_14741-1][x] === this.CELL_EMPTY
                    && map[yPoint_14741-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14741][x+1] === this.CELL_EMPTY
                    && map[yPoint_14741+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14741+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14741, yPoint_14741, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14741;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14741] === this.CELL_EMPTY
                    && map[y+1][xPoint_14741] === this.CELL_EMPTY
                    && map[y+1][xPoint_14741+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14741] === this.CELL_EMPTY
                    && map[y][xPoint_14741-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14741-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
