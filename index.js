...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_586=0;yPoint_586<this.gameFieldBorderY.length; yPoint_586++){
                for(var xPoint_586=0;xPoint_586<this.gameFieldBorderX.length; xPoint_586++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_586, xPoint_586);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_586][xPoint_586] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_586, xPoint_586, 'user');
                    if(this._userShipsMap[yPoint_586][xPoint_586] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_586, xPoint_586, type){
            var id = this.getPointBlockIdByCoords(yPoint_586, xPoint_586, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_586);
                block.setAttribute('data-y', yPoint_586);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_586
         * @param {type} xPoint_586
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_586, xPoint_586, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_586 + '_y' + yPoint_586;
            }
            return 'pc_x' + xPoint_586 + '_y' + yPoint_586;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_586=0;yPoint_586<this.gameFieldBorderY.length; yPoint_586++){
                for(var xPoint_586=0;xPoint_586<this.gameFieldBorderX.length; xPoint_586++){
                    map.push({y: yPoint_586, x: xPoint_586});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_586=-1;yPoint_586<(this.gameFieldBorderY.length+1); yPoint_586++){
                for(var xPoint_586=-1;xPoint_586<(this.gameFieldBorderX.length+1); xPoint_586++){
                    if(!map[yPoint_586]){
                        map[yPoint_586] = [];
                    }
                    map[yPoint_586][xPoint_586] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_586 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_586 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_586, yPoint_586) === true){
                    if(this.canPutHorizontal(map, xPoint_586, yPoint_586, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_586][xPoint_586 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_586, yPoint_586, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_586 + i][xPoint_586] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_586, yPoint_586){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_586][xPoint_586] === this.CELL_EMPTY
                && map[yPoint_586-1][xPoint_586] === this.CELL_EMPTY
                && map[yPoint_586-1][xPoint_586+1] === this.CELL_EMPTY
                && map[yPoint_586][xPoint_586+1] === this.CELL_EMPTY
                && map[yPoint_586+1][xPoint_586+1] === this.CELL_EMPTY
                && map[yPoint_586+1][xPoint_586] === this.CELL_EMPTY
                && map[yPoint_586+1][xPoint_586-1] === this.CELL_EMPTY
                && map[yPoint_586][xPoint_586-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_586, yPoint_586, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_586;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_586][x] === this.CELL_EMPTY
                    && map[yPoint_586-1][x] === this.CELL_EMPTY
                    && map[yPoint_586-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_586][x+1] === this.CELL_EMPTY
                    && map[yPoint_586+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_586+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_586, yPoint_586, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_586;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_586] === this.CELL_EMPTY
                    && map[y+1][xPoint_586] === this.CELL_EMPTY
                    && map[y+1][xPoint_586+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_586] === this.CELL_EMPTY
                    && map[y][xPoint_586-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_586-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
