...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22566=0;yPoint_22566<this.gameFieldBorderY.length; yPoint_22566++){
                for(var xPoint_22566=0;xPoint_22566<this.gameFieldBorderX.length; xPoint_22566++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22566, xPoint_22566);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22566][xPoint_22566] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22566, xPoint_22566, 'user');
                    if(this._userShipsMap[yPoint_22566][xPoint_22566] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22566, xPoint_22566, type){
            var id = this.getPointBlockIdByCoords(yPoint_22566, xPoint_22566, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22566);
                block.setAttribute('data-y', yPoint_22566);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22566
         * @param {type} xPoint_22566
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22566, xPoint_22566, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22566 + '_y' + yPoint_22566;
            }
            return 'pc_x' + xPoint_22566 + '_y' + yPoint_22566;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22566=0;yPoint_22566<this.gameFieldBorderY.length; yPoint_22566++){
                for(var xPoint_22566=0;xPoint_22566<this.gameFieldBorderX.length; xPoint_22566++){
                    map.push({y: yPoint_22566, x: xPoint_22566});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22566=-1;yPoint_22566<(this.gameFieldBorderY.length+1); yPoint_22566++){
                for(var xPoint_22566=-1;xPoint_22566<(this.gameFieldBorderX.length+1); xPoint_22566++){
                    if(!map[yPoint_22566]){
                        map[yPoint_22566] = [];
                    }
                    map[yPoint_22566][xPoint_22566] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22566 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22566 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22566, yPoint_22566) === true){
                    if(this.canPutHorizontal(map, xPoint_22566, yPoint_22566, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22566][xPoint_22566 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22566, yPoint_22566, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22566 + i][xPoint_22566] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22566, yPoint_22566){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22566][xPoint_22566] === this.CELL_EMPTY
                && map[yPoint_22566-1][xPoint_22566] === this.CELL_EMPTY
                && map[yPoint_22566-1][xPoint_22566+1] === this.CELL_EMPTY
                && map[yPoint_22566][xPoint_22566+1] === this.CELL_EMPTY
                && map[yPoint_22566+1][xPoint_22566+1] === this.CELL_EMPTY
                && map[yPoint_22566+1][xPoint_22566] === this.CELL_EMPTY
                && map[yPoint_22566+1][xPoint_22566-1] === this.CELL_EMPTY
                && map[yPoint_22566][xPoint_22566-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22566, yPoint_22566, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22566;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22566][x] === this.CELL_EMPTY
                    && map[yPoint_22566-1][x] === this.CELL_EMPTY
                    && map[yPoint_22566-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22566][x+1] === this.CELL_EMPTY
                    && map[yPoint_22566+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22566+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22566, yPoint_22566, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22566;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22566] === this.CELL_EMPTY
                    && map[y+1][xPoint_22566] === this.CELL_EMPTY
                    && map[y+1][xPoint_22566+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22566] === this.CELL_EMPTY
                    && map[y][xPoint_22566-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22566-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
