...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_236=0;yPoint_236<this.gameFieldBorderY.length; yPoint_236++){
                for(var xPoint_236=0;xPoint_236<this.gameFieldBorderX.length; xPoint_236++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_236, xPoint_236);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_236][xPoint_236] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_236, xPoint_236, 'user');
                    if(this._userShipsMap[yPoint_236][xPoint_236] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_236, xPoint_236, type){
            var id = this.getPointBlockIdByCoords(yPoint_236, xPoint_236, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_236);
                block.setAttribute('data-y', yPoint_236);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_236
         * @param {type} xPoint_236
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_236, xPoint_236, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_236 + '_y' + yPoint_236;
            }
            return 'pc_x' + xPoint_236 + '_y' + yPoint_236;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_236=0;yPoint_236<this.gameFieldBorderY.length; yPoint_236++){
                for(var xPoint_236=0;xPoint_236<this.gameFieldBorderX.length; xPoint_236++){
                    map.push({y: yPoint_236, x: xPoint_236});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_236=-1;yPoint_236<(this.gameFieldBorderY.length+1); yPoint_236++){
                for(var xPoint_236=-1;xPoint_236<(this.gameFieldBorderX.length+1); xPoint_236++){
                    if(!map[yPoint_236]){
                        map[yPoint_236] = [];
                    }
                    map[yPoint_236][xPoint_236] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_236 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_236 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_236, yPoint_236) === true){
                    if(this.canPutHorizontal(map, xPoint_236, yPoint_236, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_236][xPoint_236 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_236, yPoint_236, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_236 + i][xPoint_236] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_236, yPoint_236){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_236][xPoint_236] === this.CELL_EMPTY
                && map[yPoint_236-1][xPoint_236] === this.CELL_EMPTY
                && map[yPoint_236-1][xPoint_236+1] === this.CELL_EMPTY
                && map[yPoint_236][xPoint_236+1] === this.CELL_EMPTY
                && map[yPoint_236+1][xPoint_236+1] === this.CELL_EMPTY
                && map[yPoint_236+1][xPoint_236] === this.CELL_EMPTY
                && map[yPoint_236+1][xPoint_236-1] === this.CELL_EMPTY
                && map[yPoint_236][xPoint_236-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_236, yPoint_236, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_236;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_236][x] === this.CELL_EMPTY
                    && map[yPoint_236-1][x] === this.CELL_EMPTY
                    && map[yPoint_236-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_236][x+1] === this.CELL_EMPTY
                    && map[yPoint_236+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_236+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_236, yPoint_236, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_236;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_236] === this.CELL_EMPTY
                    && map[y+1][xPoint_236] === this.CELL_EMPTY
                    && map[y+1][xPoint_236+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_236] === this.CELL_EMPTY
                    && map[y][xPoint_236-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_236-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
