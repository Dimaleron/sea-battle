...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26107=0;yPoint_26107<this.gameFieldBorderY.length; yPoint_26107++){
                for(var xPoint_26107=0;xPoint_26107<this.gameFieldBorderX.length; xPoint_26107++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26107, xPoint_26107);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26107][xPoint_26107] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26107, xPoint_26107, 'user');
                    if(this._userShipsMap[yPoint_26107][xPoint_26107] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26107, xPoint_26107, type){
            var id = this.getPointBlockIdByCoords(yPoint_26107, xPoint_26107, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26107);
                block.setAttribute('data-y', yPoint_26107);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26107
         * @param {type} xPoint_26107
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26107, xPoint_26107, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26107 + '_y' + yPoint_26107;
            }
            return 'pc_x' + xPoint_26107 + '_y' + yPoint_26107;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26107=0;yPoint_26107<this.gameFieldBorderY.length; yPoint_26107++){
                for(var xPoint_26107=0;xPoint_26107<this.gameFieldBorderX.length; xPoint_26107++){
                    map.push({y: yPoint_26107, x: xPoint_26107});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26107=-1;yPoint_26107<(this.gameFieldBorderY.length+1); yPoint_26107++){
                for(var xPoint_26107=-1;xPoint_26107<(this.gameFieldBorderX.length+1); xPoint_26107++){
                    if(!map[yPoint_26107]){
                        map[yPoint_26107] = [];
                    }
                    map[yPoint_26107][xPoint_26107] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26107 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26107 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26107, yPoint_26107) === true){
                    if(this.canPutHorizontal(map, xPoint_26107, yPoint_26107, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26107][xPoint_26107 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26107, yPoint_26107, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26107 + i][xPoint_26107] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26107, yPoint_26107){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26107][xPoint_26107] === this.CELL_EMPTY
                && map[yPoint_26107-1][xPoint_26107] === this.CELL_EMPTY
                && map[yPoint_26107-1][xPoint_26107+1] === this.CELL_EMPTY
                && map[yPoint_26107][xPoint_26107+1] === this.CELL_EMPTY
                && map[yPoint_26107+1][xPoint_26107+1] === this.CELL_EMPTY
                && map[yPoint_26107+1][xPoint_26107] === this.CELL_EMPTY
                && map[yPoint_26107+1][xPoint_26107-1] === this.CELL_EMPTY
                && map[yPoint_26107][xPoint_26107-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26107, yPoint_26107, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26107;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26107][x] === this.CELL_EMPTY
                    && map[yPoint_26107-1][x] === this.CELL_EMPTY
                    && map[yPoint_26107-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26107][x+1] === this.CELL_EMPTY
                    && map[yPoint_26107+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26107+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26107, yPoint_26107, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26107;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26107] === this.CELL_EMPTY
                    && map[y+1][xPoint_26107] === this.CELL_EMPTY
                    && map[y+1][xPoint_26107+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26107] === this.CELL_EMPTY
                    && map[y][xPoint_26107-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26107-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
