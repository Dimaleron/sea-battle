...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25125=0;yPoint_25125<this.gameFieldBorderY.length; yPoint_25125++){
                for(var xPoint_25125=0;xPoint_25125<this.gameFieldBorderX.length; xPoint_25125++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25125, xPoint_25125);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25125][xPoint_25125] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25125, xPoint_25125, 'user');
                    if(this._userShipsMap[yPoint_25125][xPoint_25125] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25125, xPoint_25125, type){
            var id = this.getPointBlockIdByCoords(yPoint_25125, xPoint_25125, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25125);
                block.setAttribute('data-y', yPoint_25125);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25125
         * @param {type} xPoint_25125
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25125, xPoint_25125, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25125 + '_y' + yPoint_25125;
            }
            return 'pc_x' + xPoint_25125 + '_y' + yPoint_25125;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25125=0;yPoint_25125<this.gameFieldBorderY.length; yPoint_25125++){
                for(var xPoint_25125=0;xPoint_25125<this.gameFieldBorderX.length; xPoint_25125++){
                    map.push({y: yPoint_25125, x: xPoint_25125});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25125=-1;yPoint_25125<(this.gameFieldBorderY.length+1); yPoint_25125++){
                for(var xPoint_25125=-1;xPoint_25125<(this.gameFieldBorderX.length+1); xPoint_25125++){
                    if(!map[yPoint_25125]){
                        map[yPoint_25125] = [];
                    }
                    map[yPoint_25125][xPoint_25125] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25125 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25125 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25125, yPoint_25125) === true){
                    if(this.canPutHorizontal(map, xPoint_25125, yPoint_25125, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25125][xPoint_25125 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25125, yPoint_25125, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25125 + i][xPoint_25125] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25125, yPoint_25125){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25125][xPoint_25125] === this.CELL_EMPTY
                && map[yPoint_25125-1][xPoint_25125] === this.CELL_EMPTY
                && map[yPoint_25125-1][xPoint_25125+1] === this.CELL_EMPTY
                && map[yPoint_25125][xPoint_25125+1] === this.CELL_EMPTY
                && map[yPoint_25125+1][xPoint_25125+1] === this.CELL_EMPTY
                && map[yPoint_25125+1][xPoint_25125] === this.CELL_EMPTY
                && map[yPoint_25125+1][xPoint_25125-1] === this.CELL_EMPTY
                && map[yPoint_25125][xPoint_25125-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25125, yPoint_25125, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25125;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25125][x] === this.CELL_EMPTY
                    && map[yPoint_25125-1][x] === this.CELL_EMPTY
                    && map[yPoint_25125-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25125][x+1] === this.CELL_EMPTY
                    && map[yPoint_25125+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25125+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25125, yPoint_25125, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25125;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25125] === this.CELL_EMPTY
                    && map[y+1][xPoint_25125] === this.CELL_EMPTY
                    && map[y+1][xPoint_25125+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25125] === this.CELL_EMPTY
                    && map[y][xPoint_25125-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25125-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
