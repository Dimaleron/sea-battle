...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_93=0;yPoint_93<this.gameFieldBorderY.length; yPoint_93++){
                for(var xPoint_93=0;xPoint_93<this.gameFieldBorderX.length; xPoint_93++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_93, xPoint_93);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_93][xPoint_93] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_93, xPoint_93, 'user');
                    if(this._userShipsMap[yPoint_93][xPoint_93] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_93, xPoint_93, type){
            var id = this.getPointBlockIdByCoords(yPoint_93, xPoint_93, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_93);
                block.setAttribute('data-y', yPoint_93);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_93
         * @param {type} xPoint_93
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_93, xPoint_93, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_93 + '_y' + yPoint_93;
            }
            return 'pc_x' + xPoint_93 + '_y' + yPoint_93;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_93=0;yPoint_93<this.gameFieldBorderY.length; yPoint_93++){
                for(var xPoint_93=0;xPoint_93<this.gameFieldBorderX.length; xPoint_93++){
                    map.push({y: yPoint_93, x: xPoint_93});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_93=-1;yPoint_93<(this.gameFieldBorderY.length+1); yPoint_93++){
                for(var xPoint_93=-1;xPoint_93<(this.gameFieldBorderX.length+1); xPoint_93++){
                    if(!map[yPoint_93]){
                        map[yPoint_93] = [];
                    }
                    map[yPoint_93][xPoint_93] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_93 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_93 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_93, yPoint_93) === true){
                    if(this.canPutHorizontal(map, xPoint_93, yPoint_93, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_93][xPoint_93 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_93, yPoint_93, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_93 + i][xPoint_93] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_93, yPoint_93){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_93][xPoint_93] === this.CELL_EMPTY
                && map[yPoint_93-1][xPoint_93] === this.CELL_EMPTY
                && map[yPoint_93-1][xPoint_93+1] === this.CELL_EMPTY
                && map[yPoint_93][xPoint_93+1] === this.CELL_EMPTY
                && map[yPoint_93+1][xPoint_93+1] === this.CELL_EMPTY
                && map[yPoint_93+1][xPoint_93] === this.CELL_EMPTY
                && map[yPoint_93+1][xPoint_93-1] === this.CELL_EMPTY
                && map[yPoint_93][xPoint_93-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_93, yPoint_93, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_93;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_93][x] === this.CELL_EMPTY
                    && map[yPoint_93-1][x] === this.CELL_EMPTY
                    && map[yPoint_93-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_93][x+1] === this.CELL_EMPTY
                    && map[yPoint_93+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_93+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_93, yPoint_93, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_93;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_93] === this.CELL_EMPTY
                    && map[y+1][xPoint_93] === this.CELL_EMPTY
                    && map[y+1][xPoint_93+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_93] === this.CELL_EMPTY
                    && map[y][xPoint_93-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_93-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
