...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7817=0;yPoint_7817<this.gameFieldBorderY.length; yPoint_7817++){
                for(var xPoint_7817=0;xPoint_7817<this.gameFieldBorderX.length; xPoint_7817++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7817, xPoint_7817);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7817][xPoint_7817] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7817, xPoint_7817, 'user');
                    if(this._userShipsMap[yPoint_7817][xPoint_7817] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7817, xPoint_7817, type){
            var id = this.getPointBlockIdByCoords(yPoint_7817, xPoint_7817, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7817);
                block.setAttribute('data-y', yPoint_7817);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7817
         * @param {type} xPoint_7817
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7817, xPoint_7817, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7817 + '_y' + yPoint_7817;
            }
            return 'pc_x' + xPoint_7817 + '_y' + yPoint_7817;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7817=0;yPoint_7817<this.gameFieldBorderY.length; yPoint_7817++){
                for(var xPoint_7817=0;xPoint_7817<this.gameFieldBorderX.length; xPoint_7817++){
                    map.push({y: yPoint_7817, x: xPoint_7817});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7817=-1;yPoint_7817<(this.gameFieldBorderY.length+1); yPoint_7817++){
                for(var xPoint_7817=-1;xPoint_7817<(this.gameFieldBorderX.length+1); xPoint_7817++){
                    if(!map[yPoint_7817]){
                        map[yPoint_7817] = [];
                    }
                    map[yPoint_7817][xPoint_7817] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7817 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7817 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7817, yPoint_7817) === true){
                    if(this.canPutHorizontal(map, xPoint_7817, yPoint_7817, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7817][xPoint_7817 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7817, yPoint_7817, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7817 + i][xPoint_7817] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7817, yPoint_7817){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7817][xPoint_7817] === this.CELL_EMPTY
                && map[yPoint_7817-1][xPoint_7817] === this.CELL_EMPTY
                && map[yPoint_7817-1][xPoint_7817+1] === this.CELL_EMPTY
                && map[yPoint_7817][xPoint_7817+1] === this.CELL_EMPTY
                && map[yPoint_7817+1][xPoint_7817+1] === this.CELL_EMPTY
                && map[yPoint_7817+1][xPoint_7817] === this.CELL_EMPTY
                && map[yPoint_7817+1][xPoint_7817-1] === this.CELL_EMPTY
                && map[yPoint_7817][xPoint_7817-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7817, yPoint_7817, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7817;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7817][x] === this.CELL_EMPTY
                    && map[yPoint_7817-1][x] === this.CELL_EMPTY
                    && map[yPoint_7817-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7817][x+1] === this.CELL_EMPTY
                    && map[yPoint_7817+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7817+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7817, yPoint_7817, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7817;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7817] === this.CELL_EMPTY
                    && map[y+1][xPoint_7817] === this.CELL_EMPTY
                    && map[y+1][xPoint_7817+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7817] === this.CELL_EMPTY
                    && map[y][xPoint_7817-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7817-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
