...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_247=0;yPoint_247<this.gameFieldBorderY.length; yPoint_247++){
                for(var xPoint_247=0;xPoint_247<this.gameFieldBorderX.length; xPoint_247++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_247, xPoint_247);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_247][xPoint_247] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_247, xPoint_247, 'user');
                    if(this._userShipsMap[yPoint_247][xPoint_247] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_247, xPoint_247, type){
            var id = this.getPointBlockIdByCoords(yPoint_247, xPoint_247, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_247);
                block.setAttribute('data-y', yPoint_247);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_247
         * @param {type} xPoint_247
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_247, xPoint_247, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_247 + '_y' + yPoint_247;
            }
            return 'pc_x' + xPoint_247 + '_y' + yPoint_247;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_247=0;yPoint_247<this.gameFieldBorderY.length; yPoint_247++){
                for(var xPoint_247=0;xPoint_247<this.gameFieldBorderX.length; xPoint_247++){
                    map.push({y: yPoint_247, x: xPoint_247});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_247=-1;yPoint_247<(this.gameFieldBorderY.length+1); yPoint_247++){
                for(var xPoint_247=-1;xPoint_247<(this.gameFieldBorderX.length+1); xPoint_247++){
                    if(!map[yPoint_247]){
                        map[yPoint_247] = [];
                    }
                    map[yPoint_247][xPoint_247] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_247 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_247 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_247, yPoint_247) === true){
                    if(this.canPutHorizontal(map, xPoint_247, yPoint_247, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_247][xPoint_247 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_247, yPoint_247, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_247 + i][xPoint_247] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_247, yPoint_247){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_247][xPoint_247] === this.CELL_EMPTY
                && map[yPoint_247-1][xPoint_247] === this.CELL_EMPTY
                && map[yPoint_247-1][xPoint_247+1] === this.CELL_EMPTY
                && map[yPoint_247][xPoint_247+1] === this.CELL_EMPTY
                && map[yPoint_247+1][xPoint_247+1] === this.CELL_EMPTY
                && map[yPoint_247+1][xPoint_247] === this.CELL_EMPTY
                && map[yPoint_247+1][xPoint_247-1] === this.CELL_EMPTY
                && map[yPoint_247][xPoint_247-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_247, yPoint_247, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_247;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_247][x] === this.CELL_EMPTY
                    && map[yPoint_247-1][x] === this.CELL_EMPTY
                    && map[yPoint_247-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_247][x+1] === this.CELL_EMPTY
                    && map[yPoint_247+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_247+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_247, yPoint_247, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_247;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_247] === this.CELL_EMPTY
                    && map[y+1][xPoint_247] === this.CELL_EMPTY
                    && map[y+1][xPoint_247+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_247] === this.CELL_EMPTY
                    && map[y][xPoint_247-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_247-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
