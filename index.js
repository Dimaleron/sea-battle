...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3118=0;yPoint_3118<this.gameFieldBorderY.length; yPoint_3118++){
                for(var xPoint_3118=0;xPoint_3118<this.gameFieldBorderX.length; xPoint_3118++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3118, xPoint_3118);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3118][xPoint_3118] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3118, xPoint_3118, 'user');
                    if(this._userShipsMap[yPoint_3118][xPoint_3118] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3118, xPoint_3118, type){
            var id = this.getPointBlockIdByCoords(yPoint_3118, xPoint_3118, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3118);
                block.setAttribute('data-y', yPoint_3118);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3118
         * @param {type} xPoint_3118
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3118, xPoint_3118, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3118 + '_y' + yPoint_3118;
            }
            return 'pc_x' + xPoint_3118 + '_y' + yPoint_3118;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3118=0;yPoint_3118<this.gameFieldBorderY.length; yPoint_3118++){
                for(var xPoint_3118=0;xPoint_3118<this.gameFieldBorderX.length; xPoint_3118++){
                    map.push({y: yPoint_3118, x: xPoint_3118});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3118=-1;yPoint_3118<(this.gameFieldBorderY.length+1); yPoint_3118++){
                for(var xPoint_3118=-1;xPoint_3118<(this.gameFieldBorderX.length+1); xPoint_3118++){
                    if(!map[yPoint_3118]){
                        map[yPoint_3118] = [];
                    }
                    map[yPoint_3118][xPoint_3118] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3118 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3118 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3118, yPoint_3118) === true){
                    if(this.canPutHorizontal(map, xPoint_3118, yPoint_3118, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3118][xPoint_3118 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3118, yPoint_3118, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3118 + i][xPoint_3118] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3118, yPoint_3118){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3118][xPoint_3118] === this.CELL_EMPTY
                && map[yPoint_3118-1][xPoint_3118] === this.CELL_EMPTY
                && map[yPoint_3118-1][xPoint_3118+1] === this.CELL_EMPTY
                && map[yPoint_3118][xPoint_3118+1] === this.CELL_EMPTY
                && map[yPoint_3118+1][xPoint_3118+1] === this.CELL_EMPTY
                && map[yPoint_3118+1][xPoint_3118] === this.CELL_EMPTY
                && map[yPoint_3118+1][xPoint_3118-1] === this.CELL_EMPTY
                && map[yPoint_3118][xPoint_3118-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3118, yPoint_3118, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3118;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3118][x] === this.CELL_EMPTY
                    && map[yPoint_3118-1][x] === this.CELL_EMPTY
                    && map[yPoint_3118-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3118][x+1] === this.CELL_EMPTY
                    && map[yPoint_3118+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3118+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3118, yPoint_3118, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3118;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3118] === this.CELL_EMPTY
                    && map[y+1][xPoint_3118] === this.CELL_EMPTY
                    && map[y+1][xPoint_3118+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3118] === this.CELL_EMPTY
                    && map[y][xPoint_3118-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3118-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
