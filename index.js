...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7824=0;yPoint_7824<this.gameFieldBorderY.length; yPoint_7824++){
                for(var xPoint_7824=0;xPoint_7824<this.gameFieldBorderX.length; xPoint_7824++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7824, xPoint_7824);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7824][xPoint_7824] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7824, xPoint_7824, 'user');
                    if(this._userShipsMap[yPoint_7824][xPoint_7824] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7824, xPoint_7824, type){
            var id = this.getPointBlockIdByCoords(yPoint_7824, xPoint_7824, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7824);
                block.setAttribute('data-y', yPoint_7824);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7824
         * @param {type} xPoint_7824
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7824, xPoint_7824, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7824 + '_y' + yPoint_7824;
            }
            return 'pc_x' + xPoint_7824 + '_y' + yPoint_7824;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7824=0;yPoint_7824<this.gameFieldBorderY.length; yPoint_7824++){
                for(var xPoint_7824=0;xPoint_7824<this.gameFieldBorderX.length; xPoint_7824++){
                    map.push({y: yPoint_7824, x: xPoint_7824});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7824=-1;yPoint_7824<(this.gameFieldBorderY.length+1); yPoint_7824++){
                for(var xPoint_7824=-1;xPoint_7824<(this.gameFieldBorderX.length+1); xPoint_7824++){
                    if(!map[yPoint_7824]){
                        map[yPoint_7824] = [];
                    }
                    map[yPoint_7824][xPoint_7824] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7824 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7824 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7824, yPoint_7824) === true){
                    if(this.canPutHorizontal(map, xPoint_7824, yPoint_7824, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7824][xPoint_7824 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7824, yPoint_7824, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7824 + i][xPoint_7824] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7824, yPoint_7824){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7824][xPoint_7824] === this.CELL_EMPTY
                && map[yPoint_7824-1][xPoint_7824] === this.CELL_EMPTY
                && map[yPoint_7824-1][xPoint_7824+1] === this.CELL_EMPTY
                && map[yPoint_7824][xPoint_7824+1] === this.CELL_EMPTY
                && map[yPoint_7824+1][xPoint_7824+1] === this.CELL_EMPTY
                && map[yPoint_7824+1][xPoint_7824] === this.CELL_EMPTY
                && map[yPoint_7824+1][xPoint_7824-1] === this.CELL_EMPTY
                && map[yPoint_7824][xPoint_7824-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7824, yPoint_7824, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7824;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7824][x] === this.CELL_EMPTY
                    && map[yPoint_7824-1][x] === this.CELL_EMPTY
                    && map[yPoint_7824-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7824][x+1] === this.CELL_EMPTY
                    && map[yPoint_7824+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7824+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7824, yPoint_7824, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7824;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7824] === this.CELL_EMPTY
                    && map[y+1][xPoint_7824] === this.CELL_EMPTY
                    && map[y+1][xPoint_7824+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7824] === this.CELL_EMPTY
                    && map[y][xPoint_7824-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7824-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
