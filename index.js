...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_12198=0;yPoint_12198<this.gameFieldBorderY.length; yPoint_12198++){
                for(var xPoint_12198=0;xPoint_12198<this.gameFieldBorderX.length; xPoint_12198++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_12198, xPoint_12198);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_12198][xPoint_12198] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_12198, xPoint_12198, 'user');
                    if(this._userShipsMap[yPoint_12198][xPoint_12198] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_12198, xPoint_12198, type){
            var id = this.getPointBlockIdByCoords(yPoint_12198, xPoint_12198, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_12198);
                block.setAttribute('data-y', yPoint_12198);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_12198
         * @param {type} xPoint_12198
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_12198, xPoint_12198, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_12198 + '_y' + yPoint_12198;
            }
            return 'pc_x' + xPoint_12198 + '_y' + yPoint_12198;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_12198=0;yPoint_12198<this.gameFieldBorderY.length; yPoint_12198++){
                for(var xPoint_12198=0;xPoint_12198<this.gameFieldBorderX.length; xPoint_12198++){
                    map.push({y: yPoint_12198, x: xPoint_12198});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_12198=-1;yPoint_12198<(this.gameFieldBorderY.length+1); yPoint_12198++){
                for(var xPoint_12198=-1;xPoint_12198<(this.gameFieldBorderX.length+1); xPoint_12198++){
                    if(!map[yPoint_12198]){
                        map[yPoint_12198] = [];
                    }
                    map[yPoint_12198][xPoint_12198] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_12198 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_12198 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_12198, yPoint_12198) === true){
                    if(this.canPutHorizontal(map, xPoint_12198, yPoint_12198, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12198][xPoint_12198 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_12198, yPoint_12198, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12198 + i][xPoint_12198] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_12198, yPoint_12198){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_12198][xPoint_12198] === this.CELL_EMPTY
                && map[yPoint_12198-1][xPoint_12198] === this.CELL_EMPTY
                && map[yPoint_12198-1][xPoint_12198+1] === this.CELL_EMPTY
                && map[yPoint_12198][xPoint_12198+1] === this.CELL_EMPTY
                && map[yPoint_12198+1][xPoint_12198+1] === this.CELL_EMPTY
                && map[yPoint_12198+1][xPoint_12198] === this.CELL_EMPTY
                && map[yPoint_12198+1][xPoint_12198-1] === this.CELL_EMPTY
                && map[yPoint_12198][xPoint_12198-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_12198, yPoint_12198, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_12198;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_12198][x] === this.CELL_EMPTY
                    && map[yPoint_12198-1][x] === this.CELL_EMPTY
                    && map[yPoint_12198-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12198][x+1] === this.CELL_EMPTY
                    && map[yPoint_12198+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12198+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_12198, yPoint_12198, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_12198;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_12198] === this.CELL_EMPTY
                    && map[y+1][xPoint_12198] === this.CELL_EMPTY
                    && map[y+1][xPoint_12198+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_12198] === this.CELL_EMPTY
                    && map[y][xPoint_12198-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_12198-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
