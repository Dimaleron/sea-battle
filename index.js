...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24153=0;yPoint_24153<this.gameFieldBorderY.length; yPoint_24153++){
                for(var xPoint_24153=0;xPoint_24153<this.gameFieldBorderX.length; xPoint_24153++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24153, xPoint_24153);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24153][xPoint_24153] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24153, xPoint_24153, 'user');
                    if(this._userShipsMap[yPoint_24153][xPoint_24153] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24153, xPoint_24153, type){
            var id = this.getPointBlockIdByCoords(yPoint_24153, xPoint_24153, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24153);
                block.setAttribute('data-y', yPoint_24153);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24153
         * @param {type} xPoint_24153
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24153, xPoint_24153, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24153 + '_y' + yPoint_24153;
            }
            return 'pc_x' + xPoint_24153 + '_y' + yPoint_24153;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24153=0;yPoint_24153<this.gameFieldBorderY.length; yPoint_24153++){
                for(var xPoint_24153=0;xPoint_24153<this.gameFieldBorderX.length; xPoint_24153++){
                    map.push({y: yPoint_24153, x: xPoint_24153});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24153=-1;yPoint_24153<(this.gameFieldBorderY.length+1); yPoint_24153++){
                for(var xPoint_24153=-1;xPoint_24153<(this.gameFieldBorderX.length+1); xPoint_24153++){
                    if(!map[yPoint_24153]){
                        map[yPoint_24153] = [];
                    }
                    map[yPoint_24153][xPoint_24153] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24153 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24153 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24153, yPoint_24153) === true){
                    if(this.canPutHorizontal(map, xPoint_24153, yPoint_24153, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24153][xPoint_24153 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24153, yPoint_24153, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24153 + i][xPoint_24153] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24153, yPoint_24153){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24153][xPoint_24153] === this.CELL_EMPTY
                && map[yPoint_24153-1][xPoint_24153] === this.CELL_EMPTY
                && map[yPoint_24153-1][xPoint_24153+1] === this.CELL_EMPTY
                && map[yPoint_24153][xPoint_24153+1] === this.CELL_EMPTY
                && map[yPoint_24153+1][xPoint_24153+1] === this.CELL_EMPTY
                && map[yPoint_24153+1][xPoint_24153] === this.CELL_EMPTY
                && map[yPoint_24153+1][xPoint_24153-1] === this.CELL_EMPTY
                && map[yPoint_24153][xPoint_24153-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24153, yPoint_24153, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24153;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24153][x] === this.CELL_EMPTY
                    && map[yPoint_24153-1][x] === this.CELL_EMPTY
                    && map[yPoint_24153-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24153][x+1] === this.CELL_EMPTY
                    && map[yPoint_24153+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24153+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24153, yPoint_24153, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24153;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24153] === this.CELL_EMPTY
                    && map[y+1][xPoint_24153] === this.CELL_EMPTY
                    && map[y+1][xPoint_24153+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24153] === this.CELL_EMPTY
                    && map[y][xPoint_24153-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24153-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
