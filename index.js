...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27588=0;yPoint_27588<this.gameFieldBorderY.length; yPoint_27588++){
                for(var xPoint_27588=0;xPoint_27588<this.gameFieldBorderX.length; xPoint_27588++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27588, xPoint_27588);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27588][xPoint_27588] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27588, xPoint_27588, 'user');
                    if(this._userShipsMap[yPoint_27588][xPoint_27588] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27588, xPoint_27588, type){
            var id = this.getPointBlockIdByCoords(yPoint_27588, xPoint_27588, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27588);
                block.setAttribute('data-y', yPoint_27588);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27588
         * @param {type} xPoint_27588
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27588, xPoint_27588, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27588 + '_y' + yPoint_27588;
            }
            return 'pc_x' + xPoint_27588 + '_y' + yPoint_27588;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27588=0;yPoint_27588<this.gameFieldBorderY.length; yPoint_27588++){
                for(var xPoint_27588=0;xPoint_27588<this.gameFieldBorderX.length; xPoint_27588++){
                    map.push({y: yPoint_27588, x: xPoint_27588});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27588=-1;yPoint_27588<(this.gameFieldBorderY.length+1); yPoint_27588++){
                for(var xPoint_27588=-1;xPoint_27588<(this.gameFieldBorderX.length+1); xPoint_27588++){
                    if(!map[yPoint_27588]){
                        map[yPoint_27588] = [];
                    }
                    map[yPoint_27588][xPoint_27588] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27588 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27588 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27588, yPoint_27588) === true){
                    if(this.canPutHorizontal(map, xPoint_27588, yPoint_27588, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27588][xPoint_27588 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27588, yPoint_27588, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27588 + i][xPoint_27588] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27588, yPoint_27588){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27588][xPoint_27588] === this.CELL_EMPTY
                && map[yPoint_27588-1][xPoint_27588] === this.CELL_EMPTY
                && map[yPoint_27588-1][xPoint_27588+1] === this.CELL_EMPTY
                && map[yPoint_27588][xPoint_27588+1] === this.CELL_EMPTY
                && map[yPoint_27588+1][xPoint_27588+1] === this.CELL_EMPTY
                && map[yPoint_27588+1][xPoint_27588] === this.CELL_EMPTY
                && map[yPoint_27588+1][xPoint_27588-1] === this.CELL_EMPTY
                && map[yPoint_27588][xPoint_27588-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27588, yPoint_27588, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27588;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27588][x] === this.CELL_EMPTY
                    && map[yPoint_27588-1][x] === this.CELL_EMPTY
                    && map[yPoint_27588-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27588][x+1] === this.CELL_EMPTY
                    && map[yPoint_27588+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27588+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27588, yPoint_27588, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27588;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27588] === this.CELL_EMPTY
                    && map[y+1][xPoint_27588] === this.CELL_EMPTY
                    && map[y+1][xPoint_27588+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27588] === this.CELL_EMPTY
                    && map[y][xPoint_27588-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27588-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
