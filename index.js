...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_979=0;yPoint_979<this.gameFieldBorderY.length; yPoint_979++){
                for(var xPoint_979=0;xPoint_979<this.gameFieldBorderX.length; xPoint_979++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_979, xPoint_979);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_979][xPoint_979] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_979, xPoint_979, 'user');
                    if(this._userShipsMap[yPoint_979][xPoint_979] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_979, xPoint_979, type){
            var id = this.getPointBlockIdByCoords(yPoint_979, xPoint_979, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_979);
                block.setAttribute('data-y', yPoint_979);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_979
         * @param {type} xPoint_979
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_979, xPoint_979, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_979 + '_y' + yPoint_979;
            }
            return 'pc_x' + xPoint_979 + '_y' + yPoint_979;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_979=0;yPoint_979<this.gameFieldBorderY.length; yPoint_979++){
                for(var xPoint_979=0;xPoint_979<this.gameFieldBorderX.length; xPoint_979++){
                    map.push({y: yPoint_979, x: xPoint_979});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_979=-1;yPoint_979<(this.gameFieldBorderY.length+1); yPoint_979++){
                for(var xPoint_979=-1;xPoint_979<(this.gameFieldBorderX.length+1); xPoint_979++){
                    if(!map[yPoint_979]){
                        map[yPoint_979] = [];
                    }
                    map[yPoint_979][xPoint_979] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_979 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_979 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_979, yPoint_979) === true){
                    if(this.canPutHorizontal(map, xPoint_979, yPoint_979, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_979][xPoint_979 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_979, yPoint_979, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_979 + i][xPoint_979] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_979, yPoint_979){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_979][xPoint_979] === this.CELL_EMPTY
                && map[yPoint_979-1][xPoint_979] === this.CELL_EMPTY
                && map[yPoint_979-1][xPoint_979+1] === this.CELL_EMPTY
                && map[yPoint_979][xPoint_979+1] === this.CELL_EMPTY
                && map[yPoint_979+1][xPoint_979+1] === this.CELL_EMPTY
                && map[yPoint_979+1][xPoint_979] === this.CELL_EMPTY
                && map[yPoint_979+1][xPoint_979-1] === this.CELL_EMPTY
                && map[yPoint_979][xPoint_979-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_979, yPoint_979, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_979;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_979][x] === this.CELL_EMPTY
                    && map[yPoint_979-1][x] === this.CELL_EMPTY
                    && map[yPoint_979-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_979][x+1] === this.CELL_EMPTY
                    && map[yPoint_979+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_979+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_979, yPoint_979, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_979;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_979] === this.CELL_EMPTY
                    && map[y+1][xPoint_979] === this.CELL_EMPTY
                    && map[y+1][xPoint_979+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_979] === this.CELL_EMPTY
                    && map[y][xPoint_979-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_979-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
