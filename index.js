...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21541=0;yPoint_21541<this.gameFieldBorderY.length; yPoint_21541++){
                for(var xPoint_21541=0;xPoint_21541<this.gameFieldBorderX.length; xPoint_21541++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21541, xPoint_21541);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21541][xPoint_21541] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21541, xPoint_21541, 'user');
                    if(this._userShipsMap[yPoint_21541][xPoint_21541] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21541, xPoint_21541, type){
            var id = this.getPointBlockIdByCoords(yPoint_21541, xPoint_21541, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21541);
                block.setAttribute('data-y', yPoint_21541);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21541
         * @param {type} xPoint_21541
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21541, xPoint_21541, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21541 + '_y' + yPoint_21541;
            }
            return 'pc_x' + xPoint_21541 + '_y' + yPoint_21541;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21541=0;yPoint_21541<this.gameFieldBorderY.length; yPoint_21541++){
                for(var xPoint_21541=0;xPoint_21541<this.gameFieldBorderX.length; xPoint_21541++){
                    map.push({y: yPoint_21541, x: xPoint_21541});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21541=-1;yPoint_21541<(this.gameFieldBorderY.length+1); yPoint_21541++){
                for(var xPoint_21541=-1;xPoint_21541<(this.gameFieldBorderX.length+1); xPoint_21541++){
                    if(!map[yPoint_21541]){
                        map[yPoint_21541] = [];
                    }
                    map[yPoint_21541][xPoint_21541] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21541 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21541 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21541, yPoint_21541) === true){
                    if(this.canPutHorizontal(map, xPoint_21541, yPoint_21541, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21541][xPoint_21541 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21541, yPoint_21541, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21541 + i][xPoint_21541] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21541, yPoint_21541){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21541][xPoint_21541] === this.CELL_EMPTY
                && map[yPoint_21541-1][xPoint_21541] === this.CELL_EMPTY
                && map[yPoint_21541-1][xPoint_21541+1] === this.CELL_EMPTY
                && map[yPoint_21541][xPoint_21541+1] === this.CELL_EMPTY
                && map[yPoint_21541+1][xPoint_21541+1] === this.CELL_EMPTY
                && map[yPoint_21541+1][xPoint_21541] === this.CELL_EMPTY
                && map[yPoint_21541+1][xPoint_21541-1] === this.CELL_EMPTY
                && map[yPoint_21541][xPoint_21541-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21541, yPoint_21541, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21541;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21541][x] === this.CELL_EMPTY
                    && map[yPoint_21541-1][x] === this.CELL_EMPTY
                    && map[yPoint_21541-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21541][x+1] === this.CELL_EMPTY
                    && map[yPoint_21541+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21541+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21541, yPoint_21541, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21541;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21541] === this.CELL_EMPTY
                    && map[y+1][xPoint_21541] === this.CELL_EMPTY
                    && map[y+1][xPoint_21541+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21541] === this.CELL_EMPTY
                    && map[y][xPoint_21541-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21541-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
