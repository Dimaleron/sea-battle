...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13399=0;yPoint_13399<this.gameFieldBorderY.length; yPoint_13399++){
                for(var xPoint_13399=0;xPoint_13399<this.gameFieldBorderX.length; xPoint_13399++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13399, xPoint_13399);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13399][xPoint_13399] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13399, xPoint_13399, 'user');
                    if(this._userShipsMap[yPoint_13399][xPoint_13399] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13399, xPoint_13399, type){
            var id = this.getPointBlockIdByCoords(yPoint_13399, xPoint_13399, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13399);
                block.setAttribute('data-y', yPoint_13399);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13399
         * @param {type} xPoint_13399
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13399, xPoint_13399, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13399 + '_y' + yPoint_13399;
            }
            return 'pc_x' + xPoint_13399 + '_y' + yPoint_13399;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13399=0;yPoint_13399<this.gameFieldBorderY.length; yPoint_13399++){
                for(var xPoint_13399=0;xPoint_13399<this.gameFieldBorderX.length; xPoint_13399++){
                    map.push({y: yPoint_13399, x: xPoint_13399});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13399=-1;yPoint_13399<(this.gameFieldBorderY.length+1); yPoint_13399++){
                for(var xPoint_13399=-1;xPoint_13399<(this.gameFieldBorderX.length+1); xPoint_13399++){
                    if(!map[yPoint_13399]){
                        map[yPoint_13399] = [];
                    }
                    map[yPoint_13399][xPoint_13399] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13399 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13399 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13399, yPoint_13399) === true){
                    if(this.canPutHorizontal(map, xPoint_13399, yPoint_13399, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13399][xPoint_13399 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13399, yPoint_13399, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13399 + i][xPoint_13399] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13399, yPoint_13399){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13399][xPoint_13399] === this.CELL_EMPTY
                && map[yPoint_13399-1][xPoint_13399] === this.CELL_EMPTY
                && map[yPoint_13399-1][xPoint_13399+1] === this.CELL_EMPTY
                && map[yPoint_13399][xPoint_13399+1] === this.CELL_EMPTY
                && map[yPoint_13399+1][xPoint_13399+1] === this.CELL_EMPTY
                && map[yPoint_13399+1][xPoint_13399] === this.CELL_EMPTY
                && map[yPoint_13399+1][xPoint_13399-1] === this.CELL_EMPTY
                && map[yPoint_13399][xPoint_13399-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13399, yPoint_13399, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13399;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13399][x] === this.CELL_EMPTY
                    && map[yPoint_13399-1][x] === this.CELL_EMPTY
                    && map[yPoint_13399-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13399][x+1] === this.CELL_EMPTY
                    && map[yPoint_13399+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13399+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13399, yPoint_13399, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13399;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13399] === this.CELL_EMPTY
                    && map[y+1][xPoint_13399] === this.CELL_EMPTY
                    && map[y+1][xPoint_13399+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13399] === this.CELL_EMPTY
                    && map[y][xPoint_13399-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13399-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
