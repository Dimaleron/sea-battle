...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_740=0;yPoint_740<this.gameFieldBorderY.length; yPoint_740++){
                for(var xPoint_740=0;xPoint_740<this.gameFieldBorderX.length; xPoint_740++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_740, xPoint_740);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_740][xPoint_740] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_740, xPoint_740, 'user');
                    if(this._userShipsMap[yPoint_740][xPoint_740] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_740, xPoint_740, type){
            var id = this.getPointBlockIdByCoords(yPoint_740, xPoint_740, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_740);
                block.setAttribute('data-y', yPoint_740);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_740
         * @param {type} xPoint_740
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_740, xPoint_740, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_740 + '_y' + yPoint_740;
            }
            return 'pc_x' + xPoint_740 + '_y' + yPoint_740;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_740=0;yPoint_740<this.gameFieldBorderY.length; yPoint_740++){
                for(var xPoint_740=0;xPoint_740<this.gameFieldBorderX.length; xPoint_740++){
                    map.push({y: yPoint_740, x: xPoint_740});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_740=-1;yPoint_740<(this.gameFieldBorderY.length+1); yPoint_740++){
                for(var xPoint_740=-1;xPoint_740<(this.gameFieldBorderX.length+1); xPoint_740++){
                    if(!map[yPoint_740]){
                        map[yPoint_740] = [];
                    }
                    map[yPoint_740][xPoint_740] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_740 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_740 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_740, yPoint_740) === true){
                    if(this.canPutHorizontal(map, xPoint_740, yPoint_740, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_740][xPoint_740 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_740, yPoint_740, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_740 + i][xPoint_740] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_740, yPoint_740){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_740][xPoint_740] === this.CELL_EMPTY
                && map[yPoint_740-1][xPoint_740] === this.CELL_EMPTY
                && map[yPoint_740-1][xPoint_740+1] === this.CELL_EMPTY
                && map[yPoint_740][xPoint_740+1] === this.CELL_EMPTY
                && map[yPoint_740+1][xPoint_740+1] === this.CELL_EMPTY
                && map[yPoint_740+1][xPoint_740] === this.CELL_EMPTY
                && map[yPoint_740+1][xPoint_740-1] === this.CELL_EMPTY
                && map[yPoint_740][xPoint_740-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_740, yPoint_740, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_740;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_740][x] === this.CELL_EMPTY
                    && map[yPoint_740-1][x] === this.CELL_EMPTY
                    && map[yPoint_740-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_740][x+1] === this.CELL_EMPTY
                    && map[yPoint_740+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_740+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_740, yPoint_740, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_740;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_740] === this.CELL_EMPTY
                    && map[y+1][xPoint_740] === this.CELL_EMPTY
                    && map[y+1][xPoint_740+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_740] === this.CELL_EMPTY
                    && map[y][xPoint_740-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_740-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
