...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28281=0;yPoint_28281<this.gameFieldBorderY.length; yPoint_28281++){
                for(var xPoint_28281=0;xPoint_28281<this.gameFieldBorderX.length; xPoint_28281++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28281, xPoint_28281);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28281][xPoint_28281] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28281, xPoint_28281, 'user');
                    if(this._userShipsMap[yPoint_28281][xPoint_28281] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28281, xPoint_28281, type){
            var id = this.getPointBlockIdByCoords(yPoint_28281, xPoint_28281, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28281);
                block.setAttribute('data-y', yPoint_28281);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28281
         * @param {type} xPoint_28281
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28281, xPoint_28281, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28281 + '_y' + yPoint_28281;
            }
            return 'pc_x' + xPoint_28281 + '_y' + yPoint_28281;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28281=0;yPoint_28281<this.gameFieldBorderY.length; yPoint_28281++){
                for(var xPoint_28281=0;xPoint_28281<this.gameFieldBorderX.length; xPoint_28281++){
                    map.push({y: yPoint_28281, x: xPoint_28281});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28281=-1;yPoint_28281<(this.gameFieldBorderY.length+1); yPoint_28281++){
                for(var xPoint_28281=-1;xPoint_28281<(this.gameFieldBorderX.length+1); xPoint_28281++){
                    if(!map[yPoint_28281]){
                        map[yPoint_28281] = [];
                    }
                    map[yPoint_28281][xPoint_28281] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28281 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28281 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28281, yPoint_28281) === true){
                    if(this.canPutHorizontal(map, xPoint_28281, yPoint_28281, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28281][xPoint_28281 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28281, yPoint_28281, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28281 + i][xPoint_28281] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28281, yPoint_28281){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28281][xPoint_28281] === this.CELL_EMPTY
                && map[yPoint_28281-1][xPoint_28281] === this.CELL_EMPTY
                && map[yPoint_28281-1][xPoint_28281+1] === this.CELL_EMPTY
                && map[yPoint_28281][xPoint_28281+1] === this.CELL_EMPTY
                && map[yPoint_28281+1][xPoint_28281+1] === this.CELL_EMPTY
                && map[yPoint_28281+1][xPoint_28281] === this.CELL_EMPTY
                && map[yPoint_28281+1][xPoint_28281-1] === this.CELL_EMPTY
                && map[yPoint_28281][xPoint_28281-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28281, yPoint_28281, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28281;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28281][x] === this.CELL_EMPTY
                    && map[yPoint_28281-1][x] === this.CELL_EMPTY
                    && map[yPoint_28281-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28281][x+1] === this.CELL_EMPTY
                    && map[yPoint_28281+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28281+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28281, yPoint_28281, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28281;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28281] === this.CELL_EMPTY
                    && map[y+1][xPoint_28281] === this.CELL_EMPTY
                    && map[y+1][xPoint_28281+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28281] === this.CELL_EMPTY
                    && map[y][xPoint_28281-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28281-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
