...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4646=0;yPoint_4646<this.gameFieldBorderY.length; yPoint_4646++){
                for(var xPoint_4646=0;xPoint_4646<this.gameFieldBorderX.length; xPoint_4646++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4646, xPoint_4646);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4646][xPoint_4646] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4646, xPoint_4646, 'user');
                    if(this._userShipsMap[yPoint_4646][xPoint_4646] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4646, xPoint_4646, type){
            var id = this.getPointBlockIdByCoords(yPoint_4646, xPoint_4646, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4646);
                block.setAttribute('data-y', yPoint_4646);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4646
         * @param {type} xPoint_4646
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4646, xPoint_4646, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4646 + '_y' + yPoint_4646;
            }
            return 'pc_x' + xPoint_4646 + '_y' + yPoint_4646;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4646=0;yPoint_4646<this.gameFieldBorderY.length; yPoint_4646++){
                for(var xPoint_4646=0;xPoint_4646<this.gameFieldBorderX.length; xPoint_4646++){
                    map.push({y: yPoint_4646, x: xPoint_4646});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4646=-1;yPoint_4646<(this.gameFieldBorderY.length+1); yPoint_4646++){
                for(var xPoint_4646=-1;xPoint_4646<(this.gameFieldBorderX.length+1); xPoint_4646++){
                    if(!map[yPoint_4646]){
                        map[yPoint_4646] = [];
                    }
                    map[yPoint_4646][xPoint_4646] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4646 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4646 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4646, yPoint_4646) === true){
                    if(this.canPutHorizontal(map, xPoint_4646, yPoint_4646, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4646][xPoint_4646 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4646, yPoint_4646, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4646 + i][xPoint_4646] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4646, yPoint_4646){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4646][xPoint_4646] === this.CELL_EMPTY
                && map[yPoint_4646-1][xPoint_4646] === this.CELL_EMPTY
                && map[yPoint_4646-1][xPoint_4646+1] === this.CELL_EMPTY
                && map[yPoint_4646][xPoint_4646+1] === this.CELL_EMPTY
                && map[yPoint_4646+1][xPoint_4646+1] === this.CELL_EMPTY
                && map[yPoint_4646+1][xPoint_4646] === this.CELL_EMPTY
                && map[yPoint_4646+1][xPoint_4646-1] === this.CELL_EMPTY
                && map[yPoint_4646][xPoint_4646-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4646, yPoint_4646, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4646;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4646][x] === this.CELL_EMPTY
                    && map[yPoint_4646-1][x] === this.CELL_EMPTY
                    && map[yPoint_4646-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4646][x+1] === this.CELL_EMPTY
                    && map[yPoint_4646+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4646+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4646, yPoint_4646, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4646;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4646] === this.CELL_EMPTY
                    && map[y+1][xPoint_4646] === this.CELL_EMPTY
                    && map[y+1][xPoint_4646+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4646] === this.CELL_EMPTY
                    && map[y][xPoint_4646-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4646-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
