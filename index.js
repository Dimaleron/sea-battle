...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_442=0;yPoint_442<this.gameFieldBorderY.length; yPoint_442++){
                for(var xPoint_442=0;xPoint_442<this.gameFieldBorderX.length; xPoint_442++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_442, xPoint_442);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_442][xPoint_442] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_442, xPoint_442, 'user');
                    if(this._userShipsMap[yPoint_442][xPoint_442] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_442, xPoint_442, type){
            var id = this.getPointBlockIdByCoords(yPoint_442, xPoint_442, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_442);
                block.setAttribute('data-y', yPoint_442);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_442
         * @param {type} xPoint_442
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_442, xPoint_442, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_442 + '_y' + yPoint_442;
            }
            return 'pc_x' + xPoint_442 + '_y' + yPoint_442;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_442=0;yPoint_442<this.gameFieldBorderY.length; yPoint_442++){
                for(var xPoint_442=0;xPoint_442<this.gameFieldBorderX.length; xPoint_442++){
                    map.push({y: yPoint_442, x: xPoint_442});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_442=-1;yPoint_442<(this.gameFieldBorderY.length+1); yPoint_442++){
                for(var xPoint_442=-1;xPoint_442<(this.gameFieldBorderX.length+1); xPoint_442++){
                    if(!map[yPoint_442]){
                        map[yPoint_442] = [];
                    }
                    map[yPoint_442][xPoint_442] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_442 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_442 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_442, yPoint_442) === true){
                    if(this.canPutHorizontal(map, xPoint_442, yPoint_442, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_442][xPoint_442 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_442, yPoint_442, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_442 + i][xPoint_442] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_442, yPoint_442){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_442][xPoint_442] === this.CELL_EMPTY
                && map[yPoint_442-1][xPoint_442] === this.CELL_EMPTY
                && map[yPoint_442-1][xPoint_442+1] === this.CELL_EMPTY
                && map[yPoint_442][xPoint_442+1] === this.CELL_EMPTY
                && map[yPoint_442+1][xPoint_442+1] === this.CELL_EMPTY
                && map[yPoint_442+1][xPoint_442] === this.CELL_EMPTY
                && map[yPoint_442+1][xPoint_442-1] === this.CELL_EMPTY
                && map[yPoint_442][xPoint_442-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_442, yPoint_442, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_442;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_442][x] === this.CELL_EMPTY
                    && map[yPoint_442-1][x] === this.CELL_EMPTY
                    && map[yPoint_442-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_442][x+1] === this.CELL_EMPTY
                    && map[yPoint_442+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_442+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_442, yPoint_442, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_442;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_442] === this.CELL_EMPTY
                    && map[y+1][xPoint_442] === this.CELL_EMPTY
                    && map[y+1][xPoint_442+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_442] === this.CELL_EMPTY
                    && map[y][xPoint_442-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_442-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
