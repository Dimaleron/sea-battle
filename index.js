...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_371=0;yPoint_371<this.gameFieldBorderY.length; yPoint_371++){
                for(var xPoint_371=0;xPoint_371<this.gameFieldBorderX.length; xPoint_371++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_371, xPoint_371);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_371][xPoint_371] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_371, xPoint_371, 'user');
                    if(this._userShipsMap[yPoint_371][xPoint_371] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_371, xPoint_371, type){
            var id = this.getPointBlockIdByCoords(yPoint_371, xPoint_371, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_371);
                block.setAttribute('data-y', yPoint_371);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_371
         * @param {type} xPoint_371
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_371, xPoint_371, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_371 + '_y' + yPoint_371;
            }
            return 'pc_x' + xPoint_371 + '_y' + yPoint_371;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_371=0;yPoint_371<this.gameFieldBorderY.length; yPoint_371++){
                for(var xPoint_371=0;xPoint_371<this.gameFieldBorderX.length; xPoint_371++){
                    map.push({y: yPoint_371, x: xPoint_371});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_371=-1;yPoint_371<(this.gameFieldBorderY.length+1); yPoint_371++){
                for(var xPoint_371=-1;xPoint_371<(this.gameFieldBorderX.length+1); xPoint_371++){
                    if(!map[yPoint_371]){
                        map[yPoint_371] = [];
                    }
                    map[yPoint_371][xPoint_371] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_371 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_371 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_371, yPoint_371) === true){
                    if(this.canPutHorizontal(map, xPoint_371, yPoint_371, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_371][xPoint_371 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_371, yPoint_371, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_371 + i][xPoint_371] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_371, yPoint_371){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_371][xPoint_371] === this.CELL_EMPTY
                && map[yPoint_371-1][xPoint_371] === this.CELL_EMPTY
                && map[yPoint_371-1][xPoint_371+1] === this.CELL_EMPTY
                && map[yPoint_371][xPoint_371+1] === this.CELL_EMPTY
                && map[yPoint_371+1][xPoint_371+1] === this.CELL_EMPTY
                && map[yPoint_371+1][xPoint_371] === this.CELL_EMPTY
                && map[yPoint_371+1][xPoint_371-1] === this.CELL_EMPTY
                && map[yPoint_371][xPoint_371-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_371, yPoint_371, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_371;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_371][x] === this.CELL_EMPTY
                    && map[yPoint_371-1][x] === this.CELL_EMPTY
                    && map[yPoint_371-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_371][x+1] === this.CELL_EMPTY
                    && map[yPoint_371+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_371+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_371, yPoint_371, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_371;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_371] === this.CELL_EMPTY
                    && map[y+1][xPoint_371] === this.CELL_EMPTY
                    && map[y+1][xPoint_371+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_371] === this.CELL_EMPTY
                    && map[y][xPoint_371-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_371-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
