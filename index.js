...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11189=0;yPoint_11189<this.gameFieldBorderY.length; yPoint_11189++){
                for(var xPoint_11189=0;xPoint_11189<this.gameFieldBorderX.length; xPoint_11189++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11189, xPoint_11189);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11189][xPoint_11189] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11189, xPoint_11189, 'user');
                    if(this._userShipsMap[yPoint_11189][xPoint_11189] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11189, xPoint_11189, type){
            var id = this.getPointBlockIdByCoords(yPoint_11189, xPoint_11189, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11189);
                block.setAttribute('data-y', yPoint_11189);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11189
         * @param {type} xPoint_11189
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11189, xPoint_11189, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11189 + '_y' + yPoint_11189;
            }
            return 'pc_x' + xPoint_11189 + '_y' + yPoint_11189;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11189=0;yPoint_11189<this.gameFieldBorderY.length; yPoint_11189++){
                for(var xPoint_11189=0;xPoint_11189<this.gameFieldBorderX.length; xPoint_11189++){
                    map.push({y: yPoint_11189, x: xPoint_11189});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11189=-1;yPoint_11189<(this.gameFieldBorderY.length+1); yPoint_11189++){
                for(var xPoint_11189=-1;xPoint_11189<(this.gameFieldBorderX.length+1); xPoint_11189++){
                    if(!map[yPoint_11189]){
                        map[yPoint_11189] = [];
                    }
                    map[yPoint_11189][xPoint_11189] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11189 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11189 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11189, yPoint_11189) === true){
                    if(this.canPutHorizontal(map, xPoint_11189, yPoint_11189, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11189][xPoint_11189 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11189, yPoint_11189, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11189 + i][xPoint_11189] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11189, yPoint_11189){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11189][xPoint_11189] === this.CELL_EMPTY
                && map[yPoint_11189-1][xPoint_11189] === this.CELL_EMPTY
                && map[yPoint_11189-1][xPoint_11189+1] === this.CELL_EMPTY
                && map[yPoint_11189][xPoint_11189+1] === this.CELL_EMPTY
                && map[yPoint_11189+1][xPoint_11189+1] === this.CELL_EMPTY
                && map[yPoint_11189+1][xPoint_11189] === this.CELL_EMPTY
                && map[yPoint_11189+1][xPoint_11189-1] === this.CELL_EMPTY
                && map[yPoint_11189][xPoint_11189-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11189, yPoint_11189, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11189;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11189][x] === this.CELL_EMPTY
                    && map[yPoint_11189-1][x] === this.CELL_EMPTY
                    && map[yPoint_11189-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11189][x+1] === this.CELL_EMPTY
                    && map[yPoint_11189+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11189+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11189, yPoint_11189, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11189;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11189] === this.CELL_EMPTY
                    && map[y+1][xPoint_11189] === this.CELL_EMPTY
                    && map[y+1][xPoint_11189+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11189] === this.CELL_EMPTY
                    && map[y][xPoint_11189-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11189-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
