...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6003=0;yPoint_6003<this.gameFieldBorderY.length; yPoint_6003++){
                for(var xPoint_6003=0;xPoint_6003<this.gameFieldBorderX.length; xPoint_6003++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6003, xPoint_6003);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6003][xPoint_6003] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6003, xPoint_6003, 'user');
                    if(this._userShipsMap[yPoint_6003][xPoint_6003] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6003, xPoint_6003, type){
            var id = this.getPointBlockIdByCoords(yPoint_6003, xPoint_6003, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6003);
                block.setAttribute('data-y', yPoint_6003);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6003
         * @param {type} xPoint_6003
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6003, xPoint_6003, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6003 + '_y' + yPoint_6003;
            }
            return 'pc_x' + xPoint_6003 + '_y' + yPoint_6003;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6003=0;yPoint_6003<this.gameFieldBorderY.length; yPoint_6003++){
                for(var xPoint_6003=0;xPoint_6003<this.gameFieldBorderX.length; xPoint_6003++){
                    map.push({y: yPoint_6003, x: xPoint_6003});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6003=-1;yPoint_6003<(this.gameFieldBorderY.length+1); yPoint_6003++){
                for(var xPoint_6003=-1;xPoint_6003<(this.gameFieldBorderX.length+1); xPoint_6003++){
                    if(!map[yPoint_6003]){
                        map[yPoint_6003] = [];
                    }
                    map[yPoint_6003][xPoint_6003] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6003 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6003 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6003, yPoint_6003) === true){
                    if(this.canPutHorizontal(map, xPoint_6003, yPoint_6003, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6003][xPoint_6003 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6003, yPoint_6003, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6003 + i][xPoint_6003] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6003, yPoint_6003){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6003][xPoint_6003] === this.CELL_EMPTY
                && map[yPoint_6003-1][xPoint_6003] === this.CELL_EMPTY
                && map[yPoint_6003-1][xPoint_6003+1] === this.CELL_EMPTY
                && map[yPoint_6003][xPoint_6003+1] === this.CELL_EMPTY
                && map[yPoint_6003+1][xPoint_6003+1] === this.CELL_EMPTY
                && map[yPoint_6003+1][xPoint_6003] === this.CELL_EMPTY
                && map[yPoint_6003+1][xPoint_6003-1] === this.CELL_EMPTY
                && map[yPoint_6003][xPoint_6003-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6003, yPoint_6003, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6003;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6003][x] === this.CELL_EMPTY
                    && map[yPoint_6003-1][x] === this.CELL_EMPTY
                    && map[yPoint_6003-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6003][x+1] === this.CELL_EMPTY
                    && map[yPoint_6003+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6003+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6003, yPoint_6003, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6003;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6003] === this.CELL_EMPTY
                    && map[y+1][xPoint_6003] === this.CELL_EMPTY
                    && map[y+1][xPoint_6003+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6003] === this.CELL_EMPTY
                    && map[y][xPoint_6003-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6003-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
