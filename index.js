...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7315=0;yPoint_7315<this.gameFieldBorderY.length; yPoint_7315++){
                for(var xPoint_7315=0;xPoint_7315<this.gameFieldBorderX.length; xPoint_7315++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7315, xPoint_7315);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7315][xPoint_7315] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7315, xPoint_7315, 'user');
                    if(this._userShipsMap[yPoint_7315][xPoint_7315] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7315, xPoint_7315, type){
            var id = this.getPointBlockIdByCoords(yPoint_7315, xPoint_7315, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7315);
                block.setAttribute('data-y', yPoint_7315);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7315
         * @param {type} xPoint_7315
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7315, xPoint_7315, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7315 + '_y' + yPoint_7315;
            }
            return 'pc_x' + xPoint_7315 + '_y' + yPoint_7315;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7315=0;yPoint_7315<this.gameFieldBorderY.length; yPoint_7315++){
                for(var xPoint_7315=0;xPoint_7315<this.gameFieldBorderX.length; xPoint_7315++){
                    map.push({y: yPoint_7315, x: xPoint_7315});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7315=-1;yPoint_7315<(this.gameFieldBorderY.length+1); yPoint_7315++){
                for(var xPoint_7315=-1;xPoint_7315<(this.gameFieldBorderX.length+1); xPoint_7315++){
                    if(!map[yPoint_7315]){
                        map[yPoint_7315] = [];
                    }
                    map[yPoint_7315][xPoint_7315] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7315 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7315 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7315, yPoint_7315) === true){
                    if(this.canPutHorizontal(map, xPoint_7315, yPoint_7315, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7315][xPoint_7315 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7315, yPoint_7315, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7315 + i][xPoint_7315] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7315, yPoint_7315){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7315][xPoint_7315] === this.CELL_EMPTY
                && map[yPoint_7315-1][xPoint_7315] === this.CELL_EMPTY
                && map[yPoint_7315-1][xPoint_7315+1] === this.CELL_EMPTY
                && map[yPoint_7315][xPoint_7315+1] === this.CELL_EMPTY
                && map[yPoint_7315+1][xPoint_7315+1] === this.CELL_EMPTY
                && map[yPoint_7315+1][xPoint_7315] === this.CELL_EMPTY
                && map[yPoint_7315+1][xPoint_7315-1] === this.CELL_EMPTY
                && map[yPoint_7315][xPoint_7315-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7315, yPoint_7315, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7315;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7315][x] === this.CELL_EMPTY
                    && map[yPoint_7315-1][x] === this.CELL_EMPTY
                    && map[yPoint_7315-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7315][x+1] === this.CELL_EMPTY
                    && map[yPoint_7315+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7315+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7315, yPoint_7315, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7315;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7315] === this.CELL_EMPTY
                    && map[y+1][xPoint_7315] === this.CELL_EMPTY
                    && map[y+1][xPoint_7315+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7315] === this.CELL_EMPTY
                    && map[y][xPoint_7315-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7315-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
