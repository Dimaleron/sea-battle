...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19731=0;yPoint_19731<this.gameFieldBorderY.length; yPoint_19731++){
                for(var xPoint_19731=0;xPoint_19731<this.gameFieldBorderX.length; xPoint_19731++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19731, xPoint_19731);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19731][xPoint_19731] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19731, xPoint_19731, 'user');
                    if(this._userShipsMap[yPoint_19731][xPoint_19731] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19731, xPoint_19731, type){
            var id = this.getPointBlockIdByCoords(yPoint_19731, xPoint_19731, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19731);
                block.setAttribute('data-y', yPoint_19731);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19731
         * @param {type} xPoint_19731
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19731, xPoint_19731, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19731 + '_y' + yPoint_19731;
            }
            return 'pc_x' + xPoint_19731 + '_y' + yPoint_19731;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19731=0;yPoint_19731<this.gameFieldBorderY.length; yPoint_19731++){
                for(var xPoint_19731=0;xPoint_19731<this.gameFieldBorderX.length; xPoint_19731++){
                    map.push({y: yPoint_19731, x: xPoint_19731});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19731=-1;yPoint_19731<(this.gameFieldBorderY.length+1); yPoint_19731++){
                for(var xPoint_19731=-1;xPoint_19731<(this.gameFieldBorderX.length+1); xPoint_19731++){
                    if(!map[yPoint_19731]){
                        map[yPoint_19731] = [];
                    }
                    map[yPoint_19731][xPoint_19731] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19731 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19731 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19731, yPoint_19731) === true){
                    if(this.canPutHorizontal(map, xPoint_19731, yPoint_19731, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19731][xPoint_19731 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19731, yPoint_19731, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19731 + i][xPoint_19731] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19731, yPoint_19731){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19731][xPoint_19731] === this.CELL_EMPTY
                && map[yPoint_19731-1][xPoint_19731] === this.CELL_EMPTY
                && map[yPoint_19731-1][xPoint_19731+1] === this.CELL_EMPTY
                && map[yPoint_19731][xPoint_19731+1] === this.CELL_EMPTY
                && map[yPoint_19731+1][xPoint_19731+1] === this.CELL_EMPTY
                && map[yPoint_19731+1][xPoint_19731] === this.CELL_EMPTY
                && map[yPoint_19731+1][xPoint_19731-1] === this.CELL_EMPTY
                && map[yPoint_19731][xPoint_19731-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19731, yPoint_19731, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19731;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19731][x] === this.CELL_EMPTY
                    && map[yPoint_19731-1][x] === this.CELL_EMPTY
                    && map[yPoint_19731-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19731][x+1] === this.CELL_EMPTY
                    && map[yPoint_19731+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19731+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19731, yPoint_19731, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19731;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19731] === this.CELL_EMPTY
                    && map[y+1][xPoint_19731] === this.CELL_EMPTY
                    && map[y+1][xPoint_19731+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19731] === this.CELL_EMPTY
                    && map[y][xPoint_19731-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19731-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
