...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26202=0;yPoint_26202<this.gameFieldBorderY.length; yPoint_26202++){
                for(var xPoint_26202=0;xPoint_26202<this.gameFieldBorderX.length; xPoint_26202++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26202, xPoint_26202);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26202][xPoint_26202] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26202, xPoint_26202, 'user');
                    if(this._userShipsMap[yPoint_26202][xPoint_26202] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26202, xPoint_26202, type){
            var id = this.getPointBlockIdByCoords(yPoint_26202, xPoint_26202, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26202);
                block.setAttribute('data-y', yPoint_26202);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26202
         * @param {type} xPoint_26202
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26202, xPoint_26202, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26202 + '_y' + yPoint_26202;
            }
            return 'pc_x' + xPoint_26202 + '_y' + yPoint_26202;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26202=0;yPoint_26202<this.gameFieldBorderY.length; yPoint_26202++){
                for(var xPoint_26202=0;xPoint_26202<this.gameFieldBorderX.length; xPoint_26202++){
                    map.push({y: yPoint_26202, x: xPoint_26202});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26202=-1;yPoint_26202<(this.gameFieldBorderY.length+1); yPoint_26202++){
                for(var xPoint_26202=-1;xPoint_26202<(this.gameFieldBorderX.length+1); xPoint_26202++){
                    if(!map[yPoint_26202]){
                        map[yPoint_26202] = [];
                    }
                    map[yPoint_26202][xPoint_26202] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26202 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26202 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26202, yPoint_26202) === true){
                    if(this.canPutHorizontal(map, xPoint_26202, yPoint_26202, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26202][xPoint_26202 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26202, yPoint_26202, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26202 + i][xPoint_26202] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26202, yPoint_26202){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26202][xPoint_26202] === this.CELL_EMPTY
                && map[yPoint_26202-1][xPoint_26202] === this.CELL_EMPTY
                && map[yPoint_26202-1][xPoint_26202+1] === this.CELL_EMPTY
                && map[yPoint_26202][xPoint_26202+1] === this.CELL_EMPTY
                && map[yPoint_26202+1][xPoint_26202+1] === this.CELL_EMPTY
                && map[yPoint_26202+1][xPoint_26202] === this.CELL_EMPTY
                && map[yPoint_26202+1][xPoint_26202-1] === this.CELL_EMPTY
                && map[yPoint_26202][xPoint_26202-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26202, yPoint_26202, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26202;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26202][x] === this.CELL_EMPTY
                    && map[yPoint_26202-1][x] === this.CELL_EMPTY
                    && map[yPoint_26202-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26202][x+1] === this.CELL_EMPTY
                    && map[yPoint_26202+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26202+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26202, yPoint_26202, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26202;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26202] === this.CELL_EMPTY
                    && map[y+1][xPoint_26202] === this.CELL_EMPTY
                    && map[y+1][xPoint_26202+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26202] === this.CELL_EMPTY
                    && map[y][xPoint_26202-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26202-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
