...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9578=0;yPoint_9578<this.gameFieldBorderY.length; yPoint_9578++){
                for(var xPoint_9578=0;xPoint_9578<this.gameFieldBorderX.length; xPoint_9578++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9578, xPoint_9578);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9578][xPoint_9578] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9578, xPoint_9578, 'user');
                    if(this._userShipsMap[yPoint_9578][xPoint_9578] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9578, xPoint_9578, type){
            var id = this.getPointBlockIdByCoords(yPoint_9578, xPoint_9578, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9578);
                block.setAttribute('data-y', yPoint_9578);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9578
         * @param {type} xPoint_9578
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9578, xPoint_9578, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9578 + '_y' + yPoint_9578;
            }
            return 'pc_x' + xPoint_9578 + '_y' + yPoint_9578;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9578=0;yPoint_9578<this.gameFieldBorderY.length; yPoint_9578++){
                for(var xPoint_9578=0;xPoint_9578<this.gameFieldBorderX.length; xPoint_9578++){
                    map.push({y: yPoint_9578, x: xPoint_9578});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9578=-1;yPoint_9578<(this.gameFieldBorderY.length+1); yPoint_9578++){
                for(var xPoint_9578=-1;xPoint_9578<(this.gameFieldBorderX.length+1); xPoint_9578++){
                    if(!map[yPoint_9578]){
                        map[yPoint_9578] = [];
                    }
                    map[yPoint_9578][xPoint_9578] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9578 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9578 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9578, yPoint_9578) === true){
                    if(this.canPutHorizontal(map, xPoint_9578, yPoint_9578, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9578][xPoint_9578 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9578, yPoint_9578, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9578 + i][xPoint_9578] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9578, yPoint_9578){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9578][xPoint_9578] === this.CELL_EMPTY
                && map[yPoint_9578-1][xPoint_9578] === this.CELL_EMPTY
                && map[yPoint_9578-1][xPoint_9578+1] === this.CELL_EMPTY
                && map[yPoint_9578][xPoint_9578+1] === this.CELL_EMPTY
                && map[yPoint_9578+1][xPoint_9578+1] === this.CELL_EMPTY
                && map[yPoint_9578+1][xPoint_9578] === this.CELL_EMPTY
                && map[yPoint_9578+1][xPoint_9578-1] === this.CELL_EMPTY
                && map[yPoint_9578][xPoint_9578-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9578, yPoint_9578, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9578;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9578][x] === this.CELL_EMPTY
                    && map[yPoint_9578-1][x] === this.CELL_EMPTY
                    && map[yPoint_9578-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9578][x+1] === this.CELL_EMPTY
                    && map[yPoint_9578+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9578+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9578, yPoint_9578, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9578;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9578] === this.CELL_EMPTY
                    && map[y+1][xPoint_9578] === this.CELL_EMPTY
                    && map[y+1][xPoint_9578+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9578] === this.CELL_EMPTY
                    && map[y][xPoint_9578-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9578-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
