...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27267=0;yPoint_27267<this.gameFieldBorderY.length; yPoint_27267++){
                for(var xPoint_27267=0;xPoint_27267<this.gameFieldBorderX.length; xPoint_27267++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27267, xPoint_27267);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27267][xPoint_27267] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27267, xPoint_27267, 'user');
                    if(this._userShipsMap[yPoint_27267][xPoint_27267] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27267, xPoint_27267, type){
            var id = this.getPointBlockIdByCoords(yPoint_27267, xPoint_27267, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27267);
                block.setAttribute('data-y', yPoint_27267);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27267
         * @param {type} xPoint_27267
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27267, xPoint_27267, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27267 + '_y' + yPoint_27267;
            }
            return 'pc_x' + xPoint_27267 + '_y' + yPoint_27267;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27267=0;yPoint_27267<this.gameFieldBorderY.length; yPoint_27267++){
                for(var xPoint_27267=0;xPoint_27267<this.gameFieldBorderX.length; xPoint_27267++){
                    map.push({y: yPoint_27267, x: xPoint_27267});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27267=-1;yPoint_27267<(this.gameFieldBorderY.length+1); yPoint_27267++){
                for(var xPoint_27267=-1;xPoint_27267<(this.gameFieldBorderX.length+1); xPoint_27267++){
                    if(!map[yPoint_27267]){
                        map[yPoint_27267] = [];
                    }
                    map[yPoint_27267][xPoint_27267] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27267 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27267 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27267, yPoint_27267) === true){
                    if(this.canPutHorizontal(map, xPoint_27267, yPoint_27267, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27267][xPoint_27267 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27267, yPoint_27267, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27267 + i][xPoint_27267] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27267, yPoint_27267){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27267][xPoint_27267] === this.CELL_EMPTY
                && map[yPoint_27267-1][xPoint_27267] === this.CELL_EMPTY
                && map[yPoint_27267-1][xPoint_27267+1] === this.CELL_EMPTY
                && map[yPoint_27267][xPoint_27267+1] === this.CELL_EMPTY
                && map[yPoint_27267+1][xPoint_27267+1] === this.CELL_EMPTY
                && map[yPoint_27267+1][xPoint_27267] === this.CELL_EMPTY
                && map[yPoint_27267+1][xPoint_27267-1] === this.CELL_EMPTY
                && map[yPoint_27267][xPoint_27267-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27267, yPoint_27267, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27267;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27267][x] === this.CELL_EMPTY
                    && map[yPoint_27267-1][x] === this.CELL_EMPTY
                    && map[yPoint_27267-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27267][x+1] === this.CELL_EMPTY
                    && map[yPoint_27267+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27267+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27267, yPoint_27267, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27267;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27267] === this.CELL_EMPTY
                    && map[y+1][xPoint_27267] === this.CELL_EMPTY
                    && map[y+1][xPoint_27267+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27267] === this.CELL_EMPTY
                    && map[y][xPoint_27267-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27267-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
