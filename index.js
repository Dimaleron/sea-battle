...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11946=0;yPoint_11946<this.gameFieldBorderY.length; yPoint_11946++){
                for(var xPoint_11946=0;xPoint_11946<this.gameFieldBorderX.length; xPoint_11946++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11946, xPoint_11946);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11946][xPoint_11946] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11946, xPoint_11946, 'user');
                    if(this._userShipsMap[yPoint_11946][xPoint_11946] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11946, xPoint_11946, type){
            var id = this.getPointBlockIdByCoords(yPoint_11946, xPoint_11946, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11946);
                block.setAttribute('data-y', yPoint_11946);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11946
         * @param {type} xPoint_11946
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11946, xPoint_11946, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11946 + '_y' + yPoint_11946;
            }
            return 'pc_x' + xPoint_11946 + '_y' + yPoint_11946;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11946=0;yPoint_11946<this.gameFieldBorderY.length; yPoint_11946++){
                for(var xPoint_11946=0;xPoint_11946<this.gameFieldBorderX.length; xPoint_11946++){
                    map.push({y: yPoint_11946, x: xPoint_11946});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11946=-1;yPoint_11946<(this.gameFieldBorderY.length+1); yPoint_11946++){
                for(var xPoint_11946=-1;xPoint_11946<(this.gameFieldBorderX.length+1); xPoint_11946++){
                    if(!map[yPoint_11946]){
                        map[yPoint_11946] = [];
                    }
                    map[yPoint_11946][xPoint_11946] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11946 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11946 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11946, yPoint_11946) === true){
                    if(this.canPutHorizontal(map, xPoint_11946, yPoint_11946, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11946][xPoint_11946 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11946, yPoint_11946, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11946 + i][xPoint_11946] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11946, yPoint_11946){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11946][xPoint_11946] === this.CELL_EMPTY
                && map[yPoint_11946-1][xPoint_11946] === this.CELL_EMPTY
                && map[yPoint_11946-1][xPoint_11946+1] === this.CELL_EMPTY
                && map[yPoint_11946][xPoint_11946+1] === this.CELL_EMPTY
                && map[yPoint_11946+1][xPoint_11946+1] === this.CELL_EMPTY
                && map[yPoint_11946+1][xPoint_11946] === this.CELL_EMPTY
                && map[yPoint_11946+1][xPoint_11946-1] === this.CELL_EMPTY
                && map[yPoint_11946][xPoint_11946-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11946, yPoint_11946, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11946;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11946][x] === this.CELL_EMPTY
                    && map[yPoint_11946-1][x] === this.CELL_EMPTY
                    && map[yPoint_11946-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11946][x+1] === this.CELL_EMPTY
                    && map[yPoint_11946+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11946+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11946, yPoint_11946, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11946;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11946] === this.CELL_EMPTY
                    && map[y+1][xPoint_11946] === this.CELL_EMPTY
                    && map[y+1][xPoint_11946+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11946] === this.CELL_EMPTY
                    && map[y][xPoint_11946-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11946-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
