...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14885=0;yPoint_14885<this.gameFieldBorderY.length; yPoint_14885++){
                for(var xPoint_14885=0;xPoint_14885<this.gameFieldBorderX.length; xPoint_14885++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14885, xPoint_14885);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14885][xPoint_14885] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14885, xPoint_14885, 'user');
                    if(this._userShipsMap[yPoint_14885][xPoint_14885] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14885, xPoint_14885, type){
            var id = this.getPointBlockIdByCoords(yPoint_14885, xPoint_14885, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14885);
                block.setAttribute('data-y', yPoint_14885);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14885
         * @param {type} xPoint_14885
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14885, xPoint_14885, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14885 + '_y' + yPoint_14885;
            }
            return 'pc_x' + xPoint_14885 + '_y' + yPoint_14885;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14885=0;yPoint_14885<this.gameFieldBorderY.length; yPoint_14885++){
                for(var xPoint_14885=0;xPoint_14885<this.gameFieldBorderX.length; xPoint_14885++){
                    map.push({y: yPoint_14885, x: xPoint_14885});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14885=-1;yPoint_14885<(this.gameFieldBorderY.length+1); yPoint_14885++){
                for(var xPoint_14885=-1;xPoint_14885<(this.gameFieldBorderX.length+1); xPoint_14885++){
                    if(!map[yPoint_14885]){
                        map[yPoint_14885] = [];
                    }
                    map[yPoint_14885][xPoint_14885] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14885 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14885 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14885, yPoint_14885) === true){
                    if(this.canPutHorizontal(map, xPoint_14885, yPoint_14885, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14885][xPoint_14885 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14885, yPoint_14885, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14885 + i][xPoint_14885] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14885, yPoint_14885){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14885][xPoint_14885] === this.CELL_EMPTY
                && map[yPoint_14885-1][xPoint_14885] === this.CELL_EMPTY
                && map[yPoint_14885-1][xPoint_14885+1] === this.CELL_EMPTY
                && map[yPoint_14885][xPoint_14885+1] === this.CELL_EMPTY
                && map[yPoint_14885+1][xPoint_14885+1] === this.CELL_EMPTY
                && map[yPoint_14885+1][xPoint_14885] === this.CELL_EMPTY
                && map[yPoint_14885+1][xPoint_14885-1] === this.CELL_EMPTY
                && map[yPoint_14885][xPoint_14885-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14885, yPoint_14885, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14885;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14885][x] === this.CELL_EMPTY
                    && map[yPoint_14885-1][x] === this.CELL_EMPTY
                    && map[yPoint_14885-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14885][x+1] === this.CELL_EMPTY
                    && map[yPoint_14885+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14885+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14885, yPoint_14885, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14885;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14885] === this.CELL_EMPTY
                    && map[y+1][xPoint_14885] === this.CELL_EMPTY
                    && map[y+1][xPoint_14885+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14885] === this.CELL_EMPTY
                    && map[y][xPoint_14885-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14885-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
