...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8360=0;yPoint_8360<this.gameFieldBorderY.length; yPoint_8360++){
                for(var xPoint_8360=0;xPoint_8360<this.gameFieldBorderX.length; xPoint_8360++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8360, xPoint_8360);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8360][xPoint_8360] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8360, xPoint_8360, 'user');
                    if(this._userShipsMap[yPoint_8360][xPoint_8360] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8360, xPoint_8360, type){
            var id = this.getPointBlockIdByCoords(yPoint_8360, xPoint_8360, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8360);
                block.setAttribute('data-y', yPoint_8360);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8360
         * @param {type} xPoint_8360
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8360, xPoint_8360, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8360 + '_y' + yPoint_8360;
            }
            return 'pc_x' + xPoint_8360 + '_y' + yPoint_8360;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8360=0;yPoint_8360<this.gameFieldBorderY.length; yPoint_8360++){
                for(var xPoint_8360=0;xPoint_8360<this.gameFieldBorderX.length; xPoint_8360++){
                    map.push({y: yPoint_8360, x: xPoint_8360});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8360=-1;yPoint_8360<(this.gameFieldBorderY.length+1); yPoint_8360++){
                for(var xPoint_8360=-1;xPoint_8360<(this.gameFieldBorderX.length+1); xPoint_8360++){
                    if(!map[yPoint_8360]){
                        map[yPoint_8360] = [];
                    }
                    map[yPoint_8360][xPoint_8360] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8360 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8360 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8360, yPoint_8360) === true){
                    if(this.canPutHorizontal(map, xPoint_8360, yPoint_8360, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8360][xPoint_8360 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8360, yPoint_8360, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8360 + i][xPoint_8360] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8360, yPoint_8360){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8360][xPoint_8360] === this.CELL_EMPTY
                && map[yPoint_8360-1][xPoint_8360] === this.CELL_EMPTY
                && map[yPoint_8360-1][xPoint_8360+1] === this.CELL_EMPTY
                && map[yPoint_8360][xPoint_8360+1] === this.CELL_EMPTY
                && map[yPoint_8360+1][xPoint_8360+1] === this.CELL_EMPTY
                && map[yPoint_8360+1][xPoint_8360] === this.CELL_EMPTY
                && map[yPoint_8360+1][xPoint_8360-1] === this.CELL_EMPTY
                && map[yPoint_8360][xPoint_8360-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8360, yPoint_8360, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8360;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8360][x] === this.CELL_EMPTY
                    && map[yPoint_8360-1][x] === this.CELL_EMPTY
                    && map[yPoint_8360-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8360][x+1] === this.CELL_EMPTY
                    && map[yPoint_8360+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8360+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8360, yPoint_8360, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8360;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8360] === this.CELL_EMPTY
                    && map[y+1][xPoint_8360] === this.CELL_EMPTY
                    && map[y+1][xPoint_8360+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8360] === this.CELL_EMPTY
                    && map[y][xPoint_8360-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8360-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
