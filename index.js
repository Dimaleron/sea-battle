...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32083=0;yPoint_32083<this.gameFieldBorderY.length; yPoint_32083++){
                for(var xPoint_32083=0;xPoint_32083<this.gameFieldBorderX.length; xPoint_32083++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32083, xPoint_32083);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32083][xPoint_32083] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32083, xPoint_32083, 'user');
                    if(this._userShipsMap[yPoint_32083][xPoint_32083] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32083, xPoint_32083, type){
            var id = this.getPointBlockIdByCoords(yPoint_32083, xPoint_32083, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32083);
                block.setAttribute('data-y', yPoint_32083);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32083
         * @param {type} xPoint_32083
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32083, xPoint_32083, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32083 + '_y' + yPoint_32083;
            }
            return 'pc_x' + xPoint_32083 + '_y' + yPoint_32083;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32083=0;yPoint_32083<this.gameFieldBorderY.length; yPoint_32083++){
                for(var xPoint_32083=0;xPoint_32083<this.gameFieldBorderX.length; xPoint_32083++){
                    map.push({y: yPoint_32083, x: xPoint_32083});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32083=-1;yPoint_32083<(this.gameFieldBorderY.length+1); yPoint_32083++){
                for(var xPoint_32083=-1;xPoint_32083<(this.gameFieldBorderX.length+1); xPoint_32083++){
                    if(!map[yPoint_32083]){
                        map[yPoint_32083] = [];
                    }
                    map[yPoint_32083][xPoint_32083] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32083 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32083 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32083, yPoint_32083) === true){
                    if(this.canPutHorizontal(map, xPoint_32083, yPoint_32083, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32083][xPoint_32083 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32083, yPoint_32083, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32083 + i][xPoint_32083] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32083, yPoint_32083){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32083][xPoint_32083] === this.CELL_EMPTY
                && map[yPoint_32083-1][xPoint_32083] === this.CELL_EMPTY
                && map[yPoint_32083-1][xPoint_32083+1] === this.CELL_EMPTY
                && map[yPoint_32083][xPoint_32083+1] === this.CELL_EMPTY
                && map[yPoint_32083+1][xPoint_32083+1] === this.CELL_EMPTY
                && map[yPoint_32083+1][xPoint_32083] === this.CELL_EMPTY
                && map[yPoint_32083+1][xPoint_32083-1] === this.CELL_EMPTY
                && map[yPoint_32083][xPoint_32083-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32083, yPoint_32083, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32083;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32083][x] === this.CELL_EMPTY
                    && map[yPoint_32083-1][x] === this.CELL_EMPTY
                    && map[yPoint_32083-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32083][x+1] === this.CELL_EMPTY
                    && map[yPoint_32083+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32083+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32083, yPoint_32083, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32083;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32083] === this.CELL_EMPTY
                    && map[y+1][xPoint_32083] === this.CELL_EMPTY
                    && map[y+1][xPoint_32083+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32083] === this.CELL_EMPTY
                    && map[y][xPoint_32083-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32083-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
