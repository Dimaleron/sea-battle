...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_12634=0;yPoint_12634<this.gameFieldBorderY.length; yPoint_12634++){
                for(var xPoint_12634=0;xPoint_12634<this.gameFieldBorderX.length; xPoint_12634++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_12634, xPoint_12634);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_12634][xPoint_12634] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_12634, xPoint_12634, 'user');
                    if(this._userShipsMap[yPoint_12634][xPoint_12634] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_12634, xPoint_12634, type){
            var id = this.getPointBlockIdByCoords(yPoint_12634, xPoint_12634, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_12634);
                block.setAttribute('data-y', yPoint_12634);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_12634
         * @param {type} xPoint_12634
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_12634, xPoint_12634, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_12634 + '_y' + yPoint_12634;
            }
            return 'pc_x' + xPoint_12634 + '_y' + yPoint_12634;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_12634=0;yPoint_12634<this.gameFieldBorderY.length; yPoint_12634++){
                for(var xPoint_12634=0;xPoint_12634<this.gameFieldBorderX.length; xPoint_12634++){
                    map.push({y: yPoint_12634, x: xPoint_12634});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_12634=-1;yPoint_12634<(this.gameFieldBorderY.length+1); yPoint_12634++){
                for(var xPoint_12634=-1;xPoint_12634<(this.gameFieldBorderX.length+1); xPoint_12634++){
                    if(!map[yPoint_12634]){
                        map[yPoint_12634] = [];
                    }
                    map[yPoint_12634][xPoint_12634] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_12634 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_12634 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_12634, yPoint_12634) === true){
                    if(this.canPutHorizontal(map, xPoint_12634, yPoint_12634, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12634][xPoint_12634 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_12634, yPoint_12634, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12634 + i][xPoint_12634] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_12634, yPoint_12634){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_12634][xPoint_12634] === this.CELL_EMPTY
                && map[yPoint_12634-1][xPoint_12634] === this.CELL_EMPTY
                && map[yPoint_12634-1][xPoint_12634+1] === this.CELL_EMPTY
                && map[yPoint_12634][xPoint_12634+1] === this.CELL_EMPTY
                && map[yPoint_12634+1][xPoint_12634+1] === this.CELL_EMPTY
                && map[yPoint_12634+1][xPoint_12634] === this.CELL_EMPTY
                && map[yPoint_12634+1][xPoint_12634-1] === this.CELL_EMPTY
                && map[yPoint_12634][xPoint_12634-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_12634, yPoint_12634, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_12634;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_12634][x] === this.CELL_EMPTY
                    && map[yPoint_12634-1][x] === this.CELL_EMPTY
                    && map[yPoint_12634-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12634][x+1] === this.CELL_EMPTY
                    && map[yPoint_12634+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12634+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_12634, yPoint_12634, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_12634;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_12634] === this.CELL_EMPTY
                    && map[y+1][xPoint_12634] === this.CELL_EMPTY
                    && map[y+1][xPoint_12634+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_12634] === this.CELL_EMPTY
                    && map[y][xPoint_12634-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_12634-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
