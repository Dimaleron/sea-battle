...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17978=0;yPoint_17978<this.gameFieldBorderY.length; yPoint_17978++){
                for(var xPoint_17978=0;xPoint_17978<this.gameFieldBorderX.length; xPoint_17978++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17978, xPoint_17978);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17978][xPoint_17978] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17978, xPoint_17978, 'user');
                    if(this._userShipsMap[yPoint_17978][xPoint_17978] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17978, xPoint_17978, type){
            var id = this.getPointBlockIdByCoords(yPoint_17978, xPoint_17978, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17978);
                block.setAttribute('data-y', yPoint_17978);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17978
         * @param {type} xPoint_17978
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17978, xPoint_17978, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17978 + '_y' + yPoint_17978;
            }
            return 'pc_x' + xPoint_17978 + '_y' + yPoint_17978;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17978=0;yPoint_17978<this.gameFieldBorderY.length; yPoint_17978++){
                for(var xPoint_17978=0;xPoint_17978<this.gameFieldBorderX.length; xPoint_17978++){
                    map.push({y: yPoint_17978, x: xPoint_17978});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17978=-1;yPoint_17978<(this.gameFieldBorderY.length+1); yPoint_17978++){
                for(var xPoint_17978=-1;xPoint_17978<(this.gameFieldBorderX.length+1); xPoint_17978++){
                    if(!map[yPoint_17978]){
                        map[yPoint_17978] = [];
                    }
                    map[yPoint_17978][xPoint_17978] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17978 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17978 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17978, yPoint_17978) === true){
                    if(this.canPutHorizontal(map, xPoint_17978, yPoint_17978, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17978][xPoint_17978 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17978, yPoint_17978, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17978 + i][xPoint_17978] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17978, yPoint_17978){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17978][xPoint_17978] === this.CELL_EMPTY
                && map[yPoint_17978-1][xPoint_17978] === this.CELL_EMPTY
                && map[yPoint_17978-1][xPoint_17978+1] === this.CELL_EMPTY
                && map[yPoint_17978][xPoint_17978+1] === this.CELL_EMPTY
                && map[yPoint_17978+1][xPoint_17978+1] === this.CELL_EMPTY
                && map[yPoint_17978+1][xPoint_17978] === this.CELL_EMPTY
                && map[yPoint_17978+1][xPoint_17978-1] === this.CELL_EMPTY
                && map[yPoint_17978][xPoint_17978-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17978, yPoint_17978, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17978;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17978][x] === this.CELL_EMPTY
                    && map[yPoint_17978-1][x] === this.CELL_EMPTY
                    && map[yPoint_17978-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17978][x+1] === this.CELL_EMPTY
                    && map[yPoint_17978+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17978+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17978, yPoint_17978, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17978;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17978] === this.CELL_EMPTY
                    && map[y+1][xPoint_17978] === this.CELL_EMPTY
                    && map[y+1][xPoint_17978+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17978] === this.CELL_EMPTY
                    && map[y][xPoint_17978-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17978-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
