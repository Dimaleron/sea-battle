...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_897=0;yPoint_897<this.gameFieldBorderY.length; yPoint_897++){
                for(var xPoint_897=0;xPoint_897<this.gameFieldBorderX.length; xPoint_897++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_897, xPoint_897);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_897][xPoint_897] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_897, xPoint_897, 'user');
                    if(this._userShipsMap[yPoint_897][xPoint_897] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_897, xPoint_897, type){
            var id = this.getPointBlockIdByCoords(yPoint_897, xPoint_897, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_897);
                block.setAttribute('data-y', yPoint_897);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_897
         * @param {type} xPoint_897
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_897, xPoint_897, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_897 + '_y' + yPoint_897;
            }
            return 'pc_x' + xPoint_897 + '_y' + yPoint_897;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_897=0;yPoint_897<this.gameFieldBorderY.length; yPoint_897++){
                for(var xPoint_897=0;xPoint_897<this.gameFieldBorderX.length; xPoint_897++){
                    map.push({y: yPoint_897, x: xPoint_897});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_897=-1;yPoint_897<(this.gameFieldBorderY.length+1); yPoint_897++){
                for(var xPoint_897=-1;xPoint_897<(this.gameFieldBorderX.length+1); xPoint_897++){
                    if(!map[yPoint_897]){
                        map[yPoint_897] = [];
                    }
                    map[yPoint_897][xPoint_897] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_897 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_897 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_897, yPoint_897) === true){
                    if(this.canPutHorizontal(map, xPoint_897, yPoint_897, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_897][xPoint_897 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_897, yPoint_897, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_897 + i][xPoint_897] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_897, yPoint_897){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_897][xPoint_897] === this.CELL_EMPTY
                && map[yPoint_897-1][xPoint_897] === this.CELL_EMPTY
                && map[yPoint_897-1][xPoint_897+1] === this.CELL_EMPTY
                && map[yPoint_897][xPoint_897+1] === this.CELL_EMPTY
                && map[yPoint_897+1][xPoint_897+1] === this.CELL_EMPTY
                && map[yPoint_897+1][xPoint_897] === this.CELL_EMPTY
                && map[yPoint_897+1][xPoint_897-1] === this.CELL_EMPTY
                && map[yPoint_897][xPoint_897-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_897, yPoint_897, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_897;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_897][x] === this.CELL_EMPTY
                    && map[yPoint_897-1][x] === this.CELL_EMPTY
                    && map[yPoint_897-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_897][x+1] === this.CELL_EMPTY
                    && map[yPoint_897+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_897+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_897, yPoint_897, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_897;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_897] === this.CELL_EMPTY
                    && map[y+1][xPoint_897] === this.CELL_EMPTY
                    && map[y+1][xPoint_897+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_897] === this.CELL_EMPTY
                    && map[y][xPoint_897-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_897-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
