...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11196=0;yPoint_11196<this.gameFieldBorderY.length; yPoint_11196++){
                for(var xPoint_11196=0;xPoint_11196<this.gameFieldBorderX.length; xPoint_11196++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11196, xPoint_11196);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11196][xPoint_11196] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11196, xPoint_11196, 'user');
                    if(this._userShipsMap[yPoint_11196][xPoint_11196] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11196, xPoint_11196, type){
            var id = this.getPointBlockIdByCoords(yPoint_11196, xPoint_11196, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11196);
                block.setAttribute('data-y', yPoint_11196);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11196
         * @param {type} xPoint_11196
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11196, xPoint_11196, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11196 + '_y' + yPoint_11196;
            }
            return 'pc_x' + xPoint_11196 + '_y' + yPoint_11196;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11196=0;yPoint_11196<this.gameFieldBorderY.length; yPoint_11196++){
                for(var xPoint_11196=0;xPoint_11196<this.gameFieldBorderX.length; xPoint_11196++){
                    map.push({y: yPoint_11196, x: xPoint_11196});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11196=-1;yPoint_11196<(this.gameFieldBorderY.length+1); yPoint_11196++){
                for(var xPoint_11196=-1;xPoint_11196<(this.gameFieldBorderX.length+1); xPoint_11196++){
                    if(!map[yPoint_11196]){
                        map[yPoint_11196] = [];
                    }
                    map[yPoint_11196][xPoint_11196] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11196 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11196 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11196, yPoint_11196) === true){
                    if(this.canPutHorizontal(map, xPoint_11196, yPoint_11196, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11196][xPoint_11196 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11196, yPoint_11196, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11196 + i][xPoint_11196] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11196, yPoint_11196){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11196][xPoint_11196] === this.CELL_EMPTY
                && map[yPoint_11196-1][xPoint_11196] === this.CELL_EMPTY
                && map[yPoint_11196-1][xPoint_11196+1] === this.CELL_EMPTY
                && map[yPoint_11196][xPoint_11196+1] === this.CELL_EMPTY
                && map[yPoint_11196+1][xPoint_11196+1] === this.CELL_EMPTY
                && map[yPoint_11196+1][xPoint_11196] === this.CELL_EMPTY
                && map[yPoint_11196+1][xPoint_11196-1] === this.CELL_EMPTY
                && map[yPoint_11196][xPoint_11196-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11196, yPoint_11196, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11196;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11196][x] === this.CELL_EMPTY
                    && map[yPoint_11196-1][x] === this.CELL_EMPTY
                    && map[yPoint_11196-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11196][x+1] === this.CELL_EMPTY
                    && map[yPoint_11196+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11196+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11196, yPoint_11196, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11196;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11196] === this.CELL_EMPTY
                    && map[y+1][xPoint_11196] === this.CELL_EMPTY
                    && map[y+1][xPoint_11196+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11196] === this.CELL_EMPTY
                    && map[y][xPoint_11196-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11196-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
