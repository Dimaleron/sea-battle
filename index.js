...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30804=0;yPoint_30804<this.gameFieldBorderY.length; yPoint_30804++){
                for(var xPoint_30804=0;xPoint_30804<this.gameFieldBorderX.length; xPoint_30804++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30804, xPoint_30804);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30804][xPoint_30804] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30804, xPoint_30804, 'user');
                    if(this._userShipsMap[yPoint_30804][xPoint_30804] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30804, xPoint_30804, type){
            var id = this.getPointBlockIdByCoords(yPoint_30804, xPoint_30804, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30804);
                block.setAttribute('data-y', yPoint_30804);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30804
         * @param {type} xPoint_30804
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30804, xPoint_30804, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30804 + '_y' + yPoint_30804;
            }
            return 'pc_x' + xPoint_30804 + '_y' + yPoint_30804;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30804=0;yPoint_30804<this.gameFieldBorderY.length; yPoint_30804++){
                for(var xPoint_30804=0;xPoint_30804<this.gameFieldBorderX.length; xPoint_30804++){
                    map.push({y: yPoint_30804, x: xPoint_30804});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30804=-1;yPoint_30804<(this.gameFieldBorderY.length+1); yPoint_30804++){
                for(var xPoint_30804=-1;xPoint_30804<(this.gameFieldBorderX.length+1); xPoint_30804++){
                    if(!map[yPoint_30804]){
                        map[yPoint_30804] = [];
                    }
                    map[yPoint_30804][xPoint_30804] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30804 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30804 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30804, yPoint_30804) === true){
                    if(this.canPutHorizontal(map, xPoint_30804, yPoint_30804, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30804][xPoint_30804 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30804, yPoint_30804, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30804 + i][xPoint_30804] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30804, yPoint_30804){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30804][xPoint_30804] === this.CELL_EMPTY
                && map[yPoint_30804-1][xPoint_30804] === this.CELL_EMPTY
                && map[yPoint_30804-1][xPoint_30804+1] === this.CELL_EMPTY
                && map[yPoint_30804][xPoint_30804+1] === this.CELL_EMPTY
                && map[yPoint_30804+1][xPoint_30804+1] === this.CELL_EMPTY
                && map[yPoint_30804+1][xPoint_30804] === this.CELL_EMPTY
                && map[yPoint_30804+1][xPoint_30804-1] === this.CELL_EMPTY
                && map[yPoint_30804][xPoint_30804-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30804, yPoint_30804, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30804;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30804][x] === this.CELL_EMPTY
                    && map[yPoint_30804-1][x] === this.CELL_EMPTY
                    && map[yPoint_30804-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30804][x+1] === this.CELL_EMPTY
                    && map[yPoint_30804+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30804+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30804, yPoint_30804, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30804;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30804] === this.CELL_EMPTY
                    && map[y+1][xPoint_30804] === this.CELL_EMPTY
                    && map[y+1][xPoint_30804+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30804] === this.CELL_EMPTY
                    && map[y][xPoint_30804-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30804-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
