...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30134=0;yPoint_30134<this.gameFieldBorderY.length; yPoint_30134++){
                for(var xPoint_30134=0;xPoint_30134<this.gameFieldBorderX.length; xPoint_30134++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30134, xPoint_30134);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30134][xPoint_30134] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30134, xPoint_30134, 'user');
                    if(this._userShipsMap[yPoint_30134][xPoint_30134] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30134, xPoint_30134, type){
            var id = this.getPointBlockIdByCoords(yPoint_30134, xPoint_30134, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30134);
                block.setAttribute('data-y', yPoint_30134);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30134
         * @param {type} xPoint_30134
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30134, xPoint_30134, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30134 + '_y' + yPoint_30134;
            }
            return 'pc_x' + xPoint_30134 + '_y' + yPoint_30134;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30134=0;yPoint_30134<this.gameFieldBorderY.length; yPoint_30134++){
                for(var xPoint_30134=0;xPoint_30134<this.gameFieldBorderX.length; xPoint_30134++){
                    map.push({y: yPoint_30134, x: xPoint_30134});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30134=-1;yPoint_30134<(this.gameFieldBorderY.length+1); yPoint_30134++){
                for(var xPoint_30134=-1;xPoint_30134<(this.gameFieldBorderX.length+1); xPoint_30134++){
                    if(!map[yPoint_30134]){
                        map[yPoint_30134] = [];
                    }
                    map[yPoint_30134][xPoint_30134] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30134 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30134 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30134, yPoint_30134) === true){
                    if(this.canPutHorizontal(map, xPoint_30134, yPoint_30134, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30134][xPoint_30134 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30134, yPoint_30134, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30134 + i][xPoint_30134] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30134, yPoint_30134){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30134][xPoint_30134] === this.CELL_EMPTY
                && map[yPoint_30134-1][xPoint_30134] === this.CELL_EMPTY
                && map[yPoint_30134-1][xPoint_30134+1] === this.CELL_EMPTY
                && map[yPoint_30134][xPoint_30134+1] === this.CELL_EMPTY
                && map[yPoint_30134+1][xPoint_30134+1] === this.CELL_EMPTY
                && map[yPoint_30134+1][xPoint_30134] === this.CELL_EMPTY
                && map[yPoint_30134+1][xPoint_30134-1] === this.CELL_EMPTY
                && map[yPoint_30134][xPoint_30134-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30134, yPoint_30134, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30134;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30134][x] === this.CELL_EMPTY
                    && map[yPoint_30134-1][x] === this.CELL_EMPTY
                    && map[yPoint_30134-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30134][x+1] === this.CELL_EMPTY
                    && map[yPoint_30134+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30134+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30134, yPoint_30134, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30134;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30134] === this.CELL_EMPTY
                    && map[y+1][xPoint_30134] === this.CELL_EMPTY
                    && map[y+1][xPoint_30134+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30134] === this.CELL_EMPTY
                    && map[y][xPoint_30134-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30134-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
