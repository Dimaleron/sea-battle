...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13011=0;yPoint_13011<this.gameFieldBorderY.length; yPoint_13011++){
                for(var xPoint_13011=0;xPoint_13011<this.gameFieldBorderX.length; xPoint_13011++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13011, xPoint_13011);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13011][xPoint_13011] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13011, xPoint_13011, 'user');
                    if(this._userShipsMap[yPoint_13011][xPoint_13011] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13011, xPoint_13011, type){
            var id = this.getPointBlockIdByCoords(yPoint_13011, xPoint_13011, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13011);
                block.setAttribute('data-y', yPoint_13011);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13011
         * @param {type} xPoint_13011
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13011, xPoint_13011, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13011 + '_y' + yPoint_13011;
            }
            return 'pc_x' + xPoint_13011 + '_y' + yPoint_13011;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13011=0;yPoint_13011<this.gameFieldBorderY.length; yPoint_13011++){
                for(var xPoint_13011=0;xPoint_13011<this.gameFieldBorderX.length; xPoint_13011++){
                    map.push({y: yPoint_13011, x: xPoint_13011});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13011=-1;yPoint_13011<(this.gameFieldBorderY.length+1); yPoint_13011++){
                for(var xPoint_13011=-1;xPoint_13011<(this.gameFieldBorderX.length+1); xPoint_13011++){
                    if(!map[yPoint_13011]){
                        map[yPoint_13011] = [];
                    }
                    map[yPoint_13011][xPoint_13011] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13011 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13011 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13011, yPoint_13011) === true){
                    if(this.canPutHorizontal(map, xPoint_13011, yPoint_13011, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13011][xPoint_13011 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13011, yPoint_13011, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13011 + i][xPoint_13011] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13011, yPoint_13011){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13011][xPoint_13011] === this.CELL_EMPTY
                && map[yPoint_13011-1][xPoint_13011] === this.CELL_EMPTY
                && map[yPoint_13011-1][xPoint_13011+1] === this.CELL_EMPTY
                && map[yPoint_13011][xPoint_13011+1] === this.CELL_EMPTY
                && map[yPoint_13011+1][xPoint_13011+1] === this.CELL_EMPTY
                && map[yPoint_13011+1][xPoint_13011] === this.CELL_EMPTY
                && map[yPoint_13011+1][xPoint_13011-1] === this.CELL_EMPTY
                && map[yPoint_13011][xPoint_13011-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13011, yPoint_13011, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13011;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13011][x] === this.CELL_EMPTY
                    && map[yPoint_13011-1][x] === this.CELL_EMPTY
                    && map[yPoint_13011-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13011][x+1] === this.CELL_EMPTY
                    && map[yPoint_13011+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13011+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13011, yPoint_13011, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13011;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13011] === this.CELL_EMPTY
                    && map[y+1][xPoint_13011] === this.CELL_EMPTY
                    && map[y+1][xPoint_13011+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13011] === this.CELL_EMPTY
                    && map[y][xPoint_13011-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13011-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
