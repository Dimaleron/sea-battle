...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29498=0;yPoint_29498<this.gameFieldBorderY.length; yPoint_29498++){
                for(var xPoint_29498=0;xPoint_29498<this.gameFieldBorderX.length; xPoint_29498++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29498, xPoint_29498);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29498][xPoint_29498] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29498, xPoint_29498, 'user');
                    if(this._userShipsMap[yPoint_29498][xPoint_29498] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29498, xPoint_29498, type){
            var id = this.getPointBlockIdByCoords(yPoint_29498, xPoint_29498, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29498);
                block.setAttribute('data-y', yPoint_29498);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29498
         * @param {type} xPoint_29498
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29498, xPoint_29498, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29498 + '_y' + yPoint_29498;
            }
            return 'pc_x' + xPoint_29498 + '_y' + yPoint_29498;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29498=0;yPoint_29498<this.gameFieldBorderY.length; yPoint_29498++){
                for(var xPoint_29498=0;xPoint_29498<this.gameFieldBorderX.length; xPoint_29498++){
                    map.push({y: yPoint_29498, x: xPoint_29498});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29498=-1;yPoint_29498<(this.gameFieldBorderY.length+1); yPoint_29498++){
                for(var xPoint_29498=-1;xPoint_29498<(this.gameFieldBorderX.length+1); xPoint_29498++){
                    if(!map[yPoint_29498]){
                        map[yPoint_29498] = [];
                    }
                    map[yPoint_29498][xPoint_29498] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29498 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29498 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29498, yPoint_29498) === true){
                    if(this.canPutHorizontal(map, xPoint_29498, yPoint_29498, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29498][xPoint_29498 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29498, yPoint_29498, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29498 + i][xPoint_29498] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29498, yPoint_29498){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29498][xPoint_29498] === this.CELL_EMPTY
                && map[yPoint_29498-1][xPoint_29498] === this.CELL_EMPTY
                && map[yPoint_29498-1][xPoint_29498+1] === this.CELL_EMPTY
                && map[yPoint_29498][xPoint_29498+1] === this.CELL_EMPTY
                && map[yPoint_29498+1][xPoint_29498+1] === this.CELL_EMPTY
                && map[yPoint_29498+1][xPoint_29498] === this.CELL_EMPTY
                && map[yPoint_29498+1][xPoint_29498-1] === this.CELL_EMPTY
                && map[yPoint_29498][xPoint_29498-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29498, yPoint_29498, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29498;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29498][x] === this.CELL_EMPTY
                    && map[yPoint_29498-1][x] === this.CELL_EMPTY
                    && map[yPoint_29498-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29498][x+1] === this.CELL_EMPTY
                    && map[yPoint_29498+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29498+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29498, yPoint_29498, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29498;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29498] === this.CELL_EMPTY
                    && map[y+1][xPoint_29498] === this.CELL_EMPTY
                    && map[y+1][xPoint_29498+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29498] === this.CELL_EMPTY
                    && map[y][xPoint_29498-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29498-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
