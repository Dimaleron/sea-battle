...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_567=0;yPoint_567<this.gameFieldBorderY.length; yPoint_567++){
                for(var xPoint_567=0;xPoint_567<this.gameFieldBorderX.length; xPoint_567++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_567, xPoint_567);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_567][xPoint_567] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_567, xPoint_567, 'user');
                    if(this._userShipsMap[yPoint_567][xPoint_567] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_567, xPoint_567, type){
            var id = this.getPointBlockIdByCoords(yPoint_567, xPoint_567, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_567);
                block.setAttribute('data-y', yPoint_567);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_567
         * @param {type} xPoint_567
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_567, xPoint_567, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_567 + '_y' + yPoint_567;
            }
            return 'pc_x' + xPoint_567 + '_y' + yPoint_567;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_567=0;yPoint_567<this.gameFieldBorderY.length; yPoint_567++){
                for(var xPoint_567=0;xPoint_567<this.gameFieldBorderX.length; xPoint_567++){
                    map.push({y: yPoint_567, x: xPoint_567});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_567=-1;yPoint_567<(this.gameFieldBorderY.length+1); yPoint_567++){
                for(var xPoint_567=-1;xPoint_567<(this.gameFieldBorderX.length+1); xPoint_567++){
                    if(!map[yPoint_567]){
                        map[yPoint_567] = [];
                    }
                    map[yPoint_567][xPoint_567] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_567 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_567 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_567, yPoint_567) === true){
                    if(this.canPutHorizontal(map, xPoint_567, yPoint_567, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_567][xPoint_567 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_567, yPoint_567, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_567 + i][xPoint_567] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_567, yPoint_567){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_567][xPoint_567] === this.CELL_EMPTY
                && map[yPoint_567-1][xPoint_567] === this.CELL_EMPTY
                && map[yPoint_567-1][xPoint_567+1] === this.CELL_EMPTY
                && map[yPoint_567][xPoint_567+1] === this.CELL_EMPTY
                && map[yPoint_567+1][xPoint_567+1] === this.CELL_EMPTY
                && map[yPoint_567+1][xPoint_567] === this.CELL_EMPTY
                && map[yPoint_567+1][xPoint_567-1] === this.CELL_EMPTY
                && map[yPoint_567][xPoint_567-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_567, yPoint_567, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_567;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_567][x] === this.CELL_EMPTY
                    && map[yPoint_567-1][x] === this.CELL_EMPTY
                    && map[yPoint_567-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_567][x+1] === this.CELL_EMPTY
                    && map[yPoint_567+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_567+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_567, yPoint_567, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_567;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_567] === this.CELL_EMPTY
                    && map[y+1][xPoint_567] === this.CELL_EMPTY
                    && map[y+1][xPoint_567+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_567] === this.CELL_EMPTY
                    && map[y][xPoint_567-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_567-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
