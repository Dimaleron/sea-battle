...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3299=0;yPoint_3299<this.gameFieldBorderY.length; yPoint_3299++){
                for(var xPoint_3299=0;xPoint_3299<this.gameFieldBorderX.length; xPoint_3299++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3299, xPoint_3299);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3299][xPoint_3299] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3299, xPoint_3299, 'user');
                    if(this._userShipsMap[yPoint_3299][xPoint_3299] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3299, xPoint_3299, type){
            var id = this.getPointBlockIdByCoords(yPoint_3299, xPoint_3299, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3299);
                block.setAttribute('data-y', yPoint_3299);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3299
         * @param {type} xPoint_3299
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3299, xPoint_3299, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3299 + '_y' + yPoint_3299;
            }
            return 'pc_x' + xPoint_3299 + '_y' + yPoint_3299;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3299=0;yPoint_3299<this.gameFieldBorderY.length; yPoint_3299++){
                for(var xPoint_3299=0;xPoint_3299<this.gameFieldBorderX.length; xPoint_3299++){
                    map.push({y: yPoint_3299, x: xPoint_3299});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3299=-1;yPoint_3299<(this.gameFieldBorderY.length+1); yPoint_3299++){
                for(var xPoint_3299=-1;xPoint_3299<(this.gameFieldBorderX.length+1); xPoint_3299++){
                    if(!map[yPoint_3299]){
                        map[yPoint_3299] = [];
                    }
                    map[yPoint_3299][xPoint_3299] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3299 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3299 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3299, yPoint_3299) === true){
                    if(this.canPutHorizontal(map, xPoint_3299, yPoint_3299, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3299][xPoint_3299 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3299, yPoint_3299, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3299 + i][xPoint_3299] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3299, yPoint_3299){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3299][xPoint_3299] === this.CELL_EMPTY
                && map[yPoint_3299-1][xPoint_3299] === this.CELL_EMPTY
                && map[yPoint_3299-1][xPoint_3299+1] === this.CELL_EMPTY
                && map[yPoint_3299][xPoint_3299+1] === this.CELL_EMPTY
                && map[yPoint_3299+1][xPoint_3299+1] === this.CELL_EMPTY
                && map[yPoint_3299+1][xPoint_3299] === this.CELL_EMPTY
                && map[yPoint_3299+1][xPoint_3299-1] === this.CELL_EMPTY
                && map[yPoint_3299][xPoint_3299-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3299, yPoint_3299, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3299;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3299][x] === this.CELL_EMPTY
                    && map[yPoint_3299-1][x] === this.CELL_EMPTY
                    && map[yPoint_3299-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3299][x+1] === this.CELL_EMPTY
                    && map[yPoint_3299+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3299+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3299, yPoint_3299, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3299;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3299] === this.CELL_EMPTY
                    && map[y+1][xPoint_3299] === this.CELL_EMPTY
                    && map[y+1][xPoint_3299+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3299] === this.CELL_EMPTY
                    && map[y][xPoint_3299-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3299-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
