...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_66=0;yPoint_66<this.gameFieldBorderY.length; yPoint_66++){
                for(var xPoint_66=0;xPoint_66<this.gameFieldBorderX.length; xPoint_66++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_66, xPoint_66);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_66][xPoint_66] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_66, xPoint_66, 'user');
                    if(this._userShipsMap[yPoint_66][xPoint_66] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_66, xPoint_66, type){
            var id = this.getPointBlockIdByCoords(yPoint_66, xPoint_66, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_66);
                block.setAttribute('data-y', yPoint_66);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_66
         * @param {type} xPoint_66
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_66, xPoint_66, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_66 + '_y' + yPoint_66;
            }
            return 'pc_x' + xPoint_66 + '_y' + yPoint_66;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_66=0;yPoint_66<this.gameFieldBorderY.length; yPoint_66++){
                for(var xPoint_66=0;xPoint_66<this.gameFieldBorderX.length; xPoint_66++){
                    map.push({y: yPoint_66, x: xPoint_66});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_66=-1;yPoint_66<(this.gameFieldBorderY.length+1); yPoint_66++){
                for(var xPoint_66=-1;xPoint_66<(this.gameFieldBorderX.length+1); xPoint_66++){
                    if(!map[yPoint_66]){
                        map[yPoint_66] = [];
                    }
                    map[yPoint_66][xPoint_66] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_66 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_66 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_66, yPoint_66) === true){
                    if(this.canPutHorizontal(map, xPoint_66, yPoint_66, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_66][xPoint_66 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_66, yPoint_66, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_66 + i][xPoint_66] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_66, yPoint_66){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_66][xPoint_66] === this.CELL_EMPTY
                && map[yPoint_66-1][xPoint_66] === this.CELL_EMPTY
                && map[yPoint_66-1][xPoint_66+1] === this.CELL_EMPTY
                && map[yPoint_66][xPoint_66+1] === this.CELL_EMPTY
                && map[yPoint_66+1][xPoint_66+1] === this.CELL_EMPTY
                && map[yPoint_66+1][xPoint_66] === this.CELL_EMPTY
                && map[yPoint_66+1][xPoint_66-1] === this.CELL_EMPTY
                && map[yPoint_66][xPoint_66-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_66, yPoint_66, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_66;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_66][x] === this.CELL_EMPTY
                    && map[yPoint_66-1][x] === this.CELL_EMPTY
                    && map[yPoint_66-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_66][x+1] === this.CELL_EMPTY
                    && map[yPoint_66+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_66+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_66, yPoint_66, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_66;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_66] === this.CELL_EMPTY
                    && map[y+1][xPoint_66] === this.CELL_EMPTY
                    && map[y+1][xPoint_66+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_66] === this.CELL_EMPTY
                    && map[y][xPoint_66-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_66-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
