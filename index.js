...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31294=0;yPoint_31294<this.gameFieldBorderY.length; yPoint_31294++){
                for(var xPoint_31294=0;xPoint_31294<this.gameFieldBorderX.length; xPoint_31294++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31294, xPoint_31294);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31294][xPoint_31294] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31294, xPoint_31294, 'user');
                    if(this._userShipsMap[yPoint_31294][xPoint_31294] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31294, xPoint_31294, type){
            var id = this.getPointBlockIdByCoords(yPoint_31294, xPoint_31294, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31294);
                block.setAttribute('data-y', yPoint_31294);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31294
         * @param {type} xPoint_31294
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31294, xPoint_31294, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31294 + '_y' + yPoint_31294;
            }
            return 'pc_x' + xPoint_31294 + '_y' + yPoint_31294;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31294=0;yPoint_31294<this.gameFieldBorderY.length; yPoint_31294++){
                for(var xPoint_31294=0;xPoint_31294<this.gameFieldBorderX.length; xPoint_31294++){
                    map.push({y: yPoint_31294, x: xPoint_31294});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31294=-1;yPoint_31294<(this.gameFieldBorderY.length+1); yPoint_31294++){
                for(var xPoint_31294=-1;xPoint_31294<(this.gameFieldBorderX.length+1); xPoint_31294++){
                    if(!map[yPoint_31294]){
                        map[yPoint_31294] = [];
                    }
                    map[yPoint_31294][xPoint_31294] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31294 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31294 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31294, yPoint_31294) === true){
                    if(this.canPutHorizontal(map, xPoint_31294, yPoint_31294, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31294][xPoint_31294 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31294, yPoint_31294, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31294 + i][xPoint_31294] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31294, yPoint_31294){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31294][xPoint_31294] === this.CELL_EMPTY
                && map[yPoint_31294-1][xPoint_31294] === this.CELL_EMPTY
                && map[yPoint_31294-1][xPoint_31294+1] === this.CELL_EMPTY
                && map[yPoint_31294][xPoint_31294+1] === this.CELL_EMPTY
                && map[yPoint_31294+1][xPoint_31294+1] === this.CELL_EMPTY
                && map[yPoint_31294+1][xPoint_31294] === this.CELL_EMPTY
                && map[yPoint_31294+1][xPoint_31294-1] === this.CELL_EMPTY
                && map[yPoint_31294][xPoint_31294-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31294, yPoint_31294, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31294;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31294][x] === this.CELL_EMPTY
                    && map[yPoint_31294-1][x] === this.CELL_EMPTY
                    && map[yPoint_31294-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31294][x+1] === this.CELL_EMPTY
                    && map[yPoint_31294+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31294+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31294, yPoint_31294, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31294;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31294] === this.CELL_EMPTY
                    && map[y+1][xPoint_31294] === this.CELL_EMPTY
                    && map[y+1][xPoint_31294+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31294] === this.CELL_EMPTY
                    && map[y][xPoint_31294-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31294-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
