...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_644=0;yPoint_644<this.gameFieldBorderY.length; yPoint_644++){
                for(var xPoint_644=0;xPoint_644<this.gameFieldBorderX.length; xPoint_644++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_644, xPoint_644);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_644][xPoint_644] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_644, xPoint_644, 'user');
                    if(this._userShipsMap[yPoint_644][xPoint_644] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_644, xPoint_644, type){
            var id = this.getPointBlockIdByCoords(yPoint_644, xPoint_644, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_644);
                block.setAttribute('data-y', yPoint_644);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_644
         * @param {type} xPoint_644
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_644, xPoint_644, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_644 + '_y' + yPoint_644;
            }
            return 'pc_x' + xPoint_644 + '_y' + yPoint_644;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_644=0;yPoint_644<this.gameFieldBorderY.length; yPoint_644++){
                for(var xPoint_644=0;xPoint_644<this.gameFieldBorderX.length; xPoint_644++){
                    map.push({y: yPoint_644, x: xPoint_644});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_644=-1;yPoint_644<(this.gameFieldBorderY.length+1); yPoint_644++){
                for(var xPoint_644=-1;xPoint_644<(this.gameFieldBorderX.length+1); xPoint_644++){
                    if(!map[yPoint_644]){
                        map[yPoint_644] = [];
                    }
                    map[yPoint_644][xPoint_644] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_644 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_644 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_644, yPoint_644) === true){
                    if(this.canPutHorizontal(map, xPoint_644, yPoint_644, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_644][xPoint_644 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_644, yPoint_644, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_644 + i][xPoint_644] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_644, yPoint_644){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_644][xPoint_644] === this.CELL_EMPTY
                && map[yPoint_644-1][xPoint_644] === this.CELL_EMPTY
                && map[yPoint_644-1][xPoint_644+1] === this.CELL_EMPTY
                && map[yPoint_644][xPoint_644+1] === this.CELL_EMPTY
                && map[yPoint_644+1][xPoint_644+1] === this.CELL_EMPTY
                && map[yPoint_644+1][xPoint_644] === this.CELL_EMPTY
                && map[yPoint_644+1][xPoint_644-1] === this.CELL_EMPTY
                && map[yPoint_644][xPoint_644-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_644, yPoint_644, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_644;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_644][x] === this.CELL_EMPTY
                    && map[yPoint_644-1][x] === this.CELL_EMPTY
                    && map[yPoint_644-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_644][x+1] === this.CELL_EMPTY
                    && map[yPoint_644+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_644+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_644, yPoint_644, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_644;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_644] === this.CELL_EMPTY
                    && map[y+1][xPoint_644] === this.CELL_EMPTY
                    && map[y+1][xPoint_644+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_644] === this.CELL_EMPTY
                    && map[y][xPoint_644-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_644-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
