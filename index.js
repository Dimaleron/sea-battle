...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_12326=0;yPoint_12326<this.gameFieldBorderY.length; yPoint_12326++){
                for(var xPoint_12326=0;xPoint_12326<this.gameFieldBorderX.length; xPoint_12326++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_12326, xPoint_12326);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_12326][xPoint_12326] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_12326, xPoint_12326, 'user');
                    if(this._userShipsMap[yPoint_12326][xPoint_12326] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_12326, xPoint_12326, type){
            var id = this.getPointBlockIdByCoords(yPoint_12326, xPoint_12326, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_12326);
                block.setAttribute('data-y', yPoint_12326);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_12326
         * @param {type} xPoint_12326
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_12326, xPoint_12326, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_12326 + '_y' + yPoint_12326;
            }
            return 'pc_x' + xPoint_12326 + '_y' + yPoint_12326;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_12326=0;yPoint_12326<this.gameFieldBorderY.length; yPoint_12326++){
                for(var xPoint_12326=0;xPoint_12326<this.gameFieldBorderX.length; xPoint_12326++){
                    map.push({y: yPoint_12326, x: xPoint_12326});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_12326=-1;yPoint_12326<(this.gameFieldBorderY.length+1); yPoint_12326++){
                for(var xPoint_12326=-1;xPoint_12326<(this.gameFieldBorderX.length+1); xPoint_12326++){
                    if(!map[yPoint_12326]){
                        map[yPoint_12326] = [];
                    }
                    map[yPoint_12326][xPoint_12326] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_12326 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_12326 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_12326, yPoint_12326) === true){
                    if(this.canPutHorizontal(map, xPoint_12326, yPoint_12326, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12326][xPoint_12326 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_12326, yPoint_12326, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12326 + i][xPoint_12326] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_12326, yPoint_12326){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_12326][xPoint_12326] === this.CELL_EMPTY
                && map[yPoint_12326-1][xPoint_12326] === this.CELL_EMPTY
                && map[yPoint_12326-1][xPoint_12326+1] === this.CELL_EMPTY
                && map[yPoint_12326][xPoint_12326+1] === this.CELL_EMPTY
                && map[yPoint_12326+1][xPoint_12326+1] === this.CELL_EMPTY
                && map[yPoint_12326+1][xPoint_12326] === this.CELL_EMPTY
                && map[yPoint_12326+1][xPoint_12326-1] === this.CELL_EMPTY
                && map[yPoint_12326][xPoint_12326-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_12326, yPoint_12326, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_12326;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_12326][x] === this.CELL_EMPTY
                    && map[yPoint_12326-1][x] === this.CELL_EMPTY
                    && map[yPoint_12326-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12326][x+1] === this.CELL_EMPTY
                    && map[yPoint_12326+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12326+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_12326, yPoint_12326, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_12326;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_12326] === this.CELL_EMPTY
                    && map[y+1][xPoint_12326] === this.CELL_EMPTY
                    && map[y+1][xPoint_12326+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_12326] === this.CELL_EMPTY
                    && map[y][xPoint_12326-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_12326-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
