...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19913=0;yPoint_19913<this.gameFieldBorderY.length; yPoint_19913++){
                for(var xPoint_19913=0;xPoint_19913<this.gameFieldBorderX.length; xPoint_19913++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19913, xPoint_19913);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19913][xPoint_19913] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19913, xPoint_19913, 'user');
                    if(this._userShipsMap[yPoint_19913][xPoint_19913] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19913, xPoint_19913, type){
            var id = this.getPointBlockIdByCoords(yPoint_19913, xPoint_19913, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19913);
                block.setAttribute('data-y', yPoint_19913);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19913
         * @param {type} xPoint_19913
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19913, xPoint_19913, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19913 + '_y' + yPoint_19913;
            }
            return 'pc_x' + xPoint_19913 + '_y' + yPoint_19913;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19913=0;yPoint_19913<this.gameFieldBorderY.length; yPoint_19913++){
                for(var xPoint_19913=0;xPoint_19913<this.gameFieldBorderX.length; xPoint_19913++){
                    map.push({y: yPoint_19913, x: xPoint_19913});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19913=-1;yPoint_19913<(this.gameFieldBorderY.length+1); yPoint_19913++){
                for(var xPoint_19913=-1;xPoint_19913<(this.gameFieldBorderX.length+1); xPoint_19913++){
                    if(!map[yPoint_19913]){
                        map[yPoint_19913] = [];
                    }
                    map[yPoint_19913][xPoint_19913] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19913 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19913 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19913, yPoint_19913) === true){
                    if(this.canPutHorizontal(map, xPoint_19913, yPoint_19913, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19913][xPoint_19913 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19913, yPoint_19913, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19913 + i][xPoint_19913] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19913, yPoint_19913){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19913][xPoint_19913] === this.CELL_EMPTY
                && map[yPoint_19913-1][xPoint_19913] === this.CELL_EMPTY
                && map[yPoint_19913-1][xPoint_19913+1] === this.CELL_EMPTY
                && map[yPoint_19913][xPoint_19913+1] === this.CELL_EMPTY
                && map[yPoint_19913+1][xPoint_19913+1] === this.CELL_EMPTY
                && map[yPoint_19913+1][xPoint_19913] === this.CELL_EMPTY
                && map[yPoint_19913+1][xPoint_19913-1] === this.CELL_EMPTY
                && map[yPoint_19913][xPoint_19913-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19913, yPoint_19913, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19913;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19913][x] === this.CELL_EMPTY
                    && map[yPoint_19913-1][x] === this.CELL_EMPTY
                    && map[yPoint_19913-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19913][x+1] === this.CELL_EMPTY
                    && map[yPoint_19913+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19913+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19913, yPoint_19913, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19913;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19913] === this.CELL_EMPTY
                    && map[y+1][xPoint_19913] === this.CELL_EMPTY
                    && map[y+1][xPoint_19913+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19913] === this.CELL_EMPTY
                    && map[y][xPoint_19913-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19913-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
