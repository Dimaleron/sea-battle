...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3170=0;yPoint_3170<this.gameFieldBorderY.length; yPoint_3170++){
                for(var xPoint_3170=0;xPoint_3170<this.gameFieldBorderX.length; xPoint_3170++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3170, xPoint_3170);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3170][xPoint_3170] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3170, xPoint_3170, 'user');
                    if(this._userShipsMap[yPoint_3170][xPoint_3170] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3170, xPoint_3170, type){
            var id = this.getPointBlockIdByCoords(yPoint_3170, xPoint_3170, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3170);
                block.setAttribute('data-y', yPoint_3170);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3170
         * @param {type} xPoint_3170
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3170, xPoint_3170, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3170 + '_y' + yPoint_3170;
            }
            return 'pc_x' + xPoint_3170 + '_y' + yPoint_3170;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3170=0;yPoint_3170<this.gameFieldBorderY.length; yPoint_3170++){
                for(var xPoint_3170=0;xPoint_3170<this.gameFieldBorderX.length; xPoint_3170++){
                    map.push({y: yPoint_3170, x: xPoint_3170});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3170=-1;yPoint_3170<(this.gameFieldBorderY.length+1); yPoint_3170++){
                for(var xPoint_3170=-1;xPoint_3170<(this.gameFieldBorderX.length+1); xPoint_3170++){
                    if(!map[yPoint_3170]){
                        map[yPoint_3170] = [];
                    }
                    map[yPoint_3170][xPoint_3170] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3170 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3170 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3170, yPoint_3170) === true){
                    if(this.canPutHorizontal(map, xPoint_3170, yPoint_3170, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3170][xPoint_3170 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3170, yPoint_3170, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3170 + i][xPoint_3170] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3170, yPoint_3170){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3170][xPoint_3170] === this.CELL_EMPTY
                && map[yPoint_3170-1][xPoint_3170] === this.CELL_EMPTY
                && map[yPoint_3170-1][xPoint_3170+1] === this.CELL_EMPTY
                && map[yPoint_3170][xPoint_3170+1] === this.CELL_EMPTY
                && map[yPoint_3170+1][xPoint_3170+1] === this.CELL_EMPTY
                && map[yPoint_3170+1][xPoint_3170] === this.CELL_EMPTY
                && map[yPoint_3170+1][xPoint_3170-1] === this.CELL_EMPTY
                && map[yPoint_3170][xPoint_3170-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3170, yPoint_3170, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3170;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3170][x] === this.CELL_EMPTY
                    && map[yPoint_3170-1][x] === this.CELL_EMPTY
                    && map[yPoint_3170-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3170][x+1] === this.CELL_EMPTY
                    && map[yPoint_3170+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3170+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3170, yPoint_3170, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3170;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3170] === this.CELL_EMPTY
                    && map[y+1][xPoint_3170] === this.CELL_EMPTY
                    && map[y+1][xPoint_3170+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3170] === this.CELL_EMPTY
                    && map[y][xPoint_3170-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3170-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
