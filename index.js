...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32490=0;yPoint_32490<this.gameFieldBorderY.length; yPoint_32490++){
                for(var xPoint_32490=0;xPoint_32490<this.gameFieldBorderX.length; xPoint_32490++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32490, xPoint_32490);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32490][xPoint_32490] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32490, xPoint_32490, 'user');
                    if(this._userShipsMap[yPoint_32490][xPoint_32490] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32490, xPoint_32490, type){
            var id = this.getPointBlockIdByCoords(yPoint_32490, xPoint_32490, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32490);
                block.setAttribute('data-y', yPoint_32490);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32490
         * @param {type} xPoint_32490
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32490, xPoint_32490, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32490 + '_y' + yPoint_32490;
            }
            return 'pc_x' + xPoint_32490 + '_y' + yPoint_32490;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32490=0;yPoint_32490<this.gameFieldBorderY.length; yPoint_32490++){
                for(var xPoint_32490=0;xPoint_32490<this.gameFieldBorderX.length; xPoint_32490++){
                    map.push({y: yPoint_32490, x: xPoint_32490});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32490=-1;yPoint_32490<(this.gameFieldBorderY.length+1); yPoint_32490++){
                for(var xPoint_32490=-1;xPoint_32490<(this.gameFieldBorderX.length+1); xPoint_32490++){
                    if(!map[yPoint_32490]){
                        map[yPoint_32490] = [];
                    }
                    map[yPoint_32490][xPoint_32490] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32490 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32490 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32490, yPoint_32490) === true){
                    if(this.canPutHorizontal(map, xPoint_32490, yPoint_32490, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32490][xPoint_32490 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32490, yPoint_32490, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32490 + i][xPoint_32490] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32490, yPoint_32490){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32490][xPoint_32490] === this.CELL_EMPTY
                && map[yPoint_32490-1][xPoint_32490] === this.CELL_EMPTY
                && map[yPoint_32490-1][xPoint_32490+1] === this.CELL_EMPTY
                && map[yPoint_32490][xPoint_32490+1] === this.CELL_EMPTY
                && map[yPoint_32490+1][xPoint_32490+1] === this.CELL_EMPTY
                && map[yPoint_32490+1][xPoint_32490] === this.CELL_EMPTY
                && map[yPoint_32490+1][xPoint_32490-1] === this.CELL_EMPTY
                && map[yPoint_32490][xPoint_32490-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32490, yPoint_32490, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32490;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32490][x] === this.CELL_EMPTY
                    && map[yPoint_32490-1][x] === this.CELL_EMPTY
                    && map[yPoint_32490-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32490][x+1] === this.CELL_EMPTY
                    && map[yPoint_32490+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32490+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32490, yPoint_32490, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32490;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32490] === this.CELL_EMPTY
                    && map[y+1][xPoint_32490] === this.CELL_EMPTY
                    && map[y+1][xPoint_32490+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32490] === this.CELL_EMPTY
                    && map[y][xPoint_32490-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32490-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
