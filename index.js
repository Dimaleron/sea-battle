...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26789=0;yPoint_26789<this.gameFieldBorderY.length; yPoint_26789++){
                for(var xPoint_26789=0;xPoint_26789<this.gameFieldBorderX.length; xPoint_26789++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26789, xPoint_26789);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26789][xPoint_26789] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26789, xPoint_26789, 'user');
                    if(this._userShipsMap[yPoint_26789][xPoint_26789] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26789, xPoint_26789, type){
            var id = this.getPointBlockIdByCoords(yPoint_26789, xPoint_26789, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26789);
                block.setAttribute('data-y', yPoint_26789);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26789
         * @param {type} xPoint_26789
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26789, xPoint_26789, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26789 + '_y' + yPoint_26789;
            }
            return 'pc_x' + xPoint_26789 + '_y' + yPoint_26789;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26789=0;yPoint_26789<this.gameFieldBorderY.length; yPoint_26789++){
                for(var xPoint_26789=0;xPoint_26789<this.gameFieldBorderX.length; xPoint_26789++){
                    map.push({y: yPoint_26789, x: xPoint_26789});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26789=-1;yPoint_26789<(this.gameFieldBorderY.length+1); yPoint_26789++){
                for(var xPoint_26789=-1;xPoint_26789<(this.gameFieldBorderX.length+1); xPoint_26789++){
                    if(!map[yPoint_26789]){
                        map[yPoint_26789] = [];
                    }
                    map[yPoint_26789][xPoint_26789] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26789 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26789 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26789, yPoint_26789) === true){
                    if(this.canPutHorizontal(map, xPoint_26789, yPoint_26789, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26789][xPoint_26789 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26789, yPoint_26789, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26789 + i][xPoint_26789] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26789, yPoint_26789){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26789][xPoint_26789] === this.CELL_EMPTY
                && map[yPoint_26789-1][xPoint_26789] === this.CELL_EMPTY
                && map[yPoint_26789-1][xPoint_26789+1] === this.CELL_EMPTY
                && map[yPoint_26789][xPoint_26789+1] === this.CELL_EMPTY
                && map[yPoint_26789+1][xPoint_26789+1] === this.CELL_EMPTY
                && map[yPoint_26789+1][xPoint_26789] === this.CELL_EMPTY
                && map[yPoint_26789+1][xPoint_26789-1] === this.CELL_EMPTY
                && map[yPoint_26789][xPoint_26789-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26789, yPoint_26789, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26789;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26789][x] === this.CELL_EMPTY
                    && map[yPoint_26789-1][x] === this.CELL_EMPTY
                    && map[yPoint_26789-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26789][x+1] === this.CELL_EMPTY
                    && map[yPoint_26789+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26789+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26789, yPoint_26789, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26789;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26789] === this.CELL_EMPTY
                    && map[y+1][xPoint_26789] === this.CELL_EMPTY
                    && map[y+1][xPoint_26789+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26789] === this.CELL_EMPTY
                    && map[y][xPoint_26789-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26789-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
