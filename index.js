...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_684=0;yPoint_684<this.gameFieldBorderY.length; yPoint_684++){
                for(var xPoint_684=0;xPoint_684<this.gameFieldBorderX.length; xPoint_684++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_684, xPoint_684);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_684][xPoint_684] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_684, xPoint_684, 'user');
                    if(this._userShipsMap[yPoint_684][xPoint_684] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_684, xPoint_684, type){
            var id = this.getPointBlockIdByCoords(yPoint_684, xPoint_684, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_684);
                block.setAttribute('data-y', yPoint_684);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_684
         * @param {type} xPoint_684
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_684, xPoint_684, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_684 + '_y' + yPoint_684;
            }
            return 'pc_x' + xPoint_684 + '_y' + yPoint_684;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_684=0;yPoint_684<this.gameFieldBorderY.length; yPoint_684++){
                for(var xPoint_684=0;xPoint_684<this.gameFieldBorderX.length; xPoint_684++){
                    map.push({y: yPoint_684, x: xPoint_684});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_684=-1;yPoint_684<(this.gameFieldBorderY.length+1); yPoint_684++){
                for(var xPoint_684=-1;xPoint_684<(this.gameFieldBorderX.length+1); xPoint_684++){
                    if(!map[yPoint_684]){
                        map[yPoint_684] = [];
                    }
                    map[yPoint_684][xPoint_684] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_684 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_684 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_684, yPoint_684) === true){
                    if(this.canPutHorizontal(map, xPoint_684, yPoint_684, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_684][xPoint_684 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_684, yPoint_684, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_684 + i][xPoint_684] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_684, yPoint_684){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_684][xPoint_684] === this.CELL_EMPTY
                && map[yPoint_684-1][xPoint_684] === this.CELL_EMPTY
                && map[yPoint_684-1][xPoint_684+1] === this.CELL_EMPTY
                && map[yPoint_684][xPoint_684+1] === this.CELL_EMPTY
                && map[yPoint_684+1][xPoint_684+1] === this.CELL_EMPTY
                && map[yPoint_684+1][xPoint_684] === this.CELL_EMPTY
                && map[yPoint_684+1][xPoint_684-1] === this.CELL_EMPTY
                && map[yPoint_684][xPoint_684-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_684, yPoint_684, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_684;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_684][x] === this.CELL_EMPTY
                    && map[yPoint_684-1][x] === this.CELL_EMPTY
                    && map[yPoint_684-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_684][x+1] === this.CELL_EMPTY
                    && map[yPoint_684+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_684+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_684, yPoint_684, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_684;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_684] === this.CELL_EMPTY
                    && map[y+1][xPoint_684] === this.CELL_EMPTY
                    && map[y+1][xPoint_684+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_684] === this.CELL_EMPTY
                    && map[y][xPoint_684-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_684-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
