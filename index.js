...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14459=0;yPoint_14459<this.gameFieldBorderY.length; yPoint_14459++){
                for(var xPoint_14459=0;xPoint_14459<this.gameFieldBorderX.length; xPoint_14459++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14459, xPoint_14459);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14459][xPoint_14459] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14459, xPoint_14459, 'user');
                    if(this._userShipsMap[yPoint_14459][xPoint_14459] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14459, xPoint_14459, type){
            var id = this.getPointBlockIdByCoords(yPoint_14459, xPoint_14459, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14459);
                block.setAttribute('data-y', yPoint_14459);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14459
         * @param {type} xPoint_14459
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14459, xPoint_14459, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14459 + '_y' + yPoint_14459;
            }
            return 'pc_x' + xPoint_14459 + '_y' + yPoint_14459;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14459=0;yPoint_14459<this.gameFieldBorderY.length; yPoint_14459++){
                for(var xPoint_14459=0;xPoint_14459<this.gameFieldBorderX.length; xPoint_14459++){
                    map.push({y: yPoint_14459, x: xPoint_14459});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14459=-1;yPoint_14459<(this.gameFieldBorderY.length+1); yPoint_14459++){
                for(var xPoint_14459=-1;xPoint_14459<(this.gameFieldBorderX.length+1); xPoint_14459++){
                    if(!map[yPoint_14459]){
                        map[yPoint_14459] = [];
                    }
                    map[yPoint_14459][xPoint_14459] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14459 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14459 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14459, yPoint_14459) === true){
                    if(this.canPutHorizontal(map, xPoint_14459, yPoint_14459, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14459][xPoint_14459 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14459, yPoint_14459, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14459 + i][xPoint_14459] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14459, yPoint_14459){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14459][xPoint_14459] === this.CELL_EMPTY
                && map[yPoint_14459-1][xPoint_14459] === this.CELL_EMPTY
                && map[yPoint_14459-1][xPoint_14459+1] === this.CELL_EMPTY
                && map[yPoint_14459][xPoint_14459+1] === this.CELL_EMPTY
                && map[yPoint_14459+1][xPoint_14459+1] === this.CELL_EMPTY
                && map[yPoint_14459+1][xPoint_14459] === this.CELL_EMPTY
                && map[yPoint_14459+1][xPoint_14459-1] === this.CELL_EMPTY
                && map[yPoint_14459][xPoint_14459-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14459, yPoint_14459, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14459;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14459][x] === this.CELL_EMPTY
                    && map[yPoint_14459-1][x] === this.CELL_EMPTY
                    && map[yPoint_14459-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14459][x+1] === this.CELL_EMPTY
                    && map[yPoint_14459+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14459+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14459, yPoint_14459, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14459;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14459] === this.CELL_EMPTY
                    && map[y+1][xPoint_14459] === this.CELL_EMPTY
                    && map[y+1][xPoint_14459+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14459] === this.CELL_EMPTY
                    && map[y][xPoint_14459-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14459-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
