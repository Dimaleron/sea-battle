...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8300=0;yPoint_8300<this.gameFieldBorderY.length; yPoint_8300++){
                for(var xPoint_8300=0;xPoint_8300<this.gameFieldBorderX.length; xPoint_8300++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8300, xPoint_8300);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8300][xPoint_8300] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8300, xPoint_8300, 'user');
                    if(this._userShipsMap[yPoint_8300][xPoint_8300] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8300, xPoint_8300, type){
            var id = this.getPointBlockIdByCoords(yPoint_8300, xPoint_8300, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8300);
                block.setAttribute('data-y', yPoint_8300);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8300
         * @param {type} xPoint_8300
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8300, xPoint_8300, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8300 + '_y' + yPoint_8300;
            }
            return 'pc_x' + xPoint_8300 + '_y' + yPoint_8300;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8300=0;yPoint_8300<this.gameFieldBorderY.length; yPoint_8300++){
                for(var xPoint_8300=0;xPoint_8300<this.gameFieldBorderX.length; xPoint_8300++){
                    map.push({y: yPoint_8300, x: xPoint_8300});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8300=-1;yPoint_8300<(this.gameFieldBorderY.length+1); yPoint_8300++){
                for(var xPoint_8300=-1;xPoint_8300<(this.gameFieldBorderX.length+1); xPoint_8300++){
                    if(!map[yPoint_8300]){
                        map[yPoint_8300] = [];
                    }
                    map[yPoint_8300][xPoint_8300] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8300 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8300 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8300, yPoint_8300) === true){
                    if(this.canPutHorizontal(map, xPoint_8300, yPoint_8300, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8300][xPoint_8300 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8300, yPoint_8300, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8300 + i][xPoint_8300] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8300, yPoint_8300){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8300][xPoint_8300] === this.CELL_EMPTY
                && map[yPoint_8300-1][xPoint_8300] === this.CELL_EMPTY
                && map[yPoint_8300-1][xPoint_8300+1] === this.CELL_EMPTY
                && map[yPoint_8300][xPoint_8300+1] === this.CELL_EMPTY
                && map[yPoint_8300+1][xPoint_8300+1] === this.CELL_EMPTY
                && map[yPoint_8300+1][xPoint_8300] === this.CELL_EMPTY
                && map[yPoint_8300+1][xPoint_8300-1] === this.CELL_EMPTY
                && map[yPoint_8300][xPoint_8300-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8300, yPoint_8300, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8300;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8300][x] === this.CELL_EMPTY
                    && map[yPoint_8300-1][x] === this.CELL_EMPTY
                    && map[yPoint_8300-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8300][x+1] === this.CELL_EMPTY
                    && map[yPoint_8300+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8300+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8300, yPoint_8300, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8300;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8300] === this.CELL_EMPTY
                    && map[y+1][xPoint_8300] === this.CELL_EMPTY
                    && map[y+1][xPoint_8300+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8300] === this.CELL_EMPTY
                    && map[y][xPoint_8300-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8300-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
