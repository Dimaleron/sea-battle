...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15132=0;yPoint_15132<this.gameFieldBorderY.length; yPoint_15132++){
                for(var xPoint_15132=0;xPoint_15132<this.gameFieldBorderX.length; xPoint_15132++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15132, xPoint_15132);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15132][xPoint_15132] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15132, xPoint_15132, 'user');
                    if(this._userShipsMap[yPoint_15132][xPoint_15132] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15132, xPoint_15132, type){
            var id = this.getPointBlockIdByCoords(yPoint_15132, xPoint_15132, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15132);
                block.setAttribute('data-y', yPoint_15132);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15132
         * @param {type} xPoint_15132
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15132, xPoint_15132, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15132 + '_y' + yPoint_15132;
            }
            return 'pc_x' + xPoint_15132 + '_y' + yPoint_15132;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15132=0;yPoint_15132<this.gameFieldBorderY.length; yPoint_15132++){
                for(var xPoint_15132=0;xPoint_15132<this.gameFieldBorderX.length; xPoint_15132++){
                    map.push({y: yPoint_15132, x: xPoint_15132});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15132=-1;yPoint_15132<(this.gameFieldBorderY.length+1); yPoint_15132++){
                for(var xPoint_15132=-1;xPoint_15132<(this.gameFieldBorderX.length+1); xPoint_15132++){
                    if(!map[yPoint_15132]){
                        map[yPoint_15132] = [];
                    }
                    map[yPoint_15132][xPoint_15132] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15132 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15132 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15132, yPoint_15132) === true){
                    if(this.canPutHorizontal(map, xPoint_15132, yPoint_15132, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15132][xPoint_15132 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15132, yPoint_15132, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15132 + i][xPoint_15132] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15132, yPoint_15132){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15132][xPoint_15132] === this.CELL_EMPTY
                && map[yPoint_15132-1][xPoint_15132] === this.CELL_EMPTY
                && map[yPoint_15132-1][xPoint_15132+1] === this.CELL_EMPTY
                && map[yPoint_15132][xPoint_15132+1] === this.CELL_EMPTY
                && map[yPoint_15132+1][xPoint_15132+1] === this.CELL_EMPTY
                && map[yPoint_15132+1][xPoint_15132] === this.CELL_EMPTY
                && map[yPoint_15132+1][xPoint_15132-1] === this.CELL_EMPTY
                && map[yPoint_15132][xPoint_15132-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15132, yPoint_15132, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15132;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15132][x] === this.CELL_EMPTY
                    && map[yPoint_15132-1][x] === this.CELL_EMPTY
                    && map[yPoint_15132-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15132][x+1] === this.CELL_EMPTY
                    && map[yPoint_15132+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15132+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15132, yPoint_15132, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15132;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15132] === this.CELL_EMPTY
                    && map[y+1][xPoint_15132] === this.CELL_EMPTY
                    && map[y+1][xPoint_15132+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15132] === this.CELL_EMPTY
                    && map[y][xPoint_15132-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15132-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
