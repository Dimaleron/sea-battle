...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13419=0;yPoint_13419<this.gameFieldBorderY.length; yPoint_13419++){
                for(var xPoint_13419=0;xPoint_13419<this.gameFieldBorderX.length; xPoint_13419++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13419, xPoint_13419);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13419][xPoint_13419] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13419, xPoint_13419, 'user');
                    if(this._userShipsMap[yPoint_13419][xPoint_13419] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13419, xPoint_13419, type){
            var id = this.getPointBlockIdByCoords(yPoint_13419, xPoint_13419, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13419);
                block.setAttribute('data-y', yPoint_13419);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13419
         * @param {type} xPoint_13419
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13419, xPoint_13419, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13419 + '_y' + yPoint_13419;
            }
            return 'pc_x' + xPoint_13419 + '_y' + yPoint_13419;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13419=0;yPoint_13419<this.gameFieldBorderY.length; yPoint_13419++){
                for(var xPoint_13419=0;xPoint_13419<this.gameFieldBorderX.length; xPoint_13419++){
                    map.push({y: yPoint_13419, x: xPoint_13419});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13419=-1;yPoint_13419<(this.gameFieldBorderY.length+1); yPoint_13419++){
                for(var xPoint_13419=-1;xPoint_13419<(this.gameFieldBorderX.length+1); xPoint_13419++){
                    if(!map[yPoint_13419]){
                        map[yPoint_13419] = [];
                    }
                    map[yPoint_13419][xPoint_13419] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13419 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13419 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13419, yPoint_13419) === true){
                    if(this.canPutHorizontal(map, xPoint_13419, yPoint_13419, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13419][xPoint_13419 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13419, yPoint_13419, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13419 + i][xPoint_13419] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13419, yPoint_13419){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13419][xPoint_13419] === this.CELL_EMPTY
                && map[yPoint_13419-1][xPoint_13419] === this.CELL_EMPTY
                && map[yPoint_13419-1][xPoint_13419+1] === this.CELL_EMPTY
                && map[yPoint_13419][xPoint_13419+1] === this.CELL_EMPTY
                && map[yPoint_13419+1][xPoint_13419+1] === this.CELL_EMPTY
                && map[yPoint_13419+1][xPoint_13419] === this.CELL_EMPTY
                && map[yPoint_13419+1][xPoint_13419-1] === this.CELL_EMPTY
                && map[yPoint_13419][xPoint_13419-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13419, yPoint_13419, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13419;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13419][x] === this.CELL_EMPTY
                    && map[yPoint_13419-1][x] === this.CELL_EMPTY
                    && map[yPoint_13419-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13419][x+1] === this.CELL_EMPTY
                    && map[yPoint_13419+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13419+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13419, yPoint_13419, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13419;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13419] === this.CELL_EMPTY
                    && map[y+1][xPoint_13419] === this.CELL_EMPTY
                    && map[y+1][xPoint_13419+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13419] === this.CELL_EMPTY
                    && map[y][xPoint_13419-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13419-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
