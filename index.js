...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18517=0;yPoint_18517<this.gameFieldBorderY.length; yPoint_18517++){
                for(var xPoint_18517=0;xPoint_18517<this.gameFieldBorderX.length; xPoint_18517++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18517, xPoint_18517);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18517][xPoint_18517] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18517, xPoint_18517, 'user');
                    if(this._userShipsMap[yPoint_18517][xPoint_18517] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18517, xPoint_18517, type){
            var id = this.getPointBlockIdByCoords(yPoint_18517, xPoint_18517, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18517);
                block.setAttribute('data-y', yPoint_18517);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18517
         * @param {type} xPoint_18517
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18517, xPoint_18517, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18517 + '_y' + yPoint_18517;
            }
            return 'pc_x' + xPoint_18517 + '_y' + yPoint_18517;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18517=0;yPoint_18517<this.gameFieldBorderY.length; yPoint_18517++){
                for(var xPoint_18517=0;xPoint_18517<this.gameFieldBorderX.length; xPoint_18517++){
                    map.push({y: yPoint_18517, x: xPoint_18517});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18517=-1;yPoint_18517<(this.gameFieldBorderY.length+1); yPoint_18517++){
                for(var xPoint_18517=-1;xPoint_18517<(this.gameFieldBorderX.length+1); xPoint_18517++){
                    if(!map[yPoint_18517]){
                        map[yPoint_18517] = [];
                    }
                    map[yPoint_18517][xPoint_18517] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18517 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18517 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18517, yPoint_18517) === true){
                    if(this.canPutHorizontal(map, xPoint_18517, yPoint_18517, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18517][xPoint_18517 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18517, yPoint_18517, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18517 + i][xPoint_18517] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18517, yPoint_18517){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18517][xPoint_18517] === this.CELL_EMPTY
                && map[yPoint_18517-1][xPoint_18517] === this.CELL_EMPTY
                && map[yPoint_18517-1][xPoint_18517+1] === this.CELL_EMPTY
                && map[yPoint_18517][xPoint_18517+1] === this.CELL_EMPTY
                && map[yPoint_18517+1][xPoint_18517+1] === this.CELL_EMPTY
                && map[yPoint_18517+1][xPoint_18517] === this.CELL_EMPTY
                && map[yPoint_18517+1][xPoint_18517-1] === this.CELL_EMPTY
                && map[yPoint_18517][xPoint_18517-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18517, yPoint_18517, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18517;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18517][x] === this.CELL_EMPTY
                    && map[yPoint_18517-1][x] === this.CELL_EMPTY
                    && map[yPoint_18517-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18517][x+1] === this.CELL_EMPTY
                    && map[yPoint_18517+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18517+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18517, yPoint_18517, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18517;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18517] === this.CELL_EMPTY
                    && map[y+1][xPoint_18517] === this.CELL_EMPTY
                    && map[y+1][xPoint_18517+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18517] === this.CELL_EMPTY
                    && map[y][xPoint_18517-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18517-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
