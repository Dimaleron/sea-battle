...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11968=0;yPoint_11968<this.gameFieldBorderY.length; yPoint_11968++){
                for(var xPoint_11968=0;xPoint_11968<this.gameFieldBorderX.length; xPoint_11968++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11968, xPoint_11968);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11968][xPoint_11968] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11968, xPoint_11968, 'user');
                    if(this._userShipsMap[yPoint_11968][xPoint_11968] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11968, xPoint_11968, type){
            var id = this.getPointBlockIdByCoords(yPoint_11968, xPoint_11968, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11968);
                block.setAttribute('data-y', yPoint_11968);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11968
         * @param {type} xPoint_11968
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11968, xPoint_11968, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11968 + '_y' + yPoint_11968;
            }
            return 'pc_x' + xPoint_11968 + '_y' + yPoint_11968;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11968=0;yPoint_11968<this.gameFieldBorderY.length; yPoint_11968++){
                for(var xPoint_11968=0;xPoint_11968<this.gameFieldBorderX.length; xPoint_11968++){
                    map.push({y: yPoint_11968, x: xPoint_11968});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11968=-1;yPoint_11968<(this.gameFieldBorderY.length+1); yPoint_11968++){
                for(var xPoint_11968=-1;xPoint_11968<(this.gameFieldBorderX.length+1); xPoint_11968++){
                    if(!map[yPoint_11968]){
                        map[yPoint_11968] = [];
                    }
                    map[yPoint_11968][xPoint_11968] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11968 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11968 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11968, yPoint_11968) === true){
                    if(this.canPutHorizontal(map, xPoint_11968, yPoint_11968, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11968][xPoint_11968 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11968, yPoint_11968, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11968 + i][xPoint_11968] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11968, yPoint_11968){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11968][xPoint_11968] === this.CELL_EMPTY
                && map[yPoint_11968-1][xPoint_11968] === this.CELL_EMPTY
                && map[yPoint_11968-1][xPoint_11968+1] === this.CELL_EMPTY
                && map[yPoint_11968][xPoint_11968+1] === this.CELL_EMPTY
                && map[yPoint_11968+1][xPoint_11968+1] === this.CELL_EMPTY
                && map[yPoint_11968+1][xPoint_11968] === this.CELL_EMPTY
                && map[yPoint_11968+1][xPoint_11968-1] === this.CELL_EMPTY
                && map[yPoint_11968][xPoint_11968-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11968, yPoint_11968, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11968;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11968][x] === this.CELL_EMPTY
                    && map[yPoint_11968-1][x] === this.CELL_EMPTY
                    && map[yPoint_11968-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11968][x+1] === this.CELL_EMPTY
                    && map[yPoint_11968+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11968+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11968, yPoint_11968, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11968;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11968] === this.CELL_EMPTY
                    && map[y+1][xPoint_11968] === this.CELL_EMPTY
                    && map[y+1][xPoint_11968+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11968] === this.CELL_EMPTY
                    && map[y][xPoint_11968-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11968-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
