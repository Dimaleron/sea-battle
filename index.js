...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_656=0;yPoint_656<this.gameFieldBorderY.length; yPoint_656++){
                for(var xPoint_656=0;xPoint_656<this.gameFieldBorderX.length; xPoint_656++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_656, xPoint_656);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_656][xPoint_656] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_656, xPoint_656, 'user');
                    if(this._userShipsMap[yPoint_656][xPoint_656] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_656, xPoint_656, type){
            var id = this.getPointBlockIdByCoords(yPoint_656, xPoint_656, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_656);
                block.setAttribute('data-y', yPoint_656);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_656
         * @param {type} xPoint_656
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_656, xPoint_656, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_656 + '_y' + yPoint_656;
            }
            return 'pc_x' + xPoint_656 + '_y' + yPoint_656;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_656=0;yPoint_656<this.gameFieldBorderY.length; yPoint_656++){
                for(var xPoint_656=0;xPoint_656<this.gameFieldBorderX.length; xPoint_656++){
                    map.push({y: yPoint_656, x: xPoint_656});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_656=-1;yPoint_656<(this.gameFieldBorderY.length+1); yPoint_656++){
                for(var xPoint_656=-1;xPoint_656<(this.gameFieldBorderX.length+1); xPoint_656++){
                    if(!map[yPoint_656]){
                        map[yPoint_656] = [];
                    }
                    map[yPoint_656][xPoint_656] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_656 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_656 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_656, yPoint_656) === true){
                    if(this.canPutHorizontal(map, xPoint_656, yPoint_656, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_656][xPoint_656 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_656, yPoint_656, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_656 + i][xPoint_656] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_656, yPoint_656){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_656][xPoint_656] === this.CELL_EMPTY
                && map[yPoint_656-1][xPoint_656] === this.CELL_EMPTY
                && map[yPoint_656-1][xPoint_656+1] === this.CELL_EMPTY
                && map[yPoint_656][xPoint_656+1] === this.CELL_EMPTY
                && map[yPoint_656+1][xPoint_656+1] === this.CELL_EMPTY
                && map[yPoint_656+1][xPoint_656] === this.CELL_EMPTY
                && map[yPoint_656+1][xPoint_656-1] === this.CELL_EMPTY
                && map[yPoint_656][xPoint_656-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_656, yPoint_656, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_656;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_656][x] === this.CELL_EMPTY
                    && map[yPoint_656-1][x] === this.CELL_EMPTY
                    && map[yPoint_656-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_656][x+1] === this.CELL_EMPTY
                    && map[yPoint_656+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_656+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_656, yPoint_656, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_656;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_656] === this.CELL_EMPTY
                    && map[y+1][xPoint_656] === this.CELL_EMPTY
                    && map[y+1][xPoint_656+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_656] === this.CELL_EMPTY
                    && map[y][xPoint_656-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_656-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
