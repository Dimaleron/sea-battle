...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18742=0;yPoint_18742<this.gameFieldBorderY.length; yPoint_18742++){
                for(var xPoint_18742=0;xPoint_18742<this.gameFieldBorderX.length; xPoint_18742++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18742, xPoint_18742);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18742][xPoint_18742] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18742, xPoint_18742, 'user');
                    if(this._userShipsMap[yPoint_18742][xPoint_18742] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18742, xPoint_18742, type){
            var id = this.getPointBlockIdByCoords(yPoint_18742, xPoint_18742, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18742);
                block.setAttribute('data-y', yPoint_18742);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18742
         * @param {type} xPoint_18742
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18742, xPoint_18742, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18742 + '_y' + yPoint_18742;
            }
            return 'pc_x' + xPoint_18742 + '_y' + yPoint_18742;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18742=0;yPoint_18742<this.gameFieldBorderY.length; yPoint_18742++){
                for(var xPoint_18742=0;xPoint_18742<this.gameFieldBorderX.length; xPoint_18742++){
                    map.push({y: yPoint_18742, x: xPoint_18742});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18742=-1;yPoint_18742<(this.gameFieldBorderY.length+1); yPoint_18742++){
                for(var xPoint_18742=-1;xPoint_18742<(this.gameFieldBorderX.length+1); xPoint_18742++){
                    if(!map[yPoint_18742]){
                        map[yPoint_18742] = [];
                    }
                    map[yPoint_18742][xPoint_18742] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18742 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18742 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18742, yPoint_18742) === true){
                    if(this.canPutHorizontal(map, xPoint_18742, yPoint_18742, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18742][xPoint_18742 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18742, yPoint_18742, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18742 + i][xPoint_18742] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18742, yPoint_18742){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18742][xPoint_18742] === this.CELL_EMPTY
                && map[yPoint_18742-1][xPoint_18742] === this.CELL_EMPTY
                && map[yPoint_18742-1][xPoint_18742+1] === this.CELL_EMPTY
                && map[yPoint_18742][xPoint_18742+1] === this.CELL_EMPTY
                && map[yPoint_18742+1][xPoint_18742+1] === this.CELL_EMPTY
                && map[yPoint_18742+1][xPoint_18742] === this.CELL_EMPTY
                && map[yPoint_18742+1][xPoint_18742-1] === this.CELL_EMPTY
                && map[yPoint_18742][xPoint_18742-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18742, yPoint_18742, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18742;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18742][x] === this.CELL_EMPTY
                    && map[yPoint_18742-1][x] === this.CELL_EMPTY
                    && map[yPoint_18742-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18742][x+1] === this.CELL_EMPTY
                    && map[yPoint_18742+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18742+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18742, yPoint_18742, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18742;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18742] === this.CELL_EMPTY
                    && map[y+1][xPoint_18742] === this.CELL_EMPTY
                    && map[y+1][xPoint_18742+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18742] === this.CELL_EMPTY
                    && map[y][xPoint_18742-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18742-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
