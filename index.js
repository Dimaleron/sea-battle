...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_188=0;yPoint_188<this.gameFieldBorderY.length; yPoint_188++){
                for(var xPoint_188=0;xPoint_188<this.gameFieldBorderX.length; xPoint_188++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_188, xPoint_188);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_188][xPoint_188] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_188, xPoint_188, 'user');
                    if(this._userShipsMap[yPoint_188][xPoint_188] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_188, xPoint_188, type){
            var id = this.getPointBlockIdByCoords(yPoint_188, xPoint_188, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_188);
                block.setAttribute('data-y', yPoint_188);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_188
         * @param {type} xPoint_188
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_188, xPoint_188, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_188 + '_y' + yPoint_188;
            }
            return 'pc_x' + xPoint_188 + '_y' + yPoint_188;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_188=0;yPoint_188<this.gameFieldBorderY.length; yPoint_188++){
                for(var xPoint_188=0;xPoint_188<this.gameFieldBorderX.length; xPoint_188++){
                    map.push({y: yPoint_188, x: xPoint_188});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_188=-1;yPoint_188<(this.gameFieldBorderY.length+1); yPoint_188++){
                for(var xPoint_188=-1;xPoint_188<(this.gameFieldBorderX.length+1); xPoint_188++){
                    if(!map[yPoint_188]){
                        map[yPoint_188] = [];
                    }
                    map[yPoint_188][xPoint_188] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_188 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_188 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_188, yPoint_188) === true){
                    if(this.canPutHorizontal(map, xPoint_188, yPoint_188, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_188][xPoint_188 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_188, yPoint_188, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_188 + i][xPoint_188] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_188, yPoint_188){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_188][xPoint_188] === this.CELL_EMPTY
                && map[yPoint_188-1][xPoint_188] === this.CELL_EMPTY
                && map[yPoint_188-1][xPoint_188+1] === this.CELL_EMPTY
                && map[yPoint_188][xPoint_188+1] === this.CELL_EMPTY
                && map[yPoint_188+1][xPoint_188+1] === this.CELL_EMPTY
                && map[yPoint_188+1][xPoint_188] === this.CELL_EMPTY
                && map[yPoint_188+1][xPoint_188-1] === this.CELL_EMPTY
                && map[yPoint_188][xPoint_188-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_188, yPoint_188, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_188;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_188][x] === this.CELL_EMPTY
                    && map[yPoint_188-1][x] === this.CELL_EMPTY
                    && map[yPoint_188-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_188][x+1] === this.CELL_EMPTY
                    && map[yPoint_188+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_188+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_188, yPoint_188, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_188;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_188] === this.CELL_EMPTY
                    && map[y+1][xPoint_188] === this.CELL_EMPTY
                    && map[y+1][xPoint_188+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_188] === this.CELL_EMPTY
                    && map[y][xPoint_188-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_188-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
