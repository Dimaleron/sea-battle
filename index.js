...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14818=0;yPoint_14818<this.gameFieldBorderY.length; yPoint_14818++){
                for(var xPoint_14818=0;xPoint_14818<this.gameFieldBorderX.length; xPoint_14818++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14818, xPoint_14818);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14818][xPoint_14818] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14818, xPoint_14818, 'user');
                    if(this._userShipsMap[yPoint_14818][xPoint_14818] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14818, xPoint_14818, type){
            var id = this.getPointBlockIdByCoords(yPoint_14818, xPoint_14818, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14818);
                block.setAttribute('data-y', yPoint_14818);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14818
         * @param {type} xPoint_14818
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14818, xPoint_14818, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14818 + '_y' + yPoint_14818;
            }
            return 'pc_x' + xPoint_14818 + '_y' + yPoint_14818;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14818=0;yPoint_14818<this.gameFieldBorderY.length; yPoint_14818++){
                for(var xPoint_14818=0;xPoint_14818<this.gameFieldBorderX.length; xPoint_14818++){
                    map.push({y: yPoint_14818, x: xPoint_14818});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14818=-1;yPoint_14818<(this.gameFieldBorderY.length+1); yPoint_14818++){
                for(var xPoint_14818=-1;xPoint_14818<(this.gameFieldBorderX.length+1); xPoint_14818++){
                    if(!map[yPoint_14818]){
                        map[yPoint_14818] = [];
                    }
                    map[yPoint_14818][xPoint_14818] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14818 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14818 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14818, yPoint_14818) === true){
                    if(this.canPutHorizontal(map, xPoint_14818, yPoint_14818, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14818][xPoint_14818 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14818, yPoint_14818, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14818 + i][xPoint_14818] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14818, yPoint_14818){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14818][xPoint_14818] === this.CELL_EMPTY
                && map[yPoint_14818-1][xPoint_14818] === this.CELL_EMPTY
                && map[yPoint_14818-1][xPoint_14818+1] === this.CELL_EMPTY
                && map[yPoint_14818][xPoint_14818+1] === this.CELL_EMPTY
                && map[yPoint_14818+1][xPoint_14818+1] === this.CELL_EMPTY
                && map[yPoint_14818+1][xPoint_14818] === this.CELL_EMPTY
                && map[yPoint_14818+1][xPoint_14818-1] === this.CELL_EMPTY
                && map[yPoint_14818][xPoint_14818-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14818, yPoint_14818, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14818;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14818][x] === this.CELL_EMPTY
                    && map[yPoint_14818-1][x] === this.CELL_EMPTY
                    && map[yPoint_14818-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14818][x+1] === this.CELL_EMPTY
                    && map[yPoint_14818+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14818+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14818, yPoint_14818, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14818;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14818] === this.CELL_EMPTY
                    && map[y+1][xPoint_14818] === this.CELL_EMPTY
                    && map[y+1][xPoint_14818+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14818] === this.CELL_EMPTY
                    && map[y][xPoint_14818-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14818-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
