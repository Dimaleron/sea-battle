...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8727=0;yPoint_8727<this.gameFieldBorderY.length; yPoint_8727++){
                for(var xPoint_8727=0;xPoint_8727<this.gameFieldBorderX.length; xPoint_8727++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8727, xPoint_8727);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8727][xPoint_8727] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8727, xPoint_8727, 'user');
                    if(this._userShipsMap[yPoint_8727][xPoint_8727] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8727, xPoint_8727, type){
            var id = this.getPointBlockIdByCoords(yPoint_8727, xPoint_8727, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8727);
                block.setAttribute('data-y', yPoint_8727);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8727
         * @param {type} xPoint_8727
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8727, xPoint_8727, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8727 + '_y' + yPoint_8727;
            }
            return 'pc_x' + xPoint_8727 + '_y' + yPoint_8727;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8727=0;yPoint_8727<this.gameFieldBorderY.length; yPoint_8727++){
                for(var xPoint_8727=0;xPoint_8727<this.gameFieldBorderX.length; xPoint_8727++){
                    map.push({y: yPoint_8727, x: xPoint_8727});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8727=-1;yPoint_8727<(this.gameFieldBorderY.length+1); yPoint_8727++){
                for(var xPoint_8727=-1;xPoint_8727<(this.gameFieldBorderX.length+1); xPoint_8727++){
                    if(!map[yPoint_8727]){
                        map[yPoint_8727] = [];
                    }
                    map[yPoint_8727][xPoint_8727] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8727 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8727 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8727, yPoint_8727) === true){
                    if(this.canPutHorizontal(map, xPoint_8727, yPoint_8727, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8727][xPoint_8727 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8727, yPoint_8727, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8727 + i][xPoint_8727] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8727, yPoint_8727){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8727][xPoint_8727] === this.CELL_EMPTY
                && map[yPoint_8727-1][xPoint_8727] === this.CELL_EMPTY
                && map[yPoint_8727-1][xPoint_8727+1] === this.CELL_EMPTY
                && map[yPoint_8727][xPoint_8727+1] === this.CELL_EMPTY
                && map[yPoint_8727+1][xPoint_8727+1] === this.CELL_EMPTY
                && map[yPoint_8727+1][xPoint_8727] === this.CELL_EMPTY
                && map[yPoint_8727+1][xPoint_8727-1] === this.CELL_EMPTY
                && map[yPoint_8727][xPoint_8727-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8727, yPoint_8727, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8727;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8727][x] === this.CELL_EMPTY
                    && map[yPoint_8727-1][x] === this.CELL_EMPTY
                    && map[yPoint_8727-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8727][x+1] === this.CELL_EMPTY
                    && map[yPoint_8727+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8727+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8727, yPoint_8727, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8727;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8727] === this.CELL_EMPTY
                    && map[y+1][xPoint_8727] === this.CELL_EMPTY
                    && map[y+1][xPoint_8727+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8727] === this.CELL_EMPTY
                    && map[y][xPoint_8727-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8727-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
