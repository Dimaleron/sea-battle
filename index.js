...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23432=0;yPoint_23432<this.gameFieldBorderY.length; yPoint_23432++){
                for(var xPoint_23432=0;xPoint_23432<this.gameFieldBorderX.length; xPoint_23432++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23432, xPoint_23432);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23432][xPoint_23432] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23432, xPoint_23432, 'user');
                    if(this._userShipsMap[yPoint_23432][xPoint_23432] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23432, xPoint_23432, type){
            var id = this.getPointBlockIdByCoords(yPoint_23432, xPoint_23432, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23432);
                block.setAttribute('data-y', yPoint_23432);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23432
         * @param {type} xPoint_23432
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23432, xPoint_23432, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23432 + '_y' + yPoint_23432;
            }
            return 'pc_x' + xPoint_23432 + '_y' + yPoint_23432;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23432=0;yPoint_23432<this.gameFieldBorderY.length; yPoint_23432++){
                for(var xPoint_23432=0;xPoint_23432<this.gameFieldBorderX.length; xPoint_23432++){
                    map.push({y: yPoint_23432, x: xPoint_23432});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23432=-1;yPoint_23432<(this.gameFieldBorderY.length+1); yPoint_23432++){
                for(var xPoint_23432=-1;xPoint_23432<(this.gameFieldBorderX.length+1); xPoint_23432++){
                    if(!map[yPoint_23432]){
                        map[yPoint_23432] = [];
                    }
                    map[yPoint_23432][xPoint_23432] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23432 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23432 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23432, yPoint_23432) === true){
                    if(this.canPutHorizontal(map, xPoint_23432, yPoint_23432, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23432][xPoint_23432 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23432, yPoint_23432, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23432 + i][xPoint_23432] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23432, yPoint_23432){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23432][xPoint_23432] === this.CELL_EMPTY
                && map[yPoint_23432-1][xPoint_23432] === this.CELL_EMPTY
                && map[yPoint_23432-1][xPoint_23432+1] === this.CELL_EMPTY
                && map[yPoint_23432][xPoint_23432+1] === this.CELL_EMPTY
                && map[yPoint_23432+1][xPoint_23432+1] === this.CELL_EMPTY
                && map[yPoint_23432+1][xPoint_23432] === this.CELL_EMPTY
                && map[yPoint_23432+1][xPoint_23432-1] === this.CELL_EMPTY
                && map[yPoint_23432][xPoint_23432-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23432, yPoint_23432, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23432;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23432][x] === this.CELL_EMPTY
                    && map[yPoint_23432-1][x] === this.CELL_EMPTY
                    && map[yPoint_23432-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23432][x+1] === this.CELL_EMPTY
                    && map[yPoint_23432+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23432+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23432, yPoint_23432, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23432;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23432] === this.CELL_EMPTY
                    && map[y+1][xPoint_23432] === this.CELL_EMPTY
                    && map[y+1][xPoint_23432+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23432] === this.CELL_EMPTY
                    && map[y][xPoint_23432-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23432-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
