...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17542=0;yPoint_17542<this.gameFieldBorderY.length; yPoint_17542++){
                for(var xPoint_17542=0;xPoint_17542<this.gameFieldBorderX.length; xPoint_17542++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17542, xPoint_17542);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17542][xPoint_17542] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17542, xPoint_17542, 'user');
                    if(this._userShipsMap[yPoint_17542][xPoint_17542] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17542, xPoint_17542, type){
            var id = this.getPointBlockIdByCoords(yPoint_17542, xPoint_17542, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17542);
                block.setAttribute('data-y', yPoint_17542);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17542
         * @param {type} xPoint_17542
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17542, xPoint_17542, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17542 + '_y' + yPoint_17542;
            }
            return 'pc_x' + xPoint_17542 + '_y' + yPoint_17542;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17542=0;yPoint_17542<this.gameFieldBorderY.length; yPoint_17542++){
                for(var xPoint_17542=0;xPoint_17542<this.gameFieldBorderX.length; xPoint_17542++){
                    map.push({y: yPoint_17542, x: xPoint_17542});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17542=-1;yPoint_17542<(this.gameFieldBorderY.length+1); yPoint_17542++){
                for(var xPoint_17542=-1;xPoint_17542<(this.gameFieldBorderX.length+1); xPoint_17542++){
                    if(!map[yPoint_17542]){
                        map[yPoint_17542] = [];
                    }
                    map[yPoint_17542][xPoint_17542] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17542 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17542 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17542, yPoint_17542) === true){
                    if(this.canPutHorizontal(map, xPoint_17542, yPoint_17542, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17542][xPoint_17542 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17542, yPoint_17542, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17542 + i][xPoint_17542] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17542, yPoint_17542){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17542][xPoint_17542] === this.CELL_EMPTY
                && map[yPoint_17542-1][xPoint_17542] === this.CELL_EMPTY
                && map[yPoint_17542-1][xPoint_17542+1] === this.CELL_EMPTY
                && map[yPoint_17542][xPoint_17542+1] === this.CELL_EMPTY
                && map[yPoint_17542+1][xPoint_17542+1] === this.CELL_EMPTY
                && map[yPoint_17542+1][xPoint_17542] === this.CELL_EMPTY
                && map[yPoint_17542+1][xPoint_17542-1] === this.CELL_EMPTY
                && map[yPoint_17542][xPoint_17542-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17542, yPoint_17542, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17542;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17542][x] === this.CELL_EMPTY
                    && map[yPoint_17542-1][x] === this.CELL_EMPTY
                    && map[yPoint_17542-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17542][x+1] === this.CELL_EMPTY
                    && map[yPoint_17542+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17542+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17542, yPoint_17542, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17542;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17542] === this.CELL_EMPTY
                    && map[y+1][xPoint_17542] === this.CELL_EMPTY
                    && map[y+1][xPoint_17542+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17542] === this.CELL_EMPTY
                    && map[y][xPoint_17542-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17542-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
