...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1139=0;yPoint_1139<this.gameFieldBorderY.length; yPoint_1139++){
                for(var xPoint_1139=0;xPoint_1139<this.gameFieldBorderX.length; xPoint_1139++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1139, xPoint_1139);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1139][xPoint_1139] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1139, xPoint_1139, 'user');
                    if(this._userShipsMap[yPoint_1139][xPoint_1139] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1139, xPoint_1139, type){
            var id = this.getPointBlockIdByCoords(yPoint_1139, xPoint_1139, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1139);
                block.setAttribute('data-y', yPoint_1139);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1139
         * @param {type} xPoint_1139
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1139, xPoint_1139, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1139 + '_y' + yPoint_1139;
            }
            return 'pc_x' + xPoint_1139 + '_y' + yPoint_1139;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1139=0;yPoint_1139<this.gameFieldBorderY.length; yPoint_1139++){
                for(var xPoint_1139=0;xPoint_1139<this.gameFieldBorderX.length; xPoint_1139++){
                    map.push({y: yPoint_1139, x: xPoint_1139});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1139=-1;yPoint_1139<(this.gameFieldBorderY.length+1); yPoint_1139++){
                for(var xPoint_1139=-1;xPoint_1139<(this.gameFieldBorderX.length+1); xPoint_1139++){
                    if(!map[yPoint_1139]){
                        map[yPoint_1139] = [];
                    }
                    map[yPoint_1139][xPoint_1139] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1139 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1139 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1139, yPoint_1139) === true){
                    if(this.canPutHorizontal(map, xPoint_1139, yPoint_1139, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1139][xPoint_1139 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1139, yPoint_1139, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1139 + i][xPoint_1139] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1139, yPoint_1139){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1139][xPoint_1139] === this.CELL_EMPTY
                && map[yPoint_1139-1][xPoint_1139] === this.CELL_EMPTY
                && map[yPoint_1139-1][xPoint_1139+1] === this.CELL_EMPTY
                && map[yPoint_1139][xPoint_1139+1] === this.CELL_EMPTY
                && map[yPoint_1139+1][xPoint_1139+1] === this.CELL_EMPTY
                && map[yPoint_1139+1][xPoint_1139] === this.CELL_EMPTY
                && map[yPoint_1139+1][xPoint_1139-1] === this.CELL_EMPTY
                && map[yPoint_1139][xPoint_1139-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1139, yPoint_1139, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1139;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1139][x] === this.CELL_EMPTY
                    && map[yPoint_1139-1][x] === this.CELL_EMPTY
                    && map[yPoint_1139-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1139][x+1] === this.CELL_EMPTY
                    && map[yPoint_1139+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1139+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1139, yPoint_1139, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1139;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1139] === this.CELL_EMPTY
                    && map[y+1][xPoint_1139] === this.CELL_EMPTY
                    && map[y+1][xPoint_1139+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1139] === this.CELL_EMPTY
                    && map[y][xPoint_1139-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1139-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
