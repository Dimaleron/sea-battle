...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26876=0;yPoint_26876<this.gameFieldBorderY.length; yPoint_26876++){
                for(var xPoint_26876=0;xPoint_26876<this.gameFieldBorderX.length; xPoint_26876++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26876, xPoint_26876);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26876][xPoint_26876] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26876, xPoint_26876, 'user');
                    if(this._userShipsMap[yPoint_26876][xPoint_26876] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26876, xPoint_26876, type){
            var id = this.getPointBlockIdByCoords(yPoint_26876, xPoint_26876, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26876);
                block.setAttribute('data-y', yPoint_26876);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26876
         * @param {type} xPoint_26876
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26876, xPoint_26876, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26876 + '_y' + yPoint_26876;
            }
            return 'pc_x' + xPoint_26876 + '_y' + yPoint_26876;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26876=0;yPoint_26876<this.gameFieldBorderY.length; yPoint_26876++){
                for(var xPoint_26876=0;xPoint_26876<this.gameFieldBorderX.length; xPoint_26876++){
                    map.push({y: yPoint_26876, x: xPoint_26876});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26876=-1;yPoint_26876<(this.gameFieldBorderY.length+1); yPoint_26876++){
                for(var xPoint_26876=-1;xPoint_26876<(this.gameFieldBorderX.length+1); xPoint_26876++){
                    if(!map[yPoint_26876]){
                        map[yPoint_26876] = [];
                    }
                    map[yPoint_26876][xPoint_26876] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26876 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26876 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26876, yPoint_26876) === true){
                    if(this.canPutHorizontal(map, xPoint_26876, yPoint_26876, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26876][xPoint_26876 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26876, yPoint_26876, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26876 + i][xPoint_26876] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26876, yPoint_26876){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26876][xPoint_26876] === this.CELL_EMPTY
                && map[yPoint_26876-1][xPoint_26876] === this.CELL_EMPTY
                && map[yPoint_26876-1][xPoint_26876+1] === this.CELL_EMPTY
                && map[yPoint_26876][xPoint_26876+1] === this.CELL_EMPTY
                && map[yPoint_26876+1][xPoint_26876+1] === this.CELL_EMPTY
                && map[yPoint_26876+1][xPoint_26876] === this.CELL_EMPTY
                && map[yPoint_26876+1][xPoint_26876-1] === this.CELL_EMPTY
                && map[yPoint_26876][xPoint_26876-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26876, yPoint_26876, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26876;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26876][x] === this.CELL_EMPTY
                    && map[yPoint_26876-1][x] === this.CELL_EMPTY
                    && map[yPoint_26876-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26876][x+1] === this.CELL_EMPTY
                    && map[yPoint_26876+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26876+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26876, yPoint_26876, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26876;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26876] === this.CELL_EMPTY
                    && map[y+1][xPoint_26876] === this.CELL_EMPTY
                    && map[y+1][xPoint_26876+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26876] === this.CELL_EMPTY
                    && map[y][xPoint_26876-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26876-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
