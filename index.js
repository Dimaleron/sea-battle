...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27066=0;yPoint_27066<this.gameFieldBorderY.length; yPoint_27066++){
                for(var xPoint_27066=0;xPoint_27066<this.gameFieldBorderX.length; xPoint_27066++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27066, xPoint_27066);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27066][xPoint_27066] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27066, xPoint_27066, 'user');
                    if(this._userShipsMap[yPoint_27066][xPoint_27066] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27066, xPoint_27066, type){
            var id = this.getPointBlockIdByCoords(yPoint_27066, xPoint_27066, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27066);
                block.setAttribute('data-y', yPoint_27066);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27066
         * @param {type} xPoint_27066
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27066, xPoint_27066, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27066 + '_y' + yPoint_27066;
            }
            return 'pc_x' + xPoint_27066 + '_y' + yPoint_27066;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27066=0;yPoint_27066<this.gameFieldBorderY.length; yPoint_27066++){
                for(var xPoint_27066=0;xPoint_27066<this.gameFieldBorderX.length; xPoint_27066++){
                    map.push({y: yPoint_27066, x: xPoint_27066});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27066=-1;yPoint_27066<(this.gameFieldBorderY.length+1); yPoint_27066++){
                for(var xPoint_27066=-1;xPoint_27066<(this.gameFieldBorderX.length+1); xPoint_27066++){
                    if(!map[yPoint_27066]){
                        map[yPoint_27066] = [];
                    }
                    map[yPoint_27066][xPoint_27066] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27066 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27066 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27066, yPoint_27066) === true){
                    if(this.canPutHorizontal(map, xPoint_27066, yPoint_27066, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27066][xPoint_27066 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27066, yPoint_27066, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27066 + i][xPoint_27066] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27066, yPoint_27066){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27066][xPoint_27066] === this.CELL_EMPTY
                && map[yPoint_27066-1][xPoint_27066] === this.CELL_EMPTY
                && map[yPoint_27066-1][xPoint_27066+1] === this.CELL_EMPTY
                && map[yPoint_27066][xPoint_27066+1] === this.CELL_EMPTY
                && map[yPoint_27066+1][xPoint_27066+1] === this.CELL_EMPTY
                && map[yPoint_27066+1][xPoint_27066] === this.CELL_EMPTY
                && map[yPoint_27066+1][xPoint_27066-1] === this.CELL_EMPTY
                && map[yPoint_27066][xPoint_27066-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27066, yPoint_27066, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27066;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27066][x] === this.CELL_EMPTY
                    && map[yPoint_27066-1][x] === this.CELL_EMPTY
                    && map[yPoint_27066-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27066][x+1] === this.CELL_EMPTY
                    && map[yPoint_27066+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27066+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27066, yPoint_27066, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27066;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27066] === this.CELL_EMPTY
                    && map[y+1][xPoint_27066] === this.CELL_EMPTY
                    && map[y+1][xPoint_27066+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27066] === this.CELL_EMPTY
                    && map[y][xPoint_27066-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27066-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
