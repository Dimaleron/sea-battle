...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18046=0;yPoint_18046<this.gameFieldBorderY.length; yPoint_18046++){
                for(var xPoint_18046=0;xPoint_18046<this.gameFieldBorderX.length; xPoint_18046++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18046, xPoint_18046);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18046][xPoint_18046] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18046, xPoint_18046, 'user');
                    if(this._userShipsMap[yPoint_18046][xPoint_18046] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18046, xPoint_18046, type){
            var id = this.getPointBlockIdByCoords(yPoint_18046, xPoint_18046, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18046);
                block.setAttribute('data-y', yPoint_18046);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18046
         * @param {type} xPoint_18046
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18046, xPoint_18046, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18046 + '_y' + yPoint_18046;
            }
            return 'pc_x' + xPoint_18046 + '_y' + yPoint_18046;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18046=0;yPoint_18046<this.gameFieldBorderY.length; yPoint_18046++){
                for(var xPoint_18046=0;xPoint_18046<this.gameFieldBorderX.length; xPoint_18046++){
                    map.push({y: yPoint_18046, x: xPoint_18046});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18046=-1;yPoint_18046<(this.gameFieldBorderY.length+1); yPoint_18046++){
                for(var xPoint_18046=-1;xPoint_18046<(this.gameFieldBorderX.length+1); xPoint_18046++){
                    if(!map[yPoint_18046]){
                        map[yPoint_18046] = [];
                    }
                    map[yPoint_18046][xPoint_18046] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18046 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18046 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18046, yPoint_18046) === true){
                    if(this.canPutHorizontal(map, xPoint_18046, yPoint_18046, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18046][xPoint_18046 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18046, yPoint_18046, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18046 + i][xPoint_18046] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18046, yPoint_18046){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18046][xPoint_18046] === this.CELL_EMPTY
                && map[yPoint_18046-1][xPoint_18046] === this.CELL_EMPTY
                && map[yPoint_18046-1][xPoint_18046+1] === this.CELL_EMPTY
                && map[yPoint_18046][xPoint_18046+1] === this.CELL_EMPTY
                && map[yPoint_18046+1][xPoint_18046+1] === this.CELL_EMPTY
                && map[yPoint_18046+1][xPoint_18046] === this.CELL_EMPTY
                && map[yPoint_18046+1][xPoint_18046-1] === this.CELL_EMPTY
                && map[yPoint_18046][xPoint_18046-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18046, yPoint_18046, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18046;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18046][x] === this.CELL_EMPTY
                    && map[yPoint_18046-1][x] === this.CELL_EMPTY
                    && map[yPoint_18046-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18046][x+1] === this.CELL_EMPTY
                    && map[yPoint_18046+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18046+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18046, yPoint_18046, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18046;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18046] === this.CELL_EMPTY
                    && map[y+1][xPoint_18046] === this.CELL_EMPTY
                    && map[y+1][xPoint_18046+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18046] === this.CELL_EMPTY
                    && map[y][xPoint_18046-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18046-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
