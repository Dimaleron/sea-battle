...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2444=0;yPoint_2444<this.gameFieldBorderY.length; yPoint_2444++){
                for(var xPoint_2444=0;xPoint_2444<this.gameFieldBorderX.length; xPoint_2444++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2444, xPoint_2444);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2444][xPoint_2444] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2444, xPoint_2444, 'user');
                    if(this._userShipsMap[yPoint_2444][xPoint_2444] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2444, xPoint_2444, type){
            var id = this.getPointBlockIdByCoords(yPoint_2444, xPoint_2444, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2444);
                block.setAttribute('data-y', yPoint_2444);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2444
         * @param {type} xPoint_2444
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2444, xPoint_2444, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2444 + '_y' + yPoint_2444;
            }
            return 'pc_x' + xPoint_2444 + '_y' + yPoint_2444;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2444=0;yPoint_2444<this.gameFieldBorderY.length; yPoint_2444++){
                for(var xPoint_2444=0;xPoint_2444<this.gameFieldBorderX.length; xPoint_2444++){
                    map.push({y: yPoint_2444, x: xPoint_2444});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2444=-1;yPoint_2444<(this.gameFieldBorderY.length+1); yPoint_2444++){
                for(var xPoint_2444=-1;xPoint_2444<(this.gameFieldBorderX.length+1); xPoint_2444++){
                    if(!map[yPoint_2444]){
                        map[yPoint_2444] = [];
                    }
                    map[yPoint_2444][xPoint_2444] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2444 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2444 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2444, yPoint_2444) === true){
                    if(this.canPutHorizontal(map, xPoint_2444, yPoint_2444, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2444][xPoint_2444 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2444, yPoint_2444, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2444 + i][xPoint_2444] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2444, yPoint_2444){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2444][xPoint_2444] === this.CELL_EMPTY
                && map[yPoint_2444-1][xPoint_2444] === this.CELL_EMPTY
                && map[yPoint_2444-1][xPoint_2444+1] === this.CELL_EMPTY
                && map[yPoint_2444][xPoint_2444+1] === this.CELL_EMPTY
                && map[yPoint_2444+1][xPoint_2444+1] === this.CELL_EMPTY
                && map[yPoint_2444+1][xPoint_2444] === this.CELL_EMPTY
                && map[yPoint_2444+1][xPoint_2444-1] === this.CELL_EMPTY
                && map[yPoint_2444][xPoint_2444-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2444, yPoint_2444, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2444;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2444][x] === this.CELL_EMPTY
                    && map[yPoint_2444-1][x] === this.CELL_EMPTY
                    && map[yPoint_2444-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2444][x+1] === this.CELL_EMPTY
                    && map[yPoint_2444+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2444+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2444, yPoint_2444, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2444;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2444] === this.CELL_EMPTY
                    && map[y+1][xPoint_2444] === this.CELL_EMPTY
                    && map[y+1][xPoint_2444+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2444] === this.CELL_EMPTY
                    && map[y][xPoint_2444-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2444-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
