...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_23089=0;yPoint_23089<this.gameFieldBorderY.length; yPoint_23089++){
                for(var xPoint_23089=0;xPoint_23089<this.gameFieldBorderX.length; xPoint_23089++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_23089, xPoint_23089);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_23089][xPoint_23089] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_23089, xPoint_23089, 'user');
                    if(this._userShipsMap[yPoint_23089][xPoint_23089] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_23089, xPoint_23089, type){
            var id = this.getPointBlockIdByCoords(yPoint_23089, xPoint_23089, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_23089);
                block.setAttribute('data-y', yPoint_23089);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_23089
         * @param {type} xPoint_23089
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_23089, xPoint_23089, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_23089 + '_y' + yPoint_23089;
            }
            return 'pc_x' + xPoint_23089 + '_y' + yPoint_23089;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_23089=0;yPoint_23089<this.gameFieldBorderY.length; yPoint_23089++){
                for(var xPoint_23089=0;xPoint_23089<this.gameFieldBorderX.length; xPoint_23089++){
                    map.push({y: yPoint_23089, x: xPoint_23089});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_23089=-1;yPoint_23089<(this.gameFieldBorderY.length+1); yPoint_23089++){
                for(var xPoint_23089=-1;xPoint_23089<(this.gameFieldBorderX.length+1); xPoint_23089++){
                    if(!map[yPoint_23089]){
                        map[yPoint_23089] = [];
                    }
                    map[yPoint_23089][xPoint_23089] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_23089 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_23089 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_23089, yPoint_23089) === true){
                    if(this.canPutHorizontal(map, xPoint_23089, yPoint_23089, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23089][xPoint_23089 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_23089, yPoint_23089, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_23089 + i][xPoint_23089] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_23089, yPoint_23089){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_23089][xPoint_23089] === this.CELL_EMPTY
                && map[yPoint_23089-1][xPoint_23089] === this.CELL_EMPTY
                && map[yPoint_23089-1][xPoint_23089+1] === this.CELL_EMPTY
                && map[yPoint_23089][xPoint_23089+1] === this.CELL_EMPTY
                && map[yPoint_23089+1][xPoint_23089+1] === this.CELL_EMPTY
                && map[yPoint_23089+1][xPoint_23089] === this.CELL_EMPTY
                && map[yPoint_23089+1][xPoint_23089-1] === this.CELL_EMPTY
                && map[yPoint_23089][xPoint_23089-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_23089, yPoint_23089, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_23089;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_23089][x] === this.CELL_EMPTY
                    && map[yPoint_23089-1][x] === this.CELL_EMPTY
                    && map[yPoint_23089-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23089][x+1] === this.CELL_EMPTY
                    && map[yPoint_23089+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_23089+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_23089, yPoint_23089, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_23089;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_23089] === this.CELL_EMPTY
                    && map[y+1][xPoint_23089] === this.CELL_EMPTY
                    && map[y+1][xPoint_23089+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_23089] === this.CELL_EMPTY
                    && map[y][xPoint_23089-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_23089-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
