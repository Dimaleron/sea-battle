...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29874=0;yPoint_29874<this.gameFieldBorderY.length; yPoint_29874++){
                for(var xPoint_29874=0;xPoint_29874<this.gameFieldBorderX.length; xPoint_29874++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29874, xPoint_29874);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29874][xPoint_29874] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29874, xPoint_29874, 'user');
                    if(this._userShipsMap[yPoint_29874][xPoint_29874] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29874, xPoint_29874, type){
            var id = this.getPointBlockIdByCoords(yPoint_29874, xPoint_29874, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29874);
                block.setAttribute('data-y', yPoint_29874);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29874
         * @param {type} xPoint_29874
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29874, xPoint_29874, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29874 + '_y' + yPoint_29874;
            }
            return 'pc_x' + xPoint_29874 + '_y' + yPoint_29874;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29874=0;yPoint_29874<this.gameFieldBorderY.length; yPoint_29874++){
                for(var xPoint_29874=0;xPoint_29874<this.gameFieldBorderX.length; xPoint_29874++){
                    map.push({y: yPoint_29874, x: xPoint_29874});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29874=-1;yPoint_29874<(this.gameFieldBorderY.length+1); yPoint_29874++){
                for(var xPoint_29874=-1;xPoint_29874<(this.gameFieldBorderX.length+1); xPoint_29874++){
                    if(!map[yPoint_29874]){
                        map[yPoint_29874] = [];
                    }
                    map[yPoint_29874][xPoint_29874] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29874 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29874 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29874, yPoint_29874) === true){
                    if(this.canPutHorizontal(map, xPoint_29874, yPoint_29874, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29874][xPoint_29874 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29874, yPoint_29874, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29874 + i][xPoint_29874] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29874, yPoint_29874){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29874][xPoint_29874] === this.CELL_EMPTY
                && map[yPoint_29874-1][xPoint_29874] === this.CELL_EMPTY
                && map[yPoint_29874-1][xPoint_29874+1] === this.CELL_EMPTY
                && map[yPoint_29874][xPoint_29874+1] === this.CELL_EMPTY
                && map[yPoint_29874+1][xPoint_29874+1] === this.CELL_EMPTY
                && map[yPoint_29874+1][xPoint_29874] === this.CELL_EMPTY
                && map[yPoint_29874+1][xPoint_29874-1] === this.CELL_EMPTY
                && map[yPoint_29874][xPoint_29874-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29874, yPoint_29874, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29874;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29874][x] === this.CELL_EMPTY
                    && map[yPoint_29874-1][x] === this.CELL_EMPTY
                    && map[yPoint_29874-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29874][x+1] === this.CELL_EMPTY
                    && map[yPoint_29874+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29874+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29874, yPoint_29874, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29874;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29874] === this.CELL_EMPTY
                    && map[y+1][xPoint_29874] === this.CELL_EMPTY
                    && map[y+1][xPoint_29874+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29874] === this.CELL_EMPTY
                    && map[y][xPoint_29874-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29874-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
