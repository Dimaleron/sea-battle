...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22838=0;yPoint_22838<this.gameFieldBorderY.length; yPoint_22838++){
                for(var xPoint_22838=0;xPoint_22838<this.gameFieldBorderX.length; xPoint_22838++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22838, xPoint_22838);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22838][xPoint_22838] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22838, xPoint_22838, 'user');
                    if(this._userShipsMap[yPoint_22838][xPoint_22838] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22838, xPoint_22838, type){
            var id = this.getPointBlockIdByCoords(yPoint_22838, xPoint_22838, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22838);
                block.setAttribute('data-y', yPoint_22838);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22838
         * @param {type} xPoint_22838
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22838, xPoint_22838, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22838 + '_y' + yPoint_22838;
            }
            return 'pc_x' + xPoint_22838 + '_y' + yPoint_22838;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22838=0;yPoint_22838<this.gameFieldBorderY.length; yPoint_22838++){
                for(var xPoint_22838=0;xPoint_22838<this.gameFieldBorderX.length; xPoint_22838++){
                    map.push({y: yPoint_22838, x: xPoint_22838});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22838=-1;yPoint_22838<(this.gameFieldBorderY.length+1); yPoint_22838++){
                for(var xPoint_22838=-1;xPoint_22838<(this.gameFieldBorderX.length+1); xPoint_22838++){
                    if(!map[yPoint_22838]){
                        map[yPoint_22838] = [];
                    }
                    map[yPoint_22838][xPoint_22838] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22838 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22838 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22838, yPoint_22838) === true){
                    if(this.canPutHorizontal(map, xPoint_22838, yPoint_22838, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22838][xPoint_22838 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22838, yPoint_22838, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22838 + i][xPoint_22838] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22838, yPoint_22838){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22838][xPoint_22838] === this.CELL_EMPTY
                && map[yPoint_22838-1][xPoint_22838] === this.CELL_EMPTY
                && map[yPoint_22838-1][xPoint_22838+1] === this.CELL_EMPTY
                && map[yPoint_22838][xPoint_22838+1] === this.CELL_EMPTY
                && map[yPoint_22838+1][xPoint_22838+1] === this.CELL_EMPTY
                && map[yPoint_22838+1][xPoint_22838] === this.CELL_EMPTY
                && map[yPoint_22838+1][xPoint_22838-1] === this.CELL_EMPTY
                && map[yPoint_22838][xPoint_22838-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22838, yPoint_22838, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22838;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22838][x] === this.CELL_EMPTY
                    && map[yPoint_22838-1][x] === this.CELL_EMPTY
                    && map[yPoint_22838-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22838][x+1] === this.CELL_EMPTY
                    && map[yPoint_22838+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22838+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22838, yPoint_22838, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22838;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22838] === this.CELL_EMPTY
                    && map[y+1][xPoint_22838] === this.CELL_EMPTY
                    && map[y+1][xPoint_22838+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22838] === this.CELL_EMPTY
                    && map[y][xPoint_22838-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22838-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
