...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_985=0;yPoint_985<this.gameFieldBorderY.length; yPoint_985++){
                for(var xPoint_985=0;xPoint_985<this.gameFieldBorderX.length; xPoint_985++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_985, xPoint_985);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_985][xPoint_985] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_985, xPoint_985, 'user');
                    if(this._userShipsMap[yPoint_985][xPoint_985] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_985, xPoint_985, type){
            var id = this.getPointBlockIdByCoords(yPoint_985, xPoint_985, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_985);
                block.setAttribute('data-y', yPoint_985);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_985
         * @param {type} xPoint_985
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_985, xPoint_985, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_985 + '_y' + yPoint_985;
            }
            return 'pc_x' + xPoint_985 + '_y' + yPoint_985;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_985=0;yPoint_985<this.gameFieldBorderY.length; yPoint_985++){
                for(var xPoint_985=0;xPoint_985<this.gameFieldBorderX.length; xPoint_985++){
                    map.push({y: yPoint_985, x: xPoint_985});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_985=-1;yPoint_985<(this.gameFieldBorderY.length+1); yPoint_985++){
                for(var xPoint_985=-1;xPoint_985<(this.gameFieldBorderX.length+1); xPoint_985++){
                    if(!map[yPoint_985]){
                        map[yPoint_985] = [];
                    }
                    map[yPoint_985][xPoint_985] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_985 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_985 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_985, yPoint_985) === true){
                    if(this.canPutHorizontal(map, xPoint_985, yPoint_985, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_985][xPoint_985 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_985, yPoint_985, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_985 + i][xPoint_985] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_985, yPoint_985){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_985][xPoint_985] === this.CELL_EMPTY
                && map[yPoint_985-1][xPoint_985] === this.CELL_EMPTY
                && map[yPoint_985-1][xPoint_985+1] === this.CELL_EMPTY
                && map[yPoint_985][xPoint_985+1] === this.CELL_EMPTY
                && map[yPoint_985+1][xPoint_985+1] === this.CELL_EMPTY
                && map[yPoint_985+1][xPoint_985] === this.CELL_EMPTY
                && map[yPoint_985+1][xPoint_985-1] === this.CELL_EMPTY
                && map[yPoint_985][xPoint_985-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_985, yPoint_985, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_985;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_985][x] === this.CELL_EMPTY
                    && map[yPoint_985-1][x] === this.CELL_EMPTY
                    && map[yPoint_985-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_985][x+1] === this.CELL_EMPTY
                    && map[yPoint_985+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_985+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_985, yPoint_985, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_985;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_985] === this.CELL_EMPTY
                    && map[y+1][xPoint_985] === this.CELL_EMPTY
                    && map[y+1][xPoint_985+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_985] === this.CELL_EMPTY
                    && map[y][xPoint_985-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_985-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
