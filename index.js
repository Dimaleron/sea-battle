...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25317=0;yPoint_25317<this.gameFieldBorderY.length; yPoint_25317++){
                for(var xPoint_25317=0;xPoint_25317<this.gameFieldBorderX.length; xPoint_25317++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25317, xPoint_25317);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25317][xPoint_25317] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25317, xPoint_25317, 'user');
                    if(this._userShipsMap[yPoint_25317][xPoint_25317] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25317, xPoint_25317, type){
            var id = this.getPointBlockIdByCoords(yPoint_25317, xPoint_25317, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25317);
                block.setAttribute('data-y', yPoint_25317);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25317
         * @param {type} xPoint_25317
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25317, xPoint_25317, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25317 + '_y' + yPoint_25317;
            }
            return 'pc_x' + xPoint_25317 + '_y' + yPoint_25317;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25317=0;yPoint_25317<this.gameFieldBorderY.length; yPoint_25317++){
                for(var xPoint_25317=0;xPoint_25317<this.gameFieldBorderX.length; xPoint_25317++){
                    map.push({y: yPoint_25317, x: xPoint_25317});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25317=-1;yPoint_25317<(this.gameFieldBorderY.length+1); yPoint_25317++){
                for(var xPoint_25317=-1;xPoint_25317<(this.gameFieldBorderX.length+1); xPoint_25317++){
                    if(!map[yPoint_25317]){
                        map[yPoint_25317] = [];
                    }
                    map[yPoint_25317][xPoint_25317] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25317 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25317 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25317, yPoint_25317) === true){
                    if(this.canPutHorizontal(map, xPoint_25317, yPoint_25317, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25317][xPoint_25317 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25317, yPoint_25317, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25317 + i][xPoint_25317] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25317, yPoint_25317){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25317][xPoint_25317] === this.CELL_EMPTY
                && map[yPoint_25317-1][xPoint_25317] === this.CELL_EMPTY
                && map[yPoint_25317-1][xPoint_25317+1] === this.CELL_EMPTY
                && map[yPoint_25317][xPoint_25317+1] === this.CELL_EMPTY
                && map[yPoint_25317+1][xPoint_25317+1] === this.CELL_EMPTY
                && map[yPoint_25317+1][xPoint_25317] === this.CELL_EMPTY
                && map[yPoint_25317+1][xPoint_25317-1] === this.CELL_EMPTY
                && map[yPoint_25317][xPoint_25317-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25317, yPoint_25317, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25317;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25317][x] === this.CELL_EMPTY
                    && map[yPoint_25317-1][x] === this.CELL_EMPTY
                    && map[yPoint_25317-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25317][x+1] === this.CELL_EMPTY
                    && map[yPoint_25317+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25317+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25317, yPoint_25317, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25317;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25317] === this.CELL_EMPTY
                    && map[y+1][xPoint_25317] === this.CELL_EMPTY
                    && map[y+1][xPoint_25317+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25317] === this.CELL_EMPTY
                    && map[y][xPoint_25317-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25317-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
