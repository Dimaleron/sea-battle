...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13328=0;yPoint_13328<this.gameFieldBorderY.length; yPoint_13328++){
                for(var xPoint_13328=0;xPoint_13328<this.gameFieldBorderX.length; xPoint_13328++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13328, xPoint_13328);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13328][xPoint_13328] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13328, xPoint_13328, 'user');
                    if(this._userShipsMap[yPoint_13328][xPoint_13328] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13328, xPoint_13328, type){
            var id = this.getPointBlockIdByCoords(yPoint_13328, xPoint_13328, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13328);
                block.setAttribute('data-y', yPoint_13328);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13328
         * @param {type} xPoint_13328
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13328, xPoint_13328, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13328 + '_y' + yPoint_13328;
            }
            return 'pc_x' + xPoint_13328 + '_y' + yPoint_13328;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13328=0;yPoint_13328<this.gameFieldBorderY.length; yPoint_13328++){
                for(var xPoint_13328=0;xPoint_13328<this.gameFieldBorderX.length; xPoint_13328++){
                    map.push({y: yPoint_13328, x: xPoint_13328});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13328=-1;yPoint_13328<(this.gameFieldBorderY.length+1); yPoint_13328++){
                for(var xPoint_13328=-1;xPoint_13328<(this.gameFieldBorderX.length+1); xPoint_13328++){
                    if(!map[yPoint_13328]){
                        map[yPoint_13328] = [];
                    }
                    map[yPoint_13328][xPoint_13328] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13328 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13328 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13328, yPoint_13328) === true){
                    if(this.canPutHorizontal(map, xPoint_13328, yPoint_13328, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13328][xPoint_13328 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13328, yPoint_13328, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13328 + i][xPoint_13328] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13328, yPoint_13328){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13328][xPoint_13328] === this.CELL_EMPTY
                && map[yPoint_13328-1][xPoint_13328] === this.CELL_EMPTY
                && map[yPoint_13328-1][xPoint_13328+1] === this.CELL_EMPTY
                && map[yPoint_13328][xPoint_13328+1] === this.CELL_EMPTY
                && map[yPoint_13328+1][xPoint_13328+1] === this.CELL_EMPTY
                && map[yPoint_13328+1][xPoint_13328] === this.CELL_EMPTY
                && map[yPoint_13328+1][xPoint_13328-1] === this.CELL_EMPTY
                && map[yPoint_13328][xPoint_13328-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13328, yPoint_13328, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13328;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13328][x] === this.CELL_EMPTY
                    && map[yPoint_13328-1][x] === this.CELL_EMPTY
                    && map[yPoint_13328-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13328][x+1] === this.CELL_EMPTY
                    && map[yPoint_13328+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13328+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13328, yPoint_13328, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13328;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13328] === this.CELL_EMPTY
                    && map[y+1][xPoint_13328] === this.CELL_EMPTY
                    && map[y+1][xPoint_13328+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13328] === this.CELL_EMPTY
                    && map[y][xPoint_13328-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13328-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
