...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17725=0;yPoint_17725<this.gameFieldBorderY.length; yPoint_17725++){
                for(var xPoint_17725=0;xPoint_17725<this.gameFieldBorderX.length; xPoint_17725++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17725, xPoint_17725);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17725][xPoint_17725] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17725, xPoint_17725, 'user');
                    if(this._userShipsMap[yPoint_17725][xPoint_17725] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17725, xPoint_17725, type){
            var id = this.getPointBlockIdByCoords(yPoint_17725, xPoint_17725, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17725);
                block.setAttribute('data-y', yPoint_17725);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17725
         * @param {type} xPoint_17725
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17725, xPoint_17725, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17725 + '_y' + yPoint_17725;
            }
            return 'pc_x' + xPoint_17725 + '_y' + yPoint_17725;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17725=0;yPoint_17725<this.gameFieldBorderY.length; yPoint_17725++){
                for(var xPoint_17725=0;xPoint_17725<this.gameFieldBorderX.length; xPoint_17725++){
                    map.push({y: yPoint_17725, x: xPoint_17725});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17725=-1;yPoint_17725<(this.gameFieldBorderY.length+1); yPoint_17725++){
                for(var xPoint_17725=-1;xPoint_17725<(this.gameFieldBorderX.length+1); xPoint_17725++){
                    if(!map[yPoint_17725]){
                        map[yPoint_17725] = [];
                    }
                    map[yPoint_17725][xPoint_17725] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17725 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17725 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17725, yPoint_17725) === true){
                    if(this.canPutHorizontal(map, xPoint_17725, yPoint_17725, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17725][xPoint_17725 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17725, yPoint_17725, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17725 + i][xPoint_17725] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17725, yPoint_17725){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17725][xPoint_17725] === this.CELL_EMPTY
                && map[yPoint_17725-1][xPoint_17725] === this.CELL_EMPTY
                && map[yPoint_17725-1][xPoint_17725+1] === this.CELL_EMPTY
                && map[yPoint_17725][xPoint_17725+1] === this.CELL_EMPTY
                && map[yPoint_17725+1][xPoint_17725+1] === this.CELL_EMPTY
                && map[yPoint_17725+1][xPoint_17725] === this.CELL_EMPTY
                && map[yPoint_17725+1][xPoint_17725-1] === this.CELL_EMPTY
                && map[yPoint_17725][xPoint_17725-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17725, yPoint_17725, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17725;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17725][x] === this.CELL_EMPTY
                    && map[yPoint_17725-1][x] === this.CELL_EMPTY
                    && map[yPoint_17725-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17725][x+1] === this.CELL_EMPTY
                    && map[yPoint_17725+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17725+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17725, yPoint_17725, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17725;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17725] === this.CELL_EMPTY
                    && map[y+1][xPoint_17725] === this.CELL_EMPTY
                    && map[y+1][xPoint_17725+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17725] === this.CELL_EMPTY
                    && map[y][xPoint_17725-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17725-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
