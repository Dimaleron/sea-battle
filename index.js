...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_416=0;yPoint_416<this.gameFieldBorderY.length; yPoint_416++){
                for(var xPoint_416=0;xPoint_416<this.gameFieldBorderX.length; xPoint_416++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_416, xPoint_416);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_416][xPoint_416] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_416, xPoint_416, 'user');
                    if(this._userShipsMap[yPoint_416][xPoint_416] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_416, xPoint_416, type){
            var id = this.getPointBlockIdByCoords(yPoint_416, xPoint_416, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_416);
                block.setAttribute('data-y', yPoint_416);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_416
         * @param {type} xPoint_416
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_416, xPoint_416, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_416 + '_y' + yPoint_416;
            }
            return 'pc_x' + xPoint_416 + '_y' + yPoint_416;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_416=0;yPoint_416<this.gameFieldBorderY.length; yPoint_416++){
                for(var xPoint_416=0;xPoint_416<this.gameFieldBorderX.length; xPoint_416++){
                    map.push({y: yPoint_416, x: xPoint_416});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_416=-1;yPoint_416<(this.gameFieldBorderY.length+1); yPoint_416++){
                for(var xPoint_416=-1;xPoint_416<(this.gameFieldBorderX.length+1); xPoint_416++){
                    if(!map[yPoint_416]){
                        map[yPoint_416] = [];
                    }
                    map[yPoint_416][xPoint_416] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_416 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_416 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_416, yPoint_416) === true){
                    if(this.canPutHorizontal(map, xPoint_416, yPoint_416, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_416][xPoint_416 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_416, yPoint_416, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_416 + i][xPoint_416] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_416, yPoint_416){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_416][xPoint_416] === this.CELL_EMPTY
                && map[yPoint_416-1][xPoint_416] === this.CELL_EMPTY
                && map[yPoint_416-1][xPoint_416+1] === this.CELL_EMPTY
                && map[yPoint_416][xPoint_416+1] === this.CELL_EMPTY
                && map[yPoint_416+1][xPoint_416+1] === this.CELL_EMPTY
                && map[yPoint_416+1][xPoint_416] === this.CELL_EMPTY
                && map[yPoint_416+1][xPoint_416-1] === this.CELL_EMPTY
                && map[yPoint_416][xPoint_416-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_416, yPoint_416, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_416;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_416][x] === this.CELL_EMPTY
                    && map[yPoint_416-1][x] === this.CELL_EMPTY
                    && map[yPoint_416-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_416][x+1] === this.CELL_EMPTY
                    && map[yPoint_416+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_416+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_416, yPoint_416, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_416;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_416] === this.CELL_EMPTY
                    && map[y+1][xPoint_416] === this.CELL_EMPTY
                    && map[y+1][xPoint_416+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_416] === this.CELL_EMPTY
                    && map[y][xPoint_416-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_416-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
