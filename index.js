...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_574=0;yPoint_574<this.gameFieldBorderY.length; yPoint_574++){
                for(var xPoint_574=0;xPoint_574<this.gameFieldBorderX.length; xPoint_574++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_574, xPoint_574);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_574][xPoint_574] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_574, xPoint_574, 'user');
                    if(this._userShipsMap[yPoint_574][xPoint_574] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_574, xPoint_574, type){
            var id = this.getPointBlockIdByCoords(yPoint_574, xPoint_574, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_574);
                block.setAttribute('data-y', yPoint_574);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_574
         * @param {type} xPoint_574
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_574, xPoint_574, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_574 + '_y' + yPoint_574;
            }
            return 'pc_x' + xPoint_574 + '_y' + yPoint_574;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_574=0;yPoint_574<this.gameFieldBorderY.length; yPoint_574++){
                for(var xPoint_574=0;xPoint_574<this.gameFieldBorderX.length; xPoint_574++){
                    map.push({y: yPoint_574, x: xPoint_574});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_574=-1;yPoint_574<(this.gameFieldBorderY.length+1); yPoint_574++){
                for(var xPoint_574=-1;xPoint_574<(this.gameFieldBorderX.length+1); xPoint_574++){
                    if(!map[yPoint_574]){
                        map[yPoint_574] = [];
                    }
                    map[yPoint_574][xPoint_574] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_574 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_574 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_574, yPoint_574) === true){
                    if(this.canPutHorizontal(map, xPoint_574, yPoint_574, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_574][xPoint_574 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_574, yPoint_574, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_574 + i][xPoint_574] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_574, yPoint_574){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_574][xPoint_574] === this.CELL_EMPTY
                && map[yPoint_574-1][xPoint_574] === this.CELL_EMPTY
                && map[yPoint_574-1][xPoint_574+1] === this.CELL_EMPTY
                && map[yPoint_574][xPoint_574+1] === this.CELL_EMPTY
                && map[yPoint_574+1][xPoint_574+1] === this.CELL_EMPTY
                && map[yPoint_574+1][xPoint_574] === this.CELL_EMPTY
                && map[yPoint_574+1][xPoint_574-1] === this.CELL_EMPTY
                && map[yPoint_574][xPoint_574-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_574, yPoint_574, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_574;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_574][x] === this.CELL_EMPTY
                    && map[yPoint_574-1][x] === this.CELL_EMPTY
                    && map[yPoint_574-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_574][x+1] === this.CELL_EMPTY
                    && map[yPoint_574+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_574+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_574, yPoint_574, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_574;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_574] === this.CELL_EMPTY
                    && map[y+1][xPoint_574] === this.CELL_EMPTY
                    && map[y+1][xPoint_574+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_574] === this.CELL_EMPTY
                    && map[y][xPoint_574-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_574-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
