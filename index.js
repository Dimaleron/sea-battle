...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16123=0;yPoint_16123<this.gameFieldBorderY.length; yPoint_16123++){
                for(var xPoint_16123=0;xPoint_16123<this.gameFieldBorderX.length; xPoint_16123++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16123, xPoint_16123);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16123][xPoint_16123] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16123, xPoint_16123, 'user');
                    if(this._userShipsMap[yPoint_16123][xPoint_16123] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16123, xPoint_16123, type){
            var id = this.getPointBlockIdByCoords(yPoint_16123, xPoint_16123, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16123);
                block.setAttribute('data-y', yPoint_16123);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16123
         * @param {type} xPoint_16123
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16123, xPoint_16123, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16123 + '_y' + yPoint_16123;
            }
            return 'pc_x' + xPoint_16123 + '_y' + yPoint_16123;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16123=0;yPoint_16123<this.gameFieldBorderY.length; yPoint_16123++){
                for(var xPoint_16123=0;xPoint_16123<this.gameFieldBorderX.length; xPoint_16123++){
                    map.push({y: yPoint_16123, x: xPoint_16123});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16123=-1;yPoint_16123<(this.gameFieldBorderY.length+1); yPoint_16123++){
                for(var xPoint_16123=-1;xPoint_16123<(this.gameFieldBorderX.length+1); xPoint_16123++){
                    if(!map[yPoint_16123]){
                        map[yPoint_16123] = [];
                    }
                    map[yPoint_16123][xPoint_16123] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16123 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16123 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16123, yPoint_16123) === true){
                    if(this.canPutHorizontal(map, xPoint_16123, yPoint_16123, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16123][xPoint_16123 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16123, yPoint_16123, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16123 + i][xPoint_16123] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16123, yPoint_16123){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16123][xPoint_16123] === this.CELL_EMPTY
                && map[yPoint_16123-1][xPoint_16123] === this.CELL_EMPTY
                && map[yPoint_16123-1][xPoint_16123+1] === this.CELL_EMPTY
                && map[yPoint_16123][xPoint_16123+1] === this.CELL_EMPTY
                && map[yPoint_16123+1][xPoint_16123+1] === this.CELL_EMPTY
                && map[yPoint_16123+1][xPoint_16123] === this.CELL_EMPTY
                && map[yPoint_16123+1][xPoint_16123-1] === this.CELL_EMPTY
                && map[yPoint_16123][xPoint_16123-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16123, yPoint_16123, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16123;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16123][x] === this.CELL_EMPTY
                    && map[yPoint_16123-1][x] === this.CELL_EMPTY
                    && map[yPoint_16123-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16123][x+1] === this.CELL_EMPTY
                    && map[yPoint_16123+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16123+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16123, yPoint_16123, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16123;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16123] === this.CELL_EMPTY
                    && map[y+1][xPoint_16123] === this.CELL_EMPTY
                    && map[y+1][xPoint_16123+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16123] === this.CELL_EMPTY
                    && map[y][xPoint_16123-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16123-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
