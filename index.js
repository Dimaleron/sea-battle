...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14525=0;yPoint_14525<this.gameFieldBorderY.length; yPoint_14525++){
                for(var xPoint_14525=0;xPoint_14525<this.gameFieldBorderX.length; xPoint_14525++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14525, xPoint_14525);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14525][xPoint_14525] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14525, xPoint_14525, 'user');
                    if(this._userShipsMap[yPoint_14525][xPoint_14525] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14525, xPoint_14525, type){
            var id = this.getPointBlockIdByCoords(yPoint_14525, xPoint_14525, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14525);
                block.setAttribute('data-y', yPoint_14525);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14525
         * @param {type} xPoint_14525
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14525, xPoint_14525, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14525 + '_y' + yPoint_14525;
            }
            return 'pc_x' + xPoint_14525 + '_y' + yPoint_14525;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14525=0;yPoint_14525<this.gameFieldBorderY.length; yPoint_14525++){
                for(var xPoint_14525=0;xPoint_14525<this.gameFieldBorderX.length; xPoint_14525++){
                    map.push({y: yPoint_14525, x: xPoint_14525});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14525=-1;yPoint_14525<(this.gameFieldBorderY.length+1); yPoint_14525++){
                for(var xPoint_14525=-1;xPoint_14525<(this.gameFieldBorderX.length+1); xPoint_14525++){
                    if(!map[yPoint_14525]){
                        map[yPoint_14525] = [];
                    }
                    map[yPoint_14525][xPoint_14525] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14525 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14525 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14525, yPoint_14525) === true){
                    if(this.canPutHorizontal(map, xPoint_14525, yPoint_14525, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14525][xPoint_14525 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14525, yPoint_14525, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14525 + i][xPoint_14525] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14525, yPoint_14525){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14525][xPoint_14525] === this.CELL_EMPTY
                && map[yPoint_14525-1][xPoint_14525] === this.CELL_EMPTY
                && map[yPoint_14525-1][xPoint_14525+1] === this.CELL_EMPTY
                && map[yPoint_14525][xPoint_14525+1] === this.CELL_EMPTY
                && map[yPoint_14525+1][xPoint_14525+1] === this.CELL_EMPTY
                && map[yPoint_14525+1][xPoint_14525] === this.CELL_EMPTY
                && map[yPoint_14525+1][xPoint_14525-1] === this.CELL_EMPTY
                && map[yPoint_14525][xPoint_14525-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14525, yPoint_14525, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14525;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14525][x] === this.CELL_EMPTY
                    && map[yPoint_14525-1][x] === this.CELL_EMPTY
                    && map[yPoint_14525-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14525][x+1] === this.CELL_EMPTY
                    && map[yPoint_14525+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14525+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14525, yPoint_14525, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14525;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14525] === this.CELL_EMPTY
                    && map[y+1][xPoint_14525] === this.CELL_EMPTY
                    && map[y+1][xPoint_14525+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14525] === this.CELL_EMPTY
                    && map[y][xPoint_14525-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14525-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
