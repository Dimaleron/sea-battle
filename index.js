...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17343=0;yPoint_17343<this.gameFieldBorderY.length; yPoint_17343++){
                for(var xPoint_17343=0;xPoint_17343<this.gameFieldBorderX.length; xPoint_17343++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17343, xPoint_17343);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17343][xPoint_17343] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17343, xPoint_17343, 'user');
                    if(this._userShipsMap[yPoint_17343][xPoint_17343] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17343, xPoint_17343, type){
            var id = this.getPointBlockIdByCoords(yPoint_17343, xPoint_17343, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17343);
                block.setAttribute('data-y', yPoint_17343);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17343
         * @param {type} xPoint_17343
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17343, xPoint_17343, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17343 + '_y' + yPoint_17343;
            }
            return 'pc_x' + xPoint_17343 + '_y' + yPoint_17343;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17343=0;yPoint_17343<this.gameFieldBorderY.length; yPoint_17343++){
                for(var xPoint_17343=0;xPoint_17343<this.gameFieldBorderX.length; xPoint_17343++){
                    map.push({y: yPoint_17343, x: xPoint_17343});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17343=-1;yPoint_17343<(this.gameFieldBorderY.length+1); yPoint_17343++){
                for(var xPoint_17343=-1;xPoint_17343<(this.gameFieldBorderX.length+1); xPoint_17343++){
                    if(!map[yPoint_17343]){
                        map[yPoint_17343] = [];
                    }
                    map[yPoint_17343][xPoint_17343] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17343 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17343 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17343, yPoint_17343) === true){
                    if(this.canPutHorizontal(map, xPoint_17343, yPoint_17343, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17343][xPoint_17343 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17343, yPoint_17343, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17343 + i][xPoint_17343] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17343, yPoint_17343){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17343][xPoint_17343] === this.CELL_EMPTY
                && map[yPoint_17343-1][xPoint_17343] === this.CELL_EMPTY
                && map[yPoint_17343-1][xPoint_17343+1] === this.CELL_EMPTY
                && map[yPoint_17343][xPoint_17343+1] === this.CELL_EMPTY
                && map[yPoint_17343+1][xPoint_17343+1] === this.CELL_EMPTY
                && map[yPoint_17343+1][xPoint_17343] === this.CELL_EMPTY
                && map[yPoint_17343+1][xPoint_17343-1] === this.CELL_EMPTY
                && map[yPoint_17343][xPoint_17343-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17343, yPoint_17343, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17343;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17343][x] === this.CELL_EMPTY
                    && map[yPoint_17343-1][x] === this.CELL_EMPTY
                    && map[yPoint_17343-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17343][x+1] === this.CELL_EMPTY
                    && map[yPoint_17343+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17343+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17343, yPoint_17343, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17343;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17343] === this.CELL_EMPTY
                    && map[y+1][xPoint_17343] === this.CELL_EMPTY
                    && map[y+1][xPoint_17343+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17343] === this.CELL_EMPTY
                    && map[y][xPoint_17343-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17343-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
