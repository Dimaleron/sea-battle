...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4838=0;yPoint_4838<this.gameFieldBorderY.length; yPoint_4838++){
                for(var xPoint_4838=0;xPoint_4838<this.gameFieldBorderX.length; xPoint_4838++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4838, xPoint_4838);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4838][xPoint_4838] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4838, xPoint_4838, 'user');
                    if(this._userShipsMap[yPoint_4838][xPoint_4838] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4838, xPoint_4838, type){
            var id = this.getPointBlockIdByCoords(yPoint_4838, xPoint_4838, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4838);
                block.setAttribute('data-y', yPoint_4838);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4838
         * @param {type} xPoint_4838
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4838, xPoint_4838, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4838 + '_y' + yPoint_4838;
            }
            return 'pc_x' + xPoint_4838 + '_y' + yPoint_4838;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4838=0;yPoint_4838<this.gameFieldBorderY.length; yPoint_4838++){
                for(var xPoint_4838=0;xPoint_4838<this.gameFieldBorderX.length; xPoint_4838++){
                    map.push({y: yPoint_4838, x: xPoint_4838});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4838=-1;yPoint_4838<(this.gameFieldBorderY.length+1); yPoint_4838++){
                for(var xPoint_4838=-1;xPoint_4838<(this.gameFieldBorderX.length+1); xPoint_4838++){
                    if(!map[yPoint_4838]){
                        map[yPoint_4838] = [];
                    }
                    map[yPoint_4838][xPoint_4838] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4838 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4838 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4838, yPoint_4838) === true){
                    if(this.canPutHorizontal(map, xPoint_4838, yPoint_4838, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4838][xPoint_4838 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4838, yPoint_4838, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4838 + i][xPoint_4838] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4838, yPoint_4838){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4838][xPoint_4838] === this.CELL_EMPTY
                && map[yPoint_4838-1][xPoint_4838] === this.CELL_EMPTY
                && map[yPoint_4838-1][xPoint_4838+1] === this.CELL_EMPTY
                && map[yPoint_4838][xPoint_4838+1] === this.CELL_EMPTY
                && map[yPoint_4838+1][xPoint_4838+1] === this.CELL_EMPTY
                && map[yPoint_4838+1][xPoint_4838] === this.CELL_EMPTY
                && map[yPoint_4838+1][xPoint_4838-1] === this.CELL_EMPTY
                && map[yPoint_4838][xPoint_4838-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4838, yPoint_4838, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4838;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4838][x] === this.CELL_EMPTY
                    && map[yPoint_4838-1][x] === this.CELL_EMPTY
                    && map[yPoint_4838-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4838][x+1] === this.CELL_EMPTY
                    && map[yPoint_4838+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4838+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4838, yPoint_4838, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4838;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4838] === this.CELL_EMPTY
                    && map[y+1][xPoint_4838] === this.CELL_EMPTY
                    && map[y+1][xPoint_4838+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4838] === this.CELL_EMPTY
                    && map[y][xPoint_4838-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4838-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
