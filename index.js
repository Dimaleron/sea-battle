...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14979=0;yPoint_14979<this.gameFieldBorderY.length; yPoint_14979++){
                for(var xPoint_14979=0;xPoint_14979<this.gameFieldBorderX.length; xPoint_14979++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14979, xPoint_14979);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14979][xPoint_14979] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14979, xPoint_14979, 'user');
                    if(this._userShipsMap[yPoint_14979][xPoint_14979] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14979, xPoint_14979, type){
            var id = this.getPointBlockIdByCoords(yPoint_14979, xPoint_14979, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14979);
                block.setAttribute('data-y', yPoint_14979);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14979
         * @param {type} xPoint_14979
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14979, xPoint_14979, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14979 + '_y' + yPoint_14979;
            }
            return 'pc_x' + xPoint_14979 + '_y' + yPoint_14979;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14979=0;yPoint_14979<this.gameFieldBorderY.length; yPoint_14979++){
                for(var xPoint_14979=0;xPoint_14979<this.gameFieldBorderX.length; xPoint_14979++){
                    map.push({y: yPoint_14979, x: xPoint_14979});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14979=-1;yPoint_14979<(this.gameFieldBorderY.length+1); yPoint_14979++){
                for(var xPoint_14979=-1;xPoint_14979<(this.gameFieldBorderX.length+1); xPoint_14979++){
                    if(!map[yPoint_14979]){
                        map[yPoint_14979] = [];
                    }
                    map[yPoint_14979][xPoint_14979] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14979 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14979 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14979, yPoint_14979) === true){
                    if(this.canPutHorizontal(map, xPoint_14979, yPoint_14979, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14979][xPoint_14979 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14979, yPoint_14979, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14979 + i][xPoint_14979] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14979, yPoint_14979){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14979][xPoint_14979] === this.CELL_EMPTY
                && map[yPoint_14979-1][xPoint_14979] === this.CELL_EMPTY
                && map[yPoint_14979-1][xPoint_14979+1] === this.CELL_EMPTY
                && map[yPoint_14979][xPoint_14979+1] === this.CELL_EMPTY
                && map[yPoint_14979+1][xPoint_14979+1] === this.CELL_EMPTY
                && map[yPoint_14979+1][xPoint_14979] === this.CELL_EMPTY
                && map[yPoint_14979+1][xPoint_14979-1] === this.CELL_EMPTY
                && map[yPoint_14979][xPoint_14979-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14979, yPoint_14979, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14979;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14979][x] === this.CELL_EMPTY
                    && map[yPoint_14979-1][x] === this.CELL_EMPTY
                    && map[yPoint_14979-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14979][x+1] === this.CELL_EMPTY
                    && map[yPoint_14979+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14979+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14979, yPoint_14979, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14979;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14979] === this.CELL_EMPTY
                    && map[y+1][xPoint_14979] === this.CELL_EMPTY
                    && map[y+1][xPoint_14979+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14979] === this.CELL_EMPTY
                    && map[y][xPoint_14979-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14979-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
