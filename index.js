...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_74=0;yPoint_74<this.gameFieldBorderY.length; yPoint_74++){
                for(var xPoint_74=0;xPoint_74<this.gameFieldBorderX.length; xPoint_74++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_74, xPoint_74);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_74][xPoint_74] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_74, xPoint_74, 'user');
                    if(this._userShipsMap[yPoint_74][xPoint_74] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_74, xPoint_74, type){
            var id = this.getPointBlockIdByCoords(yPoint_74, xPoint_74, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_74);
                block.setAttribute('data-y', yPoint_74);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_74
         * @param {type} xPoint_74
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_74, xPoint_74, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_74 + '_y' + yPoint_74;
            }
            return 'pc_x' + xPoint_74 + '_y' + yPoint_74;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_74=0;yPoint_74<this.gameFieldBorderY.length; yPoint_74++){
                for(var xPoint_74=0;xPoint_74<this.gameFieldBorderX.length; xPoint_74++){
                    map.push({y: yPoint_74, x: xPoint_74});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_74=-1;yPoint_74<(this.gameFieldBorderY.length+1); yPoint_74++){
                for(var xPoint_74=-1;xPoint_74<(this.gameFieldBorderX.length+1); xPoint_74++){
                    if(!map[yPoint_74]){
                        map[yPoint_74] = [];
                    }
                    map[yPoint_74][xPoint_74] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_74 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_74 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_74, yPoint_74) === true){
                    if(this.canPutHorizontal(map, xPoint_74, yPoint_74, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_74][xPoint_74 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_74, yPoint_74, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_74 + i][xPoint_74] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_74, yPoint_74){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_74][xPoint_74] === this.CELL_EMPTY
                && map[yPoint_74-1][xPoint_74] === this.CELL_EMPTY
                && map[yPoint_74-1][xPoint_74+1] === this.CELL_EMPTY
                && map[yPoint_74][xPoint_74+1] === this.CELL_EMPTY
                && map[yPoint_74+1][xPoint_74+1] === this.CELL_EMPTY
                && map[yPoint_74+1][xPoint_74] === this.CELL_EMPTY
                && map[yPoint_74+1][xPoint_74-1] === this.CELL_EMPTY
                && map[yPoint_74][xPoint_74-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_74, yPoint_74, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_74;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_74][x] === this.CELL_EMPTY
                    && map[yPoint_74-1][x] === this.CELL_EMPTY
                    && map[yPoint_74-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_74][x+1] === this.CELL_EMPTY
                    && map[yPoint_74+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_74+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_74, yPoint_74, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_74;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_74] === this.CELL_EMPTY
                    && map[y+1][xPoint_74] === this.CELL_EMPTY
                    && map[y+1][xPoint_74+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_74] === this.CELL_EMPTY
                    && map[y][xPoint_74-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_74-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
