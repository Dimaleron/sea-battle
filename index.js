...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18489=0;yPoint_18489<this.gameFieldBorderY.length; yPoint_18489++){
                for(var xPoint_18489=0;xPoint_18489<this.gameFieldBorderX.length; xPoint_18489++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18489, xPoint_18489);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18489][xPoint_18489] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18489, xPoint_18489, 'user');
                    if(this._userShipsMap[yPoint_18489][xPoint_18489] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18489, xPoint_18489, type){
            var id = this.getPointBlockIdByCoords(yPoint_18489, xPoint_18489, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18489);
                block.setAttribute('data-y', yPoint_18489);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18489
         * @param {type} xPoint_18489
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18489, xPoint_18489, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18489 + '_y' + yPoint_18489;
            }
            return 'pc_x' + xPoint_18489 + '_y' + yPoint_18489;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18489=0;yPoint_18489<this.gameFieldBorderY.length; yPoint_18489++){
                for(var xPoint_18489=0;xPoint_18489<this.gameFieldBorderX.length; xPoint_18489++){
                    map.push({y: yPoint_18489, x: xPoint_18489});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18489=-1;yPoint_18489<(this.gameFieldBorderY.length+1); yPoint_18489++){
                for(var xPoint_18489=-1;xPoint_18489<(this.gameFieldBorderX.length+1); xPoint_18489++){
                    if(!map[yPoint_18489]){
                        map[yPoint_18489] = [];
                    }
                    map[yPoint_18489][xPoint_18489] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18489 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18489 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18489, yPoint_18489) === true){
                    if(this.canPutHorizontal(map, xPoint_18489, yPoint_18489, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18489][xPoint_18489 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18489, yPoint_18489, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18489 + i][xPoint_18489] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18489, yPoint_18489){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18489][xPoint_18489] === this.CELL_EMPTY
                && map[yPoint_18489-1][xPoint_18489] === this.CELL_EMPTY
                && map[yPoint_18489-1][xPoint_18489+1] === this.CELL_EMPTY
                && map[yPoint_18489][xPoint_18489+1] === this.CELL_EMPTY
                && map[yPoint_18489+1][xPoint_18489+1] === this.CELL_EMPTY
                && map[yPoint_18489+1][xPoint_18489] === this.CELL_EMPTY
                && map[yPoint_18489+1][xPoint_18489-1] === this.CELL_EMPTY
                && map[yPoint_18489][xPoint_18489-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18489, yPoint_18489, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18489;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18489][x] === this.CELL_EMPTY
                    && map[yPoint_18489-1][x] === this.CELL_EMPTY
                    && map[yPoint_18489-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18489][x+1] === this.CELL_EMPTY
                    && map[yPoint_18489+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18489+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18489, yPoint_18489, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18489;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18489] === this.CELL_EMPTY
                    && map[y+1][xPoint_18489] === this.CELL_EMPTY
                    && map[y+1][xPoint_18489+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18489] === this.CELL_EMPTY
                    && map[y][xPoint_18489-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18489-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
