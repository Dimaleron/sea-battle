...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8891=0;yPoint_8891<this.gameFieldBorderY.length; yPoint_8891++){
                for(var xPoint_8891=0;xPoint_8891<this.gameFieldBorderX.length; xPoint_8891++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8891, xPoint_8891);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8891][xPoint_8891] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8891, xPoint_8891, 'user');
                    if(this._userShipsMap[yPoint_8891][xPoint_8891] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8891, xPoint_8891, type){
            var id = this.getPointBlockIdByCoords(yPoint_8891, xPoint_8891, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8891);
                block.setAttribute('data-y', yPoint_8891);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8891
         * @param {type} xPoint_8891
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8891, xPoint_8891, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8891 + '_y' + yPoint_8891;
            }
            return 'pc_x' + xPoint_8891 + '_y' + yPoint_8891;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8891=0;yPoint_8891<this.gameFieldBorderY.length; yPoint_8891++){
                for(var xPoint_8891=0;xPoint_8891<this.gameFieldBorderX.length; xPoint_8891++){
                    map.push({y: yPoint_8891, x: xPoint_8891});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8891=-1;yPoint_8891<(this.gameFieldBorderY.length+1); yPoint_8891++){
                for(var xPoint_8891=-1;xPoint_8891<(this.gameFieldBorderX.length+1); xPoint_8891++){
                    if(!map[yPoint_8891]){
                        map[yPoint_8891] = [];
                    }
                    map[yPoint_8891][xPoint_8891] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8891 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8891 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8891, yPoint_8891) === true){
                    if(this.canPutHorizontal(map, xPoint_8891, yPoint_8891, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8891][xPoint_8891 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8891, yPoint_8891, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8891 + i][xPoint_8891] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8891, yPoint_8891){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8891][xPoint_8891] === this.CELL_EMPTY
                && map[yPoint_8891-1][xPoint_8891] === this.CELL_EMPTY
                && map[yPoint_8891-1][xPoint_8891+1] === this.CELL_EMPTY
                && map[yPoint_8891][xPoint_8891+1] === this.CELL_EMPTY
                && map[yPoint_8891+1][xPoint_8891+1] === this.CELL_EMPTY
                && map[yPoint_8891+1][xPoint_8891] === this.CELL_EMPTY
                && map[yPoint_8891+1][xPoint_8891-1] === this.CELL_EMPTY
                && map[yPoint_8891][xPoint_8891-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8891, yPoint_8891, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8891;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8891][x] === this.CELL_EMPTY
                    && map[yPoint_8891-1][x] === this.CELL_EMPTY
                    && map[yPoint_8891-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8891][x+1] === this.CELL_EMPTY
                    && map[yPoint_8891+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8891+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8891, yPoint_8891, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8891;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8891] === this.CELL_EMPTY
                    && map[y+1][xPoint_8891] === this.CELL_EMPTY
                    && map[y+1][xPoint_8891+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8891] === this.CELL_EMPTY
                    && map[y][xPoint_8891-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8891-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
