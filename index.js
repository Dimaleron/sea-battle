...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22374=0;yPoint_22374<this.gameFieldBorderY.length; yPoint_22374++){
                for(var xPoint_22374=0;xPoint_22374<this.gameFieldBorderX.length; xPoint_22374++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22374, xPoint_22374);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22374][xPoint_22374] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22374, xPoint_22374, 'user');
                    if(this._userShipsMap[yPoint_22374][xPoint_22374] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22374, xPoint_22374, type){
            var id = this.getPointBlockIdByCoords(yPoint_22374, xPoint_22374, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22374);
                block.setAttribute('data-y', yPoint_22374);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22374
         * @param {type} xPoint_22374
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22374, xPoint_22374, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22374 + '_y' + yPoint_22374;
            }
            return 'pc_x' + xPoint_22374 + '_y' + yPoint_22374;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22374=0;yPoint_22374<this.gameFieldBorderY.length; yPoint_22374++){
                for(var xPoint_22374=0;xPoint_22374<this.gameFieldBorderX.length; xPoint_22374++){
                    map.push({y: yPoint_22374, x: xPoint_22374});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22374=-1;yPoint_22374<(this.gameFieldBorderY.length+1); yPoint_22374++){
                for(var xPoint_22374=-1;xPoint_22374<(this.gameFieldBorderX.length+1); xPoint_22374++){
                    if(!map[yPoint_22374]){
                        map[yPoint_22374] = [];
                    }
                    map[yPoint_22374][xPoint_22374] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22374 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22374 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22374, yPoint_22374) === true){
                    if(this.canPutHorizontal(map, xPoint_22374, yPoint_22374, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22374][xPoint_22374 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22374, yPoint_22374, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22374 + i][xPoint_22374] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22374, yPoint_22374){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22374][xPoint_22374] === this.CELL_EMPTY
                && map[yPoint_22374-1][xPoint_22374] === this.CELL_EMPTY
                && map[yPoint_22374-1][xPoint_22374+1] === this.CELL_EMPTY
                && map[yPoint_22374][xPoint_22374+1] === this.CELL_EMPTY
                && map[yPoint_22374+1][xPoint_22374+1] === this.CELL_EMPTY
                && map[yPoint_22374+1][xPoint_22374] === this.CELL_EMPTY
                && map[yPoint_22374+1][xPoint_22374-1] === this.CELL_EMPTY
                && map[yPoint_22374][xPoint_22374-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22374, yPoint_22374, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22374;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22374][x] === this.CELL_EMPTY
                    && map[yPoint_22374-1][x] === this.CELL_EMPTY
                    && map[yPoint_22374-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22374][x+1] === this.CELL_EMPTY
                    && map[yPoint_22374+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22374+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22374, yPoint_22374, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22374;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22374] === this.CELL_EMPTY
                    && map[y+1][xPoint_22374] === this.CELL_EMPTY
                    && map[y+1][xPoint_22374+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22374] === this.CELL_EMPTY
                    && map[y][xPoint_22374-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22374-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
