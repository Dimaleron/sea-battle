...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6103=0;yPoint_6103<this.gameFieldBorderY.length; yPoint_6103++){
                for(var xPoint_6103=0;xPoint_6103<this.gameFieldBorderX.length; xPoint_6103++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6103, xPoint_6103);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6103][xPoint_6103] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6103, xPoint_6103, 'user');
                    if(this._userShipsMap[yPoint_6103][xPoint_6103] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6103, xPoint_6103, type){
            var id = this.getPointBlockIdByCoords(yPoint_6103, xPoint_6103, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6103);
                block.setAttribute('data-y', yPoint_6103);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6103
         * @param {type} xPoint_6103
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6103, xPoint_6103, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6103 + '_y' + yPoint_6103;
            }
            return 'pc_x' + xPoint_6103 + '_y' + yPoint_6103;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6103=0;yPoint_6103<this.gameFieldBorderY.length; yPoint_6103++){
                for(var xPoint_6103=0;xPoint_6103<this.gameFieldBorderX.length; xPoint_6103++){
                    map.push({y: yPoint_6103, x: xPoint_6103});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6103=-1;yPoint_6103<(this.gameFieldBorderY.length+1); yPoint_6103++){
                for(var xPoint_6103=-1;xPoint_6103<(this.gameFieldBorderX.length+1); xPoint_6103++){
                    if(!map[yPoint_6103]){
                        map[yPoint_6103] = [];
                    }
                    map[yPoint_6103][xPoint_6103] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6103 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6103 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6103, yPoint_6103) === true){
                    if(this.canPutHorizontal(map, xPoint_6103, yPoint_6103, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6103][xPoint_6103 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6103, yPoint_6103, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6103 + i][xPoint_6103] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6103, yPoint_6103){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6103][xPoint_6103] === this.CELL_EMPTY
                && map[yPoint_6103-1][xPoint_6103] === this.CELL_EMPTY
                && map[yPoint_6103-1][xPoint_6103+1] === this.CELL_EMPTY
                && map[yPoint_6103][xPoint_6103+1] === this.CELL_EMPTY
                && map[yPoint_6103+1][xPoint_6103+1] === this.CELL_EMPTY
                && map[yPoint_6103+1][xPoint_6103] === this.CELL_EMPTY
                && map[yPoint_6103+1][xPoint_6103-1] === this.CELL_EMPTY
                && map[yPoint_6103][xPoint_6103-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6103, yPoint_6103, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6103;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6103][x] === this.CELL_EMPTY
                    && map[yPoint_6103-1][x] === this.CELL_EMPTY
                    && map[yPoint_6103-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6103][x+1] === this.CELL_EMPTY
                    && map[yPoint_6103+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6103+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6103, yPoint_6103, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6103;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6103] === this.CELL_EMPTY
                    && map[y+1][xPoint_6103] === this.CELL_EMPTY
                    && map[y+1][xPoint_6103+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6103] === this.CELL_EMPTY
                    && map[y][xPoint_6103-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6103-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
