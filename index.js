...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13764=0;yPoint_13764<this.gameFieldBorderY.length; yPoint_13764++){
                for(var xPoint_13764=0;xPoint_13764<this.gameFieldBorderX.length; xPoint_13764++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13764, xPoint_13764);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13764][xPoint_13764] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13764, xPoint_13764, 'user');
                    if(this._userShipsMap[yPoint_13764][xPoint_13764] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13764, xPoint_13764, type){
            var id = this.getPointBlockIdByCoords(yPoint_13764, xPoint_13764, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13764);
                block.setAttribute('data-y', yPoint_13764);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13764
         * @param {type} xPoint_13764
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13764, xPoint_13764, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13764 + '_y' + yPoint_13764;
            }
            return 'pc_x' + xPoint_13764 + '_y' + yPoint_13764;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13764=0;yPoint_13764<this.gameFieldBorderY.length; yPoint_13764++){
                for(var xPoint_13764=0;xPoint_13764<this.gameFieldBorderX.length; xPoint_13764++){
                    map.push({y: yPoint_13764, x: xPoint_13764});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13764=-1;yPoint_13764<(this.gameFieldBorderY.length+1); yPoint_13764++){
                for(var xPoint_13764=-1;xPoint_13764<(this.gameFieldBorderX.length+1); xPoint_13764++){
                    if(!map[yPoint_13764]){
                        map[yPoint_13764] = [];
                    }
                    map[yPoint_13764][xPoint_13764] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13764 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13764 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13764, yPoint_13764) === true){
                    if(this.canPutHorizontal(map, xPoint_13764, yPoint_13764, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13764][xPoint_13764 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13764, yPoint_13764, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13764 + i][xPoint_13764] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13764, yPoint_13764){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13764][xPoint_13764] === this.CELL_EMPTY
                && map[yPoint_13764-1][xPoint_13764] === this.CELL_EMPTY
                && map[yPoint_13764-1][xPoint_13764+1] === this.CELL_EMPTY
                && map[yPoint_13764][xPoint_13764+1] === this.CELL_EMPTY
                && map[yPoint_13764+1][xPoint_13764+1] === this.CELL_EMPTY
                && map[yPoint_13764+1][xPoint_13764] === this.CELL_EMPTY
                && map[yPoint_13764+1][xPoint_13764-1] === this.CELL_EMPTY
                && map[yPoint_13764][xPoint_13764-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13764, yPoint_13764, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13764;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13764][x] === this.CELL_EMPTY
                    && map[yPoint_13764-1][x] === this.CELL_EMPTY
                    && map[yPoint_13764-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13764][x+1] === this.CELL_EMPTY
                    && map[yPoint_13764+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13764+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13764, yPoint_13764, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13764;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13764] === this.CELL_EMPTY
                    && map[y+1][xPoint_13764] === this.CELL_EMPTY
                    && map[y+1][xPoint_13764+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13764] === this.CELL_EMPTY
                    && map[y][xPoint_13764-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13764-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
