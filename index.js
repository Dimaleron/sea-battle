...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16341=0;yPoint_16341<this.gameFieldBorderY.length; yPoint_16341++){
                for(var xPoint_16341=0;xPoint_16341<this.gameFieldBorderX.length; xPoint_16341++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16341, xPoint_16341);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16341][xPoint_16341] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16341, xPoint_16341, 'user');
                    if(this._userShipsMap[yPoint_16341][xPoint_16341] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16341, xPoint_16341, type){
            var id = this.getPointBlockIdByCoords(yPoint_16341, xPoint_16341, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16341);
                block.setAttribute('data-y', yPoint_16341);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16341
         * @param {type} xPoint_16341
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16341, xPoint_16341, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16341 + '_y' + yPoint_16341;
            }
            return 'pc_x' + xPoint_16341 + '_y' + yPoint_16341;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16341=0;yPoint_16341<this.gameFieldBorderY.length; yPoint_16341++){
                for(var xPoint_16341=0;xPoint_16341<this.gameFieldBorderX.length; xPoint_16341++){
                    map.push({y: yPoint_16341, x: xPoint_16341});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16341=-1;yPoint_16341<(this.gameFieldBorderY.length+1); yPoint_16341++){
                for(var xPoint_16341=-1;xPoint_16341<(this.gameFieldBorderX.length+1); xPoint_16341++){
                    if(!map[yPoint_16341]){
                        map[yPoint_16341] = [];
                    }
                    map[yPoint_16341][xPoint_16341] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16341 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16341 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16341, yPoint_16341) === true){
                    if(this.canPutHorizontal(map, xPoint_16341, yPoint_16341, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16341][xPoint_16341 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16341, yPoint_16341, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16341 + i][xPoint_16341] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16341, yPoint_16341){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16341][xPoint_16341] === this.CELL_EMPTY
                && map[yPoint_16341-1][xPoint_16341] === this.CELL_EMPTY
                && map[yPoint_16341-1][xPoint_16341+1] === this.CELL_EMPTY
                && map[yPoint_16341][xPoint_16341+1] === this.CELL_EMPTY
                && map[yPoint_16341+1][xPoint_16341+1] === this.CELL_EMPTY
                && map[yPoint_16341+1][xPoint_16341] === this.CELL_EMPTY
                && map[yPoint_16341+1][xPoint_16341-1] === this.CELL_EMPTY
                && map[yPoint_16341][xPoint_16341-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16341, yPoint_16341, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16341;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16341][x] === this.CELL_EMPTY
                    && map[yPoint_16341-1][x] === this.CELL_EMPTY
                    && map[yPoint_16341-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16341][x+1] === this.CELL_EMPTY
                    && map[yPoint_16341+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16341+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16341, yPoint_16341, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16341;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16341] === this.CELL_EMPTY
                    && map[y+1][xPoint_16341] === this.CELL_EMPTY
                    && map[y+1][xPoint_16341+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16341] === this.CELL_EMPTY
                    && map[y][xPoint_16341-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16341-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
