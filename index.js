...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10060=0;yPoint_10060<this.gameFieldBorderY.length; yPoint_10060++){
                for(var xPoint_10060=0;xPoint_10060<this.gameFieldBorderX.length; xPoint_10060++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10060, xPoint_10060);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10060][xPoint_10060] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10060, xPoint_10060, 'user');
                    if(this._userShipsMap[yPoint_10060][xPoint_10060] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10060, xPoint_10060, type){
            var id = this.getPointBlockIdByCoords(yPoint_10060, xPoint_10060, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10060);
                block.setAttribute('data-y', yPoint_10060);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10060
         * @param {type} xPoint_10060
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10060, xPoint_10060, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10060 + '_y' + yPoint_10060;
            }
            return 'pc_x' + xPoint_10060 + '_y' + yPoint_10060;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10060=0;yPoint_10060<this.gameFieldBorderY.length; yPoint_10060++){
                for(var xPoint_10060=0;xPoint_10060<this.gameFieldBorderX.length; xPoint_10060++){
                    map.push({y: yPoint_10060, x: xPoint_10060});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10060=-1;yPoint_10060<(this.gameFieldBorderY.length+1); yPoint_10060++){
                for(var xPoint_10060=-1;xPoint_10060<(this.gameFieldBorderX.length+1); xPoint_10060++){
                    if(!map[yPoint_10060]){
                        map[yPoint_10060] = [];
                    }
                    map[yPoint_10060][xPoint_10060] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10060 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10060 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10060, yPoint_10060) === true){
                    if(this.canPutHorizontal(map, xPoint_10060, yPoint_10060, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10060][xPoint_10060 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10060, yPoint_10060, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10060 + i][xPoint_10060] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10060, yPoint_10060){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10060][xPoint_10060] === this.CELL_EMPTY
                && map[yPoint_10060-1][xPoint_10060] === this.CELL_EMPTY
                && map[yPoint_10060-1][xPoint_10060+1] === this.CELL_EMPTY
                && map[yPoint_10060][xPoint_10060+1] === this.CELL_EMPTY
                && map[yPoint_10060+1][xPoint_10060+1] === this.CELL_EMPTY
                && map[yPoint_10060+1][xPoint_10060] === this.CELL_EMPTY
                && map[yPoint_10060+1][xPoint_10060-1] === this.CELL_EMPTY
                && map[yPoint_10060][xPoint_10060-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10060, yPoint_10060, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10060;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10060][x] === this.CELL_EMPTY
                    && map[yPoint_10060-1][x] === this.CELL_EMPTY
                    && map[yPoint_10060-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10060][x+1] === this.CELL_EMPTY
                    && map[yPoint_10060+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10060+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10060, yPoint_10060, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10060;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10060] === this.CELL_EMPTY
                    && map[y+1][xPoint_10060] === this.CELL_EMPTY
                    && map[y+1][xPoint_10060+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10060] === this.CELL_EMPTY
                    && map[y][xPoint_10060-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10060-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
