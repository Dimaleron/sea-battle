...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28879=0;yPoint_28879<this.gameFieldBorderY.length; yPoint_28879++){
                for(var xPoint_28879=0;xPoint_28879<this.gameFieldBorderX.length; xPoint_28879++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28879, xPoint_28879);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28879][xPoint_28879] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28879, xPoint_28879, 'user');
                    if(this._userShipsMap[yPoint_28879][xPoint_28879] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28879, xPoint_28879, type){
            var id = this.getPointBlockIdByCoords(yPoint_28879, xPoint_28879, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28879);
                block.setAttribute('data-y', yPoint_28879);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28879
         * @param {type} xPoint_28879
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28879, xPoint_28879, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28879 + '_y' + yPoint_28879;
            }
            return 'pc_x' + xPoint_28879 + '_y' + yPoint_28879;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28879=0;yPoint_28879<this.gameFieldBorderY.length; yPoint_28879++){
                for(var xPoint_28879=0;xPoint_28879<this.gameFieldBorderX.length; xPoint_28879++){
                    map.push({y: yPoint_28879, x: xPoint_28879});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28879=-1;yPoint_28879<(this.gameFieldBorderY.length+1); yPoint_28879++){
                for(var xPoint_28879=-1;xPoint_28879<(this.gameFieldBorderX.length+1); xPoint_28879++){
                    if(!map[yPoint_28879]){
                        map[yPoint_28879] = [];
                    }
                    map[yPoint_28879][xPoint_28879] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28879 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28879 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28879, yPoint_28879) === true){
                    if(this.canPutHorizontal(map, xPoint_28879, yPoint_28879, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28879][xPoint_28879 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28879, yPoint_28879, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28879 + i][xPoint_28879] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28879, yPoint_28879){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28879][xPoint_28879] === this.CELL_EMPTY
                && map[yPoint_28879-1][xPoint_28879] === this.CELL_EMPTY
                && map[yPoint_28879-1][xPoint_28879+1] === this.CELL_EMPTY
                && map[yPoint_28879][xPoint_28879+1] === this.CELL_EMPTY
                && map[yPoint_28879+1][xPoint_28879+1] === this.CELL_EMPTY
                && map[yPoint_28879+1][xPoint_28879] === this.CELL_EMPTY
                && map[yPoint_28879+1][xPoint_28879-1] === this.CELL_EMPTY
                && map[yPoint_28879][xPoint_28879-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28879, yPoint_28879, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28879;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28879][x] === this.CELL_EMPTY
                    && map[yPoint_28879-1][x] === this.CELL_EMPTY
                    && map[yPoint_28879-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28879][x+1] === this.CELL_EMPTY
                    && map[yPoint_28879+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28879+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28879, yPoint_28879, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28879;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28879] === this.CELL_EMPTY
                    && map[y+1][xPoint_28879] === this.CELL_EMPTY
                    && map[y+1][xPoint_28879+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28879] === this.CELL_EMPTY
                    && map[y][xPoint_28879-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28879-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
