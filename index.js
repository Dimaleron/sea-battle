...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17585=0;yPoint_17585<this.gameFieldBorderY.length; yPoint_17585++){
                for(var xPoint_17585=0;xPoint_17585<this.gameFieldBorderX.length; xPoint_17585++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17585, xPoint_17585);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17585][xPoint_17585] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17585, xPoint_17585, 'user');
                    if(this._userShipsMap[yPoint_17585][xPoint_17585] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17585, xPoint_17585, type){
            var id = this.getPointBlockIdByCoords(yPoint_17585, xPoint_17585, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17585);
                block.setAttribute('data-y', yPoint_17585);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17585
         * @param {type} xPoint_17585
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17585, xPoint_17585, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17585 + '_y' + yPoint_17585;
            }
            return 'pc_x' + xPoint_17585 + '_y' + yPoint_17585;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17585=0;yPoint_17585<this.gameFieldBorderY.length; yPoint_17585++){
                for(var xPoint_17585=0;xPoint_17585<this.gameFieldBorderX.length; xPoint_17585++){
                    map.push({y: yPoint_17585, x: xPoint_17585});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17585=-1;yPoint_17585<(this.gameFieldBorderY.length+1); yPoint_17585++){
                for(var xPoint_17585=-1;xPoint_17585<(this.gameFieldBorderX.length+1); xPoint_17585++){
                    if(!map[yPoint_17585]){
                        map[yPoint_17585] = [];
                    }
                    map[yPoint_17585][xPoint_17585] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17585 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17585 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17585, yPoint_17585) === true){
                    if(this.canPutHorizontal(map, xPoint_17585, yPoint_17585, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17585][xPoint_17585 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17585, yPoint_17585, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17585 + i][xPoint_17585] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17585, yPoint_17585){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17585][xPoint_17585] === this.CELL_EMPTY
                && map[yPoint_17585-1][xPoint_17585] === this.CELL_EMPTY
                && map[yPoint_17585-1][xPoint_17585+1] === this.CELL_EMPTY
                && map[yPoint_17585][xPoint_17585+1] === this.CELL_EMPTY
                && map[yPoint_17585+1][xPoint_17585+1] === this.CELL_EMPTY
                && map[yPoint_17585+1][xPoint_17585] === this.CELL_EMPTY
                && map[yPoint_17585+1][xPoint_17585-1] === this.CELL_EMPTY
                && map[yPoint_17585][xPoint_17585-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17585, yPoint_17585, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17585;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17585][x] === this.CELL_EMPTY
                    && map[yPoint_17585-1][x] === this.CELL_EMPTY
                    && map[yPoint_17585-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17585][x+1] === this.CELL_EMPTY
                    && map[yPoint_17585+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17585+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17585, yPoint_17585, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17585;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17585] === this.CELL_EMPTY
                    && map[y+1][xPoint_17585] === this.CELL_EMPTY
                    && map[y+1][xPoint_17585+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17585] === this.CELL_EMPTY
                    && map[y][xPoint_17585-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17585-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
