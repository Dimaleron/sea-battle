...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13517=0;yPoint_13517<this.gameFieldBorderY.length; yPoint_13517++){
                for(var xPoint_13517=0;xPoint_13517<this.gameFieldBorderX.length; xPoint_13517++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13517, xPoint_13517);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13517][xPoint_13517] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13517, xPoint_13517, 'user');
                    if(this._userShipsMap[yPoint_13517][xPoint_13517] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13517, xPoint_13517, type){
            var id = this.getPointBlockIdByCoords(yPoint_13517, xPoint_13517, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13517);
                block.setAttribute('data-y', yPoint_13517);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13517
         * @param {type} xPoint_13517
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13517, xPoint_13517, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13517 + '_y' + yPoint_13517;
            }
            return 'pc_x' + xPoint_13517 + '_y' + yPoint_13517;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13517=0;yPoint_13517<this.gameFieldBorderY.length; yPoint_13517++){
                for(var xPoint_13517=0;xPoint_13517<this.gameFieldBorderX.length; xPoint_13517++){
                    map.push({y: yPoint_13517, x: xPoint_13517});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13517=-1;yPoint_13517<(this.gameFieldBorderY.length+1); yPoint_13517++){
                for(var xPoint_13517=-1;xPoint_13517<(this.gameFieldBorderX.length+1); xPoint_13517++){
                    if(!map[yPoint_13517]){
                        map[yPoint_13517] = [];
                    }
                    map[yPoint_13517][xPoint_13517] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13517 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13517 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13517, yPoint_13517) === true){
                    if(this.canPutHorizontal(map, xPoint_13517, yPoint_13517, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13517][xPoint_13517 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13517, yPoint_13517, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13517 + i][xPoint_13517] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13517, yPoint_13517){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13517][xPoint_13517] === this.CELL_EMPTY
                && map[yPoint_13517-1][xPoint_13517] === this.CELL_EMPTY
                && map[yPoint_13517-1][xPoint_13517+1] === this.CELL_EMPTY
                && map[yPoint_13517][xPoint_13517+1] === this.CELL_EMPTY
                && map[yPoint_13517+1][xPoint_13517+1] === this.CELL_EMPTY
                && map[yPoint_13517+1][xPoint_13517] === this.CELL_EMPTY
                && map[yPoint_13517+1][xPoint_13517-1] === this.CELL_EMPTY
                && map[yPoint_13517][xPoint_13517-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13517, yPoint_13517, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13517;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13517][x] === this.CELL_EMPTY
                    && map[yPoint_13517-1][x] === this.CELL_EMPTY
                    && map[yPoint_13517-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13517][x+1] === this.CELL_EMPTY
                    && map[yPoint_13517+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13517+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13517, yPoint_13517, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13517;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13517] === this.CELL_EMPTY
                    && map[y+1][xPoint_13517] === this.CELL_EMPTY
                    && map[y+1][xPoint_13517+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13517] === this.CELL_EMPTY
                    && map[y][xPoint_13517-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13517-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
