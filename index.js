...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2488=0;yPoint_2488<this.gameFieldBorderY.length; yPoint_2488++){
                for(var xPoint_2488=0;xPoint_2488<this.gameFieldBorderX.length; xPoint_2488++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2488, xPoint_2488);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2488][xPoint_2488] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2488, xPoint_2488, 'user');
                    if(this._userShipsMap[yPoint_2488][xPoint_2488] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2488, xPoint_2488, type){
            var id = this.getPointBlockIdByCoords(yPoint_2488, xPoint_2488, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2488);
                block.setAttribute('data-y', yPoint_2488);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2488
         * @param {type} xPoint_2488
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2488, xPoint_2488, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2488 + '_y' + yPoint_2488;
            }
            return 'pc_x' + xPoint_2488 + '_y' + yPoint_2488;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2488=0;yPoint_2488<this.gameFieldBorderY.length; yPoint_2488++){
                for(var xPoint_2488=0;xPoint_2488<this.gameFieldBorderX.length; xPoint_2488++){
                    map.push({y: yPoint_2488, x: xPoint_2488});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2488=-1;yPoint_2488<(this.gameFieldBorderY.length+1); yPoint_2488++){
                for(var xPoint_2488=-1;xPoint_2488<(this.gameFieldBorderX.length+1); xPoint_2488++){
                    if(!map[yPoint_2488]){
                        map[yPoint_2488] = [];
                    }
                    map[yPoint_2488][xPoint_2488] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2488 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2488 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2488, yPoint_2488) === true){
                    if(this.canPutHorizontal(map, xPoint_2488, yPoint_2488, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2488][xPoint_2488 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2488, yPoint_2488, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2488 + i][xPoint_2488] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2488, yPoint_2488){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2488][xPoint_2488] === this.CELL_EMPTY
                && map[yPoint_2488-1][xPoint_2488] === this.CELL_EMPTY
                && map[yPoint_2488-1][xPoint_2488+1] === this.CELL_EMPTY
                && map[yPoint_2488][xPoint_2488+1] === this.CELL_EMPTY
                && map[yPoint_2488+1][xPoint_2488+1] === this.CELL_EMPTY
                && map[yPoint_2488+1][xPoint_2488] === this.CELL_EMPTY
                && map[yPoint_2488+1][xPoint_2488-1] === this.CELL_EMPTY
                && map[yPoint_2488][xPoint_2488-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2488, yPoint_2488, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2488;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2488][x] === this.CELL_EMPTY
                    && map[yPoint_2488-1][x] === this.CELL_EMPTY
                    && map[yPoint_2488-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2488][x+1] === this.CELL_EMPTY
                    && map[yPoint_2488+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2488+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2488, yPoint_2488, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2488;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2488] === this.CELL_EMPTY
                    && map[y+1][xPoint_2488] === this.CELL_EMPTY
                    && map[y+1][xPoint_2488+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2488] === this.CELL_EMPTY
                    && map[y][xPoint_2488-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2488-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
