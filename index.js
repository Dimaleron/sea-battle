...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32756=0;yPoint_32756<this.gameFieldBorderY.length; yPoint_32756++){
                for(var xPoint_32756=0;xPoint_32756<this.gameFieldBorderX.length; xPoint_32756++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32756, xPoint_32756);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32756][xPoint_32756] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32756, xPoint_32756, 'user');
                    if(this._userShipsMap[yPoint_32756][xPoint_32756] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32756, xPoint_32756, type){
            var id = this.getPointBlockIdByCoords(yPoint_32756, xPoint_32756, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32756);
                block.setAttribute('data-y', yPoint_32756);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32756
         * @param {type} xPoint_32756
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32756, xPoint_32756, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32756 + '_y' + yPoint_32756;
            }
            return 'pc_x' + xPoint_32756 + '_y' + yPoint_32756;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32756=0;yPoint_32756<this.gameFieldBorderY.length; yPoint_32756++){
                for(var xPoint_32756=0;xPoint_32756<this.gameFieldBorderX.length; xPoint_32756++){
                    map.push({y: yPoint_32756, x: xPoint_32756});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32756=-1;yPoint_32756<(this.gameFieldBorderY.length+1); yPoint_32756++){
                for(var xPoint_32756=-1;xPoint_32756<(this.gameFieldBorderX.length+1); xPoint_32756++){
                    if(!map[yPoint_32756]){
                        map[yPoint_32756] = [];
                    }
                    map[yPoint_32756][xPoint_32756] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32756 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32756 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32756, yPoint_32756) === true){
                    if(this.canPutHorizontal(map, xPoint_32756, yPoint_32756, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32756][xPoint_32756 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32756, yPoint_32756, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32756 + i][xPoint_32756] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32756, yPoint_32756){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32756][xPoint_32756] === this.CELL_EMPTY
                && map[yPoint_32756-1][xPoint_32756] === this.CELL_EMPTY
                && map[yPoint_32756-1][xPoint_32756+1] === this.CELL_EMPTY
                && map[yPoint_32756][xPoint_32756+1] === this.CELL_EMPTY
                && map[yPoint_32756+1][xPoint_32756+1] === this.CELL_EMPTY
                && map[yPoint_32756+1][xPoint_32756] === this.CELL_EMPTY
                && map[yPoint_32756+1][xPoint_32756-1] === this.CELL_EMPTY
                && map[yPoint_32756][xPoint_32756-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32756, yPoint_32756, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32756;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32756][x] === this.CELL_EMPTY
                    && map[yPoint_32756-1][x] === this.CELL_EMPTY
                    && map[yPoint_32756-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32756][x+1] === this.CELL_EMPTY
                    && map[yPoint_32756+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32756+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32756, yPoint_32756, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32756;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32756] === this.CELL_EMPTY
                    && map[y+1][xPoint_32756] === this.CELL_EMPTY
                    && map[y+1][xPoint_32756+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32756] === this.CELL_EMPTY
                    && map[y][xPoint_32756-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32756-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
