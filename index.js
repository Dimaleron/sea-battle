...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15357=0;yPoint_15357<this.gameFieldBorderY.length; yPoint_15357++){
                for(var xPoint_15357=0;xPoint_15357<this.gameFieldBorderX.length; xPoint_15357++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15357, xPoint_15357);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15357][xPoint_15357] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15357, xPoint_15357, 'user');
                    if(this._userShipsMap[yPoint_15357][xPoint_15357] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15357, xPoint_15357, type){
            var id = this.getPointBlockIdByCoords(yPoint_15357, xPoint_15357, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15357);
                block.setAttribute('data-y', yPoint_15357);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15357
         * @param {type} xPoint_15357
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15357, xPoint_15357, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15357 + '_y' + yPoint_15357;
            }
            return 'pc_x' + xPoint_15357 + '_y' + yPoint_15357;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15357=0;yPoint_15357<this.gameFieldBorderY.length; yPoint_15357++){
                for(var xPoint_15357=0;xPoint_15357<this.gameFieldBorderX.length; xPoint_15357++){
                    map.push({y: yPoint_15357, x: xPoint_15357});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15357=-1;yPoint_15357<(this.gameFieldBorderY.length+1); yPoint_15357++){
                for(var xPoint_15357=-1;xPoint_15357<(this.gameFieldBorderX.length+1); xPoint_15357++){
                    if(!map[yPoint_15357]){
                        map[yPoint_15357] = [];
                    }
                    map[yPoint_15357][xPoint_15357] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15357 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15357 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15357, yPoint_15357) === true){
                    if(this.canPutHorizontal(map, xPoint_15357, yPoint_15357, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15357][xPoint_15357 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15357, yPoint_15357, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15357 + i][xPoint_15357] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15357, yPoint_15357){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15357][xPoint_15357] === this.CELL_EMPTY
                && map[yPoint_15357-1][xPoint_15357] === this.CELL_EMPTY
                && map[yPoint_15357-1][xPoint_15357+1] === this.CELL_EMPTY
                && map[yPoint_15357][xPoint_15357+1] === this.CELL_EMPTY
                && map[yPoint_15357+1][xPoint_15357+1] === this.CELL_EMPTY
                && map[yPoint_15357+1][xPoint_15357] === this.CELL_EMPTY
                && map[yPoint_15357+1][xPoint_15357-1] === this.CELL_EMPTY
                && map[yPoint_15357][xPoint_15357-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15357, yPoint_15357, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15357;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15357][x] === this.CELL_EMPTY
                    && map[yPoint_15357-1][x] === this.CELL_EMPTY
                    && map[yPoint_15357-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15357][x+1] === this.CELL_EMPTY
                    && map[yPoint_15357+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15357+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15357, yPoint_15357, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15357;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15357] === this.CELL_EMPTY
                    && map[y+1][xPoint_15357] === this.CELL_EMPTY
                    && map[y+1][xPoint_15357+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15357] === this.CELL_EMPTY
                    && map[y][xPoint_15357-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15357-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
