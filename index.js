...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10524=0;yPoint_10524<this.gameFieldBorderY.length; yPoint_10524++){
                for(var xPoint_10524=0;xPoint_10524<this.gameFieldBorderX.length; xPoint_10524++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10524, xPoint_10524);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10524][xPoint_10524] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10524, xPoint_10524, 'user');
                    if(this._userShipsMap[yPoint_10524][xPoint_10524] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10524, xPoint_10524, type){
            var id = this.getPointBlockIdByCoords(yPoint_10524, xPoint_10524, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10524);
                block.setAttribute('data-y', yPoint_10524);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10524
         * @param {type} xPoint_10524
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10524, xPoint_10524, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10524 + '_y' + yPoint_10524;
            }
            return 'pc_x' + xPoint_10524 + '_y' + yPoint_10524;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10524=0;yPoint_10524<this.gameFieldBorderY.length; yPoint_10524++){
                for(var xPoint_10524=0;xPoint_10524<this.gameFieldBorderX.length; xPoint_10524++){
                    map.push({y: yPoint_10524, x: xPoint_10524});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10524=-1;yPoint_10524<(this.gameFieldBorderY.length+1); yPoint_10524++){
                for(var xPoint_10524=-1;xPoint_10524<(this.gameFieldBorderX.length+1); xPoint_10524++){
                    if(!map[yPoint_10524]){
                        map[yPoint_10524] = [];
                    }
                    map[yPoint_10524][xPoint_10524] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10524 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10524 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10524, yPoint_10524) === true){
                    if(this.canPutHorizontal(map, xPoint_10524, yPoint_10524, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10524][xPoint_10524 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10524, yPoint_10524, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10524 + i][xPoint_10524] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10524, yPoint_10524){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10524][xPoint_10524] === this.CELL_EMPTY
                && map[yPoint_10524-1][xPoint_10524] === this.CELL_EMPTY
                && map[yPoint_10524-1][xPoint_10524+1] === this.CELL_EMPTY
                && map[yPoint_10524][xPoint_10524+1] === this.CELL_EMPTY
                && map[yPoint_10524+1][xPoint_10524+1] === this.CELL_EMPTY
                && map[yPoint_10524+1][xPoint_10524] === this.CELL_EMPTY
                && map[yPoint_10524+1][xPoint_10524-1] === this.CELL_EMPTY
                && map[yPoint_10524][xPoint_10524-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10524, yPoint_10524, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10524;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10524][x] === this.CELL_EMPTY
                    && map[yPoint_10524-1][x] === this.CELL_EMPTY
                    && map[yPoint_10524-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10524][x+1] === this.CELL_EMPTY
                    && map[yPoint_10524+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10524+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10524, yPoint_10524, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10524;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10524] === this.CELL_EMPTY
                    && map[y+1][xPoint_10524] === this.CELL_EMPTY
                    && map[y+1][xPoint_10524+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10524] === this.CELL_EMPTY
                    && map[y][xPoint_10524-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10524-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
