...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14409=0;yPoint_14409<this.gameFieldBorderY.length; yPoint_14409++){
                for(var xPoint_14409=0;xPoint_14409<this.gameFieldBorderX.length; xPoint_14409++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14409, xPoint_14409);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14409][xPoint_14409] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14409, xPoint_14409, 'user');
                    if(this._userShipsMap[yPoint_14409][xPoint_14409] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14409, xPoint_14409, type){
            var id = this.getPointBlockIdByCoords(yPoint_14409, xPoint_14409, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14409);
                block.setAttribute('data-y', yPoint_14409);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14409
         * @param {type} xPoint_14409
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14409, xPoint_14409, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14409 + '_y' + yPoint_14409;
            }
            return 'pc_x' + xPoint_14409 + '_y' + yPoint_14409;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14409=0;yPoint_14409<this.gameFieldBorderY.length; yPoint_14409++){
                for(var xPoint_14409=0;xPoint_14409<this.gameFieldBorderX.length; xPoint_14409++){
                    map.push({y: yPoint_14409, x: xPoint_14409});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14409=-1;yPoint_14409<(this.gameFieldBorderY.length+1); yPoint_14409++){
                for(var xPoint_14409=-1;xPoint_14409<(this.gameFieldBorderX.length+1); xPoint_14409++){
                    if(!map[yPoint_14409]){
                        map[yPoint_14409] = [];
                    }
                    map[yPoint_14409][xPoint_14409] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14409 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14409 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14409, yPoint_14409) === true){
                    if(this.canPutHorizontal(map, xPoint_14409, yPoint_14409, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14409][xPoint_14409 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14409, yPoint_14409, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14409 + i][xPoint_14409] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14409, yPoint_14409){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14409][xPoint_14409] === this.CELL_EMPTY
                && map[yPoint_14409-1][xPoint_14409] === this.CELL_EMPTY
                && map[yPoint_14409-1][xPoint_14409+1] === this.CELL_EMPTY
                && map[yPoint_14409][xPoint_14409+1] === this.CELL_EMPTY
                && map[yPoint_14409+1][xPoint_14409+1] === this.CELL_EMPTY
                && map[yPoint_14409+1][xPoint_14409] === this.CELL_EMPTY
                && map[yPoint_14409+1][xPoint_14409-1] === this.CELL_EMPTY
                && map[yPoint_14409][xPoint_14409-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14409, yPoint_14409, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14409;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14409][x] === this.CELL_EMPTY
                    && map[yPoint_14409-1][x] === this.CELL_EMPTY
                    && map[yPoint_14409-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14409][x+1] === this.CELL_EMPTY
                    && map[yPoint_14409+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14409+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14409, yPoint_14409, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14409;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14409] === this.CELL_EMPTY
                    && map[y+1][xPoint_14409] === this.CELL_EMPTY
                    && map[y+1][xPoint_14409+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14409] === this.CELL_EMPTY
                    && map[y][xPoint_14409-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14409-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
