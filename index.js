...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_201=0;yPoint_201<this.gameFieldBorderY.length; yPoint_201++){
                for(var xPoint_201=0;xPoint_201<this.gameFieldBorderX.length; xPoint_201++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_201, xPoint_201);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_201][xPoint_201] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_201, xPoint_201, 'user');
                    if(this._userShipsMap[yPoint_201][xPoint_201] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_201, xPoint_201, type){
            var id = this.getPointBlockIdByCoords(yPoint_201, xPoint_201, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_201);
                block.setAttribute('data-y', yPoint_201);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_201
         * @param {type} xPoint_201
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_201, xPoint_201, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_201 + '_y' + yPoint_201;
            }
            return 'pc_x' + xPoint_201 + '_y' + yPoint_201;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_201=0;yPoint_201<this.gameFieldBorderY.length; yPoint_201++){
                for(var xPoint_201=0;xPoint_201<this.gameFieldBorderX.length; xPoint_201++){
                    map.push({y: yPoint_201, x: xPoint_201});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_201=-1;yPoint_201<(this.gameFieldBorderY.length+1); yPoint_201++){
                for(var xPoint_201=-1;xPoint_201<(this.gameFieldBorderX.length+1); xPoint_201++){
                    if(!map[yPoint_201]){
                        map[yPoint_201] = [];
                    }
                    map[yPoint_201][xPoint_201] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_201 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_201 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_201, yPoint_201) === true){
                    if(this.canPutHorizontal(map, xPoint_201, yPoint_201, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_201][xPoint_201 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_201, yPoint_201, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_201 + i][xPoint_201] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_201, yPoint_201){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_201][xPoint_201] === this.CELL_EMPTY
                && map[yPoint_201-1][xPoint_201] === this.CELL_EMPTY
                && map[yPoint_201-1][xPoint_201+1] === this.CELL_EMPTY
                && map[yPoint_201][xPoint_201+1] === this.CELL_EMPTY
                && map[yPoint_201+1][xPoint_201+1] === this.CELL_EMPTY
                && map[yPoint_201+1][xPoint_201] === this.CELL_EMPTY
                && map[yPoint_201+1][xPoint_201-1] === this.CELL_EMPTY
                && map[yPoint_201][xPoint_201-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_201, yPoint_201, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_201;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_201][x] === this.CELL_EMPTY
                    && map[yPoint_201-1][x] === this.CELL_EMPTY
                    && map[yPoint_201-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_201][x+1] === this.CELL_EMPTY
                    && map[yPoint_201+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_201+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_201, yPoint_201, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_201;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_201] === this.CELL_EMPTY
                    && map[y+1][xPoint_201] === this.CELL_EMPTY
                    && map[y+1][xPoint_201+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_201] === this.CELL_EMPTY
                    && map[y][xPoint_201-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_201-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
