...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32360=0;yPoint_32360<this.gameFieldBorderY.length; yPoint_32360++){
                for(var xPoint_32360=0;xPoint_32360<this.gameFieldBorderX.length; xPoint_32360++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32360, xPoint_32360);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32360][xPoint_32360] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32360, xPoint_32360, 'user');
                    if(this._userShipsMap[yPoint_32360][xPoint_32360] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32360, xPoint_32360, type){
            var id = this.getPointBlockIdByCoords(yPoint_32360, xPoint_32360, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32360);
                block.setAttribute('data-y', yPoint_32360);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32360
         * @param {type} xPoint_32360
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32360, xPoint_32360, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32360 + '_y' + yPoint_32360;
            }
            return 'pc_x' + xPoint_32360 + '_y' + yPoint_32360;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32360=0;yPoint_32360<this.gameFieldBorderY.length; yPoint_32360++){
                for(var xPoint_32360=0;xPoint_32360<this.gameFieldBorderX.length; xPoint_32360++){
                    map.push({y: yPoint_32360, x: xPoint_32360});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32360=-1;yPoint_32360<(this.gameFieldBorderY.length+1); yPoint_32360++){
                for(var xPoint_32360=-1;xPoint_32360<(this.gameFieldBorderX.length+1); xPoint_32360++){
                    if(!map[yPoint_32360]){
                        map[yPoint_32360] = [];
                    }
                    map[yPoint_32360][xPoint_32360] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32360 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32360 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32360, yPoint_32360) === true){
                    if(this.canPutHorizontal(map, xPoint_32360, yPoint_32360, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32360][xPoint_32360 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32360, yPoint_32360, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32360 + i][xPoint_32360] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32360, yPoint_32360){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32360][xPoint_32360] === this.CELL_EMPTY
                && map[yPoint_32360-1][xPoint_32360] === this.CELL_EMPTY
                && map[yPoint_32360-1][xPoint_32360+1] === this.CELL_EMPTY
                && map[yPoint_32360][xPoint_32360+1] === this.CELL_EMPTY
                && map[yPoint_32360+1][xPoint_32360+1] === this.CELL_EMPTY
                && map[yPoint_32360+1][xPoint_32360] === this.CELL_EMPTY
                && map[yPoint_32360+1][xPoint_32360-1] === this.CELL_EMPTY
                && map[yPoint_32360][xPoint_32360-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32360, yPoint_32360, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32360;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32360][x] === this.CELL_EMPTY
                    && map[yPoint_32360-1][x] === this.CELL_EMPTY
                    && map[yPoint_32360-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32360][x+1] === this.CELL_EMPTY
                    && map[yPoint_32360+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32360+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32360, yPoint_32360, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32360;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32360] === this.CELL_EMPTY
                    && map[y+1][xPoint_32360] === this.CELL_EMPTY
                    && map[y+1][xPoint_32360+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32360] === this.CELL_EMPTY
                    && map[y][xPoint_32360-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32360-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
