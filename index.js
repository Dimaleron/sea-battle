...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_12134=0;yPoint_12134<this.gameFieldBorderY.length; yPoint_12134++){
                for(var xPoint_12134=0;xPoint_12134<this.gameFieldBorderX.length; xPoint_12134++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_12134, xPoint_12134);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_12134][xPoint_12134] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_12134, xPoint_12134, 'user');
                    if(this._userShipsMap[yPoint_12134][xPoint_12134] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_12134, xPoint_12134, type){
            var id = this.getPointBlockIdByCoords(yPoint_12134, xPoint_12134, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_12134);
                block.setAttribute('data-y', yPoint_12134);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_12134
         * @param {type} xPoint_12134
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_12134, xPoint_12134, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_12134 + '_y' + yPoint_12134;
            }
            return 'pc_x' + xPoint_12134 + '_y' + yPoint_12134;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_12134=0;yPoint_12134<this.gameFieldBorderY.length; yPoint_12134++){
                for(var xPoint_12134=0;xPoint_12134<this.gameFieldBorderX.length; xPoint_12134++){
                    map.push({y: yPoint_12134, x: xPoint_12134});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_12134=-1;yPoint_12134<(this.gameFieldBorderY.length+1); yPoint_12134++){
                for(var xPoint_12134=-1;xPoint_12134<(this.gameFieldBorderX.length+1); xPoint_12134++){
                    if(!map[yPoint_12134]){
                        map[yPoint_12134] = [];
                    }
                    map[yPoint_12134][xPoint_12134] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_12134 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_12134 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_12134, yPoint_12134) === true){
                    if(this.canPutHorizontal(map, xPoint_12134, yPoint_12134, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12134][xPoint_12134 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_12134, yPoint_12134, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_12134 + i][xPoint_12134] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_12134, yPoint_12134){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_12134][xPoint_12134] === this.CELL_EMPTY
                && map[yPoint_12134-1][xPoint_12134] === this.CELL_EMPTY
                && map[yPoint_12134-1][xPoint_12134+1] === this.CELL_EMPTY
                && map[yPoint_12134][xPoint_12134+1] === this.CELL_EMPTY
                && map[yPoint_12134+1][xPoint_12134+1] === this.CELL_EMPTY
                && map[yPoint_12134+1][xPoint_12134] === this.CELL_EMPTY
                && map[yPoint_12134+1][xPoint_12134-1] === this.CELL_EMPTY
                && map[yPoint_12134][xPoint_12134-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_12134, yPoint_12134, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_12134;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_12134][x] === this.CELL_EMPTY
                    && map[yPoint_12134-1][x] === this.CELL_EMPTY
                    && map[yPoint_12134-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12134][x+1] === this.CELL_EMPTY
                    && map[yPoint_12134+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_12134+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_12134, yPoint_12134, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_12134;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_12134] === this.CELL_EMPTY
                    && map[y+1][xPoint_12134] === this.CELL_EMPTY
                    && map[y+1][xPoint_12134+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_12134] === this.CELL_EMPTY
                    && map[y][xPoint_12134-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_12134-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
