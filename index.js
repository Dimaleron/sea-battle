...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31096=0;yPoint_31096<this.gameFieldBorderY.length; yPoint_31096++){
                for(var xPoint_31096=0;xPoint_31096<this.gameFieldBorderX.length; xPoint_31096++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31096, xPoint_31096);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31096][xPoint_31096] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31096, xPoint_31096, 'user');
                    if(this._userShipsMap[yPoint_31096][xPoint_31096] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31096, xPoint_31096, type){
            var id = this.getPointBlockIdByCoords(yPoint_31096, xPoint_31096, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31096);
                block.setAttribute('data-y', yPoint_31096);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31096
         * @param {type} xPoint_31096
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31096, xPoint_31096, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31096 + '_y' + yPoint_31096;
            }
            return 'pc_x' + xPoint_31096 + '_y' + yPoint_31096;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31096=0;yPoint_31096<this.gameFieldBorderY.length; yPoint_31096++){
                for(var xPoint_31096=0;xPoint_31096<this.gameFieldBorderX.length; xPoint_31096++){
                    map.push({y: yPoint_31096, x: xPoint_31096});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31096=-1;yPoint_31096<(this.gameFieldBorderY.length+1); yPoint_31096++){
                for(var xPoint_31096=-1;xPoint_31096<(this.gameFieldBorderX.length+1); xPoint_31096++){
                    if(!map[yPoint_31096]){
                        map[yPoint_31096] = [];
                    }
                    map[yPoint_31096][xPoint_31096] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31096 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31096 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31096, yPoint_31096) === true){
                    if(this.canPutHorizontal(map, xPoint_31096, yPoint_31096, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31096][xPoint_31096 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31096, yPoint_31096, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31096 + i][xPoint_31096] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31096, yPoint_31096){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31096][xPoint_31096] === this.CELL_EMPTY
                && map[yPoint_31096-1][xPoint_31096] === this.CELL_EMPTY
                && map[yPoint_31096-1][xPoint_31096+1] === this.CELL_EMPTY
                && map[yPoint_31096][xPoint_31096+1] === this.CELL_EMPTY
                && map[yPoint_31096+1][xPoint_31096+1] === this.CELL_EMPTY
                && map[yPoint_31096+1][xPoint_31096] === this.CELL_EMPTY
                && map[yPoint_31096+1][xPoint_31096-1] === this.CELL_EMPTY
                && map[yPoint_31096][xPoint_31096-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31096, yPoint_31096, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31096;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31096][x] === this.CELL_EMPTY
                    && map[yPoint_31096-1][x] === this.CELL_EMPTY
                    && map[yPoint_31096-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31096][x+1] === this.CELL_EMPTY
                    && map[yPoint_31096+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31096+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31096, yPoint_31096, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31096;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31096] === this.CELL_EMPTY
                    && map[y+1][xPoint_31096] === this.CELL_EMPTY
                    && map[y+1][xPoint_31096+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31096] === this.CELL_EMPTY
                    && map[y][xPoint_31096-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31096-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
