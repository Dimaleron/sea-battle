...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27381=0;yPoint_27381<this.gameFieldBorderY.length; yPoint_27381++){
                for(var xPoint_27381=0;xPoint_27381<this.gameFieldBorderX.length; xPoint_27381++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27381, xPoint_27381);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27381][xPoint_27381] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27381, xPoint_27381, 'user');
                    if(this._userShipsMap[yPoint_27381][xPoint_27381] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27381, xPoint_27381, type){
            var id = this.getPointBlockIdByCoords(yPoint_27381, xPoint_27381, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27381);
                block.setAttribute('data-y', yPoint_27381);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27381
         * @param {type} xPoint_27381
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27381, xPoint_27381, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27381 + '_y' + yPoint_27381;
            }
            return 'pc_x' + xPoint_27381 + '_y' + yPoint_27381;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27381=0;yPoint_27381<this.gameFieldBorderY.length; yPoint_27381++){
                for(var xPoint_27381=0;xPoint_27381<this.gameFieldBorderX.length; xPoint_27381++){
                    map.push({y: yPoint_27381, x: xPoint_27381});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27381=-1;yPoint_27381<(this.gameFieldBorderY.length+1); yPoint_27381++){
                for(var xPoint_27381=-1;xPoint_27381<(this.gameFieldBorderX.length+1); xPoint_27381++){
                    if(!map[yPoint_27381]){
                        map[yPoint_27381] = [];
                    }
                    map[yPoint_27381][xPoint_27381] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27381 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27381 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27381, yPoint_27381) === true){
                    if(this.canPutHorizontal(map, xPoint_27381, yPoint_27381, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27381][xPoint_27381 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27381, yPoint_27381, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27381 + i][xPoint_27381] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27381, yPoint_27381){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27381][xPoint_27381] === this.CELL_EMPTY
                && map[yPoint_27381-1][xPoint_27381] === this.CELL_EMPTY
                && map[yPoint_27381-1][xPoint_27381+1] === this.CELL_EMPTY
                && map[yPoint_27381][xPoint_27381+1] === this.CELL_EMPTY
                && map[yPoint_27381+1][xPoint_27381+1] === this.CELL_EMPTY
                && map[yPoint_27381+1][xPoint_27381] === this.CELL_EMPTY
                && map[yPoint_27381+1][xPoint_27381-1] === this.CELL_EMPTY
                && map[yPoint_27381][xPoint_27381-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27381, yPoint_27381, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27381;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27381][x] === this.CELL_EMPTY
                    && map[yPoint_27381-1][x] === this.CELL_EMPTY
                    && map[yPoint_27381-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27381][x+1] === this.CELL_EMPTY
                    && map[yPoint_27381+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27381+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27381, yPoint_27381, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27381;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27381] === this.CELL_EMPTY
                    && map[y+1][xPoint_27381] === this.CELL_EMPTY
                    && map[y+1][xPoint_27381+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27381] === this.CELL_EMPTY
                    && map[y][xPoint_27381-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27381-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
