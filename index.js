...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9261=0;yPoint_9261<this.gameFieldBorderY.length; yPoint_9261++){
                for(var xPoint_9261=0;xPoint_9261<this.gameFieldBorderX.length; xPoint_9261++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9261, xPoint_9261);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9261][xPoint_9261] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9261, xPoint_9261, 'user');
                    if(this._userShipsMap[yPoint_9261][xPoint_9261] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9261, xPoint_9261, type){
            var id = this.getPointBlockIdByCoords(yPoint_9261, xPoint_9261, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9261);
                block.setAttribute('data-y', yPoint_9261);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9261
         * @param {type} xPoint_9261
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9261, xPoint_9261, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9261 + '_y' + yPoint_9261;
            }
            return 'pc_x' + xPoint_9261 + '_y' + yPoint_9261;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9261=0;yPoint_9261<this.gameFieldBorderY.length; yPoint_9261++){
                for(var xPoint_9261=0;xPoint_9261<this.gameFieldBorderX.length; xPoint_9261++){
                    map.push({y: yPoint_9261, x: xPoint_9261});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9261=-1;yPoint_9261<(this.gameFieldBorderY.length+1); yPoint_9261++){
                for(var xPoint_9261=-1;xPoint_9261<(this.gameFieldBorderX.length+1); xPoint_9261++){
                    if(!map[yPoint_9261]){
                        map[yPoint_9261] = [];
                    }
                    map[yPoint_9261][xPoint_9261] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9261 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9261 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9261, yPoint_9261) === true){
                    if(this.canPutHorizontal(map, xPoint_9261, yPoint_9261, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9261][xPoint_9261 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9261, yPoint_9261, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9261 + i][xPoint_9261] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9261, yPoint_9261){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9261][xPoint_9261] === this.CELL_EMPTY
                && map[yPoint_9261-1][xPoint_9261] === this.CELL_EMPTY
                && map[yPoint_9261-1][xPoint_9261+1] === this.CELL_EMPTY
                && map[yPoint_9261][xPoint_9261+1] === this.CELL_EMPTY
                && map[yPoint_9261+1][xPoint_9261+1] === this.CELL_EMPTY
                && map[yPoint_9261+1][xPoint_9261] === this.CELL_EMPTY
                && map[yPoint_9261+1][xPoint_9261-1] === this.CELL_EMPTY
                && map[yPoint_9261][xPoint_9261-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9261, yPoint_9261, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9261;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9261][x] === this.CELL_EMPTY
                    && map[yPoint_9261-1][x] === this.CELL_EMPTY
                    && map[yPoint_9261-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9261][x+1] === this.CELL_EMPTY
                    && map[yPoint_9261+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9261+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9261, yPoint_9261, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9261;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9261] === this.CELL_EMPTY
                    && map[y+1][xPoint_9261] === this.CELL_EMPTY
                    && map[y+1][xPoint_9261+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9261] === this.CELL_EMPTY
                    && map[y][xPoint_9261-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9261-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
