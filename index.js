...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3730=0;yPoint_3730<this.gameFieldBorderY.length; yPoint_3730++){
                for(var xPoint_3730=0;xPoint_3730<this.gameFieldBorderX.length; xPoint_3730++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3730, xPoint_3730);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3730][xPoint_3730] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3730, xPoint_3730, 'user');
                    if(this._userShipsMap[yPoint_3730][xPoint_3730] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3730, xPoint_3730, type){
            var id = this.getPointBlockIdByCoords(yPoint_3730, xPoint_3730, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3730);
                block.setAttribute('data-y', yPoint_3730);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3730
         * @param {type} xPoint_3730
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3730, xPoint_3730, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3730 + '_y' + yPoint_3730;
            }
            return 'pc_x' + xPoint_3730 + '_y' + yPoint_3730;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3730=0;yPoint_3730<this.gameFieldBorderY.length; yPoint_3730++){
                for(var xPoint_3730=0;xPoint_3730<this.gameFieldBorderX.length; xPoint_3730++){
                    map.push({y: yPoint_3730, x: xPoint_3730});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3730=-1;yPoint_3730<(this.gameFieldBorderY.length+1); yPoint_3730++){
                for(var xPoint_3730=-1;xPoint_3730<(this.gameFieldBorderX.length+1); xPoint_3730++){
                    if(!map[yPoint_3730]){
                        map[yPoint_3730] = [];
                    }
                    map[yPoint_3730][xPoint_3730] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3730 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3730 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3730, yPoint_3730) === true){
                    if(this.canPutHorizontal(map, xPoint_3730, yPoint_3730, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3730][xPoint_3730 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3730, yPoint_3730, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3730 + i][xPoint_3730] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3730, yPoint_3730){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3730][xPoint_3730] === this.CELL_EMPTY
                && map[yPoint_3730-1][xPoint_3730] === this.CELL_EMPTY
                && map[yPoint_3730-1][xPoint_3730+1] === this.CELL_EMPTY
                && map[yPoint_3730][xPoint_3730+1] === this.CELL_EMPTY
                && map[yPoint_3730+1][xPoint_3730+1] === this.CELL_EMPTY
                && map[yPoint_3730+1][xPoint_3730] === this.CELL_EMPTY
                && map[yPoint_3730+1][xPoint_3730-1] === this.CELL_EMPTY
                && map[yPoint_3730][xPoint_3730-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3730, yPoint_3730, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3730;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3730][x] === this.CELL_EMPTY
                    && map[yPoint_3730-1][x] === this.CELL_EMPTY
                    && map[yPoint_3730-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3730][x+1] === this.CELL_EMPTY
                    && map[yPoint_3730+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3730+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3730, yPoint_3730, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3730;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3730] === this.CELL_EMPTY
                    && map[y+1][xPoint_3730] === this.CELL_EMPTY
                    && map[y+1][xPoint_3730+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3730] === this.CELL_EMPTY
                    && map[y][xPoint_3730-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3730-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
