...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8291=0;yPoint_8291<this.gameFieldBorderY.length; yPoint_8291++){
                for(var xPoint_8291=0;xPoint_8291<this.gameFieldBorderX.length; xPoint_8291++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8291, xPoint_8291);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8291][xPoint_8291] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8291, xPoint_8291, 'user');
                    if(this._userShipsMap[yPoint_8291][xPoint_8291] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8291, xPoint_8291, type){
            var id = this.getPointBlockIdByCoords(yPoint_8291, xPoint_8291, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8291);
                block.setAttribute('data-y', yPoint_8291);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8291
         * @param {type} xPoint_8291
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8291, xPoint_8291, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8291 + '_y' + yPoint_8291;
            }
            return 'pc_x' + xPoint_8291 + '_y' + yPoint_8291;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8291=0;yPoint_8291<this.gameFieldBorderY.length; yPoint_8291++){
                for(var xPoint_8291=0;xPoint_8291<this.gameFieldBorderX.length; xPoint_8291++){
                    map.push({y: yPoint_8291, x: xPoint_8291});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8291=-1;yPoint_8291<(this.gameFieldBorderY.length+1); yPoint_8291++){
                for(var xPoint_8291=-1;xPoint_8291<(this.gameFieldBorderX.length+1); xPoint_8291++){
                    if(!map[yPoint_8291]){
                        map[yPoint_8291] = [];
                    }
                    map[yPoint_8291][xPoint_8291] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8291 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8291 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8291, yPoint_8291) === true){
                    if(this.canPutHorizontal(map, xPoint_8291, yPoint_8291, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8291][xPoint_8291 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8291, yPoint_8291, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8291 + i][xPoint_8291] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8291, yPoint_8291){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8291][xPoint_8291] === this.CELL_EMPTY
                && map[yPoint_8291-1][xPoint_8291] === this.CELL_EMPTY
                && map[yPoint_8291-1][xPoint_8291+1] === this.CELL_EMPTY
                && map[yPoint_8291][xPoint_8291+1] === this.CELL_EMPTY
                && map[yPoint_8291+1][xPoint_8291+1] === this.CELL_EMPTY
                && map[yPoint_8291+1][xPoint_8291] === this.CELL_EMPTY
                && map[yPoint_8291+1][xPoint_8291-1] === this.CELL_EMPTY
                && map[yPoint_8291][xPoint_8291-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8291, yPoint_8291, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8291;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8291][x] === this.CELL_EMPTY
                    && map[yPoint_8291-1][x] === this.CELL_EMPTY
                    && map[yPoint_8291-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8291][x+1] === this.CELL_EMPTY
                    && map[yPoint_8291+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8291+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8291, yPoint_8291, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8291;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8291] === this.CELL_EMPTY
                    && map[y+1][xPoint_8291] === this.CELL_EMPTY
                    && map[y+1][xPoint_8291+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8291] === this.CELL_EMPTY
                    && map[y][xPoint_8291-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8291-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
