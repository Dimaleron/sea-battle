...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29830=0;yPoint_29830<this.gameFieldBorderY.length; yPoint_29830++){
                for(var xPoint_29830=0;xPoint_29830<this.gameFieldBorderX.length; xPoint_29830++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29830, xPoint_29830);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29830][xPoint_29830] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29830, xPoint_29830, 'user');
                    if(this._userShipsMap[yPoint_29830][xPoint_29830] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29830, xPoint_29830, type){
            var id = this.getPointBlockIdByCoords(yPoint_29830, xPoint_29830, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29830);
                block.setAttribute('data-y', yPoint_29830);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29830
         * @param {type} xPoint_29830
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29830, xPoint_29830, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29830 + '_y' + yPoint_29830;
            }
            return 'pc_x' + xPoint_29830 + '_y' + yPoint_29830;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29830=0;yPoint_29830<this.gameFieldBorderY.length; yPoint_29830++){
                for(var xPoint_29830=0;xPoint_29830<this.gameFieldBorderX.length; xPoint_29830++){
                    map.push({y: yPoint_29830, x: xPoint_29830});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29830=-1;yPoint_29830<(this.gameFieldBorderY.length+1); yPoint_29830++){
                for(var xPoint_29830=-1;xPoint_29830<(this.gameFieldBorderX.length+1); xPoint_29830++){
                    if(!map[yPoint_29830]){
                        map[yPoint_29830] = [];
                    }
                    map[yPoint_29830][xPoint_29830] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29830 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29830 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29830, yPoint_29830) === true){
                    if(this.canPutHorizontal(map, xPoint_29830, yPoint_29830, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29830][xPoint_29830 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29830, yPoint_29830, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29830 + i][xPoint_29830] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29830, yPoint_29830){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29830][xPoint_29830] === this.CELL_EMPTY
                && map[yPoint_29830-1][xPoint_29830] === this.CELL_EMPTY
                && map[yPoint_29830-1][xPoint_29830+1] === this.CELL_EMPTY
                && map[yPoint_29830][xPoint_29830+1] === this.CELL_EMPTY
                && map[yPoint_29830+1][xPoint_29830+1] === this.CELL_EMPTY
                && map[yPoint_29830+1][xPoint_29830] === this.CELL_EMPTY
                && map[yPoint_29830+1][xPoint_29830-1] === this.CELL_EMPTY
                && map[yPoint_29830][xPoint_29830-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29830, yPoint_29830, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29830;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29830][x] === this.CELL_EMPTY
                    && map[yPoint_29830-1][x] === this.CELL_EMPTY
                    && map[yPoint_29830-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29830][x+1] === this.CELL_EMPTY
                    && map[yPoint_29830+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29830+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29830, yPoint_29830, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29830;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29830] === this.CELL_EMPTY
                    && map[y+1][xPoint_29830] === this.CELL_EMPTY
                    && map[y+1][xPoint_29830+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29830] === this.CELL_EMPTY
                    && map[y][xPoint_29830-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29830-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
