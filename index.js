...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_823=0;yPoint_823<this.gameFieldBorderY.length; yPoint_823++){
                for(var xPoint_823=0;xPoint_823<this.gameFieldBorderX.length; xPoint_823++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_823, xPoint_823);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_823][xPoint_823] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_823, xPoint_823, 'user');
                    if(this._userShipsMap[yPoint_823][xPoint_823] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_823, xPoint_823, type){
            var id = this.getPointBlockIdByCoords(yPoint_823, xPoint_823, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_823);
                block.setAttribute('data-y', yPoint_823);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_823
         * @param {type} xPoint_823
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_823, xPoint_823, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_823 + '_y' + yPoint_823;
            }
            return 'pc_x' + xPoint_823 + '_y' + yPoint_823;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_823=0;yPoint_823<this.gameFieldBorderY.length; yPoint_823++){
                for(var xPoint_823=0;xPoint_823<this.gameFieldBorderX.length; xPoint_823++){
                    map.push({y: yPoint_823, x: xPoint_823});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_823=-1;yPoint_823<(this.gameFieldBorderY.length+1); yPoint_823++){
                for(var xPoint_823=-1;xPoint_823<(this.gameFieldBorderX.length+1); xPoint_823++){
                    if(!map[yPoint_823]){
                        map[yPoint_823] = [];
                    }
                    map[yPoint_823][xPoint_823] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_823 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_823 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_823, yPoint_823) === true){
                    if(this.canPutHorizontal(map, xPoint_823, yPoint_823, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_823][xPoint_823 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_823, yPoint_823, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_823 + i][xPoint_823] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_823, yPoint_823){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_823][xPoint_823] === this.CELL_EMPTY
                && map[yPoint_823-1][xPoint_823] === this.CELL_EMPTY
                && map[yPoint_823-1][xPoint_823+1] === this.CELL_EMPTY
                && map[yPoint_823][xPoint_823+1] === this.CELL_EMPTY
                && map[yPoint_823+1][xPoint_823+1] === this.CELL_EMPTY
                && map[yPoint_823+1][xPoint_823] === this.CELL_EMPTY
                && map[yPoint_823+1][xPoint_823-1] === this.CELL_EMPTY
                && map[yPoint_823][xPoint_823-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_823, yPoint_823, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_823;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_823][x] === this.CELL_EMPTY
                    && map[yPoint_823-1][x] === this.CELL_EMPTY
                    && map[yPoint_823-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_823][x+1] === this.CELL_EMPTY
                    && map[yPoint_823+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_823+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_823, yPoint_823, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_823;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_823] === this.CELL_EMPTY
                    && map[y+1][xPoint_823] === this.CELL_EMPTY
                    && map[y+1][xPoint_823+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_823] === this.CELL_EMPTY
                    && map[y][xPoint_823-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_823-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
