...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13811=0;yPoint_13811<this.gameFieldBorderY.length; yPoint_13811++){
                for(var xPoint_13811=0;xPoint_13811<this.gameFieldBorderX.length; xPoint_13811++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13811, xPoint_13811);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13811][xPoint_13811] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13811, xPoint_13811, 'user');
                    if(this._userShipsMap[yPoint_13811][xPoint_13811] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13811, xPoint_13811, type){
            var id = this.getPointBlockIdByCoords(yPoint_13811, xPoint_13811, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13811);
                block.setAttribute('data-y', yPoint_13811);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13811
         * @param {type} xPoint_13811
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13811, xPoint_13811, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13811 + '_y' + yPoint_13811;
            }
            return 'pc_x' + xPoint_13811 + '_y' + yPoint_13811;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13811=0;yPoint_13811<this.gameFieldBorderY.length; yPoint_13811++){
                for(var xPoint_13811=0;xPoint_13811<this.gameFieldBorderX.length; xPoint_13811++){
                    map.push({y: yPoint_13811, x: xPoint_13811});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13811=-1;yPoint_13811<(this.gameFieldBorderY.length+1); yPoint_13811++){
                for(var xPoint_13811=-1;xPoint_13811<(this.gameFieldBorderX.length+1); xPoint_13811++){
                    if(!map[yPoint_13811]){
                        map[yPoint_13811] = [];
                    }
                    map[yPoint_13811][xPoint_13811] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13811 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13811 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13811, yPoint_13811) === true){
                    if(this.canPutHorizontal(map, xPoint_13811, yPoint_13811, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13811][xPoint_13811 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13811, yPoint_13811, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13811 + i][xPoint_13811] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13811, yPoint_13811){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13811][xPoint_13811] === this.CELL_EMPTY
                && map[yPoint_13811-1][xPoint_13811] === this.CELL_EMPTY
                && map[yPoint_13811-1][xPoint_13811+1] === this.CELL_EMPTY
                && map[yPoint_13811][xPoint_13811+1] === this.CELL_EMPTY
                && map[yPoint_13811+1][xPoint_13811+1] === this.CELL_EMPTY
                && map[yPoint_13811+1][xPoint_13811] === this.CELL_EMPTY
                && map[yPoint_13811+1][xPoint_13811-1] === this.CELL_EMPTY
                && map[yPoint_13811][xPoint_13811-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13811, yPoint_13811, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13811;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13811][x] === this.CELL_EMPTY
                    && map[yPoint_13811-1][x] === this.CELL_EMPTY
                    && map[yPoint_13811-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13811][x+1] === this.CELL_EMPTY
                    && map[yPoint_13811+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13811+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13811, yPoint_13811, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13811;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13811] === this.CELL_EMPTY
                    && map[y+1][xPoint_13811] === this.CELL_EMPTY
                    && map[y+1][xPoint_13811+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13811] === this.CELL_EMPTY
                    && map[y][xPoint_13811-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13811-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
