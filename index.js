...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_904=0;yPoint_904<this.gameFieldBorderY.length; yPoint_904++){
                for(var xPoint_904=0;xPoint_904<this.gameFieldBorderX.length; xPoint_904++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_904, xPoint_904);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_904][xPoint_904] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_904, xPoint_904, 'user');
                    if(this._userShipsMap[yPoint_904][xPoint_904] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_904, xPoint_904, type){
            var id = this.getPointBlockIdByCoords(yPoint_904, xPoint_904, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_904);
                block.setAttribute('data-y', yPoint_904);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_904
         * @param {type} xPoint_904
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_904, xPoint_904, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_904 + '_y' + yPoint_904;
            }
            return 'pc_x' + xPoint_904 + '_y' + yPoint_904;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_904=0;yPoint_904<this.gameFieldBorderY.length; yPoint_904++){
                for(var xPoint_904=0;xPoint_904<this.gameFieldBorderX.length; xPoint_904++){
                    map.push({y: yPoint_904, x: xPoint_904});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_904=-1;yPoint_904<(this.gameFieldBorderY.length+1); yPoint_904++){
                for(var xPoint_904=-1;xPoint_904<(this.gameFieldBorderX.length+1); xPoint_904++){
                    if(!map[yPoint_904]){
                        map[yPoint_904] = [];
                    }
                    map[yPoint_904][xPoint_904] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_904 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_904 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_904, yPoint_904) === true){
                    if(this.canPutHorizontal(map, xPoint_904, yPoint_904, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_904][xPoint_904 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_904, yPoint_904, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_904 + i][xPoint_904] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_904, yPoint_904){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_904][xPoint_904] === this.CELL_EMPTY
                && map[yPoint_904-1][xPoint_904] === this.CELL_EMPTY
                && map[yPoint_904-1][xPoint_904+1] === this.CELL_EMPTY
                && map[yPoint_904][xPoint_904+1] === this.CELL_EMPTY
                && map[yPoint_904+1][xPoint_904+1] === this.CELL_EMPTY
                && map[yPoint_904+1][xPoint_904] === this.CELL_EMPTY
                && map[yPoint_904+1][xPoint_904-1] === this.CELL_EMPTY
                && map[yPoint_904][xPoint_904-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_904, yPoint_904, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_904;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_904][x] === this.CELL_EMPTY
                    && map[yPoint_904-1][x] === this.CELL_EMPTY
                    && map[yPoint_904-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_904][x+1] === this.CELL_EMPTY
                    && map[yPoint_904+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_904+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_904, yPoint_904, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_904;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_904] === this.CELL_EMPTY
                    && map[y+1][xPoint_904] === this.CELL_EMPTY
                    && map[y+1][xPoint_904+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_904] === this.CELL_EMPTY
                    && map[y][xPoint_904-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_904-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
