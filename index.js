...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4978=0;yPoint_4978<this.gameFieldBorderY.length; yPoint_4978++){
                for(var xPoint_4978=0;xPoint_4978<this.gameFieldBorderX.length; xPoint_4978++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4978, xPoint_4978);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4978][xPoint_4978] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4978, xPoint_4978, 'user');
                    if(this._userShipsMap[yPoint_4978][xPoint_4978] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4978, xPoint_4978, type){
            var id = this.getPointBlockIdByCoords(yPoint_4978, xPoint_4978, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4978);
                block.setAttribute('data-y', yPoint_4978);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4978
         * @param {type} xPoint_4978
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4978, xPoint_4978, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4978 + '_y' + yPoint_4978;
            }
            return 'pc_x' + xPoint_4978 + '_y' + yPoint_4978;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4978=0;yPoint_4978<this.gameFieldBorderY.length; yPoint_4978++){
                for(var xPoint_4978=0;xPoint_4978<this.gameFieldBorderX.length; xPoint_4978++){
                    map.push({y: yPoint_4978, x: xPoint_4978});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4978=-1;yPoint_4978<(this.gameFieldBorderY.length+1); yPoint_4978++){
                for(var xPoint_4978=-1;xPoint_4978<(this.gameFieldBorderX.length+1); xPoint_4978++){
                    if(!map[yPoint_4978]){
                        map[yPoint_4978] = [];
                    }
                    map[yPoint_4978][xPoint_4978] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4978 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4978 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4978, yPoint_4978) === true){
                    if(this.canPutHorizontal(map, xPoint_4978, yPoint_4978, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4978][xPoint_4978 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4978, yPoint_4978, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4978 + i][xPoint_4978] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4978, yPoint_4978){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4978][xPoint_4978] === this.CELL_EMPTY
                && map[yPoint_4978-1][xPoint_4978] === this.CELL_EMPTY
                && map[yPoint_4978-1][xPoint_4978+1] === this.CELL_EMPTY
                && map[yPoint_4978][xPoint_4978+1] === this.CELL_EMPTY
                && map[yPoint_4978+1][xPoint_4978+1] === this.CELL_EMPTY
                && map[yPoint_4978+1][xPoint_4978] === this.CELL_EMPTY
                && map[yPoint_4978+1][xPoint_4978-1] === this.CELL_EMPTY
                && map[yPoint_4978][xPoint_4978-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4978, yPoint_4978, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4978;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4978][x] === this.CELL_EMPTY
                    && map[yPoint_4978-1][x] === this.CELL_EMPTY
                    && map[yPoint_4978-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4978][x+1] === this.CELL_EMPTY
                    && map[yPoint_4978+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4978+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4978, yPoint_4978, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4978;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4978] === this.CELL_EMPTY
                    && map[y+1][xPoint_4978] === this.CELL_EMPTY
                    && map[y+1][xPoint_4978+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4978] === this.CELL_EMPTY
                    && map[y][xPoint_4978-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4978-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
