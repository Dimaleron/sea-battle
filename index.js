...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3334=0;yPoint_3334<this.gameFieldBorderY.length; yPoint_3334++){
                for(var xPoint_3334=0;xPoint_3334<this.gameFieldBorderX.length; xPoint_3334++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3334, xPoint_3334);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3334][xPoint_3334] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3334, xPoint_3334, 'user');
                    if(this._userShipsMap[yPoint_3334][xPoint_3334] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3334, xPoint_3334, type){
            var id = this.getPointBlockIdByCoords(yPoint_3334, xPoint_3334, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3334);
                block.setAttribute('data-y', yPoint_3334);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3334
         * @param {type} xPoint_3334
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3334, xPoint_3334, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3334 + '_y' + yPoint_3334;
            }
            return 'pc_x' + xPoint_3334 + '_y' + yPoint_3334;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3334=0;yPoint_3334<this.gameFieldBorderY.length; yPoint_3334++){
                for(var xPoint_3334=0;xPoint_3334<this.gameFieldBorderX.length; xPoint_3334++){
                    map.push({y: yPoint_3334, x: xPoint_3334});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3334=-1;yPoint_3334<(this.gameFieldBorderY.length+1); yPoint_3334++){
                for(var xPoint_3334=-1;xPoint_3334<(this.gameFieldBorderX.length+1); xPoint_3334++){
                    if(!map[yPoint_3334]){
                        map[yPoint_3334] = [];
                    }
                    map[yPoint_3334][xPoint_3334] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3334 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3334 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3334, yPoint_3334) === true){
                    if(this.canPutHorizontal(map, xPoint_3334, yPoint_3334, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3334][xPoint_3334 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3334, yPoint_3334, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3334 + i][xPoint_3334] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3334, yPoint_3334){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3334][xPoint_3334] === this.CELL_EMPTY
                && map[yPoint_3334-1][xPoint_3334] === this.CELL_EMPTY
                && map[yPoint_3334-1][xPoint_3334+1] === this.CELL_EMPTY
                && map[yPoint_3334][xPoint_3334+1] === this.CELL_EMPTY
                && map[yPoint_3334+1][xPoint_3334+1] === this.CELL_EMPTY
                && map[yPoint_3334+1][xPoint_3334] === this.CELL_EMPTY
                && map[yPoint_3334+1][xPoint_3334-1] === this.CELL_EMPTY
                && map[yPoint_3334][xPoint_3334-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3334, yPoint_3334, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3334;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3334][x] === this.CELL_EMPTY
                    && map[yPoint_3334-1][x] === this.CELL_EMPTY
                    && map[yPoint_3334-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3334][x+1] === this.CELL_EMPTY
                    && map[yPoint_3334+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3334+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3334, yPoint_3334, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3334;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3334] === this.CELL_EMPTY
                    && map[y+1][xPoint_3334] === this.CELL_EMPTY
                    && map[y+1][xPoint_3334+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3334] === this.CELL_EMPTY
                    && map[y][xPoint_3334-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3334-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
