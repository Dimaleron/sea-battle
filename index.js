...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21742=0;yPoint_21742<this.gameFieldBorderY.length; yPoint_21742++){
                for(var xPoint_21742=0;xPoint_21742<this.gameFieldBorderX.length; xPoint_21742++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21742, xPoint_21742);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21742][xPoint_21742] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21742, xPoint_21742, 'user');
                    if(this._userShipsMap[yPoint_21742][xPoint_21742] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21742, xPoint_21742, type){
            var id = this.getPointBlockIdByCoords(yPoint_21742, xPoint_21742, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21742);
                block.setAttribute('data-y', yPoint_21742);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21742
         * @param {type} xPoint_21742
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21742, xPoint_21742, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21742 + '_y' + yPoint_21742;
            }
            return 'pc_x' + xPoint_21742 + '_y' + yPoint_21742;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21742=0;yPoint_21742<this.gameFieldBorderY.length; yPoint_21742++){
                for(var xPoint_21742=0;xPoint_21742<this.gameFieldBorderX.length; xPoint_21742++){
                    map.push({y: yPoint_21742, x: xPoint_21742});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21742=-1;yPoint_21742<(this.gameFieldBorderY.length+1); yPoint_21742++){
                for(var xPoint_21742=-1;xPoint_21742<(this.gameFieldBorderX.length+1); xPoint_21742++){
                    if(!map[yPoint_21742]){
                        map[yPoint_21742] = [];
                    }
                    map[yPoint_21742][xPoint_21742] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21742 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21742 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21742, yPoint_21742) === true){
                    if(this.canPutHorizontal(map, xPoint_21742, yPoint_21742, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21742][xPoint_21742 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21742, yPoint_21742, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21742 + i][xPoint_21742] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21742, yPoint_21742){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21742][xPoint_21742] === this.CELL_EMPTY
                && map[yPoint_21742-1][xPoint_21742] === this.CELL_EMPTY
                && map[yPoint_21742-1][xPoint_21742+1] === this.CELL_EMPTY
                && map[yPoint_21742][xPoint_21742+1] === this.CELL_EMPTY
                && map[yPoint_21742+1][xPoint_21742+1] === this.CELL_EMPTY
                && map[yPoint_21742+1][xPoint_21742] === this.CELL_EMPTY
                && map[yPoint_21742+1][xPoint_21742-1] === this.CELL_EMPTY
                && map[yPoint_21742][xPoint_21742-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21742, yPoint_21742, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21742;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21742][x] === this.CELL_EMPTY
                    && map[yPoint_21742-1][x] === this.CELL_EMPTY
                    && map[yPoint_21742-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21742][x+1] === this.CELL_EMPTY
                    && map[yPoint_21742+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21742+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21742, yPoint_21742, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21742;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21742] === this.CELL_EMPTY
                    && map[y+1][xPoint_21742] === this.CELL_EMPTY
                    && map[y+1][xPoint_21742+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21742] === this.CELL_EMPTY
                    && map[y][xPoint_21742-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21742-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
