...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9917=0;yPoint_9917<this.gameFieldBorderY.length; yPoint_9917++){
                for(var xPoint_9917=0;xPoint_9917<this.gameFieldBorderX.length; xPoint_9917++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9917, xPoint_9917);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9917][xPoint_9917] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9917, xPoint_9917, 'user');
                    if(this._userShipsMap[yPoint_9917][xPoint_9917] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9917, xPoint_9917, type){
            var id = this.getPointBlockIdByCoords(yPoint_9917, xPoint_9917, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9917);
                block.setAttribute('data-y', yPoint_9917);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9917
         * @param {type} xPoint_9917
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9917, xPoint_9917, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9917 + '_y' + yPoint_9917;
            }
            return 'pc_x' + xPoint_9917 + '_y' + yPoint_9917;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9917=0;yPoint_9917<this.gameFieldBorderY.length; yPoint_9917++){
                for(var xPoint_9917=0;xPoint_9917<this.gameFieldBorderX.length; xPoint_9917++){
                    map.push({y: yPoint_9917, x: xPoint_9917});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9917=-1;yPoint_9917<(this.gameFieldBorderY.length+1); yPoint_9917++){
                for(var xPoint_9917=-1;xPoint_9917<(this.gameFieldBorderX.length+1); xPoint_9917++){
                    if(!map[yPoint_9917]){
                        map[yPoint_9917] = [];
                    }
                    map[yPoint_9917][xPoint_9917] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9917 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9917 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9917, yPoint_9917) === true){
                    if(this.canPutHorizontal(map, xPoint_9917, yPoint_9917, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9917][xPoint_9917 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9917, yPoint_9917, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9917 + i][xPoint_9917] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9917, yPoint_9917){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9917][xPoint_9917] === this.CELL_EMPTY
                && map[yPoint_9917-1][xPoint_9917] === this.CELL_EMPTY
                && map[yPoint_9917-1][xPoint_9917+1] === this.CELL_EMPTY
                && map[yPoint_9917][xPoint_9917+1] === this.CELL_EMPTY
                && map[yPoint_9917+1][xPoint_9917+1] === this.CELL_EMPTY
                && map[yPoint_9917+1][xPoint_9917] === this.CELL_EMPTY
                && map[yPoint_9917+1][xPoint_9917-1] === this.CELL_EMPTY
                && map[yPoint_9917][xPoint_9917-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9917, yPoint_9917, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9917;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9917][x] === this.CELL_EMPTY
                    && map[yPoint_9917-1][x] === this.CELL_EMPTY
                    && map[yPoint_9917-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9917][x+1] === this.CELL_EMPTY
                    && map[yPoint_9917+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9917+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9917, yPoint_9917, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9917;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9917] === this.CELL_EMPTY
                    && map[y+1][xPoint_9917] === this.CELL_EMPTY
                    && map[y+1][xPoint_9917+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9917] === this.CELL_EMPTY
                    && map[y][xPoint_9917-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9917-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
