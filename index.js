...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3832=0;yPoint_3832<this.gameFieldBorderY.length; yPoint_3832++){
                for(var xPoint_3832=0;xPoint_3832<this.gameFieldBorderX.length; xPoint_3832++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3832, xPoint_3832);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3832][xPoint_3832] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3832, xPoint_3832, 'user');
                    if(this._userShipsMap[yPoint_3832][xPoint_3832] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3832, xPoint_3832, type){
            var id = this.getPointBlockIdByCoords(yPoint_3832, xPoint_3832, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3832);
                block.setAttribute('data-y', yPoint_3832);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3832
         * @param {type} xPoint_3832
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3832, xPoint_3832, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3832 + '_y' + yPoint_3832;
            }
            return 'pc_x' + xPoint_3832 + '_y' + yPoint_3832;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3832=0;yPoint_3832<this.gameFieldBorderY.length; yPoint_3832++){
                for(var xPoint_3832=0;xPoint_3832<this.gameFieldBorderX.length; xPoint_3832++){
                    map.push({y: yPoint_3832, x: xPoint_3832});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3832=-1;yPoint_3832<(this.gameFieldBorderY.length+1); yPoint_3832++){
                for(var xPoint_3832=-1;xPoint_3832<(this.gameFieldBorderX.length+1); xPoint_3832++){
                    if(!map[yPoint_3832]){
                        map[yPoint_3832] = [];
                    }
                    map[yPoint_3832][xPoint_3832] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3832 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3832 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3832, yPoint_3832) === true){
                    if(this.canPutHorizontal(map, xPoint_3832, yPoint_3832, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3832][xPoint_3832 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3832, yPoint_3832, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3832 + i][xPoint_3832] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3832, yPoint_3832){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3832][xPoint_3832] === this.CELL_EMPTY
                && map[yPoint_3832-1][xPoint_3832] === this.CELL_EMPTY
                && map[yPoint_3832-1][xPoint_3832+1] === this.CELL_EMPTY
                && map[yPoint_3832][xPoint_3832+1] === this.CELL_EMPTY
                && map[yPoint_3832+1][xPoint_3832+1] === this.CELL_EMPTY
                && map[yPoint_3832+1][xPoint_3832] === this.CELL_EMPTY
                && map[yPoint_3832+1][xPoint_3832-1] === this.CELL_EMPTY
                && map[yPoint_3832][xPoint_3832-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3832, yPoint_3832, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3832;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3832][x] === this.CELL_EMPTY
                    && map[yPoint_3832-1][x] === this.CELL_EMPTY
                    && map[yPoint_3832-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3832][x+1] === this.CELL_EMPTY
                    && map[yPoint_3832+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3832+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3832, yPoint_3832, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3832;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3832] === this.CELL_EMPTY
                    && map[y+1][xPoint_3832] === this.CELL_EMPTY
                    && map[y+1][xPoint_3832+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3832] === this.CELL_EMPTY
                    && map[y][xPoint_3832-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3832-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
