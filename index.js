...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15718=0;yPoint_15718<this.gameFieldBorderY.length; yPoint_15718++){
                for(var xPoint_15718=0;xPoint_15718<this.gameFieldBorderX.length; xPoint_15718++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15718, xPoint_15718);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15718][xPoint_15718] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15718, xPoint_15718, 'user');
                    if(this._userShipsMap[yPoint_15718][xPoint_15718] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15718, xPoint_15718, type){
            var id = this.getPointBlockIdByCoords(yPoint_15718, xPoint_15718, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15718);
                block.setAttribute('data-y', yPoint_15718);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15718
         * @param {type} xPoint_15718
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15718, xPoint_15718, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15718 + '_y' + yPoint_15718;
            }
            return 'pc_x' + xPoint_15718 + '_y' + yPoint_15718;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15718=0;yPoint_15718<this.gameFieldBorderY.length; yPoint_15718++){
                for(var xPoint_15718=0;xPoint_15718<this.gameFieldBorderX.length; xPoint_15718++){
                    map.push({y: yPoint_15718, x: xPoint_15718});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15718=-1;yPoint_15718<(this.gameFieldBorderY.length+1); yPoint_15718++){
                for(var xPoint_15718=-1;xPoint_15718<(this.gameFieldBorderX.length+1); xPoint_15718++){
                    if(!map[yPoint_15718]){
                        map[yPoint_15718] = [];
                    }
                    map[yPoint_15718][xPoint_15718] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15718 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15718 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15718, yPoint_15718) === true){
                    if(this.canPutHorizontal(map, xPoint_15718, yPoint_15718, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15718][xPoint_15718 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15718, yPoint_15718, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15718 + i][xPoint_15718] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15718, yPoint_15718){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15718][xPoint_15718] === this.CELL_EMPTY
                && map[yPoint_15718-1][xPoint_15718] === this.CELL_EMPTY
                && map[yPoint_15718-1][xPoint_15718+1] === this.CELL_EMPTY
                && map[yPoint_15718][xPoint_15718+1] === this.CELL_EMPTY
                && map[yPoint_15718+1][xPoint_15718+1] === this.CELL_EMPTY
                && map[yPoint_15718+1][xPoint_15718] === this.CELL_EMPTY
                && map[yPoint_15718+1][xPoint_15718-1] === this.CELL_EMPTY
                && map[yPoint_15718][xPoint_15718-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15718, yPoint_15718, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15718;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15718][x] === this.CELL_EMPTY
                    && map[yPoint_15718-1][x] === this.CELL_EMPTY
                    && map[yPoint_15718-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15718][x+1] === this.CELL_EMPTY
                    && map[yPoint_15718+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15718+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15718, yPoint_15718, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15718;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15718] === this.CELL_EMPTY
                    && map[y+1][xPoint_15718] === this.CELL_EMPTY
                    && map[y+1][xPoint_15718+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15718] === this.CELL_EMPTY
                    && map[y][xPoint_15718-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15718-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
