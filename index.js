...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26661=0;yPoint_26661<this.gameFieldBorderY.length; yPoint_26661++){
                for(var xPoint_26661=0;xPoint_26661<this.gameFieldBorderX.length; xPoint_26661++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26661, xPoint_26661);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26661][xPoint_26661] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26661, xPoint_26661, 'user');
                    if(this._userShipsMap[yPoint_26661][xPoint_26661] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26661, xPoint_26661, type){
            var id = this.getPointBlockIdByCoords(yPoint_26661, xPoint_26661, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26661);
                block.setAttribute('data-y', yPoint_26661);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26661
         * @param {type} xPoint_26661
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26661, xPoint_26661, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26661 + '_y' + yPoint_26661;
            }
            return 'pc_x' + xPoint_26661 + '_y' + yPoint_26661;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26661=0;yPoint_26661<this.gameFieldBorderY.length; yPoint_26661++){
                for(var xPoint_26661=0;xPoint_26661<this.gameFieldBorderX.length; xPoint_26661++){
                    map.push({y: yPoint_26661, x: xPoint_26661});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26661=-1;yPoint_26661<(this.gameFieldBorderY.length+1); yPoint_26661++){
                for(var xPoint_26661=-1;xPoint_26661<(this.gameFieldBorderX.length+1); xPoint_26661++){
                    if(!map[yPoint_26661]){
                        map[yPoint_26661] = [];
                    }
                    map[yPoint_26661][xPoint_26661] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26661 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26661 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26661, yPoint_26661) === true){
                    if(this.canPutHorizontal(map, xPoint_26661, yPoint_26661, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26661][xPoint_26661 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26661, yPoint_26661, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26661 + i][xPoint_26661] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26661, yPoint_26661){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26661][xPoint_26661] === this.CELL_EMPTY
                && map[yPoint_26661-1][xPoint_26661] === this.CELL_EMPTY
                && map[yPoint_26661-1][xPoint_26661+1] === this.CELL_EMPTY
                && map[yPoint_26661][xPoint_26661+1] === this.CELL_EMPTY
                && map[yPoint_26661+1][xPoint_26661+1] === this.CELL_EMPTY
                && map[yPoint_26661+1][xPoint_26661] === this.CELL_EMPTY
                && map[yPoint_26661+1][xPoint_26661-1] === this.CELL_EMPTY
                && map[yPoint_26661][xPoint_26661-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26661, yPoint_26661, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26661;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26661][x] === this.CELL_EMPTY
                    && map[yPoint_26661-1][x] === this.CELL_EMPTY
                    && map[yPoint_26661-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26661][x+1] === this.CELL_EMPTY
                    && map[yPoint_26661+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26661+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26661, yPoint_26661, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26661;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26661] === this.CELL_EMPTY
                    && map[y+1][xPoint_26661] === this.CELL_EMPTY
                    && map[y+1][xPoint_26661+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26661] === this.CELL_EMPTY
                    && map[y][xPoint_26661-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26661-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
