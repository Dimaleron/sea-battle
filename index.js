...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28541=0;yPoint_28541<this.gameFieldBorderY.length; yPoint_28541++){
                for(var xPoint_28541=0;xPoint_28541<this.gameFieldBorderX.length; xPoint_28541++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28541, xPoint_28541);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28541][xPoint_28541] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28541, xPoint_28541, 'user');
                    if(this._userShipsMap[yPoint_28541][xPoint_28541] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28541, xPoint_28541, type){
            var id = this.getPointBlockIdByCoords(yPoint_28541, xPoint_28541, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28541);
                block.setAttribute('data-y', yPoint_28541);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28541
         * @param {type} xPoint_28541
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28541, xPoint_28541, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28541 + '_y' + yPoint_28541;
            }
            return 'pc_x' + xPoint_28541 + '_y' + yPoint_28541;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28541=0;yPoint_28541<this.gameFieldBorderY.length; yPoint_28541++){
                for(var xPoint_28541=0;xPoint_28541<this.gameFieldBorderX.length; xPoint_28541++){
                    map.push({y: yPoint_28541, x: xPoint_28541});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28541=-1;yPoint_28541<(this.gameFieldBorderY.length+1); yPoint_28541++){
                for(var xPoint_28541=-1;xPoint_28541<(this.gameFieldBorderX.length+1); xPoint_28541++){
                    if(!map[yPoint_28541]){
                        map[yPoint_28541] = [];
                    }
                    map[yPoint_28541][xPoint_28541] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28541 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28541 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28541, yPoint_28541) === true){
                    if(this.canPutHorizontal(map, xPoint_28541, yPoint_28541, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28541][xPoint_28541 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28541, yPoint_28541, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28541 + i][xPoint_28541] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28541, yPoint_28541){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28541][xPoint_28541] === this.CELL_EMPTY
                && map[yPoint_28541-1][xPoint_28541] === this.CELL_EMPTY
                && map[yPoint_28541-1][xPoint_28541+1] === this.CELL_EMPTY
                && map[yPoint_28541][xPoint_28541+1] === this.CELL_EMPTY
                && map[yPoint_28541+1][xPoint_28541+1] === this.CELL_EMPTY
                && map[yPoint_28541+1][xPoint_28541] === this.CELL_EMPTY
                && map[yPoint_28541+1][xPoint_28541-1] === this.CELL_EMPTY
                && map[yPoint_28541][xPoint_28541-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28541, yPoint_28541, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28541;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28541][x] === this.CELL_EMPTY
                    && map[yPoint_28541-1][x] === this.CELL_EMPTY
                    && map[yPoint_28541-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28541][x+1] === this.CELL_EMPTY
                    && map[yPoint_28541+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28541+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28541, yPoint_28541, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28541;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28541] === this.CELL_EMPTY
                    && map[y+1][xPoint_28541] === this.CELL_EMPTY
                    && map[y+1][xPoint_28541+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28541] === this.CELL_EMPTY
                    && map[y][xPoint_28541-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28541-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
