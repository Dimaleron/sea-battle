...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13575=0;yPoint_13575<this.gameFieldBorderY.length; yPoint_13575++){
                for(var xPoint_13575=0;xPoint_13575<this.gameFieldBorderX.length; xPoint_13575++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13575, xPoint_13575);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13575][xPoint_13575] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13575, xPoint_13575, 'user');
                    if(this._userShipsMap[yPoint_13575][xPoint_13575] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13575, xPoint_13575, type){
            var id = this.getPointBlockIdByCoords(yPoint_13575, xPoint_13575, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13575);
                block.setAttribute('data-y', yPoint_13575);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13575
         * @param {type} xPoint_13575
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13575, xPoint_13575, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13575 + '_y' + yPoint_13575;
            }
            return 'pc_x' + xPoint_13575 + '_y' + yPoint_13575;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13575=0;yPoint_13575<this.gameFieldBorderY.length; yPoint_13575++){
                for(var xPoint_13575=0;xPoint_13575<this.gameFieldBorderX.length; xPoint_13575++){
                    map.push({y: yPoint_13575, x: xPoint_13575});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13575=-1;yPoint_13575<(this.gameFieldBorderY.length+1); yPoint_13575++){
                for(var xPoint_13575=-1;xPoint_13575<(this.gameFieldBorderX.length+1); xPoint_13575++){
                    if(!map[yPoint_13575]){
                        map[yPoint_13575] = [];
                    }
                    map[yPoint_13575][xPoint_13575] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13575 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13575 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13575, yPoint_13575) === true){
                    if(this.canPutHorizontal(map, xPoint_13575, yPoint_13575, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13575][xPoint_13575 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13575, yPoint_13575, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13575 + i][xPoint_13575] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13575, yPoint_13575){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13575][xPoint_13575] === this.CELL_EMPTY
                && map[yPoint_13575-1][xPoint_13575] === this.CELL_EMPTY
                && map[yPoint_13575-1][xPoint_13575+1] === this.CELL_EMPTY
                && map[yPoint_13575][xPoint_13575+1] === this.CELL_EMPTY
                && map[yPoint_13575+1][xPoint_13575+1] === this.CELL_EMPTY
                && map[yPoint_13575+1][xPoint_13575] === this.CELL_EMPTY
                && map[yPoint_13575+1][xPoint_13575-1] === this.CELL_EMPTY
                && map[yPoint_13575][xPoint_13575-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13575, yPoint_13575, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13575;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13575][x] === this.CELL_EMPTY
                    && map[yPoint_13575-1][x] === this.CELL_EMPTY
                    && map[yPoint_13575-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13575][x+1] === this.CELL_EMPTY
                    && map[yPoint_13575+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13575+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13575, yPoint_13575, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13575;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13575] === this.CELL_EMPTY
                    && map[y+1][xPoint_13575] === this.CELL_EMPTY
                    && map[y+1][xPoint_13575+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13575] === this.CELL_EMPTY
                    && map[y][xPoint_13575-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13575-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
