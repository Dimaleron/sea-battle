...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30822=0;yPoint_30822<this.gameFieldBorderY.length; yPoint_30822++){
                for(var xPoint_30822=0;xPoint_30822<this.gameFieldBorderX.length; xPoint_30822++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30822, xPoint_30822);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30822][xPoint_30822] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30822, xPoint_30822, 'user');
                    if(this._userShipsMap[yPoint_30822][xPoint_30822] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30822, xPoint_30822, type){
            var id = this.getPointBlockIdByCoords(yPoint_30822, xPoint_30822, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30822);
                block.setAttribute('data-y', yPoint_30822);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30822
         * @param {type} xPoint_30822
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30822, xPoint_30822, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30822 + '_y' + yPoint_30822;
            }
            return 'pc_x' + xPoint_30822 + '_y' + yPoint_30822;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30822=0;yPoint_30822<this.gameFieldBorderY.length; yPoint_30822++){
                for(var xPoint_30822=0;xPoint_30822<this.gameFieldBorderX.length; xPoint_30822++){
                    map.push({y: yPoint_30822, x: xPoint_30822});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30822=-1;yPoint_30822<(this.gameFieldBorderY.length+1); yPoint_30822++){
                for(var xPoint_30822=-1;xPoint_30822<(this.gameFieldBorderX.length+1); xPoint_30822++){
                    if(!map[yPoint_30822]){
                        map[yPoint_30822] = [];
                    }
                    map[yPoint_30822][xPoint_30822] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30822 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30822 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30822, yPoint_30822) === true){
                    if(this.canPutHorizontal(map, xPoint_30822, yPoint_30822, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30822][xPoint_30822 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30822, yPoint_30822, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30822 + i][xPoint_30822] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30822, yPoint_30822){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30822][xPoint_30822] === this.CELL_EMPTY
                && map[yPoint_30822-1][xPoint_30822] === this.CELL_EMPTY
                && map[yPoint_30822-1][xPoint_30822+1] === this.CELL_EMPTY
                && map[yPoint_30822][xPoint_30822+1] === this.CELL_EMPTY
                && map[yPoint_30822+1][xPoint_30822+1] === this.CELL_EMPTY
                && map[yPoint_30822+1][xPoint_30822] === this.CELL_EMPTY
                && map[yPoint_30822+1][xPoint_30822-1] === this.CELL_EMPTY
                && map[yPoint_30822][xPoint_30822-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30822, yPoint_30822, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30822;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30822][x] === this.CELL_EMPTY
                    && map[yPoint_30822-1][x] === this.CELL_EMPTY
                    && map[yPoint_30822-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30822][x+1] === this.CELL_EMPTY
                    && map[yPoint_30822+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30822+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30822, yPoint_30822, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30822;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30822] === this.CELL_EMPTY
                    && map[y+1][xPoint_30822] === this.CELL_EMPTY
                    && map[y+1][xPoint_30822+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30822] === this.CELL_EMPTY
                    && map[y][xPoint_30822-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30822-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
