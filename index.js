...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_17364=0;yPoint_17364<this.gameFieldBorderY.length; yPoint_17364++){
                for(var xPoint_17364=0;xPoint_17364<this.gameFieldBorderX.length; xPoint_17364++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_17364, xPoint_17364);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_17364][xPoint_17364] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_17364, xPoint_17364, 'user');
                    if(this._userShipsMap[yPoint_17364][xPoint_17364] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_17364, xPoint_17364, type){
            var id = this.getPointBlockIdByCoords(yPoint_17364, xPoint_17364, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_17364);
                block.setAttribute('data-y', yPoint_17364);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_17364
         * @param {type} xPoint_17364
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_17364, xPoint_17364, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_17364 + '_y' + yPoint_17364;
            }
            return 'pc_x' + xPoint_17364 + '_y' + yPoint_17364;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_17364=0;yPoint_17364<this.gameFieldBorderY.length; yPoint_17364++){
                for(var xPoint_17364=0;xPoint_17364<this.gameFieldBorderX.length; xPoint_17364++){
                    map.push({y: yPoint_17364, x: xPoint_17364});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_17364=-1;yPoint_17364<(this.gameFieldBorderY.length+1); yPoint_17364++){
                for(var xPoint_17364=-1;xPoint_17364<(this.gameFieldBorderX.length+1); xPoint_17364++){
                    if(!map[yPoint_17364]){
                        map[yPoint_17364] = [];
                    }
                    map[yPoint_17364][xPoint_17364] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_17364 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_17364 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_17364, yPoint_17364) === true){
                    if(this.canPutHorizontal(map, xPoint_17364, yPoint_17364, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17364][xPoint_17364 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_17364, yPoint_17364, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_17364 + i][xPoint_17364] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_17364, yPoint_17364){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_17364][xPoint_17364] === this.CELL_EMPTY
                && map[yPoint_17364-1][xPoint_17364] === this.CELL_EMPTY
                && map[yPoint_17364-1][xPoint_17364+1] === this.CELL_EMPTY
                && map[yPoint_17364][xPoint_17364+1] === this.CELL_EMPTY
                && map[yPoint_17364+1][xPoint_17364+1] === this.CELL_EMPTY
                && map[yPoint_17364+1][xPoint_17364] === this.CELL_EMPTY
                && map[yPoint_17364+1][xPoint_17364-1] === this.CELL_EMPTY
                && map[yPoint_17364][xPoint_17364-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_17364, yPoint_17364, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_17364;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_17364][x] === this.CELL_EMPTY
                    && map[yPoint_17364-1][x] === this.CELL_EMPTY
                    && map[yPoint_17364-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17364][x+1] === this.CELL_EMPTY
                    && map[yPoint_17364+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_17364+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_17364, yPoint_17364, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_17364;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_17364] === this.CELL_EMPTY
                    && map[y+1][xPoint_17364] === this.CELL_EMPTY
                    && map[y+1][xPoint_17364+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_17364] === this.CELL_EMPTY
                    && map[y][xPoint_17364-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_17364-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
