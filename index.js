...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19534=0;yPoint_19534<this.gameFieldBorderY.length; yPoint_19534++){
                for(var xPoint_19534=0;xPoint_19534<this.gameFieldBorderX.length; xPoint_19534++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19534, xPoint_19534);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19534][xPoint_19534] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19534, xPoint_19534, 'user');
                    if(this._userShipsMap[yPoint_19534][xPoint_19534] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19534, xPoint_19534, type){
            var id = this.getPointBlockIdByCoords(yPoint_19534, xPoint_19534, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19534);
                block.setAttribute('data-y', yPoint_19534);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19534
         * @param {type} xPoint_19534
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19534, xPoint_19534, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19534 + '_y' + yPoint_19534;
            }
            return 'pc_x' + xPoint_19534 + '_y' + yPoint_19534;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19534=0;yPoint_19534<this.gameFieldBorderY.length; yPoint_19534++){
                for(var xPoint_19534=0;xPoint_19534<this.gameFieldBorderX.length; xPoint_19534++){
                    map.push({y: yPoint_19534, x: xPoint_19534});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19534=-1;yPoint_19534<(this.gameFieldBorderY.length+1); yPoint_19534++){
                for(var xPoint_19534=-1;xPoint_19534<(this.gameFieldBorderX.length+1); xPoint_19534++){
                    if(!map[yPoint_19534]){
                        map[yPoint_19534] = [];
                    }
                    map[yPoint_19534][xPoint_19534] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19534 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19534 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19534, yPoint_19534) === true){
                    if(this.canPutHorizontal(map, xPoint_19534, yPoint_19534, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19534][xPoint_19534 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19534, yPoint_19534, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19534 + i][xPoint_19534] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19534, yPoint_19534){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19534][xPoint_19534] === this.CELL_EMPTY
                && map[yPoint_19534-1][xPoint_19534] === this.CELL_EMPTY
                && map[yPoint_19534-1][xPoint_19534+1] === this.CELL_EMPTY
                && map[yPoint_19534][xPoint_19534+1] === this.CELL_EMPTY
                && map[yPoint_19534+1][xPoint_19534+1] === this.CELL_EMPTY
                && map[yPoint_19534+1][xPoint_19534] === this.CELL_EMPTY
                && map[yPoint_19534+1][xPoint_19534-1] === this.CELL_EMPTY
                && map[yPoint_19534][xPoint_19534-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19534, yPoint_19534, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19534;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19534][x] === this.CELL_EMPTY
                    && map[yPoint_19534-1][x] === this.CELL_EMPTY
                    && map[yPoint_19534-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19534][x+1] === this.CELL_EMPTY
                    && map[yPoint_19534+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19534+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19534, yPoint_19534, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19534;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19534] === this.CELL_EMPTY
                    && map[y+1][xPoint_19534] === this.CELL_EMPTY
                    && map[y+1][xPoint_19534+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19534] === this.CELL_EMPTY
                    && map[y][xPoint_19534-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19534-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
