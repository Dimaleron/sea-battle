...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18012=0;yPoint_18012<this.gameFieldBorderY.length; yPoint_18012++){
                for(var xPoint_18012=0;xPoint_18012<this.gameFieldBorderX.length; xPoint_18012++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18012, xPoint_18012);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18012][xPoint_18012] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18012, xPoint_18012, 'user');
                    if(this._userShipsMap[yPoint_18012][xPoint_18012] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18012, xPoint_18012, type){
            var id = this.getPointBlockIdByCoords(yPoint_18012, xPoint_18012, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18012);
                block.setAttribute('data-y', yPoint_18012);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18012
         * @param {type} xPoint_18012
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18012, xPoint_18012, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18012 + '_y' + yPoint_18012;
            }
            return 'pc_x' + xPoint_18012 + '_y' + yPoint_18012;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18012=0;yPoint_18012<this.gameFieldBorderY.length; yPoint_18012++){
                for(var xPoint_18012=0;xPoint_18012<this.gameFieldBorderX.length; xPoint_18012++){
                    map.push({y: yPoint_18012, x: xPoint_18012});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18012=-1;yPoint_18012<(this.gameFieldBorderY.length+1); yPoint_18012++){
                for(var xPoint_18012=-1;xPoint_18012<(this.gameFieldBorderX.length+1); xPoint_18012++){
                    if(!map[yPoint_18012]){
                        map[yPoint_18012] = [];
                    }
                    map[yPoint_18012][xPoint_18012] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18012 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18012 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18012, yPoint_18012) === true){
                    if(this.canPutHorizontal(map, xPoint_18012, yPoint_18012, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18012][xPoint_18012 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18012, yPoint_18012, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18012 + i][xPoint_18012] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18012, yPoint_18012){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18012][xPoint_18012] === this.CELL_EMPTY
                && map[yPoint_18012-1][xPoint_18012] === this.CELL_EMPTY
                && map[yPoint_18012-1][xPoint_18012+1] === this.CELL_EMPTY
                && map[yPoint_18012][xPoint_18012+1] === this.CELL_EMPTY
                && map[yPoint_18012+1][xPoint_18012+1] === this.CELL_EMPTY
                && map[yPoint_18012+1][xPoint_18012] === this.CELL_EMPTY
                && map[yPoint_18012+1][xPoint_18012-1] === this.CELL_EMPTY
                && map[yPoint_18012][xPoint_18012-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18012, yPoint_18012, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18012;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18012][x] === this.CELL_EMPTY
                    && map[yPoint_18012-1][x] === this.CELL_EMPTY
                    && map[yPoint_18012-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18012][x+1] === this.CELL_EMPTY
                    && map[yPoint_18012+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18012+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18012, yPoint_18012, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18012;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18012] === this.CELL_EMPTY
                    && map[y+1][xPoint_18012] === this.CELL_EMPTY
                    && map[y+1][xPoint_18012+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18012] === this.CELL_EMPTY
                    && map[y][xPoint_18012-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18012-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
