...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18585=0;yPoint_18585<this.gameFieldBorderY.length; yPoint_18585++){
                for(var xPoint_18585=0;xPoint_18585<this.gameFieldBorderX.length; xPoint_18585++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18585, xPoint_18585);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18585][xPoint_18585] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18585, xPoint_18585, 'user');
                    if(this._userShipsMap[yPoint_18585][xPoint_18585] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18585, xPoint_18585, type){
            var id = this.getPointBlockIdByCoords(yPoint_18585, xPoint_18585, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18585);
                block.setAttribute('data-y', yPoint_18585);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18585
         * @param {type} xPoint_18585
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18585, xPoint_18585, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18585 + '_y' + yPoint_18585;
            }
            return 'pc_x' + xPoint_18585 + '_y' + yPoint_18585;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18585=0;yPoint_18585<this.gameFieldBorderY.length; yPoint_18585++){
                for(var xPoint_18585=0;xPoint_18585<this.gameFieldBorderX.length; xPoint_18585++){
                    map.push({y: yPoint_18585, x: xPoint_18585});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18585=-1;yPoint_18585<(this.gameFieldBorderY.length+1); yPoint_18585++){
                for(var xPoint_18585=-1;xPoint_18585<(this.gameFieldBorderX.length+1); xPoint_18585++){
                    if(!map[yPoint_18585]){
                        map[yPoint_18585] = [];
                    }
                    map[yPoint_18585][xPoint_18585] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18585 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18585 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18585, yPoint_18585) === true){
                    if(this.canPutHorizontal(map, xPoint_18585, yPoint_18585, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18585][xPoint_18585 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18585, yPoint_18585, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18585 + i][xPoint_18585] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18585, yPoint_18585){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18585][xPoint_18585] === this.CELL_EMPTY
                && map[yPoint_18585-1][xPoint_18585] === this.CELL_EMPTY
                && map[yPoint_18585-1][xPoint_18585+1] === this.CELL_EMPTY
                && map[yPoint_18585][xPoint_18585+1] === this.CELL_EMPTY
                && map[yPoint_18585+1][xPoint_18585+1] === this.CELL_EMPTY
                && map[yPoint_18585+1][xPoint_18585] === this.CELL_EMPTY
                && map[yPoint_18585+1][xPoint_18585-1] === this.CELL_EMPTY
                && map[yPoint_18585][xPoint_18585-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18585, yPoint_18585, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18585;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18585][x] === this.CELL_EMPTY
                    && map[yPoint_18585-1][x] === this.CELL_EMPTY
                    && map[yPoint_18585-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18585][x+1] === this.CELL_EMPTY
                    && map[yPoint_18585+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18585+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18585, yPoint_18585, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18585;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18585] === this.CELL_EMPTY
                    && map[y+1][xPoint_18585] === this.CELL_EMPTY
                    && map[y+1][xPoint_18585+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18585] === this.CELL_EMPTY
                    && map[y][xPoint_18585-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18585-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
