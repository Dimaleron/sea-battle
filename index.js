...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14120=0;yPoint_14120<this.gameFieldBorderY.length; yPoint_14120++){
                for(var xPoint_14120=0;xPoint_14120<this.gameFieldBorderX.length; xPoint_14120++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14120, xPoint_14120);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14120][xPoint_14120] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14120, xPoint_14120, 'user');
                    if(this._userShipsMap[yPoint_14120][xPoint_14120] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14120, xPoint_14120, type){
            var id = this.getPointBlockIdByCoords(yPoint_14120, xPoint_14120, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14120);
                block.setAttribute('data-y', yPoint_14120);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14120
         * @param {type} xPoint_14120
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14120, xPoint_14120, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14120 + '_y' + yPoint_14120;
            }
            return 'pc_x' + xPoint_14120 + '_y' + yPoint_14120;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14120=0;yPoint_14120<this.gameFieldBorderY.length; yPoint_14120++){
                for(var xPoint_14120=0;xPoint_14120<this.gameFieldBorderX.length; xPoint_14120++){
                    map.push({y: yPoint_14120, x: xPoint_14120});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14120=-1;yPoint_14120<(this.gameFieldBorderY.length+1); yPoint_14120++){
                for(var xPoint_14120=-1;xPoint_14120<(this.gameFieldBorderX.length+1); xPoint_14120++){
                    if(!map[yPoint_14120]){
                        map[yPoint_14120] = [];
                    }
                    map[yPoint_14120][xPoint_14120] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14120 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14120 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14120, yPoint_14120) === true){
                    if(this.canPutHorizontal(map, xPoint_14120, yPoint_14120, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14120][xPoint_14120 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14120, yPoint_14120, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14120 + i][xPoint_14120] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14120, yPoint_14120){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14120][xPoint_14120] === this.CELL_EMPTY
                && map[yPoint_14120-1][xPoint_14120] === this.CELL_EMPTY
                && map[yPoint_14120-1][xPoint_14120+1] === this.CELL_EMPTY
                && map[yPoint_14120][xPoint_14120+1] === this.CELL_EMPTY
                && map[yPoint_14120+1][xPoint_14120+1] === this.CELL_EMPTY
                && map[yPoint_14120+1][xPoint_14120] === this.CELL_EMPTY
                && map[yPoint_14120+1][xPoint_14120-1] === this.CELL_EMPTY
                && map[yPoint_14120][xPoint_14120-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14120, yPoint_14120, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14120;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14120][x] === this.CELL_EMPTY
                    && map[yPoint_14120-1][x] === this.CELL_EMPTY
                    && map[yPoint_14120-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14120][x+1] === this.CELL_EMPTY
                    && map[yPoint_14120+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14120+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14120, yPoint_14120, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14120;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14120] === this.CELL_EMPTY
                    && map[y+1][xPoint_14120] === this.CELL_EMPTY
                    && map[y+1][xPoint_14120+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14120] === this.CELL_EMPTY
                    && map[y][xPoint_14120-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14120-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
