...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27005=0;yPoint_27005<this.gameFieldBorderY.length; yPoint_27005++){
                for(var xPoint_27005=0;xPoint_27005<this.gameFieldBorderX.length; xPoint_27005++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27005, xPoint_27005);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27005][xPoint_27005] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27005, xPoint_27005, 'user');
                    if(this._userShipsMap[yPoint_27005][xPoint_27005] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27005, xPoint_27005, type){
            var id = this.getPointBlockIdByCoords(yPoint_27005, xPoint_27005, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27005);
                block.setAttribute('data-y', yPoint_27005);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27005
         * @param {type} xPoint_27005
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27005, xPoint_27005, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27005 + '_y' + yPoint_27005;
            }
            return 'pc_x' + xPoint_27005 + '_y' + yPoint_27005;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27005=0;yPoint_27005<this.gameFieldBorderY.length; yPoint_27005++){
                for(var xPoint_27005=0;xPoint_27005<this.gameFieldBorderX.length; xPoint_27005++){
                    map.push({y: yPoint_27005, x: xPoint_27005});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27005=-1;yPoint_27005<(this.gameFieldBorderY.length+1); yPoint_27005++){
                for(var xPoint_27005=-1;xPoint_27005<(this.gameFieldBorderX.length+1); xPoint_27005++){
                    if(!map[yPoint_27005]){
                        map[yPoint_27005] = [];
                    }
                    map[yPoint_27005][xPoint_27005] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27005 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27005 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27005, yPoint_27005) === true){
                    if(this.canPutHorizontal(map, xPoint_27005, yPoint_27005, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27005][xPoint_27005 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27005, yPoint_27005, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27005 + i][xPoint_27005] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27005, yPoint_27005){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27005][xPoint_27005] === this.CELL_EMPTY
                && map[yPoint_27005-1][xPoint_27005] === this.CELL_EMPTY
                && map[yPoint_27005-1][xPoint_27005+1] === this.CELL_EMPTY
                && map[yPoint_27005][xPoint_27005+1] === this.CELL_EMPTY
                && map[yPoint_27005+1][xPoint_27005+1] === this.CELL_EMPTY
                && map[yPoint_27005+1][xPoint_27005] === this.CELL_EMPTY
                && map[yPoint_27005+1][xPoint_27005-1] === this.CELL_EMPTY
                && map[yPoint_27005][xPoint_27005-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27005, yPoint_27005, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27005;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27005][x] === this.CELL_EMPTY
                    && map[yPoint_27005-1][x] === this.CELL_EMPTY
                    && map[yPoint_27005-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27005][x+1] === this.CELL_EMPTY
                    && map[yPoint_27005+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27005+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27005, yPoint_27005, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27005;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27005] === this.CELL_EMPTY
                    && map[y+1][xPoint_27005] === this.CELL_EMPTY
                    && map[y+1][xPoint_27005+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27005] === this.CELL_EMPTY
                    && map[y][xPoint_27005-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27005-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
