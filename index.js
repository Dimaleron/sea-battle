...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24380=0;yPoint_24380<this.gameFieldBorderY.length; yPoint_24380++){
                for(var xPoint_24380=0;xPoint_24380<this.gameFieldBorderX.length; xPoint_24380++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24380, xPoint_24380);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24380][xPoint_24380] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24380, xPoint_24380, 'user');
                    if(this._userShipsMap[yPoint_24380][xPoint_24380] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24380, xPoint_24380, type){
            var id = this.getPointBlockIdByCoords(yPoint_24380, xPoint_24380, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24380);
                block.setAttribute('data-y', yPoint_24380);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24380
         * @param {type} xPoint_24380
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24380, xPoint_24380, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24380 + '_y' + yPoint_24380;
            }
            return 'pc_x' + xPoint_24380 + '_y' + yPoint_24380;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24380=0;yPoint_24380<this.gameFieldBorderY.length; yPoint_24380++){
                for(var xPoint_24380=0;xPoint_24380<this.gameFieldBorderX.length; xPoint_24380++){
                    map.push({y: yPoint_24380, x: xPoint_24380});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24380=-1;yPoint_24380<(this.gameFieldBorderY.length+1); yPoint_24380++){
                for(var xPoint_24380=-1;xPoint_24380<(this.gameFieldBorderX.length+1); xPoint_24380++){
                    if(!map[yPoint_24380]){
                        map[yPoint_24380] = [];
                    }
                    map[yPoint_24380][xPoint_24380] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24380 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24380 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24380, yPoint_24380) === true){
                    if(this.canPutHorizontal(map, xPoint_24380, yPoint_24380, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24380][xPoint_24380 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24380, yPoint_24380, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24380 + i][xPoint_24380] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24380, yPoint_24380){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24380][xPoint_24380] === this.CELL_EMPTY
                && map[yPoint_24380-1][xPoint_24380] === this.CELL_EMPTY
                && map[yPoint_24380-1][xPoint_24380+1] === this.CELL_EMPTY
                && map[yPoint_24380][xPoint_24380+1] === this.CELL_EMPTY
                && map[yPoint_24380+1][xPoint_24380+1] === this.CELL_EMPTY
                && map[yPoint_24380+1][xPoint_24380] === this.CELL_EMPTY
                && map[yPoint_24380+1][xPoint_24380-1] === this.CELL_EMPTY
                && map[yPoint_24380][xPoint_24380-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24380, yPoint_24380, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24380;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24380][x] === this.CELL_EMPTY
                    && map[yPoint_24380-1][x] === this.CELL_EMPTY
                    && map[yPoint_24380-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24380][x+1] === this.CELL_EMPTY
                    && map[yPoint_24380+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24380+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24380, yPoint_24380, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24380;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24380] === this.CELL_EMPTY
                    && map[y+1][xPoint_24380] === this.CELL_EMPTY
                    && map[y+1][xPoint_24380+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24380] === this.CELL_EMPTY
                    && map[y][xPoint_24380-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24380-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
