...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16867=0;yPoint_16867<this.gameFieldBorderY.length; yPoint_16867++){
                for(var xPoint_16867=0;xPoint_16867<this.gameFieldBorderX.length; xPoint_16867++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16867, xPoint_16867);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16867][xPoint_16867] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16867, xPoint_16867, 'user');
                    if(this._userShipsMap[yPoint_16867][xPoint_16867] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16867, xPoint_16867, type){
            var id = this.getPointBlockIdByCoords(yPoint_16867, xPoint_16867, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16867);
                block.setAttribute('data-y', yPoint_16867);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16867
         * @param {type} xPoint_16867
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16867, xPoint_16867, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16867 + '_y' + yPoint_16867;
            }
            return 'pc_x' + xPoint_16867 + '_y' + yPoint_16867;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16867=0;yPoint_16867<this.gameFieldBorderY.length; yPoint_16867++){
                for(var xPoint_16867=0;xPoint_16867<this.gameFieldBorderX.length; xPoint_16867++){
                    map.push({y: yPoint_16867, x: xPoint_16867});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16867=-1;yPoint_16867<(this.gameFieldBorderY.length+1); yPoint_16867++){
                for(var xPoint_16867=-1;xPoint_16867<(this.gameFieldBorderX.length+1); xPoint_16867++){
                    if(!map[yPoint_16867]){
                        map[yPoint_16867] = [];
                    }
                    map[yPoint_16867][xPoint_16867] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16867 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16867 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16867, yPoint_16867) === true){
                    if(this.canPutHorizontal(map, xPoint_16867, yPoint_16867, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16867][xPoint_16867 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16867, yPoint_16867, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16867 + i][xPoint_16867] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16867, yPoint_16867){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16867][xPoint_16867] === this.CELL_EMPTY
                && map[yPoint_16867-1][xPoint_16867] === this.CELL_EMPTY
                && map[yPoint_16867-1][xPoint_16867+1] === this.CELL_EMPTY
                && map[yPoint_16867][xPoint_16867+1] === this.CELL_EMPTY
                && map[yPoint_16867+1][xPoint_16867+1] === this.CELL_EMPTY
                && map[yPoint_16867+1][xPoint_16867] === this.CELL_EMPTY
                && map[yPoint_16867+1][xPoint_16867-1] === this.CELL_EMPTY
                && map[yPoint_16867][xPoint_16867-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16867, yPoint_16867, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16867;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16867][x] === this.CELL_EMPTY
                    && map[yPoint_16867-1][x] === this.CELL_EMPTY
                    && map[yPoint_16867-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16867][x+1] === this.CELL_EMPTY
                    && map[yPoint_16867+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16867+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16867, yPoint_16867, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16867;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16867] === this.CELL_EMPTY
                    && map[y+1][xPoint_16867] === this.CELL_EMPTY
                    && map[y+1][xPoint_16867+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16867] === this.CELL_EMPTY
                    && map[y][xPoint_16867-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16867-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
