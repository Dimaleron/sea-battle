...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14545=0;yPoint_14545<this.gameFieldBorderY.length; yPoint_14545++){
                for(var xPoint_14545=0;xPoint_14545<this.gameFieldBorderX.length; xPoint_14545++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14545, xPoint_14545);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14545][xPoint_14545] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14545, xPoint_14545, 'user');
                    if(this._userShipsMap[yPoint_14545][xPoint_14545] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14545, xPoint_14545, type){
            var id = this.getPointBlockIdByCoords(yPoint_14545, xPoint_14545, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14545);
                block.setAttribute('data-y', yPoint_14545);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14545
         * @param {type} xPoint_14545
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14545, xPoint_14545, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14545 + '_y' + yPoint_14545;
            }
            return 'pc_x' + xPoint_14545 + '_y' + yPoint_14545;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14545=0;yPoint_14545<this.gameFieldBorderY.length; yPoint_14545++){
                for(var xPoint_14545=0;xPoint_14545<this.gameFieldBorderX.length; xPoint_14545++){
                    map.push({y: yPoint_14545, x: xPoint_14545});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14545=-1;yPoint_14545<(this.gameFieldBorderY.length+1); yPoint_14545++){
                for(var xPoint_14545=-1;xPoint_14545<(this.gameFieldBorderX.length+1); xPoint_14545++){
                    if(!map[yPoint_14545]){
                        map[yPoint_14545] = [];
                    }
                    map[yPoint_14545][xPoint_14545] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14545 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14545 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14545, yPoint_14545) === true){
                    if(this.canPutHorizontal(map, xPoint_14545, yPoint_14545, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14545][xPoint_14545 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14545, yPoint_14545, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14545 + i][xPoint_14545] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14545, yPoint_14545){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14545][xPoint_14545] === this.CELL_EMPTY
                && map[yPoint_14545-1][xPoint_14545] === this.CELL_EMPTY
                && map[yPoint_14545-1][xPoint_14545+1] === this.CELL_EMPTY
                && map[yPoint_14545][xPoint_14545+1] === this.CELL_EMPTY
                && map[yPoint_14545+1][xPoint_14545+1] === this.CELL_EMPTY
                && map[yPoint_14545+1][xPoint_14545] === this.CELL_EMPTY
                && map[yPoint_14545+1][xPoint_14545-1] === this.CELL_EMPTY
                && map[yPoint_14545][xPoint_14545-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14545, yPoint_14545, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14545;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14545][x] === this.CELL_EMPTY
                    && map[yPoint_14545-1][x] === this.CELL_EMPTY
                    && map[yPoint_14545-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14545][x+1] === this.CELL_EMPTY
                    && map[yPoint_14545+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14545+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14545, yPoint_14545, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14545;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14545] === this.CELL_EMPTY
                    && map[y+1][xPoint_14545] === this.CELL_EMPTY
                    && map[y+1][xPoint_14545+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14545] === this.CELL_EMPTY
                    && map[y][xPoint_14545-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14545-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
