...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22723=0;yPoint_22723<this.gameFieldBorderY.length; yPoint_22723++){
                for(var xPoint_22723=0;xPoint_22723<this.gameFieldBorderX.length; xPoint_22723++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22723, xPoint_22723);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22723][xPoint_22723] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22723, xPoint_22723, 'user');
                    if(this._userShipsMap[yPoint_22723][xPoint_22723] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22723, xPoint_22723, type){
            var id = this.getPointBlockIdByCoords(yPoint_22723, xPoint_22723, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22723);
                block.setAttribute('data-y', yPoint_22723);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22723
         * @param {type} xPoint_22723
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22723, xPoint_22723, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22723 + '_y' + yPoint_22723;
            }
            return 'pc_x' + xPoint_22723 + '_y' + yPoint_22723;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22723=0;yPoint_22723<this.gameFieldBorderY.length; yPoint_22723++){
                for(var xPoint_22723=0;xPoint_22723<this.gameFieldBorderX.length; xPoint_22723++){
                    map.push({y: yPoint_22723, x: xPoint_22723});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22723=-1;yPoint_22723<(this.gameFieldBorderY.length+1); yPoint_22723++){
                for(var xPoint_22723=-1;xPoint_22723<(this.gameFieldBorderX.length+1); xPoint_22723++){
                    if(!map[yPoint_22723]){
                        map[yPoint_22723] = [];
                    }
                    map[yPoint_22723][xPoint_22723] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22723 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22723 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22723, yPoint_22723) === true){
                    if(this.canPutHorizontal(map, xPoint_22723, yPoint_22723, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22723][xPoint_22723 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22723, yPoint_22723, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22723 + i][xPoint_22723] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22723, yPoint_22723){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22723][xPoint_22723] === this.CELL_EMPTY
                && map[yPoint_22723-1][xPoint_22723] === this.CELL_EMPTY
                && map[yPoint_22723-1][xPoint_22723+1] === this.CELL_EMPTY
                && map[yPoint_22723][xPoint_22723+1] === this.CELL_EMPTY
                && map[yPoint_22723+1][xPoint_22723+1] === this.CELL_EMPTY
                && map[yPoint_22723+1][xPoint_22723] === this.CELL_EMPTY
                && map[yPoint_22723+1][xPoint_22723-1] === this.CELL_EMPTY
                && map[yPoint_22723][xPoint_22723-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22723, yPoint_22723, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22723;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22723][x] === this.CELL_EMPTY
                    && map[yPoint_22723-1][x] === this.CELL_EMPTY
                    && map[yPoint_22723-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22723][x+1] === this.CELL_EMPTY
                    && map[yPoint_22723+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22723+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22723, yPoint_22723, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22723;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22723] === this.CELL_EMPTY
                    && map[y+1][xPoint_22723] === this.CELL_EMPTY
                    && map[y+1][xPoint_22723+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22723] === this.CELL_EMPTY
                    && map[y][xPoint_22723-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22723-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
