...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3885=0;yPoint_3885<this.gameFieldBorderY.length; yPoint_3885++){
                for(var xPoint_3885=0;xPoint_3885<this.gameFieldBorderX.length; xPoint_3885++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3885, xPoint_3885);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3885][xPoint_3885] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3885, xPoint_3885, 'user');
                    if(this._userShipsMap[yPoint_3885][xPoint_3885] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3885, xPoint_3885, type){
            var id = this.getPointBlockIdByCoords(yPoint_3885, xPoint_3885, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3885);
                block.setAttribute('data-y', yPoint_3885);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3885
         * @param {type} xPoint_3885
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3885, xPoint_3885, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3885 + '_y' + yPoint_3885;
            }
            return 'pc_x' + xPoint_3885 + '_y' + yPoint_3885;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3885=0;yPoint_3885<this.gameFieldBorderY.length; yPoint_3885++){
                for(var xPoint_3885=0;xPoint_3885<this.gameFieldBorderX.length; xPoint_3885++){
                    map.push({y: yPoint_3885, x: xPoint_3885});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3885=-1;yPoint_3885<(this.gameFieldBorderY.length+1); yPoint_3885++){
                for(var xPoint_3885=-1;xPoint_3885<(this.gameFieldBorderX.length+1); xPoint_3885++){
                    if(!map[yPoint_3885]){
                        map[yPoint_3885] = [];
                    }
                    map[yPoint_3885][xPoint_3885] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3885 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3885 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3885, yPoint_3885) === true){
                    if(this.canPutHorizontal(map, xPoint_3885, yPoint_3885, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3885][xPoint_3885 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3885, yPoint_3885, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3885 + i][xPoint_3885] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3885, yPoint_3885){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3885][xPoint_3885] === this.CELL_EMPTY
                && map[yPoint_3885-1][xPoint_3885] === this.CELL_EMPTY
                && map[yPoint_3885-1][xPoint_3885+1] === this.CELL_EMPTY
                && map[yPoint_3885][xPoint_3885+1] === this.CELL_EMPTY
                && map[yPoint_3885+1][xPoint_3885+1] === this.CELL_EMPTY
                && map[yPoint_3885+1][xPoint_3885] === this.CELL_EMPTY
                && map[yPoint_3885+1][xPoint_3885-1] === this.CELL_EMPTY
                && map[yPoint_3885][xPoint_3885-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3885, yPoint_3885, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3885;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3885][x] === this.CELL_EMPTY
                    && map[yPoint_3885-1][x] === this.CELL_EMPTY
                    && map[yPoint_3885-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3885][x+1] === this.CELL_EMPTY
                    && map[yPoint_3885+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3885+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3885, yPoint_3885, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3885;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3885] === this.CELL_EMPTY
                    && map[y+1][xPoint_3885] === this.CELL_EMPTY
                    && map[y+1][xPoint_3885+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3885] === this.CELL_EMPTY
                    && map[y][xPoint_3885-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3885-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
