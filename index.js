...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15110=0;yPoint_15110<this.gameFieldBorderY.length; yPoint_15110++){
                for(var xPoint_15110=0;xPoint_15110<this.gameFieldBorderX.length; xPoint_15110++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15110, xPoint_15110);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15110][xPoint_15110] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15110, xPoint_15110, 'user');
                    if(this._userShipsMap[yPoint_15110][xPoint_15110] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15110, xPoint_15110, type){
            var id = this.getPointBlockIdByCoords(yPoint_15110, xPoint_15110, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15110);
                block.setAttribute('data-y', yPoint_15110);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15110
         * @param {type} xPoint_15110
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15110, xPoint_15110, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15110 + '_y' + yPoint_15110;
            }
            return 'pc_x' + xPoint_15110 + '_y' + yPoint_15110;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15110=0;yPoint_15110<this.gameFieldBorderY.length; yPoint_15110++){
                for(var xPoint_15110=0;xPoint_15110<this.gameFieldBorderX.length; xPoint_15110++){
                    map.push({y: yPoint_15110, x: xPoint_15110});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15110=-1;yPoint_15110<(this.gameFieldBorderY.length+1); yPoint_15110++){
                for(var xPoint_15110=-1;xPoint_15110<(this.gameFieldBorderX.length+1); xPoint_15110++){
                    if(!map[yPoint_15110]){
                        map[yPoint_15110] = [];
                    }
                    map[yPoint_15110][xPoint_15110] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15110 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15110 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15110, yPoint_15110) === true){
                    if(this.canPutHorizontal(map, xPoint_15110, yPoint_15110, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15110][xPoint_15110 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15110, yPoint_15110, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15110 + i][xPoint_15110] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15110, yPoint_15110){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15110][xPoint_15110] === this.CELL_EMPTY
                && map[yPoint_15110-1][xPoint_15110] === this.CELL_EMPTY
                && map[yPoint_15110-1][xPoint_15110+1] === this.CELL_EMPTY
                && map[yPoint_15110][xPoint_15110+1] === this.CELL_EMPTY
                && map[yPoint_15110+1][xPoint_15110+1] === this.CELL_EMPTY
                && map[yPoint_15110+1][xPoint_15110] === this.CELL_EMPTY
                && map[yPoint_15110+1][xPoint_15110-1] === this.CELL_EMPTY
                && map[yPoint_15110][xPoint_15110-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15110, yPoint_15110, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15110;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15110][x] === this.CELL_EMPTY
                    && map[yPoint_15110-1][x] === this.CELL_EMPTY
                    && map[yPoint_15110-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15110][x+1] === this.CELL_EMPTY
                    && map[yPoint_15110+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15110+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15110, yPoint_15110, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15110;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15110] === this.CELL_EMPTY
                    && map[y+1][xPoint_15110] === this.CELL_EMPTY
                    && map[y+1][xPoint_15110+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15110] === this.CELL_EMPTY
                    && map[y][xPoint_15110-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15110-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
