...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14940=0;yPoint_14940<this.gameFieldBorderY.length; yPoint_14940++){
                for(var xPoint_14940=0;xPoint_14940<this.gameFieldBorderX.length; xPoint_14940++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14940, xPoint_14940);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14940][xPoint_14940] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14940, xPoint_14940, 'user');
                    if(this._userShipsMap[yPoint_14940][xPoint_14940] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14940, xPoint_14940, type){
            var id = this.getPointBlockIdByCoords(yPoint_14940, xPoint_14940, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14940);
                block.setAttribute('data-y', yPoint_14940);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14940
         * @param {type} xPoint_14940
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14940, xPoint_14940, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14940 + '_y' + yPoint_14940;
            }
            return 'pc_x' + xPoint_14940 + '_y' + yPoint_14940;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14940=0;yPoint_14940<this.gameFieldBorderY.length; yPoint_14940++){
                for(var xPoint_14940=0;xPoint_14940<this.gameFieldBorderX.length; xPoint_14940++){
                    map.push({y: yPoint_14940, x: xPoint_14940});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14940=-1;yPoint_14940<(this.gameFieldBorderY.length+1); yPoint_14940++){
                for(var xPoint_14940=-1;xPoint_14940<(this.gameFieldBorderX.length+1); xPoint_14940++){
                    if(!map[yPoint_14940]){
                        map[yPoint_14940] = [];
                    }
                    map[yPoint_14940][xPoint_14940] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14940 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14940 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14940, yPoint_14940) === true){
                    if(this.canPutHorizontal(map, xPoint_14940, yPoint_14940, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14940][xPoint_14940 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14940, yPoint_14940, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14940 + i][xPoint_14940] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14940, yPoint_14940){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14940][xPoint_14940] === this.CELL_EMPTY
                && map[yPoint_14940-1][xPoint_14940] === this.CELL_EMPTY
                && map[yPoint_14940-1][xPoint_14940+1] === this.CELL_EMPTY
                && map[yPoint_14940][xPoint_14940+1] === this.CELL_EMPTY
                && map[yPoint_14940+1][xPoint_14940+1] === this.CELL_EMPTY
                && map[yPoint_14940+1][xPoint_14940] === this.CELL_EMPTY
                && map[yPoint_14940+1][xPoint_14940-1] === this.CELL_EMPTY
                && map[yPoint_14940][xPoint_14940-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14940, yPoint_14940, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14940;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14940][x] === this.CELL_EMPTY
                    && map[yPoint_14940-1][x] === this.CELL_EMPTY
                    && map[yPoint_14940-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14940][x+1] === this.CELL_EMPTY
                    && map[yPoint_14940+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14940+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14940, yPoint_14940, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14940;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14940] === this.CELL_EMPTY
                    && map[y+1][xPoint_14940] === this.CELL_EMPTY
                    && map[y+1][xPoint_14940+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14940] === this.CELL_EMPTY
                    && map[y][xPoint_14940-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14940-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
