...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20594=0;yPoint_20594<this.gameFieldBorderY.length; yPoint_20594++){
                for(var xPoint_20594=0;xPoint_20594<this.gameFieldBorderX.length; xPoint_20594++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20594, xPoint_20594);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20594][xPoint_20594] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20594, xPoint_20594, 'user');
                    if(this._userShipsMap[yPoint_20594][xPoint_20594] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20594, xPoint_20594, type){
            var id = this.getPointBlockIdByCoords(yPoint_20594, xPoint_20594, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20594);
                block.setAttribute('data-y', yPoint_20594);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20594
         * @param {type} xPoint_20594
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20594, xPoint_20594, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20594 + '_y' + yPoint_20594;
            }
            return 'pc_x' + xPoint_20594 + '_y' + yPoint_20594;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20594=0;yPoint_20594<this.gameFieldBorderY.length; yPoint_20594++){
                for(var xPoint_20594=0;xPoint_20594<this.gameFieldBorderX.length; xPoint_20594++){
                    map.push({y: yPoint_20594, x: xPoint_20594});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20594=-1;yPoint_20594<(this.gameFieldBorderY.length+1); yPoint_20594++){
                for(var xPoint_20594=-1;xPoint_20594<(this.gameFieldBorderX.length+1); xPoint_20594++){
                    if(!map[yPoint_20594]){
                        map[yPoint_20594] = [];
                    }
                    map[yPoint_20594][xPoint_20594] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20594 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20594 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20594, yPoint_20594) === true){
                    if(this.canPutHorizontal(map, xPoint_20594, yPoint_20594, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20594][xPoint_20594 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20594, yPoint_20594, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20594 + i][xPoint_20594] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20594, yPoint_20594){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20594][xPoint_20594] === this.CELL_EMPTY
                && map[yPoint_20594-1][xPoint_20594] === this.CELL_EMPTY
                && map[yPoint_20594-1][xPoint_20594+1] === this.CELL_EMPTY
                && map[yPoint_20594][xPoint_20594+1] === this.CELL_EMPTY
                && map[yPoint_20594+1][xPoint_20594+1] === this.CELL_EMPTY
                && map[yPoint_20594+1][xPoint_20594] === this.CELL_EMPTY
                && map[yPoint_20594+1][xPoint_20594-1] === this.CELL_EMPTY
                && map[yPoint_20594][xPoint_20594-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20594, yPoint_20594, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20594;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20594][x] === this.CELL_EMPTY
                    && map[yPoint_20594-1][x] === this.CELL_EMPTY
                    && map[yPoint_20594-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20594][x+1] === this.CELL_EMPTY
                    && map[yPoint_20594+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20594+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20594, yPoint_20594, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20594;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20594] === this.CELL_EMPTY
                    && map[y+1][xPoint_20594] === this.CELL_EMPTY
                    && map[y+1][xPoint_20594+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20594] === this.CELL_EMPTY
                    && map[y][xPoint_20594-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20594-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
