...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24874=0;yPoint_24874<this.gameFieldBorderY.length; yPoint_24874++){
                for(var xPoint_24874=0;xPoint_24874<this.gameFieldBorderX.length; xPoint_24874++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24874, xPoint_24874);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24874][xPoint_24874] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24874, xPoint_24874, 'user');
                    if(this._userShipsMap[yPoint_24874][xPoint_24874] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24874, xPoint_24874, type){
            var id = this.getPointBlockIdByCoords(yPoint_24874, xPoint_24874, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24874);
                block.setAttribute('data-y', yPoint_24874);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24874
         * @param {type} xPoint_24874
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24874, xPoint_24874, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24874 + '_y' + yPoint_24874;
            }
            return 'pc_x' + xPoint_24874 + '_y' + yPoint_24874;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24874=0;yPoint_24874<this.gameFieldBorderY.length; yPoint_24874++){
                for(var xPoint_24874=0;xPoint_24874<this.gameFieldBorderX.length; xPoint_24874++){
                    map.push({y: yPoint_24874, x: xPoint_24874});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24874=-1;yPoint_24874<(this.gameFieldBorderY.length+1); yPoint_24874++){
                for(var xPoint_24874=-1;xPoint_24874<(this.gameFieldBorderX.length+1); xPoint_24874++){
                    if(!map[yPoint_24874]){
                        map[yPoint_24874] = [];
                    }
                    map[yPoint_24874][xPoint_24874] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24874 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24874 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24874, yPoint_24874) === true){
                    if(this.canPutHorizontal(map, xPoint_24874, yPoint_24874, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24874][xPoint_24874 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24874, yPoint_24874, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24874 + i][xPoint_24874] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24874, yPoint_24874){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24874][xPoint_24874] === this.CELL_EMPTY
                && map[yPoint_24874-1][xPoint_24874] === this.CELL_EMPTY
                && map[yPoint_24874-1][xPoint_24874+1] === this.CELL_EMPTY
                && map[yPoint_24874][xPoint_24874+1] === this.CELL_EMPTY
                && map[yPoint_24874+1][xPoint_24874+1] === this.CELL_EMPTY
                && map[yPoint_24874+1][xPoint_24874] === this.CELL_EMPTY
                && map[yPoint_24874+1][xPoint_24874-1] === this.CELL_EMPTY
                && map[yPoint_24874][xPoint_24874-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24874, yPoint_24874, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24874;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24874][x] === this.CELL_EMPTY
                    && map[yPoint_24874-1][x] === this.CELL_EMPTY
                    && map[yPoint_24874-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24874][x+1] === this.CELL_EMPTY
                    && map[yPoint_24874+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24874+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24874, yPoint_24874, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24874;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24874] === this.CELL_EMPTY
                    && map[y+1][xPoint_24874] === this.CELL_EMPTY
                    && map[y+1][xPoint_24874+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24874] === this.CELL_EMPTY
                    && map[y][xPoint_24874-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24874-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
