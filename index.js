...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18845=0;yPoint_18845<this.gameFieldBorderY.length; yPoint_18845++){
                for(var xPoint_18845=0;xPoint_18845<this.gameFieldBorderX.length; xPoint_18845++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18845, xPoint_18845);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18845][xPoint_18845] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18845, xPoint_18845, 'user');
                    if(this._userShipsMap[yPoint_18845][xPoint_18845] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18845, xPoint_18845, type){
            var id = this.getPointBlockIdByCoords(yPoint_18845, xPoint_18845, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18845);
                block.setAttribute('data-y', yPoint_18845);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18845
         * @param {type} xPoint_18845
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18845, xPoint_18845, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18845 + '_y' + yPoint_18845;
            }
            return 'pc_x' + xPoint_18845 + '_y' + yPoint_18845;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18845=0;yPoint_18845<this.gameFieldBorderY.length; yPoint_18845++){
                for(var xPoint_18845=0;xPoint_18845<this.gameFieldBorderX.length; xPoint_18845++){
                    map.push({y: yPoint_18845, x: xPoint_18845});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18845=-1;yPoint_18845<(this.gameFieldBorderY.length+1); yPoint_18845++){
                for(var xPoint_18845=-1;xPoint_18845<(this.gameFieldBorderX.length+1); xPoint_18845++){
                    if(!map[yPoint_18845]){
                        map[yPoint_18845] = [];
                    }
                    map[yPoint_18845][xPoint_18845] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18845 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18845 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18845, yPoint_18845) === true){
                    if(this.canPutHorizontal(map, xPoint_18845, yPoint_18845, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18845][xPoint_18845 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18845, yPoint_18845, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18845 + i][xPoint_18845] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18845, yPoint_18845){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18845][xPoint_18845] === this.CELL_EMPTY
                && map[yPoint_18845-1][xPoint_18845] === this.CELL_EMPTY
                && map[yPoint_18845-1][xPoint_18845+1] === this.CELL_EMPTY
                && map[yPoint_18845][xPoint_18845+1] === this.CELL_EMPTY
                && map[yPoint_18845+1][xPoint_18845+1] === this.CELL_EMPTY
                && map[yPoint_18845+1][xPoint_18845] === this.CELL_EMPTY
                && map[yPoint_18845+1][xPoint_18845-1] === this.CELL_EMPTY
                && map[yPoint_18845][xPoint_18845-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18845, yPoint_18845, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18845;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18845][x] === this.CELL_EMPTY
                    && map[yPoint_18845-1][x] === this.CELL_EMPTY
                    && map[yPoint_18845-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18845][x+1] === this.CELL_EMPTY
                    && map[yPoint_18845+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18845+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18845, yPoint_18845, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18845;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18845] === this.CELL_EMPTY
                    && map[y+1][xPoint_18845] === this.CELL_EMPTY
                    && map[y+1][xPoint_18845+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18845] === this.CELL_EMPTY
                    && map[y][xPoint_18845-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18845-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
