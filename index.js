...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7300=0;yPoint_7300<this.gameFieldBorderY.length; yPoint_7300++){
                for(var xPoint_7300=0;xPoint_7300<this.gameFieldBorderX.length; xPoint_7300++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7300, xPoint_7300);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7300][xPoint_7300] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7300, xPoint_7300, 'user');
                    if(this._userShipsMap[yPoint_7300][xPoint_7300] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7300, xPoint_7300, type){
            var id = this.getPointBlockIdByCoords(yPoint_7300, xPoint_7300, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7300);
                block.setAttribute('data-y', yPoint_7300);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7300
         * @param {type} xPoint_7300
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7300, xPoint_7300, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7300 + '_y' + yPoint_7300;
            }
            return 'pc_x' + xPoint_7300 + '_y' + yPoint_7300;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7300=0;yPoint_7300<this.gameFieldBorderY.length; yPoint_7300++){
                for(var xPoint_7300=0;xPoint_7300<this.gameFieldBorderX.length; xPoint_7300++){
                    map.push({y: yPoint_7300, x: xPoint_7300});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7300=-1;yPoint_7300<(this.gameFieldBorderY.length+1); yPoint_7300++){
                for(var xPoint_7300=-1;xPoint_7300<(this.gameFieldBorderX.length+1); xPoint_7300++){
                    if(!map[yPoint_7300]){
                        map[yPoint_7300] = [];
                    }
                    map[yPoint_7300][xPoint_7300] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7300 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7300 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7300, yPoint_7300) === true){
                    if(this.canPutHorizontal(map, xPoint_7300, yPoint_7300, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7300][xPoint_7300 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7300, yPoint_7300, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7300 + i][xPoint_7300] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7300, yPoint_7300){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7300][xPoint_7300] === this.CELL_EMPTY
                && map[yPoint_7300-1][xPoint_7300] === this.CELL_EMPTY
                && map[yPoint_7300-1][xPoint_7300+1] === this.CELL_EMPTY
                && map[yPoint_7300][xPoint_7300+1] === this.CELL_EMPTY
                && map[yPoint_7300+1][xPoint_7300+1] === this.CELL_EMPTY
                && map[yPoint_7300+1][xPoint_7300] === this.CELL_EMPTY
                && map[yPoint_7300+1][xPoint_7300-1] === this.CELL_EMPTY
                && map[yPoint_7300][xPoint_7300-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7300, yPoint_7300, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7300;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7300][x] === this.CELL_EMPTY
                    && map[yPoint_7300-1][x] === this.CELL_EMPTY
                    && map[yPoint_7300-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7300][x+1] === this.CELL_EMPTY
                    && map[yPoint_7300+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7300+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7300, yPoint_7300, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7300;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7300] === this.CELL_EMPTY
                    && map[y+1][xPoint_7300] === this.CELL_EMPTY
                    && map[y+1][xPoint_7300+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7300] === this.CELL_EMPTY
                    && map[y][xPoint_7300-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7300-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
