...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14152=0;yPoint_14152<this.gameFieldBorderY.length; yPoint_14152++){
                for(var xPoint_14152=0;xPoint_14152<this.gameFieldBorderX.length; xPoint_14152++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14152, xPoint_14152);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14152][xPoint_14152] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14152, xPoint_14152, 'user');
                    if(this._userShipsMap[yPoint_14152][xPoint_14152] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14152, xPoint_14152, type){
            var id = this.getPointBlockIdByCoords(yPoint_14152, xPoint_14152, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14152);
                block.setAttribute('data-y', yPoint_14152);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14152
         * @param {type} xPoint_14152
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14152, xPoint_14152, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14152 + '_y' + yPoint_14152;
            }
            return 'pc_x' + xPoint_14152 + '_y' + yPoint_14152;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14152=0;yPoint_14152<this.gameFieldBorderY.length; yPoint_14152++){
                for(var xPoint_14152=0;xPoint_14152<this.gameFieldBorderX.length; xPoint_14152++){
                    map.push({y: yPoint_14152, x: xPoint_14152});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14152=-1;yPoint_14152<(this.gameFieldBorderY.length+1); yPoint_14152++){
                for(var xPoint_14152=-1;xPoint_14152<(this.gameFieldBorderX.length+1); xPoint_14152++){
                    if(!map[yPoint_14152]){
                        map[yPoint_14152] = [];
                    }
                    map[yPoint_14152][xPoint_14152] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14152 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14152 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14152, yPoint_14152) === true){
                    if(this.canPutHorizontal(map, xPoint_14152, yPoint_14152, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14152][xPoint_14152 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14152, yPoint_14152, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14152 + i][xPoint_14152] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14152, yPoint_14152){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14152][xPoint_14152] === this.CELL_EMPTY
                && map[yPoint_14152-1][xPoint_14152] === this.CELL_EMPTY
                && map[yPoint_14152-1][xPoint_14152+1] === this.CELL_EMPTY
                && map[yPoint_14152][xPoint_14152+1] === this.CELL_EMPTY
                && map[yPoint_14152+1][xPoint_14152+1] === this.CELL_EMPTY
                && map[yPoint_14152+1][xPoint_14152] === this.CELL_EMPTY
                && map[yPoint_14152+1][xPoint_14152-1] === this.CELL_EMPTY
                && map[yPoint_14152][xPoint_14152-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14152, yPoint_14152, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14152;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14152][x] === this.CELL_EMPTY
                    && map[yPoint_14152-1][x] === this.CELL_EMPTY
                    && map[yPoint_14152-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14152][x+1] === this.CELL_EMPTY
                    && map[yPoint_14152+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14152+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14152, yPoint_14152, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14152;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14152] === this.CELL_EMPTY
                    && map[y+1][xPoint_14152] === this.CELL_EMPTY
                    && map[y+1][xPoint_14152+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14152] === this.CELL_EMPTY
                    && map[y][xPoint_14152-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14152-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
