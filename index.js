...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25846=0;yPoint_25846<this.gameFieldBorderY.length; yPoint_25846++){
                for(var xPoint_25846=0;xPoint_25846<this.gameFieldBorderX.length; xPoint_25846++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25846, xPoint_25846);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25846][xPoint_25846] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25846, xPoint_25846, 'user');
                    if(this._userShipsMap[yPoint_25846][xPoint_25846] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25846, xPoint_25846, type){
            var id = this.getPointBlockIdByCoords(yPoint_25846, xPoint_25846, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25846);
                block.setAttribute('data-y', yPoint_25846);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25846
         * @param {type} xPoint_25846
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25846, xPoint_25846, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25846 + '_y' + yPoint_25846;
            }
            return 'pc_x' + xPoint_25846 + '_y' + yPoint_25846;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25846=0;yPoint_25846<this.gameFieldBorderY.length; yPoint_25846++){
                for(var xPoint_25846=0;xPoint_25846<this.gameFieldBorderX.length; xPoint_25846++){
                    map.push({y: yPoint_25846, x: xPoint_25846});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25846=-1;yPoint_25846<(this.gameFieldBorderY.length+1); yPoint_25846++){
                for(var xPoint_25846=-1;xPoint_25846<(this.gameFieldBorderX.length+1); xPoint_25846++){
                    if(!map[yPoint_25846]){
                        map[yPoint_25846] = [];
                    }
                    map[yPoint_25846][xPoint_25846] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25846 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25846 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25846, yPoint_25846) === true){
                    if(this.canPutHorizontal(map, xPoint_25846, yPoint_25846, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25846][xPoint_25846 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25846, yPoint_25846, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25846 + i][xPoint_25846] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25846, yPoint_25846){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25846][xPoint_25846] === this.CELL_EMPTY
                && map[yPoint_25846-1][xPoint_25846] === this.CELL_EMPTY
                && map[yPoint_25846-1][xPoint_25846+1] === this.CELL_EMPTY
                && map[yPoint_25846][xPoint_25846+1] === this.CELL_EMPTY
                && map[yPoint_25846+1][xPoint_25846+1] === this.CELL_EMPTY
                && map[yPoint_25846+1][xPoint_25846] === this.CELL_EMPTY
                && map[yPoint_25846+1][xPoint_25846-1] === this.CELL_EMPTY
                && map[yPoint_25846][xPoint_25846-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25846, yPoint_25846, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25846;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25846][x] === this.CELL_EMPTY
                    && map[yPoint_25846-1][x] === this.CELL_EMPTY
                    && map[yPoint_25846-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25846][x+1] === this.CELL_EMPTY
                    && map[yPoint_25846+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25846+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25846, yPoint_25846, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25846;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25846] === this.CELL_EMPTY
                    && map[y+1][xPoint_25846] === this.CELL_EMPTY
                    && map[y+1][xPoint_25846+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25846] === this.CELL_EMPTY
                    && map[y][xPoint_25846-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25846-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
