...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_448=0;yPoint_448<this.gameFieldBorderY.length; yPoint_448++){
                for(var xPoint_448=0;xPoint_448<this.gameFieldBorderX.length; xPoint_448++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_448, xPoint_448);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_448][xPoint_448] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_448, xPoint_448, 'user');
                    if(this._userShipsMap[yPoint_448][xPoint_448] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_448, xPoint_448, type){
            var id = this.getPointBlockIdByCoords(yPoint_448, xPoint_448, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_448);
                block.setAttribute('data-y', yPoint_448);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_448
         * @param {type} xPoint_448
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_448, xPoint_448, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_448 + '_y' + yPoint_448;
            }
            return 'pc_x' + xPoint_448 + '_y' + yPoint_448;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_448=0;yPoint_448<this.gameFieldBorderY.length; yPoint_448++){
                for(var xPoint_448=0;xPoint_448<this.gameFieldBorderX.length; xPoint_448++){
                    map.push({y: yPoint_448, x: xPoint_448});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_448=-1;yPoint_448<(this.gameFieldBorderY.length+1); yPoint_448++){
                for(var xPoint_448=-1;xPoint_448<(this.gameFieldBorderX.length+1); xPoint_448++){
                    if(!map[yPoint_448]){
                        map[yPoint_448] = [];
                    }
                    map[yPoint_448][xPoint_448] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_448 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_448 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_448, yPoint_448) === true){
                    if(this.canPutHorizontal(map, xPoint_448, yPoint_448, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_448][xPoint_448 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_448, yPoint_448, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_448 + i][xPoint_448] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_448, yPoint_448){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_448][xPoint_448] === this.CELL_EMPTY
                && map[yPoint_448-1][xPoint_448] === this.CELL_EMPTY
                && map[yPoint_448-1][xPoint_448+1] === this.CELL_EMPTY
                && map[yPoint_448][xPoint_448+1] === this.CELL_EMPTY
                && map[yPoint_448+1][xPoint_448+1] === this.CELL_EMPTY
                && map[yPoint_448+1][xPoint_448] === this.CELL_EMPTY
                && map[yPoint_448+1][xPoint_448-1] === this.CELL_EMPTY
                && map[yPoint_448][xPoint_448-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_448, yPoint_448, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_448;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_448][x] === this.CELL_EMPTY
                    && map[yPoint_448-1][x] === this.CELL_EMPTY
                    && map[yPoint_448-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_448][x+1] === this.CELL_EMPTY
                    && map[yPoint_448+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_448+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_448, yPoint_448, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_448;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_448] === this.CELL_EMPTY
                    && map[y+1][xPoint_448] === this.CELL_EMPTY
                    && map[y+1][xPoint_448+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_448] === this.CELL_EMPTY
                    && map[y][xPoint_448-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_448-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
