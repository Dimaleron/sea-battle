...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16361=0;yPoint_16361<this.gameFieldBorderY.length; yPoint_16361++){
                for(var xPoint_16361=0;xPoint_16361<this.gameFieldBorderX.length; xPoint_16361++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16361, xPoint_16361);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16361][xPoint_16361] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16361, xPoint_16361, 'user');
                    if(this._userShipsMap[yPoint_16361][xPoint_16361] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16361, xPoint_16361, type){
            var id = this.getPointBlockIdByCoords(yPoint_16361, xPoint_16361, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16361);
                block.setAttribute('data-y', yPoint_16361);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16361
         * @param {type} xPoint_16361
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16361, xPoint_16361, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16361 + '_y' + yPoint_16361;
            }
            return 'pc_x' + xPoint_16361 + '_y' + yPoint_16361;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16361=0;yPoint_16361<this.gameFieldBorderY.length; yPoint_16361++){
                for(var xPoint_16361=0;xPoint_16361<this.gameFieldBorderX.length; xPoint_16361++){
                    map.push({y: yPoint_16361, x: xPoint_16361});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16361=-1;yPoint_16361<(this.gameFieldBorderY.length+1); yPoint_16361++){
                for(var xPoint_16361=-1;xPoint_16361<(this.gameFieldBorderX.length+1); xPoint_16361++){
                    if(!map[yPoint_16361]){
                        map[yPoint_16361] = [];
                    }
                    map[yPoint_16361][xPoint_16361] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16361 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16361 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16361, yPoint_16361) === true){
                    if(this.canPutHorizontal(map, xPoint_16361, yPoint_16361, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16361][xPoint_16361 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16361, yPoint_16361, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16361 + i][xPoint_16361] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16361, yPoint_16361){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16361][xPoint_16361] === this.CELL_EMPTY
                && map[yPoint_16361-1][xPoint_16361] === this.CELL_EMPTY
                && map[yPoint_16361-1][xPoint_16361+1] === this.CELL_EMPTY
                && map[yPoint_16361][xPoint_16361+1] === this.CELL_EMPTY
                && map[yPoint_16361+1][xPoint_16361+1] === this.CELL_EMPTY
                && map[yPoint_16361+1][xPoint_16361] === this.CELL_EMPTY
                && map[yPoint_16361+1][xPoint_16361-1] === this.CELL_EMPTY
                && map[yPoint_16361][xPoint_16361-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16361, yPoint_16361, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16361;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16361][x] === this.CELL_EMPTY
                    && map[yPoint_16361-1][x] === this.CELL_EMPTY
                    && map[yPoint_16361-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16361][x+1] === this.CELL_EMPTY
                    && map[yPoint_16361+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16361+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16361, yPoint_16361, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16361;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16361] === this.CELL_EMPTY
                    && map[y+1][xPoint_16361] === this.CELL_EMPTY
                    && map[y+1][xPoint_16361+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16361] === this.CELL_EMPTY
                    && map[y][xPoint_16361-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16361-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
