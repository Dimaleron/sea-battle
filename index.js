...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_334=0;yPoint_334<this.gameFieldBorderY.length; yPoint_334++){
                for(var xPoint_334=0;xPoint_334<this.gameFieldBorderX.length; xPoint_334++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_334, xPoint_334);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_334][xPoint_334] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_334, xPoint_334, 'user');
                    if(this._userShipsMap[yPoint_334][xPoint_334] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_334, xPoint_334, type){
            var id = this.getPointBlockIdByCoords(yPoint_334, xPoint_334, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_334);
                block.setAttribute('data-y', yPoint_334);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_334
         * @param {type} xPoint_334
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_334, xPoint_334, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_334 + '_y' + yPoint_334;
            }
            return 'pc_x' + xPoint_334 + '_y' + yPoint_334;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_334=0;yPoint_334<this.gameFieldBorderY.length; yPoint_334++){
                for(var xPoint_334=0;xPoint_334<this.gameFieldBorderX.length; xPoint_334++){
                    map.push({y: yPoint_334, x: xPoint_334});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_334=-1;yPoint_334<(this.gameFieldBorderY.length+1); yPoint_334++){
                for(var xPoint_334=-1;xPoint_334<(this.gameFieldBorderX.length+1); xPoint_334++){
                    if(!map[yPoint_334]){
                        map[yPoint_334] = [];
                    }
                    map[yPoint_334][xPoint_334] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_334 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_334 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_334, yPoint_334) === true){
                    if(this.canPutHorizontal(map, xPoint_334, yPoint_334, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_334][xPoint_334 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_334, yPoint_334, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_334 + i][xPoint_334] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_334, yPoint_334){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_334][xPoint_334] === this.CELL_EMPTY
                && map[yPoint_334-1][xPoint_334] === this.CELL_EMPTY
                && map[yPoint_334-1][xPoint_334+1] === this.CELL_EMPTY
                && map[yPoint_334][xPoint_334+1] === this.CELL_EMPTY
                && map[yPoint_334+1][xPoint_334+1] === this.CELL_EMPTY
                && map[yPoint_334+1][xPoint_334] === this.CELL_EMPTY
                && map[yPoint_334+1][xPoint_334-1] === this.CELL_EMPTY
                && map[yPoint_334][xPoint_334-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_334, yPoint_334, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_334;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_334][x] === this.CELL_EMPTY
                    && map[yPoint_334-1][x] === this.CELL_EMPTY
                    && map[yPoint_334-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_334][x+1] === this.CELL_EMPTY
                    && map[yPoint_334+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_334+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_334, yPoint_334, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_334;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_334] === this.CELL_EMPTY
                    && map[y+1][xPoint_334] === this.CELL_EMPTY
                    && map[y+1][xPoint_334+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_334] === this.CELL_EMPTY
                    && map[y][xPoint_334-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_334-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
