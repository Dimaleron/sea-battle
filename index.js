...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27120=0;yPoint_27120<this.gameFieldBorderY.length; yPoint_27120++){
                for(var xPoint_27120=0;xPoint_27120<this.gameFieldBorderX.length; xPoint_27120++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27120, xPoint_27120);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27120][xPoint_27120] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27120, xPoint_27120, 'user');
                    if(this._userShipsMap[yPoint_27120][xPoint_27120] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27120, xPoint_27120, type){
            var id = this.getPointBlockIdByCoords(yPoint_27120, xPoint_27120, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27120);
                block.setAttribute('data-y', yPoint_27120);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27120
         * @param {type} xPoint_27120
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27120, xPoint_27120, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27120 + '_y' + yPoint_27120;
            }
            return 'pc_x' + xPoint_27120 + '_y' + yPoint_27120;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27120=0;yPoint_27120<this.gameFieldBorderY.length; yPoint_27120++){
                for(var xPoint_27120=0;xPoint_27120<this.gameFieldBorderX.length; xPoint_27120++){
                    map.push({y: yPoint_27120, x: xPoint_27120});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27120=-1;yPoint_27120<(this.gameFieldBorderY.length+1); yPoint_27120++){
                for(var xPoint_27120=-1;xPoint_27120<(this.gameFieldBorderX.length+1); xPoint_27120++){
                    if(!map[yPoint_27120]){
                        map[yPoint_27120] = [];
                    }
                    map[yPoint_27120][xPoint_27120] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27120 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27120 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27120, yPoint_27120) === true){
                    if(this.canPutHorizontal(map, xPoint_27120, yPoint_27120, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27120][xPoint_27120 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27120, yPoint_27120, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27120 + i][xPoint_27120] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27120, yPoint_27120){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27120][xPoint_27120] === this.CELL_EMPTY
                && map[yPoint_27120-1][xPoint_27120] === this.CELL_EMPTY
                && map[yPoint_27120-1][xPoint_27120+1] === this.CELL_EMPTY
                && map[yPoint_27120][xPoint_27120+1] === this.CELL_EMPTY
                && map[yPoint_27120+1][xPoint_27120+1] === this.CELL_EMPTY
                && map[yPoint_27120+1][xPoint_27120] === this.CELL_EMPTY
                && map[yPoint_27120+1][xPoint_27120-1] === this.CELL_EMPTY
                && map[yPoint_27120][xPoint_27120-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27120, yPoint_27120, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27120;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27120][x] === this.CELL_EMPTY
                    && map[yPoint_27120-1][x] === this.CELL_EMPTY
                    && map[yPoint_27120-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27120][x+1] === this.CELL_EMPTY
                    && map[yPoint_27120+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27120+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27120, yPoint_27120, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27120;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27120] === this.CELL_EMPTY
                    && map[y+1][xPoint_27120] === this.CELL_EMPTY
                    && map[y+1][xPoint_27120+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27120] === this.CELL_EMPTY
                    && map[y][xPoint_27120-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27120-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
