...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21072=0;yPoint_21072<this.gameFieldBorderY.length; yPoint_21072++){
                for(var xPoint_21072=0;xPoint_21072<this.gameFieldBorderX.length; xPoint_21072++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21072, xPoint_21072);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21072][xPoint_21072] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21072, xPoint_21072, 'user');
                    if(this._userShipsMap[yPoint_21072][xPoint_21072] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21072, xPoint_21072, type){
            var id = this.getPointBlockIdByCoords(yPoint_21072, xPoint_21072, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21072);
                block.setAttribute('data-y', yPoint_21072);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21072
         * @param {type} xPoint_21072
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21072, xPoint_21072, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21072 + '_y' + yPoint_21072;
            }
            return 'pc_x' + xPoint_21072 + '_y' + yPoint_21072;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21072=0;yPoint_21072<this.gameFieldBorderY.length; yPoint_21072++){
                for(var xPoint_21072=0;xPoint_21072<this.gameFieldBorderX.length; xPoint_21072++){
                    map.push({y: yPoint_21072, x: xPoint_21072});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21072=-1;yPoint_21072<(this.gameFieldBorderY.length+1); yPoint_21072++){
                for(var xPoint_21072=-1;xPoint_21072<(this.gameFieldBorderX.length+1); xPoint_21072++){
                    if(!map[yPoint_21072]){
                        map[yPoint_21072] = [];
                    }
                    map[yPoint_21072][xPoint_21072] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21072 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21072 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21072, yPoint_21072) === true){
                    if(this.canPutHorizontal(map, xPoint_21072, yPoint_21072, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21072][xPoint_21072 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21072, yPoint_21072, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21072 + i][xPoint_21072] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21072, yPoint_21072){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21072][xPoint_21072] === this.CELL_EMPTY
                && map[yPoint_21072-1][xPoint_21072] === this.CELL_EMPTY
                && map[yPoint_21072-1][xPoint_21072+1] === this.CELL_EMPTY
                && map[yPoint_21072][xPoint_21072+1] === this.CELL_EMPTY
                && map[yPoint_21072+1][xPoint_21072+1] === this.CELL_EMPTY
                && map[yPoint_21072+1][xPoint_21072] === this.CELL_EMPTY
                && map[yPoint_21072+1][xPoint_21072-1] === this.CELL_EMPTY
                && map[yPoint_21072][xPoint_21072-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21072, yPoint_21072, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21072;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21072][x] === this.CELL_EMPTY
                    && map[yPoint_21072-1][x] === this.CELL_EMPTY
                    && map[yPoint_21072-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21072][x+1] === this.CELL_EMPTY
                    && map[yPoint_21072+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21072+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21072, yPoint_21072, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21072;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21072] === this.CELL_EMPTY
                    && map[y+1][xPoint_21072] === this.CELL_EMPTY
                    && map[y+1][xPoint_21072+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21072] === this.CELL_EMPTY
                    && map[y][xPoint_21072-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21072-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
