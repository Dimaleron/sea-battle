...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24797=0;yPoint_24797<this.gameFieldBorderY.length; yPoint_24797++){
                for(var xPoint_24797=0;xPoint_24797<this.gameFieldBorderX.length; xPoint_24797++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24797, xPoint_24797);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24797][xPoint_24797] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24797, xPoint_24797, 'user');
                    if(this._userShipsMap[yPoint_24797][xPoint_24797] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24797, xPoint_24797, type){
            var id = this.getPointBlockIdByCoords(yPoint_24797, xPoint_24797, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24797);
                block.setAttribute('data-y', yPoint_24797);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24797
         * @param {type} xPoint_24797
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24797, xPoint_24797, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24797 + '_y' + yPoint_24797;
            }
            return 'pc_x' + xPoint_24797 + '_y' + yPoint_24797;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24797=0;yPoint_24797<this.gameFieldBorderY.length; yPoint_24797++){
                for(var xPoint_24797=0;xPoint_24797<this.gameFieldBorderX.length; xPoint_24797++){
                    map.push({y: yPoint_24797, x: xPoint_24797});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24797=-1;yPoint_24797<(this.gameFieldBorderY.length+1); yPoint_24797++){
                for(var xPoint_24797=-1;xPoint_24797<(this.gameFieldBorderX.length+1); xPoint_24797++){
                    if(!map[yPoint_24797]){
                        map[yPoint_24797] = [];
                    }
                    map[yPoint_24797][xPoint_24797] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24797 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24797 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24797, yPoint_24797) === true){
                    if(this.canPutHorizontal(map, xPoint_24797, yPoint_24797, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24797][xPoint_24797 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24797, yPoint_24797, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24797 + i][xPoint_24797] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24797, yPoint_24797){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24797][xPoint_24797] === this.CELL_EMPTY
                && map[yPoint_24797-1][xPoint_24797] === this.CELL_EMPTY
                && map[yPoint_24797-1][xPoint_24797+1] === this.CELL_EMPTY
                && map[yPoint_24797][xPoint_24797+1] === this.CELL_EMPTY
                && map[yPoint_24797+1][xPoint_24797+1] === this.CELL_EMPTY
                && map[yPoint_24797+1][xPoint_24797] === this.CELL_EMPTY
                && map[yPoint_24797+1][xPoint_24797-1] === this.CELL_EMPTY
                && map[yPoint_24797][xPoint_24797-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24797, yPoint_24797, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24797;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24797][x] === this.CELL_EMPTY
                    && map[yPoint_24797-1][x] === this.CELL_EMPTY
                    && map[yPoint_24797-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24797][x+1] === this.CELL_EMPTY
                    && map[yPoint_24797+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24797+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24797, yPoint_24797, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24797;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24797] === this.CELL_EMPTY
                    && map[y+1][xPoint_24797] === this.CELL_EMPTY
                    && map[y+1][xPoint_24797+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24797] === this.CELL_EMPTY
                    && map[y][xPoint_24797-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24797-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
