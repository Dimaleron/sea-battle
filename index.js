...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14580=0;yPoint_14580<this.gameFieldBorderY.length; yPoint_14580++){
                for(var xPoint_14580=0;xPoint_14580<this.gameFieldBorderX.length; xPoint_14580++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14580, xPoint_14580);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14580][xPoint_14580] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14580, xPoint_14580, 'user');
                    if(this._userShipsMap[yPoint_14580][xPoint_14580] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14580, xPoint_14580, type){
            var id = this.getPointBlockIdByCoords(yPoint_14580, xPoint_14580, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14580);
                block.setAttribute('data-y', yPoint_14580);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14580
         * @param {type} xPoint_14580
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14580, xPoint_14580, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14580 + '_y' + yPoint_14580;
            }
            return 'pc_x' + xPoint_14580 + '_y' + yPoint_14580;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14580=0;yPoint_14580<this.gameFieldBorderY.length; yPoint_14580++){
                for(var xPoint_14580=0;xPoint_14580<this.gameFieldBorderX.length; xPoint_14580++){
                    map.push({y: yPoint_14580, x: xPoint_14580});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14580=-1;yPoint_14580<(this.gameFieldBorderY.length+1); yPoint_14580++){
                for(var xPoint_14580=-1;xPoint_14580<(this.gameFieldBorderX.length+1); xPoint_14580++){
                    if(!map[yPoint_14580]){
                        map[yPoint_14580] = [];
                    }
                    map[yPoint_14580][xPoint_14580] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14580 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14580 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14580, yPoint_14580) === true){
                    if(this.canPutHorizontal(map, xPoint_14580, yPoint_14580, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14580][xPoint_14580 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14580, yPoint_14580, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14580 + i][xPoint_14580] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14580, yPoint_14580){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14580][xPoint_14580] === this.CELL_EMPTY
                && map[yPoint_14580-1][xPoint_14580] === this.CELL_EMPTY
                && map[yPoint_14580-1][xPoint_14580+1] === this.CELL_EMPTY
                && map[yPoint_14580][xPoint_14580+1] === this.CELL_EMPTY
                && map[yPoint_14580+1][xPoint_14580+1] === this.CELL_EMPTY
                && map[yPoint_14580+1][xPoint_14580] === this.CELL_EMPTY
                && map[yPoint_14580+1][xPoint_14580-1] === this.CELL_EMPTY
                && map[yPoint_14580][xPoint_14580-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14580, yPoint_14580, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14580;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14580][x] === this.CELL_EMPTY
                    && map[yPoint_14580-1][x] === this.CELL_EMPTY
                    && map[yPoint_14580-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14580][x+1] === this.CELL_EMPTY
                    && map[yPoint_14580+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14580+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14580, yPoint_14580, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14580;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14580] === this.CELL_EMPTY
                    && map[y+1][xPoint_14580] === this.CELL_EMPTY
                    && map[y+1][xPoint_14580+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14580] === this.CELL_EMPTY
                    && map[y][xPoint_14580-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14580-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
