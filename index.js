...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10407=0;yPoint_10407<this.gameFieldBorderY.length; yPoint_10407++){
                for(var xPoint_10407=0;xPoint_10407<this.gameFieldBorderX.length; xPoint_10407++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10407, xPoint_10407);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10407][xPoint_10407] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10407, xPoint_10407, 'user');
                    if(this._userShipsMap[yPoint_10407][xPoint_10407] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10407, xPoint_10407, type){
            var id = this.getPointBlockIdByCoords(yPoint_10407, xPoint_10407, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10407);
                block.setAttribute('data-y', yPoint_10407);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10407
         * @param {type} xPoint_10407
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10407, xPoint_10407, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10407 + '_y' + yPoint_10407;
            }
            return 'pc_x' + xPoint_10407 + '_y' + yPoint_10407;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10407=0;yPoint_10407<this.gameFieldBorderY.length; yPoint_10407++){
                for(var xPoint_10407=0;xPoint_10407<this.gameFieldBorderX.length; xPoint_10407++){
                    map.push({y: yPoint_10407, x: xPoint_10407});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10407=-1;yPoint_10407<(this.gameFieldBorderY.length+1); yPoint_10407++){
                for(var xPoint_10407=-1;xPoint_10407<(this.gameFieldBorderX.length+1); xPoint_10407++){
                    if(!map[yPoint_10407]){
                        map[yPoint_10407] = [];
                    }
                    map[yPoint_10407][xPoint_10407] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10407 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10407 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10407, yPoint_10407) === true){
                    if(this.canPutHorizontal(map, xPoint_10407, yPoint_10407, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10407][xPoint_10407 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10407, yPoint_10407, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10407 + i][xPoint_10407] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10407, yPoint_10407){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10407][xPoint_10407] === this.CELL_EMPTY
                && map[yPoint_10407-1][xPoint_10407] === this.CELL_EMPTY
                && map[yPoint_10407-1][xPoint_10407+1] === this.CELL_EMPTY
                && map[yPoint_10407][xPoint_10407+1] === this.CELL_EMPTY
                && map[yPoint_10407+1][xPoint_10407+1] === this.CELL_EMPTY
                && map[yPoint_10407+1][xPoint_10407] === this.CELL_EMPTY
                && map[yPoint_10407+1][xPoint_10407-1] === this.CELL_EMPTY
                && map[yPoint_10407][xPoint_10407-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10407, yPoint_10407, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10407;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10407][x] === this.CELL_EMPTY
                    && map[yPoint_10407-1][x] === this.CELL_EMPTY
                    && map[yPoint_10407-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10407][x+1] === this.CELL_EMPTY
                    && map[yPoint_10407+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10407+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10407, yPoint_10407, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10407;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10407] === this.CELL_EMPTY
                    && map[y+1][xPoint_10407] === this.CELL_EMPTY
                    && map[y+1][xPoint_10407+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10407] === this.CELL_EMPTY
                    && map[y][xPoint_10407-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10407-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
