...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1491=0;yPoint_1491<this.gameFieldBorderY.length; yPoint_1491++){
                for(var xPoint_1491=0;xPoint_1491<this.gameFieldBorderX.length; xPoint_1491++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1491, xPoint_1491);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1491][xPoint_1491] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1491, xPoint_1491, 'user');
                    if(this._userShipsMap[yPoint_1491][xPoint_1491] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1491, xPoint_1491, type){
            var id = this.getPointBlockIdByCoords(yPoint_1491, xPoint_1491, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1491);
                block.setAttribute('data-y', yPoint_1491);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1491
         * @param {type} xPoint_1491
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1491, xPoint_1491, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1491 + '_y' + yPoint_1491;
            }
            return 'pc_x' + xPoint_1491 + '_y' + yPoint_1491;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1491=0;yPoint_1491<this.gameFieldBorderY.length; yPoint_1491++){
                for(var xPoint_1491=0;xPoint_1491<this.gameFieldBorderX.length; xPoint_1491++){
                    map.push({y: yPoint_1491, x: xPoint_1491});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1491=-1;yPoint_1491<(this.gameFieldBorderY.length+1); yPoint_1491++){
                for(var xPoint_1491=-1;xPoint_1491<(this.gameFieldBorderX.length+1); xPoint_1491++){
                    if(!map[yPoint_1491]){
                        map[yPoint_1491] = [];
                    }
                    map[yPoint_1491][xPoint_1491] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1491 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1491 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1491, yPoint_1491) === true){
                    if(this.canPutHorizontal(map, xPoint_1491, yPoint_1491, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1491][xPoint_1491 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1491, yPoint_1491, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1491 + i][xPoint_1491] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1491, yPoint_1491){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1491][xPoint_1491] === this.CELL_EMPTY
                && map[yPoint_1491-1][xPoint_1491] === this.CELL_EMPTY
                && map[yPoint_1491-1][xPoint_1491+1] === this.CELL_EMPTY
                && map[yPoint_1491][xPoint_1491+1] === this.CELL_EMPTY
                && map[yPoint_1491+1][xPoint_1491+1] === this.CELL_EMPTY
                && map[yPoint_1491+1][xPoint_1491] === this.CELL_EMPTY
                && map[yPoint_1491+1][xPoint_1491-1] === this.CELL_EMPTY
                && map[yPoint_1491][xPoint_1491-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1491, yPoint_1491, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1491;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1491][x] === this.CELL_EMPTY
                    && map[yPoint_1491-1][x] === this.CELL_EMPTY
                    && map[yPoint_1491-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1491][x+1] === this.CELL_EMPTY
                    && map[yPoint_1491+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1491+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1491, yPoint_1491, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1491;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1491] === this.CELL_EMPTY
                    && map[y+1][xPoint_1491] === this.CELL_EMPTY
                    && map[y+1][xPoint_1491+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1491] === this.CELL_EMPTY
                    && map[y][xPoint_1491-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1491-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
