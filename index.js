...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14751=0;yPoint_14751<this.gameFieldBorderY.length; yPoint_14751++){
                for(var xPoint_14751=0;xPoint_14751<this.gameFieldBorderX.length; xPoint_14751++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14751, xPoint_14751);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14751][xPoint_14751] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14751, xPoint_14751, 'user');
                    if(this._userShipsMap[yPoint_14751][xPoint_14751] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14751, xPoint_14751, type){
            var id = this.getPointBlockIdByCoords(yPoint_14751, xPoint_14751, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14751);
                block.setAttribute('data-y', yPoint_14751);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14751
         * @param {type} xPoint_14751
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14751, xPoint_14751, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14751 + '_y' + yPoint_14751;
            }
            return 'pc_x' + xPoint_14751 + '_y' + yPoint_14751;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14751=0;yPoint_14751<this.gameFieldBorderY.length; yPoint_14751++){
                for(var xPoint_14751=0;xPoint_14751<this.gameFieldBorderX.length; xPoint_14751++){
                    map.push({y: yPoint_14751, x: xPoint_14751});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14751=-1;yPoint_14751<(this.gameFieldBorderY.length+1); yPoint_14751++){
                for(var xPoint_14751=-1;xPoint_14751<(this.gameFieldBorderX.length+1); xPoint_14751++){
                    if(!map[yPoint_14751]){
                        map[yPoint_14751] = [];
                    }
                    map[yPoint_14751][xPoint_14751] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14751 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14751 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14751, yPoint_14751) === true){
                    if(this.canPutHorizontal(map, xPoint_14751, yPoint_14751, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14751][xPoint_14751 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14751, yPoint_14751, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14751 + i][xPoint_14751] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14751, yPoint_14751){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14751][xPoint_14751] === this.CELL_EMPTY
                && map[yPoint_14751-1][xPoint_14751] === this.CELL_EMPTY
                && map[yPoint_14751-1][xPoint_14751+1] === this.CELL_EMPTY
                && map[yPoint_14751][xPoint_14751+1] === this.CELL_EMPTY
                && map[yPoint_14751+1][xPoint_14751+1] === this.CELL_EMPTY
                && map[yPoint_14751+1][xPoint_14751] === this.CELL_EMPTY
                && map[yPoint_14751+1][xPoint_14751-1] === this.CELL_EMPTY
                && map[yPoint_14751][xPoint_14751-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14751, yPoint_14751, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14751;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14751][x] === this.CELL_EMPTY
                    && map[yPoint_14751-1][x] === this.CELL_EMPTY
                    && map[yPoint_14751-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14751][x+1] === this.CELL_EMPTY
                    && map[yPoint_14751+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14751+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14751, yPoint_14751, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14751;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14751] === this.CELL_EMPTY
                    && map[y+1][xPoint_14751] === this.CELL_EMPTY
                    && map[y+1][xPoint_14751+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14751] === this.CELL_EMPTY
                    && map[y][xPoint_14751-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14751-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
