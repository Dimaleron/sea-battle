...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25677=0;yPoint_25677<this.gameFieldBorderY.length; yPoint_25677++){
                for(var xPoint_25677=0;xPoint_25677<this.gameFieldBorderX.length; xPoint_25677++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25677, xPoint_25677);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25677][xPoint_25677] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25677, xPoint_25677, 'user');
                    if(this._userShipsMap[yPoint_25677][xPoint_25677] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25677, xPoint_25677, type){
            var id = this.getPointBlockIdByCoords(yPoint_25677, xPoint_25677, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25677);
                block.setAttribute('data-y', yPoint_25677);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25677
         * @param {type} xPoint_25677
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25677, xPoint_25677, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25677 + '_y' + yPoint_25677;
            }
            return 'pc_x' + xPoint_25677 + '_y' + yPoint_25677;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25677=0;yPoint_25677<this.gameFieldBorderY.length; yPoint_25677++){
                for(var xPoint_25677=0;xPoint_25677<this.gameFieldBorderX.length; xPoint_25677++){
                    map.push({y: yPoint_25677, x: xPoint_25677});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25677=-1;yPoint_25677<(this.gameFieldBorderY.length+1); yPoint_25677++){
                for(var xPoint_25677=-1;xPoint_25677<(this.gameFieldBorderX.length+1); xPoint_25677++){
                    if(!map[yPoint_25677]){
                        map[yPoint_25677] = [];
                    }
                    map[yPoint_25677][xPoint_25677] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25677 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25677 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25677, yPoint_25677) === true){
                    if(this.canPutHorizontal(map, xPoint_25677, yPoint_25677, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25677][xPoint_25677 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25677, yPoint_25677, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25677 + i][xPoint_25677] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25677, yPoint_25677){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25677][xPoint_25677] === this.CELL_EMPTY
                && map[yPoint_25677-1][xPoint_25677] === this.CELL_EMPTY
                && map[yPoint_25677-1][xPoint_25677+1] === this.CELL_EMPTY
                && map[yPoint_25677][xPoint_25677+1] === this.CELL_EMPTY
                && map[yPoint_25677+1][xPoint_25677+1] === this.CELL_EMPTY
                && map[yPoint_25677+1][xPoint_25677] === this.CELL_EMPTY
                && map[yPoint_25677+1][xPoint_25677-1] === this.CELL_EMPTY
                && map[yPoint_25677][xPoint_25677-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25677, yPoint_25677, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25677;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25677][x] === this.CELL_EMPTY
                    && map[yPoint_25677-1][x] === this.CELL_EMPTY
                    && map[yPoint_25677-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25677][x+1] === this.CELL_EMPTY
                    && map[yPoint_25677+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25677+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25677, yPoint_25677, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25677;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25677] === this.CELL_EMPTY
                    && map[y+1][xPoint_25677] === this.CELL_EMPTY
                    && map[y+1][xPoint_25677+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25677] === this.CELL_EMPTY
                    && map[y][xPoint_25677-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25677-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
