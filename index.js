...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24448=0;yPoint_24448<this.gameFieldBorderY.length; yPoint_24448++){
                for(var xPoint_24448=0;xPoint_24448<this.gameFieldBorderX.length; xPoint_24448++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24448, xPoint_24448);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24448][xPoint_24448] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24448, xPoint_24448, 'user');
                    if(this._userShipsMap[yPoint_24448][xPoint_24448] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24448, xPoint_24448, type){
            var id = this.getPointBlockIdByCoords(yPoint_24448, xPoint_24448, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24448);
                block.setAttribute('data-y', yPoint_24448);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24448
         * @param {type} xPoint_24448
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24448, xPoint_24448, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24448 + '_y' + yPoint_24448;
            }
            return 'pc_x' + xPoint_24448 + '_y' + yPoint_24448;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24448=0;yPoint_24448<this.gameFieldBorderY.length; yPoint_24448++){
                for(var xPoint_24448=0;xPoint_24448<this.gameFieldBorderX.length; xPoint_24448++){
                    map.push({y: yPoint_24448, x: xPoint_24448});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24448=-1;yPoint_24448<(this.gameFieldBorderY.length+1); yPoint_24448++){
                for(var xPoint_24448=-1;xPoint_24448<(this.gameFieldBorderX.length+1); xPoint_24448++){
                    if(!map[yPoint_24448]){
                        map[yPoint_24448] = [];
                    }
                    map[yPoint_24448][xPoint_24448] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24448 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24448 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24448, yPoint_24448) === true){
                    if(this.canPutHorizontal(map, xPoint_24448, yPoint_24448, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24448][xPoint_24448 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24448, yPoint_24448, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24448 + i][xPoint_24448] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24448, yPoint_24448){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24448][xPoint_24448] === this.CELL_EMPTY
                && map[yPoint_24448-1][xPoint_24448] === this.CELL_EMPTY
                && map[yPoint_24448-1][xPoint_24448+1] === this.CELL_EMPTY
                && map[yPoint_24448][xPoint_24448+1] === this.CELL_EMPTY
                && map[yPoint_24448+1][xPoint_24448+1] === this.CELL_EMPTY
                && map[yPoint_24448+1][xPoint_24448] === this.CELL_EMPTY
                && map[yPoint_24448+1][xPoint_24448-1] === this.CELL_EMPTY
                && map[yPoint_24448][xPoint_24448-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24448, yPoint_24448, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24448;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24448][x] === this.CELL_EMPTY
                    && map[yPoint_24448-1][x] === this.CELL_EMPTY
                    && map[yPoint_24448-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24448][x+1] === this.CELL_EMPTY
                    && map[yPoint_24448+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24448+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24448, yPoint_24448, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24448;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24448] === this.CELL_EMPTY
                    && map[y+1][xPoint_24448] === this.CELL_EMPTY
                    && map[y+1][xPoint_24448+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24448] === this.CELL_EMPTY
                    && map[y][xPoint_24448-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24448-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
