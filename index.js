...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_939=0;yPoint_939<this.gameFieldBorderY.length; yPoint_939++){
                for(var xPoint_939=0;xPoint_939<this.gameFieldBorderX.length; xPoint_939++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_939, xPoint_939);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_939][xPoint_939] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_939, xPoint_939, 'user');
                    if(this._userShipsMap[yPoint_939][xPoint_939] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_939, xPoint_939, type){
            var id = this.getPointBlockIdByCoords(yPoint_939, xPoint_939, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_939);
                block.setAttribute('data-y', yPoint_939);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_939
         * @param {type} xPoint_939
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_939, xPoint_939, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_939 + '_y' + yPoint_939;
            }
            return 'pc_x' + xPoint_939 + '_y' + yPoint_939;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_939=0;yPoint_939<this.gameFieldBorderY.length; yPoint_939++){
                for(var xPoint_939=0;xPoint_939<this.gameFieldBorderX.length; xPoint_939++){
                    map.push({y: yPoint_939, x: xPoint_939});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_939=-1;yPoint_939<(this.gameFieldBorderY.length+1); yPoint_939++){
                for(var xPoint_939=-1;xPoint_939<(this.gameFieldBorderX.length+1); xPoint_939++){
                    if(!map[yPoint_939]){
                        map[yPoint_939] = [];
                    }
                    map[yPoint_939][xPoint_939] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_939 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_939 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_939, yPoint_939) === true){
                    if(this.canPutHorizontal(map, xPoint_939, yPoint_939, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_939][xPoint_939 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_939, yPoint_939, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_939 + i][xPoint_939] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_939, yPoint_939){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_939][xPoint_939] === this.CELL_EMPTY
                && map[yPoint_939-1][xPoint_939] === this.CELL_EMPTY
                && map[yPoint_939-1][xPoint_939+1] === this.CELL_EMPTY
                && map[yPoint_939][xPoint_939+1] === this.CELL_EMPTY
                && map[yPoint_939+1][xPoint_939+1] === this.CELL_EMPTY
                && map[yPoint_939+1][xPoint_939] === this.CELL_EMPTY
                && map[yPoint_939+1][xPoint_939-1] === this.CELL_EMPTY
                && map[yPoint_939][xPoint_939-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_939, yPoint_939, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_939;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_939][x] === this.CELL_EMPTY
                    && map[yPoint_939-1][x] === this.CELL_EMPTY
                    && map[yPoint_939-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_939][x+1] === this.CELL_EMPTY
                    && map[yPoint_939+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_939+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_939, yPoint_939, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_939;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_939] === this.CELL_EMPTY
                    && map[y+1][xPoint_939] === this.CELL_EMPTY
                    && map[y+1][xPoint_939+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_939] === this.CELL_EMPTY
                    && map[y][xPoint_939-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_939-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
