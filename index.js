...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_776=0;yPoint_776<this.gameFieldBorderY.length; yPoint_776++){
                for(var xPoint_776=0;xPoint_776<this.gameFieldBorderX.length; xPoint_776++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_776, xPoint_776);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_776][xPoint_776] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_776, xPoint_776, 'user');
                    if(this._userShipsMap[yPoint_776][xPoint_776] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_776, xPoint_776, type){
            var id = this.getPointBlockIdByCoords(yPoint_776, xPoint_776, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_776);
                block.setAttribute('data-y', yPoint_776);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_776
         * @param {type} xPoint_776
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_776, xPoint_776, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_776 + '_y' + yPoint_776;
            }
            return 'pc_x' + xPoint_776 + '_y' + yPoint_776;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_776=0;yPoint_776<this.gameFieldBorderY.length; yPoint_776++){
                for(var xPoint_776=0;xPoint_776<this.gameFieldBorderX.length; xPoint_776++){
                    map.push({y: yPoint_776, x: xPoint_776});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_776=-1;yPoint_776<(this.gameFieldBorderY.length+1); yPoint_776++){
                for(var xPoint_776=-1;xPoint_776<(this.gameFieldBorderX.length+1); xPoint_776++){
                    if(!map[yPoint_776]){
                        map[yPoint_776] = [];
                    }
                    map[yPoint_776][xPoint_776] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_776 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_776 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_776, yPoint_776) === true){
                    if(this.canPutHorizontal(map, xPoint_776, yPoint_776, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_776][xPoint_776 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_776, yPoint_776, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_776 + i][xPoint_776] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_776, yPoint_776){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_776][xPoint_776] === this.CELL_EMPTY
                && map[yPoint_776-1][xPoint_776] === this.CELL_EMPTY
                && map[yPoint_776-1][xPoint_776+1] === this.CELL_EMPTY
                && map[yPoint_776][xPoint_776+1] === this.CELL_EMPTY
                && map[yPoint_776+1][xPoint_776+1] === this.CELL_EMPTY
                && map[yPoint_776+1][xPoint_776] === this.CELL_EMPTY
                && map[yPoint_776+1][xPoint_776-1] === this.CELL_EMPTY
                && map[yPoint_776][xPoint_776-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_776, yPoint_776, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_776;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_776][x] === this.CELL_EMPTY
                    && map[yPoint_776-1][x] === this.CELL_EMPTY
                    && map[yPoint_776-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_776][x+1] === this.CELL_EMPTY
                    && map[yPoint_776+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_776+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_776, yPoint_776, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_776;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_776] === this.CELL_EMPTY
                    && map[y+1][xPoint_776] === this.CELL_EMPTY
                    && map[y+1][xPoint_776+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_776] === this.CELL_EMPTY
                    && map[y][xPoint_776-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_776-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
