...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5097=0;yPoint_5097<this.gameFieldBorderY.length; yPoint_5097++){
                for(var xPoint_5097=0;xPoint_5097<this.gameFieldBorderX.length; xPoint_5097++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5097, xPoint_5097);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5097][xPoint_5097] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5097, xPoint_5097, 'user');
                    if(this._userShipsMap[yPoint_5097][xPoint_5097] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5097, xPoint_5097, type){
            var id = this.getPointBlockIdByCoords(yPoint_5097, xPoint_5097, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5097);
                block.setAttribute('data-y', yPoint_5097);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5097
         * @param {type} xPoint_5097
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5097, xPoint_5097, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5097 + '_y' + yPoint_5097;
            }
            return 'pc_x' + xPoint_5097 + '_y' + yPoint_5097;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5097=0;yPoint_5097<this.gameFieldBorderY.length; yPoint_5097++){
                for(var xPoint_5097=0;xPoint_5097<this.gameFieldBorderX.length; xPoint_5097++){
                    map.push({y: yPoint_5097, x: xPoint_5097});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5097=-1;yPoint_5097<(this.gameFieldBorderY.length+1); yPoint_5097++){
                for(var xPoint_5097=-1;xPoint_5097<(this.gameFieldBorderX.length+1); xPoint_5097++){
                    if(!map[yPoint_5097]){
                        map[yPoint_5097] = [];
                    }
                    map[yPoint_5097][xPoint_5097] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5097 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5097 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5097, yPoint_5097) === true){
                    if(this.canPutHorizontal(map, xPoint_5097, yPoint_5097, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5097][xPoint_5097 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5097, yPoint_5097, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5097 + i][xPoint_5097] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5097, yPoint_5097){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5097][xPoint_5097] === this.CELL_EMPTY
                && map[yPoint_5097-1][xPoint_5097] === this.CELL_EMPTY
                && map[yPoint_5097-1][xPoint_5097+1] === this.CELL_EMPTY
                && map[yPoint_5097][xPoint_5097+1] === this.CELL_EMPTY
                && map[yPoint_5097+1][xPoint_5097+1] === this.CELL_EMPTY
                && map[yPoint_5097+1][xPoint_5097] === this.CELL_EMPTY
                && map[yPoint_5097+1][xPoint_5097-1] === this.CELL_EMPTY
                && map[yPoint_5097][xPoint_5097-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5097, yPoint_5097, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5097;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5097][x] === this.CELL_EMPTY
                    && map[yPoint_5097-1][x] === this.CELL_EMPTY
                    && map[yPoint_5097-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5097][x+1] === this.CELL_EMPTY
                    && map[yPoint_5097+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5097+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5097, yPoint_5097, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5097;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5097] === this.CELL_EMPTY
                    && map[y+1][xPoint_5097] === this.CELL_EMPTY
                    && map[y+1][xPoint_5097+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5097] === this.CELL_EMPTY
                    && map[y][xPoint_5097-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5097-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
