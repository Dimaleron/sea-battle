...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4789=0;yPoint_4789<this.gameFieldBorderY.length; yPoint_4789++){
                for(var xPoint_4789=0;xPoint_4789<this.gameFieldBorderX.length; xPoint_4789++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4789, xPoint_4789);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4789][xPoint_4789] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4789, xPoint_4789, 'user');
                    if(this._userShipsMap[yPoint_4789][xPoint_4789] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4789, xPoint_4789, type){
            var id = this.getPointBlockIdByCoords(yPoint_4789, xPoint_4789, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4789);
                block.setAttribute('data-y', yPoint_4789);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4789
         * @param {type} xPoint_4789
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4789, xPoint_4789, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4789 + '_y' + yPoint_4789;
            }
            return 'pc_x' + xPoint_4789 + '_y' + yPoint_4789;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4789=0;yPoint_4789<this.gameFieldBorderY.length; yPoint_4789++){
                for(var xPoint_4789=0;xPoint_4789<this.gameFieldBorderX.length; xPoint_4789++){
                    map.push({y: yPoint_4789, x: xPoint_4789});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4789=-1;yPoint_4789<(this.gameFieldBorderY.length+1); yPoint_4789++){
                for(var xPoint_4789=-1;xPoint_4789<(this.gameFieldBorderX.length+1); xPoint_4789++){
                    if(!map[yPoint_4789]){
                        map[yPoint_4789] = [];
                    }
                    map[yPoint_4789][xPoint_4789] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4789 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4789 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4789, yPoint_4789) === true){
                    if(this.canPutHorizontal(map, xPoint_4789, yPoint_4789, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4789][xPoint_4789 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4789, yPoint_4789, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4789 + i][xPoint_4789] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4789, yPoint_4789){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4789][xPoint_4789] === this.CELL_EMPTY
                && map[yPoint_4789-1][xPoint_4789] === this.CELL_EMPTY
                && map[yPoint_4789-1][xPoint_4789+1] === this.CELL_EMPTY
                && map[yPoint_4789][xPoint_4789+1] === this.CELL_EMPTY
                && map[yPoint_4789+1][xPoint_4789+1] === this.CELL_EMPTY
                && map[yPoint_4789+1][xPoint_4789] === this.CELL_EMPTY
                && map[yPoint_4789+1][xPoint_4789-1] === this.CELL_EMPTY
                && map[yPoint_4789][xPoint_4789-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4789, yPoint_4789, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4789;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4789][x] === this.CELL_EMPTY
                    && map[yPoint_4789-1][x] === this.CELL_EMPTY
                    && map[yPoint_4789-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4789][x+1] === this.CELL_EMPTY
                    && map[yPoint_4789+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4789+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4789, yPoint_4789, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4789;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4789] === this.CELL_EMPTY
                    && map[y+1][xPoint_4789] === this.CELL_EMPTY
                    && map[y+1][xPoint_4789+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4789] === this.CELL_EMPTY
                    && map[y][xPoint_4789-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4789-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
