...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_606=0;yPoint_606<this.gameFieldBorderY.length; yPoint_606++){
                for(var xPoint_606=0;xPoint_606<this.gameFieldBorderX.length; xPoint_606++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_606, xPoint_606);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_606][xPoint_606] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_606, xPoint_606, 'user');
                    if(this._userShipsMap[yPoint_606][xPoint_606] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_606, xPoint_606, type){
            var id = this.getPointBlockIdByCoords(yPoint_606, xPoint_606, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_606);
                block.setAttribute('data-y', yPoint_606);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_606
         * @param {type} xPoint_606
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_606, xPoint_606, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_606 + '_y' + yPoint_606;
            }
            return 'pc_x' + xPoint_606 + '_y' + yPoint_606;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_606=0;yPoint_606<this.gameFieldBorderY.length; yPoint_606++){
                for(var xPoint_606=0;xPoint_606<this.gameFieldBorderX.length; xPoint_606++){
                    map.push({y: yPoint_606, x: xPoint_606});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_606=-1;yPoint_606<(this.gameFieldBorderY.length+1); yPoint_606++){
                for(var xPoint_606=-1;xPoint_606<(this.gameFieldBorderX.length+1); xPoint_606++){
                    if(!map[yPoint_606]){
                        map[yPoint_606] = [];
                    }
                    map[yPoint_606][xPoint_606] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_606 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_606 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_606, yPoint_606) === true){
                    if(this.canPutHorizontal(map, xPoint_606, yPoint_606, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_606][xPoint_606 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_606, yPoint_606, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_606 + i][xPoint_606] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_606, yPoint_606){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_606][xPoint_606] === this.CELL_EMPTY
                && map[yPoint_606-1][xPoint_606] === this.CELL_EMPTY
                && map[yPoint_606-1][xPoint_606+1] === this.CELL_EMPTY
                && map[yPoint_606][xPoint_606+1] === this.CELL_EMPTY
                && map[yPoint_606+1][xPoint_606+1] === this.CELL_EMPTY
                && map[yPoint_606+1][xPoint_606] === this.CELL_EMPTY
                && map[yPoint_606+1][xPoint_606-1] === this.CELL_EMPTY
                && map[yPoint_606][xPoint_606-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_606, yPoint_606, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_606;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_606][x] === this.CELL_EMPTY
                    && map[yPoint_606-1][x] === this.CELL_EMPTY
                    && map[yPoint_606-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_606][x+1] === this.CELL_EMPTY
                    && map[yPoint_606+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_606+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_606, yPoint_606, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_606;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_606] === this.CELL_EMPTY
                    && map[y+1][xPoint_606] === this.CELL_EMPTY
                    && map[y+1][xPoint_606+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_606] === this.CELL_EMPTY
                    && map[y][xPoint_606-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_606-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
