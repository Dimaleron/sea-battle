...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4419=0;yPoint_4419<this.gameFieldBorderY.length; yPoint_4419++){
                for(var xPoint_4419=0;xPoint_4419<this.gameFieldBorderX.length; xPoint_4419++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4419, xPoint_4419);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4419][xPoint_4419] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4419, xPoint_4419, 'user');
                    if(this._userShipsMap[yPoint_4419][xPoint_4419] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4419, xPoint_4419, type){
            var id = this.getPointBlockIdByCoords(yPoint_4419, xPoint_4419, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4419);
                block.setAttribute('data-y', yPoint_4419);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4419
         * @param {type} xPoint_4419
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4419, xPoint_4419, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4419 + '_y' + yPoint_4419;
            }
            return 'pc_x' + xPoint_4419 + '_y' + yPoint_4419;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4419=0;yPoint_4419<this.gameFieldBorderY.length; yPoint_4419++){
                for(var xPoint_4419=0;xPoint_4419<this.gameFieldBorderX.length; xPoint_4419++){
                    map.push({y: yPoint_4419, x: xPoint_4419});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4419=-1;yPoint_4419<(this.gameFieldBorderY.length+1); yPoint_4419++){
                for(var xPoint_4419=-1;xPoint_4419<(this.gameFieldBorderX.length+1); xPoint_4419++){
                    if(!map[yPoint_4419]){
                        map[yPoint_4419] = [];
                    }
                    map[yPoint_4419][xPoint_4419] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4419 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4419 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4419, yPoint_4419) === true){
                    if(this.canPutHorizontal(map, xPoint_4419, yPoint_4419, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4419][xPoint_4419 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4419, yPoint_4419, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4419 + i][xPoint_4419] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4419, yPoint_4419){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4419][xPoint_4419] === this.CELL_EMPTY
                && map[yPoint_4419-1][xPoint_4419] === this.CELL_EMPTY
                && map[yPoint_4419-1][xPoint_4419+1] === this.CELL_EMPTY
                && map[yPoint_4419][xPoint_4419+1] === this.CELL_EMPTY
                && map[yPoint_4419+1][xPoint_4419+1] === this.CELL_EMPTY
                && map[yPoint_4419+1][xPoint_4419] === this.CELL_EMPTY
                && map[yPoint_4419+1][xPoint_4419-1] === this.CELL_EMPTY
                && map[yPoint_4419][xPoint_4419-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4419, yPoint_4419, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4419;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4419][x] === this.CELL_EMPTY
                    && map[yPoint_4419-1][x] === this.CELL_EMPTY
                    && map[yPoint_4419-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4419][x+1] === this.CELL_EMPTY
                    && map[yPoint_4419+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4419+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4419, yPoint_4419, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4419;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4419] === this.CELL_EMPTY
                    && map[y+1][xPoint_4419] === this.CELL_EMPTY
                    && map[y+1][xPoint_4419+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4419] === this.CELL_EMPTY
                    && map[y][xPoint_4419-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4419-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
