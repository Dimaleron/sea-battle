...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9304=0;yPoint_9304<this.gameFieldBorderY.length; yPoint_9304++){
                for(var xPoint_9304=0;xPoint_9304<this.gameFieldBorderX.length; xPoint_9304++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9304, xPoint_9304);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9304][xPoint_9304] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9304, xPoint_9304, 'user');
                    if(this._userShipsMap[yPoint_9304][xPoint_9304] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9304, xPoint_9304, type){
            var id = this.getPointBlockIdByCoords(yPoint_9304, xPoint_9304, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9304);
                block.setAttribute('data-y', yPoint_9304);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9304
         * @param {type} xPoint_9304
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9304, xPoint_9304, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9304 + '_y' + yPoint_9304;
            }
            return 'pc_x' + xPoint_9304 + '_y' + yPoint_9304;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9304=0;yPoint_9304<this.gameFieldBorderY.length; yPoint_9304++){
                for(var xPoint_9304=0;xPoint_9304<this.gameFieldBorderX.length; xPoint_9304++){
                    map.push({y: yPoint_9304, x: xPoint_9304});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9304=-1;yPoint_9304<(this.gameFieldBorderY.length+1); yPoint_9304++){
                for(var xPoint_9304=-1;xPoint_9304<(this.gameFieldBorderX.length+1); xPoint_9304++){
                    if(!map[yPoint_9304]){
                        map[yPoint_9304] = [];
                    }
                    map[yPoint_9304][xPoint_9304] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9304 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9304 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9304, yPoint_9304) === true){
                    if(this.canPutHorizontal(map, xPoint_9304, yPoint_9304, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9304][xPoint_9304 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9304, yPoint_9304, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9304 + i][xPoint_9304] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9304, yPoint_9304){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9304][xPoint_9304] === this.CELL_EMPTY
                && map[yPoint_9304-1][xPoint_9304] === this.CELL_EMPTY
                && map[yPoint_9304-1][xPoint_9304+1] === this.CELL_EMPTY
                && map[yPoint_9304][xPoint_9304+1] === this.CELL_EMPTY
                && map[yPoint_9304+1][xPoint_9304+1] === this.CELL_EMPTY
                && map[yPoint_9304+1][xPoint_9304] === this.CELL_EMPTY
                && map[yPoint_9304+1][xPoint_9304-1] === this.CELL_EMPTY
                && map[yPoint_9304][xPoint_9304-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9304, yPoint_9304, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9304;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9304][x] === this.CELL_EMPTY
                    && map[yPoint_9304-1][x] === this.CELL_EMPTY
                    && map[yPoint_9304-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9304][x+1] === this.CELL_EMPTY
                    && map[yPoint_9304+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9304+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9304, yPoint_9304, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9304;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9304] === this.CELL_EMPTY
                    && map[y+1][xPoint_9304] === this.CELL_EMPTY
                    && map[y+1][xPoint_9304+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9304] === this.CELL_EMPTY
                    && map[y][xPoint_9304-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9304-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
