...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18826=0;yPoint_18826<this.gameFieldBorderY.length; yPoint_18826++){
                for(var xPoint_18826=0;xPoint_18826<this.gameFieldBorderX.length; xPoint_18826++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18826, xPoint_18826);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18826][xPoint_18826] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18826, xPoint_18826, 'user');
                    if(this._userShipsMap[yPoint_18826][xPoint_18826] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18826, xPoint_18826, type){
            var id = this.getPointBlockIdByCoords(yPoint_18826, xPoint_18826, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18826);
                block.setAttribute('data-y', yPoint_18826);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18826
         * @param {type} xPoint_18826
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18826, xPoint_18826, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18826 + '_y' + yPoint_18826;
            }
            return 'pc_x' + xPoint_18826 + '_y' + yPoint_18826;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18826=0;yPoint_18826<this.gameFieldBorderY.length; yPoint_18826++){
                for(var xPoint_18826=0;xPoint_18826<this.gameFieldBorderX.length; xPoint_18826++){
                    map.push({y: yPoint_18826, x: xPoint_18826});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18826=-1;yPoint_18826<(this.gameFieldBorderY.length+1); yPoint_18826++){
                for(var xPoint_18826=-1;xPoint_18826<(this.gameFieldBorderX.length+1); xPoint_18826++){
                    if(!map[yPoint_18826]){
                        map[yPoint_18826] = [];
                    }
                    map[yPoint_18826][xPoint_18826] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18826 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18826 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18826, yPoint_18826) === true){
                    if(this.canPutHorizontal(map, xPoint_18826, yPoint_18826, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18826][xPoint_18826 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18826, yPoint_18826, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18826 + i][xPoint_18826] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18826, yPoint_18826){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18826][xPoint_18826] === this.CELL_EMPTY
                && map[yPoint_18826-1][xPoint_18826] === this.CELL_EMPTY
                && map[yPoint_18826-1][xPoint_18826+1] === this.CELL_EMPTY
                && map[yPoint_18826][xPoint_18826+1] === this.CELL_EMPTY
                && map[yPoint_18826+1][xPoint_18826+1] === this.CELL_EMPTY
                && map[yPoint_18826+1][xPoint_18826] === this.CELL_EMPTY
                && map[yPoint_18826+1][xPoint_18826-1] === this.CELL_EMPTY
                && map[yPoint_18826][xPoint_18826-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18826, yPoint_18826, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18826;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18826][x] === this.CELL_EMPTY
                    && map[yPoint_18826-1][x] === this.CELL_EMPTY
                    && map[yPoint_18826-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18826][x+1] === this.CELL_EMPTY
                    && map[yPoint_18826+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18826+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18826, yPoint_18826, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18826;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18826] === this.CELL_EMPTY
                    && map[y+1][xPoint_18826] === this.CELL_EMPTY
                    && map[y+1][xPoint_18826+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18826] === this.CELL_EMPTY
                    && map[y][xPoint_18826-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18826-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
