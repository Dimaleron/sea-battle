...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21569=0;yPoint_21569<this.gameFieldBorderY.length; yPoint_21569++){
                for(var xPoint_21569=0;xPoint_21569<this.gameFieldBorderX.length; xPoint_21569++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21569, xPoint_21569);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21569][xPoint_21569] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21569, xPoint_21569, 'user');
                    if(this._userShipsMap[yPoint_21569][xPoint_21569] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21569, xPoint_21569, type){
            var id = this.getPointBlockIdByCoords(yPoint_21569, xPoint_21569, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21569);
                block.setAttribute('data-y', yPoint_21569);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21569
         * @param {type} xPoint_21569
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21569, xPoint_21569, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21569 + '_y' + yPoint_21569;
            }
            return 'pc_x' + xPoint_21569 + '_y' + yPoint_21569;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21569=0;yPoint_21569<this.gameFieldBorderY.length; yPoint_21569++){
                for(var xPoint_21569=0;xPoint_21569<this.gameFieldBorderX.length; xPoint_21569++){
                    map.push({y: yPoint_21569, x: xPoint_21569});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21569=-1;yPoint_21569<(this.gameFieldBorderY.length+1); yPoint_21569++){
                for(var xPoint_21569=-1;xPoint_21569<(this.gameFieldBorderX.length+1); xPoint_21569++){
                    if(!map[yPoint_21569]){
                        map[yPoint_21569] = [];
                    }
                    map[yPoint_21569][xPoint_21569] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21569 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21569 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21569, yPoint_21569) === true){
                    if(this.canPutHorizontal(map, xPoint_21569, yPoint_21569, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21569][xPoint_21569 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21569, yPoint_21569, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21569 + i][xPoint_21569] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21569, yPoint_21569){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21569][xPoint_21569] === this.CELL_EMPTY
                && map[yPoint_21569-1][xPoint_21569] === this.CELL_EMPTY
                && map[yPoint_21569-1][xPoint_21569+1] === this.CELL_EMPTY
                && map[yPoint_21569][xPoint_21569+1] === this.CELL_EMPTY
                && map[yPoint_21569+1][xPoint_21569+1] === this.CELL_EMPTY
                && map[yPoint_21569+1][xPoint_21569] === this.CELL_EMPTY
                && map[yPoint_21569+1][xPoint_21569-1] === this.CELL_EMPTY
                && map[yPoint_21569][xPoint_21569-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21569, yPoint_21569, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21569;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21569][x] === this.CELL_EMPTY
                    && map[yPoint_21569-1][x] === this.CELL_EMPTY
                    && map[yPoint_21569-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21569][x+1] === this.CELL_EMPTY
                    && map[yPoint_21569+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21569+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21569, yPoint_21569, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21569;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21569] === this.CELL_EMPTY
                    && map[y+1][xPoint_21569] === this.CELL_EMPTY
                    && map[y+1][xPoint_21569+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21569] === this.CELL_EMPTY
                    && map[y][xPoint_21569-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21569-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
