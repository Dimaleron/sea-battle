...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3969=0;yPoint_3969<this.gameFieldBorderY.length; yPoint_3969++){
                for(var xPoint_3969=0;xPoint_3969<this.gameFieldBorderX.length; xPoint_3969++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3969, xPoint_3969);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3969][xPoint_3969] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3969, xPoint_3969, 'user');
                    if(this._userShipsMap[yPoint_3969][xPoint_3969] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3969, xPoint_3969, type){
            var id = this.getPointBlockIdByCoords(yPoint_3969, xPoint_3969, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3969);
                block.setAttribute('data-y', yPoint_3969);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3969
         * @param {type} xPoint_3969
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3969, xPoint_3969, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3969 + '_y' + yPoint_3969;
            }
            return 'pc_x' + xPoint_3969 + '_y' + yPoint_3969;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3969=0;yPoint_3969<this.gameFieldBorderY.length; yPoint_3969++){
                for(var xPoint_3969=0;xPoint_3969<this.gameFieldBorderX.length; xPoint_3969++){
                    map.push({y: yPoint_3969, x: xPoint_3969});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3969=-1;yPoint_3969<(this.gameFieldBorderY.length+1); yPoint_3969++){
                for(var xPoint_3969=-1;xPoint_3969<(this.gameFieldBorderX.length+1); xPoint_3969++){
                    if(!map[yPoint_3969]){
                        map[yPoint_3969] = [];
                    }
                    map[yPoint_3969][xPoint_3969] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3969 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3969 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3969, yPoint_3969) === true){
                    if(this.canPutHorizontal(map, xPoint_3969, yPoint_3969, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3969][xPoint_3969 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3969, yPoint_3969, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3969 + i][xPoint_3969] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3969, yPoint_3969){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3969][xPoint_3969] === this.CELL_EMPTY
                && map[yPoint_3969-1][xPoint_3969] === this.CELL_EMPTY
                && map[yPoint_3969-1][xPoint_3969+1] === this.CELL_EMPTY
                && map[yPoint_3969][xPoint_3969+1] === this.CELL_EMPTY
                && map[yPoint_3969+1][xPoint_3969+1] === this.CELL_EMPTY
                && map[yPoint_3969+1][xPoint_3969] === this.CELL_EMPTY
                && map[yPoint_3969+1][xPoint_3969-1] === this.CELL_EMPTY
                && map[yPoint_3969][xPoint_3969-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3969, yPoint_3969, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3969;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3969][x] === this.CELL_EMPTY
                    && map[yPoint_3969-1][x] === this.CELL_EMPTY
                    && map[yPoint_3969-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3969][x+1] === this.CELL_EMPTY
                    && map[yPoint_3969+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3969+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3969, yPoint_3969, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3969;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3969] === this.CELL_EMPTY
                    && map[y+1][xPoint_3969] === this.CELL_EMPTY
                    && map[y+1][xPoint_3969+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3969] === this.CELL_EMPTY
                    && map[y][xPoint_3969-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3969-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
