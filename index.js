...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16290=0;yPoint_16290<this.gameFieldBorderY.length; yPoint_16290++){
                for(var xPoint_16290=0;xPoint_16290<this.gameFieldBorderX.length; xPoint_16290++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16290, xPoint_16290);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16290][xPoint_16290] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16290, xPoint_16290, 'user');
                    if(this._userShipsMap[yPoint_16290][xPoint_16290] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16290, xPoint_16290, type){
            var id = this.getPointBlockIdByCoords(yPoint_16290, xPoint_16290, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16290);
                block.setAttribute('data-y', yPoint_16290);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16290
         * @param {type} xPoint_16290
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16290, xPoint_16290, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16290 + '_y' + yPoint_16290;
            }
            return 'pc_x' + xPoint_16290 + '_y' + yPoint_16290;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16290=0;yPoint_16290<this.gameFieldBorderY.length; yPoint_16290++){
                for(var xPoint_16290=0;xPoint_16290<this.gameFieldBorderX.length; xPoint_16290++){
                    map.push({y: yPoint_16290, x: xPoint_16290});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16290=-1;yPoint_16290<(this.gameFieldBorderY.length+1); yPoint_16290++){
                for(var xPoint_16290=-1;xPoint_16290<(this.gameFieldBorderX.length+1); xPoint_16290++){
                    if(!map[yPoint_16290]){
                        map[yPoint_16290] = [];
                    }
                    map[yPoint_16290][xPoint_16290] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16290 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16290 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16290, yPoint_16290) === true){
                    if(this.canPutHorizontal(map, xPoint_16290, yPoint_16290, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16290][xPoint_16290 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16290, yPoint_16290, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16290 + i][xPoint_16290] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16290, yPoint_16290){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16290][xPoint_16290] === this.CELL_EMPTY
                && map[yPoint_16290-1][xPoint_16290] === this.CELL_EMPTY
                && map[yPoint_16290-1][xPoint_16290+1] === this.CELL_EMPTY
                && map[yPoint_16290][xPoint_16290+1] === this.CELL_EMPTY
                && map[yPoint_16290+1][xPoint_16290+1] === this.CELL_EMPTY
                && map[yPoint_16290+1][xPoint_16290] === this.CELL_EMPTY
                && map[yPoint_16290+1][xPoint_16290-1] === this.CELL_EMPTY
                && map[yPoint_16290][xPoint_16290-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16290, yPoint_16290, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16290;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16290][x] === this.CELL_EMPTY
                    && map[yPoint_16290-1][x] === this.CELL_EMPTY
                    && map[yPoint_16290-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16290][x+1] === this.CELL_EMPTY
                    && map[yPoint_16290+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16290+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16290, yPoint_16290, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16290;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16290] === this.CELL_EMPTY
                    && map[y+1][xPoint_16290] === this.CELL_EMPTY
                    && map[y+1][xPoint_16290+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16290] === this.CELL_EMPTY
                    && map[y][xPoint_16290-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16290-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
