...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1792=0;yPoint_1792<this.gameFieldBorderY.length; yPoint_1792++){
                for(var xPoint_1792=0;xPoint_1792<this.gameFieldBorderX.length; xPoint_1792++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1792, xPoint_1792);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1792][xPoint_1792] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1792, xPoint_1792, 'user');
                    if(this._userShipsMap[yPoint_1792][xPoint_1792] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1792, xPoint_1792, type){
            var id = this.getPointBlockIdByCoords(yPoint_1792, xPoint_1792, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1792);
                block.setAttribute('data-y', yPoint_1792);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1792
         * @param {type} xPoint_1792
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1792, xPoint_1792, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1792 + '_y' + yPoint_1792;
            }
            return 'pc_x' + xPoint_1792 + '_y' + yPoint_1792;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1792=0;yPoint_1792<this.gameFieldBorderY.length; yPoint_1792++){
                for(var xPoint_1792=0;xPoint_1792<this.gameFieldBorderX.length; xPoint_1792++){
                    map.push({y: yPoint_1792, x: xPoint_1792});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1792=-1;yPoint_1792<(this.gameFieldBorderY.length+1); yPoint_1792++){
                for(var xPoint_1792=-1;xPoint_1792<(this.gameFieldBorderX.length+1); xPoint_1792++){
                    if(!map[yPoint_1792]){
                        map[yPoint_1792] = [];
                    }
                    map[yPoint_1792][xPoint_1792] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1792 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1792 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1792, yPoint_1792) === true){
                    if(this.canPutHorizontal(map, xPoint_1792, yPoint_1792, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1792][xPoint_1792 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1792, yPoint_1792, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1792 + i][xPoint_1792] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1792, yPoint_1792){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1792][xPoint_1792] === this.CELL_EMPTY
                && map[yPoint_1792-1][xPoint_1792] === this.CELL_EMPTY
                && map[yPoint_1792-1][xPoint_1792+1] === this.CELL_EMPTY
                && map[yPoint_1792][xPoint_1792+1] === this.CELL_EMPTY
                && map[yPoint_1792+1][xPoint_1792+1] === this.CELL_EMPTY
                && map[yPoint_1792+1][xPoint_1792] === this.CELL_EMPTY
                && map[yPoint_1792+1][xPoint_1792-1] === this.CELL_EMPTY
                && map[yPoint_1792][xPoint_1792-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1792, yPoint_1792, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1792;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1792][x] === this.CELL_EMPTY
                    && map[yPoint_1792-1][x] === this.CELL_EMPTY
                    && map[yPoint_1792-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1792][x+1] === this.CELL_EMPTY
                    && map[yPoint_1792+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1792+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1792, yPoint_1792, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1792;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1792] === this.CELL_EMPTY
                    && map[y+1][xPoint_1792] === this.CELL_EMPTY
                    && map[y+1][xPoint_1792+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1792] === this.CELL_EMPTY
                    && map[y][xPoint_1792-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1792-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
