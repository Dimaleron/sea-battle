...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32204=0;yPoint_32204<this.gameFieldBorderY.length; yPoint_32204++){
                for(var xPoint_32204=0;xPoint_32204<this.gameFieldBorderX.length; xPoint_32204++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32204, xPoint_32204);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32204][xPoint_32204] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32204, xPoint_32204, 'user');
                    if(this._userShipsMap[yPoint_32204][xPoint_32204] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32204, xPoint_32204, type){
            var id = this.getPointBlockIdByCoords(yPoint_32204, xPoint_32204, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32204);
                block.setAttribute('data-y', yPoint_32204);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32204
         * @param {type} xPoint_32204
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32204, xPoint_32204, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32204 + '_y' + yPoint_32204;
            }
            return 'pc_x' + xPoint_32204 + '_y' + yPoint_32204;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32204=0;yPoint_32204<this.gameFieldBorderY.length; yPoint_32204++){
                for(var xPoint_32204=0;xPoint_32204<this.gameFieldBorderX.length; xPoint_32204++){
                    map.push({y: yPoint_32204, x: xPoint_32204});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32204=-1;yPoint_32204<(this.gameFieldBorderY.length+1); yPoint_32204++){
                for(var xPoint_32204=-1;xPoint_32204<(this.gameFieldBorderX.length+1); xPoint_32204++){
                    if(!map[yPoint_32204]){
                        map[yPoint_32204] = [];
                    }
                    map[yPoint_32204][xPoint_32204] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32204 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32204 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32204, yPoint_32204) === true){
                    if(this.canPutHorizontal(map, xPoint_32204, yPoint_32204, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32204][xPoint_32204 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32204, yPoint_32204, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32204 + i][xPoint_32204] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32204, yPoint_32204){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32204][xPoint_32204] === this.CELL_EMPTY
                && map[yPoint_32204-1][xPoint_32204] === this.CELL_EMPTY
                && map[yPoint_32204-1][xPoint_32204+1] === this.CELL_EMPTY
                && map[yPoint_32204][xPoint_32204+1] === this.CELL_EMPTY
                && map[yPoint_32204+1][xPoint_32204+1] === this.CELL_EMPTY
                && map[yPoint_32204+1][xPoint_32204] === this.CELL_EMPTY
                && map[yPoint_32204+1][xPoint_32204-1] === this.CELL_EMPTY
                && map[yPoint_32204][xPoint_32204-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32204, yPoint_32204, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32204;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32204][x] === this.CELL_EMPTY
                    && map[yPoint_32204-1][x] === this.CELL_EMPTY
                    && map[yPoint_32204-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32204][x+1] === this.CELL_EMPTY
                    && map[yPoint_32204+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32204+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32204, yPoint_32204, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32204;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32204] === this.CELL_EMPTY
                    && map[y+1][xPoint_32204] === this.CELL_EMPTY
                    && map[y+1][xPoint_32204+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32204] === this.CELL_EMPTY
                    && map[y][xPoint_32204-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32204-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
