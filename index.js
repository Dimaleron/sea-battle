...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6102=0;yPoint_6102<this.gameFieldBorderY.length; yPoint_6102++){
                for(var xPoint_6102=0;xPoint_6102<this.gameFieldBorderX.length; xPoint_6102++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6102, xPoint_6102);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6102][xPoint_6102] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6102, xPoint_6102, 'user');
                    if(this._userShipsMap[yPoint_6102][xPoint_6102] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6102, xPoint_6102, type){
            var id = this.getPointBlockIdByCoords(yPoint_6102, xPoint_6102, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6102);
                block.setAttribute('data-y', yPoint_6102);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6102
         * @param {type} xPoint_6102
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6102, xPoint_6102, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6102 + '_y' + yPoint_6102;
            }
            return 'pc_x' + xPoint_6102 + '_y' + yPoint_6102;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6102=0;yPoint_6102<this.gameFieldBorderY.length; yPoint_6102++){
                for(var xPoint_6102=0;xPoint_6102<this.gameFieldBorderX.length; xPoint_6102++){
                    map.push({y: yPoint_6102, x: xPoint_6102});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6102=-1;yPoint_6102<(this.gameFieldBorderY.length+1); yPoint_6102++){
                for(var xPoint_6102=-1;xPoint_6102<(this.gameFieldBorderX.length+1); xPoint_6102++){
                    if(!map[yPoint_6102]){
                        map[yPoint_6102] = [];
                    }
                    map[yPoint_6102][xPoint_6102] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6102 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6102 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6102, yPoint_6102) === true){
                    if(this.canPutHorizontal(map, xPoint_6102, yPoint_6102, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6102][xPoint_6102 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6102, yPoint_6102, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6102 + i][xPoint_6102] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6102, yPoint_6102){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6102][xPoint_6102] === this.CELL_EMPTY
                && map[yPoint_6102-1][xPoint_6102] === this.CELL_EMPTY
                && map[yPoint_6102-1][xPoint_6102+1] === this.CELL_EMPTY
                && map[yPoint_6102][xPoint_6102+1] === this.CELL_EMPTY
                && map[yPoint_6102+1][xPoint_6102+1] === this.CELL_EMPTY
                && map[yPoint_6102+1][xPoint_6102] === this.CELL_EMPTY
                && map[yPoint_6102+1][xPoint_6102-1] === this.CELL_EMPTY
                && map[yPoint_6102][xPoint_6102-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6102, yPoint_6102, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6102;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6102][x] === this.CELL_EMPTY
                    && map[yPoint_6102-1][x] === this.CELL_EMPTY
                    && map[yPoint_6102-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6102][x+1] === this.CELL_EMPTY
                    && map[yPoint_6102+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6102+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6102, yPoint_6102, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6102;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6102] === this.CELL_EMPTY
                    && map[y+1][xPoint_6102] === this.CELL_EMPTY
                    && map[y+1][xPoint_6102+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6102] === this.CELL_EMPTY
                    && map[y][xPoint_6102-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6102-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
