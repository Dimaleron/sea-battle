...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16155=0;yPoint_16155<this.gameFieldBorderY.length; yPoint_16155++){
                for(var xPoint_16155=0;xPoint_16155<this.gameFieldBorderX.length; xPoint_16155++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16155, xPoint_16155);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16155][xPoint_16155] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16155, xPoint_16155, 'user');
                    if(this._userShipsMap[yPoint_16155][xPoint_16155] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16155, xPoint_16155, type){
            var id = this.getPointBlockIdByCoords(yPoint_16155, xPoint_16155, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16155);
                block.setAttribute('data-y', yPoint_16155);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16155
         * @param {type} xPoint_16155
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16155, xPoint_16155, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16155 + '_y' + yPoint_16155;
            }
            return 'pc_x' + xPoint_16155 + '_y' + yPoint_16155;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16155=0;yPoint_16155<this.gameFieldBorderY.length; yPoint_16155++){
                for(var xPoint_16155=0;xPoint_16155<this.gameFieldBorderX.length; xPoint_16155++){
                    map.push({y: yPoint_16155, x: xPoint_16155});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16155=-1;yPoint_16155<(this.gameFieldBorderY.length+1); yPoint_16155++){
                for(var xPoint_16155=-1;xPoint_16155<(this.gameFieldBorderX.length+1); xPoint_16155++){
                    if(!map[yPoint_16155]){
                        map[yPoint_16155] = [];
                    }
                    map[yPoint_16155][xPoint_16155] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16155 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16155 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16155, yPoint_16155) === true){
                    if(this.canPutHorizontal(map, xPoint_16155, yPoint_16155, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16155][xPoint_16155 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16155, yPoint_16155, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16155 + i][xPoint_16155] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16155, yPoint_16155){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16155][xPoint_16155] === this.CELL_EMPTY
                && map[yPoint_16155-1][xPoint_16155] === this.CELL_EMPTY
                && map[yPoint_16155-1][xPoint_16155+1] === this.CELL_EMPTY
                && map[yPoint_16155][xPoint_16155+1] === this.CELL_EMPTY
                && map[yPoint_16155+1][xPoint_16155+1] === this.CELL_EMPTY
                && map[yPoint_16155+1][xPoint_16155] === this.CELL_EMPTY
                && map[yPoint_16155+1][xPoint_16155-1] === this.CELL_EMPTY
                && map[yPoint_16155][xPoint_16155-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16155, yPoint_16155, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16155;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16155][x] === this.CELL_EMPTY
                    && map[yPoint_16155-1][x] === this.CELL_EMPTY
                    && map[yPoint_16155-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16155][x+1] === this.CELL_EMPTY
                    && map[yPoint_16155+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16155+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16155, yPoint_16155, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16155;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16155] === this.CELL_EMPTY
                    && map[y+1][xPoint_16155] === this.CELL_EMPTY
                    && map[y+1][xPoint_16155+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16155] === this.CELL_EMPTY
                    && map[y][xPoint_16155-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16155-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
