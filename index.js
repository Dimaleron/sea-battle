...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9807=0;yPoint_9807<this.gameFieldBorderY.length; yPoint_9807++){
                for(var xPoint_9807=0;xPoint_9807<this.gameFieldBorderX.length; xPoint_9807++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9807, xPoint_9807);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9807][xPoint_9807] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9807, xPoint_9807, 'user');
                    if(this._userShipsMap[yPoint_9807][xPoint_9807] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9807, xPoint_9807, type){
            var id = this.getPointBlockIdByCoords(yPoint_9807, xPoint_9807, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9807);
                block.setAttribute('data-y', yPoint_9807);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9807
         * @param {type} xPoint_9807
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9807, xPoint_9807, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9807 + '_y' + yPoint_9807;
            }
            return 'pc_x' + xPoint_9807 + '_y' + yPoint_9807;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9807=0;yPoint_9807<this.gameFieldBorderY.length; yPoint_9807++){
                for(var xPoint_9807=0;xPoint_9807<this.gameFieldBorderX.length; xPoint_9807++){
                    map.push({y: yPoint_9807, x: xPoint_9807});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9807=-1;yPoint_9807<(this.gameFieldBorderY.length+1); yPoint_9807++){
                for(var xPoint_9807=-1;xPoint_9807<(this.gameFieldBorderX.length+1); xPoint_9807++){
                    if(!map[yPoint_9807]){
                        map[yPoint_9807] = [];
                    }
                    map[yPoint_9807][xPoint_9807] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9807 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9807 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9807, yPoint_9807) === true){
                    if(this.canPutHorizontal(map, xPoint_9807, yPoint_9807, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9807][xPoint_9807 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9807, yPoint_9807, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9807 + i][xPoint_9807] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9807, yPoint_9807){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9807][xPoint_9807] === this.CELL_EMPTY
                && map[yPoint_9807-1][xPoint_9807] === this.CELL_EMPTY
                && map[yPoint_9807-1][xPoint_9807+1] === this.CELL_EMPTY
                && map[yPoint_9807][xPoint_9807+1] === this.CELL_EMPTY
                && map[yPoint_9807+1][xPoint_9807+1] === this.CELL_EMPTY
                && map[yPoint_9807+1][xPoint_9807] === this.CELL_EMPTY
                && map[yPoint_9807+1][xPoint_9807-1] === this.CELL_EMPTY
                && map[yPoint_9807][xPoint_9807-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9807, yPoint_9807, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9807;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9807][x] === this.CELL_EMPTY
                    && map[yPoint_9807-1][x] === this.CELL_EMPTY
                    && map[yPoint_9807-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9807][x+1] === this.CELL_EMPTY
                    && map[yPoint_9807+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9807+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9807, yPoint_9807, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9807;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9807] === this.CELL_EMPTY
                    && map[y+1][xPoint_9807] === this.CELL_EMPTY
                    && map[y+1][xPoint_9807+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9807] === this.CELL_EMPTY
                    && map[y][xPoint_9807-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9807-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
