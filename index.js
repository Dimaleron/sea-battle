...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15395=0;yPoint_15395<this.gameFieldBorderY.length; yPoint_15395++){
                for(var xPoint_15395=0;xPoint_15395<this.gameFieldBorderX.length; xPoint_15395++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15395, xPoint_15395);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15395][xPoint_15395] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15395, xPoint_15395, 'user');
                    if(this._userShipsMap[yPoint_15395][xPoint_15395] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15395, xPoint_15395, type){
            var id = this.getPointBlockIdByCoords(yPoint_15395, xPoint_15395, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15395);
                block.setAttribute('data-y', yPoint_15395);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15395
         * @param {type} xPoint_15395
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15395, xPoint_15395, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15395 + '_y' + yPoint_15395;
            }
            return 'pc_x' + xPoint_15395 + '_y' + yPoint_15395;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15395=0;yPoint_15395<this.gameFieldBorderY.length; yPoint_15395++){
                for(var xPoint_15395=0;xPoint_15395<this.gameFieldBorderX.length; xPoint_15395++){
                    map.push({y: yPoint_15395, x: xPoint_15395});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15395=-1;yPoint_15395<(this.gameFieldBorderY.length+1); yPoint_15395++){
                for(var xPoint_15395=-1;xPoint_15395<(this.gameFieldBorderX.length+1); xPoint_15395++){
                    if(!map[yPoint_15395]){
                        map[yPoint_15395] = [];
                    }
                    map[yPoint_15395][xPoint_15395] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15395 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15395 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15395, yPoint_15395) === true){
                    if(this.canPutHorizontal(map, xPoint_15395, yPoint_15395, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15395][xPoint_15395 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15395, yPoint_15395, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15395 + i][xPoint_15395] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15395, yPoint_15395){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15395][xPoint_15395] === this.CELL_EMPTY
                && map[yPoint_15395-1][xPoint_15395] === this.CELL_EMPTY
                && map[yPoint_15395-1][xPoint_15395+1] === this.CELL_EMPTY
                && map[yPoint_15395][xPoint_15395+1] === this.CELL_EMPTY
                && map[yPoint_15395+1][xPoint_15395+1] === this.CELL_EMPTY
                && map[yPoint_15395+1][xPoint_15395] === this.CELL_EMPTY
                && map[yPoint_15395+1][xPoint_15395-1] === this.CELL_EMPTY
                && map[yPoint_15395][xPoint_15395-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15395, yPoint_15395, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15395;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15395][x] === this.CELL_EMPTY
                    && map[yPoint_15395-1][x] === this.CELL_EMPTY
                    && map[yPoint_15395-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15395][x+1] === this.CELL_EMPTY
                    && map[yPoint_15395+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15395+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15395, yPoint_15395, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15395;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15395] === this.CELL_EMPTY
                    && map[y+1][xPoint_15395] === this.CELL_EMPTY
                    && map[y+1][xPoint_15395+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15395] === this.CELL_EMPTY
                    && map[y][xPoint_15395-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15395-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
