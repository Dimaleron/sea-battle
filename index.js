...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1171=0;yPoint_1171<this.gameFieldBorderY.length; yPoint_1171++){
                for(var xPoint_1171=0;xPoint_1171<this.gameFieldBorderX.length; xPoint_1171++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1171, xPoint_1171);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1171][xPoint_1171] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1171, xPoint_1171, 'user');
                    if(this._userShipsMap[yPoint_1171][xPoint_1171] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1171, xPoint_1171, type){
            var id = this.getPointBlockIdByCoords(yPoint_1171, xPoint_1171, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1171);
                block.setAttribute('data-y', yPoint_1171);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1171
         * @param {type} xPoint_1171
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1171, xPoint_1171, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1171 + '_y' + yPoint_1171;
            }
            return 'pc_x' + xPoint_1171 + '_y' + yPoint_1171;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1171=0;yPoint_1171<this.gameFieldBorderY.length; yPoint_1171++){
                for(var xPoint_1171=0;xPoint_1171<this.gameFieldBorderX.length; xPoint_1171++){
                    map.push({y: yPoint_1171, x: xPoint_1171});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1171=-1;yPoint_1171<(this.gameFieldBorderY.length+1); yPoint_1171++){
                for(var xPoint_1171=-1;xPoint_1171<(this.gameFieldBorderX.length+1); xPoint_1171++){
                    if(!map[yPoint_1171]){
                        map[yPoint_1171] = [];
                    }
                    map[yPoint_1171][xPoint_1171] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1171 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1171 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1171, yPoint_1171) === true){
                    if(this.canPutHorizontal(map, xPoint_1171, yPoint_1171, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1171][xPoint_1171 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1171, yPoint_1171, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1171 + i][xPoint_1171] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1171, yPoint_1171){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1171][xPoint_1171] === this.CELL_EMPTY
                && map[yPoint_1171-1][xPoint_1171] === this.CELL_EMPTY
                && map[yPoint_1171-1][xPoint_1171+1] === this.CELL_EMPTY
                && map[yPoint_1171][xPoint_1171+1] === this.CELL_EMPTY
                && map[yPoint_1171+1][xPoint_1171+1] === this.CELL_EMPTY
                && map[yPoint_1171+1][xPoint_1171] === this.CELL_EMPTY
                && map[yPoint_1171+1][xPoint_1171-1] === this.CELL_EMPTY
                && map[yPoint_1171][xPoint_1171-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1171, yPoint_1171, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1171;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1171][x] === this.CELL_EMPTY
                    && map[yPoint_1171-1][x] === this.CELL_EMPTY
                    && map[yPoint_1171-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1171][x+1] === this.CELL_EMPTY
                    && map[yPoint_1171+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1171+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1171, yPoint_1171, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1171;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1171] === this.CELL_EMPTY
                    && map[y+1][xPoint_1171] === this.CELL_EMPTY
                    && map[y+1][xPoint_1171+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1171] === this.CELL_EMPTY
                    && map[y][xPoint_1171-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1171-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
