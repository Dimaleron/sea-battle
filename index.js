...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31353=0;yPoint_31353<this.gameFieldBorderY.length; yPoint_31353++){
                for(var xPoint_31353=0;xPoint_31353<this.gameFieldBorderX.length; xPoint_31353++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31353, xPoint_31353);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31353][xPoint_31353] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31353, xPoint_31353, 'user');
                    if(this._userShipsMap[yPoint_31353][xPoint_31353] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31353, xPoint_31353, type){
            var id = this.getPointBlockIdByCoords(yPoint_31353, xPoint_31353, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31353);
                block.setAttribute('data-y', yPoint_31353);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31353
         * @param {type} xPoint_31353
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31353, xPoint_31353, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31353 + '_y' + yPoint_31353;
            }
            return 'pc_x' + xPoint_31353 + '_y' + yPoint_31353;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31353=0;yPoint_31353<this.gameFieldBorderY.length; yPoint_31353++){
                for(var xPoint_31353=0;xPoint_31353<this.gameFieldBorderX.length; xPoint_31353++){
                    map.push({y: yPoint_31353, x: xPoint_31353});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31353=-1;yPoint_31353<(this.gameFieldBorderY.length+1); yPoint_31353++){
                for(var xPoint_31353=-1;xPoint_31353<(this.gameFieldBorderX.length+1); xPoint_31353++){
                    if(!map[yPoint_31353]){
                        map[yPoint_31353] = [];
                    }
                    map[yPoint_31353][xPoint_31353] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31353 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31353 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31353, yPoint_31353) === true){
                    if(this.canPutHorizontal(map, xPoint_31353, yPoint_31353, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31353][xPoint_31353 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31353, yPoint_31353, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31353 + i][xPoint_31353] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31353, yPoint_31353){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31353][xPoint_31353] === this.CELL_EMPTY
                && map[yPoint_31353-1][xPoint_31353] === this.CELL_EMPTY
                && map[yPoint_31353-1][xPoint_31353+1] === this.CELL_EMPTY
                && map[yPoint_31353][xPoint_31353+1] === this.CELL_EMPTY
                && map[yPoint_31353+1][xPoint_31353+1] === this.CELL_EMPTY
                && map[yPoint_31353+1][xPoint_31353] === this.CELL_EMPTY
                && map[yPoint_31353+1][xPoint_31353-1] === this.CELL_EMPTY
                && map[yPoint_31353][xPoint_31353-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31353, yPoint_31353, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31353;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31353][x] === this.CELL_EMPTY
                    && map[yPoint_31353-1][x] === this.CELL_EMPTY
                    && map[yPoint_31353-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31353][x+1] === this.CELL_EMPTY
                    && map[yPoint_31353+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31353+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31353, yPoint_31353, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31353;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31353] === this.CELL_EMPTY
                    && map[y+1][xPoint_31353] === this.CELL_EMPTY
                    && map[y+1][xPoint_31353+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31353] === this.CELL_EMPTY
                    && map[y][xPoint_31353-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31353-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
