...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6584=0;yPoint_6584<this.gameFieldBorderY.length; yPoint_6584++){
                for(var xPoint_6584=0;xPoint_6584<this.gameFieldBorderX.length; xPoint_6584++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6584, xPoint_6584);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6584][xPoint_6584] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6584, xPoint_6584, 'user');
                    if(this._userShipsMap[yPoint_6584][xPoint_6584] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6584, xPoint_6584, type){
            var id = this.getPointBlockIdByCoords(yPoint_6584, xPoint_6584, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6584);
                block.setAttribute('data-y', yPoint_6584);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6584
         * @param {type} xPoint_6584
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6584, xPoint_6584, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6584 + '_y' + yPoint_6584;
            }
            return 'pc_x' + xPoint_6584 + '_y' + yPoint_6584;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6584=0;yPoint_6584<this.gameFieldBorderY.length; yPoint_6584++){
                for(var xPoint_6584=0;xPoint_6584<this.gameFieldBorderX.length; xPoint_6584++){
                    map.push({y: yPoint_6584, x: xPoint_6584});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6584=-1;yPoint_6584<(this.gameFieldBorderY.length+1); yPoint_6584++){
                for(var xPoint_6584=-1;xPoint_6584<(this.gameFieldBorderX.length+1); xPoint_6584++){
                    if(!map[yPoint_6584]){
                        map[yPoint_6584] = [];
                    }
                    map[yPoint_6584][xPoint_6584] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6584 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6584 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6584, yPoint_6584) === true){
                    if(this.canPutHorizontal(map, xPoint_6584, yPoint_6584, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6584][xPoint_6584 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6584, yPoint_6584, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6584 + i][xPoint_6584] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6584, yPoint_6584){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6584][xPoint_6584] === this.CELL_EMPTY
                && map[yPoint_6584-1][xPoint_6584] === this.CELL_EMPTY
                && map[yPoint_6584-1][xPoint_6584+1] === this.CELL_EMPTY
                && map[yPoint_6584][xPoint_6584+1] === this.CELL_EMPTY
                && map[yPoint_6584+1][xPoint_6584+1] === this.CELL_EMPTY
                && map[yPoint_6584+1][xPoint_6584] === this.CELL_EMPTY
                && map[yPoint_6584+1][xPoint_6584-1] === this.CELL_EMPTY
                && map[yPoint_6584][xPoint_6584-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6584, yPoint_6584, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6584;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6584][x] === this.CELL_EMPTY
                    && map[yPoint_6584-1][x] === this.CELL_EMPTY
                    && map[yPoint_6584-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6584][x+1] === this.CELL_EMPTY
                    && map[yPoint_6584+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6584+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6584, yPoint_6584, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6584;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6584] === this.CELL_EMPTY
                    && map[y+1][xPoint_6584] === this.CELL_EMPTY
                    && map[y+1][xPoint_6584+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6584] === this.CELL_EMPTY
                    && map[y][xPoint_6584-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6584-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
