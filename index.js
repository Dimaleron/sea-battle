...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_8347=0;yPoint_8347<this.gameFieldBorderY.length; yPoint_8347++){
                for(var xPoint_8347=0;xPoint_8347<this.gameFieldBorderX.length; xPoint_8347++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_8347, xPoint_8347);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_8347][xPoint_8347] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_8347, xPoint_8347, 'user');
                    if(this._userShipsMap[yPoint_8347][xPoint_8347] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_8347, xPoint_8347, type){
            var id = this.getPointBlockIdByCoords(yPoint_8347, xPoint_8347, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_8347);
                block.setAttribute('data-y', yPoint_8347);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_8347
         * @param {type} xPoint_8347
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_8347, xPoint_8347, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_8347 + '_y' + yPoint_8347;
            }
            return 'pc_x' + xPoint_8347 + '_y' + yPoint_8347;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_8347=0;yPoint_8347<this.gameFieldBorderY.length; yPoint_8347++){
                for(var xPoint_8347=0;xPoint_8347<this.gameFieldBorderX.length; xPoint_8347++){
                    map.push({y: yPoint_8347, x: xPoint_8347});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_8347=-1;yPoint_8347<(this.gameFieldBorderY.length+1); yPoint_8347++){
                for(var xPoint_8347=-1;xPoint_8347<(this.gameFieldBorderX.length+1); xPoint_8347++){
                    if(!map[yPoint_8347]){
                        map[yPoint_8347] = [];
                    }
                    map[yPoint_8347][xPoint_8347] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_8347 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_8347 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_8347, yPoint_8347) === true){
                    if(this.canPutHorizontal(map, xPoint_8347, yPoint_8347, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8347][xPoint_8347 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_8347, yPoint_8347, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_8347 + i][xPoint_8347] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_8347, yPoint_8347){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_8347][xPoint_8347] === this.CELL_EMPTY
                && map[yPoint_8347-1][xPoint_8347] === this.CELL_EMPTY
                && map[yPoint_8347-1][xPoint_8347+1] === this.CELL_EMPTY
                && map[yPoint_8347][xPoint_8347+1] === this.CELL_EMPTY
                && map[yPoint_8347+1][xPoint_8347+1] === this.CELL_EMPTY
                && map[yPoint_8347+1][xPoint_8347] === this.CELL_EMPTY
                && map[yPoint_8347+1][xPoint_8347-1] === this.CELL_EMPTY
                && map[yPoint_8347][xPoint_8347-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_8347, yPoint_8347, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_8347;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_8347][x] === this.CELL_EMPTY
                    && map[yPoint_8347-1][x] === this.CELL_EMPTY
                    && map[yPoint_8347-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8347][x+1] === this.CELL_EMPTY
                    && map[yPoint_8347+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_8347+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_8347, yPoint_8347, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_8347;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_8347] === this.CELL_EMPTY
                    && map[y+1][xPoint_8347] === this.CELL_EMPTY
                    && map[y+1][xPoint_8347+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_8347] === this.CELL_EMPTY
                    && map[y][xPoint_8347-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_8347-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
