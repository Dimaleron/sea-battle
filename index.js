...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28689=0;yPoint_28689<this.gameFieldBorderY.length; yPoint_28689++){
                for(var xPoint_28689=0;xPoint_28689<this.gameFieldBorderX.length; xPoint_28689++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28689, xPoint_28689);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28689][xPoint_28689] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28689, xPoint_28689, 'user');
                    if(this._userShipsMap[yPoint_28689][xPoint_28689] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28689, xPoint_28689, type){
            var id = this.getPointBlockIdByCoords(yPoint_28689, xPoint_28689, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28689);
                block.setAttribute('data-y', yPoint_28689);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28689
         * @param {type} xPoint_28689
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28689, xPoint_28689, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28689 + '_y' + yPoint_28689;
            }
            return 'pc_x' + xPoint_28689 + '_y' + yPoint_28689;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28689=0;yPoint_28689<this.gameFieldBorderY.length; yPoint_28689++){
                for(var xPoint_28689=0;xPoint_28689<this.gameFieldBorderX.length; xPoint_28689++){
                    map.push({y: yPoint_28689, x: xPoint_28689});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28689=-1;yPoint_28689<(this.gameFieldBorderY.length+1); yPoint_28689++){
                for(var xPoint_28689=-1;xPoint_28689<(this.gameFieldBorderX.length+1); xPoint_28689++){
                    if(!map[yPoint_28689]){
                        map[yPoint_28689] = [];
                    }
                    map[yPoint_28689][xPoint_28689] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28689 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28689 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28689, yPoint_28689) === true){
                    if(this.canPutHorizontal(map, xPoint_28689, yPoint_28689, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28689][xPoint_28689 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28689, yPoint_28689, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28689 + i][xPoint_28689] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28689, yPoint_28689){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28689][xPoint_28689] === this.CELL_EMPTY
                && map[yPoint_28689-1][xPoint_28689] === this.CELL_EMPTY
                && map[yPoint_28689-1][xPoint_28689+1] === this.CELL_EMPTY
                && map[yPoint_28689][xPoint_28689+1] === this.CELL_EMPTY
                && map[yPoint_28689+1][xPoint_28689+1] === this.CELL_EMPTY
                && map[yPoint_28689+1][xPoint_28689] === this.CELL_EMPTY
                && map[yPoint_28689+1][xPoint_28689-1] === this.CELL_EMPTY
                && map[yPoint_28689][xPoint_28689-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28689, yPoint_28689, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28689;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28689][x] === this.CELL_EMPTY
                    && map[yPoint_28689-1][x] === this.CELL_EMPTY
                    && map[yPoint_28689-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28689][x+1] === this.CELL_EMPTY
                    && map[yPoint_28689+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28689+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28689, yPoint_28689, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28689;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28689] === this.CELL_EMPTY
                    && map[y+1][xPoint_28689] === this.CELL_EMPTY
                    && map[y+1][xPoint_28689+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28689] === this.CELL_EMPTY
                    && map[y][xPoint_28689-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28689-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
