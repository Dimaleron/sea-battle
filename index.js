...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16038=0;yPoint_16038<this.gameFieldBorderY.length; yPoint_16038++){
                for(var xPoint_16038=0;xPoint_16038<this.gameFieldBorderX.length; xPoint_16038++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16038, xPoint_16038);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16038][xPoint_16038] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16038, xPoint_16038, 'user');
                    if(this._userShipsMap[yPoint_16038][xPoint_16038] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16038, xPoint_16038, type){
            var id = this.getPointBlockIdByCoords(yPoint_16038, xPoint_16038, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16038);
                block.setAttribute('data-y', yPoint_16038);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16038
         * @param {type} xPoint_16038
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16038, xPoint_16038, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16038 + '_y' + yPoint_16038;
            }
            return 'pc_x' + xPoint_16038 + '_y' + yPoint_16038;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16038=0;yPoint_16038<this.gameFieldBorderY.length; yPoint_16038++){
                for(var xPoint_16038=0;xPoint_16038<this.gameFieldBorderX.length; xPoint_16038++){
                    map.push({y: yPoint_16038, x: xPoint_16038});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16038=-1;yPoint_16038<(this.gameFieldBorderY.length+1); yPoint_16038++){
                for(var xPoint_16038=-1;xPoint_16038<(this.gameFieldBorderX.length+1); xPoint_16038++){
                    if(!map[yPoint_16038]){
                        map[yPoint_16038] = [];
                    }
                    map[yPoint_16038][xPoint_16038] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16038 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16038 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16038, yPoint_16038) === true){
                    if(this.canPutHorizontal(map, xPoint_16038, yPoint_16038, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16038][xPoint_16038 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16038, yPoint_16038, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16038 + i][xPoint_16038] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16038, yPoint_16038){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16038][xPoint_16038] === this.CELL_EMPTY
                && map[yPoint_16038-1][xPoint_16038] === this.CELL_EMPTY
                && map[yPoint_16038-1][xPoint_16038+1] === this.CELL_EMPTY
                && map[yPoint_16038][xPoint_16038+1] === this.CELL_EMPTY
                && map[yPoint_16038+1][xPoint_16038+1] === this.CELL_EMPTY
                && map[yPoint_16038+1][xPoint_16038] === this.CELL_EMPTY
                && map[yPoint_16038+1][xPoint_16038-1] === this.CELL_EMPTY
                && map[yPoint_16038][xPoint_16038-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16038, yPoint_16038, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16038;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16038][x] === this.CELL_EMPTY
                    && map[yPoint_16038-1][x] === this.CELL_EMPTY
                    && map[yPoint_16038-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16038][x+1] === this.CELL_EMPTY
                    && map[yPoint_16038+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16038+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16038, yPoint_16038, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16038;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16038] === this.CELL_EMPTY
                    && map[y+1][xPoint_16038] === this.CELL_EMPTY
                    && map[y+1][xPoint_16038+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16038] === this.CELL_EMPTY
                    && map[y][xPoint_16038-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16038-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
