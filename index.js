...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_427=0;yPoint_427<this.gameFieldBorderY.length; yPoint_427++){
                for(var xPoint_427=0;xPoint_427<this.gameFieldBorderX.length; xPoint_427++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_427, xPoint_427);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_427][xPoint_427] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_427, xPoint_427, 'user');
                    if(this._userShipsMap[yPoint_427][xPoint_427] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_427, xPoint_427, type){
            var id = this.getPointBlockIdByCoords(yPoint_427, xPoint_427, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_427);
                block.setAttribute('data-y', yPoint_427);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_427
         * @param {type} xPoint_427
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_427, xPoint_427, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_427 + '_y' + yPoint_427;
            }
            return 'pc_x' + xPoint_427 + '_y' + yPoint_427;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_427=0;yPoint_427<this.gameFieldBorderY.length; yPoint_427++){
                for(var xPoint_427=0;xPoint_427<this.gameFieldBorderX.length; xPoint_427++){
                    map.push({y: yPoint_427, x: xPoint_427});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_427=-1;yPoint_427<(this.gameFieldBorderY.length+1); yPoint_427++){
                for(var xPoint_427=-1;xPoint_427<(this.gameFieldBorderX.length+1); xPoint_427++){
                    if(!map[yPoint_427]){
                        map[yPoint_427] = [];
                    }
                    map[yPoint_427][xPoint_427] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_427 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_427 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_427, yPoint_427) === true){
                    if(this.canPutHorizontal(map, xPoint_427, yPoint_427, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_427][xPoint_427 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_427, yPoint_427, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_427 + i][xPoint_427] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_427, yPoint_427){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_427][xPoint_427] === this.CELL_EMPTY
                && map[yPoint_427-1][xPoint_427] === this.CELL_EMPTY
                && map[yPoint_427-1][xPoint_427+1] === this.CELL_EMPTY
                && map[yPoint_427][xPoint_427+1] === this.CELL_EMPTY
                && map[yPoint_427+1][xPoint_427+1] === this.CELL_EMPTY
                && map[yPoint_427+1][xPoint_427] === this.CELL_EMPTY
                && map[yPoint_427+1][xPoint_427-1] === this.CELL_EMPTY
                && map[yPoint_427][xPoint_427-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_427, yPoint_427, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_427;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_427][x] === this.CELL_EMPTY
                    && map[yPoint_427-1][x] === this.CELL_EMPTY
                    && map[yPoint_427-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_427][x+1] === this.CELL_EMPTY
                    && map[yPoint_427+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_427+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_427, yPoint_427, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_427;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_427] === this.CELL_EMPTY
                    && map[y+1][xPoint_427] === this.CELL_EMPTY
                    && map[y+1][xPoint_427+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_427] === this.CELL_EMPTY
                    && map[y][xPoint_427-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_427-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
