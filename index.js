...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_512=0;yPoint_512<this.gameFieldBorderY.length; yPoint_512++){
                for(var xPoint_512=0;xPoint_512<this.gameFieldBorderX.length; xPoint_512++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_512, xPoint_512);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_512][xPoint_512] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_512, xPoint_512, 'user');
                    if(this._userShipsMap[yPoint_512][xPoint_512] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_512, xPoint_512, type){
            var id = this.getPointBlockIdByCoords(yPoint_512, xPoint_512, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_512);
                block.setAttribute('data-y', yPoint_512);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_512
         * @param {type} xPoint_512
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_512, xPoint_512, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_512 + '_y' + yPoint_512;
            }
            return 'pc_x' + xPoint_512 + '_y' + yPoint_512;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_512=0;yPoint_512<this.gameFieldBorderY.length; yPoint_512++){
                for(var xPoint_512=0;xPoint_512<this.gameFieldBorderX.length; xPoint_512++){
                    map.push({y: yPoint_512, x: xPoint_512});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_512=-1;yPoint_512<(this.gameFieldBorderY.length+1); yPoint_512++){
                for(var xPoint_512=-1;xPoint_512<(this.gameFieldBorderX.length+1); xPoint_512++){
                    if(!map[yPoint_512]){
                        map[yPoint_512] = [];
                    }
                    map[yPoint_512][xPoint_512] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_512 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_512 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_512, yPoint_512) === true){
                    if(this.canPutHorizontal(map, xPoint_512, yPoint_512, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_512][xPoint_512 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_512, yPoint_512, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_512 + i][xPoint_512] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_512, yPoint_512){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_512][xPoint_512] === this.CELL_EMPTY
                && map[yPoint_512-1][xPoint_512] === this.CELL_EMPTY
                && map[yPoint_512-1][xPoint_512+1] === this.CELL_EMPTY
                && map[yPoint_512][xPoint_512+1] === this.CELL_EMPTY
                && map[yPoint_512+1][xPoint_512+1] === this.CELL_EMPTY
                && map[yPoint_512+1][xPoint_512] === this.CELL_EMPTY
                && map[yPoint_512+1][xPoint_512-1] === this.CELL_EMPTY
                && map[yPoint_512][xPoint_512-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_512, yPoint_512, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_512;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_512][x] === this.CELL_EMPTY
                    && map[yPoint_512-1][x] === this.CELL_EMPTY
                    && map[yPoint_512-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_512][x+1] === this.CELL_EMPTY
                    && map[yPoint_512+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_512+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_512, yPoint_512, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_512;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_512] === this.CELL_EMPTY
                    && map[y+1][xPoint_512] === this.CELL_EMPTY
                    && map[y+1][xPoint_512+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_512] === this.CELL_EMPTY
                    && map[y][xPoint_512-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_512-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
