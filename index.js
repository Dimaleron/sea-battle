...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10877=0;yPoint_10877<this.gameFieldBorderY.length; yPoint_10877++){
                for(var xPoint_10877=0;xPoint_10877<this.gameFieldBorderX.length; xPoint_10877++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10877, xPoint_10877);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10877][xPoint_10877] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10877, xPoint_10877, 'user');
                    if(this._userShipsMap[yPoint_10877][xPoint_10877] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10877, xPoint_10877, type){
            var id = this.getPointBlockIdByCoords(yPoint_10877, xPoint_10877, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10877);
                block.setAttribute('data-y', yPoint_10877);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10877
         * @param {type} xPoint_10877
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10877, xPoint_10877, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10877 + '_y' + yPoint_10877;
            }
            return 'pc_x' + xPoint_10877 + '_y' + yPoint_10877;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10877=0;yPoint_10877<this.gameFieldBorderY.length; yPoint_10877++){
                for(var xPoint_10877=0;xPoint_10877<this.gameFieldBorderX.length; xPoint_10877++){
                    map.push({y: yPoint_10877, x: xPoint_10877});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10877=-1;yPoint_10877<(this.gameFieldBorderY.length+1); yPoint_10877++){
                for(var xPoint_10877=-1;xPoint_10877<(this.gameFieldBorderX.length+1); xPoint_10877++){
                    if(!map[yPoint_10877]){
                        map[yPoint_10877] = [];
                    }
                    map[yPoint_10877][xPoint_10877] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10877 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10877 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10877, yPoint_10877) === true){
                    if(this.canPutHorizontal(map, xPoint_10877, yPoint_10877, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10877][xPoint_10877 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10877, yPoint_10877, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10877 + i][xPoint_10877] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10877, yPoint_10877){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10877][xPoint_10877] === this.CELL_EMPTY
                && map[yPoint_10877-1][xPoint_10877] === this.CELL_EMPTY
                && map[yPoint_10877-1][xPoint_10877+1] === this.CELL_EMPTY
                && map[yPoint_10877][xPoint_10877+1] === this.CELL_EMPTY
                && map[yPoint_10877+1][xPoint_10877+1] === this.CELL_EMPTY
                && map[yPoint_10877+1][xPoint_10877] === this.CELL_EMPTY
                && map[yPoint_10877+1][xPoint_10877-1] === this.CELL_EMPTY
                && map[yPoint_10877][xPoint_10877-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10877, yPoint_10877, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10877;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10877][x] === this.CELL_EMPTY
                    && map[yPoint_10877-1][x] === this.CELL_EMPTY
                    && map[yPoint_10877-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10877][x+1] === this.CELL_EMPTY
                    && map[yPoint_10877+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10877+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10877, yPoint_10877, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10877;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10877] === this.CELL_EMPTY
                    && map[y+1][xPoint_10877] === this.CELL_EMPTY
                    && map[y+1][xPoint_10877+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10877] === this.CELL_EMPTY
                    && map[y][xPoint_10877-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10877-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
