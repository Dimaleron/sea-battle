...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_943=0;yPoint_943<this.gameFieldBorderY.length; yPoint_943++){
                for(var xPoint_943=0;xPoint_943<this.gameFieldBorderX.length; xPoint_943++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_943, xPoint_943);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_943][xPoint_943] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_943, xPoint_943, 'user');
                    if(this._userShipsMap[yPoint_943][xPoint_943] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_943, xPoint_943, type){
            var id = this.getPointBlockIdByCoords(yPoint_943, xPoint_943, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_943);
                block.setAttribute('data-y', yPoint_943);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_943
         * @param {type} xPoint_943
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_943, xPoint_943, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_943 + '_y' + yPoint_943;
            }
            return 'pc_x' + xPoint_943 + '_y' + yPoint_943;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_943=0;yPoint_943<this.gameFieldBorderY.length; yPoint_943++){
                for(var xPoint_943=0;xPoint_943<this.gameFieldBorderX.length; xPoint_943++){
                    map.push({y: yPoint_943, x: xPoint_943});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_943=-1;yPoint_943<(this.gameFieldBorderY.length+1); yPoint_943++){
                for(var xPoint_943=-1;xPoint_943<(this.gameFieldBorderX.length+1); xPoint_943++){
                    if(!map[yPoint_943]){
                        map[yPoint_943] = [];
                    }
                    map[yPoint_943][xPoint_943] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_943 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_943 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_943, yPoint_943) === true){
                    if(this.canPutHorizontal(map, xPoint_943, yPoint_943, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_943][xPoint_943 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_943, yPoint_943, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_943 + i][xPoint_943] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_943, yPoint_943){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_943][xPoint_943] === this.CELL_EMPTY
                && map[yPoint_943-1][xPoint_943] === this.CELL_EMPTY
                && map[yPoint_943-1][xPoint_943+1] === this.CELL_EMPTY
                && map[yPoint_943][xPoint_943+1] === this.CELL_EMPTY
                && map[yPoint_943+1][xPoint_943+1] === this.CELL_EMPTY
                && map[yPoint_943+1][xPoint_943] === this.CELL_EMPTY
                && map[yPoint_943+1][xPoint_943-1] === this.CELL_EMPTY
                && map[yPoint_943][xPoint_943-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_943, yPoint_943, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_943;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_943][x] === this.CELL_EMPTY
                    && map[yPoint_943-1][x] === this.CELL_EMPTY
                    && map[yPoint_943-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_943][x+1] === this.CELL_EMPTY
                    && map[yPoint_943+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_943+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_943, yPoint_943, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_943;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_943] === this.CELL_EMPTY
                    && map[y+1][xPoint_943] === this.CELL_EMPTY
                    && map[y+1][xPoint_943+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_943] === this.CELL_EMPTY
                    && map[y][xPoint_943-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_943-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
