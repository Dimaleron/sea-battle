...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16287=0;yPoint_16287<this.gameFieldBorderY.length; yPoint_16287++){
                for(var xPoint_16287=0;xPoint_16287<this.gameFieldBorderX.length; xPoint_16287++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16287, xPoint_16287);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16287][xPoint_16287] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16287, xPoint_16287, 'user');
                    if(this._userShipsMap[yPoint_16287][xPoint_16287] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16287, xPoint_16287, type){
            var id = this.getPointBlockIdByCoords(yPoint_16287, xPoint_16287, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16287);
                block.setAttribute('data-y', yPoint_16287);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16287
         * @param {type} xPoint_16287
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16287, xPoint_16287, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16287 + '_y' + yPoint_16287;
            }
            return 'pc_x' + xPoint_16287 + '_y' + yPoint_16287;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16287=0;yPoint_16287<this.gameFieldBorderY.length; yPoint_16287++){
                for(var xPoint_16287=0;xPoint_16287<this.gameFieldBorderX.length; xPoint_16287++){
                    map.push({y: yPoint_16287, x: xPoint_16287});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16287=-1;yPoint_16287<(this.gameFieldBorderY.length+1); yPoint_16287++){
                for(var xPoint_16287=-1;xPoint_16287<(this.gameFieldBorderX.length+1); xPoint_16287++){
                    if(!map[yPoint_16287]){
                        map[yPoint_16287] = [];
                    }
                    map[yPoint_16287][xPoint_16287] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16287 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16287 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16287, yPoint_16287) === true){
                    if(this.canPutHorizontal(map, xPoint_16287, yPoint_16287, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16287][xPoint_16287 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16287, yPoint_16287, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16287 + i][xPoint_16287] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16287, yPoint_16287){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16287][xPoint_16287] === this.CELL_EMPTY
                && map[yPoint_16287-1][xPoint_16287] === this.CELL_EMPTY
                && map[yPoint_16287-1][xPoint_16287+1] === this.CELL_EMPTY
                && map[yPoint_16287][xPoint_16287+1] === this.CELL_EMPTY
                && map[yPoint_16287+1][xPoint_16287+1] === this.CELL_EMPTY
                && map[yPoint_16287+1][xPoint_16287] === this.CELL_EMPTY
                && map[yPoint_16287+1][xPoint_16287-1] === this.CELL_EMPTY
                && map[yPoint_16287][xPoint_16287-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16287, yPoint_16287, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16287;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16287][x] === this.CELL_EMPTY
                    && map[yPoint_16287-1][x] === this.CELL_EMPTY
                    && map[yPoint_16287-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16287][x+1] === this.CELL_EMPTY
                    && map[yPoint_16287+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16287+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16287, yPoint_16287, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16287;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16287] === this.CELL_EMPTY
                    && map[y+1][xPoint_16287] === this.CELL_EMPTY
                    && map[y+1][xPoint_16287+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16287] === this.CELL_EMPTY
                    && map[y][xPoint_16287-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16287-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
