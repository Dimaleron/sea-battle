...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19954=0;yPoint_19954<this.gameFieldBorderY.length; yPoint_19954++){
                for(var xPoint_19954=0;xPoint_19954<this.gameFieldBorderX.length; xPoint_19954++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19954, xPoint_19954);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19954][xPoint_19954] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19954, xPoint_19954, 'user');
                    if(this._userShipsMap[yPoint_19954][xPoint_19954] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19954, xPoint_19954, type){
            var id = this.getPointBlockIdByCoords(yPoint_19954, xPoint_19954, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19954);
                block.setAttribute('data-y', yPoint_19954);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19954
         * @param {type} xPoint_19954
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19954, xPoint_19954, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19954 + '_y' + yPoint_19954;
            }
            return 'pc_x' + xPoint_19954 + '_y' + yPoint_19954;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19954=0;yPoint_19954<this.gameFieldBorderY.length; yPoint_19954++){
                for(var xPoint_19954=0;xPoint_19954<this.gameFieldBorderX.length; xPoint_19954++){
                    map.push({y: yPoint_19954, x: xPoint_19954});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19954=-1;yPoint_19954<(this.gameFieldBorderY.length+1); yPoint_19954++){
                for(var xPoint_19954=-1;xPoint_19954<(this.gameFieldBorderX.length+1); xPoint_19954++){
                    if(!map[yPoint_19954]){
                        map[yPoint_19954] = [];
                    }
                    map[yPoint_19954][xPoint_19954] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19954 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19954 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19954, yPoint_19954) === true){
                    if(this.canPutHorizontal(map, xPoint_19954, yPoint_19954, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19954][xPoint_19954 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19954, yPoint_19954, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19954 + i][xPoint_19954] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19954, yPoint_19954){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19954][xPoint_19954] === this.CELL_EMPTY
                && map[yPoint_19954-1][xPoint_19954] === this.CELL_EMPTY
                && map[yPoint_19954-1][xPoint_19954+1] === this.CELL_EMPTY
                && map[yPoint_19954][xPoint_19954+1] === this.CELL_EMPTY
                && map[yPoint_19954+1][xPoint_19954+1] === this.CELL_EMPTY
                && map[yPoint_19954+1][xPoint_19954] === this.CELL_EMPTY
                && map[yPoint_19954+1][xPoint_19954-1] === this.CELL_EMPTY
                && map[yPoint_19954][xPoint_19954-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19954, yPoint_19954, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19954;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19954][x] === this.CELL_EMPTY
                    && map[yPoint_19954-1][x] === this.CELL_EMPTY
                    && map[yPoint_19954-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19954][x+1] === this.CELL_EMPTY
                    && map[yPoint_19954+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19954+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19954, yPoint_19954, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19954;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19954] === this.CELL_EMPTY
                    && map[y+1][xPoint_19954] === this.CELL_EMPTY
                    && map[y+1][xPoint_19954+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19954] === this.CELL_EMPTY
                    && map[y][xPoint_19954-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19954-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
