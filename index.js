...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7660=0;yPoint_7660<this.gameFieldBorderY.length; yPoint_7660++){
                for(var xPoint_7660=0;xPoint_7660<this.gameFieldBorderX.length; xPoint_7660++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7660, xPoint_7660);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7660][xPoint_7660] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7660, xPoint_7660, 'user');
                    if(this._userShipsMap[yPoint_7660][xPoint_7660] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7660, xPoint_7660, type){
            var id = this.getPointBlockIdByCoords(yPoint_7660, xPoint_7660, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7660);
                block.setAttribute('data-y', yPoint_7660);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7660
         * @param {type} xPoint_7660
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7660, xPoint_7660, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7660 + '_y' + yPoint_7660;
            }
            return 'pc_x' + xPoint_7660 + '_y' + yPoint_7660;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7660=0;yPoint_7660<this.gameFieldBorderY.length; yPoint_7660++){
                for(var xPoint_7660=0;xPoint_7660<this.gameFieldBorderX.length; xPoint_7660++){
                    map.push({y: yPoint_7660, x: xPoint_7660});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7660=-1;yPoint_7660<(this.gameFieldBorderY.length+1); yPoint_7660++){
                for(var xPoint_7660=-1;xPoint_7660<(this.gameFieldBorderX.length+1); xPoint_7660++){
                    if(!map[yPoint_7660]){
                        map[yPoint_7660] = [];
                    }
                    map[yPoint_7660][xPoint_7660] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7660 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7660 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7660, yPoint_7660) === true){
                    if(this.canPutHorizontal(map, xPoint_7660, yPoint_7660, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7660][xPoint_7660 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7660, yPoint_7660, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7660 + i][xPoint_7660] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7660, yPoint_7660){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7660][xPoint_7660] === this.CELL_EMPTY
                && map[yPoint_7660-1][xPoint_7660] === this.CELL_EMPTY
                && map[yPoint_7660-1][xPoint_7660+1] === this.CELL_EMPTY
                && map[yPoint_7660][xPoint_7660+1] === this.CELL_EMPTY
                && map[yPoint_7660+1][xPoint_7660+1] === this.CELL_EMPTY
                && map[yPoint_7660+1][xPoint_7660] === this.CELL_EMPTY
                && map[yPoint_7660+1][xPoint_7660-1] === this.CELL_EMPTY
                && map[yPoint_7660][xPoint_7660-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7660, yPoint_7660, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7660;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7660][x] === this.CELL_EMPTY
                    && map[yPoint_7660-1][x] === this.CELL_EMPTY
                    && map[yPoint_7660-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7660][x+1] === this.CELL_EMPTY
                    && map[yPoint_7660+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7660+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7660, yPoint_7660, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7660;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7660] === this.CELL_EMPTY
                    && map[y+1][xPoint_7660] === this.CELL_EMPTY
                    && map[y+1][xPoint_7660+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7660] === this.CELL_EMPTY
                    && map[y][xPoint_7660-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7660-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
