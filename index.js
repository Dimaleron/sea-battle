...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_626=0;yPoint_626<this.gameFieldBorderY.length; yPoint_626++){
                for(var xPoint_626=0;xPoint_626<this.gameFieldBorderX.length; xPoint_626++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_626, xPoint_626);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_626][xPoint_626] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_626, xPoint_626, 'user');
                    if(this._userShipsMap[yPoint_626][xPoint_626] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_626, xPoint_626, type){
            var id = this.getPointBlockIdByCoords(yPoint_626, xPoint_626, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_626);
                block.setAttribute('data-y', yPoint_626);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_626
         * @param {type} xPoint_626
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_626, xPoint_626, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_626 + '_y' + yPoint_626;
            }
            return 'pc_x' + xPoint_626 + '_y' + yPoint_626;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_626=0;yPoint_626<this.gameFieldBorderY.length; yPoint_626++){
                for(var xPoint_626=0;xPoint_626<this.gameFieldBorderX.length; xPoint_626++){
                    map.push({y: yPoint_626, x: xPoint_626});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_626=-1;yPoint_626<(this.gameFieldBorderY.length+1); yPoint_626++){
                for(var xPoint_626=-1;xPoint_626<(this.gameFieldBorderX.length+1); xPoint_626++){
                    if(!map[yPoint_626]){
                        map[yPoint_626] = [];
                    }
                    map[yPoint_626][xPoint_626] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_626 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_626 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_626, yPoint_626) === true){
                    if(this.canPutHorizontal(map, xPoint_626, yPoint_626, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_626][xPoint_626 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_626, yPoint_626, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_626 + i][xPoint_626] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_626, yPoint_626){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_626][xPoint_626] === this.CELL_EMPTY
                && map[yPoint_626-1][xPoint_626] === this.CELL_EMPTY
                && map[yPoint_626-1][xPoint_626+1] === this.CELL_EMPTY
                && map[yPoint_626][xPoint_626+1] === this.CELL_EMPTY
                && map[yPoint_626+1][xPoint_626+1] === this.CELL_EMPTY
                && map[yPoint_626+1][xPoint_626] === this.CELL_EMPTY
                && map[yPoint_626+1][xPoint_626-1] === this.CELL_EMPTY
                && map[yPoint_626][xPoint_626-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_626, yPoint_626, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_626;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_626][x] === this.CELL_EMPTY
                    && map[yPoint_626-1][x] === this.CELL_EMPTY
                    && map[yPoint_626-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_626][x+1] === this.CELL_EMPTY
                    && map[yPoint_626+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_626+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_626, yPoint_626, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_626;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_626] === this.CELL_EMPTY
                    && map[y+1][xPoint_626] === this.CELL_EMPTY
                    && map[y+1][xPoint_626+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_626] === this.CELL_EMPTY
                    && map[y][xPoint_626-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_626-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
