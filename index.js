...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_25825=0;yPoint_25825<this.gameFieldBorderY.length; yPoint_25825++){
                for(var xPoint_25825=0;xPoint_25825<this.gameFieldBorderX.length; xPoint_25825++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_25825, xPoint_25825);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_25825][xPoint_25825] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_25825, xPoint_25825, 'user');
                    if(this._userShipsMap[yPoint_25825][xPoint_25825] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_25825, xPoint_25825, type){
            var id = this.getPointBlockIdByCoords(yPoint_25825, xPoint_25825, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_25825);
                block.setAttribute('data-y', yPoint_25825);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_25825
         * @param {type} xPoint_25825
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_25825, xPoint_25825, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_25825 + '_y' + yPoint_25825;
            }
            return 'pc_x' + xPoint_25825 + '_y' + yPoint_25825;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_25825=0;yPoint_25825<this.gameFieldBorderY.length; yPoint_25825++){
                for(var xPoint_25825=0;xPoint_25825<this.gameFieldBorderX.length; xPoint_25825++){
                    map.push({y: yPoint_25825, x: xPoint_25825});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_25825=-1;yPoint_25825<(this.gameFieldBorderY.length+1); yPoint_25825++){
                for(var xPoint_25825=-1;xPoint_25825<(this.gameFieldBorderX.length+1); xPoint_25825++){
                    if(!map[yPoint_25825]){
                        map[yPoint_25825] = [];
                    }
                    map[yPoint_25825][xPoint_25825] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_25825 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_25825 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_25825, yPoint_25825) === true){
                    if(this.canPutHorizontal(map, xPoint_25825, yPoint_25825, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25825][xPoint_25825 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_25825, yPoint_25825, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_25825 + i][xPoint_25825] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_25825, yPoint_25825){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_25825][xPoint_25825] === this.CELL_EMPTY
                && map[yPoint_25825-1][xPoint_25825] === this.CELL_EMPTY
                && map[yPoint_25825-1][xPoint_25825+1] === this.CELL_EMPTY
                && map[yPoint_25825][xPoint_25825+1] === this.CELL_EMPTY
                && map[yPoint_25825+1][xPoint_25825+1] === this.CELL_EMPTY
                && map[yPoint_25825+1][xPoint_25825] === this.CELL_EMPTY
                && map[yPoint_25825+1][xPoint_25825-1] === this.CELL_EMPTY
                && map[yPoint_25825][xPoint_25825-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_25825, yPoint_25825, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_25825;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_25825][x] === this.CELL_EMPTY
                    && map[yPoint_25825-1][x] === this.CELL_EMPTY
                    && map[yPoint_25825-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25825][x+1] === this.CELL_EMPTY
                    && map[yPoint_25825+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_25825+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_25825, yPoint_25825, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_25825;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_25825] === this.CELL_EMPTY
                    && map[y+1][xPoint_25825] === this.CELL_EMPTY
                    && map[y+1][xPoint_25825+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_25825] === this.CELL_EMPTY
                    && map[y][xPoint_25825-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_25825-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
