...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30813=0;yPoint_30813<this.gameFieldBorderY.length; yPoint_30813++){
                for(var xPoint_30813=0;xPoint_30813<this.gameFieldBorderX.length; xPoint_30813++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30813, xPoint_30813);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30813][xPoint_30813] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30813, xPoint_30813, 'user');
                    if(this._userShipsMap[yPoint_30813][xPoint_30813] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30813, xPoint_30813, type){
            var id = this.getPointBlockIdByCoords(yPoint_30813, xPoint_30813, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30813);
                block.setAttribute('data-y', yPoint_30813);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30813
         * @param {type} xPoint_30813
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30813, xPoint_30813, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30813 + '_y' + yPoint_30813;
            }
            return 'pc_x' + xPoint_30813 + '_y' + yPoint_30813;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30813=0;yPoint_30813<this.gameFieldBorderY.length; yPoint_30813++){
                for(var xPoint_30813=0;xPoint_30813<this.gameFieldBorderX.length; xPoint_30813++){
                    map.push({y: yPoint_30813, x: xPoint_30813});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30813=-1;yPoint_30813<(this.gameFieldBorderY.length+1); yPoint_30813++){
                for(var xPoint_30813=-1;xPoint_30813<(this.gameFieldBorderX.length+1); xPoint_30813++){
                    if(!map[yPoint_30813]){
                        map[yPoint_30813] = [];
                    }
                    map[yPoint_30813][xPoint_30813] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30813 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30813 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30813, yPoint_30813) === true){
                    if(this.canPutHorizontal(map, xPoint_30813, yPoint_30813, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30813][xPoint_30813 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30813, yPoint_30813, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30813 + i][xPoint_30813] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30813, yPoint_30813){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30813][xPoint_30813] === this.CELL_EMPTY
                && map[yPoint_30813-1][xPoint_30813] === this.CELL_EMPTY
                && map[yPoint_30813-1][xPoint_30813+1] === this.CELL_EMPTY
                && map[yPoint_30813][xPoint_30813+1] === this.CELL_EMPTY
                && map[yPoint_30813+1][xPoint_30813+1] === this.CELL_EMPTY
                && map[yPoint_30813+1][xPoint_30813] === this.CELL_EMPTY
                && map[yPoint_30813+1][xPoint_30813-1] === this.CELL_EMPTY
                && map[yPoint_30813][xPoint_30813-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30813, yPoint_30813, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30813;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30813][x] === this.CELL_EMPTY
                    && map[yPoint_30813-1][x] === this.CELL_EMPTY
                    && map[yPoint_30813-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30813][x+1] === this.CELL_EMPTY
                    && map[yPoint_30813+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30813+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30813, yPoint_30813, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30813;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30813] === this.CELL_EMPTY
                    && map[y+1][xPoint_30813] === this.CELL_EMPTY
                    && map[y+1][xPoint_30813+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30813] === this.CELL_EMPTY
                    && map[y][xPoint_30813-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30813-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
