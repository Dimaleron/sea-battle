...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_501=0;yPoint_501<this.gameFieldBorderY.length; yPoint_501++){
                for(var xPoint_501=0;xPoint_501<this.gameFieldBorderX.length; xPoint_501++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_501, xPoint_501);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_501][xPoint_501] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_501, xPoint_501, 'user');
                    if(this._userShipsMap[yPoint_501][xPoint_501] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_501, xPoint_501, type){
            var id = this.getPointBlockIdByCoords(yPoint_501, xPoint_501, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_501);
                block.setAttribute('data-y', yPoint_501);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_501
         * @param {type} xPoint_501
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_501, xPoint_501, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_501 + '_y' + yPoint_501;
            }
            return 'pc_x' + xPoint_501 + '_y' + yPoint_501;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_501=0;yPoint_501<this.gameFieldBorderY.length; yPoint_501++){
                for(var xPoint_501=0;xPoint_501<this.gameFieldBorderX.length; xPoint_501++){
                    map.push({y: yPoint_501, x: xPoint_501});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_501=-1;yPoint_501<(this.gameFieldBorderY.length+1); yPoint_501++){
                for(var xPoint_501=-1;xPoint_501<(this.gameFieldBorderX.length+1); xPoint_501++){
                    if(!map[yPoint_501]){
                        map[yPoint_501] = [];
                    }
                    map[yPoint_501][xPoint_501] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_501 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_501 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_501, yPoint_501) === true){
                    if(this.canPutHorizontal(map, xPoint_501, yPoint_501, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_501][xPoint_501 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_501, yPoint_501, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_501 + i][xPoint_501] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_501, yPoint_501){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_501][xPoint_501] === this.CELL_EMPTY
                && map[yPoint_501-1][xPoint_501] === this.CELL_EMPTY
                && map[yPoint_501-1][xPoint_501+1] === this.CELL_EMPTY
                && map[yPoint_501][xPoint_501+1] === this.CELL_EMPTY
                && map[yPoint_501+1][xPoint_501+1] === this.CELL_EMPTY
                && map[yPoint_501+1][xPoint_501] === this.CELL_EMPTY
                && map[yPoint_501+1][xPoint_501-1] === this.CELL_EMPTY
                && map[yPoint_501][xPoint_501-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_501, yPoint_501, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_501;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_501][x] === this.CELL_EMPTY
                    && map[yPoint_501-1][x] === this.CELL_EMPTY
                    && map[yPoint_501-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_501][x+1] === this.CELL_EMPTY
                    && map[yPoint_501+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_501+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_501, yPoint_501, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_501;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_501] === this.CELL_EMPTY
                    && map[y+1][xPoint_501] === this.CELL_EMPTY
                    && map[y+1][xPoint_501+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_501] === this.CELL_EMPTY
                    && map[y][xPoint_501-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_501-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
