...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_37=0;yPoint_37<this.gameFieldBorderY.length; yPoint_37++){
                for(var xPoint_37=0;xPoint_37<this.gameFieldBorderX.length; xPoint_37++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_37, xPoint_37);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_37][xPoint_37] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_37, xPoint_37, 'user');
                    if(this._userShipsMap[yPoint_37][xPoint_37] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_37, xPoint_37, type){
            var id = this.getPointBlockIdByCoords(yPoint_37, xPoint_37, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_37);
                block.setAttribute('data-y', yPoint_37);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_37
         * @param {type} xPoint_37
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_37, xPoint_37, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_37 + '_y' + yPoint_37;
            }
            return 'pc_x' + xPoint_37 + '_y' + yPoint_37;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_37=0;yPoint_37<this.gameFieldBorderY.length; yPoint_37++){
                for(var xPoint_37=0;xPoint_37<this.gameFieldBorderX.length; xPoint_37++){
                    map.push({y: yPoint_37, x: xPoint_37});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_37=-1;yPoint_37<(this.gameFieldBorderY.length+1); yPoint_37++){
                for(var xPoint_37=-1;xPoint_37<(this.gameFieldBorderX.length+1); xPoint_37++){
                    if(!map[yPoint_37]){
                        map[yPoint_37] = [];
                    }
                    map[yPoint_37][xPoint_37] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_37 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_37 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_37, yPoint_37) === true){
                    if(this.canPutHorizontal(map, xPoint_37, yPoint_37, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_37][xPoint_37 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_37, yPoint_37, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_37 + i][xPoint_37] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_37, yPoint_37){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_37][xPoint_37] === this.CELL_EMPTY
                && map[yPoint_37-1][xPoint_37] === this.CELL_EMPTY
                && map[yPoint_37-1][xPoint_37+1] === this.CELL_EMPTY
                && map[yPoint_37][xPoint_37+1] === this.CELL_EMPTY
                && map[yPoint_37+1][xPoint_37+1] === this.CELL_EMPTY
                && map[yPoint_37+1][xPoint_37] === this.CELL_EMPTY
                && map[yPoint_37+1][xPoint_37-1] === this.CELL_EMPTY
                && map[yPoint_37][xPoint_37-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_37, yPoint_37, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_37;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_37][x] === this.CELL_EMPTY
                    && map[yPoint_37-1][x] === this.CELL_EMPTY
                    && map[yPoint_37-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_37][x+1] === this.CELL_EMPTY
                    && map[yPoint_37+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_37+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_37, yPoint_37, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_37;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_37] === this.CELL_EMPTY
                    && map[y+1][xPoint_37] === this.CELL_EMPTY
                    && map[y+1][xPoint_37+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_37] === this.CELL_EMPTY
                    && map[y][xPoint_37-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_37-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
