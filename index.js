...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_860=0;yPoint_860<this.gameFieldBorderY.length; yPoint_860++){
                for(var xPoint_860=0;xPoint_860<this.gameFieldBorderX.length; xPoint_860++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_860, xPoint_860);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_860][xPoint_860] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_860, xPoint_860, 'user');
                    if(this._userShipsMap[yPoint_860][xPoint_860] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_860, xPoint_860, type){
            var id = this.getPointBlockIdByCoords(yPoint_860, xPoint_860, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_860);
                block.setAttribute('data-y', yPoint_860);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_860
         * @param {type} xPoint_860
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_860, xPoint_860, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_860 + '_y' + yPoint_860;
            }
            return 'pc_x' + xPoint_860 + '_y' + yPoint_860;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_860=0;yPoint_860<this.gameFieldBorderY.length; yPoint_860++){
                for(var xPoint_860=0;xPoint_860<this.gameFieldBorderX.length; xPoint_860++){
                    map.push({y: yPoint_860, x: xPoint_860});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_860=-1;yPoint_860<(this.gameFieldBorderY.length+1); yPoint_860++){
                for(var xPoint_860=-1;xPoint_860<(this.gameFieldBorderX.length+1); xPoint_860++){
                    if(!map[yPoint_860]){
                        map[yPoint_860] = [];
                    }
                    map[yPoint_860][xPoint_860] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_860 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_860 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_860, yPoint_860) === true){
                    if(this.canPutHorizontal(map, xPoint_860, yPoint_860, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_860][xPoint_860 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_860, yPoint_860, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_860 + i][xPoint_860] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_860, yPoint_860){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_860][xPoint_860] === this.CELL_EMPTY
                && map[yPoint_860-1][xPoint_860] === this.CELL_EMPTY
                && map[yPoint_860-1][xPoint_860+1] === this.CELL_EMPTY
                && map[yPoint_860][xPoint_860+1] === this.CELL_EMPTY
                && map[yPoint_860+1][xPoint_860+1] === this.CELL_EMPTY
                && map[yPoint_860+1][xPoint_860] === this.CELL_EMPTY
                && map[yPoint_860+1][xPoint_860-1] === this.CELL_EMPTY
                && map[yPoint_860][xPoint_860-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_860, yPoint_860, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_860;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_860][x] === this.CELL_EMPTY
                    && map[yPoint_860-1][x] === this.CELL_EMPTY
                    && map[yPoint_860-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_860][x+1] === this.CELL_EMPTY
                    && map[yPoint_860+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_860+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_860, yPoint_860, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_860;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_860] === this.CELL_EMPTY
                    && map[y+1][xPoint_860] === this.CELL_EMPTY
                    && map[y+1][xPoint_860+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_860] === this.CELL_EMPTY
                    && map[y][xPoint_860-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_860-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
