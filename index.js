...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_15379=0;yPoint_15379<this.gameFieldBorderY.length; yPoint_15379++){
                for(var xPoint_15379=0;xPoint_15379<this.gameFieldBorderX.length; xPoint_15379++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_15379, xPoint_15379);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_15379][xPoint_15379] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_15379, xPoint_15379, 'user');
                    if(this._userShipsMap[yPoint_15379][xPoint_15379] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_15379, xPoint_15379, type){
            var id = this.getPointBlockIdByCoords(yPoint_15379, xPoint_15379, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_15379);
                block.setAttribute('data-y', yPoint_15379);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_15379
         * @param {type} xPoint_15379
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_15379, xPoint_15379, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_15379 + '_y' + yPoint_15379;
            }
            return 'pc_x' + xPoint_15379 + '_y' + yPoint_15379;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_15379=0;yPoint_15379<this.gameFieldBorderY.length; yPoint_15379++){
                for(var xPoint_15379=0;xPoint_15379<this.gameFieldBorderX.length; xPoint_15379++){
                    map.push({y: yPoint_15379, x: xPoint_15379});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_15379=-1;yPoint_15379<(this.gameFieldBorderY.length+1); yPoint_15379++){
                for(var xPoint_15379=-1;xPoint_15379<(this.gameFieldBorderX.length+1); xPoint_15379++){
                    if(!map[yPoint_15379]){
                        map[yPoint_15379] = [];
                    }
                    map[yPoint_15379][xPoint_15379] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_15379 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_15379 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_15379, yPoint_15379) === true){
                    if(this.canPutHorizontal(map, xPoint_15379, yPoint_15379, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15379][xPoint_15379 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_15379, yPoint_15379, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_15379 + i][xPoint_15379] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_15379, yPoint_15379){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_15379][xPoint_15379] === this.CELL_EMPTY
                && map[yPoint_15379-1][xPoint_15379] === this.CELL_EMPTY
                && map[yPoint_15379-1][xPoint_15379+1] === this.CELL_EMPTY
                && map[yPoint_15379][xPoint_15379+1] === this.CELL_EMPTY
                && map[yPoint_15379+1][xPoint_15379+1] === this.CELL_EMPTY
                && map[yPoint_15379+1][xPoint_15379] === this.CELL_EMPTY
                && map[yPoint_15379+1][xPoint_15379-1] === this.CELL_EMPTY
                && map[yPoint_15379][xPoint_15379-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_15379, yPoint_15379, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_15379;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_15379][x] === this.CELL_EMPTY
                    && map[yPoint_15379-1][x] === this.CELL_EMPTY
                    && map[yPoint_15379-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15379][x+1] === this.CELL_EMPTY
                    && map[yPoint_15379+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_15379+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_15379, yPoint_15379, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_15379;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_15379] === this.CELL_EMPTY
                    && map[y+1][xPoint_15379] === this.CELL_EMPTY
                    && map[y+1][xPoint_15379+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_15379] === this.CELL_EMPTY
                    && map[y][xPoint_15379-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_15379-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
