...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_52=0;yPoint_52<this.gameFieldBorderY.length; yPoint_52++){
                for(var xPoint_52=0;xPoint_52<this.gameFieldBorderX.length; xPoint_52++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_52, xPoint_52);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_52][xPoint_52] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_52, xPoint_52, 'user');
                    if(this._userShipsMap[yPoint_52][xPoint_52] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_52, xPoint_52, type){
            var id = this.getPointBlockIdByCoords(yPoint_52, xPoint_52, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_52);
                block.setAttribute('data-y', yPoint_52);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_52
         * @param {type} xPoint_52
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_52, xPoint_52, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_52 + '_y' + yPoint_52;
            }
            return 'pc_x' + xPoint_52 + '_y' + yPoint_52;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_52=0;yPoint_52<this.gameFieldBorderY.length; yPoint_52++){
                for(var xPoint_52=0;xPoint_52<this.gameFieldBorderX.length; xPoint_52++){
                    map.push({y: yPoint_52, x: xPoint_52});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_52=-1;yPoint_52<(this.gameFieldBorderY.length+1); yPoint_52++){
                for(var xPoint_52=-1;xPoint_52<(this.gameFieldBorderX.length+1); xPoint_52++){
                    if(!map[yPoint_52]){
                        map[yPoint_52] = [];
                    }
                    map[yPoint_52][xPoint_52] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_52 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_52 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_52, yPoint_52) === true){
                    if(this.canPutHorizontal(map, xPoint_52, yPoint_52, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_52][xPoint_52 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_52, yPoint_52, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_52 + i][xPoint_52] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_52, yPoint_52){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_52][xPoint_52] === this.CELL_EMPTY
                && map[yPoint_52-1][xPoint_52] === this.CELL_EMPTY
                && map[yPoint_52-1][xPoint_52+1] === this.CELL_EMPTY
                && map[yPoint_52][xPoint_52+1] === this.CELL_EMPTY
                && map[yPoint_52+1][xPoint_52+1] === this.CELL_EMPTY
                && map[yPoint_52+1][xPoint_52] === this.CELL_EMPTY
                && map[yPoint_52+1][xPoint_52-1] === this.CELL_EMPTY
                && map[yPoint_52][xPoint_52-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_52, yPoint_52, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_52;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_52][x] === this.CELL_EMPTY
                    && map[yPoint_52-1][x] === this.CELL_EMPTY
                    && map[yPoint_52-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_52][x+1] === this.CELL_EMPTY
                    && map[yPoint_52+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_52+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_52, yPoint_52, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_52;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_52] === this.CELL_EMPTY
                    && map[y+1][xPoint_52] === this.CELL_EMPTY
                    && map[y+1][xPoint_52+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_52] === this.CELL_EMPTY
                    && map[y][xPoint_52-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_52-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
