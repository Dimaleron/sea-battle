...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_818=0;yPoint_818<this.gameFieldBorderY.length; yPoint_818++){
                for(var xPoint_818=0;xPoint_818<this.gameFieldBorderX.length; xPoint_818++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_818, xPoint_818);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_818][xPoint_818] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_818, xPoint_818, 'user');
                    if(this._userShipsMap[yPoint_818][xPoint_818] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_818, xPoint_818, type){
            var id = this.getPointBlockIdByCoords(yPoint_818, xPoint_818, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_818);
                block.setAttribute('data-y', yPoint_818);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_818
         * @param {type} xPoint_818
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_818, xPoint_818, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_818 + '_y' + yPoint_818;
            }
            return 'pc_x' + xPoint_818 + '_y' + yPoint_818;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_818=0;yPoint_818<this.gameFieldBorderY.length; yPoint_818++){
                for(var xPoint_818=0;xPoint_818<this.gameFieldBorderX.length; xPoint_818++){
                    map.push({y: yPoint_818, x: xPoint_818});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_818=-1;yPoint_818<(this.gameFieldBorderY.length+1); yPoint_818++){
                for(var xPoint_818=-1;xPoint_818<(this.gameFieldBorderX.length+1); xPoint_818++){
                    if(!map[yPoint_818]){
                        map[yPoint_818] = [];
                    }
                    map[yPoint_818][xPoint_818] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_818 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_818 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_818, yPoint_818) === true){
                    if(this.canPutHorizontal(map, xPoint_818, yPoint_818, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_818][xPoint_818 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_818, yPoint_818, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_818 + i][xPoint_818] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_818, yPoint_818){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_818][xPoint_818] === this.CELL_EMPTY
                && map[yPoint_818-1][xPoint_818] === this.CELL_EMPTY
                && map[yPoint_818-1][xPoint_818+1] === this.CELL_EMPTY
                && map[yPoint_818][xPoint_818+1] === this.CELL_EMPTY
                && map[yPoint_818+1][xPoint_818+1] === this.CELL_EMPTY
                && map[yPoint_818+1][xPoint_818] === this.CELL_EMPTY
                && map[yPoint_818+1][xPoint_818-1] === this.CELL_EMPTY
                && map[yPoint_818][xPoint_818-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_818, yPoint_818, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_818;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_818][x] === this.CELL_EMPTY
                    && map[yPoint_818-1][x] === this.CELL_EMPTY
                    && map[yPoint_818-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_818][x+1] === this.CELL_EMPTY
                    && map[yPoint_818+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_818+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_818, yPoint_818, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_818;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_818] === this.CELL_EMPTY
                    && map[y+1][xPoint_818] === this.CELL_EMPTY
                    && map[y+1][xPoint_818+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_818] === this.CELL_EMPTY
                    && map[y][xPoint_818-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_818-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
