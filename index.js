...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20621=0;yPoint_20621<this.gameFieldBorderY.length; yPoint_20621++){
                for(var xPoint_20621=0;xPoint_20621<this.gameFieldBorderX.length; xPoint_20621++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20621, xPoint_20621);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20621][xPoint_20621] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20621, xPoint_20621, 'user');
                    if(this._userShipsMap[yPoint_20621][xPoint_20621] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20621, xPoint_20621, type){
            var id = this.getPointBlockIdByCoords(yPoint_20621, xPoint_20621, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20621);
                block.setAttribute('data-y', yPoint_20621);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20621
         * @param {type} xPoint_20621
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20621, xPoint_20621, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20621 + '_y' + yPoint_20621;
            }
            return 'pc_x' + xPoint_20621 + '_y' + yPoint_20621;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20621=0;yPoint_20621<this.gameFieldBorderY.length; yPoint_20621++){
                for(var xPoint_20621=0;xPoint_20621<this.gameFieldBorderX.length; xPoint_20621++){
                    map.push({y: yPoint_20621, x: xPoint_20621});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20621=-1;yPoint_20621<(this.gameFieldBorderY.length+1); yPoint_20621++){
                for(var xPoint_20621=-1;xPoint_20621<(this.gameFieldBorderX.length+1); xPoint_20621++){
                    if(!map[yPoint_20621]){
                        map[yPoint_20621] = [];
                    }
                    map[yPoint_20621][xPoint_20621] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20621 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20621 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20621, yPoint_20621) === true){
                    if(this.canPutHorizontal(map, xPoint_20621, yPoint_20621, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20621][xPoint_20621 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20621, yPoint_20621, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20621 + i][xPoint_20621] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20621, yPoint_20621){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20621][xPoint_20621] === this.CELL_EMPTY
                && map[yPoint_20621-1][xPoint_20621] === this.CELL_EMPTY
                && map[yPoint_20621-1][xPoint_20621+1] === this.CELL_EMPTY
                && map[yPoint_20621][xPoint_20621+1] === this.CELL_EMPTY
                && map[yPoint_20621+1][xPoint_20621+1] === this.CELL_EMPTY
                && map[yPoint_20621+1][xPoint_20621] === this.CELL_EMPTY
                && map[yPoint_20621+1][xPoint_20621-1] === this.CELL_EMPTY
                && map[yPoint_20621][xPoint_20621-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20621, yPoint_20621, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20621;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20621][x] === this.CELL_EMPTY
                    && map[yPoint_20621-1][x] === this.CELL_EMPTY
                    && map[yPoint_20621-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20621][x+1] === this.CELL_EMPTY
                    && map[yPoint_20621+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20621+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20621, yPoint_20621, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20621;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20621] === this.CELL_EMPTY
                    && map[y+1][xPoint_20621] === this.CELL_EMPTY
                    && map[y+1][xPoint_20621+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20621] === this.CELL_EMPTY
                    && map[y][xPoint_20621-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20621-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
