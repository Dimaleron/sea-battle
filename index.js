...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20333=0;yPoint_20333<this.gameFieldBorderY.length; yPoint_20333++){
                for(var xPoint_20333=0;xPoint_20333<this.gameFieldBorderX.length; xPoint_20333++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20333, xPoint_20333);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20333][xPoint_20333] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20333, xPoint_20333, 'user');
                    if(this._userShipsMap[yPoint_20333][xPoint_20333] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20333, xPoint_20333, type){
            var id = this.getPointBlockIdByCoords(yPoint_20333, xPoint_20333, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20333);
                block.setAttribute('data-y', yPoint_20333);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20333
         * @param {type} xPoint_20333
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20333, xPoint_20333, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20333 + '_y' + yPoint_20333;
            }
            return 'pc_x' + xPoint_20333 + '_y' + yPoint_20333;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20333=0;yPoint_20333<this.gameFieldBorderY.length; yPoint_20333++){
                for(var xPoint_20333=0;xPoint_20333<this.gameFieldBorderX.length; xPoint_20333++){
                    map.push({y: yPoint_20333, x: xPoint_20333});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20333=-1;yPoint_20333<(this.gameFieldBorderY.length+1); yPoint_20333++){
                for(var xPoint_20333=-1;xPoint_20333<(this.gameFieldBorderX.length+1); xPoint_20333++){
                    if(!map[yPoint_20333]){
                        map[yPoint_20333] = [];
                    }
                    map[yPoint_20333][xPoint_20333] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20333 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20333 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20333, yPoint_20333) === true){
                    if(this.canPutHorizontal(map, xPoint_20333, yPoint_20333, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20333][xPoint_20333 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20333, yPoint_20333, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20333 + i][xPoint_20333] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20333, yPoint_20333){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20333][xPoint_20333] === this.CELL_EMPTY
                && map[yPoint_20333-1][xPoint_20333] === this.CELL_EMPTY
                && map[yPoint_20333-1][xPoint_20333+1] === this.CELL_EMPTY
                && map[yPoint_20333][xPoint_20333+1] === this.CELL_EMPTY
                && map[yPoint_20333+1][xPoint_20333+1] === this.CELL_EMPTY
                && map[yPoint_20333+1][xPoint_20333] === this.CELL_EMPTY
                && map[yPoint_20333+1][xPoint_20333-1] === this.CELL_EMPTY
                && map[yPoint_20333][xPoint_20333-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20333, yPoint_20333, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20333;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20333][x] === this.CELL_EMPTY
                    && map[yPoint_20333-1][x] === this.CELL_EMPTY
                    && map[yPoint_20333-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20333][x+1] === this.CELL_EMPTY
                    && map[yPoint_20333+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20333+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20333, yPoint_20333, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20333;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20333] === this.CELL_EMPTY
                    && map[y+1][xPoint_20333] === this.CELL_EMPTY
                    && map[y+1][xPoint_20333+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20333] === this.CELL_EMPTY
                    && map[y][xPoint_20333-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20333-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
