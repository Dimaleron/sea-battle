...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21794=0;yPoint_21794<this.gameFieldBorderY.length; yPoint_21794++){
                for(var xPoint_21794=0;xPoint_21794<this.gameFieldBorderX.length; xPoint_21794++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21794, xPoint_21794);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21794][xPoint_21794] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21794, xPoint_21794, 'user');
                    if(this._userShipsMap[yPoint_21794][xPoint_21794] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21794, xPoint_21794, type){
            var id = this.getPointBlockIdByCoords(yPoint_21794, xPoint_21794, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21794);
                block.setAttribute('data-y', yPoint_21794);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21794
         * @param {type} xPoint_21794
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21794, xPoint_21794, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21794 + '_y' + yPoint_21794;
            }
            return 'pc_x' + xPoint_21794 + '_y' + yPoint_21794;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21794=0;yPoint_21794<this.gameFieldBorderY.length; yPoint_21794++){
                for(var xPoint_21794=0;xPoint_21794<this.gameFieldBorderX.length; xPoint_21794++){
                    map.push({y: yPoint_21794, x: xPoint_21794});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21794=-1;yPoint_21794<(this.gameFieldBorderY.length+1); yPoint_21794++){
                for(var xPoint_21794=-1;xPoint_21794<(this.gameFieldBorderX.length+1); xPoint_21794++){
                    if(!map[yPoint_21794]){
                        map[yPoint_21794] = [];
                    }
                    map[yPoint_21794][xPoint_21794] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21794 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21794 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21794, yPoint_21794) === true){
                    if(this.canPutHorizontal(map, xPoint_21794, yPoint_21794, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21794][xPoint_21794 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21794, yPoint_21794, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21794 + i][xPoint_21794] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21794, yPoint_21794){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21794][xPoint_21794] === this.CELL_EMPTY
                && map[yPoint_21794-1][xPoint_21794] === this.CELL_EMPTY
                && map[yPoint_21794-1][xPoint_21794+1] === this.CELL_EMPTY
                && map[yPoint_21794][xPoint_21794+1] === this.CELL_EMPTY
                && map[yPoint_21794+1][xPoint_21794+1] === this.CELL_EMPTY
                && map[yPoint_21794+1][xPoint_21794] === this.CELL_EMPTY
                && map[yPoint_21794+1][xPoint_21794-1] === this.CELL_EMPTY
                && map[yPoint_21794][xPoint_21794-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21794, yPoint_21794, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21794;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21794][x] === this.CELL_EMPTY
                    && map[yPoint_21794-1][x] === this.CELL_EMPTY
                    && map[yPoint_21794-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21794][x+1] === this.CELL_EMPTY
                    && map[yPoint_21794+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21794+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21794, yPoint_21794, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21794;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21794] === this.CELL_EMPTY
                    && map[y+1][xPoint_21794] === this.CELL_EMPTY
                    && map[y+1][xPoint_21794+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21794] === this.CELL_EMPTY
                    && map[y][xPoint_21794-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21794-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
