...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9618=0;yPoint_9618<this.gameFieldBorderY.length; yPoint_9618++){
                for(var xPoint_9618=0;xPoint_9618<this.gameFieldBorderX.length; xPoint_9618++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9618, xPoint_9618);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9618][xPoint_9618] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9618, xPoint_9618, 'user');
                    if(this._userShipsMap[yPoint_9618][xPoint_9618] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9618, xPoint_9618, type){
            var id = this.getPointBlockIdByCoords(yPoint_9618, xPoint_9618, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9618);
                block.setAttribute('data-y', yPoint_9618);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9618
         * @param {type} xPoint_9618
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9618, xPoint_9618, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9618 + '_y' + yPoint_9618;
            }
            return 'pc_x' + xPoint_9618 + '_y' + yPoint_9618;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9618=0;yPoint_9618<this.gameFieldBorderY.length; yPoint_9618++){
                for(var xPoint_9618=0;xPoint_9618<this.gameFieldBorderX.length; xPoint_9618++){
                    map.push({y: yPoint_9618, x: xPoint_9618});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9618=-1;yPoint_9618<(this.gameFieldBorderY.length+1); yPoint_9618++){
                for(var xPoint_9618=-1;xPoint_9618<(this.gameFieldBorderX.length+1); xPoint_9618++){
                    if(!map[yPoint_9618]){
                        map[yPoint_9618] = [];
                    }
                    map[yPoint_9618][xPoint_9618] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9618 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9618 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9618, yPoint_9618) === true){
                    if(this.canPutHorizontal(map, xPoint_9618, yPoint_9618, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9618][xPoint_9618 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9618, yPoint_9618, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9618 + i][xPoint_9618] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9618, yPoint_9618){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9618][xPoint_9618] === this.CELL_EMPTY
                && map[yPoint_9618-1][xPoint_9618] === this.CELL_EMPTY
                && map[yPoint_9618-1][xPoint_9618+1] === this.CELL_EMPTY
                && map[yPoint_9618][xPoint_9618+1] === this.CELL_EMPTY
                && map[yPoint_9618+1][xPoint_9618+1] === this.CELL_EMPTY
                && map[yPoint_9618+1][xPoint_9618] === this.CELL_EMPTY
                && map[yPoint_9618+1][xPoint_9618-1] === this.CELL_EMPTY
                && map[yPoint_9618][xPoint_9618-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9618, yPoint_9618, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9618;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9618][x] === this.CELL_EMPTY
                    && map[yPoint_9618-1][x] === this.CELL_EMPTY
                    && map[yPoint_9618-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9618][x+1] === this.CELL_EMPTY
                    && map[yPoint_9618+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9618+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9618, yPoint_9618, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9618;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9618] === this.CELL_EMPTY
                    && map[y+1][xPoint_9618] === this.CELL_EMPTY
                    && map[y+1][xPoint_9618+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9618] === this.CELL_EMPTY
                    && map[y][xPoint_9618-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9618-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
