...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3014=0;yPoint_3014<this.gameFieldBorderY.length; yPoint_3014++){
                for(var xPoint_3014=0;xPoint_3014<this.gameFieldBorderX.length; xPoint_3014++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3014, xPoint_3014);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3014][xPoint_3014] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3014, xPoint_3014, 'user');
                    if(this._userShipsMap[yPoint_3014][xPoint_3014] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3014, xPoint_3014, type){
            var id = this.getPointBlockIdByCoords(yPoint_3014, xPoint_3014, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3014);
                block.setAttribute('data-y', yPoint_3014);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3014
         * @param {type} xPoint_3014
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3014, xPoint_3014, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3014 + '_y' + yPoint_3014;
            }
            return 'pc_x' + xPoint_3014 + '_y' + yPoint_3014;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3014=0;yPoint_3014<this.gameFieldBorderY.length; yPoint_3014++){
                for(var xPoint_3014=0;xPoint_3014<this.gameFieldBorderX.length; xPoint_3014++){
                    map.push({y: yPoint_3014, x: xPoint_3014});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3014=-1;yPoint_3014<(this.gameFieldBorderY.length+1); yPoint_3014++){
                for(var xPoint_3014=-1;xPoint_3014<(this.gameFieldBorderX.length+1); xPoint_3014++){
                    if(!map[yPoint_3014]){
                        map[yPoint_3014] = [];
                    }
                    map[yPoint_3014][xPoint_3014] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3014 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3014 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3014, yPoint_3014) === true){
                    if(this.canPutHorizontal(map, xPoint_3014, yPoint_3014, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3014][xPoint_3014 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3014, yPoint_3014, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3014 + i][xPoint_3014] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3014, yPoint_3014){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3014][xPoint_3014] === this.CELL_EMPTY
                && map[yPoint_3014-1][xPoint_3014] === this.CELL_EMPTY
                && map[yPoint_3014-1][xPoint_3014+1] === this.CELL_EMPTY
                && map[yPoint_3014][xPoint_3014+1] === this.CELL_EMPTY
                && map[yPoint_3014+1][xPoint_3014+1] === this.CELL_EMPTY
                && map[yPoint_3014+1][xPoint_3014] === this.CELL_EMPTY
                && map[yPoint_3014+1][xPoint_3014-1] === this.CELL_EMPTY
                && map[yPoint_3014][xPoint_3014-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3014, yPoint_3014, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3014;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3014][x] === this.CELL_EMPTY
                    && map[yPoint_3014-1][x] === this.CELL_EMPTY
                    && map[yPoint_3014-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3014][x+1] === this.CELL_EMPTY
                    && map[yPoint_3014+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3014+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3014, yPoint_3014, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3014;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3014] === this.CELL_EMPTY
                    && map[y+1][xPoint_3014] === this.CELL_EMPTY
                    && map[y+1][xPoint_3014+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3014] === this.CELL_EMPTY
                    && map[y][xPoint_3014-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3014-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
