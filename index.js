...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_162=0;yPoint_162<this.gameFieldBorderY.length; yPoint_162++){
                for(var xPoint_162=0;xPoint_162<this.gameFieldBorderX.length; xPoint_162++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_162, xPoint_162);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_162][xPoint_162] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_162, xPoint_162, 'user');
                    if(this._userShipsMap[yPoint_162][xPoint_162] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_162, xPoint_162, type){
            var id = this.getPointBlockIdByCoords(yPoint_162, xPoint_162, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_162);
                block.setAttribute('data-y', yPoint_162);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_162
         * @param {type} xPoint_162
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_162, xPoint_162, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_162 + '_y' + yPoint_162;
            }
            return 'pc_x' + xPoint_162 + '_y' + yPoint_162;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_162=0;yPoint_162<this.gameFieldBorderY.length; yPoint_162++){
                for(var xPoint_162=0;xPoint_162<this.gameFieldBorderX.length; xPoint_162++){
                    map.push({y: yPoint_162, x: xPoint_162});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_162=-1;yPoint_162<(this.gameFieldBorderY.length+1); yPoint_162++){
                for(var xPoint_162=-1;xPoint_162<(this.gameFieldBorderX.length+1); xPoint_162++){
                    if(!map[yPoint_162]){
                        map[yPoint_162] = [];
                    }
                    map[yPoint_162][xPoint_162] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_162 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_162 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_162, yPoint_162) === true){
                    if(this.canPutHorizontal(map, xPoint_162, yPoint_162, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_162][xPoint_162 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_162, yPoint_162, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_162 + i][xPoint_162] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_162, yPoint_162){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_162][xPoint_162] === this.CELL_EMPTY
                && map[yPoint_162-1][xPoint_162] === this.CELL_EMPTY
                && map[yPoint_162-1][xPoint_162+1] === this.CELL_EMPTY
                && map[yPoint_162][xPoint_162+1] === this.CELL_EMPTY
                && map[yPoint_162+1][xPoint_162+1] === this.CELL_EMPTY
                && map[yPoint_162+1][xPoint_162] === this.CELL_EMPTY
                && map[yPoint_162+1][xPoint_162-1] === this.CELL_EMPTY
                && map[yPoint_162][xPoint_162-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_162, yPoint_162, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_162;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_162][x] === this.CELL_EMPTY
                    && map[yPoint_162-1][x] === this.CELL_EMPTY
                    && map[yPoint_162-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_162][x+1] === this.CELL_EMPTY
                    && map[yPoint_162+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_162+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_162, yPoint_162, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_162;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_162] === this.CELL_EMPTY
                    && map[y+1][xPoint_162] === this.CELL_EMPTY
                    && map[y+1][xPoint_162+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_162] === this.CELL_EMPTY
                    && map[y][xPoint_162-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_162-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
