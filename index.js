...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32102=0;yPoint_32102<this.gameFieldBorderY.length; yPoint_32102++){
                for(var xPoint_32102=0;xPoint_32102<this.gameFieldBorderX.length; xPoint_32102++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32102, xPoint_32102);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32102][xPoint_32102] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32102, xPoint_32102, 'user');
                    if(this._userShipsMap[yPoint_32102][xPoint_32102] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32102, xPoint_32102, type){
            var id = this.getPointBlockIdByCoords(yPoint_32102, xPoint_32102, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32102);
                block.setAttribute('data-y', yPoint_32102);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32102
         * @param {type} xPoint_32102
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32102, xPoint_32102, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32102 + '_y' + yPoint_32102;
            }
            return 'pc_x' + xPoint_32102 + '_y' + yPoint_32102;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32102=0;yPoint_32102<this.gameFieldBorderY.length; yPoint_32102++){
                for(var xPoint_32102=0;xPoint_32102<this.gameFieldBorderX.length; xPoint_32102++){
                    map.push({y: yPoint_32102, x: xPoint_32102});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32102=-1;yPoint_32102<(this.gameFieldBorderY.length+1); yPoint_32102++){
                for(var xPoint_32102=-1;xPoint_32102<(this.gameFieldBorderX.length+1); xPoint_32102++){
                    if(!map[yPoint_32102]){
                        map[yPoint_32102] = [];
                    }
                    map[yPoint_32102][xPoint_32102] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32102 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32102 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32102, yPoint_32102) === true){
                    if(this.canPutHorizontal(map, xPoint_32102, yPoint_32102, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32102][xPoint_32102 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32102, yPoint_32102, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32102 + i][xPoint_32102] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32102, yPoint_32102){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32102][xPoint_32102] === this.CELL_EMPTY
                && map[yPoint_32102-1][xPoint_32102] === this.CELL_EMPTY
                && map[yPoint_32102-1][xPoint_32102+1] === this.CELL_EMPTY
                && map[yPoint_32102][xPoint_32102+1] === this.CELL_EMPTY
                && map[yPoint_32102+1][xPoint_32102+1] === this.CELL_EMPTY
                && map[yPoint_32102+1][xPoint_32102] === this.CELL_EMPTY
                && map[yPoint_32102+1][xPoint_32102-1] === this.CELL_EMPTY
                && map[yPoint_32102][xPoint_32102-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32102, yPoint_32102, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32102;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32102][x] === this.CELL_EMPTY
                    && map[yPoint_32102-1][x] === this.CELL_EMPTY
                    && map[yPoint_32102-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32102][x+1] === this.CELL_EMPTY
                    && map[yPoint_32102+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32102+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32102, yPoint_32102, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32102;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32102] === this.CELL_EMPTY
                    && map[y+1][xPoint_32102] === this.CELL_EMPTY
                    && map[y+1][xPoint_32102+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32102] === this.CELL_EMPTY
                    && map[y][xPoint_32102-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32102-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
