...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4665=0;yPoint_4665<this.gameFieldBorderY.length; yPoint_4665++){
                for(var xPoint_4665=0;xPoint_4665<this.gameFieldBorderX.length; xPoint_4665++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4665, xPoint_4665);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4665][xPoint_4665] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4665, xPoint_4665, 'user');
                    if(this._userShipsMap[yPoint_4665][xPoint_4665] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4665, xPoint_4665, type){
            var id = this.getPointBlockIdByCoords(yPoint_4665, xPoint_4665, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4665);
                block.setAttribute('data-y', yPoint_4665);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4665
         * @param {type} xPoint_4665
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4665, xPoint_4665, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4665 + '_y' + yPoint_4665;
            }
            return 'pc_x' + xPoint_4665 + '_y' + yPoint_4665;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4665=0;yPoint_4665<this.gameFieldBorderY.length; yPoint_4665++){
                for(var xPoint_4665=0;xPoint_4665<this.gameFieldBorderX.length; xPoint_4665++){
                    map.push({y: yPoint_4665, x: xPoint_4665});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4665=-1;yPoint_4665<(this.gameFieldBorderY.length+1); yPoint_4665++){
                for(var xPoint_4665=-1;xPoint_4665<(this.gameFieldBorderX.length+1); xPoint_4665++){
                    if(!map[yPoint_4665]){
                        map[yPoint_4665] = [];
                    }
                    map[yPoint_4665][xPoint_4665] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4665 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4665 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4665, yPoint_4665) === true){
                    if(this.canPutHorizontal(map, xPoint_4665, yPoint_4665, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4665][xPoint_4665 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4665, yPoint_4665, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4665 + i][xPoint_4665] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4665, yPoint_4665){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4665][xPoint_4665] === this.CELL_EMPTY
                && map[yPoint_4665-1][xPoint_4665] === this.CELL_EMPTY
                && map[yPoint_4665-1][xPoint_4665+1] === this.CELL_EMPTY
                && map[yPoint_4665][xPoint_4665+1] === this.CELL_EMPTY
                && map[yPoint_4665+1][xPoint_4665+1] === this.CELL_EMPTY
                && map[yPoint_4665+1][xPoint_4665] === this.CELL_EMPTY
                && map[yPoint_4665+1][xPoint_4665-1] === this.CELL_EMPTY
                && map[yPoint_4665][xPoint_4665-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4665, yPoint_4665, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4665;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4665][x] === this.CELL_EMPTY
                    && map[yPoint_4665-1][x] === this.CELL_EMPTY
                    && map[yPoint_4665-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4665][x+1] === this.CELL_EMPTY
                    && map[yPoint_4665+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4665+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4665, yPoint_4665, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4665;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4665] === this.CELL_EMPTY
                    && map[y+1][xPoint_4665] === this.CELL_EMPTY
                    && map[y+1][xPoint_4665+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4665] === this.CELL_EMPTY
                    && map[y][xPoint_4665-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4665-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
