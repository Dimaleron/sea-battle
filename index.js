...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11284=0;yPoint_11284<this.gameFieldBorderY.length; yPoint_11284++){
                for(var xPoint_11284=0;xPoint_11284<this.gameFieldBorderX.length; xPoint_11284++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11284, xPoint_11284);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11284][xPoint_11284] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11284, xPoint_11284, 'user');
                    if(this._userShipsMap[yPoint_11284][xPoint_11284] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11284, xPoint_11284, type){
            var id = this.getPointBlockIdByCoords(yPoint_11284, xPoint_11284, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11284);
                block.setAttribute('data-y', yPoint_11284);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11284
         * @param {type} xPoint_11284
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11284, xPoint_11284, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11284 + '_y' + yPoint_11284;
            }
            return 'pc_x' + xPoint_11284 + '_y' + yPoint_11284;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11284=0;yPoint_11284<this.gameFieldBorderY.length; yPoint_11284++){
                for(var xPoint_11284=0;xPoint_11284<this.gameFieldBorderX.length; xPoint_11284++){
                    map.push({y: yPoint_11284, x: xPoint_11284});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11284=-1;yPoint_11284<(this.gameFieldBorderY.length+1); yPoint_11284++){
                for(var xPoint_11284=-1;xPoint_11284<(this.gameFieldBorderX.length+1); xPoint_11284++){
                    if(!map[yPoint_11284]){
                        map[yPoint_11284] = [];
                    }
                    map[yPoint_11284][xPoint_11284] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11284 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11284 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11284, yPoint_11284) === true){
                    if(this.canPutHorizontal(map, xPoint_11284, yPoint_11284, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11284][xPoint_11284 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11284, yPoint_11284, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11284 + i][xPoint_11284] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11284, yPoint_11284){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11284][xPoint_11284] === this.CELL_EMPTY
                && map[yPoint_11284-1][xPoint_11284] === this.CELL_EMPTY
                && map[yPoint_11284-1][xPoint_11284+1] === this.CELL_EMPTY
                && map[yPoint_11284][xPoint_11284+1] === this.CELL_EMPTY
                && map[yPoint_11284+1][xPoint_11284+1] === this.CELL_EMPTY
                && map[yPoint_11284+1][xPoint_11284] === this.CELL_EMPTY
                && map[yPoint_11284+1][xPoint_11284-1] === this.CELL_EMPTY
                && map[yPoint_11284][xPoint_11284-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11284, yPoint_11284, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11284;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11284][x] === this.CELL_EMPTY
                    && map[yPoint_11284-1][x] === this.CELL_EMPTY
                    && map[yPoint_11284-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11284][x+1] === this.CELL_EMPTY
                    && map[yPoint_11284+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11284+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11284, yPoint_11284, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11284;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11284] === this.CELL_EMPTY
                    && map[y+1][xPoint_11284] === this.CELL_EMPTY
                    && map[y+1][xPoint_11284+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11284] === this.CELL_EMPTY
                    && map[y][xPoint_11284-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11284-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
