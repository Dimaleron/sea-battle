...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_22488=0;yPoint_22488<this.gameFieldBorderY.length; yPoint_22488++){
                for(var xPoint_22488=0;xPoint_22488<this.gameFieldBorderX.length; xPoint_22488++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_22488, xPoint_22488);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_22488][xPoint_22488] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_22488, xPoint_22488, 'user');
                    if(this._userShipsMap[yPoint_22488][xPoint_22488] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_22488, xPoint_22488, type){
            var id = this.getPointBlockIdByCoords(yPoint_22488, xPoint_22488, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_22488);
                block.setAttribute('data-y', yPoint_22488);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_22488
         * @param {type} xPoint_22488
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_22488, xPoint_22488, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_22488 + '_y' + yPoint_22488;
            }
            return 'pc_x' + xPoint_22488 + '_y' + yPoint_22488;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_22488=0;yPoint_22488<this.gameFieldBorderY.length; yPoint_22488++){
                for(var xPoint_22488=0;xPoint_22488<this.gameFieldBorderX.length; xPoint_22488++){
                    map.push({y: yPoint_22488, x: xPoint_22488});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_22488=-1;yPoint_22488<(this.gameFieldBorderY.length+1); yPoint_22488++){
                for(var xPoint_22488=-1;xPoint_22488<(this.gameFieldBorderX.length+1); xPoint_22488++){
                    if(!map[yPoint_22488]){
                        map[yPoint_22488] = [];
                    }
                    map[yPoint_22488][xPoint_22488] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_22488 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_22488 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_22488, yPoint_22488) === true){
                    if(this.canPutHorizontal(map, xPoint_22488, yPoint_22488, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22488][xPoint_22488 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_22488, yPoint_22488, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_22488 + i][xPoint_22488] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_22488, yPoint_22488){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_22488][xPoint_22488] === this.CELL_EMPTY
                && map[yPoint_22488-1][xPoint_22488] === this.CELL_EMPTY
                && map[yPoint_22488-1][xPoint_22488+1] === this.CELL_EMPTY
                && map[yPoint_22488][xPoint_22488+1] === this.CELL_EMPTY
                && map[yPoint_22488+1][xPoint_22488+1] === this.CELL_EMPTY
                && map[yPoint_22488+1][xPoint_22488] === this.CELL_EMPTY
                && map[yPoint_22488+1][xPoint_22488-1] === this.CELL_EMPTY
                && map[yPoint_22488][xPoint_22488-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_22488, yPoint_22488, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_22488;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_22488][x] === this.CELL_EMPTY
                    && map[yPoint_22488-1][x] === this.CELL_EMPTY
                    && map[yPoint_22488-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22488][x+1] === this.CELL_EMPTY
                    && map[yPoint_22488+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_22488+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_22488, yPoint_22488, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_22488;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_22488] === this.CELL_EMPTY
                    && map[y+1][xPoint_22488] === this.CELL_EMPTY
                    && map[y+1][xPoint_22488+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_22488] === this.CELL_EMPTY
                    && map[y][xPoint_22488-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_22488-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
