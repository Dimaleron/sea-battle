...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_21701=0;yPoint_21701<this.gameFieldBorderY.length; yPoint_21701++){
                for(var xPoint_21701=0;xPoint_21701<this.gameFieldBorderX.length; xPoint_21701++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_21701, xPoint_21701);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_21701][xPoint_21701] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_21701, xPoint_21701, 'user');
                    if(this._userShipsMap[yPoint_21701][xPoint_21701] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_21701, xPoint_21701, type){
            var id = this.getPointBlockIdByCoords(yPoint_21701, xPoint_21701, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_21701);
                block.setAttribute('data-y', yPoint_21701);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_21701
         * @param {type} xPoint_21701
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_21701, xPoint_21701, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_21701 + '_y' + yPoint_21701;
            }
            return 'pc_x' + xPoint_21701 + '_y' + yPoint_21701;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_21701=0;yPoint_21701<this.gameFieldBorderY.length; yPoint_21701++){
                for(var xPoint_21701=0;xPoint_21701<this.gameFieldBorderX.length; xPoint_21701++){
                    map.push({y: yPoint_21701, x: xPoint_21701});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_21701=-1;yPoint_21701<(this.gameFieldBorderY.length+1); yPoint_21701++){
                for(var xPoint_21701=-1;xPoint_21701<(this.gameFieldBorderX.length+1); xPoint_21701++){
                    if(!map[yPoint_21701]){
                        map[yPoint_21701] = [];
                    }
                    map[yPoint_21701][xPoint_21701] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_21701 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_21701 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_21701, yPoint_21701) === true){
                    if(this.canPutHorizontal(map, xPoint_21701, yPoint_21701, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21701][xPoint_21701 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_21701, yPoint_21701, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_21701 + i][xPoint_21701] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_21701, yPoint_21701){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_21701][xPoint_21701] === this.CELL_EMPTY
                && map[yPoint_21701-1][xPoint_21701] === this.CELL_EMPTY
                && map[yPoint_21701-1][xPoint_21701+1] === this.CELL_EMPTY
                && map[yPoint_21701][xPoint_21701+1] === this.CELL_EMPTY
                && map[yPoint_21701+1][xPoint_21701+1] === this.CELL_EMPTY
                && map[yPoint_21701+1][xPoint_21701] === this.CELL_EMPTY
                && map[yPoint_21701+1][xPoint_21701-1] === this.CELL_EMPTY
                && map[yPoint_21701][xPoint_21701-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_21701, yPoint_21701, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_21701;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_21701][x] === this.CELL_EMPTY
                    && map[yPoint_21701-1][x] === this.CELL_EMPTY
                    && map[yPoint_21701-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21701][x+1] === this.CELL_EMPTY
                    && map[yPoint_21701+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_21701+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_21701, yPoint_21701, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_21701;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_21701] === this.CELL_EMPTY
                    && map[y+1][xPoint_21701] === this.CELL_EMPTY
                    && map[y+1][xPoint_21701+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_21701] === this.CELL_EMPTY
                    && map[y][xPoint_21701-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_21701-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
