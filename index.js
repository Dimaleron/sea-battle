...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3728=0;yPoint_3728<this.gameFieldBorderY.length; yPoint_3728++){
                for(var xPoint_3728=0;xPoint_3728<this.gameFieldBorderX.length; xPoint_3728++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3728, xPoint_3728);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3728][xPoint_3728] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3728, xPoint_3728, 'user');
                    if(this._userShipsMap[yPoint_3728][xPoint_3728] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3728, xPoint_3728, type){
            var id = this.getPointBlockIdByCoords(yPoint_3728, xPoint_3728, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3728);
                block.setAttribute('data-y', yPoint_3728);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3728
         * @param {type} xPoint_3728
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3728, xPoint_3728, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3728 + '_y' + yPoint_3728;
            }
            return 'pc_x' + xPoint_3728 + '_y' + yPoint_3728;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3728=0;yPoint_3728<this.gameFieldBorderY.length; yPoint_3728++){
                for(var xPoint_3728=0;xPoint_3728<this.gameFieldBorderX.length; xPoint_3728++){
                    map.push({y: yPoint_3728, x: xPoint_3728});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3728=-1;yPoint_3728<(this.gameFieldBorderY.length+1); yPoint_3728++){
                for(var xPoint_3728=-1;xPoint_3728<(this.gameFieldBorderX.length+1); xPoint_3728++){
                    if(!map[yPoint_3728]){
                        map[yPoint_3728] = [];
                    }
                    map[yPoint_3728][xPoint_3728] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3728 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3728 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3728, yPoint_3728) === true){
                    if(this.canPutHorizontal(map, xPoint_3728, yPoint_3728, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3728][xPoint_3728 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3728, yPoint_3728, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3728 + i][xPoint_3728] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3728, yPoint_3728){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3728][xPoint_3728] === this.CELL_EMPTY
                && map[yPoint_3728-1][xPoint_3728] === this.CELL_EMPTY
                && map[yPoint_3728-1][xPoint_3728+1] === this.CELL_EMPTY
                && map[yPoint_3728][xPoint_3728+1] === this.CELL_EMPTY
                && map[yPoint_3728+1][xPoint_3728+1] === this.CELL_EMPTY
                && map[yPoint_3728+1][xPoint_3728] === this.CELL_EMPTY
                && map[yPoint_3728+1][xPoint_3728-1] === this.CELL_EMPTY
                && map[yPoint_3728][xPoint_3728-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3728, yPoint_3728, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3728;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3728][x] === this.CELL_EMPTY
                    && map[yPoint_3728-1][x] === this.CELL_EMPTY
                    && map[yPoint_3728-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3728][x+1] === this.CELL_EMPTY
                    && map[yPoint_3728+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3728+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3728, yPoint_3728, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3728;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3728] === this.CELL_EMPTY
                    && map[y+1][xPoint_3728] === this.CELL_EMPTY
                    && map[y+1][xPoint_3728+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3728] === this.CELL_EMPTY
                    && map[y][xPoint_3728-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3728-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
