...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32297=0;yPoint_32297<this.gameFieldBorderY.length; yPoint_32297++){
                for(var xPoint_32297=0;xPoint_32297<this.gameFieldBorderX.length; xPoint_32297++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32297, xPoint_32297);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32297][xPoint_32297] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32297, xPoint_32297, 'user');
                    if(this._userShipsMap[yPoint_32297][xPoint_32297] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32297, xPoint_32297, type){
            var id = this.getPointBlockIdByCoords(yPoint_32297, xPoint_32297, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32297);
                block.setAttribute('data-y', yPoint_32297);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32297
         * @param {type} xPoint_32297
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32297, xPoint_32297, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32297 + '_y' + yPoint_32297;
            }
            return 'pc_x' + xPoint_32297 + '_y' + yPoint_32297;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32297=0;yPoint_32297<this.gameFieldBorderY.length; yPoint_32297++){
                for(var xPoint_32297=0;xPoint_32297<this.gameFieldBorderX.length; xPoint_32297++){
                    map.push({y: yPoint_32297, x: xPoint_32297});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32297=-1;yPoint_32297<(this.gameFieldBorderY.length+1); yPoint_32297++){
                for(var xPoint_32297=-1;xPoint_32297<(this.gameFieldBorderX.length+1); xPoint_32297++){
                    if(!map[yPoint_32297]){
                        map[yPoint_32297] = [];
                    }
                    map[yPoint_32297][xPoint_32297] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32297 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32297 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32297, yPoint_32297) === true){
                    if(this.canPutHorizontal(map, xPoint_32297, yPoint_32297, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32297][xPoint_32297 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32297, yPoint_32297, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32297 + i][xPoint_32297] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32297, yPoint_32297){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32297][xPoint_32297] === this.CELL_EMPTY
                && map[yPoint_32297-1][xPoint_32297] === this.CELL_EMPTY
                && map[yPoint_32297-1][xPoint_32297+1] === this.CELL_EMPTY
                && map[yPoint_32297][xPoint_32297+1] === this.CELL_EMPTY
                && map[yPoint_32297+1][xPoint_32297+1] === this.CELL_EMPTY
                && map[yPoint_32297+1][xPoint_32297] === this.CELL_EMPTY
                && map[yPoint_32297+1][xPoint_32297-1] === this.CELL_EMPTY
                && map[yPoint_32297][xPoint_32297-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32297, yPoint_32297, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32297;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32297][x] === this.CELL_EMPTY
                    && map[yPoint_32297-1][x] === this.CELL_EMPTY
                    && map[yPoint_32297-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32297][x+1] === this.CELL_EMPTY
                    && map[yPoint_32297+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32297+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32297, yPoint_32297, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32297;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32297] === this.CELL_EMPTY
                    && map[y+1][xPoint_32297] === this.CELL_EMPTY
                    && map[y+1][xPoint_32297+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32297] === this.CELL_EMPTY
                    && map[y][xPoint_32297-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32297-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
