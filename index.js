...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7695=0;yPoint_7695<this.gameFieldBorderY.length; yPoint_7695++){
                for(var xPoint_7695=0;xPoint_7695<this.gameFieldBorderX.length; xPoint_7695++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7695, xPoint_7695);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7695][xPoint_7695] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7695, xPoint_7695, 'user');
                    if(this._userShipsMap[yPoint_7695][xPoint_7695] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7695, xPoint_7695, type){
            var id = this.getPointBlockIdByCoords(yPoint_7695, xPoint_7695, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7695);
                block.setAttribute('data-y', yPoint_7695);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7695
         * @param {type} xPoint_7695
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7695, xPoint_7695, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7695 + '_y' + yPoint_7695;
            }
            return 'pc_x' + xPoint_7695 + '_y' + yPoint_7695;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7695=0;yPoint_7695<this.gameFieldBorderY.length; yPoint_7695++){
                for(var xPoint_7695=0;xPoint_7695<this.gameFieldBorderX.length; xPoint_7695++){
                    map.push({y: yPoint_7695, x: xPoint_7695});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7695=-1;yPoint_7695<(this.gameFieldBorderY.length+1); yPoint_7695++){
                for(var xPoint_7695=-1;xPoint_7695<(this.gameFieldBorderX.length+1); xPoint_7695++){
                    if(!map[yPoint_7695]){
                        map[yPoint_7695] = [];
                    }
                    map[yPoint_7695][xPoint_7695] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7695 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7695 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7695, yPoint_7695) === true){
                    if(this.canPutHorizontal(map, xPoint_7695, yPoint_7695, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7695][xPoint_7695 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7695, yPoint_7695, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7695 + i][xPoint_7695] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7695, yPoint_7695){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7695][xPoint_7695] === this.CELL_EMPTY
                && map[yPoint_7695-1][xPoint_7695] === this.CELL_EMPTY
                && map[yPoint_7695-1][xPoint_7695+1] === this.CELL_EMPTY
                && map[yPoint_7695][xPoint_7695+1] === this.CELL_EMPTY
                && map[yPoint_7695+1][xPoint_7695+1] === this.CELL_EMPTY
                && map[yPoint_7695+1][xPoint_7695] === this.CELL_EMPTY
                && map[yPoint_7695+1][xPoint_7695-1] === this.CELL_EMPTY
                && map[yPoint_7695][xPoint_7695-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7695, yPoint_7695, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7695;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7695][x] === this.CELL_EMPTY
                    && map[yPoint_7695-1][x] === this.CELL_EMPTY
                    && map[yPoint_7695-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7695][x+1] === this.CELL_EMPTY
                    && map[yPoint_7695+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7695+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7695, yPoint_7695, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7695;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7695] === this.CELL_EMPTY
                    && map[y+1][xPoint_7695] === this.CELL_EMPTY
                    && map[y+1][xPoint_7695+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7695] === this.CELL_EMPTY
                    && map[y][xPoint_7695-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7695-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
