...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11361=0;yPoint_11361<this.gameFieldBorderY.length; yPoint_11361++){
                for(var xPoint_11361=0;xPoint_11361<this.gameFieldBorderX.length; xPoint_11361++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11361, xPoint_11361);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11361][xPoint_11361] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11361, xPoint_11361, 'user');
                    if(this._userShipsMap[yPoint_11361][xPoint_11361] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11361, xPoint_11361, type){
            var id = this.getPointBlockIdByCoords(yPoint_11361, xPoint_11361, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11361);
                block.setAttribute('data-y', yPoint_11361);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11361
         * @param {type} xPoint_11361
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11361, xPoint_11361, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11361 + '_y' + yPoint_11361;
            }
            return 'pc_x' + xPoint_11361 + '_y' + yPoint_11361;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11361=0;yPoint_11361<this.gameFieldBorderY.length; yPoint_11361++){
                for(var xPoint_11361=0;xPoint_11361<this.gameFieldBorderX.length; xPoint_11361++){
                    map.push({y: yPoint_11361, x: xPoint_11361});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11361=-1;yPoint_11361<(this.gameFieldBorderY.length+1); yPoint_11361++){
                for(var xPoint_11361=-1;xPoint_11361<(this.gameFieldBorderX.length+1); xPoint_11361++){
                    if(!map[yPoint_11361]){
                        map[yPoint_11361] = [];
                    }
                    map[yPoint_11361][xPoint_11361] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11361 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11361 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11361, yPoint_11361) === true){
                    if(this.canPutHorizontal(map, xPoint_11361, yPoint_11361, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11361][xPoint_11361 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11361, yPoint_11361, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11361 + i][xPoint_11361] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11361, yPoint_11361){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11361][xPoint_11361] === this.CELL_EMPTY
                && map[yPoint_11361-1][xPoint_11361] === this.CELL_EMPTY
                && map[yPoint_11361-1][xPoint_11361+1] === this.CELL_EMPTY
                && map[yPoint_11361][xPoint_11361+1] === this.CELL_EMPTY
                && map[yPoint_11361+1][xPoint_11361+1] === this.CELL_EMPTY
                && map[yPoint_11361+1][xPoint_11361] === this.CELL_EMPTY
                && map[yPoint_11361+1][xPoint_11361-1] === this.CELL_EMPTY
                && map[yPoint_11361][xPoint_11361-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11361, yPoint_11361, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11361;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11361][x] === this.CELL_EMPTY
                    && map[yPoint_11361-1][x] === this.CELL_EMPTY
                    && map[yPoint_11361-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11361][x+1] === this.CELL_EMPTY
                    && map[yPoint_11361+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11361+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11361, yPoint_11361, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11361;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11361] === this.CELL_EMPTY
                    && map[y+1][xPoint_11361] === this.CELL_EMPTY
                    && map[y+1][xPoint_11361+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11361] === this.CELL_EMPTY
                    && map[y][xPoint_11361-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11361-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
