...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_310=0;yPoint_310<this.gameFieldBorderY.length; yPoint_310++){
                for(var xPoint_310=0;xPoint_310<this.gameFieldBorderX.length; xPoint_310++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_310, xPoint_310);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_310][xPoint_310] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_310, xPoint_310, 'user');
                    if(this._userShipsMap[yPoint_310][xPoint_310] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_310, xPoint_310, type){
            var id = this.getPointBlockIdByCoords(yPoint_310, xPoint_310, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_310);
                block.setAttribute('data-y', yPoint_310);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_310
         * @param {type} xPoint_310
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_310, xPoint_310, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_310 + '_y' + yPoint_310;
            }
            return 'pc_x' + xPoint_310 + '_y' + yPoint_310;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_310=0;yPoint_310<this.gameFieldBorderY.length; yPoint_310++){
                for(var xPoint_310=0;xPoint_310<this.gameFieldBorderX.length; xPoint_310++){
                    map.push({y: yPoint_310, x: xPoint_310});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_310=-1;yPoint_310<(this.gameFieldBorderY.length+1); yPoint_310++){
                for(var xPoint_310=-1;xPoint_310<(this.gameFieldBorderX.length+1); xPoint_310++){
                    if(!map[yPoint_310]){
                        map[yPoint_310] = [];
                    }
                    map[yPoint_310][xPoint_310] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_310 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_310 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_310, yPoint_310) === true){
                    if(this.canPutHorizontal(map, xPoint_310, yPoint_310, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_310][xPoint_310 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_310, yPoint_310, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_310 + i][xPoint_310] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_310, yPoint_310){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_310][xPoint_310] === this.CELL_EMPTY
                && map[yPoint_310-1][xPoint_310] === this.CELL_EMPTY
                && map[yPoint_310-1][xPoint_310+1] === this.CELL_EMPTY
                && map[yPoint_310][xPoint_310+1] === this.CELL_EMPTY
                && map[yPoint_310+1][xPoint_310+1] === this.CELL_EMPTY
                && map[yPoint_310+1][xPoint_310] === this.CELL_EMPTY
                && map[yPoint_310+1][xPoint_310-1] === this.CELL_EMPTY
                && map[yPoint_310][xPoint_310-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_310, yPoint_310, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_310;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_310][x] === this.CELL_EMPTY
                    && map[yPoint_310-1][x] === this.CELL_EMPTY
                    && map[yPoint_310-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_310][x+1] === this.CELL_EMPTY
                    && map[yPoint_310+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_310+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_310, yPoint_310, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_310;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_310] === this.CELL_EMPTY
                    && map[y+1][xPoint_310] === this.CELL_EMPTY
                    && map[y+1][xPoint_310+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_310] === this.CELL_EMPTY
                    && map[y][xPoint_310-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_310-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
