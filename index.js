...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20655=0;yPoint_20655<this.gameFieldBorderY.length; yPoint_20655++){
                for(var xPoint_20655=0;xPoint_20655<this.gameFieldBorderX.length; xPoint_20655++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20655, xPoint_20655);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20655][xPoint_20655] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20655, xPoint_20655, 'user');
                    if(this._userShipsMap[yPoint_20655][xPoint_20655] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20655, xPoint_20655, type){
            var id = this.getPointBlockIdByCoords(yPoint_20655, xPoint_20655, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20655);
                block.setAttribute('data-y', yPoint_20655);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20655
         * @param {type} xPoint_20655
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20655, xPoint_20655, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20655 + '_y' + yPoint_20655;
            }
            return 'pc_x' + xPoint_20655 + '_y' + yPoint_20655;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20655=0;yPoint_20655<this.gameFieldBorderY.length; yPoint_20655++){
                for(var xPoint_20655=0;xPoint_20655<this.gameFieldBorderX.length; xPoint_20655++){
                    map.push({y: yPoint_20655, x: xPoint_20655});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20655=-1;yPoint_20655<(this.gameFieldBorderY.length+1); yPoint_20655++){
                for(var xPoint_20655=-1;xPoint_20655<(this.gameFieldBorderX.length+1); xPoint_20655++){
                    if(!map[yPoint_20655]){
                        map[yPoint_20655] = [];
                    }
                    map[yPoint_20655][xPoint_20655] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20655 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20655 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20655, yPoint_20655) === true){
                    if(this.canPutHorizontal(map, xPoint_20655, yPoint_20655, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20655][xPoint_20655 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20655, yPoint_20655, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20655 + i][xPoint_20655] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20655, yPoint_20655){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20655][xPoint_20655] === this.CELL_EMPTY
                && map[yPoint_20655-1][xPoint_20655] === this.CELL_EMPTY
                && map[yPoint_20655-1][xPoint_20655+1] === this.CELL_EMPTY
                && map[yPoint_20655][xPoint_20655+1] === this.CELL_EMPTY
                && map[yPoint_20655+1][xPoint_20655+1] === this.CELL_EMPTY
                && map[yPoint_20655+1][xPoint_20655] === this.CELL_EMPTY
                && map[yPoint_20655+1][xPoint_20655-1] === this.CELL_EMPTY
                && map[yPoint_20655][xPoint_20655-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20655, yPoint_20655, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20655;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20655][x] === this.CELL_EMPTY
                    && map[yPoint_20655-1][x] === this.CELL_EMPTY
                    && map[yPoint_20655-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20655][x+1] === this.CELL_EMPTY
                    && map[yPoint_20655+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20655+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20655, yPoint_20655, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20655;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20655] === this.CELL_EMPTY
                    && map[y+1][xPoint_20655] === this.CELL_EMPTY
                    && map[y+1][xPoint_20655+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20655] === this.CELL_EMPTY
                    && map[y][xPoint_20655-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20655-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
