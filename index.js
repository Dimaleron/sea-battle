...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20662=0;yPoint_20662<this.gameFieldBorderY.length; yPoint_20662++){
                for(var xPoint_20662=0;xPoint_20662<this.gameFieldBorderX.length; xPoint_20662++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20662, xPoint_20662);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20662][xPoint_20662] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20662, xPoint_20662, 'user');
                    if(this._userShipsMap[yPoint_20662][xPoint_20662] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20662, xPoint_20662, type){
            var id = this.getPointBlockIdByCoords(yPoint_20662, xPoint_20662, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20662);
                block.setAttribute('data-y', yPoint_20662);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20662
         * @param {type} xPoint_20662
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20662, xPoint_20662, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20662 + '_y' + yPoint_20662;
            }
            return 'pc_x' + xPoint_20662 + '_y' + yPoint_20662;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20662=0;yPoint_20662<this.gameFieldBorderY.length; yPoint_20662++){
                for(var xPoint_20662=0;xPoint_20662<this.gameFieldBorderX.length; xPoint_20662++){
                    map.push({y: yPoint_20662, x: xPoint_20662});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20662=-1;yPoint_20662<(this.gameFieldBorderY.length+1); yPoint_20662++){
                for(var xPoint_20662=-1;xPoint_20662<(this.gameFieldBorderX.length+1); xPoint_20662++){
                    if(!map[yPoint_20662]){
                        map[yPoint_20662] = [];
                    }
                    map[yPoint_20662][xPoint_20662] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20662 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20662 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20662, yPoint_20662) === true){
                    if(this.canPutHorizontal(map, xPoint_20662, yPoint_20662, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20662][xPoint_20662 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20662, yPoint_20662, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20662 + i][xPoint_20662] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20662, yPoint_20662){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20662][xPoint_20662] === this.CELL_EMPTY
                && map[yPoint_20662-1][xPoint_20662] === this.CELL_EMPTY
                && map[yPoint_20662-1][xPoint_20662+1] === this.CELL_EMPTY
                && map[yPoint_20662][xPoint_20662+1] === this.CELL_EMPTY
                && map[yPoint_20662+1][xPoint_20662+1] === this.CELL_EMPTY
                && map[yPoint_20662+1][xPoint_20662] === this.CELL_EMPTY
                && map[yPoint_20662+1][xPoint_20662-1] === this.CELL_EMPTY
                && map[yPoint_20662][xPoint_20662-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20662, yPoint_20662, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20662;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20662][x] === this.CELL_EMPTY
                    && map[yPoint_20662-1][x] === this.CELL_EMPTY
                    && map[yPoint_20662-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20662][x+1] === this.CELL_EMPTY
                    && map[yPoint_20662+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20662+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20662, yPoint_20662, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20662;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20662] === this.CELL_EMPTY
                    && map[y+1][xPoint_20662] === this.CELL_EMPTY
                    && map[y+1][xPoint_20662+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20662] === this.CELL_EMPTY
                    && map[y][xPoint_20662-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20662-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
