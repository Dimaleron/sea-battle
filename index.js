...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31165=0;yPoint_31165<this.gameFieldBorderY.length; yPoint_31165++){
                for(var xPoint_31165=0;xPoint_31165<this.gameFieldBorderX.length; xPoint_31165++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31165, xPoint_31165);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31165][xPoint_31165] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31165, xPoint_31165, 'user');
                    if(this._userShipsMap[yPoint_31165][xPoint_31165] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31165, xPoint_31165, type){
            var id = this.getPointBlockIdByCoords(yPoint_31165, xPoint_31165, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31165);
                block.setAttribute('data-y', yPoint_31165);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31165
         * @param {type} xPoint_31165
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31165, xPoint_31165, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31165 + '_y' + yPoint_31165;
            }
            return 'pc_x' + xPoint_31165 + '_y' + yPoint_31165;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31165=0;yPoint_31165<this.gameFieldBorderY.length; yPoint_31165++){
                for(var xPoint_31165=0;xPoint_31165<this.gameFieldBorderX.length; xPoint_31165++){
                    map.push({y: yPoint_31165, x: xPoint_31165});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31165=-1;yPoint_31165<(this.gameFieldBorderY.length+1); yPoint_31165++){
                for(var xPoint_31165=-1;xPoint_31165<(this.gameFieldBorderX.length+1); xPoint_31165++){
                    if(!map[yPoint_31165]){
                        map[yPoint_31165] = [];
                    }
                    map[yPoint_31165][xPoint_31165] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31165 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31165 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31165, yPoint_31165) === true){
                    if(this.canPutHorizontal(map, xPoint_31165, yPoint_31165, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31165][xPoint_31165 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31165, yPoint_31165, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31165 + i][xPoint_31165] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31165, yPoint_31165){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31165][xPoint_31165] === this.CELL_EMPTY
                && map[yPoint_31165-1][xPoint_31165] === this.CELL_EMPTY
                && map[yPoint_31165-1][xPoint_31165+1] === this.CELL_EMPTY
                && map[yPoint_31165][xPoint_31165+1] === this.CELL_EMPTY
                && map[yPoint_31165+1][xPoint_31165+1] === this.CELL_EMPTY
                && map[yPoint_31165+1][xPoint_31165] === this.CELL_EMPTY
                && map[yPoint_31165+1][xPoint_31165-1] === this.CELL_EMPTY
                && map[yPoint_31165][xPoint_31165-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31165, yPoint_31165, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31165;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31165][x] === this.CELL_EMPTY
                    && map[yPoint_31165-1][x] === this.CELL_EMPTY
                    && map[yPoint_31165-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31165][x+1] === this.CELL_EMPTY
                    && map[yPoint_31165+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31165+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31165, yPoint_31165, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31165;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31165] === this.CELL_EMPTY
                    && map[y+1][xPoint_31165] === this.CELL_EMPTY
                    && map[y+1][xPoint_31165+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31165] === this.CELL_EMPTY
                    && map[y][xPoint_31165-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31165-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
