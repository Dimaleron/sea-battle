...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4966=0;yPoint_4966<this.gameFieldBorderY.length; yPoint_4966++){
                for(var xPoint_4966=0;xPoint_4966<this.gameFieldBorderX.length; xPoint_4966++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4966, xPoint_4966);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4966][xPoint_4966] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4966, xPoint_4966, 'user');
                    if(this._userShipsMap[yPoint_4966][xPoint_4966] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4966, xPoint_4966, type){
            var id = this.getPointBlockIdByCoords(yPoint_4966, xPoint_4966, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4966);
                block.setAttribute('data-y', yPoint_4966);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4966
         * @param {type} xPoint_4966
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4966, xPoint_4966, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4966 + '_y' + yPoint_4966;
            }
            return 'pc_x' + xPoint_4966 + '_y' + yPoint_4966;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4966=0;yPoint_4966<this.gameFieldBorderY.length; yPoint_4966++){
                for(var xPoint_4966=0;xPoint_4966<this.gameFieldBorderX.length; xPoint_4966++){
                    map.push({y: yPoint_4966, x: xPoint_4966});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4966=-1;yPoint_4966<(this.gameFieldBorderY.length+1); yPoint_4966++){
                for(var xPoint_4966=-1;xPoint_4966<(this.gameFieldBorderX.length+1); xPoint_4966++){
                    if(!map[yPoint_4966]){
                        map[yPoint_4966] = [];
                    }
                    map[yPoint_4966][xPoint_4966] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4966 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4966 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4966, yPoint_4966) === true){
                    if(this.canPutHorizontal(map, xPoint_4966, yPoint_4966, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4966][xPoint_4966 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4966, yPoint_4966, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4966 + i][xPoint_4966] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4966, yPoint_4966){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4966][xPoint_4966] === this.CELL_EMPTY
                && map[yPoint_4966-1][xPoint_4966] === this.CELL_EMPTY
                && map[yPoint_4966-1][xPoint_4966+1] === this.CELL_EMPTY
                && map[yPoint_4966][xPoint_4966+1] === this.CELL_EMPTY
                && map[yPoint_4966+1][xPoint_4966+1] === this.CELL_EMPTY
                && map[yPoint_4966+1][xPoint_4966] === this.CELL_EMPTY
                && map[yPoint_4966+1][xPoint_4966-1] === this.CELL_EMPTY
                && map[yPoint_4966][xPoint_4966-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4966, yPoint_4966, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4966;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4966][x] === this.CELL_EMPTY
                    && map[yPoint_4966-1][x] === this.CELL_EMPTY
                    && map[yPoint_4966-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4966][x+1] === this.CELL_EMPTY
                    && map[yPoint_4966+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4966+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4966, yPoint_4966, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4966;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4966] === this.CELL_EMPTY
                    && map[y+1][xPoint_4966] === this.CELL_EMPTY
                    && map[y+1][xPoint_4966+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4966] === this.CELL_EMPTY
                    && map[y][xPoint_4966-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4966-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
