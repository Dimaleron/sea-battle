...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5165=0;yPoint_5165<this.gameFieldBorderY.length; yPoint_5165++){
                for(var xPoint_5165=0;xPoint_5165<this.gameFieldBorderX.length; xPoint_5165++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5165, xPoint_5165);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5165][xPoint_5165] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5165, xPoint_5165, 'user');
                    if(this._userShipsMap[yPoint_5165][xPoint_5165] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5165, xPoint_5165, type){
            var id = this.getPointBlockIdByCoords(yPoint_5165, xPoint_5165, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5165);
                block.setAttribute('data-y', yPoint_5165);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5165
         * @param {type} xPoint_5165
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5165, xPoint_5165, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5165 + '_y' + yPoint_5165;
            }
            return 'pc_x' + xPoint_5165 + '_y' + yPoint_5165;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5165=0;yPoint_5165<this.gameFieldBorderY.length; yPoint_5165++){
                for(var xPoint_5165=0;xPoint_5165<this.gameFieldBorderX.length; xPoint_5165++){
                    map.push({y: yPoint_5165, x: xPoint_5165});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5165=-1;yPoint_5165<(this.gameFieldBorderY.length+1); yPoint_5165++){
                for(var xPoint_5165=-1;xPoint_5165<(this.gameFieldBorderX.length+1); xPoint_5165++){
                    if(!map[yPoint_5165]){
                        map[yPoint_5165] = [];
                    }
                    map[yPoint_5165][xPoint_5165] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5165 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5165 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5165, yPoint_5165) === true){
                    if(this.canPutHorizontal(map, xPoint_5165, yPoint_5165, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5165][xPoint_5165 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5165, yPoint_5165, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5165 + i][xPoint_5165] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5165, yPoint_5165){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5165][xPoint_5165] === this.CELL_EMPTY
                && map[yPoint_5165-1][xPoint_5165] === this.CELL_EMPTY
                && map[yPoint_5165-1][xPoint_5165+1] === this.CELL_EMPTY
                && map[yPoint_5165][xPoint_5165+1] === this.CELL_EMPTY
                && map[yPoint_5165+1][xPoint_5165+1] === this.CELL_EMPTY
                && map[yPoint_5165+1][xPoint_5165] === this.CELL_EMPTY
                && map[yPoint_5165+1][xPoint_5165-1] === this.CELL_EMPTY
                && map[yPoint_5165][xPoint_5165-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5165, yPoint_5165, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5165;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5165][x] === this.CELL_EMPTY
                    && map[yPoint_5165-1][x] === this.CELL_EMPTY
                    && map[yPoint_5165-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5165][x+1] === this.CELL_EMPTY
                    && map[yPoint_5165+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5165+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5165, yPoint_5165, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5165;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5165] === this.CELL_EMPTY
                    && map[y+1][xPoint_5165] === this.CELL_EMPTY
                    && map[y+1][xPoint_5165+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5165] === this.CELL_EMPTY
                    && map[y][xPoint_5165-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5165-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
