...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18677=0;yPoint_18677<this.gameFieldBorderY.length; yPoint_18677++){
                for(var xPoint_18677=0;xPoint_18677<this.gameFieldBorderX.length; xPoint_18677++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18677, xPoint_18677);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18677][xPoint_18677] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18677, xPoint_18677, 'user');
                    if(this._userShipsMap[yPoint_18677][xPoint_18677] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18677, xPoint_18677, type){
            var id = this.getPointBlockIdByCoords(yPoint_18677, xPoint_18677, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18677);
                block.setAttribute('data-y', yPoint_18677);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18677
         * @param {type} xPoint_18677
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18677, xPoint_18677, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18677 + '_y' + yPoint_18677;
            }
            return 'pc_x' + xPoint_18677 + '_y' + yPoint_18677;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18677=0;yPoint_18677<this.gameFieldBorderY.length; yPoint_18677++){
                for(var xPoint_18677=0;xPoint_18677<this.gameFieldBorderX.length; xPoint_18677++){
                    map.push({y: yPoint_18677, x: xPoint_18677});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18677=-1;yPoint_18677<(this.gameFieldBorderY.length+1); yPoint_18677++){
                for(var xPoint_18677=-1;xPoint_18677<(this.gameFieldBorderX.length+1); xPoint_18677++){
                    if(!map[yPoint_18677]){
                        map[yPoint_18677] = [];
                    }
                    map[yPoint_18677][xPoint_18677] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18677 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18677 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18677, yPoint_18677) === true){
                    if(this.canPutHorizontal(map, xPoint_18677, yPoint_18677, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18677][xPoint_18677 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18677, yPoint_18677, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18677 + i][xPoint_18677] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18677, yPoint_18677){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18677][xPoint_18677] === this.CELL_EMPTY
                && map[yPoint_18677-1][xPoint_18677] === this.CELL_EMPTY
                && map[yPoint_18677-1][xPoint_18677+1] === this.CELL_EMPTY
                && map[yPoint_18677][xPoint_18677+1] === this.CELL_EMPTY
                && map[yPoint_18677+1][xPoint_18677+1] === this.CELL_EMPTY
                && map[yPoint_18677+1][xPoint_18677] === this.CELL_EMPTY
                && map[yPoint_18677+1][xPoint_18677-1] === this.CELL_EMPTY
                && map[yPoint_18677][xPoint_18677-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18677, yPoint_18677, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18677;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18677][x] === this.CELL_EMPTY
                    && map[yPoint_18677-1][x] === this.CELL_EMPTY
                    && map[yPoint_18677-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18677][x+1] === this.CELL_EMPTY
                    && map[yPoint_18677+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18677+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18677, yPoint_18677, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18677;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18677] === this.CELL_EMPTY
                    && map[y+1][xPoint_18677] === this.CELL_EMPTY
                    && map[y+1][xPoint_18677+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18677] === this.CELL_EMPTY
                    && map[y][xPoint_18677-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18677-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
