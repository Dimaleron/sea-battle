...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11102=0;yPoint_11102<this.gameFieldBorderY.length; yPoint_11102++){
                for(var xPoint_11102=0;xPoint_11102<this.gameFieldBorderX.length; xPoint_11102++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11102, xPoint_11102);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11102][xPoint_11102] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11102, xPoint_11102, 'user');
                    if(this._userShipsMap[yPoint_11102][xPoint_11102] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11102, xPoint_11102, type){
            var id = this.getPointBlockIdByCoords(yPoint_11102, xPoint_11102, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11102);
                block.setAttribute('data-y', yPoint_11102);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11102
         * @param {type} xPoint_11102
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11102, xPoint_11102, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11102 + '_y' + yPoint_11102;
            }
            return 'pc_x' + xPoint_11102 + '_y' + yPoint_11102;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11102=0;yPoint_11102<this.gameFieldBorderY.length; yPoint_11102++){
                for(var xPoint_11102=0;xPoint_11102<this.gameFieldBorderX.length; xPoint_11102++){
                    map.push({y: yPoint_11102, x: xPoint_11102});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11102=-1;yPoint_11102<(this.gameFieldBorderY.length+1); yPoint_11102++){
                for(var xPoint_11102=-1;xPoint_11102<(this.gameFieldBorderX.length+1); xPoint_11102++){
                    if(!map[yPoint_11102]){
                        map[yPoint_11102] = [];
                    }
                    map[yPoint_11102][xPoint_11102] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11102 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11102 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11102, yPoint_11102) === true){
                    if(this.canPutHorizontal(map, xPoint_11102, yPoint_11102, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11102][xPoint_11102 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11102, yPoint_11102, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11102 + i][xPoint_11102] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11102, yPoint_11102){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11102][xPoint_11102] === this.CELL_EMPTY
                && map[yPoint_11102-1][xPoint_11102] === this.CELL_EMPTY
                && map[yPoint_11102-1][xPoint_11102+1] === this.CELL_EMPTY
                && map[yPoint_11102][xPoint_11102+1] === this.CELL_EMPTY
                && map[yPoint_11102+1][xPoint_11102+1] === this.CELL_EMPTY
                && map[yPoint_11102+1][xPoint_11102] === this.CELL_EMPTY
                && map[yPoint_11102+1][xPoint_11102-1] === this.CELL_EMPTY
                && map[yPoint_11102][xPoint_11102-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11102, yPoint_11102, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11102;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11102][x] === this.CELL_EMPTY
                    && map[yPoint_11102-1][x] === this.CELL_EMPTY
                    && map[yPoint_11102-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11102][x+1] === this.CELL_EMPTY
                    && map[yPoint_11102+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11102+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11102, yPoint_11102, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11102;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11102] === this.CELL_EMPTY
                    && map[y+1][xPoint_11102] === this.CELL_EMPTY
                    && map[y+1][xPoint_11102+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11102] === this.CELL_EMPTY
                    && map[y][xPoint_11102-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11102-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
