...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_2984=0;yPoint_2984<this.gameFieldBorderY.length; yPoint_2984++){
                for(var xPoint_2984=0;xPoint_2984<this.gameFieldBorderX.length; xPoint_2984++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_2984, xPoint_2984);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_2984][xPoint_2984] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_2984, xPoint_2984, 'user');
                    if(this._userShipsMap[yPoint_2984][xPoint_2984] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_2984, xPoint_2984, type){
            var id = this.getPointBlockIdByCoords(yPoint_2984, xPoint_2984, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_2984);
                block.setAttribute('data-y', yPoint_2984);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_2984
         * @param {type} xPoint_2984
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_2984, xPoint_2984, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_2984 + '_y' + yPoint_2984;
            }
            return 'pc_x' + xPoint_2984 + '_y' + yPoint_2984;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_2984=0;yPoint_2984<this.gameFieldBorderY.length; yPoint_2984++){
                for(var xPoint_2984=0;xPoint_2984<this.gameFieldBorderX.length; xPoint_2984++){
                    map.push({y: yPoint_2984, x: xPoint_2984});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_2984=-1;yPoint_2984<(this.gameFieldBorderY.length+1); yPoint_2984++){
                for(var xPoint_2984=-1;xPoint_2984<(this.gameFieldBorderX.length+1); xPoint_2984++){
                    if(!map[yPoint_2984]){
                        map[yPoint_2984] = [];
                    }
                    map[yPoint_2984][xPoint_2984] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_2984 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_2984 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_2984, yPoint_2984) === true){
                    if(this.canPutHorizontal(map, xPoint_2984, yPoint_2984, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2984][xPoint_2984 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_2984, yPoint_2984, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_2984 + i][xPoint_2984] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_2984, yPoint_2984){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_2984][xPoint_2984] === this.CELL_EMPTY
                && map[yPoint_2984-1][xPoint_2984] === this.CELL_EMPTY
                && map[yPoint_2984-1][xPoint_2984+1] === this.CELL_EMPTY
                && map[yPoint_2984][xPoint_2984+1] === this.CELL_EMPTY
                && map[yPoint_2984+1][xPoint_2984+1] === this.CELL_EMPTY
                && map[yPoint_2984+1][xPoint_2984] === this.CELL_EMPTY
                && map[yPoint_2984+1][xPoint_2984-1] === this.CELL_EMPTY
                && map[yPoint_2984][xPoint_2984-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_2984, yPoint_2984, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_2984;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_2984][x] === this.CELL_EMPTY
                    && map[yPoint_2984-1][x] === this.CELL_EMPTY
                    && map[yPoint_2984-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2984][x+1] === this.CELL_EMPTY
                    && map[yPoint_2984+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_2984+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_2984, yPoint_2984, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_2984;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_2984] === this.CELL_EMPTY
                    && map[y+1][xPoint_2984] === this.CELL_EMPTY
                    && map[y+1][xPoint_2984+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_2984] === this.CELL_EMPTY
                    && map[y][xPoint_2984-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_2984-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
