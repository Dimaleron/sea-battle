...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19085=0;yPoint_19085<this.gameFieldBorderY.length; yPoint_19085++){
                for(var xPoint_19085=0;xPoint_19085<this.gameFieldBorderX.length; xPoint_19085++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19085, xPoint_19085);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19085][xPoint_19085] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19085, xPoint_19085, 'user');
                    if(this._userShipsMap[yPoint_19085][xPoint_19085] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19085, xPoint_19085, type){
            var id = this.getPointBlockIdByCoords(yPoint_19085, xPoint_19085, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19085);
                block.setAttribute('data-y', yPoint_19085);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19085
         * @param {type} xPoint_19085
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19085, xPoint_19085, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19085 + '_y' + yPoint_19085;
            }
            return 'pc_x' + xPoint_19085 + '_y' + yPoint_19085;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19085=0;yPoint_19085<this.gameFieldBorderY.length; yPoint_19085++){
                for(var xPoint_19085=0;xPoint_19085<this.gameFieldBorderX.length; xPoint_19085++){
                    map.push({y: yPoint_19085, x: xPoint_19085});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19085=-1;yPoint_19085<(this.gameFieldBorderY.length+1); yPoint_19085++){
                for(var xPoint_19085=-1;xPoint_19085<(this.gameFieldBorderX.length+1); xPoint_19085++){
                    if(!map[yPoint_19085]){
                        map[yPoint_19085] = [];
                    }
                    map[yPoint_19085][xPoint_19085] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19085 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19085 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19085, yPoint_19085) === true){
                    if(this.canPutHorizontal(map, xPoint_19085, yPoint_19085, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19085][xPoint_19085 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19085, yPoint_19085, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19085 + i][xPoint_19085] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19085, yPoint_19085){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19085][xPoint_19085] === this.CELL_EMPTY
                && map[yPoint_19085-1][xPoint_19085] === this.CELL_EMPTY
                && map[yPoint_19085-1][xPoint_19085+1] === this.CELL_EMPTY
                && map[yPoint_19085][xPoint_19085+1] === this.CELL_EMPTY
                && map[yPoint_19085+1][xPoint_19085+1] === this.CELL_EMPTY
                && map[yPoint_19085+1][xPoint_19085] === this.CELL_EMPTY
                && map[yPoint_19085+1][xPoint_19085-1] === this.CELL_EMPTY
                && map[yPoint_19085][xPoint_19085-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19085, yPoint_19085, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19085;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19085][x] === this.CELL_EMPTY
                    && map[yPoint_19085-1][x] === this.CELL_EMPTY
                    && map[yPoint_19085-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19085][x+1] === this.CELL_EMPTY
                    && map[yPoint_19085+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19085+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19085, yPoint_19085, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19085;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19085] === this.CELL_EMPTY
                    && map[y+1][xPoint_19085] === this.CELL_EMPTY
                    && map[y+1][xPoint_19085+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19085] === this.CELL_EMPTY
                    && map[y][xPoint_19085-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19085-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
