...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11608=0;yPoint_11608<this.gameFieldBorderY.length; yPoint_11608++){
                for(var xPoint_11608=0;xPoint_11608<this.gameFieldBorderX.length; xPoint_11608++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11608, xPoint_11608);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11608][xPoint_11608] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11608, xPoint_11608, 'user');
                    if(this._userShipsMap[yPoint_11608][xPoint_11608] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11608, xPoint_11608, type){
            var id = this.getPointBlockIdByCoords(yPoint_11608, xPoint_11608, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11608);
                block.setAttribute('data-y', yPoint_11608);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11608
         * @param {type} xPoint_11608
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11608, xPoint_11608, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11608 + '_y' + yPoint_11608;
            }
            return 'pc_x' + xPoint_11608 + '_y' + yPoint_11608;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11608=0;yPoint_11608<this.gameFieldBorderY.length; yPoint_11608++){
                for(var xPoint_11608=0;xPoint_11608<this.gameFieldBorderX.length; xPoint_11608++){
                    map.push({y: yPoint_11608, x: xPoint_11608});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11608=-1;yPoint_11608<(this.gameFieldBorderY.length+1); yPoint_11608++){
                for(var xPoint_11608=-1;xPoint_11608<(this.gameFieldBorderX.length+1); xPoint_11608++){
                    if(!map[yPoint_11608]){
                        map[yPoint_11608] = [];
                    }
                    map[yPoint_11608][xPoint_11608] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11608 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11608 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11608, yPoint_11608) === true){
                    if(this.canPutHorizontal(map, xPoint_11608, yPoint_11608, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11608][xPoint_11608 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11608, yPoint_11608, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11608 + i][xPoint_11608] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11608, yPoint_11608){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11608][xPoint_11608] === this.CELL_EMPTY
                && map[yPoint_11608-1][xPoint_11608] === this.CELL_EMPTY
                && map[yPoint_11608-1][xPoint_11608+1] === this.CELL_EMPTY
                && map[yPoint_11608][xPoint_11608+1] === this.CELL_EMPTY
                && map[yPoint_11608+1][xPoint_11608+1] === this.CELL_EMPTY
                && map[yPoint_11608+1][xPoint_11608] === this.CELL_EMPTY
                && map[yPoint_11608+1][xPoint_11608-1] === this.CELL_EMPTY
                && map[yPoint_11608][xPoint_11608-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11608, yPoint_11608, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11608;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11608][x] === this.CELL_EMPTY
                    && map[yPoint_11608-1][x] === this.CELL_EMPTY
                    && map[yPoint_11608-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11608][x+1] === this.CELL_EMPTY
                    && map[yPoint_11608+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11608+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11608, yPoint_11608, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11608;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11608] === this.CELL_EMPTY
                    && map[y+1][xPoint_11608] === this.CELL_EMPTY
                    && map[y+1][xPoint_11608+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11608] === this.CELL_EMPTY
                    && map[y][xPoint_11608-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11608-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
