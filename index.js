...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_734=0;yPoint_734<this.gameFieldBorderY.length; yPoint_734++){
                for(var xPoint_734=0;xPoint_734<this.gameFieldBorderX.length; xPoint_734++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_734, xPoint_734);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_734][xPoint_734] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_734, xPoint_734, 'user');
                    if(this._userShipsMap[yPoint_734][xPoint_734] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_734, xPoint_734, type){
            var id = this.getPointBlockIdByCoords(yPoint_734, xPoint_734, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_734);
                block.setAttribute('data-y', yPoint_734);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_734
         * @param {type} xPoint_734
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_734, xPoint_734, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_734 + '_y' + yPoint_734;
            }
            return 'pc_x' + xPoint_734 + '_y' + yPoint_734;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_734=0;yPoint_734<this.gameFieldBorderY.length; yPoint_734++){
                for(var xPoint_734=0;xPoint_734<this.gameFieldBorderX.length; xPoint_734++){
                    map.push({y: yPoint_734, x: xPoint_734});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_734=-1;yPoint_734<(this.gameFieldBorderY.length+1); yPoint_734++){
                for(var xPoint_734=-1;xPoint_734<(this.gameFieldBorderX.length+1); xPoint_734++){
                    if(!map[yPoint_734]){
                        map[yPoint_734] = [];
                    }
                    map[yPoint_734][xPoint_734] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_734 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_734 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_734, yPoint_734) === true){
                    if(this.canPutHorizontal(map, xPoint_734, yPoint_734, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_734][xPoint_734 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_734, yPoint_734, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_734 + i][xPoint_734] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_734, yPoint_734){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_734][xPoint_734] === this.CELL_EMPTY
                && map[yPoint_734-1][xPoint_734] === this.CELL_EMPTY
                && map[yPoint_734-1][xPoint_734+1] === this.CELL_EMPTY
                && map[yPoint_734][xPoint_734+1] === this.CELL_EMPTY
                && map[yPoint_734+1][xPoint_734+1] === this.CELL_EMPTY
                && map[yPoint_734+1][xPoint_734] === this.CELL_EMPTY
                && map[yPoint_734+1][xPoint_734-1] === this.CELL_EMPTY
                && map[yPoint_734][xPoint_734-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_734, yPoint_734, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_734;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_734][x] === this.CELL_EMPTY
                    && map[yPoint_734-1][x] === this.CELL_EMPTY
                    && map[yPoint_734-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_734][x+1] === this.CELL_EMPTY
                    && map[yPoint_734+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_734+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_734, yPoint_734, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_734;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_734] === this.CELL_EMPTY
                    && map[y+1][xPoint_734] === this.CELL_EMPTY
                    && map[y+1][xPoint_734+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_734] === this.CELL_EMPTY
                    && map[y][xPoint_734-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_734-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
