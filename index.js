...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_950=0;yPoint_950<this.gameFieldBorderY.length; yPoint_950++){
                for(var xPoint_950=0;xPoint_950<this.gameFieldBorderX.length; xPoint_950++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_950, xPoint_950);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_950][xPoint_950] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_950, xPoint_950, 'user');
                    if(this._userShipsMap[yPoint_950][xPoint_950] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_950, xPoint_950, type){
            var id = this.getPointBlockIdByCoords(yPoint_950, xPoint_950, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_950);
                block.setAttribute('data-y', yPoint_950);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_950
         * @param {type} xPoint_950
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_950, xPoint_950, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_950 + '_y' + yPoint_950;
            }
            return 'pc_x' + xPoint_950 + '_y' + yPoint_950;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_950=0;yPoint_950<this.gameFieldBorderY.length; yPoint_950++){
                for(var xPoint_950=0;xPoint_950<this.gameFieldBorderX.length; xPoint_950++){
                    map.push({y: yPoint_950, x: xPoint_950});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_950=-1;yPoint_950<(this.gameFieldBorderY.length+1); yPoint_950++){
                for(var xPoint_950=-1;xPoint_950<(this.gameFieldBorderX.length+1); xPoint_950++){
                    if(!map[yPoint_950]){
                        map[yPoint_950] = [];
                    }
                    map[yPoint_950][xPoint_950] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_950 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_950 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_950, yPoint_950) === true){
                    if(this.canPutHorizontal(map, xPoint_950, yPoint_950, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_950][xPoint_950 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_950, yPoint_950, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_950 + i][xPoint_950] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_950, yPoint_950){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_950][xPoint_950] === this.CELL_EMPTY
                && map[yPoint_950-1][xPoint_950] === this.CELL_EMPTY
                && map[yPoint_950-1][xPoint_950+1] === this.CELL_EMPTY
                && map[yPoint_950][xPoint_950+1] === this.CELL_EMPTY
                && map[yPoint_950+1][xPoint_950+1] === this.CELL_EMPTY
                && map[yPoint_950+1][xPoint_950] === this.CELL_EMPTY
                && map[yPoint_950+1][xPoint_950-1] === this.CELL_EMPTY
                && map[yPoint_950][xPoint_950-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_950, yPoint_950, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_950;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_950][x] === this.CELL_EMPTY
                    && map[yPoint_950-1][x] === this.CELL_EMPTY
                    && map[yPoint_950-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_950][x+1] === this.CELL_EMPTY
                    && map[yPoint_950+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_950+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_950, yPoint_950, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_950;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_950] === this.CELL_EMPTY
                    && map[y+1][xPoint_950] === this.CELL_EMPTY
                    && map[y+1][xPoint_950+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_950] === this.CELL_EMPTY
                    && map[y][xPoint_950-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_950-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
