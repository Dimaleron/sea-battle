...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_26374=0;yPoint_26374<this.gameFieldBorderY.length; yPoint_26374++){
                for(var xPoint_26374=0;xPoint_26374<this.gameFieldBorderX.length; xPoint_26374++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_26374, xPoint_26374);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_26374][xPoint_26374] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_26374, xPoint_26374, 'user');
                    if(this._userShipsMap[yPoint_26374][xPoint_26374] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_26374, xPoint_26374, type){
            var id = this.getPointBlockIdByCoords(yPoint_26374, xPoint_26374, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_26374);
                block.setAttribute('data-y', yPoint_26374);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_26374
         * @param {type} xPoint_26374
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_26374, xPoint_26374, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_26374 + '_y' + yPoint_26374;
            }
            return 'pc_x' + xPoint_26374 + '_y' + yPoint_26374;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_26374=0;yPoint_26374<this.gameFieldBorderY.length; yPoint_26374++){
                for(var xPoint_26374=0;xPoint_26374<this.gameFieldBorderX.length; xPoint_26374++){
                    map.push({y: yPoint_26374, x: xPoint_26374});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_26374=-1;yPoint_26374<(this.gameFieldBorderY.length+1); yPoint_26374++){
                for(var xPoint_26374=-1;xPoint_26374<(this.gameFieldBorderX.length+1); xPoint_26374++){
                    if(!map[yPoint_26374]){
                        map[yPoint_26374] = [];
                    }
                    map[yPoint_26374][xPoint_26374] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_26374 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_26374 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_26374, yPoint_26374) === true){
                    if(this.canPutHorizontal(map, xPoint_26374, yPoint_26374, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26374][xPoint_26374 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_26374, yPoint_26374, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_26374 + i][xPoint_26374] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_26374, yPoint_26374){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_26374][xPoint_26374] === this.CELL_EMPTY
                && map[yPoint_26374-1][xPoint_26374] === this.CELL_EMPTY
                && map[yPoint_26374-1][xPoint_26374+1] === this.CELL_EMPTY
                && map[yPoint_26374][xPoint_26374+1] === this.CELL_EMPTY
                && map[yPoint_26374+1][xPoint_26374+1] === this.CELL_EMPTY
                && map[yPoint_26374+1][xPoint_26374] === this.CELL_EMPTY
                && map[yPoint_26374+1][xPoint_26374-1] === this.CELL_EMPTY
                && map[yPoint_26374][xPoint_26374-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_26374, yPoint_26374, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_26374;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_26374][x] === this.CELL_EMPTY
                    && map[yPoint_26374-1][x] === this.CELL_EMPTY
                    && map[yPoint_26374-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26374][x+1] === this.CELL_EMPTY
                    && map[yPoint_26374+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_26374+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_26374, yPoint_26374, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_26374;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_26374] === this.CELL_EMPTY
                    && map[y+1][xPoint_26374] === this.CELL_EMPTY
                    && map[y+1][xPoint_26374+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_26374] === this.CELL_EMPTY
                    && map[y][xPoint_26374-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_26374-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
