...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6106=0;yPoint_6106<this.gameFieldBorderY.length; yPoint_6106++){
                for(var xPoint_6106=0;xPoint_6106<this.gameFieldBorderX.length; xPoint_6106++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6106, xPoint_6106);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6106][xPoint_6106] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6106, xPoint_6106, 'user');
                    if(this._userShipsMap[yPoint_6106][xPoint_6106] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6106, xPoint_6106, type){
            var id = this.getPointBlockIdByCoords(yPoint_6106, xPoint_6106, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6106);
                block.setAttribute('data-y', yPoint_6106);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6106
         * @param {type} xPoint_6106
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6106, xPoint_6106, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6106 + '_y' + yPoint_6106;
            }
            return 'pc_x' + xPoint_6106 + '_y' + yPoint_6106;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6106=0;yPoint_6106<this.gameFieldBorderY.length; yPoint_6106++){
                for(var xPoint_6106=0;xPoint_6106<this.gameFieldBorderX.length; xPoint_6106++){
                    map.push({y: yPoint_6106, x: xPoint_6106});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6106=-1;yPoint_6106<(this.gameFieldBorderY.length+1); yPoint_6106++){
                for(var xPoint_6106=-1;xPoint_6106<(this.gameFieldBorderX.length+1); xPoint_6106++){
                    if(!map[yPoint_6106]){
                        map[yPoint_6106] = [];
                    }
                    map[yPoint_6106][xPoint_6106] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6106 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6106 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6106, yPoint_6106) === true){
                    if(this.canPutHorizontal(map, xPoint_6106, yPoint_6106, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6106][xPoint_6106 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6106, yPoint_6106, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6106 + i][xPoint_6106] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6106, yPoint_6106){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6106][xPoint_6106] === this.CELL_EMPTY
                && map[yPoint_6106-1][xPoint_6106] === this.CELL_EMPTY
                && map[yPoint_6106-1][xPoint_6106+1] === this.CELL_EMPTY
                && map[yPoint_6106][xPoint_6106+1] === this.CELL_EMPTY
                && map[yPoint_6106+1][xPoint_6106+1] === this.CELL_EMPTY
                && map[yPoint_6106+1][xPoint_6106] === this.CELL_EMPTY
                && map[yPoint_6106+1][xPoint_6106-1] === this.CELL_EMPTY
                && map[yPoint_6106][xPoint_6106-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6106, yPoint_6106, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6106;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6106][x] === this.CELL_EMPTY
                    && map[yPoint_6106-1][x] === this.CELL_EMPTY
                    && map[yPoint_6106-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6106][x+1] === this.CELL_EMPTY
                    && map[yPoint_6106+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6106+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6106, yPoint_6106, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6106;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6106] === this.CELL_EMPTY
                    && map[y+1][xPoint_6106] === this.CELL_EMPTY
                    && map[y+1][xPoint_6106+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6106] === this.CELL_EMPTY
                    && map[y][xPoint_6106-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6106-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
