...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24346=0;yPoint_24346<this.gameFieldBorderY.length; yPoint_24346++){
                for(var xPoint_24346=0;xPoint_24346<this.gameFieldBorderX.length; xPoint_24346++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24346, xPoint_24346);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24346][xPoint_24346] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24346, xPoint_24346, 'user');
                    if(this._userShipsMap[yPoint_24346][xPoint_24346] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24346, xPoint_24346, type){
            var id = this.getPointBlockIdByCoords(yPoint_24346, xPoint_24346, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24346);
                block.setAttribute('data-y', yPoint_24346);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24346
         * @param {type} xPoint_24346
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24346, xPoint_24346, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24346 + '_y' + yPoint_24346;
            }
            return 'pc_x' + xPoint_24346 + '_y' + yPoint_24346;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24346=0;yPoint_24346<this.gameFieldBorderY.length; yPoint_24346++){
                for(var xPoint_24346=0;xPoint_24346<this.gameFieldBorderX.length; xPoint_24346++){
                    map.push({y: yPoint_24346, x: xPoint_24346});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24346=-1;yPoint_24346<(this.gameFieldBorderY.length+1); yPoint_24346++){
                for(var xPoint_24346=-1;xPoint_24346<(this.gameFieldBorderX.length+1); xPoint_24346++){
                    if(!map[yPoint_24346]){
                        map[yPoint_24346] = [];
                    }
                    map[yPoint_24346][xPoint_24346] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24346 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24346 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24346, yPoint_24346) === true){
                    if(this.canPutHorizontal(map, xPoint_24346, yPoint_24346, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24346][xPoint_24346 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24346, yPoint_24346, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24346 + i][xPoint_24346] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24346, yPoint_24346){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24346][xPoint_24346] === this.CELL_EMPTY
                && map[yPoint_24346-1][xPoint_24346] === this.CELL_EMPTY
                && map[yPoint_24346-1][xPoint_24346+1] === this.CELL_EMPTY
                && map[yPoint_24346][xPoint_24346+1] === this.CELL_EMPTY
                && map[yPoint_24346+1][xPoint_24346+1] === this.CELL_EMPTY
                && map[yPoint_24346+1][xPoint_24346] === this.CELL_EMPTY
                && map[yPoint_24346+1][xPoint_24346-1] === this.CELL_EMPTY
                && map[yPoint_24346][xPoint_24346-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24346, yPoint_24346, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24346;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24346][x] === this.CELL_EMPTY
                    && map[yPoint_24346-1][x] === this.CELL_EMPTY
                    && map[yPoint_24346-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24346][x+1] === this.CELL_EMPTY
                    && map[yPoint_24346+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24346+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24346, yPoint_24346, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24346;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24346] === this.CELL_EMPTY
                    && map[y+1][xPoint_24346] === this.CELL_EMPTY
                    && map[y+1][xPoint_24346+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24346] === this.CELL_EMPTY
                    && map[y][xPoint_24346-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24346-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
