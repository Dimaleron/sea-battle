...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31971=0;yPoint_31971<this.gameFieldBorderY.length; yPoint_31971++){
                for(var xPoint_31971=0;xPoint_31971<this.gameFieldBorderX.length; xPoint_31971++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31971, xPoint_31971);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31971][xPoint_31971] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31971, xPoint_31971, 'user');
                    if(this._userShipsMap[yPoint_31971][xPoint_31971] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31971, xPoint_31971, type){
            var id = this.getPointBlockIdByCoords(yPoint_31971, xPoint_31971, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31971);
                block.setAttribute('data-y', yPoint_31971);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31971
         * @param {type} xPoint_31971
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31971, xPoint_31971, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31971 + '_y' + yPoint_31971;
            }
            return 'pc_x' + xPoint_31971 + '_y' + yPoint_31971;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31971=0;yPoint_31971<this.gameFieldBorderY.length; yPoint_31971++){
                for(var xPoint_31971=0;xPoint_31971<this.gameFieldBorderX.length; xPoint_31971++){
                    map.push({y: yPoint_31971, x: xPoint_31971});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31971=-1;yPoint_31971<(this.gameFieldBorderY.length+1); yPoint_31971++){
                for(var xPoint_31971=-1;xPoint_31971<(this.gameFieldBorderX.length+1); xPoint_31971++){
                    if(!map[yPoint_31971]){
                        map[yPoint_31971] = [];
                    }
                    map[yPoint_31971][xPoint_31971] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31971 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31971 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31971, yPoint_31971) === true){
                    if(this.canPutHorizontal(map, xPoint_31971, yPoint_31971, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31971][xPoint_31971 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31971, yPoint_31971, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31971 + i][xPoint_31971] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31971, yPoint_31971){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31971][xPoint_31971] === this.CELL_EMPTY
                && map[yPoint_31971-1][xPoint_31971] === this.CELL_EMPTY
                && map[yPoint_31971-1][xPoint_31971+1] === this.CELL_EMPTY
                && map[yPoint_31971][xPoint_31971+1] === this.CELL_EMPTY
                && map[yPoint_31971+1][xPoint_31971+1] === this.CELL_EMPTY
                && map[yPoint_31971+1][xPoint_31971] === this.CELL_EMPTY
                && map[yPoint_31971+1][xPoint_31971-1] === this.CELL_EMPTY
                && map[yPoint_31971][xPoint_31971-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31971, yPoint_31971, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31971;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31971][x] === this.CELL_EMPTY
                    && map[yPoint_31971-1][x] === this.CELL_EMPTY
                    && map[yPoint_31971-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31971][x+1] === this.CELL_EMPTY
                    && map[yPoint_31971+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31971+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31971, yPoint_31971, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31971;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31971] === this.CELL_EMPTY
                    && map[y+1][xPoint_31971] === this.CELL_EMPTY
                    && map[y+1][xPoint_31971+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31971] === this.CELL_EMPTY
                    && map[y][xPoint_31971-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31971-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
