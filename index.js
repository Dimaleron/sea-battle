...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32601=0;yPoint_32601<this.gameFieldBorderY.length; yPoint_32601++){
                for(var xPoint_32601=0;xPoint_32601<this.gameFieldBorderX.length; xPoint_32601++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32601, xPoint_32601);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32601][xPoint_32601] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32601, xPoint_32601, 'user');
                    if(this._userShipsMap[yPoint_32601][xPoint_32601] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32601, xPoint_32601, type){
            var id = this.getPointBlockIdByCoords(yPoint_32601, xPoint_32601, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32601);
                block.setAttribute('data-y', yPoint_32601);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32601
         * @param {type} xPoint_32601
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32601, xPoint_32601, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32601 + '_y' + yPoint_32601;
            }
            return 'pc_x' + xPoint_32601 + '_y' + yPoint_32601;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32601=0;yPoint_32601<this.gameFieldBorderY.length; yPoint_32601++){
                for(var xPoint_32601=0;xPoint_32601<this.gameFieldBorderX.length; xPoint_32601++){
                    map.push({y: yPoint_32601, x: xPoint_32601});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32601=-1;yPoint_32601<(this.gameFieldBorderY.length+1); yPoint_32601++){
                for(var xPoint_32601=-1;xPoint_32601<(this.gameFieldBorderX.length+1); xPoint_32601++){
                    if(!map[yPoint_32601]){
                        map[yPoint_32601] = [];
                    }
                    map[yPoint_32601][xPoint_32601] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32601 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32601 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32601, yPoint_32601) === true){
                    if(this.canPutHorizontal(map, xPoint_32601, yPoint_32601, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32601][xPoint_32601 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32601, yPoint_32601, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32601 + i][xPoint_32601] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32601, yPoint_32601){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32601][xPoint_32601] === this.CELL_EMPTY
                && map[yPoint_32601-1][xPoint_32601] === this.CELL_EMPTY
                && map[yPoint_32601-1][xPoint_32601+1] === this.CELL_EMPTY
                && map[yPoint_32601][xPoint_32601+1] === this.CELL_EMPTY
                && map[yPoint_32601+1][xPoint_32601+1] === this.CELL_EMPTY
                && map[yPoint_32601+1][xPoint_32601] === this.CELL_EMPTY
                && map[yPoint_32601+1][xPoint_32601-1] === this.CELL_EMPTY
                && map[yPoint_32601][xPoint_32601-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32601, yPoint_32601, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32601;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32601][x] === this.CELL_EMPTY
                    && map[yPoint_32601-1][x] === this.CELL_EMPTY
                    && map[yPoint_32601-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32601][x+1] === this.CELL_EMPTY
                    && map[yPoint_32601+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32601+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32601, yPoint_32601, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32601;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32601] === this.CELL_EMPTY
                    && map[y+1][xPoint_32601] === this.CELL_EMPTY
                    && map[y+1][xPoint_32601+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32601] === this.CELL_EMPTY
                    && map[y][xPoint_32601-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32601-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
