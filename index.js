...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5002=0;yPoint_5002<this.gameFieldBorderY.length; yPoint_5002++){
                for(var xPoint_5002=0;xPoint_5002<this.gameFieldBorderX.length; xPoint_5002++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5002, xPoint_5002);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5002][xPoint_5002] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5002, xPoint_5002, 'user');
                    if(this._userShipsMap[yPoint_5002][xPoint_5002] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5002, xPoint_5002, type){
            var id = this.getPointBlockIdByCoords(yPoint_5002, xPoint_5002, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5002);
                block.setAttribute('data-y', yPoint_5002);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5002
         * @param {type} xPoint_5002
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5002, xPoint_5002, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5002 + '_y' + yPoint_5002;
            }
            return 'pc_x' + xPoint_5002 + '_y' + yPoint_5002;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5002=0;yPoint_5002<this.gameFieldBorderY.length; yPoint_5002++){
                for(var xPoint_5002=0;xPoint_5002<this.gameFieldBorderX.length; xPoint_5002++){
                    map.push({y: yPoint_5002, x: xPoint_5002});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5002=-1;yPoint_5002<(this.gameFieldBorderY.length+1); yPoint_5002++){
                for(var xPoint_5002=-1;xPoint_5002<(this.gameFieldBorderX.length+1); xPoint_5002++){
                    if(!map[yPoint_5002]){
                        map[yPoint_5002] = [];
                    }
                    map[yPoint_5002][xPoint_5002] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5002 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5002 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5002, yPoint_5002) === true){
                    if(this.canPutHorizontal(map, xPoint_5002, yPoint_5002, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5002][xPoint_5002 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5002, yPoint_5002, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5002 + i][xPoint_5002] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5002, yPoint_5002){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5002][xPoint_5002] === this.CELL_EMPTY
                && map[yPoint_5002-1][xPoint_5002] === this.CELL_EMPTY
                && map[yPoint_5002-1][xPoint_5002+1] === this.CELL_EMPTY
                && map[yPoint_5002][xPoint_5002+1] === this.CELL_EMPTY
                && map[yPoint_5002+1][xPoint_5002+1] === this.CELL_EMPTY
                && map[yPoint_5002+1][xPoint_5002] === this.CELL_EMPTY
                && map[yPoint_5002+1][xPoint_5002-1] === this.CELL_EMPTY
                && map[yPoint_5002][xPoint_5002-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5002, yPoint_5002, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5002;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5002][x] === this.CELL_EMPTY
                    && map[yPoint_5002-1][x] === this.CELL_EMPTY
                    && map[yPoint_5002-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5002][x+1] === this.CELL_EMPTY
                    && map[yPoint_5002+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5002+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5002, yPoint_5002, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5002;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5002] === this.CELL_EMPTY
                    && map[y+1][xPoint_5002] === this.CELL_EMPTY
                    && map[y+1][xPoint_5002+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5002] === this.CELL_EMPTY
                    && map[y][xPoint_5002-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5002-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
