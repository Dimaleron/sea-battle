...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7727=0;yPoint_7727<this.gameFieldBorderY.length; yPoint_7727++){
                for(var xPoint_7727=0;xPoint_7727<this.gameFieldBorderX.length; xPoint_7727++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7727, xPoint_7727);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7727][xPoint_7727] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7727, xPoint_7727, 'user');
                    if(this._userShipsMap[yPoint_7727][xPoint_7727] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7727, xPoint_7727, type){
            var id = this.getPointBlockIdByCoords(yPoint_7727, xPoint_7727, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7727);
                block.setAttribute('data-y', yPoint_7727);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7727
         * @param {type} xPoint_7727
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7727, xPoint_7727, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7727 + '_y' + yPoint_7727;
            }
            return 'pc_x' + xPoint_7727 + '_y' + yPoint_7727;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7727=0;yPoint_7727<this.gameFieldBorderY.length; yPoint_7727++){
                for(var xPoint_7727=0;xPoint_7727<this.gameFieldBorderX.length; xPoint_7727++){
                    map.push({y: yPoint_7727, x: xPoint_7727});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7727=-1;yPoint_7727<(this.gameFieldBorderY.length+1); yPoint_7727++){
                for(var xPoint_7727=-1;xPoint_7727<(this.gameFieldBorderX.length+1); xPoint_7727++){
                    if(!map[yPoint_7727]){
                        map[yPoint_7727] = [];
                    }
                    map[yPoint_7727][xPoint_7727] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7727 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7727 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7727, yPoint_7727) === true){
                    if(this.canPutHorizontal(map, xPoint_7727, yPoint_7727, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7727][xPoint_7727 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7727, yPoint_7727, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7727 + i][xPoint_7727] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7727, yPoint_7727){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7727][xPoint_7727] === this.CELL_EMPTY
                && map[yPoint_7727-1][xPoint_7727] === this.CELL_EMPTY
                && map[yPoint_7727-1][xPoint_7727+1] === this.CELL_EMPTY
                && map[yPoint_7727][xPoint_7727+1] === this.CELL_EMPTY
                && map[yPoint_7727+1][xPoint_7727+1] === this.CELL_EMPTY
                && map[yPoint_7727+1][xPoint_7727] === this.CELL_EMPTY
                && map[yPoint_7727+1][xPoint_7727-1] === this.CELL_EMPTY
                && map[yPoint_7727][xPoint_7727-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7727, yPoint_7727, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7727;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7727][x] === this.CELL_EMPTY
                    && map[yPoint_7727-1][x] === this.CELL_EMPTY
                    && map[yPoint_7727-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7727][x+1] === this.CELL_EMPTY
                    && map[yPoint_7727+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7727+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7727, yPoint_7727, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7727;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7727] === this.CELL_EMPTY
                    && map[y+1][xPoint_7727] === this.CELL_EMPTY
                    && map[y+1][xPoint_7727+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7727] === this.CELL_EMPTY
                    && map[y][xPoint_7727-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7727-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
