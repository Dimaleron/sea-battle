...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5400=0;yPoint_5400<this.gameFieldBorderY.length; yPoint_5400++){
                for(var xPoint_5400=0;xPoint_5400<this.gameFieldBorderX.length; xPoint_5400++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5400, xPoint_5400);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5400][xPoint_5400] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5400, xPoint_5400, 'user');
                    if(this._userShipsMap[yPoint_5400][xPoint_5400] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5400, xPoint_5400, type){
            var id = this.getPointBlockIdByCoords(yPoint_5400, xPoint_5400, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5400);
                block.setAttribute('data-y', yPoint_5400);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5400
         * @param {type} xPoint_5400
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5400, xPoint_5400, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5400 + '_y' + yPoint_5400;
            }
            return 'pc_x' + xPoint_5400 + '_y' + yPoint_5400;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5400=0;yPoint_5400<this.gameFieldBorderY.length; yPoint_5400++){
                for(var xPoint_5400=0;xPoint_5400<this.gameFieldBorderX.length; xPoint_5400++){
                    map.push({y: yPoint_5400, x: xPoint_5400});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5400=-1;yPoint_5400<(this.gameFieldBorderY.length+1); yPoint_5400++){
                for(var xPoint_5400=-1;xPoint_5400<(this.gameFieldBorderX.length+1); xPoint_5400++){
                    if(!map[yPoint_5400]){
                        map[yPoint_5400] = [];
                    }
                    map[yPoint_5400][xPoint_5400] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5400 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5400 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5400, yPoint_5400) === true){
                    if(this.canPutHorizontal(map, xPoint_5400, yPoint_5400, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5400][xPoint_5400 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5400, yPoint_5400, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5400 + i][xPoint_5400] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5400, yPoint_5400){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5400][xPoint_5400] === this.CELL_EMPTY
                && map[yPoint_5400-1][xPoint_5400] === this.CELL_EMPTY
                && map[yPoint_5400-1][xPoint_5400+1] === this.CELL_EMPTY
                && map[yPoint_5400][xPoint_5400+1] === this.CELL_EMPTY
                && map[yPoint_5400+1][xPoint_5400+1] === this.CELL_EMPTY
                && map[yPoint_5400+1][xPoint_5400] === this.CELL_EMPTY
                && map[yPoint_5400+1][xPoint_5400-1] === this.CELL_EMPTY
                && map[yPoint_5400][xPoint_5400-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5400, yPoint_5400, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5400;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5400][x] === this.CELL_EMPTY
                    && map[yPoint_5400-1][x] === this.CELL_EMPTY
                    && map[yPoint_5400-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5400][x+1] === this.CELL_EMPTY
                    && map[yPoint_5400+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5400+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5400, yPoint_5400, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5400;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5400] === this.CELL_EMPTY
                    && map[y+1][xPoint_5400] === this.CELL_EMPTY
                    && map[y+1][xPoint_5400+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5400] === this.CELL_EMPTY
                    && map[y][xPoint_5400-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5400-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
