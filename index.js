...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29167=0;yPoint_29167<this.gameFieldBorderY.length; yPoint_29167++){
                for(var xPoint_29167=0;xPoint_29167<this.gameFieldBorderX.length; xPoint_29167++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29167, xPoint_29167);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29167][xPoint_29167] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29167, xPoint_29167, 'user');
                    if(this._userShipsMap[yPoint_29167][xPoint_29167] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29167, xPoint_29167, type){
            var id = this.getPointBlockIdByCoords(yPoint_29167, xPoint_29167, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29167);
                block.setAttribute('data-y', yPoint_29167);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29167
         * @param {type} xPoint_29167
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29167, xPoint_29167, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29167 + '_y' + yPoint_29167;
            }
            return 'pc_x' + xPoint_29167 + '_y' + yPoint_29167;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29167=0;yPoint_29167<this.gameFieldBorderY.length; yPoint_29167++){
                for(var xPoint_29167=0;xPoint_29167<this.gameFieldBorderX.length; xPoint_29167++){
                    map.push({y: yPoint_29167, x: xPoint_29167});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29167=-1;yPoint_29167<(this.gameFieldBorderY.length+1); yPoint_29167++){
                for(var xPoint_29167=-1;xPoint_29167<(this.gameFieldBorderX.length+1); xPoint_29167++){
                    if(!map[yPoint_29167]){
                        map[yPoint_29167] = [];
                    }
                    map[yPoint_29167][xPoint_29167] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29167 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29167 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29167, yPoint_29167) === true){
                    if(this.canPutHorizontal(map, xPoint_29167, yPoint_29167, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29167][xPoint_29167 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29167, yPoint_29167, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29167 + i][xPoint_29167] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29167, yPoint_29167){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29167][xPoint_29167] === this.CELL_EMPTY
                && map[yPoint_29167-1][xPoint_29167] === this.CELL_EMPTY
                && map[yPoint_29167-1][xPoint_29167+1] === this.CELL_EMPTY
                && map[yPoint_29167][xPoint_29167+1] === this.CELL_EMPTY
                && map[yPoint_29167+1][xPoint_29167+1] === this.CELL_EMPTY
                && map[yPoint_29167+1][xPoint_29167] === this.CELL_EMPTY
                && map[yPoint_29167+1][xPoint_29167-1] === this.CELL_EMPTY
                && map[yPoint_29167][xPoint_29167-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29167, yPoint_29167, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29167;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29167][x] === this.CELL_EMPTY
                    && map[yPoint_29167-1][x] === this.CELL_EMPTY
                    && map[yPoint_29167-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29167][x+1] === this.CELL_EMPTY
                    && map[yPoint_29167+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29167+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29167, yPoint_29167, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29167;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29167] === this.CELL_EMPTY
                    && map[y+1][xPoint_29167] === this.CELL_EMPTY
                    && map[y+1][xPoint_29167+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29167] === this.CELL_EMPTY
                    && map[y][xPoint_29167-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29167-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
