...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29920=0;yPoint_29920<this.gameFieldBorderY.length; yPoint_29920++){
                for(var xPoint_29920=0;xPoint_29920<this.gameFieldBorderX.length; xPoint_29920++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29920, xPoint_29920);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29920][xPoint_29920] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29920, xPoint_29920, 'user');
                    if(this._userShipsMap[yPoint_29920][xPoint_29920] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29920, xPoint_29920, type){
            var id = this.getPointBlockIdByCoords(yPoint_29920, xPoint_29920, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29920);
                block.setAttribute('data-y', yPoint_29920);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29920
         * @param {type} xPoint_29920
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29920, xPoint_29920, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29920 + '_y' + yPoint_29920;
            }
            return 'pc_x' + xPoint_29920 + '_y' + yPoint_29920;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29920=0;yPoint_29920<this.gameFieldBorderY.length; yPoint_29920++){
                for(var xPoint_29920=0;xPoint_29920<this.gameFieldBorderX.length; xPoint_29920++){
                    map.push({y: yPoint_29920, x: xPoint_29920});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29920=-1;yPoint_29920<(this.gameFieldBorderY.length+1); yPoint_29920++){
                for(var xPoint_29920=-1;xPoint_29920<(this.gameFieldBorderX.length+1); xPoint_29920++){
                    if(!map[yPoint_29920]){
                        map[yPoint_29920] = [];
                    }
                    map[yPoint_29920][xPoint_29920] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29920 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29920 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29920, yPoint_29920) === true){
                    if(this.canPutHorizontal(map, xPoint_29920, yPoint_29920, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29920][xPoint_29920 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29920, yPoint_29920, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29920 + i][xPoint_29920] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29920, yPoint_29920){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29920][xPoint_29920] === this.CELL_EMPTY
                && map[yPoint_29920-1][xPoint_29920] === this.CELL_EMPTY
                && map[yPoint_29920-1][xPoint_29920+1] === this.CELL_EMPTY
                && map[yPoint_29920][xPoint_29920+1] === this.CELL_EMPTY
                && map[yPoint_29920+1][xPoint_29920+1] === this.CELL_EMPTY
                && map[yPoint_29920+1][xPoint_29920] === this.CELL_EMPTY
                && map[yPoint_29920+1][xPoint_29920-1] === this.CELL_EMPTY
                && map[yPoint_29920][xPoint_29920-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29920, yPoint_29920, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29920;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29920][x] === this.CELL_EMPTY
                    && map[yPoint_29920-1][x] === this.CELL_EMPTY
                    && map[yPoint_29920-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29920][x+1] === this.CELL_EMPTY
                    && map[yPoint_29920+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29920+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29920, yPoint_29920, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29920;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29920] === this.CELL_EMPTY
                    && map[y+1][xPoint_29920] === this.CELL_EMPTY
                    && map[y+1][xPoint_29920+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29920] === this.CELL_EMPTY
                    && map[y][xPoint_29920-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29920-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
