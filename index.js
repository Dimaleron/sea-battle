...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_151=0;yPoint_151<this.gameFieldBorderY.length; yPoint_151++){
                for(var xPoint_151=0;xPoint_151<this.gameFieldBorderX.length; xPoint_151++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_151, xPoint_151);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_151][xPoint_151] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_151, xPoint_151, 'user');
                    if(this._userShipsMap[yPoint_151][xPoint_151] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_151, xPoint_151, type){
            var id = this.getPointBlockIdByCoords(yPoint_151, xPoint_151, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_151);
                block.setAttribute('data-y', yPoint_151);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_151
         * @param {type} xPoint_151
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_151, xPoint_151, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_151 + '_y' + yPoint_151;
            }
            return 'pc_x' + xPoint_151 + '_y' + yPoint_151;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_151=0;yPoint_151<this.gameFieldBorderY.length; yPoint_151++){
                for(var xPoint_151=0;xPoint_151<this.gameFieldBorderX.length; xPoint_151++){
                    map.push({y: yPoint_151, x: xPoint_151});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_151=-1;yPoint_151<(this.gameFieldBorderY.length+1); yPoint_151++){
                for(var xPoint_151=-1;xPoint_151<(this.gameFieldBorderX.length+1); xPoint_151++){
                    if(!map[yPoint_151]){
                        map[yPoint_151] = [];
                    }
                    map[yPoint_151][xPoint_151] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_151 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_151 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_151, yPoint_151) === true){
                    if(this.canPutHorizontal(map, xPoint_151, yPoint_151, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_151][xPoint_151 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_151, yPoint_151, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_151 + i][xPoint_151] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_151, yPoint_151){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_151][xPoint_151] === this.CELL_EMPTY
                && map[yPoint_151-1][xPoint_151] === this.CELL_EMPTY
                && map[yPoint_151-1][xPoint_151+1] === this.CELL_EMPTY
                && map[yPoint_151][xPoint_151+1] === this.CELL_EMPTY
                && map[yPoint_151+1][xPoint_151+1] === this.CELL_EMPTY
                && map[yPoint_151+1][xPoint_151] === this.CELL_EMPTY
                && map[yPoint_151+1][xPoint_151-1] === this.CELL_EMPTY
                && map[yPoint_151][xPoint_151-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_151, yPoint_151, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_151;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_151][x] === this.CELL_EMPTY
                    && map[yPoint_151-1][x] === this.CELL_EMPTY
                    && map[yPoint_151-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_151][x+1] === this.CELL_EMPTY
                    && map[yPoint_151+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_151+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_151, yPoint_151, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_151;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_151] === this.CELL_EMPTY
                    && map[y+1][xPoint_151] === this.CELL_EMPTY
                    && map[y+1][xPoint_151+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_151] === this.CELL_EMPTY
                    && map[y][xPoint_151-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_151-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
