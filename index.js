...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30268=0;yPoint_30268<this.gameFieldBorderY.length; yPoint_30268++){
                for(var xPoint_30268=0;xPoint_30268<this.gameFieldBorderX.length; xPoint_30268++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30268, xPoint_30268);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30268][xPoint_30268] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30268, xPoint_30268, 'user');
                    if(this._userShipsMap[yPoint_30268][xPoint_30268] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30268, xPoint_30268, type){
            var id = this.getPointBlockIdByCoords(yPoint_30268, xPoint_30268, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30268);
                block.setAttribute('data-y', yPoint_30268);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30268
         * @param {type} xPoint_30268
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30268, xPoint_30268, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30268 + '_y' + yPoint_30268;
            }
            return 'pc_x' + xPoint_30268 + '_y' + yPoint_30268;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30268=0;yPoint_30268<this.gameFieldBorderY.length; yPoint_30268++){
                for(var xPoint_30268=0;xPoint_30268<this.gameFieldBorderX.length; xPoint_30268++){
                    map.push({y: yPoint_30268, x: xPoint_30268});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30268=-1;yPoint_30268<(this.gameFieldBorderY.length+1); yPoint_30268++){
                for(var xPoint_30268=-1;xPoint_30268<(this.gameFieldBorderX.length+1); xPoint_30268++){
                    if(!map[yPoint_30268]){
                        map[yPoint_30268] = [];
                    }
                    map[yPoint_30268][xPoint_30268] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30268 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30268 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30268, yPoint_30268) === true){
                    if(this.canPutHorizontal(map, xPoint_30268, yPoint_30268, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30268][xPoint_30268 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30268, yPoint_30268, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30268 + i][xPoint_30268] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30268, yPoint_30268){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30268][xPoint_30268] === this.CELL_EMPTY
                && map[yPoint_30268-1][xPoint_30268] === this.CELL_EMPTY
                && map[yPoint_30268-1][xPoint_30268+1] === this.CELL_EMPTY
                && map[yPoint_30268][xPoint_30268+1] === this.CELL_EMPTY
                && map[yPoint_30268+1][xPoint_30268+1] === this.CELL_EMPTY
                && map[yPoint_30268+1][xPoint_30268] === this.CELL_EMPTY
                && map[yPoint_30268+1][xPoint_30268-1] === this.CELL_EMPTY
                && map[yPoint_30268][xPoint_30268-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30268, yPoint_30268, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30268;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30268][x] === this.CELL_EMPTY
                    && map[yPoint_30268-1][x] === this.CELL_EMPTY
                    && map[yPoint_30268-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30268][x+1] === this.CELL_EMPTY
                    && map[yPoint_30268+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30268+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30268, yPoint_30268, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30268;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30268] === this.CELL_EMPTY
                    && map[y+1][xPoint_30268] === this.CELL_EMPTY
                    && map[y+1][xPoint_30268+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30268] === this.CELL_EMPTY
                    && map[y][xPoint_30268-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30268-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
