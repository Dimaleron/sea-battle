...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10507=0;yPoint_10507<this.gameFieldBorderY.length; yPoint_10507++){
                for(var xPoint_10507=0;xPoint_10507<this.gameFieldBorderX.length; xPoint_10507++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10507, xPoint_10507);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10507][xPoint_10507] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10507, xPoint_10507, 'user');
                    if(this._userShipsMap[yPoint_10507][xPoint_10507] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10507, xPoint_10507, type){
            var id = this.getPointBlockIdByCoords(yPoint_10507, xPoint_10507, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10507);
                block.setAttribute('data-y', yPoint_10507);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10507
         * @param {type} xPoint_10507
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10507, xPoint_10507, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10507 + '_y' + yPoint_10507;
            }
            return 'pc_x' + xPoint_10507 + '_y' + yPoint_10507;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10507=0;yPoint_10507<this.gameFieldBorderY.length; yPoint_10507++){
                for(var xPoint_10507=0;xPoint_10507<this.gameFieldBorderX.length; xPoint_10507++){
                    map.push({y: yPoint_10507, x: xPoint_10507});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10507=-1;yPoint_10507<(this.gameFieldBorderY.length+1); yPoint_10507++){
                for(var xPoint_10507=-1;xPoint_10507<(this.gameFieldBorderX.length+1); xPoint_10507++){
                    if(!map[yPoint_10507]){
                        map[yPoint_10507] = [];
                    }
                    map[yPoint_10507][xPoint_10507] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10507 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10507 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10507, yPoint_10507) === true){
                    if(this.canPutHorizontal(map, xPoint_10507, yPoint_10507, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10507][xPoint_10507 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10507, yPoint_10507, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10507 + i][xPoint_10507] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10507, yPoint_10507){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10507][xPoint_10507] === this.CELL_EMPTY
                && map[yPoint_10507-1][xPoint_10507] === this.CELL_EMPTY
                && map[yPoint_10507-1][xPoint_10507+1] === this.CELL_EMPTY
                && map[yPoint_10507][xPoint_10507+1] === this.CELL_EMPTY
                && map[yPoint_10507+1][xPoint_10507+1] === this.CELL_EMPTY
                && map[yPoint_10507+1][xPoint_10507] === this.CELL_EMPTY
                && map[yPoint_10507+1][xPoint_10507-1] === this.CELL_EMPTY
                && map[yPoint_10507][xPoint_10507-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10507, yPoint_10507, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10507;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10507][x] === this.CELL_EMPTY
                    && map[yPoint_10507-1][x] === this.CELL_EMPTY
                    && map[yPoint_10507-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10507][x+1] === this.CELL_EMPTY
                    && map[yPoint_10507+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10507+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10507, yPoint_10507, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10507;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10507] === this.CELL_EMPTY
                    && map[y+1][xPoint_10507] === this.CELL_EMPTY
                    && map[y+1][xPoint_10507+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10507] === this.CELL_EMPTY
                    && map[y][xPoint_10507-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10507-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
