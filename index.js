...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_94=0;yPoint_94<this.gameFieldBorderY.length; yPoint_94++){
                for(var xPoint_94=0;xPoint_94<this.gameFieldBorderX.length; xPoint_94++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_94, xPoint_94);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_94][xPoint_94] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_94, xPoint_94, 'user');
                    if(this._userShipsMap[yPoint_94][xPoint_94] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_94, xPoint_94, type){
            var id = this.getPointBlockIdByCoords(yPoint_94, xPoint_94, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_94);
                block.setAttribute('data-y', yPoint_94);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_94
         * @param {type} xPoint_94
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_94, xPoint_94, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_94 + '_y' + yPoint_94;
            }
            return 'pc_x' + xPoint_94 + '_y' + yPoint_94;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_94=0;yPoint_94<this.gameFieldBorderY.length; yPoint_94++){
                for(var xPoint_94=0;xPoint_94<this.gameFieldBorderX.length; xPoint_94++){
                    map.push({y: yPoint_94, x: xPoint_94});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_94=-1;yPoint_94<(this.gameFieldBorderY.length+1); yPoint_94++){
                for(var xPoint_94=-1;xPoint_94<(this.gameFieldBorderX.length+1); xPoint_94++){
                    if(!map[yPoint_94]){
                        map[yPoint_94] = [];
                    }
                    map[yPoint_94][xPoint_94] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_94 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_94 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_94, yPoint_94) === true){
                    if(this.canPutHorizontal(map, xPoint_94, yPoint_94, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_94][xPoint_94 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_94, yPoint_94, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_94 + i][xPoint_94] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_94, yPoint_94){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_94][xPoint_94] === this.CELL_EMPTY
                && map[yPoint_94-1][xPoint_94] === this.CELL_EMPTY
                && map[yPoint_94-1][xPoint_94+1] === this.CELL_EMPTY
                && map[yPoint_94][xPoint_94+1] === this.CELL_EMPTY
                && map[yPoint_94+1][xPoint_94+1] === this.CELL_EMPTY
                && map[yPoint_94+1][xPoint_94] === this.CELL_EMPTY
                && map[yPoint_94+1][xPoint_94-1] === this.CELL_EMPTY
                && map[yPoint_94][xPoint_94-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_94, yPoint_94, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_94;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_94][x] === this.CELL_EMPTY
                    && map[yPoint_94-1][x] === this.CELL_EMPTY
                    && map[yPoint_94-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_94][x+1] === this.CELL_EMPTY
                    && map[yPoint_94+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_94+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_94, yPoint_94, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_94;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_94] === this.CELL_EMPTY
                    && map[y+1][xPoint_94] === this.CELL_EMPTY
                    && map[y+1][xPoint_94+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_94] === this.CELL_EMPTY
                    && map[y][xPoint_94-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_94-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
