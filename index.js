...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24504=0;yPoint_24504<this.gameFieldBorderY.length; yPoint_24504++){
                for(var xPoint_24504=0;xPoint_24504<this.gameFieldBorderX.length; xPoint_24504++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24504, xPoint_24504);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24504][xPoint_24504] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24504, xPoint_24504, 'user');
                    if(this._userShipsMap[yPoint_24504][xPoint_24504] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24504, xPoint_24504, type){
            var id = this.getPointBlockIdByCoords(yPoint_24504, xPoint_24504, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24504);
                block.setAttribute('data-y', yPoint_24504);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24504
         * @param {type} xPoint_24504
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24504, xPoint_24504, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24504 + '_y' + yPoint_24504;
            }
            return 'pc_x' + xPoint_24504 + '_y' + yPoint_24504;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24504=0;yPoint_24504<this.gameFieldBorderY.length; yPoint_24504++){
                for(var xPoint_24504=0;xPoint_24504<this.gameFieldBorderX.length; xPoint_24504++){
                    map.push({y: yPoint_24504, x: xPoint_24504});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24504=-1;yPoint_24504<(this.gameFieldBorderY.length+1); yPoint_24504++){
                for(var xPoint_24504=-1;xPoint_24504<(this.gameFieldBorderX.length+1); xPoint_24504++){
                    if(!map[yPoint_24504]){
                        map[yPoint_24504] = [];
                    }
                    map[yPoint_24504][xPoint_24504] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24504 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24504 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24504, yPoint_24504) === true){
                    if(this.canPutHorizontal(map, xPoint_24504, yPoint_24504, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24504][xPoint_24504 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24504, yPoint_24504, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24504 + i][xPoint_24504] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24504, yPoint_24504){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24504][xPoint_24504] === this.CELL_EMPTY
                && map[yPoint_24504-1][xPoint_24504] === this.CELL_EMPTY
                && map[yPoint_24504-1][xPoint_24504+1] === this.CELL_EMPTY
                && map[yPoint_24504][xPoint_24504+1] === this.CELL_EMPTY
                && map[yPoint_24504+1][xPoint_24504+1] === this.CELL_EMPTY
                && map[yPoint_24504+1][xPoint_24504] === this.CELL_EMPTY
                && map[yPoint_24504+1][xPoint_24504-1] === this.CELL_EMPTY
                && map[yPoint_24504][xPoint_24504-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24504, yPoint_24504, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24504;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24504][x] === this.CELL_EMPTY
                    && map[yPoint_24504-1][x] === this.CELL_EMPTY
                    && map[yPoint_24504-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24504][x+1] === this.CELL_EMPTY
                    && map[yPoint_24504+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24504+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24504, yPoint_24504, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24504;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24504] === this.CELL_EMPTY
                    && map[y+1][xPoint_24504] === this.CELL_EMPTY
                    && map[y+1][xPoint_24504+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24504] === this.CELL_EMPTY
                    && map[y][xPoint_24504-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24504-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
