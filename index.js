...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_488=0;yPoint_488<this.gameFieldBorderY.length; yPoint_488++){
                for(var xPoint_488=0;xPoint_488<this.gameFieldBorderX.length; xPoint_488++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_488, xPoint_488);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_488][xPoint_488] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_488, xPoint_488, 'user');
                    if(this._userShipsMap[yPoint_488][xPoint_488] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_488, xPoint_488, type){
            var id = this.getPointBlockIdByCoords(yPoint_488, xPoint_488, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_488);
                block.setAttribute('data-y', yPoint_488);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_488
         * @param {type} xPoint_488
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_488, xPoint_488, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_488 + '_y' + yPoint_488;
            }
            return 'pc_x' + xPoint_488 + '_y' + yPoint_488;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_488=0;yPoint_488<this.gameFieldBorderY.length; yPoint_488++){
                for(var xPoint_488=0;xPoint_488<this.gameFieldBorderX.length; xPoint_488++){
                    map.push({y: yPoint_488, x: xPoint_488});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_488=-1;yPoint_488<(this.gameFieldBorderY.length+1); yPoint_488++){
                for(var xPoint_488=-1;xPoint_488<(this.gameFieldBorderX.length+1); xPoint_488++){
                    if(!map[yPoint_488]){
                        map[yPoint_488] = [];
                    }
                    map[yPoint_488][xPoint_488] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_488 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_488 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_488, yPoint_488) === true){
                    if(this.canPutHorizontal(map, xPoint_488, yPoint_488, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_488][xPoint_488 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_488, yPoint_488, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_488 + i][xPoint_488] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_488, yPoint_488){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_488][xPoint_488] === this.CELL_EMPTY
                && map[yPoint_488-1][xPoint_488] === this.CELL_EMPTY
                && map[yPoint_488-1][xPoint_488+1] === this.CELL_EMPTY
                && map[yPoint_488][xPoint_488+1] === this.CELL_EMPTY
                && map[yPoint_488+1][xPoint_488+1] === this.CELL_EMPTY
                && map[yPoint_488+1][xPoint_488] === this.CELL_EMPTY
                && map[yPoint_488+1][xPoint_488-1] === this.CELL_EMPTY
                && map[yPoint_488][xPoint_488-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_488, yPoint_488, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_488;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_488][x] === this.CELL_EMPTY
                    && map[yPoint_488-1][x] === this.CELL_EMPTY
                    && map[yPoint_488-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_488][x+1] === this.CELL_EMPTY
                    && map[yPoint_488+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_488+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_488, yPoint_488, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_488;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_488] === this.CELL_EMPTY
                    && map[y+1][xPoint_488] === this.CELL_EMPTY
                    && map[y+1][xPoint_488+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_488] === this.CELL_EMPTY
                    && map[y][xPoint_488-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_488-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
