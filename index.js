...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7181=0;yPoint_7181<this.gameFieldBorderY.length; yPoint_7181++){
                for(var xPoint_7181=0;xPoint_7181<this.gameFieldBorderX.length; xPoint_7181++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7181, xPoint_7181);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7181][xPoint_7181] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7181, xPoint_7181, 'user');
                    if(this._userShipsMap[yPoint_7181][xPoint_7181] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7181, xPoint_7181, type){
            var id = this.getPointBlockIdByCoords(yPoint_7181, xPoint_7181, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7181);
                block.setAttribute('data-y', yPoint_7181);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7181
         * @param {type} xPoint_7181
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7181, xPoint_7181, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7181 + '_y' + yPoint_7181;
            }
            return 'pc_x' + xPoint_7181 + '_y' + yPoint_7181;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7181=0;yPoint_7181<this.gameFieldBorderY.length; yPoint_7181++){
                for(var xPoint_7181=0;xPoint_7181<this.gameFieldBorderX.length; xPoint_7181++){
                    map.push({y: yPoint_7181, x: xPoint_7181});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7181=-1;yPoint_7181<(this.gameFieldBorderY.length+1); yPoint_7181++){
                for(var xPoint_7181=-1;xPoint_7181<(this.gameFieldBorderX.length+1); xPoint_7181++){
                    if(!map[yPoint_7181]){
                        map[yPoint_7181] = [];
                    }
                    map[yPoint_7181][xPoint_7181] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7181 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7181 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7181, yPoint_7181) === true){
                    if(this.canPutHorizontal(map, xPoint_7181, yPoint_7181, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7181][xPoint_7181 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7181, yPoint_7181, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7181 + i][xPoint_7181] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7181, yPoint_7181){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7181][xPoint_7181] === this.CELL_EMPTY
                && map[yPoint_7181-1][xPoint_7181] === this.CELL_EMPTY
                && map[yPoint_7181-1][xPoint_7181+1] === this.CELL_EMPTY
                && map[yPoint_7181][xPoint_7181+1] === this.CELL_EMPTY
                && map[yPoint_7181+1][xPoint_7181+1] === this.CELL_EMPTY
                && map[yPoint_7181+1][xPoint_7181] === this.CELL_EMPTY
                && map[yPoint_7181+1][xPoint_7181-1] === this.CELL_EMPTY
                && map[yPoint_7181][xPoint_7181-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7181, yPoint_7181, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7181;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7181][x] === this.CELL_EMPTY
                    && map[yPoint_7181-1][x] === this.CELL_EMPTY
                    && map[yPoint_7181-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7181][x+1] === this.CELL_EMPTY
                    && map[yPoint_7181+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7181+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7181, yPoint_7181, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7181;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7181] === this.CELL_EMPTY
                    && map[y+1][xPoint_7181] === this.CELL_EMPTY
                    && map[y+1][xPoint_7181+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7181] === this.CELL_EMPTY
                    && map[y][xPoint_7181-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7181-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
