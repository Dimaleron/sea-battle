...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11964=0;yPoint_11964<this.gameFieldBorderY.length; yPoint_11964++){
                for(var xPoint_11964=0;xPoint_11964<this.gameFieldBorderX.length; xPoint_11964++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11964, xPoint_11964);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11964][xPoint_11964] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11964, xPoint_11964, 'user');
                    if(this._userShipsMap[yPoint_11964][xPoint_11964] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11964, xPoint_11964, type){
            var id = this.getPointBlockIdByCoords(yPoint_11964, xPoint_11964, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11964);
                block.setAttribute('data-y', yPoint_11964);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11964
         * @param {type} xPoint_11964
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11964, xPoint_11964, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11964 + '_y' + yPoint_11964;
            }
            return 'pc_x' + xPoint_11964 + '_y' + yPoint_11964;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11964=0;yPoint_11964<this.gameFieldBorderY.length; yPoint_11964++){
                for(var xPoint_11964=0;xPoint_11964<this.gameFieldBorderX.length; xPoint_11964++){
                    map.push({y: yPoint_11964, x: xPoint_11964});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11964=-1;yPoint_11964<(this.gameFieldBorderY.length+1); yPoint_11964++){
                for(var xPoint_11964=-1;xPoint_11964<(this.gameFieldBorderX.length+1); xPoint_11964++){
                    if(!map[yPoint_11964]){
                        map[yPoint_11964] = [];
                    }
                    map[yPoint_11964][xPoint_11964] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11964 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11964 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11964, yPoint_11964) === true){
                    if(this.canPutHorizontal(map, xPoint_11964, yPoint_11964, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11964][xPoint_11964 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11964, yPoint_11964, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11964 + i][xPoint_11964] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11964, yPoint_11964){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11964][xPoint_11964] === this.CELL_EMPTY
                && map[yPoint_11964-1][xPoint_11964] === this.CELL_EMPTY
                && map[yPoint_11964-1][xPoint_11964+1] === this.CELL_EMPTY
                && map[yPoint_11964][xPoint_11964+1] === this.CELL_EMPTY
                && map[yPoint_11964+1][xPoint_11964+1] === this.CELL_EMPTY
                && map[yPoint_11964+1][xPoint_11964] === this.CELL_EMPTY
                && map[yPoint_11964+1][xPoint_11964-1] === this.CELL_EMPTY
                && map[yPoint_11964][xPoint_11964-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11964, yPoint_11964, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11964;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11964][x] === this.CELL_EMPTY
                    && map[yPoint_11964-1][x] === this.CELL_EMPTY
                    && map[yPoint_11964-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11964][x+1] === this.CELL_EMPTY
                    && map[yPoint_11964+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11964+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11964, yPoint_11964, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11964;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11964] === this.CELL_EMPTY
                    && map[y+1][xPoint_11964] === this.CELL_EMPTY
                    && map[y+1][xPoint_11964+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11964] === this.CELL_EMPTY
                    && map[y][xPoint_11964-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11964-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
