...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32555=0;yPoint_32555<this.gameFieldBorderY.length; yPoint_32555++){
                for(var xPoint_32555=0;xPoint_32555<this.gameFieldBorderX.length; xPoint_32555++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32555, xPoint_32555);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32555][xPoint_32555] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32555, xPoint_32555, 'user');
                    if(this._userShipsMap[yPoint_32555][xPoint_32555] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32555, xPoint_32555, type){
            var id = this.getPointBlockIdByCoords(yPoint_32555, xPoint_32555, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32555);
                block.setAttribute('data-y', yPoint_32555);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32555
         * @param {type} xPoint_32555
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32555, xPoint_32555, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32555 + '_y' + yPoint_32555;
            }
            return 'pc_x' + xPoint_32555 + '_y' + yPoint_32555;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32555=0;yPoint_32555<this.gameFieldBorderY.length; yPoint_32555++){
                for(var xPoint_32555=0;xPoint_32555<this.gameFieldBorderX.length; xPoint_32555++){
                    map.push({y: yPoint_32555, x: xPoint_32555});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32555=-1;yPoint_32555<(this.gameFieldBorderY.length+1); yPoint_32555++){
                for(var xPoint_32555=-1;xPoint_32555<(this.gameFieldBorderX.length+1); xPoint_32555++){
                    if(!map[yPoint_32555]){
                        map[yPoint_32555] = [];
                    }
                    map[yPoint_32555][xPoint_32555] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32555 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32555 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32555, yPoint_32555) === true){
                    if(this.canPutHorizontal(map, xPoint_32555, yPoint_32555, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32555][xPoint_32555 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32555, yPoint_32555, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32555 + i][xPoint_32555] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32555, yPoint_32555){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32555][xPoint_32555] === this.CELL_EMPTY
                && map[yPoint_32555-1][xPoint_32555] === this.CELL_EMPTY
                && map[yPoint_32555-1][xPoint_32555+1] === this.CELL_EMPTY
                && map[yPoint_32555][xPoint_32555+1] === this.CELL_EMPTY
                && map[yPoint_32555+1][xPoint_32555+1] === this.CELL_EMPTY
                && map[yPoint_32555+1][xPoint_32555] === this.CELL_EMPTY
                && map[yPoint_32555+1][xPoint_32555-1] === this.CELL_EMPTY
                && map[yPoint_32555][xPoint_32555-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32555, yPoint_32555, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32555;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32555][x] === this.CELL_EMPTY
                    && map[yPoint_32555-1][x] === this.CELL_EMPTY
                    && map[yPoint_32555-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32555][x+1] === this.CELL_EMPTY
                    && map[yPoint_32555+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32555+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32555, yPoint_32555, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32555;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32555] === this.CELL_EMPTY
                    && map[y+1][xPoint_32555] === this.CELL_EMPTY
                    && map[y+1][xPoint_32555+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32555] === this.CELL_EMPTY
                    && map[y][xPoint_32555-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32555-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
