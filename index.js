...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_11573=0;yPoint_11573<this.gameFieldBorderY.length; yPoint_11573++){
                for(var xPoint_11573=0;xPoint_11573<this.gameFieldBorderX.length; xPoint_11573++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_11573, xPoint_11573);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_11573][xPoint_11573] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_11573, xPoint_11573, 'user');
                    if(this._userShipsMap[yPoint_11573][xPoint_11573] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_11573, xPoint_11573, type){
            var id = this.getPointBlockIdByCoords(yPoint_11573, xPoint_11573, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_11573);
                block.setAttribute('data-y', yPoint_11573);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_11573
         * @param {type} xPoint_11573
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_11573, xPoint_11573, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_11573 + '_y' + yPoint_11573;
            }
            return 'pc_x' + xPoint_11573 + '_y' + yPoint_11573;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_11573=0;yPoint_11573<this.gameFieldBorderY.length; yPoint_11573++){
                for(var xPoint_11573=0;xPoint_11573<this.gameFieldBorderX.length; xPoint_11573++){
                    map.push({y: yPoint_11573, x: xPoint_11573});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_11573=-1;yPoint_11573<(this.gameFieldBorderY.length+1); yPoint_11573++){
                for(var xPoint_11573=-1;xPoint_11573<(this.gameFieldBorderX.length+1); xPoint_11573++){
                    if(!map[yPoint_11573]){
                        map[yPoint_11573] = [];
                    }
                    map[yPoint_11573][xPoint_11573] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_11573 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_11573 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_11573, yPoint_11573) === true){
                    if(this.canPutHorizontal(map, xPoint_11573, yPoint_11573, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11573][xPoint_11573 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_11573, yPoint_11573, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_11573 + i][xPoint_11573] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_11573, yPoint_11573){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_11573][xPoint_11573] === this.CELL_EMPTY
                && map[yPoint_11573-1][xPoint_11573] === this.CELL_EMPTY
                && map[yPoint_11573-1][xPoint_11573+1] === this.CELL_EMPTY
                && map[yPoint_11573][xPoint_11573+1] === this.CELL_EMPTY
                && map[yPoint_11573+1][xPoint_11573+1] === this.CELL_EMPTY
                && map[yPoint_11573+1][xPoint_11573] === this.CELL_EMPTY
                && map[yPoint_11573+1][xPoint_11573-1] === this.CELL_EMPTY
                && map[yPoint_11573][xPoint_11573-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_11573, yPoint_11573, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_11573;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_11573][x] === this.CELL_EMPTY
                    && map[yPoint_11573-1][x] === this.CELL_EMPTY
                    && map[yPoint_11573-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11573][x+1] === this.CELL_EMPTY
                    && map[yPoint_11573+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_11573+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_11573, yPoint_11573, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_11573;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_11573] === this.CELL_EMPTY
                    && map[y+1][xPoint_11573] === this.CELL_EMPTY
                    && map[y+1][xPoint_11573+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_11573] === this.CELL_EMPTY
                    && map[y][xPoint_11573-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_11573-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
