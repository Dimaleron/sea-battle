...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24748=0;yPoint_24748<this.gameFieldBorderY.length; yPoint_24748++){
                for(var xPoint_24748=0;xPoint_24748<this.gameFieldBorderX.length; xPoint_24748++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24748, xPoint_24748);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24748][xPoint_24748] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24748, xPoint_24748, 'user');
                    if(this._userShipsMap[yPoint_24748][xPoint_24748] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24748, xPoint_24748, type){
            var id = this.getPointBlockIdByCoords(yPoint_24748, xPoint_24748, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24748);
                block.setAttribute('data-y', yPoint_24748);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24748
         * @param {type} xPoint_24748
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24748, xPoint_24748, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24748 + '_y' + yPoint_24748;
            }
            return 'pc_x' + xPoint_24748 + '_y' + yPoint_24748;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24748=0;yPoint_24748<this.gameFieldBorderY.length; yPoint_24748++){
                for(var xPoint_24748=0;xPoint_24748<this.gameFieldBorderX.length; xPoint_24748++){
                    map.push({y: yPoint_24748, x: xPoint_24748});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24748=-1;yPoint_24748<(this.gameFieldBorderY.length+1); yPoint_24748++){
                for(var xPoint_24748=-1;xPoint_24748<(this.gameFieldBorderX.length+1); xPoint_24748++){
                    if(!map[yPoint_24748]){
                        map[yPoint_24748] = [];
                    }
                    map[yPoint_24748][xPoint_24748] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24748 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24748 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24748, yPoint_24748) === true){
                    if(this.canPutHorizontal(map, xPoint_24748, yPoint_24748, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24748][xPoint_24748 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24748, yPoint_24748, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24748 + i][xPoint_24748] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24748, yPoint_24748){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24748][xPoint_24748] === this.CELL_EMPTY
                && map[yPoint_24748-1][xPoint_24748] === this.CELL_EMPTY
                && map[yPoint_24748-1][xPoint_24748+1] === this.CELL_EMPTY
                && map[yPoint_24748][xPoint_24748+1] === this.CELL_EMPTY
                && map[yPoint_24748+1][xPoint_24748+1] === this.CELL_EMPTY
                && map[yPoint_24748+1][xPoint_24748] === this.CELL_EMPTY
                && map[yPoint_24748+1][xPoint_24748-1] === this.CELL_EMPTY
                && map[yPoint_24748][xPoint_24748-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24748, yPoint_24748, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24748;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24748][x] === this.CELL_EMPTY
                    && map[yPoint_24748-1][x] === this.CELL_EMPTY
                    && map[yPoint_24748-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24748][x+1] === this.CELL_EMPTY
                    && map[yPoint_24748+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24748+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24748, yPoint_24748, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24748;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24748] === this.CELL_EMPTY
                    && map[y+1][xPoint_24748] === this.CELL_EMPTY
                    && map[y+1][xPoint_24748+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24748] === this.CELL_EMPTY
                    && map[y][xPoint_24748-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24748-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
