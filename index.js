...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7661=0;yPoint_7661<this.gameFieldBorderY.length; yPoint_7661++){
                for(var xPoint_7661=0;xPoint_7661<this.gameFieldBorderX.length; xPoint_7661++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7661, xPoint_7661);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7661][xPoint_7661] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7661, xPoint_7661, 'user');
                    if(this._userShipsMap[yPoint_7661][xPoint_7661] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7661, xPoint_7661, type){
            var id = this.getPointBlockIdByCoords(yPoint_7661, xPoint_7661, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7661);
                block.setAttribute('data-y', yPoint_7661);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7661
         * @param {type} xPoint_7661
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7661, xPoint_7661, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7661 + '_y' + yPoint_7661;
            }
            return 'pc_x' + xPoint_7661 + '_y' + yPoint_7661;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7661=0;yPoint_7661<this.gameFieldBorderY.length; yPoint_7661++){
                for(var xPoint_7661=0;xPoint_7661<this.gameFieldBorderX.length; xPoint_7661++){
                    map.push({y: yPoint_7661, x: xPoint_7661});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7661=-1;yPoint_7661<(this.gameFieldBorderY.length+1); yPoint_7661++){
                for(var xPoint_7661=-1;xPoint_7661<(this.gameFieldBorderX.length+1); xPoint_7661++){
                    if(!map[yPoint_7661]){
                        map[yPoint_7661] = [];
                    }
                    map[yPoint_7661][xPoint_7661] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7661 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7661 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7661, yPoint_7661) === true){
                    if(this.canPutHorizontal(map, xPoint_7661, yPoint_7661, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7661][xPoint_7661 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7661, yPoint_7661, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7661 + i][xPoint_7661] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7661, yPoint_7661){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7661][xPoint_7661] === this.CELL_EMPTY
                && map[yPoint_7661-1][xPoint_7661] === this.CELL_EMPTY
                && map[yPoint_7661-1][xPoint_7661+1] === this.CELL_EMPTY
                && map[yPoint_7661][xPoint_7661+1] === this.CELL_EMPTY
                && map[yPoint_7661+1][xPoint_7661+1] === this.CELL_EMPTY
                && map[yPoint_7661+1][xPoint_7661] === this.CELL_EMPTY
                && map[yPoint_7661+1][xPoint_7661-1] === this.CELL_EMPTY
                && map[yPoint_7661][xPoint_7661-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7661, yPoint_7661, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7661;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7661][x] === this.CELL_EMPTY
                    && map[yPoint_7661-1][x] === this.CELL_EMPTY
                    && map[yPoint_7661-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7661][x+1] === this.CELL_EMPTY
                    && map[yPoint_7661+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7661+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7661, yPoint_7661, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7661;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7661] === this.CELL_EMPTY
                    && map[y+1][xPoint_7661] === this.CELL_EMPTY
                    && map[y+1][xPoint_7661+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7661] === this.CELL_EMPTY
                    && map[y][xPoint_7661-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7661-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
