...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_806=0;yPoint_806<this.gameFieldBorderY.length; yPoint_806++){
                for(var xPoint_806=0;xPoint_806<this.gameFieldBorderX.length; xPoint_806++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_806, xPoint_806);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_806][xPoint_806] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_806, xPoint_806, 'user');
                    if(this._userShipsMap[yPoint_806][xPoint_806] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_806, xPoint_806, type){
            var id = this.getPointBlockIdByCoords(yPoint_806, xPoint_806, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_806);
                block.setAttribute('data-y', yPoint_806);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_806
         * @param {type} xPoint_806
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_806, xPoint_806, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_806 + '_y' + yPoint_806;
            }
            return 'pc_x' + xPoint_806 + '_y' + yPoint_806;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_806=0;yPoint_806<this.gameFieldBorderY.length; yPoint_806++){
                for(var xPoint_806=0;xPoint_806<this.gameFieldBorderX.length; xPoint_806++){
                    map.push({y: yPoint_806, x: xPoint_806});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_806=-1;yPoint_806<(this.gameFieldBorderY.length+1); yPoint_806++){
                for(var xPoint_806=-1;xPoint_806<(this.gameFieldBorderX.length+1); xPoint_806++){
                    if(!map[yPoint_806]){
                        map[yPoint_806] = [];
                    }
                    map[yPoint_806][xPoint_806] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_806 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_806 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_806, yPoint_806) === true){
                    if(this.canPutHorizontal(map, xPoint_806, yPoint_806, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_806][xPoint_806 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_806, yPoint_806, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_806 + i][xPoint_806] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_806, yPoint_806){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_806][xPoint_806] === this.CELL_EMPTY
                && map[yPoint_806-1][xPoint_806] === this.CELL_EMPTY
                && map[yPoint_806-1][xPoint_806+1] === this.CELL_EMPTY
                && map[yPoint_806][xPoint_806+1] === this.CELL_EMPTY
                && map[yPoint_806+1][xPoint_806+1] === this.CELL_EMPTY
                && map[yPoint_806+1][xPoint_806] === this.CELL_EMPTY
                && map[yPoint_806+1][xPoint_806-1] === this.CELL_EMPTY
                && map[yPoint_806][xPoint_806-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_806, yPoint_806, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_806;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_806][x] === this.CELL_EMPTY
                    && map[yPoint_806-1][x] === this.CELL_EMPTY
                    && map[yPoint_806-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_806][x+1] === this.CELL_EMPTY
                    && map[yPoint_806+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_806+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_806, yPoint_806, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_806;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_806] === this.CELL_EMPTY
                    && map[y+1][xPoint_806] === this.CELL_EMPTY
                    && map[y+1][xPoint_806+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_806] === this.CELL_EMPTY
                    && map[y][xPoint_806-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_806-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
