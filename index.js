...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14972=0;yPoint_14972<this.gameFieldBorderY.length; yPoint_14972++){
                for(var xPoint_14972=0;xPoint_14972<this.gameFieldBorderX.length; xPoint_14972++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14972, xPoint_14972);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14972][xPoint_14972] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14972, xPoint_14972, 'user');
                    if(this._userShipsMap[yPoint_14972][xPoint_14972] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14972, xPoint_14972, type){
            var id = this.getPointBlockIdByCoords(yPoint_14972, xPoint_14972, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14972);
                block.setAttribute('data-y', yPoint_14972);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14972
         * @param {type} xPoint_14972
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14972, xPoint_14972, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14972 + '_y' + yPoint_14972;
            }
            return 'pc_x' + xPoint_14972 + '_y' + yPoint_14972;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14972=0;yPoint_14972<this.gameFieldBorderY.length; yPoint_14972++){
                for(var xPoint_14972=0;xPoint_14972<this.gameFieldBorderX.length; xPoint_14972++){
                    map.push({y: yPoint_14972, x: xPoint_14972});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14972=-1;yPoint_14972<(this.gameFieldBorderY.length+1); yPoint_14972++){
                for(var xPoint_14972=-1;xPoint_14972<(this.gameFieldBorderX.length+1); xPoint_14972++){
                    if(!map[yPoint_14972]){
                        map[yPoint_14972] = [];
                    }
                    map[yPoint_14972][xPoint_14972] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14972 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14972 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14972, yPoint_14972) === true){
                    if(this.canPutHorizontal(map, xPoint_14972, yPoint_14972, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14972][xPoint_14972 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14972, yPoint_14972, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14972 + i][xPoint_14972] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14972, yPoint_14972){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14972][xPoint_14972] === this.CELL_EMPTY
                && map[yPoint_14972-1][xPoint_14972] === this.CELL_EMPTY
                && map[yPoint_14972-1][xPoint_14972+1] === this.CELL_EMPTY
                && map[yPoint_14972][xPoint_14972+1] === this.CELL_EMPTY
                && map[yPoint_14972+1][xPoint_14972+1] === this.CELL_EMPTY
                && map[yPoint_14972+1][xPoint_14972] === this.CELL_EMPTY
                && map[yPoint_14972+1][xPoint_14972-1] === this.CELL_EMPTY
                && map[yPoint_14972][xPoint_14972-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14972, yPoint_14972, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14972;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14972][x] === this.CELL_EMPTY
                    && map[yPoint_14972-1][x] === this.CELL_EMPTY
                    && map[yPoint_14972-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14972][x+1] === this.CELL_EMPTY
                    && map[yPoint_14972+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14972+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14972, yPoint_14972, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14972;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14972] === this.CELL_EMPTY
                    && map[y+1][xPoint_14972] === this.CELL_EMPTY
                    && map[y+1][xPoint_14972+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14972] === this.CELL_EMPTY
                    && map[y][xPoint_14972-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14972-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
