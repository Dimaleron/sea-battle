...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_175=0;yPoint_175<this.gameFieldBorderY.length; yPoint_175++){
                for(var xPoint_175=0;xPoint_175<this.gameFieldBorderX.length; xPoint_175++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_175, xPoint_175);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_175][xPoint_175] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_175, xPoint_175, 'user');
                    if(this._userShipsMap[yPoint_175][xPoint_175] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_175, xPoint_175, type){
            var id = this.getPointBlockIdByCoords(yPoint_175, xPoint_175, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_175);
                block.setAttribute('data-y', yPoint_175);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_175
         * @param {type} xPoint_175
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_175, xPoint_175, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_175 + '_y' + yPoint_175;
            }
            return 'pc_x' + xPoint_175 + '_y' + yPoint_175;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_175=0;yPoint_175<this.gameFieldBorderY.length; yPoint_175++){
                for(var xPoint_175=0;xPoint_175<this.gameFieldBorderX.length; xPoint_175++){
                    map.push({y: yPoint_175, x: xPoint_175});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_175=-1;yPoint_175<(this.gameFieldBorderY.length+1); yPoint_175++){
                for(var xPoint_175=-1;xPoint_175<(this.gameFieldBorderX.length+1); xPoint_175++){
                    if(!map[yPoint_175]){
                        map[yPoint_175] = [];
                    }
                    map[yPoint_175][xPoint_175] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_175 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_175 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_175, yPoint_175) === true){
                    if(this.canPutHorizontal(map, xPoint_175, yPoint_175, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_175][xPoint_175 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_175, yPoint_175, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_175 + i][xPoint_175] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_175, yPoint_175){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_175][xPoint_175] === this.CELL_EMPTY
                && map[yPoint_175-1][xPoint_175] === this.CELL_EMPTY
                && map[yPoint_175-1][xPoint_175+1] === this.CELL_EMPTY
                && map[yPoint_175][xPoint_175+1] === this.CELL_EMPTY
                && map[yPoint_175+1][xPoint_175+1] === this.CELL_EMPTY
                && map[yPoint_175+1][xPoint_175] === this.CELL_EMPTY
                && map[yPoint_175+1][xPoint_175-1] === this.CELL_EMPTY
                && map[yPoint_175][xPoint_175-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_175, yPoint_175, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_175;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_175][x] === this.CELL_EMPTY
                    && map[yPoint_175-1][x] === this.CELL_EMPTY
                    && map[yPoint_175-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_175][x+1] === this.CELL_EMPTY
                    && map[yPoint_175+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_175+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_175, yPoint_175, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_175;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_175] === this.CELL_EMPTY
                    && map[y+1][xPoint_175] === this.CELL_EMPTY
                    && map[y+1][xPoint_175+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_175] === this.CELL_EMPTY
                    && map[y][xPoint_175-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_175-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
