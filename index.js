...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_13542=0;yPoint_13542<this.gameFieldBorderY.length; yPoint_13542++){
                for(var xPoint_13542=0;xPoint_13542<this.gameFieldBorderX.length; xPoint_13542++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_13542, xPoint_13542);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_13542][xPoint_13542] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_13542, xPoint_13542, 'user');
                    if(this._userShipsMap[yPoint_13542][xPoint_13542] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_13542, xPoint_13542, type){
            var id = this.getPointBlockIdByCoords(yPoint_13542, xPoint_13542, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_13542);
                block.setAttribute('data-y', yPoint_13542);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_13542
         * @param {type} xPoint_13542
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_13542, xPoint_13542, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_13542 + '_y' + yPoint_13542;
            }
            return 'pc_x' + xPoint_13542 + '_y' + yPoint_13542;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_13542=0;yPoint_13542<this.gameFieldBorderY.length; yPoint_13542++){
                for(var xPoint_13542=0;xPoint_13542<this.gameFieldBorderX.length; xPoint_13542++){
                    map.push({y: yPoint_13542, x: xPoint_13542});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_13542=-1;yPoint_13542<(this.gameFieldBorderY.length+1); yPoint_13542++){
                for(var xPoint_13542=-1;xPoint_13542<(this.gameFieldBorderX.length+1); xPoint_13542++){
                    if(!map[yPoint_13542]){
                        map[yPoint_13542] = [];
                    }
                    map[yPoint_13542][xPoint_13542] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_13542 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_13542 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_13542, yPoint_13542) === true){
                    if(this.canPutHorizontal(map, xPoint_13542, yPoint_13542, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13542][xPoint_13542 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_13542, yPoint_13542, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_13542 + i][xPoint_13542] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_13542, yPoint_13542){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_13542][xPoint_13542] === this.CELL_EMPTY
                && map[yPoint_13542-1][xPoint_13542] === this.CELL_EMPTY
                && map[yPoint_13542-1][xPoint_13542+1] === this.CELL_EMPTY
                && map[yPoint_13542][xPoint_13542+1] === this.CELL_EMPTY
                && map[yPoint_13542+1][xPoint_13542+1] === this.CELL_EMPTY
                && map[yPoint_13542+1][xPoint_13542] === this.CELL_EMPTY
                && map[yPoint_13542+1][xPoint_13542-1] === this.CELL_EMPTY
                && map[yPoint_13542][xPoint_13542-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_13542, yPoint_13542, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_13542;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_13542][x] === this.CELL_EMPTY
                    && map[yPoint_13542-1][x] === this.CELL_EMPTY
                    && map[yPoint_13542-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13542][x+1] === this.CELL_EMPTY
                    && map[yPoint_13542+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_13542+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_13542, yPoint_13542, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_13542;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_13542] === this.CELL_EMPTY
                    && map[y+1][xPoint_13542] === this.CELL_EMPTY
                    && map[y+1][xPoint_13542+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_13542] === this.CELL_EMPTY
                    && map[y][xPoint_13542-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_13542-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
