...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29584=0;yPoint_29584<this.gameFieldBorderY.length; yPoint_29584++){
                for(var xPoint_29584=0;xPoint_29584<this.gameFieldBorderX.length; xPoint_29584++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29584, xPoint_29584);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29584][xPoint_29584] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29584, xPoint_29584, 'user');
                    if(this._userShipsMap[yPoint_29584][xPoint_29584] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29584, xPoint_29584, type){
            var id = this.getPointBlockIdByCoords(yPoint_29584, xPoint_29584, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29584);
                block.setAttribute('data-y', yPoint_29584);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29584
         * @param {type} xPoint_29584
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29584, xPoint_29584, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29584 + '_y' + yPoint_29584;
            }
            return 'pc_x' + xPoint_29584 + '_y' + yPoint_29584;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29584=0;yPoint_29584<this.gameFieldBorderY.length; yPoint_29584++){
                for(var xPoint_29584=0;xPoint_29584<this.gameFieldBorderX.length; xPoint_29584++){
                    map.push({y: yPoint_29584, x: xPoint_29584});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29584=-1;yPoint_29584<(this.gameFieldBorderY.length+1); yPoint_29584++){
                for(var xPoint_29584=-1;xPoint_29584<(this.gameFieldBorderX.length+1); xPoint_29584++){
                    if(!map[yPoint_29584]){
                        map[yPoint_29584] = [];
                    }
                    map[yPoint_29584][xPoint_29584] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29584 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29584 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29584, yPoint_29584) === true){
                    if(this.canPutHorizontal(map, xPoint_29584, yPoint_29584, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29584][xPoint_29584 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29584, yPoint_29584, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29584 + i][xPoint_29584] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29584, yPoint_29584){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29584][xPoint_29584] === this.CELL_EMPTY
                && map[yPoint_29584-1][xPoint_29584] === this.CELL_EMPTY
                && map[yPoint_29584-1][xPoint_29584+1] === this.CELL_EMPTY
                && map[yPoint_29584][xPoint_29584+1] === this.CELL_EMPTY
                && map[yPoint_29584+1][xPoint_29584+1] === this.CELL_EMPTY
                && map[yPoint_29584+1][xPoint_29584] === this.CELL_EMPTY
                && map[yPoint_29584+1][xPoint_29584-1] === this.CELL_EMPTY
                && map[yPoint_29584][xPoint_29584-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29584, yPoint_29584, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29584;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29584][x] === this.CELL_EMPTY
                    && map[yPoint_29584-1][x] === this.CELL_EMPTY
                    && map[yPoint_29584-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29584][x+1] === this.CELL_EMPTY
                    && map[yPoint_29584+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29584+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29584, yPoint_29584, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29584;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29584] === this.CELL_EMPTY
                    && map[y+1][xPoint_29584] === this.CELL_EMPTY
                    && map[y+1][xPoint_29584+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29584] === this.CELL_EMPTY
                    && map[y][xPoint_29584-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29584-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
