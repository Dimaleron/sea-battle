...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31098=0;yPoint_31098<this.gameFieldBorderY.length; yPoint_31098++){
                for(var xPoint_31098=0;xPoint_31098<this.gameFieldBorderX.length; xPoint_31098++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31098, xPoint_31098);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31098][xPoint_31098] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31098, xPoint_31098, 'user');
                    if(this._userShipsMap[yPoint_31098][xPoint_31098] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31098, xPoint_31098, type){
            var id = this.getPointBlockIdByCoords(yPoint_31098, xPoint_31098, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31098);
                block.setAttribute('data-y', yPoint_31098);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31098
         * @param {type} xPoint_31098
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31098, xPoint_31098, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31098 + '_y' + yPoint_31098;
            }
            return 'pc_x' + xPoint_31098 + '_y' + yPoint_31098;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31098=0;yPoint_31098<this.gameFieldBorderY.length; yPoint_31098++){
                for(var xPoint_31098=0;xPoint_31098<this.gameFieldBorderX.length; xPoint_31098++){
                    map.push({y: yPoint_31098, x: xPoint_31098});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31098=-1;yPoint_31098<(this.gameFieldBorderY.length+1); yPoint_31098++){
                for(var xPoint_31098=-1;xPoint_31098<(this.gameFieldBorderX.length+1); xPoint_31098++){
                    if(!map[yPoint_31098]){
                        map[yPoint_31098] = [];
                    }
                    map[yPoint_31098][xPoint_31098] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31098 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31098 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31098, yPoint_31098) === true){
                    if(this.canPutHorizontal(map, xPoint_31098, yPoint_31098, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31098][xPoint_31098 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31098, yPoint_31098, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31098 + i][xPoint_31098] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31098, yPoint_31098){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31098][xPoint_31098] === this.CELL_EMPTY
                && map[yPoint_31098-1][xPoint_31098] === this.CELL_EMPTY
                && map[yPoint_31098-1][xPoint_31098+1] === this.CELL_EMPTY
                && map[yPoint_31098][xPoint_31098+1] === this.CELL_EMPTY
                && map[yPoint_31098+1][xPoint_31098+1] === this.CELL_EMPTY
                && map[yPoint_31098+1][xPoint_31098] === this.CELL_EMPTY
                && map[yPoint_31098+1][xPoint_31098-1] === this.CELL_EMPTY
                && map[yPoint_31098][xPoint_31098-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31098, yPoint_31098, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31098;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31098][x] === this.CELL_EMPTY
                    && map[yPoint_31098-1][x] === this.CELL_EMPTY
                    && map[yPoint_31098-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31098][x+1] === this.CELL_EMPTY
                    && map[yPoint_31098+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31098+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31098, yPoint_31098, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31098;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31098] === this.CELL_EMPTY
                    && map[y+1][xPoint_31098] === this.CELL_EMPTY
                    && map[y+1][xPoint_31098+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31098] === this.CELL_EMPTY
                    && map[y][xPoint_31098-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31098-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
