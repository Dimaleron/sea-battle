...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31693=0;yPoint_31693<this.gameFieldBorderY.length; yPoint_31693++){
                for(var xPoint_31693=0;xPoint_31693<this.gameFieldBorderX.length; xPoint_31693++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31693, xPoint_31693);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31693][xPoint_31693] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31693, xPoint_31693, 'user');
                    if(this._userShipsMap[yPoint_31693][xPoint_31693] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31693, xPoint_31693, type){
            var id = this.getPointBlockIdByCoords(yPoint_31693, xPoint_31693, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31693);
                block.setAttribute('data-y', yPoint_31693);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31693
         * @param {type} xPoint_31693
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31693, xPoint_31693, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31693 + '_y' + yPoint_31693;
            }
            return 'pc_x' + xPoint_31693 + '_y' + yPoint_31693;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31693=0;yPoint_31693<this.gameFieldBorderY.length; yPoint_31693++){
                for(var xPoint_31693=0;xPoint_31693<this.gameFieldBorderX.length; xPoint_31693++){
                    map.push({y: yPoint_31693, x: xPoint_31693});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31693=-1;yPoint_31693<(this.gameFieldBorderY.length+1); yPoint_31693++){
                for(var xPoint_31693=-1;xPoint_31693<(this.gameFieldBorderX.length+1); xPoint_31693++){
                    if(!map[yPoint_31693]){
                        map[yPoint_31693] = [];
                    }
                    map[yPoint_31693][xPoint_31693] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31693 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31693 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31693, yPoint_31693) === true){
                    if(this.canPutHorizontal(map, xPoint_31693, yPoint_31693, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31693][xPoint_31693 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31693, yPoint_31693, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31693 + i][xPoint_31693] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31693, yPoint_31693){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31693][xPoint_31693] === this.CELL_EMPTY
                && map[yPoint_31693-1][xPoint_31693] === this.CELL_EMPTY
                && map[yPoint_31693-1][xPoint_31693+1] === this.CELL_EMPTY
                && map[yPoint_31693][xPoint_31693+1] === this.CELL_EMPTY
                && map[yPoint_31693+1][xPoint_31693+1] === this.CELL_EMPTY
                && map[yPoint_31693+1][xPoint_31693] === this.CELL_EMPTY
                && map[yPoint_31693+1][xPoint_31693-1] === this.CELL_EMPTY
                && map[yPoint_31693][xPoint_31693-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31693, yPoint_31693, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31693;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31693][x] === this.CELL_EMPTY
                    && map[yPoint_31693-1][x] === this.CELL_EMPTY
                    && map[yPoint_31693-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31693][x+1] === this.CELL_EMPTY
                    && map[yPoint_31693+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31693+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31693, yPoint_31693, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31693;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31693] === this.CELL_EMPTY
                    && map[y+1][xPoint_31693] === this.CELL_EMPTY
                    && map[y+1][xPoint_31693+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31693] === this.CELL_EMPTY
                    && map[y][xPoint_31693-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31693-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
