...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7836=0;yPoint_7836<this.gameFieldBorderY.length; yPoint_7836++){
                for(var xPoint_7836=0;xPoint_7836<this.gameFieldBorderX.length; xPoint_7836++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7836, xPoint_7836);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7836][xPoint_7836] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7836, xPoint_7836, 'user');
                    if(this._userShipsMap[yPoint_7836][xPoint_7836] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7836, xPoint_7836, type){
            var id = this.getPointBlockIdByCoords(yPoint_7836, xPoint_7836, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7836);
                block.setAttribute('data-y', yPoint_7836);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7836
         * @param {type} xPoint_7836
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7836, xPoint_7836, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7836 + '_y' + yPoint_7836;
            }
            return 'pc_x' + xPoint_7836 + '_y' + yPoint_7836;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7836=0;yPoint_7836<this.gameFieldBorderY.length; yPoint_7836++){
                for(var xPoint_7836=0;xPoint_7836<this.gameFieldBorderX.length; xPoint_7836++){
                    map.push({y: yPoint_7836, x: xPoint_7836});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7836=-1;yPoint_7836<(this.gameFieldBorderY.length+1); yPoint_7836++){
                for(var xPoint_7836=-1;xPoint_7836<(this.gameFieldBorderX.length+1); xPoint_7836++){
                    if(!map[yPoint_7836]){
                        map[yPoint_7836] = [];
                    }
                    map[yPoint_7836][xPoint_7836] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7836 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7836 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7836, yPoint_7836) === true){
                    if(this.canPutHorizontal(map, xPoint_7836, yPoint_7836, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7836][xPoint_7836 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7836, yPoint_7836, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7836 + i][xPoint_7836] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7836, yPoint_7836){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7836][xPoint_7836] === this.CELL_EMPTY
                && map[yPoint_7836-1][xPoint_7836] === this.CELL_EMPTY
                && map[yPoint_7836-1][xPoint_7836+1] === this.CELL_EMPTY
                && map[yPoint_7836][xPoint_7836+1] === this.CELL_EMPTY
                && map[yPoint_7836+1][xPoint_7836+1] === this.CELL_EMPTY
                && map[yPoint_7836+1][xPoint_7836] === this.CELL_EMPTY
                && map[yPoint_7836+1][xPoint_7836-1] === this.CELL_EMPTY
                && map[yPoint_7836][xPoint_7836-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7836, yPoint_7836, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7836;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7836][x] === this.CELL_EMPTY
                    && map[yPoint_7836-1][x] === this.CELL_EMPTY
                    && map[yPoint_7836-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7836][x+1] === this.CELL_EMPTY
                    && map[yPoint_7836+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7836+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7836, yPoint_7836, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7836;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7836] === this.CELL_EMPTY
                    && map[y+1][xPoint_7836] === this.CELL_EMPTY
                    && map[y+1][xPoint_7836+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7836] === this.CELL_EMPTY
                    && map[y][xPoint_7836-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7836-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
