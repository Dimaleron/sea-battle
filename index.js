...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_702=0;yPoint_702<this.gameFieldBorderY.length; yPoint_702++){
                for(var xPoint_702=0;xPoint_702<this.gameFieldBorderX.length; xPoint_702++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_702, xPoint_702);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_702][xPoint_702] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_702, xPoint_702, 'user');
                    if(this._userShipsMap[yPoint_702][xPoint_702] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_702, xPoint_702, type){
            var id = this.getPointBlockIdByCoords(yPoint_702, xPoint_702, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_702);
                block.setAttribute('data-y', yPoint_702);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_702
         * @param {type} xPoint_702
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_702, xPoint_702, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_702 + '_y' + yPoint_702;
            }
            return 'pc_x' + xPoint_702 + '_y' + yPoint_702;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_702=0;yPoint_702<this.gameFieldBorderY.length; yPoint_702++){
                for(var xPoint_702=0;xPoint_702<this.gameFieldBorderX.length; xPoint_702++){
                    map.push({y: yPoint_702, x: xPoint_702});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_702=-1;yPoint_702<(this.gameFieldBorderY.length+1); yPoint_702++){
                for(var xPoint_702=-1;xPoint_702<(this.gameFieldBorderX.length+1); xPoint_702++){
                    if(!map[yPoint_702]){
                        map[yPoint_702] = [];
                    }
                    map[yPoint_702][xPoint_702] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_702 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_702 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_702, yPoint_702) === true){
                    if(this.canPutHorizontal(map, xPoint_702, yPoint_702, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_702][xPoint_702 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_702, yPoint_702, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_702 + i][xPoint_702] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_702, yPoint_702){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_702][xPoint_702] === this.CELL_EMPTY
                && map[yPoint_702-1][xPoint_702] === this.CELL_EMPTY
                && map[yPoint_702-1][xPoint_702+1] === this.CELL_EMPTY
                && map[yPoint_702][xPoint_702+1] === this.CELL_EMPTY
                && map[yPoint_702+1][xPoint_702+1] === this.CELL_EMPTY
                && map[yPoint_702+1][xPoint_702] === this.CELL_EMPTY
                && map[yPoint_702+1][xPoint_702-1] === this.CELL_EMPTY
                && map[yPoint_702][xPoint_702-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_702, yPoint_702, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_702;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_702][x] === this.CELL_EMPTY
                    && map[yPoint_702-1][x] === this.CELL_EMPTY
                    && map[yPoint_702-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_702][x+1] === this.CELL_EMPTY
                    && map[yPoint_702+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_702+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_702, yPoint_702, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_702;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_702] === this.CELL_EMPTY
                    && map[y+1][xPoint_702] === this.CELL_EMPTY
                    && map[y+1][xPoint_702+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_702] === this.CELL_EMPTY
                    && map[y][xPoint_702-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_702-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
