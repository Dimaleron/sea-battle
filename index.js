...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_265=0;yPoint_265<this.gameFieldBorderY.length; yPoint_265++){
                for(var xPoint_265=0;xPoint_265<this.gameFieldBorderX.length; xPoint_265++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_265, xPoint_265);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_265][xPoint_265] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_265, xPoint_265, 'user');
                    if(this._userShipsMap[yPoint_265][xPoint_265] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_265, xPoint_265, type){
            var id = this.getPointBlockIdByCoords(yPoint_265, xPoint_265, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_265);
                block.setAttribute('data-y', yPoint_265);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_265
         * @param {type} xPoint_265
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_265, xPoint_265, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_265 + '_y' + yPoint_265;
            }
            return 'pc_x' + xPoint_265 + '_y' + yPoint_265;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_265=0;yPoint_265<this.gameFieldBorderY.length; yPoint_265++){
                for(var xPoint_265=0;xPoint_265<this.gameFieldBorderX.length; xPoint_265++){
                    map.push({y: yPoint_265, x: xPoint_265});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_265=-1;yPoint_265<(this.gameFieldBorderY.length+1); yPoint_265++){
                for(var xPoint_265=-1;xPoint_265<(this.gameFieldBorderX.length+1); xPoint_265++){
                    if(!map[yPoint_265]){
                        map[yPoint_265] = [];
                    }
                    map[yPoint_265][xPoint_265] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_265 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_265 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_265, yPoint_265) === true){
                    if(this.canPutHorizontal(map, xPoint_265, yPoint_265, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_265][xPoint_265 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_265, yPoint_265, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_265 + i][xPoint_265] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_265, yPoint_265){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_265][xPoint_265] === this.CELL_EMPTY
                && map[yPoint_265-1][xPoint_265] === this.CELL_EMPTY
                && map[yPoint_265-1][xPoint_265+1] === this.CELL_EMPTY
                && map[yPoint_265][xPoint_265+1] === this.CELL_EMPTY
                && map[yPoint_265+1][xPoint_265+1] === this.CELL_EMPTY
                && map[yPoint_265+1][xPoint_265] === this.CELL_EMPTY
                && map[yPoint_265+1][xPoint_265-1] === this.CELL_EMPTY
                && map[yPoint_265][xPoint_265-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_265, yPoint_265, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_265;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_265][x] === this.CELL_EMPTY
                    && map[yPoint_265-1][x] === this.CELL_EMPTY
                    && map[yPoint_265-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_265][x+1] === this.CELL_EMPTY
                    && map[yPoint_265+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_265+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_265, yPoint_265, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_265;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_265] === this.CELL_EMPTY
                    && map[y+1][xPoint_265] === this.CELL_EMPTY
                    && map[y+1][xPoint_265+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_265] === this.CELL_EMPTY
                    && map[y][xPoint_265-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_265-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
