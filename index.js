...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14905=0;yPoint_14905<this.gameFieldBorderY.length; yPoint_14905++){
                for(var xPoint_14905=0;xPoint_14905<this.gameFieldBorderX.length; xPoint_14905++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14905, xPoint_14905);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14905][xPoint_14905] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14905, xPoint_14905, 'user');
                    if(this._userShipsMap[yPoint_14905][xPoint_14905] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14905, xPoint_14905, type){
            var id = this.getPointBlockIdByCoords(yPoint_14905, xPoint_14905, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14905);
                block.setAttribute('data-y', yPoint_14905);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14905
         * @param {type} xPoint_14905
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14905, xPoint_14905, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14905 + '_y' + yPoint_14905;
            }
            return 'pc_x' + xPoint_14905 + '_y' + yPoint_14905;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14905=0;yPoint_14905<this.gameFieldBorderY.length; yPoint_14905++){
                for(var xPoint_14905=0;xPoint_14905<this.gameFieldBorderX.length; xPoint_14905++){
                    map.push({y: yPoint_14905, x: xPoint_14905});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14905=-1;yPoint_14905<(this.gameFieldBorderY.length+1); yPoint_14905++){
                for(var xPoint_14905=-1;xPoint_14905<(this.gameFieldBorderX.length+1); xPoint_14905++){
                    if(!map[yPoint_14905]){
                        map[yPoint_14905] = [];
                    }
                    map[yPoint_14905][xPoint_14905] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14905 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14905 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14905, yPoint_14905) === true){
                    if(this.canPutHorizontal(map, xPoint_14905, yPoint_14905, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14905][xPoint_14905 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14905, yPoint_14905, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14905 + i][xPoint_14905] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14905, yPoint_14905){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14905][xPoint_14905] === this.CELL_EMPTY
                && map[yPoint_14905-1][xPoint_14905] === this.CELL_EMPTY
                && map[yPoint_14905-1][xPoint_14905+1] === this.CELL_EMPTY
                && map[yPoint_14905][xPoint_14905+1] === this.CELL_EMPTY
                && map[yPoint_14905+1][xPoint_14905+1] === this.CELL_EMPTY
                && map[yPoint_14905+1][xPoint_14905] === this.CELL_EMPTY
                && map[yPoint_14905+1][xPoint_14905-1] === this.CELL_EMPTY
                && map[yPoint_14905][xPoint_14905-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14905, yPoint_14905, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14905;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14905][x] === this.CELL_EMPTY
                    && map[yPoint_14905-1][x] === this.CELL_EMPTY
                    && map[yPoint_14905-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14905][x+1] === this.CELL_EMPTY
                    && map[yPoint_14905+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14905+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14905, yPoint_14905, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14905;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14905] === this.CELL_EMPTY
                    && map[y+1][xPoint_14905] === this.CELL_EMPTY
                    && map[y+1][xPoint_14905+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14905] === this.CELL_EMPTY
                    && map[y][xPoint_14905-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14905-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
