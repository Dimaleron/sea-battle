...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31358=0;yPoint_31358<this.gameFieldBorderY.length; yPoint_31358++){
                for(var xPoint_31358=0;xPoint_31358<this.gameFieldBorderX.length; xPoint_31358++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31358, xPoint_31358);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31358][xPoint_31358] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31358, xPoint_31358, 'user');
                    if(this._userShipsMap[yPoint_31358][xPoint_31358] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31358, xPoint_31358, type){
            var id = this.getPointBlockIdByCoords(yPoint_31358, xPoint_31358, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31358);
                block.setAttribute('data-y', yPoint_31358);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31358
         * @param {type} xPoint_31358
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31358, xPoint_31358, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31358 + '_y' + yPoint_31358;
            }
            return 'pc_x' + xPoint_31358 + '_y' + yPoint_31358;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31358=0;yPoint_31358<this.gameFieldBorderY.length; yPoint_31358++){
                for(var xPoint_31358=0;xPoint_31358<this.gameFieldBorderX.length; xPoint_31358++){
                    map.push({y: yPoint_31358, x: xPoint_31358});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31358=-1;yPoint_31358<(this.gameFieldBorderY.length+1); yPoint_31358++){
                for(var xPoint_31358=-1;xPoint_31358<(this.gameFieldBorderX.length+1); xPoint_31358++){
                    if(!map[yPoint_31358]){
                        map[yPoint_31358] = [];
                    }
                    map[yPoint_31358][xPoint_31358] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31358 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31358 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31358, yPoint_31358) === true){
                    if(this.canPutHorizontal(map, xPoint_31358, yPoint_31358, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31358][xPoint_31358 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31358, yPoint_31358, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31358 + i][xPoint_31358] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31358, yPoint_31358){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31358][xPoint_31358] === this.CELL_EMPTY
                && map[yPoint_31358-1][xPoint_31358] === this.CELL_EMPTY
                && map[yPoint_31358-1][xPoint_31358+1] === this.CELL_EMPTY
                && map[yPoint_31358][xPoint_31358+1] === this.CELL_EMPTY
                && map[yPoint_31358+1][xPoint_31358+1] === this.CELL_EMPTY
                && map[yPoint_31358+1][xPoint_31358] === this.CELL_EMPTY
                && map[yPoint_31358+1][xPoint_31358-1] === this.CELL_EMPTY
                && map[yPoint_31358][xPoint_31358-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31358, yPoint_31358, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31358;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31358][x] === this.CELL_EMPTY
                    && map[yPoint_31358-1][x] === this.CELL_EMPTY
                    && map[yPoint_31358-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31358][x+1] === this.CELL_EMPTY
                    && map[yPoint_31358+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31358+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31358, yPoint_31358, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31358;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31358] === this.CELL_EMPTY
                    && map[y+1][xPoint_31358] === this.CELL_EMPTY
                    && map[y+1][xPoint_31358+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31358] === this.CELL_EMPTY
                    && map[y][xPoint_31358-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31358-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
