...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_4349=0;yPoint_4349<this.gameFieldBorderY.length; yPoint_4349++){
                for(var xPoint_4349=0;xPoint_4349<this.gameFieldBorderX.length; xPoint_4349++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_4349, xPoint_4349);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_4349][xPoint_4349] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_4349, xPoint_4349, 'user');
                    if(this._userShipsMap[yPoint_4349][xPoint_4349] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_4349, xPoint_4349, type){
            var id = this.getPointBlockIdByCoords(yPoint_4349, xPoint_4349, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_4349);
                block.setAttribute('data-y', yPoint_4349);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_4349
         * @param {type} xPoint_4349
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_4349, xPoint_4349, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_4349 + '_y' + yPoint_4349;
            }
            return 'pc_x' + xPoint_4349 + '_y' + yPoint_4349;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_4349=0;yPoint_4349<this.gameFieldBorderY.length; yPoint_4349++){
                for(var xPoint_4349=0;xPoint_4349<this.gameFieldBorderX.length; xPoint_4349++){
                    map.push({y: yPoint_4349, x: xPoint_4349});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_4349=-1;yPoint_4349<(this.gameFieldBorderY.length+1); yPoint_4349++){
                for(var xPoint_4349=-1;xPoint_4349<(this.gameFieldBorderX.length+1); xPoint_4349++){
                    if(!map[yPoint_4349]){
                        map[yPoint_4349] = [];
                    }
                    map[yPoint_4349][xPoint_4349] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_4349 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_4349 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_4349, yPoint_4349) === true){
                    if(this.canPutHorizontal(map, xPoint_4349, yPoint_4349, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4349][xPoint_4349 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_4349, yPoint_4349, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_4349 + i][xPoint_4349] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_4349, yPoint_4349){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_4349][xPoint_4349] === this.CELL_EMPTY
                && map[yPoint_4349-1][xPoint_4349] === this.CELL_EMPTY
                && map[yPoint_4349-1][xPoint_4349+1] === this.CELL_EMPTY
                && map[yPoint_4349][xPoint_4349+1] === this.CELL_EMPTY
                && map[yPoint_4349+1][xPoint_4349+1] === this.CELL_EMPTY
                && map[yPoint_4349+1][xPoint_4349] === this.CELL_EMPTY
                && map[yPoint_4349+1][xPoint_4349-1] === this.CELL_EMPTY
                && map[yPoint_4349][xPoint_4349-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_4349, yPoint_4349, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_4349;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_4349][x] === this.CELL_EMPTY
                    && map[yPoint_4349-1][x] === this.CELL_EMPTY
                    && map[yPoint_4349-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4349][x+1] === this.CELL_EMPTY
                    && map[yPoint_4349+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_4349+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_4349, yPoint_4349, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_4349;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_4349] === this.CELL_EMPTY
                    && map[y+1][xPoint_4349] === this.CELL_EMPTY
                    && map[y+1][xPoint_4349+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_4349] === this.CELL_EMPTY
                    && map[y][xPoint_4349-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_4349-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
