...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5995=0;yPoint_5995<this.gameFieldBorderY.length; yPoint_5995++){
                for(var xPoint_5995=0;xPoint_5995<this.gameFieldBorderX.length; xPoint_5995++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5995, xPoint_5995);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5995][xPoint_5995] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5995, xPoint_5995, 'user');
                    if(this._userShipsMap[yPoint_5995][xPoint_5995] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5995, xPoint_5995, type){
            var id = this.getPointBlockIdByCoords(yPoint_5995, xPoint_5995, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5995);
                block.setAttribute('data-y', yPoint_5995);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5995
         * @param {type} xPoint_5995
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5995, xPoint_5995, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5995 + '_y' + yPoint_5995;
            }
            return 'pc_x' + xPoint_5995 + '_y' + yPoint_5995;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5995=0;yPoint_5995<this.gameFieldBorderY.length; yPoint_5995++){
                for(var xPoint_5995=0;xPoint_5995<this.gameFieldBorderX.length; xPoint_5995++){
                    map.push({y: yPoint_5995, x: xPoint_5995});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5995=-1;yPoint_5995<(this.gameFieldBorderY.length+1); yPoint_5995++){
                for(var xPoint_5995=-1;xPoint_5995<(this.gameFieldBorderX.length+1); xPoint_5995++){
                    if(!map[yPoint_5995]){
                        map[yPoint_5995] = [];
                    }
                    map[yPoint_5995][xPoint_5995] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5995 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5995 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5995, yPoint_5995) === true){
                    if(this.canPutHorizontal(map, xPoint_5995, yPoint_5995, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5995][xPoint_5995 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5995, yPoint_5995, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5995 + i][xPoint_5995] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5995, yPoint_5995){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5995][xPoint_5995] === this.CELL_EMPTY
                && map[yPoint_5995-1][xPoint_5995] === this.CELL_EMPTY
                && map[yPoint_5995-1][xPoint_5995+1] === this.CELL_EMPTY
                && map[yPoint_5995][xPoint_5995+1] === this.CELL_EMPTY
                && map[yPoint_5995+1][xPoint_5995+1] === this.CELL_EMPTY
                && map[yPoint_5995+1][xPoint_5995] === this.CELL_EMPTY
                && map[yPoint_5995+1][xPoint_5995-1] === this.CELL_EMPTY
                && map[yPoint_5995][xPoint_5995-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5995, yPoint_5995, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5995;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5995][x] === this.CELL_EMPTY
                    && map[yPoint_5995-1][x] === this.CELL_EMPTY
                    && map[yPoint_5995-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5995][x+1] === this.CELL_EMPTY
                    && map[yPoint_5995+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5995+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5995, yPoint_5995, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5995;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5995] === this.CELL_EMPTY
                    && map[y+1][xPoint_5995] === this.CELL_EMPTY
                    && map[y+1][xPoint_5995+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5995] === this.CELL_EMPTY
                    && map[y][xPoint_5995-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5995-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
