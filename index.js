...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_1311=0;yPoint_1311<this.gameFieldBorderY.length; yPoint_1311++){
                for(var xPoint_1311=0;xPoint_1311<this.gameFieldBorderX.length; xPoint_1311++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_1311, xPoint_1311);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_1311][xPoint_1311] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_1311, xPoint_1311, 'user');
                    if(this._userShipsMap[yPoint_1311][xPoint_1311] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_1311, xPoint_1311, type){
            var id = this.getPointBlockIdByCoords(yPoint_1311, xPoint_1311, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_1311);
                block.setAttribute('data-y', yPoint_1311);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_1311
         * @param {type} xPoint_1311
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_1311, xPoint_1311, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_1311 + '_y' + yPoint_1311;
            }
            return 'pc_x' + xPoint_1311 + '_y' + yPoint_1311;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_1311=0;yPoint_1311<this.gameFieldBorderY.length; yPoint_1311++){
                for(var xPoint_1311=0;xPoint_1311<this.gameFieldBorderX.length; xPoint_1311++){
                    map.push({y: yPoint_1311, x: xPoint_1311});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_1311=-1;yPoint_1311<(this.gameFieldBorderY.length+1); yPoint_1311++){
                for(var xPoint_1311=-1;xPoint_1311<(this.gameFieldBorderX.length+1); xPoint_1311++){
                    if(!map[yPoint_1311]){
                        map[yPoint_1311] = [];
                    }
                    map[yPoint_1311][xPoint_1311] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_1311 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_1311 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_1311, yPoint_1311) === true){
                    if(this.canPutHorizontal(map, xPoint_1311, yPoint_1311, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1311][xPoint_1311 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_1311, yPoint_1311, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_1311 + i][xPoint_1311] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_1311, yPoint_1311){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_1311][xPoint_1311] === this.CELL_EMPTY
                && map[yPoint_1311-1][xPoint_1311] === this.CELL_EMPTY
                && map[yPoint_1311-1][xPoint_1311+1] === this.CELL_EMPTY
                && map[yPoint_1311][xPoint_1311+1] === this.CELL_EMPTY
                && map[yPoint_1311+1][xPoint_1311+1] === this.CELL_EMPTY
                && map[yPoint_1311+1][xPoint_1311] === this.CELL_EMPTY
                && map[yPoint_1311+1][xPoint_1311-1] === this.CELL_EMPTY
                && map[yPoint_1311][xPoint_1311-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_1311, yPoint_1311, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_1311;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_1311][x] === this.CELL_EMPTY
                    && map[yPoint_1311-1][x] === this.CELL_EMPTY
                    && map[yPoint_1311-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1311][x+1] === this.CELL_EMPTY
                    && map[yPoint_1311+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_1311+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_1311, yPoint_1311, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_1311;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_1311] === this.CELL_EMPTY
                    && map[y+1][xPoint_1311] === this.CELL_EMPTY
                    && map[y+1][xPoint_1311+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_1311] === this.CELL_EMPTY
                    && map[y][xPoint_1311-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_1311-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
