...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_19499=0;yPoint_19499<this.gameFieldBorderY.length; yPoint_19499++){
                for(var xPoint_19499=0;xPoint_19499<this.gameFieldBorderX.length; xPoint_19499++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_19499, xPoint_19499);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_19499][xPoint_19499] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_19499, xPoint_19499, 'user');
                    if(this._userShipsMap[yPoint_19499][xPoint_19499] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_19499, xPoint_19499, type){
            var id = this.getPointBlockIdByCoords(yPoint_19499, xPoint_19499, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_19499);
                block.setAttribute('data-y', yPoint_19499);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_19499
         * @param {type} xPoint_19499
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_19499, xPoint_19499, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_19499 + '_y' + yPoint_19499;
            }
            return 'pc_x' + xPoint_19499 + '_y' + yPoint_19499;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_19499=0;yPoint_19499<this.gameFieldBorderY.length; yPoint_19499++){
                for(var xPoint_19499=0;xPoint_19499<this.gameFieldBorderX.length; xPoint_19499++){
                    map.push({y: yPoint_19499, x: xPoint_19499});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_19499=-1;yPoint_19499<(this.gameFieldBorderY.length+1); yPoint_19499++){
                for(var xPoint_19499=-1;xPoint_19499<(this.gameFieldBorderX.length+1); xPoint_19499++){
                    if(!map[yPoint_19499]){
                        map[yPoint_19499] = [];
                    }
                    map[yPoint_19499][xPoint_19499] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_19499 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_19499 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_19499, yPoint_19499) === true){
                    if(this.canPutHorizontal(map, xPoint_19499, yPoint_19499, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19499][xPoint_19499 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_19499, yPoint_19499, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_19499 + i][xPoint_19499] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_19499, yPoint_19499){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_19499][xPoint_19499] === this.CELL_EMPTY
                && map[yPoint_19499-1][xPoint_19499] === this.CELL_EMPTY
                && map[yPoint_19499-1][xPoint_19499+1] === this.CELL_EMPTY
                && map[yPoint_19499][xPoint_19499+1] === this.CELL_EMPTY
                && map[yPoint_19499+1][xPoint_19499+1] === this.CELL_EMPTY
                && map[yPoint_19499+1][xPoint_19499] === this.CELL_EMPTY
                && map[yPoint_19499+1][xPoint_19499-1] === this.CELL_EMPTY
                && map[yPoint_19499][xPoint_19499-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_19499, yPoint_19499, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_19499;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_19499][x] === this.CELL_EMPTY
                    && map[yPoint_19499-1][x] === this.CELL_EMPTY
                    && map[yPoint_19499-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19499][x+1] === this.CELL_EMPTY
                    && map[yPoint_19499+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_19499+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_19499, yPoint_19499, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_19499;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_19499] === this.CELL_EMPTY
                    && map[y+1][xPoint_19499] === this.CELL_EMPTY
                    && map[y+1][xPoint_19499+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_19499] === this.CELL_EMPTY
                    && map[y][xPoint_19499-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_19499-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
