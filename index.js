...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_108=0;yPoint_108<this.gameFieldBorderY.length; yPoint_108++){
                for(var xPoint_108=0;xPoint_108<this.gameFieldBorderX.length; xPoint_108++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_108, xPoint_108);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_108][xPoint_108] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_108, xPoint_108, 'user');
                    if(this._userShipsMap[yPoint_108][xPoint_108] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_108, xPoint_108, type){
            var id = this.getPointBlockIdByCoords(yPoint_108, xPoint_108, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_108);
                block.setAttribute('data-y', yPoint_108);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_108
         * @param {type} xPoint_108
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_108, xPoint_108, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_108 + '_y' + yPoint_108;
            }
            return 'pc_x' + xPoint_108 + '_y' + yPoint_108;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_108=0;yPoint_108<this.gameFieldBorderY.length; yPoint_108++){
                for(var xPoint_108=0;xPoint_108<this.gameFieldBorderX.length; xPoint_108++){
                    map.push({y: yPoint_108, x: xPoint_108});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_108=-1;yPoint_108<(this.gameFieldBorderY.length+1); yPoint_108++){
                for(var xPoint_108=-1;xPoint_108<(this.gameFieldBorderX.length+1); xPoint_108++){
                    if(!map[yPoint_108]){
                        map[yPoint_108] = [];
                    }
                    map[yPoint_108][xPoint_108] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_108 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_108 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_108, yPoint_108) === true){
                    if(this.canPutHorizontal(map, xPoint_108, yPoint_108, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_108][xPoint_108 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_108, yPoint_108, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_108 + i][xPoint_108] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_108, yPoint_108){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_108][xPoint_108] === this.CELL_EMPTY
                && map[yPoint_108-1][xPoint_108] === this.CELL_EMPTY
                && map[yPoint_108-1][xPoint_108+1] === this.CELL_EMPTY
                && map[yPoint_108][xPoint_108+1] === this.CELL_EMPTY
                && map[yPoint_108+1][xPoint_108+1] === this.CELL_EMPTY
                && map[yPoint_108+1][xPoint_108] === this.CELL_EMPTY
                && map[yPoint_108+1][xPoint_108-1] === this.CELL_EMPTY
                && map[yPoint_108][xPoint_108-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_108, yPoint_108, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_108;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_108][x] === this.CELL_EMPTY
                    && map[yPoint_108-1][x] === this.CELL_EMPTY
                    && map[yPoint_108-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_108][x+1] === this.CELL_EMPTY
                    && map[yPoint_108+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_108+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_108, yPoint_108, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_108;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_108] === this.CELL_EMPTY
                    && map[y+1][xPoint_108] === this.CELL_EMPTY
                    && map[y+1][xPoint_108+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_108] === this.CELL_EMPTY
                    && map[y][xPoint_108-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_108-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
