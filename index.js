...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14191=0;yPoint_14191<this.gameFieldBorderY.length; yPoint_14191++){
                for(var xPoint_14191=0;xPoint_14191<this.gameFieldBorderX.length; xPoint_14191++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14191, xPoint_14191);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14191][xPoint_14191] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14191, xPoint_14191, 'user');
                    if(this._userShipsMap[yPoint_14191][xPoint_14191] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14191, xPoint_14191, type){
            var id = this.getPointBlockIdByCoords(yPoint_14191, xPoint_14191, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14191);
                block.setAttribute('data-y', yPoint_14191);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14191
         * @param {type} xPoint_14191
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14191, xPoint_14191, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14191 + '_y' + yPoint_14191;
            }
            return 'pc_x' + xPoint_14191 + '_y' + yPoint_14191;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14191=0;yPoint_14191<this.gameFieldBorderY.length; yPoint_14191++){
                for(var xPoint_14191=0;xPoint_14191<this.gameFieldBorderX.length; xPoint_14191++){
                    map.push({y: yPoint_14191, x: xPoint_14191});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14191=-1;yPoint_14191<(this.gameFieldBorderY.length+1); yPoint_14191++){
                for(var xPoint_14191=-1;xPoint_14191<(this.gameFieldBorderX.length+1); xPoint_14191++){
                    if(!map[yPoint_14191]){
                        map[yPoint_14191] = [];
                    }
                    map[yPoint_14191][xPoint_14191] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14191 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14191 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14191, yPoint_14191) === true){
                    if(this.canPutHorizontal(map, xPoint_14191, yPoint_14191, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14191][xPoint_14191 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14191, yPoint_14191, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14191 + i][xPoint_14191] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14191, yPoint_14191){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14191][xPoint_14191] === this.CELL_EMPTY
                && map[yPoint_14191-1][xPoint_14191] === this.CELL_EMPTY
                && map[yPoint_14191-1][xPoint_14191+1] === this.CELL_EMPTY
                && map[yPoint_14191][xPoint_14191+1] === this.CELL_EMPTY
                && map[yPoint_14191+1][xPoint_14191+1] === this.CELL_EMPTY
                && map[yPoint_14191+1][xPoint_14191] === this.CELL_EMPTY
                && map[yPoint_14191+1][xPoint_14191-1] === this.CELL_EMPTY
                && map[yPoint_14191][xPoint_14191-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14191, yPoint_14191, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14191;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14191][x] === this.CELL_EMPTY
                    && map[yPoint_14191-1][x] === this.CELL_EMPTY
                    && map[yPoint_14191-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14191][x+1] === this.CELL_EMPTY
                    && map[yPoint_14191+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14191+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14191, yPoint_14191, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14191;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14191] === this.CELL_EMPTY
                    && map[y+1][xPoint_14191] === this.CELL_EMPTY
                    && map[y+1][xPoint_14191+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14191] === this.CELL_EMPTY
                    && map[y][xPoint_14191-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14191-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
