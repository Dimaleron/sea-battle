...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32740=0;yPoint_32740<this.gameFieldBorderY.length; yPoint_32740++){
                for(var xPoint_32740=0;xPoint_32740<this.gameFieldBorderX.length; xPoint_32740++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32740, xPoint_32740);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32740][xPoint_32740] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32740, xPoint_32740, 'user');
                    if(this._userShipsMap[yPoint_32740][xPoint_32740] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32740, xPoint_32740, type){
            var id = this.getPointBlockIdByCoords(yPoint_32740, xPoint_32740, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32740);
                block.setAttribute('data-y', yPoint_32740);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32740
         * @param {type} xPoint_32740
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32740, xPoint_32740, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32740 + '_y' + yPoint_32740;
            }
            return 'pc_x' + xPoint_32740 + '_y' + yPoint_32740;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32740=0;yPoint_32740<this.gameFieldBorderY.length; yPoint_32740++){
                for(var xPoint_32740=0;xPoint_32740<this.gameFieldBorderX.length; xPoint_32740++){
                    map.push({y: yPoint_32740, x: xPoint_32740});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32740=-1;yPoint_32740<(this.gameFieldBorderY.length+1); yPoint_32740++){
                for(var xPoint_32740=-1;xPoint_32740<(this.gameFieldBorderX.length+1); xPoint_32740++){
                    if(!map[yPoint_32740]){
                        map[yPoint_32740] = [];
                    }
                    map[yPoint_32740][xPoint_32740] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32740 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32740 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32740, yPoint_32740) === true){
                    if(this.canPutHorizontal(map, xPoint_32740, yPoint_32740, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32740][xPoint_32740 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32740, yPoint_32740, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32740 + i][xPoint_32740] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32740, yPoint_32740){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32740][xPoint_32740] === this.CELL_EMPTY
                && map[yPoint_32740-1][xPoint_32740] === this.CELL_EMPTY
                && map[yPoint_32740-1][xPoint_32740+1] === this.CELL_EMPTY
                && map[yPoint_32740][xPoint_32740+1] === this.CELL_EMPTY
                && map[yPoint_32740+1][xPoint_32740+1] === this.CELL_EMPTY
                && map[yPoint_32740+1][xPoint_32740] === this.CELL_EMPTY
                && map[yPoint_32740+1][xPoint_32740-1] === this.CELL_EMPTY
                && map[yPoint_32740][xPoint_32740-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32740, yPoint_32740, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32740;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32740][x] === this.CELL_EMPTY
                    && map[yPoint_32740-1][x] === this.CELL_EMPTY
                    && map[yPoint_32740-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32740][x+1] === this.CELL_EMPTY
                    && map[yPoint_32740+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32740+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32740, yPoint_32740, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32740;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32740] === this.CELL_EMPTY
                    && map[y+1][xPoint_32740] === this.CELL_EMPTY
                    && map[y+1][xPoint_32740+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32740] === this.CELL_EMPTY
                    && map[y][xPoint_32740-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32740-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
