...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20835=0;yPoint_20835<this.gameFieldBorderY.length; yPoint_20835++){
                for(var xPoint_20835=0;xPoint_20835<this.gameFieldBorderX.length; xPoint_20835++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20835, xPoint_20835);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20835][xPoint_20835] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20835, xPoint_20835, 'user');
                    if(this._userShipsMap[yPoint_20835][xPoint_20835] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20835, xPoint_20835, type){
            var id = this.getPointBlockIdByCoords(yPoint_20835, xPoint_20835, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20835);
                block.setAttribute('data-y', yPoint_20835);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20835
         * @param {type} xPoint_20835
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20835, xPoint_20835, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20835 + '_y' + yPoint_20835;
            }
            return 'pc_x' + xPoint_20835 + '_y' + yPoint_20835;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20835=0;yPoint_20835<this.gameFieldBorderY.length; yPoint_20835++){
                for(var xPoint_20835=0;xPoint_20835<this.gameFieldBorderX.length; xPoint_20835++){
                    map.push({y: yPoint_20835, x: xPoint_20835});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20835=-1;yPoint_20835<(this.gameFieldBorderY.length+1); yPoint_20835++){
                for(var xPoint_20835=-1;xPoint_20835<(this.gameFieldBorderX.length+1); xPoint_20835++){
                    if(!map[yPoint_20835]){
                        map[yPoint_20835] = [];
                    }
                    map[yPoint_20835][xPoint_20835] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20835 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20835 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20835, yPoint_20835) === true){
                    if(this.canPutHorizontal(map, xPoint_20835, yPoint_20835, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20835][xPoint_20835 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20835, yPoint_20835, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20835 + i][xPoint_20835] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20835, yPoint_20835){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20835][xPoint_20835] === this.CELL_EMPTY
                && map[yPoint_20835-1][xPoint_20835] === this.CELL_EMPTY
                && map[yPoint_20835-1][xPoint_20835+1] === this.CELL_EMPTY
                && map[yPoint_20835][xPoint_20835+1] === this.CELL_EMPTY
                && map[yPoint_20835+1][xPoint_20835+1] === this.CELL_EMPTY
                && map[yPoint_20835+1][xPoint_20835] === this.CELL_EMPTY
                && map[yPoint_20835+1][xPoint_20835-1] === this.CELL_EMPTY
                && map[yPoint_20835][xPoint_20835-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20835, yPoint_20835, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20835;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20835][x] === this.CELL_EMPTY
                    && map[yPoint_20835-1][x] === this.CELL_EMPTY
                    && map[yPoint_20835-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20835][x+1] === this.CELL_EMPTY
                    && map[yPoint_20835+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20835+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20835, yPoint_20835, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20835;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20835] === this.CELL_EMPTY
                    && map[y+1][xPoint_20835] === this.CELL_EMPTY
                    && map[y+1][xPoint_20835+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20835] === this.CELL_EMPTY
                    && map[y][xPoint_20835-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20835-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
