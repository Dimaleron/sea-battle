...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18909=0;yPoint_18909<this.gameFieldBorderY.length; yPoint_18909++){
                for(var xPoint_18909=0;xPoint_18909<this.gameFieldBorderX.length; xPoint_18909++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18909, xPoint_18909);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18909][xPoint_18909] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18909, xPoint_18909, 'user');
                    if(this._userShipsMap[yPoint_18909][xPoint_18909] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18909, xPoint_18909, type){
            var id = this.getPointBlockIdByCoords(yPoint_18909, xPoint_18909, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18909);
                block.setAttribute('data-y', yPoint_18909);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18909
         * @param {type} xPoint_18909
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18909, xPoint_18909, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18909 + '_y' + yPoint_18909;
            }
            return 'pc_x' + xPoint_18909 + '_y' + yPoint_18909;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18909=0;yPoint_18909<this.gameFieldBorderY.length; yPoint_18909++){
                for(var xPoint_18909=0;xPoint_18909<this.gameFieldBorderX.length; xPoint_18909++){
                    map.push({y: yPoint_18909, x: xPoint_18909});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18909=-1;yPoint_18909<(this.gameFieldBorderY.length+1); yPoint_18909++){
                for(var xPoint_18909=-1;xPoint_18909<(this.gameFieldBorderX.length+1); xPoint_18909++){
                    if(!map[yPoint_18909]){
                        map[yPoint_18909] = [];
                    }
                    map[yPoint_18909][xPoint_18909] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18909 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18909 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18909, yPoint_18909) === true){
                    if(this.canPutHorizontal(map, xPoint_18909, yPoint_18909, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18909][xPoint_18909 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18909, yPoint_18909, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18909 + i][xPoint_18909] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18909, yPoint_18909){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18909][xPoint_18909] === this.CELL_EMPTY
                && map[yPoint_18909-1][xPoint_18909] === this.CELL_EMPTY
                && map[yPoint_18909-1][xPoint_18909+1] === this.CELL_EMPTY
                && map[yPoint_18909][xPoint_18909+1] === this.CELL_EMPTY
                && map[yPoint_18909+1][xPoint_18909+1] === this.CELL_EMPTY
                && map[yPoint_18909+1][xPoint_18909] === this.CELL_EMPTY
                && map[yPoint_18909+1][xPoint_18909-1] === this.CELL_EMPTY
                && map[yPoint_18909][xPoint_18909-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18909, yPoint_18909, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18909;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18909][x] === this.CELL_EMPTY
                    && map[yPoint_18909-1][x] === this.CELL_EMPTY
                    && map[yPoint_18909-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18909][x+1] === this.CELL_EMPTY
                    && map[yPoint_18909+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18909+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18909, yPoint_18909, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18909;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18909] === this.CELL_EMPTY
                    && map[y+1][xPoint_18909] === this.CELL_EMPTY
                    && map[y+1][xPoint_18909+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18909] === this.CELL_EMPTY
                    && map[y][xPoint_18909-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18909-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
