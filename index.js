...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31328=0;yPoint_31328<this.gameFieldBorderY.length; yPoint_31328++){
                for(var xPoint_31328=0;xPoint_31328<this.gameFieldBorderX.length; xPoint_31328++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31328, xPoint_31328);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31328][xPoint_31328] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31328, xPoint_31328, 'user');
                    if(this._userShipsMap[yPoint_31328][xPoint_31328] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31328, xPoint_31328, type){
            var id = this.getPointBlockIdByCoords(yPoint_31328, xPoint_31328, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31328);
                block.setAttribute('data-y', yPoint_31328);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31328
         * @param {type} xPoint_31328
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31328, xPoint_31328, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31328 + '_y' + yPoint_31328;
            }
            return 'pc_x' + xPoint_31328 + '_y' + yPoint_31328;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31328=0;yPoint_31328<this.gameFieldBorderY.length; yPoint_31328++){
                for(var xPoint_31328=0;xPoint_31328<this.gameFieldBorderX.length; xPoint_31328++){
                    map.push({y: yPoint_31328, x: xPoint_31328});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31328=-1;yPoint_31328<(this.gameFieldBorderY.length+1); yPoint_31328++){
                for(var xPoint_31328=-1;xPoint_31328<(this.gameFieldBorderX.length+1); xPoint_31328++){
                    if(!map[yPoint_31328]){
                        map[yPoint_31328] = [];
                    }
                    map[yPoint_31328][xPoint_31328] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31328 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31328 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31328, yPoint_31328) === true){
                    if(this.canPutHorizontal(map, xPoint_31328, yPoint_31328, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31328][xPoint_31328 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31328, yPoint_31328, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31328 + i][xPoint_31328] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31328, yPoint_31328){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31328][xPoint_31328] === this.CELL_EMPTY
                && map[yPoint_31328-1][xPoint_31328] === this.CELL_EMPTY
                && map[yPoint_31328-1][xPoint_31328+1] === this.CELL_EMPTY
                && map[yPoint_31328][xPoint_31328+1] === this.CELL_EMPTY
                && map[yPoint_31328+1][xPoint_31328+1] === this.CELL_EMPTY
                && map[yPoint_31328+1][xPoint_31328] === this.CELL_EMPTY
                && map[yPoint_31328+1][xPoint_31328-1] === this.CELL_EMPTY
                && map[yPoint_31328][xPoint_31328-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31328, yPoint_31328, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31328;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31328][x] === this.CELL_EMPTY
                    && map[yPoint_31328-1][x] === this.CELL_EMPTY
                    && map[yPoint_31328-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31328][x+1] === this.CELL_EMPTY
                    && map[yPoint_31328+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31328+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31328, yPoint_31328, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31328;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31328] === this.CELL_EMPTY
                    && map[y+1][xPoint_31328] === this.CELL_EMPTY
                    && map[y+1][xPoint_31328+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31328] === this.CELL_EMPTY
                    && map[y][xPoint_31328-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31328-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
