...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6310=0;yPoint_6310<this.gameFieldBorderY.length; yPoint_6310++){
                for(var xPoint_6310=0;xPoint_6310<this.gameFieldBorderX.length; xPoint_6310++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6310, xPoint_6310);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6310][xPoint_6310] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6310, xPoint_6310, 'user');
                    if(this._userShipsMap[yPoint_6310][xPoint_6310] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6310, xPoint_6310, type){
            var id = this.getPointBlockIdByCoords(yPoint_6310, xPoint_6310, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6310);
                block.setAttribute('data-y', yPoint_6310);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6310
         * @param {type} xPoint_6310
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6310, xPoint_6310, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6310 + '_y' + yPoint_6310;
            }
            return 'pc_x' + xPoint_6310 + '_y' + yPoint_6310;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6310=0;yPoint_6310<this.gameFieldBorderY.length; yPoint_6310++){
                for(var xPoint_6310=0;xPoint_6310<this.gameFieldBorderX.length; xPoint_6310++){
                    map.push({y: yPoint_6310, x: xPoint_6310});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6310=-1;yPoint_6310<(this.gameFieldBorderY.length+1); yPoint_6310++){
                for(var xPoint_6310=-1;xPoint_6310<(this.gameFieldBorderX.length+1); xPoint_6310++){
                    if(!map[yPoint_6310]){
                        map[yPoint_6310] = [];
                    }
                    map[yPoint_6310][xPoint_6310] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6310 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6310 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6310, yPoint_6310) === true){
                    if(this.canPutHorizontal(map, xPoint_6310, yPoint_6310, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6310][xPoint_6310 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6310, yPoint_6310, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6310 + i][xPoint_6310] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6310, yPoint_6310){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6310][xPoint_6310] === this.CELL_EMPTY
                && map[yPoint_6310-1][xPoint_6310] === this.CELL_EMPTY
                && map[yPoint_6310-1][xPoint_6310+1] === this.CELL_EMPTY
                && map[yPoint_6310][xPoint_6310+1] === this.CELL_EMPTY
                && map[yPoint_6310+1][xPoint_6310+1] === this.CELL_EMPTY
                && map[yPoint_6310+1][xPoint_6310] === this.CELL_EMPTY
                && map[yPoint_6310+1][xPoint_6310-1] === this.CELL_EMPTY
                && map[yPoint_6310][xPoint_6310-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6310, yPoint_6310, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6310;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6310][x] === this.CELL_EMPTY
                    && map[yPoint_6310-1][x] === this.CELL_EMPTY
                    && map[yPoint_6310-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6310][x+1] === this.CELL_EMPTY
                    && map[yPoint_6310+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6310+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6310, yPoint_6310, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6310;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6310] === this.CELL_EMPTY
                    && map[y+1][xPoint_6310] === this.CELL_EMPTY
                    && map[y+1][xPoint_6310+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6310] === this.CELL_EMPTY
                    && map[y][xPoint_6310-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6310-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
