...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_14227=0;yPoint_14227<this.gameFieldBorderY.length; yPoint_14227++){
                for(var xPoint_14227=0;xPoint_14227<this.gameFieldBorderX.length; xPoint_14227++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_14227, xPoint_14227);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_14227][xPoint_14227] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_14227, xPoint_14227, 'user');
                    if(this._userShipsMap[yPoint_14227][xPoint_14227] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_14227, xPoint_14227, type){
            var id = this.getPointBlockIdByCoords(yPoint_14227, xPoint_14227, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_14227);
                block.setAttribute('data-y', yPoint_14227);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_14227
         * @param {type} xPoint_14227
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_14227, xPoint_14227, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_14227 + '_y' + yPoint_14227;
            }
            return 'pc_x' + xPoint_14227 + '_y' + yPoint_14227;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_14227=0;yPoint_14227<this.gameFieldBorderY.length; yPoint_14227++){
                for(var xPoint_14227=0;xPoint_14227<this.gameFieldBorderX.length; xPoint_14227++){
                    map.push({y: yPoint_14227, x: xPoint_14227});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_14227=-1;yPoint_14227<(this.gameFieldBorderY.length+1); yPoint_14227++){
                for(var xPoint_14227=-1;xPoint_14227<(this.gameFieldBorderX.length+1); xPoint_14227++){
                    if(!map[yPoint_14227]){
                        map[yPoint_14227] = [];
                    }
                    map[yPoint_14227][xPoint_14227] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_14227 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_14227 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_14227, yPoint_14227) === true){
                    if(this.canPutHorizontal(map, xPoint_14227, yPoint_14227, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14227][xPoint_14227 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_14227, yPoint_14227, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_14227 + i][xPoint_14227] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_14227, yPoint_14227){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_14227][xPoint_14227] === this.CELL_EMPTY
                && map[yPoint_14227-1][xPoint_14227] === this.CELL_EMPTY
                && map[yPoint_14227-1][xPoint_14227+1] === this.CELL_EMPTY
                && map[yPoint_14227][xPoint_14227+1] === this.CELL_EMPTY
                && map[yPoint_14227+1][xPoint_14227+1] === this.CELL_EMPTY
                && map[yPoint_14227+1][xPoint_14227] === this.CELL_EMPTY
                && map[yPoint_14227+1][xPoint_14227-1] === this.CELL_EMPTY
                && map[yPoint_14227][xPoint_14227-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_14227, yPoint_14227, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_14227;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_14227][x] === this.CELL_EMPTY
                    && map[yPoint_14227-1][x] === this.CELL_EMPTY
                    && map[yPoint_14227-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14227][x+1] === this.CELL_EMPTY
                    && map[yPoint_14227+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_14227+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_14227, yPoint_14227, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_14227;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_14227] === this.CELL_EMPTY
                    && map[y+1][xPoint_14227] === this.CELL_EMPTY
                    && map[y+1][xPoint_14227+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_14227] === this.CELL_EMPTY
                    && map[y][xPoint_14227-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_14227-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
