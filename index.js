...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28292=0;yPoint_28292<this.gameFieldBorderY.length; yPoint_28292++){
                for(var xPoint_28292=0;xPoint_28292<this.gameFieldBorderX.length; xPoint_28292++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28292, xPoint_28292);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28292][xPoint_28292] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28292, xPoint_28292, 'user');
                    if(this._userShipsMap[yPoint_28292][xPoint_28292] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28292, xPoint_28292, type){
            var id = this.getPointBlockIdByCoords(yPoint_28292, xPoint_28292, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28292);
                block.setAttribute('data-y', yPoint_28292);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28292
         * @param {type} xPoint_28292
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28292, xPoint_28292, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28292 + '_y' + yPoint_28292;
            }
            return 'pc_x' + xPoint_28292 + '_y' + yPoint_28292;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28292=0;yPoint_28292<this.gameFieldBorderY.length; yPoint_28292++){
                for(var xPoint_28292=0;xPoint_28292<this.gameFieldBorderX.length; xPoint_28292++){
                    map.push({y: yPoint_28292, x: xPoint_28292});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28292=-1;yPoint_28292<(this.gameFieldBorderY.length+1); yPoint_28292++){
                for(var xPoint_28292=-1;xPoint_28292<(this.gameFieldBorderX.length+1); xPoint_28292++){
                    if(!map[yPoint_28292]){
                        map[yPoint_28292] = [];
                    }
                    map[yPoint_28292][xPoint_28292] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28292 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28292 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28292, yPoint_28292) === true){
                    if(this.canPutHorizontal(map, xPoint_28292, yPoint_28292, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28292][xPoint_28292 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28292, yPoint_28292, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28292 + i][xPoint_28292] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28292, yPoint_28292){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28292][xPoint_28292] === this.CELL_EMPTY
                && map[yPoint_28292-1][xPoint_28292] === this.CELL_EMPTY
                && map[yPoint_28292-1][xPoint_28292+1] === this.CELL_EMPTY
                && map[yPoint_28292][xPoint_28292+1] === this.CELL_EMPTY
                && map[yPoint_28292+1][xPoint_28292+1] === this.CELL_EMPTY
                && map[yPoint_28292+1][xPoint_28292] === this.CELL_EMPTY
                && map[yPoint_28292+1][xPoint_28292-1] === this.CELL_EMPTY
                && map[yPoint_28292][xPoint_28292-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28292, yPoint_28292, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28292;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28292][x] === this.CELL_EMPTY
                    && map[yPoint_28292-1][x] === this.CELL_EMPTY
                    && map[yPoint_28292-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28292][x+1] === this.CELL_EMPTY
                    && map[yPoint_28292+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28292+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28292, yPoint_28292, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28292;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28292] === this.CELL_EMPTY
                    && map[y+1][xPoint_28292] === this.CELL_EMPTY
                    && map[y+1][xPoint_28292+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28292] === this.CELL_EMPTY
                    && map[y][xPoint_28292-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28292-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
