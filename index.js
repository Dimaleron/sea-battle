...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_3966=0;yPoint_3966<this.gameFieldBorderY.length; yPoint_3966++){
                for(var xPoint_3966=0;xPoint_3966<this.gameFieldBorderX.length; xPoint_3966++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_3966, xPoint_3966);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_3966][xPoint_3966] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_3966, xPoint_3966, 'user');
                    if(this._userShipsMap[yPoint_3966][xPoint_3966] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_3966, xPoint_3966, type){
            var id = this.getPointBlockIdByCoords(yPoint_3966, xPoint_3966, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_3966);
                block.setAttribute('data-y', yPoint_3966);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_3966
         * @param {type} xPoint_3966
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_3966, xPoint_3966, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_3966 + '_y' + yPoint_3966;
            }
            return 'pc_x' + xPoint_3966 + '_y' + yPoint_3966;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_3966=0;yPoint_3966<this.gameFieldBorderY.length; yPoint_3966++){
                for(var xPoint_3966=0;xPoint_3966<this.gameFieldBorderX.length; xPoint_3966++){
                    map.push({y: yPoint_3966, x: xPoint_3966});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_3966=-1;yPoint_3966<(this.gameFieldBorderY.length+1); yPoint_3966++){
                for(var xPoint_3966=-1;xPoint_3966<(this.gameFieldBorderX.length+1); xPoint_3966++){
                    if(!map[yPoint_3966]){
                        map[yPoint_3966] = [];
                    }
                    map[yPoint_3966][xPoint_3966] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_3966 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_3966 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_3966, yPoint_3966) === true){
                    if(this.canPutHorizontal(map, xPoint_3966, yPoint_3966, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3966][xPoint_3966 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_3966, yPoint_3966, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_3966 + i][xPoint_3966] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_3966, yPoint_3966){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_3966][xPoint_3966] === this.CELL_EMPTY
                && map[yPoint_3966-1][xPoint_3966] === this.CELL_EMPTY
                && map[yPoint_3966-1][xPoint_3966+1] === this.CELL_EMPTY
                && map[yPoint_3966][xPoint_3966+1] === this.CELL_EMPTY
                && map[yPoint_3966+1][xPoint_3966+1] === this.CELL_EMPTY
                && map[yPoint_3966+1][xPoint_3966] === this.CELL_EMPTY
                && map[yPoint_3966+1][xPoint_3966-1] === this.CELL_EMPTY
                && map[yPoint_3966][xPoint_3966-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_3966, yPoint_3966, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_3966;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_3966][x] === this.CELL_EMPTY
                    && map[yPoint_3966-1][x] === this.CELL_EMPTY
                    && map[yPoint_3966-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3966][x+1] === this.CELL_EMPTY
                    && map[yPoint_3966+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_3966+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_3966, yPoint_3966, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_3966;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_3966] === this.CELL_EMPTY
                    && map[y+1][xPoint_3966] === this.CELL_EMPTY
                    && map[y+1][xPoint_3966+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_3966] === this.CELL_EMPTY
                    && map[y][xPoint_3966-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_3966-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
