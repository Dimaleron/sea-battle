...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_20729=0;yPoint_20729<this.gameFieldBorderY.length; yPoint_20729++){
                for(var xPoint_20729=0;xPoint_20729<this.gameFieldBorderX.length; xPoint_20729++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_20729, xPoint_20729);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_20729][xPoint_20729] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_20729, xPoint_20729, 'user');
                    if(this._userShipsMap[yPoint_20729][xPoint_20729] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_20729, xPoint_20729, type){
            var id = this.getPointBlockIdByCoords(yPoint_20729, xPoint_20729, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_20729);
                block.setAttribute('data-y', yPoint_20729);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_20729
         * @param {type} xPoint_20729
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_20729, xPoint_20729, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_20729 + '_y' + yPoint_20729;
            }
            return 'pc_x' + xPoint_20729 + '_y' + yPoint_20729;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_20729=0;yPoint_20729<this.gameFieldBorderY.length; yPoint_20729++){
                for(var xPoint_20729=0;xPoint_20729<this.gameFieldBorderX.length; xPoint_20729++){
                    map.push({y: yPoint_20729, x: xPoint_20729});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_20729=-1;yPoint_20729<(this.gameFieldBorderY.length+1); yPoint_20729++){
                for(var xPoint_20729=-1;xPoint_20729<(this.gameFieldBorderX.length+1); xPoint_20729++){
                    if(!map[yPoint_20729]){
                        map[yPoint_20729] = [];
                    }
                    map[yPoint_20729][xPoint_20729] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_20729 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_20729 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_20729, yPoint_20729) === true){
                    if(this.canPutHorizontal(map, xPoint_20729, yPoint_20729, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20729][xPoint_20729 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_20729, yPoint_20729, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_20729 + i][xPoint_20729] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_20729, yPoint_20729){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_20729][xPoint_20729] === this.CELL_EMPTY
                && map[yPoint_20729-1][xPoint_20729] === this.CELL_EMPTY
                && map[yPoint_20729-1][xPoint_20729+1] === this.CELL_EMPTY
                && map[yPoint_20729][xPoint_20729+1] === this.CELL_EMPTY
                && map[yPoint_20729+1][xPoint_20729+1] === this.CELL_EMPTY
                && map[yPoint_20729+1][xPoint_20729] === this.CELL_EMPTY
                && map[yPoint_20729+1][xPoint_20729-1] === this.CELL_EMPTY
                && map[yPoint_20729][xPoint_20729-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_20729, yPoint_20729, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_20729;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_20729][x] === this.CELL_EMPTY
                    && map[yPoint_20729-1][x] === this.CELL_EMPTY
                    && map[yPoint_20729-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20729][x+1] === this.CELL_EMPTY
                    && map[yPoint_20729+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_20729+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_20729, yPoint_20729, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_20729;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_20729] === this.CELL_EMPTY
                    && map[y+1][xPoint_20729] === this.CELL_EMPTY
                    && map[y+1][xPoint_20729+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_20729] === this.CELL_EMPTY
                    && map[y][xPoint_20729-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_20729-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
