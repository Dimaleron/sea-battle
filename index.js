...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7549=0;yPoint_7549<this.gameFieldBorderY.length; yPoint_7549++){
                for(var xPoint_7549=0;xPoint_7549<this.gameFieldBorderX.length; xPoint_7549++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7549, xPoint_7549);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7549][xPoint_7549] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7549, xPoint_7549, 'user');
                    if(this._userShipsMap[yPoint_7549][xPoint_7549] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7549, xPoint_7549, type){
            var id = this.getPointBlockIdByCoords(yPoint_7549, xPoint_7549, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7549);
                block.setAttribute('data-y', yPoint_7549);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7549
         * @param {type} xPoint_7549
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7549, xPoint_7549, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7549 + '_y' + yPoint_7549;
            }
            return 'pc_x' + xPoint_7549 + '_y' + yPoint_7549;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7549=0;yPoint_7549<this.gameFieldBorderY.length; yPoint_7549++){
                for(var xPoint_7549=0;xPoint_7549<this.gameFieldBorderX.length; xPoint_7549++){
                    map.push({y: yPoint_7549, x: xPoint_7549});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7549=-1;yPoint_7549<(this.gameFieldBorderY.length+1); yPoint_7549++){
                for(var xPoint_7549=-1;xPoint_7549<(this.gameFieldBorderX.length+1); xPoint_7549++){
                    if(!map[yPoint_7549]){
                        map[yPoint_7549] = [];
                    }
                    map[yPoint_7549][xPoint_7549] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7549 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7549 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7549, yPoint_7549) === true){
                    if(this.canPutHorizontal(map, xPoint_7549, yPoint_7549, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7549][xPoint_7549 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7549, yPoint_7549, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7549 + i][xPoint_7549] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7549, yPoint_7549){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7549][xPoint_7549] === this.CELL_EMPTY
                && map[yPoint_7549-1][xPoint_7549] === this.CELL_EMPTY
                && map[yPoint_7549-1][xPoint_7549+1] === this.CELL_EMPTY
                && map[yPoint_7549][xPoint_7549+1] === this.CELL_EMPTY
                && map[yPoint_7549+1][xPoint_7549+1] === this.CELL_EMPTY
                && map[yPoint_7549+1][xPoint_7549] === this.CELL_EMPTY
                && map[yPoint_7549+1][xPoint_7549-1] === this.CELL_EMPTY
                && map[yPoint_7549][xPoint_7549-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7549, yPoint_7549, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7549;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7549][x] === this.CELL_EMPTY
                    && map[yPoint_7549-1][x] === this.CELL_EMPTY
                    && map[yPoint_7549-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7549][x+1] === this.CELL_EMPTY
                    && map[yPoint_7549+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7549+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7549, yPoint_7549, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7549;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7549] === this.CELL_EMPTY
                    && map[y+1][xPoint_7549] === this.CELL_EMPTY
                    && map[y+1][xPoint_7549+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7549] === this.CELL_EMPTY
                    && map[y][xPoint_7549-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7549-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
