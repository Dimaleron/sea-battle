...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7734=0;yPoint_7734<this.gameFieldBorderY.length; yPoint_7734++){
                for(var xPoint_7734=0;xPoint_7734<this.gameFieldBorderX.length; xPoint_7734++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7734, xPoint_7734);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7734][xPoint_7734] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7734, xPoint_7734, 'user');
                    if(this._userShipsMap[yPoint_7734][xPoint_7734] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7734, xPoint_7734, type){
            var id = this.getPointBlockIdByCoords(yPoint_7734, xPoint_7734, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7734);
                block.setAttribute('data-y', yPoint_7734);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7734
         * @param {type} xPoint_7734
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7734, xPoint_7734, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7734 + '_y' + yPoint_7734;
            }
            return 'pc_x' + xPoint_7734 + '_y' + yPoint_7734;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7734=0;yPoint_7734<this.gameFieldBorderY.length; yPoint_7734++){
                for(var xPoint_7734=0;xPoint_7734<this.gameFieldBorderX.length; xPoint_7734++){
                    map.push({y: yPoint_7734, x: xPoint_7734});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7734=-1;yPoint_7734<(this.gameFieldBorderY.length+1); yPoint_7734++){
                for(var xPoint_7734=-1;xPoint_7734<(this.gameFieldBorderX.length+1); xPoint_7734++){
                    if(!map[yPoint_7734]){
                        map[yPoint_7734] = [];
                    }
                    map[yPoint_7734][xPoint_7734] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7734 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7734 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7734, yPoint_7734) === true){
                    if(this.canPutHorizontal(map, xPoint_7734, yPoint_7734, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7734][xPoint_7734 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7734, yPoint_7734, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7734 + i][xPoint_7734] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7734, yPoint_7734){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7734][xPoint_7734] === this.CELL_EMPTY
                && map[yPoint_7734-1][xPoint_7734] === this.CELL_EMPTY
                && map[yPoint_7734-1][xPoint_7734+1] === this.CELL_EMPTY
                && map[yPoint_7734][xPoint_7734+1] === this.CELL_EMPTY
                && map[yPoint_7734+1][xPoint_7734+1] === this.CELL_EMPTY
                && map[yPoint_7734+1][xPoint_7734] === this.CELL_EMPTY
                && map[yPoint_7734+1][xPoint_7734-1] === this.CELL_EMPTY
                && map[yPoint_7734][xPoint_7734-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7734, yPoint_7734, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7734;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7734][x] === this.CELL_EMPTY
                    && map[yPoint_7734-1][x] === this.CELL_EMPTY
                    && map[yPoint_7734-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7734][x+1] === this.CELL_EMPTY
                    && map[yPoint_7734+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7734+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7734, yPoint_7734, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7734;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7734] === this.CELL_EMPTY
                    && map[y+1][xPoint_7734] === this.CELL_EMPTY
                    && map[y+1][xPoint_7734+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7734] === this.CELL_EMPTY
                    && map[y][xPoint_7734-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7734-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
