...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_30344=0;yPoint_30344<this.gameFieldBorderY.length; yPoint_30344++){
                for(var xPoint_30344=0;xPoint_30344<this.gameFieldBorderX.length; xPoint_30344++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_30344, xPoint_30344);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_30344][xPoint_30344] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_30344, xPoint_30344, 'user');
                    if(this._userShipsMap[yPoint_30344][xPoint_30344] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_30344, xPoint_30344, type){
            var id = this.getPointBlockIdByCoords(yPoint_30344, xPoint_30344, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_30344);
                block.setAttribute('data-y', yPoint_30344);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_30344
         * @param {type} xPoint_30344
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_30344, xPoint_30344, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_30344 + '_y' + yPoint_30344;
            }
            return 'pc_x' + xPoint_30344 + '_y' + yPoint_30344;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_30344=0;yPoint_30344<this.gameFieldBorderY.length; yPoint_30344++){
                for(var xPoint_30344=0;xPoint_30344<this.gameFieldBorderX.length; xPoint_30344++){
                    map.push({y: yPoint_30344, x: xPoint_30344});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_30344=-1;yPoint_30344<(this.gameFieldBorderY.length+1); yPoint_30344++){
                for(var xPoint_30344=-1;xPoint_30344<(this.gameFieldBorderX.length+1); xPoint_30344++){
                    if(!map[yPoint_30344]){
                        map[yPoint_30344] = [];
                    }
                    map[yPoint_30344][xPoint_30344] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_30344 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_30344 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_30344, yPoint_30344) === true){
                    if(this.canPutHorizontal(map, xPoint_30344, yPoint_30344, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30344][xPoint_30344 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_30344, yPoint_30344, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_30344 + i][xPoint_30344] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_30344, yPoint_30344){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_30344][xPoint_30344] === this.CELL_EMPTY
                && map[yPoint_30344-1][xPoint_30344] === this.CELL_EMPTY
                && map[yPoint_30344-1][xPoint_30344+1] === this.CELL_EMPTY
                && map[yPoint_30344][xPoint_30344+1] === this.CELL_EMPTY
                && map[yPoint_30344+1][xPoint_30344+1] === this.CELL_EMPTY
                && map[yPoint_30344+1][xPoint_30344] === this.CELL_EMPTY
                && map[yPoint_30344+1][xPoint_30344-1] === this.CELL_EMPTY
                && map[yPoint_30344][xPoint_30344-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_30344, yPoint_30344, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_30344;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_30344][x] === this.CELL_EMPTY
                    && map[yPoint_30344-1][x] === this.CELL_EMPTY
                    && map[yPoint_30344-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30344][x+1] === this.CELL_EMPTY
                    && map[yPoint_30344+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_30344+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_30344, yPoint_30344, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_30344;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_30344] === this.CELL_EMPTY
                    && map[y+1][xPoint_30344] === this.CELL_EMPTY
                    && map[y+1][xPoint_30344+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_30344] === this.CELL_EMPTY
                    && map[y][xPoint_30344-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_30344-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
