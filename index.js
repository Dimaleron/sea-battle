...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_31911=0;yPoint_31911<this.gameFieldBorderY.length; yPoint_31911++){
                for(var xPoint_31911=0;xPoint_31911<this.gameFieldBorderX.length; xPoint_31911++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_31911, xPoint_31911);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_31911][xPoint_31911] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_31911, xPoint_31911, 'user');
                    if(this._userShipsMap[yPoint_31911][xPoint_31911] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_31911, xPoint_31911, type){
            var id = this.getPointBlockIdByCoords(yPoint_31911, xPoint_31911, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_31911);
                block.setAttribute('data-y', yPoint_31911);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_31911
         * @param {type} xPoint_31911
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_31911, xPoint_31911, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_31911 + '_y' + yPoint_31911;
            }
            return 'pc_x' + xPoint_31911 + '_y' + yPoint_31911;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_31911=0;yPoint_31911<this.gameFieldBorderY.length; yPoint_31911++){
                for(var xPoint_31911=0;xPoint_31911<this.gameFieldBorderX.length; xPoint_31911++){
                    map.push({y: yPoint_31911, x: xPoint_31911});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_31911=-1;yPoint_31911<(this.gameFieldBorderY.length+1); yPoint_31911++){
                for(var xPoint_31911=-1;xPoint_31911<(this.gameFieldBorderX.length+1); xPoint_31911++){
                    if(!map[yPoint_31911]){
                        map[yPoint_31911] = [];
                    }
                    map[yPoint_31911][xPoint_31911] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_31911 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_31911 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_31911, yPoint_31911) === true){
                    if(this.canPutHorizontal(map, xPoint_31911, yPoint_31911, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31911][xPoint_31911 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_31911, yPoint_31911, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_31911 + i][xPoint_31911] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_31911, yPoint_31911){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_31911][xPoint_31911] === this.CELL_EMPTY
                && map[yPoint_31911-1][xPoint_31911] === this.CELL_EMPTY
                && map[yPoint_31911-1][xPoint_31911+1] === this.CELL_EMPTY
                && map[yPoint_31911][xPoint_31911+1] === this.CELL_EMPTY
                && map[yPoint_31911+1][xPoint_31911+1] === this.CELL_EMPTY
                && map[yPoint_31911+1][xPoint_31911] === this.CELL_EMPTY
                && map[yPoint_31911+1][xPoint_31911-1] === this.CELL_EMPTY
                && map[yPoint_31911][xPoint_31911-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_31911, yPoint_31911, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_31911;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_31911][x] === this.CELL_EMPTY
                    && map[yPoint_31911-1][x] === this.CELL_EMPTY
                    && map[yPoint_31911-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31911][x+1] === this.CELL_EMPTY
                    && map[yPoint_31911+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_31911+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_31911, yPoint_31911, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_31911;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_31911] === this.CELL_EMPTY
                    && map[y+1][xPoint_31911] === this.CELL_EMPTY
                    && map[y+1][xPoint_31911+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_31911] === this.CELL_EMPTY
                    && map[y][xPoint_31911-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_31911-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
