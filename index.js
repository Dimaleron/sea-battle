...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24641=0;yPoint_24641<this.gameFieldBorderY.length; yPoint_24641++){
                for(var xPoint_24641=0;xPoint_24641<this.gameFieldBorderX.length; xPoint_24641++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24641, xPoint_24641);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24641][xPoint_24641] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24641, xPoint_24641, 'user');
                    if(this._userShipsMap[yPoint_24641][xPoint_24641] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24641, xPoint_24641, type){
            var id = this.getPointBlockIdByCoords(yPoint_24641, xPoint_24641, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24641);
                block.setAttribute('data-y', yPoint_24641);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24641
         * @param {type} xPoint_24641
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24641, xPoint_24641, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24641 + '_y' + yPoint_24641;
            }
            return 'pc_x' + xPoint_24641 + '_y' + yPoint_24641;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24641=0;yPoint_24641<this.gameFieldBorderY.length; yPoint_24641++){
                for(var xPoint_24641=0;xPoint_24641<this.gameFieldBorderX.length; xPoint_24641++){
                    map.push({y: yPoint_24641, x: xPoint_24641});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24641=-1;yPoint_24641<(this.gameFieldBorderY.length+1); yPoint_24641++){
                for(var xPoint_24641=-1;xPoint_24641<(this.gameFieldBorderX.length+1); xPoint_24641++){
                    if(!map[yPoint_24641]){
                        map[yPoint_24641] = [];
                    }
                    map[yPoint_24641][xPoint_24641] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24641 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24641 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24641, yPoint_24641) === true){
                    if(this.canPutHorizontal(map, xPoint_24641, yPoint_24641, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24641][xPoint_24641 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24641, yPoint_24641, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24641 + i][xPoint_24641] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24641, yPoint_24641){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24641][xPoint_24641] === this.CELL_EMPTY
                && map[yPoint_24641-1][xPoint_24641] === this.CELL_EMPTY
                && map[yPoint_24641-1][xPoint_24641+1] === this.CELL_EMPTY
                && map[yPoint_24641][xPoint_24641+1] === this.CELL_EMPTY
                && map[yPoint_24641+1][xPoint_24641+1] === this.CELL_EMPTY
                && map[yPoint_24641+1][xPoint_24641] === this.CELL_EMPTY
                && map[yPoint_24641+1][xPoint_24641-1] === this.CELL_EMPTY
                && map[yPoint_24641][xPoint_24641-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24641, yPoint_24641, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24641;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24641][x] === this.CELL_EMPTY
                    && map[yPoint_24641-1][x] === this.CELL_EMPTY
                    && map[yPoint_24641-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24641][x+1] === this.CELL_EMPTY
                    && map[yPoint_24641+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24641+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24641, yPoint_24641, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24641;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24641] === this.CELL_EMPTY
                    && map[y+1][xPoint_24641] === this.CELL_EMPTY
                    && map[y+1][xPoint_24641+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24641] === this.CELL_EMPTY
                    && map[y][xPoint_24641-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24641-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
