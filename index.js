...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_7782=0;yPoint_7782<this.gameFieldBorderY.length; yPoint_7782++){
                for(var xPoint_7782=0;xPoint_7782<this.gameFieldBorderX.length; xPoint_7782++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_7782, xPoint_7782);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_7782][xPoint_7782] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_7782, xPoint_7782, 'user');
                    if(this._userShipsMap[yPoint_7782][xPoint_7782] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_7782, xPoint_7782, type){
            var id = this.getPointBlockIdByCoords(yPoint_7782, xPoint_7782, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_7782);
                block.setAttribute('data-y', yPoint_7782);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_7782
         * @param {type} xPoint_7782
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_7782, xPoint_7782, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_7782 + '_y' + yPoint_7782;
            }
            return 'pc_x' + xPoint_7782 + '_y' + yPoint_7782;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_7782=0;yPoint_7782<this.gameFieldBorderY.length; yPoint_7782++){
                for(var xPoint_7782=0;xPoint_7782<this.gameFieldBorderX.length; xPoint_7782++){
                    map.push({y: yPoint_7782, x: xPoint_7782});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_7782=-1;yPoint_7782<(this.gameFieldBorderY.length+1); yPoint_7782++){
                for(var xPoint_7782=-1;xPoint_7782<(this.gameFieldBorderX.length+1); xPoint_7782++){
                    if(!map[yPoint_7782]){
                        map[yPoint_7782] = [];
                    }
                    map[yPoint_7782][xPoint_7782] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_7782 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_7782 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_7782, yPoint_7782) === true){
                    if(this.canPutHorizontal(map, xPoint_7782, yPoint_7782, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7782][xPoint_7782 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_7782, yPoint_7782, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_7782 + i][xPoint_7782] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_7782, yPoint_7782){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_7782][xPoint_7782] === this.CELL_EMPTY
                && map[yPoint_7782-1][xPoint_7782] === this.CELL_EMPTY
                && map[yPoint_7782-1][xPoint_7782+1] === this.CELL_EMPTY
                && map[yPoint_7782][xPoint_7782+1] === this.CELL_EMPTY
                && map[yPoint_7782+1][xPoint_7782+1] === this.CELL_EMPTY
                && map[yPoint_7782+1][xPoint_7782] === this.CELL_EMPTY
                && map[yPoint_7782+1][xPoint_7782-1] === this.CELL_EMPTY
                && map[yPoint_7782][xPoint_7782-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_7782, yPoint_7782, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_7782;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_7782][x] === this.CELL_EMPTY
                    && map[yPoint_7782-1][x] === this.CELL_EMPTY
                    && map[yPoint_7782-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7782][x+1] === this.CELL_EMPTY
                    && map[yPoint_7782+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_7782+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_7782, yPoint_7782, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_7782;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_7782] === this.CELL_EMPTY
                    && map[y+1][xPoint_7782] === this.CELL_EMPTY
                    && map[y+1][xPoint_7782+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_7782] === this.CELL_EMPTY
                    && map[y][xPoint_7782-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_7782-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
