...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_808=0;yPoint_808<this.gameFieldBorderY.length; yPoint_808++){
                for(var xPoint_808=0;xPoint_808<this.gameFieldBorderX.length; xPoint_808++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_808, xPoint_808);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_808][xPoint_808] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_808, xPoint_808, 'user');
                    if(this._userShipsMap[yPoint_808][xPoint_808] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_808, xPoint_808, type){
            var id = this.getPointBlockIdByCoords(yPoint_808, xPoint_808, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_808);
                block.setAttribute('data-y', yPoint_808);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_808
         * @param {type} xPoint_808
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_808, xPoint_808, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_808 + '_y' + yPoint_808;
            }
            return 'pc_x' + xPoint_808 + '_y' + yPoint_808;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_808=0;yPoint_808<this.gameFieldBorderY.length; yPoint_808++){
                for(var xPoint_808=0;xPoint_808<this.gameFieldBorderX.length; xPoint_808++){
                    map.push({y: yPoint_808, x: xPoint_808});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_808=-1;yPoint_808<(this.gameFieldBorderY.length+1); yPoint_808++){
                for(var xPoint_808=-1;xPoint_808<(this.gameFieldBorderX.length+1); xPoint_808++){
                    if(!map[yPoint_808]){
                        map[yPoint_808] = [];
                    }
                    map[yPoint_808][xPoint_808] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_808 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_808 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_808, yPoint_808) === true){
                    if(this.canPutHorizontal(map, xPoint_808, yPoint_808, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_808][xPoint_808 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_808, yPoint_808, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_808 + i][xPoint_808] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_808, yPoint_808){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_808][xPoint_808] === this.CELL_EMPTY
                && map[yPoint_808-1][xPoint_808] === this.CELL_EMPTY
                && map[yPoint_808-1][xPoint_808+1] === this.CELL_EMPTY
                && map[yPoint_808][xPoint_808+1] === this.CELL_EMPTY
                && map[yPoint_808+1][xPoint_808+1] === this.CELL_EMPTY
                && map[yPoint_808+1][xPoint_808] === this.CELL_EMPTY
                && map[yPoint_808+1][xPoint_808-1] === this.CELL_EMPTY
                && map[yPoint_808][xPoint_808-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_808, yPoint_808, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_808;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_808][x] === this.CELL_EMPTY
                    && map[yPoint_808-1][x] === this.CELL_EMPTY
                    && map[yPoint_808-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_808][x+1] === this.CELL_EMPTY
                    && map[yPoint_808+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_808+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_808, yPoint_808, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_808;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_808] === this.CELL_EMPTY
                    && map[y+1][xPoint_808] === this.CELL_EMPTY
                    && map[y+1][xPoint_808+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_808] === this.CELL_EMPTY
                    && map[y][xPoint_808-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_808-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
