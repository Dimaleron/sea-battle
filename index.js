...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9283=0;yPoint_9283<this.gameFieldBorderY.length; yPoint_9283++){
                for(var xPoint_9283=0;xPoint_9283<this.gameFieldBorderX.length; xPoint_9283++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9283, xPoint_9283);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9283][xPoint_9283] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9283, xPoint_9283, 'user');
                    if(this._userShipsMap[yPoint_9283][xPoint_9283] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9283, xPoint_9283, type){
            var id = this.getPointBlockIdByCoords(yPoint_9283, xPoint_9283, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9283);
                block.setAttribute('data-y', yPoint_9283);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9283
         * @param {type} xPoint_9283
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9283, xPoint_9283, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9283 + '_y' + yPoint_9283;
            }
            return 'pc_x' + xPoint_9283 + '_y' + yPoint_9283;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9283=0;yPoint_9283<this.gameFieldBorderY.length; yPoint_9283++){
                for(var xPoint_9283=0;xPoint_9283<this.gameFieldBorderX.length; xPoint_9283++){
                    map.push({y: yPoint_9283, x: xPoint_9283});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9283=-1;yPoint_9283<(this.gameFieldBorderY.length+1); yPoint_9283++){
                for(var xPoint_9283=-1;xPoint_9283<(this.gameFieldBorderX.length+1); xPoint_9283++){
                    if(!map[yPoint_9283]){
                        map[yPoint_9283] = [];
                    }
                    map[yPoint_9283][xPoint_9283] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9283 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9283 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9283, yPoint_9283) === true){
                    if(this.canPutHorizontal(map, xPoint_9283, yPoint_9283, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9283][xPoint_9283 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9283, yPoint_9283, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9283 + i][xPoint_9283] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9283, yPoint_9283){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9283][xPoint_9283] === this.CELL_EMPTY
                && map[yPoint_9283-1][xPoint_9283] === this.CELL_EMPTY
                && map[yPoint_9283-1][xPoint_9283+1] === this.CELL_EMPTY
                && map[yPoint_9283][xPoint_9283+1] === this.CELL_EMPTY
                && map[yPoint_9283+1][xPoint_9283+1] === this.CELL_EMPTY
                && map[yPoint_9283+1][xPoint_9283] === this.CELL_EMPTY
                && map[yPoint_9283+1][xPoint_9283-1] === this.CELL_EMPTY
                && map[yPoint_9283][xPoint_9283-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9283, yPoint_9283, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9283;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9283][x] === this.CELL_EMPTY
                    && map[yPoint_9283-1][x] === this.CELL_EMPTY
                    && map[yPoint_9283-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9283][x+1] === this.CELL_EMPTY
                    && map[yPoint_9283+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9283+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9283, yPoint_9283, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9283;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9283] === this.CELL_EMPTY
                    && map[y+1][xPoint_9283] === this.CELL_EMPTY
                    && map[y+1][xPoint_9283+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9283] === this.CELL_EMPTY
                    && map[y][xPoint_9283-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9283-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
