...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_5741=0;yPoint_5741<this.gameFieldBorderY.length; yPoint_5741++){
                for(var xPoint_5741=0;xPoint_5741<this.gameFieldBorderX.length; xPoint_5741++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_5741, xPoint_5741);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_5741][xPoint_5741] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_5741, xPoint_5741, 'user');
                    if(this._userShipsMap[yPoint_5741][xPoint_5741] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_5741, xPoint_5741, type){
            var id = this.getPointBlockIdByCoords(yPoint_5741, xPoint_5741, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_5741);
                block.setAttribute('data-y', yPoint_5741);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_5741
         * @param {type} xPoint_5741
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_5741, xPoint_5741, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_5741 + '_y' + yPoint_5741;
            }
            return 'pc_x' + xPoint_5741 + '_y' + yPoint_5741;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_5741=0;yPoint_5741<this.gameFieldBorderY.length; yPoint_5741++){
                for(var xPoint_5741=0;xPoint_5741<this.gameFieldBorderX.length; xPoint_5741++){
                    map.push({y: yPoint_5741, x: xPoint_5741});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_5741=-1;yPoint_5741<(this.gameFieldBorderY.length+1); yPoint_5741++){
                for(var xPoint_5741=-1;xPoint_5741<(this.gameFieldBorderX.length+1); xPoint_5741++){
                    if(!map[yPoint_5741]){
                        map[yPoint_5741] = [];
                    }
                    map[yPoint_5741][xPoint_5741] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_5741 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_5741 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_5741, yPoint_5741) === true){
                    if(this.canPutHorizontal(map, xPoint_5741, yPoint_5741, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5741][xPoint_5741 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_5741, yPoint_5741, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_5741 + i][xPoint_5741] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_5741, yPoint_5741){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_5741][xPoint_5741] === this.CELL_EMPTY
                && map[yPoint_5741-1][xPoint_5741] === this.CELL_EMPTY
                && map[yPoint_5741-1][xPoint_5741+1] === this.CELL_EMPTY
                && map[yPoint_5741][xPoint_5741+1] === this.CELL_EMPTY
                && map[yPoint_5741+1][xPoint_5741+1] === this.CELL_EMPTY
                && map[yPoint_5741+1][xPoint_5741] === this.CELL_EMPTY
                && map[yPoint_5741+1][xPoint_5741-1] === this.CELL_EMPTY
                && map[yPoint_5741][xPoint_5741-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_5741, yPoint_5741, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_5741;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_5741][x] === this.CELL_EMPTY
                    && map[yPoint_5741-1][x] === this.CELL_EMPTY
                    && map[yPoint_5741-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5741][x+1] === this.CELL_EMPTY
                    && map[yPoint_5741+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_5741+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_5741, yPoint_5741, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_5741;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_5741] === this.CELL_EMPTY
                    && map[y+1][xPoint_5741] === this.CELL_EMPTY
                    && map[y+1][xPoint_5741+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_5741] === this.CELL_EMPTY
                    && map[y][xPoint_5741-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_5741-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
