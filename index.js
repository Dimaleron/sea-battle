...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_10167=0;yPoint_10167<this.gameFieldBorderY.length; yPoint_10167++){
                for(var xPoint_10167=0;xPoint_10167<this.gameFieldBorderX.length; xPoint_10167++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_10167, xPoint_10167);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_10167][xPoint_10167] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_10167, xPoint_10167, 'user');
                    if(this._userShipsMap[yPoint_10167][xPoint_10167] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_10167, xPoint_10167, type){
            var id = this.getPointBlockIdByCoords(yPoint_10167, xPoint_10167, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_10167);
                block.setAttribute('data-y', yPoint_10167);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_10167
         * @param {type} xPoint_10167
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_10167, xPoint_10167, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_10167 + '_y' + yPoint_10167;
            }
            return 'pc_x' + xPoint_10167 + '_y' + yPoint_10167;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_10167=0;yPoint_10167<this.gameFieldBorderY.length; yPoint_10167++){
                for(var xPoint_10167=0;xPoint_10167<this.gameFieldBorderX.length; xPoint_10167++){
                    map.push({y: yPoint_10167, x: xPoint_10167});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_10167=-1;yPoint_10167<(this.gameFieldBorderY.length+1); yPoint_10167++){
                for(var xPoint_10167=-1;xPoint_10167<(this.gameFieldBorderX.length+1); xPoint_10167++){
                    if(!map[yPoint_10167]){
                        map[yPoint_10167] = [];
                    }
                    map[yPoint_10167][xPoint_10167] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_10167 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_10167 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_10167, yPoint_10167) === true){
                    if(this.canPutHorizontal(map, xPoint_10167, yPoint_10167, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10167][xPoint_10167 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_10167, yPoint_10167, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_10167 + i][xPoint_10167] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_10167, yPoint_10167){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_10167][xPoint_10167] === this.CELL_EMPTY
                && map[yPoint_10167-1][xPoint_10167] === this.CELL_EMPTY
                && map[yPoint_10167-1][xPoint_10167+1] === this.CELL_EMPTY
                && map[yPoint_10167][xPoint_10167+1] === this.CELL_EMPTY
                && map[yPoint_10167+1][xPoint_10167+1] === this.CELL_EMPTY
                && map[yPoint_10167+1][xPoint_10167] === this.CELL_EMPTY
                && map[yPoint_10167+1][xPoint_10167-1] === this.CELL_EMPTY
                && map[yPoint_10167][xPoint_10167-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_10167, yPoint_10167, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_10167;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_10167][x] === this.CELL_EMPTY
                    && map[yPoint_10167-1][x] === this.CELL_EMPTY
                    && map[yPoint_10167-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10167][x+1] === this.CELL_EMPTY
                    && map[yPoint_10167+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_10167+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_10167, yPoint_10167, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_10167;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_10167] === this.CELL_EMPTY
                    && map[y+1][xPoint_10167] === this.CELL_EMPTY
                    && map[y+1][xPoint_10167+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_10167] === this.CELL_EMPTY
                    && map[y][xPoint_10167-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_10167-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
