...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_28310=0;yPoint_28310<this.gameFieldBorderY.length; yPoint_28310++){
                for(var xPoint_28310=0;xPoint_28310<this.gameFieldBorderX.length; xPoint_28310++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_28310, xPoint_28310);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_28310][xPoint_28310] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_28310, xPoint_28310, 'user');
                    if(this._userShipsMap[yPoint_28310][xPoint_28310] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_28310, xPoint_28310, type){
            var id = this.getPointBlockIdByCoords(yPoint_28310, xPoint_28310, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_28310);
                block.setAttribute('data-y', yPoint_28310);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_28310
         * @param {type} xPoint_28310
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_28310, xPoint_28310, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_28310 + '_y' + yPoint_28310;
            }
            return 'pc_x' + xPoint_28310 + '_y' + yPoint_28310;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_28310=0;yPoint_28310<this.gameFieldBorderY.length; yPoint_28310++){
                for(var xPoint_28310=0;xPoint_28310<this.gameFieldBorderX.length; xPoint_28310++){
                    map.push({y: yPoint_28310, x: xPoint_28310});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_28310=-1;yPoint_28310<(this.gameFieldBorderY.length+1); yPoint_28310++){
                for(var xPoint_28310=-1;xPoint_28310<(this.gameFieldBorderX.length+1); xPoint_28310++){
                    if(!map[yPoint_28310]){
                        map[yPoint_28310] = [];
                    }
                    map[yPoint_28310][xPoint_28310] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_28310 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_28310 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_28310, yPoint_28310) === true){
                    if(this.canPutHorizontal(map, xPoint_28310, yPoint_28310, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28310][xPoint_28310 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_28310, yPoint_28310, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_28310 + i][xPoint_28310] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_28310, yPoint_28310){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_28310][xPoint_28310] === this.CELL_EMPTY
                && map[yPoint_28310-1][xPoint_28310] === this.CELL_EMPTY
                && map[yPoint_28310-1][xPoint_28310+1] === this.CELL_EMPTY
                && map[yPoint_28310][xPoint_28310+1] === this.CELL_EMPTY
                && map[yPoint_28310+1][xPoint_28310+1] === this.CELL_EMPTY
                && map[yPoint_28310+1][xPoint_28310] === this.CELL_EMPTY
                && map[yPoint_28310+1][xPoint_28310-1] === this.CELL_EMPTY
                && map[yPoint_28310][xPoint_28310-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_28310, yPoint_28310, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_28310;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_28310][x] === this.CELL_EMPTY
                    && map[yPoint_28310-1][x] === this.CELL_EMPTY
                    && map[yPoint_28310-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28310][x+1] === this.CELL_EMPTY
                    && map[yPoint_28310+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_28310+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_28310, yPoint_28310, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_28310;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_28310] === this.CELL_EMPTY
                    && map[y+1][xPoint_28310] === this.CELL_EMPTY
                    && map[y+1][xPoint_28310+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_28310] === this.CELL_EMPTY
                    && map[y][xPoint_28310-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_28310-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
