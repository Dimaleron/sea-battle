...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_32415=0;yPoint_32415<this.gameFieldBorderY.length; yPoint_32415++){
                for(var xPoint_32415=0;xPoint_32415<this.gameFieldBorderX.length; xPoint_32415++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_32415, xPoint_32415);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_32415][xPoint_32415] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_32415, xPoint_32415, 'user');
                    if(this._userShipsMap[yPoint_32415][xPoint_32415] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_32415, xPoint_32415, type){
            var id = this.getPointBlockIdByCoords(yPoint_32415, xPoint_32415, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_32415);
                block.setAttribute('data-y', yPoint_32415);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_32415
         * @param {type} xPoint_32415
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_32415, xPoint_32415, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_32415 + '_y' + yPoint_32415;
            }
            return 'pc_x' + xPoint_32415 + '_y' + yPoint_32415;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_32415=0;yPoint_32415<this.gameFieldBorderY.length; yPoint_32415++){
                for(var xPoint_32415=0;xPoint_32415<this.gameFieldBorderX.length; xPoint_32415++){
                    map.push({y: yPoint_32415, x: xPoint_32415});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_32415=-1;yPoint_32415<(this.gameFieldBorderY.length+1); yPoint_32415++){
                for(var xPoint_32415=-1;xPoint_32415<(this.gameFieldBorderX.length+1); xPoint_32415++){
                    if(!map[yPoint_32415]){
                        map[yPoint_32415] = [];
                    }
                    map[yPoint_32415][xPoint_32415] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_32415 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_32415 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_32415, yPoint_32415) === true){
                    if(this.canPutHorizontal(map, xPoint_32415, yPoint_32415, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32415][xPoint_32415 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_32415, yPoint_32415, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_32415 + i][xPoint_32415] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_32415, yPoint_32415){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_32415][xPoint_32415] === this.CELL_EMPTY
                && map[yPoint_32415-1][xPoint_32415] === this.CELL_EMPTY
                && map[yPoint_32415-1][xPoint_32415+1] === this.CELL_EMPTY
                && map[yPoint_32415][xPoint_32415+1] === this.CELL_EMPTY
                && map[yPoint_32415+1][xPoint_32415+1] === this.CELL_EMPTY
                && map[yPoint_32415+1][xPoint_32415] === this.CELL_EMPTY
                && map[yPoint_32415+1][xPoint_32415-1] === this.CELL_EMPTY
                && map[yPoint_32415][xPoint_32415-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_32415, yPoint_32415, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_32415;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_32415][x] === this.CELL_EMPTY
                    && map[yPoint_32415-1][x] === this.CELL_EMPTY
                    && map[yPoint_32415-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32415][x+1] === this.CELL_EMPTY
                    && map[yPoint_32415+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_32415+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_32415, yPoint_32415, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_32415;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_32415] === this.CELL_EMPTY
                    && map[y+1][xPoint_32415] === this.CELL_EMPTY
                    && map[y+1][xPoint_32415+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_32415] === this.CELL_EMPTY
                    && map[y][xPoint_32415-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_32415-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
