...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6716=0;yPoint_6716<this.gameFieldBorderY.length; yPoint_6716++){
                for(var xPoint_6716=0;xPoint_6716<this.gameFieldBorderX.length; xPoint_6716++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6716, xPoint_6716);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6716][xPoint_6716] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6716, xPoint_6716, 'user');
                    if(this._userShipsMap[yPoint_6716][xPoint_6716] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6716, xPoint_6716, type){
            var id = this.getPointBlockIdByCoords(yPoint_6716, xPoint_6716, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6716);
                block.setAttribute('data-y', yPoint_6716);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6716
         * @param {type} xPoint_6716
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6716, xPoint_6716, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6716 + '_y' + yPoint_6716;
            }
            return 'pc_x' + xPoint_6716 + '_y' + yPoint_6716;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6716=0;yPoint_6716<this.gameFieldBorderY.length; yPoint_6716++){
                for(var xPoint_6716=0;xPoint_6716<this.gameFieldBorderX.length; xPoint_6716++){
                    map.push({y: yPoint_6716, x: xPoint_6716});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6716=-1;yPoint_6716<(this.gameFieldBorderY.length+1); yPoint_6716++){
                for(var xPoint_6716=-1;xPoint_6716<(this.gameFieldBorderX.length+1); xPoint_6716++){
                    if(!map[yPoint_6716]){
                        map[yPoint_6716] = [];
                    }
                    map[yPoint_6716][xPoint_6716] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6716 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6716 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6716, yPoint_6716) === true){
                    if(this.canPutHorizontal(map, xPoint_6716, yPoint_6716, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6716][xPoint_6716 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6716, yPoint_6716, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6716 + i][xPoint_6716] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6716, yPoint_6716){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6716][xPoint_6716] === this.CELL_EMPTY
                && map[yPoint_6716-1][xPoint_6716] === this.CELL_EMPTY
                && map[yPoint_6716-1][xPoint_6716+1] === this.CELL_EMPTY
                && map[yPoint_6716][xPoint_6716+1] === this.CELL_EMPTY
                && map[yPoint_6716+1][xPoint_6716+1] === this.CELL_EMPTY
                && map[yPoint_6716+1][xPoint_6716] === this.CELL_EMPTY
                && map[yPoint_6716+1][xPoint_6716-1] === this.CELL_EMPTY
                && map[yPoint_6716][xPoint_6716-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6716, yPoint_6716, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6716;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6716][x] === this.CELL_EMPTY
                    && map[yPoint_6716-1][x] === this.CELL_EMPTY
                    && map[yPoint_6716-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6716][x+1] === this.CELL_EMPTY
                    && map[yPoint_6716+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6716+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6716, yPoint_6716, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6716;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6716] === this.CELL_EMPTY
                    && map[y+1][xPoint_6716] === this.CELL_EMPTY
                    && map[y+1][xPoint_6716+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6716] === this.CELL_EMPTY
                    && map[y][xPoint_6716-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6716-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
