...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_27956=0;yPoint_27956<this.gameFieldBorderY.length; yPoint_27956++){
                for(var xPoint_27956=0;xPoint_27956<this.gameFieldBorderX.length; xPoint_27956++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_27956, xPoint_27956);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_27956][xPoint_27956] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_27956, xPoint_27956, 'user');
                    if(this._userShipsMap[yPoint_27956][xPoint_27956] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_27956, xPoint_27956, type){
            var id = this.getPointBlockIdByCoords(yPoint_27956, xPoint_27956, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_27956);
                block.setAttribute('data-y', yPoint_27956);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_27956
         * @param {type} xPoint_27956
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_27956, xPoint_27956, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_27956 + '_y' + yPoint_27956;
            }
            return 'pc_x' + xPoint_27956 + '_y' + yPoint_27956;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_27956=0;yPoint_27956<this.gameFieldBorderY.length; yPoint_27956++){
                for(var xPoint_27956=0;xPoint_27956<this.gameFieldBorderX.length; xPoint_27956++){
                    map.push({y: yPoint_27956, x: xPoint_27956});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_27956=-1;yPoint_27956<(this.gameFieldBorderY.length+1); yPoint_27956++){
                for(var xPoint_27956=-1;xPoint_27956<(this.gameFieldBorderX.length+1); xPoint_27956++){
                    if(!map[yPoint_27956]){
                        map[yPoint_27956] = [];
                    }
                    map[yPoint_27956][xPoint_27956] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_27956 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_27956 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_27956, yPoint_27956) === true){
                    if(this.canPutHorizontal(map, xPoint_27956, yPoint_27956, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27956][xPoint_27956 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_27956, yPoint_27956, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_27956 + i][xPoint_27956] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_27956, yPoint_27956){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_27956][xPoint_27956] === this.CELL_EMPTY
                && map[yPoint_27956-1][xPoint_27956] === this.CELL_EMPTY
                && map[yPoint_27956-1][xPoint_27956+1] === this.CELL_EMPTY
                && map[yPoint_27956][xPoint_27956+1] === this.CELL_EMPTY
                && map[yPoint_27956+1][xPoint_27956+1] === this.CELL_EMPTY
                && map[yPoint_27956+1][xPoint_27956] === this.CELL_EMPTY
                && map[yPoint_27956+1][xPoint_27956-1] === this.CELL_EMPTY
                && map[yPoint_27956][xPoint_27956-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_27956, yPoint_27956, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_27956;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_27956][x] === this.CELL_EMPTY
                    && map[yPoint_27956-1][x] === this.CELL_EMPTY
                    && map[yPoint_27956-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27956][x+1] === this.CELL_EMPTY
                    && map[yPoint_27956+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_27956+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_27956, yPoint_27956, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_27956;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_27956] === this.CELL_EMPTY
                    && map[y+1][xPoint_27956] === this.CELL_EMPTY
                    && map[y+1][xPoint_27956+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_27956] === this.CELL_EMPTY
                    && map[y][xPoint_27956-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_27956-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
