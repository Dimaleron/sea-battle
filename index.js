...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_18014=0;yPoint_18014<this.gameFieldBorderY.length; yPoint_18014++){
                for(var xPoint_18014=0;xPoint_18014<this.gameFieldBorderX.length; xPoint_18014++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_18014, xPoint_18014);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_18014][xPoint_18014] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_18014, xPoint_18014, 'user');
                    if(this._userShipsMap[yPoint_18014][xPoint_18014] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_18014, xPoint_18014, type){
            var id = this.getPointBlockIdByCoords(yPoint_18014, xPoint_18014, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_18014);
                block.setAttribute('data-y', yPoint_18014);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_18014
         * @param {type} xPoint_18014
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_18014, xPoint_18014, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_18014 + '_y' + yPoint_18014;
            }
            return 'pc_x' + xPoint_18014 + '_y' + yPoint_18014;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_18014=0;yPoint_18014<this.gameFieldBorderY.length; yPoint_18014++){
                for(var xPoint_18014=0;xPoint_18014<this.gameFieldBorderX.length; xPoint_18014++){
                    map.push({y: yPoint_18014, x: xPoint_18014});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_18014=-1;yPoint_18014<(this.gameFieldBorderY.length+1); yPoint_18014++){
                for(var xPoint_18014=-1;xPoint_18014<(this.gameFieldBorderX.length+1); xPoint_18014++){
                    if(!map[yPoint_18014]){
                        map[yPoint_18014] = [];
                    }
                    map[yPoint_18014][xPoint_18014] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_18014 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_18014 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_18014, yPoint_18014) === true){
                    if(this.canPutHorizontal(map, xPoint_18014, yPoint_18014, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18014][xPoint_18014 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_18014, yPoint_18014, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_18014 + i][xPoint_18014] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_18014, yPoint_18014){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_18014][xPoint_18014] === this.CELL_EMPTY
                && map[yPoint_18014-1][xPoint_18014] === this.CELL_EMPTY
                && map[yPoint_18014-1][xPoint_18014+1] === this.CELL_EMPTY
                && map[yPoint_18014][xPoint_18014+1] === this.CELL_EMPTY
                && map[yPoint_18014+1][xPoint_18014+1] === this.CELL_EMPTY
                && map[yPoint_18014+1][xPoint_18014] === this.CELL_EMPTY
                && map[yPoint_18014+1][xPoint_18014-1] === this.CELL_EMPTY
                && map[yPoint_18014][xPoint_18014-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_18014, yPoint_18014, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_18014;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_18014][x] === this.CELL_EMPTY
                    && map[yPoint_18014-1][x] === this.CELL_EMPTY
                    && map[yPoint_18014-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18014][x+1] === this.CELL_EMPTY
                    && map[yPoint_18014+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_18014+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_18014, yPoint_18014, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_18014;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_18014] === this.CELL_EMPTY
                    && map[y+1][xPoint_18014] === this.CELL_EMPTY
                    && map[y+1][xPoint_18014+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_18014] === this.CELL_EMPTY
                    && map[y][xPoint_18014-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_18014-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
