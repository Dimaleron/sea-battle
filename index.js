...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_445=0;yPoint_445<this.gameFieldBorderY.length; yPoint_445++){
                for(var xPoint_445=0;xPoint_445<this.gameFieldBorderX.length; xPoint_445++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_445, xPoint_445);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_445][xPoint_445] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_445, xPoint_445, 'user');
                    if(this._userShipsMap[yPoint_445][xPoint_445] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_445, xPoint_445, type){
            var id = this.getPointBlockIdByCoords(yPoint_445, xPoint_445, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_445);
                block.setAttribute('data-y', yPoint_445);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_445
         * @param {type} xPoint_445
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_445, xPoint_445, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_445 + '_y' + yPoint_445;
            }
            return 'pc_x' + xPoint_445 + '_y' + yPoint_445;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_445=0;yPoint_445<this.gameFieldBorderY.length; yPoint_445++){
                for(var xPoint_445=0;xPoint_445<this.gameFieldBorderX.length; xPoint_445++){
                    map.push({y: yPoint_445, x: xPoint_445});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_445=-1;yPoint_445<(this.gameFieldBorderY.length+1); yPoint_445++){
                for(var xPoint_445=-1;xPoint_445<(this.gameFieldBorderX.length+1); xPoint_445++){
                    if(!map[yPoint_445]){
                        map[yPoint_445] = [];
                    }
                    map[yPoint_445][xPoint_445] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_445 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_445 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_445, yPoint_445) === true){
                    if(this.canPutHorizontal(map, xPoint_445, yPoint_445, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_445][xPoint_445 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_445, yPoint_445, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_445 + i][xPoint_445] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_445, yPoint_445){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_445][xPoint_445] === this.CELL_EMPTY
                && map[yPoint_445-1][xPoint_445] === this.CELL_EMPTY
                && map[yPoint_445-1][xPoint_445+1] === this.CELL_EMPTY
                && map[yPoint_445][xPoint_445+1] === this.CELL_EMPTY
                && map[yPoint_445+1][xPoint_445+1] === this.CELL_EMPTY
                && map[yPoint_445+1][xPoint_445] === this.CELL_EMPTY
                && map[yPoint_445+1][xPoint_445-1] === this.CELL_EMPTY
                && map[yPoint_445][xPoint_445-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_445, yPoint_445, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_445;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_445][x] === this.CELL_EMPTY
                    && map[yPoint_445-1][x] === this.CELL_EMPTY
                    && map[yPoint_445-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_445][x+1] === this.CELL_EMPTY
                    && map[yPoint_445+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_445+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_445, yPoint_445, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_445;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_445] === this.CELL_EMPTY
                    && map[y+1][xPoint_445] === this.CELL_EMPTY
                    && map[y+1][xPoint_445+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_445] === this.CELL_EMPTY
                    && map[y][xPoint_445-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_445-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
