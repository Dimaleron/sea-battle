...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_6240=0;yPoint_6240<this.gameFieldBorderY.length; yPoint_6240++){
                for(var xPoint_6240=0;xPoint_6240<this.gameFieldBorderX.length; xPoint_6240++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_6240, xPoint_6240);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_6240][xPoint_6240] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_6240, xPoint_6240, 'user');
                    if(this._userShipsMap[yPoint_6240][xPoint_6240] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_6240, xPoint_6240, type){
            var id = this.getPointBlockIdByCoords(yPoint_6240, xPoint_6240, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_6240);
                block.setAttribute('data-y', yPoint_6240);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_6240
         * @param {type} xPoint_6240
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_6240, xPoint_6240, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_6240 + '_y' + yPoint_6240;
            }
            return 'pc_x' + xPoint_6240 + '_y' + yPoint_6240;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_6240=0;yPoint_6240<this.gameFieldBorderY.length; yPoint_6240++){
                for(var xPoint_6240=0;xPoint_6240<this.gameFieldBorderX.length; xPoint_6240++){
                    map.push({y: yPoint_6240, x: xPoint_6240});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_6240=-1;yPoint_6240<(this.gameFieldBorderY.length+1); yPoint_6240++){
                for(var xPoint_6240=-1;xPoint_6240<(this.gameFieldBorderX.length+1); xPoint_6240++){
                    if(!map[yPoint_6240]){
                        map[yPoint_6240] = [];
                    }
                    map[yPoint_6240][xPoint_6240] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_6240 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_6240 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_6240, yPoint_6240) === true){
                    if(this.canPutHorizontal(map, xPoint_6240, yPoint_6240, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6240][xPoint_6240 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_6240, yPoint_6240, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_6240 + i][xPoint_6240] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_6240, yPoint_6240){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_6240][xPoint_6240] === this.CELL_EMPTY
                && map[yPoint_6240-1][xPoint_6240] === this.CELL_EMPTY
                && map[yPoint_6240-1][xPoint_6240+1] === this.CELL_EMPTY
                && map[yPoint_6240][xPoint_6240+1] === this.CELL_EMPTY
                && map[yPoint_6240+1][xPoint_6240+1] === this.CELL_EMPTY
                && map[yPoint_6240+1][xPoint_6240] === this.CELL_EMPTY
                && map[yPoint_6240+1][xPoint_6240-1] === this.CELL_EMPTY
                && map[yPoint_6240][xPoint_6240-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_6240, yPoint_6240, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_6240;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_6240][x] === this.CELL_EMPTY
                    && map[yPoint_6240-1][x] === this.CELL_EMPTY
                    && map[yPoint_6240-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6240][x+1] === this.CELL_EMPTY
                    && map[yPoint_6240+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_6240+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_6240, yPoint_6240, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_6240;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_6240] === this.CELL_EMPTY
                    && map[y+1][xPoint_6240] === this.CELL_EMPTY
                    && map[y+1][xPoint_6240+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_6240] === this.CELL_EMPTY
                    && map[y][xPoint_6240-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_6240-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
