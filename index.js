...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16803=0;yPoint_16803<this.gameFieldBorderY.length; yPoint_16803++){
                for(var xPoint_16803=0;xPoint_16803<this.gameFieldBorderX.length; xPoint_16803++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16803, xPoint_16803);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16803][xPoint_16803] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16803, xPoint_16803, 'user');
                    if(this._userShipsMap[yPoint_16803][xPoint_16803] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16803, xPoint_16803, type){
            var id = this.getPointBlockIdByCoords(yPoint_16803, xPoint_16803, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16803);
                block.setAttribute('data-y', yPoint_16803);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16803
         * @param {type} xPoint_16803
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16803, xPoint_16803, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16803 + '_y' + yPoint_16803;
            }
            return 'pc_x' + xPoint_16803 + '_y' + yPoint_16803;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16803=0;yPoint_16803<this.gameFieldBorderY.length; yPoint_16803++){
                for(var xPoint_16803=0;xPoint_16803<this.gameFieldBorderX.length; xPoint_16803++){
                    map.push({y: yPoint_16803, x: xPoint_16803});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16803=-1;yPoint_16803<(this.gameFieldBorderY.length+1); yPoint_16803++){
                for(var xPoint_16803=-1;xPoint_16803<(this.gameFieldBorderX.length+1); xPoint_16803++){
                    if(!map[yPoint_16803]){
                        map[yPoint_16803] = [];
                    }
                    map[yPoint_16803][xPoint_16803] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16803 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16803 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16803, yPoint_16803) === true){
                    if(this.canPutHorizontal(map, xPoint_16803, yPoint_16803, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16803][xPoint_16803 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16803, yPoint_16803, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16803 + i][xPoint_16803] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16803, yPoint_16803){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16803][xPoint_16803] === this.CELL_EMPTY
                && map[yPoint_16803-1][xPoint_16803] === this.CELL_EMPTY
                && map[yPoint_16803-1][xPoint_16803+1] === this.CELL_EMPTY
                && map[yPoint_16803][xPoint_16803+1] === this.CELL_EMPTY
                && map[yPoint_16803+1][xPoint_16803+1] === this.CELL_EMPTY
                && map[yPoint_16803+1][xPoint_16803] === this.CELL_EMPTY
                && map[yPoint_16803+1][xPoint_16803-1] === this.CELL_EMPTY
                && map[yPoint_16803][xPoint_16803-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16803, yPoint_16803, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16803;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16803][x] === this.CELL_EMPTY
                    && map[yPoint_16803-1][x] === this.CELL_EMPTY
                    && map[yPoint_16803-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16803][x+1] === this.CELL_EMPTY
                    && map[yPoint_16803+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16803+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16803, yPoint_16803, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16803;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16803] === this.CELL_EMPTY
                    && map[y+1][xPoint_16803] === this.CELL_EMPTY
                    && map[y+1][xPoint_16803+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16803] === this.CELL_EMPTY
                    && map[y][xPoint_16803-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16803-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
