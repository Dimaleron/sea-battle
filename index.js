...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_770=0;yPoint_770<this.gameFieldBorderY.length; yPoint_770++){
                for(var xPoint_770=0;xPoint_770<this.gameFieldBorderX.length; xPoint_770++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_770, xPoint_770);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_770][xPoint_770] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_770, xPoint_770, 'user');
                    if(this._userShipsMap[yPoint_770][xPoint_770] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_770, xPoint_770, type){
            var id = this.getPointBlockIdByCoords(yPoint_770, xPoint_770, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_770);
                block.setAttribute('data-y', yPoint_770);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_770
         * @param {type} xPoint_770
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_770, xPoint_770, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_770 + '_y' + yPoint_770;
            }
            return 'pc_x' + xPoint_770 + '_y' + yPoint_770;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_770=0;yPoint_770<this.gameFieldBorderY.length; yPoint_770++){
                for(var xPoint_770=0;xPoint_770<this.gameFieldBorderX.length; xPoint_770++){
                    map.push({y: yPoint_770, x: xPoint_770});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_770=-1;yPoint_770<(this.gameFieldBorderY.length+1); yPoint_770++){
                for(var xPoint_770=-1;xPoint_770<(this.gameFieldBorderX.length+1); xPoint_770++){
                    if(!map[yPoint_770]){
                        map[yPoint_770] = [];
                    }
                    map[yPoint_770][xPoint_770] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_770 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_770 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_770, yPoint_770) === true){
                    if(this.canPutHorizontal(map, xPoint_770, yPoint_770, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_770][xPoint_770 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_770, yPoint_770, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_770 + i][xPoint_770] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_770, yPoint_770){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_770][xPoint_770] === this.CELL_EMPTY
                && map[yPoint_770-1][xPoint_770] === this.CELL_EMPTY
                && map[yPoint_770-1][xPoint_770+1] === this.CELL_EMPTY
                && map[yPoint_770][xPoint_770+1] === this.CELL_EMPTY
                && map[yPoint_770+1][xPoint_770+1] === this.CELL_EMPTY
                && map[yPoint_770+1][xPoint_770] === this.CELL_EMPTY
                && map[yPoint_770+1][xPoint_770-1] === this.CELL_EMPTY
                && map[yPoint_770][xPoint_770-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_770, yPoint_770, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_770;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_770][x] === this.CELL_EMPTY
                    && map[yPoint_770-1][x] === this.CELL_EMPTY
                    && map[yPoint_770-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_770][x+1] === this.CELL_EMPTY
                    && map[yPoint_770+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_770+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_770, yPoint_770, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_770;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_770] === this.CELL_EMPTY
                    && map[y+1][xPoint_770] === this.CELL_EMPTY
                    && map[y+1][xPoint_770+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_770] === this.CELL_EMPTY
                    && map[y][xPoint_770-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_770-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
