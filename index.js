...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_16479=0;yPoint_16479<this.gameFieldBorderY.length; yPoint_16479++){
                for(var xPoint_16479=0;xPoint_16479<this.gameFieldBorderX.length; xPoint_16479++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_16479, xPoint_16479);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_16479][xPoint_16479] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_16479, xPoint_16479, 'user');
                    if(this._userShipsMap[yPoint_16479][xPoint_16479] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_16479, xPoint_16479, type){
            var id = this.getPointBlockIdByCoords(yPoint_16479, xPoint_16479, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_16479);
                block.setAttribute('data-y', yPoint_16479);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_16479
         * @param {type} xPoint_16479
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_16479, xPoint_16479, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_16479 + '_y' + yPoint_16479;
            }
            return 'pc_x' + xPoint_16479 + '_y' + yPoint_16479;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_16479=0;yPoint_16479<this.gameFieldBorderY.length; yPoint_16479++){
                for(var xPoint_16479=0;xPoint_16479<this.gameFieldBorderX.length; xPoint_16479++){
                    map.push({y: yPoint_16479, x: xPoint_16479});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_16479=-1;yPoint_16479<(this.gameFieldBorderY.length+1); yPoint_16479++){
                for(var xPoint_16479=-1;xPoint_16479<(this.gameFieldBorderX.length+1); xPoint_16479++){
                    if(!map[yPoint_16479]){
                        map[yPoint_16479] = [];
                    }
                    map[yPoint_16479][xPoint_16479] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_16479 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_16479 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_16479, yPoint_16479) === true){
                    if(this.canPutHorizontal(map, xPoint_16479, yPoint_16479, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16479][xPoint_16479 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_16479, yPoint_16479, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_16479 + i][xPoint_16479] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_16479, yPoint_16479){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_16479][xPoint_16479] === this.CELL_EMPTY
                && map[yPoint_16479-1][xPoint_16479] === this.CELL_EMPTY
                && map[yPoint_16479-1][xPoint_16479+1] === this.CELL_EMPTY
                && map[yPoint_16479][xPoint_16479+1] === this.CELL_EMPTY
                && map[yPoint_16479+1][xPoint_16479+1] === this.CELL_EMPTY
                && map[yPoint_16479+1][xPoint_16479] === this.CELL_EMPTY
                && map[yPoint_16479+1][xPoint_16479-1] === this.CELL_EMPTY
                && map[yPoint_16479][xPoint_16479-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_16479, yPoint_16479, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_16479;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_16479][x] === this.CELL_EMPTY
                    && map[yPoint_16479-1][x] === this.CELL_EMPTY
                    && map[yPoint_16479-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16479][x+1] === this.CELL_EMPTY
                    && map[yPoint_16479+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_16479+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_16479, yPoint_16479, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_16479;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_16479] === this.CELL_EMPTY
                    && map[y+1][xPoint_16479] === this.CELL_EMPTY
                    && map[y+1][xPoint_16479+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_16479] === this.CELL_EMPTY
                    && map[y][xPoint_16479-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_16479-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
