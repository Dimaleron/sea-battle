...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24240=0;yPoint_24240<this.gameFieldBorderY.length; yPoint_24240++){
                for(var xPoint_24240=0;xPoint_24240<this.gameFieldBorderX.length; xPoint_24240++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24240, xPoint_24240);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24240][xPoint_24240] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24240, xPoint_24240, 'user');
                    if(this._userShipsMap[yPoint_24240][xPoint_24240] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24240, xPoint_24240, type){
            var id = this.getPointBlockIdByCoords(yPoint_24240, xPoint_24240, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24240);
                block.setAttribute('data-y', yPoint_24240);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24240
         * @param {type} xPoint_24240
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24240, xPoint_24240, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24240 + '_y' + yPoint_24240;
            }
            return 'pc_x' + xPoint_24240 + '_y' + yPoint_24240;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24240=0;yPoint_24240<this.gameFieldBorderY.length; yPoint_24240++){
                for(var xPoint_24240=0;xPoint_24240<this.gameFieldBorderX.length; xPoint_24240++){
                    map.push({y: yPoint_24240, x: xPoint_24240});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24240=-1;yPoint_24240<(this.gameFieldBorderY.length+1); yPoint_24240++){
                for(var xPoint_24240=-1;xPoint_24240<(this.gameFieldBorderX.length+1); xPoint_24240++){
                    if(!map[yPoint_24240]){
                        map[yPoint_24240] = [];
                    }
                    map[yPoint_24240][xPoint_24240] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24240 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24240 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24240, yPoint_24240) === true){
                    if(this.canPutHorizontal(map, xPoint_24240, yPoint_24240, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24240][xPoint_24240 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24240, yPoint_24240, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24240 + i][xPoint_24240] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24240, yPoint_24240){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24240][xPoint_24240] === this.CELL_EMPTY
                && map[yPoint_24240-1][xPoint_24240] === this.CELL_EMPTY
                && map[yPoint_24240-1][xPoint_24240+1] === this.CELL_EMPTY
                && map[yPoint_24240][xPoint_24240+1] === this.CELL_EMPTY
                && map[yPoint_24240+1][xPoint_24240+1] === this.CELL_EMPTY
                && map[yPoint_24240+1][xPoint_24240] === this.CELL_EMPTY
                && map[yPoint_24240+1][xPoint_24240-1] === this.CELL_EMPTY
                && map[yPoint_24240][xPoint_24240-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24240, yPoint_24240, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24240;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24240][x] === this.CELL_EMPTY
                    && map[yPoint_24240-1][x] === this.CELL_EMPTY
                    && map[yPoint_24240-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24240][x+1] === this.CELL_EMPTY
                    && map[yPoint_24240+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24240+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24240, yPoint_24240, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24240;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24240] === this.CELL_EMPTY
                    && map[y+1][xPoint_24240] === this.CELL_EMPTY
                    && map[y+1][xPoint_24240+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24240] === this.CELL_EMPTY
                    && map[y][xPoint_24240-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24240-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
