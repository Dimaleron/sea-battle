...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_24764=0;yPoint_24764<this.gameFieldBorderY.length; yPoint_24764++){
                for(var xPoint_24764=0;xPoint_24764<this.gameFieldBorderX.length; xPoint_24764++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_24764, xPoint_24764);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_24764][xPoint_24764] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_24764, xPoint_24764, 'user');
                    if(this._userShipsMap[yPoint_24764][xPoint_24764] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_24764, xPoint_24764, type){
            var id = this.getPointBlockIdByCoords(yPoint_24764, xPoint_24764, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_24764);
                block.setAttribute('data-y', yPoint_24764);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_24764
         * @param {type} xPoint_24764
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_24764, xPoint_24764, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_24764 + '_y' + yPoint_24764;
            }
            return 'pc_x' + xPoint_24764 + '_y' + yPoint_24764;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_24764=0;yPoint_24764<this.gameFieldBorderY.length; yPoint_24764++){
                for(var xPoint_24764=0;xPoint_24764<this.gameFieldBorderX.length; xPoint_24764++){
                    map.push({y: yPoint_24764, x: xPoint_24764});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_24764=-1;yPoint_24764<(this.gameFieldBorderY.length+1); yPoint_24764++){
                for(var xPoint_24764=-1;xPoint_24764<(this.gameFieldBorderX.length+1); xPoint_24764++){
                    if(!map[yPoint_24764]){
                        map[yPoint_24764] = [];
                    }
                    map[yPoint_24764][xPoint_24764] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_24764 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_24764 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_24764, yPoint_24764) === true){
                    if(this.canPutHorizontal(map, xPoint_24764, yPoint_24764, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24764][xPoint_24764 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_24764, yPoint_24764, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_24764 + i][xPoint_24764] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_24764, yPoint_24764){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_24764][xPoint_24764] === this.CELL_EMPTY
                && map[yPoint_24764-1][xPoint_24764] === this.CELL_EMPTY
                && map[yPoint_24764-1][xPoint_24764+1] === this.CELL_EMPTY
                && map[yPoint_24764][xPoint_24764+1] === this.CELL_EMPTY
                && map[yPoint_24764+1][xPoint_24764+1] === this.CELL_EMPTY
                && map[yPoint_24764+1][xPoint_24764] === this.CELL_EMPTY
                && map[yPoint_24764+1][xPoint_24764-1] === this.CELL_EMPTY
                && map[yPoint_24764][xPoint_24764-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_24764, yPoint_24764, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_24764;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_24764][x] === this.CELL_EMPTY
                    && map[yPoint_24764-1][x] === this.CELL_EMPTY
                    && map[yPoint_24764-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24764][x+1] === this.CELL_EMPTY
                    && map[yPoint_24764+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_24764+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_24764, yPoint_24764, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_24764;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_24764] === this.CELL_EMPTY
                    && map[y+1][xPoint_24764] === this.CELL_EMPTY
                    && map[y+1][xPoint_24764+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_24764] === this.CELL_EMPTY
                    && map[y][xPoint_24764-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_24764-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
