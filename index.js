...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_9485=0;yPoint_9485<this.gameFieldBorderY.length; yPoint_9485++){
                for(var xPoint_9485=0;xPoint_9485<this.gameFieldBorderX.length; xPoint_9485++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_9485, xPoint_9485);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_9485][xPoint_9485] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_9485, xPoint_9485, 'user');
                    if(this._userShipsMap[yPoint_9485][xPoint_9485] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_9485, xPoint_9485, type){
            var id = this.getPointBlockIdByCoords(yPoint_9485, xPoint_9485, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_9485);
                block.setAttribute('data-y', yPoint_9485);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_9485
         * @param {type} xPoint_9485
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_9485, xPoint_9485, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_9485 + '_y' + yPoint_9485;
            }
            return 'pc_x' + xPoint_9485 + '_y' + yPoint_9485;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_9485=0;yPoint_9485<this.gameFieldBorderY.length; yPoint_9485++){
                for(var xPoint_9485=0;xPoint_9485<this.gameFieldBorderX.length; xPoint_9485++){
                    map.push({y: yPoint_9485, x: xPoint_9485});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_9485=-1;yPoint_9485<(this.gameFieldBorderY.length+1); yPoint_9485++){
                for(var xPoint_9485=-1;xPoint_9485<(this.gameFieldBorderX.length+1); xPoint_9485++){
                    if(!map[yPoint_9485]){
                        map[yPoint_9485] = [];
                    }
                    map[yPoint_9485][xPoint_9485] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_9485 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_9485 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_9485, yPoint_9485) === true){
                    if(this.canPutHorizontal(map, xPoint_9485, yPoint_9485, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9485][xPoint_9485 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_9485, yPoint_9485, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_9485 + i][xPoint_9485] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_9485, yPoint_9485){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_9485][xPoint_9485] === this.CELL_EMPTY
                && map[yPoint_9485-1][xPoint_9485] === this.CELL_EMPTY
                && map[yPoint_9485-1][xPoint_9485+1] === this.CELL_EMPTY
                && map[yPoint_9485][xPoint_9485+1] === this.CELL_EMPTY
                && map[yPoint_9485+1][xPoint_9485+1] === this.CELL_EMPTY
                && map[yPoint_9485+1][xPoint_9485] === this.CELL_EMPTY
                && map[yPoint_9485+1][xPoint_9485-1] === this.CELL_EMPTY
                && map[yPoint_9485][xPoint_9485-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_9485, yPoint_9485, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_9485;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_9485][x] === this.CELL_EMPTY
                    && map[yPoint_9485-1][x] === this.CELL_EMPTY
                    && map[yPoint_9485-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9485][x+1] === this.CELL_EMPTY
                    && map[yPoint_9485+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_9485+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_9485, yPoint_9485, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_9485;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_9485] === this.CELL_EMPTY
                    && map[y+1][xPoint_9485] === this.CELL_EMPTY
                    && map[y+1][xPoint_9485+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_9485] === this.CELL_EMPTY
                    && map[y][xPoint_9485-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_9485-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
