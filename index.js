...
(function (root, factory) {
    ...
    SeeBattle.prototype = {
        ...
        drawGamePoints: function(){
            for(var yPoint_29103=0;yPoint_29103<this.gameFieldBorderY.length; yPoint_29103++){
                for(var xPoint_29103=0;xPoint_29103<this.gameFieldBorderX.length; xPoint_29103++){
                    var pcPointBlock = this.getOrCreatePointBlock(yPoint_29103, xPoint_29103);
                    pcPointBlock.onclick = function(e){
                        this.userFire(e);
                    }.bind(this);
                    // если нужно отобразить корабли компбютера
                    /*if(this._pcShipsMap[yPoint_29103][xPoint_29103] === this.CELL_WITH_SHIP){
                        pcPointBlock.setAttribute('class', 'ship');
                    }*/

                    var userPointBlock = this.getOrCreatePointBlock(yPoint_29103, xPoint_29103, 'user');
                    if(this._userShipsMap[yPoint_29103][xPoint_29103] === this.CELL_WITH_SHIP){
         ...
         * @return {type}
         */
        getOrCreatePointBlock: function(yPoint_29103, xPoint_29103, type){
            var id = this.getPointBlockIdByCoords(yPoint_29103, xPoint_29103, type);
            ...
            if(block){
                ...
                block.setAttribute('data-x', xPoint_29103);
                block.setAttribute('data-y', yPoint_29103);
                if(type && type === 'user'){
         ...
         * @param {type} yPoint_29103
         * @param {type} xPoint_29103
         * @param {type} type
         * @return {String}
         */
        getPointBlockIdByCoords: function(yPoint_29103, xPoint_29103, type){
            if(type && type === 'user'){
                return 'user_x' + xPoint_29103 + '_y' + yPoint_29103;
            }
            return 'pc_x' + xPoint_29103 + '_y' + yPoint_29103;
        },
        ...
        generateShotMap: function(){
            ...
            for(var yPoint_29103=0;yPoint_29103<this.gameFieldBorderY.length; yPoint_29103++){
                for(var xPoint_29103=0;xPoint_29103<this.gameFieldBorderX.length; xPoint_29103++){
                    map.push({y: yPoint_29103, x: xPoint_29103});
                }
            }
            return map;
        },
        ...
        generateRandomShipMap: function(){
            ...
            for(var yPoint_29103=-1;yPoint_29103<(this.gameFieldBorderY.length+1); yPoint_29103++){
                for(var xPoint_29103=-1;xPoint_29103<(this.gameFieldBorderX.length+1); xPoint_29103++){
                    if(!map[yPoint_29103]){
                        map[yPoint_29103] = [];
                    }
                    map[yPoint_29103][xPoint_29103] = this.CELL_EMPTY;
                }
            }
            ...
            while(allShipsPlaced === false){
                var xPoint_29103 = this.getRandomInt(0, this.gameFieldBorderX.length);
                var yPoint_29103 = this.getRandomInt(0, this.gameFieldBorderY.length);
                if(this.isPointFree(map, xPoint_29103, yPoint_29103) === true){
                    if(this.canPutHorizontal(map, xPoint_29103, yPoint_29103, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29103][xPoint_29103 + i] = this.CELL_WITH_SHIP;
                        }
                    }else if(this.canPutVertical(map, xPoint_29103, yPoint_29103, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)){
                        for(var i=0;i<shipsConfiguration[0].pointCount;i++){
                            map[yPoint_29103 + i][xPoint_29103] = this.CELL_WITH_SHIP;
                        }
                    }else{
                        continue;
                    }
         ...
         * @return {Boolean}
         */
        isPointFree: function(map, xPoint_29103, yPoint_29103){
            // текущая и далее по часовй стрелке вокруг
            if(map[yPoint_29103][xPoint_29103] === this.CELL_EMPTY
                && map[yPoint_29103-1][xPoint_29103] === this.CELL_EMPTY
                && map[yPoint_29103-1][xPoint_29103+1] === this.CELL_EMPTY
                && map[yPoint_29103][xPoint_29103+1] === this.CELL_EMPTY
                && map[yPoint_29103+1][xPoint_29103+1] === this.CELL_EMPTY
                && map[yPoint_29103+1][xPoint_29103] === this.CELL_EMPTY
                && map[yPoint_29103+1][xPoint_29103-1] === this.CELL_EMPTY
                && map[yPoint_29103][xPoint_29103-1] === this.CELL_EMPTY
         ...
         * @return {Boolean}
         */
        canPutHorizontal: function(map, xPoint_29103, yPoint_29103, shipLength, coordLength){
            var freePoints = 0;
            for(var x=xPoint_29103;x<coordLength;x++){
                // текущая и далее по часовй стрелке в гориз направл
                if(map[yPoint_29103][x] === this.CELL_EMPTY
                    && map[yPoint_29103-1][x] === this.CELL_EMPTY
                    && map[yPoint_29103-1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29103][x+1] === this.CELL_EMPTY
                    && map[yPoint_29103+1][x+1] === this.CELL_EMPTY
                    && map[yPoint_29103+1][x] === this.CELL_EMPTY
                ){
                    freePoints++;
         ...
         * @return {Boolean}
         */
        canPutVertical: function(map, xPoint_29103, yPoint_29103, shipLength, coordLength){
            var freePoints = 0;
            for(var y=yPoint_29103;y<coordLength;y++){
                // текущая и далее по часовй стрелке в вертикальном направлении
                if(map[y][xPoint_29103] === this.CELL_EMPTY
                    && map[y+1][xPoint_29103] === this.CELL_EMPTY
                    && map[y+1][xPoint_29103+1] === this.CELL_EMPTY
                    && map[y+1][xPoint_29103] === this.CELL_EMPTY
                    && map[y][xPoint_29103-1] === this.CELL_EMPTY
                    && map[y-1][xPoint_29103-1] === this.CELL_EMPTY
                ){
                    freePoints++;
                }else{
